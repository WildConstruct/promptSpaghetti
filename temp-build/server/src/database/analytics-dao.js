...
export class AnalyticsDAO {
  ...
  getProjectCollaborationStats(projectId, days = 30) {
    ...
    const collaborativeSessions = this.db
      .prepare(
        `
      SELECT 
        session_id,
    ...
    `
      ...
      .get(projectId, cutoffTime);
    const topCollaborators = this.db
      .prepare(
        `
      SELECT 
        user_id as userId,
    ...
    const averageSessionParticipants =
      totalCollaborativeSessions > 0
        ? collaborativeSessions.reduce((sum, s) => sum + s.participants, 0) / totalCollaborativeSessions
        : 0;
    const collaborationRate = totalSessions?.total > 0 ? totalCollaborativeSessions / totalSessions.total : 0;
    return {
  ...
  getProjectHealthScore(projectId) {
    ...
    const score = activityLevel * 0.3 + errorRate * 0.3 + collaborationLevel * 0.2 + userEngagement * 0.2;
