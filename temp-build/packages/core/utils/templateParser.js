...
class TemplateParser {
  ...
  inferVariableType(variableName, template, context) {
    ...
    if (
      ...
      const defaultValue = matchingSuggestion ? matchingSuggestion.examples[0] : `sample ${name}`;
      return {
        type: 'string',
        confidence: 0.9,
    ...
    const defaultValue = genericSuggestion ? genericSuggestion.examples[0] : `sample ${name}`;
    return {
      type: 'string',
      confidence: 0.5,
  ...
  getContextualSuggestions() {
    ...
    if (this.currentVariables.length > 0) {
      const relatedSuggestions = COMMON_VARIABLES.filter(
        suggestion =>
          suggestion.relatedVariables?.some(related => this.currentVariables.includes(related)) &&
    ...
    const uniqueSuggestions = contextualSuggestions.filter(
      (suggestion, index, self) => index === self.findIndex(s => s.name === suggestion.name)
    );
  ...
  getPreviewWithSamples(template) {
    ...
    for (const variable of parseResult.variables) {
      if (variable.isValid) {
        ...
        sampleValues[variable.name] = suggestion ? suggestion.examples[0] : `[${variable.name}]`;
      }
    }
    return {
...
export const getVariableSuggestions = (partialName, context, includeHistory = true) =>
...
export const setVariableContext = (nodeType, existingVariables = []) =>
