...
export const SecureValidation = {
  ...
  safeExpression: (maxLength = 500) =>
    z
      ...
      .refine(val => SecurityValidation.validateSafeExpression(val), {
        message: 'Expression contains unsafe patterns',
      }),
  /**
   * Safe property key validation
   */
  safePropertyKey: (maxLength = 64) =>
    z
      .string()
      .max(maxLength, `Property key must be no longer than ${maxLength} characters`)
      .refine(val => SecurityValidation.validateSafePropertyKey(val), {
  ...
  variableName: () =>
    z
      ...
      .refine(val => SecurityValidation.validateVariableName(val), {
  ...
  safeValue: () =>
    z.union([
      z
        .string()
        .max(10000)
        .refine(val => SecurityValidation.validateSafeString(val), {
          message: 'String value contains dangerous patterns',
        }),
      z
        .number()
        .finite()
        .refine(val => !isNaN(val), { message: 'Number value must be finite' }),
      z.boolean(),
      z
        .array(z.string().max(1000))
        .max(1000)
        .refine(val => val.every(item => SecurityValidation.validateSafeString(item)), {
  ...
  safeObject: (valueSchema, maxKeys = 100) =>
    z
      .record(valueSchema)
      .refine(val => Object.keys(val).length <= maxKeys, {
        message: `Object must contain no more than ${maxKeys} properties`,
      })
      .refine(val => Object.keys(val).every(key => SecurityValidation.validateSafePropertyKey(key)), {
