...
export class ConditionalNode extends AdvancedRuntimeNode {
  ...
  evaluateCondition(expression, ctx) {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (
        errorMessage.includes('Dangerous pattern detected') ||
        errorMessage.includes('Expression evaluation failed')
      ) {
        throw error;
      }
      if (this.conditionalConfig.strictMode) {
  ...
  createEvaluationContext(ctx) {
    ...
    evalContext.matches = (str, pattern) => {
      try {
        ...
        throw ErrorFactory.createValidationError('pattern', pattern, 'valid regex pattern', {
