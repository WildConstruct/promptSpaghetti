...
export class SafeExpressionEvaluator {
  ...
  static parseExpressionWithAcorn(expression) {
    try {
      ...
      // Parse using acorn
      const program = acorn.parse(wrappedExpression, ACORN_OPTIONS);
      // Extract the expression from the ExpressionStatement
      if (program.type !== 'Program' || program.body.length !== 1 || program.body[0].type !== 'ExpressionStatement') {
...
SafeExpressionEvaluator.astFilter = createConditionalNodeFilter();
