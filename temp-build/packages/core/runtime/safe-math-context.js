 ...
 */
export const SAFE_MATH_FUNCTIONS = ['min', 'max', 'floor', 'ceil', 'round', 'abs', 'sign', 'trunc'];
/**
...
function validateNumericInput(value, functionName) {
  ...
  if (typeof value !== 'number') {
    ...
    securityAudit.logEvent(
      ...
      error,
      { functionName, additionalData: { valueType: typeof value } },
      true
    );
  ...
  if (value > NUMERIC_LIMITS.MAX_SAFE_VALUE || value < NUMERIC_LIMITS.MIN_SAFE_VALUE) {
    ...
    securityAudit.logEvent(
      ...
      SecurityEventCategory.MATH_VALIDATION_FAILED,
      error,
      { functionName, additionalData: { value } },
      true
    );
...
MathFunctionAuditor.auditLog = [];
MathFunctionAuditor.MAX_AUDIT_ENTRIES = 1000;
/**
