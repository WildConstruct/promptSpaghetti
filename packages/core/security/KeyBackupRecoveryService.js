...
(function (RecoveryType) {
    RecoveryType["COMPLETE"] = "complete";
    RecoveryType["SELECTIVE"] = "selective";
    RecoveryType["POINT_IN_TIME"] = "point_in_time";
    RecoveryType["EMERGENCY"] = "emergency";
    RecoveryType["VERIFICATION"] = "verification"; // Verify backup integrity only
})(RecoveryType || (RecoveryType = {}));
// Backup storage tiers
export var BackupStorageTier;
...
export class KeyBackupRecoveryService extends EventEmitter {
    ...
    statistics;
    constructor(keyManagementService, config) {
        super();
     ...
     * Delete backup
     */
    async deleteBackup(backupId, reason) {
        const backup = this.backups.get(backupId);
        if (!backup) {
            throw new Error('Backup not found');
        }
    ...
    determineKeysToRecover(backupPackage, request) {
        if (request.specificKeys) {
            return backupPackage.keyManifest.filter(entry => request.specificKeys.includes(entry.keyId));
        }
        return backupPackage.keyManifest;
    }
    extractKeyData(decryptedData, keyEntry) {
        return decryptedData.slice(keyEntry.offset, keyEntry.offset + keyEntry.length);
    }
    async restoreKey(keyData, keyEntry) {
    ...
    logBackupAccess(backup, action, userId) {
        const event = {
    ...
    updateStatistics(operation, duration, success) {
        switch (operation) {
            case 'backup_created':
