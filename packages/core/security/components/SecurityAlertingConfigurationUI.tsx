...
export const SecurityAlertingConfigurationUI: React.FC<SecurityAlertingConfigurationUIProps> = ({ )
  ...
  const updateConfig = useCallback((field: keyof SecurityAlertingConfig, value: any) => { setState(prev => ({)
  ...prev }
  ...
  const updateEscalationThresholds = useCallback((thresholds: Partial<EscalationThresholds>) => { setState(prev => ({)
  ...prev
      config: {
        ...prev.config }
  ...
  const toggleSection = useCallback((sectionId: string) => { setState(prev => {)
  ...
  useEffect(() => { if (state.hasUnsavedChanges) {
      const timeoutId = setTimeout(validateConfiguration, 500);
      return () => clearTimeout(timeoutId) }, [state.config, state.hasUnsavedChanges, validateConfiguration]);
  // Render validation status
  const renderValidationStatus = () => { if (state.isLoading) {
      return;
        <div style={{
          padding: '12px'
