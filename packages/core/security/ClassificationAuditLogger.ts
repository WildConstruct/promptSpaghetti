...
export class ClassificationAuditLogger extends EventEmitter {
  ...
  private async createAuditEntry(
    data: Omit<AuditLogEntry, 'id' | 'timestamp' | 'integrity'>
  ...
  private calculateLogHash(entry: AuditLogEntry): string {
    const content = {
      id: entry.id,
  ...
  private signLog(entry: AuditLogEntry): string {
  ...
  private verifySignature(entry: AuditLogEntry): boolean {
    if (!entry.integrity.signature) return false;
  ...
  private exportAsJSON(logs: AuditLogEntry[]): string {
  ...
  private exportAsCEF(logs: AuditLogEntry[]): string {
    return logs.map(log => {
      const cef = [
        'CEF:0',
        'SecurityAudit',
  ...
  private exportAsLEEF(logs: AuditLogEntry[]): string {
    return logs.map(log => {
      const leef = [
        'LEEF:2.0',
        'SecurityAudit',
  ...
  private mapToSeverity(log: AuditLogEntry): number {
    if (log.action.result === 'failure') return 7;
