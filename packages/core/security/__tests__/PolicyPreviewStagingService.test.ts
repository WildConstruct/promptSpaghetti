 ...
 */
import { PolicyPreviewStagingService,
  PolicyPreviewConfig,
  EnvironmentType,
  PreviewStatus,
...
describe('PolicyPreviewStagingService', () => { let service: PolicyPreviewStagingService;
  ...
  beforeEach(() => { testConfig = createTestConfig();
    service = new PolicyPreviewStagingService(testConfig) });
  describe('Policy Preview Creation', () => { test('should create policy preview successfully', async () => {
  ...
  expect(preview.validationResults).toBeInstanceOf(Array);
    ...
    test('should run default validations on preview creation', async () => { const preview = await service.createPolicyPreview(;);
  'policy-123',
  'v1.0.0',
  [{
  changeId: 'change-1',
  ...
  expect(result.validatedAt).toBeInstanceOf(Date) });
    ...
    test('should set preview status to REJECTED if validation has blockers', async () => { // Create preview with changes that might trigger validation failures
  const changes: PreviewChange = [
  {
  changeId: 'change-1',
  section: 'Data Rights',
  ...
  if (hasBlockers) { expect(preview.status).toBe(PreviewStatus.REJECTED) });
    test('should emit previewCreated event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  timestamp: expect.any(Date) }
    ...
    test('should handle custom expiration days', async () => { const customExpirationDays = 7;
  const preview = await service.createPolicyPreview(;);
  'policy-123'
  'v1.0.0'
  [{
  ...
  describe('Staging Deployment', () => { let testPreview: unknown;
  beforeEach(async () => {
  ...
  testPreview.status = PreviewStatus.STAGED;
    ...
    test('should deploy preview to staging environment', async () => { const environmentId = 'test-staging';
  ...
  expect(deployment.rollbackTriggers.length).toBeGreaterThan(0);
    ...
    test('should fail deployment to non-existent environment', async () => { await expect()
        service.deployToStaging()
          testPreview.previewId }
          'non-existent-env').rejects.toThrow('Staging environment not found');
    });
    test('should fail deployment if preview not found', async () => { await expect()
        service.deployToStaging()
          'non-existent-preview' }
          'test-staging').rejects.toThrow('Preview not found');
    });
    test('should respect max active deployments limit', async () => { const environmentId = 'test-staging';
    ...
    test('should emit stagingDeploymentCreated event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  timestamp: expect.any(Date) }
    ...
    test('should initialize default rollback triggers', async () => { const deployment = await service.deployToStaging(;);
        testPreview.previewId }
        'test-staging');
      expect(deployment.rollbackTriggers).toBeInstanceOf(Array);
      expect(deployment.rollbackTriggers.length).toBeGreaterThan(0);
  ...
  describe('Validation System', () => { let testPreview: unknown;
  beforeEach(async () => {
  testPreview = await service.createPolicyPreview()
  'policy-123'
  'v1.0.0'
  ...
  createdBy: 'user-123');
    ...
    test('should run specific validations', async () => { const validationTypes = [ValidationType.LEGAL, ValidationType.ACCESSIBILITY];
    ...
    test('should emit validationCompleted event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  timestamp: expect.any(Date) }
    ...
    test('should handle validation findings', async () => { const results = await service.runValidations(;);
        testPreview }
  ...
  describe('User Feedback Collection', () => { let testPreview: unknown;
  beforeEach(async () => {
  testPreview = await service.createPolicyPreview()
  'policy-123'
  'v1.0.0'
  ...
  createdBy: 'user-123');
    ...
    test('should collect user feedback successfully', async () => { const userId = 'user-456';
  const feedback = {
  ...
  categories: [FeedbackCategory.POSITIVE, FeedbackCategory.SUGGESTION] }
    ...
    test('should mark feedback as action required for low ratings', async () => { const feedback = {
  ...
  categories: [FeedbackCategory.NEGATIVE] }
    ...
    test('should mark feedback as action required for bug reports', async () => { const feedback = {
  ...
  categories: [FeedbackCategory.BUG_REPORT] }
    ...
    test('should emit userFeedbackReceived event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  timestamp: expect.any(Date) }
    ...
    test('should fail feedback collection for non-existent preview', async () => { await expect()
  ...
  describe('Policy Comparison Reports', () => { test('should generate policy comparison report', async () => {
      const baseVersion = 'v1.0.0';
    ...
    test('should emit comparisonReportGenerated event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  describe('Preview Analytics', () => { let testPreview: unknown;
  beforeEach(async () => {
  testPreview = await service.createPolicyPreview()
  'policy-123'
  'v1.0.0'
  ...
  createdBy: 'user-123');
    ...
    test('should get preview analytics', async () => { const analytics = await service.getPreviewAnalytics(testPreview.previewId);
    ...
    test('should fail to get analytics for non-existent preview', async () => { await expect()
        service.getPreviewAnalytics('non-existent-preview')
      ).rejects.toThrow('Preview not found') });
  });
  describe('Production Promotion', () => { let testPreview: unknown;
  beforeEach(async () => {
  testPreview = await service.createPolicyPreview()
  'policy-123'
  'v1.0.0'
  ...
  testPreview.status = PreviewStatus.APPROVED;
    ...
    test('should promote preview to production successfully', async () => { const options = {
  ...
  rolloutStrategy: 'gradual' }
    ...
    test('should fail promotion for non-approved preview', async () => { testPreview.status = PreviewStatus.TESTING;
  await expect()
  ...
  effectiveDate: new Date(Date.now() + 24 * 60 * 60 * 1000)).rejects.toThrow('Preview must be approved before promotion');
    ...
    test('should emit previewPromoted event on successful promotion', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  describe('Rollback Functionality', () => { let testDeployment: unknown;
  beforeEach(async () => {
  ...
  'test-staging');
    ...
    test('should rollback staging deployment successfully', async () => { const reason = 'High error rate detected';
    ...
    test('should emit stagingRollback event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  timestamp: expect.any(Date) }
    ...
    test('should fail rollback for non-existent deployment', async () => { await expect()
  ...
  describe('Configuration and Limits', () => { test('should respect preview retention days configuration', async () => {
  const customConfig = {
  ...testConfig
  ...
  expect(Math.abs(actualExpiration.getTime() - expectedExpiration.getTime())).toBeLessThan(60000);
    ...
    test('should enforce staging environment configuration', () => { const stagingEnv = testConfig.stagingEnvironments[0];
  ...
  createdBy: 'user-123').rejects.toThrow();
    ...
    test('should emit error events for monitoring', () => { const errorHandler = jest.fn<unknown, unknown>();
