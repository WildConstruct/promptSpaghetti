...
describe('DataPermissionHierarchyManager', () => {
    ...
    describe('Permission Granting', () => {
        it('should create permission grants with proper audit trail', async () => {
            ...
            const grant = await manager.grantPermission(request, 'manager-001', [], new Date(Date.now() + 24 * 60 * 60 * 1000), // 24 hours
            10 // 10 uses
            );
        ...
        it('should emit permission granted event', async () => {
            ...
            expect(eventData.grant).toBeDefined();
            expect(eventData.request).toBe(request);
            expect(eventData.grantedBy).toBe('manager-001');
        });
    });
    describe('Escalation Process', () => {
        it('should initiate escalation process for denied requests', async () => {
            ...
            const escalation = await manager.initiateEscalation(request, 'data_access_request');
        ...
        it('should emit escalation initiated event', async () => {
            ...
            const eventData = await eventPromise;
            expect(eventData).toBeDefined();
            expect(eventData.escalationRequest).toBeDefined();
            expect(eventData.originalRequest).toBe(request);
        });
        it('should handle invalid escalation paths', async () => {
            ...
            await expect(manager.initiateEscalation(request, 'invalid-path')).rejects.toThrow('Escalation path not found');
        });
    });
    describe('Permission Delegation', () => {
        it('should allow delegation between appropriate levels', async () => {
            const delegation = await manager.delegatePermissions('data-owner-001', 'analyst-001', ['read', 'WRITE'], new Date(Date.now() + 48 * 60 * 60 * 1000), // 48 hours
            [{
                    type: 'TEMPORAL',
        ...
        it('should prevent delegation to higher privilege users', async () => {
            await expect(manager.delegatePermissions('analyst-001', 'data-owner-001', ['read'], new Date(Date.now() + 24 * 60 * 60 * 1000), [], 'Invalid delegation attempt')).rejects.toThrow('Cannot delegate to user with equal or higher privilege level');
        });
        it('should prevent delegation by users without delegation rights', async () => {
            await expect(manager.delegatePermissions('guest-001', 'user-001', ['read'], new Date(Date.now() + 24 * 60 * 60 * 1000), [], 'Invalid delegation by guest')).rejects.toThrow('User does not have delegation privileges');
        });
        it('should emit delegation requested event', async () => {
            ...
            await manager.delegatePermissions('team-lead-001', 'analyst-001', ['read'], new Date(Date.now() + 24 * 60 * 60 * 1000), [], 'Project delegation');
            const eventData = await eventPromise;
            expect(eventData).toBeDefined();
            expect(eventData.delegatorId).toBe('team-lead-001');
            expect(eventData.delegateeId).toBe('analyst-001');
        });
    });
    ...
    describe('Cleanup and Maintenance', () => {
        ...
        it('should emit permission expired events for time-limited grants', async () => {
            ...
            const grant = await manager.grantPermission(request, 'manager-001', [], pastTime);
    ...
    describe('Error Handling', () => {
        it('should handle malformed permission requests gracefully', async () => {
            ...
            const result = await manager.evaluatePermissionRequest(malformedRequest);
            expect(result.granted).toBe(false);
