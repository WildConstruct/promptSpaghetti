 ...
 */
import { SecureSessionManager, SessionSecurityLevel, SessionState, SessionTerminationReason } from '../SecureSessionManager';
describe('SecureSessionManager', () => {
    ...
    describe('Session Creation', () => {
        test('should create secure session with all required properties', async () => {
            ...
            const result = await manager.createSession('user123', context, SessionSecurityLevel.MEDIUM, true);
        ...
        test('should handle high security level sessions', async () => {
            ...
            const result = await manager.createSession('user_secure', context, SessionSecurityLevel.HIGH);
            expect(result.session.securityLevel).toBe(SessionSecurityLevel.HIGH);
            expect(result.session.refreshToken).toBeDefined();
            expect(result.session.expiresAt.getTime()).toBeLessThan(mockDate.getTime() + 8 * 60 * 60 * 1000 // Less than 8 hours for HIGH security
            );
        });
    ...
    describe('Session Validation', () => {
        ...
        test('should detect device fingerprint mismatch', async () => {
            ...
            expect(validation.isValid).toBe(false);
            expect(validation.securityIssues.some(issue => issue.type === 'critical' && issue.description.includes('fingerprint mismatch'))).toBe(true);
        });
        test('should handle expired sessions', async () => {
            ...
            const { session, token } = await manager.createSession('user123', context, SessionSecurityLevel.CRITICAL // 1 hour expiry
            );
            ...
            const validation = await manager.validateSession(session.id, token, context);
            expect(validation.isValid).toBe(false);
            expect(validation.securityIssues.some(issue => issue.description.includes('expired'))).toBe(true);
        });
    ...
    describe('Session Termination', () => {
        test('should terminate session successfully', async () => {
            ...
            const { session } = await manager.createSession('user123', context);
            const terminated = await manager.terminateSession(session.id, SessionTerminationReason.MANUAL_LOGOUT);
        ...
        test('should terminate all user sessions except excluded', async () => {
            ...
            const terminated = await manager.terminateAllUserSessions('user123', SessionTerminationReason.PASSWORD_CHANGE, session1.session.id // Exclude this session
            );
    ...
    describe('Security Features', () => {
        ...
        test('should handle critical security level restrictions', async () => {
            ...
            const { session } = await manager.createSession('user_critical', context, SessionSecurityLevel.CRITICAL);
            // CRITICAL level allows only 1 concurrent session
            const secondSession = await manager.createSession('user_critical', context, SessionSecurityLevel.CRITICAL);
    ...
    describe('Error Handling', () => {
        ...
        test('should handle termination of non-existent session', async () => {
            const terminated = await manager.terminateSession('non-existent-id', SessionTerminationReason.MANUAL_LOGOUT);
