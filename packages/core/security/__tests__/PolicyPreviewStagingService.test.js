 ...
 */
import { PolicyPreviewStagingService, EnvironmentType, PreviewStatus, StagingDeploymentStatus, ValidationType, ValidationStatus, FeedbackType, FeedbackCategory, RollbackTriggerType } from '../PolicyPreviewStagingService';
describe('PolicyPreviewStagingService', () => {
    ...
    describe('Policy Preview Creation', () => {
        test('should create policy preview successfully', async () => {
            ...
            const preview = await service.createPolicyPreview(policyId, baseVersion, changes, {
        ...
        test('should run default validations on preview creation', async () => {
            const preview = await service.createPolicyPreview('policy-123', 'v1.0.0', [{
                    changeId: 'change-1',
                    section: 'Terms',
                    type: 'addition',
        ...
        test('should set preview status to REJECTED if validation has blockers', async () => {
            ...
            const preview = await service.createPolicyPreview('policy-123', 'v1.0.0', changes, {
        ...
        test('should emit previewCreated event', async () => {
            ...
            await service.createPolicyPreview('policy-123', 'v1.0.0', [{
                    changeId: 'change-1',
                    section: 'Test',
                    type: 'addition',
                    after: 'Test content',
    ...
    describe('Staging Deployment', () => {
        let testPreview;
        beforeEach(async () => {
            testPreview = await service.createPolicyPreview('policy-123', 'v1.0.0', [{
                    changeId: 'change-1',
                    section: 'Test',
                    type: 'modification',
                    before: 'Old content',
        ...
        test('should deploy preview to staging environment', async () => {
            ...
            const deployment = await service.deployToStaging(testPreview.previewId, environmentId, {
        ...
        test('should fail deployment to non-existent environment', async () => {
            await expect(service.deployToStaging(testPreview.previewId, 'non-existent-env', {})).rejects.toThrow('Staging environment not found');
        });
        test('should fail deployment if preview not found', async () => {
            await expect(service.deployToStaging('non-existent-preview', 'test-staging', {})).rejects.toThrow('Preview not found');
        });
        test('should respect max active deployments limit', async () => {
            ...
            for (let i = 0; i < environment.maxActiveDeployments; i++) {
                await service.deployToStaging(testPreview.previewId, environmentId, {});
            }
            // Try to deploy one more - should fail
            await expect(service.deployToStaging(testPreview.previewId, environmentId, {})).rejects.toThrow('Maximum active deployments reached');
        });
        test('should emit stagingDeploymentCreated event', async () => {
            const eventHandler = jest.fn();
            service.on('stagingDeploymentCreated', eventHandler);
            await service.deployToStaging(testPreview.previewId, 'test-staging', {});
            expect(eventHandler).toHaveBeenCalledWith(expect.objectContaining({
        ...
        test('should initialize default rollback triggers', async () => {
            const deployment = await service.deployToStaging(testPreview.previewId, 'test-staging', {});
            expect(deployment.rollbackTriggers).toBeInstanceOf(Array);
            expect(deployment.rollbackTriggers.length).toBeGreaterThan(0);
            const errorRateTrigger = deployment.rollbackTriggers.find(t => t.triggerType === RollbackTriggerType.ERROR_RATE);
    ...
    describe('Validation System', () => {
        ...
        test('should handle validation findings', async () => {
            const results = await service.runValidations(testPreview, [ValidationType.LEGAL, ValidationType.COMPLIANCE]);
    ...
    describe('User Feedback Collection', () => {
        ...
        test('should collect user feedback successfully', async () => {
            ...
            const userFeedback = await service.collectUserFeedback(testPreview.previewId, userId, feedback);
        ...
        test('should mark feedback as action required for low ratings', async () => {
            ...
            const userFeedback = await service.collectUserFeedback(testPreview.previewId, 'user-456', feedback);
        ...
        test('should emit userFeedbackReceived event', async () => {
            ...
            await service.collectUserFeedback(testPreview.previewId, 'user-456', {
        ...
        test('should fail feedback collection for non-existent preview', async () => {
            await expect(service.collectUserFeedback('non-existent-preview', 'user-456', {
                feedbackType: FeedbackType.GENERAL,
                rating: 3,
    ...
    describe('Policy Comparison Reports', () => {
        test('should generate policy comparison report', async () => {
            ...
            const policyId = 'policy-123';
            const report = await service.generateComparisonReport(baseVersion, compareVersion, policyId);
    ...
    describe('Preview Analytics', () => {
        ...
        test('should fail to get analytics for non-existent preview', async () => {
            await expect(service.getPreviewAnalytics('non-existent-preview')).rejects.toThrow('Preview not found');
        });
    });
    describe('Production Promotion', () => {
        ...
        test('should promote preview to production successfully', async () => {
            ...
            const result = await service.promoteToProduction(testPreview.previewId, options);
        ...
        test('should fail promotion for non-approved preview', async () => {
            ...
            await expect(service.promoteToProduction(testPreview.previewId, {
                approvedBy: 'user-admin',
        ...
        test('should emit previewPromoted event on successful promotion', async () => {
            ...
            await service.promoteToProduction(testPreview.previewId, {
                approvedBy: 'user-admin',
                effectiveDate: new Date(Date.now() + 24 * 60 * 60 * 1000)
            });
            expect(eventHandler).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Rollback Functionality', () => {
        let testDeployment;
        beforeEach(async () => {
            const testPreview = await service.createPolicyPreview('policy-123', 'v1.0.0', [{
                    changeId: 'change-1',
            ...
            testPreview.status = PreviewStatus.STAGED;
            testDeployment = await service.deployToStaging(testPreview.previewId, 'test-staging', {});
        });
        test('should rollback staging deployment successfully', async () => {
            ...
            const result = await service.rollbackStagingDeployment(testDeployment.deploymentId, reason, triggeredBy);
            expect(result).toBeDefined();
        ...
        test('should emit stagingRollback event', async () => {
            ...
            await service.rollbackStagingDeployment(testDeployment.deploymentId, 'Testing rollback', 'user-admin');
            expect(eventHandler).toHaveBeenCalledWith(expect.objectContaining({
        ...
        test('should fail rollback for non-existent deployment', async () => {
            await expect(service.rollbackStagingDeployment('non-existent-deployment', 'Test reason', 'user-admin')).rejects.toThrow('Staging deployment not found');
        });
    });
    describe('Configuration and Limits', () => {
        test('should respect preview retention days configuration', async () => {
            ...
            const preview = await customService.createPolicyPreview('policy-123', 'v1.0.0', [{
                    changeId: 'change-1',
                    section: 'Test',
                    type: 'modification',
                    before: 'Old',
    ...
    describe('Error Handling', () => {
        test('should handle service errors gracefully', async () => {
            ...
            await expect(service.createPolicyPreview('', // Invalid policy ID
            'v1.0.0', [], {
                title: 'Test',
                description: 'Test description',
                createdBy: 'user-123'
    ...
    describe('Performance and Scalability', () => {
        test('should handle multiple concurrent operations', async () => {
            const operations = [];
            for (let i = 0; i < 5; i++) {
                operations.push(service.createPolicyPreview(`policy-${i}`, 'v1.0.0', [{
                        changeId: `change-${i}`,
                        section: `Section ${i}`,
                        type: 'modification',
        ...
        test('should efficiently manage memory usage', async () => {
            ...
            for (let i = 0; i < 20; i++) {
                await service.createPolicyPreview(`policy-${i}`, 'v1.0.0', [{
                        changeId: `change-${i}`,
