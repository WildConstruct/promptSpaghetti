...
describe('DataClassificationAccessControl', () => {
    let mockRBACModel;
    let mockABACModel;
    let mockClassificationPolicies;
    let accessControlEngine;
    beforeEach(() => {
        ...
        accessControlEngine = new DataClassificationAccessControlEngine(mockRBACModel, mockABACModel, mockClassificationPolicies);
    });
    describe('RBAC Evaluation', () => {
        ...
        it('should enforce time-based constraints', async () => {
            ...
            const rbacDecision = await accessControlEngine.evaluateRBAC(request);
            expect(rbacDecision.permitted).toBe(false);
            expect(rbacDecision.denialReasons).toContain(expect.stringContaining('constraints not satisfied'));
        });
        it('should validate access control matrix compliance', async () => {
            ...
            const rbacDecision = await accessControlEngine.evaluateRBAC(request);
            expect(rbacDecision.permitted).toBe(false);
            expect(rbacDecision.denialReasons).toContain('Operation not permitted by access control matrix');
        });
    });
