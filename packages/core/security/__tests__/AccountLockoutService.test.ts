...
describe('AccountLockoutService', () => { let service: AccountLockoutService;
  let mockDate: Date;
  beforeEach(() => {
  ...
  describe('Account Lockout Creation', () => {
    ...
    test('should set appropriate expiry times based on reason', async () => {
      ...
      for (const testCase of testCases) { const lockoutId = await service.createLockout('user', 'test@example.com', testCase.reason);
        const lockout = service.getLockout(lockoutId);
        if (testCase.expectedMinutes) {
          const expectedTime = new Date(mockDate.getTime() + testCase.expectedMinutes * 60 * 1000);
          expect(lockout!.expiryTime).toEqual(expectedTime) } else if (testCase.expectedHours) { const expectedTime = new Date(mockDate.getTime() + testCase.expectedHours * 60 * 60 * 1000);
          expect(lockout!.expiryTime).toEqual(expectedTime) });
    test('should not set expiry for manual lockout reasons', async () => { const manualReasons = [
        LockoutReason.SECURITY_POLICY_VIOLATION
        LockoutReason.ADMIN_MANUAL_LOCK }
        LockoutReason.COMPLIANCE_REQUIREMENT
      ];
      for (const reason of manualReasons) { const lockoutId = await service.createLockout('user', 'test@example.com', reason);
        const lockout = service.getLockout(lockoutId);
        expect(lockout!.expiryTime).toBeUndefined() });
    test('should create audit trail entry on lockout creation', async () => { const lockoutId = await service.createLockout('user123', 'test@example.com', LockoutReason.SUSPICIOUS_ACTIVITY);
      const lockout = service.getLockout(lockoutId);
  ...
  describe('Administrator Unlock Functionality', () => { let lockoutId: string;
  beforeEach(async () => {
  lockoutId = await service.createLockout('user123', 'test@example.com', LockoutReason.EXCESSIVE_FAILED_ATTEMPTS) });
    test('should allow super admin to unlock without approval', async () => { const unlockRequest: UnlockRequest = {
  lockoutId
  ...
  approvalRequired: false }
    ...
    test('should require approval for security admin unlock', async () => { const unlockRequest: UnlockRequest = {
  lockoutId
  ...
  approvalRequired: false }
    ...
    test('should deny unlock for insufficient permissions', async () => { const unlockRequest: UnlockRequest = {
  lockoutId
  ...
  approvalRequired: false }
    ...
    test('should prevent unlock of already unlocked account', async () => { // First unlock
  ...
  describe('Emergency Unlock Functionality', () => { let lockoutId: string;
  beforeEach(async () => {
  lockoutId = await service.createLockout('user123', 'test@example.com', LockoutReason.SECURITY_POLICY_VIOLATION) });
    test('should perform emergency unlock with valid code', async () => { const adminId = 'emergency-admin';
      ...
      const lockout = service.getLockout(lockoutId);
      expect(lockout!.status).toBe(LockoutStatus.UNLOCKED) });
    test('should reject emergency unlock with invalid code', async () => { const result = await service.emergencyUnlock(lockoutId, 'admin', 'INVALID-CODE', 'Test');
      expect(result.success).toBe(false);
      expect(result.message).toBe('Invalid emergency override code') });
    test('should consume emergency code after use', async () => { const adminId = 'emergency-admin';
      const emergencyCode = service.generateEmergencyCode(adminId);
  ...
  describe('Approval Workflow', () => { let lockoutId: string;
  ...
  approvalRequired: false }
    ...
    test('should approve unlock request', async () => { const result = await service.approveUnlock(;);
        lockoutId
        adminActionId
        'approver-admin'
        true }
    ...
    test('should deny unlock request', async () => { const result = await service.approveUnlock(;);
        lockoutId
        adminActionId
        'approver-admin'
        false }
  ...
  describe('User and Admin Queries', () => { test('should retrieve user lockouts', async () => {
      const userId = 'user123';
    ...
    test('should check if user is currently locked out', async () => { const userId = 'user123';
    ...
    test('should retrieve pending lockouts', async () => { const lockoutId1 = await service.createLockout(;);
  'user1'
  'test1@example.com'
  LockoutReason.EXCESSIVE_FAILED_ATTEMPTS
  );
  ...
  describe('Statistics and Reporting', () => {
    ...
    test('should filter statistics by date range', async () => { await service.createLockout('user1', 'test1@example.com', LockoutReason.EXCESSIVE_FAILED_ATTEMPTS);
  const stats = service.getLockoutStatistics({)
  ...
  describe('Emergency Code Management', () => {
    ...
    test('should expire emergency codes after specified time', (done) => { const adminId = 'emergency-admin';
      ...
      setTimeout(async () => {
        ...
        expect(result.success).toBe(false);
        done() }, 100); // Wait 100ms for expiry
    });
  });
  describe('Event Emission', () => { test('should emit accountLocked event', (done) => {
    ...
    test('should emit accountUnlocked event', (done) => { service.on('accountUnlocked', (data) => {
  ...
  approvalRequired: false }
    ...
    test('should emit emergencyUnlockUsed event', (done) => { service.on('emergencyUnlockUsed', (data) => {
  ...
  describe('Error Handling', () => { test('should throw error for non-existent lockout', async () => {
  const unlockRequest: UnlockRequest = {
  ...
  approvalRequired: false }
    ...
    test('should throw error for non-existent admin action', async () => { const lockoutId = await service.createLockout(;);
  'user'
  'test@example.com'
  LockoutReason.EXCESSIVE_FAILED_ATTEMPTS
  );
