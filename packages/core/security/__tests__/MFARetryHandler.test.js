 ...
 */
import { MFARetryHandler, MFAOperation, RetryStrategy, FailureType } from '../services/MFARetryHandler';
describe('MFARetryHandler', () => {
    ...
    describe('Basic Operation Execution', () => {
        test('should execute successful operation without retries', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation, { userId: 'test-user' });
        ...
        test('should handle operation failure with retries', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.SMS_SEND, mockOperation, { userId: 'test-user' });
            expect(result.success).toBe(true);
            expect(result.data).toBe('success');
        ...
        test('should fail after max retry attempts', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.EMAIL_SEND, mockOperation, { userId: 'test-user' });
    ...
    describe('Retry Strategies', () => {
        test('should use exponential backoff strategy', async () => {
            ...
            const mockOperation = jest
                ...
                .mockRejectedValueOnce(new Error('Network error'))
                .mockResolvedValue('success');
            const resultPromise = customHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation);
        ...
        test('should use linear backoff strategy', async () => {
            ...
            const mockOperation = jest
                ...
                .mockRejectedValueOnce(new Error('Network error'))
                .mockResolvedValue('success');
            const resultPromise = customHandler.executeWithRetry(MFAOperation.SMS_SEND, mockOperation);
            jest.advanceTimersByTime(100); // First retry delay
            await Promise.resolve();
    ...
    describe('Error Classification', () => {
        test('should not retry non-retryable errors', async () => {
            const mockOperation = jest.fn().mockRejectedValue(new Error('Invalid code'));
            const result = await retryHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation, { userId: 'test-user' });
            expect(result.success).toBe(false);
            expect(result.attempts).toHaveLength(1);
            expect(mockOperation).toHaveBeenCalledTimes(1);
    ...
    describe('Timeout Handling', () => {
        test('should timeout long-running operations', async () => {
            ...
            const mockOperation = jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 200)) // Takes longer than timeout
            );
            const result = await customHandler.executeWithRetry(MFAOperation.EMAIL_SEND, mockOperation);
            expect(result.success).toBe(false);
    ...
    describe('Metrics Collection', () => {
        test('should collect and provide metrics', async () => {
            const mockOperation = jest
                ...
                .mockRejectedValueOnce(new Error('Network error'))
                .mockResolvedValue('success');
            await retryHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation, { userId: 'test-user' });
        ...
        test('should track failed operations in metrics', async () => {
            const mockOperation = jest.fn().mockRejectedValue(new Error('Persistent error'));
            await retryHandler.executeWithRetry(MFAOperation.SMS_SEND, mockOperation, { userId: 'test-user' });
            const metrics = retryHandler.getMetrics();
            expect(metrics.totalOperations).toBe(1);
    ...
    describe('Active Operations Tracking', () => {
        test('should track active operations', async () => {
            ...
            const operationPromise = retryHandler.executeWithRetry(MFAOperation.DEVICE_REGISTRATION, mockOperation, { userId: 'test-user' });
    ...
    describe('Event Emission', () => {
        test('should emit operation success events', async () => {
            ...
            const mockOperation = jest.fn().mockResolvedValue('success');
            await retryHandler.executeWithRetry(MFAOperation.BACKUP_CODE_VERIFICATION, mockOperation, { userId: 'test-user' });
        ...
        test('should emit operation failure events', async () => {
            ...
            await retryHandler.executeWithRetry(MFAOperation.METHOD_SETUP, mockOperation, { userId: 'test-user' });
    ...
    describe('Configuration', () => {
        test('should update configuration dynamically', () => {
            ...
            expect(configHandler).toHaveBeenCalledWith(expect.objectContaining({
        ...
        test('should use custom retry configuration', async () => {
            ...
            const mockOperation = jest.fn().mockRejectedValue(new Error('Any error'));
            const result = await customHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation);
    ...
    describe('Edge Cases', () => {
        test('should handle immediate success', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.TOTP_VERIFICATION, mockOperation);
            expect(result.success).toBe(true);
            expect(result.attempts).toHaveLength(1);
        ...
        test('should handle operation that throws non-Error objects', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.EMAIL_SEND, mockOperation);
            expect(result.success).toBe(false);
            expect(result.error).toBeInstanceOf(Error);
        ...
        test('should handle operation with undefined result', async () => {
            ...
            const result = await retryHandler.executeWithRetry(MFAOperation.METHOD_DISABLE, mockOperation);
            expect(result.success).toBe(true);
