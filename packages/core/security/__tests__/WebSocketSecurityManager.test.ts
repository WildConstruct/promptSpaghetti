 ...
 */
import { WebSocketSecurityManager,
...
describe('WebSocketSecurityManager', () => { let securityManager: WebSocketSecurityManager;
  ...
  const testConnectionRequest = { ipAddress: '192.168.1.100'
 ...
 as any);
  ...
  describe('Connection Initialization', () => { test('should initialize connection security context', async () => {
  ...
  algorithm: 'aes-256-gcm' }
    ...
    test('should handle high-risk connections', async () => { mockFingerprintService.assessRisk.mockReturnValue({)
  deviceId: 'fingerprint-123'
    ...
    test('should handle trusted devices', async () => { mockTrustedDeviceManager.checkDeviceTrust.mockResolvedValue({)
  trusted: true
 ...
 as any);
    ...
    test('should handle connection initialization errors', async () => { mockFingerprintService.generateFingerprint.mockRejectedValue(new Error('Fingerprint generation failed'));
      await expect()
  ...
  describe('Authentication', () => { let context: ConnectionSecurityContext;
  beforeEach(async () => {
  ...
  testConnectionRequest
    ...
    test('should authenticate with valid credentials', async () => { const result = await securityManager.authenticateConnection('conn-123', {)
  ...
  mfaCode: '123456' }
    ...
    test('should reject invalid token', async () => { const result = await securityManager.authenticateConnection('conn-123', {)
  token: 'invalid' }
});
    ...
    test('should require MFA for high-risk connections', async () => { // Create high-risk context
  mockFingerprintService.assessRisk.mockReturnValue({)
  deviceId: 'fingerprint-123'
  overallRisk: RiskLevel.HIGH
  riskScore: 85
  ...
  mfaCode: '123456' }
    ...
    test('should handle device verification', async () => { const result = await securityManager.authenticateConnection('conn-123', {)
  token: 'valid-token-12345'
  ...
  describe('Message Encryption/Decryption', () => { let context: ConnectionSecurityContext;
  beforeEach(async () => {
  context = await securityManager.initializeConnection()
  'conn-123'
  'user-456'
  ...
  token: 'valid-token-12345' }
    ...
    test('should encrypt outgoing messages', async () => { const message = {
        type: 'graph_update'
        payload: {
  nodeId: 'node-123' }
          data: { name: 'Test Node' }
    ...
    test('should decrypt incoming messages', async () => { // First encrypt a message
  ...
  cursor: { x: 100, y: 200 }
    ...
    test('should handle classification-based encryption', async () => { // Mock confidential data classification
  mockDataClassifier.classify.mockReturnValue({)
  ...
  ssn: '123-45-6789' }
    ...
    test('should reject messages with invalid signatures', async () => { const secureMessage: SecureWebSocketMessage = {
  id: 'msg-123'
  type: 'test'
  ...
  describe('Threat Detection', () => { let context: ConnectionSecurityContext;
  beforeEach(async () => {
  context = await securityManager.initializeConnection()
  'conn-123'
  'user-456'
  ...
  token: 'valid-token-12345' }
    ...
    test('should detect rate limit violations', async () => { const eventHandler = jest.fn();
  ...
  description: 'Rate limit exceeded' }
    ...
    test('should detect anomalous message patterns', async () => { const eventHandler = jest.fn();
  securityManager.on('securityEvent', eventHandler);
  ...
  description: 'Unusually large message detected' }
    ...
    test('should block connections for security violations', async () => { const blockHandler = jest.fn();
  ...
  duration: 60000 }
    ...
    test('should clean up blocked connections after timeout', async () => { await securityManager.blockConnection('conn-123', 'Temporary block', 1000);
      expect(securityManager.isConnectionBlocked('conn-123', testConnectionRequest.ipAddress)).toBe(true);
  ...
  describe('Connection Management', () => { test('should track multiple connections', async () => {
    ...
    test('should clean up connection resources', async () => { const context = await securityManager.initializeConnection(;);
        'conn-123',
        'user-456',
        testConnectionRequest
      );
    ...
    test('should handle cleanup of non-existent connections', async () => { // Should not throw error
      await expect()
        securityManager.cleanupConnection('non-existent-conn')
      ).resolves.not.toThrow() });
  });
  describe('Security Statistics', () => { test('should provide comprehensive security statistics', async () => {
  ...
  avgRiskScore: expect.any(Number) }
    ...
    test('should handle empty statistics gracefully', async () => { const stats = securityManager.getSecurityStats();
      expect(stats.totalConnections).toBe(0);
      expect(stats.authenticatedConnections).toBe(0);
      expect(stats.avgRiskScore).toBe(0) });
  });
  describe('Configuration and Cleanup', () => { test('should handle destruction properly', async () => {
      // Create connections
      await securityManager.initializeConnection('conn-1', 'user-1', testConnectionRequest);
      await securityManager.initializeConnection('conn-2', 'user-2', testConnectionRequest);
    ...
    test('should handle key rotation', async () => { const context = await securityManager.initializeConnection(;);
        'conn-123',
        'user-456' }
        testConnectionRequest
      );
  ...
  describe('Error Handling', () => { test('should handle encryption errors gracefully', async () => {
      const context = await securityManager.initializeConnection(;);
        'conn-123',
        'user-456' }
        testConnectionRequest
    ...
    test('should handle classification service errors', async () => { mockDataClassifier.classify.mockImplementation(() => {
