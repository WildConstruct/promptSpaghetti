 ...
 */
import { LockoutNotificationService,
  NotificationChannel,
...
describe('LockoutNotificationService', () => { let service: LockoutNotificationService;
  ...
  describe('Lockout Notifications', () => { test('should send lockout notification with default channels', async () => {
    ...
    test('should send lockout notification with specified channels', async () => { const channels = [NotificationChannel.EMAIL, NotificationChannel.SMS];
    ...
    test('should use high priority for suspicious activity', async () => { const suspiciousLockout = {
  ...mockLockout
  reason: LockoutReason.SUSPICIOUS_ACTIVITY }
    ...
    test('should use urgent priority for system security alerts', async () => { const alertLockout = {
  ...mockLockout
  ...
  describe('Unlock Notifications', () => { test('should send unlock notification', async () => {
  ...
  unlockTime: new Date(mockDate.getTime() + 60 * 60 * 1000) }
    ...
    test('should send unlock notification without admin name', async () => { const unlockedLockout = {
  ...mockLockout
  status: LockoutStatus.UNLOCKED
  unlockTime: new Date() }
};
  ...
  describe('Security Alert Notifications', () => { test('should send security alert with urgent priority', async () => {
  const alertDetails = {
  ...
  describe('Admin Notifications', () => {
    test('should send admin notifications to specified roles', async () => {
      ...
      for (const notificationId of notificationIds) { const status = service.getNotificationStatus(notificationId);
        expect(status.request!.type).toBe(NotificationType.ADMIN_ACTION_REQUIRED);
        expect(status.request!.priority).toBe(NotificationPriority.HIGH) });
  });
  describe('User Preferences', () => { test('should respect user notification preferences', async () => {
  ...
  metadata: {}
    ...
    test('should emit preferences updated event', (done) => {
      ...
      service.on('preferencesUpdated', (data) => { expect(data.userId).toBe('user123');
        expect(data.preferences.channels.sms).toBe(true);
        done() });
      service.updateUserPreferences('user123', preferences);
    });
  });
  describe('Template Management', () => { test('should create custom notification template', () => {
      const template = {
  ...
  expiryMinutes: 60
    ...
    test('should emit template created event', (done) => { service.on('templateCreated', (template) => {
  ...
  describe('Notification Delivery', () => { test('should track delivery status progression', async () => {
      const notificationId = await service.sendLockoutNotification(mockLockout);
    ...
    test('should emit notification delivered events', (done) => { service.on('notificationDelivered', (delivery) => {
    ...
    test('should handle delivery failures', (done) => { service.on('notificationFailed', (delivery) => {
  ...
  describe('Test Notifications', () => { test('should send test notification', async () => {
  const variables = {
  userName: 'Test User'
  userEmail: 'test@example.com'
  ...
  describe('User Notification History', () => {
    ...
    test('should limit notification history results', async () => { // Send multiple notifications
      for (let i = 0; i < 5; i++) {
        await service.sendLockoutNotification(mockLockout);
  ...
  describe('Statistics and Reporting', () => {
    ...
    test('should filter statistics by date range', async () => { await service.sendLockoutNotification(mockLockout);
  const stats = service.getNotificationStatistics({)
  start: new Date(mockDate.getTime() - 24 * 60 * 60 * 1000),
    ...
    test('should calculate delivery rate correctly', async () => { await service.sendLockoutNotification(mockLockout);
  ...
  describe('Event Emission', () => { test('should emit notification queued events', (done) => {
  ...
  describe('Error Handling', () => { test('should handle missing template gracefully', async () => {
    ...
    test('should handle invalid notification request gracefully', () => { const status = service.getNotificationStatus('non-existent-id');
      expect(status.request).toBeNull();
      expect(status.deliveries).toHaveLength(0);
      expect(status.summary.totalDeliveries).toBe(0) });
  });
  describe('Channel-Specific Behavior', () => { test('should handle different channels appropriately', async () => {
      const channels = [
        NotificationChannel.EMAIL,
  ...
  describe('Priority Handling', () => { test('should process urgent notifications immediately', async () => {
  const urgentLockout = {
  ...mockLockout,
  reason: LockoutReason.SYSTEM_SECURITY_ALERT }
};
