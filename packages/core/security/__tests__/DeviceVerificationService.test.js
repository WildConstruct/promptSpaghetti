 ...
 */
import { DeviceVerificationService, VerificationStep, ChallengeType, VerificationOutcome } from '../DeviceVerificationService';
import { DeviceFingerprintingService, RiskLevel, FingerprintType } from '../DeviceFingerprintingService';
...
describe('DeviceVerificationService', () => {
    ...
    let mockVerificationCodeManager;
    let mockEmailTracker;
    // Test data
    ...
    beforeEach(() => {
        ...
        mockVerificationCodeManager = new VerificationCodeManager();
        mockEmailTracker = new EmailDeliveryTracker();
        // Setup spies
        ...
        mockFingerprintService.generateFingerprint.mockResolvedValue(testFingerprint);
        ...
        mockEmailTracker.sendEmail.mockResolvedValue('email-123');
        service = new DeviceVerificationService(mockFingerprintService, mockTrustedDeviceManager, mockVerificationCodeManager, mockEmailTracker, {
    ...
    describe('Challenge Handling', () => {
        ...
        test('should submit valid email code challenge', async () => {
            ...
            const result = await service.submitChallengeResponse(session.id, challenge.id, '123456');
        ...
        test('should reject invalid challenge response', async () => {
            const challenge = session.challenges.find(c => c.type === ChallengeType.EMAIL_CODE);
            const result = await service.submitChallengeResponse(session.id, challenge.id, 'wrong-code');
        ...
        test('should handle challenge not found', async () => {
            await expect(service.submitChallengeResponse(session.id, 'invalid-challenge-id', '123456')).rejects.toThrow('Challenge not found');
        });
        test('should handle expired session', async () => {
            // Fast forward past session expiration
            jest.advanceTimersByTime(31 * 60 * 1000); // 31 minutes
            const challenge = session.challenges.find(c => c.type === ChallengeType.EMAIL_CODE);
            await expect(service.submitChallengeResponse(session.id, challenge.id, '123456')).rejects.toThrow('Verification session expired');
        });
    });
    describe('Session Management', () => {
        ...
        test('should get user sessions', async () => {
            ...
            const sessions = await Promise.all(requests.map(request => service.startVerification(request)));
    ...
    describe('Risk Assessment and Security Flags', () => {
        ...
        test('should detect device spoofing', async () => {
            ...
            mockFingerprintService.generateFingerprint.mockResolvedValue(spoofedFingerprint);
            const request = {
                userId: testUserId,
    ...
    describe('Device Registration', () => {
        test('should register trusted device on successful verification', async () => {
            ...
            const challenge = session.challenges.find(c => c.type === ChallengeType.EMAIL_CODE);
            await service.submitChallengeResponse(session.id, challenge.id, '123456');
            expect(mockTrustedDeviceManager.registerTrustedDevice).toHaveBeenCalledWith(expect.objectContaining({
                userId: testUserId,
    ...
    describe('Error Handling', () => {
        test('should handle verification code generation failure', async () => {
            mockVerificationCodeManager.generateCode.mockResolvedValue(null);
            const request = {
                userId: testUserId,
                fingerprintContext: testContext,
                verificationMethod: VerificationMethod.EMAIL,
        ...
        test('should handle session not found errors', async () => {
            await expect(service.submitChallengeResponse('invalid-session-id', 'challenge-id', 'response')).rejects.toThrow('Verification session not found');
        });
        test('should handle completed session submission', async () => {
            ...
            await service.submitChallengeResponse(session.id, challenge.id, '123456');
            // Try to submit again
            await expect(service.submitChallengeResponse(session.id, challenge.id, '123456')).rejects.toThrow('Verification session already completed');
        });
    });
});
