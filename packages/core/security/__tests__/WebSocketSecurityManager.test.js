 ...
 */
import { WebSocketSecurityManager } from '../WebSocketSecurityManager';
...
describe('WebSocketSecurityManager', () => {
    ...
    let mockFingerprintService;
    let mockTrustedDeviceManager;
    const testConfig = {
    ...
    beforeEach(() => {
        ...
        mockKeyManagementService = new KeyManagementService({});
        mockDataClassifier = new DataClassifier();
        mockFingerprintService = new DeviceFingerprintingService();
        mockTrustedDeviceManager = new TrustedDeviceManager(mockFingerprintService);
        // Setup spies
        ...
        securityManager = new WebSocketSecurityManager(testConfig, mockKeyManagementService, mockDataClassifier, mockFingerprintService, mockTrustedDeviceManager);
    });
    afterEach(() => {
        jest.useRealTimers();
    ...
    describe('Connection Initialization', () => {
        test('should initialize connection security context', async () => {
            const context = await securityManager.initializeConnection('conn-123', 'user-456', testConnectionRequest);
            ...
            expect(mockKeyManagementService.generateKey).toHaveBeenCalledWith(expect.objectContaining({
                type: 'symmetric',
        ...
        test('should handle connection initialization errors', async () => {
            ...
            await expect(securityManager.initializeConnection('conn-123', 'user-456', testConnectionRequest)).rejects.toThrow('Fingerprint generation failed');
        });
    });
    describe('Authentication', () => {
        let context;
        beforeEach(async () => {
            context = await securityManager.initializeConnection('conn-123', 'user-456', testConnectionRequest);
        });
        ...
        test('should require MFA for high-risk connections', async () => {
            ...
            const highRiskContext = await securityManager.initializeConnection('conn-456', 'user-456', testConnectionRequest);
    ...
    describe('Message Encryption/Decryption', () => {
        ...
        test('should reject messages with invalid signatures', async () => {
            ...
            await expect(securityManager.decryptMessage('conn-123', secureMessage)).rejects.toThrow('Message signature verification failed');
        });
    });
    describe('Threat Detection', () => {
        ...
        test('should block connections for security violations', async () => {
            ...
            expect(blockHandler).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Connection Management', () => {
        test('should track multiple connections', async () => {
            const context1 = await securityManager.initializeConnection('conn-1', 'user-1', testConnectionRequest);
            const context2 = await securityManager.initializeConnection('conn-2', 'user-2', testConnectionRequest);
        ...
        test('should clean up connection resources', async () => {
            ...
            expect(mockKeyManagementService.revokeKey).toHaveBeenCalledWith('session-key-123', 'system', 'Session ended');
        });
        test('should handle cleanup of non-existent connections', async () => {
            // Should not throw error
            await expect(securityManager.cleanupConnection('non-existent-conn')).resolves.not.toThrow();
        });
    });
    ...
    describe('Error Handling', () => {
        ...
        test('should handle classification service errors', async () => {
            ...
            await expect(securityManager.encryptMessage('conn-123', message)).rejects.toThrow('Classification failed');
        });
        test('should handle invalid connection contexts', async () => {
            await expect(securityManager.authenticateConnection('non-existent', { token: 'test' })).rejects.toThrow('Connection context not found');
            await expect(securityManager.encryptMessage('non-existent', { type: 'test', payload: {} })).rejects.toThrow('Connection context not found');
        });
    });
});
