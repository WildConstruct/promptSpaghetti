 ...
 */
import { PasswordResetTokenManager,
  TokenType,
  TokenStatus,
  SecurityLevel,
  TokenConfig,
...
describe('PasswordResetTokenManager', () => { let manager: PasswordResetTokenManager;
  beforeEach(() => {
  ...
  describe('Token Generation', () => { test('should generate a password reset token successfully', async () => {
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should generate tokens with different security levels', async () => { const baseRequest: TokenRequest = {,
  ...
  securityLevel: SecurityLevel.MAXIMUM }
    ...
    test('should emit token generated event', (done) => { manager.on('tokenGenerated', (data) => {
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should handle custom expiration times', async () => { const request: TokenRequest = {,
  userId: 'user-custom',
  ...
  expirationMinutes: 30 // 30 minutes }
    ...
    test('should revoke existing tokens when multiple not allowed', async () => { const singleTokenManager = new PasswordResetTokenManager({)
  ...
  describe('Token Validation', () => { test('should validate a valid token successfully', async () => {
  const request: TokenRequest = {,
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should reject invalid tokens', async () => { const validation = await manager.validateToken(;);
        'invalid-token-12345',
        '192.168.1.1' }
        'Mozilla/5.0'
      );
    ...
    test('should detect expired tokens', async () => { const shortLivedManager = new PasswordResetTokenManager({)
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should calculate risk scores for validation', async () => { const request: TokenRequest = {,
  userId: 'user-risk',
  email: 'risk@test.com',
  type: TokenType.PASSWORD_RESET,
  ipAddress: '192.168.1.1',
  userAgent: 'Mozilla/5.0' }
    ...
    test('should emit validation events', (done) => { let eventsReceived = 0;
  ...
  describe('Token Usage', () => { test('should use a valid token successfully', async () => {
  const request: TokenRequest = {,
  userId: 'user-use',
  email: 'use@test.com',
  type: TokenType.PASSWORD_RESET,
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should prevent reuse of used tokens', async () => { const request: TokenRequest = {,
  userId: 'user-reuse',
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should emit token used event', (done) => { manager.on('tokenUsed', (data) => {
        expect(data.tokenId).toBeDefined();
  ...
  describe('Token Revocation', () => { test('should revoke a specific token', async () => {
  const request: TokenRequest = {,
  userId: 'user-revoke',
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should revoke all user tokens', async () => { const userId = 'user-revoke-all';
  ...
  expect(emailVerificationToken?.status).toBe(TokenStatus.ACTIVE);
    ...
    test('should emit revocation events', (done) => { let eventsReceived = 0;
  ...
  describe('Rate Limiting', () => { test('should enforce rate limiting by email', async () => {
  ...
  userId: 'user-rate-limit-3' }
    ...
    test('should enforce rate limiting by IP address', async () => { const rateLimitedManager = new PasswordResetTokenManager({)
  rateLimitCount: 2,
  ...
  describe('Token Information and Management', () => { test('should get token information safely', async () => {
  const request: TokenRequest = {,
  userId: 'user-info',
  email: 'info@test.com',
  type: TokenType.PASSWORD_RESET,
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should get user tokens', async () => { const userId = 'user-tokens';
  const requests: TokenRequest = [
  {
  userId,
  ...
  expect(passwordResetTokens[0].type).toBe(TokenType.PASSWORD_RESET);
    ...
    test('should return null for non-existent token', () => { const tokenInfo = manager.getTokenInfo('non-existent-token');
  ...
  describe('Security Events and Audit Logging', () => { test('should emit security events', (done) => {
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should log rate limit violations', async () => { const rateLimitedManager = new PasswordResetTokenManager({)
  rateLimitCount: 1,
  rateLimitWindow: 60000,
  antiEnumerationDelay: 1,
  enableAuditLogging: true }
      ...
      rateLimitedManager.on('securityEvent', (event) => { if (event.event === SecurityEvent.RATE_LIMIT_EXCEEDED) {
  ...
  rateLimitCount: 5 }
    ...
    test('should use custom configuration on initialization', () => { const customManager = new PasswordResetTokenManager({)
  ...
  describe('Cleanup and Destruction', () => { test('should perform cleanup and emit event', (done) => {
    ...
    test('should clean up expired tokens', async () => { const quickCleanupManager = new PasswordResetTokenManager({)
  defaultExpiration: 1, // 1 millisecond,
  antiEnumerationDelay: 1 }
});
      const request: TokenRequest = { ,
  ...
  describe('Error Handling and Edge Cases', () => { test('should handle token generation errors gracefully', async () => {
  ...
  userAgent: 'Mozilla/5.0' }
    ...
    test('should handle validation errors gracefully', async () => { const validation = await manager.validateToken(;);
        '', // Empty token
        '192.168.1.1' }
        'Mozilla/5.0'
      );
    ...
    test('should handle usage of invalid tokens', async () => { const usage = await manager.useToken(;);
    ...
    test('should handle revocation of non-existent tokens', async () => { const success = await manager.revokeToken(;);
        'non-existent-token' }
        'test_revocation'
      );
      expect(success).toBe(false);
    });
    test('should handle multiple revocations of same token', async () => { const request: TokenRequest = {,
