...
describe('PasswordResetFlow', () => {
    ...
    describe('Step 1: Request Reset', () => {
        ...
        test('should handle successful reset request', async () => {
            ...
            await waitFor(() => {
                ...
                expect(defaultProps.onSecurityEvent).toHaveBeenCalledWith('password_reset_requested', expect.objectContaining({
    ...
    describe('Step 3: Reset Password', () => {
        const navigateToResetStep = async (user) => {
            const emailInput = screen.getByLabelText('Email Address');
            await user.type(emailInput, 'test@example.com');
            await user.click(screen.getByRole('button', { name: /send reset code/i }));
            act(() => {
    ...
    describe('Custom Validation', () => {
        test('should use custom password validation when provided', async () => {
            ...
            const user = userEvent.setup({ advanceTimers: jest.advanceTimersByTime });
            render(_jsx(PasswordResetFlow, { ...defaultProps, customValidation: customValidation }));
