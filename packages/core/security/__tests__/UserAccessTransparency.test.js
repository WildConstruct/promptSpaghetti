 ...
 * - Settings management
 */
import { UserAccessTransparencyService, TransparencyEventType } from '../UserAccessTransparency';
describe('UserAccessTransparencyService', () => {
    ...
    describe('User Access Activity Tracking', () => {
        test('should retrieve user access activities', async () => {
            ...
            const activities = await transparencyService.getUserAccessActivity(userId, timeRange, 50);
        ...
        test('should filter activities by time range', async () => {
            ...
            const activities = await transparencyService.getUserAccessActivity(userId, timeRange, 100);
        ...
        test('should limit number of activities returned', async () => {
            const userId = 'user-123';
            const limit = 10;
            const activities = await transparencyService.getUserAccessActivity(userId, undefined, limit);
    ...
    describe('Data Subject Access Requests (DSAR)', () => {
        test('should submit DSAR request successfully', async () => {
            ...
            const dsarRequest = await transparencyService.submitDSAR(userId, requestType, details);
        ...
        test('should calculate correct completion deadline', async () => {
            ...
            const daysDifference = Math.ceil((deadline.getTime() - requestTime.getTime()) / (1000 * 60 * 60 * 24));
        ...
        test('should support different DSAR types', async () => {
            ...
            for (const requestType of requestTypes) {
                const dsarRequest = await transparencyService.submitDSAR(userId, requestType, { identityVerified: true, urgency: 'STANDARD' });
                expect(dsarRequest.requestType).toBe(requestType);
                expect(dsarRequest.status).toBe('PENDING');
            }
        });
    ...
    describe('Privacy Score Calculation', () => {
        ...
        test('should throw error when privacy scoring is disabled', async () => {
            ...
            const userId = 'user-123';
            await expect(service.getPrivacyScore(userId)).rejects.toThrow('Privacy scoring is disabled');
        });
    ...
    describe('Transparency Settings Management', () => {
        test('should update user transparency settings', async () => {
            ...
            const updatedSettings = await transparencyService.updateTransparencySettings(userId, settingsUpdate);
        ...
        test('should validate settings format', async () => {
            ...
            await expect(transparencyService.updateTransparencySettings(userId, invalidSettings)).resolves.toBeDefined();
        });
    });
    ...
    describe('Data Export Functionality', () => {
        test('should export user data in JSON format', async () => {
            const userId = 'user-123';
            const format = 'JSON';
            const exportResponse = await transparencyService.exportUserData(userId, format);
        ...
        test('should throw error when data portability is disabled', async () => {
            ...
            const service = new UserAccessTransparencyService(disabledConfig);
            const userId = 'user-123';
            await expect(service.exportUserData(userId)).rejects.toThrow('Data portability is disabled');
        });
        ...
        test('should filter by categories when specified', async () => {
            ...
            const exportResponse = await transparencyService.exportUserData(userId, 'JSON', categories);
            expect(exportResponse.dataIncluded).toEqual(expect.arrayContaining(categories));
        });
    });
    ...
    describe('Error Handling', () => {
        test('should handle service errors gracefully', async () => {
            ...
            await expect(transparencyService.generateUserDataInventory(userId)).resolves.toBeDefined();
        });
        ...
        test('should validate input parameters', async () => {
            ...
            await expect(transparencyService.submitDSAR('', 'ACCESS', { identityVerified: false, urgency: 'STANDARD' })).rejects.toThrow();
        });
    });
    describe('Performance and Scalability', () => {
        ...
        test('should handle high-frequency notification sending', async () => {
            ...
            const promises = notifications.map(notification => transparencyService.sendTransparencyNotification(notification));
