 ...
 */
import { AuditOperation, createAuditLogger } from '../AuditLogger';
...
jest.mock('../DataClassifier', () => ({
    DataClassifier: jest.fn().mockImplementation(() => ({
        classifyData: jest.fn().mockResolvedValue({
...
jest.mock('../ClassificationEnforcer', () => ({
    ClassificationEnforcer: jest.fn().mockImplementation(() => ({
        enforceClassification: jest.fn().mockResolvedValue({
            allowed: true,
            riskScore: 25,
...
describe('AuditIntegration', () => {
    ...
    describe('Data Access Logging', () => {
        it('should log data access with context enrichment', async () => {
            ...
            await auditIntegration.logDataAccess(context, 'customer_profile', 'cust_456', testData, { purpose: 'compliance_check' });
        ...
        it('should handle access denial logging', async () => {
            ...
            await denialIntegration.logDataAccess(context, 'sensitive_data', 'secret_123', null, { attemptedAccess: true });
            const logs = await auditLogger.query({});
    ...
    describe('Administrative Operations', () => {
        it('should log admin operations correctly', async () => {
            ...
            await auditIntegration.logAdminOperation('GRANT_ACCESS', adminContext, {
                userId: 'analyst456',
    ...
    describe('Batch Operations', () => {
        it('should log batch operations with highest classification', async () => {
            ...
            await auditIntegration.logBatchOperation(context, AuditOperation.EXPORT, resources, true, { exportFormat: 'JSON' });
            const logs = await auditLogger.query({});
            expect(logs).toHaveLength(1);
            const log = logs[0];
    ...
    describe('Security Events', () => {
        it('should log security events', async () => {
            ...
            await auditIntegration.logSecurityEvent('ANOMALOUS_ACCESS_PATTERN', context, {
    ...
    describe('Audit Trails', () => {
        it('should create workflow audit trails', async () => {
            ...
            const correlationId = await auditIntegration.startAuditTrail('training_workflow_001', context, { modelType: 'classification' });
    ...
    describe('Compliance Reporting', () => {
        ...
        it('should generate compliance reports', async () => {
            ...
            const report = await auditIntegration.generateComplianceReport(startDate, endDate, { includeDetails: false });
        ...
        it('should include details when requested', async () => {
            const startDate = new Date(Date.now() - 24 * 60 * 60 * 1000);
            const endDate = new Date();
            const report = await auditIntegration.generateComplianceReport(startDate, endDate, { includeDetails: true });
    ...
    describe('Error Handling', () => {
        it('should handle missing optional components gracefully', async () => {
            ...
            await expect(minimalIntegration.logDataAccess(context, 'test_resource', 'test_123', { some: 'data' })).resolves.not.toThrow();
            const logs = await auditLogger.query({});
            expect(logs).toHaveLength(1);
        });
    });
