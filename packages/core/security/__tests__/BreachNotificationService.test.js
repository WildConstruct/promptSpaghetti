 ...
 */
import { BreachNotificationService, BreachSeverity, NotificationType, IncidentStatus } from '../BreachNotificationService';
...
describe('BreachNotificationService', () => {
    ...
    beforeEach(() => {
        ...
        const mockDateConstructor = jest.fn().mockImplementation((dateString) => {
            if (dateString) {
    ...
    describe('Incident Creation and Management', () => {
        test('should create incident with proper classification', async () => {
            const incidentData = {
            ...
            expect(incident.complianceRequirements).toContainEqual(expect.objectContaining({
                framework: 'GDPR',
                requirement: 'Notify supervisory authority within 72 hours'
            }));
        });
        test('should update incident status and maintain timeline', async () => {
        ...
        test('should handle incident filtering and querying', async () => {
    ...
    describe('Breach Classification System', () => {
        test('should classify PII breaches as CRITICAL', async () => {
            const incidentId = await service.reportBreach({
                title: 'PII Breach',
        ...
        test('should classify authentication system breaches as HIGH', async () => {
            const incidentId = await service.reportBreach({
        ...
        test('should classify low-impact breaches as MEDIUM or LOW', async () => {
            const incidentId = await service.reportBreach({
    ...
    describe('GDPR Compliance Management', () => {
        test('should require GDPR notification for PII breaches', async () => {
            const incidentId = await service.reportBreach({
            ...
            expect(gdprNotification.deadline.getTime()).toBe(mockDate.getTime() + (72 * 60 * 60 * 1000) // 72 hours
            );
        });
        ...
        test('should detect GDPR deadline violations', async () => {
            const incidentId = await service.reportBreach({
                title: 'Old Breach',
        ...
        test('should not require GDPR notification for non-PII breaches', async () => {
            const incidentId = await service.reportBreach({
    ...
    describe('Notification System', () => {
        test('should send notifications to correct recipients', async () => {
            const incidentId = await service.reportBreach({
            ...
            const notifications = await service.sendNotification(incidentId, NotificationType.INTERNAL_ALERT, ['security@test.com', 'management@test.com'], 'critical_breach_alert');
        ...
        test('should handle notification delivery failures', async () => {
            ...
            const originalDeliverNotification = service.deliverNotification;
            service.deliverNotification = jest.fn().mockRejectedValue(new Error('Delivery failed'));
            const incidentId = await service.reportBreach({
                title: 'Test Breach',
                description: 'Test',
                dataTypes: ['logs'],
            ...
            const notifications = await service.sendNotification(incidentId, NotificationType.INTERNAL_ALERT, ['failing@test.com']);
            expect(notifications[0].status).toBe('failed');
            expect(notifications[0].metadata.error).toBe('Delivery failed');
            // Restore original method
            service.deliverNotification = originalDeliverNotification;
        });
        test('should determine appropriate notification channels', async () => {
            const incidentId = await service.reportBreach({
                title: 'Test Breach',
                description: 'Test',
                dataTypes: ['logs'],
            ...
            const emailNotifications = await service.sendNotification(incidentId, NotificationType.INTERNAL_ALERT, ['email@test.com']);
            const smsNotifications = await service.sendNotification(incidentId, NotificationType.INTERNAL_ALERT, ['+1234567890']);
            expect(emailNotifications[0].channel).toBe('email');
            expect(smsNotifications[0].channel).toBe('sms');
        });
    });
    describe('Report Generation', () => {
        test('should generate summary reports', async () => {
            const incidentId = await service.reportBreach({
                title: 'Test Incident',
                description: 'Test description',
                dataTypes: ['test_data'],
    ...
    describe('Event System and Monitoring', () => {
        test('should emit events for incident lifecycle', async () => {
            const events = [];
            ...
            await service.sendNotification(incidentId, NotificationType.INTERNAL_ALERT, ['test@example.com']);
            expect(events).toContain('created');
            expect(events).toContain('updated');
            expect(events).toContain('notifications');
        });
        test('should emit GDPR deadline warnings', (done) => {
            service.on('gdprDeadlineApproaching', (incident) => {
        ...
        test('should track compliance violations', (done) => {
            service.on('complianceViolation', (data) => {
    ...
    describe('Error Handling and Edge Cases', () => {
        test('should handle invalid incident IDs', () => {
        ...
        test('should handle unknown report formats', async () => {
            const incidentId = await service.reportBreach({
                title: 'Test',
                description: 'Test',
                dataTypes: ['test'],
            ...
            expect(() => service.generateReport(incidentId, 'unknown'))
        ...
        test('should validate incident data completeness', async () => {
            const incidentId = await service.reportBreach({
    ...
    describe('Performance and Scalability', () => {
        test('should handle multiple concurrent incidents', async () => {
            const promises = Array.from({ length: 10 }, (_, i) => service.reportBreach({
        ...
        test('should efficiently filter large incident collections', async () => {
            ...
            const promises = Array.from({ length: 100 }, (_, i) => service.reportBreach({
    ...
    describe('Integration and Configuration', () => {
        ...
        test('should maintain audit trail integrity', async () => {
            ...
            for (let i = 1; i < incident.timeline.length; i++) {
                expect(incident.timeline[i].timestamp.getTime())
