 ...
 */
import { ExemptionManager, ExemptionType, ExemptionScope, ExemptionStatus, ExemptionPriority, ExemptionReason } from '../ExemptionManager';
...
describe('ExemptionManager', () => {
    ...
    describe('Exemption Request Creation', () => {
        test('should create exemption request with approval required', async () => {
            ...
            const exemptionId = await manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN);
        ...
        test('should reject request from unauthorized role', async () => {
            ...
            await expect(manager.requestExemption(request, 'helpdesk1', 'helpdesk@company.com', AdminRole.HELP_DESK)).rejects.toThrow('Role help_desk is not authorized');
        });
        test('should reject request exceeding maximum duration', async () => {
            ...
            await expect(manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN)).rejects.toThrow('Requested duration 365 days exceeds maximum allowed 30 days');
        });
    });
    describe('Exemption Approval Workflow', () => {
        ...
        beforeEach(async () => {
            ...
            exemptionId = await manager.requestExemption(request, 'requester1', 'requester@company.com', AdminRole.HELP_DESK);
        });
        test('should approve pending exemption', async () => {
            const approved = await manager.approveExemption(exemptionId, 'approver1', 'approver@company.com', AdminRole.SECURITY_ADMIN, 'Approved after review');
        ...
        test('should deny pending exemption', async () => {
            const denied = await manager.denyExemption(exemptionId, 'approver1', 'approver@company.com', AdminRole.SECURITY_ADMIN, 'Insufficient justification');
        ...
        test('should reject approval from unauthorized role', async () => {
            await expect(manager.approveExemption(exemptionId, 'unauthorized', 'unauthorized@company.com', AdminRole.HELP_DESK, 'Unauthorized approval')).rejects.toThrow('Role help_desk is not authorized to approve');
        });
        test('should reject approval of non-pending exemption', async () => {
            // First approve the exemption
            await manager.approveExemption(exemptionId, 'approver1', 'approver@company.com', AdminRole.SECURITY_ADMIN);
            // Try to approve again
            await expect(manager.approveExemption(exemptionId, 'approver2', 'approver2@company.com', AdminRole.SECURITY_ADMIN)).rejects.toThrow('is not pending approval');
        });
    });
    describe('Exemption Checking and Usage', () => {
        let activeExemptionId;
        beforeEach(async () => {
            const request = {
                type: ExemptionType.RATE_LIMITING,
                scope: ExemptionScope.USER,
            ...
            activeExemptionId = await manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN);
            // Auto-approved for API_KEY scope, but we need to approve for USER scope
            await manager.approveExemption(activeExemptionId, 'approver1', 'approver@company.com', AdminRole.SECURITY_ADMIN);
        });
        test('should grant exemption when conditions are met', () => {
            ...
            const result = manager.checkExemption(ExemptionType.RATE_LIMITING, ExemptionScope.USER, 'user789', context);
        ...
        test('should deny exemption when no active exemption exists', () => {
            const result = manager.checkExemption(ExemptionType.RATE_LIMITING, ExemptionScope.USER, 'nonexistent-user');
            expect(result.granted).toBe(false);
    ...
    describe('Emergency Exemptions', () => {
        test('should create emergency exemption with valid code', async () => {
            ...
            const exemptionId = await manager.createEmergencyExemption(request, 'emergency-admin', 'emergency@company.com', AdminRole.SUPER_ADMIN, 'EMERGENCY-SA-ABCD1234', 'Critical business impact - system down');
            expect(exemptionId).toBeTruthy();
            const query = manager.queryExemptions({
        ...
        test('should reject emergency exemption with invalid code', async () => {
            ...
            await expect(manager.createEmergencyExemption(request, 'admin', 'admin@company.com', AdminRole.SUPER_ADMIN, 'INVALID-CODE', 'Emergency')).rejects.toThrow('Invalid emergency override code');
        });
        test('should reject emergency exemption for non-emergency type', async () => {
            ...
            await expect(manager.createEmergencyExemption(request, 'admin', 'admin@company.com', AdminRole.SUPER_ADMIN, 'EMERGENCY-SA-ABCD1234', 'Emergency')).rejects.toThrow('Emergency exemptions not allowed for type mfa_requirement');
        });
    });
    describe('Exemption Revocation', () => {
        ...
        test('should revoke active exemption', () => {
            const revoked = manager.revokeExemption(activeExemptionId, 'revoker1', 'revoker@company.com', AdminRole.SECURITY_ADMIN, 'No longer needed');
        ...
        test('should reject revocation of non-active exemption', () => {
            ...
            manager.revokeExemption(activeExemptionId, 'revoker1', 'revoker@company.com', AdminRole.SECURITY_ADMIN, 'Test');
            // Try to revoke again
            expect(() => manager.revokeExemption(activeExemptionId, 'revoker2', 'revoker2@company.com', AdminRole.SECURITY_ADMIN, 'Second revocation')).toThrow('Cannot revoke exemption with status: revoked');
        });
    });
    describe('Exemption Querying', () => {
        beforeEach(async () => {
            ...
            for (const req of requests) {
                ...
                await manager.requestExemption(fullRequest, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN);
            }
        });
    ...
    describe('Event Emission', () => {
        test('should emit exemption requested event', (done) => {
            ...
            manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.HELP_DESK);
        });
        test('should emit exemption granted event for auto-approved', (done) => {
            ...
            manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN);
        });
        test('should emit exemption used event', (done) => {
            ...
            manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN).then(() => {
                // Use the exemption
                manager.checkExemption(ExemptionType.RATE_LIMITING, ExemptionScope.API_KEY, 'api-used-test', { operation: 'test_operation' });
            });
        });
    });
    describe('Error Handling', () => {
        test('should throw error for non-existent exemption', async () => {
            await expect(manager.approveExemption('non-existent-id', 'approver', 'approver@company.com', AdminRole.SUPER_ADMIN)).rejects.toThrow('Exemption not found: non-existent-id');
        });
        test('should throw error for non-existent policy', async () => {
            ...
            await expect(manager.requestExemption(request, 'admin123', 'admin@company.com', AdminRole.SYSTEM_ADMIN)).rejects.toThrow('No policy found for exemption type invalid_type');
        });
    });
});
