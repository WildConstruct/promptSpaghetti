 ...
 */
import { MFARetryHandler,
  MFAOperation,
  RetryStrategy,
  FailureType }
  CircuitBreakerStateEnum
 from '../services/MFARetryHandler';
describe('MFARetryHandler', () => { let retryHandler: MFARetryHandler;
  beforeEach(() => {
  jest.useFakeTimers();
  ...
  describe('Basic Operation Execution', () => { test('should execute successful operation without retries', async () => {
    ...
    test('should handle operation failure with retries', async () => { const mockOperation = jest;
        .fn()
    ...
    test('should fail after max retry attempts', async () => { const mockOperation = jest.fn<unknown, unknown>().mockRejectedValue(new Error('Network error'));
      const resultPromise = retryHandler.executeWithRetry(;);
  ...
  describe('Retry Strategies', () => { test('should use exponential backoff strategy', async () => {
 ...
 as any
    ...
    test('should use linear backoff strategy', async () => { const customHandler = new MFARetryHandler({)
  operationConfigs: {
  [MFAOperation.SMS_SEND]: {
  maxAttempts: 3
 ...
 as any
  ...
  describe('Error Classification', () => { test('should not retry non-retryable errors', async () => {
    ...
    test('should retry retryable errors', async () => { const mockOperation = jest;
        .fn()
        .mockRejectedValueOnce(new Error('Network error'))
        .mockResolvedValue('success' as unknown as unknown as unknown as unknown);
      const resultPromise = retryHandler.executeWithRetry(;);
  ...
  describe('Circuit Breaker', () => { test('should open circuit breaker after failure threshold', async () => {
  const customHandler = new MFARetryHandler({)
  circuitBreaker: {
  failureThreshold: 2
  ...
  halfOpenMaxAttempts: 1 }
    ...
    test('should allow operation when circuit breaker can attempt', () => { expect(retryHandler.canAttemptOperation(MFAOperation.TOTP_VERIFICATION)).toBe(true) });
    test('should reset circuit breaker manually', () => { // This test verifies the reset functionality exists
  ...
  describe('Timeout Handling', () => { test('should timeout long-running operations', async () => {
  const customHandler = new MFARetryHandler({)
  operationConfigs: {
  [MFAOperation.EMAIL_SEND]: {
  maxAttempts: 1
 ...
 as any
  ...
  describe('Metrics Collection', () => { test('should collect and provide metrics', async () => {
      const mockOperation = jest;
        .fn()
        .mockRejectedValueOnce(new Error('Network error'))
        .mockResolvedValue('success' as unknown as unknown as unknown as unknown);
    ...
    test('should track failed operations in metrics', async () => { const mockOperation = jest.fn<unknown, unknown>().mockRejectedValue(new Error('Persistent error'));
  ...
  describe('Active Operations Tracking', () => { test('should track active operations', async () => {
      const mockOperation = jest.fn<unknown, unknown>().mockImplementation(;);
  ...
  describe('Event Emission', () => { test('should emit operation success events', async () => {
  ...
  userId: 'test-user' }
    ...
    test('should emit operation failure events', async () => { const failureHandler = jest.fn<unknown, unknown>();
  ...
  error: expect.any(Error) }
    ...
    test('should emit circuit breaker events', async () => { const circuitBreakerHandler = jest.fn<unknown, unknown>();
  ...
  describe('Configuration', () => { test('should update configuration dynamically', () => {
  ...
  enableMetrics: false }
    ...
    test('should use custom retry configuration', async () => { const customHandler = new MFARetryHandler({)
  operationConfigs: {
  [MFAOperation.TOTP_VERIFICATION]: {
  maxAttempts: 1
  strategy: RetryStrategy.FIXED
 ...
 as any
  ...
  describe('Edge Cases', () => { test('should handle immediate success', async () => {
    ...
    test('should handle operation that throws non-Error objects', async () => { const mockOperation = jest.fn<unknown, unknown>().mockRejectedValue('string error');
      const result = await retryHandler.executeWithRetry(;);
        MFAOperation.EMAIL_SEND }
        mockOperation
      );
    ...
    test('should handle operation with undefined result', async () => { const mockOperation = jest.fn<unknown, unknown>().mockResolvedValue(undefined as unknown as unknown as unknown as unknown);
      const result = await retryHandler.executeWithRetry(;);
