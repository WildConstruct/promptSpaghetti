 ...
 */
import { PasswordGuidanceService, RiskLevel, CompromiseType, ActionPriority, GuidanceCategory } from '../PasswordGuidanceService';
describe('PasswordGuidanceService', () => {
    ...
    describe('Risk Assessment and Classification', () => {
        test('should classify data breach as high risk', () => {
            ...
            const riskLevel = service.calculateRiskLevel(indicators);
    ...
    describe('MFA Recommendations', () => {
        test('should generate comprehensive MFA recommendation', () => {
            const mfaRec = service.getMFARecommendation();
    ...
    describe('Password Manager Recommendations', () => {
        test('should generate password manager setup recommendation', () => {
            const pwdMgrRec = service.getPasswordManagerRecommendation();
    ...
    describe('User Security Dashboard', () => {
        ...
        test('should provide personalized security tips based on risk score', () => {
            const lowRiskTips = service.getSecurityTips(30);
            const highRiskTips = service.getSecurityTips(80);
            ...
            expect(highRiskTips.some((tip) => tip.includes('elevated'))).toBe(true);
        });
        test('should include MFA recommendation for users without MFA', () => {
            ...
            const recommendations = service.getPersonalizedRecommendations(userProfile);
            const mfaRec = recommendations.find((r) => r.id === 'enable-mfa');
    ...
    describe('Risk Score Calculation', () => {
        test('should calculate user risk score based on profile factors', () => {
            ...
            const riskScore = service.calculateUserRiskScore(highRiskProfile);
        ...
        test('should calculate lower risk for secure profile', () => {
            ...
            const riskScore = service.calculateUserRiskScore(lowRiskProfile);
    ...
    describe('Monitoring Recommendations', () => {
        test('should generate basic monitoring for all risk levels', () => {
            const basicMonitoring = service.getMonitoringRecommendations(RiskLevel.LOW);
            expect(basicMonitoring.length).toBeGreaterThan(0);
            const alertsRec = basicMonitoring.find((r) => r.id === 'enable-security-alerts');
            expect(alertsRec).toBeDefined();
        });
        test('should generate enhanced monitoring for high risk', () => {
            const highRiskMonitoring = service.getMonitoringRecommendations(RiskLevel.HIGH);
            const lowRiskMonitoring = service.getMonitoringRecommendations(RiskLevel.LOW);
            expect(highRiskMonitoring.length).toBeGreaterThan(lowRiskMonitoring.length);
            const creditMonitoring = highRiskMonitoring.find((r) => r.id === 'credit-monitoring');
            expect(creditMonitoring).toBeDefined();
        });
    });
    describe('Prevention Recommendations', () => {
        test('should generate security awareness training', () => {
            const preventionRecs = service.getPreventionRecommendations();
            const trainingRec = preventionRecs.find((r) => r.id === 'security-awareness-training');
        ...
        test('should generate regular security checkup recommendation', () => {
            const preventionRecs = service.getPreventionRecommendations();
            const checkupRec = preventionRecs.find((r) => r.id === 'regular-security-checkups');
    ...
    describe('Recommendation Prioritization', () => {
        test('should prioritize immediate actions first', () => {
            ...
            const prioritized = service.prioritizeRecommendations(recommendations, RiskLevel.HIGH);
