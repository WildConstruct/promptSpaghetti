 ...
 */
import { ChallengeTelemetryService, ChallengeType, ChallengeOutcome, DifficultyLevel, UserAgentType } from '../ChallengeTelemetryService';
describe('ChallengeTelemetryService', () => {
    let service;
    let mockDate;
    beforeEach(() => {
    ...
    describe('Challenge Event Recording', () => {
        test('should record challenge event with complete data', () => {
            const eventData = {
        ...
        test('should emit challengeEventRecorded event', (done) => {
            service.on('challengeEventRecorded', (event) => {
    ...
    describe('Challenge Session Management', () => {
        test('should start challenge session', () => {
            ...
            service.recordChallengeCompletion('session-456', 'sms-123', ChallengeOutcome.SUCCESS, 8000, {
        ...
        test('should emit challengeSessionStarted event', (done) => {
            service.on('challengeSessionStarted', (sessionData) => {
        ...
        test('should throw error for invalid session in completion', () => {
            expect(() => {
                service.recordChallengeCompletion('non-existent-session', 'challenge-123', ChallengeOutcome.FAILURE, 5000, { totalInteractionTime: 4000, hesitationTime: 1000 });
    ...
    describe('Challenge Statistics', () => {
        ...
        test('should calculate challenge statistics correctly', () => {
            const startTime = new Date(mockDate.getTime() - 30 * 60 * 1000); // 30 minutes ago
        ...
        test('should provide optimization recommendations', () => {
            const startTime = new Date(mockDate.getTime() - 30 * 60 * 1000);
    ...
    describe('Event Querying', () => {
        ...
        test('should query events by challenge type', () => {
            const query = {
        ...
        test('should query events by outcome', () => {
            const query = {
                startTime: new Date(mockDate.getTime() - 60 * 60 * 1000),
                endTime: new Date(mockDate.getTime() + 60 * 60 * 1000),
        ...
        test('should query events by risk score range', () => {
            const query = {
                startTime: new Date(mockDate.getTime() - 60 * 60 * 1000),
                endTime: new Date(mockDate.getTime() + 60 * 60 * 1000),
                minRiskScore: 50,
        ...
        test('should query events by country', () => {
            const query = {
                startTime: new Date(mockDate.getTime() - 60 * 60 * 1000),
                endTime: new Date(mockDate.getTime() + 60 * 60 * 1000),
                countries: ['US']
        ...
        test('should support pagination in queries', () => {
            const query = {
                startTime: new Date(mockDate.getTime() - 60 * 60 * 1000),
                endTime: new Date(mockDate.getTime() + 60 * 60 * 1000),
                limit: 2,
    ...
    describe('Fraud Pattern Detection', () => {
        ...
        test('should emit fraud detection events', (done) => {
            service.on('fraudDetected', (data) => {
                expect(data.riskScore).toBeGreaterThan(70);
                expect(data.patterns.length).toBeGreaterThan(0);
                done();
            });
            const fraudEvent = {
                sessionId: 'fraud-session-2',
                challengeType: ChallengeType.CAPTCHA_IMAGE,
        ...
        test('should handle legitimate user patterns', () => {
    ...
    describe('A/B Testing', () => {
        test('should create A/B test configuration', () => {
            const testConfig = {
        ...
        test('should emit abTestCreated event', (done) => {
            service.on('abTestCreated', (config) => {
        ...
        test('should reject A/B test with invalid traffic percentages', () => {
            expect(() => {
                service.createABTest({
                    name: 'Invalid Test',
                    challengeType: ChallengeType.CAPTCHA_IMAGE,
        ...
        test('should assign A/B test variants deterministically', () => {
            const testId = service.createABTest({
        ...
        test('should return null for inactive A/B tests', () => {
            service.createABTest({
                name: 'Inactive Test',
        ...
        test('should generate A/B test results', () => {
            const testId = service.createABTest({
                name: 'Results Test',
    ...
    describe('Event Emission and Real-time Updates', () => {
        test('should emit real-time updates on event recording', (done) => {
    ...
    describe('Error Handling', () => {
        test('should throw error for non-existent A/B test', () => {
            expect(() => {
                service.getABTestResults('non-existent-test-id');
            }).toThrow('A/B test not found: non-existent-test-id');
        });
    });
});
