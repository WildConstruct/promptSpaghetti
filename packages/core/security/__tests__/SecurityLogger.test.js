 ...
 */
import { SecurityLogger, LogLevel, SecurityEventType, ComplianceFramework } from '../SecurityLogger';
import { LockoutReason, LockoutStatus, UnlockMethod } from '../AccountLockoutService';
describe('SecurityLogger', () => {
    ...
    describe('Account Lockout Logging', () => {
        ...
        test('should use appropriate log levels for different lockout reasons', () => {
            ...
            testCases.forEach(({ reason, expectedLevel }) => {
                ...
                const result = logger.queryLogs(query);
                const logEntry = result.logs.find(log => log.details.reason === reason);
    ...
    describe('Account Unlock Logging', () => {
        test('should log account unlock with admin details', () => {
            ...
            const logId = logger.logAccountUnlocked(unlockedLockout, UnlockMethod.ADMIN_OVERRIDE, 'admin123', context);
            expect(logId).toBeTruthy();
            const result = logger.queryLogs({
        ...
        test('should log system unlock without admin details', () => {
            ...
            logger.logAccountUnlocked(unlockedLockout, UnlockMethod.AUTOMATIC_EXPIRY);
            const result = logger.queryLogs({
                eventTypes: [SecurityEventType.ACCOUNT_UNLOCKED]
            });
            const logEntry = result.logs[0];
    ...
    describe('Unlock Attempt Logging', () => {
        test('should log successful unlock attempt', () => {
            logger.logUnlockAttempt('lockout-123', 'admin456', 'User verification completed', 'success', { ipAddress: '192.168.1.100' });
            const result = logger.queryLogs({
        ...
        test('should log failed unlock attempt with warning level', () => {
            logger.logUnlockAttempt('lockout-123', 'admin456', 'Insufficient permissions', 'failure');
            const result = logger.queryLogs({
                eventTypes: [SecurityEventType.UNLOCK_ATTEMPT]
            });
            const logEntry = result.logs[0];
    ...
    describe('Emergency Unlock Logging', () => {
        test('should log emergency unlock with critical level', () => {
            logger.logEmergencyUnlock('lockout-critical', 'emergency-admin', 'EMERGENCY-CODE-123456', 'Critical business impact - user locked out of essential systems');
            const result = logger.queryLogs({
        ...
        test('should include multiple compliance frameworks for emergency unlock', () => {
            logger.logEmergencyUnlock('lockout-emergency', 'super-admin', 'EMERGENCY-ABC123', 'System outage requiring immediate access');
            const result = logger.queryLogs({
                eventTypes: [SecurityEventType.EMERGENCY_UNLOCK]
            });
            const logEntry = result.logs[0];
    ...
    describe('Security Alert Logging', () => {
        test('should log security alerts with appropriate severity', () => {
            ...
            logger.logSecurityAlert('Multiple Failed Login Attempts Detected', 'high', details, { ipAddress: '203.0.113.1' });
            const result = logger.queryLogs({
        ...
        test('should map severity levels to log levels correctly', () => {
            const severityMappings = [
                { severity: 'low', expectedLevel: LogLevel.INFO },
                { severity: 'medium', expectedLevel: LogLevel.WARN },
                { severity: 'high', expectedLevel: LogLevel.ERROR },
                { severity: 'critical', expectedLevel: LogLevel.CRITICAL }
            ];
            severityMappings.forEach(({ severity, expectedLevel }, index) => {
                logger.logSecurityAlert(`Test Alert ${index}`, severity, { testIndex: index });
            });
            const result = logger.queryLogs({
                eventTypes: [SecurityEventType.SECURITY_ALERT]
            });
            severityMappings.forEach(({ expectedLevel }, index) => {
                const logEntry = result.logs.find(log => log.details.testIndex === index);
                expect(logEntry?.level).toBe(expectedLevel);
            });
        });
    });
    describe('Audit Trail Creation', () => {
        test('should create audit trail entry', () => {
            const auditId = logger.createAuditTrail('UPDATE', 'lockout', 'lockout-123', 'admin', 'admin789', {
                before: { status: 'active' },
                after: { status: 'unlocked' },
                fields: ['status']
            }, 'User verification completed', { ipAddress: '10.0.0.5' });
        ...
        test('should emit audit trail created event', (done) => {
            ...
            logger.createAuditTrail('DELETE', 'lockout', 'old-lockout-456', 'system', 'cleanup-service', {
    ...
    describe('Compliance Export', () => {
        ...
        test('should export logs for SOX compliance in JSON format', () => {
            ...
            const exportData = logger.exportLogsForCompliance(ComplianceFramework.SOX, startTime, endTime, 'json');
        ...
        test('should export logs in CSV format', () => {
            const startTime = new Date(mockDate.getTime() - 24 * 60 * 60 * 1000);
            const endTime = new Date(mockDate.getTime() + 24 * 60 * 60 * 1000);
            const exportData = logger.exportLogsForCompliance(ComplianceFramework.GDPR, startTime, endTime, 'csv');
        ...
        test('should export logs in XML format', () => {
            const startTime = new Date(mockDate.getTime() - 24 * 60 * 60 * 1000);
            const endTime = new Date(mockDate.getTime() + 24 * 60 * 60 * 1000);
            const exportData = logger.exportLogsForCompliance(ComplianceFramework.ISO_27001, startTime, endTime, 'xml');
    ...
    describe('Data Protection and Security', () => {
        test('should mask sensitive data appropriately', () => {
            logger.logEmergencyUnlock('lockout-test', 'admin-test', 'EMERGENCY123456789', 'Test emergency unlock');
            const result = logger.queryLogs({
                eventTypes: [SecurityEventType.EMERGENCY_UNLOCK]
            });
            const logEntry = result.logs[0];
