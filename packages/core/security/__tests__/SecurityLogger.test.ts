...
describe('SecurityLogger', () => { let logger: SecurityLogger;
  ...
  return mockDate });
    ...
    mockLockout = { id: 'LOCK-1234567890-ABCD1234'
  userId: 'user123'
  ...
  describe('Account Lockout Logging', () => { test('should log account lockout with proper structure', () => {
  const context = {
  ipAddress: '192.168.1.1'
  userAgent: 'Mozilla/5.0'
  requestId: 'req-123' }
  ...
  eventTypes: [SecurityEventType.ACCOUNT_LOCKED] }
    ...
    test('should include compliance frameworks based on lockout reason', () => { const suspiciousLockout = {
  ...mockLockout
  ...
  describe('Account Unlock Logging', () => { test('should log account unlock with admin details', () => {
  ...
  eventTypes: [SecurityEventType.ACCOUNT_UNLOCKED] }
    ...
    test('should log system unlock without admin details', () => { const unlockedLockout = {
  ...mockLockout
  status: LockoutStatus.UNLOCKED
  ...
  describe('Unlock Attempt Logging', () => { test('should log successful unlock attempt', () => {
  ...
  eventTypes: [SecurityEventType.UNLOCK_ATTEMPT] }
    ...
    test('should log failed unlock attempt with warning level', () => { logger.logUnlockAttempt()
  'lockout-123'
  'admin456'
  ...
  describe('Emergency Unlock Logging', () => { test('should log emergency unlock with critical level', () => {
  ...
  eventTypes: [SecurityEventType.EMERGENCY_UNLOCK] }
    ...
    test('should include multiple compliance frameworks for emergency unlock', () => { logger.logEmergencyUnlock()
  'lockout-emergency'
  'super-admin'
  'EMERGENCY-ABC123'
  ...
  describe('Security Alert Logging', () => { test('should log security alerts with appropriate severity', () => {
  const details = {
  ...
  eventTypes: [SecurityEventType.SECURITY_ALERT] }
      ...
      severityMappings.forEach(({ expectedLevel }, index) => { const logEntry = result.logs.find(log => ;);
          log.details.testIndex === index
        );
        expect(logEntry?.level).toBe(expectedLevel) });
    });
  });
  describe('Audit Trail Creation', () => { test('should create audit trail entry', () => {
    ...
    test('should emit audit trail created event', (done) => { logger.on('auditTrailCreated', (auditEntry) => {
  ...
  describe('Log Querying', () => {
    ...
    test('should query logs by event type', () => { const result = logger.queryLogs({)
  eventTypes: [SecurityEventType.ACCOUNT_LOCKED] }
});
      expect(result.logs).toHaveLength(1);
    ...
    test('should query logs by date range', () => { const startTime = new Date(mockDate.getTime() - 60 * 1000); // 1 minute before;
  ...
  expect(log.timestamp.getTime()).toBeGreaterThanOrEqual(startTime.getTime());
    ...
    test('should query logs by severity', () => { const result = logger.queryLogs({)
  severity: ['medium'] }
});
      expect(result.logs.length).toBeGreaterThan(0);
      result.logs.forEach(log => { )
  expect(log.severity).toBe('medium') });
    ...
    test('should query logs by actor', () => { const result = logger.queryLogs({)
  actors: ['admin123'] }
});
      expect(result.logs).toHaveLength(1);
      expect(result.logs[0].actor.id).toBe('admin123');
    });
    test('should support pagination', () => { const result = logger.queryLogs({)
  limit: 2
  offset: 0 }
});
    ...
    test('should support search functionality', () => { const result = logger.queryLogs({)
  search: 'admin123' }
});
      expect(result.logs.length).toBeGreaterThan(0);
      result.logs.forEach(log => {)
  const searchableText = `${log.message} ${log.actor.id} ${log.target?.id || ''}`.toLowerCase();}
    ...
    test('should sort logs correctly', () => { const resultDesc = logger.queryLogs({)
  sortBy: 'timestamp'
  sortOrder: 'desc' }
});
      for (let i = 1; i < resultDesc.logs.length; i++) { expect(resultDesc.logs[i-1].timestamp.getTime())
  .toBeGreaterThanOrEqual(resultDesc.logs[i].timestamp.getTime());
  const resultAsc = logger.queryLogs({)
  sortBy: 'timestamp',
  sortOrder: 'asc' }
});
      for (let i = 1; i < resultAsc.logs.length; i++) { expect(resultAsc.logs[i-1].timestamp.getTime())
  ...
  describe('Security Metrics', () => {
    ...
    test('should generate comprehensive security metrics', () => { const metrics = logger.getSecurityMetrics();
    ...
    test('should calculate lockout metrics by reason', () => { const metrics = logger.getSecurityMetrics();
    ...
    test('should calculate unlock metrics by method', () => { const metrics = logger.getSecurityMetrics();
      expect(metrics.unlockEvents.byMethod[UnlockMethod.ADMIN_OVERRIDE]).toBe(1);
      expect(metrics.unlockEvents.adminUnlocks).toBe(1) });
  });
  describe('Compliance Export', () => { beforeEach(() => {
      logger.logAccountLocked(mockLockout);
      logger.logAccountUnlocked(mockLockout, UnlockMethod.ADMIN_OVERRIDE, 'admin123') });
    test('should export logs for SOX compliance in JSON format', () => { const startTime = new Date(mockDate.getTime() - 24 * 60 * 60 * 1000);
    ...
    test('should export logs in CSV format', () => { const startTime = new Date(mockDate.getTime() - 24 * 60 * 60 * 1000);
      const endTime = new Date(mockDate.getTime() + 24 * 60 * 60 * 1000);
      const exportData = logger.exportLogsForCompliance(;);
    ...
    test('should export logs in XML format', () => { const startTime = new Date(mockDate.getTime() - 24 * 60 * 60 * 1000);
      const endTime = new Date(mockDate.getTime() + 24 * 60 * 60 * 1000);
      const exportData = logger.exportLogsForCompliance(;);
  ...
  describe('Event Emission', () => { test('should emit security log created events', (done) => {
    ...
    test('should emit critical security events', (done) => { logger.on('criticalSecurityEvent', (logEntry) => {
        expect(logEntry.severity).toBe('critical');
        expect(logEntry.level).toBe(LogLevel.CRITICAL);
        done() });
  ...
  describe('Data Protection and Security', () => { test('should mask sensitive data appropriately', () => {
  logger.logEmergencyUnlock()
  'lockout-test'
  'admin-test'
  'EMERGENCY123456789'
  ...
  eventTypes: [SecurityEventType.EMERGENCY_UNLOCK] }
    ...
    test('should generate checksums for log integrity', () => { logger.logAccountLocked(mockLockout);
  const result = logger.queryLogs({)
  eventTypes: [SecurityEventType.ACCOUNT_LOCKED] }
});
      const logEntry = result.logs[0];
