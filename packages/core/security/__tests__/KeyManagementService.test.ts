...
describe('KeyManagementService', () => { let service: KeyManagementService;
  ...
  afterEach(() => { jest.useRealTimers();
    ...
    service.destroy() });
  describe('Key Generation', () => { test('should generate symmetric encryption key', async () => {
  ...
  keySize: 256 }
    ...
    test('should generate RSA key pair', async () => { const options: KeyGenerationOptions = {,
  ...
  keySize: 2048 }
    ...
    test('should generate HMAC key', async () => { const options: KeyGenerationOptions = {,
  type: KeyType.HMAC,
  ...
  keySize: 256 }
    ...
    test('should generate derivation key with parameters', async () => { const options: KeyGenerationOptions = {,
  ...
  keyLength: 32 }
    ...
    test('should emit keyGenerated event', async () => { const eventHandler = jest.fn<unknown, unknown>();
 ...
 as KeyGenerationOptions;
      await expect(service.generateKey(invalidOptions))
        .rejects.toThrow('Key type, purpose, and algorithm are required');
    });
    test('should handle key generation errors', async () => { const errorHandler = jest.fn<unknown, unknown>();
  ...
  describe('Key Retrieval', () => { test('should retrieve generated key', async () => {
  const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  tier: StorageTier.HOT }
    ...
    test('should return null for non-existent key', async () => { const key = await service.getKey('non-existent-key', testRequesterId);
      expect(key).toBeNull() });
    test('should update key usage on retrieval', async () => { const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM }
};
    ...
    test('should cache hot-tier keys', async () => { const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.SESSION_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  tier: StorageTier.HOT }
    ...
    test('should handle expired keys', async () => { const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  expirationDays: 1 }
  ...
  describe('Key Rotation', () => { test('should rotate expiring key', async () => {
  const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  ...
  rotationReason: 'scheduled_rotation' }
    ...
    test('should force rotation when requested', async () => { const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  ...
  rotationReason: 'security_concern' }
    ...
    test('should emit keyRotated event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  describe('Key Revocation', () => { test('should revoke key', async () => {
  const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM }
    ...
    test('should emit keyRevoked event', async () => { const eventHandler = jest.fn<unknown, unknown>();
  ...
  requesterId: testRequesterId }
    ...
    test('should handle non-existent key revocation', async () => { await expect(service.revokeKey('non-existent', testRequesterId, 'reason'))
        .rejects.toThrow('Key not found') });
  });
  describe('Key Derivation', () => { test('should derive key using PBKDF2', async () => {
  ...
  keyLength: 32 }
    ...
    test('should derive key using Scrypt', async () => { const parentOptions: KeyGenerationOptions = {
  type: KeyType.DERIVATION
  purpose: KeyPurpose.PASSWORD_HASHING
  ...
  keyLength: 32 }
    ...
    test('should handle parent key not found', async () => { const derivationParams: KeyDerivationParameters = {
  algorithm: KeyAlgorithm.PBKDF2_SHA256
  salt: Buffer.from('test-salt')
  iterations: 100000
  keyLength: 32 }
  ...
  describe('Key Search', () => { test('should search keys by type', async () => {
  ...
  type: KeyType.SYMMETRIC }
    ...
    test('should search keys by purpose', async () => { await service.generateKey({)
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM }
});
  ...
  purpose: KeyPurpose.DATA_ENCRYPTION }
    ...
    test('should search keys by status', async () => { const key = await service.generateKey({)
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM }
});
  ...
  status: KeyStatus.REVOKED }
    ...
    test('should search keys by tags', async () => { await service.generateKey({)
  type: KeyType.SYMMETRIC
        purpose: KeyPurpose.DATA_ENCRYPTION
        algorithm: KeyAlgorithm.AES_256_GCM }
  ...
  tags: { environment: 'production' }
    ...
    test('should search keys by expiration date', async () => { const futureDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days from now;
  await service.generateKey({)
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM
  ...
  describe('Key Export', () => { test('should export key for backup', async () => {
  const options: KeyGenerationOptions = {
  type: KeyType.SYMMETRIC
  purpose: KeyPurpose.DATA_ENCRYPTION
  algorithm: KeyAlgorithm.AES_256_GCM }
  ...
  describe('Performance Metrics', () => { test('should track performance metrics', async () => {
  // Generate some keys
  for (let i = 0; i < 5; i++) {
  await service.generateKey({)
  type: KeyType.SYMMETRIC,
  ...
  describe('Error Handling', () => { test('should handle invalid key algorithms', async () => {
  const options: KeyGenerationOptions = {,
  type: KeyType.SYMMETRIC,
  purpose: KeyPurpose.DATA_ENCRYPTION,
  algorithm: 'invalid-algorithm' as KeyAlgorithm }
    ...
    test('should handle invalid key sizes', async () => { const options: KeyGenerationOptions = {,
  type: KeyType.SYMMETRIC,
  purpose: KeyPurpose.DATA_ENCRYPTION,
  algorithm: KeyAlgorithm.AES_256_GCM,
  keySize: 64 // Too small }
    ...
    test('should handle invalid expiration days', async () => { const options: KeyGenerationOptions = {,
  type: KeyType.SYMMETRIC,
  purpose: KeyPurpose.DATA_ENCRYPTION,
  algorithm: KeyAlgorithm.AES_256_GCM,
  expirationDays: 0 }
  ...
  describe('Audit Trail', () => { test('should log key events', async () => {
  const options: KeyGenerationOptions = {,
  type: KeyType.SYMMETRIC,
  purpose: KeyPurpose.DATA_ENCRYPTION,
  algorithm: KeyAlgorithm.AES_256_GCM }
  ...
  describe('Key Wrapping', () => { test('should wrap and unwrap keys properly', async () => {
  const options: KeyGenerationOptions = {,
  type: KeyType.SYMMETRIC,
  purpose: KeyPurpose.DATA_ENCRYPTION,
  algorithm: KeyAlgorithm.AES_256_GCM,
  ...
  describe('Cleanup', () => { test('should cleanup resources on destroy', () => {
