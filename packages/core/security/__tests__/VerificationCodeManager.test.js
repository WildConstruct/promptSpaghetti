 ...
 */
import { VerificationCodeManager, VerificationCodeType, CodeFormat, CodeStatus, DeliveryChannel, SecurityEvent } from '../VerificationCodeManager';
describe('VerificationCodeManager', () => {
    ...
    describe('Code Revocation', () => {
        ...
        test('should revoke all user codes of specific type', async () => {
            ...
            const results = await Promise.all(requests.map(request => manager.generateCode(request)));
            results.forEach(result => expect(result).not.toBeNull());
            // Revoke all email verification codes
            const revokedCount = await manager.revokeUserCodes(userId, VerificationCodeType.EMAIL_VERIFICATION, 'user_requested');
    ...
    describe('Code Information and Management', () => {
        test('should get code information safely', async () => {
            ...
            expect(codeInfo.hashedCode).toBeUndefined();
            expect(codeInfo.salt).toBeUndefined();
        });
        ...
        test('should clean up expired codes', async () => {
            ...
            quickCleanupManager.performCleanup = function () {
                ...
                for (const [codeId, code] of this.codes) {
                    const shouldCleanup = (code.status === CodeStatus.EXPIRED ||
                        code.status === CodeStatus.USED ||
                        code.status === CodeStatus.REVOKED) && code.createdAt < cleanupThreshold;
                    if (shouldCleanup) {
    ...
    describe('Error Handling and Edge Cases', () => {
        test('should handle code generation errors gracefully', async () => {
            ...
            failingManager.generateCodeByFormat = () => {
