 ...
 */
import { LockoutNotificationService, NotificationChannel, NotificationPriority, NotificationStatus, NotificationType } from '../LockoutNotificationService';
import { LockoutReason, LockoutStatus, AdminRole } from '../AccountLockoutService';
describe('LockoutNotificationService', () => {
    ...
    describe('Admin Notifications', () => {
        test('should send admin notifications to specified roles', async () => {
            ...
            const notificationIds = await service.sendAdminNotification(NotificationType.ADMIN_ACTION_REQUIRED, mockLockout, adminRoles, details);
    ...
    describe('Notification Delivery', () => {
        test('should track delivery status progression', async () => {
            ...
            expect(finalStatus.deliveries.some(d => d.status === NotificationStatus.SENT ||
                d.status === NotificationStatus.DELIVERED)).toBe(true);
        });
        ...
        test('should handle delivery failures', (done) => {
            ...
            service.testNotification('test@example.com', NotificationChannel.EMAIL, 'non-existent-template', {}).catch(() => {
                // Expected to fail
            });
        });
    });
    describe('Test Notifications', () => {
        test('should send test notification', async () => {
            ...
            const notificationId = await service.testNotification('test@example.com', NotificationChannel.EMAIL, 'lockout_en', variables);
            const status = service.getNotificationStatus(notificationId);
    ...
    describe('Error Handling', () => {
        test('should handle missing template gracefully', async () => {
            await expect(service.testNotification('test@example.com', NotificationChannel.EMAIL, 'non-existent-template', {})).rejects.toThrow('Template not found: non-existent-template');
        });
