 ...
 */
import { EmailDeliveryTracker, DeliveryStatus, EmailType, EmailProvider, BounceType, BounceSubType } from '../services/EmailDeliveryTracker';
describe('EmailDeliveryTracker', () => {
    ...
    describe('Email Sending and Basic Tracking', () => {
        ...
        test('should handle email sending failure', async () => {
            ...
            failingTracker.sendEmailViaProvider = jest.fn().mockRejectedValue(new Error('SMTP connection failed'));
            const request = {
    ...
    describe('Status Updates', () => {
        test('should update email status correctly', () => {
            ...
            tracker.deliveryRecords.set(mockEmailId, mockRecord);
        ...
        test('should emit status change events', (done) => {
            ...
            // Create mock record
            tracker.deliveryRecords.set(mockEmailId, {
                id: mockEmailId,
                status: DeliveryStatus.SENT,
                tracking: { opens: [], clicks: [], unsubscribes: [] }
            });
    ...
    describe('Statistics and Analytics', () => {
        ...
        test('should calculate bounce rate correctly', () => {
            ...
            tracker.statistics.totalEmails = 10;
            tracker.statistics.bouncedEmails = 2;
            tracker.recalculateStatistics();
            const stats = tracker.getStatistics();
    ...
    describe('Email Retry Functionality', () => {
        test('should retry failed email delivery', async () => {
            ...
            tracker.deliveryRecords.set(mockEmailId, failedRecord);
        ...
        test('should not retry non-failed emails', async () => {
            ...
            tracker.deliveryRecords.set(mockEmailId, deliveredRecord);
            const retrySuccess = await tracker.retryDelivery(mockEmailId);
            expect(retrySuccess).toBe(false);
        });
    });
