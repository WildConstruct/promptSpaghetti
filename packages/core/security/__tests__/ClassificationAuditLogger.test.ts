 ...
 */
import { ClassificationAuditLogger,
  AuditLoggerConfig,
  AuditEventType,
  AuditQueryFilter,
  ExportFormat,
...
describe('ClassificationAuditLogger', () => { let auditLogger: ClassificationAuditLogger;
  ...
  afterEach(() => { auditLogger.destroy();
    jest.useRealTimers() });
  describe('Classification Logging', () => { test('should log classification events', async () => {
  ...
  encryptionApplied: true }
    ...
    test('should log classification updates', async () => { const actor = createTestActor();
  ...
  describe('Policy Violation Logging', () => { test('should log policy violations', async () => {
  const actor = createTestActor();
  ...
  frameworks: [ComplianceFramework.GDPR] }
    ...
    test('should emit alert for critical violations', async () => { const alertHandler = jest.fn<unknown, unknown>();
  ...
  describe('Access Logging', () => { test('should log access granted events', async () => {
      const actor = createTestActor();
  ...
  target: {
    ...
    test('should log access denied events', async () => { const actor = createTestActor();
      await auditLogger.logDataAccess()
        'data-123'
        false
  ...
  framework: ComplianceFramework.GDPR }
    ...
    test('should filter by event type', async () => { const classificationLogs = await auditLogger.queryLogs({)
  ...
  startDate: new Date(Date.now() - 1800000) // Last 30 minutes }
    ...
    test('should filter by user ID', async () => { const differentActor = {
  ...createTestActor(),
  userId: 'user-999' }
};
      await auditLogger.logClassification()
  ...
  userIds: ['user-999'] }
    ...
    test('should support text search', async () => { const logs = await auditLogger.queryLogs({)
  searchText: 'Violation' }
});
      expect(logs).toHaveLength(1);
      expect(logs[0].eventType).toBe(AuditEventType.POLICY_VIOLATION);
    });
    test('should support pagination', async () => { const page1 = await auditLogger.queryLogs({)
  limit: 3,
  offset: 0 }
});
  ...
  describe('Integrity Verification', () => {
    ...
    test('should detect hash chain tampering', async () => { const actor = createTestActor();
      // Create logs
      await auditLogger.logClassification()
      ...
      if (logs[0]) { logs[0].action.duration = 999; // Modify content
      const verification = await auditLogger.verifyIntegrity();
      expect(verification.valid).toBe(false);
      expect(verification.errors).toHaveLength(1);
      expect(verification.errors[0].error).toBe('Log hash mismatch') });
    test('should verify signatures in high security mode', async () => { const secureConfig: AuditLoggerConfig = {,
  ...testConfig,
  ...
  framework: ComplianceFramework.GDPR }
    ...
    test('should generate compliance report', async () => { const report = await auditLogger.generateComplianceReport(;);
  ...
  describe('Export Functionality', () => { beforeEach(async () => {
  const actor = createTestActor();
  await auditLogger.logClassification()
  createTestDataElement(),
  createTestClassificationResult(),
  ...
  describe('Log Rotation', () => { test.skip('should rotate logs based on size', async () => {
  ...
  size: expect.any(Number) }
    ...
    test('should rotate logs based on time interval', async () => { const rotationHandler = jest.fn<unknown, unknown>();
  ...
  describe('Retention Policies', () => { test.skip('should enforce retention policies', async () => {
  ...
  describe('Performance Modes', () => { test.skip('should batch logs in high performance mode', async () => {
  ...
  describe('Real-time Streaming', () => { test('should stream logs to configured endpoints', async () => {
