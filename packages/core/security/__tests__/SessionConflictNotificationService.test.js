 ...
 */
import { SessionConflictNotificationService, NotificationType, NotificationChannel, NotificationPriority, DeliveryStatus } from '../SessionConflictNotificationService';
import { SessionPriority, ConflictResolution } from '../SessionPriorityManager';
describe('SessionConflictNotificationService', () => {
    ...
    describe('User Preferences Management', () => {
        ...
        test('should return default preferences for unknown user', () => {
            const preferences = service.getUserPreferences('unknown-user');
    ...
    describe('Conflict Notifications', () => {
        ...
        test('should handle notification delivery failures', async () => {
            ...
            const originalSendNotification = failingService.sendNotification;
            failingService.sendNotification = jest.fn().mockResolvedValue(false);
            const conflict = {
                id: 'conflict-fail',
                type: 'device_limit',
        ...
        test('should emit notification error event on failure', (done) => {
            service.on('notificationError', (data) => {
    ...
    describe('Session Eviction Notifications', () => {
        test('should send eviction notification', async () => {
            const result = await service.sendEvictionNotification('session-evict', 'user-evict', 'session limit exceeded', 5);
            expect(result).toBe(true);
        });
        test('should send eviction notification without grace period', async () => {
            const result = await service.sendEvictionNotification('session-evict-no-grace', 'user-evict-no-grace', 'security violation');
            expect(result).toBe(true);
        });
        test('should handle eviction notification errors', async () => {
            // Create service with failing delivery
            const failingService = new SessionConflictNotificationService();
            failingService.sendNotification = jest.fn().mockResolvedValue(false);
            const result = await failingService.sendEvictionNotification('session-fail', 'user-fail', 'test failure');
    ...
    describe('Emergency Override Notifications', () => {
        test('should send emergency override notification', async () => {
            const result = await service.sendEmergencyOverrideNotification('admin-123', 'user-target', ['session-1', 'session-2']);
            expect(result).toBe(true);
        });
        test('should handle emergency notification errors', async () => {
            const failingService = new SessionConflictNotificationService();
            failingService.sendNotification = jest.fn().mockResolvedValue(false);
            const result = await failingService.sendEmergencyOverrideNotification('admin-fail', 'user-fail', ['session-fail']);
            expect(result).toBe(false);
            failingService.destroy();
        });
    });
    describe('Grace Period Warnings', () => {
        test('should send grace period warning', async () => {
            const result = await service.sendGracePeriodWarning('session-grace', 'user-grace', 3);
            expect(result).toBe(true);
        });
        test('should handle grace period warning errors', async () => {
            const failingService = new SessionConflictNotificationService();
            failingService.sendNotification = jest.fn().mockResolvedValue(false);
            const result = await failingService.sendGracePeriodWarning('session-fail', 'user-fail', 2);
            expect(result).toBe(false);
            failingService.destroy();
        });
    });
    describe('User Choice Prompts', () => {
        test('should handle auto-resolution for users with preferences', async () => {
    ...
    describe('Notification Management', () => {
        ...
        test('should mark notification as read', () => {
            ...
            service.notifications.set(notificationId, mockNotification);
            // Mark as read
            service.markAsRead(notificationId, NotificationChannel.EMAIL);
            // Verify it was marked as read
            const notification = service.notifications.get(notificationId);
        ...
        test('should handle notification action clicks', () => {
            const notificationId = 'notification-action';
            ...
            service.on('notificationActionClicked', (data) => {
    ...
    describe('Event Emission', () => {
        ...
        test('should emit notification read event', (done) => {
            service.on('notificationRead', (data) => {
            ...
            service.notifications.set('notification-read', mockNotification);
    ...
    describe('Cleanup and Destruction', () => {
        test('should clean up expired notifications', () => {
            ...
            service.notifications.set('expired-notification', expiredNotification);
            // Trigger cleanup
            service.cleanupExpiredNotifications();
            // Notification should be removed
            const notification = service.notifications.get('expired-notification');
        ...
        test('should destroy service and clean up resources', (done) => {
            service.on('destroyed', () => {
                done();
            });
            service.destroy();
        });
    ...
    describe('Batch Processing', () => {
        ...
        test('should emit batch processed event', (done) => {
            ...
            batchService.addToBatch(mockNotification);
        });
    });
    describe('Error Handling', () => {
        test('should handle notification delivery errors gracefully', async () => {
        ...
        test('should handle invalid notification IDs in actions', () => {
