 ...
 */
import { PasswordResetTokenManager, TokenType, TokenStatus, SecurityLevel, SecurityEvent } from '../PasswordResetTokenManager';
describe('PasswordResetTokenManager', () => {
    ...
    describe('Token Validation', () => {
        test('should validate a valid token successfully', async () => {
            ...
            const result = await manager.generateToken(request);
            expect(result).not.toBeNull();
            const validation = await manager.validateToken(result.token, '192.168.1.1', 'Mozilla/5.0');
        ...
        test('should reject invalid tokens', async () => {
            const validation = await manager.validateToken('invalid-token-12345', '192.168.1.1', 'Mozilla/5.0');
        ...
        test('should detect expired tokens', async () => {
            ...
            const validation = await shortLivedManager.validateToken(result.token, '192.168.1.1', 'Mozilla/5.0');
            expect(validation.valid).toBe(false);
        ...
        test('should calculate risk scores for validation', async () => {
            ...
            const lowRiskValidation = await manager.validateToken(result.token, '192.168.1.1', 'Mozilla/5.0');
            // Different IP - higher risk
            const highRiskValidation = await manager.validateToken(result.token, '192.168.1.100', 'Different Agent');
    ...
    describe('Token Usage', () => {
        test('should use a valid token successfully', async () => {
            ...
            const result = await manager.generateToken(request);
            expect(result).not.toBeNull();
            const usage = await manager.useToken(result.token, '192.168.1.1', 'Mozilla/5.0');
        ...
        test('should prevent reuse of used tokens', async () => {
            ...
            const firstUsage = await manager.useToken(result.token, '192.168.1.1', 'Mozilla/5.0');
            expect(firstUsage.success).toBe(true);
            // Try to use token second time
            const secondUsage = await manager.useToken(result.token, '192.168.1.1', 'Mozilla/5.0');
    ...
    describe('Token Revocation', () => {
        test('should revoke a specific token', async () => {
            ...
            const result = await manager.generateToken(request);
            expect(result).not.toBeNull();
            const success = await manager.revokeToken(result.tokenId, 'manual_revocation', '192.168.1.1', 'Mozilla/5.0');
        ...
        test('should revoke all user tokens', async () => {
            ...
            const tokens = await Promise.all(requests.map(request => manager.generateToken(request)));
            tokens.forEach(token => expect(token).not.toBeNull());
            // Revoke all password reset tokens for user
            const revokedCount = await manager.revokeUserTokens(userId, TokenType.PASSWORD_RESET, 'user_requested');
    ...
    describe('Token Information and Management', () => {
        test('should get token information safely', async () => {
            ...
            expect(tokenInfo.hashedToken).toBeUndefined();
            expect(tokenInfo.salt).toBeUndefined();
        });
    ...
    describe('Cleanup and Destruction', () => {
        ...
        test('should clean up expired tokens', async () => {
            ...
            const originalPerformCleanup = quickCleanupManager.performCleanup;
            quickCleanupManager.performCleanup = function () {
                const now = new Date();
                const expiredThreshold = new Date(now.getTime() - 1); // 1ms ago instead of 24 hours
                let tokensRemoved = 0;
                for (const [tokenId, token] of this.tokens) {
                    const shouldCleanup = (token.status === TokenStatus.EXPIRED ||
                        token.status === TokenStatus.USED ||
                        token.status === TokenStatus.REVOKED) && (token.createdAt < expiredThreshold ||
            ...
            // Trigger cleanup
            quickCleanupManager.performCleanup();
    ...
    describe('Error Handling and Edge Cases', () => {
        test('should handle token generation errors gracefully', async () => {
            ...
            const originalGenerateSecureToken = failingManager.generateSecureToken;
            failingManager.generateSecureToken = () => {
        ...
        test('should handle validation errors gracefully', async () => {
            const validation = await manager.validateToken('', // Empty token
        ...
        test('should handle usage of invalid tokens', async () => {
            const usage = await manager.useToken('completely-invalid-token', '192.168.1.1', 'Mozilla/5.0');
            expect(usage.success).toBe(false);
            expect(usage.reason).toBeDefined();
        });
        test('should handle revocation of non-existent tokens', async () => {
            const success = await manager.revokeToken('non-existent-token', 'test_revocation');
