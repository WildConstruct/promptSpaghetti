 ...
 */
import { DataProtectionEventLogger,
  DataProtectionEvent,
  DataDeletionEvent,
  PrivacyRequestEvent,
...
describe('DataProtectionEventLogger', () => { let logger: DataProtectionEventLogger;
  ...
  describe('Constructor', () => { it('should initialize with default options', () => {
      const defaultLogger = new DataProtectionEventLogger();
      expect(defaultLogger).toBeInstanceOf(DataProtectionEventLogger) });
    it('should initialize with custom options', () => { const retentionPolicies = new Map([[ComplianceFramework.GDPR, 2000]]);
  ...
  describe('logDataProtectionEvent', () => { it('should log a data protection event successfully', async () => {
  ...
  consentId: 'consent-123' }
    ...
    it('should determine correct event levels', async () => { const criticalEvent: DataProtectionEvent = {
  ...
  level: SecurityEventLevel.CRITICAL }
    ...
    it('should handle high-priority events', async () => { const highPriorityEvent: DataProtectionEvent = {
  ...mockDataProtectionEvent
  ...
  level: SecurityEventLevel.HIGH }
    ...
    it('should validate required event fields', async () => { const invalidEvent: Partial<DataProtectionEvent> = {
  eventType: DataProtectionEventType.DATA_SUBJECT_ACCESS
  timestamp: new Date() }
  // Missing required fields
    ...
    it('should validate compliance frameworks', async () => { const invalidEvent: DataProtectionEvent = {
  ...mockDataProtectionEvent
  complianceFrameworks: [] }
    ...
    it('should handle logging failures gracefully', async () => { mockSecurityLogger.log.mockRejectedValue(new Error('Logging failed'));
  ...
  .rejects.toThrow('Data protection event logging failed: Logging failed') });
    it('should skip audit logging when compliance mode is disabled', async () => { const nonComplianceLogger = new DataProtectionEventLogger(mockSecurityLogger, mockAuditLogger, {)
  complianceMode: false }
});
  ...
  describe('logDataDeletionEvent', () => { const mockDeletionEvent: DataDeletionEvent = {
  ...mockDataProtectionEvent
  ...
  failed: 5 }
    ...
    it('should log deletion event with extended metadata', async () => { await logger.logDataDeletionEvent(mockDeletionEvent);
  expect(mockSecurityLogger.log).toHaveBeenCalledWith()
  expect.objectContaining({)
  details: expect.objectContaining({)
  deletionJobId: 'job-123'
  ...
  failed: 5 }
    ...
    it('should trigger alert for failed deletions', async () => { const failedDeletionEvent: DataDeletionEvent = {
  ...mockDeletionEvent
  ...
  describe('logPrivacyRequestEvent', () => { const mockPrivacyRequestEvent: PrivacyRequestEvent = {
  ...mockDataProtectionEvent
  ...
  processingPurposes: ['service_delivery', 'analytics'] }
    ...
    it('should log privacy request with extended metadata', async () => { await logger.logPrivacyRequestEvent(mockPrivacyRequestEvent);
  expect(mockSecurityLogger.log).toHaveBeenCalledWith()
  expect.objectContaining({)
  details: expect.objectContaining({)
  requestType: 'access'
  ...
  processingPurposes: ['service_delivery', 'analytics'] }
    ...
    it('should trigger alert for overdue requests', async () => { const overdueRequestEvent: PrivacyRequestEvent = {
  ...
  action: 'privacy_request_overdue' }
    ...
    it('should trigger alert for requests past deadline', async () => { const pastDeadlineEvent: PrivacyRequestEvent = {
  ...mockPrivacyRequestEvent
  ...
  describe('logPolicyViolationEvent', () => { const mockViolationEvent: PolicyViolationEvent = {
  ...mockDataProtectionEvent
  eventType: DataProtectionEventType.POLICY_VIOLATION_DETECTED
  ...
  notificationDeadline: new Date('2024-01-02T10:00:00Z') }
    ...
    it('should log policy violation with extended metadata', async () => { await logger.logPolicyViolationEvent(mockViolationEvent);
  expect(mockSecurityLogger.log).toHaveBeenCalledWith()
  expect.objectContaining({)
  details: expect.objectContaining({)
  violationType: 'unauthorized_access'
  ...
  requiresNotification: true }
    ...
    it('should trigger immediate alert for critical violations', async () => { const criticalViolationEvent: PolicyViolationEvent = {
  ...mockViolationEvent
  severity: 'critical' }
};
  ...
  action: 'critical_violation_detected' }
    ...
    it('should trigger immediate alert for high-severity violations', async () => { await logger.logPolicyViolationEvent(mockViolationEvent); // severity: 'high' }
  expect(mockSecurityLogger.log).toHaveBeenCalledTimes(2);
});
    it('should not trigger alert for low-severity violations', async () => { const lowSeverityEvent: PolicyViolationEvent = {
  ...mockViolationEvent
  severity: 'low' }
};
  ...
  describe('generateComplianceReport', () => { const mockEvents = [
  {
  ...
  beforeEach(() => { mockAuditLogger.getEventsByDateRange.mockResolvedValue(mockEvents) });
    it('should generate GDPR compliance report', async () => { const startDate = new Date('2024-01-01');
  ...
  framework: ComplianceFramework.GDPR }
        ...
        eventTypes: { [DataProtectionEventType.DATA_SUBJECT_ACCESS]: 1
  [DataProtectionEventType.POLICY_VIOLATION_DETECTED]: 1 }

  dataSubjects: { 'subject-1': 1
  'subject-2': 1 }
  ...
  compliancePercentage: expect.any(Number) }
    ...
    it('should generate CCPA compliance report', async () => { const startDate = new Date('2024-01-01');
      const endDate = new Date('2024-01-31');
      const report = await logger.generateComplianceReport(;);
    ...
    it('should calculate retention compliance metrics', async () => { const oldEvents = [
  {
  ...
  describe('Event Validation', () => {
    ...
    it('should validate event with all required fields', async () => { const minimalEvent: DataProtectionEvent = {
  eventType: DataProtectionEventType.DATA_SUBJECT_ACCESS
  timestamp: new Date()
  correlationId: 'corr-123'
  userId: 'user-456'
  ...
  describe('Error Handling', () => { it('should handle security logger failures', async () => {
  ...
  .rejects.toThrow('Data protection event logging failed: Security logging failed') });
    it('should handle audit logger failures', async () => { mockAuditLogger.logEvent.mockRejectedValue(new Error('Audit logging failed'));
  await expect(logger.logDataProtectionEvent(mockDataProtectionEvent))
  .rejects.toThrow('Data protection event logging failed: Audit logging failed') });
    it('should continue when audit logger is unavailable', async () => { const loggerWithoutAudit = new DataProtectionEventLogger(mockSecurityLogger, undefined);
