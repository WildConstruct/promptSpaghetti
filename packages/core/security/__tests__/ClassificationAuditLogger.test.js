 ...
 */
import { ClassificationAuditLogger, AuditEventType, ExportFormat } from '../ClassificationAuditLogger';
...
describe('ClassificationAuditLogger', () => {
    ...
    describe('Classification Logging', () => {
        test('should log classification events', async () => {
            ...
            const logId = await auditLogger.logClassification(dataElement, result, actor, 25);
        ...
        test('should log classification updates', async () => {
            const actor = createTestActor();
            const logId = await auditLogger.logClassificationUpdate('data-123', ClassificationLevel.INTERNAL, ClassificationLevel.CONFIDENTIAL, 'Reclassified after review', actor);
    ...
    describe('Access Logging', () => {
        test('should log access granted events', async () => {
            ...
            await auditLogger.logDataAccess('data-123', true, 'User has required permissions', actor, classification);
        ...
        test('should log access denied events', async () => {
            const actor = createTestActor();
            await auditLogger.logDataAccess('data-123', false, 'Insufficient permissions', actor);
    ...
    describe('Query Functionality', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await auditLogger.logClassification(createTestDataElement({ id: `data-${i}` }), createTestClassificationResult(), actor, 20 + i);
            }
            // Access events
        ...
        test('should filter by date range', async () => {
            ...
            // Add new log
            await auditLogger.logClassification(createTestDataElement({ id: 'data-new' }), createTestClassificationResult(), createTestActor(), 30);
        ...
        test('should filter by user ID', async () => {
            ...
            await auditLogger.logClassification(createTestDataElement({ id: 'data-different' }), createTestClassificationResult(), differentActor, 25);
    ...
    describe('Integrity Verification', () => {
        test('should maintain hash chain integrity', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await auditLogger.logClassification(createTestDataElement({ id: `data-${i}` }), createTestClassificationResult(), actor, 20);
            }
        ...
        test('should detect hash chain tampering', async () => {
            const actor = createTestActor();
            // Create logs
            await auditLogger.logClassification(createTestDataElement(), createTestClassificationResult(), actor, 20);
        ...
        test('should verify signatures in high security mode', async () => {
            ...
            const actor = createTestActor();
            await secureLogger.logClassification(createTestDataElement(), createTestClassificationResult(), actor, 20);
    ...
    describe('Compliance Reporting', () => {
        beforeEach(async () => {
            ...
            for (let i = 0; i < 10; i++) {
                await auditLogger.logClassification(createTestDataElement({ id: `data-${i}` }), createTestClassificationResult({
                    complianceRequirements: [ComplianceFramework.GDPR]
                }), actor, 20);
            }
            // Add violations
        ...
        test('should generate compliance report', async () => {
            const report = await auditLogger.generateComplianceReport(ComplianceFramework.GDPR, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), // 30 days ago
            new Date());
            ...
            expect(report.recommendations).toContain('Ensure data minimization principles are followed');
        });
        test('should include recommendations based on violations', async () => {
            ...
            const report = await auditLogger.generateComplianceReport(ComplianceFramework.GDPR, new Date(Date.now() - 1000), new Date());
            expect(report.recommendations).toContain('Review and update classification rules to reduce false positives');
        });
    });
    describe('Export Functionality', () => {
        beforeEach(async () => {
            const actor = createTestActor();
            await auditLogger.logClassification(createTestDataElement(), createTestClassificationResult(), actor, 25);
            await auditLogger.logPolicyViolation({
                dataId: 'data-123',
                policyId: 'policy-456',
        ...
        test('should export logs as CSV', async () => {
            ...
            expect(lines[0]).toBe('id,timestamp,eventType,userId,ipAddress,resourceType,resourceId,operation,result,classificationLevel,framework,hash');
    ...
    describe('Log Rotation', () => {
        test('should rotate logs based on size', async () => {
            ...
            for (let i = 0; i < 100; i++) {
                await auditLogger.logClassification(createTestDataElement({
                    id: `data-${i}`,
                    value: largeData
                }), createTestClassificationResult(), actor, 20);
                // Check if rotation occurred
                if (rotationHandler.mock.calls.length > 0)
                    break;
            }
            expect(rotationHandler).toHaveBeenCalled();
            expect(rotationHandler).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Retention Policies', () => {
        test('should enforce retention policies', async () => {
            const archiveHandler = jest.fn();
            auditLogger.on('logsArchived', archiveHandler);
            const actor = createTestActor();
            // Create old logs
            await auditLogger.logClassification(createTestDataElement(), createTestClassificationResult({
                complianceRequirements: [ComplianceFramework.GDPR]
            }), actor, 20);
            ...
            expect(archiveHandler).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Performance Modes', () => {
        test('should batch logs in high performance mode', async () => {
            ...
            // Log multiple events
            for (let i = 0; i < 5; i++) {
                await performanceLogger.logClassification(createTestDataElement({ id: `data-${i}` }), createTestClassificationResult(), actor, 20);
            }
    ...
    describe('Real-time Streaming', () => {
        test('should stream logs to configured endpoints', async () => {
            ...
            const actor = createTestActor();
            await streamLogger.logClassification(createTestDataElement(), createTestClassificationResult(), actor, 20);
            expect(streamHandler).toHaveBeenCalledWith(expect.objectContaining({
