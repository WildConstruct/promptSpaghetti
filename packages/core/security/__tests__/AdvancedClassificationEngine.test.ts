...
describe('AdvancedClassificationEngine', () => {
  let engine: AdvancedClassificationEngine;
  ...
  const lowRiskData: DataElement = {,
  ...testDataElement,
  value: 'public information',
  fieldName: 'description',
};
      const highRiskData: DataElement = {
  ...testDataElement,
  ...
  const sensitiveData: DataElement = {,
  ...testDataElement,
  ...
  actions: 1,
    ...
    test('should execute workflows based on triggers', async () => {
      const notificationHandler = jest.fn<unknown, unknown>();
