 ...
 */
const HTTP_METHOD_TO_OPERATION: Record<string, OperationContext['operation']> = { GET: 'read'
  POST: 'write'
  PUT: 'update'
  PATCH: 'update'
  DELETE: 'delete' }
...
: (req: ClassificationAwareRequest, res: Response, next: NextFunction) => Promise<void> {
  ...
  return async (req: ClassificationAwareRequest, res: Response, next: NextFunction) => { try {
      ...
      if (!classificationLevel) { // No classification detected, proceed without enforcement
        return next() }
      ...
      if (!enforcementResult.allowed) { return handleEnforcementDenial(req, res, enforcementResult, finalConfig) }
      ...
      next();
 catch (error) { handleMiddlewareError(error as Error, req, res, finalConfig) }
  };
...
export function createAccessControlMiddleware(operation: OperationContext['operation']): (req: ClassificationAwareRequest, res: Response, next: NextFunction) => Promise<void> {
  ...
  requestId: req.headers['x-request-id'] as string || generateRequestId() }
      ...
      if (req.classification) { req.classification.accessDecision = decision } else { req.classification = {
  level: classification
  accessDecision: decision }
};

      // Handle access decision
      if (!decision.granted) { return res.status(403).json({
  error: 'Access denied'
  ...
  requiredAuthorization: decision.requiredAuthorization }
      ...
      if (decision.conditions && decision.conditions.length > 0) { res.setHeader('X-Access-Conditions', decision.conditions.join('; ')) }
      if (decision.expiresAt) { res.setHeader('X-Access-Expires', decision.expiresAt.toISOString()) }
      next();
 catch (error) {
...
export function createOperationValidationMiddleware(): 
  (req: ClassificationAwareRequest, res: Response, next: NextFunction) => Promise<void> { const enforcer = new ClassificationEnforcer();
  return async (req: ClassificationAwareRequest, res: Response, next: NextFunction) => { }
  try {
  ...
  requiredControls: validation.controls }
      ...
      if (validation.controls.length > 0) { res.setHeader('X-Required-Controls', validation.controls.join(',')) }
      next();
 catch (error) {
...
async function extractClassification(
    ...
    config: ClassificationEnforcementMiddlewareConfig): Promise<DataClassificationLevel | null> { // Use custom extractor if provided
  ...
  if (headerClassification && isValidClassification(headerClassification)) { return headerClassification as DataClassificationLevel }
  // Check route parameters
  if (req.params.classification && isValidClassification(req.params.classification)) { return req.params.classification as DataClassificationLevel }
  // Auto-detect from request data
  if (req.body) { const detectedLevel = await detectClassificationFromData(req.body);
    if (detectedLevel) {
...
async function extractClassificationFromPath(req: Request): Promise<DataClassificationLevel | null> {
  ...
  for (const { pattern, level } of patterns) { if (pattern.test(req.path)) {
      return level }
 ...
 */
function buildOperationContext(req: ClassificationAwareRequest): OperationContext { const operation = HTTP_METHOD_TO_OPERATION[req.method] || 'read';
  // Handle special cases
  let finalOperation = operation;
  if (req.path.includes('/export')) {
    finalOperation = 'export' } else if (req.path.includes('/share')) { finalOperation = 'share' }
  return { operation: finalOperation,
...
function extractCurrentControls(
    req: ClassificationAwareRequest,
    config: ClassificationEnforcementMiddlewareConfig): string { // Use custom extractor if provided
  ...
  if (req.headers['x-mfa-verified'] === 'true') { controls.push('auth-mfa') }
  // Authorization
  if (req.user?.roles && req.user.roles.length > 0) { controls.push('authorization') }
  // Encryption
  if (req.secure || req.headers['x-forwarded-proto'] === 'https') { controls.push('encryption-in-transit') }
  ...
  if (headerControls) { controls.push(...headerControls.split(',').map(c => c.trim())) }
  return [...new Set(controls)]; // Remove duplicates

/**
 * * Handle enforcement denial
 */
function handleEnforcementDenial(
  req: ClassificationAwareRequest,
  res: Response,
  result: EnforcementResult,
  config: ClassificationEnforcementMiddlewareConfig
): void { const statusCode = result.riskScore > 80 ? 403 : 401;
  const response: any = {,
  error: 'Classification policy violation',
  classification: result.classification,
  riskScore: result.riskScore }
};
  if (config.detailedErrors || config.environment === 'development') { response.violations = result.violations;
...
function setupResponseMonitoring(
  ...
  res: Response,
  result: EnforcementResult
): void { const originalSend = res.send;
...
function handleMiddlewareError(
  ...
  res: Response,
  config: ClassificationEnforcementMiddlewareConfig
): void { console.error('Classification enforcement middleware error:', error);
  if (config.errorHandler) {
  config.errorHandler(error, req, res) } else { res.status(500).json({
 ...
 */
function isValidClassification(value: string): boolean { return ['PUBLIC', 'INTERNAL', 'CONFIDENTIAL', 'RESTRICTED'].includes(value) }
/**
 * Detect classification from data
 */
async function detectClassificationFromData(data: any): Promise<DataClassificationLevel | null> { if (!data || typeof data !== 'object') {
    return null }
  // Check for explicit classification field
  if (data.classification && isValidClassification(data.classification)) { return data.classification as DataClassificationLevel }
  ...
  try { const element = {
    ...
    if (enhanced.sensitivityLevel) { // Map sensitivity level to classification level
 ...
 catch (error) { console.error('Error detecting classification:', error) }
  return null;
...
export function enforceClassification(
  ...
  options: { allowedOperations?: OperationContext['operation'][];
  ...
  return (req: ClassificationAwareRequest, res: Response, next: NextFunction) => {
    ...
    if (options.allowedOperations) { const operation = HTTP_METHOD_TO_OPERATION[req.method] || 'read';
  ...
  allowedOperations: options.allowedOperations }
    ...
    if (options.requiredControls) { const currentControls = extractCurrentControls(req, DEFAULT_CONFIG);
  ...
  classification
    ...
    if (options.customValidation && !options.customValidation(req)) { return res.status(403).json({
 ...
 */
export { createClassificationEnforcementMiddleware as classificationEnforcement
