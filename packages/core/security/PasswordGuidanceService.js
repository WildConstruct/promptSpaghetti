...
// Compromise Types
export var CompromiseType;
(function (CompromiseType) {
    CompromiseType["DATA_BREACH"] = "data_breach";
    CompromiseType["CREDENTIAL_STUFFING"] = "credential_stuffing";
    CompromiseType["PHISHING"] = "phishing";
    CompromiseType["MALWARE"] = "malware";
    CompromiseType["SOCIAL_ENGINEERING"] = "social_engineering";
    CompromiseType["INSIDER_THREAT"] = "insider_threat";
    CompromiseType["WEAK_PASSWORD"] = "weak_password";
    CompromiseType["REUSED_PASSWORD"] = "reused_password";
})(CompromiseType || (CompromiseType = {}));
// Action Priority Levels
export var ActionPriority;
(function (ActionPriority) {
    ActionPriority["IMMEDIATE"] = "immediate";
    ActionPriority["URGENT"] = "urgent";
    ActionPriority["HIGH"] = "high";
    ActionPriority["MEDIUM"] = "medium";
    ActionPriority["LOW"] = "low"; // When convenient
})(ActionPriority || (ActionPriority = {}));
// Guidance Categories
export var GuidanceCategory;
(function (GuidanceCategory) {
    GuidanceCategory["IMMEDIATE_ACTIONS"] = "immediate_actions";
    GuidanceCategory["ACCOUNT_SECURITY"] = "account_security";
    GuidanceCategory["PASSWORD_MANAGEMENT"] = "password_management";
    GuidanceCategory["MFA_SETUP"] = "mfa_setup";
    GuidanceCategory["MONITORING"] = "monitoring";
    GuidanceCategory["PREVENTION"] = "prevention";
    GuidanceCategory["RECOVERY"] = "recovery";
...
export class PasswordGuidanceService extends EventEmitter {
     ...
     */
    async assessPasswordCompromise(userId, indicators) {
     ...
     */
    getImmediateActions(compromiseType) {
     ...
     */
    async generateRecommendations(indicators, riskLevel) {
     ...
     */
    getPasswordManagerRecommendation() {
        return {
     ...
     */
    getMonitoringRecommendations(riskLevel) {
        const recommendations = [];
     ...
     */
    prioritizeRecommendations(recommendations, riskLevel) {
     ...
     */
    markActionCompleted(sessionId, actionId) {
    ...
    calculateProgress(session) {
        ...
        const remainingCritical = session.recommendations
            .filter(r => (r.priority === ActionPriority.IMMEDIATE || r.priority === ActionPriority.URGENT) &&
            !session.completedActions.includes(r.id)).length;
     ...
     * Update user security profile
     */
    async updateUserSecurityProfile(userId, indicators) {
        ...
        profile.recentBreaches = [
            ...indicators,
            ...profile.recentBreaches.filter(existing => !indicators.some(indicator => indicator.type === existing.type &&
    ...
    calculateUserRiskScore(profile) {
        ...
        const recentBreaches = profile.recentBreaches.filter(breach => (Date.now() - breach.detectedAt.getTime()) < (30 * 24 * 60 * 60 * 1000) // 30 days
        );
        ...
        if (passwordAge > 365)
            riskScore += 20; // 1+ years
        else if (passwordAge > 180)
            riskScore += 10; // 6+ months
        return Math.min(100, riskScore);
    }
    /**
     * Get user security dashboard
     */
    getUserSecurityDashboard(userId) {
    ...
    getSecurityTips(riskScore) {
        ...
        if (riskScore > 50) {
            tips.unshift('Your security risk is elevated - consider changing passwords', 'Review recent account activity for suspicious behavior');
        }
        return tips;
    }
}
