...
export var SessionTerminationReason;
...
export class SecureSessionManager extends EventEmitter {
     ...
     * Terminate session
     */
    async terminateSession(sessionId, reason, terminatedBy) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return false;
        }
    ...
    calculateTrustLevel(userId, context) {
    ...
    recordActivity(session, context, action) {
        const activity = {
            timestamp: new Date(),
            action,
    ...
    updateActivityPattern(userId, deviceId, context) {
    ...
    async enforceConcurrentSessionLimits(userId, config) {
        ...
        if (userSessions.length >= config.maxConcurrentSessions) {
            ...
            await this.terminateSession(oldestSession.id, SessionTerminationReason.MANUAL_LOGOUT);
        }
    }
    ...
    detectAnomalies() {
        ...
        for (const [sessionId, session] of this.sessions) {
            ...
            const recentActivities = session.activities.filter(activity => activity.timestamp >= fiveMinutesAgo);
