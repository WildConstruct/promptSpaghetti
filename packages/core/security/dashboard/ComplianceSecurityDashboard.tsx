...
export enum ComplianceFramework { SOC2_TYPE1 = 'soc2_type1',
...
export const ComplianceSecurityDashboard: React.FC<ComplianceSecurityDashboardProps> = ({ )
  ...
  // Auto-refresh
  useEffect(() => { if (refreshInterval > 0) {
      const interval = setInterval(() => {
        setLastUpdate(new Date()) }, refreshInterval * 60 * 1000);
      return () => clearInterval(interval);
  }, [refreshInterval]);
  // Filter requirements
  const filteredRequirements = useMemo(() => { return requirements.filter(req => {)
  const frameworkMatch = selectedFramework === 'all' || req.framework === selectedFramework;
      ...
      return frameworkMatch && statusMatch }).sort((a, b) => {
      // Sort by risk level, then status
      const riskOrder = { critical: 4, high: 3, medium: 2, low: 1 };
      if (riskOrder[a.riskLevel] !== riskOrder[b.riskLevel]) { return riskOrder[b.riskLevel] - riskOrder[a.riskLevel];
      return a.requirement.localeCompare(b.requirement) });
  }, [requirements, selectedFramework, selectedStatus]);
  // Get status color
  const getStatusColor = useCallback((status: ComplianceStatus) => { switch (status) {
  ...
  const getFrameworkName = useCallback((framework: ComplianceFramework) => { const names = {
  ...
  const handleGenerateReport = useCallback(async (framework: ComplianceFramework, format: string) => {
    ...
    try { await onGenerateReport?.(framework, format) } finally { setReportGenerating(null) }, [onGenerateReport]);
  // Render compliance score gauge
  const renderComplianceGauge = (framework: ComplianceFramework, score: number) => { const radius = 35;
