...
export interface DashboardTemplate { id: string;
  type: DashboardType;
  name: string;
  description: string;
...
export interface DashboardPreset { id: string;
  name: string;
  description: string;
...
export interface RegistryOptions { enableCache: boolean;
...
export class DashboardRegistry {
  ...
  constructor(options: Partial<RegistryOptions> = {}) { this.options = {
  enableCache: true
   ...
   */
  private initializeDefaultDashboards(): void { // Executive Dashboard Template
   ...
   */
  private initializeDefaultPresets(): void { // Executive Standard Preset
    this.registerPreset({)
   ...
   */
  private initializeBuiltInWidgets(): void { // Security Score Gauge Widget
    this.registerWidget({)
  type: 'security-gauge'
   ...
   */
  registerDashboard(template: DashboardTemplate): boolean { if (this.options.validateConfigs) {
  ...
  */
  ...
  return typeMatch && roleMatch && categoryMatch }).sort((a, b) => a.name.localeCompare(b.name));
  /**
  ...
  getDashboardPresets();
    dashboardType?: DashboardType
    layout?: string
  ): DashboardPreset { const presets = Array.from(this.presets.values());
   ...
   */
  getWidgetDefinitions(category?: string, tags?: string): WidgetDefinition { const widgets = Array.from(this.widgets.values());
    return widgets.filter(widget => {)
  const categoryMatch = !category || widget.category === category;
      const tagsMatch = !tags || tags.some(tag => widget.tags.includes(tag));
      return categoryMatch && tagsMatch }).sort((a, b) => a.name.localeCompare(b.name));
  /**
  ...
  ): DashboardConfig | null { const template = this.dashboards.get(templateId);
    if (!template) {
      return null;
 ...
 as DashboardConfig;
  ...
  createDashboardFromPreset();
    presetId: string
    overrides?: Partial<DashboardConfig>
  ): DashboardConfig | null { const preset = this.presets.get(presetId);
    if (!preset) {
      return null;
 ...
 as DashboardConfig;
   ...
   */
  private validateDashboardTemplate(template: DashboardTemplate): boolean { if (!template.id || !template.name || !template.type) {
  return false;
  ...
  */
  ...
  [DashboardType.AUDIT]: 0 }
    ...
    for (const template of this.dashboards.values()) { categoriesByType[template.type]++;
  return {
