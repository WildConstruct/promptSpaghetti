...
export interface ExecutiveMetrics { securityScore: number; // 0-100;
...
export interface ExecutiveInsight { id: string;
...
export interface ExecutiveSecurityDashboardProps { metrics: ExecutiveMetrics;
...
export const ExecutiveSecurityDashboard: React.FC<ExecutiveSecurityDashboardProps> = ({ )
  ...
  const getRiskColor = useCallback((level: string) => { switch (level) {
  ...
  // Get trend icon
  const getTrendIcon = useCallback((trend: string) => { switch (trend) {
  case 'improving':,
  ...
  const renderSecurityScoreGauge = () => { const score = metrics.securityScore;
  ...
  // Render top insights
  const renderTopInsights = () => { const topInsights = insights;
