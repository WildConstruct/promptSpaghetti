...
interface SecurityConfigurationPageProps { userRole: 'admin' | 'security_admin' | 'security_analyst';
  userId: string;
  ...
  */
  ...
  useEffect(() => {
    const loadConfiguration = async () => {
      try {
        ...
        if (config) { setCurrentConfig(config) } else { // Create default configuration for first-time setup
  ...
  if (success) { setCurrentConfig(DEFAULT_SECURITY_CONFIG) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load configuration') } finally { setIsLoading(false) };
    loadConfiguration();
  ...
  tags: ['user-update'] }
  classification: 'INTERNAL');
  if (success) { setCurrentConfig(newConfig);
  setSuccessMessage('Configuration saved successfully!');
  // Clear success message after 3 seconds
  setTimeout(() => setSuccessMessage(null), 3000) } else { throw new Error('Failed to save configuration') } catch (err) { setError(err instanceof Error ? err.message : 'Failed to save configuration') }, [organizationId, userId]);
  // Handle configuration validation
  const handleValidateConfig = useCallback(async (config: SecurityAlertingConfig) => { try {
 ...
 */
export const BasicSecurityConfigExample = () => { return null; });
      score -= 10;
 ...
 */
export const HookBasedExample = () => { return null; });
  const handleConfigChange = useCallback(async (newConfig: SecurityAlertingConfig) => { // Update configuration using the hook's action
    actions.updateConfig(newConfig) }, [actions]);
  const handleValidateConfig = useCallback(async () => { // Use the hook's validation method
