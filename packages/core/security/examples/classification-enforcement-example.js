...
app.get('/api/public/news', enforceClassification('PUBLIC'), (req, res) => {
    res.json({
        articles: [
...
app.get('/api/internal/reports/:id', enforceClassification('INTERNAL', {
...
app.get('/api/confidential/customer/:id', enforceClassification('CONFIDENTIAL'), createAccessControlMiddleware('read'), async (req, res) => {
...
app.post('/api/restricted/financial-data', enforceClassification('RESTRICTED', {
    allowedOperations: ['write'],
    requiredControls: ['auth-strong_mfa', 'encryption', 'approval-workflow']
}), createOperationValidationMiddleware(), async (req, res) => {
...
app.post('/api/data/:id/export', async (req, res) => {
...
async function processDataOperation(userId, dataId, operation, classification) {
    // Make access decision
    const decision = await enforcer.makeAccessDecision(userId, dataId, classification, operation, {
...
async function batchProcessData(userId, dataItems) {
    const results = [];
    for (const item of dataItems) {
        const enforcementResult = await enforcer.enforceClassification(item.classification, {
            operation: 'read',
            userId,
        ...
        if (enforcementResult.allowed) {
            ...
            console.warn(`Access denied for item ${item.id}:`, enforcementResult.violations);
        }
    }
    return results;
}
