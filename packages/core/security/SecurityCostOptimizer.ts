...
export interface CostCenter { id: string;
  name: string;
  description: string;
  department: string;
  // Cost allocation
...
export interface CostAlert { id: string;
  name: string;
  description: string;
...
export interface AutoCostAction { id: string;
  name: string;
  ...
  // Execution tracking
  execution: { last_executed?: number;
...
export interface CostMetrics { id: string;
  ...
  user_costs?: Array<{ user_id: string;
  cost: number;
  requests: number;
  cost_per_request: number }>;
  project_costs?: Array<{ project_id: string;
  cost: number;
  resources: number;
...
export interface CostOptimizationRecommendation { id: string;
  title: string;
  description: string;
...
export interface CostBudget { id: string;
  name: string;
  description: string;
  cost_center_id: string;
  // Budget definition
...
export interface CostReport { id: string;
  title: string;
...
>;
  ...
  budget_analysis?: { budget_utilization: number;
...
export interface CostEvent { id: string;
...
export class SecurityCostOptimizer extends EventEmitter { private costCenters: Map<string, CostCenter> = new Map();
...
> = [];
  ...
  async createCostCenter(costCenter: Omit<CostCenter, 'id' | 'created_at' | 'last_updated'>): Promise<string> {
    ...
    const newCostCenter: CostCenter = { ...costCenter
  id
  created_at: Date.now()
  ...
  async createCostAlert(alert: Omit<CostAlert, 'id' | 'created_at' | 'trigger_count'>): Promise<string> {
    ...
    const newAlert: CostAlert = { ...alert
  id
  created_at: Date.now()
  trigger_count: 0 }
};
  ...
  services: this.generateServiceBreakdown(totalCost)
    ...
    if (costCenter.tracking.track_by_user) { metrics.user_costs = this.generateUserCostBreakdown(totalCost);
  ...
  efficiency_score: metrics.efficiency.efficiency_score }
    ...
    return id;
  private generateRealisticCost(costCenter: CostCenter): number { const baseCost = costCenter.allocation.budget_monthly / (30 * 24); // Hourly base cost;
...
> { const services = [
  ...
  cost: number;
  requests: number;
  cost_per_request: number }> { const users = ['user_001', 'user_002', 'user_003', 'admin_001', 'service_account_001'];
  ...
  cost: number;
  resources: number;
  cost_per_resource: number }> { const projects = ['security-monitoring', 'compliance-reporting', 'threat-hunting', 'incident-response'];
  ...
  private async evaluateAlert(costCenterId: string, alert: CostAlert, metrics: CostMetrics): Promise<void> {
    ...
    switch (alert.type) {
      ...
      case 'budget_variance':
        ...
        break;
    if (shouldTrigger) { await this.triggerCostAlert(costCenterId, alert, metrics, alertData);
  ...
  if (trend.trend_direction === 'increasing') { return trendPercentage > trend.trend_threshold_percentage } else { return trendPercentage < -trend.trend_threshold_percentage;
  ...
  related_events: [] }
    ...
    for (const autoAction of alert.actions.auto_actions) { await this.executeAutoAction(autoAction, costCenterId, event);
  // Send notifications
  ...
  private mapAlertTypeToEventType(alertType: CostAlert['type']): CostEvent['type'] { const mapping: Record<CostAlert['type'], CostEvent['type']> = {
  ...
  private async executeAutoAction(action: AutoCostAction, costCenterId: string, event: CostEvent): Promise<void> {
    ...
    if (action.parameters.max_executions_per_day) {
      ...
      if (todayExecutions >= action.parameters.max_executions_per_day) {
        ...
        return;
    try { // Execute the action based on type
  ...
  private isBusinessHours(): boolean { const now = new Date();
  ...
  monitoring_required: ['Resource utilization', 'Response times', 'Error rates'] }
  ...
  monitoring_required: ['Storage utilization by tier', 'Data retrieval times'] }
  ...
  monitoring_required: ['Reserved instance utilization', 'Cost savings tracking'] }
  ...
  monitoring_required: ['License usage rates', 'Compliance status'] }
  ...
  async generateCostReport(costCenterId: string, reportType: CostReport['report_type'], period: { start: number; end: number }): Promise<string> {
    ...
    if (periodMetrics.length === 0) { throw new Error('No metrics available for the specified period');
  ...
  total_potential_savings: centerRecommendations.reduce((sum, r) => sum + r.cost_impact.estimated_savings_monthly, 0) }
},
  generated_by: 'cost_optimizer',
    ...
    if (budget) { const monthlyActual = totalCost * (period.end - period.start) / (30 * 24 * 60 * 60 * 1000);
  ...
  monthlyActual > budget.budget.amount * 0.9 ? 'at_risk' : 'on_track' }
    ...
    if (costCenter.reporting.automated_reports) { await this.sendCostReport(report, costCenter.reporting.report_recipients);
  ...
  private generateReportInsights(metrics: CostMetrics): string {
    ...
    return insights;
  private generateCostBreakdownAnalysis(metrics: CostMetrics): CostReport['analysis']['cost_breakdown'] { if (metrics.length < 2) return [];
  ...
  private rateEfficiency(utilization: number, cost: number): 'excellent' | 'good' | 'fair' | 'poor' { if (utilization > 80) return 'excellent';
  ...
  for (const channel of alert.notifications.channels) { for (const recipient of alert.notifications.recipients) {
  ...
  await this.sendEscalationNotification(recipient, alert, event) }, alert.notifications.escalation_delay_minutes * 60 * 1000);
...
View Full Report: /cost-optimizer/reports/${report.id}
  ...
  allServices.push({ name: service.service_name, cost: service.cost * 24 * 30 });
    ...
    const servicesSummary = allServices.reduce((acc, service) => { acc[service.name] = (acc[service.name] || 0) + service.cost;
    ...
    const recentEvents = this.events;
      ...
      .slice(0, 10);
    return { cost_centers: activeCostCenters.length,
  ...
  private initializeDefaultCostCenters(): void { const defaultCostCenters = [
  {
  ...
  created_by: 'system'
        active: true];
    defaultCostCenters.forEach(async (costCenter) => { await this.createCostCenter(costCenter) });
  private startCostMonitoring(): void { // Monitor costs every 5 minutes
    this.costMonitoringInterval = setInterval(async () => {
      for (const [costCenterId] of this.costCenters) {
        try {
          await this.collectCostMetrics(costCenterId) } catch (error) {
          console.error(`Failed to collect cost metrics for ${costCenterId}:`, error);}
    }, 300000);
  private startMetricsCollection(): void { // Collect detailed metrics every hour
    this.metricsCollectionInterval = setInterval(() => {
      this.updateSystemMetrics() }, 3600000);
  private startOptimizationEngine(): void { // Run optimization analysis every 6 hours
    this.optimizationInterval = setInterval(async () => {
      await this.runOptimizationAnalysis() }, 6 * 3600000);
  private startReportGeneration(): void { // Generate weekly reports
    this.reportGenerationInterval = setInterval(async () => {
      await this.generateScheduledReports() }, 7 * 24 * 3600000);
  private updateSystemMetrics(): void { // Update system-wide cost optimization metrics
  ...
  private async runOptimizationAnalysis(): Promise<void> { console.log('🔍 Running cost optimization analysis...');
  ...
  private async generateScheduledReports(): Promise<void> { console.log('📊 Generating scheduled cost reports...');
  const now = Date.now();
  ...
  // Public API methods
  getCostCenters(): CostCenter { return Array.from(this.costCenters.values());
