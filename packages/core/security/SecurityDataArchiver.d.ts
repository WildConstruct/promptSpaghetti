...
export interface DataPartitionConfig { id: string;
    ...
    storage: { hot_storage: StorageTier;
...
export interface RetentionPolicy { id: string;
    ...
    compliance_overrides: Array<{ regulation: string;
        min_retention_days: number;
        max_retention_days?: number;
        special_handling: string[] }>;
    exceptions: Array<{ condition: string;
...
export interface ArchivalJob { id: string;
    ...
    schedule: { type: 'manual' | 'scheduled' | 'event_triggered';
        cron_expression?: string;
        trigger_events?: string[];
        dependencies?: string[] };
    processing: { validate_data_integrity: boolean;
        create_checksums: boolean;
        compress_data: boolean;
        encrypt_data: boolean;
        deduplicate: boolean;
        index_data: boolean };
    monitoring: { progress_reporting: boolean;
...
export interface ArchivalExecution { id: string;
    ...
    performance: { throughput_records_per_second: number;
    ...
    errors: Array<{ timestamp: number;
        ...
        retry_count: number;
        resolution: string }>;
    quality_checks: Array<{ check_name: string;
        check_type: 'integrity' | 'completeness' | 'format' | 'compliance';
        result: 'passed' | 'failed' | 'warning';
        details: string;
        timestamp: number }>;
    results: { output_locations: string[];
...
export interface DataRetrievalRequest { id: string;
    ...
    options: { output_format: 'json' | 'csv' | 'parquet' | 'avro' | 'native';
        compression: 'none' | 'gzip' | 'lz4' | 'snappy';
        encryption: boolean;
        include_metadata: boolean;
        max_results: number;
        timeout_minutes: number };
    justification: { business_purpose: string;
        legal_basis?: string;
    ...
    execution: { status: 'pending_approval' | 'approved' | 'queued' | 'processing' | 'completed' | 'failed' | 'expired';
        started_at?: number;
        completed_at?: number;
        estimated_completion?: number;
        progress_percentage: number };
    cost_tracking: { estimated_cost: number;
        actual_cost?: number;
        cost_breakdown: {
...
export interface PartitionMetrics { id: string;
    ...
    storage: { total_partitions: number;
    ...
    quality: { data_integrity_score: number;
...
export interface ArchivalEvent { id: string;
    ...
    response: { acknowledged: boolean;
        ...
        resolved_at?: number };
    follow_up: { monitoring_required: boolean;
        escalation_required: boolean;
        compliance_reporting_required: boolean;
        review_date?: number };

export declare class SecurityDataArchiver extends EventEmitter { private partitionConfigs;
