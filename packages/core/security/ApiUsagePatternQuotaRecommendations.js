...
export var AbuseSeverity;
(function (AbuseSeverity) {
...
export var ImplementationCost;
(function (ImplementationCost) {
    ImplementationCost["LOW"] = "low";
    ImplementationCost["MEDIUM"] = "medium";
    ImplementationCost["HIGH"] = "high";
...
export var AbuseStatus;
(function (AbuseStatus) {
    AbuseStatus["DETECTED"] = "detected";
    AbuseStatus["INVESTIGATING"] = "investigating";
    AbuseStatus["CONFIRMED"] = "confirmed";
    AbuseStatus["FALSE_POSITIVE"] = "false_positive";
    AbuseStatus["RESOLVED"] = "resolved";
    AbuseStatus["ESCALATED"] = "escalated";
...
export class ApiUsagePatternQuotaRecommendations extends EventEmitter {
    ...
    async optimizeQuotaDistribution() {
    ...
    async detectPattern(usageData, patternConfig) {
    ...
    calculatePatternMetrics(usageData) {
    ...
    calculateRecommendedQuota(pattern, currentQuota) {
        ...
        switch (impact.adjustmentType) {
            case QuotaAdjustmentType.INCREASE:
                newQuota.requestsPerSecond = Math.min(currentQuota.requestsPerSecond * multiplier, currentQuota.requestsPerSecond + impact.maxAdjustment);
            ...
            case QuotaAdjustmentType.DECREASE:
                newQuota.requestsPerSecond = Math.max(currentQuota.requestsPerSecond / multiplier, currentQuota.requestsPerSecond - impact.maxAdjustment);
                newQuota.requestsPerMinute = newQuota.requestsPerSecond * 60;
                newQuota.requestsPerHour = newQuota.requestsPerSecond * 3600;
                break;
            case QuotaAdjustmentType.TEMPORARY_BOOST:
                newQuota.burstAllowance = Math.max(currentQuota.burstAllowance, pattern.metrics.peakRequestRate * 1.2);
                break;
        }
    ...
    async applyFairnessAdjustments(recommendations) {
        ...
        for (const recommendation of recommendations) {
            ...
            if (fairnessImpact < -0.1) { // Negative impact on fairness
                ...
                recommendation.recommendedQuota = this.adjustForFairness(recommendation.recommendedQuota, recommendation.currentQuota, fairnessImpact);
    ...
    async detectAbusePattern(usageData, abusePattern) {
        const evidence = [];
    ...
    evaluateAbuseRule(usageData, rule) {
    ...
    async applyAbuseResponseActions(detection) {
    ...
    analyzeTimingPattern(usageData) {
    ...
    calculateSeasonalCorrelation(usageData, period) {
    ...
    calculateSeasonalPhase(usageData, period) {
    ...
    calculateBurstFrequency(usageData) {
    ...
    calculateSteadyStateRatio(usageData) {
        if (usageData.length === 0)
            return 0;
        const requestRates = usageData.map(d => d.requestCount);
        const avgRate = requestRates.reduce((sum, rate) => sum + rate, 0) / requestRates.length;
        const steadyThreshold = avgRate * 0.2; // Within 20% of average
        const steadyCount = requestRates.filter(rate => Math.abs(rate - avgRate) <= steadyThreshold).length;
        return steadyCount / requestRates.length;
    }
    calculatePredictability(usageData) {
    ...
    calculateConsistency(usageData) {
        const requestRates = usageData.map(d => d.requestCount);
        if (requestRates.length < 2)
            return 1;
        const variance = this.calculateVariance(requestRates);
    ...
    detectBurstPattern(usageData, threshold, timeWindow) {
    ...
    detectSteadyState(usageData, threshold) {
        const requestRates = usageData.map(d => d.requestCount);
        const avgRate = requestRates.reduce((sum, rate) => sum + rate, 0) / requestRates.length;
        const steadyCount = requestRates.filter(rate => Math.abs(rate - avgRate) / avgRate < (threshold / 100)).length;
        return (steadyCount / requestRates.length) > 0.8; // 80% of data points are steady
    }
    detectGrowthPattern(usageData, threshold) {
        const growthRate = this.calculateGrowthRate(usageData);
        return growthRate > threshold;
    }
    detectCyclicalPattern(usageData, timeWindow) {
        const seasonality = this.detectSeasonality(usageData);
        return seasonality.hasSeasonality && seasonality.amplitude > 0.6;
    }
    async generateUsageForecast(usageData, characteristics) {
    ...
    calculateRecentTrend(usageData) {
    ...
    getSeasonalAdjustment(timestamp, characteristics) {
    ...
    generatePatternRecommendations(patternConfig, metrics, characteristics) {
        const recommendations = [];
    ...
    getActiveUsers() {
        ...
        for (const [userId, userHistory] of this.usageHistory) {
            const hasRecentActivity = userHistory.some(dataPoint => dataPoint.timestamp.getTime() > activeThreshold);
    ...
    async getCurrentQuota(userId, apiEndpoint) {
    ...
    async analyzeQuotaImpact(pattern, current, recommended) {
    ...
    adjustForFairness(recommended, current, fairnessImpact) {
    ...
    getFairnessMetricWeight(metricId) {
    ...
    identifyInequalityIssues(metrics) {
        const issues = [];
        for (const metric of metrics) {
    ...
    async findRedistributionOpportunities(users) {
        const opportunities = [];
    ...
    async collectAndStoreUsageData() {
    ...
    async determineResponseActions(pattern, confidence, impact) {
        const actions = [];
    ...
    evaluateRateSpikeRule(usageData, rule) {
        const recentData = usageData.slice(-5); // Last 5 data points
        if (recentData.length === 0)
            return { detected: false, strength: 0, description: '', data: {} };
        const avgRate = recentData.reduce((sum, d) => sum + d.requestCount, 0) / recentData.length;
    ...
    evaluateQuotaExhaustionRule(usageData, rule) {
    ...
    evaluateErrorFloodRule(usageData, rule) {
    ...
    async executeAbuseResponseAction(userId, action) {
    ...
    monitorRecommendationImpact(recommendationId) {
    ...
    compareImpacts(expected, actual) {
