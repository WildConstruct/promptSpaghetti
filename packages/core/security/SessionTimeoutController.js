...
export var TimeoutReason;
(function (TimeoutReason) {
    TimeoutReason["IDLE"] = "idle";
    TimeoutReason["ABSOLUTE"] = "absolute";
    TimeoutReason["SECURITY"] = "security";
    TimeoutReason["MANUAL"] = "manual";
    TimeoutReason["POLICY"] = "policy";
    TimeoutReason["EMERGENCY"] = "emergency";
...
export class SessionTimeoutController extends EventEmitter {
     ...
     */
    startCriticalOperation(sessionId, operationType) {
        const state = this.sessionStates.get(sessionId);
        if (!state || !state.isActive) {
            return false;
        }
     ...
     */
    endCriticalOperation(sessionId) {
        const state = this.sessionStates.get(sessionId);
        if (!state) {
            return;
        }
    ...
    scheduleTimeoutWarnings(sessionId) {
        const state = this.sessionStates.get(sessionId);
        if (!state || !state.isActive) {
            return;
        }
    ...
    issueTimeoutWarning(sessionId, threshold, warningIndex) {
        const state = this.sessionStates.get(sessionId);
    ...
    handleSessionTimeout(sessionId, reason) {
        const state = this.sessionStates.get(sessionId);
        if (!state) {
            return;
        }
