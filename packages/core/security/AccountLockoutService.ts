...
export enum LockoutStatus { ACTIVE = 'active',
...
export class AccountLockoutService extends EventEmitter { private lockouts: Map<string, AccountLockout> = new Map();
  ...
  ): Promise<string> { const lockout: AccountLockout = {
  ...
  public async adminUnlock(((
    ...
    if (lockout.status !== LockoutStatus.ACTIVE && lockout.status !== LockoutStatus.PENDING_REVIEW) { return {
        success: false }
    ...
    if (!adminPolicy || !adminPolicy.canUnlock) { return {
  success: false,
  message: 'Insufficient permissions to unlock account' }
};
    // Check if approval is required
    if (adminPolicy.requiresApproval && unlockRequest.urgency !== 'emergency') { return await this.requestUnlockApproval(lockout, unlockRequest);
    // Perform the unlock
  ...
  message: 'Invalid emergency override code' }
    ...
    if (!lockout) {
      throw new Error(`Lockout not found: ${lockoutId}`);}
    const unlockRequest: UnlockRequest = { lockoutId,
      adminId }
  ...
  public async approveUnlock(
    ...
    if (!adminAction.approvalRequired) { return {
  success: false,
  ...
  id: crypto.randomUUID(),
    ...
    if (approved) { // Perform the unlock
  ...
  approvalRequired: false;
      ...
      return await this.performUnlock(lockout, unlockRequest);
 else { lockout.status = LockoutStatus.ACTIVE;
   ...
   */
  public getLockout(lockoutId: string): AccountLockout | null { return this.lockouts.get(lockoutId) || null;
  /**
  ...
  */
  ...
  // Set expiry
  setTimeout(() => { this.emergencyOverrides.delete(code) }, expiryHours * 60 * 60 * 1000);
  ...
  topAffectedUsers: Array<{ userId: string; count: number }>;
    ...
    if (dateRange) { lockouts = lockouts.filter(l => )
  ...
  if (action.action === 'unlock') {
          if (action.metadata.emergencyCode) {
            emergencyUnlocks++ } else { adminUnlocks++ });
    });
    const topAffectedUsers = Object.entries(userCounts);
      .sort(([ a], [ b]) => b - a)
      .slice(0, 10)
      .map(([userId, count]) => ({ userId, count }));
    return { totalLockouts: lockouts.length
  ...
  private async requestUnlockApproval(((
    ...
    unlockRequest: UnlockRequest
  ): Promise<{ success: boolean; message: string; requiresApproval: boolean }> { const adminAction: AdminAction = {,
  id: crypto.randomUUID(),
  ...
  details: unlockRequest }
    ...
    if (policy?.approverRoles) { for (const approverRole of policy.approverRoles) {
        await this.sendNotification(lockout, {)
  type: NotificationType.ADMIN_ACTION_REQUIRED }
  ...
  private async performUnlock(((
    lockout: AccountLockout,
    unlockRequest: UnlockRequest
  ): Promise<{ success: boolean; message: string }> { // Update lockout status
  ...
  private async sendLockoutNotification(lockout: AccountLockout): Promise<void> { const content = this.generateLockoutNotificationContent(lockout);
  await this.sendNotification(lockout, {)
  ...
  ): Promise<void> { const notification: LockoutNotification = {
  id: crypto.randomUUID()
  ...
  private getLockoutReasonText(reason: LockoutReason): string { switch (reason) {
  ...
  private startCleanupTimer(): void { // Clean up expired lockouts every hour
    setInterval(() => {
      const now = new Date();
  ...
  id: crypto.randomUUID(),
        ...
        if (lockout.lockoutTime < oneYearAgo) { this.lockouts.delete(id) }, 60 * 60 * 1000); // Every hour
