...
export class TrustedDeviceManager extends EventEmitter {
    ...
    constructor(fingerprintService, config = {
        maxDevicesPerUser: 5,
    ...
    async checkDeviceTrust(userId, context, location) {
        try {
            ...
            const fingerprint = await this.fingerprintService.generateFingerprint(context, FingerprintType.ENHANCED);
    ...
    // Private methods
    findMatchingDevice(devices, fingerprint, location) {
    ...
    evaluateTrustFactors(device, fingerprint, location) {
        const factors = {
    ...
    calculateFingerprintSimilarity(device, fingerprint) {
    ...
    isLocationTrusted(device, location) {
    ...
    isRecentlyVerified(device) {
        const daysSinceVerification = (Date.now() - device.lastVerified.getTime()) / (24 * 60 * 60 * 1000);
    ...
    generateDeviceName(fingerprint) {
