...
export class PasswordGuidanceService {};
   ...
   */
  public getImmediateActions(compromiseType: CompromiseType): SecurityRecommendation { const immediateActions: SecurityRecommendation = [];
  ...
  'Could indicate need for more extensive security measures'
    ...
    if (compromiseType === CompromiseType.DATA_BREACH || )
        compromiseType === CompromiseType.CREDENTIAL_STUFFING) { immediateActions.push({)
  ...
  */
   ...
   */
  private getPasswordManagerRecommendation(): SecurityRecommendation { return {
   ...
   */
  private getMonitoringRecommendations(riskLevel: RiskLevel): SecurityRecommendation { const recommendations: SecurityRecommendation = [];
  ...
  'False positives may cause unnecessary concern'
    ...
    if (riskLevel === RiskLevel.HIGH || riskLevel === RiskLevel.CRITICAL) { recommendations.push({)
   ...
   */
  private getPreventionRecommendations(): SecurityRecommendation { return [
  {
  ...
  */
  ...
  private prioritizeRecommendations(((
    recommendations: SecurityRecommendation
    riskLevel: RiskLevel
  ): SecurityRecommendation { // Sort by priority, then by category importance
  const priorityOrder = {
  ...
  [ActionPriority.LOW]: 4 }
    ...
    const categoryOrder = { [GuidanceCategory.IMMEDIATE_ACTIONS]: 0
  ...
  [GuidanceCategory.PREVENTION]: 6 }
    ...
    return recommendations.sort((a, b) => { // First sort by priority
   ...
   */
  public markActionCompleted(sessionId: string, actionId: string): boolean { const session = this.sessions.get(sessionId);
  ...
  completed: number;
  percentage: number;
  remainingCritical: number } | null { const session = this.sessions.get(sessionId);
  ...
  private async updateUserSecurityProfile(((
    userId: string
    indicators: CompromiseIndicator
  ): Promise<void> { let profile = this.userProfiles.get(userId);
  if (!profile) {
  profile = {
  userId
  riskScore: 0
   ...
   */
  private calculateUserRiskScore(profile: UserSecurityProfile): number { let riskScore = 0;
  ...
  */
   ...
   */
  private getPersonalizedRecommendations(profile: UserSecurityProfile): SecurityRecommendation { const recommendations: SecurityRecommendation = [];
    if (!profile.mfaEnabled) {
      recommendations.push(this.getMFARecommendation());
