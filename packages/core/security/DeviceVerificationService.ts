...
// Verification steps
export enum VerificationStep { FINGERPRINT_COLLECTION = 'fingerprint_collection',
...
export class DeviceVerificationService extends EventEmitter { private sessions: Map<string, VerificationSession> = new Map();
  ...
  timestamp: new Date() }
      ...
      return session;
 catch (error) { this.emit('verificationError', {)
  userId: request.userId
  ...
  public async submitChallengeResponse(
    ...
    response: string
    metadata: Record<string, any> = {}
  ): Promise<{ success: boolean; session: VerificationSession; nextStep?: VerificationStep }> { const session = this.sessions.get(sessionId);
  if (!session) {
   ...
   */
  public getSession(sessionId: string): VerificationSession | null { return this.sessions.get(sessionId) || null;
  /**
  ...
  */
  ...
  timestamp: new Date() }
    ...
    if (approved) { // Register trusted device
  ...
  timestamp: new Date() }
    ...
    return true;
  // Private methods
  private async progressSession(session: VerificationSession): Promise<void> { session.updatedAt = new Date();
  ...
  if (session.riskScore < this.config.riskThresholds.lowRisk && this.config.enableAutomaticApproval) { session.currentStep = VerificationStep.DEVICE_REGISTRATION;
  await this.completeVerification(session) } else { session.currentStep = VerificationStep.CHALLENGE_REQUIRED;
  ...
  if (requiredCompleted) { if (session.riskScore >= this.config.riskThresholds.requireManualReview) {
  session.currentStep = VerificationStep.MANUAL_REVIEW;
  await this.requestManualReview(session) } else { session.currentStep = VerificationStep.DEVICE_REGISTRATION;
  ...
  metadata: {}
    ...
    switch (type) { case ChallengeType.EMAIL_CODE:
  ...
  ipAddress: session.ipAddress
    ...
    if (!codeResult) { throw new Error('Failed to generate verification code');
  ...
  private async createSMSChallenge(((
    challenge: DeviceChallenge,
    session: VerificationSession
  ): Promise<void> { // Generate verification code
    const codeRequest: CodeGenerationRequest = {
  userId: session.userId
      type: VerificationCodeType.DEVICE_VERIFICATION
  ...
  private async validateChallengeResponse(((
    challenge: DeviceChallenge
    response: string
  ): Promise<boolean> { switch (challenge.type) {
  ...
  private async registerTrustedDevice(session: VerificationSession): Promise<void> { try {
  ...
  verificationTimestamp: new Date() }
      ...
      if (session.outcome === VerificationOutcome.APPROVED && trustedDevice.verificationToken) { await this.trustedDeviceManager.verifyDevice(trustedDevice.verificationToken);
      session.metadata.trustedDeviceId = trustedDevice.id } catch (error) { this.emit('deviceRegistrationError', {)
  sessionId: session.id
  userId: session.userId
  error: error instanceof Error ? error.message : 'Unknown error'
  timestamp: new Date() }
});
  private async requestManualReview(session: VerificationSession): Promise<void> { session.outcome = VerificationOutcome.REQUIRES_REVIEW;
  this.emit('manualReviewRequired', {)
  sessionId: session.id
  userId: session.userId
  riskScore: session.riskScore
  ...
  private async checkForSuspiciousActivity(session: VerificationSession): Promise<void> { // Check for repeated verification attempts
  ...
  if (riskScore >= this.config.riskThresholds.requireManualReview) { return RiskLevel.CRITICAL } else if (riskScore >= this.config.riskThresholds.highRisk) { return RiskLevel.HIGH } else if (riskScore >= this.config.riskThresholds.mediumRisk) { return RiskLevel.MEDIUM } else { return RiskLevel.LOW;
  ...
  private async getLocationFromIP(ipAddress: string): Promise<LocationData> { // Simplified location lookup - would use a real GeoIP service
  return {
  id: 'loc-ip-' + Date.now()
  timestamp: new Date()
  source: 'ip'
  ...
  private startCleanupTimer(): void { // Clean up expired sessions every hour
    setInterval(() => {
      this.performCleanup() }, 60 * 60 * 1000);
