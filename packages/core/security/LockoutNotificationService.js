...
export class LockoutNotificationService extends EventEmitter {
    ...
    async queueNotification(request) {
    ...
    async processNotification(requestId) {
    ...
    async scheduleRetry(delivery) {
        ...
        const delay = Math.min(template.retryPolicy.baseDelaySeconds * Math.pow(template.retryPolicy.backoffMultiplier, delivery.attempts - 1), template.retryPolicy.maxDelaySeconds) * 1000;
    ...
    buildTemplateVariables(lockout, additional = {}) {
        return {
    ...
    buildAdminTemplateVariables(lockout, details) {
