...
export interface MetricsHookReturn { // Data
...
export const useRateLimitingMetrics = ({ )
  ...
  throttlingEngine }
  options = {}
: MetricsServiceConfig): MetricsHookReturn => { // Default options
  const {
    autoRefresh = true
    refreshInterval = 5
    timeRange = '1h'
 ...
 = options;
  ...
  // ========================================
  const initializeMetricsService = useCallback(() => { try {
 ...
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to initialize metrics service';
  setError(errorMessage);
  setIsConnected(false);
  return null }, [rateLimitingService, throttlingEngine, autoRefresh, refreshInterval, enableAlerts, retainHistoryHours]);
  // ========================================
  // Data Loading Functions
  // ========================================
  const loadMetricsData = useCallback(async () => { if (!metricsServiceRef.current || !mountedRef.current) return;
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (mountedRef.current) {
        ...
        setLastUpdate(new Date()) } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to load metrics data';
  if (mountedRef.current) {
  setError(errorMessage) } finally { if (mountedRef.current) {
        setIsLoading(false) }, [selectedTimeRange]);
  const refreshMetrics = useCallback(async () => { await loadMetricsData() }, [loadMetricsData]);
  // ========================================
  // Auto-refresh Management
  // ========================================
  const startMonitoring = useCallback(() => { if (!metricsServiceRef.current) return;
    ...
    if (autoRefresh && refreshInterval > 0) {
      ...
      refreshTimerRef.current = setInterval(() => {
        if (mountedRef.current) {
          loadMetricsData() }, refreshInterval * 1000);
    setIsConnected(true);
  }, [loadMetricsData, autoRefresh, refreshInterval]);
  const stopMonitoring = useCallback(() => { if (refreshTimerRef.current) {
      clearInterval(refreshTimerRef.current);
  ...
  // Event Handlers
  // ========================================
  const setupEventListeners = useCallback((service: RateLimitingPerformanceMetrics) => { const handleMetricsUpdate = () => {
      if (mountedRef.current) {
        loadMetricsData() };
    const handleAlertCreated = (alert: AlertCondition) => { if (mountedRef.current) {
        setActiveAlerts(prev => [...prev, alert]) };
    const handleAlertAcknowledged = (data: { alertId: string }) => { if (mountedRef.current) {
        setActiveAlerts(prev => prev.filter(alert => alert.alertId !== data.alertId)) };
    const handleWidgetAdded = () => { if (mountedRef.current && metricsServiceRef.current) {
        setWidgets(metricsServiceRef.current.getWidgets()) };
    const handleWidgetRemoved = () => { if (mountedRef.current && metricsServiceRef.current) {
        setWidgets(metricsServiceRef.current.getWidgets()) };
    const handleError = (errorData: { error: any }) => { if (mountedRef.current) {
  ...
  setError(errorMessage) };
    ...
    return () => { service.off('metricsUpdated', handleMetricsUpdate);
  ...
  // Widget Management
  // ========================================
  const addWidget = useCallback((widget: DashboardWidget) => { if (metricsServiceRef.current) {
      metricsServiceRef.current.addWidget(widget);
      setWidgets(metricsServiceRef.current.getWidgets()) }, []);
  const removeWidget = useCallback((widgetId: string) => { if (metricsServiceRef.current) {
      metricsServiceRef.current.removeWidget(widgetId);
      setWidgets(metricsServiceRef.current.getWidgets()) }, []);
  // ========================================
  // Alert Management
  // ========================================
  const acknowledgeAlert = useCallback((alertId: string) => { if (metricsServiceRef.current) {
      metricsServiceRef.current.acknowledgeAlert(alertId);
      setActiveAlerts(prev => prev.filter(alert => alert.alertId !== alertId)) }, []);
  // ========================================
  // Data Export
  // ========================================
  const exportMetrics = useCallback((format: 'json' | 'csv'): string => { if (!metricsServiceRef.current) {
      throw new Error('Metrics service not initialized');
    return metricsServiceRef.current.exportMetrics(format) }, []);
  // ========================================
  // Time Range Management
  // ========================================
  const updateTimeRange = useCallback((range: string) => { setSelectedTimeRange(range) }, []);
  // ========================================
  // Error Handling
  // ========================================
  ...
  useEffect(() => { const service = initializeMetricsService();
    if (service) {
  ...
  useEffect(() => { if (isConnected) {
      loadMetricsData() }, [selectedTimeRange, loadMetricsData, isConnected]);
  // Cleanup on unmount
...
export function useRateLimitingMetricsWidget(widgetId: string, metricsHook: unknown) { const [widgetData, setWidgetData] = useState<unknown>(null);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  useEffect(() => {
  ...
  data = null;
  setWidgetData(data);
 catch (error) { console.error('Error loading widget data:', error);
  setWidgetData(null) } finally { setIsLoading(false) };
    loadWidgetData();
