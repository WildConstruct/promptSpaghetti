...
export const usePasswordReset = (options = {}) => {
    ...
    const verifyToken = useCallback(async () => {
        ...
        clearMessages();
        try {
            const validation = await passwordResetTokenManager.validateToken(currentToken.token, '192.168.1.100', navigator.userAgent);
            if (!validation.valid) {
    ...
    const resetPassword = useCallback(async () => {
        ...
        clearMessages();
        try {
            const usage = await passwordResetTokenManager.useToken(currentToken.token, '192.168.1.100', navigator.userAgent);
            if (!usage.success) {
