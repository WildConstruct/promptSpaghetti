...
export class LockoutNotificationService {};
  ...
  ): Promise<string> { const userPrefs = this.getUserPreferences(lockout.userId);
  ...
  ): Promise<string> { const notifications: string = [];
  ...
  pending: number;
    ...
    const summary = { totalDeliveries: deliveries.length
  ...
  public getUserNotificationHistory(((
    userId: string
    limit: number = 50
  ): NotificationRequest { return Array.from(this.notifications.values())
  ...
  */
  public createTemplate(template: Omit<NotificationTemplate, 'id'>): string {
  ...
  public async testNotification(
    ...
    channel: NotificationChannel
    templateId: string
    variables: TemplateVariables): Promise<string> { 
  const request: NotificationRequest = {
  id: crypto.randomUUID()
  ...
  averageDeliveryTime: number;
    ...
    if (dateRange) { notifications = notifications.filter(n => {)
  ...
  byChannel[d.channel] = (byChannel[d.channel] || 0) + 1;
    ...
    const totalDeliveryTime = successfulDeliveries.reduce((sum, d) => { if (d.sentAt && d.deliveredAt) {
  ...
  private async queueNotification(request: NotificationRequest): Promise<string> { request.metadata.createdAt = Date.now();
  ...
  type: request.type }
    ...
    if (request.priority === NotificationPriority.URGENT || request.priority === NotificationPriority.CRITICAL) { setImmediate(() => this.processNotification(request.id));
  return request.id;
  private async processNotification(requestId: string): Promise<void> {
  ...
  delivery.attempts++;
  try { // Simulate delivery based on channel
  ...
  this.emit('notificationDelivered', delivery) }, 1000 + Math.random() * 2000);
 else { delivery.status = NotificationStatus.FAILED;
        delivery.failureReason = 'Simulated delivery failure';
        this.emit('notificationFailed', delivery);
        // Schedule retry if applicable
        await this.scheduleRetry(delivery) } catch (error) { delivery.status = NotificationStatus.FAILED;
  delivery.failureReason = error instanceof Error ? error.message : 'Unknown error';
  this.emit('notificationFailed', delivery);
  await this.scheduleRetry(delivery);
  private async simulateDelivery(delivery: NotificationDelivery): Promise<boolean> {
  // Simulate different success rates based on channel
  const successRates: Record<NotificationChannel, number> = {
  ...
  private async scheduleRetry(delivery: NotificationDelivery): Promise<void> { const template = this.templates.get(this.notifications.get(delivery.requestId)?.templateId || '');
  ...
  variables: TemplateVariables
    ...
    const variableMap = variables as any;
    for (const [key, value] of Object.entries(variableMap)) {
      const placeholder = `{{${key}}}`;}
      const stringValue = String(value);
      if (subject) subject = subject.replace(new RegExp(placeholder, 'g'), stringValue);
  ...
  details: Record<string, any>
  ): TemplateVariables { const base = this.buildTemplateVariables(lockout, details);
  return {
  ...base
  nextSteps: [
  ...
  private getLockoutReasonText(reason: LockoutReason): string { const reasonMap: Record<LockoutReason, string> = {
  ...
  private getNextSteps(reason: LockoutReason): string { const stepMap: Record<LockoutReason, string> = {
  ...
  private getEstimatedResolution(reason: LockoutReason): string { const resolutionMap: Record<LockoutReason, string> = {
  ...
  private calculatePriority(lockout: AccountLockout): NotificationPriority { const priorityMap: Record<LockoutReason, NotificationPriority> = {
  ...
  private getUserPreferences(userId: string): UserNotificationPreferences { return this.preferences.get(userId) || {
  userId
  channels: {
  email: true
  sms: false
  ...
  private getPreferredChannels(preferences: UserNotificationPreferences): NotificationChannel {
  ...
  private selectTemplate(type: NotificationType, language: string): string {
  ...
  private getAdminRecipients(role: AdminRole): string { // In production, this would query an admin directory service
  const adminMap: Record<AdminRole, string> = {
  ...
  private initializeDefaultTemplates(): void { // Lockout notification template
...
{{companyName}} Security Team`,
  ...
  private initializeAdminRules(): void { // High-priority lockouts require immediate admin notification
  ...
  private startDeliveryProcessor(): void { // Process queued notifications every 30 seconds
    setInterval(() => {
      ...
      for (const delivery of queuedDeliveries) {
        this.attemptDelivery(delivery) }, 30000);
