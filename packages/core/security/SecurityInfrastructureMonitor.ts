...
export class SecurityInfrastructureMonitor extends EventEmitter {
  ...
  private events: Map<string, InfrastructureEvent[]> = new Map();
  ...
  private initializeEventHandlers(): void {
    ...
    this.on('alert_generated', (alert: InfrastructureAlert) => {
      this.processAlert(alert);
    });
    this.on('component_status_changed', (componentId: string, oldStatus: string, newStatus: string) => {
      this.handleStatusChange(componentId, oldStatus as any, newStatus as any);
    });
  }
  ...
  private calculateOverallHealth()
    results: Array<{ check: HealthCheckConfig; success: boolean; responseTime: number; error?: string }>
  ...
  private async generateStatusChangeAlert()
    componentId: string, 
  ...
  private getMetricValue(metrics: InfrastructureMetrics, metricName: string): number | undefined {
  ...
  private matchesEscalationPolicy(alert: InfrastructureAlert, policy: EscalationPolicy): boolean {
    // Check severity
  ...
  private isAlertSuppressed(alert: InfrastructureAlert): boolean {
    const component = this.components.get(alert.component_id);
  ...
  getInfrastructureEvents(componentId?: string, hours: number = 24): InfrastructureEvent[] {
    const cutoffTime = Date.now() - hours * 60 * 60 * 1000;
