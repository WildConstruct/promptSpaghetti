...
export class AuditIntegration {};
  ...
  async logDataAccess(
    ...
    if (this.classifier && data && this.config.enrichWithClassification) { const result = await this.classifier.classifyData(data);
  ...
  fieldClassifications: result.fieldClassifications }
    ...
    if (this.enforcer) { enforcementResult = await this.enforcer.enforceClassification(
        classification }
        context
      );
 ...
 : undefined
    ...
    if (!enforcementResult?.allowed && this.config.logDeniedAccess) { await this.logSecurityEvent(
  'ACCESS_DENIED'
  context
  {
  resourceType
  ...
  */
  ...
  classification?: DataClassificationLevel }
    metadata?: Record<string, any>
  ): Promise<void> { await this.logger.log({
  ...
  classification?: DataClassificationLevel }>,
    success: boolean,
    metadata?: Record<string, any>
  ): Promise<void> { const highestClassification = resources.reduce((highest, resource) => {
  ...
  ? level : highest }, DataClassificationLevel.PUBLIC);
    await this.logger.log({ userId: context.userId
      userRole: context.userRole
   ...
   */
  private setupEventListeners(): void { // Listen to enforcement events
  if (this.enforcer) {
  ...
  recommendations: event.recommendations }
      ...
      this.enforcer.on('anomaly', async (event) => { await this.logSecurityEvent('ANOMALY_DETECTED', event.context, {
  ...
  details: event.details }
    ...
    if (this.classifier) { this.classifier.on('classification', async (event) => {
   ...
   */
  private isSensitiveClassification(level: DataClassificationLevel): boolean { return [
   ...
   */
  private getClassificationPriority(level: DataClassificationLevel): number { const priorities: Record<DataClassificationLevel, number> = {
  ...
  async generateComplianceReport(
    startDate: Date
    endDate: Date
    options?: { groupBy?: 'user' | 'classification' | 'operation';
  includeDetails?: boolean }
  ): Promise<ComplianceReport> { const logs = await this.logger.query({
      startDate }
      endDate
    });
  ...
  highRiskOperations: 0 }
    ...
    for (const log of logs) { // Classification breakdown
    ...
    if (logsWithRisk.length > 0) { report.riskMetrics.averageRiskScore /= logsWithRisk.length }

    // Add details if requested
    if (options?.includeDetails) { report.details = logs }
...
export interface ComplianceReport { period: { }
  start: Date;
  end: Date;
...
export function createAuditIntegration(config: AuditIntegrationConfig): AuditIntegration { return new AuditIntegration(config) }