...
export interface QueryPerformanceProfile { id: string;
    ...
    performance_metrics: { execution_times_ms: number[];
    ...
    optimization: { optimization_applied: boolean;
    ...
    caching: { cacheable: boolean;
        cache_strategy: CacheStrategy;
        cache_ttl_seconds: number;
        cache_hit_rate: number;
        cache_size_mb: number;
        invalidation_triggers: string[] };
    usage_patterns: { frequency_per_hour: number;
...
export interface QueryOptimizationRule { id: string;
    ...
    effectiveness: { applications_count: number;
        success_rate: number;
...
export interface QueryExecution { id: string;
    ...
    optimization: { optimizations_applied: string[];
        optimization_time_ms: number;
        execution_plan: ExecutionPlan;
        index_usage: IndexUsage[];
        cache_interactions: CacheInteraction[] };
    quality: { result_accuracy: number;
...
export interface CacheConfiguration { cache_id: string;
    ...
    eviction: { policy: 'lru' | 'lfu' | 'ttl' | 'size_based' | 'intelligent';
        ttl_seconds: number;
        max_idle_time_seconds: number;
        priority_levels: number };
    performance: { concurrent_access_limit: number;
        compression_enabled: boolean;
        encryption_enabled: boolean;
        prefetching_enabled: boolean;
        async_write_back: boolean };
    monitoring: { hit_rate_threshold: number;
...
export interface OptimizationReport { report_id: string;
    ...
    performance_summary: { total_queries_analyzed: number;
    ...
    optimization_opportunities: Array<{ opportunity_type: string;
        estimated_impact: string;
    ...
    recommendations: { high_priority: string[];
        medium_priority: string[];
        low_priority: string[];
        infrastructure_changes: string[];
        configuration_changes: Record<string, any> };
    metrics_comparison: { before_optimization: PerformanceMetrics;
...
export declare class SecurityQueryPerformanceOptimizer extends EventEmitter {
    ...
    getPerformanceHistory(hours?: number): Array<{ timestamp: number;
