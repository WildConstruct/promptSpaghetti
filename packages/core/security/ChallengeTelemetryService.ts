...
// Challenge Types
export enum ChallengeType { CAPTCHA_IMAGE = 'captcha_image',
...
export interface ChallengeEvent { id: string;
  sessionId: string;
  userId?: string;
  ...
  challengeData: { variant?: string; // For A/B testing }
...
export interface ChallengeStatistics { challengeType: ChallengeType;
  period: { }
  start: Date;
  end: Date;
  ...
  timeoutRate: number;
  errorRate: number };
  byDifficulty: Record<DifficultyLevel, { attempts: number;
  successRate: number;
  averageTime: number }>;
  byUserType: Record<UserAgentType, { attempts: number;
  successRate: number;
  fraudScore: number }>;
  fraudDetection: { 
  botAttempts: number;
...
export interface ABTestConfig { id: string;
  name: string;
  challengeType: ChallengeType;
  variants: Array<{ }
  id: string;
...
export interface FraudPattern { id: string;
  name: string;
  description: string;
  conditions: Array<{ }
  field: string;
...
export class ChallengeTelemetryService extends EventEmitter { private events: Map<string, ChallengeEvent> = new Map();
  ...
  */
  ...
  challengeId: string
    ...
    const event: Omit<ChallengeEvent, 'id' | 'timestamp'> = { sessionId
  ...
  startTime: Date
    ...
    if (this.statistics.has(cacheKey)) { return this.statistics.get(cacheKey)!;
   ...
   */
  public queryEvents(query: TelemetryQuery): ChallengeEvent { let events = Array.from(this.events.values());
  // Apply filters
  ...
  */
  ...
  recommendations.push('Increase challenge difficulty') } else if (action.type === 'block') { recommendations.push('Consider blocking this user/IP') } else if (action.type === 'monitor') { recommendations.push('Enable enhanced monitoring') });
   ...
   */
  public createABTest(config: Omit<ABTestConfig, 'id'>): string { const testId = crypto.randomUUID();
  ...
  ...config
    ...
    if (Math.abs(totalTraffic - 100) > 0.1) { throw new Error('Variant traffic percentages must sum to 100%');
  ...
  public getABTestVariant(((
    challengeType: ChallengeType }
    sessionId: string
  ): { testId: string; variantId: string; parameters: Record<string, any> } | null { // Find active A/B test for this challenge type
...
>;
   ...
   */
  public getDashboardData(): { overview: { }
  ...
  if (!challengeTypeCounts[event.challengeType]) {
      ...
      if (event.outcome === ChallengeOutcome.SUCCESS) { challengeTypeCounts[event.challengeType].successful++ });
  ...
  const country = event.context.geolocation?.country || 'Unknown';
      ...
      if (event.outcome === ChallengeOutcome.SUCCESS) { geoCounts[country].successful++ });
  ...
  fraudScore: avgFraudScore }
    ...
    if (successRate > 90) { recommendedDifficulty = DifficultyLevel.HARD } else if (successRate < 60) { recommendedDifficulty = DifficultyLevel.EASY;
    return {
      challengeType }
  ...
  optimization: { recommendedDifficulty,
        performanceScore,
        userExperienceScore }
        securityScore
    };
  private getEventFieldValue(event: ChallengeEvent, field: string): any { const fieldParts = field.split('.');
  let value: any = event;
  ...
  private initializeFraudPatterns(): void { // Bot-like behavior pattern
  ...
  private startMetricsAggregation(): void { // Aggregate metrics every 5 minutes
    setInterval(() => {
      this.aggregateMetrics() }, 5 * 60 * 1000);
  private startFraudDetection(): void { // Run fraud detection every minute
    setInterval(() => {
      this.runFraudDetection() }, 60 * 1000);
  ...
  private runFraudDetection(): void { // Analyze recent events for new fraud patterns
    const now = new Date();
  ...
  const analysis = this.analyzeFraudPattern(event);
      if (analysis.riskScore > 80) {
        highRiskEvents++ });
    if (highRiskEvents > 5) { this.emit('fraudAlert', {)
  level: 'high' }
