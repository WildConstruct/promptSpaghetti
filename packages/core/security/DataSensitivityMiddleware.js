 ...
 */
export function createDataSensitivityMiddleware(config = {}) {
 ...
 * Analyze request for data sensitivity
 */
async function analyzeSensitivity(req, config) {
    const detectedElements = [];
    let highestLevel = DataSensitivityLevel.PUBLIC;
    const complianceRequirements = new Set();
    if (config.autoDetection) {
        ...
        for (const element of detectedElements) {
            if (element.sensitivityLevel) {
                const comparison = DataSensitivityUtils.compareSensitivityLevels(highestLevel, element.sensitivityLevel);
                if (comparison < 0) {
    ...
    if (config.maxSensitivityLevel) {
        const comparison = DataSensitivityUtils.compareSensitivityLevels(highestLevel, config.maxSensitivityLevel);
        if (comparison > 0) {
    ...
    // Enforce policies
    const policyEnforcement = DataClassificationHelpers.enforceSecurityPolicies(aggregateElement, currentSecurity);
    return {
        level: highestLevel,
        detectedElements,
        policyEnforcement,
 ...
 */
function analyzeObjectForSensitivity(obj, source) {
    const elements = [];
 ...
 * Analyze headers for sensitive data
 */
function analyzeHeadersForSensitivity(headers) {
    const elements = [];
 ...
 */
function extractAccessControlFromRequest(req) {
    const controls = [];
 ...
 */
async function enforceSensitivityPolicies(req, config) {
    const violations = [];
    ...
    if (config.maxSensitivityLevel) {
        const comparison = DataSensitivityUtils.compareSensitivityLevels(level, config.maxSensitivityLevel);
        if (comparison > 0) {
 ...
 * Apply dynamic security headers based on sensitivity level
 */
function applyDynamicSecurityHeaders(res, level) {
    let securityConfig;
    switch (level) {
 ...
 */
export function createEndpointSensitivityMiddleware(endpointConfig) {
