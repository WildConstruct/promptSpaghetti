...
import { CLASSIFICATION_LEVEL_MAPPING,
...
export interface EnforcementResult { allowed: boolean;
...
export class ClassificationEnforcer {
  ...
  constructor(config: Partial<ClassificationEnforcementConfig> = {}) { this.config = {
  strictMode: false
  ...
  async enforceClassification(
  ...
  ): Promise<EnforcementResult> { const auditId = this.generateAuditId(operation);
    ...
    try {
      ...
      if (this.config.auditLogging) { await this.logEnforcementDecision(
          auditId
          operation
          classification
          allowed
      ...
      if (!allowed && this.config.alertingEnabled) { await this.alertOnViolation(
          classification
          operation
          allViolations }
          riskScore
 ...
 catch (error) { // Log enforcement error
  ...
  ): Promise<AccessDecision> { // Build full operation context
  ...
  requiredAuthorization: authzDecision.required ? [authzDecision.required] : [] }
    ...
    if (requirements.access.timeRestrictions) { const timeDecision = this.checkTimeRestrictions(fullContext);
  if (!timeDecision.met) {
  return {
  granted: false
  reason: timeDecision.reason || 'Outside allowed time window' }
    ...
    if (requirements.access.purposeLimitation) { const purposeDecision = this.checkPurposeLimitation(
  fullContext.purpose
  classification
  );
  ...
  async validateOperation(
    ...
    classification: DataClassificationLevel
    dataElement: any
  ): Promise<{ valid: boolean; issues: string; controls: string }> { const requirements = this.getEffectiveRequirements(classification);
  const issues: string = [];
  const requiredControls: string = [];
  // Validate based on operation type
  switch (operation.operation) {
  case 'read': }
  if (requirements.access.auditLogging !== 'STANDARD') { requiredControls.push('enhanced-audit-logging') }
        break;
      case 'write':
      case 'update':
        if (requirements.storage.encryptionRequired) { requiredControls.push('encryption-at-rest') }
        if (requirements.processing.auditTrailRequired) { requiredControls.push('audit-trail') }
        break;
      case 'delete':
        if (requirements.processing.auditTrailRequired) { requiredControls.push('deletion-audit') }
        if (requirements.storage.retentionDays > 0) {
          issues.push(`Data must be retained for ${requirements.storage.retentionDays} days`);

        break;
      case 'export':
        if (requirements.access.exportRestrictions) { issues.push('Export restrictions apply to this classification');
          requiredControls.push('export-control') }
        if (requirements.transmission.endToEndEncryption) { requiredControls.push('end-to-end-encryption') }
        break;
      case 'share':
        if (requirements.access.approvalWorkflow) { issues.push('Approval workflow required for sharing');
          requiredControls.push('approval-workflow') }
        if (requirements.transmission.certificatePinning) { requiredControls.push('certificate-pinning') }
        break;
    ...
    if (!requirements.processing.thirdPartyProcessing && operation.source === 'third-party') { issues.push('Third-party processing not allowed for this classification') }
   ...
   */
  private getEffectiveRequirements(classification: DataClassificationLevel): HandlingRequirements { const sensitivityLevel = CLASSIFICATION_LEVEL_MAPPING[classification];
  ...
  if (this.config.policyOverrides?.has(classification)) { const overrides = this.config.policyOverrides.get(classification)!;
  ...
  private async validateAccessRequirements(
    ...
    // Check authorization
    if (requirements.authorizationRequired && !currentControls.includes('authorization')) { violations.push('Authorization required but not present') }
    
    // Check approval workflow
    if (requirements.approvalWorkflow && !currentControls.includes('approval-workflow')) { violations.push('Approval workflow required but not completed') }
  ...
  private async validateOperationRequirements(
    ...
    currentControls: string
  ): Promise<string> { const violations: string = [];
  ...
  violations.push('Encryption required for storage') }
    ...
    if (['export', 'share'].includes(operation.operation)) { if (requirements.transmission.endToEndEncryption && 
      ...
      if (requirements.transmission.certificatePinning && 
          !currentControls.includes('cert-pinning')) { violations.push('Certificate pinning required') }

    
    // Processing requirements
    if (requirements.processing.isolationRequired && 
        !currentControls.includes('process-isolation')) { violations.push('Process isolation required') }
    
    // Caching restrictions
    if (!requirements.processing.cachingRestrictions.allowed && 
        currentControls.includes('caching')) { violations.push('Caching not allowed for this classification') }
  ...
  RESTRICTED: 90 }
    ...
    const operationMultipliers: Record<string, number> = { read: 0.8
  write: 1.0
  update: 1.0
  delete: 1.2
  export: 1.5
  ...
  private shouldAllowOperation(
    ...
    classification: DataClassificationLevel
    operation: OperationContext
  ): boolean { // In strict mode, any violation blocks the operation
    if (this.config.strictMode && violations.length > 0) {
      return false }
    
    // Check if violations should block
    if (this.config.blockViolations && violations.length > 0) { // Allow with high risk score in non-strict mode
      return riskScore < 80 }
    
    // Check grace period for legacy data
    if (this.isInGracePeriod(operation.timestamp)) { return riskScore < 90 }
  ...
  private getRequiredControls(
    ...
    if (['export', 'share'].includes(operation.operation)) {
      ...
      if (requirements.transmission.endToEndEncryption) { controls.push('e2e-encryption') }
    ...
    // Monitoring controls
    if (requirements.monitoring.realtimeMonitoring) { controls.push('realtime-monitoring') }
   ...
   * Check if user is exempted
   */
  private isUserExempted(userId: string): boolean { if (!this.config.exemptions) {
      return false }
    
    // Check direct user exemption
    if (this.config.exemptions.users?.includes(userId)) { return true }
   ...
   */
  private isInGracePeriod(timestamp: Date): boolean { const gracePeriodMs = this.config.gracePeriodDays * 24 * 60 * 60 * 1000;
  ...
  private checkPurposeLimitation(
    purpose: string,
    classification: DataClassificationLevel
  ): { met: boolean } { // Define allowed purposes by classification
  ...
  private getAccessConditions(
    classification: DataClassificationLevel
    context: OperationContext
  ): string { const conditions: string = [];
  ...
  conditions.push('Must not be printed') }
    ...
    if (classification === 'RESTRICTED') { conditions.push('Access monitored in real-time');
   ...
   */
  private calculateAccessExpiration(classification: DataClassificationLevel): Date { const now = new Date();
  ...
  private async logEnforcementDecision(
    ...
    violations: string
    riskScore: number
  ): Promise<void> { const event: ClassificationAuditEvent = {
  id: auditId
  timestamp: new Date()
  ...
  sessionId: operation.sessionId }
    ...
    if (this.config.realtimeMonitoring) { // Send to monitoring system
  ...
  private async alertOnViolation(
    ...
    violations: string
    riskScore: number
  ): Promise<void> { if (riskScore > 80 || classification === 'RESTRICTED') {
  // High priority alert
  ...
  recommendations.push('Enable encryption at rest for this data') }
    ...
    if (violations.includes('Authorization required but not present')) { recommendations.push('Implement role-based access control') }
    
    if (violations.includes('Certificate pinning required')) { recommendations.push('Configure certificate pinning for secure connections') }
    
    if (classification === 'RESTRICTED') { recommendations.push('Consider implementing additional monitoring');
  ...
  private convertDataHandlingToHandlingRequirements(
    ...
    classification: DataClassificationLevel
  ): HandlingRequirements { // Map authentication levels
  ...
  mfa: 'STRONG_MFA' }
    ...
    // Map audit levels
    const auditLevelMap: Record<string, HandlingRequirements['access']['auditLogging']> = { none: 'STANDARD'
  basic: 'STANDARD'
  enhanced: 'ENHANCED'
  continuous: 'REALTIME' }
};
    ...
    const monitoringLevelMap: Record<string, HandlingRequirements['monitoring']['alertThreshold']> = { none: 'LOW'
  basic: 'MEDIUM'
  enhanced: 'HIGH'
  continuous: 'CRITICAL' }
};
   ...
   */
  private getAccessControlsForClassification(classification: DataClassificationLevel): string { const controls: Record<DataClassificationLevel, string> = {
  PUBLIC: ['read-only']
   ...
   */
  private getRetentionDaysForClassification(classification: DataClassificationLevel): number { const retention: Record<DataClassificationLevel, number> = {
  PUBLIC: 365
  INTERNAL: 730
  CONFIDENTIAL: 90
  RESTRICTED: 30 }
   ...
   */
  private getApprovedLocationsForClassification(classification: DataClassificationLevel): string { const locations: Record<DataClassificationLevel, string> = {
  PUBLIC: ['any']
   ...
   */
  private getRedundancyLevelForClassification(classification: DataClassificationLevel): 'NONE' | 'STANDARD' | 'HIGH' | 'CRITICAL' { const redundancy: Record<DataClassificationLevel, 'NONE' | 'STANDARD' | 'HIGH' | 'CRITICAL'> = {
  PUBLIC: 'STANDARD'
  INTERNAL: 'STANDARD'
  CONFIDENTIAL: 'HIGH'
  RESTRICTED: 'CRITICAL' }
   ...
   */
  private getNetworkRestrictionsForClassification(classification: DataClassificationLevel): string { const restrictions: Record<DataClassificationLevel, string> = {
  PUBLIC: []
   ...
   */
  private getCacheTtlForClassification(classification: DataClassificationLevel): number { const ttl: Record<DataClassificationLevel, number> = {
  PUBLIC: 3600, // 1 hour
   ...
   */
  getAuditLog(): ClassificationAuditEvent { return [...this.auditLog] }
 ...
 */
export function createClassificationEnforcer(preset: 'development' | 'staging' | 'production' = 'production'): ClassificationEnforcer { const configs: Record<string, Partial<ClassificationEnforcementConfig>> = {
  development: {
  strictMode: false
