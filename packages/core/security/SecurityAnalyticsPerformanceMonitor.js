...
export class SecurityAnalyticsPerformanceMonitor extends EventEmitter {
    ...
    anomalyDetectionIntervals = new Map();
    ...
    // Profile management
    async createPerformanceProfile(profile) {
    ...
    async updatePerformanceProfile(profileId, updates) {
    ...
    async deletePerformanceProfile(profileId) {
        this.stopProfileMonitoring(profileId);
    ...
    // Monitoring control
    startProfileMonitoring(profileId) {
        const profile = this.profiles.get(profileId);
        ...
        const interval = setInterval(async () => {
            await this.collectPerformanceMetrics(profileId);
        }, monitoringInterval);
        this.monitoringIntervals.set(profileId, interval);
        // Start anomaly detection if enabled
        if (profile.configuration.anomaly_detection_enabled) {
        ...
        setTimeout(() => {
            this.startBaselineCollection(profileId);
        }, 5000);
    }
    stopProfileMonitoring(profileId) {
        const monitoringInterval = this.monitoringIntervals.get(profileId);
    ...
    async collectPerformanceMetrics(profileId) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
        ...
        try {
            ...
            if (profileMetrics.length > 1440) {
                profileMetrics.splice(0, profileMetrics.length - 1440);
            }
            ...
            this.emit('performance_metrics_collected', profileId, metrics);
        }
        catch (error) {
    ...
    async collectThroughputMetrics(profile) {
    ...
    async collectLatencyMetrics(profile) {
    ...
    async collectAccuracyMetrics(profile) {
    ...
    async collectAvailabilityMetrics(profile) {
        return {
    ...
    async collectScalabilityMetrics(profile) {
        return {
    ...
    async collectCustomMetrics(profile) {
        const customMetrics = {};
        for (const customResource of profile.resource_tracking.custom_resource_monitoring) {
            try {
                const value = await this.collectCustomResourceValue(customResource);
                customMetrics[customResource.name] = value;
            }
            catch (error) {
    ...
    async collectCustomResourceValue(config) {
    ...
    async collectQualityIndicators(profile) {
        return {
    ...
    getAnalyticsTypeMultiplier(analyticsType) {
        const multipliers = {
    ...
    getComplexityMultiplier(analyticsType) {
        const multipliers = {
    ...
    async updatePerformanceScores(profileId, metrics) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
        ...
        const overallScore = (throughputScore * 0.2 +
            latencyScore * 0.25 +
    ...
    calculateThroughputScore(profile, throughput) {
        if (!profile.performance_dimensions.throughput_tracking.enabled)
            return 100;
    ...
    calculateLatencyScore(profile, latency) {
        if (!profile.performance_dimensions.latency_tracking.enabled)
            return 100;
    ...
    calculateAccuracyScore(profile, accuracy) {
        if (!profile.performance_dimensions.accuracy_tracking.enabled)
            return 100;
    ...
    calculateAvailabilityScore(profile, availability) {
        if (!profile.performance_dimensions.availability_tracking.enabled)
            return 100;
    ...
    calculateTrendDirection(profileId, currentScore) {
        const metrics = this.metricsHistory.get(profileId) || [];
        if (metrics.length < 10)
            return 'stable'; // Need enough data points
    ...
    async checkPerformanceTargets(profileId, metrics) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
    ...
    checkTargetViolation(target, currentValue) {
            ...
            case 'range':
                if (target.range_min !== undefined && target.range_max !== undefined) {
                    if (currentValue < target.range_min || currentValue > target.range_max) {
                        const deviation = Math.max(Math.abs(currentValue - target.range_min), Math.abs(currentValue - target.range_max));
    ...
    async generatePerformanceAlert(profileId, target, currentValue, severity) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
        console.log(`Performance alert: ${severity} - ${target.name} for profile ${profile.name}`);
        console.log(`Target: ${target.target_value} ${target.measurement_unit}, Current: ${currentValue} ${target.measurement_unit}`);
    ...
    // Baseline management
    async startBaselineCollection(profileId) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
    ...
    async finalizeBaseline(profileId) {
        const baseline = this.baselines.get(profileId) || [];
    ...
    async detectAnomalies(profileId) {
        const metrics = this.metricsHistory.get(profileId) || [];
        const baseline = this.baselines.get(profileId) || [];
        if (metrics.length < 10 || baseline.length === 0)
    ...
    async checkForAnomalies(profileId, current, baseline, recent) {
        const anomalies = [];
        // Check CPU usage anomaly
        if (baseline['system_resources.cpu_usage_percent']) {
            const cpuAnomaly = this.detectStatisticalAnomaly(current.system_resources.cpu_usage_percent, baseline['system_resources.cpu_usage_percent'], 'cpu_usage_percent');
            if (cpuAnomaly)
                anomalies.push(cpuAnomaly);
        }
        // Check memory usage anomaly
        if (baseline['system_resources.memory_usage_percent']) {
            const memoryAnomaly = this.detectStatisticalAnomaly(current.system_resources.memory_usage_percent, baseline['system_resources.memory_usage_percent'], 'memory_usage_percent');
            if (memoryAnomaly)
                anomalies.push(memoryAnomaly);
        }
        // Check response time anomaly
        if (baseline['latency.avg_response_time_ms']) {
            const latencyAnomaly = this.detectStatisticalAnomaly(current.latency.avg_response_time_ms, baseline['latency.avg_response_time_ms'], 'avg_response_time_ms');
            if (latencyAnomaly)
                anomalies.push(latencyAnomaly);
        }
        // Check throughput anomaly
        if (baseline['throughput.requests_per_second']) {
            const throughputAnomaly = this.detectStatisticalAnomaly(current.throughput.requests_per_second, baseline['throughput.requests_per_second'], 'requests_per_second');
        ...
        if (baseline['accuracy.detection_accuracy_percent']) {
            const accuracyAnomaly = this.detectStatisticalAnomaly(current.accuracy.detection_accuracy_percent, baseline['accuracy.detection_accuracy_percent'], 'detection_accuracy_percent');
            if (accuracyAnomaly)
    ...
    detectStatisticalAnomaly(currentValue, baselineStats, metricName) {
    ...
    async generateAnomalyRecord(profileId, anomaly, currentMetrics) {
        const profile = this.profiles.get(profileId);
        if (!profile)
            return;
    ...
    assessBusinessImpact(severity, metric) {
        if (severity === 'critical')
            return 'critical';
        if (severity === 'warning') {
            if (metric.includes('accuracy') || metric.includes('availability'))
                return 'high';
            return 'medium';
        }
        return 'low';
    }
    estimateCostImpact(severity, metric) {
        ...
        const baseCost = baseCosts[severity] || 100;
        const multiplier = multipliers[metric] || 1.0;
        return Math.round(baseCost * multiplier);
    }
    generateUserImpactDescription(metric, severity) {
        const descriptions = {
            'cpu_usage_percent': {
    ...
    generateContributingFactors(metrics, anomalyMetric) {
        const factors = [];
    ...
    async performGlobalPerformanceAnalysis() {
    ...
    async analyzeOptimizationOpportunities(profileId) {
        const profile = this.profiles.get(profileId);
        const metrics = this.metricsHistory.get(profileId) || [];
        if (!profile || metrics.length < 10)
            return [];
    ...
    async generateGlobalOptimizationRecommendations() {
    ...
    // Public API methods
    getPerformanceProfile(profileId) {
        return this.profiles.get(profileId);
    }
    getPerformanceProfiles(analyticsType) {
    ...
    getPerformanceMetrics(profileId, hours = 24) {
        const metrics = this.metricsHistory.get(profileId) || [];
        const cutoffTime = Date.now() - hours * 60 * 60 * 1000;
        return metrics.filter(m => m.timestamp > cutoffTime);
    }
    getCurrentPerformanceState(profileId) {
        const profile = this.profiles.get(profileId);
        return profile?.current_state;
    }
    getActiveAnomalies(profileId, severity) {
        let anomalies = [];
        if (profileId) {
            anomalies = this.anomalies.get(profileId) || [];
        }
        else {
    ...
    getOptimizationRecommendations(profileId, status) {
    ...
    async acknowledgeAnomaly(anomalyId, acknowledgedBy) {
    ...
    async resolveAnomaly(anomalyId, resolvedBy, resolutionMethod, notes) {
        for (const anomalies of this.anomalies.values()) {
            const anomaly = anomalies.find(a => a.id === anomalyId);
            if (anomaly) {
    ...
    async generatePerformanceReport(profileId, hours = 24) {
        const profile = this.profiles.get(profileId);
        if (!profile) {
            throw new Error(`Performance profile ${profileId} not found`);
        }
    ...
    analyzeThroughputTrends(metrics) {
        ...
        if (changePercent > 5)
            ...
            trend = 'decreasing';
        return {
            average_throughput: Math.round(avgThroughput),
    ...
    analyzeResourceTrends(metrics) {
        ...
        const opportunities = [];
        if (avgCpu < 30) {
            waste.push('CPU underutilization');
    ...
    analyzeAccuracyTrends(metrics) {
        ...
        return {
            average_accuracy_percent: Math.round(avgAccuracy * 100) / 100,
    ...
    analyzeAvailabilityTrends(metrics) {
        if (metrics.length === 0) {
            return {
                uptime_percentage: 0,
        ...
        const risks = [];
        if (avgUptime < 99)
            risks.push('Uptime below 99%');
        if (avgErrorRate > 2)
            risks.push('High error rate');
        if (incidents > 3)
            risks.push('Frequent incidents');
        return {
            uptime_percentage: Math.round(avgUptime * 100) / 100,
    ...
    analyzeScalabilityTrends(metrics) {
        if (metrics.length === 0) {
            return {
        ...
        return {
            current_scale_factor: Math.round(avgScaleFactor * 100) / 100,
    ...
    identifyKeyAchievements(profile, metrics) {
        const achievements = [];
    ...
    identifyCriticalIssues(anomalies) {
        const issues = [];
    ...
    async performDataCleanup() {
        ...
        for (const [profileId, anomalies] of this.anomalies.entries()) {
            const filteredAnomalies = anomalies.filter(a => a.detected_at > cutoffTime || !a.resolution.resolved);
