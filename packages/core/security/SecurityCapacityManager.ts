...
export interface CapacityPlan { id: string;
  name: string;
  description: string;
  service: string;
  // Planning parameters
...
export interface ResourceRequirements { cpu_cores: number;
  memory_gb: number;
  storage_gb: number;
...
export interface GrowthProjection { period: 'monthly' | 'quarterly' | 'yearly';
...
export interface SeasonalFactor { period: 'daily' | 'weekly' | 'monthly' | 'yearly';
...
export interface PerformanceTargets { response_time_p95_ms: number;
...
export interface AvailabilityRequirements { target_availability: number; // 99.9%;
...
export interface ScalingPolicy { id: string;
  name: string;
  description: string;
...
export interface MetricTrigger { metric_name: string;
...
export interface TimeTrigger { schedule_type: 'cron' | 'recurring' | 'one_time';
...
export interface EventTrigger { event_type: 'security_incident' | 'high_alert_volume' | 'system_failure' | 'maintenance_mode';
  event_source: string;
...
export interface ScalingAction { action_type: 'instance_count' | 'resource_adjustment' | 'load_balancer_weight';
...
export interface NotificationAction { channel: 'email' | 'slack' | 'webhook' | 'sms';
  target: string;
...
export interface CustomMetricTarget { metric_name: string;
  target_value: number;
...
export interface InstanceTypeConfig { instance_type: string;
  cpu_cores: number;
  memory_gb: number;
...
export interface CapacityMetrics { id: string;
  service: string;
  timestamp: number;
  collection_period: { }
  start: number;
...
export interface ScalingEvent { id: string;
  timestamp: number;
  service: string;
  policy_id: string;
  // Event details
...
export interface CapacityForecast { id: string;
  service: string;
  generated_at: number;
...
export interface CapacityRecommendation { id: string;
...
export class SecurityCapacityManager extends EventEmitter {
  ...
  async createCapacityPlan(plan: Omit<CapacityPlan, 'id' | 'created_at' | 'last_updated' | 'last_reviewed' | 'next_review_date'>): Promise<string> {
    ...
    const newPlan: CapacityPlan = { ...plan
  id
  created_at: Date.now()
  ...
  next_review_date: Date.now() + (90 * 24 * 60 * 60 * 1000) // 90 days }
    ...
    if (plan.scaling.auto_scaling_enabled) { await this.createDefaultScalingPolicies(id, plan.service);
  ...
  async createScalingPolicy(policy: Omit<ScalingPolicy, 'id' | 'created_at' | 'last_triggered' | 'trigger_count'>): Promise<string> {
    ...
    const newPolicy: ScalingPolicy = { ...policy
  id
  created_at: Date.now()
  last_triggered: 0
  trigger_count: 0 }
  ...
  async collectCapacityMetrics(service: string): Promise<string> {
    ...
    const metrics: CapacityMetrics = { id
  service
  timestamp: Date.now()
  collection_period: {
  ...
  cost_optimization_opportunities: this.identifyCostOptimizations(currentInstances, cpuUtilization) }
    ...
    if (!this.metrics.has(service)) { this.metrics.set(service, []);
  ...
  private evaluateTimeTrigger(trigger: TimeTrigger): boolean {
    ...
    return false;
  private async executeScalingAction(service: string, policy: ScalingPolicy, triggerReason: string, triggerType: string): Promise<void> { if (this.activeScaling.get(service)) {
  ...
  scalingAction = policy.actions.scale_up } else { scalingAction = policy.actions.scale_down;
    // Calculate new capacity
    if (scalingAction.target_value !== undefined) {
      newCapacity = scalingAction.target_value } else if (scalingAction.adjustment_value !== undefined) { if (scalingAction.adjustment_type === 'percentage') {
        const adjustment = Math.floor(currentCapacity * (scalingAction.adjustment_value / 100));
        newCapacity = currentCapacity + adjustment } else { newCapacity = currentCapacity + scalingAction.adjustment_value;
    // Apply constraints
    ...
    if (newCapacity > currentCapacity) {
      newCapacity = Math.min(newCapacity, currentCapacity + maxScaleUp) } else if (newCapacity < currentCapacity) {
    ...
    // Send notifications
    for (const notification of policy.actions.notification) { await this.sendScalingNotification(notification, service, scalingEvent);
  ...
  async generateCapacityForecast(service: string, horizonDays: number = 30): Promise<string> {
    ...
    const forecast: CapacityForecast = { id
  service
  ...
  peak_capacity_needed: Math.max(...forecast.forecasts.map(f => f.required_capacity.cpu_cores)) }
    ...
    return id;
  private generateForecastData(service: string, horizonDays: number): CapacityForecast['forecasts'] { const forecasts: CapacityForecast['forecasts'] = [];
  ...
  status: 'pending'
    ...
    return { immediate_actions: immediate
  ...
  getCapacityStatus(): { services: Array<{ }
  service: string;
...
>;
 ...
 else if (currentCapacity >= plan.scaling.max_instances || currentCapacity <= plan.scaling.min_instances) { scalingStatus = 'at_limits';
  return {
  service: plan.service,
  ...
  private calculateCostEfficiency(service: string, currentCapacity: number, metrics?: CapacityMetrics): number { if (!metrics) return 50; // Default neutral score
  ...
  created_by: 'system',
        enabled: true];
    defaultPlans.forEach(async (plan) => { await this.createCapacityPlan(plan) });
  private startMetricsCollection(): void { // Collect metrics every 5 minutes
    setInterval(async () => {
      const services = Array.from(this.capacityPlans.values()).map(plan => plan.service);
      for (const service of services) {
        try {
          await this.collectCapacityMetrics(service) } catch (error) {
          console.error(`Failed to collect capacity metrics for ${service}:`, error);}
    }, 300000);
  private startCapacityMonitoring(): void { // Monitor capacity thresholds every minute
    setInterval(() => {
      for (const [service] of this.currentCapacity) {
        this.checkCapacityThresholds(service) }, 60000);
  private startForecastGeneration(): void { // Generate forecasts daily
    setInterval(async () => {
      const services = Array.from(this.capacityPlans.values()).map(plan => plan.service);
      for (const service of services) {
        try {
          await this.generateCapacityForecast(service, 30) } catch (error) {
          console.error(`Failed to generate forecast for ${service}:`, error);}
    }, 24 * 60 * 60 * 1000);
  private checkCapacityThresholds(service: string): void { const recentMetrics = this.metrics.get(service)?.slice(-1)[0];
 ...
 else if (avgUtilization > plan.monitoring.capacity_thresholds.warning_percentage) { this.emit('capacity_threshold_warning', {)
  service,
  utilization: avgUtilization,
  threshold: plan.monitoring.capacity_thresholds.warning_percentage }
});
  // Public API methods
  getCapacityPlans(): CapacityPlan { return Array.from(this.capacityPlans.values());
