...
export interface CapacityPlan { id: string;
    ...
    requirements: { baseline_capacity: ResourceRequirements;
    ...
    scaling: { auto_scaling_enabled: boolean;
    ...
    monitoring: { capacity_thresholds: {
            warning_percentage: number;
            critical_percentage: number;
            forecast_breach_days: number };
        metrics_collection: { interval_seconds: number;
            retention_days: number;
            custom_metrics: string[] };
        alerting: { notification_channels: string[];
...
export interface AvailabilityRequirements { target_availability: number;
    ...
    disaster_recovery: { rto_minutes: number;
        rpo_minutes: number;
...
export interface ScalingPolicy { id: string;
    ...
    actions: { scale_up: ScalingAction;
        scale_down: ScalingAction;
        notification: NotificationAction[] };
    constraints: { max_scale_up_percentage: number;
...
export interface CapacityMetrics { id: string;
    ...
    current_utilization: { cpu_percentage: number;
        memory_percentage: number;
    ...
    resources: { allocated_instances: number;
...
export interface CapacityForecast { id: string;
    ...
    forecasts: Array<{ date: number;
        ...
        risk_factors: string[] }>;
    recommendations: { immediate_actions: CapacityRecommendation[];
        short_term_planning: CapacityRecommendation[];
        long_term_strategy: CapacityRecommendation[] };
    accuracy: { last_forecast_accuracy: number;
...
export interface CapacityRecommendation { id: string;
    ...
    timeline: { recommended_start: number;
...
export declare class SecurityCapacityManager extends EventEmitter { private capacityPlans;
