...
export interface FailoverPolicy { id: string;
    ...
    triggers: {
        ...
        performance_degradation: { enabled: boolean;
            cpu_threshold: number;
            memory_threshold: number;
            response_time_threshold: number;
            error_rate_threshold: number;
            duration_threshold: number };
        capacity_limits: { enabled: boolean;
            connection_threshold: number;
            queue_threshold: number;
            throughput_threshold: number };
        dependency_failure: { enabled: boolean;
            cascade_failover: boolean;
            dependency_timeout: number };
    };
    strategy: { type: 'immediate' | 'graceful' | 'planned';
    ...
    compliance: { require_approval: boolean;
...
export interface RedundancyGroup { id: string;
    ...
    health: { overall_status: 'healthy' | 'degraded' | 'critical' | 'offline';
    ...
    load_distribution: { strategy: 'round_robin' | 'weighted' | 'least_connections' | 'resource_based';
...
export interface FailoverMetrics { availability: {
    ...
    failover_performance: { total_failovers: number;
    ...
    system_health: { healthy_nodes: number;
        total_nodes: number;
        degraded_nodes: number;
        offline_nodes: number;
        ...
        error_rate: number };
    data_consistency: { replication_lag: number;
        consistency_violations: number;
        data_loss_incidents: number;
        sync_success_rate: number };
    compliance: { rto_compliance: number;
 ...
 */
export declare class SecurityFailoverManager extends EventEmitter { private config;
    private nodes;
    private policies;
