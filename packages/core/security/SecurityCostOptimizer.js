...
export class SecurityCostOptimizer extends EventEmitter {
    ...
    async collectCostMetrics(costCenterId) {
    ...
    generateRealisticCost(costCenter) {
    ...
    calculateEfficiencyScore(totalCost, costCenter) {
    ...
    generateServiceBreakdown(totalCost) {
        const services = [
    ...
    generateUserCostBreakdown(totalCost) {
    ...
    generateProjectCostBreakdown(totalCost) {
    ...
    async evaluateCostAlerts(costCenterId, metrics) {
    ...
    mapAlertTypeToEventType(alertType) {
        const mapping = {
    ...
    determineSeverity(alert, metrics) {
    ...
    generateAlertRecommendations(alert, metrics) {
    ...
    async executeAutoAction(action, costCenterId, event) {
        ...
        if (action.parameters.max_executions_per_day) {
            const todayExecutions = this.optimizationHistory.filter(h => h.action_id === action.id &&
                Date.now() - h.executed_at < 24 * 60 * 60 * 1000).length;
    ...
    async executeScaleDownAction(action, costCenterId) {
    ...
    async executeShutdownAction(action, costCenterId) {
    ...
    async executeOptimizeAction(action, costCenterId) {
    ...
    async executeThrottleAction(action, costCenterId) {
    ...
    async executeNotifyAction(action, costCenterId, event) {
    ...
    async generateOptimizationRecommendations(costCenterId, metrics) {
        const recommendations = [];
    ...
    async createResourceRightsizingRecommendation(costCenterId, metrics) {
    ...
    async createStorageOptimizationRecommendation(costCenterId, metrics) {
    ...
    async createReservedInstanceRecommendation(costCenterId, metrics) {
        const currentCost = metrics.costs.compute_cost;
    ...
    async createLicensingOptimizationRecommendation(costCenterId, metrics) {
    ...
    // Report Generation
    async generateCostReport(costCenterId, reportType, period) {
        const costCenter = this.costCenters.get(costCenterId);
        if (!costCenter) {
            throw new Error(`Cost center not found: ${costCenterId}`);
        }
        ...
        const centerMetrics = this.metrics.get(costCenterId) || [];
        const periodMetrics = centerMetrics.filter(m => m.collected_at >= period.start && m.collected_at <= period.end);
    ...
    generateReportInsights(metrics) {
        const insights = [];
        ...
        const topCostService = metrics.services.reduce((max, service) => service.cost > max.cost ? service : max, metrics.services[0]);
        insights.push(`${topCostService.service_name} is the largest cost driver at ${topCostService.percentage.toFixed(1)}%`);
        return insights;
    }
    generateCostBreakdownAnalysis(metrics) {
        if (metrics.length < 2)
            return [];
    ...
    generateUtilizationAnalysis(metrics) {
    ...
    rateEfficiency(utilization, cost) {
        if (utilization > 80)
            return 'excellent';
        if (utilization > 65)
            return 'good';
    ...
    calculateOptimizationPotential(utilization) {
        if (utilization > 80)
    ...
    generateTrendingAnalysis(metrics) {
        if (metrics.length < 2)
            return [];
        const latest = metrics[metrics.length - 1];
    ...
    async sendCostNotifications(alert, event) {
    ...
    async sendNotification(channel, recipient, alert, event) {
    ...
    async sendEscalationNotification(recipient, alert, event) {
    ...
    createCostNotificationMessage(alert, event) {
        return `
...
View Details: /cost-optimizer/events/${event.id}
    ...
    async sendCostReport(report, recipients) {
    ...
    createReportSummaryMessage(report) {
        return `
...
View Full Report: /cost-optimizer/reports/${report.id}
    ...
    async createDefaultCostAlertsForCenter(costCenterId) {
        const costCenter = this.costCenters.get(costCenterId);
        if (!costCenter)
            return;
