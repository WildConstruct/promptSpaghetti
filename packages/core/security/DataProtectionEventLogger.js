...
(function (DataSensitivityLevel) {
    ...
    DataSensitivityLevel["PII"] = "pii";
    DataSensitivityLevel["SPECIAL_CATEGORY"] = "special_category";
...
export class DataProtectionEventLogger {
    ...
    complianceMode;
    retentionPolicies;
    constructor(securityLogger, auditLogger, options = {}) {
     ...
     */
    async logDataProtectionEvent(event) {
        try {
            // Validate event data
     ...
     */
    async logDataDeletionEvent(event) {
        const extendedEvent = {
            ...event,
            metadata: {
                ...event.metadata,
     ...
     */
    async logPrivacyRequestEvent(event) {
        const extendedEvent = {
            ...event,
            metadata: {
                ...event.metadata,
     ...
     */
    async logPolicyViolationEvent(event) {
        const extendedEvent = {
            ...event,
    ...
    determineEventLevel(eventType) {
        ...
        if (highEvents.includes(eventType))
            return SecurityEventLevel.HIGH;
        return SecurityEventLevel.MEDIUM;
    }
    async checkAlertingRules(event) {
    ...
    aggregatePrivacyRequests(events) {
        ...
        return events.filter(event => privacyRequestTypes.includes(event.action));
    }
