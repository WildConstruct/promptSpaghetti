...
export class EmailDeliveryTracker extends EventEmitter {
  ...
  constructor(config: Partial<EmailDeliveryConfig> = {}) { super();
  ...
  */
  ...
  providerData: {}
    ...
    // Send the email
    try { await this.sendEmailViaProvider(deliveryRecord, request);
      this.updateStatus(emailId, DeliveryStatus.QUEUED) } catch (error) { this.updateStatus(emailId, DeliveryStatus.FAILED);
   ...
   */
  public async handleWebhook(provider: EmailProvider, payload: any): Promise<void> { try {
      const events = this.parseWebhookPayload(provider, payload);
      for (const event of events) {
        const emailId = this.findEmailByMessageId(event.messageId);
        if (emailId) {
          this.processWebhookEvent(emailId, event) } catch (error) { this.emit('webhookError', {)
  provider
   ...
   */
  public getEmailStatus(emailId: string): DeliveryStatus | null { const record = this.deliveryRecords.get(emailId);
  ...
  */
  ...
  metadata: record.metadata
      ...
      return true;
 catch (error) { this.addDeliveryAttempt(emailId, {)
  ...
  ): Promise<void> { // Simulate email sending based on provider
  ...
  providerResponse: 'Email queued for delivery' }
    ...
    setTimeout(() => { this.updateStatus(record.id, DeliveryStatus.SENT);
      setTimeout(() => {
        this.updateStatus(record.id, DeliveryStatus.DELIVERED) }, 1000 + Math.random() * 5000); // 1-6 seconds for delivery
    }, 500); // 500ms for sending
  private addDeliveryAttempt(emailId: string, attempt: EmailDeliveryAttempt): void { const record = this.deliveryRecords.get(emailId);
  if (record) {
  ...
  private handleBounce(record: EmailDeliveryRecord): void { this.emit('emailBounced', {)
  emailId: record.id
  record
  bounceInfo: record.bounceInfo }
});
    // If it's a hard bounce, mark the email address for suppression
    if (record.bounceInfo?.type === BounceType.HARD) { this.emit('hardBounce', {)
  emailAddress: record.recipient
  reason: record.bounceInfo.reason
  timestamp: record.bouncedAt }
});
  private handleSpamReport(record: EmailDeliveryRecord): void { this.emit('spamReport', {)
  emailId: record.id
  record
  timestamp: new Date() }
});
  private parseWebhookPayload(provider: EmailProvider, payload: any): any { // This would be implemented based on each provider's webhook format
  ...
  private findEmailByMessageId(messageId: string): string | null { for (const [emailId, record] of this.deliveryRecords) {
  ...
  private getSenderAddress(emailType: EmailType): string { // Return appropriate sender address based on email type
    switch (emailType) {
