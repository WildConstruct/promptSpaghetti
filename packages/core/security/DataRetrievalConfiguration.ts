 ...
 */
export const STANDARD_DATA_RETRIEVAL_LIMITS: Record<DataClassificationLevel, DataRetrievalLimits> = { [DataClassificationLevel.PUBLIC]: {
  ...
  [DataClassificationLevel.INTERNAL]: { classification: DataClassificationLevel.INTERNAL
  operation: 'READ'
  limits: {
  requestsPerMinute: 100
  requestsPerHour: 2000
  requestsPerDay: 20000
  bytesPerMinute: 20971520, // 20MB
  bytesPerHour: 209715200, // 200MB
  recordsPerMinute: 2000
  ...
  deviceTrustMultiplier: 0.7 }

  [DataClassificationLevel.CONFIDENTIAL]: { classification: DataClassificationLevel.CONFIDENTIAL
  operation: 'READ'
  limits: {
  requestsPerMinute: 30
  requestsPerHour: 500
  requestsPerDay: 2000
  bytesPerMinute: 5242880, // 5MB
  bytesPerHour: 52428800, // 50MB
  recordsPerMinute: 500
  recordsPerHour: 5000
  ...
  deviceTrustMultiplier: 0.5 }

  [DataClassificationLevel.RESTRICTED]: { classification: DataClassificationLevel.RESTRICTED
  operation: 'READ'
  limits: {
  requestsPerMinute: 10
  requestsPerHour: 100
  requestsPerDay: 500
  bytesPerMinute: 1048576, // 1MB
  ...
  [DataClassificationLevel.TOP_SECRET]: { classification: DataClassificationLevel.TOP_SECRET
  operation: 'READ'
  limits: {
  requestsPerMinute: 5
  requestsPerHour: 25
 ...
 */
export const OPERATION_MODIFIERS: Record<DataOperation, OperationModifier> = { READ: {
...
export const ENVIRONMENT_CONFIGURATIONS = { DEVELOPMENT: {
  globalLimits: {
 ...
 */
export const ROLE_EXEMPTION_TEMPLATES = { SYSTEM_ADMIN: {
  ...
  exemptionType: 'QUOTA' as const
    ...
    conditions: [
      {
        type: 'BUSINESS_CRITICAL' as const }
 ...
 */
export class DataRetrievalConfigurationFactory { /**
  ...
  public static createConfiguration(environment: 'DEVELOPMENT' | 'STAGING' | 'PRODUCTION')
    ...
    const baseConfig: DataRetrievalConfig = {
  ...
  ): DataRetrievalLimits { const modifier = OPERATION_MODIFIERS[operation];
  if (!modifier) {
  return baseLimits;
  const operationLimits: DataRetrievalLimits = JSON.parse(JSON.stringify(baseLimits));
  // Apply modifiers
  ...
  */
  ...
  expiresAt?: Date): DataAccessExemption {
   ...
   */
  public static validateConfiguration(config: DataRetrievalConfig): ValidationResult {
    const errors: string = [];
    ...
    if (config.alertThresholds.userQuotaUsage.warningPercent >= config.alertThresholds.userQuotaUsage.criticalPercent) { errors.push('Warning quota threshold must be less than critical threshold');
  return {
   ...
   */
  public static optimizeForPerformance(config: DataRetrievalConfig): DataRetrievalConfig { const optimized = JSON.parse(JSON.stringify(config)) as DataRetrievalConfig;
