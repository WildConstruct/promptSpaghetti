...
export interface DisasterRecoveryPlan {
    ...
    objectives: { recovery_time_objective: number;
        recovery_point_objective: number;
        maximum_tolerable_outage: number;
        minimum_service_level: number };
    strategies: RecoveryStrategy[];
    backup_requirements: { backup_frequency: 'continuous' | 'hourly' | 'daily' | 'weekly';
        retention_policy: {
    ...
    testing: { test_frequency: 'monthly' | 'quarterly' | 'biannually' | 'annually';
        last_test_date: number;
        next_test_date: number;
        test_results: TestResult[];
        automated_testing: boolean;
        test_scenarios: string[] };
    communication: { notification_tree: NotificationTreeNode[];
    ...
    dependencies: { required_systems: string[];
...
export interface RecoveryStrategy { id: string;
    ...
    resources: { infrastructure: {
        ...
        personnel: { required_roles: string[];
            minimum_staff: number;
            on_call_requirements: boolean };
        estimated_cost: { setup_cost: number;
            monthly_cost: number;
            activation_cost: number };
    };
    success_criteria: { rto_compliance: boolean;
...
export interface RecoveryProcedure { id: string;
    ...
    validation: { success_criteria: string[];
...
export interface BackupJob { id: string;
    ...
    performance: { max_bandwidth_mbps: number;
...
export interface BackupExecution { id: string;
    ...
    metrics: { data_size_bytes: number;
    ...
    resource_usage: { cpu_usage_avg: number;
...
export interface DisasterRecoveryEvent {
    ...
    execution: { strategy_used: string;
    ...
    results: { recovery_successful: boolean;
        actual_rto: number;
        actual_rpo: number;
    ...
    communications: { stakeholders_notified: string[];
...
export interface TestResult { id: string;
    ...
    results: { overall_success: boolean;
    ...
    metrics: { actual_rto: number;
        actual_rpo: number;
        data_recovery_percentage: number;
        system_recovery_percentage: number;
        communication_effectiveness: number };
    follow_up: { action_items: ActionItem[];
        plan_updates: string[];
...
export interface NotificationTreeNode { id: string;
    ...
    contact_methods: {
        ...
        secondary?: { type: 'email' | 'sms' | 'phone';
            value: string };
        backup?: { type: 'email' | 'sms' | 'phone';
            value: string };
    };
    ...
    availability_schedule?: { timezone: string;
        business_hours: {
            start: string;
            end: string };
...
export interface ExternalDependency { id: string;
    ...
    sla_commitments: { availability: number;
...
export interface DisasterRecoveryMetrics { availability: {
    ...
    backup_performance: { backup_success_rate: number;
    ...
    recovery_readiness: { plans_current: number;
        plans_total: number;
        last_test_results: {
            plan_id: string;
            success: boolean;
    ...
    dr_events: { total_events: number;
        events_by_type: Record<string, number>;
        events_by_severity: Record<string, number>;
        successful_recoveries: number;
        average_recovery_time: number;
        total_downtime: number };
    cost_metrics: { backup_storage_cost: number;
...
export interface DisasterRecoveryConfig { enabled: boolean;
    default_rto: number;
    default_rpo: number;
    backup: {
        enabled: boolean;
 ...
 */
export declare class SecurityDisasterRecoveryManager extends EventEmitter { private config;
