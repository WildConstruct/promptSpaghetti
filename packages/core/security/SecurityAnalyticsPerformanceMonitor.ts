...
export class SecurityAnalyticsPerformanceMonitor extends EventEmitter {
  ...
  private stopProfileMonitoring(profileId: string): void {
  ...
  private getAnalyticsTypeMultiplier(analyticsType: AnalyticsPerformanceProfile['analytics_type']): number {
    const multipliers = {
  ...
  private getComplexityMultiplier(analyticsType: AnalyticsPerformanceProfile['analytics_type']): number {
    const multipliers = {
  ...
  private getBaseAccuracy(analyticsType: AnalyticsPerformanceProfile['analytics_type']): number {
    const accuracies = {
      'threat_detection': 92,
  ...
  private detectStatisticalAnomaly()
    ...
    metricName: string,
  ): { metric: string; type: string; severity: 'info' | 'warning' | 'critical'; details: any } | null {
  ...
  private async generateAnomalyRecord()
    profileId: string, 
  ...
  private mapAnomalyType(type: string): PerformanceAnomaly['anomaly_type'] {
    const mappings: Record<string, PerformanceAnomaly['anomaly_type']> = {
  ...
  private generateSuspectedCauses(metric: string, anomalyType: string): string[] {
  ...
  private analyzeThroughputTrends(metrics: PerformanceMetrics[]): ThroughputAnalysis {
    if (metrics.length === 0) {
      return {
  ...
  private analyzeResourceTrends(metrics: PerformanceMetrics[]): ResourceAnalysis {
    if (metrics.length === 0) {
      return {
  ...
  private analyzeAccuracyTrends(metrics: PerformanceMetrics[]): AccuracyAnalysis {
    if (metrics.length === 0) {
      return {
  ...
  private analyzeAvailabilityTrends(metrics: PerformanceMetrics[]): AvailabilityAnalysis {
    if (metrics.length === 0) {
      return {
        uptime_percentage: 0,
  ...
  private analyzeScalabilityTrends(metrics: PerformanceMetrics[]): ScalabilityAnalysis {
    if (metrics.length === 0) {
      return {
