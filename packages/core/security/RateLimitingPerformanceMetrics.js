...
export class RateLimitingPerformanceMetrics extends EventEmitter {
     ...
     */
    generateTimeSeriesData(timeRange = '1h') {
        ...
        const relevantMetrics = this.metricsHistory.filter(metric => metric.timestamp >= startTime && metric.timestamp <= endTime);
        return {
    ...
    generateHeatmapData() {
        ...
        const activityMatrix = endpoints.map(() => timeSlots.map(() => Math.floor(Math.random() * 1000)));
        // Generate block matrix (blocks per hour per endpoint)
        const blockMatrix = endpoints.map((_, i) => timeSlots.map((_, j) => Math.floor(activityMatrix[i][j] * 0.1 * Math.random())));
        return {
            endpoints,
            timeSlots,
            activityMatrix,
