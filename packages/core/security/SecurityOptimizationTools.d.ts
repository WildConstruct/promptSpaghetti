...
export interface OptimizationTool { id: string;
    ...
    config: { execution_timeout_minutes: number;
        resource_limits: {
...
export interface OptimizationJob { id: string;
    ...
    schedule: { type: 'manual' | 'scheduled' | 'triggered' | 'continuous';
    ...
    results: { optimization_recommendations: OptimizationRecommendation[];
...
export interface OptimizationRecommendation {
    ...
    supporting_data: { analysis_results: Record<string, any>;
...
export interface PerformanceAnalysis { analysis_id: string;
    ...
    system_metrics: { throughput: {
        ...
        latency: { p50_ms: number;
            ...
            max_ms: number;
            trend_percentage: number };
        resource_utilization: { cpu_percentage: number;
            memory_percentage: number;
            disk_io_percentage: number;
            network_percentage: number };
        error_rates: { total_errors: number;
            error_rate_percentage: number;
            error_types: Record<string, number> };
    };
    bottlenecks: Array<{ component: string;
        severity: 'low' | 'medium' | 'high' | 'critical';
        description: string;
        impact_percentage: number;
        recommended_actions: string[] }>;
    capacity_analysis: { current_capacity_utilization: number;
        projected_growth_rate: number;
        time_to_capacity_limit_days: number;
        scaling_recommendations: string[] };
    comparative_analysis: { vs_previous_period: Record<string, number>;
...
export interface CostBenefitAnalysis { analysis_id: string;
    ...
    optimization_costs: { implementation_one_time: number;
        additional_infrastructure_monthly: number;
        training_and_support: number;
        risk_mitigation: number;
        total_investment: number };
    expected_benefits: { cost_savings_monthly: number;
    ...
    financial_metrics: { roi_percentage: number;
        payback_period_months: number;
        net_present_value: number;
        break_even_point_months: number };
    sensitivity_analysis: { best_case_scenario: Record<string, number>;
...
export interface RiskAssessment { assessment_id: string;
    ...
    overall_risk: { risk_level: 'low' | 'medium' | 'high' | 'critical';
        confidence_score: number;
        key_risk_factors: string[];
        recommended_risk_controls: string[] };
    compliance_impact: { affected_regulations: string[];
...
export interface ExecutionSummary { summary_id: string;
    ...
    resource_usage: { peak_cpu_percentage: number;
        peak_memory_mb: number;
        total_disk_io_mb: number;
        network_data_mb: number;
        execution_cost: number };
    quality_metrics: { data_completeness_percentage: number;
        analysis_accuracy_score: number;
        recommendation_confidence_score: number;
        user_satisfaction_score?: number };
    issues: Array<{ severity: 'info' | 'warning' | 'error' | 'critical';
        component: string;
        message: string;
...
export interface OptimizationResult { result_id: string;
    ...
    cost_impact: { implementation_cost: number;
        monthly_savings: number;
        annual_savings: number;
        roi_percentage: number };
    validation: { tests_passed: number;
        tests_failed: number;
...
export interface OptimizationEvent {
    ...
    context: { system_state: Record<string, any>;
...
export declare class SecurityOptimizationTools extends EventEmitter { private optimizationProfiles;
