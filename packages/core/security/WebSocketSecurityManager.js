...
export class WebSocketSecurityManager extends EventEmitter {
    ...
    constructor(config, keyManagementService, dataClassifier, fingerprintService, trustedDeviceManager) {
        super();
        this.config = config;
     ...
     */
    async initializeConnection(connectionId, userId, requestInfo) {
        try {
            ...
            const deviceFingerprint = await this.fingerprintService.generateFingerprint(fingerprintContext);
            // Assess initial risk
            ...
            const trustDecision = await this.trustedDeviceManager.checkDeviceTrust(userId, fingerprintContext);
     ...
     */
    async authenticateConnection(connectionId, credentials) {
        ...
        try {
            ...
            if (this.config.requireDeviceVerification && credentials.deviceVerificationToken) {
                const verifiedDevice = await this.trustedDeviceManager.verifyDevice(credentials.deviceVerificationToken);
     ...
     */
    async encryptMessage(connectionId, message) {
        const context = this.connectionContexts.get(connectionId);
        if (!context) {
            throw new Error('Connection context not found');
        }
     ...
     */
    async decryptMessage(connectionId, secureMessage) {
        const context = this.connectionContexts.get(connectionId);
        if (!context) {
            throw new Error('Connection context not found');
        }
     ...
     */
    isConnectionBlocked(connectionId, ipAddress) {
     ...
     */
    async blockConnection(connectionId, reason, duration) {
        const context = this.connectionContexts.get(connectionId);
    ...
    async cleanupConnection(connectionId) {
        ...
        if (context) {
            ...
            if (context.encryptionKeyId) {
                try {
                    await this.keyManagementService.revokeKey(context.encryptionKeyId, 'system', 'Session ended');
                }
                catch (error) {
                    // Log but don't throw
    ...
    async checkForThreats(connectionId, message) {
        const context = this.connectionContexts.get(connectionId);
        if (!context)
            return;
        // Rate limiting check
    ...
    async detectAnomalies(connectionId, message) {
        const context = this.connectionContexts.get(connectionId);
        if (!context)
            return;
