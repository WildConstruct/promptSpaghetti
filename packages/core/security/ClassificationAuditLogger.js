...
export class ClassificationAuditLogger extends EventEmitter {
     ...
     */
    async logClassification(dataElement, result, actor, duration) {
     ...
     */
    async logClassificationUpdate(dataId, oldLevel, newLevel, reason, actor) {
        const entry = await this.createAuditEntry({
     ...
     */
    async logPolicyViolation(violation, actor) {
        const entry = await this.createAuditEntry({
     ...
     */
    async logDataAccess(dataId, accessGranted, reason, actor, classification) {
        const entry = await this.createAuditEntry({
    ...
    async queryLogs(filter) {
        ...
        if (filter.userIds && filter.userIds.length > 0) {
            results = results.filter(log => log.actor.userId && filter.userIds.includes(log.actor.userId));
        }
        if (filter.dataIds && filter.dataIds.length > 0) {
            results = results.filter(log => log.target.dataId && filter.dataIds.includes(log.target.dataId));
        }
        if (filter.classificationLevels && filter.classificationLevels.length > 0) {
            results = results.filter(log => log.target.classification &&
        ...
        if (filter.complianceFrameworks && filter.complianceFrameworks.length > 0) {
            results = results.filter(log => log.compliance.frameworks.some(f => filter.complianceFrameworks.includes(f)));
        }
        ...
        if (filter.searchText) {
            ...
            results = results.filter(log => JSON.stringify(log).toLowerCase().includes(searchLower));
        }
        // Apply pagination
    ...
    async generateComplianceReport(framework, startDate, endDate) {
        ...
        const violations = relevantLogs.filter(log => log.eventType === AuditEventType.POLICY_VIOLATION);
        const dataProcessing = {
            classified: relevantLogs.filter(log => log.eventType === AuditEventType.CLASSIFICATION_PERFORMED).length,
            accessed: relevantLogs.filter(log => log.eventType === AuditEventType.ACCESS_GRANTED ||
                log.eventType === AuditEventType.ACCESS_DENIED).length,
            exported: relevantLogs.filter(log => log.eventType === AuditEventType.DATA_EXPORTED).length,
            deleted: relevantLogs.filter(log => log.eventType === AuditEventType.DATA_DELETED).length
        };
        const violationDetails = violations.map(log => ({
            timestamp: log.timestamp,
            eventId: log.id,
            description: log.action.reason || 'Policy violation',
            severity: this.mapAlertSeverityToComplianceSeverity(log.context.metadata.severity || AlertSeverity.WARNING),
     ...
     */
    async exportLogs(filter, format) {
    ...
    async createAuditEntry(data) {
        ...
        if (this.config.performanceMode === 'high_security') {
    ...
    mapAlertSeverityToComplianceSeverity(severity) {
        switch (severity) {
    ...
    generateRecommendations(framework, violations) {
        const recommendations = [];
        if (violations.length > 10) {
            recommendations.push('Review and update classification rules to reduce false positives');
        }
        const criticalCount = violations.filter(v => v.context.metadata.severity === AlertSeverity.CRITICAL).length;
        if (criticalCount > 0) {
    ...
    initializeTimers() {
        ...
        if (this.config.logRotationIntervalHours > 0) {
            this.rotationTimer = setInterval(() => this.rotateLogs(), this.config.logRotationIntervalHours * 60 * 60 * 1000);
        }
        // Archive timer for retention policies
        this.archiveTimer = setInterval(() => this.enforceRetentionPolicies(), 24 * 60 * 60 * 1000 // Daily
        );
    }
    async enforceRetentionPolicies() {
        for (const policy of this.config.retentionPolicies) {
            const cutoffDate = new Date(Date.now() - policy.retentionDays * 24 * 60 * 60 * 1000);
            const logsToArchive = this.logs.filter(log => log.eventType === policy.eventType &&
