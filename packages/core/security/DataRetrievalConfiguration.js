 ...
 */
export const STANDARD_ALERT_THRESHOLDS = {
    volumeSpike: {
        percentIncrease: 300,
        timeWindow: 15
    },
...
export class DataRetrievalConfigurationFactory {
     ...
     */
    static createConfiguration(environment, customizations) {
     ...
     */
    static createOperationLimits(baseLimits, operation) {
        ...
        // Apply modifiers
        operationLimits.limits.requestsPerMinute = Math.floor(baseLimits.limits.requestsPerMinute * modifier.requestMultiplier);
        operationLimits.limits.requestsPerHour = Math.floor(baseLimits.limits.requestsPerHour * modifier.requestMultiplier);
        operationLimits.limits.bytesPerMinute = Math.floor(baseLimits.limits.bytesPerMinute * modifier.volumeMultiplier);
        operationLimits.limits.bytesPerHour = Math.floor(baseLimits.limits.bytesPerHour * modifier.volumeMultiplier);
        operationLimits.limits.concurrentRequests = Math.floor(baseLimits.limits.concurrentRequests * modifier.concurrencyMultiplier);
        // Adjust backoff based on risk
        if (modifier.riskMultiplier > 2.0) {
            operationLimits.backoff.baseDelay = Math.min(baseLimits.backoff.baseDelay * modifier.riskMultiplier, operationLimits.backoff.maxDelay / 4);
        }
     ...
     */
    static createRoleExemption(exemptionId, role, userId, approvedBy = 'system', expiresAt) {
     ...
     */
    static optimizeForPerformance(config) {
