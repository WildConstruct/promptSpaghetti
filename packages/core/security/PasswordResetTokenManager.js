...
export class PasswordResetTokenManager extends EventEmitter {
    ...
    async generateToken(request) {
        try {
            ...
            const expirationMs = this.calculateExpiration(request.expirationMinutes, request.securityLevel || this.config.securityLevel);
            // Create token record
            const tokenRecord = {
     ...
     */
    async validateToken(tokenValue, ipAddress, userAgent) {
        try {
            const validation = await this.performTokenValidation(tokenValue, ipAddress, userAgent);
            // Log validation attempt
            this.logSecurityEvent(validation.valid ? SecurityEvent.TOKEN_VALIDATED : SecurityEvent.INVALID_TOKEN_ATTEMPT, {
     ...
     */
    async useToken(tokenValue, ipAddress, userAgent) {
        try {
     ...
     */
    async revokeToken(tokenId, reason, ipAddress = 'system', userAgent = 'system') {
     ...
     */
    async revokeUserTokens(userId, type, reason = 'user_requested') {
        let revokedCount = 0;
    ...
    getStatistics() {
        ...
        stats.securityViolations = this.auditLog.filter(entry => entry.event === SecurityEvent.SUSPICIOUS_ACTIVITY).length;
        stats.rateLimitViolations = this.auditLog.filter(entry => entry.event === SecurityEvent.RATE_LIMIT_EXCEEDED).length;
        return stats;
    }
    /**
    ...
    async performTokenValidation(tokenValue, ipAddress, userAgent) {
        // Apply anti-enumeration delay
        await this.antiEnumerationDelay();
    ...
    async antiEnumerationDelay() {
        return new Promise(resolve => {
            setTimeout(resolve, this.config.antiEnumerationDelay);
        });
    }
    logSecurityEvent(event, details, ipAddress = 'system', userAgent = 'system') {
