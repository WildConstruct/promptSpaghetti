...
export interface ClassificationAnalytics { totalClassifications: number;
    ...
    temporalTrends: Array<{ timestamp: Date;
        count: number;
...
export interface ClassificationContext { source: string;
    ...
    environmentContext: { system: string;
        network: string;
        location: string;
        timezone: string };
    dataFlow?: DataFlow;
...
export interface EnhancedClassificationResult extends ClassificationResult { mlPredictions: Array<{
    ...
    contextualFactors: Array<{ factor: string;
        impact: number;
        description: string }>;
    riskScore: number;
    remediation: Array<{ action: string;
 ...
 */
export declare class AdvancedClassificationEngine extends EventEmitter { private baseClassifier;
    ...
    trainMLModel(modelId: string, trainingData: Array<{ )
        ...
        context?: ClassificationContext }>): Promise<{ accuracy: number;
    ...
    generateComplianceReport(framework: ComplianceFramework, dateRange: { )
        start: Date;
        end: Date }): { framework: ComplianceFramework;
        period: {
            start: Date;
            end: Date };
        totalClassifications: number;
        compliantClassifications: number;
        violations: Array<{ dataId: string;
            violation: string;
            severity: string;
            timestamp: Date }>;
