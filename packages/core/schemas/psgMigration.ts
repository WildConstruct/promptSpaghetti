...
import { PsgFile as PsgFileV1 } from './psgSchema';
...
export interface MigrationResult {
  success: boolean;
  data?: z.infer<typeof PsgFileV2Schema>;
  errors?: string[];
  warnings?: string[];
}
 ...
 */
export function migratePsgV1ToV2(v1File: PsgFileV1): MigrationResult {
  const errors: string[] = [];
  ...
  try {
    ...
    // Migrate nodes
    const nodes: Node[] = v1File.graph.nodes.map((v1Node: any) => {
    ...
    // Migrate edges
    const edges: Edge[] = v1File.graph.edges.map((v1Edge: any) => ({
      id: v1Edge.id,
 ...
 */
function mapNodeType(v1Type: string): Node['type'] {
 ...
 */
function migrateNodeData(v1Node: any): any {
  ...
  switch (v1Node.type) {
    ...
    case 'weightedChoice':
      ...
      return {
        ...baseData,
        value: choices.map((choice: any, index: number) => ({
 ...
 */
export function needsMigration(fileData: any): boolean {
 ...
 */
export function autoMigrate(fileData: any): {
  data: any;
  migrated: boolean;
  warnings?: string[];
} {
