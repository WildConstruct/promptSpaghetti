...
export const CollaboratorSchema = UserInfoSchema.extend({ )
  ...
  invitedBy: z.string().regex(uuidRegex),
...
export const SharingConfigSchema = z.object({ )
  ...
  shareToken: z.string().regex(shareTokenRegex, 'Invalid share token format'),
...
export const VersionControlSchema = z.object({ )
  ...
  isLatest: z.boolean(),
  changesFromPrevious: z.array(z.string()).optional(),
  mergeConflicts: z.array(MergeConflictSchema).optional() }
});

// Annotation schemas
export const ConnectionLabelSchema = z.object({ )
  id: z.string().regex(uuidRegex),
  sourceNodeId: z.string().regex(uuidRegex),
  targetNodeId: z.string().regex(uuidRegex),
...
export const StickyNoteSchema = z.object({ )
  ...
  height: z.number().min(30).max(1000),
...
export const ContentAnnotationsSchema = z.object({ )
  ...
  comments: z.array(ShareCommentSchema).default([]) }
});
...
export const CollaborationEventSchema = z.object({ )
  id: z.string().regex(uuidRegex),
  type: z.enum(['comment', 'edit', 'annotation', 'permission_change']),
  user: UserInfoSchema,
  timestamp: z.date(),
  details: z.any(),
...
export const ShareAnalyticsSchema = z.object({ )
  ...
  downloads: z.array(ShareDownloadSchema).default([]),
  collaborations: z.array(CollaborationEventSchema).default([]),
  totalViews: z.number().min(0),
  uniqueViewers: z.number().min(0),
  averageViewDuration: z.number().min(0),
  peakConcurrentUsers: z.number().min(0),
  geographicDistribution: z.array(GeographicStatsSchema).default([]),
  deviceStats: z.array(DeviceStatsSchema).default([]),
  conversionMetrics: ConversionMetricsSchema }
});
...
export const SharedContentSchema = z.object({ )
  ...
  analytics: ShareAnalyticsSchema,
  createdAt: z.date(),
  updatedAt: z.date(),
...
export const CreateShareRequestSchema = z.object({ )
  ...
  title: z.string().min(1).max(255),
  description: z.string().max(2000).optional(),
  sharing: SharingConfigSchema.partial(),
  security: ShareSecurityConfigSchema.partial().optional(),
  collaborators: z.array(z.string().regex(uuidRegex)).max(100).optional() }
});
...
export const UpdateShareRequestSchema = z.object({ )
  ...
  description: z.string().max(2000).optional(),
  sharing: SharingConfigSchema.partial().optional(),
  security: ShareSecurityConfigSchema.partial().optional() }
});
...
export const ShareAccessResponseSchema = z.object({ )
  success: z.boolean(),
  content: SharedContentSchema.optional(),
...
export const SharingSystemConfigSchema = z.object({ )
  ...
  maxCollaborators: z.number().min(1).max(1000),
...
export function validateShareContent(content: unknown, type: string): boolean { try {
  switch (type) {
  case 'graph':,
  return z.object({)
  nodes: z.array(z.any()),
...
export const SharingSchemas = { // Core types
