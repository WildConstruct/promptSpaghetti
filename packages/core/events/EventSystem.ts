...
export enum EventPriority { CRITICAL = 'critical',   // System errors, security issues
...
export interface EventFilter { types?: string;
...
export class EventBus extends EventEmitter { private subscriptions: Map<string, EventSubscription> = new Map();
  ...
  */
  ...
  once: options?.once ?? false }
    ...
    if (filter.types) { for (const type of filter.types) {
  const wrappedHandler = (event: BaseEvent) => { }
  if (this.matchesFilter(event, filter)) { handler(event as T);
   ...
   */
  unsubscribe(subscriptionId: string): boolean { const subscription = this.subscriptions.get(subscriptionId);
  ...
  */
  ...
  if (!event.type || !event.timestamp || !event.id || !event.source) { throw new Error('Event missing required fields: type, timestamp, id, source');
  ...
  if (subscription.once) {
  this.unsubscribe(subscription.id) } catch (error) { // Emit error event for handler failures
  ...
  */
  ...
  private async processMiddleware(event: BaseEvent): Promise<void> { let index = 0;
    const next = async () => {
      if (index < this.middleware.length) {
        const middleware = this.middleware[index++];
        await middleware(event, next) };
    await next();
  private matchesFilter(event: BaseEvent, filter: EventFilter): boolean { // Type filter
...
export const createValidationMiddleware = () => (event: any, next: () => void) => { if (!event.type || !event.id) {
