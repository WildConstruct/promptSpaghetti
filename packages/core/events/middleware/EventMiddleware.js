...
export const createRateLimitMiddleware = (options) => {
    const { maxEventsPerSecond = 100, maxEventsPerMinute = 1000, maxEventsPerHour = 10000, strategy = 'error', keyGenerator = (event) => `${event.source}-${event.type}`, whitelist = [] } = options;
    ...
    return (event, next) => {
        ...
        // Check rate limits
        const exceeded = counts.secondCount >= maxEventsPerSecond ||
