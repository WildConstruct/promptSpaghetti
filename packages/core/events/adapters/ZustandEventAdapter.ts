...
export class ZustandEventAdapter { private config: ZustandEventConfig;
   ...
   */
  public middleware = <T extends object>(stateCreator: StateCreator<T & EventableStore, [], [], T & EventableStore>)
  ): StateCreator<T & EventableStore, [], [], T & EventableStore> => { return (set, get, api) => {
   ...
   */
  private setupSubscriptions(api: any): void { if (!this.config.enabledPaths) {
   ...
   */
  private publishStateChangeEvents(previousState: any, currentState: any): void { const changedPaths = this.getChangedPaths(previousState, currentState);
  ...
  private publishPathChangeEvent(path: string, previousValue: unknown, currentValue: unknown): void {
    ...
    if (this.config.debounceMs && this.config.debounceMs > 0) { const existingTimer = this.debounceTimers.get(eventId);
      if (existingTimer) {
        clearTimeout(existingTimer);
      const timer = setTimeout(() => {
        this.publishStateEvent(path, previousValue, currentValue);
        this.debounceTimers.delete(eventId) }, this.config.debounceMs);
      this.debounceTimers.set(eventId, timer);
 else { this.publishStateEvent(path, previousValue, currentValue);
  /**
  ...
  */
  ...
  private getChangedPaths(previous: any, current: any, basePath: string = ''): string {
    ...
    for (const key of allKeys) {
      ...
      if (prevValue !== currValue) { if (typeof prevValue === 'object' && typeof currValue === 'object') {
          // Recursive check for nested objects
          changes.push(...this.getChangedPaths(prevValue, currValue, currentPath)) } else { changes.push(currentPath);
  return changes;
  /**
  ...
  */
  ...
  ): string { return globalEventBus.subscribe()
  {
  types: ['state_changed']
  sources: ['zustand-adapter'] }

      (event: BaseEvent) => { const stateEvent = event as StateChangeEvent;
  ...
  ): void { if (typeof store.setState === 'function') {
 ...
 */
export const createZustandEventMiddleware = () => { const adapter = new ZustandEventAdapter();
  return adapter.middleware };
/**
 * Enhanced Zustand store creator with event integration
 */
export const createEventEnabledStore = <T>(stateCreator: StateCreator<T>) => { const adapter = new ZustandEventAdapter();
  ...
  */
  ...
  createEventUpdater: <T>()
    ...
    return globalEventBus.subscribe()
      { types: eventTypes }
      (event: BaseEvent) => { // Custom logic to update state based on event
  const currentState = store.getState();
