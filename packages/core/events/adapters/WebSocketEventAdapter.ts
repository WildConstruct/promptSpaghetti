 ...
 */
const WS_MESSAGE_TYPE_MAP: Record<WSMessageType, string> = { // Connection events
...
export class WebSocketEventAdapter {});
  ...
  console.error('Failed to publish WebSocket event to Event Bus:', error) });
    ...
    if (typeof this.collaborationService.onMessage === 'function') { this.collaborationService.onMessage((message: WSMessage) => { }
   ...
   */
  private bridgeEventBusToWebSocket(): void { // Subscribe to collaboration events from Event Bus
  ...
  'chat_message_sent', 'typing_started', 'typing_stopped'
      ...
      (event: BaseEvent) => { if (this.collaborationService) {
   ...
   */
  private convertWSMessageToEvent(message: WSMessage): BaseEvent { const eventType = WS_MESSAGE_TYPE_MAP[message.type] || message.type;
  ...
  priority = EventPriority.HIGH } else if (message.type.startsWith('workflow_')) { category = EventCategory.WORKFLOW;
      priority = EventPriority.HIGH } else if (message.type.startsWith('system_') || 
               ['rate_limit_exceeded', 'invalid_message_format'].includes(message.type)) { category = EventCategory.SYSTEM;
  priority = message.type === 'system_error' ? EventPriority.CRITICAL : EventPriority.MEDIUM } else if (['analytics_event', 'performance_metric', 'user_action'].includes(message.type)) { category = EventCategory.ANALYTICS;
      priority = EventPriority.MEDIUM } else { category = EventCategory.COLLABORATION; // Default
   ...
   */
  private convertEventToWSMessage(event: BaseEvent): WSMessage | null { // Find reverse mapping from event type to WebSocket message type
  ...
  data: any
    userId?: string
    sessionId?: string
  ): void { // Create event for Event Bus
  ...
  console.error('Failed to publish collaboration event:', error) });
    ...
    if (this.collaborationService) { this.collaborationService.emit(type, data);
  /**
  ...
  */
   ...
   */
  public cleanup(): void { if (this.collaborationService) {
