...
interface UserCursorProps { user: UserPresence }
},
 ...
 */
const UserCursor: React.FC<UserCursorProps> = ({ user, position, nodeId }) => { const cursorStyle: React.CSSProperties = {
  position: 'absolute'
  left: position.x
  top: position.y
  pointerEvents: 'none'
...
interface NodeSelectionOverlayProps { nodeId: string;
  users: UserPresence }
},
 ...
 */
const NodeSelectionOverlay: React.FC<NodeSelectionOverlayProps> = ({ nodeId
  users
  nodePosition
  nodeWidth }
  nodeHeight
...
interface CollaborativePresenceProps { userCursors: Array<{ }
  userId: string;
  user: UserPresence;
 ...
 */
export const CollaborativePresence: React.FC<CollaborativePresenceProps> = ({ userCursors
  remoteSelections }
  className
...
interface CollaborationStatusProps { isCollaborative: boolean;
 ...
 */
export const CollaborationStatus: React.FC<CollaborationStatusProps> = ({ isCollaborative
  connectionStatus
  connectedUserCount }
  className
...
interface UserAvatarsProps { connectedUsers: Map<string, UserPresence>;
  localUserId?: string;
  maxVisible?: number;
  className?: string }
 ...
 */
export const UserAvatars: React.FC<UserAvatarsProps> = ({ connectedUsers
  localUserId
  maxVisible = 5 }
  className
  ...
  const getInitials = (name: string) => { return name.split(' ')
      .map(word => word[0])
      .join('')
      .toUpperCase()
      .slice(0, 2) };
