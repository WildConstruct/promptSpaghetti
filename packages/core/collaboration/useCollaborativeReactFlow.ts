...
export function useCollaborativeReactFlow() {
  ...
  const getRemoteSelections = useCallback(() => {
    if (!isCollaborative) return new Map();
 ...
 */
function isEdgeBeingEdited(edgeId: string, connectedUsers: Map<string, UserPresence>): boolean {
 ...
 */
export function useNodeCollaborators(nodeId: string) {
  const connectedUsers = useConnectedUsers();
