...
export interface UserPresence { userId: string;
  name: string;
  color: string;
  cursor?: { }
  nodeId?: string;
...
export interface CollaborativeGraphState { // Core graph state
  graph: Graph;
  // Collaboration state
...
export const useCollaborativeGraphStore = create<CollaborativeGraphState>()(
  ...
  // Initial state
    ...
    enableCollaboration: async (options: CollaborativeGraphOptions) => { const currentGraph = get().graph;
      // Create CRDT adapter
      const crdtAdapter = new GraphCRDTAdapter({
        ...
        onUserPresence: (awareness: Map<string, unknown>) => { const connectedUsers = new Map<string, UserPresence>();
  ...
  lastSeen: presence.timestamp }
        ...
        onConnectionStatus: (connected: boolean) => { set({
  ...
  lastSeen: Date.now() }
    ...
    disableCollaboration: () => {
      ...
      if (crdtAdapter) { crdtAdapter.destroy();
  set({
  ...
  connectionStatus: 'disconnected' }
    ...
    setGraph: (graph: Graph) => {
      ...
      if (isCollaborative && crdtAdapter) { // For collaborative mode, we don't directly replace the graph
        // Instead, the graph should be updated through collaborative operations
        console.warn('Direct graph replacement not recommended in collaborative mode') } else {
        set({ graph });
    ...
    addNode: (node: Node, position?: { x: number; y: number }) => {
      ...
      if (isCollaborative && crdtAdapter) { crdtAdapter.addNode(node, position) } else { // Non-collaborative mode
  const newGraph = {
  ...graph
  ...
  edges: graph.edges }
    ...
    updateNode: (nodeId: string, updates: Partial<Node>) => {
      const { crdtAdapter, isCollaborative, graph } = get();
      if (isCollaborative && crdtAdapter) { crdtAdapter.updateNode(nodeId, updates) } else { // Non-collaborative mode
        const newGraph = {
          ...graph }
    ...
    deleteNode: (nodeId: string) => {
      const { crdtAdapter, isCollaborative, graph } = get();
      if (isCollaborative && crdtAdapter) { crdtAdapter.deleteNode(nodeId) } else { // Non-collaborative mode
  const newGraph = {
  ...graph
  ...
  edge.source !== nodeId && edge.target !== nodeId
    ...
    addEdge: (edge: Edge) => {
      const { crdtAdapter, isCollaborative, graph } = get();
      if (isCollaborative && crdtAdapter) { crdtAdapter.addEdge(edge) } else { // Non-collaborative mode
  const newGraph = {
  ...graph
  nodes: graph.nodes
  edges: [...graph.edges, edge] }
    ...
    deleteEdge: (edgeId: string) => {
      const { crdtAdapter, isCollaborative, graph } = get();
      if (isCollaborative && crdtAdapter) { crdtAdapter.deleteEdge(edgeId) } else { // Non-collaborative mode
  const newGraph = {
  ...graph
  nodes: graph.nodes
  edges: graph.edges.filter(edge => edge.id !== edgeId) }
    ...
    updateNodePosition: (nodeId: string, position: { x: number; y: number }) => {
      const { crdtAdapter, isCollaborative } = get();
      if (isCollaborative && crdtAdapter) { crdtAdapter.updateNodePosition(nodeId, position) }
    ...
    updateLocalPresence: (presence: Partial<UserPresence>) => {
      ...
      if (localPresence && userId) {
        ...
        if (crdtAdapter) { crdtAdapter.setUserPresence({
  ...
  color: newPresence.color }
    ...
    updateUserSelection: (nodeIds: string) => {
      const { updateLocalPresence } = get();
      updateLocalPresence({ selection: nodeIds }
})

    // Apply remote update
    applyRemoteUpdate: (update: Uint8Array) => {
      const { crdtAdapter } = get();
      if (crdtAdapter) { crdtAdapter.applyRemoteUpdate(update) }
...
// Action hooks for selective updates
export const useCollaborativeActions = () => useCollaborativeGraphStore(
  (state) => ({ enableCollaboration: state.enableCollaboration
...
export const useCollaborativeState = () => useCollaborativeGraphStore(
