...
export class WorkspaceDAO implements WorkspaceOperations {
  ...
  async updateWorkspace(id: WorkspaceId, data: Partial<Workspace>): Promise<Workspace> {
    ...
    if (data.settings !== undefined) updateData.settings = bindParams.json(data.settings);
  ...
  async createProject(data: Omit<Project, 'id' | 'created_at' | 'updated_at' | 'last_activity_at'>): Promise<Project> {
    ...
    const { query, params } = QueryBuilder
      .insert('projects')
      .values({ workspace_id: bindParams.workspaceId(data.workspace_id)
  name: data.name
  description: data.description
  settings: bindParams.json(data.settings)
  created_by: bindParams.userId(data.created_by)
  ...
  async createResource(data: Omit<Resource, 'id' | 'created_at' | 'updated_at' | 'version'>): Promise<Resource> {
    ...
    const { query, params } = QueryBuilder
      .insert('resources')
      .values({ project_id: bindParams.projectId(data.project_id)
  name: data.name
  type: data.type
  ...
  async updateResource(id: ResourceId, data: Partial<Resource>): Promise<Resource> {
    ...
    if (data.content !== undefined) updateData.content = bindParams.json(data.content);
    if (data.metadata !== undefined) updateData.metadata = bindParams.json(data.metadata);
    if (data.is_active !== undefined) updateData.is_active = data.is_active;
    const { query, params } = QueryBuilder
      .update('resources')
      .set(updateData)
  ...
  userId: UserId
    ...
    const { query, params } = QueryBuilder
      ...
      .values({ workspace_id: bindParams.workspaceId(workspaceId)
  ...
  async removeWorkspaceMember(workspaceId: WorkspaceId, userId: UserId): Promise<void> {
    ...
    const { query, params } = QueryBuilder
      ...
      .build();
    const result = await this.db.query(query, params);
    if (result.rowCount === 0) { throw new Error('Workspace member not found');
  ...
  userId: UserId
    ...
    const { query, params } = QueryBuilder
      ...
      .values({ project_id: bindParams.projectId(projectId)
  user_id: bindParams.userId(userId)
  role
  invited_by: bindParams.userId(invitedBy) }
  ...
  async removeProjectMember(projectId: ProjectId, userId: UserId): Promise<void> {
    ...
    const { query, params } = QueryBuilder
      ...
      .build();
    const result = await this.db.query(query, params);
    if (result.rowCount === 0) { throw new Error('Project member not found');
  ...
  async logActivity(event: Omit<ActivityEvent, 'id' | 'created_at'>): Promise<ActivityEvent> {
    ...
    const { query, params } = QueryBuilder
      ...
      .values({ workspace_id: bindParams.workspaceId(event.workspace_id)
  ...
  async createNotification(data: Omit<Notification, 'id' | 'created_at'>): Promise<Notification> {
    ...
    const { query, params } = QueryBuilder
      ...
      .values({ user_id: bindParams.userId(data.user_id)
