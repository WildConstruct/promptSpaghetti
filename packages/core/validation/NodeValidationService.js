...
export class NodeValidationService extends EventEmitter {
    ...
    async validateNodeBatch(nodes) {
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            const batchResults = await Promise.all(batch.map(node => this.validateNode(node)));
     ...
     */
    async *validateNodeStream(nodes) {
        for (let i = 0; i < nodes.length; i++) {
            const node = nodes[i];
            const result = await this.validateNode(node);
            yield { index: i, node, result };
        }
    }
    /**
     ...
     */
    exportValidationReport(nodes, results) {
        const report = {
    ...
    updateMetrics(startTime, result, fromCache) {
    ...
    emitValidationEvent(eventType, data) {
