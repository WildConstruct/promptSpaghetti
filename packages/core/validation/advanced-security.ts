...
interface PatternSignature { pattern: RegExp;
  weight: number;
  ...
  */
  ...
  if (confirmed) { const currentScore = this.threatHistory.get(signature) || 0;
  this.threatHistory.set(signature, currentScore + 1) } else { this.falsePositives.add(signature);
  /**
  ...
  */
  ...
  confidence: 1.0 }
    ...
    if (this.patternCache.has(cacheKey)) { const cachedScore = this.patternCache.get(cacheKey)!;
  ...
  */
  ...
  private calculateRiskScore(pattern: PatternSignature): number { const severityMultiplier = {
  'low': 0.3,
  'medium': 0.5,
  'high': 0.8,
  'critical': 1.0 }
  ...
  private calculateConfidence(input: string, riskScore: number): number { // Higher confidence for extreme scores, lower for middle ranges
