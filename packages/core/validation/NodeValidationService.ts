 ...
 */
import { NodeValidationFramework, NodeValidationResult, NodeValidationConfig } from './NodeValidationFramework';
...
export class NodeValidationService extends EventEmitter {
  ...
  private cacheResult(nodeData: AdvancedNodeData, result: NodeValidationResult): void {
    const nodeHash = this.generateNodeHash(nodeData);
