...
export interface SegmentServiceConfig { maxSegmentSize: number;
...
export interface SegmentQuery { ids?: string[];
    names?: string[];
    categories?: string[];
    tags?: string[];
    isActive?: boolean;
...
export interface SegmentPerformanceMetrics { segmentId: string;
    ...
    conditionPerformance: Array<{ conditionId: string;
        evaluationTime: number;
        matchRate: number;
        errorRate: number }>;
    memoryUsage: number;
...
export interface SegmentExperiment { id: string;
    ...
    successCriteria: Array<{ metric: string;
...
export interface SegmentDataPipeline { id: string;
    ...
    executionHistory: Array<{ startTime: Date;
        endTime: Date;
...
export interface SegmentComplianceConfig { dataClassification: 'public' | 'internal' | 'confidential' | 'restricted';
    ...
    consentTypes: string[];
    consentValidation: Array<{ condition: string;
        errorMessage: string }>;
    auditTrail: boolean;
...
export interface SegmentIntegration { id: string;
    ...
    errorCount: number;
    syncHistory: Array<{ startTime: Date;
        endTime: Date;
...
export interface IUserSegmentService { createSegment(segment: Omit<UserSegment, 'id' | 'createdAt' | 'lastModifiedAt'>): Promise<UserSegment>;
    ...
    getSegment(id: string, options?: { )
        includeAnalytics?: boolean }): Promise<UserSegment | null>;
    querySegments(query: SegmentQuery): Promise<{ segments: UserSegment[];
        totalCount: number;
        hasMore: boolean }>;
    evaluateSegment(segmentId: string, options?: { )
        userId?: string }): Promise<SegmentEvaluationResult>;
    ...
    addUserToSegment(userId: string, segmentId: string, options?: { )
        source?: string }): Promise<SegmentMembership>;
    ...
    getSegmentUsers(segmentId: string, options?: { )
        limit?: number;
        offset?: number }): Promise<{ users: SegmentMembership[];
        totalCount: number }>;
    getSegmentAnalytics(segmentId: string, timeRange?: { )
        start: Date;
        end: Date }): Promise<SegmentAnalytics>;
    ...
    bulkUpdateSegments(updates: Array<{ )
        id: string;
        changes: Partial<UserSegment> }>): Promise<SegmentBulkOperationResult>;
    ...
    validateSegment(segmentId: string): Promise<{ isValid: boolean;
        errors: string[];
        warnings: string[] }>;
