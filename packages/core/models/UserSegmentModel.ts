...
export interface UserAttributes { // Identity attributes
  userId: string;
  email?: string;
...
export const UserAttributesSchema = z.object({ )
  ...
  accountType: z.enum(['free', 'trial', 'paid', 'enterprise', 'beta']),
  subscriptionTier: z.enum(['basic', 'pro', 'premium', 'enterprise']),
  subscriptionStatus: z.enum(['active', 'inactive', 'cancelled', 'expired', 'pending']),
  accountAge: z.number().nonnegative(),
  userRole: z.enum(['admin', 'user', 'viewer', 'editor', 'owner']),
  ...
  experimentGroups: z.record(z.string()),
  abTestParticipation: z.array(z.string()),
  betaFeatures: z.array(z.string()),
  riskOfChurn: z.number().min(0).max(1),
  upsellProbability: z.number().min(0).max(1),
...
export const UserSegmentSchema = z.object({ )
  ...
  joinLogic: z.enum(['all', 'any', 'complex']).default('all'),
  complexLogicExpression: z.string().optional(),
  ...
  evaluationFrequency: z.enum(['realtime', 'hourly', 'daily', 'weekly', 'manual']).default('daily'),
  lastEvaluated: z.date().optional(),
  nextEvaluation: z.date().optional(),
  createdBy: z.string().min(1),
  createdAt: z.date(),
  lastModifiedBy: z.string().min(1),
  lastModifiedAt: z.date(),
  accessLevel: z.enum(['public', 'team', 'organization', 'private']).default('organization'),
...
export class SegmentUtils { /**
  ...
  */
  ...
  condition: SegmentCondition,
    behaviorHistory?: BehaviorEvent
  ): boolean { const fieldValue = this.getFieldValue(userAttributes, condition.field, behaviorHistory);
  ...
  */
  ...
  for (const part of fieldParts) { if (value && typeof value === 'object') {
  value = value[part] } else { return undefined;
  return value;
  /**
  ...
  */
  ...
  for (const condition of conditions) { const matches = matchingConditionIds.includes(condition.id);
  ...
  try {
  return eval(result) } catch (e) { console.error('Failed to evaluate complex logic expression:', e);
      return false;
  /**
  ...
  severity: 'medium',
    ...
    // Churn risk analysis
    if (segment.churnRate && segment.churnRate > 0.15) { insights.push({)
  type: 'risk',
  ...
  severity: 'high',
    ...
    // Revenue opportunity
    if (analytics.averageRevenuePerUser > analytics.customerLifetimeValue * 0.8) { insights.push({)
  type: 'opportunity',
