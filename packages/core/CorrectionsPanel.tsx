...
interface CorrectionsPanelProps { isOpen: boolean;
 ...
 = useCorrectionsStore();
  ...
  const handleLoadDefaults = useCallback(() => { if (window.confirm('This will add default correction rules. Continue?')) {
      DEFAULT_CORRECTION_RULES.forEach(rule => addRule(rule)) }, [addRule]);
