 ...
 */
import { AccessibilityManager, AccessibilityNeed, SeverityLevel, FallbackMethod, AssistiveTechnology, InterfaceAdaptation } from '../AccessibilityManager';
describe('AccessibilityManager', () => {
    ...
    describe('Accessibility Compliance Validation', () => {
        ...
        test('should provide user-specific validation for visual impairment', () => {
            ...
            expect(result.issues.some(issue => issue.type === 'critical' &&
                issue.description.includes('audio fallback'))).toBe(true);
    ...
    describe('Emergency Accessibility Bypass', () => {
        test('should create emergency bypass', async () => {
            const bypassId = await manager.createEmergencyBypass('user123', 'User unable to access due to assistive technology failure', 'admin456', 12, // 12 hours
            2 // 2 uses
            );
        ...
        test('should use emergency bypass successfully', async () => {
            const bypassId = await manager.createEmergencyBypass('user123', 'Emergency access needed', 'admin456');
        ...
        test('should reject expired emergency bypass', async () => {
            const bypassId = await manager.createEmergencyBypass('user123', 'Test bypass', 'admin456', -1 // Expired 1 hour ago
            );
            const result = manager.useEmergencyBypass(bypassId, {});
        ...
        test('should reject bypass after maximum usages', async () => {
            const bypassId = await manager.createEmergencyBypass('user123', 'Test bypass', 'admin456', 24, 1 // Only 1 use allowed
            );
    ...
    describe('Interface Adaptation Recommendations', () => {
        ...
        test('should recommend contrast enhancement', async () => {
            ...
            expect(recommendations.adaptations.some(a => a.type === InterfaceAdaptation.CONTRAST_ENHANCEMENT)).toBe(true);
        });
        test('should recommend motion reduction', async () => {
            ...
            expect(recommendations.adaptations.some(a => a.type === InterfaceAdaptation.MOTION_REDUCTION)).toBe(true);
        });
        test('should recommend timeout extension', async () => {
            ...
            expect(recommendations.adaptations.some(a => a.type === InterfaceAdaptation.TIMEOUT_EXTENSION)).toBe(true);
        });
        test('should handle color blindness adaptations', async () => {
            ...
            expect(recommendations.adaptations.some(a => a.type === InterfaceAdaptation.COLOR_ADJUSTMENT)).toBe(true);
        });
