...
export interface IntegratedSecurityMetrics { performanceMetrics: {
    ...
    adminMetrics: { adminOperationMetrics: AdminPerformanceMetric[];
    ...
    securityMetrics: { systemHealth: Map<string, SecuritySystemHealth>;
    ...
    correlations: { performanceSecurityCorrelation: number;
...
export interface IntegratedDashboardData { realTimeStatus: {
        timestamp: number;
    ...
    performanceOverview: { nodeExecutions: {
            total: number;
            successful: number;
            failed: number;
            averageTime: number };
        topPerformingNodes: Array<{ nodeType: string;
            avgTime: number }>;
        bottomPerformingNodes: Array<{ nodeType: string;
            avgTime: number }>;
        memoryTrend: Array<{ timestamp: number;
            usage: number }>;
    };
    adminOverview: { activeAdminSessions: number;
        recentOperations: Array<{
            ...
            success: boolean;
            adminUser: string }>;
        integrationStatus: Array<{ integration: SystemIntegration;
    ...
    securityOverview: {
        ...
        recentSecurityEvents: Array<{ type: SecurityEvent['type'];
            severity: SecurityEvent['severity'];
            timestamp: number;
            source: string }>;
    };
    crossSystemInsights: { correlatedEvents: Array<{
            type: 'performance_security' | 'admin_security' | 'performance_admin';
            description: string;
            confidence: number;
            timestamp: number;
            affectedSystems: string[] }>;
        recommendations: Array<{ category: 'performance' | 'security' | 'admin' | 'compliance';
 ...
 */
export declare class Epic1Epic17SecurityIntegration extends EventEmitter { private config;
    ...
    generateIntegratedReport(timeRange: { )
        ...
        performanceAnalysis: { totalExecutions: number;
            averagePerformance: number;
            performanceTrend: 'improving' | 'stable' | 'degrading';
            topBottlenecks: string[] };
        adminAnalysis: { adminOperationsCount: number;
            complianceScore: number;
            integrationHealthScore: number;
            criticalAdminAlerts: number };
        securityAnalysis: { threatsDetected: number;
            securityScore: number;
            vulnerabilitiesFound: number;
            incidentResponseTime: number };
        correlationAnalysis: { correlatedEvents: number;
            riskFactors: Array<{
                factor: string;
                severity: number }>;
