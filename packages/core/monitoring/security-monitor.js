...
import { AdvancedSecurityAnalyzer } from '../validation/advanced-security';
...
export class SecurityEventMonitor extends EventEmitter {
     ...
     */
    recordEvent(type, severity, source, input, analysis, context = {}, metadata = {}) {
        const event = {
     ...
     */
    recordValidationFailure(input, inputType, source, analysis, context) {
        const severity = analysis.riskScore > 0.8 ? SecurityEventSeverity.HIGH :
            analysis.riskScore > 0.5 ? SecurityEventSeverity.MEDIUM :
                SecurityEventSeverity.LOW;
        return this.recordEvent(SecurityEventType.VALIDATION_FAILURE, severity, source, { raw: input, type: inputType }, analysis, context, { patterns: analysis.threatsDetected });
    }
    /**
     * Record injection attempt
     */
    recordInjectionAttempt(input, inputType, source, detectedPatterns, context) {
        const analysis = {
            isSecure: false,
            riskScore: 0.9,
        ...
        return this.recordEvent(SecurityEventType.INJECTION_ATTEMPT, SecurityEventSeverity.CRITICAL, source, { raw: input, type: inputType }, analysis, context, { injectionPatterns: detectedPatterns });
    }
    /**
     * Record anomalous activity
     */
    recordAnomaly(description, source, riskScore, metadata = {}) {
        const analysis = {
        ...
        const severity = riskScore > 0.8 ? SecurityEventSeverity.HIGH :
            riskScore > 0.5 ? SecurityEventSeverity.MEDIUM :
                SecurityEventSeverity.LOW;
        return this.recordEvent(SecurityEventType.ANOMALY_DETECTED, severity, source, { raw: description, type: 'anomaly' }, analysis, {}, metadata);
    }
    /**
     * Get recent security events
     */
    getRecentEvents(limit = 100, severity) {
    ...
    removeAlertChannel(channelType) {
        this.alertConfig.channels = this.alertConfig.channels.filter(channel => channel.type !== channelType);
    }
    /**
    ...
    updateRateLimit(event) {
        ...
        if (recentTimestamps.length > this.alertConfig.rateThreshold.events) {
            this.recordEvent(SecurityEventType.RATE_LIMIT_EXCEEDED, SecurityEventSeverity.HIGH, 'rate_limiter', { raw: `Rate limit exceeded: ${recentTimestamps.length} events`, type: 'rate_limit' }, {
                isSecure: false,
                riskScore: 0.8,
                threatsDetected: ['Rate limit exceeded'],
                confidence: 1.0
            }, event.context, { eventCount: recentTimestamps.length, timeWindow: this.alertConfig.rateThreshold.timeWindowMs });
        }
    }
...
export const SecurityMonitoringIntegration = {
    /**
     * Integrate with SecurityValidation
     */
    wrapSecurityValidation: (validationFn, source) => {
        return ((...args) => {
            const input = args[0];
