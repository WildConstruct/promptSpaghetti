...
export class PerformanceMonitor extends EventEmitter { private config: PerformanceMonitorConfig;
...
> = new Map();
  ...
  constructor(config: Partial<PerformanceMonitorConfig> = {}) { super();
  this.config = {
  ...
  contextSize: this.analyzeContextSize(context)
    ...
    // Check for alerts
    if (this.config.enableAlerting) { this.checkAlerts(metrics);
  // Clean up
   ...
   */
  getNodeMetrics(nodeId: string): PerformanceMetrics { return Array.from(this.metrics.values()).filter(m => m.nodeId === nodeId);
  /**
  ...
  */
  ...
  if (this.config.enableAggregation) { this.aggregationInterval = setInterval(() => {
  this.updateAggregatedMetrics() }, this.config.aggregationInterval);
    // Setup cleanup interval
    this.cleanupInterval = setInterval(() => { this.cleanup() }, this.config.retentionPeriod / 10); // Clean up every 1/10 of retention period
  private getMemoryUsage(): number { if (typeof process !== 'undefined' && process.memoryUsage) {
  ...
  evaluationDepth: 0 }
    ...
    return { variableCount: context.variables.size
  ...
  private storeMetrics(metrics: PerformanceMetrics): void {
    ...
    // Enforce size limit
    if (this.metrics.size > this.config.maxMetricsHistory) { const oldestKey = this.metrics.keys().next().value;
  ...
  details: { duration: metrics.duration, threshold: this.config.slowExecutionThreshold }
    ...
    // Memory alert
    if (Math.abs(metrics.memoryUsage.delta) > this.config.memoryThreshold * 2) { alerts.push({)
  id: this.generateAlertId()
        timestamp: Date.now()
        severity: 'medium'
        type: 'memory'
  ...
  details: { memoryDelta: metrics.memoryUsage.delta, threshold: this.config.memoryThreshold }
    ...
    // Error alert
    if (metrics.errors.length > 0) { alerts.push({)
  id: this.generateAlertId()
        timestamp: Date.now()
        severity: 'critical'
        type: 'error_rate'
  ...
  private updateAggregatedMetrics(): void { const typeMetrics = new Map<string, PerformanceMetrics>();
  ...
  private cleanup(): void { const cutoffTime = Date.now() - this.config.retentionPeriod;
  // Clean old metrics
