...
import { apm, WorkflowMetric } from './APMService';
...
export class WorkflowMonitor {
   ...
   */
  private initializeCriticalWorkflows(): void {
  ...
  async executeWorkflow(
    ...
    const stepResults: Array<{ name: string; duration: number; success: boolean; error?: string }> = [];
    ...
    const workflowMetric: WorkflowMetric = {
   ...
   */
  createTimingHelper(workflowName: string): {
    startStep: (stepName: string) => void;
    endStep: (stepName: string, success?: boolean) => void;
    finish: (success?: boolean) => WorkflowMetric;
  } {
    const steps: Array<{ name: string; startTime: number; duration?: number; success: boolean }> = [];
    const workflowStartTime = performance.now();
    let currentStep: { name: string; startTime: number } | null = null;
    ...
    return {
      ...
      endStep: (stepName: string, success: boolean = true) => {
 ...
 */
export function monitorWorkflow(workflowName?: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    const originalMethod = descriptor.value;
    const name = workflowName || `${target.constructor.name}.${propertyKey}`;
    
    descriptor.value = async function (...args: any[]) {
      return workflowMonitor.monitorCustomWorkflow(
        name,
