...
export class MonitoringService extends EventEmitter {
   ...
   */
  getMetricStats(name: string, windowMinutes: number = 60): {
    avg: number;
    min: number;
    max: number;
    p50: number;
...
export class AnalyticsService {
  ...
  getFunnelConversion(
    ...
    const funnelEvents = this.events.filter(
      e => e.name.startsWith(`funnel.${funnelName}.`) && e.timestamp > cutoff
    );
...
export class ErrorTracker {
   ...
   */
  getErrorStats(windowHours: number = 24): {
    total: number;
