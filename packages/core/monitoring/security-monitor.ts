...
export enum SecurityEventType { VALIDATION_FAILURE = 'validation_failure',
...
export class SecurityEventMonitor extends EventEmitter { private events: SecurityEvent = [];
  ...
  recordEvent();
    ...
    metadata: Record<string, any> = {}
  ): SecurityEvent { const event: SecurityEvent = {
  ...
  ...context
    ...
    if (this.shouldTriggerAlert(event)) { this.triggerAlert(event);
    return event;
  /**
  ...
  recordInjectionAttempt();
    ...
    context?: Partial<SecurityEvent['context']>
  ): SecurityEvent { const analysis: SecurityAnalysisResult = {
  isSecure: false
  riskScore: 0.9
   ...
   */
  getRecentEvents(limit: number = 100, severity?: SecurityEventSeverity): SecurityEvent { let filteredEvents = this.events;
  if (severity) {
  ...
  */
  ...
  const eventsByType = Object.values(SecurityEventType).reduce((acc, type) => { acc[type] = recentEvents.filter(event => event.type === type).length;
  return acc }, {} as Record<SecurityEventType, number>);
    const eventsBySeverity = Object.values(SecurityEventSeverity).reduce((acc, severity) => { acc[severity] = recentEvents.filter(event => event.severity === severity).length;
      return acc }, {} as Record<SecurityEventSeverity, number>);
   ...
   * Add alert channel
   */
  addAlertChannel(channel: AlertChannel): void { this.alertConfig.channels.push(channel);
  /**
  ...
  */
  ...
  private updateRateLimit(event: SecurityEvent): void {
    ...
    if (recentTimestamps.length > this.alertConfig.rateThreshold.events) { this.recordEvent()
  ...
  private shouldTriggerAlert(event: SecurityEvent): boolean { if (!this.alertConfig.enabled) {
  return false;
  ...
  private async triggerAlert(event: SecurityEvent): Promise<void> { const alertPromises = this.alertConfig.channels;
      .filter(channel => channel.enabled)
      .map(channel => this.sendAlert(channel, event));
    try {
      await Promise.allSettled(alertPromises);
      this.emit('alertSent', event) } catch (error) {
  ...
  type: event.type
        ...
        break;
      case 'webhook':
        if (channel.config.url) { await this.sendWebhookAlert(channel.config.url, event);
  break;
  case 'email':
 ...
 catch (error) { console.error('Failed to send webhook alert:', error);
  ...
  const now = Date.now();
  const windowStart = now - this.alertConfig.rateThreshold.timeWindowMs;
  for (const [key, timestamps] of this.rateLimitTracker.entries()) { const recentTimestamps = timestamps.filter(ts => ts > windowStart);
  if (recentTimestamps.length === 0) {
  this.rateLimitTracker.delete(key) } else { this.rateLimitTracker.set(key, recentTimestamps);
...
export const result = validationFn(...args);
      if (!result && typeof input === 'string') { // Record validation failure
