...
describe('ConversionDataRelationshipManager', () => { let manager: ConversionDataRelationshipManager;
  ...
  describe('Event Enrichment', () => { it('should enrich conversion event with full entity relationships', async () => {
    ...
    it('should handle events without template context', async () => { const eventWithoutTemplate = {
  ...mockBaseEvent,
  properties: {,
  ...
  stepId: 'profile-setup' }
    ...
    it('should properly infer property types', async () => { const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should calculate profile completeness correctly', async () => { const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should categorize referrers correctly', async () => {
      ...
      for (const testCase of testCases) { const eventWithReferrer = {
  ...mockBaseEvent,
  metadata: {,
  ...
  describe('User Entity Management', () => { it('should create default user entity for new users', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should update user profile information', () => { const userId = 'user-456';
  ...
  deviceHistory: ['device-1', 'device-2'] }
      ...
      const eventForUpdatedUser = { ...mockBaseEvent }
        userId
      };
  ...
  describe('Template Entity Management', () => { it('should create default template entity for new templates', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
      expect(enrichedEvent.templateContext?.templateId).toBe('tpl-character-dev-001');
    ...
    it('should derive template conversion rate correctly', async () => { const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
  ...
  describe('Funnel Context Building', () => { it('should build funnel context correctly', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
      expect(enrichedEvent.funnelContext.funnelId).toBe('marketplace-discovery');
      expect(enrichedEvent.funnelContext.stepId).toBe('template-purchase');
      expect(enrichedEvent.funnelContext.stepOrder).toBe(4);
      ...
      expect(enrichedEvent.funnelContext.timeInFunnel).toBeGreaterThan(0) });
    it('should handle missing funnel properties gracefully', async () => { const eventWithoutFunnel = {
  ...mockBaseEvent,
  properties: {,
  templateId: 'tpl-001' }
};
  ...
  describe('Session Context Building', () => { it('should build session context correctly', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should handle missing device fingerprint', async () => { const eventWithoutFingerprint = {
  ...mockBaseEvent,
  ...
  describe('Validation Integration', () => { it('should include validation results in enriched event', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should handle validation errors gracefully', async () => { // This test would require modifying the validation logic to simulate errors
  ...
  describe('Property Metadata', () => { it('should set correct metadata for all flexible properties', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
    ...
    it('should assign correct confidence scores by source', async () => { const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
  ...
  describe('Schema Versioning', () => { it('should include schema version in enriched events', async () => {
      const enrichedEvent = await manager.enrichConversionEvent(mockBaseEvent);
      expect(enrichedEvent.schemaVersion).toBe('1.0.0') });
    it('should be backward compatible with different schema versions', async () => { // Test that enrichment works regardless of input event structure
      const minimalEvent = {
        id: 'minimal-event',
        userId: 'user-minimal',
 ...
 as EnhancedConversionEvent;
  ...
  describe('Error Handling', () => { it('should handle missing required fields gracefully', async () => {
  ...
  sessionId: '' // Empty session ID }
    ...
    it('should handle null/undefined property values', async () => { const eventWithNulls = {
  ...mockBaseEvent,
  value: null,
  properties: {,
