...
export class AttributionTracker extends EventEmitter {
    ...
    async mergeJourneys(sourceJourneyId, targetJourneyId) {
    ...
    async calculateAttribution(conversionId, modelId) {
    ...
    async getAttributionReport(timeRange, options = {}) {
    ...
    async updateAttributionModel(modelId, updates) {
    ...
    async removeAttributionModel(modelId) {
    ...
    async linkDevices(deviceIds, userId) {
        ...
        for (const deviceId of deviceIds) {
            const journeys = Array.from(this.journeys.values()).filter(journey => journey.deviceId === deviceId);
    ...
    async deleteUserData(userId) {
        ...
        const userJourneys = Array.from(this.journeys.values()).filter(journey => journey.userId === userId);
        // Delete journeys
    ...
    async getConversionPaths(timeRange, options = {}) {
        const journeys = this.getJourneysInRange(timeRange);
        return this.analyzeConversionPaths(journeys, options);
    }
    async getAttributionInsights(timeRange) {
        const journeys = this.getJourneysInRange(timeRange);
        const conversions = this.getConversionsInRange(timeRange);
    ...
    computeAttribution(journey, conversion, model) {
    ...
    computeTimeDecayAttribution(touchPoints, model, conversion) {
    ...
    computeCustomAttribution(touchPoints, model, conversion) {
    ...
    getJourneysInRange(timeRange) {
        return Array.from(this.journeys.values()).filter(journey => journey.timeline.firstTouch >= timeRange.start &&
    ...
    async generateAttributionReport(journeys, conversions, options) {
        return {
            id: 'report_' + Date.now(),
