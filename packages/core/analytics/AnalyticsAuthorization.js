...
export class AnalyticsAuthorizationService {
     ...
     */
    async authorizeEventPublication(event, authContext) {
        try {
     ...
     */
    async authorizeEventAccess(event, authContext) {
        try {
     ...
     */
    async authorizeAnalyticsQuery(filter, authContext) {
        try {
     ...
     */
    async authorizeDashboardAccess(dashboardType, authContext) {
        try {
            ...
            const hasAccess = permissions.every(permission => this.hasPermission(authContext, permission));
            return {
                allowed: hasAccess,
     ...
     */
    async applyPolicies(event, authContext, action) {
        const appliedRules = [];
        let finalResult = { allowed: false };
     ...
     */
    ruleMatches(rule, event, authContext) {
        ...
        if (condition.requiredPermissions) {
            const hasAllPermissions = condition.requiredPermissions.every(permission => this.hasPermission(authContext, permission));
            if (!hasAllPermissions)
                return false;
        }
        // Check required roles
        if (condition.requiredRoles) {
            const hasRequiredRole = condition.requiredRoles.some(role => authContext.roles.includes(role));
            if (!hasRequiredRole)
                return false;
        }
        return true;
     ...
     * Filter event fields
     */
    filterFields(event, fields, mode) {
     ...
     */
    async createAuthContextFromToken(token) {
        if (!this.authService) {
     ...
     */
    getAuthorizationSummary(authContext) {
        return {
