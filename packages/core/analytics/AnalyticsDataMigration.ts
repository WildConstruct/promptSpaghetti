...
export class AnalyticsDataMigrationService {
  ...
  */
   ...
   */
  private updateMigrationStatus(migrationId: string, status: MigrationStatus, message?: string): void {
   ...
   */
  private updateCurrentOperation(migrationId: string, operation: string): void {
    const progress = this.activeMigrations.get(migrationId);
    if (progress) {
   ...
   */
  private updateProgress(migrationId: string, progressUpdate: Partial<MigrationProgress['progress']>): void {
    const progress = this.activeMigrations.get(migrationId);
    if (progress) {
