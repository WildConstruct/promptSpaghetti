...
export abstract class BaseAnalyticsAdapter {
   ...
   * Get adapter status
   */
  getStatus(): { name: string; enabled: boolean; systemName: string } { return {
 ...
 */
export class MainAnalyticsAdapter extends BaseAnalyticsAdapter { constructor(eventBus: UnifiedEventBus) {
  ...
  private getEventType(legacyType: string): AnalyticsEventType { switch (legacyType) {
  ...
  async recordNodeExecution(execution: any): Promise<string | null> { return this.publishEvent({)
  eventType: 'node_execution' }
  ...execution
});
  async recordTokenUsage(usage: any): Promise<string | null> { return this.publishEvent({)
  eventType: 'token_usage' }
  ...usage
});
  async recordUserInteraction(interaction: any): Promise<string | null> { return this.publishEvent({)
 ...
 */
export class IntegrationAnalyticsAdapter extends BaseAnalyticsAdapter { constructor(eventBus: UnifiedEventBus) {
  ...
  async recordIntegrationEvent(event: any): Promise<string | null> { return this.publishEvent(event);
  /**
  ...
  */
  ...
  async recordBehaviorEvent(event: any): Promise<string | null> { return this.publishEvent(event);
  /**
  ...
  */
  ...
  private getPerformanceSeverity(event: any): EventSeverity { if (event.threshold && event.value > event.threshold * 2) return EventSeverity.CRITICAL;
  ...
  */
  ...
  private getSecuritySeverity(riskLevel: string): EventSeverity { switch (riskLevel?.toLowerCase()) {
  ...
  */
  ...
  async recordRevenueEvent(event: any): Promise<string | null> { return this.publishEvent(event);
  /**
...
> {
    ...
    for (const [systemName, events] of Object.entries(migrationData)) { const adapter = this.adapters.get(systemName);
  if (adapter) {
