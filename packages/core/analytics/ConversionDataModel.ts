...
export interface ConversionFunnelDefinition { id: string;
  name: string;
  description: string;
...
export interface EventCriteria { eventType: string;
...
export interface PropertyMatcher { propertyPath: string; // Dot notation: "user.profile.tier";
...
export interface ValueConstraint { field: 'value' | 'timestamp' | 'duration';
  min?: number;
  max?: number;
  exactValues?: number;
...
export interface ContextRequirement { type: 'device' | 'location' | 'session' | 'user_attribute' | 'time_of_day' | 'referrer' }
  condition: string;
  value: any;
...
export interface ConditionLogic { operator: 'AND' | 'OR' | 'NOT';
...
export interface SimpleCondition { field: string;
  operator: string;
  value: any;
...
export interface TimeWindow { start: string; // ISO time format "HH:MM";
...
export interface StepBranch { id: string;
  name: string;
...
export interface ConditionalPath { id: string;
  name: string;
  description: string;
...
export interface SuccessCriteria { primary: { }
  stepId: string;
...
export interface SegmentationRule { id: string;
  name: string;
  description: string;
...
export interface BenchmarkData { industryAverageConversionRate: number;
...
export interface CompetitorBenchmark { name: string;
...
export interface FlexibleConversionEvent extends EnhancedConversionEvent { // Flexible properties with schema validation
  flexibleProperties: { }
  ...
  templateContext?: { templateId: string;
  templateType: string;
  creatorId: string;
  category: string;
  price: number;
...
export interface FlexibleProperty { value: any;
  type: PropertyType;
...
export interface PropertySchema { type: PropertyType;
  required: boolean;
...
export interface PropertyConstraint { type: 'format' | 'range' | 'length' | 'pattern' | 'custom';
  value: any;
  errorMessage: string;
...
export interface PropertyTransformation { type: 'normalize' | 'encode' | 'hash' | 'encrypt' | 'custom' }
...
export interface PropertyRelationship { type: 'depends_on' | 'conflicts_with' | 'derives_from' | 'validates_against' }
...
export interface ValidationError { propertyPath: string;
  constraint: string;
  message: string;
...
export interface ValidationWarning { propertyPath: string;
  issue: string;
  message: string;
  impact: string;
...
export interface ConversionCohort { id: string;
  name: string;
  description: string;
  // Cohort definition
  definition: {;
...
export interface MetricCalculation { id: string;
  name: string;
...
export interface TimeSeriesData { timestamp: number;
  value: number;
  metadata?: Record<string, any> }
...
export interface ValueMetrics { totalRevenue: number;
...
export interface FunnelSegmentMetrics { funnelId: string;
  conversionRate: number;
  averageTimeToConvert: number;
...
export interface UserEntity { id: string;
  // Basic information
  profile: {;
  email?: string;
  name?: string;
...
export interface TemplateEntity { id: string;
  // Basic information
  metadata: { }
  name: string;
  description: string;
...
export interface ConversionSummary { funnelId: string;
  totalConversions: number;
  averageTimeToConvert: number;
  averageValue: number;
  lastConversionDate: number;
...
export interface DevicePreference { deviceType: string;
...
export interface ActivityPattern { type: 'temporal' | 'behavioral' | 'contextual';
  pattern: string;
  frequency: number;
  strength: number;
...
export interface SegmentChange { date: number;
  fromSegment: string;
  toSegment: string;
  reason: string;
  triggerEvent?: string }
...
export interface PrivacySettings { trackingConsent: boolean;
...
export interface CommunicationPreference { channel: 'email' | 'sms' | 'push' | 'in_app' }
...
export interface ContentPreference { category: string;
...
export interface NotificationSettings { marketing: boolean;
  product: boolean;
  security: boolean;
  social: boolean;
  quietHours?: { }
...
export interface TemplateFunnelMetrics { funnelId: string;
  views: number;
  conversions: number;
  conversionRate: number;
  averageTimeToConvert: number;
...
export interface PriceChange { date: number;
  oldPrice: number;
  newPrice: number;
  reason: string;
...
export interface SeasonalityData { pattern: 'weekly' | 'monthly' | 'quarterly' | 'yearly';
  peaks: Array<{ }
  period: string;
  multiplier: number;
...
export class ConversionDataRelationshipManager { private userCache: Map<string, UserEntity> = new Map();
  ...
  */
   ...
   */
  private async getUserEntity(userId: string): Promise<UserEntity> { let user = this.userCache.get(userId);
    if (!user) {
  ...
  user: UserEntity }
    ...
    Object.entries(event.properties || {}).forEach(([key, value]) => { properties[key] = {
  value
  ...
  validationStatus: 'valid' }
    ...
    properties['user_lifetime_value'] = { value: user.value.lifetimeValue
  type: 'number'
  metadata: {
  source: 'derived'
  confidence: 0.9
  ...
  validationStatus: 'valid' }
    ...
    if (template) { properties['template_conversion_rate'] = {
  ...
  validationStatus: 'valid' }
    ...
    return properties;
  private buildUserContext(user: UserEntity) { return {
  ...
  private buildTemplateContext(template: TemplateEntity) { return {
  ...
  private buildSessionContext(event: EnhancedConversionEvent, user: UserEntity) { return {
  ...
  // Helper methods
  private createDefaultUserEntity(userId: string): UserEntity { return {
  id: userId
  profile: {
  ...
  private createDefaultTemplateEntity(templateId: string): TemplateEntity { return {
      id: templateId
      metadata: { }
  ...
  private inferPropertyType(value: any): PropertyType { if (typeof value === 'string') return 'string';
