...
export class MedievalDemo { private metadataManager: NodeMetadataManager;
   ...
   */
  private async loadMaterialDatabase(): Promise<void> { const materialData: MedievalMaterial = [
  {
  id: 'wool_rough'
  name: 'Rough Wool'
  type: 'fabric'
   ...
   */
  private async loadLocationDatabase(): Promise<void> { const locationData: MedievalLocation = [
  {
   ...
   */
  private async loadCharacterDatabase(): Promise<void> { const characterData: MedievalCharacter = [
  {
  id: 'sir_gareth'
  name: 'Sir Gareth'
   ...
   */
  private async loadSceneDatabase(): Promise<void> { const sceneData: MedievalScene = [
  {
  ...
  timeOfDay?: string;
  season?: string;
  theme?: string } = {}): MedievalScene | null { const availableScenes = Array.from(this.sceneDatabase.values());
  ...
  if (options.socialClass && !scene.characters.some(char => )
      ...
      if (options.season && scene.timeContext.season !== options.season) {
        return false;
      return true });
    if (filteredScenes.length === 0) { filteredScenes = availableScenes;
...
}): MedievalClothing { const availableClothing = Array.from(this.clothingDatabase.values());
...
manuscript illustrations, and documented practices of the time.
   ...
   */
  public async integrateWithMetadata(nodeId: string, sceneId: string): Promise<void> { const scene = this.sceneDatabase.get(sceneId);
    if (!scene) return (
    ...
    if (scene.narrativeHooks.some(hook => hook.includes('romance'))) { await this.metadataManager.setNodeGenre(nodeId, 'romance') } else if (scene.activities.some(activity => activity.includes('war') || activity.includes('battle'))) { await this.metadataManager.setNodeGenre(nodeId, 'adventure') } else { await this.metadataManager.setNodeGenre(nodeId, 'historical_fiction');
  /**
