...
interface PerformanceMetrics { responseTime: number;
    throughput: number;
    errorRate: number;
...
interface ResourceUsage { memory: number;
    cpu: number;
    disk: number;
    network: number;
...
export declare export interface OptimizationCategories { categories: {
        caching: {
            score: number;
            recommendations: string[] }
}
