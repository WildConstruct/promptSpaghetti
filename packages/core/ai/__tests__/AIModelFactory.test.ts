...
describe('AIModelFactory', () => { let factory: AIModelFactory;
  let mockFetch: jest.MockedFunction<typeof fetch>;
  beforeEach(() => {
  factory = new AIModelFactory();
  mockFetch = fetch as jest.MockedFunction<typeof fetch>;
  mockFetch.mockClear() });
  afterEach(() => { factory.destroyAllModels() });
  describe('Factory Configuration', () => { test('should create factory with default configuration', () => {
      const config = factory.getFactoryConfig();
      expect(config.defaultTimeout).toBe(30000);
      expect(config.defaultRetries).toBe(3);
      expect(config.enableLogging).toBe(true);
      expect(config.logLevel).toBe('info') });
    test('should create factory with custom configuration', () => { const customConfig: FactoryConfig = {
  ...
  describe('Supported Types and Default Configurations', () => { test('should return all supported AI model types', () => {
    ...
    test('should provide default configuration for text models', () => { const config = factory.getDefaultConfiguration(AIModelType.TEXT);
    ...
    test('should provide default configuration for image models', () => { const config = factory.getDefaultConfiguration(AIModelType.IMAGE);
    ...
    test('should provide default configuration for audio models', () => { const config = factory.getDefaultConfiguration(AIModelType.AUDIO);
      expect(config.type).toBe(AIModelType.AUDIO);
      expect(config.provider).toBe(AIModelProvider.OPENAI);
      expect(config.modelName).toBe('tts-1');
      expect(config.parameters?.voice).toBe('alloy') });
    test('should provide default configuration for multimodal models', () => { const config = factory.getDefaultConfiguration(AIModelType.MULTIMODAL);
      expect(config.type).toBe(AIModelType.MULTIMODAL);
      expect(config.provider).toBe(AIModelProvider.OPENAI);
      expect(config.modelName).toBe('gpt-4-vision-preview') });
  });
  describe('OpenAI Model Creation', () => { beforeEach(() => {
 ...
 as Response);
    ...
    test('should create OpenAI model successfully', async () => { const config: ModelConfiguration = {
  id: 'test-openai'
  ...
  apiKey: 'test-key' }
    ...
    test('should fail to create OpenAI model without API key', async () => { const config: ModelConfiguration = {
  ...
  describe('Anthropic Model Creation', () => { beforeEach(() => {
 ...
 as Response)
    ...
    test('should create Anthropic model successfully', async () => { const config: ModelConfiguration = {
  id: 'test-anthropic'
  type: AIModelType.TEXT
  ...
  describe('Local Model Creation', () => { beforeEach(() => {
 ...
 as Response);
        if (urlString.includes('/api/show')) { return Promise.resolve({)
  ok: true
            json: () => Promise.resolve({);
 ...
 as Response);
    ...
    test('should create local model successfully', async () => { const config: ModelConfiguration = {
  id: 'test-local'
  type: AIModelType.TEXT
  ...
  describe('Custom HTTP Model Creation', () => { beforeEach(() => {
 ...
 as Response)
    ...
    test('should create custom HTTP model successfully', async () => { const registration: ModelRegistration = {
  id: 'test-custom'
  ...
  describe('Model Registration and Management', () => { test('should register and retrieve model', () => {
  ...
  id: 'registered-model'
    ...
    test('should unregister model', () => { const registration: ModelRegistration = {
  id: 'temp-model'
        provider: AIModelProvider.CUSTOM
        modelName: 'temp' }
        config: {}
    ...
    test('should get model by ID', async () => { // Mock for model creation
 ...
 as Response);
  ...
  apiKey: 'test-key' }
    ...
    test('should return null for non-existent model', async () => { const model = await factory.getModel('non-existent');
      expect(model).toBeNull() });
  });
  describe('Batch Operations', () => { beforeEach(() => {
      mockFetch.mockResolvedValue({)
  ok: true }
        json: () => Promise.resolve({ data: [] })
 as Response);
    });
    test('should create multiple models', async () => { const configs: ModelConfiguration = [
  {
  id: 'batch-1'
  type: AIModelType.TEXT
  provider: AIModelProvider.OPENAI
  ...
  expect(models[1].id).toBe('batch-2');
    ...
    test('should test all models', async () => { // Create test models first
  const config: ModelConfiguration = {
  id: 'test-health'
  type: AIModelType.TEXT
  provider: AIModelProvider.OPENAI
  ...
  describe('Statistics and Monitoring', () => { test('should provide factory statistics', async () => {
 ...
 as Response);
  ...
  apiKey: 'test-key' }
    ...
    test('should get model metadata', async () => { // Mock for model creation
      mockFetch.mockResolvedValue({)
  ok: true }
        json: () => Promise.resolve({ data: [] })
 as Response);
  ...
  describe('Error Handling', () => { test('should handle unsupported provider', async () => {
  const config: ModelConfiguration = {
  id: 'unsupported'
  type: AIModelType.TEXT
  ...
  apiKey: 'test-key' }
    ...
    test('should handle network errors during model creation', async () => { mockFetch.mockRejectedValue(new Error('Network error'));
  const config: ModelConfiguration = {
  id: 'network-error'
  type: AIModelType.TEXT
  provider: AIModelProvider.OPENAI
  apiKey: 'test-key' }
    ...
    test('should handle API errors during model creation', async () => { mockFetch.mockResolvedValue({)
  ok: false
        status: 401
 ...
 as Response);
  ...
  describe('Model Lifecycle', () => { test('should destroy individual model', async () => {
      mockFetch.mockResolvedValue({)
  ok: true }
        json: () => Promise.resolve({ data: [] })
 as Response);
  ...
  apiKey: 'test-key' }
    ...
    test('should destroy all models', async () => { mockFetch.mockResolvedValue({)
  ok: true }
        json: () => Promise.resolve({ data: [] })
 as Response);
