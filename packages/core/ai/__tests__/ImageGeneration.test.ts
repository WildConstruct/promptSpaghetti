...
describe('Image Generation System', () => {
  ...
  describe('DALL-E Adapter', () => {
  let dalleAdapter: DALLEAdapter;
  beforeEach(() => {
  ...
  ok: true,
    ...
    test('should initialize DALL-E adapter successfully', async () => {
      await dalleAdapter.initialize();
  ...
  let midjourneyAdapter: MidjourneyAdapter;
  beforeEach(() => {
  ...
  ok: true,
    ...
    test('should initialize Midjourney adapter successfully', async () => {
      await midjourneyAdapter.initialize();
      expect(midjourneyAdapter.status).toBe(AIModelStatus.READY);
      expect(midjourneyAdapter.metadata.provider).toBe('midjourney');
    });
    test('should generate image with Midjourney', async () => {
  await midjourneyAdapter.initialize();
  ...
  stylize: 250,
    ...
    test('should handle prompt templating correctly', async () => {
  await midjourneyAdapter.initialize();
  ...
  describe('Stable Diffusion Adapter', () => {
  let stableDiffusionAdapter: StableDiffusionAdapter;
  beforeEach(() => {
  ...
  ok: true,
    ...
    test('should initialize Stable Diffusion adapter successfully', async () => {
    ...
    test('should load available models and samplers', async () => {
      await stableDiffusionAdapter.initialize();
    ...
    test('should generate image with Stable Diffusion', async () => {
  await stableDiffusionAdapter.initialize();
  ...
  removeEventListener: jest.fn(),
    ...
    test('should extract metadata from data URL', async () => {
  ...
  mockImage.onload();
    ...
    test('should process image batches', async () => {
  const testImages = [;
  ...
  outputFormat: 'jpeg',
    ...
    test('should compare image similarity', async () => {
  const image1 = 'data:image/png;base64,test1';
  ...
  describe('Image Generation Workflow Nodes', () => {
  test('should create and execute ImageGenerationNode', async () => {
  ...
  seed: 12345,
    ...
    test('should validate image generation inputs', async () => {
  const node = new ImageGenerationNode('test-node', {)
  provider: 'dalle',
  apiKey: 'test-key',
});
  ...
  seed: -1 // Negative seed,
    ...
    test('should create and execute ImageVariationNode', async () => {
  const node = new ImageVariationNode('test-variation', {)
  provider: 'dalle',
  apiKey: 'test-key',
});
      const inputs = {
  ...
  count: 2,
    ...
    test('should create and execute ImageUpscaleNode', async () => {
  const node = new ImageUpscaleNode('test-upscale');
  const inputs = {
  getString: jest.fn().mockImplementation((key, defaultValue) => {,
  const values: Record<string, string> = {,
  ...
  ok: true,
    ...
    test('should validate configuration parameters', () => {
      ...
      expect(() => {
  new StableDiffusionAdapter('invalid', {)
  endpoint: '',
  apiType: 'automatic1111',
});
      }).not.toThrow(); // Constructor shouldn't throw, initialization should
