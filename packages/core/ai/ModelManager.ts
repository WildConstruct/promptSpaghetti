...
export class ModelCache {
  ...
  constructor(config: CacheConfig) { this.cache = new Map();
  this.config = config;
  ...
  delete(modelId: string): boolean { const entry = this.cache.get(modelId);
    if (entry) {
  ...
  getStats(): unknown {
    ...
    for (const [modelId, entry] of this.cache) { stats.models.push({)
  id: modelId,
  ...
  status: entry.model.status }
    ...
    return stats;
  private _evictModel(): void { if (this.cache.size === 0) return;
  ...
  this._cleanupExpiredEntries() }, 60000);
  private _cleanupExpiredEntries(): void { const now = Date.now();
  ...
  removeModel(modelId: string): void { this.models = this.models.filter(m => m.id !== modelId);
  ...
  if (error) {
  metrics.errorRate = (metrics.errorRate * (metrics.totalRequests - 1) + 1) / metrics.totalRequests } else { metrics.errorRate = (metrics.errorRate * (metrics.totalRequests - 1)) / metrics.totalRequests;
  ...
  return models.reduce((best, current) => { const bestMetrics = this.metrics.get(best.id);
  ...
  private _selectByResponseTime(models: BaseAIModel): BaseAIModel { return models.reduce((best, current) => {
  ...
  private _selectByCost(models: BaseAIModel, request: AIRequest): BaseAIModel { return models.reduce((best, current) => {
  const bestMetrics = this.metrics.get(best.id);
  const currentMetrics = this.metrics.get(current.id);
  const bestCost = bestMetrics?.costPerRequest || Infinity;
  const currentCost = currentMetrics?.costPerRequest || Infinity;
  return currentCost < bestCost ? current : best });
  private _selectByCapability(models: BaseAIModel, request: AIRequest): BaseAIModel { // Score models based on capability match
  ...
  this._performHealthChecks() }, this.checkInterval);
  stop(): void { if (this.intervalId) {
  ...
  removeModel(modelId: string): void { this.healthChecks.delete(modelId);
  ...
  const checkPromises = Array.from(this.healthChecks.keys()).map(async (modelId) => { // In a real implementation, we would get the model instance and call its health method
...
export class ModelManager {
  ...
  async createModelPool(poolId: string, registrations: ModelRegistration): Promise<ModelPool> { const models: BaseAIModel = [];
  ...
  this.cache.set(registration.id, model) } catch (error) {
        console.warn(`Failed to create model ${registration.id}:`, error);}
    // Warm up models if enabled
    if (this.warmupStrategy.enabled) { await this._warmupModels(models);
  ...
  async processRequest(poolId: string, request: AIRequest): Promise<AIResponse> {
    ...
    if (!model) { throw new Error('No available models in pool');
