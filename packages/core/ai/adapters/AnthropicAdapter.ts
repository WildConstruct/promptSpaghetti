...
export class AnthropicAdapter extends BaseAIModel { private config: AnthropicConfig;
  ...
  if (typeof msg === 'string') {
        ...
        return { role: msg.role === 'assistant' ? 'assistant' : 'user'
  content: msg.content }
};
      });
  ...
  private _extractContent(response: AnthropicResponse): unknown { const textContent = response.content;
  ...
  private _estimateTokenCount(messages: ClaudeMessage): number { // Rough estimation: 4 characters per token for Claude }
