...
export interface StableDiffusionConfig { endpoint: string; // API endpoint (e.g., Automatic1111, ComfyUI, or hosted service);
...
export interface StableDiffusionRequestOptions { // Core parameters
  prompt: string;
...
export interface StableDiffusionResponse { images: string; // Base64 encoded images }
...
export class StableDiffusionAdapter extends BaseAIModel { private config: StableDiffusionConfig;
  ...
  return this._processGenerationResponse(response, prompt, processedOptions, generationTime) } catch (error) { throw new ModelProcessingError(this._id, error instanceof Error ? error.message : 'Unknown error');
  ...
  ): Promise<StableDiffusionGenerationResult> { const img2imgOptions: StableDiffusionRequestOptions = {
  prompt
  ...
  mask: string
    prompt: string
    options?: Partial<StableDiffusionRequestOptions>
  ): Promise<StableDiffusionGenerationResult> { const inpaintOptions: StableDiffusionRequestOptions = {
  prompt
  init_image: initImage
  mask
  ...
  static getEstimatedCost(apiType: string): number { const costs: Record<string, number> = {
  ...
  try { if (this.config.apiType === 'automatic1111') {
  const response = await this._makeRequest('/sdapi/v1/samplers', 'GET');
  this.availableSamplers = response.map((sampler: any) => sampler.name) } else { // Default samplers for other API types
 ...
 catch (error) { console.warn('Failed to load available samplers:', error);
  ...
  private async _generateImages(options: StableDiffusionRequestOptions): Promise<StableDiffusionResponse> { let endpoint: string;
  let payload: any;
  switch (this.config.apiType) {
  case 'automatic1111':
  ...
  enable_hr: options.enable_hr || false }
    ...
    if (options.init_image) { payload.init_images = [options.init_image];
  ...
  private _buildGenericPayload(options: StableDiffusionRequestOptions): any { return {
  prompt: options.prompt
  negative_prompt: options.negative_prompt
  width: options.width
  height: options.height
  ...
  private _extractSeedFromResponse(response: StableDiffusionResponse, index: number): number { // Try to extract seed from response info
    try {
      if (response.info) {
        const info = JSON.parse(response.info);
        return info.seed || info.all_seeds?.[index] || -1 } catch {
