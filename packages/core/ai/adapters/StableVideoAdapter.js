...
export class StableVideoAdapter extends BaseAIModel {
    ...
    async generateImageToVideo(imageData, motionIntensity = 127, numFrames = 14, options) {
        const svdOptions = {
            image: imageData,
    ...
    async generateVideoLoop(imageData, options) {
        const loopOptions = {
            image: imageData,
            loop_video: true,
    ...
    async generateHighQualityVideo(imageData, options) {
        const hqOptions = {
            image: imageData,
            model: 'svd-xt-1-1',
            num_frames: 25,
    ...
    async enhanceVideoQuality(frames, upscaleFactor = 2) {
        ...
        try {
            if (this.config.apiType === 'automatic1111') {
                ...
                if (response.ok) {
                    ...
                    const videoModels = models.filter((model) => model.title.toLowerCase().includes('svd') ||
                        model.title.toLowerCase().includes('video'));
                    if (videoModels.length > 0) {
                        this.availableModels = videoModels.map((model) => ({
                            name: model.title,
