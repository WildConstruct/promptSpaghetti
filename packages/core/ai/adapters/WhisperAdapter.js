...
export class WhisperAdapter extends BaseAIModel {
    ...
    async transcribeFile(file, language, options) {
    ...
    async translateToEnglish(file, options) {
        const whisperOptions = {
            file,
            task: 'translate',
            ...options
    ...
    async transcribeWithTimestamps(file, granularity = 'segment', options) {
        const timestamp_granularities = granularity === 'both' ? ['word', 'segment'] : [granularity];
        const whisperOptions = {
            file,
    ...
    async _transcribeAudio(file, options) {
        const formData = new FormData();
    ...
    _calculateConfidenceScore(transcriptionData) {
        if (transcriptionData.segments && transcriptionData.segments.length > 0) {
            ...
            const avgLogprob = transcriptionData.segments.reduce((sum, segment) => sum + (segment.avg_logprob || 0), 0) / transcriptionData.segments.length;
