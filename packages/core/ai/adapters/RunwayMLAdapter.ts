...
export interface RunwayMLRequestOptions { // Core parameters
  text_prompt: string;
...
export interface RunwayMLTask { id: string;
...
export class RunwayMLAdapter {},
  ...
  ): Promise<RunwayMLGenerationResult> { const runwayOptions: RunwayMLRequestOptions = {
  text_prompt: prompt
  duration
  mode: 'text_to_video' }
  ...options
  ...
  private _processOptions(options?: RunwayMLRequestOptions)
    prompt?: string
  ): Required<Pick<RunwayMLRequestOptions, 'model' | 'duration' | 'resolution' | 'motion' | 'mode' | 'text_prompt'>> & Omit<RunwayMLRequestOptions, 'text_prompt'> & { text_prompt: string } { const defaults = {
  model: 'gen3' as const
  duration: 4
  ...
  private async _createGenerationTask(prompt: string, options: RunwayMLRequestOptions): Promise<RunwayMLTask> { const payload = {
  ...
  if (videoResponse.ok) {
  videoData = await videoResponse.arrayBuffer() } catch (error) { console.warn('Failed to download video data:', error);
