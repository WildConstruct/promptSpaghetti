...
export class MultimodalAdapter extends BaseAIModel {
    ...
    async understandContent(inputs, options) {
    ...
    async compareContent(inputs, comparisonAspects = ['content', 'style', 'emotion', 'quality'], options) {
        const multimodalOptions = {
            inputs,
            task: 'compare',
            cross_reference: true,
    ...
    async describeMultimodal(inputs, detailLevel = 'detailed', options) {
        const multimodalOptions = {
            inputs,
            task: 'describe',
    ...
    async analyzeContent(inputs, analysisTypes = ['entities', 'emotions', 'sentiment', 'topics'], options) {
        const multimodalOptions = {
            inputs,
            task: 'analyze',
    ...
    async summarizeMultimodal(inputs, summaryLength = 'medium', options) {
    ...
    async extractInformation(inputs, extractionTargets = ['text', 'entities', 'objects', 'emotions'], options) {
    ...
    async _processMultimodalContent(inputs, options) {
    ...
    _buildProviderPayload(inputs, options) {
        switch (this.config.provider) {
            case 'openai':
    ...
    _buildOpenAIPayload(inputs, options) {
    ...
    _buildAnthropicPayload(inputs, options) {
    ...
    _buildGooglePayload(inputs, options) {
    ...
    _buildGenericPayload(inputs, options) {
        return {
    ...
    _processProviderResponse(response, inputs, options) {
    ...
    _parseMultimodalResponse(content, inputs) {
    ...
    _extractElements(content, contentType) {
        const elements = [];
    ...
    _extractConnections(content, inputs) {
        const connections = [];
        if (inputs.length > 1) {
    ...
    _extractTopics(content) {
        ...
        Object.entries(topicKeywords).forEach(([topic, keywords]) => {
            const matches = keywords.filter(keyword => new RegExp(`\\b${keyword}\\b`, 'i').test(content));
            if (matches.length > 0) {
                topics.push({
                    topic,
    ...
    async _estimateInputTokens(inputs) {
        let totalTokens = 0;
        for (const input of inputs) {
            switch (input.type) {
                case 'text':
