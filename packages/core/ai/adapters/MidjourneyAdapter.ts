...
export class MidjourneyAdapter extends BaseAIModel { private config: MidjourneyConfig;
  ...
  default: 1 }
    ...
    this.config = { pollInterval: 5000, // 5 seconds
  ...
  async cancelJob(jobId: string): Promise<boolean> {
    ...
    try {
      ...
      return false;
  async upscaleImage(jobId: string, imageIndex: number): Promise<MidjourneyGenerationResult> { try {
  const payload = {
  jobId
  action: 'upscale'
  index: imageIndex }
  ...
  private _parseJobStatus(response: any): MidjourneyJobStatus { return {
