...
export class StableDiffusionAdapter extends BaseAIModel {
    ...
    async inpaint(initImage, mask, prompt, options) {
    ...
    async upscale(image, upscaler = 'ESRGAN_4x', scale = 2) {
        if (this.config.apiType !== 'automatic1111') {
    ...
    async _loadAvailableModels() {
        try {
            if (this.config.apiType === 'automatic1111') {
                ...
                this.availableModels = response.map((model) => ({
                    name: model.title,
    ...
    _processOptions(options, prompt) {
    ...
    _buildAutomatic1111Payload(options) {
        const payload = {
    ...
    _buildStabilityAIPayload(options) {
        return {
            text_prompts: [
                {
                    text: options.prompt,
    ...
    _processGenerationResponse(response, prompt, options, generationTime) {
