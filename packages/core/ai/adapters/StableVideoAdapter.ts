...
export interface StableVideoConfig { endpoint: string; // API endpoint (local, Stability AI, or custom);
...
export interface StableVideoRequestOptions { // Core parameters
...
export interface SVDModelInfo { name: string;
  type: 'svd' | 'svd-xt';
  max_frames: number;
  resolution: string;
  description: string;
...
export class StableVideoAdapter {},
  ...
  ): Promise<StableVideoGenerationResult> { const svdOptions: StableVideoRequestOptions = {
  image: imageData
  ...
  async generateVideoLoop(imageData: string)
    options?: Partial<StableVideoRequestOptions>
  ): Promise<StableVideoGenerationResult> { const loopOptions: StableVideoRequestOptions = {
  image: imageData
  loop_video: true
  ...
  async generateHighQualityVideo(imageData: string)
    options?: Partial<StableVideoRequestOptions>
  ): Promise<StableVideoGenerationResult> { const hqOptions: StableVideoRequestOptions = {
  image: imageData
  model: 'svd-xt-1-1'
  num_frames: 25
  steps: 30
  ...
  async getAvailableModels(): Promise<SVDModelInfo> { return [...this.availableModels];
  ...
  async interpolateFrames(frames: string): Promise<string> { // Frame interpolation to increase smoothness
  ...
  enhancedFrames.push(enhancedFrame) } catch (error) { console.warn('Failed to enhance frame:', error);
  ...
  static getEstimatedLatency(apiType: string): number { const latencies: Record<string, number> = {
  ...
  static getSupportedModels(): SVDModelInfo { return [
  {
  name: 'svd'
  type: 'svd'
  max_frames: 14
  ...
  'Content-Type': 'application/json' }
    ...
    if (this.config.apiKey) {
      if (this.config.apiType === 'stability-ai') {
        ...
        headers['Authorization'] = `Bearer ${this.config.apiKey}`;}
    return headers;
  private async _loadAvailableModels(): Promise<void> { try {
  if (this.config.apiType === 'automatic1111') {
 ...
 catch (error) { console.warn('Failed to load available models:', error);
  ...
  decode_chunk_size: 8 }
    ...
    return processed;
  private async _generateVideo(imageData: string, options: Omit<StableVideoRequestOptions, 'image'>): Promise<any> { let endpoint: string;
  let payload: any;
  switch (this.config.apiType) {
  case 'automatic1111':
  ...
  private _buildStabilityAIPayload(imageData: string, options: Omit<StableVideoRequestOptions, 'image'>): any { return {
  image: imageData
  cfg_scale: options.cfg_scale || 2.5
  motion_bucket_id: options.motion_bucket_id || 127
  seed: options.seed || -1 }
};
  private _buildGenericPayload(imageData: string, options: Omit<StableVideoRequestOptions, 'image'>): any { return {
  image: imageData
  ...
  let frames: string = [];
  if (response.frames) {
  frames = response.frames } else if (response.images) { frames = response.images } else if (response.output) { frames = Array.isArray(response.output) ? response.output : [response.output];
  ...
  const avgFrameSize = frames.reduce((sum, frame) => { const base64Data = frame.includes(',') ? frame.split(',')[1] : frame;
  return sum + ((base64Data.length * 3) / 4) }, 0) / frames.length;
