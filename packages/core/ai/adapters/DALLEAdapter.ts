...
export class DALLEAdapter extends BaseAIModel { private config: DALLEConfig;
  ...
  async generateVariations(imageUrl: string, options?: Partial<DALLERequestOptions>): Promise<ImageGenerationResult> { // Note: Variations are only available for DALL-E 2
  ...
  ): Promise<ImageGenerationResult> { // Note: Image editing is only available for DALL-E 2
    if (this._metadata.name === 'dall-e-3') {
  ...
  static getModelCostPerRequest(modelName: string): number { const costs: Record<string, number> = {
  ...
  private _validateParameters(payload: any): void {
    ...
    // General validations
    if (payload.prompt.length > 4000) { throw new Error('Prompt exceeds maximum length of 4000 characters');
  ...
  protected async _performHealthCheck(): Promise<void> { await this._testConnection();
  ...
  if (options?.style === 'vivid') { optimizedPrompt = this._enhanceForVividStyle(optimizedPrompt);
  styleEnhancements.push('Enhanced for vivid style') } else if (options?.style === 'natural') { optimizedPrompt = this._enhanceForNaturalStyle(optimizedPrompt);
