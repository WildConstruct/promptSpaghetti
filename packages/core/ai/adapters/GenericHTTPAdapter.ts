...
export interface HTTPConfig { baseURL: string;
  apiKey?: string;
...
export interface HTTPRequestOptions { endpoint?: string;
...
export interface HTTPRequestMapping { inputPath: string; // JSONPath for where to place input in request;
...
export interface GenericHTTPResponse { status: number;
  data: unknown;
  headers: Record<string, string>;
  usage?: { }
  input_tokens?: number;
...
export class GenericHTTPAdapter extends BaseAIModel { private config: HTTPConfig;
  ...
  private _buildRequestPayload(input: any, options?: HTTPRequestOptions): any {
    ...
    if (options?.parameters && this.requestMapping.parametersPath) { this._setValueByPath(payload, this.requestMapping.parametersPath, options.parameters);
  return payload;
