...
export interface TTSRequestOptions { // Core parameters
  text: string;
...
export interface VoiceInfo { id: string;
  name: string;
  description: string;
...
export class OpenAITTSAdapter {},
  ...
  async getAvailableVoices(): Promise<VoiceInfo> { return [...this.availableVoices];
  ...
  ): Promise<TTSGenerationResult> { const ttsOptions: TTSRequestOptions = {
  text: ssmlText
  voice: voice as any
  use_ssml: true }
  ...options
  ...
  ): Promise<TTSGenerationResult> { const results: TTSGenerationResult = [];
  ...
  static getModelCostPerCharacter(model: string): number { const costs: Record<string, number> = {
  ...
  private _initializeVoices(): void { this.availableVoices = [
  {
  id: 'alloy'
  name: 'Alloy'
  ...
  private _processOptions(options?: TTSRequestOptions)
    text?: string
  ): Required<Pick<TTSRequestOptions, 'voice' | 'model' | 'response_format' | 'speed' | 'text'>> & Omit<TTSRequestOptions, 'text'> & { text: string } { const defaults = {
  ...
  private _estimateAudioDuration(text: string, speed: number): number { // Rough estimation: average speaking rate is ~150 words per minute
  ...
  private _getBitrate(format: string): number | undefined { const bitrates: Record<string, number> = {
  'mp3': 64000, // 64 kbps
  'opus': 64000
  'aac': 64000 }
};
