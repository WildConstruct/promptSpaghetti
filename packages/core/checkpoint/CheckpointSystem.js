...
export class CheckpointSystem extends EventEmitter {
    ...
    async createCheckpoint(state, metadata = {}, options = {}) {
    ...
    async restoreCheckpoint(checkpointId, options = {}) {
        ...
        try {
            ...
            if (options.preserveCurrentState || options.createBackup) {
                await this.createCheckpoint(this.getCurrentState(), {
    ...
    async generateDiff(currentCheckpointId, previousCheckpointId) {
        ...
        if (previousCheckpoint) {
            this.compareObjects(previousCheckpoint.state, currentCheckpoint.state, '', changes);
        }
        else {
    ...
    listCheckpoints(filters) {
        ...
        if (filters) {
            if (filters.tags?.length) {
                checkpoints = checkpoints.filter(c => filters.tags.some(tag => c.tags.includes(tag)));
            }
            if (filters.dateRange) {
                checkpoints = checkpoints.filter(c => c.timestamp >= filters.dateRange.start &&
    ...
    extractAllPaths(obj, path, changes, type) {
    ...
    findPreviousCheckpoint(checkpointId) {
        const checkpoint = this.checkpoints.get(checkpointId);
    ...
    findDependentCheckpoints(checkpointId) {
        const dependents = [];
    ...
    updatePerformanceMetrics(operation, time, size) {
        if (operation === 'create') {
...
export class CheckpointManager {
    ...
    async saveProgress(name) {
        return await this.checkpointSystem.createCheckpoint(this.getCurrentState(), {
