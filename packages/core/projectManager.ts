...
import { serializeProject,
  deserializeProject,
  SerializationOptions,
  DeserializationOptions }
 from './utils/projectSerialization';
import { ProjectMetadata, ProjectSettings, createDefaultMetadata, createDefaultSettings } from './schemas/psgSchema';
...
export interface SaveProjectOptions { name: string;
  description?: string;
  author?: string;
  tags?: string;
  fileName?: string }
...
export interface LoadProjectResult { success: boolean;
  data?: { }
},
...
export interface SaveProjectResult { success: boolean;
  fileName?: string;
  error?: string;
  warnings?: string }


export interface PSGFile { id: string;
  name: string;
  path: string;
  size: number;
  lastModified: Date;
...
export interface ProjectFolder { id: string;
  name: string;
  path: string;
  parentId?: string;
...
export class ProjectManager { private static instance: ProjectManager;
  ...
  private constructor() { // Load from localStorage or API
    this.loadUserData() }
   ...
   */
  getRecentFiles(limit: number = 10): PSGFile { return this.recentFiles.slice(0, limit) }

  /**
   * Add file to recent files
   */
  addToRecentFiles(file: PSGFile): void { // Remove if already exists
   ...
   */
  toggleFavorite(fileId: string): boolean { if (this.favoriteFiles.has(fileId)) {
      this.favoriteFiles.delete(fileId);
      return false } else { this.favoriteFiles.add(fileId);
      return true }
   ...
   */
  isFavorite(fileId: string): boolean { return this.favoriteFiles.has(fileId) }

  /**
   * Get favorite files
   */
  getFavoriteFiles(): PSGFile { return this.recentFiles.filter(file => this.favoriteFiles.has(file.id)) }
 ...
 catch (error) { console.warn('Failed to load user data from localStorage:', error) }
 ...
 catch (error) { console.warn('Failed to save user data to localStorage:', error) }
   ...
   */
  static async loadProjectFromDevice(): Promise<LoadProjectResult> { return new Promise(resolve => {
  try {
   ...
   */
  static sanitizeFileName(fileName: string): string { // Remove or replace invalid characters
