...
export const ProjectMetadataSchema = z.object({ name: z.string().min(1).max(100, 'Project name must be between 1-100 characters'),
...
export const GraphContentSchema = z.object({ nodes: z.array(z.unknown()), // Using z.unknown() to allow for flexible node data structures,
...
export const EditorSettingsSchema = z.object({ autoSave: z.boolean().default(true),
...
export const ExportMetadataSchema = z.object({ exportedBy: z.string().default('PromptScape GraphEditor'),
...
export const PSGFileSchema = z.object({ metadata: ProjectMetadataSchema,
...
): PSGFile { const now = new Date().toISOString();
  return {
  metadata: {
 ...
 */
export function validatePSGFile(data: unknown): PSGFile { return PSGFileSchema.parse(data);
  /**
...
export function parsePSGFile(jsonString: string, options: { )
  ...
  allowLegacyFormat?: boolean } = {}): { success: true;
  data: PSGFile;
 ...
 = options;
  try { // Check file size before parsing
  ...
  suggestions: ['Try reducing the number of nodes', 'Compress or optimize the project', 'Split into smaller projects']
    ...
    let parsed: unknown;
    try { parsed = JSON.parse(jsonString) } catch (syntaxError) { return {
  success: false
  error: {
  ...
  suggestions: ['Verify this is a valid .psg file', 'Check if file was corrupted during transfer'] }
    ...
    if (securityViolations.length > 0) { return {
  success: false
  error: {
  ...
  suggestions: ['Try re-exporting the project', 'Contact support if problem persists'] }
    ...
    try { validated = validatePSGFile(parsed) } catch (zodError) { if (zodError instanceof z.ZodError) {
...
function checkForSecurityViolations(data: unknown): string {
  ...
  const checkObject = (obj: unknown, path: string = ''): void => {
    ...
    for (const key in obj) {
      ...
      const value = obj[key];
      if (typeof value === 'string') { const suspiciousPatterns = [
          /javascript:/i,
          /data:text\/html/i,
          /eval\s*\(/i)
          /function\s*\(/i)
      ...
      if (typeof value === 'object' && value !== null) { checkObject(value, currentPath) };
  checkObject(data);
  return violations;
/**
...
function validateDataConsistency(data: unknown): string {
  ...
  try {
    ...
    if (data.metadata) { if (data.metadata.created && data.metadata.modified) {
        const created = new Date(data.metadata.created).getTime();
        const modified = new Date(data.metadata.modified).getTime();
        if (created > modified) {
          errors.push('Created date is later than modified date') } catch (error) {
 ...
 */
function validateDataIntegrity(psgFile: PSGFile): string { const warnings: string = [];
  try {
  // Check graph complexity
  const nodeCount = psgFile.graph.nodes.length;
  const edgeCount = psgFile.graph.edges.length;
  if (nodeCount === 0) {
  warnings.push('Project contains no nodes') } else if (nodeCount > 1000) {
      warnings.push(`Large project with ${nodeCount} nodes may impact performance`);}
    if (edgeCount > nodeCount * 3) { warnings.push('Unusually high edge-to-node ratio detected');
  ...
  connectedNodes.add(edge.source);
    ...
    if (psgFile.graph.viewport) {
      ...
      if (Math.abs(x) > 10000 || Math.abs(y) > 10000) { warnings.push('Viewport position is very far from origin');
      if (zoom < 0.1 || zoom > 10) {
        warnings.push('Unusual viewport zoom level') } catch (error) {
...
export function serializePSGFile(psgFile: PSGFile, options: { )
  ...
  try { // Pre-serialization validation
  if (validate) {
