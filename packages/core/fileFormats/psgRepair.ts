...
export type PsgRepairStatus = 'canonical' | 'rewritten' | 'skipped' | 'unsafe';

export interface PsgRepairResult {
  status: PsgRepairStatus;
  changes: string[];
  reasons: string[];
...
const SOURCE_NODE_TYPE_MAP: Record<string, string> = {
...
function getCanonicalNodeType(type: unknown): string | null {
...
function sanitizeNodeData(data: JsonRecord): JsonRecord {
...
function getAdditionalNodeFields(node: JsonRecord): JsonRecord {
...
function getAdditionalRegionFields(region: JsonRecord): JsonRecord {
  const extra: JsonRecord = {};
...
function tryParseWeightedChoiceValue(value: unknown): unknown[] | undefined {
...
function normalizeNode(
  ...
  index: number,
  changes: Set<string>
): { node?: JsonRecord; reasons: string[] } {
...
function normalizeEdges(
  ...
  changes: Set<string>
): { edges?: JsonRecord[]; reasons: string[] } {
  const reasons: string[] = [];
  const sourceEdges = Array.isArray(edgesInput) ? edgesInput : [];
  const incomingByTarget = new Map<
    string,
    Array<{ index: number; edge: JsonRecord }>
  >();
  ...
  const concatAssignments = new Map<number, 'input1' | 'input2' | undefined>();
  incomingByTarget.forEach(entries => {
    ...
    const available: Array<'input1' | 'input2'> = ['input1', 'input2'].filter(
      handle => !used.has(handle)
    ) as Array<'input1' | 'input2'>;
  ...
  sourceEdges.forEach((edge, index) => {
    ...
    if (sourceHandle && /^branch-\d+$/.test(sourceHandle)) {
      ...
      return;
    } else if (sourceType === 'Output' && sourceHandle !== undefined) {
...
function normalizeRegions(
  ...
  changes: Set<string>
): { regions?: JsonRecord[]; reasons: string[] } {
  const reasons: string[] = [];
  ...
  const regions = regionsInput.map((region, index) => {
    ...
    if (isRecord(region.metadata) && Object.keys(region.metadata).length > 0) {
      ...
      const liftedMetadata: JsonRecord = {};
  ...
  return {
    regions: regions.filter((region): region is JsonRecord => region !== null),
    reasons
  };
}

export function repairPsgContent(content: string): PsgRepairResult {
  let parsed: unknown;
  try {
  ...
  const preNormalized = deepClone(parsed) as JsonRecord;
  ...
  const normalizedInput = normalizeLegacyFlatPsgShape(preNormalized);

  if (
    Array.isArray((parsed as JsonRecord).groups) &&
    !Array.isArray((parsed as JsonRecord).regions)
  ) {
    addChange(changes, 'normalized groups -> regions');
  }
  if (!Array.isArray((parsed as JsonRecord).edges)) {
  ...
  const hadOriginalTopLevelName =
    typeof preNormalized.name === 'string' &&
    preNormalized.name.trim().length > 0;
  if (!hadOriginalTopLevelName) {
    addChange(changes, 'added top-level name from metadata fallback');
  }

  const normalizedNodes: JsonRecord[] = [];
  const nodeReasons: string[] = [];
  normalizedInput.nodes.forEach((node: unknown, index: number) => {
  ...
  const additionalTopLevel: JsonRecord = {};
  ...
  const orderedDocument: JsonRecord = {
