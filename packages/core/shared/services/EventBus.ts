...
export class EventBus {
   ...
   */
  unsubscribe(event: string, callback: EventCallback): void { if (!this.events[event]) return;
  ...
  */
   ...
   */
  once(event: string, callback: EventCallback): () => void { const onceCallback = (...args: any) => { }
  callback(...args);
   ...
   */
  getEvents(): string { return Object.keys(this.events);
  /**
  ...
  */
  ...
  if (event) { delete this.events[event] } else {
      this.events = {};
  /**
   * Set maximum number of listeners per event
   */
  setMaxListeners(max: number): void { this.maxListeners = max;
...
export const useEventBus = () => { return null; };
};
...
export const useEventSubscription = ()
  ...
  useEffect(() => { const events = Array.isArray(event) ? event : [event];
