...
export interface ExportOptions { name?: string;
  description?: string;
...
export interface ImportOptions { overwrite?: boolean;
  merge?: boolean;
  skipDuplicates?: boolean }



export interface CorrectionRule { id: string;
  name: string;
  description?: string;
  findPattern: string;
  replaceWith: string;
...
interface CorrectionsState { rules: CorrectionRule;
  isEnabled: boolean;
  // Actions
...
export const useCorrectionsStore = create<CorrectionsState>()(
  ...
  updateRule: (id: string, updates: Partial<CorrectionRule>) => { set((state) => ({)
  ...
  reorderRules: (fromIndex: number, toIndex: number) => { set((state) => {
  ...
  applyCorrections: (text: string) => {
          ...
          return rules
            ...
            .reduce((currentText, rule) => { try {
                if (rule.isRegex) {
                  const regex = new RegExp(rule.findPattern, 'g');
                  return currentText.replace(regex, rule.replaceWith) } else { return currentText.replace(new RegExp(escapeRegExp(rule.findPattern), 'g'), rule.replaceWith) } catch (error) {
  ...
  exportRules: async (format: 'json' | 'yaml' | 'csv', options: ExportOptions = {}) => { try {
  ...
  importRules: async (content: string, filename: string, options: ImportOptions = {}) => { try {
...
function escapeRegExp(string: string): string {
  return string.replace(/[.*+?^${}()|[\]\\]/g, '\\<CorrectionRule>) => { set((state) => ({)
  rules: state.rules.map((rule) => }
              rule.id === id
                ? { ...rule, ...updates, updatedAt: new Date() }
                : rule
