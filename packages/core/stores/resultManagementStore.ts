...
export interface SavedResult extends EnhancedPreviewResult { // Additional persistence metadata
  savedAt: Date;
  lastModified: Date;
...
interface ResultManagementState { // Core data
...
export const useResultManagementStore = create<ResultManagementState>()()
  ...
  totalResults: 0
      ...
      saveResult: (result: EnhancedPreviewResult, metadata?: Partial<SavedResult>) => {
        ...
        const now = new Date();
        const savedResult: SavedResult = { ...result
  ...
  updateResult: (id: string, updates: Partial<SavedResult>) => { const currentResult = get().savedResults[id];
        if (!currentResult) {
  ...
  duplicateResult: async (id: string) => { 
        ...
        const duplicated: SavedResult = { ...original
          id: newId
          savedAt: new Date()
          lastModified: new Date()
          version: 1
  ...
  toggleResultSelection: (id: string) => { 
        ...
        if (isSelected) {
          get().deselectResult(id) } else { get().selectResult(id) }
      // Metadata Actions
      rateResult: (id: string, rating: number) => { if (rating < 1 || rating > 5) {
          throw ErrorFactory.createValidationError()
            'rating',
            rating,
  ...
  tagResult: (id: string, tags: string) => { get().updateResult(id, {)
  metadata: {,
  ...get().savedResults[id]?.metadata }
  tags
});
  ...
  addNote: (id: string, note: string) => { get().updateResult(id, {)
  metadata: {,
  ...get().savedResults[id]?.metadata,
  notes: note }
});
  ...
  workflow: { ...current, ...workflow }
      ...
      createCollection: (name: string, description?: string) => {
        ...
        const now = new Date();
        const collection: ResultCollection = { id
  name
  description
  createdAt: now
  lastModified: now
  ...
  updateCollection: (id: string, updates: Partial<ResultCollection>) => { const current = get().collections[id];
        if (!current) {
          throw ErrorFactory.createValidationError()
            'collectionId'
            id
  ...
  searchResults: (query: string) => { 
      ...
      // Bulk Operations
      bulkUpdateTags: (resultIds: string, tags: string) => { resultIds.forEach(id => get().tagResult(id, tags)) }
  bulkUpdateWorkflow: (resultIds: string, workflow: Partial<SavedResult['workflow']>) => { resultIds.forEach(id => get().updateWorkflowStatus(id, workflow)) }
  ...
  getFilteredResults: () => {
        ...
        // Apply filters
        if (currentFilter.tags?.length) { results = results.filter(r => )
  ...
  const rating = r.metadata?.rating || 0;
        ...
        if (currentFilter.contentType?.length) { results = results.filter(r => )
