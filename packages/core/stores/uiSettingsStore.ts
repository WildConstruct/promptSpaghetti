...
export interface UISettings { // Professional Interface Settings
...
interface UISettingsState extends UISettings { // Actions
...
export const useUISettingsStore = create<UISettingsState>()()
  ...
  ...DEFAULT_SETTINGS }
      ...
      // Per-Node Preference Actions
      setNodeDisclosureLevel: (nodeId: string, level: 'basic' | 'advanced' | 'debug') => { set((state) => ({)
  nodePreferences: {
  ...
  setNodeUseGlobalDefault: (nodeId: string, useGlobal: boolean) => { set((state) => ({)
  nodePreferences: {
  ...state.nodePreferences
  [nodeId]: {
  ...
  setNodeTypeDisclosureLevel: (nodeType: string, level: 'basic' | 'advanced' | 'debug') => { set((state) => ({)
  nodeTypePreferences: {
  ...
  setNodeTypeCollapsedSections: (nodeType: string, sections: string) => { set((state) => ({)
  nodeTypePreferences: {
  ...state.nodeTypePreferences
  [nodeType]: {
  ...state.nodeTypePreferences[nodeType]
  ...
  getEffectiveTheme: () => { 
      ...
      getNodeDisclosureLevel: (nodeId: string, nodeType?: string) => { const state = get();
      ...
      getEffectiveNodePreferences: (nodeId: string, nodeType?: string) => { const state = get();
...
export const shouldShowField = (fieldName: string, fieldType?: string, store?: ReturnType<typeof useUISettingsStore>): boolean => { // Always hide technical fields like 'id', 'nodeId', 'internalConfig', etc.
