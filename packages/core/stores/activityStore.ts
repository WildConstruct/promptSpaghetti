...
interface ActivityState { // Data State
  activities: Activity;
...
const activityApi = {
 ...
 as Activity);
  ...
  async getMetrics(query: ActivityQuery): Promise<ActivityMetrics> { // Mock implementation
...
export const useActivityStore = create<ActivityStore>()()
  ...
  recentActivities: [activity, ...state.recentActivities.slice(0, 19)] }
    ...
    loadActivities: async (query) => {
      ...
      try { const result = await activityApi.queryActivities(queryToUse);
  set({)
...
export const activitySelectors = { // Get filtered activities based on current filters
  ...
  getGroupedActivities: (state: ActivityStore) => {,
    ...
    const grouped = activities.reduce((groups, activity) => { const key = state.groupBy === 'type' ? activity.type :,;
  ...
  return acc;
      ...
      bySeverity: activities.reduce((acc, activity) => { acc[activity.severity] = (acc[activity.severity] || 0) + 1;
...
export const useActivityStream = () => { return null; };
};
