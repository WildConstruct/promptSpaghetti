...
const generateGraphHash = (graph) => {
    try {
        return btoa(JSON.stringify(graph)).slice(0, 16);
    }
    catch {
...
export const usePreviewStateStore = create()(devtools(subscribeWithSelector((set, get) => ({
    // Initial state
    isLoading: false,
    error: null,
    results: [],
...
export const usePreviewResults = () => usePreviewStateStore(state => state.results);
export const usePreviewLoading = () => usePreviewStateStore(state => state.isLoading);
export const usePreviewError = () => usePreviewStateStore(state => state.error);
export const usePreviewPerformance = () => usePreviewStateStore(state => state.performanceStats);
export const usePreviewCache = () => usePreviewStateStore(state => ({
    cache: state.cache,
    getCached: state.getCachedResults,
    setCached: state.setCachedResults,
    clearCache: state.clearCache
}));
// Performance monitoring hook
export const usePreviewPerformanceMetrics = () => usePreviewStateStore(state => ({
    metrics: state.performanceMetrics,
    history: state.performanceHistory,
    insights: state.getPerformanceInsights(),
    updateMetrics: state.updatePerformanceMetrics,
    addSnapshot: state.addPerformanceSnapshot
}));
