...
export const useResultManagementStore = create()(persist((set, get) => ({
    ...
    updateResult: (id, updates) => {
        ...
        if (!currentResult) {
            throw ErrorFactory.createValidationError('resultId', id, 'existing saved result', { operation: 'update_result' });
        }
        set((state) => ({
            savedResults: {
                ...state.savedResults,
    ...
    duplicateResult: async (id) => {
        const original = get().savedResults[id];
        if (!original) {
            throw ErrorFactory.createValidationError('resultId', id, 'existing saved result', { operation: 'duplicate_result' });
        }
    ...
    rateResult: (id, rating) => {
        if (rating < 1 || rating > 5) {
            throw ErrorFactory.createValidationError('rating', rating, 'number between 1 and 5', { operation: 'rate_result' });
        }
        get().updateResult(id, {
            metadata: {
    ...
    updateWorkflowStatus: (id, workflow) => {
    ...
    updateCollection: (id, updates) => {
        ...
        if (!current) {
            throw ErrorFactory.createValidationError('collectionId', id, 'existing collection', { operation: 'update_collection' });
        }
        set((state) => ({
            collections: {
                ...state.collections,
    ...
    addToCollection: (resultIds, collectionId) => {
    ...
    removeFromCollection: (resultIds, collectionId) => {
        const collection = get().collections[collectionId];
        if (!collection)
            return;
    ...
    searchResults: (query) => {
        ...
        return results.filter(result => result.output?.toLowerCase().includes(lowerQuery) ||
    ...
    getFilteredResults: () => {
        ...
        if (currentFilter.tags?.length) {
            results = results.filter(r => currentFilter.tags.every(tag => r.metadata?.tags?.includes(tag)));
        }
        ...
        if (currentFilter.contentType?.length) {
            results = results.filter(r => currentFilter.contentType.includes(r.metadata?.contentType || 'mixed'));
        }
    ...
    deserialize: (str) => {
        ...
        return {
            ...
            savedResults: Object.fromEntries(Object.entries(parsed.savedResults || {}).map(([id, result]) => [
                id,
                {
                    ...result,
            ...
            collections: Object.fromEntries(Object.entries(parsed.collections || {}).map(([id, collection]) => [
                id,
                {
                    ...collection,
