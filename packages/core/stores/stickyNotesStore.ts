...
export const useStickyNotesStore = create<StickyNotesStore>()()
  ...
  addToGroup: (groupId, noteId) => { set(state => ({)
  groups: {
  ...state.groups
  [groupId]: {
  ...
  removeFromGroup: (groupId, noteId) => { set(state => ({)
  groups: {
  ...state.groups
  [groupId]: {
  ...state.groups[groupId]
 ...
 catch (error) { console.error('Failed to import notes:', error) }
    // Collaboration
    lockNote: (id) => { ,
  get().updateNote(id, {)
  collaboration: {
