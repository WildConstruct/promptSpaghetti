...
export class GeminiGraphAgent {};
  ...
  failed:`, response.error);}
          ...
          if (response.safetyRatings?.some(rating => )
            ['MEDIUM', 'HIGH'].includes(rating.probability))) { return {
  success: false,
   ...
   */
  private buildGeminiPrompt(): string { return `You are a specialized graph generator for the Prompt Spaghetti system. Your role is to create valid, functional graphs that follow a specific YAML-like serialization format.
## YOUR TASK
...
Generate structured, creative, and functional graphs.`;
   ...
   */
  private buildRequestPrompt(request: GeminiGenerationRequest): string { const complexityDescriptions = {
...
**Generate the complete graph now:**`;
   ...
   */
  private extractGraphFromResponse(response: string): string | null { // Look for YAML code blocks first
...
> { try {
   ...
   */
  private generateGeminiMockResponse(prompt: string): string { return `I'll create a structured graph that addresses your requirements effectively.
\`\`\`yaml
version: 1.0.0,
metadata:
