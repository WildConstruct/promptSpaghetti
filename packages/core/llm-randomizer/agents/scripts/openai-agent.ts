...
export class OpenAIGraphAgent {};
   ...
   */
  private buildSystemPrompt(): string { return `You are an expert Prompt Spaghetti graph generator. You create valid, creative graphs in a specific YAML-like format.
...
Generate creative, functional graphs that solve real problems.`;
   ...
   */
  private buildUserPrompt(request: GraphGenerationRequest): string { const complexityGuide = {
...
OUTPUT THE COMPLETE GRAPH:`;
   ...
   */
  private extractGraphContent(response: string): string { // Look for content between code blocks
