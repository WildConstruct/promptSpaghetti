...
export class GraphMutationEngine extends EventEmitter {
    ...
    async restoreSnapshot(snapshot) {
        ...
        this.emit('state_changed', {
            ...
            operation: { id: 'restore', type: OperationType.GRAPH_IMPORT }
        });
    }
    // PRIVATE METHODS
    ...
    async executeNodeDelete(operation) {
        ...
        this.currentState.edges = this.currentState.edges.filter(e => e.source !== nodeId && e.target !== nodeId);
    }
    async executeNodeUpdate(operation) {
        const { nodeId, updates } = operation.payload;
        this.currentState.nodes = this.currentState.nodes.map(node => node.id === nodeId
    ...
    async handleConflicts(operation, conflictResult) {
    ...
    createFailureResult(operation, errors) {
        return {
            success: false,
            operation,
            error: errors.join(', '),
    ...
    createConflictResult(operation, conflictResult) {
        return {
            success: false,
            operation,
