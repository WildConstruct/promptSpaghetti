 ...
 */
export class GraphValidator extends EventEmitter { private customValidators: ValidationFunction = [];
  ...
  */
  ...
  errors.push(...customErrors) } catch (error) { warnings.push({)
 ...
 catch (error) { const validationError: ValidationError = {
   ...
   */
  async validateState(state: GraphState): Promise<ValidationResult> { const errors: ValidationError = [];
  try {
  ...
  private async validateBasicOperation(operation: GraphOperation): Promise<ValidationError> { const errors: ValidationError = [];
  ...
  severity: 'error' }
    ...
    if (!operation.type) { errors.push({)
  type: 'MISSING_OPERATION_TYPE'
  message: 'Operation type is required'
  severity: 'error' }
});
    if (!operation.timestamp) { errors.push({)
  type: 'MISSING_TIMESTAMP'
  message: 'Operation timestamp is required'
  severity: 'error' }
});
    // Validate timestamp
    if (operation.timestamp && isNaN(operation.timestamp.getTime())) { errors.push({)
  type: 'INVALID_TIMESTAMP'
  message: 'Operation timestamp is invalid'
  severity: 'error' }
});
    // Check for dangerous operations
    if (!this.config.allowDangerousOperations) { if (operation.type === OperationType.GRAPH_CLEAR) {
  errors.push({)
  ...
  ): Promise<ValidationError> { switch (operation.type) {
  ...
  private async validateNodeAdd(((
    ...
    if (state.nodes.some(n => n.id === node.id)) { errors.push({)
  ...
  nodeId: node.id }
    ...
    // Validate position
    if (!this.isValidPosition(position)) { errors.push({)
  ...
  severity: 'error'
    ...
    if (connectedEdges.length > 0) { errors.push({)
  ...
  severity: 'error'
    ...
    if (!updates || typeof updates !== 'object') { errors.push({)
  ...
  nodeId
    ...
    for (const field of protectedFields) { if (field in updates) {
        errors.push({)
  ...
  private async validateEdgeAdd(((
    ...
    if (edge.source === edge.target) { errors.push({)
  ...
  severity: 'error'
    ...
    if (!variation || variation.trim() === '') { errors.push({)
  ...
  nodeId
    ...
    if (variation.length > 1000) { errors.push({)
  ...
  severity: 'error'
    ...
    if (index < 0 || index >= variations.length) { errors.push({)
  ...
  severity: 'error',
    ...
    // Validate new value
    if (!newValue || newValue.trim() === '') { errors.push({)
  type: 'EMPTY_VARIATION',
  message: 'Variation cannot be empty',
  severity: 'error' }
  nodeId
  ...
  severity: 'error'
    ...
    // Check indices
    const variations = (node.data as any).variations || [];
    if (fromIndex < 0 || fromIndex >= variations.length) { errors.push({)
  ...
  severity: 'error',
    ...
    if (toIndex < 0 || toIndex >= variations.length) { errors.push({)
  ...
  try {
  nodeSchemas[nodeType].parse(nodeOp.payload.node.data) } catch (schemaError: any) { errors.push({)
  ...
  severity: 'error' }
    ...
    if (this.config.strictMode) { const orphanedNodes = this.findOrphanedNodes(simulatedState);
  ...
  severity: 'warning';
    ...
    return errors;
  private async validateGraphState(state: GraphState): Promise<ValidationError> { const errors: ValidationError = [];
  ...
  severity: 'error';
    ...
    if (duplicateEdgeIds.length > 0) { errors.push({)
  ...
  severity: 'error';
    ...
    return errors;
  private isValidPosition(position: { x: number; y: number }): boolean { return;
  ...
  edges: [...state.edges] }
    ...
    switch (operation.type) { case OperationType.NODE_ADD:,
  const nodeOp = operation as NodeAddOperation;
