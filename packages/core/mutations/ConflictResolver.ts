 ...
 */
export class ConflictResolver extends EventEmitter { private recentOperations: Map<string, GraphOperation> = new Map();
  ...
  */
  ...
  ): Promise<GraphOperation> { if (!conflictResult.hasConflicts) {
  return operation;
  ...
  */
  ...
  confidence: 0.7 }
      ...
      if (this.config.enableOperationalTransform) { options.push({)
  ...
  private getConflictType(localOp: GraphOperation, remoteOp: GraphOperation): ConflictType | null { // Same element operations
  ...
  confidence: 0.8 }
      ...
      break;
    case ConflictType.DELETE_MODIFY: options.push({ );
  strategy: ConflictResolutionStrategy.LAST_WRITER_WINS
  ...
  confidence: 0.6 }
      ...
      break;
    case ConflictType.MOVE_MODIFY: options.push({ );
  strategy: ConflictResolutionStrategy.OPERATIONAL_TRANSFORM
  ...
  confidence: 0.9 }
      ...
      break;
    return options;
  private canAutoResolveConflict(conflict: OperationConflict): boolean { return conflict.severity !== 'critical' &&
  ...
  ...localOp.payload.updates
    ...
    const mergedOperation: NodeUpdateOperation = { ...localOp
  id: this.generateId()
  timestamp: new Date()
  payload: {
  ...localOp.payload
  ...
  private cleanupOldOperations(): void { const now = Date.now();
    for (const [userId, operations] of this.recentOperations.entries()) {
      const validOperations = operations.filter(;);
        op => (now - op.timestamp.getTime()) < this.maxOperationAge
      );
      if (validOperations.length === 0) {
        this.recentOperations.delete(userId) } else {
