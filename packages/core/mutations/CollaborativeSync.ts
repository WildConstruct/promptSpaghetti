...
export class SimpleWebSocketService extends EventEmitter implements WebSocketService { private ws: WebSocket | null = null;
  ...
  this.emit('message', message) } catch (error) { console.error('Failed to parse WebSocket message:', error) };
 ...
 */
export class CollaborativeSync extends EventEmitter { private conflictResolver: ConflictResolver;
   ...
   */
  async disconnect(): Promise<void> { if (this.currentUser) {
  ...
  */
   ...
   */
  async updatePresence(presence: Partial<UserPresence>): Promise<void> { if (!this.currentUser || !this.webSocketService.isConnected()) {
  return;
  this.currentUser = {
  ...this.currentUser
  ...presence
   ...
   */
  getConnectedUsers(): UserPresence { return Array.from(this.connectedUsers.values());
  /**
  ...
  */
  ...
  this.handleRemoteMessage(message);
    ...
    this.webSocketService.on('connected', () => { this.emit('connection_established');
      this.processQueuedOperations() });
    this.webSocketService.on('disconnected', () => { this.emit('connection_lost') });
  ...
  private async handleRemoteMessage(message: CollaborativeMessage): Promise<void> { try {
  switch (message.type) {
  case 'GRAPH_MUTATION':
  ...
  private async handleRemoteMutation(message: CollaborativeMessage): Promise<void> { if (!message.operation) {
  ...
  conflicts: conflictResult.conflicts }
        ...
        if (this.config.conflictResolution.autoResolve && conflictResult.canAutoResolve) { // Resolve conflicts automatically
 ...
 else { // Emit for manual resolution
  this.emit('manual_conflict_resolution_required', {)
  remoteOperation: message.operation }
  conflictResult
});
 else { // No conflicts, apply the operation
  this.emit('remote_mutation_received', {)
  operation: message.operation
  resolved: false }
});
 catch (error) { this.emit('remote_mutation_error', {)
  operation: message.operation }
  error
});
  private async handleCursorUpdate(message: CollaborativeMessage): Promise<void> { const user = this.connectedUsers.get(message.userId);
  ...
  private async handleSelectionChange(message: CollaborativeMessage): Promise<void> { const user = this.connectedUsers.get(message.userId);
  ...
  private async broadcastPresence(isActive: boolean = true): Promise<void> { if (!this.currentUser || !this.webSocketService.isConnected()) {
  return;
  const message: CollaborativeMessage = {
  ...
  lastActivity: new Date() }
    ...
    await this.webSocketService.broadcast(message);
  private async processQueuedOperations(): Promise<void> { if (this.isProcessingQueue || this.operationQueue.length === 0) {
      return;
    this.isProcessingQueue = true;
    try {
      while (this.operationQueue.length > 0) {
        const operation = this.operationQueue.shift()!;
        await this.broadcastMutation(operation) } catch (error) {
