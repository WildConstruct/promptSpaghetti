...
export interface GraphEditorState { graph: Graph;
...
export interface NodeSelection { nodeId: string }
},
  position: { x: number; y: number };
  isMultiSelect: boolean;


export interface GraphOperation { type: 'ADD_NODE' | 'REMOVE_NODE' | 'UPDATE_NODE' | 'ADD_EDGE' | 'REMOVE_EDGE' | 'MOVE_NODE' }
  payload: any;
  timestamp: number;
  userId?: string;
...
export interface ValidationError { nodeId?: string;
  edgeId?: string;
...
export interface PreviewConfiguration { seeds: number;
  maxSeeds: number;
  autoRefresh: boolean;
  debounceMs: number }
...
export interface GraphEditorConfig { autosave: { }
  enabled: boolean;
  intervalMs: number;
...
export interface GraphDomainEvents { onGraphModified: (graph: Graph) => void
...
export interface GraphEditorProps { initialGraph?: Graph;
...
export interface NodePaletteProps { onNodeDragStart: (nodeType: string) => void;
...
export interface InspectorProps { selectedNodeIds: string;
  graph: Graph;
...
export interface CanvasProps { graph: Graph;
  selectedNodeIds: string;
...
export interface NodeTypeDefinition { type: string;
  label: string;
  description: string;
  icon: string;
...
export interface IODefinition { name: string;
...
export interface PropertyDefinition { name: string;
