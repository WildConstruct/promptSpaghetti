...
interface GraphEditorStore extends GraphEditorState { // Configuration
...
export const useGraphEditorStore = create<GraphEditorStore>()()
  ...
  type: 'MOVE_NODE' }
        ...
        duplicateNode: (nodeId: string) => set((state) => {
          ...
          const duplicatedNode: Node = { ...node
  id: newNodeId
  position: {
  ...
  type: 'REMOVE_EDGE' }
        ...
        // Selection management
        selectNodes: (nodeIds: string, isMultiSelect = false) => set((state) => { state.selectedNodeIds = isMultiSelect
  ...
  : nodeIds })
        ...
        toggleNodeSelection: (nodeId: string) => set((state) => { 
          ...
          if (isSelected) {
            state.selectedNodeIds = state.selectedNodeIds.filter(id => id !== nodeId) } else { state.selectedNodeIds.push(nodeId) })
        // Execution
        setExecuting: (isExecuting: boolean) => set((state) => { 
          state.isExecuting = isExecuting })
        setExecutionResults: (results: Record<string, any>) => set((state) => { state.executionResults = results })
