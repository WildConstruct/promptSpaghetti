...
export interface IGraphValidationService { validateGraph(graph: Graph): Promise<ValidationError>;
...
export interface IGraphExecutionService { executeGraph(graph: Graph, seeds: number): Promise<Record<string, any>>;
...
export interface IGraphEditorDomain { // React Components
  components: {;
...
export const GRAPH_DOMAIN_EVENTS = { GRAPH_MODIFIED: 'graph:modified'
