...
export type GraphOperation = 
  ...
  | { type: 'UPDATE_EDGE'; edgeId: string; updates: Partial<GraphEdge> }
  | { type: 'DELETE_EDGE'; edgeId: string }
  | { type: 'SELECT_NODES'; nodeIds: string[]; append?: boolean }
  | { type: 'SELECT_EDGES'; edgeIds: string[]; append?: boolean }
  | { type: 'CLEAR_SELECTION' }
  | { type: 'UPDATE_VIEWPORT'; viewport: Partial<GraphState['viewport']> }
  | { type: 'START_EXECUTION'; nodeId?: string }
  | { type: 'EXECUTION_RESULT'; nodeId: string; result: any }
  | { type: 'EXECUTION_ERROR'; error: ExecutionError }
  | { type: 'UPDATE_COLLABORATION'; updates: Partial<GraphState['collaboration']> };
