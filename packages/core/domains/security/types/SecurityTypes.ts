...
export interface UserSecurityInfo { lastLogin?: Date;
...
export interface UserMetadata { createdAt: Date;
  updatedAt: Date;
  createdBy: string;
...
export interface SecurityQuestion { id: string;
  question: string;
  answerHash: string;
  createdAt: Date }
...
export interface AccessResponse { granted: boolean;
  reason?: string;
  conditions?: string;
  expiresAt?: Date;
  auditId: string }
...
export interface PermissionCondition { type: 'time' | 'location' | 'device' | 'custom' }
...
export type AuditAction = 
  ...
  | 'security_violation' | 'data_export' | 'config_change' | 'system_action';

export type AuditOutcome = 'success' | 'failure' | 'blocked' | 'warning';


export interface AuditDetails { description: string;
  oldValue?: any;
  newValue?: any;
  reason?: string;
...
export type SecurityAlertType = 
  ...
  | 'vulnerability_detected' | 'compliance_violation' | 'system_intrusion';

export type SecuritySeverity = 'low' | 'medium' | 'high' | 'critical';
export type AlertStatus = 'open' | 'investigating' | 'resolved' | 'false_positive';


export interface SecurityAlertDetails { affectedResources: string;
  riskScore: number;
...
export interface SecurityIndicator { type: string;
  value: string;
  confidence: number;
  source: string }
...
export interface PolicyAction { type: 'allow' | 'deny' | 'require_approval' | 'log' | 'notify' }
  parameters: Record<string, any>;
...
export interface DataHandlingRule { type: 'storage' | 'transmission' | 'processing' | 'disposal' }
  requirements: string;
  restrictions: string;
  approvals: string;
...
export interface RetentionPolicy { retentionPeriod: number;
...
export interface EncryptionConfig { algorithm: string;
  keySize: number;
  mode: string;
...
export interface SecurityMetrics { authentication: { }
...
export interface SecurityDomainEvents { onUserAuthenticated: (user: User, context: AccessContext) => void
...
export interface SecurityDomainState { currentUser: User | null;
...
export interface SessionFlag { type: 'suspicious' | 'elevated_risk' | 'new_device' | 'unusual_location' }
  reason: string;
  timestamp: Date;
  // Component Props
...
export interface SecurityDashboardProps { userId: string;
  permissions: Permission;
...
export interface AccessControlProps { resource: string;
  action: PermissionAction;
...
export interface AuditLogViewerProps { userId?: string;
  resource?: string;
  actions?: AuditAction }
