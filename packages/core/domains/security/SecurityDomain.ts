...
export interface IAuthorizationService { checkPermission(userId: string, resource: string, action: PermissionAction): Promise<boolean>;
...
export interface IAuditService { logEvent(event: Omit<AuditLog, 'id' | 'timestamp'>): Promise<AuditLog>;
...
export interface ISecurityMonitoringService { getSecurityAlerts(filters?: SecurityAlertFilters): Promise<SecurityAlert>;
...
export interface IPolicyManagementService { getPolicies(type?: string): Promise<SecurityPolicy>;
...
export interface IDataClassificationService { classifyData(data: any, context?: any): Promise<DataClassification>;
...
export interface AuditLogFilters { userId?: string;
  resource?: string;
  action?: string }
...
export interface SecurityAlertFilters { type?: string;
  severity?: string;
  status?: string;
  userId?: string }
...
export interface SecurityScanResult { scanId: string;
  startedAt: Date;
  completedAt: Date;
  scope: string;
...
export interface RiskAssessment { userId: string;
  riskScore: number;
  factors: RiskFactor;
  recommendations: string;
  validUntil: Date }
...
export interface AnomalyDetection { type: string;
  description: string;
  severity: string;
  confidence: number;
  evidence: any;
...
export interface PolicyEvaluationResult { policyId: string;
  allowed: boolean;
  matchedRules: string;
...
export interface PolicyViolation { ruleId: string;
  description: string;
  severity: string;
  evidence: any }
...
export interface EnforcementAction { action: string;
...
export interface EncryptionContext { purpose: string;
  retention: number;
  classification: string }



export interface EncryptedData { data: string;
  algorithm: string;
  keyId: string;
  iv: string;
  timestamp: Date }
...
export interface KeyRotationResult { rotatedKeys: number;
  failedRotations: string;
  completedAt: Date }



export interface ReportSummary { totalEvents: number;
  uniqueUsers: number;
...
export interface ReportFinding { type: string;
  severity: string;
  description: string;
  count: number;
  examples: AuditLog }
...
export interface SecurityFinding { type: string;
  severity: string;
  resource: string;
  description: string;
  remediation: string;
...
export interface ScanSummary { totalChecks: number;
  passed: number;
  failed: number;
...
export interface ISecurityDomain { // React Components
  components: {;
...
export interface SecurityDomainFactory { create(config?: SecurityDomainConfig): ISecurityDomain }



export interface SecurityDomainConfig { encryption: { }
  algorithm: string;
  keySize: number;
...
export const SECURITY_DOMAIN_EVENTS = { USER_AUTHENTICATED: 'security:user:authenticated'
