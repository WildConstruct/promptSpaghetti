...
export interface EndpointVersioning {
  endpoint: string;
...
export class APICompatibilityManager {
  private static instance: APICompatibilityManager;
  private endpoints: Map<string, EndpointVersioning> = new Map();
  private globalVersion = 'v1';
  
  static getInstance(): APICompatibilityManager {
    if (!this.instance) {
   ...
   */
  registerEndpoint(config: EndpointVersioning): void {
   ...
   */
  getRequestVersion(endpoint: string, headers: Record<string, string>): string {
  ...
  transformRequest(
    ...
    if (!versionConfig?.transformers?.request) return data;
  ...
  transformResponse(
    ...
    const versionConfig = config.versions.get(version);
    if (!versionConfig?.transformers?.response) return data;
   ...
   */
  isVersionDeprecated(endpoint: string, version: string): {
    deprecated: boolean;
    sunsetDate?: Date;
  } {
    const config = this.endpoints.get(endpoint);
    if (!config) return { deprecated: false };
    
    const versionConfig = config.versions.get(version);
    if (!versionConfig) return { deprecated: false };
   ...
   */
  private transformPreviewV1ToV2(data: any): any {
   ...
   */
  private transformPreviewV2ToV1(data: any): any {
    // v2 format: { results: [...], metadata: {...} }
    // v1 format: { outputs: [...] }
    
    return {
      outputs: data.results?.map((r: any) => r.output) || [],
    };
  }
   ...
   */
  private transformExecuteV1ToV2(data: any): any {
   ...
   */
  private transformExecuteV2ToV1(data: any): any {
   ...
   */
  private transformGraphV1ToV2(data: any): any {
   ...
   */
  private transformGraphV2ToV1(data: any): any {
    // v2 format: PSG file format
    // v1 format: legacy graph structure
 ...
 */
export function apiVersioningMiddleware(req: any, res: any, next: any): void {
...
export class SchemaMigration {
   ...
   */
  static migrateGraphV1ToV2(v1Graph: any): any {
