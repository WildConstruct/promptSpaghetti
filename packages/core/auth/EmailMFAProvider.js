...
export class EmailMFAProvider {
    ...
    storage;
    encryptionKey;
    constructor(config, emailService, storage, encryptionKey) {
        this.config = config;
    ...
    // Enrollment Methods
    // ========================================
    async enrollMethod(userId, request) {
        // Validate input
        ...
        if (existingConfig && existingConfig.status !== MFAMethodStatus.REVOKED) {
            throw ErrorFactory.createMFAConfigurationError('already_configured', undefined, { userId,
                operation: 'enroll_email_mfa' });
        }
        // Validate email address
        const emailValid = await this.emailService.validateEmailAddress(validated.emailAddress);
        if (!emailValid) {
            throw ErrorFactory.createMFAConfigurationError('invalid_email', validated.emailAddress, { userId,
                operation: 'enroll_email_mfa' });
        }
        ...
        if (!reputation.valid || reputation.risk > 70) {
            throw ErrorFactory.createMFAConfigurationError('unsuitable_email', validated.emailAddress, { userId,
                operation: 'enroll_email_mfa' });
        }
    ...
    async completeEnrollment(userId, verificationId, code) {
    ...
    // ========================================
    async initiateVerification(userId, configurationId, context) {
        // Get configuration
        ...
        await this.storage.saveVerification(verification);
        // Send verification email
        await this.sendVerificationEmail(configuration.emailAddress, verificationCode, configuration.displayName, riskScore > 50 // Include security warning for high-risk attempts
        );
        // Update rate limit
        await this.updateRateLimit(userId, 'email_send');
        return verificationId;
    }
    async verifyCode(request, context) {
    ...
    async sendVerificationEmail(emailAddress, code, displayName, includeSecurityWarning = false) {
        const template = {
    ...
    async updateEmailAddress(userId, configurationId, newEmailAddress) {
        const validated = EmailAddressUpdateSchema.parse({
            configurationId,
            newEmailAddress
        });
        const configuration = await this.storage.getConfigurationById(configurationId);
        ...
        if (!emailValid) {
            throw ErrorFactory.createMFAConfigurationError('invalid_email', newEmailAddress, { userId,
    ...
    async disableMethod(userId, configurationId) {
        const configuration = await this.storage.getConfigurationById(configurationId);
        if (!configuration || configuration.userId !== userId) {
            throw ErrorFactory.createMFAConfigurationError('invalid_config', undefined, { userId,
                operation: 'challenge_user' });
    ...
    async revokeMethod(userId, configurationId) {
        const configuration = await this.storage.getConfigurationById(configurationId);
        if (!configuration || configuration.userId !== userId) {
            throw ErrorFactory.createMFAConfigurationError('invalid_config', undefined, { userId,
                operation: 'challenge_user' });
