...
const GraphEditorWithProvidersInner = ({ initialNodes, initialEdges, validateConnection, enableBuiltInProviders, providers = [], onProviderRegistered, onProviderUnregistered, onProviderError }) => {
    return (_jsx(EditorProviderWrapper, { initialNodes: initialNodes, initialEdges: initialEdges, selectedNodeId: null, enableBuiltInProviders: enableBuiltInProviders, providers: providers, onProviderRegistered: onProviderRegistered, onProviderUnregistered: onProviderUnregistered, onProviderError: onProviderError, children: ({ registry, editorContext, editorActions, isLoading }) => (_jsx(GraphEditorCore, { initialNodes: initialNodes, initialEdges: initialEdges, validateConnection: validateConnection, registry: registry, editorContext: editorContext, editorActions: editorActions, isProviderLoading: isLoading })) }));
};
const GraphEditorCore = ({ initialNodes, initialEdges, validateConnection, registry, editorContext, editorActions, isProviderLoading }) => {
    ...
    const NodeRender = useMemo(() => {
        const NodeRenderComponent = (props) => (_jsx(NodeRenderer, { id: props.id, data: props.data, selected: selectedNodeId === props.id, onSelect: setSelectedNodeId, getNodeMeta: getNodeMeta, getCategoryColor: getCategoryColor }));
    ...
    return (_jsxs("div", { style: { position: 'relative', width: '100%', height: '100%' }, children: [renderProviderStatus(), _jsxs("div", { style: { display: 'flex', height: '100%' }, children: [_jsx(Palette, { nodes: NODE_TYPES, collapsed: paletteCollapsed, onToggle: () => setPaletteCollapsed((c) => !c), onDragStart: () => { } }), _jsx("div", { style: { flex: 1, position: 'relative', overflow: 'visible' }, children: _jsxs(ReactFlow, { nodes: styledNodes, edges: styledEdges, onNodesChange: onNodesChange, onEdgesChange: onEdgesChange, onConnect: onConnect, onNodeClick: onNodeClick, fitView: true, style: { background: '#1a202c', height: '100%' }, nodeTypes: nodeTypes, onDrop: handleDrop, onDragOver: handleDragOver, 
                ...
                }, onExportBundle: () => editorActions.exportGraph(), onSaveProject: () => editorActions.saveGraph(), onLoadProject: () => { }, onNewProject: () => {
                    setNodes([]);
                    setEdges([]);
                    setSelectedNodeId(null);
