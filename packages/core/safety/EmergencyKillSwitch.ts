...
export interface KillSwitchConfig {
  ...
  thresholds: {
    ...
    errorCount?: number;       // Absolute count
  ...
  cooldownMinutes: number;     // Prevent flip-flopping
  notificationChannels: string[];
  autoRecover: boolean;        // Auto re-enable when metrics improve
}
...
export class EmergencyKillSwitch extends EventEmitter {
  private static instance: EmergencyKillSwitch;
  private switches: Map<string, KillSwitchConfig> = new Map();
  private activations: Map<string, Date> = new Map();
   ...
   */
  register(config: KillSwitchConfig): void {
   ...
   */
  trigger(featureFlag: string, reason: string): void {
   ...
   */
  private startMonitoring(config: KillSwitchConfig): void {
   ...
   */
  private async checkThresholds(config: KillSwitchConfig): Promise<void> {
    ...
    if (thresholds.customMetric) {
      ...
      if (metricStats) {
        ...
        switch (operator) {
          case '>': shouldTrigger = currentValue > value; break;
          case '<': shouldTrigger = currentValue < value; break;
          case '>=': shouldTrigger = currentValue >= value; break;
          case '<=': shouldTrigger = currentValue <= value; break;
          case '==': shouldTrigger = currentValue === value; break;
   ...
   */
  private async checkAutoRecovery(config: KillSwitchConfig): Promise<void> {
   ...
   */
  private getCurrentMetrics(config: KillSwitchConfig): Record<string, number> {
   ...
   */
  private areMetricsHealthy(config: KillSwitchConfig, metrics: Record<string, number>): boolean {
   ...
   */
  private sendNotifications(config: KillSwitchConfig, event: KillSwitchEvent): void {
 ...
 */
export const EPIC1_KILL_SWITCHES: KillSwitchConfig[] = [
  {
