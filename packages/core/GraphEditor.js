...
const GraphEditorInner = ({ initialNodes, initialEdges, validateConnection }) => {
    ...
    const { currentProject, hasUnsavedChanges, newProject, markProjectModified, saveAsTemplate, applyTemplate } = useGraphStore();
    ...
    const handlePaletteDragStart = (_nodeId) => {
    ...
    return (_jsx(DemoModeManager, { initialConfig: {
            ...
            console.log('Demo mode changed:', config);
                        ...
                        localStorage.removeItem('graphDraft');
                                    ...
                                    translateExtent: [[-2000, -2000], [4000, 4000]], nodeExtent: [[-1500, -1500], [3000, 3000]], ...optimizer.getOptimizedRenderSettings(nodes.length, viewport.zoom), children: [_jsx(Background, { color: "#2d3748", gap: viewport.zoom > 0.8 ? 16 : viewport.zoom > 0.4 ? 24 : 32, size: viewport.zoom > 0.8 ? 1 : viewport.zoom > 0.4 ? 1.5 : 2 }), nodes.length < 200 && (_jsx(MiniMap, { nodeColor: () => isPerformanceGood ? '#ff7c00' : '#363a45', maskColor: "#181b21BB", style: {
