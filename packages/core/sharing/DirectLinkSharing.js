...
export class DirectLinkSharing extends EventEmitter {
    ...
    async accessLink(shortCode, accessContext) {
        try {
            // Find link
    ...
    async getLinkAnalytics(linkId, timeRange) {
        const link = this.links.get(linkId);
        if (!link) {
            throw new Error('Link not found');
        }
    ...
    async getBulkAnalytics(linkIds, timeRange) {
        const reports = await Promise.all(linkIds.map(id => this.getLinkAnalytics(id, timeRange)));
        return this.aggregateAnalyticsReports(reports);
    }
    async getUserAnalytics(userId, timeRange) {
        const userLinks = Array.from(this.links.values()).filter(link => link.creator.id === userId);
        const reports = await Promise.all(userLinks.map(link => this.getLinkAnalytics(link.id, timeRange)));
        return this.aggregateUserAnalytics(userLinks, reports);
    }
    // Bulk Operations
    async createBulkLinks(requests) {
    ...
    // Scheduled Sharing
    async scheduleShare(request, schedule) {
    ...
    // Team Management
    async shareWithTeam(linkId, teamId, permissions) {
        const link = this.links.get(linkId);
        if (!link) {
            throw new Error('Link not found');
        }
    ...
    createAccessResult(status, link, message, clickEvent) {
        return {
            status,
            link,
            message,
