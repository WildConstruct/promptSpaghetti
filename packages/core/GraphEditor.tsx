...
interface GraphEditorProps { initialNodes: Node;
  initialEdges: Edge;
...
const GraphEditorInner: React.FC<GraphEditorProps> = ({ initialNodes, initialEdges, validateConnection }) => {
  ...
  const [optimizationSettings, setOptimizationSettings] = useState<OptimizationSettings>({ deadCodeElimination: true
 ...
 = useGraphStore();
  ...
  const [encryptionState, setEncryptionState] = useState<EncryptionState>({ status: 'not_encrypted'
 ...
 = usePreviewSeeds();
  ...
  const handleGlobalPreviewRequest = useCallback(() => {
    ...
    if (sinceChange < 500) { if (previewTimeoutRef.current) clearTimeout(previewTimeoutRef.current);
      previewTimeoutRef.current = setTimeout(run, 500 - sinceChange) } else { run() }
  ...
  const handlePaletteDragStart = () => { // No-op: drag data set in Palette, handled on drop }
};
  // Enhanced drop handler with smooth node creation animation
  const { addNode, updateNode } = useGraphStore();
  const handleDrop = useCallback(
    async (event: React.DragEvent) => { event.preventDefault();
  ...
  preserveIds: true }
            ...
            if (result.success && result.data) {
              ...
              if (result.warnings && result.warnings.length > 0) { console.warn('Project load warnings:', result.warnings) }
 else {
  ...
  id: nodeId
      ...
      // Add with animation
      globalAnimationManager.scheduleAnimation(() => { addNode(newNode);
        setNodes(prev => [...prev, newNode]);
        // Track progress for contextual help system
        helpContentManager.updateProgress('nodesCreated', 1);
        setTimeout(() => {
          setIsCreatingNode(false);
          setNodeCreationAnimation(null) }, 600);
      })
  ...
  const onNodesChange: OnNodesChange = useCallback(
    (changes: NodeChange) => { // Get canvas size for optimization
 ...
 else if (change && 'dragging' in change && !change.dragging) { setDragPreview(null) }

          return change ? { ...node, ...change } : node;
        });
        // Apply canvas optimization for performance
        if (updatedNodes.length > 100) { updatedNodes = optimizer.optimizeNodeVisibility(updatedNodes, viewport, canvasSize) }
  ...
  const onEdgesChange: OnEdgesChange = useCallback(
    (changes: EdgeChange) => {
      setEdges(eds => {
        ...
        if (updatedEdges.length > 200) { updatedEdges = optimizer.optimizeEdges(updatedEdges, nodes, viewport) }
  ...
  const handleEncrypt = useCallback(() => {
    ...
    setTimeout(() => { setEncryptionState(prev => ({
  ...prev
  status: 'encrypted'
  ...
  const handleChangeAlgorithm = useCallback((algorithm: string) => { setEncryptionState(prev => ({
  ...prev
  algorithm: algorithm as EncryptionAlgorithm
  strength:
  algorithm.includes('256') || algorithm.includes('4096')
  ? 'strong'
  ...
  const handleNewProject = useCallback(() => { confirmNavigation('creating a new project', () => {
      newProject();
      setNodes([]);
      setEdges([]) });
  }, [confirmNavigation, newProject]);
  const handleSaveProject = useCallback(() => { setSaveDialogOpen(true) }, []);
  const handleLoadProject = useCallback(() => { confirmNavigation('loading a project', () => {
    ...
    async (entry: RecentProjectEntry) => {
      confirmNavigation('loading a recent project', () => {
        try {
          ...
          setStatusMessage(
            `Recent project "${entry.name}" selected. Please use the Load Project button to select the file.`
          );
          setTimeout(() => setStatusMessage(''), 5000);
 catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
  ...
  const handleSaveSuccess = useCallback(
    (result: { success: boolean; error?: string; projectName?: string; metadata?: unknown }) => {
      if (result.success) {
        ...
        if (result.projectName && result.metadata) {
          try {
            ...
            //   fileSize
            // });
 catch (error) { console.warn('Failed to add project to recent list:', error) }
  ...
  const handleLoadSuccess = useCallback(
    (result: { success: boolean; error?: string; warnings?: string; projectName?: string; metadata?: unknown }) => {
      if (result.success) {
  ...
  const handleExportSuccess = useCallback((result: { success: boolean; error?: string }) => { if (result.success) {
      setStatusMessage('Bundle exported successfully!');
      setTimeout(() => setStatusMessage(''), 3000) } else {
  ...
  const handleTemplateSave = useCallback(
    async (templateData: unknown) => { try {
  ...
  const handleTemplateApply = useCallback(
    async (templateId: string, options: unknown) => { try {
  ...
  const handleOptimizationSettingsChange = useCallback((newSettings: OptimizationSettings) => { setOptimizationSettings(newSettings);
  ...
  if (!target.closest('[data-optimization-menu]') && !target.closest('[data-optimization-button]')) { setOptimizationMenuOpen(false) }
    };

    if (optimizationMenuOpen) { document.addEventListener('mousedown', handleClickOutside);
  ...
  return }
      ...
      if ((event.ctrlKey || event.metaKey) && event.key === 'o') { event.preventDefault();
        handleLoadProject();
        return }
    };
  ...
  id: `${nodeType}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
              ...
              onNodeDelete={ nodeIds => {
                ...
                setEdges(prevEdges =>
                  prevEdges.filter(e => !nodeIds.includes(e.source) && !nodeIds.includes(e.target))
                ) }}
              onExport={format => {
            ...
            <StickyNotesManager disabled={false} readonly={false} />
            {/* Epic 8.7 Task 2: Node Labels System */}
            <NodeLabelsManager disabled={false} readonly={false} selectedNodeId={selectedNodeId} />
            {/* Epic 8.7 Task 3: Region Groups System */}
            <RegionGroupsManager disabled={false} readonly={false} />
            {/* Epic 8.7 Task 4: Connection Annotations System */}
            <ConnectionAnnotationsLayer canEdit={true} showTooltips={true} visible={true} />
            ...
            >
              ...
              {showControls && (
                <div style={{ marginTop: 8, lineHeight: 1.6 }}>
                  <div>
                    <b>Pan:</b> Left-click + drag on canvas
                  </div>
                  <div>
                    <b>Zoom:</b> Mouse wheel / trackpad scroll
                  </div>
                  <div>
                    <b>Select:</b> Click node
                  </div>
                  <div>
                    <b>Multi-select:</b> Shift/Ctrl + Click
                  </div>
                  <div>
                    <b>Connect:</b> Drag from output port
                  </div>
                  <div>
                    <b>Delete:</b> Select + Delete/Backspace
                  </div>
                  <div>
                    <b>Alternative Pan:</b> Middle-click + drag
                  </div>
                </div>
              )}
            </div>
  ...
  zIndex: 10000 }
          ...
          >
            ...
            <div>
              Nodes: {metrics.visibleNodes}/{nodes.length}
            </div>
        ...
        <PreviewModal
          ...
          onResultHover={ idx => {
            ...
            if (res?.usedNodeIds) { setHighlightNodeIds(new Set(res.usedNodeIds)) } else { setHighlightNodeIds(new Set()) }

        />
        <ResponsiveCorrectionsPanel isOpen={correctionsOpen} onClose={() => setCorrectionsOpen(false)} />
        <CorrectionsStatsDashboard isOpen={statsOpen} onClose={() => setStatsOpen(false)} />
        {extensionsOpen && <ExtensionManagerPanel onClose={() => setExtensionsOpen(false)} />}
  ...
  // Settings changes are automatically handled by the SettingsManager
        ...
        />
        <PerformanceMonitor isVisible={performanceMonitorVisible} onToggle={handlePerformanceMonitorToggle} />
        {/* Optimization Menu */}
        { optimizationMenuOpen && (
          <div
            data-optimization-menu
            style={{
  position: 'fixed'
  bottom: '60px'
  left: '50%'
