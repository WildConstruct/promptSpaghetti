...
export interface ExtractedVariable { name: string;          // Variable name (without braces) }
...
class TemplateParser {
  ...
  private performParse(template: string): TemplateParseResult {
    ...
    const braceRegex = /{([^{}]*)}/g;
    let match;
    while ((match = braceRegex.exec(template)) !== null) { const [fullMatch, variableName] = match;
  ...
  private validateVariableName(name: string): { isValid: boolean; errorType?: TemplateError['type']; errorMessage?: string } { const trimmed = name.trim();
  if (trimmed === '') {
  return {
  isValid: false
  ...
  errorMessage: 'Variable name cannot be empty' }
    ...
    if (!validNameRegex.test(trimmed)) { return {
        isValid: false
        errorType: 'invalid_name' }
        errorMessage: `Invalid variable name "${trimmed}". Use letters, numbers, underscore, and hyphen only.`}
      };
    return { isValid: true };
  private findUnclosedBraces(template: string, errors: TemplateError): void { let braceCount = 0;
    let lastOpenBrace = -1;
    for (let i = 0; i < template.length; i++) {
      if (template[i] === '{') {
        if (braceCount === 0) {
          lastOpenBrace = i;
        braceCount++ } else if (template[i] === '}') { braceCount--;
    if (braceCount > 0) {
      errors.push({)
  type: 'unclosed_brace' }
  ...
  private findNestedBraces(template: string, errors: TemplateError): void { let braceDepth = 0;
  for (let i = 0; i < template.length; i++) {
  if (template[i] === '{') {
  braceDepth++;
  if (braceDepth > 1) {
  ...
  defaultValue: '1';
    ...
    if (name.startsWith('is') || name.startsWith('has') || name.startsWith('can') ||
        ...
        name.includes('selected') || name.includes('open') || name.includes('closed')) { return {
        type: 'boolean',
        confidence: 0.9 }
  ...
  defaultValue: 'true';
    ...
    if (name.includes('list') || name.includes('items') || name.includes('options') ||
        ...
        name.endsWith('s') && (name.includes('name') || name.includes('type') || name.includes('id'))) { return {
        type: 'array',
        confidence: 0.7 }
  ...
  defaultValue: '["item1", "item2"]'
    ...
    if (name.includes('config') || name.includes('settings') || name.includes('options') ||
        ...
        name.includes('data') && !name.includes('date')) { return {
        type: 'object',
        confidence: 0.7 }
  ...
  defaultValue: '{"key": "value"}'
    ...
    if (context) { const contextLower = context.toLowerCase();
  ...
  defaultValue: '1';
      ...
      if (contextLower.includes('list') || contextLower.includes('array') || contextLower.includes('multiple')) { return {
          type: 'array',
          confidence: 0.6 }
  ...
  defaultValue: '["item1", "item2"]'
    ...
    if (surroundingText) { // Look for numeric context clues
  ...
  defaultValue: '1' }
      ...
      if (/\b(is|has|can|should|will|enabled|disabled|active|inactive)\b/i.test(surroundingText)) { return {
  type: 'boolean',
  confidence: 0.6,
   ...
   */
  private getUserHistorySuggestions(): VariableSuggestion { const historySuggestions: VariableSuggestion = [];
   ...
   */
  private findRelatedVariables(): string { // This could be enhanced to track actual co-occurrence patterns
  ...
  */
  ...
  partialName: string = '',
  context?: string,
  includeHistory: boolean = true): VariableSuggestion { }
  ...
  if (includeHistory) { const historySuggestions = this.getUserHistorySuggestions();
  ...
  suggestion.examples.some(example => example.toLowerCase().includes(searchTerm))
    ...
    return filtered.sort((a, b) => { const aName = a.name.toLowerCase();
    ...
    for (const variable of parseResult.variables) {
      if (variable.isValid) {
        ...
        sampleValues[variable.name] = suggestion ? 
          ...
          `[${variable.name}]`;}
    return { preview: this.substituteVariables(template, sampleValues)
...
export const getVariableSuggestions = (partialName?: string)
  context?: string
  includeHistory: boolean = true): VariableSuggestion =>
...
export const parseResult = parseTemplate(template);
  ...
  if (variable.defaultValue) {
        defaults[variable.name] = variable.defaultValue } else {
