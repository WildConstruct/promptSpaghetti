...
export interface MarsTag { tag: string;
...
export class AdvancedPromptingMethodology {
   ...
   */
  getMarsAutocompletions(context: string, currentInput: string): VariableSuggestion { const suggestions: VariableSuggestion = [];
  ...
  tag.syntax.toLowerCase().includes(searchTerm) });
    ...
    for (const tag of matchingTags) { suggestions.push({)
  name: tag.syntax,
        category: 'custom' }
  ...
  private sortByDependencies(components: ZadaTemplateComponent): ZadaTemplateComponent { const sorted: ZadaTemplateComponent = [];
  ...
  remaining.splice(remaining.indexOf(nextComponent), 1) } else {
