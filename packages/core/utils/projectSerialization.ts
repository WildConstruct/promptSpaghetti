...
export function serializeProject(
 ...
 = options;
  ...
  exportedAt: new Date().toISOString() }
    ...
    if (includeCollaboration && graphState.annotations) { psgFile.collaboration = {
  ...
  nodeLabels: graphState.annotations.nodeLabels || {}
    ...
    try { // Generate checksum for integrity
...
export function deserializeProject(
 ...
 = options;
    // Parse JSON
    let psgData: unknown;
    try { psgData = JSON.parse(content) } catch (parseError) { return {
  success: false
  error: 'Invalid JSON format in .psg file' }
};

    // Validate file format
    if (!skipValidation) { const validation = validatePsgFile(psgData);
      if (!validation.success) {
        return {
          success: false }
