...
export const updateVariationInNode = (nodeData: NodeData,)
  index: number,
  newValue: string): NodeData => {,
  const currentVariations = nodeData.variations || [];
...
export const reorderVariationsInNode = (nodeData: NodeData,)
  fromIndex: number,
  toIndex: number): NodeData => {,
  const currentVariations = nodeData.variations || [];
  const updatedVariations = [...currentVariations];
