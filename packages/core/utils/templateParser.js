...
class TemplateParser {
    ...
    inferVariableType(variableName, template, context) {
        ...
        if (name.includes('character') || name.includes('creature') || name.includes('location') ||
            ...
            const matchingSuggestion = COMMON_VARIABLES.find(s => s.name === name ||
        ...
        const genericSuggestion = COMMON_VARIABLES.find(s => s.category === 'descriptive' || s.priority >= 7);
    ...
    getContextualSuggestions() {
        ...
        if (this.currentNodeType) {
            contextualSuggestions = COMMON_VARIABLES.filter(suggestion => suggestion.nodeTypes?.includes(this.currentNodeType));
        }
        // Related variable context
        if (this.currentVariables.length > 0) {
            const relatedSuggestions = COMMON_VARIABLES.filter(suggestion => suggestion.relatedVariables?.some(related => this.currentVariables.includes(related)) && !this.currentVariables.includes(suggestion.name));
            contextualSuggestions.push(...relatedSuggestions);
        }
        // Remove duplicates and sort by priority
        const uniqueSuggestions = contextualSuggestions.filter((suggestion, index, self) => index === self.findIndex(s => s.name === suggestion.name));
    ...
    getVariableSuggestions(partialName = '', context, includeHistory = true) {
        ...
        // Remove duplicates
        const uniqueSuggestions = allSuggestions.filter((suggestion, index, self) => index === self.findIndex(s => s.name === suggestion.name));
