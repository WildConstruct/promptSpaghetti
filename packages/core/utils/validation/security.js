...
class SecurityValidation {
     ...
     */
    static validateSafeString(value: unknown): boolean {
     ...
     */
    static validateSafeExpression(expression: unknown): boolean {
     ...
     */
    static validateVariableName(name: unknown): boolean {
        if (typeof name !== 'string')
            return false;
        if (name.length === 0)
            return false;
        if (name.length > exports.VARIABLE_NAME_MAX_LENGTH)
            return false;
     ...
     */
    static validateSafePropertyKey(key: unknown): boolean {
        if (typeof key !== 'string')
            return false;
        if (key.length === 0)
            return false;
        if (key.length > exports.VARIABLE_NAME_MAX_LENGTH)
     ...
     */
    static validateSafeValue(value: unknown): boolean {
...
exports.SecureValidation = {
     ...
     */
    safeString: (maxLength: number = 10000) => zod_1.z.string()
     ...
     */
    safeExpression: (maxLength: number = 500) => zod_1.z.string()
     ...
     */
    safePropertyKey: (maxLength: number = 64) => zod_1.z.string()
    ...
    variableName: () => zod_1.z.string()
        ...
        .max(exports.VARIABLE_NAME_MAX_LENGTH, `Variable name must be no longer than ${exports.VARIABLE_NAME_MAX_LENGTH} characters`)
        .regex(exports.VARIABLE_NAME_PATTERN, 'Variable name must contain only letters, numbers, underscore, or dash')
     ...
     */
    safeValue: () => zod_1.z.union([
        zod_1.z.string().max(10000).refine((val) => SecurityValidation.validateSafeString(val), { message: 'String value contains dangerous patterns' }),
        zod_1.z.number().finite().refine((val) => !isNaN(val), { message: 'Number value must be finite' }),
        zod_1.z.boolean(),
        zod_1.z.array(zod_1.z.string().max(1000)).max(1000).refine((val) => val.every(item => SecurityValidation.validateSafeString(item)), { message: 'Array contains dangerous values' }),
        zod_1.z.record(zod_1.z.string().max(1000)).refine((val) => {
     ...
     */
    safeArray: (itemSchema: any, maxLength: number = 1000) => zod_1.z.array(itemSchema)
        .max(maxLength, `Array must contain no more than ${maxLength} items`),
    /**
     * Safe object validation
     */
    safeObject: (valueSchema: any, maxKeys: number = 100) => zod_1.z.record(valueSchema)
...
class SecurityTesting {
     ...
     */
    static testInjectionProtection(validator: Function, testName: string = 'Unknown') {
        let passed = 0;
        let failed = 0;
