...
export class ProjectTemplateManager {
    ...
    async searchTemplates(criteria) {
        ...
        if (criteria.tags?.length) {
            filteredTemplates = filteredTemplates.filter(t => criteria.tags.some(tag => t.tags.includes(tag)));
        }
    ...
    async exportTemplate(templateId, format) {
        const template = this.templates.get(templateId);
        if (!template) {
            throw new Error(`Template ${templateId} not found`);
        }
    ...
    async importTemplate(templateData, format) {
        let template;
        switch (format) {
            case 'json':
