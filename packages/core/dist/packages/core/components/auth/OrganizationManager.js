...
import { Users, Settings, Building2, Plus, Edit2, Trash2, Globe, Crown, Shield, BarChart3, ChevronRight, Palette } from 'lucide-react';
export const OrganizationManager = ({ currentUser, onOrganizationChange, onInvitationSent, onMembershipUpdated }) => {
    ...
    const updateOrganization = async () => {
        ...
        try {
            ...
            const data = await response.json();
            setOrganizations(prev => prev.map(org => org.id === editingOrg.id ? data.data : org));
    ...
    return (_jsxs("div", { className: "max-w-7xl mx-auto p-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-8", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-3xl font-bold text-gray-900", children: "Organizations" }), _jsx("p", { className: "text-gray-600 mt-2", children: "Manage your organizations and teams" })] }), _jsxs("button", { onClick: () => setShowCreateForm(true), className: "flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors", children: [_jsx(Plus, { className: "w-4 h-4 mr-2" }), "Create Organization"] })] }), error && (_jsxs("div", { className: "mb-6 p-4 bg-red-100 border border-red-300 text-red-700 rounded-lg", children: [error, _jsx("button", { onClick: () => setError(null), className: "ml-2 text-red-500 hover:text-red-700", children: "\u00D7" })] })), _jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-4 gap-6", children: [_jsx("div", { className: "lg:col-span-1", children: _jsxs("div", { className: "bg-white rounded-lg border border-gray-200 overflow-hidden", children: [_jsx("div", { className: "p-4 border-b border-gray-200", children: _jsx("h2", { className: "text-lg font-semibold text-gray-900", children: "Organizations" }) }), _jsx("div", { className: "divide-y divide-gray-200", children: organizations.map((org) => (_jsx("div", { onClick: () => setSelectedOrg(org), className: `p-4 cursor-pointer hover:bg-gray-50 transition-colors ${selectedOrg?.id === org.id ? 'bg-blue-50 border-r-2 border-blue-600' : ''}`, children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h3", { className: "text-sm font-medium text-gray-900 truncate", children: org.name }), _jsxs("p", { className: "text-xs text-gray-500 mt-1", children: ["@", org.slug] }), _jsx("span", { className: `inline-block px-2 py-1 text-xs font-medium rounded-full mt-2 ${getPlanBadge(org.plan)}`, children: org.plan.charAt(0).toUpperCase() + org.plan.slice(1) })] }), _jsx(ChevronRight, { className: "w-4 h-4 text-gray-400" })] }) }, org.id))) })] }) }), _jsx("div", { className: "lg:col-span-3", children: selectedOrg ? (_jsxs("div", { className: "bg-white rounded-lg border border-gray-200", children: [_jsx("div", { className: "p-6 border-b border-gray-200", children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex items-center space-x-4", children: [_jsx("div", { className: "w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center", children: selectedOrg.logoUrl ? (_jsx("img", { src: selectedOrg.logoUrl, alt: selectedOrg.name, className: "w-12 h-12 object-cover rounded-lg" })) : (_jsx(Building2, { className: "w-8 h-8 text-gray-400" })) }), _jsxs("div", { children: [_jsx("h2", { className: "text-2xl font-bold text-gray-900", children: selectedOrg.name }), _jsxs("p", { className: "text-gray-600", children: ["@", selectedOrg.slug] }), selectedOrg.description && (_jsx("p", { className: "text-sm text-gray-500 mt-2", children: selectedOrg.description }))] })] }), _jsxs("div", { className: "flex space-x-2", children: [_jsx("button", { onClick: () => startEditing(selectedOrg), className: "p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors", children: _jsx(Edit2, { className: "w-4 h-4" }) }), _jsx("button", { onClick: () => deleteOrganization(selectedOrg.id), className: "p-2 text-red-600 hover:text-red-900 hover:bg-red-100 rounded-lg transition-colors", children: _jsx(Trash2, { className: "w-4 h-4" }) })] })] }) }), _jsx("div", { className: "border-b border-gray-200", children: _jsx("nav", { className: "flex space-x-8 px-6", children: [
