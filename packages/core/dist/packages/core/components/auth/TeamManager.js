...
import { Users, Plus, Edit2, Trash2, Crown, Shield, User, Eye, ChevronRight, ChevronDown, UserPlus, Settings, Activity } from 'lucide-react';
export const TeamManager = ({ organizationId, currentUser, onTeamChange, onMembershipUpdated }) => {
    ...
    const renderTeamTree = (teamList, parentId, level = 0) => {
        ...
        return filteredTeams.map((team) => {
            ...
            return (_jsxs("div", { children: [_jsxs("div", { onClick: () => setSelectedTeam(team), className: `flex items-center p-3 cursor-pointer hover:bg-gray-50 transition-colors ${selectedTeam?.id === team.id ? 'bg-blue-50 border-r-2 border-blue-600' : ''}`, style: { paddingLeft: `${level * 20 + 12}px` }, children: [hasChildren && (_jsx("button", { onClick: (e) => {
                                    e.stopPropagation();
                                    toggleTeamExpansion(team.id);
                                }, className: "mr-2", children: isExpanded ? (_jsx(ChevronDown, { className: "w-4 h-4 text-gray-400" })) : (_jsx(ChevronRight, { className: "w-4 h-4 text-gray-400" })) })), !hasChildren && _jsx("div", { className: "w-6" }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h3", { className: "text-sm font-medium text-gray-900 truncate", children: team.name }), team.description && (_jsx("p", { className: "text-xs text-gray-500 truncate", children: team.description }))] })] }), hasChildren && isExpanded && (_jsx("div", { children: renderTeamTree(teamList, team.id, level + 1) }))] }, team.id));
        });
    };
    if (loading) {
