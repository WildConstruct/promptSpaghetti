...
JSON.stringify(customization.branding),
    social;
JSON.stringify(customization.social);
;
switch (activeTab) {
    case 'iframe':
        return `<iframe
...
return;
_jsx("div", { className: "bg-white border border-gray-200 rounded-lg", children: _jsxs("div", { className: "border-b border-gray-200", children: [_jsxs("div", { className: "flex items-center justify-between p-4", children: [_jsx("h3", { className: "font-semibold text-gray-900", children: "Embed Code" }), _jsxs("button", { onClick: copyToClipboard, className: "flex items-center gap-2 px-3 py-1 bg-blue-600 text-white hover:bg-blue-700 rounded text-sm", children: [copied ? _jsx(CheckIcon, { className: "h-4 w-4" }) : _jsx(ClipboardDocumentIcon, { className: "h-4 w-4" }), copied ? 'Copied!' : 'Copy'] })] }), _jsxs("nav", { className: "flex space-x-8 px-4", children: [[
...
export const EmbedCustomizationPanel, EmbedCustomization;
onCustomizationChange: (updates) => void ;
...
const updateFeatures = useCallback((updates) => {
    onCustomizationChange({});
    features: { }
}, ...customization.features, ...updates);
;
...
;
return;
_jsxs("div", { className: "bg-white border border-gray-200 rounded-lg", children: [_jsxs("div", { className: "border-b border-gray-200", children: [_jsx("h3", { className: "font-semibold text-gray-900 p-4", children: "Customize Embed" }), _jsxs("nav", { className: "flex space-x-6 px-4", children: [[
                            ...
                            }`, ">", theme] })), "))}"] });
{
    customization.branding.customColors && ()
        < div;
    className = "space-y-3" >
        _jsx("h5", { className: "font-medium text-gray-700", children: "Custom Colors" });
    {
...
;
{
    ...
    div >
        _jsx("div", { className: "space-y-3", children: [
                ...
                (_jsx("input", { type: "checkbox", checked: customization.layout[option.key], onChange: (e) => updateLayout({ [option.key]: e.target.checked }), className: "h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" })
                    ,
                        _jsx("span", { className: "text-sm text-gray-700", children: option.label }))) });
}
div >
;
div >
;
{
    activeSection === 'features' && ()
        < div;
    className = "space-y-3" >
        _jsx("h4", { className: "font-medium text-gray-700 mb-3", children: "Features" });
    {
        Object.entries(customization.features).map(([key, value]) => ()
            < label, key = { key }, className = "flex items-center gap-3" >
            (_jsx("input", { type: "checkbox", checked: value, onChange: (e) => updateFeatures({ [key]: e.target.checked }), className: "h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" })
                ,
                    _jsx("span", { className: "text-sm text-gray-700 capitalize", children: key.replace(/([A-Z])/g, ' $1').toLowerCase() })), label >
        );
    }
    div >
    ;
}
{
    activeSection === 'branding' && ()
        < div;
    className = "space-y-4" >
        (_jsx("h4", { className: "font-medium text-gray-700 mb-3", children: "Branding Options" })
            ,
                _jsx("div", { className: "space-y-3", children: [
                        ...
                        < label, key = { option, : .key }, className = "flex items-center gap-3" >
                        (_jsx("input", { type: "checkbox", checked: customization.branding[option.key], onChange: (e) => updateBranding({ [option.key]: e.target.checked }), className: "h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" })
                            ,
                                _jsx("span", { className: "text-sm text-gray-700", children: option.label }))) }));
}
div >
...
export const EmbeddedTemplatePreview = ({
    template,
    embedConfig,
...
});
{
    ...
    size: {
        width: previewSize.width, height;
        previewSize.height, responsive;
        true;
    }
    theme: {
        name: 'light', colors;
...
;
{
    ...
    className = "flex items-center gap-2" >
        { customization, : .social.enableInteraction && ()
                <  >
                (_jsxs("button", { onClick: () => onLike?.(template), className: "flex items-center gap-1 px-2 py-1 text-xs text-gray-600 hover:text-red-500 rounded", children: [_jsx(HeartIcon, { className: "h-3 w-3" }), "Like"] })
                    ,
                        _jsxs("button", { onClick: () => handleShare({ type: 'social', platform: 'twitter' }), className: "flex items-center gap-1 px-2 py-1 text-xs text-gray-600 hover:text-blue-500 rounded", children: [_jsx(ShareIcon, { className: "h-3 w-3" }), "Share"] }))
        } >
    ;
}
{
...
className = "flex items-center gap-1 px-2 py-1 text-xs bg-blue-600 text-white hover:bg-blue-700 rounded ml-auto"
    >
        _jsx(DownloadIcon, { className: "h-3 w-3" });
$;
{
    template.price;
}
...
;
{
    customization.layout.showFooter && customization.branding.showPoweredBy && ()
        < div;
    className = "border-t border-gray-100 pt-2 mt-3" >
        _jsxs("p", { className: "text-xs text-gray-500 text-center", children: ["Powered by ", _jsx("span", { className: "text-blue-600", children: "PromptSpaghetti" })] });
    div >
    ;
}
div >
    ...
    { /* Customization panel */};
{
    isCustomizing && ()
        < div;
    className = "space-y-6" >
        (_jsx(EmbedCustomizationPanel, { customization: customization, onCustomizationChange: handleCustomizationChange, previewSize: previewSize, onPreviewSizeChange: setPreviewSize })
            ,
                _jsx(EmbedCodeGenerator, { template: template, customization: customization, onCustomizationChange: handleCustomizationChange }));
    div >
    ;
}
div >
    { /* Quick actions */};
{
    ...
    className = "border-t border-gray-200 p-4" >
        _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-4 text-sm text-gray-600", children: [_jsx("span", { children: "Share this template:" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { onClick: () => handleShare({ type: 'link' }), className: "p-1 text-gray-400 hover:text-blue-600 rounded", title: "Copy Link", children: _jsx(LinkIcon, { className: "h-4 w-4" }) }), _jsx("button", { onClick: () => handleShare({ type: 'embed' }), className: "p-1 text-gray-400 hover:text-green-600 rounded", title: "Embed Code", children: _jsx(CodeBracketIcon, { className: "h-4 w-4" }) }), _jsx("button", { onClick: () => handleShare({ type: 'social', platform: 'twitter' }), className: "p-1 text-gray-400 hover:text-blue-400 rounded", title: "Share on Twitter", children: _jsx(ChatBubbleLeftIcon, { className: "h-4 w-4" }) })] })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("button", { onClick: () => onPreview?.(template), className: "flex items-center gap-1 px-3 py-1 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded text-sm", children: [_jsx(EyeIcon, { className: "h-4 w-4" }), "Full Preview"] }), showPurchaseButton && ()
                            < button, "onClick=", () => onPurchase?.(template), "className=\"flex items-center gap-1 px-3 py-1 bg-blue-600 text-white hover:bg-blue-700 rounded text-sm\" >", _jsx(DownloadIcon, { className: "h-4 w-4" }), "Purchase $", template.price] }), ")}"] });
    div >
    ;
    div >
    ;
