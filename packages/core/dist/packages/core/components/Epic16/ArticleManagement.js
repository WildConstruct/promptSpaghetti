...
const getStatusColor = (status) => {
    ...
    const getDifficultyColor = (difficulty) => {
        ...
        ;
        return;
        _jsx("div", { className: "space-y-4", children: filteredAndSortedArticles.map((article) => ()
                < div, key = { article, : .id }, className = "bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow"
                >
                    _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [article.featured && ()
                                                < StarIcon, " className=\"h-5 w-5 text-yellow-500 fill-current\" /> )}", _jsx("h3", { className: "text-lg font-semibold text-gray-900 hover:text-blue-600 cursor-pointer", onClick: () => onView(article), children: article.title })] }), _jsx("p", { className: "text-gray-600 mb-3 line-clamp-2", children: article.excerpt }), _jsxs("div", { className: "flex items-center gap-4 text-sm text-gray-500 mb-3", children: [_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(UserIcon, { className: "h-4 w-4" }), _jsx("span", { children: article.author.name })] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(ClockIcon, { className: "h-4 w-4" }), _jsxs("span", { children: [article.readTime, " min read"] })] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(EyeIcon, { className: "h-4 w-4" }), _jsx("span", { children: article.viewCount.toLocaleString() })] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(HeartIcon, { className: "h-4 w-4" }), _jsx("span", { children: article.likeCount })] })] }), _jsxs("div", { className: "flex items-center gap-2 mb-3", children: [_jsxs("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(article.status)}`, children: ["}", article.status] }), _jsxs("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${getDifficultyColor(article.difficulty)}`, children: ["}", article.difficulty] }), _jsx("span", { className: "px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-medium", children: article.category.name })] }), _jsx("div", { className: "flex flex-wrap gap-1", children: article.tags.slice(0, 3).map((tag) => ()
                                            < span, key = { tag }, className = "px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs" >
                                            #, { tag }) }), "))}", article.tags.length > 3 && ()
                                        < span, " className=\"text-xs text-gray-500\">+", article.tags.length - 3, " more"] }), ")}"] })) })
            ,
                _jsxs("div", { className: "flex items-center gap-2 ml-4", children: [_jsx("button", { onClick: () => onView(article), className: "p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded", title: "View Article", children: _jsx(EyeIcon, { className: "h-5 w-5" }) }), (currentUser.role === 'admin' || currentUser.id === article.author.id) && ()
                            <  >
                            (_jsx("button", { onClick: () => onEdit(article), className: "p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded", title: "Edit Article", children: _jsx(PencilIcon, { className: "h-5 w-5" }) })
                                ,
                                    _jsx("button", { onClick: () => onDuplicate(article), className: "p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded", title: "Duplicate Article", children: _jsx(DocumentIcon, { className: "h-5 w-5" }) })
                                        ,
                                            _jsx("button", { onClick: () => onDelete(article), className: "p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded", title: "Delete Article", children: _jsx(TrashIcon, { className: "h-5 w-5" }) }))] });
    };
};
div >
;
...
;
{
    ...
    className = "text-center py-12" >
        (_jsx(DocumentIcon, { className: "h-12 w-12 text-gray-400 mx-auto mb-4" })
            ,
                _jsx("p", { className: "text-gray-500 text-lg", children: "No articles found" })
                    ,
                        _jsx("p", { className: "text-gray-400", children: "Try adjusting your filters or search query" }));
    div >
    ;
}
div >
...
_jsxs("div", { className: "bg-white rounded-lg border border-gray-200 p-6", children: [_jsxs("div", { className: "flex items-center justify-between mb-6", children: [_jsx("h2", { className: "text-xl font-semibold text-gray-900", children: article ? 'Edit Article' : 'Create New Article' }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsx("button", { onClick: onCancel, className: "px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md", disabled: isSaving, children: "Cancel" }), _jsx("button", { onClick: handleSave, disabled: isSaving || !formData.title?.trim(), className: "px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md disabled:opacity-50 disabled:cursor-not-allowed", children: isSaving ? 'Saving...' : (article ? 'Update' : 'Create') })] })] }), _jsxs("div", { className: "space-y-6", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Title *" }), _jsx("input", { type: "text", value: formData.title || '', onChange: (e) => handleInputChange('title', e.target.value), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: "Enter article title..." })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Excerpt" }), _jsx("textarea", { value: formData.excerpt || '', onChange: (e) => handleInputChange('excerpt', e.target.value), rows: 3, className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: "Brief description of the article..." })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Content *" }), _jsx("textarea", { value: formData.content || '', onChange: (e) => handleInputChange('content', e.target.value), rows: 12, className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: "Write your article content here... (Supports Markdown)" })] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4", children: _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: "Category" }), _jsx("select", { value: formData.category?.id || '', onChange: (e) => {
    ...
    ,
        _jsxs("div", { className: "flex gap-2", children: [_jsx("input", { type: "text", value: newTag, onChange: (e) => setNewTag(e.target.value), onKeyPress: (e) => e.key === 'Enter' && (e.preventDefault(), handleAddTag()), className: "flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: "Add tag..." }), _jsx("button", { onClick: handleAddTag, disabled: !newTag.trim(), className: "px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md disabled:opacity-50", children: "Add" })] });
div >
    { /* Featured toggle */}
    < div;
className = "flex items-center" >
    (_jsx("input", { type: "checkbox", id: "featured", checked: formData.featured || false, onChange: (e) => handleInputChange('featured', e.target.checked), className: "h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" })
        ,
            _jsx("label", { htmlFor: "featured", className: "ml-2 block text-sm text-gray-900", children: "Feature this article" }));
div >
;
div >
;
...
});
{
    ...
    _jsxs("div", { className: `bg-gray-50 min-h-screen ${className}`, children: ["}", _jsxs("div", { className: "max-w-7xl mx-auto px-4 py-8", children: [_jsxs("div", { className: "mb-8", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsx("h1", { className: "text-2xl font-bold text-gray-900", children: "Knowledge Base Management" }), _jsxs("button", { onClick: handleCreateNew, className: "flex items-center gap-2 px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md", children: [_jsx(PlusIcon, { className: "h-5 w-5" }), "New Article"] })] }), _jsx("div", { className: "border-b border-gray-200", children: _jsxs("nav", { className: "-mb-px flex space-x-8", children: [_jsxs("button", { onClick: () => setActiveTab('list'), className: `py-2 px-1 border-b-2 font-medium text-sm ${activeTab === 'list'
                ...
                < div, " className=\"bg-white rounded-lg border border-gray-200 p-6\">", _jsxs("div", { className: "flex items-center justify-between mb-6", children: [_jsx("h2", { className: "text-xl font-semibold text-gray-900", children: "Article Categories" }), _jsxs("button", { className: "flex items-center gap-2 px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-md", children: [_jsx(PlusIcon, { className: "h-5 w-5" }), "New Category"] })] }), _jsx("div", { className: "space-y-4", children: categories.map((category) => ()
                    ...
                    (_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: "w-4 h-4 rounded-full", style: { backgroundColor: category.color } }), _jsxs("div", { children: [_jsx("h3", { className: "font-medium text-gray-900", children: category.name }), _jsx("p", { className: "text-sm text-gray-500", children: category.description })] })] })
                        ,
                            _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("span", { className: "text-sm text-gray-500", children: [category.articleCount, " articles"] }), _jsx("button", { className: "p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded", children: _jsx(PencilIcon, { className: "h-4 w-4" }) })] }))) }), "))}"] });
    div >
    ;
}
div >
