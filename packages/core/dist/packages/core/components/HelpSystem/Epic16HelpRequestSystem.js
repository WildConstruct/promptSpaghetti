...
_jsxs("div", { className: "flex-1 overflow-hidden", children: [view === 'dashboard' && ()
            ...
            < div, " className=\"p-6 overflow-y-auto\">", _jsx(HelpRequestForm, { helpService: helpService, userId: userId, userType: userType, userTier: userTier, onSubmitted: handleFormSubmitted, onCancel: () => handleViewChange('dashboard') })] });
{
...
const NotificationSystem = ({ notifications }) => {
    if (notifications.length === 0)
        return null;
    return;
    _jsxs("div", { className: "fixed top-4 right-4 z-50 space-y-2", children: [notifications.slice(-3).map((notification) => {
                const colors = {
...
div >
    _jsxs("div", { className: "ml-3", children: [_jsx("p", { className: "text-sm font-medium", children: notification.message }), _jsx("p", { className: "text-xs mt-1 opacity-75", children: notification.timestamp.toLocaleTimeString() })] });
div >
;
div >
;
