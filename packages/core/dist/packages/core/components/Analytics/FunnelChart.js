 ...
 */
export const FunnelChart = ({ funnelDefinition, analyticsInfrastructure, timeRange, segments = [], cohorts = [], chartMode = 'standard', showDropoffAnalysis = true, realTimeUpdates = false, onStepClick, onExport }) => {
    ...
    const loadChartData = useCallback(async () => {
        try {
            ...
            const processedData = await processFunnelChartData(funnelDefinition, results, segments, cohorts, timeRange);
    ...
    if (error || !chartData) {
        return (_jsx(FunnelChartErrorState, { error: error || 'No data available', onRetry: loadChartData }));
    }
    return (_jsxs("div", { className: "funnel-chart", ref: containerRef, children: [_jsx(FunnelChartHeader, { funnelDefinition: funnelDefinition, overallMetrics: chartData.overallMetrics, chartMode: chartMode, onExport: handleExport }), _jsx("div", { className: "chart-container", children: chartDimensions && (_jsxs("svg", { ref: chartRef, width: chartDimensions.width, height: chartDimensions.height, className: "funnel-chart-svg", children: [chartMode === 'standard' && (_jsx(StandardFunnelChart, { data: chartData, dimensions: chartDimensions, interactionState: interactionState, onStepHover: handleStepHover, onStepClick: handleStepClick })), chartMode === 'horizontal' && (_jsx(HorizontalFunnelChart, { data: chartData, dimensions: chartDimensions, interactionState: interactionState, onStepHover: handleStepHover, onStepClick: handleStepClick })), chartMode === 'sankey' && (_jsx(SankeyFunnelChart, { data: chartData, dimensions: chartDimensions, interactionState: interactionState, onStepHover: handleStepHover, onStepClick: handleStepClick })), chartMode === 'waterfall' && (_jsx(WaterfallFunnelChart, { data: chartData, dimensions: chartDimensions, interactionState: interactionState, onStepHover: handleStepHover, onStepClick: handleStepClick }))] })) }), showDropoffAnalysis && chartData.dropoffAnalysis.length > 0 && (_jsx(DropoffAnalysisPanel, { dropoffAnalysis: chartData.dropoffAnalysis })), chartData.segmentComparisons.length > 0 && (_jsx(SegmentComparisonPanel, { segmentComparisons: chartData.segmentComparisons })), interactionState.tooltipContent && interactionState.tooltipPosition && (_jsx(StepTooltip, { content: interactionState.tooltipContent, position: interactionState.tooltipPosition, onClose: () => handleStepHover(null) }))] }));
};
const FunnelChartHeader = ({ funnelDefinition, overallMetrics, chartMode, onExport }) => {
    return (_jsxs("div", { className: "funnel-chart-header", children: [_jsxs("div", { className: "funnel-info", children: [_jsx("h3", { children: funnelDefinition.name }), _jsx("p", { children: funnelDefinition.description })] }), _jsxs("div", { className: "overall-metrics", children: [_jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Total Entries" }), _jsx("span", { className: "value", children: overallMetrics.totalEntries.toLocaleString() })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Conversion Rate" }), _jsxs("span", { className: "value", children: [overallMetrics.overallConversionRate.toFixed(2), "%"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Total Revenue" }), _jsxs("span", { className: "value", children: ["$", overallMetrics.totalRevenue.toLocaleString()] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Avg. Time to Convert" }), _jsx("span", { className: "value", children: formatDuration(overallMetrics.averageTimeToConvert) })] })] }), _jsxs("div", { className: "chart-controls", children: [_jsx("span", { className: "chart-mode", children: chartMode }), _jsx("button", { onClick: onExport, className: "export-button", children: "Export Chart" })] })] }));
};
const StandardFunnelChart = ({ data, dimensions, interactionState, onStepHover, onStepClick }) => {
    const { steps } = data;
    ...
    return (_jsx("g", { transform: `translate(${margin.left}, ${margin.top})`, children: steps.map((step, index) => {
            ...
            return (_jsxs("g", { children: [_jsx("rect", { x: x, y: y, width: width, height: stepBarHeight, fill: getStepColor(step.conversionRate, index), stroke: isSelected ? '#3b82f6' : 'none', strokeWidth: isSelected ? 2 : 0, opacity: isHovered ? 0.8 : 1, className: "funnel-step-bar", onMouseEnter: (e) => onStepHover(step.stepId, e), onMouseLeave: () => onStepHover(null), onClick: () => onStepClick(step.stepId) }), _jsx("text", { x: chartWidth / 2, y: y + stepBarHeight / 2, textAnchor: "middle", dominantBaseline: "middle", className: "step-label", fill: "white", fontSize: "14", fontWeight: "500", children: step.stepName }), _jsxs("text", { x: chartWidth / 2, y: y + stepBarHeight / 2 + 20, textAnchor: "middle", dominantBaseline: "middle", className: "conversion-rate", fill: "white", fontSize: "12", children: [step.conversionRate.toFixed(1), "%"] }), index < steps.length - 1 && step.dropOffCount > 0 && (_jsxs("g", { className: "dropoff-indicator", children: [_jsx("line", { x1: x + width, y1: y + stepBarHeight, x2: x + width + 20, y2: y + stepBarHeight + 10, stroke: "#ef4444", strokeWidth: "2" }), _jsxs("text", { x: x + width + 25, y: y + stepBarHeight + 15, fontSize: "10", fill: "#ef4444", className: "dropoff-text", children: ["-", step.dropOffCount.toLocaleString()] })] }))] }, step.stepId));
        }) }));
};
/**
 * Horizontal Funnel Chart Component
 */
const HorizontalFunnelChart = ({ data, dimensions, interactionState, onStepHover, onStepClick }) => {
    const { steps } = data;
    const { chartWidth, chartHeight, margin } = dimensions;
    const maxEntries = Math.max(...steps.map(s => s.totalEntries));
    ...
    return (_jsx("g", { transform: `translate(${margin.left}, ${margin.top})`, children: steps.map((step, index) => {
            ...
            const isHovered = interactionState.hoveredStep === step.stepId;
            const isSelected = interactionState.selectedStep === step.stepId;
            return (_jsxs("g", { children: [_jsx("rect", { x: x, y: y, width: stepBarWidth, height: height, fill: getStepColor(step.conversionRate, index), stroke: isSelected ? '#3b82f6' : 'none', strokeWidth: isSelected ? 2 : 0, opacity: isHovered ? 0.8 : 1, className: "funnel-step-bar", onMouseEnter: (e) => onStepHover(step.stepId, e), onMouseLeave: () => onStepHover(null), onClick: () => onStepClick(step.stepId) }), _jsx("text", { x: x + stepBarWidth / 2, y: chartHeight + 20, textAnchor: "middle", className: "step-label", fontSize: "12", children: step.stepName }), _jsxs("text", { x: x + stepBarWidth / 2, y: y + height / 2, textAnchor: "middle", dominantBaseline: "middle", className: "conversion-rate", fill: "white", fontSize: "12", fontWeight: "500", children: [step.conversionRate.toFixed(1), "%"] })] }, step.stepId));
        }) }));
};
// Placeholder components for other chart modes
const SankeyFunnelChart = () => (_jsx("g", { children: _jsx("text", { x: "50%", y: "50%", textAnchor: "middle", fontSize: "16", fill: "#666", children: "Sankey Chart (Coming Soon)" }) }));
const WaterfallFunnelChart = () => (_jsx("g", { children: _jsx("text", { x: "50%", y: "50%", textAnchor: "middle", fontSize: "16", fill: "#666", children: "Waterfall Chart (Coming Soon)" }) }));
const DropoffAnalysisPanel = ({ dropoffAnalysis }) => {
    const criticalDropoffs = dropoffAnalysis
        .filter(d => d.severity === 'critical' || d.severity === 'high')
        .sort((a, b) => b.dropOffRate - a.dropOffRate);
    return (_jsxs("div", { className: "dropoff-analysis-panel", children: [_jsx("h4", { children: "Drop-off Analysis" }), _jsx("div", { className: "critical-dropoffs", children: criticalDropoffs.map(dropoff => (_jsxs("div", { className: `dropoff-item ${dropoff.severity}`, children: [_jsxs("div", { className: "dropoff-header", children: [_jsx("h5", { children: dropoff.stepName }), _jsxs("span", { className: "dropoff-rate", children: [dropoff.dropOffRate.toFixed(1), "%"] })] }), _jsxs("div", { className: "dropoff-details", children: [_jsxs("p", { children: [dropoff.dropOffCount.toLocaleString(), " users dropped off at this step"] }), dropoff.dropOffReasons.slice(0, 2).map((reason, index) => (_jsxs("div", { className: "dropoff-reason", children: [_jsx("span", { className: "reason", children: reason.reason }), _jsxs("span", { className: "percentage", children: [reason.percentage.toFixed(1), "%"] })] }, index)))] }), dropoff.recommendedActions.length > 0 && (_jsxs("div", { className: "recommended-actions", children: [_jsx("strong", { children: "Recommended Actions:" }), _jsx("ul", { children: dropoff.recommendedActions.slice(0, 2).map((action, index) => (_jsx("li", { children: action }, index))) })] }))] }, dropoff.stepId))) })] }));
};
const SegmentComparisonPanel = ({ segmentComparisons }) => {
    return (_jsxs("div", { className: "segment-comparison-panel", children: [_jsx("h4", { children: "Segment Performance" }), _jsx("div", { className: "segment-grid", children: segmentComparisons.map(segment => (_jsxs("div", { className: "segment-comparison-card", children: [_jsx("h5", { children: segment.segmentName }), _jsxs("div", { className: "overall-rate", children: [segment.overallConversionRate.toFixed(1), "% conversion rate"] }), segment.insights.length > 0 && (_jsx("div", { className: "segment-insights", children: segment.insights.slice(0, 2).map((insight, index) => (_jsx("p", { className: "insight", children: insight }, index))) }))] }, segment.segmentId))) })] }));
};
const StepTooltip = ({ content, position, onClose }) => {
    return (_jsxs("div", { className: "step-tooltip", style: {
            position: 'fixed',
            left: position.x,
            top: position.y,
            zIndex: 1000
        }, children: [_jsxs("div", { className: "tooltip-header", children: [_jsx("h4", { children: content.stepName }), _jsx("button", { onClick: onClose, className: "close-button", children: "\u00D7" })] }), _jsxs("div", { className: "tooltip-metrics", children: [_jsxs("div", { className: "metric", children: [_jsx("span", { children: "Entries:" }), _jsx("span", { children: content.metrics.totalEntries.toLocaleString() })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { children: "Conversions:" }), _jsx("span", { children: content.metrics.totalConversions.toLocaleString() })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { children: "Conversion Rate:" }), _jsxs("span", { children: [content.metrics.conversionRate.toFixed(2), "%"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { children: "Drop-off:" }), _jsxs("span", { children: [content.metrics.dropOffCount.toLocaleString(), " (", content.metrics.dropOffRate.toFixed(1), "%)"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { children: "Revenue:" }), _jsxs("span", { children: ["$", content.metrics.revenue.toLocaleString()] })] })] }), content.insights.length > 0 && (_jsxs("div", { className: "tooltip-insights", children: [_jsx("strong", { children: "Insights:" }), _jsx("ul", { children: content.insights.slice(0, 3).map((insight, index) => (_jsx("li", { children: insight }, index))) })] }))] }));
};
// Loading and Error States
const FunnelChartLoadingState = () => (_jsxs("div", { className: "funnel-chart-loading", children: [_jsx("div", { className: "loading-spinner" }), _jsx("p", { children: "Loading funnel chart..." })] }));
const FunnelChartErrorState = ({ error, onRetry }) => (_jsxs("div", { className: "funnel-chart-error", children: [_jsxs("div", { className: "error-message", children: [_jsx("h3", { children: "Error Loading Chart" }), _jsx("p", { children: error })] }), _jsx("button", { onClick: onRetry, className: "retry-button", children: "Retry Loading" })] }));
// Utility Functions
...
async function processFunnelChartData(funnelDefinition, metricResults, segments, cohorts, timeRange) {
    // Simplified implementation - in production would process actual metrics
    const steps = funnelDefinition.steps.map((step, index) => ({
        stepId: step.id,
        stepName: step.name,
        order: step.order,
        ...
        comparisonData: {
            ...
            segments: segments.slice(0, 2).map(segment => ({
                ...
                performance: Math.random() > 0.5 ? 'above_average' : 'below_average'
            }))
        }
    }));
    ...
    const segmentComparisons = segments.map(segment => ({
        segmentId: segment.id,
        segmentName: segment.name,
