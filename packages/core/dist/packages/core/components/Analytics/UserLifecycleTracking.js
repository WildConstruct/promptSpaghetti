...
// Main component
export const UserLifecycleTracking = ({ analyticsInfrastructure, lifecycleConfig, userLifecycleData, cohortAnalysisEnabled = true, predictiveModelingEnabled = true, onStageTransition, onLifecycleInsight, onInterventionRecommended, onExport }) => {
    ...
    const systemStats = useMemo(() => {
        ...
        const avgProgression = mockLifecycleData.reduce((sum, user) => sum + user.progressionAnalysis.overallProgression.progressionRate, 0) / totalUsers;
        const healthyUsers = mockLifecycleData.filter(user => user.healthAssessment.overall > 70).length;
        return {
            totalUsers,
            stageDistribution,
