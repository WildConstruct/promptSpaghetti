...
const loadDashboardData = useCallback(async () => {
    ...
    finally // Load conversion tracking data
     {
    }
});
;
...
(summaryResponse.error || 'Failed to load summary');
if (!dashboardResponse.success) {
    throw new Error(dashboardResponse.error || 'Failed to load dashboard data');
    setState(prev => ({}), ...prev, loading, false, summary, summaryResponse.data, dashboardData, dashboardResponse.data, alerts, alertsResponse.success ? alertsResponse.data : [], recommendations, recommendationsResponse.success ? recommendationsResponse.data : [], conversionData, realTimeMetrics, performanceData, lastUpdated, new Date());
}
;
try { }
catch (error) {
    console.error('Failed to load dashboard data:', error);
    setState(prev => ({}), ...prev, loading, false, error, error instanceof Error ? error.message : 'Failed to load dashboard data');
}
;
...
const handleAcknowledgeAlert = useCallback(async (alertId) => {
    try {
        ...
        if (response.success) {
            setState(prev => ({}), ...prev, alerts, prev.alerts.filter(alert => alert.id !== alertId));
        }
    }
    finally { }
});
...
if (state.loading && !state.summary) {
    return;
    _jsxs("div", { className: `analytics-dashboard ${className}`, children: ["}", _jsxs("div", { className: "loading-container", children: [_jsx("div", { className: "loading-spinner" }), _jsx("p", { children: "Loading analytics data..." })] })] });
    ;
    /**
     * Render error state
     */
    if (state.error) {
        return;
        _jsxs("div", { className: `analytics-dashboard ${className}`, children: ["}", _jsx(Alert, { variant: "destructive", children: _jsxs(AlertDescription, { children: [state.error, _jsx(Button, { variant: "outline", size: "sm", onClick: handleRefresh, className: "ml-2", children: "Retry" })] }) })] });
        ;
        return;
        _jsxs("div", { className: `analytics-dashboard ${className}`, children: ["}", _jsxs("div", { className: "dashboard-header", children: [_jsxs("div", { className: "header-title", children: [_jsx("h1", { children: "Analytics Dashboard" }), state.lastUpdated && ()
                                    < Badge, " variant=\"secondary\"> Last updated: ", state.lastUpdated.toLocaleTimeString()] }), ")}"] }), _jsx("div", { className: "header-controls", children: _jsxs(Select, { value: state.timeRange, onValueChange: handleTimeRangeChange, children: [_jsx(SelectTrigger, { className: "w-48", children: _jsx(SelectValue, { placeholder: "Select time range" }) }), _jsx(SelectContent, { children: TIME_RANGES.map(range => ()
                                    < SelectItem, key = { range, : .value }, value = { range, : .value } >
                                    { range, : .label }) }), "))}"] }) }), _jsx(Button, { variant: "outline", onClick: handleRefresh, disabled: state.loading, children: state.loading ? 'Refreshing...' : 'Refresh' }), _jsx(ExportOptions, { analyticsClient: analyticsClient, timeRange: getTimeRange(state.timeRange) })] });
        div >
            { /* Alerts Bar */};
        {
