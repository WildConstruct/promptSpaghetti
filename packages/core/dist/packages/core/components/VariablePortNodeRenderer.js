...
try {
  ...
  var(--bg-tertiary)
            ...
            boxShadow: selected,
            'var(--shadow-node-selected)': 'var(--shadow-node)',
            position: 'relative',
            overflow: 'visible',
...
style = {};
{
    ...
    'var(--font-size-base)',
        marginBottom;
    (properties.length > 0 || hasVariablePorts) ? 'var(--space-2)' : 0,
        color;
    'var(--text-primary)',
        lineHeight;
    1.3,
    ...
    { /* Variable Ports Indicator */};
{
    ...
    style = {};
    {
        ...
        'var(--text-secondary)',
            marginBottom;
        properties.length > 0 ? 'var(--space-1)' : 0,
            display;
        'flex',
            flexWrap;
        'wrap',
...
{ /* Dynamic Variable Input Ports */ }
{
    variablePorts.map((port, index) => {
        ...
        _jsxs(React.Fragment, { children: [_jsx(Handle, { type: "target", position: Position.Left, id: port.id, style: {
                    ...
                    } }), ", border: `2px solid $", categoryColor, "`} }, cursor: 'crosshair', zIndex: 10, transition: 'all 0.2s ease', boxShadow: `var(--shadow-sm), 0 0 6px $", categoryColor, "40`} }, top: `$", yOffset, "px`} }, left: '-6px'; }} isConnectable=", true, "/>", _jsx("div", { style: {
                        position: 'absolute',
                        left: 12,
                        top: `${yOffset - 6}px`
                    } }), ", fontSize: 9, fontWeight: 600, color: categoryColor, background: 'var(--bg-secondary)', padding: '2px 4px', borderRadius: 'var(--radius-sm)', border: `1px solid $", categoryColor, "40`} }, whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 5, textShadow: '0 1px 2px rgba(0,0,0,0.5)', boxShadow: 'var(--shadow-xs)'; }} >", port.displayName] }, port.id);
        React.Fragment >
        ;
    });
}
