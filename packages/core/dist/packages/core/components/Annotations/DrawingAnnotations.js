...
const DRAWING_TOOLS = {
    freehand: { icon: _jsx(Pen, { className: "w-4 h-4" }), label: 'Pen', cursor: 'crosshair' },
    arrow: { icon: _jsx(ArrowRight, { className: "w-4 h-4" }), label: 'Arrow', cursor: 'crosshair' },
    circle: { icon: _jsx(Circle, { className: "w-4 h-4" }), label: 'Circle', cursor: 'crosshair' },
    rectangle: { icon: _jsx(Square, { className: "w-4 h-4" }), label: 'Rectangle', cursor: 'crosshair' },
    line: { icon: _jsx(Minus, { className: "w-4 h-4" }), label: 'Line', cursor: 'crosshair' },
    text: { icon: _jsx(Type, { className: "w-4 h-4" }), label: 'Text', cursor: 'text' },
    eraser: { icon: _jsx(Eraser, { className: "w-4 h-4" }), label: 'Eraser', cursor: 'crosshair' }
};
...
export const DrawingAnnotationsCanvas = ({ width, height, annotations, currentUser, backgroundImageUrl, onAnnotationsChange, onSave, readonly = false, showGrid = false, gridSize = 20, className = '' }) => {
    ...
    const [layerVisibility, setLayerVisibility] = useState(DRAWING_LAYERS.reduce((acc, layer) => ({ ...acc, [layer.id]: layer.defaultVisible }), {}));
    // Drawing style state
        ...
        switch (annotation.type) {
            ...
            case 'circle':
                if (annotation.points.length >= 2) {
                    ...
                    const radius = Math.sqrt(Math.pow(annotation.points[1].x - centerX, 2) +
            ...
            switch (currentTool) {
                ...
                case 'circle':
                    if (currentPoints.length === 2) {
                        ...
                        const radius = Math.sqrt(Math.pow(currentPoints[1].x - centerX, 2) +
    ...
    const handleMouseDown = useCallback((event) => {
        ...
        if (currentTool === 'eraser') {
            ...
            const annotationToErase = annotations.find(annotation => {
                ...
                return annotation.points.some(p => Math.abs(p.x - point.x) < 10 && Math.abs(p.y - point.y) < 10);
            });
    ...
    return (_jsx("div", { className: `drawing-annotations ${className}`, children: _jsxs(Card, { children: [_jsxs(CardHeader, { children: [_jsxs(CardTitle, { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Pen, { className: "w-5 h-5 text-blue-600" }), _jsx("span", { children: "Drawing Annotations" }), _jsxs(Badge, { variant: "secondary", children: [annotations.length, " drawings"] })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Button, { variant: "outline", size: "sm", onClick: () => setShowLayers(!showLayers), children: _jsx(Layers, { className: "w-4 h-4" }) }), _jsx(Button, { variant: "outline", size: "sm", onClick: () => setShowStyles(!showStyles), children: _jsx(Palette, { className: "w-4 h-4" }) }), onSave && (_jsxs(Button, { variant: "default", size: "sm", onClick: () => onSave(annotations), children: [_jsx(Save, { className: "w-4 h-4 mr-2" }), "Save"] }))] })] }), _jsxs("div", { className: "flex items-center gap-2 py-2 border-t", children: [_jsx("div", { className: "flex items-center gap-1 pr-2 border-r", children: Object.entries(DRAWING_TOOLS).map(([tool, config]) => (_jsx(Button, { variant: currentTool === tool ? 'default' : 'ghost', size: "sm", onClick: () => setCurrentTool(tool), disabled: readonly, title: config.label, children: config.icon }, tool))) }), _jsxs("div", { className: "flex items-center gap-1 pr-2 border-r", children: [_jsx(Button, { variant: "ghost", size: "sm", onClick: undo, disabled: readonly || historyIndex <= 0, title: "Undo", children: _jsx(Undo, { className: "w-4 h-4" }) }), _jsx(Button, { variant: "ghost", size: "sm", onClick: redo, disabled: readonly || historyIndex >= history.length - 1, title: "Redo", children: _jsx(Redo, { className: "w-4 h-4" }) })] }), _jsxs("div", { className: "flex items-center gap-1 pr-2 border-r", children: [_jsx(Button, { variant: "ghost", size: "sm", onClick: exportAsImage, title: "Export as Image", children: _jsx(Download, { className: "w-4 h-4" }) }), _jsx(Button, { variant: "ghost", size: "sm", onClick: clearAll, disabled: readonly, title: "Clear All", children: _jsx(Trash2, { className: "w-4 h-4" }) })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-sm text-gray-600", children: "Layer:" }), _jsxs(Select, { value: currentLayer.toString(), onValueChange: (value) => setCurrentLayer(parseInt(value)), children: [_jsx(SelectTrigger, { className: "w-32", children: _jsx(SelectValue, {}) }), _jsx(SelectContent, { children: DRAWING_LAYERS.map(layer => (_jsx(SelectItem, { value: layer.id.toString(), children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: layer.color } }), _jsx("span", { children: layer.name })] }) }, layer.id))) })] })] })] })] }), _jsx(CardContent, { children: _jsxs("div", { className: "space-y-4", children: [showStyles && (_jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: "text-sm", children: "Drawing Style" }) }), _jsx(CardContent, { className: "space-y-4", children: _jsxs("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-4", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-xs font-medium mb-2", children: "Color" }), _jsx("div", { className: "flex flex-wrap gap-1", children: DRAWING_COLORS.map(color => (_jsx("button", { className: `w-6 h-6 rounded border-2 ${drawingStyle.color === color ? 'border-gray-800' : 'border-gray-300'}`, style: { backgroundColor: color }, onClick: () => setDrawingStyle(prev => ({ ...prev, color })) }, color))) })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-xs font-medium mb-2", children: "Thickness" }), _jsx(Slider, { value: [drawingStyle.thickness], onValueChange: (value) => setDrawingStyle(prev => ({ ...prev, thickness: value[0] })), min: 1, max: 20, step: 1 }), _jsxs("div", { className: "text-xs text-gray-500 mt-1", children: [drawingStyle.thickness, "px"] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-xs font-medium mb-2", children: "Opacity" }), _jsx(Slider, { value: [drawingStyle.opacity * 100], onValueChange: (value) => setDrawingStyle(prev => ({ ...prev, opacity: value[0] / 100 })), min: 10, max: 100, step: 10 }), _jsxs("div", { className: "text-xs text-gray-500 mt-1", children: [Math.round(drawingStyle.opacity * 100), "%"] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-xs font-medium mb-2", children: "Line Style" }), _jsxs(Select, { value: drawingStyle.dashPattern ? 'dashed' : 'solid', onValueChange: (value) => setDrawingStyle(prev => ({
                                                                ...prev,
                                                                dashPattern: value === 'dashed' ? [5, 5] : undefined
                                                            })), children: [_jsx(SelectTrigger, { children: _jsx(SelectValue, {}) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "solid", children: "Solid" }), _jsx(SelectItem, { value: "dashed", children: "Dashed" })] })] })] })] }) })] })), showLayers && (_jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: "text-sm", children: "Layers" }) }), _jsx(CardContent, { children: _jsx("div", { className: "space-y-2", children: layerStats.map(layer => (_jsxs("div", { className: "flex items-center justify-between p-2 rounded border", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Switch, { checked: layer.visible, onCheckedChange: (checked) => setLayerVisibility(prev => ({ ...prev, [layer.id]: checked })) }), _jsx("div", { className: "w-4 h-4 rounded-full", style: { backgroundColor: layer.color } }), _jsx("span", { className: "text-sm font-medium", children: layer.name })] }), _jsx(Badge, { variant: "secondary", className: "text-xs", children: layer.count })] }, layer.id))) }) })] })), _jsxs("div", { className: "relative border border-gray-300 rounded-lg overflow-hidden", style: { width, height }, children: [backgroundImageUrl && (_jsx("canvas", { ref: backgroundCanvasRef, width: width, height: height, className: "absolute inset-0", style: {
                                            backgroundImage: `url(${backgroundImageUrl})`,
                                            backgroundSize: 'cover',
                                            backgroundPosition: 'center'
                                        } })), _jsx("canvas", { ref: canvasRef, width: width, height: height, className: "absolute inset-0" }), _jsx("canvas", { ref: overlayCanvasRef, width: width, height: height, className: "absolute inset-0 cursor-crosshair", onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: () => {
                                            setIsDrawing(false);
                                            setCurrentPoints([]);
                                        }, style: {
                                            cursor: DRAWING_TOOLS[currentTool]?.cursor || 'crosshair',
                                            pointerEvents: readonly ? 'none' : 'all'
                                        } }), readonly && (_jsx("div", { className: "absolute inset-0 bg-gray-500 bg-opacity-10 flex items-center justify-center", children: _jsx(Badge, { variant: "secondary", children: "Read Only" }) }))] })] }) })] }) }));
};
