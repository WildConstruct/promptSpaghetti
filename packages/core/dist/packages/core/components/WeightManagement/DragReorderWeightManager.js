    ...
    draggedItemId;
{
    ...
    onWeightChange: (optionId, weight) => void ;
    onLockToggle: (optionId) => void ;
    onItemSelect: (optionId, selected) => void ;
    isSelected: boolean;
...
'0 2px 8px rgba(0, 0, 0, 0.1)',
    transform;
isCurrentlyDragging ? 'scale(1.02)' : 'scale(1)',
    transition;
`all ${animationDuration}ms ease`;
cursor: option.locked ? 'default' : 'grab',
    opacity;
isCurrentlyDragging ? 0.9 : 1,
    userSelect;
'none';
{
    attributes;
...
_jsxs("div", { style: {
        ...
        minWidth: 'fit-content',
                ...
                position: 'relative',
                    ...
                    } }), ", height: '100%', backgroundColor: option.color || themeStyles.accent, borderRadius: '4px', transition: `width $", animationDuration, "ms ease`} }} />"] }), allowWeightEditing && ()
...
style = {};
{
    ...
    'none',
        color;
    option.locked ? themeStyles.accent : themeStyles.text,
        cursor;
    'pointer',
        padding;
    '4px',
...
{ /* Bulk Actions Panel */ }
{
    showBulkActions && enableBulkOperations && (_jsx("div", { style: {
            background: themeStyles.hover,
            border: `1px solid ${themeStyles.border}` }, "borderRadius:": true }));
    '8px',
        padding;
    '16px',
        marginBottom;
    ...
    (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '12px' }, children: [_jsxs("span", { style: { fontSize: '14px', fontWeight: 500 }, children: ["Bulk Actions (", selectedItems.size, " selected):"] }), _jsx("button", { onClick: () => handleBulkWeightChange('equal'), style: {
        ...
        ,
            _jsx("button", { onClick: () => handleBulkWeightChange('normalize'), style: {
                    background: 'transparent',
                    border: `1px solid ${themeStyles.accent}`
                }, "color:themeStyles": true, accent: true, "borderRadius:": true }));
'4px',
    padding;
'4px 8px',
    fontSize;
