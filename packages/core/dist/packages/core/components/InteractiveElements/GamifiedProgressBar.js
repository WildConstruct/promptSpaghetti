...
useEffect(() => {
    if (currentValue !== animationState.newValue) {
        ...
        setAnimationState(prev => ({}), ...prev, isAnimating, true, previousValue, newValue, currentValue);
    }
});
;
...
const triggerCelebration = useCallback(() => {
    const newParticles = Array.from({ length: 20 }, (_, i) => ({}), id, `particle-${Date.now()}-${i}`);
});
x: Math.random() * 100,
    y;
Math.random() * 100,
    color;
['#FFD700', '#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'][i % 5];
;
    ...
    return config.milestones.map((milestone) => {
                            ...
                            < div, " className=\"text-xs text-center leading-none\">", milestone.icon] }), ")}"] }, milestone.value);
        {
            showTooltip === milestone.label && ()
                < div;
            className = "absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs rounded py-1 px-2 whitespace-nowrap" >
                { milestone, : .label };
            {
                milestone.reward && ()
                    < div;
                className = "text-yellow-300" > Reward;
                {
                    milestone.reward;
                }
                div >
...
_jsx("div", { className: "absolute top-full left-1/2 transform -translate-x-1/2 border-l-2 border-r-2 border-t-4 border-transparent border-t-gray-900" });
div >
;
div >
;
...
if (recentlyUnlocked.length === 0)
    return null;
return;
_jsx("div", { className: "absolute top-full mt-4 left-0 right-0", children: recentlyUnlocked.map(achievement => ()
        < div, key = { achievement, : .id }, className = "bg-gradient-to-r from-yellow-400 to-orange-500 text-white rounded-lg p-3 mb-2 shadow-lg animate-bounce"
        >
            _jsxs("div", { className: "flex items-center space-x-3", children: [_jsx("div", { className: "text-2xl", children: achievement.icon }), _jsxs("div", { children: [_jsx("div", { className: "font-bold", children: achievement.title }), _jsx("div", { className: "text-sm opacity-90", children: achievement.description })] })] })) });
div >
;
;
;
    ...
    { /* Progress Label */};
{
    ...
    className = "flex justify-between items-center mt-2 text-sm" >
        (_jsxs("span", { style: { color: theme.text_color }, children: [formatValue(currentValue), " / ", formatValue(config.max_value)] })
            ,
                _jsxs("span", { className: "font-bold", style: { color: currentColor }, children: [progressPercentage.toFixed(1), "%"] }));
    div >
    ;
}
{ /* Next Milestone Info */ }
...
{ /* Milestone Celebration */ }
{
    ...
    className = "absolute inset-0 flex items-center justify-center" >
        _jsxs("div", { className: "bg-white rounded-lg shadow-xl p-6 text-center animate-scale-in border-4 border-yellow-400", children: [_jsx("div", { className: "text-4xl mb-2", children: "\uD83C\uDF89" }), _jsx("div", { className: "font-bold text-lg text-gray-900", children: "Milestone Reached!" }), _jsx("div", { className: "text-sm text-gray-600", children: animationState.milestoneJustReached.label }), animationState.milestoneJustReached.reward && ()
                    < div, " className=\"mt-2 text-sm text-yellow-600 font-medium\"> Reward: ", animationState.milestoneJustReached.reward] });
}
div >
;
div >
...
{ /* Completion Celebration */ }
{
    progressPercentage >= 100 && ()
        < div;
    className = "absolute top-full mt-4 left-0 right-0 text-center" >
        _jsxs("div", { className: "bg-gradient-to-r from-green-400 to-blue-500 text-white rounded-lg p-4 shadow-lg", children: [_jsx("div", { className: "text-3xl mb-2", children: "\uD83C\uDFC6" }), _jsx("div", { className: "font-bold text-xl", children: "Congratulations!" }), _jsx("div", { className: "text-sm opacity-90", children: "You've reached 100% completion!" })] });
    div >
    ;
}
_jsx("style", { children: `
