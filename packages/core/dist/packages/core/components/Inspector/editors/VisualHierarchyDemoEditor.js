...
import { useUISettingsStore } from '../../../stores/uiSettingsStore';
{
    ...
    _jsxs("div", { style: {
            ...
            background: HierarchyColors.neutral.background,
                                    ...
                                    borderRadius: 4,
                                    border: `1px solid ${HierarchyColors[currentLevel].border}`
                                } }), ", background: HierarchyColors[currentLevel].background, color: HierarchyColors[currentLevel].text, ...TypographyScale.tertiary }} />"] }), _jsx(HierarchyField, { priority: "standard", level: currentLevel, label: "Performance Mode", description: "Enable optimizations for large-scale generation", children: _jsxs("label", { style: {
                                display: 'flex',
                                alignItems: 'center',
                                gap: SpacingScale.sm,
                                cursor: 'pointer',
                                ...
                                color: HierarchyColors[currentLevel].text,
                                    ...
                                    borderRadius: 4,
                                    border: `1px solid ${HierarchyColors[currentLevel].border}`
                                } }), ", background: HierarchyColors[currentLevel].accent, color: HierarchyColors[currentLevel].text, ...TypographyScale.caption, fontFamily: 'monospace', opacity: 0.8; }} />"] }), _jsxs(HierarchyField, { priority: "supplementary", level: currentLevel, label: "Raw Configuration", description: "Complete internal configuration object", children: [_jsx("pre", { style: {
                                    padding: SpacingScale.sm,
                                    borderRadius: 4,
                                    border: `1px solid ${HierarchyColors[currentLevel].border}`
                                } }), ", background: HierarchyColors[currentLevel].accent, color: HierarchyColors[currentLevel].text, ...TypographyScale.micro, fontFamily: 'monospace', maxHeight: '200px', overflowY: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-all'; }} >", JSON.stringify(data, null, 2)] })] }), _jsx(HierarchyField, { priority: "supplementary", level: currentLevel, label: "Execution Stats", description: "Performance metrics and execution statistics", children: _jsxs("div", { style: {
                        display: 'grid',
                        ...
                        ], label, ": 'Cache Hits', value: `$", Math.floor(Math.random() * 100), "%` } }", label, ": 'Memory', value: `$", (Math.random() * 5).toFixed(1), "MB` } ].map((stat) => ()", _jsxs("div", { style: {
                                textAlign: 'center',
                                color: HierarchyColors[currentLevel].text,
    ...
    `1px solid ${HierarchyColors[currentLevel].primary}`;
}
 >
    (_jsx("div", { style: {
            ...TypographyScale.caption,
