...
export class HelpContentManager {
    ...
    markContentViewed(contentId) {
        this.userProfile.viewedContent.add(contentId);
    ...
    updateProgress(progressType, value) {
        ...
        else if (progressType === 'advancedFeaturesUsed') {
        ...
        for (const step of path.steps) {
            if (!this.userProfile.viewedContent.has(step.contentId)) {
                ...
                if (step.requiredProgress) {
                    const progressMet = Object.entries(step.requiredProgress).every(([key, value]) => this.userProfile.progress[key] >= value);
                    if (!progressMet)
                        continue;
                }
                if (step.unlockConditions) {
                    const conditionsMet = step.unlockConditions.every(condition => this.userProfile.completedTours.has(condition));
                    if (!conditionsMet)
                        continue;
                }
