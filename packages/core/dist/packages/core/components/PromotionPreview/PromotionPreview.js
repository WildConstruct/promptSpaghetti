...
className = "content-preview-card" >
    (_jsx("div", { className: "content-thumbnail", children: _jsx("img", { src: currentContent?.thumbnail || '/api/placeholder/200/150', alt: currentContent?.title || 'Preview', style: { width: '200px', height: '150px', objectFit: 'cover' } }) })
        ,
            _jsxs("div", { className: "content-info", children: [_jsx(Title, { level: 5, children: currentContent?.title || 'Featured Content' }), _jsx(Text, { type: "secondary", children: currentContent?.category || 'Category' }), currentContent && ()
                        < div, " className=\"content-metrics\">", _jsxs(Space, { children: [_jsxs("span", { children: ["\u2B50 ", currentContent.rating.toFixed(1)] }), _jsxs("span", { children: ["\u2193 ", currentContent.downloads] }), _jsx(Badge, { count: currentContent.performance_score, style: { backgroundColor: '#52c41a' }, title: "Performance Score" })] })] }));
div >
;
div >
;
{ /* Preview Controls */ }
_jsxs("div", { className: "preview-controls", children: [_jsxs(Space, { children: [_jsxs(Radio.Group, { value: previewMode, onChange: (e) => setPreviewMode(e.target.value), size: "small", children: [_jsx(Radio.Button, { value: "static", children: "Static" }), _jsx(Radio.Button, { value: "rotation", children: "Rotation" }), promotionData.ab_test_config && ()
                            < Radio.Button, " value=\"ab_test\">A/B Test"] }), ")}"] }), previewMode === 'rotation' && ()
            <  >
            (_jsx(Button, { icon: isRotating ? _jsx(PauseCircleOutlined, {}) : _jsx(PlayCircleOutlined, {}), onClick: () => setIsRotating(!isRotating), size: "small", type: isRotating ? 'primary' : 'default', children: isRotating ? 'Pause' : 'Play' })
                ,
                    _jsxs("div", { style: { width: '100px' }, children: [_jsxs(Text, { style: { fontSize: '12px' }, children: ["Speed: ", rotationSpeed, "s"] }), _jsx(Slider, { min: 1, max: 10, value: rotationSpeed, onChange: setRotationSpeed, size: "small" })] }))] });
Space >
;
div >
;
div >
    ...
    < Row;
gutter = { [16, 16]:  } >
    (_jsx(Col, { span: 6, children: _jsx(Statistic, { title: "Traffic Allocation", value: slot.traffic_allocation, suffix: "%", valueStyle: { color: '#3f8600' } }) })
        ,
            _jsx(Col, { span: 6, children: _jsx(Statistic, { title: "Slot Priority", value: slot.priority, prefix: _jsx(TrophyOutlined, {}) }) })
                ,
...
const renderPerformancePredictions = () => {
    return;
    _jsx("div", { className: "performance-predictions", children: performancePredictions.map((prediction, index) => ()
            < Card, key = { prediction, : .metric }, className = "prediction-card" >
            _jsxs(Row, { align: "middle", children: [_jsx(Col, { span: 8, children: _jsx(Statistic, { title: prediction.metric, value: prediction.predicted_value, precision: prediction.metric === 'Click-Through Rate' ? 2 : 0, suffix: prediction.metric === 'Click-Through Rate' ? '%' : '' }) }), _jsx(Col, { span: 8, children: _jsxs("div", { className: "confidence-range", children: [_jsx(Text, { strong: true, children: "Confidence Range" }), _jsx("br", {}), _jsxs(Text, { type: "secondary", children: [prediction.confidence_range[0].toFixed(0), " - ", prediction.confidence_range[1].toFixed(0), prediction.metric === 'Click-Through Rate' ? '%' : ''] })] }) }), _jsxs(Col, { span: 8, children: [_jsxs("div", { className: "impact-factors", children: [_jsx(Text, { strong: true, children: "Top Factors" }), prediction.factors.slice(0, 2).map((factor, idx) => ()
                                        < div, key = { idx }, className = "factor-item" >
                                        (_jsx(Progress, { percent: factor.impact * 100, size: "small", format: () => `${(factor.impact * 100).toFixed(0)}%`, strokeColor: "#1890ff" })
                                            ,
                                                _jsx(Tooltip, { title: factor.description, children: _jsx(Text, { style: { fontSize: '12px' }, children: factor.name }) })))] }), "))}"] })] })) });
};
Card >
;
div >
...
const renderABTestPreview = () => {
    ...
    return;
    _jsxs("div", { className: "ab-test-preview", children: [_jsx(Alert, { message: "A/B Test Configuration", description: `Testing ${ab_test_config.variants.length} variants with different content selections`, type: "info", icon: _jsx(ExperimentOutlined, {}), style: { marginBottom: '20px' } }), _jsx(Row, { gutter: [16, 16], children: ab_test_config.variants.map((variant, index) => ()
                    < Col, span = { 8:  }, key = { variant, : .id } >
                    _jsxs(Card, { title: _jsxs(Space, { children: [_jsx(Text, { strong: true, children: variant.name }), _jsxs(Tag, { color: index === 0 ? 'green' : 'blue', children: [variant.traffic_split, "% traffic"] })] }), className: selectedVariant === variant.id ? 'selected-variant' : '', onClick: () => setSelectedVariant(variant.id), style: { cursor: 'pointer' }, children: [_jsxs(Text, { type: "secondary", children: [variant.content_ids.length, " content items"] }), _jsxs("div", { className: "variant-content", children: [variant.content_ids.slice(0, 3).map(contentId => { }), "const content = promotionData.content.find(c => c.id === contentId); return content ? ()", _jsxs("div", { className: "variant-content-item", children: [_jsx("img", { src: content.thumbnail || '/api/placeholder/50/50', alt: content.title, style: { width: '50px', height: '50px', marginRight: '8px' } }), _jsxs("div", { children: [_jsx(Text, { style: { fontSize: '12px' }, children: content.title }), _jsx("br", {}), _jsxs(Text, { type: "secondary", style: { fontSize: '10px' }, children: ["Score: ", content.performance_score] })] })] }, contentId), ") : null; })}", variant.content_ids.length > 3 && ()
                                        < Text, " type=\"secondary\" style=", { fontSize: '12px' }, "> +", variant.content_ids.length - 3, " more"] }), ")}"] })) })] });
};
Row >
;
div >
...
if (!promotionData) {
    return;
    _jsx("div", { className: "promotion-preview-empty", children: _jsx(Text, { type: "secondary", children: "No promotion data available for preview" }) });
    ;
    return;
    _jsxs("div", { className: `promotion-preview ${className}`, children: ["}", _jsxs("div", { className: "preview-header", children: [_jsxs(Title, { level: 3, children: [_jsx(ThunderboltOutlined, {}), " Promotion Preview"] }), _jsxs(Space, { children: [_jsx(Switch, { checked: isLive, onChange: isLive ? onStopPreview : onStartPreview, checkedChildren: "Live", unCheckedChildren: "Preview" }), _jsx(Button, { icon: _jsx(SettingOutlined, {}), children: "Settings" })] })] }), _jsxs(Tabs, { activeKey: activeTab, onChange: setActiveTab, children: [_jsx(TabPane, { tab: "Visual Preview", children: renderSlotPreview() }, "preview"), _jsx(TabPane, { tab: "Performance Prediction", children: renderPerformancePredictions() }, "performance"), _jsx(TabPane, { tab: "Schedule Timeline", children: renderScheduleTimeline() }, "timeline"), promotionData.ab_test_config && ()
                        < TabPane, " tab=\"A/B Test Preview\" key=\"ab_test\">", renderABTestPreview()] }), ")}"] })
        ,
            _jsx("style", { children: `
        .promotion-preview {
          background: #fff;
