...
if (deadNodes.length > 0) {
    ...
    deadNodes.length > 5 ? 'high' : 'medium',
        description;
    `Remove ${deadNodes.length} unused node${deadNodes.length > 1 ? 's' : ''}`;
}
nodeIds: deadNodes,
...
if (constantNodes.length > 0) {
    ...
    'medium',
        description;
    `Pre-compute ${constantNodes.length} constant node${constantNodes.length > 1 ? 's' : ''}`;
}
...
if (parallelizableNodes.length > 2) {
    ...
    'high',
        description;
    `${parallelizableNodes.length} nodes can run in parallel`;
}
