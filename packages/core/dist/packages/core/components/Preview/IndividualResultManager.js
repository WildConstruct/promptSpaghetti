...
style = {};
{
    padding: '6px 12px',
        background;
    showAnalytics ? '#10b981' : '#e2e8f0',
        color;
    showAnalytics ? 'white' : '#374151',
        border;
    'none',
        borderRadius;
    '6px',
...
;
{
    results.slice(0, maxDisplayResults).map((result, index) => {
        ...
        _jsx("div", { style: {
            ...
            }, "borderRadius:": true }, `${result.seed}-${index}`);
        '8px',
            position;
        'relative',
            transition;
...
style = {};
{
    fontSize: '14px', lineHeight;
    '1.4', marginBottom;
    showAnalytics ? '12px' : '0';
}
 >
    { result, : .error ? ()
            < div : , style = {} };
{
    color: '#dc2626', fontStyle;
    'italic';
}
 >
    { result, : .error };
...
button >
;
_jsx("button", { onClick: () => handleResultAction({ type: 'regenerate', resultIndex: index }), disabled: result.locked || isRegenerating, style: {
        ...menuButtonStyle,
        color: result.locked || isRegenerating ? '#9ca3af' : '#374151',
        cursor: result.locked || isRegenerating ? 'not-allowed' : 'pointer',
    }, children: "\u27F3 Regenerate" })
    ,
        _jsx("div", { style: { height: '1px', background: '#e2e8f0', margin: '4px 8px' } })
            ,
                _jsx("button", { onClick: () => handleResultAction({ type: 'export', resultIndex: index, data: { format: 'text' } }), style: menuButtonStyle, children: "\uD83D\uDCC4 Export Text" })
                    ,
                        _jsx("button", { onClick: () => handleResultAction({ type: 'export', resultIndex: index, data: { format: 'json' } }), style: menuButtonStyle, children: "\uD83D\uDCCB Export JSON" });
{
    enableAnalytics && analytics && ()
        < button;
    onClick = {}();
    handleResultAction({ type: 'export', resultIndex: index, data: { format: 'csv' } });
}
style = { menuButtonStyle }
    >
;
