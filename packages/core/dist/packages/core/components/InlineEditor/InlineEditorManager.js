...
return;
_jsxs(_Fragment, { children: [editorState.isActive && activeNode && ()
            < InlineNodeEditor, "node=", activeNode, "isActive=", editorState.isActive, "onUpdate=", handleNodeUpdate, "onClose=", closeEditor, "onSubmit=", handleEditorSubmit, "position=", editorState.position, "/> )}", nodes.map(node => ()
            < NodeClickHandler, key = { node, : .id }, node = { node }, onClick = { handleNodeClick }, isEditorActive = { editorState, : .isActive && editorState.nodeId === node.id }
            /  >
        ), ")}"] });
;
;
...
({ children }) => {
    ...
    return;
    _jsx(InlineEditorContext.Provider, { value: editorState, children: children });
    ;
};
