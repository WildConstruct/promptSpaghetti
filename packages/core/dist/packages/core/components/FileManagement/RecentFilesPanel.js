...
style = {};
{
 ...
 >
    _jsxs("div", { style: {
            ...
            alignItems: 'center',
            gap: '6px',
        }, children: [file.metadata.title || file.name.replace('.psg', ''), file.isFavorite && _jsx("span", { style: { fontSize: '12px' }, children: "\u2B50" })] });
{
    !compact && ()
        < div;
    style = {};
...
_jsxs("div", { style: {
        ...
        flexDirection: 'column',
                ...
                fontWeight: '600',
                color: styles.text,
            }, children: "Recent Files" }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsxs("div", { style: { display: 'flex', backgroundColor: styles.tertiary, borderRadius: '4px', padding: '2px' }, children: [['list', 'grid'].map(mode => ()
                            ...
                            fontSize: '11px',
                            cursor: 'pointer',
                        }, ">", mode === 'list' ? '☰' : '⊞'] }), "))}"] }), showClearButton && recentFiles.length > 0 && ()
            < button, "onClick=", handleClearRecents, "style=", {
            padding: '6px 12px',
            backgroundColor: 'transparent',
            border: `1px solid ${styles.border}`
        }, ", borderRadius: '4px', color: styles.textSecondary, fontSize: '12px', cursor: 'pointer'; }} > Clear All"] });
div >
;
div >
    { /* Content */}
    ...
    { /* Favorites Section */};
{
 ...
 >
    (_jsxs("h3", { style: {
            ...
            alignItems: 'center',
            gap: '8px',
        }, children: [_jsx("span", { children: "\u2B50" }), " Favorites (", favoriteFiles.length, ")"] })
        ,
            _jsxs("div", { style: {
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '4px',
                }, children: [favoriteFiles.slice(0, 5).map(file => ()
                        < FileItem, key = { file, : .id }, file = { file }, compact = { true:  } /  >
                    ), ")}"] }));
div >
;
{ /* Recent Files Section */ }
_jsxs("div", { children: [_jsxs("h3", { style: {
                margin: '0 0 12px 0',
                fontSize: '14px',
                fontWeight: '600',
                color: styles.text,
                ...
                alignItems: 'center',
                gap: '8px',
            }, children: [_jsx("span", { children: "\uD83D\uDD50" }), " Recent Files (", recentFiles.length, ")"] }), recentFiles.length === 0 ? ()
            < div : , " style=", {
            textAlign: 'center',
            padding: '40px 20px',
            color: styles.textSecondary,
        }, ">", _jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\uD83D\uDCC4" }), _jsx("div", { style: { fontSize: '16px', marginBottom: '8px' }, children: "No recent files" }), _jsx("div", { style: { fontSize: '14px' }, children: "Open some projects to see them here" })] });
()
    < div;
style = {};
{
    display: viewMode === 'grid' ? 'grid' : 'flex',
        gridTemplateColumns;
    viewMode === 'grid' ? 'repeat(auto-fill, minmax(200px, 1fr))' : undefined,
        flexDirection;
    viewMode === 'list' ? 'column' : undefined,
        gap;
    viewMode === 'grid' ? '12px' : '4px',
    ;
}
 >
