...
export const IntegratedFileBrowser = ({ onFileSelected, onProjectLoad, onNewProject, onFileAction, theme = 'cinema', height = '100%', showCreateControls = true, currentProject }) => {
    ...
    return (_jsxs("div", { style: {
            ...
            fontSize: '14px',
                    ...
                    gap: '12px'
                }, children: [_jsxs("div", { style: { flex: 1, maxWidth: '300px' }, children: [_jsx("input", { type: "text", placeholder: "Search projects...", value: viewState.filterText, onChange: (e) => setViewState(prev => ({ ...prev, filterText: e.target.value })), style: {
                                    width: '100%',
                                    padding: '8px 12px',
                                    backgroundColor: styles.tertiary,
                                    border: `1px solid ${styles.border}`
                                } }), ", borderRadius: '6px', color: styles.text, fontSize: '13px', outline: 'none' }} />"] }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("select", { value: `${viewState.sortBy}-${viewState.sortDirection}`, onChange: (e) => {
                                    ...
                                    backgroundColor: styles.tertiary,
                                    border: `1px solid ${styles.border}`
                                } }), ", borderRadius: '4px', color: styles.text, fontSize: '12px' }} >", _jsx("option", { value: "name-asc", children: "Name A-Z" }), _jsx("option", { value: "name-desc", children: "Name Z-A" }), _jsx("option", { value: "modified-desc", children: "Recent First" }), _jsx("option", { value: "modified-asc", children: "Oldest First" }), _jsx("option", { value: "size-desc", children: "Largest First" }), _jsx("option", { value: "size-asc", children: "Smallest First" })] }), _jsxs("div", { style: { display: 'flex', backgroundColor: styles.tertiary, borderRadius: '4px', padding: '2px' }, children: [['list', 'grid', 'details'].map(mode => ()
                                < button, key = { mode }, onClick = {}()), " => setViewState(prev => (", ...(prev, viewMode), ": mode }))} style=", {
                                padding: '6px 8px',
                            ...
                            }, ">", mode === 'list' ? '☰' : mode === 'grid' ? '⊞' : '≡'] }), "))}"] }), showCreateControls && ()
                ...
                alignItems: 'center',
                gap: '6px',
            }, ">", _jsx("span", { children: "\u2795" }), " New Project"] }));
};
div >
;
div >
    ...
    { /* Recent Files */};
{
 ...
 >
    (_jsx("h3", { style: {
        ...
        ,
            _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' }, children: [recentFiles.slice(0, 5).map(file => ()
                        < button, key = { file, : .id }, onClick = {}()), " => handleFileDoubleClick(file)} style=", {
                        padding: '6px 8px',
                        backgroundColor: 'transparent',
                        border: 'none',
                        borderRadius: '4px',
                        ...
                        whiteSpace: 'nowrap',
                    }, "onMouseOver=", (e) => e.currentTarget.style.backgroundColor = styles.hover, "onMouseOut=", (e) => e.currentTarget.style.backgroundColor = 'transparent', "title=", file.name, "> \uD83D\uDCC4 ", file.name.replace('.psg', '')] }));
div >
;
div >
;
...
style = {};
{
    ...
    'none',
        color;
    index === breadcrumbs.length - 1 ? styles.text : styles.textSecondary,
        fontSize;
    '13px',
        cursor;
...
style = {};
{
 ...
 >
    (_jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\uD83D\uDCC1" })
        ,
            _jsx("div", { style: { fontSize: '16px', marginBottom: '8px' }, children: "No projects found" })
                ,
                    _jsx("div", { style: { fontSize: '14px' }, children: viewState.filterText ? 'Try adjusting your search terms' : 'Create your first project to get started' }));
div >
;
()
    < div;
style = {};
{
    ...
    viewState.viewMode === 'grid',
            ...
            : '1fr',
        gap;
    viewState.viewMode === 'grid' ? '16px' : '4px',
    ;
}
 >
...
style = {};
{
    padding: viewState.viewMode === 'grid' ? '16px' : '8px 12px',
        backgroundColor;
    selectedFile?.id === file.id ? styles.selection : 'transparent',
        border;
    `1px solid ${selectedFile?.id === file.id ? styles.accent : 'transparent'}`;
}
borderRadius: '8px',
    cursor;
'pointer',
...
'flex',
    flexDirection;
viewState.viewMode === 'grid' ? 'column' : 'row',
    alignItems;
viewState.viewMode === 'grid' ? 'center' : 'center',
    gap;
viewState.viewMode === 'grid' ? '8px' : '12px';
onMouseOver = {}(e);
{
    if (selectedFile?.id !== file.id) {
...
style = {};
{
    width: viewState.viewMode === 'grid' ? '64px' : '32px',
        height;
    viewState.viewMode === 'grid' ? '64px' : '32px',
        backgroundColor;
    styles.tertiary,
        borderRadius;
    '8px',
    ...
    'center',
        fontSize;
    viewState.viewMode === 'grid' ? '24px' : '16px',
        flexShrink;
    0,
    ;
}
...
style = {};
{
    flex: 1,
        textAlign;
    viewState.viewMode === 'grid' ? 'center' : 'left',
        overflow;
    'hidden',
    ;
}
