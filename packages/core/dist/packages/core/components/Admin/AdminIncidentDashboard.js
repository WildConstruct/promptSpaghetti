...
export const AdminIncidentDashboard = ({ onPlaybookExecute, onIncidentCreate, className = '' }) => {
    // State management
...
const IncidentCard = ({ incident }) => {
    ...
    return (_jsxs("div", { className: `incident-card ${severityColors[incident.severity]}`, children: [_jsxs("div", { className: "card-header", children: [_jsx("span", { className: "incident-id", children: incident.id }), _jsx("span", { className: `severity-badge ${incident.severity}`, children: incident.severity.toUpperCase() })] }), _jsx("h3", { className: "incident-title", children: incident.title }), _jsxs("div", { className: "incident-meta", children: [_jsxs("div", { className: "meta-item", children: [_jsx(Clock, { size: 14 }), _jsxs("span", { children: [Math.floor((Date.now() - incident.startTime.getTime()) / 60000), " min ago"] })] }), _jsxs("div", { className: "meta-item", children: [_jsx(Users, { size: 14 }), _jsxs("span", { children: [incident.businessImpact.affectedUsers.toLocaleString(), " affected"] })] })] }), _jsx("div", { className: "affected-systems", children: incident.affectedSystems.map(system => (_jsx("span", { className: "system-tag", children: system }, system))) })] }));
};
const AlertCard = ({ alert }) => {
    return (_jsxs("div", { className: `alert-card ${alert.severity}`, children: [_jsxs("div", { className: "alert-header", children: [_jsx(Bell, { size: 16 }), _jsxs("span", { className: "alert-time", children: [Math.floor((Date.now() - alert.timestamp.getTime()) / 60000), " min ago"] })] }), _jsx("h4", { className: "alert-title", children: alert.title }), _jsx("div", { className: "alert-system", children: alert.system }), alert.playbookTriggered && (_jsxs("div", { className: "playbook-triggered", children: [_jsx(CheckCircle, { size: 14 }), _jsx("span", { children: "Playbook triggered" })] }))] }));
};
...
const MetricCard = ({ label, value, trend, good }) => {
    ...
    return (_jsxs("div", { className: "metric-card", children: [_jsxs("div", { className: "metric-header", children: [_jsx("span", { className: "metric-label", children: label }), trendIcons[trend]] }), _jsx("div", { className: `metric-value ${good ? 'text-green-600' : 'text-red-600'}`, children: value })] }));
};
// Placeholder components for other tabs
const IncidentsTab = ({ incidents, onIncidentCreate: _onIncidentCreate }) => {
    return (_jsxs("div", { className: "incidents-tab", children: [_jsxs("div", { className: "tab-header", children: [_jsx("h2", { children: "Active Incidents" }), _jsx("button", { className: "create-button", children: "Create Incident" })] }), _jsx("div", { className: "incidents-list", children: incidents.map(incident => (_jsx(IncidentListItem, { incident: incident }, incident.id))) })] }));
};
const PlaybooksTab = ({ executions, onPlaybookExecute: _onPlaybookExecute }) => {
    return (_jsxs("div", { className: "playbooks-tab", children: [_jsxs("div", { className: "tab-header", children: [_jsx("h2", { children: "Playbook Executions" }), _jsx("button", { className: "execute-button", children: "Execute Playbook" })] }), _jsx("div", { className: "executions-list", children: executions.map(execution => (_jsx(PlaybookExecutionItem, { execution: execution }, execution.executionId))) })] }));
};
const SystemsTab = ({ systemHealth, alertsSummary }) => {
    return (_jsxs("div", { className: "systems-tab", children: [_jsxs("div", { className: "tab-header", children: [_jsx("h2", { children: "System Health" }), _jsxs("div", { className: "health-summary", children: [alertsSummary.total, " alerts (", alertsSummary.critical, " critical)"] })] }), _jsx("div", { className: "systems-grid", children: systemHealth.map(health => (_jsx(SystemHealthCard, { health: health }, health.system))) })] }));
};
const AnalyticsTab = ({ performanceMetrics, recentActivity: _recentActivity }) => {
    return (_jsxs("div", { className: "analytics-tab", children: [_jsx("div", { className: "tab-header", children: _jsx("h2", { children: "Performance Analytics" }) }), _jsx("div", { className: "analytics-content", children: _jsxs("div", { className: "metrics-overview", children: [_jsx(MetricCard, { label: "MTTR", value: `${performanceMetrics.mttr} min`, trend: "down", good: performanceMetrics.mttr < 20 }), _jsx(MetricCard, { label: "MTBF", value: `${performanceMetrics.mtbf} hrs`, trend: "up", good: performanceMetrics.mtbf > 100 }), _jsx(MetricCard, { label: "Success Rate", value: `${performanceMetrics.playbookSuccessRate}%`, trend: "up", good: performanceMetrics.playbookSuccessRate > 90 })] }) })] }));
};
// Placeholder item components
const IncidentListItem = ({ incident }) => (_jsxs("div", { className: "incident-list-item", children: [_jsx("span", { className: "incident-title", children: incident.title }), _jsx("span", { className: `severity-badge ${incident.severity}`, children: incident.severity })] }));
const PlaybookExecutionItem = ({ execution }) => (_jsxs("div", { className: "playbook-execution-item", children: [_jsx("span", { className: "playbook-name", children: execution.playbookName }), _jsx("span", { className: `status-badge ${execution.status}`, children: execution.status })] }));
