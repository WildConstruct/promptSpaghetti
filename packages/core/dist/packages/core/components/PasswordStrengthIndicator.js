...
// Suggestions Display
// ========================================
const SuggestionsDisplay, string;
theme: 'light' | 'dark';
 > ;
({ suggestions, theme }) => {
    if (suggestions.length === 0)
        return null;
    return;
    _jsxs("div", { className: "space-y-2", children: [_jsx("span", { className: "text-sm font-medium text-gray-700", children: "Suggestions" }), _jsx("div", { className: "space-y-1", children: suggestions.slice(0, 3).map((suggestion, index) => ()
                    < div, key = { index }, className = "flex items-start gap-2 text-xs" >
                    (_jsx("span", { className: "text-blue-500 mt-0.5", children: "\u2022" })
                        ,
                            _jsx("span", { className: "text-gray-600", children: suggestion }))) }), "))}", suggestions.length > 3 && ()
                < div, " className=\"text-xs text-gray-500 italic\"> +", suggestions.length - 3, " more suggestions"] });
};
div >
;
div >
...
// ========================================
const AdditionalInfoDisplay, PasswordValidationResult;
...
({ result, showCrackTime, showEntropy, theme }) => {
    if (!showCrackTime && !showEntropy)
        return null;
    return;
    _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-gray-200", children: [showEntropy && result.entropy && ()
                < div, " className=\"space-y-1\">", _jsx("span", { className: "text-xs font-medium text-gray-600", children: "Entropy" }), _jsxs("div", { className: "text-sm text-gray-800", children: [result.entropy.toFixed(1), " bits"] })] });
};
{
    showCrackTime && result.estimatedCrackTime && ()
        < div;
    className = "space-y-1" >
        (_jsx("span", { className: "text-xs font-medium text-gray-600", children: "Crack Time" })
            ,
                _jsxs("div", { className: "text-sm text-gray-800", children: [_jsxs("div", { children: ["Online: ", result.estimatedCrackTime.online] }), _jsxs("div", { children: ["Offline: ", result.estimatedCrackTime.offline] })] }));
    div >
    ;
}
div >
...
});
{
    ...
    return;
    _jsxs("div", { className: `space-y-3 ${compact ? 'space-y-2' : ''}`, children: ["}", _jsx(StrengthMeter, { score: result.score, strength: result.strength, compact: compact, theme: theme }), hasDetails && !compact && ()
                < div, " className=\"space-y-3\">", showDetails && ()
                < RuleResultsDisplay, "ruleResults=", result.ruleResults, "passedRules=", result.passedRules, "totalRules=", result.totalRules, "theme=", theme, "/> )}", showSuggestions && ()
                < SuggestionsDisplay, "suggestions=", result.suggestions, "theme=", theme, "/> )}", hasAdditionalInfo && ()
                < AdditionalInfoDisplay, "result=", result, "showCrackTime=", showCrackTime, "showEntropy=", showEntropy, "theme=", theme, "/> )}"] });
}
{
    result.errors.length > 0 && ()
        < div;
    className = "space-y-1" >
        (_jsx("span", { className: "text-sm font-medium text-red-600", children: "Errors" })
            ,
                _jsx("div", { className: "space-y-1", children: result.errors.map((error, index) => ()
                        < div, key = { index }, className = "text-xs text-red-600 flex items-start gap-2" >
                        (_jsx("span", { className: "text-red-500 mt-0.5", children: "\u26A0" })
                            ,
...
;
{
    result.warnings.length > 0 && !compact && ()
        < div;
    className = "space-y-1" >
        (_jsx("span", { className: "text-sm font-medium text-amber-600", children: "Warnings" })
            ,
                _jsx("div", { className: "space-y-1", children: result.warnings.map((warning, index) => ()
                        < div, key = { index }, className = "text-xs text-amber-600 flex items-start gap-2" >
                        (_jsx("span", { className: "text-amber-500 mt-0.5", children: "\u26A0" })
                            ,
...
});
{
    ...
    if (!password) {
        ...
        if (isValidating && !result) {
            return;
            _jsxs("div", { className: `flex items-center gap-2 ${className}`, children: ["}", _jsx("div", { className: "w-4 h-4 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin" }), _jsx("span", { className: "text-sm text-gray-500", children: "Analyzing password..." })] });
            ;
            // Don't render if no result available
            if (!result) {
                return null;
                return;
                _jsxs("div", { className: `password-strength-indicator ${className}`, children: ["}", _jsx(PasswordStrengthDisplay, { result: result, showDetails: showDetails, showSuggestions: showSuggestions, showCrackTime: showCrackTime, showEntropy: showEntropy, compact: compact, theme: resolvedTheme })] });
                ;
            }
            ;
            // ========================================
