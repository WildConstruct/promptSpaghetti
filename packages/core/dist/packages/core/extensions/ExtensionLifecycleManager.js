...
delete ;
async (key) => storage.delete(`${extensionId}:${key}`);
clear: async () => {
    ...
    getScoped;
(scope) => this.createStorage(`${extensionId}:${scope}`);
;
createEventEmitter(extensionId, string);
any;
{
    // This would be a scoped event emitter
    return this.eventEmitter;
    createRuntime(extensionId, string);
    any;
    {
        return {
            version: '1.0.0',
