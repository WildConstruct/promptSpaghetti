...
export const useGraphOperations = () => {
    ...
    const removeEdgesBetween = useCallback((sourceId, targetId) => {
        const state = useGraphEditorStore.getState();
        const edgesToRemove = state.graph.edges.filter(edge => edge.source === sourceId && edge.target === targetId);
        edgesToRemove.forEach(edge => actions.removeEdge(edge.id));
    }, [actions]);
    const removeAllEdgesForNode = useCallback((nodeId) => {
        const state = useGraphEditorStore.getState();
        const edgesToRemove = state.graph.edges.filter(edge => edge.source === nodeId || edge.target === nodeId);
        edgesToRemove.forEach(edge => actions.removeEdge(edge.id));
    }, [actions]);
    // Bulk operations
    ...
    const canConnect = useCallback((sourceId, targetId) => {
        if (sourceId === targetId)
            return false;
        const state = useGraphEditorStore.getState();
        const existingEdge = state.graph.edges.find(edge => edge.source === sourceId && edge.target === targetId);
        return !existingEdge;
    }, []);
