...
export class GraphStateContainer extends BaseStateContainer {
    ...
    async applyTransaction(transaction) {
        ...
        try {
            ...
            const finalState = transaction.operations.reduce((state, operation) => this.applyOperationToState(state, operation), this.getState());
            const change = {
                id: transaction.id,
    ...
    applyOperationToState(state, operation) {
        ...
        switch (operation.type) {
            ...
            case 'DELETE_NODE':
                ...
                newState.edges = Object.fromEntries(Object.entries(state.edges).filter(([, edge]) => edge.source !== operation.nodeId && edge.target !== operation.nodeId));
                newState.performance.nodeCount = Object.keys(newState.nodes).length;
