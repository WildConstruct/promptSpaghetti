...
async (resourceId, toStateId, actorId, options = {}) => {
    try {
        set({ loading: true, error: null });
...
async (data) => {
    ...
    approveWorkflow;
async (approvalId, approverId, comment) => {
    try {
        set({ loading: true, error: null });
        ...
        set(state => ({
            approvals: state.approvals.map(a => a.id === approvalId
    ...
    rejectWorkflow;
async (approvalId, rejectorId, reason) => {
    try {
        set({ loading: true, error: null });
...
async (workspaceId, filters = {}) => {
    ...
    acquireLock;
async (resourceId, userId, lockType = 'edit', options = {}) => {
    try {
        set({ loading: true, error: null });
...
async (lockId, userId) => {
    ...
    releaseLocksByResource;
async (resourceId, userId, lockType) => {
    try {
        set({ loading: true, error: null });
...
async (userId, resourceId, toStateId) => {
    try {
