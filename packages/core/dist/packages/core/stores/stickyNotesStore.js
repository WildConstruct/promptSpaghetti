...
const createDefaultNote = (id, position, content = '') => ({
    id,
    position,
...
export const useStickyNotesStore = create()(subscribeWithSelector((set, get) => ({
    // State
    notes: {},
    groups: {},
    selection: [],
    ...
    createNote: (position, content = '') => {
        ...
        const maxZ = Math.max(...Object.values(get().notes).map(n => n.appearance.zIndex), 999);
    ...
    searchNotes: (query) => {
        ...
        notes.forEach(note => {
            if (note.content.text.toLowerCase().includes(lowerQuery) ||
    ...
    exportNotes: (format) => {
        ...
        switch (format) {
            ...
            case 'markdown':
                ...
                const markdown = notes.map(note => `## ${note.appearance.category || 'Note'}\n\n${note.content.text}\n\n*Created: ${new Date(note.metadata.createdAt).toLocaleDateString()}*\n\n---\n`).join('\n');
                return `# Sticky Notes Export\n\n${markdown}`;
            case 'html':
                const htmlNotes = Object.values(state.notes);
                const html = htmlNotes.map(note => `<div class="sticky-note" style="background: ${COLOR_THEMES[note.appearance.color]?.background}">
    ...
    importNotes: (data, format) => {
        try {
            if (format === 'json') {
                ...
                if (imported.notes) {
                    set(state => ({
                        notes: { ...state.notes, ...imported.notes },
    ...
    resolveComment: (noteId, commentId) => {
        ...
        if (!note)
            return;
        const updatedComments = note.collaboration.comments.map(comment => comment.id === commentId ? { ...comment, resolved: true } : comment);
        get().updateNote(noteId, {
            collaboration: {
                ...note.collaboration,
