...
_jsxs("div", { role: "dialog", "aria-modal": "true", style: {
        ...
        justifyContent: 'center',
                ...
                color: professionalColors.text.primary,
                        ...
                        alignItems: 'center',
                        marginBottom: 16,
                    }, children: [_jsx("h2", { style: { margin: 0 }, children: "Generated Content" }), _jsxs("div", { style: { display: 'flex', gap: 8, alignItems: 'center' }, children: [hasExecutionPaths && ()
                                    < button, "onClick=", () => setShowExecutionPaths(!showExecutionPaths), "style=", {
                                    ...
                                    fontWeight: 500,
                                    cursor: 'pointer',
                                }, ">", showExecutionPaths ? '📊 Hide Paths' : '🔍 Show Paths'] }), ")}", results.length >= 2 && ()
                            < button, "onClick=", () => setShowVarianceAnalysis(!showVarianceAnalysis), "style=", {
                            ...
                            fontWeight: 500,
                            cursor: 'pointer',
                        }, ">", showVarianceAnalysis ? '📈 Hide Variance' : '📊 Show Variance'] }), ")}", _jsx("button", { onClick: () => {
                        if (compareMode) {
                            clearComparison();
                        }
                        else {
                        ...
                        fontWeight: 500,
                        cursor: 'pointer',
                    }, children: compareMode ? '⚖️ Exit Compare' : '⚖️ Compare' }), selectedForComparison.length > 0 && ()
                    < button, "onClick=", () => setExportDialog({}), "open: true, type: 'batch', })} style=", {
                    background: '#8b5cf6',
                    color: '#fff',
                    border: 'none',
                    borderRadius: 4,
                    ...
                    fontWeight: 500,
                    cursor: 'pointer',
                }, "> \uD83D\uDCBE Export Selected"] }), ")}", !compareMode && results.length > 1 && ()
            < button, "onClick=", () => setExportDialog({}), "open: true, type: 'comparison', })} style=", {
            background: '#6b7280',
            color: '#fff',
            border: 'none',
            borderRadius: 4,
...
}
{
    ...
    { /* Execution Path Visualization */ }
    {
     ...
     >
        _jsx(ExecutionPathVisualization, { results: results, onNodeHighlight: onNodeHighlight, config: {
...
{ /* Variance Analysis */ }
{
 ...
 >
    _jsx(VarianceAnalysis, { results: results, onSuggestionClick: onVarianceSuggestion });
div >
;
{ /* Comparison View */ }
{
 ...
 >
    (_jsx("h3", { style: { margin: '0 0 12px 0', color: '#0c4a6e', fontSize: 14 }, children: "\u2696\uFE0F Result Comparison" })
        ,
            _jsxs("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }, children: [selectedForComparison.map(index => { }), "const result = results[index]; return;", _jsxs("div", { style: {
                            padding: 8,
                            background: '#fff',
                            borderRadius: 4,
                            border: '1px solid #bae6fd',
                                ...
                                }, children: [result.output?.substring(0, 100), "..."] }), ('executionTimeMs' in result) && ()
                                < div, " style=", {
                                fontSize: 10,
                                color: '#6b7280',
                                marginTop: 4,
                            }, ">", result.executionTimeMs, "ms"] }, index), ")}"] }));
;
div >
;
div >
...
_jsxs("ul", { style: { padding: 0, listStyle: 'none' }, children: [results.map((res, i) => {
            ...
            return;
            _jsxs("li", { onMouseEnter: () => onResultHover?.(i), style: {
                    marginBottom: 16,
                    padding: 12,
...
{ /* Regenerating overlay */ }
{
 ...
 >
    _jsxs("div", { style: { textAlign: 'center' }, children: [_jsx("div", { style: { fontSize: 20, marginBottom: 4 }, children: "\u27F3" }), _jsx("div", { children: "Regenerating..." })] });
div >
;
{ /* Result content */ }
_jsxs("div", { style: { marginTop: 20, marginBottom: 8 }, children: [res.error ? ()
            < div : , " style=", { color: '#dc2626', fontSize: 14 }, ">\u26A0\uFE0F ", res.error] });
()
    < div;
style = {};
{
 ...
 >
    (_jsxs("div", { style: {
            ...
            overflow: 'hidden',
                ...
                } }), ", height: '100%', background: isHighProbability ? '#10b981' : isMediumProbability ? '#f59e0b' : '#6b7280', transition: 'width 0.3s ease'; }} />"] })
        ,
            _jsxs("span", { style: {
...
_jsxs("div", { style: {
    ...
    }, children: [_jsxs("div", { style: { fontSize: 12, color: '#6b7280' }, children: [lockedResults.length > 0 && ()
                    < span, " style=", { marginRight: 16 }, "> \uD83D\uDD12 ", lockedResults.length, " locked result", lockedResults.length !== 1 ? 's' : ''] }), ")}", regeneratingResults.length > 0 && (()), "\u27F3 ", regeneratingResults.length, " regenerating"] });
div >
    _jsxs("div", { style: { display: 'flex', gap: 8 }, children: [loading && onCancel && ()
...
onClose = {}();
setExportDialog({ open: false, type: 'individual' });
