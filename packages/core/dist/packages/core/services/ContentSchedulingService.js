...
export class ContentSchedulingService {
     ...
     */
    async scheduleContent(contentId, scheduling, scheduledBy) {
        const content = this.content.get(contentId);
        if (!content)
            return false;
    ...
    async unpublishContent(contentId, unpublishedBy) {
        const content = this.content.get(contentId);
        if (!content)
            return false;
        const updatedContent = await this.updateContent(contentId, {
     ...
     * Batch Operations
     */
    async createBatch(name, contentIds, operation, schedule, createdBy) {
        const batch = {
    ...
    getContent(filter) {
        ...
        if (filter.categories?.length) {
            content = content.filter(c => c.metadata.categories?.some(cat => filter.categories.includes(cat)));
        }
        if (filter.tags?.length) {
            content = content.filter(c => c.metadata.tags?.some(tag => filter.tags.includes(tag)));
        }
        ...
        if (filter.searchQuery) {
            ...
            content = content.filter(c => c.title.toLowerCase().includes(query) ||
        ...
        if (filter.hasSchedule !== undefined) {
            content = content.filter(c => filter.hasSchedule
    ...
    getSchedulingStats() {
        ...
        const publishedToday = content.filter(c => c.status === 'published' &&
            c.scheduling.publishAt &&
            c.scheduling.publishAt >= today &&
            c.scheduling.publishAt < tomorrow);
        const unpublishedToday = content.filter(c => c.status === 'unpublished' &&
    ...
    async executeBatchOperation(contentId, operation, operatorId) {
