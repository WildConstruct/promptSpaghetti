...
export var HelpCategory;
(function (HelpCategory) {
    HelpCategory["GETTING_STARTED"] = "getting_started";
    HelpCategory["TEMPLATES"] = "templates";
    HelpCategory["MARKETPLACE"] = "marketplace";
    HelpCategory["BILLING"] = "billing";
    HelpCategory["ACCOUNT"] = "account";
    HelpCategory["TECHNICAL"] = "technical";
    HelpCategory["COMMUNITY"] = "community";
    HelpCategory["PARTNERSHIPS"] = "partnerships";
    HelpCategory["COMPLIANCE"] = "compliance";
    HelpCategory["GENERAL"] = "general";
})(HelpCategory || (HelpCategory = {}));
export var HelpPriority;
...
export var HelpRequestStatus;
(function (HelpRequestStatus) {
    HelpRequestStatus["SUBMITTED"] = "submitted";
    HelpRequestStatus["TRIAGED"] = "triaged";
    HelpRequestStatus["AUTO_SUGGESTED"] = "auto_suggested";
    HelpRequestStatus["IN_PROGRESS"] = "in_progress";
    HelpRequestStatus["PENDING_USER"] = "pending_user";
    HelpRequestStatus["ESCALATED"] = "escalated";
    HelpRequestStatus["RESOLVED"] = "resolved";
    HelpRequestStatus["CLOSED"] = "closed";
    HelpRequestStatus["REOPENED"] = "reopened";
...
export class Epic16HelpRequestService extends EventEmitter {
    ...
    async suggestKnowledgeBaseArticles(request) {
        ...
        request.suggestedArticles = suggestions.filter(article => article.relevanceScore >= this.config.knowledgeBase.minRelevanceScore);
     ...
     */
    async addResponse(requestId, responseData) {
     ...
     */
    async updateStatus(requestId, newStatus, updatedBy) {
        const request = this.requests.get(requestId);
        if (!request)
            return null;
     ...
     */
    async escalateRequest(requestId, reason, escalatedBy) {
        const request = this.requests.get(requestId);
        if (!request)
            return null;
    ...
    async getHelpRequests(filters = {}) {
        ...
        if (filters.dateRange) {
            filtered = filtered.filter(r => r.createdAt >= filters.dateRange.start &&
    ...
    async getAnalytics(timeRange) {
        const requests = Array.from(this.requests.values()).filter(r => r.createdAt >= timeRange.start && r.createdAt <= timeRange.end);
        ...
        const slaBreached = requests.filter(r => r.sla.responseTime.breached || r.sla.resolutionTime.breached).length;
        const autoResolved = requests.filter(r => r.analytics.resolutionSource === 'knowledge_base' || r.autoResolvedBy).length;
...
Please review these resources. If they don't help, I'll connect you with our support team.`;
    ...
    async assignToAvailableAgent(request) {
