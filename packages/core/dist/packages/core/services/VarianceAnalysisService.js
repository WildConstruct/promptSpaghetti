...
export class VarianceAnalysisService {
    ...
    analyzeVariance(results) {
        ...
        const suggestions = this.generateVarianceSuggestions(diversityMetrics, creativeRange, overallVariance, executionPaths);
        return {
            overallVariance,
            varianceScore,
            diversityMetrics,
     ...
     */
    createDiversityIndicators(metrics) {
        const indicators = [];
    ...
    calculateVocabularyDiversity(outputs) {
        if (outputs.length < 2)
            return 0;
        const allWords = outputs.flatMap(output => output.toLowerCase()
            .replace(/[^\w\s]/g, ' ')
            .split(/\s+/)
    ...
    calculateStructuralDiversity(outputs) {
        if (outputs.length < 2)
            return 0;
    ...
    calculateExecutionPathDiversity(executionPaths) {
    ...
    calculateCreativityScore(outputs, executionPaths) {
    ...
    generateVarianceSuggestions(diversityMetrics, creativeRange, overallVariance, executionPaths) {
        const suggestions = [];
    ...
    categorizeMetric(value, lowThreshold, highThreshold) {
    ...
    getMetricColor(value, lowThreshold, highThreshold) {
        if (value < lowThreshold)
            return '#ef4444';
        if (value < highThreshold)
            return '#f59e0b';
