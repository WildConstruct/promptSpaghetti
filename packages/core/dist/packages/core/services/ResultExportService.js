...
export class ResultExportService {
     ...
     */
    async exportBatchResults(results, selectedIndices, options, sourceGraph) {
     ...
     */
    async exportComparison(results, options, sourceGraph) {
     ...
     */
    estimateExportSize(results, format, options) {
     ...
     */
    async performExport(data, options, filename, exportType) {
    ...
    transformDataForExport(data, options, exportType) {
        const baseData = {
            exportType,
    ...
    async handleCustomExport(data, options, filename, exportType) {
    ...
    exportExecutionTimeline(data, options) {
        const results = 'results' in data ? data.results : [data.result];
        const timeline = {
    ...
    exportVarianceReport(data, options) {
        const results = 'results' in data ? data.results : [data.result];
    ...
    exportBatchSummary(data, options) {
        const summary = {
    ...
    generateFilename(format, type, seeds) {
    ...
    calculateVarianceScore(results) {
        if (results.length < 2)
            return 0;
    ...
    generateComparisonAnalysis(results) {
        return {
            totalResults: results.length,
    ...
    getExecutionTimeSpread(results) {
        const times = results
            .map(r => r.executionTimeMs)
            .filter((t) => typeof t === 'number')
            .sort((a, b) => a - b);
    ...
    transformVFXData(results, vfxOptions) {
        return {
    ...
    generateAnalysis(results, analysisOptions) {
        const analysis = {};
    ...
    analyzeVarianceDistribution(results) {
