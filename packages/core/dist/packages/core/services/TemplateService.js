...
export class LocalTemplateStorage {
    ...
    async search(filter) {
        ...
        if (filter.author) {
            filtered = filtered.filter(t => t.author.toLowerCase().includes(filter.author.toLowerCase()));
        }
...
export class TemplateService {
    ...
    async instantiateTemplate(templateId, options) {
        ...
        if (!options.mergeWithCurrent) {
            ...
            graphData.annotations.nodeLabels = Object.fromEntries(Object.entries(graphData.annotations.nodeLabels).map(([nodeId, label]) => [
    ...
    extractAnnotations(nodes, edges) {
        return {
            ...
            connectionLabels: Object.fromEntries(edges.filter(edge => edge.label).map(edge => [edge.id, edge.label])),
            metadata: {
                author: 'system',
                created: new Date().toISOString(),
