...
export class SystemAccessManager {
     ...
     */
    async grantPermission(userId, permission, resource, action, grantedBy, options) {
        const user = this.users.get(userId);
        if (!user)
            return false;
     ...
     */
    hasPermission(userId, permission, resource, action) {
        const user = this.users.get(userId);
    ...
    hasSystemAccess(userId, requiredLevel) {
        const user = this.users.get(userId);
        if (!user || !user.isActive)
            return false;
    ...
    getAccessStats() {
        ...
        return {
            ...
            expiredAccesses: users.filter(u => u.roles.some(r => r.expiresAt && r.expiresAt < new Date())).length,
            ...
            recentActivity: {
                newUsers: users.filter(u => u.createdAt > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length,
                accessGranted: this.auditLog.filter(entry => entry.action === 'role_assigned' &&
                    entry.timestamp > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length,
                accessRevoked: this.auditLog.filter(entry => entry.action === 'role_revoked' &&
                    entry.timestamp > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length,
    ...
    cleanupExpiredAccess() {
        ...
        let cleanedCount = 0;
        users.forEach(user => {
            const expiredRoles = user.roles.filter(role => role.expiresAt && role.expiresAt < new Date());
            if (expiredRoles.length > 0) {
                user.roles = user.roles.filter(role => !role.expiresAt || role.expiresAt >= new Date());
                user.updatedAt = new Date();
                this.users.set(user.id, user);
                cleanedCount++;
