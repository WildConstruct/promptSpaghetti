class WorkflowService {
    ...
    constructor(baseUrl = '/api') {
        ...
        async;
        getWorkflowConfigs(workspaceId ?  : string);
...
async;
getWorkflowConfig(configId, string);
Promise < WorkflowConfig > {
...
return response.json();
async;
createWorkflowConfig(config, Omit < WorkflowConfig);
...
return response.json();
async;
updateWorkflowConfig(configId, string, updates, (Partial));
Promise < WorkflowConfig > {
    const: response = await fetch(`${this.baseUrl}/workflows/configs/${configId}`, {})
};
method: 'PUT',
...
return response.json();
async;
transitionWorkflow((), instanceId, string, request, WorkflowTransitionRequest);
Promise < WorkflowInstance > {
...
return response.json();
async;
getWorkflowStats(workspaceId ?  : string, configId ?  : string);
...
// Approval Process
async;
getApprovalRequests(workspaceId ?  : string, userId ?  : string);
...
return response.json();
async;
respondToApproval(requestId, string, response, ApprovalResponse);
Promise < ApprovalRequest > {
...
// Resource Locking
async;
getLocks(resourceId ?  : string, resourceType ?  : string, userId ?  : string);
...
async;
breakLock(lockId, string, reason ?  : string);
Promise < void  > {
...
// Scheduled Execution
async;
getScheduledExecutions(resourceId ?  : string, resourceType ?  : string);
...
return response.json();
async;
createScheduledExecution(execution, (Omit));
Promise < ScheduledExecution > {
...
return response.json();
async;
updateScheduledExecution((), executionId, string, updates, (Partial));
Promise < ScheduledExecution > {
