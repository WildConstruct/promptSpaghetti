...
z.number().int().min(0).max(Number.MAX_SAFE_INTEGER).optional(),
    history;
z.array(z.number().int()).max(10).default([]),
    autoGenerate;
...
enabled: z.boolean().default(false),
    value;
z.number().min(0.1).max(2.0).default(1.0),
    showIndicator;
z.boolean().default(true),
    presets;
z.array(z.object({}), name, z.string(), value, z.number(), description, z.string());
...
z.boolean().default(true),
    presets;
z.array(z.number().int()).default([1, 3, 5, 10, 20]),
;
;
/**
...
batchSize: z.number().int().min(1).max(100).default(5),
    outputFormat;
z.enum(['individual', 'combined', 'csv', 'json']).default('individual'),
    namingPattern;
z.string().default('result-{seed}-{timestamp}'),
    includeMetadata;
z.boolean().default(true),
    autoDownload;
...
z.string().optional(),
    type;
z.enum(['boolean', 'number', 'string', 'select', 'slider', 'multiselect']),
    validation;
z.unknown().optional(),
    defaultValue;
z.unknown(),
    options;
z.array(z.unknown()).optional(), // For select/multiselect,
    min;
z.number().optional(), // For number/slider,
    max;
z.number().optional(), // For number/slider,
    step;
z.number().optional(), // For number/slider,
...
z.boolean().default(false),
    advanced;
z.boolean().default(false); // Show only in advanced mode,
;
