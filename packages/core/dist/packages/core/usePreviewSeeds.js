...
if (controller.signal.aborted) {
    ...
    const perSeedResults = data.results.map()(result, { output: string, error: string });
    index: number;
    ({
        seed: result.seed,
        ...
        usedNodeIds: result.executionPath ? result.executionPath.nodeExecutionOrder : , }(result.output && !result.output.startsWith('Error:') ? [`node_${result.seed}`] : []));
}
...
debugInfo: result.executionPath ? generateDebugInfo(result.executionPath) : undefined,
    ...
    locked;
results[index]?.locked || false,
    lockedAt;
results[index]?.lockedAt,
    lockedNote;
results[index]?.lockedNote;
;
    ...
    const regenerateSpecificResult = useCallback(async (graph, seed, resultIndex) => {
        ...
        if (results[resultIndex]?.locked) {
            ...
            try {
                ...
                1,
                    seedStart;
                Math.floor(Math.random() * 10000); // Use new random seed,
            }
            finally { }
        }
    }), signal;
...
if (data.results && data.results.length > 0) {
    ...
    const transformedResult = {
        ...
        usedNodeIds: newResult.executionPath ? newResult.executionPath.nodeExecutionOrder : , }(newResult.output && !newResult.output.startsWith('Error:') ? [`node_${newResult.seed}`] : []);
}
