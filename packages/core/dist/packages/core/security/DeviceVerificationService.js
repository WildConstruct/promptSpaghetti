...
import { DeviceFingerprint, LocationData, RiskLevel } from './DeviceFingerprintingService';
...
})(VerificationStep || (VerificationStep = {}));
{
    ...
    export class DeviceVerificationService extends EventEmitter {
        ...
        config = {
            ...
            request: DeviceVerificationRequestData, Promise() {
                try {
        ...
        if (!session) {
            ...
            };
            {
                ...
                if (session.riskScore < this.config.riskThresholds.lowRisk && this.config.enableAutomaticApproval) {
                    ...
                    async;
                    createSMSChallenge((), challenge, DeviceChallenge, session, VerificationSession);
                    Promise < void  > {
                        // Generate verification code
                        const: codeRequest, CodeGenerationRequest = {
                            userId: session.userId,
                    ...
                    ;
                    async;
                    validateChallengeResponse((), challenge, DeviceChallenge, response, string);
                    Promise < boolean > {
                    ...
                    };
                    {
                        ...
                        return false;
                        async;
                        completeVerification(session, VerificationSession);
                        Promise < void  > {
