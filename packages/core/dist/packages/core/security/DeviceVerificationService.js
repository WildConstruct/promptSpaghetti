...
(function (ChallengeType) {
    ...
    ChallengeType["MANUAL_REVIEW"] = "manual_review";
    ChallengeType["PHONE_CALL"] = "phone_call";
    ChallengeType["SECURITY_QUESTIONS"] = "security_questions";
...
export class DeviceVerificationService extends EventEmitter {
    ...
    sessions = new Map();
    challenges = new Map();
    constructor(fingerprintService, trustedDeviceManager, verificationCodeManager, emailTracker, config = {
    ...
    async startVerification(request) {
        try {
            ...
            const fingerprint = await this.fingerprintService.generateFingerprint(request.fingerprintContext);
     ...
     */
    adminOverride(sessionId, approved, adminUserId, reason) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return false;
        }
    ...
    async progressSession(session) {
        ...
        switch (session.currentStep) {
            ...
            case VerificationStep.CHALLENGE_REQUIRED:
                ...
                const requiredCompleted = session.requiredChallenges.every(challengeType => session.challenges.some(c => c.type === challengeType && c.status === 'completed'));
    ...
    async performRiskAssessment(session) {
        ...
        if (session.flags.suspiciousActivity)
            additionalRisk += 20;
        if (session.flags.vpnDetected)
            additionalRisk += 10;
        if (session.flags.proxyDetected)
            additionalRisk += 15;
        if (session.flags.deviceSpoofing)
            additionalRisk += 25;
        if (session.flags.locationInconsistent)
            additionalRisk += 20;
    ...
    async createChallenge(session, type) {
    ...
    async createEmailChallenge(challenge, session) {
    ...
    async createSMSChallenge(challenge, session) {
        // Generate verification code
        const codeRequest = {
            userId: session.userId,
            type: VerificationCodeType.DEVICE_VERIFICATION,
    ...
    async createCaptchaChallenge(challenge, session) {
    ...
    async createManualReviewChallenge(challenge, session) {
    ...
    async completeVerification(session) {
    ...
    async registerTrustedDevice(session) {
        try {
            const deviceRequest = {
                userId: session.userId,
    ...
    async requestManualReview(session) {
    ...
    async checkForSuspiciousActivity(session) {
    ...
    async validateLocation(session) {
    ...
    async checkDeviceSpoofing(session) {
    ...
    addAttempt(session, step, success, challengeId, failureReason) {
        const attempt = {
