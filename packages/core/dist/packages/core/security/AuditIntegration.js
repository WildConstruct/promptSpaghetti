...
export class AuditIntegration {
    ...
    constructor(config) {
         ...
         */
        async;
        logDataAccess(context, OperationContext);
        resourceType: string,
            resourceId;
        string,
            data ?  : any,
        ...
        Promise < void  > {
            ...
            let, classificationMetadata: (Record) = {},
            : .classifier && data && this.config.enrichWithClassification
        };
        {
                                        ...
                                        this.enforcer.on('enforcement', async (event) => {
                                            if (this.config.logAllOperations || !event.allowed) {
                                                ...
                                                event.context.userRole,
                                                    operation;
                                                AuditOperation.AUTHORIZATION,
                                                    resourceType;
                                                'data_access',
                                                    resourceId;
                                                event.context.resourceId || 'unknown',
                                                    dataClassification;
                                                ...
                                                event.riskScore,
                                                    sensitiveAccess;
                                                this.isSensitiveClassification(event.classification),
                                                    metadata;
                                                {
                                                    appliedControls: event.appliedControls,
                                                        missingControls;
                                                    event.missingControls,
                                                        recommendations;
                        ...
                        this.classifier.on('classification', async (event) => {
                            if (this.config.logClassificationChanges) {
                                ...
                                userId: event.userId || 'system',
                                    operation;
                                AuditOperation.CHANGE_CLASSIFICATION,
                                    resourceType;
                                ...
                                true,
                                    sensitiveAccess;
                                this.isSensitiveClassification(event.newLevel),
                                    metadata;
                                {
    ...
    */
    getClassificationPriority(level) {
         ...
         */
        async;
        generateComplianceReport(startDate, Date);
        endDate: Date,
            options ?  : {
