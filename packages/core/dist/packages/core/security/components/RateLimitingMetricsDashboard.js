...
// Dashboard Component
// ========================================
export const RateLimitingMetricsDashboard = ({ metricsService, className = '', theme = 'light', autoRefresh = true, refreshInterval = 5 }) => {
    // State management
    ...
    const getTimeSeriesChartData = useMemo(() => {
        ...
        return {
            labels: timeSeriesData.timestamps.map(ts => new Date(ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })),
            datasets: [
                {
    ...
    // Render Methods
    // ========================================
    const renderMetricCard = (stat, index) => (_jsx("div", { className: `
      ...
      `, children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("p", { className: "text-sm font-medium text-gray-600 dark:text-gray-400", children: stat.label }), _jsxs("div", { className: "flex items-baseline space-x-1", children: [_jsx("p", { className: `text-2xl font-semibold ${stat.severity === 'critical' ? 'text-red-600 dark:text-red-400' :
                                        stat.severity === 'warning' ? 'text-yellow-600 dark:text-yellow-400' :
                                            'text-gray-900 dark:text-white'}`, children: stat.value }), stat.unit && (_jsx("span", { className: "text-sm text-gray-500 dark:text-gray-400", children: stat.unit }))] })] }), stat.trend && (_jsxs("div", { className: `
          ...
          `, children: [stat.trend === 'up' && '↗', stat.trend === 'down' && '↘', stat.trend === 'stable' && '→', stat.trend] }))] }) }, index));
    const renderAlerts = () => (_jsxs("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700", children: [_jsx("div", { className: "px-4 py-3 border-b border-gray-200 dark:border-gray-700", children: _jsxs("h3", { className: "text-lg font-medium text-gray-900 dark:text-white", children: ["Active Alerts (", activeAlerts.length, ")"] }) }), _jsx("div", { className: "p-4", children: activeAlerts.length === 0 ? (_jsx("p", { className: "text-sm text-gray-500 dark:text-gray-400", children: "No active alerts" })) : (_jsx("div", { className: "space-y-3", children: activeAlerts.map((alert) => (_jsx("div", { className: `
                ...
                `, children: _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsx("p", { className: "font-medium text-gray-900 dark:text-white", children: alert.condition }), _jsxs("p", { className: "text-sm text-gray-600 dark:text-gray-400 mt-1", children: ["Current: ", alert.currentValue.toFixed(2), " | Threshold: ", alert.threshold] }), _jsx("p", { className: "text-xs text-gray-500 dark:text-gray-500 mt-1", children: alert.timestamp.toLocaleString() })] }), _jsx("button", { onClick: () => handleAlertAcknowledge(alert.alertId), className: "ml-3 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 px-2 py-1 rounded text-gray-700 dark:text-gray-300", children: "Acknowledge" })] }) }, alert.alertId))) })) })] }));
    const renderTimeSeriesChart = () => {
        if (!getTimeSeriesChartData)
            return null;
        return (_jsxs("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700", children: [_jsx("div", { className: "px-4 py-3 border-b border-gray-200 dark:border-gray-700", children: _jsx("h3", { className: "text-lg font-medium text-gray-900 dark:text-white", children: "Performance Trends" }) }), _jsx("div", { className: "p-4", children: _jsx("div", { className: "h-64 flex items-center justify-center text-gray-500 dark:text-gray-400", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-4xl mb-2", children: "\uD83D\uDCCA" }), _jsx("p", { children: "Time Series Chart" }), _jsxs("p", { className: "text-sm", children: [getTimeSeriesChartData?.datasets[0]?.data.length || 0, " data points"] })] }) }) })] }));
    };
    const renderHeatmap = () => {
        if (!visualizationData)
            return null;
