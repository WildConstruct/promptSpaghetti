...
type: z.enum(['ip', 'user', 'endpoint', 'method', 'header', 'query', 'body', 'time', 'geo', 'custom']),
    operator;
z.enum(['equals', 'contains', 'startsWith', 'endsWith', 'regex', 'in', 'range', 'exists']),
    field;
z.string().optional(),
    value;
z.any().optional(),
...
type: z.enum(['block', 'delay', 'throttle', 'captcha', 'redirect', 'custom']),
    delayMs;
z.number().min(0).max(60000).optional(),
    redirectUrl;
...
z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
    windowMs;
z.number().min(1000).max(86400000).optional(),
    maxRequests;
...
z.number().min(0).max(1000),
    conditions;
z.array(RateLimitConditionSchema),
    windowMs;
z.number().min(1000).max(86400000),
    maxRequests;
z.number().min(1).max(100000),
    burstLimit;
z.number().min(1).max(100000).optional(),
    cooldownMs;
z.number().min(0).max(86400000).optional(),
    progressiveDelay;
z.boolean().optional(),
    scope;
z.nativeEnum(RateLimitScope),
    keyGenerator;
z.string().optional(),
    action;
...
z.string().optional(),
    statusCode;
z.number().min(400).max(599).optional(),
    schedule;
RateLimitScheduleSchema.optional(),
    alertThreshold;
...
return false;
evaluateGeo(condition, RateLimitCondition, geo, any);
boolean;
{
    if (!geo)
        return false;
    ...
    switch (condition.operator) {
        ...
        default:
            return false;
            evaluateCustom(condition, RateLimitCondition, context, any);
            boolean;
            {
                // Placeholder for custom condition evaluation
                // This would be implemented based on specific business logic
                return false;
                evaluateStringValue(condition, RateLimitCondition, value, string);
                boolean;
                {
                    ...
                    switch (condition.operator) {
                        ...
                        default:
                            ...
                            any;
                            {
                                ...
                                isIPInRange(ip, string, range, string);
                                boolean;
                                {
                                    ...
                                    if (range.includes('/')) {
                                        ...
                                        export class RateLimitConfigurationManager {
                                            ...
                                            loadDefaultProfiles() {
                                                 ...
                                                 */
                                                addProfile(profile, RateLimitProfile);
                                                void {
                                                    // Validate profile
                                                                ...
                                                                if (this.profiles.has(profileId)) {
                                                                    ...
                                                                    */
                                                                    ...
                                                                    RateLimitProfile;
                                                                    {
                                                                        ...
                                                                        */
                                                                        findMatchingRules(requestContext, any);
                                                                            ...
                                                                            for (const rule of activeProfile.rules) {
                                                                                ...
                                                                                if (rule.schedule && !this.isRuleScheduleActive(rule.schedule)) {
                                                                                    ...
                                                                                    if (allConditionsMet) {
                                                                                        ...
                                                                                        */
                                                                                        createRateLimitConfig(rule, DynamicRateLimitRule);
                                                                                        RateLimitConfig;
                                                                                        {
                                                                                            return {
                                                                                            ...
                                                                                            boolean;
                                                                                            {
                                                                                                 ...
                                                                                                 */
                                                                                                addRule(rule, DynamicRateLimitRule);
                                                                                                boolean;
                                                                                                {
                                                                                                    const activeProfile = this.getActiveProfile();
                                                                                                    if (!activeProfile)
                                                                                                            ...
                                                                                                            */
                                                                                                            ...
                                                                                                            void {
                                                                                                                try: {
                                                                                                                    ...
                                                                                                                    for(, profile, of, config) { }, : .profiles || []
                                                                                                                }
                                                                                                            };
                                                                                                            {
