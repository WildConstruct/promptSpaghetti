...
export class RedisRateLimitStore {
    ...
    async get(key) {
        ...
        try {
            ...
            if (this.config.enableScripting) {
                const result = await this.executeWithRetry(() => this.client.eval(RedisLuaScripts.GET_INFO, [fullKey], []));
    ...
    async increment(key, windowMs) {
        ...
        try {
            ...
            if (this.config.enableScripting) {
                const result = await this.executeWithRetry(() => this.client.eval(RedisLuaScripts.INCREMENT_WITH_EXPIRY, [fullKey], [windowMs.toString(), currentTime.toString()]));
                if (Array.isArray(result) && result.length === 2) {
    ...
    // ========================================
    async incrementFallback(key, windowMs, currentTime) {
