...
export class SecurityDataQualityMonitor extends EventEmitter {
     ...
     */
    async executeValidationRule(rule, record, context) {
        const violations = [];
    ...
    validateCompleteness(rule, record, context) {
        ...
        for (const field of requiredFields) {
            if (record[field] === undefined || record[field] === null || record[field] === '') {
                violations.push(this.createViolation(rule, QualityDimension.COMPLETENESS, context, `Missing required field: ${field}`, field, 'present', record[field]));
            }
        }
        return violations;
    }
    validateFormat(rule, record, context) {
        ...
        if (rule.id === 'timestamp-validity' && record.timestamp) {
            ...
            if (isNaN(timestamp.getTime())) {
                ...
                violations.push(this.createViolation(rule, QualityDimension.TIMELINESS, context, 'Timestamp is too far in the future', 'timestamp', `within ${rule.parameters.maxFutureMinutes} minutes`, record.timestamp));
            }
            else if (timestamp.getTime() < now.getTime() - maxPastMs) {
                violations.push(this.createViolation(rule, QualityDimension.TIMELINESS, context, 'Timestamp is too old', 'timestamp', `within ${rule.parameters.maxPastDays} days`, record.timestamp));
            }
        }
        return violations;
    }
    validateRange(rule, record, context) {
        const violations = [];
        ...
        if (rule.id === 'severity-range' && record.severity) {
            ...
            if (!allowedValues.includes(record.severity)) {
                violations.push(this.createViolation(rule, QualityDimension.VALIDITY, context, `Invalid severity value: ${record.severity}`, 'severity', allowedValues.join(', '), record.severity));
            }
        }
        ...
        if (rule.id === 'confidence-range' && record.confidence !== undefined) {
            ...
            if (record.confidence < minValue || record.confidence > maxValue) {
                violations.push(this.createViolation(rule, QualityDimension.VALIDITY, context, `Confidence value out of range: ${record.confidence}`, 'confidence', `${minValue} - ${maxValue}`, record.confidence));
            }
        }
        return violations;
    }
    async validateDuplicates(rule, record, context) {
        const violations = [];
    ...
    validateStatisticalOutliers(rule, record, context) {
        const violations = [];
        ...
        for (const field of fields) {
            if (record[field] !== undefined && typeof record[field] === 'number') {
                ...
                if (isOutlier) {
                    violations.push(this.createViolation(rule, QualityDimension.CONSISTENCY, context, `Statistical outlier detected in field: ${field}`, field, 'within normal range', record[field]));
                }
            }
        }
        return violations;
    }
    validateTemporalConsistency(rule, record, context) {
        const violations = [];
    ...
    validateCrossField(rule, record, context) {
        const violations = [];
    ...
    async remediateValidity(violation) {
    ...
    async remediateConsistency(violation) {
    ...
    async calculateQualityTrends(period) {
    ...
    async generateRecommendations(dimensions, violations) {
    ...
    async calculateDataSourceMetrics(violations) {
    ...
    // UTILITY METHODS
    // ==========================================
    createViolation(rule, dimension, context, description, fieldName, expectedValue, actualValue) {
        return {
