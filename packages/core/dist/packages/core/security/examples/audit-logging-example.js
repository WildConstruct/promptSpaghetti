...
async function basicAuditLogging() {
    ...
    await auditLogger.logDataAccess(context, 'financial_records', 'q4_2024_revenue', DataClassificationLevel.CONFIDENTIAL, true, {
        recordCount: 1500,
        dataSize: 2048000,
        exportFormat: 'CSV'
    });
...
async function integratedAuditExample() {
    ...
    await auditIntegration.logDataAccess(context, 'customer_profile', 'cust_98765', customerData, {
    ...
    await auditIntegration.logBatchOperation(context, AuditOperation.EXPORT, [
    ...
    ], true, { exportFormat: 'encrypted_zip', destination: 'secure_ftp' });
    console.log('✓ Logged batch export operation');
    // Example 3: Administrative operation
    await auditIntegration.logAdminOperation('GRANT_ACCESS', {
        userId: 'admin001',
    ...
    await auditIntegration.logSecurityEvent('SUSPICIOUS_ACCESS_PATTERN', context, {
    ...
    const report = await auditIntegration.generateComplianceReport(new Date(Date.now() - 24 * 60 * 60 * 1000), // Last 24 hours
    new Date(), { includeDetails: false });
...
async function workflowAuditExample() {
    ...
    const correlationId = await auditIntegration.startAuditTrail('ml_training_workflow_001', workflowContext, {
    ...
    console.log(`  Duration: ${correlatedLogs.length > 0
...
export { basicAuditLogging, customStorageExample, integratedAuditExample, auditEventsExample, workflowAuditExample };
