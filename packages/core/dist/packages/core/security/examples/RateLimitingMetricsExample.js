...
export function BasicMetricsExample() {
                ...
                alertThresholds: {
                    ...
                    metricsService, : .on('metricsUpdated', (data) => {
                        ...
                        responseTime: data.metrics.responseTime.average.toFixed(2) + 'ms',
                            throughput;
                        data.metrics.throughput.requestsPerSecond.toFixed(1) + ' rps',
                            blockRate;
                        data.metrics.errorRates.blockRate.toFixed(1) + '%',
                            collectionTime;
                        data.collectionTime + 'ms',
                        ;
                    })
                },
    ...
    export function RateLimitingDashboardExample() {
        ...
        if (metricsHook.error) {
            ...
            _jsx("div", { className: "min-h-screen bg-gray-50 dark:bg-gray-900", children: _jsx("div", { className: "bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700", children: _jsx("div", { className: "max-w-7xl mx-auto px-6 py-4", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-bold text-gray-900 dark:text-white", children: "Security Analytics Platform" }), _jsx("p", { className: "text-gray-600 dark:text-gray-400", children: "Rate Limiting Performance Monitoring" })] }), _jsxs("div", { className: "flex items-center space-x-4", children: [_jsxs("div", { className: `inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${metricsHook.systemStatus === 'healthy' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : ,
                                            ...
                                            }`, children: [_jsx("div", { className: `w-2 h-2 rounded-full mr-2 ${metricsHook.systemStatus === 'healthy' ? 'bg-green-500' : ,
                                                        metricsHook.systemStatus === 'warning' ? 'bg-yellow-500' : ,
                                                        'bg-red-500'}` }), metricsHook.systemStatus.charAt(0).toUpperCase() + metricsHook.systemStatus.slice(1)] }), metricsHook.isConnected && ()
                                            < div, " className=\"text-sm text-gray-500 dark:text-gray-400\"> Connected"] }), ")}"] }) }) }) });
            { /* Dashboard Content */ }
            _jsx(RateLimitingMetricsDashboard, { metricsService: new RateLimitingPerformanceMetrics(), rateLimitingService: true, throttlingEngine: true, theme: "light", autoRefresh: true, refreshInterval: 3 });
            div >
            ;
            ;
        }
        ...
        export function AdvancedVisualizationExample() {
                ...
                };
            {
                ...
                dataPoints: timeSeriesData.timestamps.length,
                    avgResponseTime;
                timeSeriesData.responseTime.reduce((a, b) => a + b, 0) / timeSeriesData.responseTime.length,
                    avgThroughput;
                timeSeriesData.throughput.reduce((a, b) => a + b, 0) / timeSeriesData.throughput.length,
                ;
            }
            ;
            // Get heatmap data
            ...
            endpoints: heatmapData.endpoints.length,
                timeSlots;
            heatmapData.timeSlots.length,
                totalDataPoints;
            heatmapData.activityMatrix.length * heatmapData.activityMatrix[0].length,
            ;
        }
        ;
        // Get geospatial data
        ...
        locations: geoData.locations.length,
            totalRequests;
        geoData.locations.reduce((sum, loc) => sum + loc.requestCount, 0),
            totalBlocks;
        geoData.locations.reduce((sum, loc) => sum + loc.blockCount, 0),
        ;
    }
    ;
    ...
    endpoints: Object.keys(distributionData.endpointDistribution).length,
        threatLevels;
    Object.keys(distributionData.threatLevelDistribution).length,
        responseTimeRanges;
    distributionData.responseTimeDistribution.length,
        userAgents;
    Object.keys(distributionData.userAgentDistribution).length,
    ;
}
;
...
export function CustomWidgetExample() {
    ...
    }
    {
            ...
            config;
        {
            metrics: ['endpointDistribution'],
                dimensions;
            ['endpoint', 'requests', 'responseTime'],
            ;
        }
        position: {
            x: 4, y;
    ...
    }
    {
        ...
        'Threat Activity Timeline',
            description;
        'Historical threat activity over time',
            dataSource;
        'timeseries',
            refreshInterval;
        30,
...
export function DataExportExample() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        setTimeout(() => {
            ...
            jsonData.timeRange,
                alertsCount;
            jsonData.alerts.length,
                configurationKeys;
            Object.keys(jsonData.configuration),
            ;
        });
        // Export as CSV
    ...
    status: systemStatus.status,
        uptime;
    `${Math.round(systemStatus.uptime / 1000)}s`;
}
...
function simulateTraffic(rateLimitingService, highVolume = false) {
    ...
    for (let i = 0; i < requestCount; i++) {
        ...
        userAgent,
            userId;
        success ? `user_${Math.floor(Math.random() * 1000)}` : undefined;
    }
}
;
// Record attempt
...
export function runAllExamples() {
    ...
    const examples = [];
    {
        name: 'Basic Metrics', fn;
        BasicMetricsExample, delay;
        0;
    }
    {
        name: 'Advanced Visualization', fn;
        AdvancedVisualizationExample, delay;
        5000;
    }
    {
        name: 'Custom Widgets', fn;
        CustomWidgetExample, delay;
        10000;
    }
    {
        name: 'Alert Management', fn;
        AlertManagementExample, delay;
        15000;
    }
    {
        name: 'Data Export', fn;
        DataExportExample, delay;
        20000;
    }
    ;
    const services = [];
...
export { AdvancedVisualizationExample, simulateTraffic };
