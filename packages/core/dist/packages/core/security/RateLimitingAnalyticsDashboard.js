...
export class RateLimitingAnalyticsDashboard extends EventEmitter {
    ...
    startTime;
    constructor(rateLimitingService, throttlingEngine, performanceMetrics, config) {
        super();
     ...
     */
    calculateCurrentThreatLevel(stats) {
        const threatCounts = stats.threatLevels || {};
        const totalThreats = Object.values(threatCounts).reduce((sum, count) => sum + (typeof count === 'number' ? count : 0), 0);
        if (totalThreats === 0)
        ...
        if (highRatio > 0.2 || criticalRatio > 0.05)
     ...
     */
    calculateSLACompliance(systemStatus) {
    ...
    async detectAnomalies() {
        ...
        this.predictiveInsights.anomalyDetections = this.predictiveInsights.anomalyDetections.filter(anomaly => anomaly.detectionTime > cutoff);
    }
    /**
     ...
     */
    async updateDashboardVisualization() {
     ...
     */
    storeHistoricalData(dataType, data) {
