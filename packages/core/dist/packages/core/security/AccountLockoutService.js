...
// Lockout Reasons
export var LockoutReason;
(function (LockoutReason) {
    LockoutReason["EXCESSIVE_FAILED_ATTEMPTS"] = "excessive_failed_attempts";
    LockoutReason["SUSPICIOUS_ACTIVITY"] = "suspicious_activity";
    LockoutReason["SECURITY_POLICY_VIOLATION"] = "security_policy_violation";
    LockoutReason["ADMIN_MANUAL_LOCK"] = "admin_manual_lock";
    LockoutReason["SYSTEM_SECURITY_ALERT"] = "system_security_alert";
    LockoutReason["COMPLIANCE_REQUIREMENT"] = "compliance_requirement";
})(LockoutReason || (LockoutReason = {}));
// Unlock Methods
export var UnlockMethod;
...
export class AccountLockoutService extends EventEmitter {
     ...
     */
    async adminUnlock(lockoutId, unlockRequest) {
     ...
     */
    async emergencyUnlock(lockoutId, adminId, emergencyCode, justification) {
     ...
     */
    async approveUnlock(lockoutId, adminActionId, approverId, approved, comments) {
        const lockout = this.lockouts.get(lockoutId);
        if (!lockout) {
            throw new Error(`Lockout not found: ${lockoutId}`);
        }
    ...
    isUserLockedOut(userId) {
        ...
        return userLockouts.some(lockout => lockout.status === LockoutStatus.ACTIVE &&
            (!lockout.expiryTime || lockout.expiryTime > new Date()));
    }
    /**
     * Generate emergency override code
     */
    generateEmergencyCode(adminId, expiryHours = 24) {
    ...
    getLockoutStatistics(dateRange) {
        ...
        if (dateRange) {
            lockouts = lockouts.filter(l => l.lockoutTime >= dateRange.start && l.lockoutTime <= dateRange.end);
        }
    ...
    async requestUnlockApproval(lockout, unlockRequest) {
        const adminAction = {
            id: crypto.randomUUID(),
    ...
    async performUnlock(lockout, unlockRequest) {
    ...
    async sendLockoutNotification(lockout) {
    ...
    async sendNotification(lockout, notificationData) {
        const notification = {
            id: crypto.randomUUID(),
    ...
    generateLockoutNotificationContent(lockout) {
    ...
    logSecurityEvent(lockout, event, details = {}) {
        const logEntry = {
            timestamp: new Date(),
            event,
