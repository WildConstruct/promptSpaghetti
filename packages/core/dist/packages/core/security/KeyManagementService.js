...
(function (KeyPurpose) {
    KeyPurpose["SESSION_ENCRYPTION"] = "session_encryption";
    KeyPurpose["DATA_ENCRYPTION"] = "data_encryption";
    KeyPurpose["TOKEN_SIGNING"] = "token_signing";
    KeyPurpose["API_AUTHENTICATION"] = "api_authentication";
    KeyPurpose["MFA_VERIFICATION"] = "mfa_verification";
    KeyPurpose["DEVICE_VERIFICATION"] = "device_verification";
    KeyPurpose["PASSWORD_HASHING"] = "password_hashing";
    KeyPurpose["DATABASE_ENCRYPTION"] = "database_encryption";
    KeyPurpose["FILE_ENCRYPTION"] = "file_encryption";
    KeyPurpose["COMMUNICATION_ENCRYPTION"] = "communication_encryption";
...
(function (StorageTier) {
    ...
    StorageTier["HSM"] = "hsm"; // Hardware Security Module
...
export class KeyManagementService extends EventEmitter {
     ...
     * Rotate a key
     */
    async rotateKey(keyId, requesterId, options = {}) {
        const startTime = Date.now();
        try {
     ...
     * Revoke a key
     */
    async revokeKey(keyId, requesterId, reason) {
        const startTime = Date.now();
        try {
            const key = await this.loadKey(keyId);
            if (!key) {
     ...
     */
    async deriveKey(parentKeyId, derivationParams, requesterId) {
        const startTime = Date.now();
        try {
    ...
    async generateKeyMaterial(options) {
        switch (options.type) {
    ...
    async validateKeyAccess(key, requesterId) {
    ...
    matchesSearchCriteria(key, criteria) {
    ...
    async logKeyEvent(key, event, details) {
        const auditEvent = {
    ...
    calculateEventRiskScore(event, details) {
        switch (event) {
