...
(function (ReportFrequency) {
    ReportFrequency["REAL_TIME"] = "real_time";
    ReportFrequency["HOURLY"] = "hourly";
...
export class SecurityIntelligenceDashboard extends EventEmitter {
    ...
    async processSecurityAnomalies(anomalies) {
        try {
            ...
            this.dashboardMetrics.anomalies.falsePositives = anomalies.filter(a => a.isResolved && a.resolvedBy === 'false_positive').length;
    ...
    async processThreatForecasts(forecasts) {
        try {
            ...
            const threatTrends = forecasts.map(forecast => ({
    ...
    // ==========================================
    async updateSecurityPosture(intelligence) {
    ...
    async updateThreatIntelligence(intelligence) {
        ...
        const highSeverityThreats = activeThreats
            ...
            affectedSystems: intel.correlatedEvents.map(e => e.metadata?.system).filter(Boolean),
    ...
    async updateDashboardMetrics(intelligence) {
    ...
    // HELPER METHODS
    // ==========================================
    async generateRiskFactors(intelligence) {
        const riskFactors = [];
    ...
    async generateRecommendations(intelligence, postureScore) {
    ...
    async updateRiskFactorsFromAnomalies(anomalies) {
    ...
    async updatePostureTrends(forecasts) {
        ...
        if (highConfidenceForecasts.length > 0) {
            const avgIntensity = highConfidenceForecasts.reduce((sum, f) => sum + f.predictedIntensity, 0) / highConfidenceForecasts.length;
            trends.push({
    ...
    async checkReportingSchedules() {
        ...
        for (const schedule of this.config.reportingSchedules) {
            if (schedule.enabled && schedule.nextExecution <= now) {
                try {
                    const report = await this.generateExecutiveReport(schedule.reportType, this.calculateReportPeriod(schedule.frequency));
    ...
    async generateAdHocReport(analysisType, parameters) {
        // Ad-hoc analysis capability
        console.log(`🔍 GENERATING AD-HOC ANALYSIS: ${analysisType}`);
        return this.generateExecutiveReport(ReportType.EXECUTIVE_SUMMARY, this.calculateReportPeriod(ReportFrequency.DAILY));
    }
