...
export class PredictiveSecurityAnalytics extends EventEmitter {
     ...
     */
    async predictBruteForceAttacks(features, model) {
        const predictions = [];
        // High failure rate with low success rate indicates brute force
        if (features.loginFailures > 10 && features.successRate < 0.1) {
            const confidence = Math.min(0.9, (features.loginFailures / 20) * (1 - features.successRate) * (features.eventsPerMinute / 10));
            if (confidence > 0.6) {
                predictions.push({
     ...
     */
    async predictAccountTakeovers(features, model) {
        const predictions = [];
        ...
        if (features.unknownLocationRatio > 0.5 && features.averageRiskScore > 60) {
            const confidence = Math.min(0.85, features.unknownLocationRatio * (features.averageRiskScore / 100) * (features.uniqueIPs / 5));
            if (confidence > 0.5) {
                predictions.push({
                    predictionId: this.generatePredictionId(),
                    timestamp: new Date(),
     ...
     */
    async predictCredentialStuffing(features, model) {
        const predictions = [];
        ...
        if (features.uniqueIPs > 5 && features.successRate > 0.05 && features.successRate < 0.3) {
            const confidence = Math.min(0.8, (features.uniqueIPs / 10) * (features.loginAttempts / 50) * (1 - Math.abs(features.successRate - 0.15)));
            if (confidence > 0.4) {
                predictions.push({
                    predictionId: this.generatePredictionId(),
                    timestamp: new Date(),
     ...
     */
    async predictInsiderThreats(features, model) {
        const predictions = [];
        ...
        if (features.uniqueCountries === 1 && features.averageRiskScore > 40 && features.uniqueSessions > 3) {
            const confidence = Math.min(0.7, (features.averageRiskScore / 100) * (features.uniqueSessions / 10) * 0.8);
            if (confidence > 0.3) {
                predictions.push({
                    predictionId: this.generatePredictionId(),
                    timestamp: new Date(),
    ...
    countUniqueGeolocations(events, field) {
    ...
    calculateUnknownLocationRatio(events) {
        const totalWithGeolocation = events.filter(e => e.geolocation).length;
        if (totalWithGeolocation === 0)
            return 0;
        const unknownLocations = events.filter(e => e.geolocation && !e.geolocation.isKnownLocation).length;
        return unknownLocations / totalWithGeolocation;
    }
    countUniqueValues(events, field) {
        const unique = new Set();
        events.forEach(event => {
    ...
    extractAffectedEntities(features) {
    ...
    async sendThreatAlert(prediction) {
    ...
    async analyzeHistoricalData(timeframeHours = 24) {
        ...
        const relevantEvents = this.eventHistory.filter(event => event.timestamp.getTime() > cutoff);
