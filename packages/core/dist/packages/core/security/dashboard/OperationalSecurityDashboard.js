...
export const OperationalSecurityDashboard = ({
    alerts,
    metrics,
    systemStatus,
    threatIntel,
...
});
{
    ...
    // Render metric card
    const renderMetricCard = (title, value, subtitle, color, onClick) => ();
    ;
    _jsx("div", { onClick: onClick, style: {
    ...
    'all 0.2s ease',
        borderLeft;
    color ? `4px solid ${color}` : undefined;
}
    >
        (_jsx("div", { style: {
                fontSize: '12px',
...
;
_jsx("div", { onClick: () => setSelectedAlert(alert), style: {
        background: themeStyles.surface,
        border: `1px solid ${themeStyles.border}`
    }, "borderLeft:": true }, alert.id);
`4px solid ${getSeverityColor(alert.severity)}`;
borderRadius: '6px',
    padding;
'12px',
    marginBottom;
    ...
    (_jsx("div", { style: {
        ...
        ,
            _jsxs("div", { style: {
                    ...
                    fontSize: '11px',
                    color: themeStyles.textSecondary,
                }, children: [_jsxs("span", { children: ["Source: ", alert.source] }), _jsx("span", { children: "\u2022" }), _jsxs("span", { children: ["Assets: ", alert.affectedAssets.length] }), alert.assignee && ()
                        <  >
                        (_jsx("span", { children: "\u2022" })
                            ,
                                _jsxs("span", { children: ["Assigned: ", alert.assignee] }))] }));
div >
;
div >
;
...
_jsxs("div", { style: {
        ...
        fontFamily: 'Inter, system-ui, sans-serif',
            ...
            } }), ", padding: '16px 24px', position: 'sticky', top: 0, zIndex: 100; }}>", _jsxs("div", { style: {
                display: 'flex',
                justifyContent: 'space-between',
                alignItems: 'center',
    ...
    ,
        _jsxs("div", { style: { padding: '24px' }, children: [_jsxs("div", { style: {
                        display: 'grid',
                        gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))',
                        gap: '16px',
                        marginBottom: '24px',
                    }, children: [renderMetricCard(), "'Active Alerts', metrics.alerts.total, `$", metrics.alerts.newLast24h, " new in 24h`} } metrics.alerts.total > 100 ? themeStyles.error : themeStyles.primary )}", renderMetricCard(), "'Active Incidents', metrics.incidents.active, `$", metrics.incidents.escalated, " escalated`} } metrics.incidents.active > 10 ? themeStyles.warning : themeStyles.success )}", renderMetricCard(), "'System Health', `$", metrics.system.overallHealth, "%`} } `$", metrics.system.componentsOperational, "/$", metrics.system.totalComponents, " operational`} } metrics.system.overallHealth ", _jsx(, {}), " 95 ? themeStyles.error : themeStyles.success )}", renderMetricCard(), "'Team Status', `$", metrics.team.onlineAnalysts, "/$", metrics.team.totalAnalysts, "`} } `Workload: $", metrics.team.workload, "`} } metrics.team.workload === 'critical' ? themeStyles.critical : themeStyles.primary )}", renderMetricCard(), "'Response Time', `$", Math.round(metrics.alerts.avgResponseTime), "m`} } `SLA: $", metrics.alerts.slaCompliance, "%`} } metrics.alerts.slaCompliance ", _jsx(, {}), " 95 ? themeStyles.warning : themeStyles.success )}"] }), _jsxs("div", { style: {
                        display: 'grid',
                        ...
                        height: 'calc(100vh - 300px)',
                                ...
                                marginBottom: '16px',
                                        ...
                                        fontWeight: 600,
                                        color: themeStyles.text,
                                    }, children: ["\uD83D\uDEA8 Alert Queue (", filteredAlerts.length, ")"] }), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [['all', 'new', 'high', 'critical'].map(filter => ()
                                            < button, key = { filter }, onClick = {}()), " => setSelectedFilter(filter as typeof selectedFilter)} style=", {
                                            padding: '4px 12px',
                        ...
                        overflowY: 'auto',
                        paddingRight: '8px',
                    }, children: [filteredAlerts.length === 0 ? ()
                            < div : , " style=", {
                            display: 'flex',
                            ...
                            height: '100%',
                            color: themeStyles.textSecondary,
                        }, ">", _jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\u2705" }), _jsx("div", { children: "No alerts matching current filter" })] }), ") : () filteredAlerts.map(renderAlertItem) )}"] });
div >
...
'16px';
 >
    (_jsx("h3", { style: {
        ...
        ,
            _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: [systemStatus.slice(0, 6).map(system => ()
                        ...
                        borderRadius: '4px',
                        fontSize: '12px',
                    }), ">", _jsx("span", { style: { color: themeStyles.text }, children: system.component }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [system.responseTime && ()
                                < span, " style=", { color: themeStyles.textSecondary }, ">", system.responseTime, "ms"] }), ")}", _jsx("div", { style: {
                            width: '8px',
                            height: '8px',
                            borderRadius: '50%',
...
'16px';
 >
    (_jsx("h3", { style: {
        ...
        ,
            _jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: threatIntel.slice(0, 3).map(intel => ()
                    < div, key = { intel, : .feed }, style = {}, {
                    padding: '8px',
                    background: themeStyles.background,
                    borderRadius: '4px',
                }
                        ...
                        (_jsxs("div", { style: {
                            ...
                            ,
                                _jsxs("div", { style: {
                                        ...
                                        display: 'flex',
                                        gap: '8px',
                                    }, children: [_jsxs("span", { children: ["New: ", intel.newIndicators] }), _jsx("span", { children: "\u2022" }), _jsxs("span", { children: ["Active: ", intel.activeThreats] })] }))) }));
div >
;
div >
;
    ...
    (_jsxs("div", { style: {
        ...
        ,
            _jsxs("div", { style: { marginBottom: '16px' }, children: [_jsx("h4", { style: {
                ...
                ,
                    _jsxs("div", { style: { display: 'flex', gap: '16px', marginTop: '16px' }, children: [_jsx("button", { onClick: () => {
                                    ...
                                    color: themeStyles.textSecondary,
                                    border: `1px solid ${themeStyles.border}`
                                } }), ", borderRadius: '4px', padding: '8px 16px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', > Mark False Positive"] }));
div >
;
div >
;
