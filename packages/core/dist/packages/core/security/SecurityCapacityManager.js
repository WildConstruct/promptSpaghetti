...
export class SecurityCapacityManager extends EventEmitter {
    ...
    async collectCapacityMetrics(service) {
    ...
    calculateHealthScore(cpuUtil, memoryUtil, instances) {
        let score = 100;
    ...
    identifyBottlenecks(cpuUtil, memoryUtil) {
        const bottlenecks = [];
        if (cpuUtil > 80)
    ...
    generateScalingRecommendations(service, cpuUtil, memoryUtil) {
    ...
    identifyCostOptimizations(instances, cpuUtil) {
        const optimizations = [];
    ...
    // Scaling Operations
    async evaluateScalingPolicies(service, metrics) {
    ...
    async evaluateMetricTrigger(trigger, metrics) {
        let metricValue;
    ...
    compareMetricValue(value, threshold, comparison) {
        switch (comparison) {
            case 'greater_than':
    ...
    async executeScalingAction(service, policy, triggerReason, triggerType) {
    ...
    async performScaling(service, currentCapacity, targetCapacity, policyId, triggerReason) {
        const scalingEvent = {
    ...
    async sendScalingNotification(notification, service, event) {
    ...
    async generateCapacityForecast(service, horizonDays = 30) {
    ...
    generateForecastData(service, horizonDays) {
        const forecasts = [];
    ...
    identifyForecastRisks(day, predictedLoad, currentCapacity) {
        const risks = [];
    ...
    async generateCapacityRecommendations(service) {
        const immediate = [];
        const shortTerm = [];
        const longTerm = [];
    ...
    async manualScale(service, targetCapacity, reason, scaledBy) {
        const currentCapacity = this.currentCapacity.get(service) || 1;
        const plan = Array.from(this.capacityPlans.values()).find(p => p.service === service);
        if (!plan) {
    ...
    getCapacityStatus() {
        ...
        const recentScalingEvents = this.scalingEvents.filter(event => Date.now() - event.timestamp < 24 * 60 * 60 * 1000).length;
    ...
    calculateCostEfficiency(service, currentCapacity, metrics) {
        if (!metrics)
