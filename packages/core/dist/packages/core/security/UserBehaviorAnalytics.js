...
export class UserBehaviorAnalytics extends EventEmitter {
    ...
    updateGeographicPatterns(profile, event) {
        ...
        const existingLocation = profile.commonLocations.find(loc => this.calculateDistance(loc, event.geolocation) < 50 // 50km threshold
        );
        if (!existingLocation) {
    ...
    detectTemporalAnomalies(event, profile) {
        ...
        if (event.actionType === UserActionType.LOGIN) {
            ...
            if (totalLogins > 10 && loginTimeFrequency / totalLogins < 0.1) {
                anomalies.push(this.createAnomaly(event, AnomalyType.UNUSUAL_LOGIN_TIME, 0.8, `Login at unusual time: ${hour}:00`, AnomalySeverity.MEDIUM));
            }
        }
        ...
        if (hour < 6 || hour > 22) {
            ...
            if (totalActivity > 50 && offHoursActivity / totalActivity < 0.1) {
                anomalies.push(this.createAnomaly(event, AnomalyType.OFF_HOURS_ACTIVITY, 0.6, `Activity during off hours: ${hour}:00`, AnomalySeverity.LOW));
            }
        }
        return anomalies;
    }
    detectGeographicAnomalies(event, profile) {
        ...
        const minDistance = Math.min(...profile.commonLocations.map(loc => this.calculateDistance(loc, event.geolocation)));
        if (minDistance > profile.suspiciousLocationThreshold) {
            ...
            `Access from unusual location: ${event.geolocation.city}, ${event.geolocation.country}`, minDistance > 5000 ? AnomalySeverity.HIGH : AnomalySeverity.MEDIUM));
        }
        ...
        for (const recentEvent of recentEvents) {
            if (recentEvent.geolocation && recentEvent.id !== event.id) {
                ...
                if (maxSpeed > 1000) { // Faster than commercial aircraft
                    anomalies.push(this.createAnomaly(event, AnomalyType.IMPOSSIBLE_TRAVEL, 0.95, `Impossible travel: ${distance.toFixed(0)}km in ${timeDiff.toFixed(1)}h`, AnomalySeverity.CRITICAL));
                }
            }
        }
        return anomalies;
    }
    detectAccessAnomalies(event, profile) {
        const anomalies = [];
        ...
        if (!resourceAccess && this.isHighRiskResource(event.resource)) {
            anomalies.push(this.createAnomaly(event, AnomalyType.UNUSUAL_RESOURCE_ACCESS, 0.7, `First-time access to high-risk resource: ${event.resource}`, AnomalySeverity.HIGH));
        }
        ...
        if (this.isPrivilegedAction(event.actionType)) {
            ...
            if (recentPrivilegedActions.length > 10) {
                anomalies.push(this.createAnomaly(event, AnomalyType.RAPID_PERMISSION_ESCALATION, 0.8, `Rapid privileged actions: ${recentPrivilegedActions.length} in last hour`, AnomalySeverity.HIGH));
            }
        }
        return anomalies;
    }
    detectVolumeAnomalies(event, profile) {
        const anomalies = [];
        // Check excessive access volume
        const recentEvents = this.getRecentEvents(event.userId, undefined, 60 * 60 * 1000); // Last hour
        const averageHourlyActivity = profile.totalEvents / Math.max(1, (Date.now() - profile.createdAt.getTime()) / (60 * 60 * 1000));
        if (recentEvents.length > averageHourlyActivity * 5) {
            anomalies.push(this.createAnomaly(event, AnomalyType.EXCESSIVE_ACCESS_VOLUME, 0.7, `Excessive activity: ${recentEvents.length} events in last hour`, AnomalySeverity.MEDIUM));
        }
        ...
        if (event.actionType === UserActionType.DATA_EXPORT || event.actionType === UserActionType.FILE_DOWNLOAD) {
            ...
            if (dataVolume > averageVolume * 10) {
                anomalies.push(this.createAnomaly(event, AnomalyType.BULK_DATA_ACCESS, 0.8, `Large data volume: ${(dataVolume / 1024 / 1024).toFixed(1)}MB`, AnomalySeverity.HIGH));
            }
        }
        return anomalies;
    }
    detectDeviceAnomalies(event, profile) {
        const anomalies = [];
        // Check unusual device/user agent
        if (!profile.commonUserAgents.includes(event.userAgent)) {
            anomalies.push(this.createAnomaly(event, AnomalyType.UNUSUAL_DEVICE_USAGE, 0.6, `New device/browser: ${event.userAgent.substring(0, 50)}...`, AnomalySeverity.LOW));
        }
        return anomalies;
    }
    // ==========================================
    ...
    createAnomaly(event, type, confidence, description, severity) {
        return {
    ...
    getRecentEvents(userId, actionType, timeframeMs = 24 * 60 * 60 * 1000) {
    ...
    assessRisk(type, confidence, severity) {
    ...
    resolveAnomaly(anomalyId, falsePositive = false) {
