...
})(UserControlLevel || (UserControlLevel = {}));
{
    ...
    constructor(config, TransparencyConfig);
    {
        ...
        Promise < UserAccessActivity > {
            ...
            details: DSARRequestDetails, Promise() {
                const request = {
        ...
        };
        {
            ...
            async;
            updateTransparencySettings((), userId, string, settings, (Partial));
                ...
                if (!userSettings?.notificationPreferences.realTimeNotifications) {
                    ...
                    if (!userSettings.notificationPreferences.eventTypes.includes(notification.eventType)) {
                        ...
                        if (this.isInQuietHours(userSettings.notificationPreferences.quietHours)) {
                            ...
                            processDBARAutomatically(request, DataSubjectAccessRequest);
                            Promise < void  > { return: Promise.resolve() };
                            getDefaultSettings(userId, string);
                            TransparencySettings;
                            {
                                return {};
                            }
                            isInQuietHours(quietHours, { start: string, end: string });
                            boolean;
                            {
                                return false;
                            }
                            queueNotification(notification, TransparencyNotification);
                            void { this: .notificationQueue.push(notification) };
                            deliverNotification(notification, TransparencyNotification);
                            Promise < void  > { return: Promise.resolve() };
                            gatherUserDataForExport(userId, string);
                            categories ?  : string;
