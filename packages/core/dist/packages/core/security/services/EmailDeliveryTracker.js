...
(function (EmailType) {
    EmailType["ACCOUNT_VERIFICATION"] = "account_verification";
    EmailType["PASSWORD_RESET"] = "password_reset";
    EmailType["MFA_CODE"] = "mfa_code";
    EmailType["LOGIN_NOTIFICATION"] = "login_notification";
    EmailType["SECURITY_ALERT"] = "security_alert";
    EmailType["DEVICE_VERIFICATION"] = "device_verification";
    EmailType["BACKUP_CODE_DELIVERY"] = "backup_code_delivery";
})(EmailType || (EmailType = {}));
...
export class EmailDeliveryTracker extends EventEmitter {
     ...
     */
    addTrackingEvent(emailId, eventType, eventData) {
        const record = this.deliveryRecords.get(emailId);
        if (!record)
            return;
        switch (eventType) {
    ...
    // Private methods
    async sendEmailViaProvider(record, request) {
    ...
    addDeliveryAttempt(emailId, attempt) {
        const record = this.deliveryRecords.get(emailId);
        if (record) {
    ...
    handleStatusChange(record, previousStatus, newStatus) {
        // Handle bounces
    ...
    findEmailByMessageId(messageId) {
    ...
    processWebhookEvent(emailId, event) {
        this.updateStatus(emailId, event.status, {
            providerData: event.providerData
        });
    }
    updateStatistics(previousStatus, newStatus, emailType, provider) {
        // Update total counts
