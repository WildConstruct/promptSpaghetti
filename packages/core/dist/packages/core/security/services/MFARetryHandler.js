...
// Operation types
export var MFAOperation;
(function (MFAOperation) {
    MFAOperation["TOTP_VERIFICATION"] = "totp_verification";
    MFAOperation["SMS_SEND"] = "sms_send";
    MFAOperation["SMS_VERIFICATION"] = "sms_verification";
    MFAOperation["EMAIL_SEND"] = "email_send";
    MFAOperation["EMAIL_VERIFICATION"] = "email_verification";
    MFAOperation["BACKUP_CODE_VERIFICATION"] = "backup_code_verification";
    MFAOperation["DEVICE_REGISTRATION"] = "device_registration";
    MFAOperation["METHOD_SETUP"] = "method_setup";
    MFAOperation["METHOD_DISABLE"] = "method_disable";
...
(function (FailureType) {
    ...
    FailureType["INVALID_CODE"] = "invalid_code";
    FailureType["EXPIRED_CODE"] = "expired_code";
    FailureType["SERVICE_UNAVAILABLE"] = "service_unavailable";
    FailureType["AUTHENTICATION_FAILED"] = "authentication_failed";
    FailureType["VALIDATION_ERROR"] = "validation_error";
    FailureType["UNKNOWN_ERROR"] = "unknown_error";
})(FailureType || (FailureType = {}));
// Circuit breaker states
export var CircuitBreakerStateEnum;
(function (CircuitBreakerStateEnum) {
    CircuitBreakerStateEnum["CLOSED"] = "closed";
    CircuitBreakerStateEnum["OPEN"] = "open";
    CircuitBreakerStateEnum["HALF_OPEN"] = "half_open";
...
export class MFARetryHandler extends EventEmitter {
     ...
     */
    async executeWithRetry(operation, operationFn, context = {}) {
    ...
    // Private methods
    async executeOperation(operation, operationFn, context) {
    ...
    handleSuccess(operation, context) {
        const circuitBreaker = this.circuitBreakers.get(operation);
        if (circuitBreaker) {
    ...
    handleFailure(operation, context, error) {
        const circuitBreaker = this.circuitBreakers.get(operation);
        if (circuitBreaker) {
            ...
            if (circuitBreaker.state === CircuitBreakerStateEnum.CLOSED &&
                ...
                circuitBreaker.nextAttemptTime = new Date(Date.now() + this.config.circuitBreaker.resetTimeoutMs);
    ...
    updateMetrics(operation, success, attempts, durationMs) {
