...
(function (FeedbackCategory) {
    FeedbackCategory["POSITIVE"] = "POSITIVE";
    FeedbackCategory["NEGATIVE"] = "NEGATIVE";
    FeedbackCategory["NEUTRAL"] = "NEUTRAL";
...
export class PolicyPreviewStagingService extends EventEmitter {
    ...
    async deployToStaging(previewId, environmentId, options = {}) {
        ...
        const activeDeployments = preview.stagingDeployments.filter(d => d.status === StagingDeploymentStatus.ACTIVE ||
     ...
     */
    async runValidations(preview, validationTypes) {
        const results = [];
     ...
     */
    async generateComparisonReport(baseVersion, compareVersion, policyId) {
     ...
     */
    async collectUserFeedback(previewId, userId, feedback) {
        const preview = this.activePreviews.get(previewId);
        if (!preview) {
            throw new Error('Preview not found');
        }
    ...
    async executeValidation(preview, validationType) {
    ...
    async executeStageDeployment(deployment, environment) {
    ...
    async analyzePolicyDifferences(baseVersion, compareVersion, policyId) { return []; }
