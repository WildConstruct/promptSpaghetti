...
export const ZADA_SCREENPLAY_TEMPLATES = [
...
export const MARS_FRAMEWORK_TAGS = [
    // Camera Tags [CAM]
    {
        tag: 'CAM:WIDE',
        category: 'camera',
...
export class AdvancedPromptingMethodology {
     ...
     */
    generateZadaTemplate(components) {
        const selectedComponents = ZADA_SCREENPLAY_TEMPLATES.filter(comp => components.includes(comp.name));
     ...
     */
    createHybridTemplate(naturalTemplate, marsFramework = false) {
     ...
     */
    getMarsAutocompletions(context, currentInput) {
        const suggestions = [];
    ...
    sortByDependencies(components) {
        ...
        while (remaining.length > 0) {
            const nextComponent = remaining.find(comp => !comp.dependencies ||
