    ...
    metadata;
{
    title: name,
        description;
    `Generated PSG file: ${name}`;
}
tags: ['generated', 'mock'],
    version;
'1.0.0',
    created;
new Date(Date.now() - Math.random() * 60 * 24 * 60 * 60 * 1000), // Last 60 days
    author;
'Mock User';
...
void {
    try: {
        ...
        settings: ProjectSettings, Promise() {
            try {
                // Create metadata
...
try { }
    ...
    success: false,
        error;
    error instanceof Error ? error.message : 'Unknown error creating file dialog',
    ;
}
;
;
