...
Promise < BatchProcessingResult > {
    ...
    query: ConversionMetricQuery, Promise() {
...
funnelId: string,
    timeWindow;
number = 3600000; // 1 hour default): Promise<RealTimeMetrics> {,
...
Promise < DataExportResult > {
    ...
    */
    ...
    event: FlexibleConversionEvent, Promise() {
        const results = [];
...
try { }
    ...
    return results;
    async;
    processBatch((), events, FlexibleConversionEvent, options, BatchProcessingOptions);
    Promise < BatchProcessingResult > {
         ...
         */
        class: ConversionMetricsCalculator };
    {
        metricCache: (Map) = new Map();
        constructor(private, config, MetricCalculationConfig);
        { }
        async;
        calculateMetrics((), query, ConversionMetricQuery, events, FlexibleConversionEvent);
        Promise < ConversionMetricResult > {
            const: results, ConversionMetricResult = [],
            for(, metricType, of, query) { }, : .metrics
        };
        {
            try {
