...
export class ConversionAnalyticsInfrastructure {
    ...
    dataWarehouse;
    analyticsAPI;
    constructor(epic1Analytics, config) {
        this.epic1Analytics = epic1Analytics;
        this.config = config;
        this.processingPipeline = new ConversionProcessingPipeline(this.epic1Analytics, this.config.processing);
        this.metricsCalculator = new ConversionMetricsCalculator(this.config.processing.calculations);
        this.dataWarehouse = new ConversionDataWarehouse(this.config.dataWarehouse);
        this.analyticsAPI = new ConversionAnalyticsAPI(this.metricsCalculator, this.dataWarehouse, this.config.api);
    }
    /**
     ...
     */
    async getRealTimeMetrics(funnelId, timeWindow = 3600000 // 1 hour default
    ) {
...
export class ConversionProcessingPipeline {
    ...
    async getHealthStatus() {
        const stageHealth = await Promise.all(this.stages.map(async (stage) => ({
            name: stage.getName(),
...
export class ConversionMetricsCalculator {
    ...
    async calculateMetrics(query, events) {
        const results = [];
        for (const metricType of query.metrics) {
            try {
                const result = await this.calculateSingleMetric(metricType, query, events);
                results.push(result);
            }
            catch (error) {
    ...
    async calculateSingleMetric(metricType, query, events) {
    ...
    async calculateMetadata(metricType, value, events) {
        return {
    ...
    async calculateComparison(metricType, value, events) {
        // TODO: Implement historical comparison logic
        return undefined;
    }
    async calculateBreakdowns(metricType, events, groupBy) {
        const breakdowns = [];
    ...
    groupEventsByDimension(events, dimension) {
    ...
    extractDimensionValue(event, dimension) {
        switch (dimension) {
            case 'funnel_step':
...
class ValidationStage {
    ...
    async process(event) {
        ...
        const hasRequiredFields = this.config.requiredFields.every(field => this.getFieldValue(event, field) !== undefined);
        return {
...
class ConversionAnalyticsAPI {
    ...
    dataWarehouse;
    config;
    constructor(metricsCalculator, dataWarehouse, config) {
...
// Factory function
export const createConversionAnalyticsInfrastructure = (epic1Analytics, config) => {
