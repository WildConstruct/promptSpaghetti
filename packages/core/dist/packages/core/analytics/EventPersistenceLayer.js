...
export class DatabaseEventRepository {
    ...
    `);
        ...
        stmt.run(event.id, event.type, event.category, event.severity, event.timestamp, event.source, event.version, event.sessionId || null, event.userId || null, event.organizationId || null, event.requestId || null, event.traceId || null, JSON.stringify(event.data), JSON.stringify(event.metadata), JSON.stringify(event.tags), event.environment, event.region || null, now, retentionDate);
        return event.id;
    }
    /**
     ...
     */
    async getTimeSeriesData(metric, granularity, filter) {
        const { whereClause, params } = this.buildWhereClause(filter);
        let timeGrouping;
        switch (granularity) {
            case 'hour':
