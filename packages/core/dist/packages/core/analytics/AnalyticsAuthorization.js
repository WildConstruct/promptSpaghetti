 ...
 */
async;
authorizeEventPublication((), event, (Partial), authContext, AuthContext);
Promise < AuthorizationResult > {
    try: {
        : .hasPermission(authContext, AnalyticsPermission.PUBLISH_EVENTS)
    }
};
{
    return {
    ...
    if (result.allowed) {
         ...
         */
        async;
        authorizeEventAccess((), event, UnifiedAnalyticsEvent, authContext, AuthContext);
        Promise < AuthorizationResult > {
            try: {
                : .hasPermission(authContext, AnalyticsPermission.VIEW_EVENTS)
            }
        };
        {
            return {
         ...
         */
        async;
        authorizeAnalyticsQuery((), filter, EventFilter, authContext, AuthContext);
        Promise < { allowed: boolean, filteredQuery: EventFilter, reason: string } > {
            try: {
                : .hasPermission(authContext, AnalyticsPermission.VIEW_ANALYTICS)
            }
        };
        {
            return {
            ...
            if (!this.hasPermission(authContext, AnalyticsPermission.VIEW_ORGANIZATION_ANALYTICS)) {
                ...
                if (!this.hasPermission(authContext, AnalyticsPermission.VIEW_ALL_EVENTS)) {
                     ...
                     */
                    async;
                    authorizeDashboardAccess((), dashboardType, 'user' | 'organization' | 'admin' | 'system', authContext, AuthContext);
                    Promise < AuthorizationResult > {
                        try: {
