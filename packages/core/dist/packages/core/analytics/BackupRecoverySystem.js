...
(function (RecoveryStatus) {
    ...
    RecoveryStatus["VALIDATING"] = "validating";
    RecoveryStatus["VALIDATED"] = "validated";
...
export class BackupRecoverySystem {
     ...
     */
    async createBackup(backupName, filter, config = {}, description) {
     ...
     */
    async performBackup(backupId, events, filePath, config) {
     ...
     */
    async restoreFromBackup(backupId, targetFilter, validateBeforeRestore = true) {
     ...
     */
    async performRecovery(recoveryId, backup, targetFilter, validateBeforeRestore = true) {
        const recovery = this.recoveryMetadata.get(recoveryId);
        if (!recovery)
     ...
     */
    async formatBatchData(events, format, isSubsequentBatch) {
        switch (format) {
            case 'json':
     ...
     * Validate recovery
     */
    async validateRecovery(recoveryId, backup, expectedCount) {
        const issues = [];
        try {
     ...
     */
    updateBackupProgress(backupId, progressUpdate, throughputUpdate) {
        const progress = this.activeBackups.get(backupId);
        if (progress) {
