...
export class DashboardIntegrationService {
    ...
    constructor(eventBus, eventRepository, authService, wsServer, adapters, config = {}) {
     ...
     */
    async getConsolidatedDashboardData(filter, authContext, cacheKey) {
        ...
        try {
            ...
            const timeSeriesData = await this.eventRepository.getTimeSeriesData('count', 'hour', authorizedFilter);
     ...
     */
    async calculateConsolidatedMetrics(events, statistics, filter) {
     ...
     */
    async getWidgetPerformanceData(widgetId, widgetType, filter, authContext) {
        const startTime = Date.now();
     ...
     */
    async getSecurityWidgetData(filter, authContext) {
     ...
     */
    async getStandardWidgetData(widgetType, filter, authContext) {
        // Authorize query
        const queryAuth = await this.authService.authorizeAnalyticsQuery(filter, authContext);
        if (!queryAuth.allowed) {
     ...
     */
    async migrateLegacyRoute(systemName, routePath) {
        try {
     ...
     */
    async simulateMigration(systemName) {
    ...
    updateWidgetPerformance(widgetId, widgetType, loadTime, isError, cacheHit = false) {
