                ...
                async initialize() {
                    try {
                        ...
                        try { }
                            ...
                            async;
                            process(input, File | Blob | ArrayBuffer | { file: File | Blob | ArrayBuffer, audio: File | Blob | ArrayBuffer, data: File | Blob | ArrayBuffer });
                                    ...
                                    return {
                                        ...
                                        Promise() {
                                            ...
                                            Promise < WhisperTranscriptionResult > {
                                                ...
                                                options: (Partial),
                                                Promise() {
                                                    const timestamp_granularities = , granularity;
                                                     === 'both' ? ['word', 'segment'] : [granularity];
                                                    const whisperOptions = {
                                                        file,
                                                            ...
                                                            if (this.config.organization) {
                                                                ...
                                                                for (let attempt = 0; attempt <= maxRetries; attempt++) {
                                                                    try {
                                                                        ...
                                                                        formData,
                                                                            signal;
                                                                        AbortSignal.timeout(this.config.timeout || 300000); // 5 minutes for large files,
                                                                    }
                                                                    finally { }
                                                                    ;
                                                                    if (!response.ok) {
                                                ...
                                                _estimateDurationFromSize(file) {
                                                    const size = file instanceof ArrayBuffer ? file.byteLength : file.size;
                                                ...
                                                _calculateConfidenceScore(transcriptionData) {
                                                    if (transcriptionData.segments && transcriptionData.segments.length > 0) {
                                                        // Calculate average confidence from segments
                                                        const avgLogprob = transcriptionData.segments.reduce((sum, segment) => );
                                                ...
                                                } // Default confidence if no segment data
                                                , // Default confidence if no segment data
                                                _calculateCost(durationInSeconds) {
                                                    const costPerMinute = this._metadata.costPerRequest || (0.006 / 60);
                                                    const durationInMinutes = durationInSeconds / 60;
                                                    return Math.max(0.006, durationInMinutes * 60 * costPerMinute);
