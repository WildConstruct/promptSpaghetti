...
export class MidjourneyAdapter extends BaseAIModel {
    ...
    async upscaleImage(jobId, imageIndex) {
        try {
            const payload = {
                jobId,
                action: 'upscale',
    ...
    async createVariation(jobId, imageIndex) {
        try {
            const payload = {
                jobId,
                action: 'variation',
    ...
    async _submitJob(prompt, options) {
        const payload = {
            prompt,
    ...
    _processJobResult(status, originalPrompt, processedPrompt, options, generationTime = 0) {
        const images = [];
        if (status.imageUrl) {
            images.push({
                url: status.imageUrl,
...
class MidjourneyPromptTemplater {
    ...
    parsePromptParameters(prompt) {
        const parameters = {};
        let cleanPrompt = prompt;
        // Parse version
        const versionMatch = prompt.match(/--v(?:ersion)?\s+(v?[0-9.]+)/i);
        if (versionMatch) {
            parameters.version = versionMatch[1].replace('v', '');
