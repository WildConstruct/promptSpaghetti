...
export class PerformanceMonitor {
    ...
    registerModel(modelId, modelType, provider) {
    ...
    recordRequest(modelId, responseTime, success, tokensUsed, cost) {
    ...
    recordResourceUsage(modelId, resourceMetrics) {
        const modelData = this.models.get(modelId);
        if (!modelData)
            return;
        const metrics = modelData.metrics;
    ...
    generateReport(timeRange) {
        ...
        const totalResponseTime = allModels.reduce((sum, m) => sum + (m.metrics.averageResponseTime * m.metrics.totalRequests), 0);
    ...
    generateAlertMessage(modelId, metric, value, type) {
