...
export class ImageProcessor {
    ...
    constructor() {
        if (typeof window !== 'undefined' && window.HTMLCanvasElement) {
            ...
            async;
            convertFormat((), imageData, string | ArrayBuffer, options, ImageProcessingOptions);
            ...
            };
            {
                ...
                if (targetWidth / targetHeight > aspectRatio) {
                    ...
                    // Metadata extraction
                    async;
                    extractMetadata(imageData, string | ArrayBuffer);
                    ...
                    };
                    {
                        ...
                        // Batch processing
                        async;
                        processBatch((), images, string, options, ImageBatchProcessingOptions);
                        ...
                        for (let i = 0; i < images.length; i += concurrency) {
                            ...
                            async;
                            generateVariations((), imageData, string, options, ImageVariationOptions);
                            Promise < string > {
                            ...
                            }++;
                            {
                                ...
                                async;
                                compareImages(image1, string, image2, string);
                                Promise < {
...
}
{
    ...
    switch (position) {
        ...
        default:
            return { x: padding, y: itemHeight + padding };
            async;
            _createVariation(imageData, string, index, number, options, ImageVariationOptions);
            Promise < string > {
                const: image = await this._loadImage(imageData),
                this: ._setupCanvas(image.width, image.height),
                // Draw original image
