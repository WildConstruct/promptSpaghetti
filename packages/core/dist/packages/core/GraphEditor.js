...
_jsx(ContextualHelpSystem, { nodes: nodes, edges: edges, selectedNodeId: selectedNodeId, selectedEdgeId: selectedEdgeId, userLevel: "beginner" // This could be dynamic based on user profile
    ...
    ,
        _jsx(StatusBar, { statusMessage: statusMessage, errors: errors, onPreview: () => {
                const now = Date.now();
                const sinceChange = now - lastChangeRef.current;
                const run = () => {
                    runPreview({ nodes, edges });
...
{ /* Professional Loading State */ }
{
    ...
    >
        _jsx(ProfessionalSpinner, { type: "dots", size: 48, color: "#ff7c00", message: isCreatingNode ? 'Creating node...' : 'Generating previews...' });
div >
;
...
_jsx(PreviewModal, { open: previewOpen, loading: previewLoading, error: previewError, results: previewResults, onClose: () => {
    ...
    ,
        _jsx(CorrectionsStatsDashboard, { isOpen: statsOpen, onClose: () => setStatsOpen(false) });
{
    extensionsOpen && ()
...
{ /* Node Creation Animation Overlay */ }
{
    ...
    style = {};
    {
        ...
        100,
            background;
        'radial-gradient(circle, rgba(255, 124, 0, 0.3), transparent)',
            borderRadius;
        12,
            animation;
        'nodeCreatePulse 0.6s ease-out',
        ;
    }
}
/>;
...
export const GraphEditor = (props) => {
    return;
    _jsx(ReactFlowProvider, { children: _jsx(GraphEditorInner, { ...props }) });
};
;
;
