    ...
    constructor(config) {
        ...
        this.config = {
            ...
            async createIndex(name, fields, configuration) {
                ...
                async;
                addDocuments(indexName, string, documents, T, idField = 'id');
                Promise < void  > {
                    ...
                    for(, doc, of, documents) {
                        ...
                        ;
                        async;
                        addFacet(indexName, string, facet, SearchFacet);
                        Promise < void  > {
                            const: index = this.indexes.get(indexName),
                            if(, index) {
                                throw new Error(`Index ${indexName} not found`);
                            ...
                            if (cachedResult) {
                                ...
                                try {
                                    ...
                                    const result = {
                                        ...
                                        : .config.performance.enableCaching }, { this:  };
                                }
                                finally { }
                            }
                        }
                                                                    ...
                                                                    if (typeof fieldValue === 'string') {
                                                                        ...
                                                                        for (const term of terms) {
                                                                            if (term.startsWith(queryLower) && term.length > queryLower.length) {
                                                                                ...
                                                                                term,
                                                                                    highlight;
                                                                                `<strong>${queryLower}</strong>${term.substring(queryLower.length)}`;
                                                                            }
                                                                        }
