...
export class VersionRestoreManager {
    ...
    restoreHistory = [];
    constructor(apiClient, projectId, userId, versionHistoryManager) {
    ...
    async createRestorePreview(snapshotId, currentGraphData, options = {}) {
        try {
    ...
    async executeRestore(snapshotId, options = {}, conflictResolutions = {}) {
        try {
    ...
    async performRestore(restoreId, snapshotId, options, conflictResolutions) {
        ...
        try {
            ...
            const changesToApply = await this.calculateChangesToApply(currentGraphData, snapshotData, options, resolvedConflicts);
            this.updateRestoreState(restoreId, {
    ...
    async createBackupSnapshot(title) {
        try {
            const currentGraphData = await this.getCurrentGraphData();
    ...
    async detectConflicts(snapshotData, currentData) {
        const conflicts = [];
    ...
    detectWorkflowConflicts(snapshotData, currentData) {
        const conflicts = [];
    ...
    detectDependencyConflicts(snapshotData, currentData) {
        const conflicts = [];
    ...
    async resolveConflicts(conflicts, conflictResolutions) {
        const resolved = [];
        const unresolved = [];
        const warnings = [];
    ...
    async calculateChangesToApply(currentData, snapshotData, options, resolvedConflicts) {
        const changes = {
            nodes_to_add: [],
            nodes_to_remove: [],
            nodes_to_modify: [],
    ...
    // Utility methods
    updateRestoreState(restoreId, updates) {
