...
type: z.literal('WeightedChoice').default('WeightedChoice'),
    choices;
z.array(z.object({}), text, z.string(), weight, z.number().min(0).default(1));
...
type: z.literal('WeightedAdvanced').default('WeightedAdvanced'),
    choices;
z.array(z.object({}), text, z.string(), weight, z.number().min(0).default(1), metadata, z.record(z.any()).optional());
([{ text: 'advanced choice', weight: 1 }]),
    distribution;
z.enum(['uniform', 'exponential', 'gaussian', 'custom']).default('uniform'),
    parameters;
...
z.string().default('true'),
    trueOutput;
z.string().default('true result'),
    falseOutput;
z.string().default('false result'),
    variables;
z.record(z.any()).optional(),
;
...
type: z.literal('Sequential').default('Sequential'),
    items;
z.array(z.string()).default(['item1', 'item2']),
    pattern;
z.enum(['linear', 'cyclical', 'random', 'weighted']).default('linear'),
    currentIndex;
z.number().default(0),
    history;
...
type: z.literal('Markov').default('Markov'),
    states;
z.array(z.string()).default(['state1', 'state2']),
    transitions;
z.record(z.record(z.number())).default({}),
    currentState;
z.string().default(''),
    maxIterations;
