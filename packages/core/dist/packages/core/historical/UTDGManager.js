                ...
                Promise < GeneratedContent > {
                    ...
                    return: {
                        ...
                        : 
                            ...
                            .map(node => ({}), name, node.content.split(' ')[0], // First word as material name,
                        properties, this.extractMaterialProperties(node), historical_basis, node.external_source?.url || node.metadata.source, authenticity_level, node.metadata.authenticity)
                    },
                    // Generate texture descriptions
                    const: textures = nodes,
                    : 
                        .filter(node => node.type === 'pattern' || node.type === 'texture')
                        .map(node => ({}), name, node.content, pattern, this.extractPatternDescription(node), color_palette, this.extractColorPalette(node), historical_source, node.metadata.source) };
                ;
                            ...
                            if (!query.category || query.category.includes('material')) {
                                ...
                                if (query.limit) {
                                    ...
                                    return {
                                         ...
                                         */
                                        generateHistoricalContentSync(config) {
                                            const startTime = performance.now();
                                            ...
                                            return: {
                                                ...
                                                filterByConfiguration(nodes, config) {
                                                ...
                                                applyCreativityFilter(nodes, config) {
                                                ...
                                                getAppliedConstraints(nodes, era) {
                                                ...
                                                calculateAccuracyScore(nodes, validation) {
                                                    if (nodes.length === 0)
                                                        return 0;
                                                ...
                                                ,
                                                // VFX Export helper methods
                                                extractMaterialProperties(node) {
                                                    return {
                                                        type: node.type,
                                                ...
                                                extractPatternDescription(node) {
                                                    return node.content.split('.')[0] || node.content;
                                                },
                                                extractColorPalette(node) {
                                                ...
                                                generateClothingCombinations(nodes) {
                                                    const combinations = [];
                                                ...
                                                calculateSocialDistribution(nodes) {
                                                    const distribution = {};
                                                    let total = 0;
                                                    nodes.forEach(node => { });
                                                    if (node.metadata.social_class) {
                                            ...
                                            return: distribution,
                                            determineArchitecturalStyle(era) {
