...
, 
// Initial state
nodes, [], edges, [], stickyNotes, [], annotations, {
    stickyNotes: [],
    nodeLabelConfigs: {},
...
((state) => ({
    ...
    updateNode;
(nodeId, partial) => set((state) => ({}), nodes, state.nodes.map((node) => , node.id === nodeId ? mergeNodeData(node, partial) : node), hasUnsavedChanges, true);
addVariation: (nodeId, variation) => set((state) => ({}), nodes, addVariationToNode(state.nodes, nodeId, variation), hasUnsavedChanges, true);
removeVariation: (nodeId, variationIndex) => set((state) => ({}), nodes, removeVariationFromNode(state.nodes, nodeId, variationIndex), hasUnsavedChanges, true);
updateVariation: (nodeId, variationIndex, newValue) => set((state) => ({}), nodes, updateVariationInNode(state.nodes, nodeId, variationIndex, newValue), hasUnsavedChanges, true);
reorderVariations: (nodeId, fromIndex, toIndex) => set((state) => ({}), nodes, reorderVariationsInNode(state.nodes, nodeId, fromIndex, toIndex), hasUnsavedChanges, true);
...
deleteNode: (nodeId) => ,
    set((state) => ({}), nodes, state.nodes.filter((n) => n.id !== nodeId), edges, state.edges.filter((e) => e.source !== nodeId && e.target !== nodeId), hasUnsavedChanges, true);
// Sticky notes operations (Epic 8.7)
setStickyNotes: (notes) => ,
    set((state) => ({}), stickyNotes, notes, annotations, {
        ...state.annotations,
        stickyNotes: notes,
        metadata: {
...
addStickyNote: (note) => ,
    set((state) => ({}), stickyNotes, [...state.stickyNotes, note], annotations, {
        ...state.annotations,
...
setConnectionAnnotationPreferences: (preferences) => ,
    set((state) => ({}), connectionAnnotationPreferences, {
