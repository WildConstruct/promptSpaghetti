...
export class UTDGFoundation {
     ...
     */
    async integrateMedievalContent() {
     ...
     */
    async buildHistoricalRelationships() {
    ...
    query(query) {
        ...
        if (query.nodeTypes && query.nodeTypes.length > 0) {
            candidateNodes = candidateNodes.filter(node => query.nodeTypes.includes(node.type));
        }
        // Filter by eras
        if (query.eras && query.eras.length > 0) {
            candidateNodes = candidateNodes.filter(node => node.metadata.era && query.eras.includes(node.metadata.era));
        }
        // Filter by genres
        if (query.genres && query.genres.length > 0) {
            candidateNodes = candidateNodes.filter(node => node.metadata.genre && query.genres.includes(node.metadata.genre));
        }
        // Filter by tags
        if (query.tags && query.tags.length > 0) {
            candidateNodes = candidateNodes.filter(node => query.tags.some(tag => node.metadata.tags.includes(tag)));
        }
        // Filter by confidence threshold
        if (query.confidenceThreshold !== undefined) {
            candidateNodes = candidateNodes.filter(node => node.metadata.confidence >= query.confidenceThreshold);
        }
    ...
    generateContentSuggestions(context) {
        ...
        const characters = relevantNodes.filter(node => node.metadata.tags.includes('character') || node.type === 'entity' && node.id.startsWith('character_'));
        ...
        const settings = relevantNodes.filter(node => node.metadata.tags.includes('location') || node.metadata.tags.includes('setting'));
        ...
        const objects = relevantNodes.filter(node => node.metadata.tags.includes('clothing') ||
     ...
     */
    validateHistoricalAccuracy(node, context) {
        const violations = [];
        const suggestions = [];
        ...
        if (context.technical.sources === 'academic' &&
            !node.metadata.sources.some(source => source.includes('academic') ||
     ...
     */
    hasCircularDependency(nodeId, visited, recursionStack) {
     ...
     */
    getSocialClassEconomicPower(socialClass) {
        const powerMap = {
            'peasant': 0.1,
            'merchant': 0.4,
            'craftsman': 0.3,
    ...
    getSocialClassPoliticalInfluence(socialClass) {
        const influenceMap = {
            'peasant': 0.0,
            'merchant': 0.2,
            'craftsman': 0.1,
    ...
    getSocialClassMobility(socialClass) {
        const mobilityMap = {
            'peasant': 0.1,
            'merchant': 0.6,
            'craftsman': 0.4,
    ...
    getSocialClassOccupations(socialClass) {
        const occupationMap = {
