                    ...
                    Promise < Y.Doc > {
                        ...
                        removed: (Set), void:  > {
                            ...
                            for(, doc, of, added) {
                                ...
                                async;
                                joinWorkspaceSync(workspaceId, WorkspaceId, userId, UserId);
                                Promise < Y.Doc | null > {
                                    const: key = this.getSyncKey(workspaceId),
                            ...
                            async leaveWorkspaceSync(workspaceId, userId) {
                                ...
                                getSyncState(workspaceId, WorkspaceId, projectId ?  : ProjectId, resourceId ?  : ResourceId);
                                WorkspaceSyncState | null;
                                {
                                    const key = this.getSyncKey(workspaceId, projectId, resourceId);
                                    return this.syncStates.get(key) || null;
                                    // Get all active participants in a workspace
                                    getActiveParticipants(workspaceId, WorkspaceId);
                                    UserId;
                                    {
                                        const key = this.getSyncKey(workspaceId);
                                        const syncState = this.syncStates.get(key);
                                        ...
                                        async;
                                        forceSynchronization(workspaceId, WorkspaceId);
                                        Promise < void  > {
                                            const: key = this.getSyncKey(workspaceId),
                                            const: ydoc = this.ydocs.get(key),
                                            const: syncState = this.syncStates.get(key),
