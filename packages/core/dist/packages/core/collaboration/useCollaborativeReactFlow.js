...
export function useCollaborativeReactFlow() {
    ...
    const flowNodes = useMemo(() => {
        return graph.nodes.map(node => ({}), id, node.id, type, node.type.toLowerCase(), position, { x: 0, y: 0 }, // Position will be managed by React Flow
        data, {
            ...node,
            isCollaborative,
...
const flowEdges = useMemo(() => {
    return graph.edges.map(edge => ({}), id, edge.id, source, edge.source, target, edge.target, sourceHandle, edge.sourceHandle, targetHandle, edge.targetHandle, type, 'step', style, {
        stroke: '#666',
        strokeWidth: 2,
        ...
        */
        export function useNodeCollaborators(nodeId) {
            ...
            export function useCollaborationStatus() {
                return useCollaborativeGraphStore(state => ({}), isCollaborative, state.isCollaborative, connectionStatus, state.connectionStatus, isConnected, state.isConnected, connectedUserCount, state.connectedUsers.size, lastSyncTime, state.lastSyncTime);
            }
            ;
        }
    }
