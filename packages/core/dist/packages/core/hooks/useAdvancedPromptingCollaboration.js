...
import { AdvancedPromptingCollaborationService } from '../services/AdvancedPromptingCollaborationService';
 ...
 */
export function useAdvancedPromptingCollaboration(config = {}) {
    ...
    const initializeCollaboration = useCallback(async (user, config = {}) => {
    ...
    // Session management
    const createSession = useCallback(async (title, methodology) => {
        if (!state.collaborationService || !state.currentUser) {
            throw new Error('Collaboration service not initialized');
        }
        const session = await state.collaborationService.createCollaborationSession(title, methodology, state.currentUser.id);
        setState(prev => ({
            ...prev,
    ...
    const joinSession = useCallback(async (sessionId) => {
        ...
        const success = await state.collaborationService.joinCollaborationSession(sessionId, state.currentUser.id);
        if (success) {
    ...
    const createMARSRegion = useCallback(async (regionData) => {
        ...
        const region = await state.collaborationService.createMARSRegion(state.currentSession.sessionId, state.currentUser.id, regionData);
        return region;
    }, [state.collaborationService, state.currentUser, state.currentSession]);
    // Zada pattern management
    const createZadaPattern = useCallback(async (patternData) => {
        if (!state.collaborationService || !state.currentUser || !state.currentSession) {
            throw new Error('Collaboration service or session not available');
        }
        const pattern = await state.collaborationService.createZadaPattern(state.currentSession.sessionId, state.currentUser.id, patternData);
        return pattern;
    }, [state.collaborationService, state.currentUser, state.currentSession]);
    // VFX export
    const exportToVFXPipeline = useCallback(async () => {
        ...
        const vfxExport = await state.collaborationService.generateVFXExport(state.currentSession.sessionId, state.currentUser.id);
        return vfxExport;
    }, [state.collaborationService, state.currentUser, state.currentSession]);
    // Graph integration methods
    const applyMARSRegionToGraph = useCallback(async (regionId, nodeIds) => {
    ...
    const isUserCompatibleWithMethodology = useCallback((methodology) => {
