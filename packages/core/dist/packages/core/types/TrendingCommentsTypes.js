...
z.date(),
    weight;
z.number().min(0).max(10).default(1), // Engagement weight for scoring
    contextData;
...
z.number().default(0),
    replyEngagement;
z.number().default(0), // Engagement on reply threads,
    viewCount;
z.number().default(0),
    uniqueEngagers;
z.number().default(0); // Unique users who engaged,
trends: z.object({});
...
z.number().min(0).max(23).optional(),
    velocityTrend;
z.enum(['accelerating', 'steady', 'declining', 'stagnant']).default('steady'),
;
;
...
z.string(),
    resourceType;
z.enum(['template', 'graph', 'collection', 'case_study', 'tutorial', 'marketplace_item']),
    parentCommentId;
z.string().uuid().optional(), // For reply threading,
    authorId;
z.string().uuid(),
    authorDisplayName;
z.string(),
...
z.string().min(1).max(5000),
    contentType;
z.enum(['text', 'markdown', 'rich']).default('text'),
    mentions;
z.array(z.string().uuid()).default([]), // Mentioned user IDs,
    hashtags;
z.array(z.string()).default([]),
    attachments;
z.array(z.object({}), attachmentId, z.string().uuid(), type, z.enum(['image', 'video', 'audio', 'document', 'link']), url, z.string().url(), thumbnail, z.string().url().optional(), title, z.string().optional(), description, z.string().optional(), fileSize, z.number().positive().optional(), mimeType, z.string().optional());
([]),
    createdAt;
z.date(),
    updatedAt;
...
z.boolean().default(false),
    isHighlighted;
z.boolean().default(false), // Author or moderator highlight
    moderationStatus;
...
z.number().default(0),
    replyTree;
z.array(z.lazy(() => TrendingCommentSchema)).default([]), // Nested replies
    visibility;
...
z.number().min(0).max(1),
    emotions;
z.record(z.number().min(0).max(1)).default({}), // emotion -> confidence
    toxicity;
...
z.enum(['template', 'graph', 'collection', 'case_study', 'tutorial', 'marketplace_item']),
    period;
z.enum(['1h', '6h', '24h', '7d', '30d', 'all_time']),
    algorithmUsed;
z.string(),
    generatedAt;
z.date(),
...
z.number().default(0),
    qualifiedComments;
z.number().default(0), // Comments meeting trending criteria,
    trendingComments;
...
z.number().default(0),
    conversationHealth;
z.enum(['excellent', 'good', 'fair', 'poor']).default('fair'),
...
z.boolean().default(false),
    dataFreshness;
z.number().default(0), // Minutes since last update,
    algorithmVersion;
z.string().default('1.0.0'),
;
;
...
z.number().default(0),
    commentsGrowthRate;
z.number().default(0), // Percentage growth,
    engagementRate;
...
([]),
    engagementTrends;
z.array(z.object({}), timestamp, z.date(), likes, z.number(), replies, z.number(), shares, z.number());
([]),
    sentimentTrends;
z.array(z.object({}), timestamp, z.date(), positive, z.number(), neutral, z.number(), negative, z.number());
([]),
    topicEvolution;
z.array(z.object({}), topic, z.string(), mentions, z.number(), sentiment, z.number(), period, z.string());
([]);
breakdowns: z.object({});
byEngagementType: z.record(z.number()).default({}),
    byUserType;
z.record(z.number()).default({}), // verified, new, regular, etc.
    byTimeOfDay;
...
z.array(z.string()).default([]), // User IDs,
    emergingTrends;
z.array(z.string()).default([]),
    contentRecommendations;
z.array(z.string()).default([]),
    moderationAlerts;
z.array(z.object({}), type, z.string(), severity, z.enum(['low', 'medium', 'high', 'critical']), message, z.string(), commentIds, z.array(z.string()));
([]);
;
...
z.enum(['template', 'graph', 'collection', 'case_study', 'tutorial', 'marketplace_item']),
    period;
z.enum(['1h', '6h', '24h', '7d', '30d', 'all_time']).default('24h'),
    sortOrder;
z.enum(['trending', 'recent', 'top_rated', 'controversial', 'oldest']).default('trending'),
    limit;
...
z.string().optional(),
    userId;
z.string().uuid().optional(); // For personalized results,
;
...
export const validateGetTrendingCommentsRequest = (data) => {
    return GetTrendingCommentsRequestSchema.parse(data);
};
export const validateTrendingComment = (data) => {
    return TrendingCommentSchema.parse(data);
};
export const validateCommentScore = (data) => {
    return CommentScoreSchema.parse(data);
};
