...
z.string().uuid().optional(),
    branchType;
BranchTypeSchema.default('feature'),
    autoMergeEnabled;
z.boolean().default(false),
    requiresReview;
z.boolean().default(false),
...
z.string().optional(),
    protectionLevel;
ProtectionLevelSchema.optional(),
    autoMergeEnabled;
z.boolean().optional(),
    requiresReview;
z.boolean().optional(),
...
mergeRequestId: z.string().uuid(),
    mergeStrategy;
z.enum(['merge', 'squash', 'rebase']).default('merge'),
    commitMessage;
z.string().optional(),
    deleteSourceBranch;
z.boolean().default(false),
...
projectId: z.string().uuid().optional(),
    branchType;
BranchTypeSchema.optional(),
    status;
BranchStatusSchema.optional(),
    protectionLevel;
ProtectionLevelSchema.optional(),
    createdBy;
z.string().uuid().optional(),
...
z.number().int().min(0).default(0),
    sortBy;
z.enum(['name', 'created_at', 'updated_at', 'last_activity_at']).default('updated_at'),
    sortOrder;
...
z.string().uuid().optional(),
    status;
MergeRequestStatusSchema.optional(),
    createdBy;
z.string().uuid().optional(),
    assignedTo;
z.string().uuid().optional(),
...
z.number().int().min(0).default(0),
    sortBy;
z.enum(['created_at', 'updated_at', 'title']).default('updated_at'),
    sortOrder;
z.enum(['asc', 'desc']).default('desc'),
;
;
...
z.record(z.number().int()),
    recentActivity;
z.array(z.object({}), branchId, z.string().uuid(), branchName, z.string(), activityType, z.string(), activityDate, z.date(), userId, z.string().uuid(), userName, z.string());
;
...
ProjectBranchSchema,
    commonAncestor;
BranchCommitSchema.optional(),
    ahead;
z.number().int(),
    behind;
z.number().int(),
    conflicts;
z.array(BranchConflictSchema),
    canMerge;
z.boolean(),
    mergeStrategy;
z.enum(['merge', 'squash', 'rebase']).optional(),
    estimatedMergeTime;
