...
z.boolean().default(false),
    // Retention settings
    retentionDays;
z.number().int().default(365),
    autoAnonymizeAfterDays;
z.number().int().default(90),
    createdAt;
z.date(),
    updatedAt;
z.date(),
...
z.string().optional(),
    changeType;
ChangeTypeSchema.optional(),
    authorId;
z.string().uuid().optional(),
    authorType;
AuthorTypeSchema.optional(),
    sessionId;
z.string().optional(),
    batchId;
z.string().uuid().optional(),
...
z.number().int().min(0).default(0),
    sortBy;
z.enum(['created_at', 'effective_at', 'change_size', 'confidence_score']).default('created_at'),
    sortOrder;
...
projectId: z.string().uuid(),
    period;
AggregationPeriodSchema.optional(),
    authorId;
z.string().uuid().optional(),
    startDate;
z.date().optional(),
...
projectId: z.string().uuid(),
    settings;
AttributionPrivacySettingsSchema.omit({ id: true, projectId: true, userId: true, createdAt: true, updatedAt: true });
;
// Response types
...
contributors: z.array(z.object({}), authorId, z.string().uuid().optional(), authorName, z.string().optional(), authorType, AuthorTypeSchema, totalChanges, z.number().int(), firstContribution, z.date(), lastContribution, z.date(), activePeriods, z.array(z.object({}), period, z.date(), changes, z.number().int())),
    expertise;
z.array(z.object({}), resourceType, ResourceTypeSchema, changes, z.number().int(), percentage, z.number());
