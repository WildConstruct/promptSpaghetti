export class RecentProjectsManager {
    static STORAGE_KEY = 'promptspaghetti_recent_projects';
    static MAX_RECENT_PROJECTS = 5;
    static STORAGE_VERSION = '1.0.0';
    /**
    * Add a project to the recent projects list
    */
    static addRecentProject(entry) {
        try {
        ...
        try {
            ...
            if (!stored) {
                ...
                if (data.version !== this.STORAGE_VERSION) {
                    ...
                    return data.projects.sort((a, b) => new Date(b.lastAccessDate).getTime() - new Date(a.lastAccessDate).getTime());
                }
                try { }
                catch (error) {
    ...
    * Update the last access date for a project
    */
    static updateLastAccess(projectName) {
        try {
            const recentProjects = this.getRecentProjects();
    ...
    * Remove a specific project from recent projects
    */
    static removeRecentProject(projectName) {
        try {
            const recentProjects = this.getRecentProjects();
    ...
    * Generate thumbnail for a project based on graph data
    */
    static generateThumbnail(nodes, edges) {
        try {
            ...
            const positions = displayNodes.map((_, index) => ({}), x, (index % 5) * 24 + 12, y, Math.floor(index / 5) * 20 + 12);
        }
        finally // Create SVG string
         { }
        ;
        // Create SVG string
    ...
    edges;
    sourceIndex = displayNodes.findIndex(n => n.id === edge.source);
    targetIndex = displayNodes.findIndex(n => n.id === edge.target);
    if(sourceIndex) { }
}
 >= 0 && targetIndex >= 0;
{
        ...
        try {
            ...
            for (const key in localStorage) {
                ...
                try { }
                    ...
                    getProjectDisplayInfo(entry, RecentProjectEntry);
                    {
                        name: string;
                        lastAccessed: string;
                        size: string;
                        ...
                        saveRecentProjects(projects, RecentProjectEntry);
                        void {
