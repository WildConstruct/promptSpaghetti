...
export class SelectiveStateManager extends EventEmitter {
    ...
    registerComponentDependency(componentId, path, selector, priority = 'normal') {
        const dependency = {
            componentId,
            path,
            selector,
    ...
    unregisterComponentDependency(componentId, path) {
    ...
    updateState(path, value, operation = 'set') {
        const affectedComponents = this.getDependentComponents(path);
        const update = { path, value, operation };
        const batch = {
    ...
    // Memoized selectors
    createSelector(selector, dependencies, options = {}) {
    ...
    shouldSkipUpdate(componentId, batches) {
        ...
        for (const dep of dependencies) {
            if (dep.selector && dep.lastValue !== undefined) {
                ...
                const hasRelevantUpdates = batches.some(batch => batch.updates.some(update => this.pathMatches(dep.path, update.path)));
    ...
    // Debug methods
    debugComponentDependencies(componentId) {
        if (componentId) {
