...
export const TimeTravelPanel = ({ timeTravel, timeTravelState, selectedDomain, onDomainChange }) => {
    ...
    const filteredTimeline = timeline.filter(entry => selectedDomain === 'all' || entry.domain === selectedDomain);
    ...
    return (_jsxs("div", { className: "timetravel-panel", children: [_jsxs("div", { className: "timetravel-controls", children: [_jsxs("div", { className: "playback-controls", children: [_jsx("button", { className: "control-btn", onClick: handleGoToStart, disabled: !canGoBack, title: "Go to Start", children: "\u23EE\uFE0F" }), _jsx("button", { className: "control-btn", onClick: handleStepBack, disabled: !canGoBack, title: "Step Back", children: "\u2B05\uFE0F" }), _jsx("button", { className: "control-btn", onClick: handleStepForward, disabled: !canGoForward, title: "Step Forward", children: "\u27A1\uFE0F" }), _jsx("button", { className: "control-btn", onClick: handleGoToEnd, disabled: !canGoForward, title: "Go to End", children: "\u23ED\uFE0F" })] }), _jsx("div", { className: "position-info", children: _jsxs("span", { children: ["Position: ", currentPosition + 1, " / ", timeline.length] }) }), _jsxs("div", { className: "replay-controls", children: [_jsxs("label", { children: ["Speed:", _jsxs("select", { value: playbackSpeed, onChange: (e) => setPlaybackSpeed(parseFloat(e.target.value)), children: [_jsx("option", { value: 0.25, children: "0.25x" }), _jsx("option", { value: 0.5, children: "0.5x" }), _jsx("option", { value: 1, children: "1x" }), _jsx("option", { value: 2, children: "2x" }), _jsx("option", { value: 4, children: "4x" })] })] }), _jsxs("label", { children: [_jsx("input", { type: "checkbox", checked: isAutoPlay, onChange: (e) => setIsAutoPlay(e.target.checked) }), "Auto-play"] }), !isReplaying ? (_jsx("button", { className: "control-btn", onClick: handleStartReplay, children: "\u25B6\uFE0F Replay" })) : (_jsx("button", { className: "control-btn", onClick: handleStopReplay, children: "\u23F9\uFE0F Stop" }))] })] }), _jsx("div", { className: "domain-filter", children: _jsxs("label", { children: ["Domain:", _jsxs("select", { value: selectedDomain, onChange: (e) => onDomainChange(e.target.value), children: [_jsx("option", { value: "all", children: "All Domains" }), [...new Set(timeline.map(entry => entry.domain))].map(domain => (_jsx("option", { value: domain, children: domain }, domain)))] })] }) }), _jsxs("div", { className: "timeline-slider", children: [_jsx("input", { type: "range", min: 0, max: Math.max(0, filteredTimeline.length - 1), value: currentPosition, onChange: (e) => handleGoToPosition(parseInt(e.target.value)), className: "slider" }), _jsx("div", { className: "timeline-markers", children: markers.map(marker => {
                            const entryIndex = filteredTimeline.findIndex(e => e.id === marker.entryId);
                            if (entryIndex === -1)
                                return null;
                            const position = (entryIndex / (filteredTimeline.length - 1)) * 100;
                            return (_jsx("div", { className: "timeline-marker", style: {
                                    left: `${position}%`,
                                    backgroundColor: marker.color
                                }, title: marker.name, onClick: () => handleGoToEntry(marker.entryId) }, marker.id));
                        }) })] }), _jsxs("div", { className: "branch-section", children: [_jsxs("div", { className: "section-header", children: [_jsx("h4", { children: "Branches" }), _jsx("button", { className: "add-btn", onClick: handleCreateBranch, children: "+ Branch" })] }), _jsx("div", { className: "branch-list", children: branches.map(branch => (_jsxs("div", { className: `branch-item ${timeTravelState?.currentBranch === branch.id ? 'active' : ''}`, onClick: () => handleSwitchBranch(branch.id), children: [_jsxs("div", { className: "branch-info", children: [_jsx("span", { className: "branch-name", children: branch.name }), _jsxs("span", { className: "branch-entries", children: [branch.entryIds.length, " entries"] })] }), _jsx("div", { className: "branch-color", style: { backgroundColor: branch.metadata.color } })] }, branch.id))) })] }), _jsxs("div", { className: "timeline-section", children: [_jsxs("div", { className: "section-header", children: [_jsx("h4", { children: "Timeline" }), _jsx("button", { className: "add-btn", onClick: handleAddMarker, disabled: !selectedEntry, children: "+ Marker" })] }), _jsx("div", { className: "timeline-list", children: filteredTimeline.map((entry, index) => {
                            const isSelected = selectedEntry === entry.id;
                            const isCurrent = index === currentPosition;
                            const entryMarkers = markers.filter(m => m.entryId === entry.id);
                            return (_jsxs("div", { className: `timeline-entry ${isSelected ? 'selected' : ''} ${isCurrent ? 'current' : ''}`, onClick: () => {
                                    setSelectedEntry(entry.id);
                                    handleGoToEntry(entry.id);
                                }, children: [_jsxs("div", { className: "entry-header", children: [_jsx("span", { className: "entry-type", children: entry.type }), _jsx("span", { className: "entry-time", children: new Date(entry.timestamp).toLocaleTimeString() })] }), _jsxs("div", { className: "entry-info", children: [_jsx("span", { className: "entry-domain", children: entry.domain }), _jsx("span", { className: "entry-description", children: entry.metadata.description })] }), entryMarkers.length > 0 && (_jsx("div", { className: "entry-markers", children: entryMarkers.map(marker => (_jsx("span", { className: "entry-marker", style: { backgroundColor: marker.color }, title: marker.name, children: marker.type === 'bookmark' ? '🔖' : '📍' }, marker.id))) })), entry.metadata.tags.length > 0 && (_jsx("div", { className: "entry-tags", children: entry.metadata.tags.map(tag => (_jsx("span", { className: "entry-tag", children: tag }, tag))) }))] }, entry.id));
