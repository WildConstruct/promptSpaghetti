...
export class PerformanceProfiler extends EventEmitter {
    ...
    sampleOperation(domain, operation, fn, metadata) {
        if (!this.isProfileActive) {
            return fn();
        }
        const startTime = performance.now();
  ...
  return expensiveCalculation(inputs);
    ...
    checkAlerts(sample) {
        ...
        if (sample.metrics.duration > thresholds.updateLatency) {
            this.createAlert('warning', `Slow operation: ${sample.operation} took ${sample.metrics.duration.toFixed(2)}ms`, 'duration', sample.metrics.duration, sample);
        }
        // Memory alert
        if (sample.metrics.memoryAfter > thresholds.memoryUsage) {
            this.createAlert('error', `High memory usage: ${(sample.metrics.memoryAfter / 1024 / 1024).toFixed(2)}MB`, 'memory', sample.metrics.memoryAfter, sample);
        }
        // Render time alert
        if (sample.metrics.renderTime && sample.metrics.renderTime > thresholds.renderTime) {
            this.createAlert('warning', `Slow render: ${sample.metrics.renderTime.toFixed(2)}ms`, 'renderTime', sample.metrics.renderTime, sample);
        }
    }
    createAlert(level, message, metric, value, sample) {
        const alert = {
