...
export class TimeTravel extends EventEmitter {
    ...
    recordStateSnapshot(snapshot, domain, options = {}) {
        if (!this.isRecording)
            return '';
    ...
    recordStateChange(change, domain, options = {}) {
        if (!this.isRecording)
            return '';
        const entryId = this.generateEntryId();
        const entry = {
    ...
    mergeBranch(sourceBranchId, targetBranchId, options = {}) {
    ...
    createReplaySession(name, options = {}) {
    ...
    stepReplay(sessionId, direction = 'forward') {
        const session = this.replaySessions.get(sessionId);
        if (!session || !this.isReplaying)
            return false;
    ...
    // State diffing
    createStateDiff(fromEntryId, toEntryId) {
    ...
    searchTimeline(searchTerm, options = {}) {
    ...
    setupAutoSnapshot() {
        if (!this.config.autoSnapshot.enabled)
            return;
        setInterval(() => {
    ...
    getFilteredTimeline(query) {
        ...
        if (query.timeRange) {
            entries = entries.filter(entry => entry.timestamp >= query.timeRange.start &&
        ...
        if (query.branches) {
            entries = entries.filter(entry => query.branches.includes(entry.metadata.branchId || ''));
        }
        if (query.tags) {
            entries = entries.filter(entry => query.tags.some(tag => entry.metadata.tags.includes(tag)));
        }
        if (query.search) {
    ...
    async executeAutoReplay(session) {
    ...
    calculateDeepDiff(from, to, path) {
        const diffs = [];
