...
import { globalRealTimeManager } from '../realtime/RealTimeStateManager';
// Main real-time state hook
export function useRealTimeState(options = {}) {
    const [connectionState, setConnectionState] = useState(globalRealTimeManager.getConnectionState());
...
export function useStateSubscription(domain, filters = [], options = {}) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = manager.current.subscribeToStateChanges(domain, callback, filters, subscriptionOptions);
...
export function useOptimisticMutation(domain, mutationFn, options = {}) {
...
export function useDomainState(options) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = manager.current.subscribeToStateChanges(domain, handleStateChange, [], // No filters
...
export function useConflictResolution(domain) {
...
export function useBatchMutation(domain) {
    ...
    const addToBatch = useCallback((mutation) => {
    ...
    const executeBatch = useCallback(async () => {
        ...
        try {
            ...
            await Promise.all(batch.map(mutation => globalRealTimeManager.optimisticUpdate(domain, mutation)));
            setBatch([]);
        }
        catch (error) {
            throw error;
