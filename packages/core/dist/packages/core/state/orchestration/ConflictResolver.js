...
export class ConflictResolver {
    ...
    async detectAndResolveConflicts(localChanges, remoteChanges, currentState, domain) {
        // Detect conflicts
    ...
    detectConflicts(localChanges, remoteChanges, currentState, domain) {
        const conflicts = [];
    ...
    analyzeChangePair(localChange, remoteChange, currentState, domain) {
        ...
        const overlappingPaths = localPaths.filter(path => remotePaths.some(remotePath => this.pathsOverlap(path, remotePath)));
    ...
    mergeChanges(conflict) {
        ...
        const mergedPayload = this.mergePaylods(conflict.localChange.payload, conflict.remoteChange.payload);
        const mergedChange = {
            ...conflict.localChange,
    ...
    securityPriorityResolution(conflict) {
        ...
        const isSecurityRelated = (change) => JSON.stringify(change.payload).includes('permission') ||
        ...
        if (isSecurityRelated(conflict.localChange) || isSecurityRelated(conflict.remoteChange)) {
            ...
            const moreRestrictive = this.selectMoreRestrictiveChange(conflict.localChange, conflict.remoteChange);
    ...
    resolveGraphConflicts(localChanges, remoteChanges) {
    ...
    resolveSecurityConflicts(localPermissions, remotePermissions) {
        // Security conflicts always favor more restrictive permissions
    ...
    resolveDashboardConflicts(localLayout, remoteLayout) {
