...
export class StateSynchronizer extends EventEmitter {
    ...
    isServer;
    clientId;
    constructor(config = {}, isServer = false, clientId) {
        super();
        this.config = {
    ...
    async broadcastStateChange(domain, change, excludeClient) {
        const message = {
    ...
    async handleRemoteStateChange(message) {
        try {
    ...
    // Optimistic updates
    async applyOptimisticUpdate(domain, change) {
        ...
        await domainContainer.setState((prevState) => this.applyChangeToState(prevState, change), { ...change, source: 'local' });
    ...
    async confirmOptimisticUpdate(updateId) {
    ...
    async handleConflict(conflict) {
        ...
        try {
            ...
            const resolutions = await globalConflictResolver.detectAndResolveConflicts([conflict.localChange], [conflict.remoteChange], this.domains.get(conflict.domain)?.getState(), conflict.domain);
            if (resolutions.length > 0) {
                ...
                this.syncState.conflictQueue = this.syncState.conflictQueue.filter(c => c.id !== conflict.id);
    ...
    setupEventHandlers() {
        ...
        this.on('clientTimeout', (clientId) => {
