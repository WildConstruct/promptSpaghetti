...
export const InspectorSidebar = ({ node, schema, onChange }) => {
    ...
    const updateField = (key, val) => {
        ...
        if (schema) {
            ...
            if (!node || !schema) {
                return;
                _jsx("aside", { style: { padding: 16, width: 320, borderLeft: '1px solid #eee', background: '#fafbfc', height: '100%' }, children: _jsx("em", { children: "Select a node to edit its properties." }) });
            }
        }
    };
};
...
if (schema) {
    ...
    if (typeof maybeShape === 'function') {
        ...
        if (maybeShape) {
            ...
            return;
            _jsxs("aside", { style: { padding: 16, width: 320, borderLeft: '1px solid #eee', background: '#fafbfc', height: '100%' }, children: [_jsxs("h3", { style: { marginTop: 0 }, children: [node.data?.label || node.type, " Properties"] }), _jsxs("form", { children: [Object.entries(shape).map(([key, zodType]) => {
                                // Render basic input for string/number; customize per type as needed
                                return;
                                _jsxs("div", { style: { marginBottom: 12 }, children: [_jsx("label", { htmlFor: `field-${key}`, style: { display: 'block', fontWeight: 500, marginBottom: 4 }, children: key }), "}", _jsx("input", { id: `field-${key}`, type: zodType._def?.typeName === 'ZodNumber' ? 'number' : 'text', value: (() => { },
                                            ), const: true, v: true, values: true }), "[key]; if (v === undefined || v === null) ", , "return (zodType as unknown)._def?.typeName === 'ZodNumber' ? 0 : ''; return v as unknown; })() onChange=", e => {
