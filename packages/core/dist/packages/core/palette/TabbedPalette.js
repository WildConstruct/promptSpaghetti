...
{ /* Favorite button (expanded view only) */ }
{
    !collapsed && showFavorites && ()
        < button;
    onClick = {}(e);
    handleFavoriteToggle(node.id, e);
}
style = {};
{
    background: 'none',
...
title = { isFavorited, 'Remove from favorites': 'Add to favorites' }
    >
        _jsx(FiStar, { size: 12, fill: isFavorited ? 'currentColor' : 'none' });
button >
;
div >
;
...
const renderCategorySection = useCallback((category) => {
    ...
    if (categoryNodes.length === 0)
        return null;
    return;
    _jsxs("div", { style: { marginBottom: 12 }, children: [_jsxs("div", { role: "button", tabIndex: 0, onClick: () => category.collapsible && handleCategoryToggle(category.id), onKeyDown: (e) => {
                    ...
                    e.currentTarget.style.backgroundColor = 'transparent';
                        ...
                        < div, " style=", { width: 12, display: 'flex', justifyContent: 'center' }, ">", isCollapsed ? ()
                        < FiChevronRight : , " size=", 10, " color=", uiColors.text.secondary, " /> ) : ()", _jsx(FiChevronDown, { size: 10, color: uiColors.text.secondary }), ")}"] }), ")}", _jsx(CategoryIcon, { size: 12, color: category.color }), _jsx("div", { style: {
                    fontSize: 10,
                    fontWeight: 600,
                    color: uiColors.text.secondary,
    ...
    { /* Category nodes */ }
    {
        ...
        style = {};
        {
            marginLeft: category.collapsible ? 18 : 0;
        }
    }
     >
...
'thin';
 >
    { availableCategories, : .slice(0, 4).map((category) => {
            ...
            const CategoryIcon = category.icon;
            return;
            _jsx("button", { onClick: () => setActiveTab(category.id), style: {
                    display: 'flex',
                    alignItems: 'center',
                    gap: 4,
                    padding: '6px 8px',
...
_jsxs("div", { style: {
        ...
        , "// Collapsed view - show all nodes as icons nodes.map(node => renderNodeItem(node)) ) : () // Expanded view - show categories availableCategories .filter(category => activeTab === category.id || availableCategories.length === 1) .map(category => renderCategorySection(category)) )}"] });
aside >
;
;
;
