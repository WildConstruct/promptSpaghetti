...
EventMiddleware => {
    const { logLevel = 'info', includeMetadata = false, filterCategories };
    filterPriorities
        = options || {};
    return (event, next) => {
        // Apply filters
...
EventMiddleware => {
    const { strictMode = false, requiredFields = ['type', 'timestamp', 'id', 'source'] };
    customValidators = []
        = options || {};
    return (event, next) => {
        const errors = [];
                        ...
                        if (typeof event.source !== 'string' || event.source.trim() === '') {
                            ...
                            if (event.userId && typeof event.userId !== 'string') {
                                ...
                                if (event.sessionId && typeof event.sessionId !== 'string') {
                                    ...
                                    EventMiddleware => {
                                        const { maxEventsPerSecond = 100, maxEventsPerMinute = 1000, maxEventsPerHour = 10000, strategy = 'error' };
                                    ...
                                    return (event, next) => {
                                        ...
                                        EventMiddleware => {
                                            ...
                                            EventMiddleware => {
                                                const { sensitiveFields = ['password', 'token', 'secret', 'key', 'ssn', 'creditCard'], maskPattern = '***', logSensitiveAccess = true };
                                                allowedSources = []
                                                    = options || {};
                                                            ...
                                                            return (event, next) => {
                                                                ...
                                                                if (now - windowStart > monitorWindowMs) {
                                                                    ...
                                                                    if (state === 'open' && now - lastFailureTime > resetTimeoutMs) {
                                                                        ...
                                                                        catch (error) { // Failure - back to open
                                                                            state = 'open';
                                                                            lastFailureTime = now;
                                                                            throw error;
                                                                            break;
