        ...
        analysis.nodes.forEach((genNode, idx) => {
            ...
            switch (data.type) {
                case 'TextBlock':
                    ...
                    break;
                case 'WeightedChoice':
                    content = data.data.options.map((opt) => opt.text).join(' | ');
                    break;
                case 'Output':
                    content = '(output)';
                    break;
...
const distBefore = Math.sqrt(Math.pow(overlappingPositions[1].x - overlappingPositions[0].x, 2) +
    Math.pow(overlappingPositions[1].y - overlappingPositions[0].y, 2));
const distAfter = Math.sqrt(Math.pow(optimized[1].x - optimized[0].x, 2) +
