 ...
 */
import { TextBlockNode, WeightedChoiceNode, ConcatNode, VariableNode, OutputNode, GraphBuilder, Epic1ExecutionEngine, generatePreview, formatExecutionResult } from '../index';
async function runExample() {
    ...
    const nameVar = new VariableNode('nameVar', { name: 'userName', defaultValue: 'Friend' }, { mode: 'set', variableType: 'string' });
    // Greeting variations
    ...
    const customNameVar = new VariableNode('customNameVar', { name: 'userName', defaultValue: 'Alice' }, { mode: 'set' });
    const customBuilder = new GraphBuilder();
    const customGreeting = new TextBlockNode('customGreeting', 'Welcome back, {{userName}}! Your last visit was {{lastVisit}}.');
    const lastVisitVar = new VariableNode('lastVisitVar', { name: 'lastVisit', defaultValue: 'yesterday' }, { mode: 'set' });
