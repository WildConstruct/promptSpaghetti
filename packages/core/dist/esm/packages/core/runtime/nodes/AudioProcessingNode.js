...
export class TextToSpeechNode extends AdvancedRuntimeNode {
    ...
    constructor(nodeId, config) {
        ...
        Promise < NodeExecutionResult > {
            ...
            async _initializeAdapter(config) {
                try {
                    ...
                    switch (config.provider) {
                        case 'openai-tts':
                            adapter = new OpenAITTSAdapter() `tts-${this.nodeId}`;
                    }
                    {
            ...
            case: 'elevenlabs',
            adapter = new ElevenLabsAdapter() `elevenlabs-${this.nodeId}`
        };
        {
            apiKey: config.apiKey || '';
        }
...
new Error(`Unsupported TTS provider: ${config.provider}`);
...
string;
{
    ...
    _buildSynthesisOptions(provider, string, params, unknown);
    unknown;
    {
        ...
        return params;
        _mapToElevenLabsFormat(format, string);
        string;
        {
            const formatMap = {
                'mp3': 'mp3_44100_128',
                    ...
                    Promise < NodeExecutionResult > {
                        ...
                        async _initializeAdapter(config) {
                            try {
                                ...
                                switch (config.provider) {
                                    case 'whisper':
                                        adapter = new WhisperAdapter() `whisper-${this.nodeId}`;
                                }
                                {
                                    apiKey: config.apiKey || '';
                                }
  ...
  adapter:`, error);
                ...
                _buildTranscriptionOptions(provider, params) {
            ...
            export class AudioAnalysisNode extends AdvancedRuntimeNode {
                constructor(nodeId, config = {}) {
                    ...
                    Promise < NodeExecutionResult > { try: {
                            ...
                            async _analyzeAudio(audioFile, analysisType) {
                            ...
                            async _convertAudio(audioFile, options) {
