...
export class SmartNodePositioner {
     ...
     */
    calculatePositions(nodes, config) {
     ...
     */
    layoutHorizontal(nodes, groups, config) {
        const positions = [];
        let currentX = config.baseX;
        let currentY = config.baseY;
        let rowHeight = 0;
        for (const group of groups) {
            const groupPositions = this.layoutGroup(group, nodes, currentX, currentY, config, 'horizontal');
     ...
     */
    layoutVertical(nodes, groups, config) {
        const positions = [];
        let currentX = config.baseX;
        let currentY = config.baseY;
        for (const group of groups) {
            const groupPositions = this.layoutGroup(group, nodes, currentX, currentY, config, 'vertical');
            // Update positions array
            for (let i = 0; i < group.nodes.length; i++) {
                positions[group.nodes[i]] = groupPositions[i];
            }
     ...
     */
    layoutDiagonal(nodes, groups, config) {
        const positions = [];
        let currentX = config.baseX;
        let currentY = config.baseY;
        ...
        for (let g = 0; g < groups.length; g++) {
            ...
            const groupPositions = this.layoutGroup(group, nodes, currentX, currentY, config, 'diagonal');
            // Update positions array
            for (let i = 0; i < group.nodes.length; i++) {
                positions[group.nodes[i]] = groupPositions[i];
            }
     ...
     */
    layoutGroup(group, nodes, startX, startY, config, direction) {
        const positions = [];
        let currentX = startX;
        let currentY = startY;
        for (let i = 0; i < group.nodes.length; i++) {
     ...
     */
    calculateGroupWidth(group, nodes) {
        let totalWidth = 0;
     ...
     */
    calculateGroupHeight(group, nodes) {
        let totalHeight = 0;
        for (const nodeIndex of group.nodes) {
            const node = nodes[nodeIndex];
            const dimensions = this.nodeDimensions[node.node.getType()];
     ...
     */
    optimizePositions(positions, nodes, iterations = 10) {
     ...
     */
    doNodesOverlap(pos1, dim1, pos2, dim2) {
        const buffer = 20; // Small buffer between nodes
        return !(pos1.x + dim1.width + buffer < pos2.x ||
     ...
     */
    calculateOverlap(pos1, dim1, pos2, dim2) {
        const horizontalOverlap = Math.min(pos1.x + dim1.width - pos2.x, pos2.x + dim2.width - pos1.x);
        const verticalOverlap = Math.min(pos1.y + dim1.height - pos2.y, pos2.y + dim2.height - pos1.y);
        return {
