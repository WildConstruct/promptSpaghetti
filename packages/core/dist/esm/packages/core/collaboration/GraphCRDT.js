...
import { NodeUpdateOperation, NodeRemoveOperation, EdgeAddOperation, EdgeUpdateOperation, EdgeRemoveOperation, ParameterUpdateOperation, GraphEdge } from './GraphMutations';
...
;
handleEdgeChanges(event, (Y.YMapEvent));
void {
    event, : .changes.keys.forEach((change, key) => {
        if (change.action === 'add') {
...
;
handleOperationChanges(event, (Y.YArrayEvent));
void {
    ...
    addNode(operation) {
        try {
            ...
            try { }
                 ...
                 */
                updateNode(operation, NodeUpdateOperation);
                boolean;
                {
                    try {
                        ...
                        try { }
                             ...
                             */
                            removeNode(operation, NodeRemoveOperation);
                            boolean;
                            {
                                try {
                                    const existingNode = this.nodes.get(operation.nodeId);
                                     ...
                                     */
                                    addEdge(operation, EdgeAddOperation);
                                    boolean;
                                    {
                                        try {
                                            ...
                                            try { }
                                                 ...
                                                 */
                                                updateEdge(operation, EdgeUpdateOperation);
                                                boolean;
                                                {
                                                    try {
                                                         ...
                                                         */
                                                        removeEdge(operation, EdgeRemoveOperation);
                                                        boolean;
                                                        {
                                                            try {
                                                                const existingEdge = this.edges.get(operation.edgeId);
                                                                 ...
                                                                 */
                                                                updateParameter(operation, ParameterUpdateOperation);
                                                                boolean;
                                                                {
                                                                    try {
                                                                        const existingNode = this.nodes.get(operation.nodeId);
