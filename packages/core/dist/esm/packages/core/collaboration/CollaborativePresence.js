...
const NodeSelectionOverlay = ({ nodeId, users, nodePosition, nodeWidth, nodeHeight }) => {
  ...
  return _jsx('div', {
    style: overlayStyle,
    children: _jsx('div', { style: labelStyle, children: isMultiple ? `${users.length} users` : userNames }),
  });
};
/**
...
export const CollaborativePresence = ({ userCursors, remoteSelections, className }) => {
  ...
  return _jsxs('div', {
    ...
    children: [
      userCursors.map(({ userId, user, position }) => _jsx(UserCursor, { user: user, position: position }, userId)),
...
export const CollaborationStatus = ({ isCollaborative, connectionStatus, connectedUserCount, className }) => {
  ...
  const getStatusText = () => {
    switch (connectionStatus) {
      case 'connected':
        return `Connected • ${connectedUserCount} user${connectedUserCount !== 1 ? 's' : ''}`;
      case 'connecting':
        return 'Connecting...';
      case 'disconnected':
        return 'Disconnected';
  ...
  return _jsxs('div', {
    className: className,
    style: statusStyle,
    children: [_jsx('div', { style: dotStyle }), _jsx('span', { children: getStatusText() })],
  });
};
/**
...
export const UserAvatars = ({ connectedUsers, localUserId, maxVisible = 5, className }) => {
  ...
  return _jsxs('div', {
    ...
    children: [
      users.map(user =>
        _jsx('div', { style: avatarStyle(user.color), title: user.name, children: getInitials(user.name) }, user.userId)
      ),
      extraCount > 0 &&
        _jsxs('div', {
