...
sentimentOverTime: z.array(z.object({}), timestamp, z.date(), positive, z.number().min(0).max(100), neutral, z.number().min(0).max(100), negative, z.number().min(0).max(100), averageScore, z.number().min(-1).max(1));
([]);
emotionTrends: z.array(z.object({}), timestamp, z.date(), dominantEmotion, z.enum(['joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'trust', 'anticipation']).optional(), emotionScores, z.record(z.number().min(0).max(1)).default({}));
([]);
...
emergingTopics: z.array(z.object({}), topic, z.string(), sentiment, z.enum(['positive', 'neutral', 'negative']), growth, z.number() // Percentage growth }
);
([]);
...
export const validateAnalyzeSentimentRequest = (data) => { return AnalyzeSentimentRequestSchema.parse(data); };
export const validateSentimentAnalysis = (data) => { return SentimentAnalysisSchema.parse(data); };
export const validateSentimentAnalytics = (data) => { return SentimentAnalyticsSchema.parse(data); };
;
