...
z.record(z.unknown()).default({}),
    // Job status
    status;
ExportJobStatusSchema.default('pending'),
    progress_percentage;
...
z.string().min(1).max(255),
    access_level;
ShareAccessLevelSchema.default('public'),
    password;
z.string().optional(),
    description;
z.string().optional(),
    // Access control
    max_downloads;
z.number().int().min(1).optional(), // null for unlimited,
    download_count;
z.number().int().min(0).default(0),
    allowed_ips;
...
include_toc: z.boolean().default(true),
    format;
z.enum(['github', 'commonmark']).default('github'),
    heading_level;
...
page_size: z.enum(['A4', 'A3', 'Letter', 'Legal']).default('A4'),
    orientation;
z.enum(['portrait', 'landscape']).default('portrait'),
    include_images;
z.boolean().default(true),
    font_size;
...
z.boolean().default(true),
    ...
    enable_controlnet_support;
z.boolean().default(true),
    // Animation support
    enable_animation_framework;
z.boolean().default(true),
    // Rendering parameters
    include_rendering_data;
z.boolean().default(true),
    include_camera_data;
z.boolean().default(true),
    include_lighting_data;
z.boolean().default(true),
    // Wild Construct ecosystem integration
    include_ecosystem_data;
z.boolean().default(false),
    // Format compatibility
    format_version;
z.string().default('1.2.0'),
    backwards_compatible;
z.boolean().default(true),
    // Reproducibility options
    include_reproducibility_data;
z.boolean().default(true),
    exact_reproduction;
z.boolean().default(true),
...
z.number().int().min(0),
    exports_by_format;
z.record(ExportFormatSchema, z.number().int().min(0)),
    exports_by_type;
z.record(ExportTypeSchema, z.number().int().min(0)),
    total_size;
z.number().int().min(0),
    average_processing_time;
z.number().min(0),
    most_used_templates;
z.array(z.object({}), template_id, z.string().uuid(), template_name, z.string(), usage_count, z.number().int().min(0));
