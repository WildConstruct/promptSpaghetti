...
FeedbackTypeSchema,
    category;
FeedbackCategorySchema.default('general'),
    // Target Information
    targetType;
z.enum(['contribution', 'template', 'user', 'platform']),
    targetId;
z.string().uuid(),
    // Author Information
    authorId;
...
z.string().min(1).max(5000),
    attachments;
z.array(z.object({}), id, z.string(), type, z.enum(['image', 'video', 'document', 'screenshot']), url, z.string().url(), filename, z.string(), size, z.number().positive(), mimeType, z.string());
([]),
...
z.number().min(1).max(5).optional(),
    // Metadata
    status;
FeedbackStatusSchema.default('pending'),
    visibility;
z.enum(['public', 'private', 'moderated']).default('public'),
    // Engagement
    helpfulVotes;
...
z.number().min(1).max(5),
    ...
    pros;
z.array(z.string().max(500)).default([]),
    cons;
z.array(z.string().max(500)).default([]),
    useCase;
...
z.enum(['low', 'medium', 'high', 'critical']).default('medium'),
    evidence;
z.array(z.object({}), type, z.enum(['screenshot', 'url', 'text', 'video']), content, z.string(), description, z.string().optional());
([]),
...
z.enum(['pending', 'in_progress', 'completed', 'rejected']).default('pending'),
    implementedBy;
z.string().uuid().optional(),
    implementedAt;
z.date().optional(),
...
z.string(),
    authorType;
z.enum(['user', 'creator', 'moderator', 'admin']),
    // Content
    content;
z.string().min(1).max(2000),
    attachments;
z.array(z.object({}), id, z.string(), type, z.enum(['image', 'document']), url, z.string().url(), filename, z.string());
([]),
    // Status
    status;
z.enum(['visible', 'hidden', 'deleted']).default('visible'),
    // Engagement
    likes;
z.number().int().default(0),
    // Timestamps
...
targetId: z.string().uuid().optional(),
    targetType;
z.enum(['contribution', 'template', 'user', 'platform']).optional(),
    type;
FeedbackTypeSchema.optional(),
    category;
FeedbackCategorySchema.optional(),
    status;
FeedbackStatusSchema.optional(),
    authorId;
z.string().uuid().optional(),
    verifiedOnly;
z.boolean().optional(),
...
z.string().optional(),
    sortBy;
z.enum(['created_at', 'rating', 'helpful_votes', 'updated_at']).default('created_at'),
    sortOrder;
