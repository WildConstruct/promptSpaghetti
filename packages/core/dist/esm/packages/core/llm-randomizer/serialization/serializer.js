...
if (includeChecksum) {
    ...
    serialized = serialized.replace() `version: ${this.FORMAT_VERSION}`;
}
`version: ${this.FORMAT_VERSION}\nchecksum: ${checksum}`;
;
return serialized;
serializeNode(node, Node, compact, boolean = false);
string;
{
    const lines = [];
