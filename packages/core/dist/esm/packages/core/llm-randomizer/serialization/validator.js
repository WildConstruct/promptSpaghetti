...
ParsedGraph;
{
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmedLine === '---NODES---') {
                    ...
                    if (currentNode) {
                                            ...
                                            if (currentNode) {
                                                if (Object.keys(currentProps).length > 0) {
                                                    ...
                                                    if (currentNode && rawLine.startsWith('  ')) {
                                                            ...
                                                            if (currentSection === 'edges') {
                                                                if (trimmedLine.includes(' -> ')) {
                                                                                ...
                                                                                try {
                                                                                    ...
                                                                                    if ((value.startsWith('"') && value.endsWith('"')) ||
                                                                                        ...
                                                                                        if (/^\d+\.?\d*$/.test(value)) {
                                                                                            ...
                                                                                            any;
                                                                                            {
                                                                                                if (!value.startsWith('[') || !value.endsWith(']')) {
                                                                                                    ...
                                                                                                    validateStructure(parsed, ParsedGraph, result, ValidationResult);
                                                                                                    void {
                                                                                                        // Check version
                                                                                                        if(, parsed) { }, : .version };
                                                                                                    {
                                                                                                    ...
                                                                                                    type: 'schema',
                                                                                                        message;
                                                                                                    `Unsupported version: ${parsed.version}. Supported versions: ${this.SUPPORTED_VERSIONS.join(', ')}`;
                                                                                                }
                                                                                            }
                                                                                            severity: 'error';
                                                                                        }
                                                                                        ...
                                                                                        if (!node.type) {
                                                                                            ...
                                                                                            type: 'schema',
                                                                                                message;
                                                                                            `Node ${node.id} missing required type field`;
                                                                                        }
                                                                                    }
                                                                                    nodeId: node.id,
                                                                                        severity;
                                                                    ...
                                                                    if (node.id && !/^[a-zA-Z0-9_-]+$/.test(node.id)) {
                                                                        ...
                                                                        type: 'schema',
                                                                            message;
                                                                        `Node ID ${node.id} contains invalid characters. Use only alphanumeric, underscore, and hyphen.`;
                                                                    }
                                                                }
                                                                nodeId: node.id,
                                                                    severity;
                                                        ...
                                                        ;
                                                        validateSemantics(parsed, ParsedGraph, result, ValidationResult);
                                                        void {
                        ...
                        if (node.inputs) {
                            ...
                            if (!nodeIds.has(inputId)) {
                                ...
                                type: 'semantic',
                                    message;
                                `Node ${node.id} references non-existent input: ${inputId}`;
                            }
                        }
                        nodeId: node.id,
                            severity;
            ...
            detectCycles(parsed, ParsedGraph, result, ValidationResult);
            void {
                ...
                const: dfs = (nodeId, path) => {
                    if (recursionStack.has(nodeId)) {
                        ...
                        type: 'semantic',
                            message;
                        `Cycle detected: ${path.join(' -> ')} -> ${nodeId}`;
                    }
                },
                nodeId: nodeId,
                severity: 'error'
                    ...
                    for (const inputId of node.inputs) {
                        ...
                        for (const node of parsed.nodes) {
                            if (!visited.has(node.id)) {
                                dfs(node.id, []);
                                validateNodeProperties(parsed, ParsedGraph, result, ValidationResult);
                                void {
                                    parsed, : .nodes.forEach(node => { }),
                                    switch(node) { }, : .type
                                };
                                {
                                    ...
                                    if (!node.props?.choices || !Array.isArray(node.props.choices)) {
                                        ...
                                        type: 'schema',
                                            message;
                                        `${node.type} node ${node.id} missing required choices array`;
                                    }
                                }
                                nodeId: node.id,
                                    severity;
                        ...
                        }
                        {
                            node.props.choices.forEach((choice, index) => {
                                if (typeof choice.value !== 'string') {
                                    ...
                                    type: 'schema',
                                        message;
                                    `${node.type} node ${node.id} choice ${index} missing value`;
                                }
                        ...
                        if (typeof choice.weight !== 'number' || choice.weight < 0) {
                            ...
                            type: 'schema',
                                message;
                            `${node.type} node ${node.id} choice ${index} has invalid weight`;
                        }
                    }
                    nodeId: node.id,
                        severity;
            ...
            if (!node.props?.key) {
                ...
                type: 'schema',
                    message;
                `${node.type} node ${node.id} missing required key property`;
            }
        }
        nodeId: node.id,
            severity;
    ...
    if (!node.props?.name) {
        ...
        type: 'schema',
            message;
        `Include node ${node.id} missing required name property`;
    }
}
nodeId: node.id,
    severity;
...
break;
;
checkOptimizations(parsed, ParsedGraph, result, ValidationResult);
void {
...
if (!connectedNodes.has(node.id) && (!node.inputs || node.inputs.length === 0)) {
    ...
    type: 'optimization',
        message;
    `Node ${node.id} appears to be isolated`;
}
...
if (!hasOutputNode) {
    ...
    'Graph has no Output nodes',
        suggestion;
    'Add at least one Output node to generate results',
    ;
}
;
// Check for unreachable nodes
this.checkReachability(parsed, result);
checkReachability(parsed, ParsedGraph, result, ValidationResult);
void {
    const: nodeMap = new Map(parsed.nodes.map(n => [n.id, n])),
...
};
{
    ...
    type: 'optimization',
        message;
    `Node ${node.id} is unreachable from root nodes`;
}
