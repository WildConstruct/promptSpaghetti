    ...
    try { }
        ...
        _jsxs("div", { className: `randomizer-panel ${className}`, children: ["}", _jsxs("div", { className: "randomizer-header", children: [_jsx("h2", { children: "LLM Graph Randomizer" }), _jsxs("div", { className: "header-controls", children: [_jsxs("button", { onClick: () => setShowPresets(!showPresets), className: `preset-btn ${showPresets ? 'active' : ''}`, children: ["Presets (", presets.length, ")"] }), _jsxs("button", { onClick: () => setShowHistory(!showHistory), className: `history-btn ${showHistory ? 'active' : ''}`, children: ["History (", history.length, ")"] }), _jsx("button", { onClick: () => setShowAdvanced(!showAdvanced), className: `advanced-btn ${showAdvanced ? 'active' : ''}`, children: "Advanced" })] })] }), showPresets && ()
                    ...
                    (_jsx("h4", { children: category })
                        ,
                            _jsxs("div", { className: "preset-grid", children: [categoryPresets.map(preset => ()
                                        < div, key = { preset, : .id }, className = {} `preset-card ${selectedPreset === preset.id ? 'selected' : ''}`), "onClick=", () => selectPreset(preset.id), ">", _jsx("div", { className: "preset-name", children: preset.name }), _jsx("div", { className: "preset-description", children: preset.description }), _jsx("div", { className: "preset-tags", children: preset.tags.map(tag => ()
                                            < span, key = { tag }, className = "preset-tag" > { tag }) }), "))}"] })))] });
    }
    div >
    ;
    div >
...
{ /* History Panel */ }
{
    ...
    className = "history-panel" >
        (_jsx("h3", { children: "Generation History" })
            ,
                _jsxs("div", { className: "history-stats", children: [_jsxs("div", { className: "stat", children: [_jsx("span", { className: "stat-label", children: "Total:" }), _jsx("span", { className: "stat-value", children: historyStats.totalGenerations })] }), _jsxs("div", { className: "stat", children: [_jsx("span", { className: "stat-label", children: "Success Rate:" }), _jsxs("span", { className: "stat-value", children: [(historyStats.successRate * 100).toFixed(1), "%"] })] }), _jsxs("div", { className: "stat", children: [_jsx("span", { className: "stat-label", children: "Avg Time:" }), _jsxs("span", { className: "stat-value", children: [historyStats.averageGenerationTime.toFixed(0), "ms"] })] })] })
                    ,
                        _jsxs("div", { className: "history-list", children: [history.slice(0, 10).map(entry => ()
                                    < div, key = { entry, : .id }, className = {} `history-entry ${entry.success ? 'success' : 'failed'}`), "onClick=", () => setParameters(entry.parameters), ">", _jsxs("div", { className: "history-purpose", children: [entry.parameters.purpose.substring(0, 60), "..."] }), _jsxs("div", { className: "history-meta", children: [_jsx("span", { children: entry.parameters.complexity }), _jsxs("span", { children: [entry.parameters.nodeCount, " nodes"] }), _jsx("span", { children: new Date(entry.timestamp).toLocaleDateString() })] })] }));
}
div >
;
div >
...
{ /* Validation Messages */ }
{
    ...
    className = "validation-errors" >
        (_jsx("h4", { children: "Errors:" })
            ,
                _jsx("ul", { children: validation.errors.map((error, index) => ()
                        < li, key = { index }, className = "error-item" >
                        _jsxs("strong", { children: [error.field, ":"] }), { error, : .message }) }));
}
ul >
;
div >
;
{
    ...
    className = "validation-warnings" >
        (_jsx("h4", { children: "Warnings:" })
            ,
                _jsx("ul", { children: validation.warnings.map((warning, index) => ()
                        < li, key = { index }, className = "warning-item" >
                        _jsxs("strong", { children: [warning.field, ":"] }), { warning, : .message }, { warning, : .suggestion && _jsxs("em", { children: [" \u2014 ", warning.suggestion] }) }) }));
}
ul >
;
div >
...
className = "generation-controls" >
    _jsx("button", { onClick: handleGenerate, disabled: !validation.isValid || isGenerating || !parameters.purpose, className: "generate-btn primary", children: isGenerating ? 'Generating...' : 'Generate Graph' });
{
    generationProgress && ()
        < div;
