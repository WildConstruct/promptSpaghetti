...
const handleTabChange = (tabId) => {
    ...
    if (tabId === 'dependencies' && !state.dependencyGraph) {
        ...
        const handleRecordingToggle = () => {
            ...
            return;
            _jsx("div", { className: panelClasses, children: _jsxs("div", { className: "devtools-header", children: [_jsxs("div", { className: "devtools-title", children: [_jsx("span", { className: "devtools-logo", children: "\uD83D\uDEE0\uFE0F" }), _jsx("h3", { children: "State DevTools" })] }), _jsxs("div", { className: "devtools-controls", children: [_jsx("button", { className: `devtools-btn ${state.isRecording ? 'recording' : ''}`, onClick: handleRecordingToggle, title: state.isRecording ? 'Stop Recording' : 'Start Recording', children: state.isRecording ? '⏸️' : '⏺️' }), _jsx("button", { className: "devtools-btn", onClick: handleClearHistory, title: "Clear History", children: "\uD83D\uDDD1\uFE0F" }), onClose && ()
                                    < button, "className=\"devtools-btn\" onClick=", onClose, "title=\"Close DevTools\" > \u2715"] }), ")}"] }) })
                ,
                    _jsxs("div", { className: "devtools-tabs", children: [TABS.map(tab => ()
                                < button, key = { tab, : .id }, className = {} `devtools-tab ${state.activeTab === tab.id ? 'active' : ''}`), "onClick=", () => handleTabChange(tab.id), ">", _jsx("span", { className: "devtools-tab-icon", children: tab.icon }), _jsx("span", { className: "devtools-tab-label", children: tab.label }), tab.id === 'performance' && state.performanceAlerts.length > 0 && ()
                                < span, " className=\"devtools-badge\">", state.performanceAlerts.length] });
        };
    }
};
button >
;
div >
    (_jsxs("div", { className: "devtools-content", children: [state.activeTab === 'inspector' && ()
                < StateInspectorPanel, "devTools=", devTools, "selectedDomain=", state.selectedDomain, "onDomainChange=", handleDomainChange, "/> )}", state.activeTab === 'timetravel' && ()
                < TimeTravelPanel, "timeTravel=", timeTravel, "timeTravelState=", state.timeTravelState, "selectedDomain=", state.selectedDomain, "onDomainChange=", handleDomainChange, "/> )}", state.activeTab === 'performance' && ()
                < PerformancePanel, "performanceProfiler=", performanceProfiler, "alerts=", state.performanceAlerts, "report=", state.performanceReport, "onGenerateReport=", generatePerformanceReport, "/> )}", state.activeTab === 'dependencies' && ()
                < DependencyGraphPanel, "dependencyGraph=", state.dependencyGraph, "onGenerateGraph=", generateDependencyGraph, "selectedDomain=", state.selectedDomain, "onDomainChange=", handleDomainChange, "/> )}", state.activeTab === 'settings' && ()
                < DevToolsSettingsPanel, "devTools=", devTools, "timeTravel=", timeTravel, "performanceProfiler=", performanceProfiler, "/> )}"] })
        ,
