    ...
    constructor(message, cause) {
        ...
        export class ConnectionError extends Error {
            ...
            constructor(message, code) {
                ...
                Promise < void  > { : .connectionState.status === 'connected' || this.connectionState.status === 'connecting' };
                {
                    return;
...
;
async;
handleOptimisticConfirm(message, any);
Promise < void  > {
...
notifySubscribers(domain, string);
change: StateChange;
...
};
{
    if (subscription.domain !== domain && subscription.domain !== '*') {
        ...
        if (!this.matchesFilters(change, subscription.filters)) {
            ...
            if (metadata.optimistic && !subscription.options.includeOptimistic) {
                ...
                try {
                    ...
                    matchesFilters(change, (StateChange), filters, SubscriptionFilter);
                    boolean;
                    {
                        if (filters.length === 0)
                            return true;
                    ...
                    // Optimistic updates
                    async;
                    optimisticUpdate(domain, string, mutation, StateMutation);
                    Promise < string > { : .config.enableOptimistic };
                    {
                ...
                try {
                    ...
                    throw new OptimisticUpdateError() `Failed to apply optimistic update: ${error.message}`;
                }
                updateId;
                ;
                async;
                syncWithServer(mutation, StateMutation);
                Promise < void  > { : .isConnected() };
                {
        ...
        void { this: .domains.delete(domainName),
    ...
    Promise < void  > { : .isConnected() || !this.connection?.isReady };
    {
        ...
        try {
            ...
            try { }
                ...
                Promise < void  > {
                    ...
                    async sendPing() {
                        ...
                        payload: {
                            timestamp: this.connection.lastPing;
                        }
                    },
                ...
                void { const: queue = [...this.messageQueue],
                    this: .messageQueue = [],
            ...
            ;
            startHeartbeat();
            void { : .config.enableHeartbeat, return: ,
            ...
            void { : .pingTimer };
            {
                ...
                void { this: .synchronizer.on('broadcastMessage', (message) => {
                        this.sendMessage({});
                        type: 'state_change';
                        payload: message;
                    })
