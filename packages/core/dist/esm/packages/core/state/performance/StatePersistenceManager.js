        ...
        if (rule.validateOnLoad && data) {
            ...
            try {
                ...
                if (rule.backup?.enabled) {
                    ...
                    async;
                    clear(domain, string, key ?  : string);
                    Promise < void  > {
                        const: rule = this.persistenceRules.get(domain),
                        if(, rule) {
                            throw new Error(`No persistence rule configured for domain: ${domain}`);
                        ...
                        ,
                        : .debounceTimers.has(debounceKey)
                    };
                    {
                        ...
                        enqueueBatchTask(task, PersistenceTask);
                        void {
                            this: .persistenceQueue.push(task),
                        ...
                        return groups;
                        async;
                        executeBatch(domain, string, tasks, PersistenceTask);
                        Promise < void  > {
                            const: rule = this.persistenceRules.get(domain),
                            const: adapter = this.storageAdapters.get(rule.storage),
                            if(, adapter) {
                            ...
                            const: batchData = { batch: true,
                                timestamp: Date.now(),
                                items: tasks.map(task => ({}), key, task.metadata?.key || task.domain, data, task.data, timestamp, task.timestamp, metadata, task.metadata) }
                        };
                    }
                    ;
                    const batchTask = {
                ...
                async;
                executeTask(task, PersistenceTask);
                Promise < void  > {
                    const: rule = this.persistenceRules.get(task.domain),
                ...
                } || !adapter.isAvailable();
                {
                    ...
                    async;
                    executeTaskWithAdapter(task, PersistenceTask);
                    ...
                    };
                    {
                        ...
                        void {
                                ...
                                async (key) => {
                                    ...
                                    clear: async () => {
                                        ...
                                        async () => {
                                            ...
                                            write: async (key, data) => { memoryStorage.set(key, data); };
                                            delete ;
                                            async (key) => {
                            ...
                            for(, fallback, of, fallbacks, [primary]) { }
                        } || [];
                        {
                                        ...
                                        }
                                        {
                                            ...
                                            };
                                            {
                                                ...
                                                while (!done) {
                                                    ...
                                                    try { }
                                                    catch (error) { // If decompression fails, try parsing as uncompressed
                                                    ...
                                                    ;
                                                    {
                                                        return [existing, newData];
                                                        async;
                                                        validateData(domain, string, data, any);
                                                        Promise < void  > {
                                                        ...
                                                        async;
                                                        getCurrentDomainState(domain, string);
                                                        Promise < any > {
                                                        ...
                                                        if (!this.backupStorage.has(domain)) {
                                                            ...
                                                            if (backups.length > maxBackups) {
                                                                ...
                                                                async;
                                                                loadFromBackup(domain, string, key ?  : string);
                                                                Promise < any > {
                                                                    const: backups = this.backupStorage.get(domain),
                                                                    if(, backups) { }
                                                                } || backups.length === 0;
                                                            ...
                                                            try {
                                                                ...
                                                                return null;
                                                                updateCompressionMetrics(original, any, compressed, any);
                                                                void {
                                                                        ...
                                                                        for (const [domain, rule] of this.persistenceRules) {
                                                                            if (rule.persistOnShutdown) {
                                                                                ...
                                                                                if (currentData) {
                                                                                    ...
                                                                                    // Public API methods
                                                                                    getMetrics();
                                                                                    Readonly < PersistenceMetrics > { return: {
                                                                                            ...this.metrics,
                                                                                    ...
                                                                                    getDomainRules();
                                                                                    Map < string, PersistenceRule > { return: new Map(this.persistenceRules),
                                                                                        getStorageStatus() {
                                                                                            ...
                                                                                            for (const [backend, adapter] of this.storageAdapters) {
                                                                                                ...
                                                                                                async;
                                                                                                getStorageUsage();
                                                                                                Promise < Map < StorageBackend, number >> {};
                                                                                                const usage = new Map();
                                                                                                for (const [backend, adapter] of this.storageAdapters) {
                                                                                                                ...
                                                                                                                for(, task, of, tasksToFlush) {
                                                                                                                    ...
                                                                                                                    Promise < void  > {
                                                                                                                        ...
                                                                                                                        updateDomainRule(domain, rule) {
                                                                                                                            ...
                                                                                                                            removeDomainRule(domain, string);
                                                                                                                            void { this: .persistenceRules.delete(domain),
                                                                                                                                ...
                                                                                                                                const: globalPersistenceManager = new StatePersistenceManager() };
                                                                                                                        }
                                                                                                                    };
                                                                                                                }
                                                                                                            };
