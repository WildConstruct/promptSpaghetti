...
Promise < { restoreId: string, result: (Promise) } > { try: {
...
try {
        ...
        Promise < { resolved: RestoreConflict, unresolved: RestoreConflict, warnings: string } > { const: resolved, RestoreConflict = [],
            ...
            async mergeNodeValues(conflict) {
                ...
                position: current.position, // Keep current position
                    data;
                { }
            }, ...restore.data, ...current.data }, // Merge data with current taking precedence
            style;
        {
            restore.style, ;
            current.style;
    ...
    async;
    mergeEdgeValues(conflict, RestoreConflict);
    Promise < void  > {
        const: current = conflict.current_value,
        const: restore = conflict.restore_value,
        const: merged = {
    ...
    async;
    mergePropertyValues(conflict, RestoreConflict);
    Promise < void  > {
    ...
    };
    {
        ...
        calculateFullRestore(currentData, any, snapshotData, any);
        any;
        {
            return {
                full_replace: true,
        ...
        ;
        calculateSelectiveRestore(currentData, any, snapshotData, any, options, RestoreOptions);
        any;
        ...
        ;
        calculateMergeRestore(currentData, any, snapshotData, any, resolvedConflicts, any);
        any;
        ...
        ;
        async;
        applyChanges(changesToApply, unknown, options, RestoreOptions);
        Promise < unknown > {
    ...
    Promise < void  > { if(options) { }, : .restore_workflow_state };
    {
