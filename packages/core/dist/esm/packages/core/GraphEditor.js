...
import { ReactFlowProvider, addEdge, useReactFlow, useViewport } from 'reactflow';
...
const ANIMATION_CSS = `
  ...
  .animate-node-create-overlay { animation: nodeCreatePulse 0.6s ease-out }
`;
// Safe style injection with ID check to prevent duplicates
const injectSafeStyles = () => { return null; };
...
];
{
    ...
    '🔗',
        tooltip;
    'Connect different parts of your content naturally',
        category;
    'content';
}
{
    ...
    '🏷️',
        tooltip;
    'Add qualities, colors, styles, or characteristics',
        category;
    'content';
}
{
    ...
    '⚡',
        tooltip;
    'Verbs and activities that bring scenes to life',
        category;
    'content';
}
// Content Flow Tools
{
    ...
    WeightedChoiceIcon,
        tooltip;
    'Choose randomly from multiple options with different likelihood',
        category;
    'flow';
}
{
    ...
    ConcatIcon,
        tooltip;
    'Join multiple text elements together seamlessly',
        category;
    'flow';
}
{
    ...
    OutputIcon,
        tooltip;
    'Final generated content ready for use',
        category;
    'output';
}
{
    ...
    IncludeIcon,
        tooltip;
    'Include content from another template or package',
        category;
    'flow';
}
{
    ...
    SetVariableIcon,
        tooltip;
    'Save a value to use later in your workflow',
        category;
    'memory';
}
{
    ...
    GetVariableIcon,
        tooltip;
    'Get a previously saved value from memory',
        category;
    'memory';
}
// Advanced Nodes
{
    ...
    '🎲',
        tooltip;
    'Advanced random selection with custom distribution patterns',
        category;
    'advanced';
}
{
    ...
    '🔀',
        tooltip;
    'Choose different creative paths based on conditions',
        category;
    'advanced';
}
// Transform & Logic
{
    ...
    '🔄',
        tooltip;
    'Process content in a specific creative sequence',
        category;
    'transform';
}
{
    ...
    '🕸️',
        tooltip;
    'Generate content based on probability patterns and transitions',
        category;
    'transform';
}
{
    ...
    '🐍',
        tooltip;
    'Apply custom processing logic to transform content',
        category;
    'process';
}
;
...
const GraphEditorInner = ({ initialNodes, initialEdges, validateConnection }) => {
    ...
    const { optimizer, metrics, isPerformanceGood } = useCanvasOptimization({ maxVisibleNodes: 150,
...
const [optimizationSettings, setOptimizationSettings] = useState({ deadCodeElimination: true,
...
const [encryptionState, setEncryptionState] = useState({ status: 'not_encrypted',
...
const { showUnsavedDialog, dialogAction, confirmNavigation, handleSave: handleUnsavedSave, handleDontSave: handleUnsavedDontSave, handleCancel: handleUnsavedCancel } = useUnsavedChanges({ hasUnsavedChanges,
...
const { errors, styledEdges, styledNodes } = useValidation({ edges,
    nodes,
    highlightNodeIds,
    highlightEdgeIds,
    validateConnection });
...
const NodeRender = useMemo(() => {
    const NodeRenderComponent = (props) => {
        ...
        if (isPerformanceGood) {
            ...
            return (_jsx(SmoothNodeWrapper, { id: props.id, data: props.data, selected: selectedNodeId === props.id, nodeType: props.data?.nodeType || 'default', isSelected: selectedNodeId === props.id, onNodeClick: setSelectedNodeId, children: _jsx(InnerNode, { id: props.id, data: props.data, selected: selectedNodeId === props.id, onSelect: setSelectedNodeId, getNodeMeta: getNodeMeta, getCategoryColor: getCategoryColor }) }));
            // Fallback to standard rendering for performance
            if (shouldUseVariablePorts) {
                return (_jsx(VariablePortNodeRenderer, { id: props.id, data: props.data, selected: selectedNodeId === props.id, onSelect: setSelectedNodeId, getNodeMeta: getNodeMeta, getCategoryColor: getCategoryColor }));
                return (_jsx(NodeRenderer, { id: props.id, data: props.data, selected: selectedNodeId === props.id, onSelect: setSelectedNodeId, getNodeMeta: getNodeMeta, getCategoryColor: getCategoryColor }));
            }
            ;
...
const handleInspectorChange = (partial) => {
    ...
    setNodes(prev => prev.map(n => (n.id === selectedNode.id ? { ...n, data: { ...n.data, ...partial } } : n)));
};
// Edge drag handler
    ...
    if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
        ...
        if (file.name.toLowerCase().endsWith('.psg')) {
            ...
            try {
                ...
                const result = deserializeProject(content, { skipValidation: false,
                    autoMigrate: true,
                    preserveIds: true });
            }
            finally { }
...
if (result.success && result.data) {
    ...
    if (result.warnings && result.warnings.length > 0) {
        ...
        try {
            ...
            const position = reactFlowInstance.screenToFlowPosition({ x: event.clientX,
                y: event.clientY });
        }
        ;
...
const handleChangeAlgorithm = useCallback((algorithm) => {
    setEncryptionState(prev => ({
        ...prev,
        algorithm: algorithm,
        strength: algorithm.includes('256') || algorithm.includes('4096')
            ? 'strong'
            : algorithm.includes('128') || algorithm.includes('2048')
                ? 'medium'
                : 'weak'
    }));
});
...
const handleSaveProject = useCallback(() => { setSaveDialogOpen(true); }, []);
    ...
    if (result.success) {
        ...
        if (result.projectName && result.metadata) {
            try {
                ...
                //
                // RecentProjectsManager.addRecentProject({ //   name: result.projectName
...
const handleExportBundle = useCallback(() => { setExportDialogOpen(true); }, []);
const handleExportSuccess = useCallback((result) => {
    if (result.success) {
...
// Template handlers
const handleSaveTemplate = useCallback(() => { setSaveTemplateDialogOpen(true); }, []);
const handleBrowseTemplates = useCallback(() => { setTemplateBrowserOpen(true); }, []);
const handleTemplateSave = useCallback(async (templateData) => {
    try {
        ...
        if (result.success) {
            ...
            try {
                ...
                setTimeout(() => setStatusMessage(''), 5000);
                return { success: false, error: errorMessage }[saveAsTemplate];
            }
        }
    }
    finally {
...
const handleTemplateApply = useCallback(async (templateId, options) => {
    try {
        ...
        if (result.success) {
            ...
            try {
                ...
                setTimeout(() => setStatusMessage(''), 5000)[applyTemplate];
            }
        }
    }
    finally {
...
const handleOptimizationOpen = useCallback(() => { setOptimizationMenuOpen(prev => !prev); }, []);
const handleOptimizationSettingsChange = useCallback((newSettings) => {
    setOptimizationSettings(newSettings);
    setStatusMessage('Optimization settings updated');
    setTimeout(() => setStatusMessage(''), 3000);
}, []);
const handlePerformanceMonitorToggle = useCallback(() => { setPerformanceMonitorVisible(prev => !prev); }, []);
...
return (_jsx(DemoModeManager, { initialConfig: {
        ...
        console.log('Demo mode changed:', config);
                    ...
                    localStorage.removeItem('graphDraft');
                                ...
                                } }), ": isPerformanceGood ? 3 : 2, filter: isPerformanceGood ? 'drop-shadow(0 0 6px rgba(255, 124, 0, 0.3))' : 'none' } } connectionLineType=", viewport.zoom > 0.5 ? ConnectionLineType.SmoothStep : ConnectionLineType.Straight, "// Dynamic edge options based on performance defaultEdgeOptions=", {
                                ...
                                type), ": 'arrow', color: isPerformanceGood ? '#ff7c00' : '#666', width: isPerformanceGood ? 16 : 12, height: isPerformanceGood ? 16 : 12 } }, // Professional zoom/pan settings with smooth transitions minZoom=", 0.05, "maxZoom=", 6, "defaultViewport=", { x: 0, y: 0, zoom: 1 }, "// Smooth zoom and pan transitions translateExtent=", [
                                [-2000, -2000]
                        ...
                        data: data || {};
                        draggable: true;
                    }, "; setNodes(prevNodes => [...prevNodes, newNode]); onNodeDelete=", nodeIds => {
                        ...
                        onSave = {}();
                    }, " => handleSaveProject()} onLoad=", () => handleLoadProject(), "theme=\"cinema\" />", _jsx(StickyNotesManager, { disabled: false, readonly: false }), _jsx(NodeLabelsManager, { disabled: false, readonly: false, selectedNodeId: selectedNodeId }), _jsx(RegionGroupsManager, { disabled: false, readonly: false }), _jsx(ConnectionAnnotationsLayer, { canEdit: true, showTooltips: true, visible: true }), _jsxs("div", { style: {
                            position: 'absolute',
                            bottom: 10,
                            right: 10,
                    ...
                    >
                        { selectedNode } && (_jsx("div", { style: {
                        transform: selectedNode ? 'translateX(0)' : 'translateX(100%)',
                        transition: 'transform 0.4s cubic-bezier(0.4, 0, 0.2, 1)',
                        width: 320,
                        height: '100%'
                    }, children: isPerformanceGood ? (_jsx(SmoothInspectorPanel, { node: selectedNode, schema: selectedSchema, onChange: handleInspectorChange, onGlobalPreviewRequest: handleGlobalPreviewRequest })) : (_jsx(InspectorPanel, { node: selectedNode, schema: selectedSchema, onChange: handleInspectorChange, onGlobalPreviewRequest: handleGlobalPreviewRequest })) })) })] }) })) /* Professional CSS Transitions and Animations - Enhanced */;
...
_jsx("style", { children: `
  ...
  border-color: var(--color-accent-orange, #ff7800) !important;
        ...
        .react-flow__node.selected { box-shadow: var(--shadow-lg, 0 10px 15px rgba(0, 0, 0, 0.5))
  ...
  var(--color-bg-tertiary, #404040)
        ...
        .react-flow__edge path { stroke: var(--color-ui-border-light, #525252) !important;
  ...
  filter: drop-shadow(0 0 12px rgba(255, 120, 0, 0.6)) !important;
        ...
        .react-flow__edge.selected path { stroke: var(--color-accent-orange, #ff7800) !important;
  stroke-width: 3px !important }
  filter: drop-shadow(0 0 8px rgba(255, 120, 0, 0.4)) !important;

        /* Professional Handle Styling */
        .react-flow__handle { background: var(--color-bg-secondary, #383838) !important;
  ...
  border-color: var(--color-accent-orange, #ff7800) !important }
        .react-flow__handle.connectable { background: var(--color-accent-blue, #0ea5e9) !important }
        /* Professional Connection Line */
        .react-flow__connection-line { stroke: var(--color-accent-orange, #ff7800) !important;
  stroke-width: 4px !important }
  filter: drop-shadow(0 0 8px rgba(255, 120, 0, 0.4)) !important;

        /* Professional Controls */
        .react-flow__controls button { background: linear-gradient(
  145deg
  var(--color-bg-tertiary, #404040) }
  var(--color-bg-secondary, #383838)
  ) !important;
  ...
  transform: scale(1.05) !important;
        ...
        .react-flow__minimap { background: linear-gradient(
  145deg
  var(--color-bg-secondary, #383838) }
  ...
  box-shadow: var(--shadow-lg, 0 10px 15px rgba(0, 0, 0, 0.5)) !important;
        ...
        .react-flow__minimap-node { fill: var(--color-accent-orange, #ff7800) !important;
  opacity: 0.8 !important }
        /* Professional Background */
        .react-flow__background { background: var(--color-bg-primary, #2c2c2c) !important }
...
_jsx(DirectorPreviewToolbar, { nodes: nodes, edges: edges, isPreviewOpen: previewOpen, onPreviewToggle: () => {
        ...
        _jsx(ContextualHelpSystem, { nodes: nodes, edges: edges, selectedNodeId: selectedNodeId, selectedEdgeId: selectedEdgeId, userLevel: "beginner" // This could be dynamic based on user profile
            ...
            }, extensionsOpen: extensionsOpen, encryptionState: encryptionState, onEncrypt: handleEncrypt, onDecrypt: handleDecrypt, onChangeAlgorithm: handleChangeAlgorithm, onOptimization: handleOptimizationOpen, optimizationEnabled: isOptimizationEnabled, onSaveTemplate: handleSaveTemplate, onBrowseTemplates: handleBrowseTemplates });
        { /* Professional Loading State */ }
        {
            (previewLoading || isCreatingNode) && (_jsxs("div", { style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    >
                        _jsx(ProfessionalSpinner, { type: "dots", size: 48, color: "#ff7c00", message: isCreatingNode ? 'Creating node...' : 'Generating previews...' }), div: true, children: [")}", process.env.NODE_ENV === 'development' && (_jsxs("div", { className: "development-only", style: {
                            position: 'fixed',
                            top: 10,
                            left: 10,
                            ...
                            fontSize: 11,
                            zIndex: 10000
                        }, children: [_jsxs("div", { children: ["FPS: ", metrics.fps] }), _jsxs("div", { children: ["Nodes: ", metrics.visibleNodes, "/", nodes.length] }), _jsxs("div", { children: ["Quality: ", isPerformanceGood ? 'High' : 'Optimized'] })] })), _jsx(PreviewModal, { open: previewOpen, loading: previewLoading, error: previewError, results: previewResults, onClose: () => {
                            cancelPreview();
                            setPreviewOpen(false);
                ...
                ,
                    _jsxs("button", { onClick: () => {
                            ...
                            >
                        , Settings: true, button: true, children: [_jsx("button", { onClick: () => {
        ...
        {
            nodeCreationAnimation && (_jsx("div", { className: "animate-node-create-overlay", style: {
                    position: 'fixed',
                    pointerEvents: 'none',
                    zIndex: 1000,
                    width: 200,
        ...
        {
            process.env.NODE_ENV === 'development' && (_jsx(DemoPerformanceTester, { onTestComplete: result => {
    ...
    export: true, const: true, "GraphEditor:React": true, FC: true }), _jsxs(GraphEditorProps, { children: [" = props => ", , "return (", _jsx(ReactFlowProvider, { children: _jsx(GraphEditorInner, { ...props }) }), "); ; export default GraphEditor;"] });
