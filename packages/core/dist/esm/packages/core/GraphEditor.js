...
const injectSafeStyles = () => {
    ...
    const GraphEditorInner = ({ initialNodes, initialEdges, validateConnection }) => {
        ...
        return (_jsxs(DemoModeManager, { initialConfig: {
                brandingVisible: true,
                debugElementsHidden: false,
            }, onModeChange: (config) => {
                console.log('Demo mode changed:', config);
            }, children: [_jsx("div", { style: { position: 'relative', width: '100%', height: '100%' } >
                        (_jsx(RestorePrompt, { show: showRestorePrompt, draft: restoreDraft, onRestore: (nodes, edges) => {
                                setNodes(nodes);
                                setEdges(edges);
                            ...
                            ,
                                _jsx("div", { style: { display: 'flex', height: '100%' } >
                                        (_jsx(TabbedPalette, { nodes: NODE_TYPES, collapsed: paletteCollapsed, onToggle: () => setPaletteCollapsed((c) => !c), onDragStart: handlePaletteDragStart, showSearch: true, showFavorites: true, maxSearchResults: 15, defaultActiveTab: "content" })
                                            ,
                                                _jsxs("div", { ref: canvasRef, style: { flex: 1, position: 'relative', overflow: 'visible' }, "data-testid": "react-flow-canvas-wrapper", children: [_jsx(ReactFlow, { nodes: styledNodes, edges: styledEdges, "data-testid": "react-flow-canvas", onNodesChange: (changes) => {
                                                                lastChangeRef.current = Date.now();
                                                                onNodesChange(changes);
                                                                onEdgesChange = {}(changes);
                                                            } }), " => ", lastChangeRef.current = Date.now(), "; onEdgesChange(changes); onConnect=", onConnect, "onNodeClick=", onNodeClick, "fitView style=", { background: 'var(--color-bg-primary, #2c2c2c)', height: '100%' }, "nodeTypes=", nodeTypes, "onDrop=", handleDrop, "onDragOver=", handleDragOver, "// Node interaction elementsSelectable=", true, "// Professional 3D-style mouse controls with performance optimization panOnScroll=", false, " // Disable scroll to pan zoomOnScroll=", true, " // Enable scroll to zoom (standard 3D behavior) panOnDrag=", [1, 2], " // Pan with left or middle mouse button selectionOnDrag=", nodes.length < 100, " // Disable box selection for large graphs zoomOnDoubleClick=", false, " // Disable double-click zoom // Performance optimizations nodesDraggable=", nodes.length < 150, "nodesConnectable=", nodes.length < 200, "snapToGrid=", viewport.zoom > 0.6, "snapGrid=", [16, 16], "// Keyboard shortcuts - completely disable all keyboard handling deleteKeyCode=", null, " // Disable delete key completely multiSelectionKeyCode=", null, " // Disable multi-selection zoomActivationKeyCode=", null, " // Disable zoom activation // Disable all keyboard event capturing onKeyDown=", (e) => {
                ...
                }, " => ", setNodes(prevNodes => prevNodes.filter(n => !nodeIds.includes(n.id))), "; setEdges(prevEdges => prevEdges.filter(e => !nodeIds.includes(e.source) && !nodeIds.includes(e.target) )); onExport=", (format) => {
                    console.log(`Exporting in format: ${format}`);
                    // Export functionality would be implemented here
                    onSave = {}();
                }, " => handleSaveProject()} onLoad=", () => handleLoadProject(), "theme=\"cinema\" />", _jsx(StickyNotesManager, { disabled: false, readonly: false }), _jsx(NodeLabelsManager, { disabled: false, readonly: false, selectedNodeId: selectedNodeId }), _jsx(RegionGroupsManager, { disabled: false, readonly: false }), _jsx(ConnectionAnnotationsLayer, { canEdit: true, showTooltips: true, visible: true }), _jsx("div", { style: {
                        position: 'absolute',
                        bottom: 10,
                        right: 10,
                        ...
                        userSelect: 'none',
                        onClick = {}()
                    } }), " => setShowControls(!showControls)} >", _jsx("div", { style: { fontWeight: 600, marginBottom: 4, color: '#e2e8f0' } >
                 }), "\uD83D\uDDB1\uFE0F Controls ", showControls ? '▼' : '▶'] }));
        {
            showControls && (_jsx("div", { style: { marginTop: 8, lineHeight: 1.6 } >
                    (_jsxs("div", { children: [_jsx("b", { children: "Pan:" }), " Left-click + drag on canvas"] })
                        ,
                            _jsxs("div", { children: [_jsx("b", { children: "Zoom:" }), " Mouse wheel / trackpad scroll"] })
                                ,
                                    _jsxs("div", { children: [_jsx("b", { children: "Select:" }), " Click node"] })
                                        ,
                                            _jsxs("div", { children: [_jsx("b", { children: "Multi-select:" }), " Shift/Ctrl + Click"] })
                                                ,
                                                    _jsxs("div", { children: [_jsx("b", { children: "Connect:" }), " Drag from output port"] })
                                                        ,
                                                            _jsxs("div", { children: [_jsx("b", { children: "Delete:" }), " Select + Delete/Backspace"] })
                                                                ,
                                                                    _jsxs("div", { children: [_jsx("b", { children: "Alternative Pan:" }), " Middle-click + drag"] })) }));
        }
    };
};
div >
...
style = {};
{
    ...
    'hidden',
        borderLeft;
    selectedNode ? '1px solid rgba(55, 65, 81, 0.6)' : 'none',
            >
                        ...
                        onUserLevelChange = {}(level);
                        {
                                ...
                                { /* Professional Loading State */};
                            {
                                (previewLoading || isCreatingNode) && (_jsxs("div", { style: {
                                        ...
                                        >
                                            _jsx(ProfessionalSpinner, { type: "dots", size: 48, color: "#ff7c00", message: isCreatingNode ? 'Creating node...' : 'Generating previews...' }), div: true, children: [")}", process.env.NODE_ENV === 'development' && (_jsx("div", { className: "development-only", style: {
                                                position: 'fixed',
                                                top: 10,
                                                left: 10,
                                                ...
                                                >
                                                    (_jsxs("div", { children: ["FPS: ", metrics.fps] })
                                                        ,
                                                            _jsxs("div", { children: ["Nodes: ", metrics.visibleNodes, "/", nodes.length] })
                                                                ,
                                                                    _jsxs("div", { children: ["Quality: ", isPerformanceGood ? 'High' : 'Optimized'] })) }))] }));
                            }
                            _jsx(PreviewModal, { open: previewOpen, loading: previewLoading, error: previewError, results: previewResults, onClose: () => {
                                    ...
                                    onResultHover = {}(idx);
                                    {
                                        ...
                                        />;
                                        {
                                            extensionsOpen && (_jsx(ExtensionManagerPanel, { onClose: () => setExtensionsOpen(false) }));
                                        }
                                        { /* Project Management Dialogs */ }
                                        _jsx(SaveProjectDialog, { isOpen: saveDialogOpen, onClose: () => setSaveDialogOpen(false), onSave: handleSaveSuccess })
                                            ,
                                                _jsx(LoadProjectDialog, { isOpen: loadDialogOpen, onClose: () => setLoadDialogOpen(false), onLoad: handleLoadSuccess });
                                        { /* Unsaved Changes Dialog (Story 6.1) */ }
                                        _jsx(UnsavedChangesDialog, { isOpen: showUnsavedDialog, projectName: currentProject?.name, actionDescription: dialogAction, onSave: handleUnsavedSave, onDontSave: handleUnsavedDontSave, onCancel: handleUnsavedCancel })
                                            ,
                                                _jsx(ExportBundleDialog, { isOpen: exportDialogOpen, onClose: () => setExportDialogOpen(false), nodes: nodes, edges: edges, onExport: handleExportSuccess });
                                        { /* Template Dialogs */ }
                                        _jsx(SaveTemplateDialog, { isOpen: saveTemplateDialogOpen, onClose: () => setSaveTemplateDialogOpen(false), onSave: handleTemplateSave })
                                            ,
                                                _jsx(TemplateBrowser, { isOpen: templateBrowserOpen, onClose: () => setTemplateBrowserOpen(false), onApplyTemplate: handleTemplateApply, currentAuthor: "current-user" });
                                        { /* Epic 7.3 - Advanced Settings Modal */ }
                                        _jsx(SettingsModal, { isOpen: settingsModalOpen, onClose: () => setSettingsModalOpen(false), onSettingsChange: (settings) => {
                                                console.log('Settings updated:', settings);
                                                // Settings changes are automatically handled by the SettingsManager
                                                />;
                                                { /* Epic 8.5 - Real-Time Preview Panels */ }
                                                _jsx(RealTimePreviewPanel, { visible: realTimePreviewOpen, onClose: () => setRealTimePreviewOpen(false), enablePerformanceMonitoring: true, maxResults: 5 })
                                                    ,
                                                        _jsx(IndividualResultManager, { visible: resultManagerOpen, onClose: () => setResultManagerOpen(false), enableComparison: true, enableAnalytics: true, maxDisplayResults: 10 });
                                                { /* Graph Optimization Components */ }
                                                _jsx(OptimizationControls, { settings: optimizationSettings, onSettingsChange: handleOptimizationSettingsChange, isOpen: optimizationControlsOpen, onClose: () => setOptimizationControlsOpen(false) })
                                                    ,
                                                        _jsx(GraphAnalysisPanel, { nodes: nodes, edges: edges, isOpen: graphAnalysisOpen, onClose: () => setGraphAnalysisOpen(false) })
                                                            ,
                                                                _jsx(PerformanceMonitor, { isVisible: performanceMonitorVisible, onToggle: handlePerformanceMonitorToggle });
                                                { /* Optimization Menu */ }
                                                {
                                                    optimizationMenuOpen && (_jsx("div", { "data-optimization-menu": true, style: {
                                                            position: 'fixed',
                                                            bottom: '60px',
                                                            left: '50%',
