...
Promise < ValidationResult > {
    ...
    : .config.rateLimiting.enabled && userId
};
{
 ...
 */
async;
invalidateExistingCodes(userId, string, purpose, string);
Promise < void  > {
    const: codes = await this.storage.findByUserAndPurpose(userId, purpose),
    for(, code, of, codes) {
         ...
         */
        async;
        getUserValidationStats(userId, string);
        Promise < {
            totalCodes: number,
            activeCodes: number,
            expiredCodes: number,
            ...
            setupMonitoring() {
                if (!this.config.monitoring.enabled) {
                    ...
                    }, userId, event.userId, purpose, event.purpose, reason, event.reason, attempts, event.attempts);
                }
                ;
            },
            this: .on('rateLimitExceeded', (event) => {
                console.warn(`[RATE_LIMIT] ${event.type.toUpperCase()} rate limit exceeded`, {});
            }, userId, event.userId, ipAddress, event.ipAddress)
        };
        ;
    },
...
if (code && new Date() > code.expiresAt && code.attempts > 0) {
        ...
        details;
    {
        codeId,
            expiredSince;
        new Date().getTime() - code.expiresAt.getTime(),
            attempts;
        code.attempts,
            ipAddress;
    }
...
ValidationService;
{
    const config = environment === 'development' ? {
        ...
        monitoring: {
            ...
            logAllValidations: true,
        } } : , { rateLimiting: { enabled: , true: , maxGenerationsPerHour: , 10: , maxValidationAttemptsPerHour: , 30: , maxValidationAttemptsPerCode: , 5: , }, monitoring: { enabled: , true: , alertOnSuspiciousActivity: , true: , logAllValidations: , false: , } };
