...
return code;
async;
createVerificationCode(code, string, userId, string, purpose, string, options, { expirationMinutes: number,
    maxAttempts: number,
    metadata: (Record) } = {});
...
async;
validateCode(inputCode, string, storedCode, VerificationCodeData, options, (Partial) = {});
    ...
    try {
        ...
        if (opts.constantTimeValidation) {
            ...
            if (delay > 0) {
                ...
                generateRecoveryCodes(count, number = 10);
                string;
                {
                    const codes = [];
            ...
            string;
            {
                if (options.customAlphabet) {
                    ...
                    for (let i = 0; i < length; i++) {
                        ...
                        return code.join('');
                        hashCode(code, string, salt, Buffer);
                        string;
                        {
                            ...
                            if (!valid && reason) {
                                ...
                                };
                                {
                                    ...
                                    constructor();
                                    {
                                         ...
                                         */
                                        async;
                                        createEmailVerificationCode(((userId) => ));
                                    }
                                    email: string;
                                 ...
                                 */
                                async;
                                createSMSVerificationCode(((userId, phoneNumber) => {
                            ...
                            } };
                        {
                             ...
                             */
                            async;
                            validateVerificationCode(((inputCode, storedCode) => {
                                ...
                                export const CodeUtils = {
                                     ...
                                     * Clean user input (remove formatting, normalize case)
                                     */
                                    cleanUserInput(input) { return input.replace(/[\s-]/g, '').toUpperCase(); }
                                    /**
                                     ...
                                     * Generate a secure random token for API keys, etc.
                                     */
                                    generateSecureToken(length = 32) { return crypto.randomBytes(length).toString('base64url'); }
                                    /**
                                     ...
                                     * Calculate entropy bits for a given alphabet and length
                                     */
                                    calculateEntropy(alphabetSize, length) { return Math.log2(Math.pow(alphabetSize, length)); },
