...
{ // Store hooks
    const { results, isLoading, error, lockedResults, regeneratingResults, lockResult, unlockResult };
...
const exportResult = useCallback((index, format) => {
...
'csv';
const csvData = [
    ['Field', 'Value']['Seed', result.seed]['Output', result.output || '']['Error', result.error || '']['Execution Time', result.executionTimeMs || '']['Locked', result.locked ? 'Yes' : 'No']['Word Count', analytics?.wordCount || '']['Readability Score', analytics?.readabilityScore || '']['Sentiment', analytics?.sentiment || '']
];
...
const comparisonSummary = useMemo(() => {
    if (!comparisonMode.enabled || selectedResults.size < 2)
        return null;
    const selected = Array.from(selectedResults);
    const analytics = selected.map(i => resultAnalytics.get(i)).filter(Boolean);
    ...
    return { count: selected.length,
        ...
        dominantSentiment: Object.keys(dominantSentiment).reduce((a, b) => dominantSentiment[a] > dominantSentiment[b] ? a : b),
...
if (!visible)
    return null;
return;
_jsxs("div", { className: `individual-result-manager ${className}`, style: {
        position: 'fixed',
        left: '20px',
        top: '20px',
        width: '600px',
        ...
        color: 'white'
    }, children: [_jsx("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' }, children: _jsxs("div", { children: [_jsx("h3", { style: { margin: 0, fontSize: '16px', fontWeight: '600' }, children: "\u2699\uFE0F Result Manager" }), _jsxs("div", { style: { fontSize: '12px', opacity: 0.9, marginTop: '4px' }, children: [results.length, " results \u2022 ", lockedResults.length, " locked", comparisonMode.enabled && selectedResults.size > 0 && (()), " \u2022 ", selectedResults.size, " selected"] }), ")}"] }) }), onClose && ()
...
style = {};
{
    ...
    >
        _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }, children: [enableComparison && ()
                    < button, "onClick=", toggleComparisonMode, "style=", {
                    padding: '6px 12px',
...
button >
;
_jsx("button", { onClick: () => setSelectedResults(new Set()), disabled: selectedResults.size === 0, style: {
        padding: '6px 12px',
        ...
        >
    , Clear: true, Selection: true, button: true });
div >
    { /* Comparison Summary */};
{
    ...
    >
        (_jsx("div", { style: { fontWeight: '600', marginBottom: '8px' }, children: "Comparison Summary" })
            ,
                _jsxs("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px' }, children: [_jsxs("div", { children: [_jsx("strong", { children: "Results:" }), " ", comparisonSummary.count] }), _jsxs("div", { children: [_jsx("strong", { children: "Avg Words:" }), " ", comparisonSummary.avgWordCount] }), _jsxs("div", { children: [_jsx("strong", { children: "Readability:" }), " ", comparisonSummary.avgReadability] }), _jsxs("div", { children: [_jsx("strong", { children: "Sentiment:" }), " ", comparisonSummary.dominantSentiment] })] }));
{
 ...
 >
    _jsx("strong", { children: "Common Topics:" });
{
    comparisonSummary.topics.slice(0, 5).join(', ');
}
div >
;
div >
    ...
    }, children: [_jsx("strong", { children: "Error:" }), " ", error] });
{
    ...
    >
        (_jsx("div", { style: { fontSize: '32px', marginBottom: '12px' }, children: "\uD83D\uDCCB" })
            ,
                _jsx("div", { children: "No results to manage" }));
div >
;
{
    results.slice(0, maxDisplayResults).map((result, index) => {
        ...
        _jsx("div", { style: {
            ...
            }, "border:": true }, `${result.seed}-${index}`);
        `2px solid ${isSelected ? '#0ea5e9' : result.locked ? '#f59e0b' : '#e2e8f0'}`;
    }, borderRadius, '8px', position, 'relative', transition, 'all 0.2s ease');
        >
            { /* Result Header */}
        < div;
    style = {};
 ...
 >
    _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: [comparisonMode.enabled && ()
                < input, "type=\"checkbox\" checked=", isSelected, "onChange=", () => handleResultAction({ type: 'compare', resultIndex: index }), "style=", { margin: 0 }, "/> )}", _jsxs("span", { style: {
                ...
                < span, " style=", { fontSize: '11px', color: '#6b7280' }, ">", result.executionTimeMs, "ms"] });
{
    result.locked && ()
        < span;
    title = {} `Locked: ${result.lockedNote || 'No note'}`;
}
style = {};
{
    fontSize: '14px';
...
;
div >
    _jsx("button", { onClick: () => setActionMenuIndex(actionMenuIndex === index ? null : index), style: {
            background: 'none',
            border: 'none',
            fontSize: '16px',
            cursor: 'pointer',
        ...
        , button: true });
{ /* Result Content */ }
_jsxs("div", { style: { fontSize: '14px', lineHeight: '1.4', marginBottom: showAnalytics ? '12px' : '0' }, children: [result.error ? ()
    ...
    { /* Analytics */};
{
    ...
    >
        _jsx("div", { style: { display: 'grid', gridTemplateColumns: 'repeat(),
                auto } - fit, minmax: true });
(80);
px;
1;
fr;
...
px;
' }}>
    < div > _jsx("strong", { children: "Words:" });
{
    analytics.wordCount;
}
div >
    (_jsxs("div", { children: [_jsx("strong", { children: "Unique:" }), " ", analytics.uniqueWords] })
        ,
            _jsxs("div", { children: [_jsx("strong", { children: "Readability:" }), " ", analytics.readabilityScore] })
                ,
                    _jsxs("div", { children: [_jsx("strong", { children: "Sentiment:" }), " ", analytics.sentiment] })
                        ,
                            _jsxs("div", { children: [_jsx("strong", { children: "Similarity:" }), " ", Math.round(analytics.similarity * 100), "%"] }));
div >
            ...
            < div, style = {} };
{
 ...
 >
    _jsx("strong", { children: "Topics:" });
{
    analytics.topics.join(', ');
}
div >
;
div >
...
{ /* Action Menu */ }
{
    ...
    ;
    onClick = {}();
    handleResultAction({ type: 'lock', resultIndex: index });
}
...
()
    < button;
onClick = {}();
handleResultAction({ type: 'unlock', resultIndex: index });
style = { menuButtonStyle }
    >
;
Unlock;
        ...
        _jsx("div", { style: {
                ...
                >
                    (_jsx("h4", { style: { margin: '0 0 12px', fontSize: '16px' }, children: "Lock Result" })
                        ,
                            _jsx("p", { style: { margin: '0 0 12px', fontSize: '14px', color: '#6b7280' }, children: "Add a note to explain why this result is locked:" })
                                ,
                                    _jsx("textarea", { value: lockDialog.note, onChange: (e) => setLockDialog(prev => ({ ...prev, note: e.target.value })), placeholder: "Important for X reason...", style: {
                                            width: '100%',
                                            height: '80px',
                                            padding: '8px',
                                            border: '1px solid #e2e8f0',
                                        ...
                                        ,
                                            _jsx("div", { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' }, children: _jsx("button", { onClick: () => setLockDialog({ visible: false, resultIndex: -1, note: '' }), style: {
                                                        padding: '8px 16px',
                                                        background: '#e2e8f0',
                                                        color: '#374151',
                                                        border: 'none',
                                                        ...
                                                        >
                                                            Cancel }) })
                                                ,
                                                    _jsx("button", { onClick: handleLockConfirmation, style: {
                                                            padding: '8px 16px',
                                                            background: '#f59e0b',
                                                            color: 'white',
                                                            border: 'none',
