 ...
 */
export function withDroppableNode(NodeComponent, options = {}) {
    return React.memo((props) => {
        ...
        const [{ isOver, canDrop }, drop] = useDrop(() => ({
            accept: 'preset',
            drop: handleDrop,
            canDrop: (item) => isPresetCompatible(item.preset, type || ''),
            collect: (monitor) => ({
        ...
        return (_jsxs("div", { ref: drop, className: `droppable-node-wrapper ${dropHighlight ? 'drop-highlight' : ''} ${justDropped ? 'just-dropped' : ''}`, children: [_jsx(NodeComponent, { ...enhancedProps }), dropHighlight && (_jsx("div", { className: "drop-indicator", children: _jsx("span", { children: "Drop to apply preset" }) }))] }));
    });
}
/**
 * Check if a preset is compatible with a node type
 */
function isPresetCompatible(preset, nodeType) {
    // Direct type match
