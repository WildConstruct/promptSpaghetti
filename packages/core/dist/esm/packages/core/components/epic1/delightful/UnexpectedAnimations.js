...
export const UnexpectedAnimations = ({ enabled = true, weirdMode = false, }) => {
    ...
    const animateNodeCreation = useCallback((nodeId) => {
        if (!enabled)
            return;
    ...
    const animateEdgeConnection = useCallback((edgeId) => {
        if (!enabled)
            return;
        setEdges(edges => edges.map(e => e.id === edgeId
    ...
    // Idle animations
    useEffect(() => {
        if (!enabled || idleTime < 10)
            return; // Start after 10 seconds idle
        const nodes = getNodes();
        if (nodes.length === 0)
            return;
        ...
        if (idleTime > 10 && idleTime < 20) {
            const interval = setInterval(() => {
                ...
                setNodes(nodes => nodes.map(n => n.id === randomNode.id
...
export function celebrateNodeClick(nodeId) {
