            ...
            (_jsx("div", { className: "flex items-center justify-between mb-4", children: _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("h3", { className: "text-lg font-medium text-gray-900", children: ["Resource: ", resourceId.substring(0, 8), "..."] }), _jsxs("span", { className: "px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full", children: [items.length, " in queue"] })] }) })
                ,
                    _jsx("div", { className: "space-y-2", children: items
                            ...
                            >
                                (_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx("div", { className: "flex-shrink-0", children: _jsx("div", { className: "w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center", children: _jsxs("span", { className: "text-sm font-medium text-blue-600", children: ["#", index + 1] }) }) }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(User, { className: "h-4 w-4 text-gray-400" }), _jsxs("span", { className: "text-sm font-medium text-gray-900", children: [item.user_id.substring(0, 8), "..."] })] }), _jsx("div", { className: "flex items-center space-x-2", children: _jsx("span", { className: "text-sm text-gray-500", children: item.lock_type.replace('_', ' ') }) }), _jsxs("div", { className: `px-2 py-1 text-xs rounded-full border ${getPriorityColor(item.priority)}`, children: ["}", getPriorityLabel(item.priority)] })] })
                                    ,
                                        _jsxs("div", { className: "flex items-center space-x-4", children: [_jsxs("div", { className: "text-right", children: [_jsxs("div", { className: "flex items-center space-x-1 text-sm text-gray-500", children: [_jsx(Clock, { className: "h-3 w-3" }), _jsx("span", { children: item.estimated_wait_time
                                                                        ? formatWaitTime(item.estimated_wait_time)
                                                                        : 'Unknown' })] }), _jsxs("div", { className: "text-xs text-gray-400", children: ["Queued: ", new Date(item.queued_at).toLocaleString()] })] }), _jsx("button", { onClick: () => onRemoveFromQueue(item.id), className: "p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded", title: "Remove from queue", children: _jsx(X, { className: "h-4 w-4" }) })] }))) }))), ")}"] });
{ /* Queue Statistics */ }
_jsx("div", { className: "mt-4 pt-4 border-t border-gray-200", children: _jsxs("div", { className: "grid grid-cols-3 gap-4 text-sm", children: [_jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-gray-500", children: "Total Wait Time" }), _jsxs("div", { className: "font-medium text-gray-900", children: [formatWaitTime(), "items.reduce((sum, item) => sum + (item.estimated_wait_time || 0), 0) )}"] })] }), _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-gray-500", children: "Avg Wait Time" }), _jsxs("div", { className: "font-medium text-gray-900", children: [formatWaitTime(), "Math.round() items.reduce((sum, item) => sum + (item.estimated_wait_time || 0), 0) / items.length )}"] })] }), _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-gray-500", children: "Queue Length" }), _jsx("div", { className: "font-medium text-gray-900", children: items.length })] })] }) });
div >
;
div >
;
