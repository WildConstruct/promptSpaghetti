...
return;
_jsx(ExecutiveSecurityDashboard, { securityMetrics: securityMetrics, alerts: securityAlerts, complianceStatus: complianceStatus, ...commonProps });
;
DashboardType.COMPLIANCE;
return;
_jsx(ComplianceSecurityDashboard, { complianceStatus: complianceStatus, alerts: securityAlerts.filter(a => a.category === 'policy_violation'), ...commonProps });
;
DashboardType.ANALYTICS;
return;
_jsx(SecurityDashboardWorkflow, { workspaceId: workspaceId, userId: userId, userRole: userRole, dashboardType: currentDashboardType });
;
return;
_jsxs("div", { style: {
        display: 'flex',
