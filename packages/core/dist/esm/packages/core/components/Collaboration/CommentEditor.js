...
_jsxs("div", { className: "relative", children: [allowFormatting && ()
            < div, " className=\"flex items-center justify-between p-2 border border-gray-300 border-b-0 rounded-t-md bg-gray-50\">", _jsxs("div", { className: "flex items-center space-x-1", children: [_jsx("button", { type: "button", onClick: () => insertFormatting('**', '**'), className: "p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors", title: "Bold", children: _jsx(Bold, { className: "w-4 h-4" }) }), _jsx("button", { type: "button", onClick: () => insertFormatting('*', '*'), className: "p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors", title: "Italic", children: _jsx(Italic, { className: "w-4 h-4" }) }), _jsx("button", { type: "button", onClick: () => insertFormatting('`', '`'), className: "p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors", title: "Code", children: _jsx(Code, { className: "w-4 h-4" }) }), _jsx("button", { type: "button", onClick: () => insertFormatting('[link text](', ')'), className: "p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors", title: "Link", children: _jsx(Link, { className: "w-4 h-4" }) }), _jsx("button", { type: "button", onClick: () => insertFormatting('- ', ''), className: "p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors", title: "List", children: _jsx(List, { className: "w-4 h-4" }) }), allowMentions && ()
                ...
                }, "className=\"p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors\" title=\"Mention\" >", _jsx(AtSign, { className: "w-4 h-4" })] }), ")}", allowAttachments && ()
            < button, "type=\"button\" className=\"p-1 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded transition-colors\" title=\"Attach file\" >", _jsx(Paperclip, { className: "w-4 h-4" })] });
div >
    { showPreview } && ()
    < div;
...
{ /* Mentions Dropdown */ }
{
    ...
    className = "absolute z-10 w-full mt-1" >
        _jsx(CommentMentions, { workspaceId: workspaceId, query: mentionQuery, onSelect: handleMentionSelect, onClose: () => setShowMentions(false) });
    div >
    ;
}
{ /* Actions */ }
_jsxs("div", { className: "flex items-center justify-between mt-3", children: [_jsxs("div", { className: "text-xs text-gray-500", children: [_jsx("span", { className: "font-medium", children: "Tip:" }), " Use **bold**, *italic*, `code`, or @mentions"] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("button", { type: "button", onClick: onCancel, className: "px-3 py-1 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors", children: "Cancel" }), _jsxs("button", { type: "button", onClick: handleSubmit, disabled: !content.trim() || isSubmitting, className: "inline-flex items-center space-x-2 px-3 py-1 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors", children: [isSubmitting ? ()
                            < div : , " className=\"w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin\" /> ) : ()", _jsx(Send, { className: "w-4 h-4" }), ")}", _jsx("span", { children: isSubmitting ? 'Posting...' : submitLabel })] })] })] });
div >
;
;
;
