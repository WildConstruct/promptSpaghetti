...
const handleCommentSubmit = async (e) => {
    ...
    try {
        ...
        userId,
            authorType;
        userRole === 'user' ? 'user' : 'agent',
            visibility;
        commentForm.visibility,
            attachments;
        ...
        const handleAssignment = async (assigneeId) => {
            ...
            try {
                ...
                const handleEscalation = async (reason) => {
                    ...
                    ;
                    return;
                    _jsxs("div", { className: "ticket-details-view h-full flex flex-col bg-white", children: [_jsxs("div", { className: "border-b border-gray-200 p-6", children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center space-x-3 mb-2", children: [_jsxs("span", { className: "text-sm font-medium text-blue-600", children: ["#", currentTicket.id.split('-').pop()] }), _jsx(StatusBadge, { status: currentTicket.status }), _jsx(PriorityBadge, { priority: currentTicket.priority }), slaStatus.responseOverdue && ()
                                                                < span, " className=\"px-2 py-1 bg-red-500 text-white text-xs font-medium rounded-full\"> RESPONSE OVERDUE"] }), ")}", slaStatus.resolutionOverdue && ()
                                                        < span, " className=\"px-2 py-1 bg-red-500 text-white text-xs font-medium rounded-full\"> RESOLUTION OVERDUE"] }), ")}"] }), _jsx("h1", { className: "text-xl font-bold text-gray-900 mb-2", children: currentTicket.title }), _jsx("p", { className: "text-gray-600", children: currentTicket.description }), _jsxs("div", { className: "flex items-center space-x-6 mt-4 text-sm text-gray-500", children: [_jsxs("span", { children: ["Type: ", currentTicket.type.replace('_', ' ')] }), _jsxs("span", { children: ["Category: ", currentTicket.category] }), currentTicket.assignedTo && _jsxs("span", { children: ["Assigned: ", currentTicket.assignedTo] }), _jsxs("span", { children: ["Created: ", currentTicket.createdAt.toLocaleDateString()] }), _jsxs("span", { children: ["Updated: ", currentTicket.updatedAt.toLocaleDateString()] })] })] }), _jsx("div", { className: "flex items-center space-x-2", children: canModify && ()
                                    <  >
                                    (_jsx("button", { onClick: () => setShowStatusUpdate(true), className: "px-3 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200", children: "Update Status" })
                                        ,
                                            _jsx("button", { onClick: () => setShowAssignment(true), className: "px-3 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200", children: "Assign" })
                                                ,
                                                    _jsx("button", { onClick: () => setShowEscalation(true), className: "px-3 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700", children: "Escalate" })) }), ")}", onClose && ()
                                < button, "onClick=", onClose, "className=\"p-2 text-gray-400 hover:text-gray-600\" >", _jsx("svg", { className: "h-5 w-5", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) })] });
                };
            }
            finally {
            }
...
className = "mt-4 bg-red-50 border border-red-200 rounded-md p-4" >
    _jsx("p", { className: "text-sm text-red-700", children: error });
div >
;
div >
    { /* Content Area */}
...
className = "flex-1 overflow-y-auto p-6" >
    (_jsxs("h2", { className: "text-lg font-medium text-gray-900 mb-4", children: ["Comments (", visibleComments.length, ")"] })
        ,
            _jsxs("div", { className: "space-y-4", children: [visibleComments.map((comment) => ()
                        < CommentItem, key = { comment, : .id }, comment = { comment }, canViewInternal = { canViewInternal }
                        /  >
                    ), ")}", visibleComments.length === 0 && ()
                        < div, " className=\"text-center py-8 text-gray-500\">", _jsx("svg", { className: "mx-auto h-12 w-12 text-gray-300", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" }) }), _jsx("p", { className: "mt-2 text-sm", children: "No comments yet" })] }));
div >
;
div >
    { /* Comment Form */}
    ...
    { /* Labels */};
{
    currentTicket.labels.length > 0 && ()
        < div;
    className = "mb-6" >
        (_jsx("h3", { className: "text-sm font-medium text-gray-900 mb-2", children: "Labels" })
            ,
                _jsx("div", { className: "flex flex-wrap gap-2", children: currentTicket.labels.map((label) => ()
...
{ /* Attachments */ }
{
    currentTicket.attachments.length > 0 && ()
        < div;
    className = "mb-6" >
        (_jsx("h3", { className: "text-sm font-medium text-gray-900 mb-2", children: "Attachments" })
            ,
                _jsxs("div", { className: "space-y-2", children: [currentTicket.attachments.map((attachment) => ()
...
{ /* External Integrations */ }
{
    currentTicket.externalIntegrations.length > 0 && ()
        < div;
    className = "mb-6" >
        (_jsx("h3", { className: "text-sm font-medium text-gray-900 mb-2", children: "External Links" })
            ,
                _jsx("div", { className: "space-y-2", children: currentTicket.externalIntegrations.map((integration) => ()
...
({ currentStatus, onStatusUpdate, onClose }) => {
    ...
    return;
    _jsxs("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: [_jsxs("div", { className: "bg-white rounded-lg shadow-xl w-full max-w-md", children: [_jsx("div", { className: "px-6 py-4 border-b border-gray-200", children: _jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Update Status" }) }), _jsxs("div", { className: "p-6", children: [_jsx("select", { value: selectedStatus, onChange: (e) => setSelectedStatus(e.target.value), className: "w-full px-3 py-2 border border-gray-300 rounded-md", children: Object.values(TicketStatus).map((status) => ()
                                    < option, key = { status }, value = { status } >
                                    { status, : .replace('_', ' ').toUpperCase() }) }), "))}"] })] }), _jsxs("div", { className: "px-6 py-4 border-t border-gray-200 flex justify-end space-x-3", children: [_jsx("button", { onClick: onClose, className: "px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-md hover:bg-gray-200", children: "Cancel" }), _jsx("button", { onClick: () => onStatusUpdate(selectedStatus), className: "px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700", children: "Update Status" })] })] });
    div >
    ;
    ;
};
