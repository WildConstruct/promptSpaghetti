...
import { Card, Table, Select, DatePicker, Button, Space, Tag, Statistic, Row, Col, Alert, Input, Modal, Tabs } from Progress;
from;
'antd';
import { Bar, Pie } from '@ant-design/plots';
import { SecurityScanOutlined, AlertOutlined, UserOutlined, FileTextOutlined, ExclamationTriangleOutlined, CheckCircleOutlined, ClockCircleOutlined, DownloadOutlined, FilterOutlined } from ReloadOutlined;
from;
'@ant-design/icons';
import { AuditEventType, AuditSeverity, ComplianceFramework, AuditStatus } from auditManagementSystem;
from;
'../audit/AuditManagementSystem';
...
const renderSeverityBadge = (severity) => {
    const colors = {
        [AuditSeverity.LOW]: 'green'[AuditSeverity.MEDIUM], 'orange': [AuditSeverity.HIGH], 'red': [AuditSeverity.CRITICAL], 'purple': 
    };
};
...
div >
    { title: 'Actions',
        ...
        render: (record) => () }
    < Space >
    _jsx(Button, { size: "small", onClick: () => setDashboardState(prev => ({ ...prev, selectedEvent: record })), children: "View Details" });
Space >
;
;
return;
...
;
return;
_jsxs(Row, { gutter: 16, children: [_jsx(Col, { span: 12, children: _jsxs(Card, { title: "Severity Distribution", style: { marginBottom: 16 }, children: [_jsx(Pie, { data: severityData, angleField: "value", colorField: "type", radius: 0.8, label: {
                            type: 'outer'
                        }, "content:": true }), " '", name, " ", percentage, "' height=", 300, "/>"] }) }), _jsx(Col, { span: 12, children: _jsx(Card, { title: "Event Type Distribution", style: { marginBottom: 16 }, children: _jsx(Bar, { data: eventTypeData, xField: "value", yField: "type", height: 300 }) }) })] });
;
;
// Event details modal
...
const SystemHealthTab = () => { return null; };
 >
    _jsxs(Row, { gutter: 16, children: [_jsx(Col, { span: 8, children: _jsx(Progress, { type: "circle", percent: 98, format: () => 'Chain Integrity', strokeColor: "#52c41a" }) }), _jsx(Col, { span: 8, children: _jsx(Progress, { type: "circle", percent: 95, format: () => 'Storage Health', strokeColor: "#1890ff" }) }), _jsx(Col, { span: 8, children: _jsx(Progress, { type: "circle", percent: 100, format: () => 'API Response', strokeColor: "#52c41a" }) })] });
Card >
    _jsx(Alert, { message: "All Systems Operational", description: "Audit management system is running optimally with no issues detected.", type: "success", showIcon: true });
div >
;
;
;
