...
getCategoryColor;
{
    ...
    try {
        ...
        return;
        _jsx("div", { ref: nodeRef, role: "button", "data-testid": `node-${id}`, tabIndex: 0, ...hoverProps, className: `animate-node-hover animate-node-select ${selected ? 'selected' : ''} ${isDeleting ? 'animate-node-delete' : ''}`, onClick: (e) => {
                e.stopPropagation();
                onSelect(id);
            }, onKeyDown: (e) => {
