...
if (loading) {
    ...
    if (error) {
        ...
        { /* Request List */ }
        _jsxs("div", { className: "divide-y divide-gray-200", children: [filteredAndSortedRequests.length === 0 ? ()
                    < div : , " className=\"text-center py-12\">", _jsx(DocumentTextIcon, { className: "mx-auto h-12 w-12 text-gray-400" }), _jsx("h3", { className: "mt-2 text-sm font-medium text-gray-900", children: "No approval requests" }), _jsx("p", { className: "mt-1 text-sm text-gray-500", children: activeTab === 'pending' ? 'No pending approvals at this time.' :
                        activeTab === 'completed' ? 'No completed approvals found.' :
                            'No approval requests match your current filters.' })] });
        ();
        filteredAndSortedRequests.map((request) => ()
            < div, key = { request, : .id }, className = "p-6 hover:bg-gray-50" >
            (_jsx("div", { className: "flex items-start justify-between", children: _jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsxs("button", { onClick: () => toggleRequestExpansion(request.id), className: "text-gray-400 hover:text-gray-600", children: [expandedRequests.has(request.id) ? ()
                                            < ChevronDownIcon : , " className=\"h-4 w-4\" /> ) : ()", _jsx(ChevronUpIcon, { className: "h-4 w-4" }), ")}"] }), _jsx("h3", { className: "text-lg font-medium text-gray-900", children: request.title }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("span", { className: `inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(request.status)}`, children: ["}", getStatusIcon(request.status), _jsx("span", { className: "ml-1", children: request.status })] }), _jsxs("span", { className: `inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getUrgencyColor(request.urgency)}`, children: ["}", request.urgency] }), isOverdue(request) && ()
                                            < span, " className=\"inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800\">", _jsx(ExclamationTriangleIcon, { className: "h-3 w-3 mr-1" }), "Overdue"] }), ")}", request.escalated_at && ()
                                    < span, " className=\"inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800\">", _jsx(BellIcon, { className: "h-3 w-3 mr-1" }), "Escalated"] }), ")}"] }) })
                ,
                    _jsxs("div", { className: "mt-2 flex items-center space-x-6 text-sm text-gray-600", children: [_jsxs("div", { className: "flex items-center space-x-1", children: [_jsx(CalendarIcon, { className: "h-4 w-4" }), _jsxs("span", { children: ["Requested ", new Date(request.requested_at).toLocaleDateString()] })] }), request.due_date && ()
                                < div, " className=\"flex items-center space-x-1\">", _jsx(ClockIcon, { className: "h-4 w-4" }), _jsxs("span", { children: ["Due ", new Date(request.due_date).toLocaleDateString()] })] })));
    }
    _jsxs("div", { className: "flex items-center space-x-1", children: [_jsx(UserGroupIcon, { className: "h-4 w-4" }), _jsxs("span", { children: ["By ", request.requester_id] })] });
    div >
        { /* Progress Bar */}
        < div;
    className = "mt-3" >
        (_jsxs("div", { className: "flex items-center justify-between text-sm", children: [_jsxs("span", { className: "text-gray-600", children: ["Approval Progress: ", request.current_approvals, " of ", request.required_approvals] }), _jsxs("span", { className: "text-gray-600", children: [Math.round(request.approval_percentage), "%"] })] })
            ,
                _jsx("div", { className: "mt-1 w-full bg-gray-200 rounded-full h-2", children: _jsx("div", { className: "bg-blue-600 h-2 rounded-full transition-all duration-300", style: { width: `${request.approval_percentage}%` } }) }));
    div >
    ;
    div >
        { /* Actions */}
    ...
    className = "flex items-center space-x-2" >
        (_jsxs("button", { onClick: () => handleReviewSubmission(request.id, 'approve'), className: "px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700", children: [_jsx(CheckCircleIcon, { className: "h-4 w-4 mr-1 inline" }), "Approve"] })
            ,
                _jsxs("button", { onClick: () => handleReviewSubmission(request.id, 'reject'), className: "px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700", children: [_jsx(XCircleIcon, { className: "h-4 w-4 mr-1 inline" }), "Reject"] }));
    div >
    ;
}
_jsxs("button", { onClick: () => setSelectedRequest(request), className: "px-3 py-1 bg-blue-600 text-white text-sm rounded hover:bg-blue-700", children: [_jsx(EyeIcon, { className: "h-4 w-4 mr-1 inline" }), "View Details"] });
div >
;
div >
    { /* Expanded Details */};
    ...
    className = "mt-4 space-y-4" >
        { request, : .description && ()
                ...
                (_jsx("h4", { className: "text-sm font-medium text-gray-900 mb-2", children: "Description" })
                    ,
                        _jsx("p", { className: "text-sm text-gray-700", children: request.description })),
            div } >
    ;
}
{
