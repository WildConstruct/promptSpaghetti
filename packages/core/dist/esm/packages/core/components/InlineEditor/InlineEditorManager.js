...
;
isActive: boolean;
onNodeUpdate: (nodeId, updates) => void ;
canvasRef ?  : React.RefObject;
export const InlineEditorManager = ({ nodes,
    onNodeUpdate });
canvasRef;
{
    const [editorState, setEditorState] = useState({
        nodeId: null
    }, position, { x: 0, y: 0 }, isActive, false);
}
;
...
const handleNodeUpdate = useCallback((nodeId, updates) => { onNodeUpdate(nodeId, updates); }, [onNodeUpdate]);
...
export const useInlineEditor = () => {
    ...
    const activateEditor = useCallback((nodeId) => { }, setActiveNodeId(nodeId));
}, [];
const deactivateEditor = useCallback(() => { setActiveNodeId(null); }, []);
const isNodeBeingEdited = useCallback((nodeId) => { return activeNodeId === nodeId; }, [activeNodeId]);
return { activeNodeId,
    activateEditor,
    deactivateEditor };
isNodeBeingEdited;
