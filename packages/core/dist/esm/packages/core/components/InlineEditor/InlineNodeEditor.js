...
import { WeightedChoiceEditor, ConcatEditor, VariableEditor, ConditionalEditor, OutputEditor } from './NodeSpecificEditors';
position: {
    x: number;
    y: number;
}
...
maxWidth ?  : number;
maxHeight ?  : number;
export const InlineNodeEditor = ({ node, isActive, onUpdate, onClose, onSubmit, position, maxWidth = 400, maxHeight = 300 }) => {
    ...
    if (!isActive)
        return null;
    return (_jsxs("div", { ref: editorRef, className: "inline-node-editor", style: {
            position: 'absolute',
            left: position.x,
            top: position.y,
            maxWidth,
            ...
            animation: 'slideIn 0.2s ease-out',
                    ...
                    borderBottom: '1px solid #4a5568',
                            ...
                            color: '#e2e8f0',
                            fontWeight: 600,
                        }, children: ["Edit ", node.type, " Node"] }), _jsxs("div", { style: { display: 'flex', gap: 8 }, children: [isDirty && (_jsx("span", { style: {
                                    fontSize: 12,
                                    color: '#f6ad55',
                                    fontStyle: 'italic',
                                }, children: "Modified" })), _jsx("button", { onClick: handleCancel, style: {
                                    background: 'none',
                                    border: 'none',
                                    color: '#a0aec0',
                                    cursor: 'pointer',
                                    ...
                                    width: 20,
                                    height: 20,
                                }, title: "Cancel (Esc)", children: "\u00D7" })] })] }), _jsxs("div", { style: { marginBottom: 16 }, children: [_jsx("label", { style: {
                            display: 'block',
                            marginBottom: 4,
                            fontSize: 12,
                            color: '#e2e8f0',
                            ...
                            color: 'white',
                            fontSize: 14,
                        }, placeholder: "Enter node label" }), errors.label && (_jsx("span", { style: { color: '#e53e3e', fontSize: 12, marginTop: 4 }, children: errors.label }))] }), _jsx(NodeSpecificEditor, { nodeType: node.type, data: localData, onChange: handleInputChange, errors: errors, theme: "cinema" }), _jsxs("div", { style: {
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
                    marginTop: 16,
                ...
                }, children: [_jsx("div", { style: { fontSize: 12, color: '#a0aec0' }, children: "Press \u2318Enter to save, Esc to cancel" }), _jsxs("div", { style: { display: 'flex', gap: 8 }, children: [_jsx("button", { onClick: handleCancel, style: {
                                    padding: '6px 12px',
                                    background: '#718096',
                                    color: 'white',
                                    border: 'none',
...
const NodeSpecificEditor = ({ nodeType, data, onChange, errors, theme = 'cinema' }) => {
