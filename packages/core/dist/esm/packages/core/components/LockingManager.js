...
if (isLoading) {
    return;
    _jsxs("div", { className: "flex items-center justify-center p-8", children: [_jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" }), _jsx("span", { className: "ml-2 text-gray-600", children: "Loading lock information..." })] });
    ;
    return;
...
`, children: "Policy" })] }) });
    { /* Error Display */ }
    {
        error && ()
            < div;
        className = "bg-red-50 border border-red-200 rounded-md p-4 m-4" >
            _jsxs("div", { className: "flex items-center", children: [_jsx(AlertTriangle, { className: "h-5 w-5 text-red-400 mr-2" }), _jsx("span", { className: "text-red-700", children: error }), _jsx("button", { onClick: clearError, className: "ml-auto text-red-400 hover:text-red-600", children: _jsx(X, { className: "h-4 w-4" }) })] });
        div >
        ;
    }
    { /* Content */ }
    _jsxs("div", { className: "p-6", children: [activeTab === 'overview' && ()
                < LockStatusOverview, "statistics=", statistics, "conflicts=", conflicts, "onConflictClick=", (conflict) => {
                setSelectedResource(conflict.resource_id);
                setShowBreakingWorkflow(true);
            }, "/> )}", activeTab === 'locks' && ()
                < div, " className=\"space-y-4\">", _jsxs("div", { className: "flex items-center space-x-4 bg-gray-50 p-4 rounded-lg", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(Search, { className: "h-4 w-4 text-gray-400" }), _jsx("input", { type: "text", placeholder: "Search locks...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: "border border-gray-300 rounded px-3 py-1 text-sm" })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(Filter, { className: "h-4 w-4 text-gray-400" }), _jsxs("select", { value: lockTypeFilter, onChange: (e) => setLockTypeFilter(e.target.value), className: "border border-gray-300 rounded px-3 py-1 text-sm", children: [_jsx("option", { value: "all", children: "All Types" }), _jsx("option", { value: "edit", children: "Edit" }), _jsx("option", { value: "state_change", children: "State Change" }), _jsx("option", { value: "delete", children: "Delete" }), _jsx("option", { value: "admin", children: "Admin" })] })] }), _jsxs("label", { className: "flex items-center space-x-2", children: [_jsx("input", { type: "checkbox", checked: showExpiredLocks, onChange: (e) => setShowExpiredLocks(e.target.checked), className: "rounded border-gray-300" }), _jsx("span", { className: "text-sm text-gray-700", children: "Show expired" })] })] }), _jsxs("div", { className: "space-y-2", children: [filteredLocks.map((lock) => ()
                        < div, key = { lock, : .id }, className = "border border-gray-200 rounded-lg p-4" >
                        (_jsx("div", { className: "flex items-center justify-between", children: _jsxs("div", { className: "flex items-center space-x-3", children: [_jsx(LockIndicator, { lock: lock, size: "small", showTooltip: false }), _jsxs("div", { children: [_jsxs("p", { className: "font-medium text-gray-900", children: ["Resource: ", lock.resource_id.substring(0, 8), "..."] }), _jsxs("p", { className: "text-sm text-gray-500", children: ["Type: ", lock.lock_type, " \u2022 By: ", lock.locked_by] }), lock.lock_reason && ()
                                                < p, " className=\"text-sm text-gray-600 mt-1\"> Reason: ", lock.lock_reason] }), ")}"] }) })
                            ,
                                _jsxs("div", { className: "flex items-center space-x-2", children: [lock.expires_at && ()
                                            < span, " className=\"text-xs text-gray-500\"> Expires: ", new Date(lock.expires_at).toLocaleString()] }))), lock.locked_by === userId ? ()
                        < button
                        :
                    , "onClick=", () => handleLockRelease(lock.id, lock.resource_id), "className=\"px-2 py-1 bg-green-100 text-green-700 rounded text-xs hover:bg-green-200\" >", _jsx(Unlock, { className: "h-3 w-3" }), "Release"] }), ") : ()", _jsxs("button", { onClick: () => {
                    setSelectedResource(lock.resource_id);
                    setShowBreakingWorkflow(true);
                }, className: "px-2 py-1 bg-red-100 text-red-700 rounded text-xs hover:bg-red-200", children: [_jsx(AlertTriangle, { className: "h-3 w-3" }), "Break"] }), ")}"] });
    div >
    ;
    div >
    ;
