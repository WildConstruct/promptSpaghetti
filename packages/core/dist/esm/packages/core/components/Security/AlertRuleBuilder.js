...
description = "Are you sure you want to delete this alert rule? This action cannot be undone."
    >
        _jsxs("div", { className: "dialog-actions", children: [_jsx(Button, { variant: "outline", onClick: () => setShowDeleteDialog(null), children: "Cancel" }), _jsx(Button, { variant: "destructive", onClick: () => handleDeleteRule(showDeleteDialog), children: "Delete Rule" })] });
Dialog >
;
div >
;
...
({ rule, open, onClose, onSave }) => {
    ...
    const handleSave = () => {
        ...
        return;
        _jsx(Dialog, { open: open, onOpenChange: onClose, title: rule.id ? 'Edit Alert Rule' : 'Create Alert Rule', className: "rule-edit-dialog", children: _jsxs("div", { className: "dialog-content", children: [errors.length > 0 && ()
                        < Alert, " variant=\"error\" className=\"mb-4\">", _jsx("strong", { children: "Validation Errors:" }), _jsx("ul", { children: errors.map((error, index) => ()
                            < li, key = { index } > { error }) }), "))}"] }) });
    };
};
_jsxs("div", { className: "edit-tabs", children: [_jsxs("div", { className: "tabs-list", children: [['basic', 'conditions', 'actions', 'notifications'].map(tab => ()
                    < button, key = { tab }, className = {} `tab-trigger ${activeTab === tab ? 'active' : ''}`), "onClick=", () => setActiveTab(tab), ">", tab.charAt(0).toUpperCase() + tab.slice(1)] }), "))}"] })
    ,
        _jsxs("div", { className: "tab-content", children: [activeTab === 'basic' && ()
                    < BasicRuleSettings, " rule=", editedRule, " onUpdate=", updateRule, " /> )}", activeTab === 'conditions' && ()
                    < ConditionBuilder, " rule=", editedRule, " onUpdate=", updateRule, " /> )}", activeTab === 'actions' && ()
                    < ActionBuilder, " rule=", editedRule, " onUpdate=", updateRule, " /> )}", activeTab === 'notifications' && ()
                    < NotificationSettings, " rule=", editedRule, " onUpdate=", updateRule, " /> )}"] });
div >
    _jsxs("div", { className: "dialog-actions", children: [_jsx(Button, { variant: "outline", onClick: onClose, children: "Cancel" }), _jsx(Button, { variant: "primary", onClick: handleSave, children: "Save Rule" })] });
div >
;
Dialog >
;
 ...
 */
const ConditionBuilder, AlertRule;
onUpdate: (updates) => void ;
 > ;
({ rule, onUpdate }) => {
    const addCondition = () => {
...
return;
_jsxs("div", { className: "condition-builder", children: [_jsxs("div", { className: "builder-header", children: [_jsx("p", { children: "Define conditions that must be met to trigger this alert rule." }), _jsx(Button, { variant: "outline", onClick: addCondition, children: "Add Condition" })] }), rule.conditions.map((condition, index) => ()
            < div, key = { condition, : .id }, className = "condition-item" >
            { index } > 0 && ()
            < div, className = "logic-operator" >
            _jsxs(Select, { value: condition.logic_operator || 'and', onValueChange: (value) => updateCondition(condition.id, { logic_operator: value }), children: [_jsx("option", { value: "and", children: "AND" }), _jsx("option", { value: "or", children: "OR" })] }))] });
_jsxs("div", { className: "condition-config", children: [_jsx(Select, { value: condition.field, onValueChange: (value) => updateCondition(condition.id, { field: value }), children: AVAILABLE_FIELDS.map(field => ()
                < option, key = { field, : .value }, value = { field, : .value } >
                { field, : .label }) }), "))}"] })
    ,
        _jsx(Select, { value: condition.operator, onValueChange: (value) => updateCondition(condition.id, { operator: value }), children: getOperatorsForField(condition.field).map(op => ()
                < option, key = { op }, value = { op } >
                {}) });
Select >
    (_jsx(Input, { value: condition.value, onChange: (e) => updateCondition(condition.id, { value: e.target.value }), placeholder: "Value" })
        ,
            _jsx(Button, { variant: "outline", size: "sm", onClick: () => removeCondition(condition.id), children: "Remove" }));
div >
;
div >
;
{
    ...
    className = "empty-conditions" >
        _jsx("p", { children: "No conditions defined. Add conditions to specify when this rule should trigger." });
    div >
    ;
}
div >
...
return;
_jsx("div", { className: "action-builder", children: _jsxs("div", { className: "builder-header", children: [_jsx("p", { children: "Configure actions to take when this alert rule is triggered." }), _jsxs("div", { className: "action-types", children: [ACTION_TYPES.map(actionType => ()
                        < Button, key = { actionType, : .type }, variant = "outline", size = "sm", onClick = {}()), " => addAction(actionType.type)} >", actionType.icon, " ", actionType.name] }), "))}"] }) })
    ,
        _jsx("div", { className: "actions-list", children: rule.actions.map(action => ()
                < Card, key = { action, : .id }, className = "action-card" >
                (_jsxs("div", { className: "action-header", children: [_jsxs("div", { className: "action-info", children: [_jsx("span", { className: "action-icon", children: ACTION_TYPES.find(t => t.type === action.type)?.icon }), _jsx("span", { className: "action-name", children: action.name }), _jsx(Badge, { variant: action.enabled ? 'success' : 'secondary', children: action.enabled ? 'Enabled' : 'Disabled' })] }), _jsxs("div", { className: "action-controls", children: [_jsx(Checkbox, { checked: action.enabled, onChange: (checked) => updateAction(action.id, { enabled: checked }) }), _jsx(Button, { variant: "outline", size: "sm", onClick: () => removeAction(action.id), children: "Remove" })] })] })
                    ,
                        _jsx("div", { className: "action-config", children: _jsx(ActionConfigForm, { action: action, onUpdate: (updates) => updateAction(action.id, updates) }) }))) });
div >
    { rule, : .actions.length === 0 && ()
            < div, className = "empty-actions" >
            _jsx("p", { children: "No actions defined. Add actions to specify what should happen when this rule triggers." }),
        div } >
;
div >
;
 ...
 */
const ActionConfigForm, AlertAction;
onUpdate: (updates) => void ;
 > ;
({ action, onUpdate }) => {
...
switch (action.type) {
    case 'notification':
        return;
        _jsx("div", { className: "config-form", children: _jsxs("div", { className: "form-group", children: [_jsx("label", { children: "Message Template" }), _jsx(Textarea, { value: action.config.message_template || '', onChange: (e) => updateConfig('message_template', e.target.value), placeholder: "Alert: {event_type} detected from {source_ip}" })] }) });
        ;
    case 'containment':
        return;
 ...
 */
const NotificationSettings, AlertRule;
onUpdate: (updates) => void ;
 > ;
({ rule, onUpdate }) => {
    return;
