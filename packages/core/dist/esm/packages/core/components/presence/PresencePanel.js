...
const getActivityText = (user) => {
    if (user.isTyping) {
        ...
        if (!showDetailedView) {
            ...
            return;
            _jsxs("div", { className: `flex items-center space-x-2 ${className}`, children: ["}", _jsx(UserAvatarList, { users: users, maxVisible: maxAvatars, onUserClick: onUserClick }), typingUsers.length > 0 && ()
                        < div, " className=\"flex items-center space-x-1 text-xs text-gray-500\">", _jsxs("div", { className: "flex space-x-1", children: [_jsx("div", { className: "w-1 h-1 bg-gray-400 rounded-full animate-bounce" }), _jsx("div", { className: "w-1 h-1 bg-gray-400 rounded-full animate-bounce", style: { animationDelay: '0.1s' } }), _jsx("div", { className: "w-1 h-1 bg-gray-400 rounded-full animate-bounce", style: { animationDelay: '0.2s' } })] }), _jsx("span", { children: typingUsers.length })] });
        }
    }
};
div >
...
return;
_jsxs("div", { className: `bg-white rounded-lg shadow-lg border ${className}`, children: ["}", _jsxs("div", { className: "flex items-center justify-between p-3 border-b cursor-pointer hover:bg-gray-50", onClick: () => setExpanded(!expanded), children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsxs("h3", { className: "font-medium text-gray-900", children: ["Collaborators (", users.length, ")"] }), _jsx(UserAvatarList, { users: users.slice(0, 3), size: "small", maxVisible: 3, showOverflow: false })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [typingUsers.length > 0 && ()
...
{ /* Expanded content */ }
{
        ...
        { /* Active users */};
    {
        activeUsers.length > 0 && ()
            < div >
            (_jsxs("h4", { className: "text-xs font-medium text-gray-500 uppercase tracking-wide mb-2", children: ["Active (", activeUsers.length, ")"] })
                ,
                    _jsxs("div", { className: "space-y-2", children: [activeUsers.map(user => ()
                                < UserPresenceItem, key = { user, : .userId }, user = { user }, isCurrentUser = { user, : .userId === currentUserId }, isFollowing = { followingUserId } === user.userId), "onUserClick=", onUserClick, "onFollowUser=", onFollowUser, "onUnfollowUser=", onUnfollowUser, "formatLastSeen=", formatLastSeen, "getActivityText=", getActivityText, "/> ))}"] }));
        div >
        ;
    }
    { /* Idle/Away users */ }
    {
        (idleUsers.length > 0 || awayUsers.length > 0) && ()
            < div >
            (_jsxs("h4", { className: "text-xs font-medium text-gray-500 uppercase tracking-wide mb-2", children: ["Away (", idleUsers.length + awayUsers.length, ")"] })
                ,
                    _jsxs("div", { className: "space-y-2", children: [[...idleUsers, ...awayUsers].map(user => ()
                                < UserPresenceItem, key = { user, : .userId }, user = { user }, isCurrentUser = { user, : .userId === currentUserId }, isFollowing = { followingUserId } === user.userId), "onUserClick=", onUserClick, "onFollowUser=", onFollowUser, "onUnfollowUser=", onUnfollowUser, "formatLastSeen=", formatLastSeen, "getActivityText=", getActivityText, "/> ))}"] }));
        div >
        ;
    }
    { /* Following indicator */ }
    {
        ...
        className = "pt-2 border-t" >
            _jsxs("div", { className: "flex items-center justify-between text-sm", children: [_jsxs("span", { className: "text-gray-600", children: ["Following ", users.find(u => u.userId === followingUserId)?.userName || followingUserId] }), _jsx("button", { onClick: onUnfollowUser, className: "text-blue-600 hover:text-blue-800 font-medium", children: "Unfollow" })] });
        div >
        ;
    }
    div >
