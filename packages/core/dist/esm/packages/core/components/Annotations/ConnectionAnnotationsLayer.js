...
if (!visible)
    return null;
return;
_jsxs("div", { ref: layerRef, "data-testid": "connection-annotations-layer", style: {
        position: 'absolute',
        inset: 0,
        pointerEvents: 'none',
        zIndex: 50,
            ...
            ` })] }), edges.map(edge => { }), "const pathData = getConnectionPath(edge.id); const annotation = annotations.connectionAnnotations?.find(a => a.connectionId === edge.id); const isHighlighted = highlightedConnection === edge.id; if (!pathData || !annotation) return null; return;", _jsxs("g", { children: ["}", _jsx("path", { d: pathData.path, stroke: annotation.color || '#6b7280', strokeWidth: annotation.strokeWidth || 2, strokeOpacity: annotation.opacity || 1, strokeDasharray: annotation.visualStyle === 'dashed' ? '8 4' :
