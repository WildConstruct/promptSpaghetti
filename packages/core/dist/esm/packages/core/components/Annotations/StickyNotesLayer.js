...
author ?  : string;
readOnly ?  : boolean;
export const layerRef = useRef(null);
        ...
        'move';
        {
            ...
            'resize';
            {
                ...
                'startEdit';
                {
                    if (!noteId)
                        break;
                    const updatedNotes = notes.map(note => ({}), ...note, isEditing, note.id === noteId);
                }
            }
            ;
            onNotesChange(updatedNotes);
...
const handleNoteClick = useCallback((noteId) => { setSelectedNoteId(noteId); }, []);
...
return;
_jsx(_Fragment, { children: _jsxs("div", { ref: layerRef, className: "sticky-notes-layer", style: {
            position: 'absolute',
            top: 0,
            left: 0,
            ...
            zIndex: 1000,
            overflow: 'hidden'
        }, onDoubleClick: handleCanvasDoubleClick, onClick: handleLayerClick, children: [_jsxs("div", { style: { pointerEvents: 'auto' }, children: [notes.map(note => ()
                        < div, key = { note, : .id }, onClick = {}()), " => handleNoteClick(note.id)} >", _jsx(StickyNote, { note: note, onAction: handleNoteAction, onContextMenu: handleContextMenu, selected: selectedNoteId === note.id, canEdit: !readOnly, canDelete: !readOnly, canMove: !readOnly, canResize: !readOnly })] }), "))}"] }) });
div >
    { /* Context Menu */};
{
    contextMenu && ()
