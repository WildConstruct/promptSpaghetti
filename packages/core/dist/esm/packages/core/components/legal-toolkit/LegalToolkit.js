...
onWorkflowComplete;
{
    ...
    const handleAnalysisComplete = (contractAnalysis) => { setAnalysis(contractAnalysis); };
    const handleComplianceResults = (results) => { setComplianceResults(results); };
    const handleCitationAdd = (citation) => { setCitations(prev => [...prev, citation]); };
    const handleCitationEdit = (id, updatedCitation) => { setCitations(prev => prev.map(c => c.id === id ? updatedCitation : c)); };
    const handleCitationDelete = (id) => { setCitations(prev => prev.filter(c => c.id !== id)); };
    const handleTerminologyResults = (results) => { setTerminologyResults(results); };
            ...
            const getHeaderTitle = (view) => {
                ...
                const isViewEnabled = (view) => {
                    ...
                    const getViewCompletionStatus = (view) => {
                        ...
                        ;
                        return;
                        _jsxs("div", { className: `legal-toolkit ${className}`, children: ["}", _jsx("style", { children: `
          .legal-toolkit {
            background: white;
            border-radius: 12px;
...
;
div >
    _jsxs("div", { className: "workflow-actions", children: [_jsx("div", { className: "workflow-info", children: workflowProgress < 100
                    ? 'Complete all sections to finish the legal document workflow'
                    : 'Legal document analysis complete - ready to export results' }), _jsxs("div", { className: "workflow-buttons", children: [workflowProgress >= 100 && ()
                        < button, "className=\"workflow-btn primary\" onClick=", handleCompleteWorkflow, "> Export Results"] }), ")}", _jsx("button", { className: "workflow-btn secondary", onClick: () => setCurrentView('upload'), children: "Start New Document" })] });
div >
;
div >
;
