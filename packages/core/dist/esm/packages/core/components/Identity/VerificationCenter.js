...
});
{
    ...
    ;
    _jsxs(Card, { className: "verification-steps", children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Verification Steps" }) }), _jsxs(CardContent, { children: [_jsxs("div", { className: "steps-list", children: [recommendedSteps.map((step, index) => ()
                                < div, key = { step, : .type }, className = {} `step-item ${activeStep === step.type ? 'active' : ''}`), ">}", _jsxs("div", { className: "step-header", children: [_jsx("div", { className: "step-icon", children: getVerificationIcon(step.type, 'incomplete') }), _jsxs("div", { className: "step-info", children: [_jsx("div", { className: "step-title", children: step.title }), _jsx("div", { className: "step-description", children: step.description })] }), _jsxs("div", { className: "step-actions", children: [_jsxs(Badge, { variant: step.priority === 'high' ? 'destructive' :
                                                    step.priority === 'medium' ? 'default' : 'secondary', children: [step.priority, " priority"] }), _jsx(Button, { onClick: () => setActiveStep(step.type), variant: "outline", size: "sm", children: "Start Verification" })] })] }), activeStep === step.type && ()
                                < div, " className=\"step-form\">", renderVerificationForm(step.type)] }), ")}"] }), "))}"] });
    CardContent >
    ;
    Card >
    ;
    ...
    ;
    return;
    _jsxs("div", { className: "verification-form", children: [_jsxs("p", { children: ["Verification form for ", type, " is coming soon."] }), _jsx(Button, { variant: "outline", onClick: () => setActiveStep(null), children: "Close" })] });
    ;
}
;
const renderValidationHistory = () => ();
;
_jsxs(Card, { className: "validation-history", children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Verification History" }) }), _jsxs(CardContent, { children: [validationSummary && ()
                    < div, " className=\"history-summary\">", _jsxs("div", { className: "summary-stats", children: [_jsxs("div", { className: "stat-item", children: [_jsx("span", { className: "stat-label", children: "Total Requests" }), _jsx("span", { className: "stat-value", children: validationSummary.totalRequests })] }), _jsxs("div", { className: "stat-item", children: [_jsx("span", { className: "stat-label", children: "Approved" }), _jsx("span", { className: "stat-value text-green-600", children: validationSummary.approvedCount })] }), _jsxs("div", { className: "stat-item", children: [_jsx("span", { className: "stat-label", children: "Pending" }), _jsx("span", { className: "stat-value text-yellow-600", children: validationSummary.pendingCount })] }), _jsxs("div", { className: "stat-item", children: [_jsx("span", { className: "stat-label", children: "Rejected" }), _jsx("span", { className: "stat-value text-red-600", children: validationSummary.rejectedCount })] })] }), userTrustScore?.badges && userTrustScore.badges.length > 0 && ()
                    < div, " className=\"earned-badges\">", _jsx("h4", { children: "Earned Badges" }), _jsx("div", { className: "badges-grid", children: userTrustScore.badges.map((badge, index) => ()
                        < Badge, key = { index }, variant = "default", className = "badge-item" >
                        _jsx(Award, { className: "w-3 h-3 mr-1" }), { badge, : .replace('_', ' ').toUpperCase() }) }), "))}"] })] });
div >
;
CardContent >
;
...
if (isLoading) {
    return;
    _jsxs("div", { className: "verification-center loading", children: [_jsx("div", { className: "loading-spinner" }), _jsx("p", { children: "Loading verification data..." })] });
    ;
    return;
