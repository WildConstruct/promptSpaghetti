...
if (deadNodes.length > 0) {
    ...
    type: 'dead_code',
        severity;
    deadNodes.length > 5 ? 'high' : 'medium';
}
description: `Remove ${deadNodes.length} unused node${deadNodes.length > 1 ? 's' : ''}`;
nodeIds: deadNodes,
    estimatedImprovement;
`${Math.round(deadNodes.length / nodes.length * 100)}% size reduction`;
;
...
nodeIds: constantNodes,
    estimatedImprovement;
`${constantNodes.length * 10}ms faster execution`;
;
...
nodeIds: parallelizableNodes,
    estimatedImprovement;
`${Math.round(parallelizableNodes.length / 2)}x faster execution`;
;
if (nodes.length > 20) {
    optimizationOpportunities.push({});
    type: 'caching',
    ...
    'medium',
        description;
    'Enable result caching for repeated executions',
        estimatedImprovement;
    '50-90% faster on repeat runs';
}
;
if (complexity === 'high') {
    optimizationOpportunities.push({});
    ...
    'high',
        description;
    'Large graph detected - enable memory optimization',
        estimatedImprovement;
...
;
export const GraphAnalysisPanel = ({
    nodes,
    edges,
    isOpen });
onClose;
        ...
        _jsxs("div", { style: {
                ...
                zIndex: 1000
                        ...
                        marginBottom: '24px'
                                ...
                                fontWeight: '600',
                                color: '#333'
                            }, children: "\uD83D\uDE80 Optimization Opportunities" }), analysis.optimizationOpportunities.length === 0 ? ()
                            < div : , " style=", {
                            padding: '16px',
                            ...
                            borderRadius: '4px',
                            color: '#155724'
                        }, "> \u2705 Your graph is already well-optimized! No major improvements detected."] }), ") : ()", _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '12px' }, children: [analysis.optimizationOpportunities.map((opportunity, index) => ()
                            < div, key = { index }, style = {}, {
                            padding: '16px'
                        }, border), ": `1px solid $", getSeverityColor(opportunity.severity), "33`} backgroundColor: `$", getSeverityColor(opportunity.severity), "11`} borderRadius: '8px'; >", _jsxs("div", { style: {
                                display: 'flex',
                                justifyContent: 'space-between',
                                ...
                                fontWeight: '500',
                                marginBottom: '8px'
                            }, children: ["\uD83D\uDCA1 Estimated improvement: ", opportunity.estimatedImprovement] }), opportunity.nodeIds && ()
                            < div, " style=", {
                            fontSize: '12px',
                            color: '#6c757d',
                            fontFamily: 'monospace'
                        }, "> Affected nodes: ", opportunity.nodeIds.slice(0, 5).join(', '), opportunity.nodeIds.length > 5 && ` (+${opportunity.nodeIds.length - 5} more)`] }), ")}"] });
    };
}
div >
;
...
style = {};
{
 ...
 >
    _jsx("button", { onClick: onClose, style: {
            ...
            >
                Apply, Optimizations: true, button: true });
div >
;
div >
;
