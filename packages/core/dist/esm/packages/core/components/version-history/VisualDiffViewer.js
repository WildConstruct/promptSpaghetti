...
});
{
    ...
    const diffEngine = useRef(new GraphDiffEngine({}), ignore_position_changes, false, ignore_style_changes, false, deep_property_comparison, true);
}
;
useEffect(() => {
    if (isOpen && !diff) {
...
className = "text-center py-8 text-gray-500" >
    (_jsx("div", { className: "text-4xl mb-2", children: "\uD83D\uDD0D" })
        ,
            _jsx("h4", { className: "font-medium text-gray-900 mb-1", children: "No changes found" })
                ,
                    _jsx("p", { className: "text-sm", children: "Try adjusting your filters to see different types of changes." }));
div >
;
()
    < div;
className = "divide-y divide-gray-100" >
    { filteredChanges, : .map((change, index) => ()
            < ChangeItem, key = {} `${change.element_id}-${index}`) };
change = { change };
isSelected = { selectedChange } === change.element_id;
onClick = {}();
handleChangeClick(change.element_id);
onApply = { onApplyChange(change) { }, : .element_id, undefined };
onReject = { onRejectChange(change) { }, : .element_id, undefined };
...
if (!isOpen)
    return null;
return;
_jsxs("div", { className: `visual-diff-viewer ${className} fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4`, children: ["}", _jsx("div", { className: "bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col", children: _jsxs("div", { className: "p-4 border-b border-gray-200", children: [_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsx("h2", { className: "text-xl font-semibold text-gray-900", children: "Visual Diff Viewer" }), _jsx("button", { onClick: onClose, className: "text-gray-400 hover:text-gray-500 transition-colors", children: _jsx("svg", { className: "h-6 w-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) }) })] }), _jsxs("div", { className: "flex space-x-1 bg-gray-100 rounded-lg p-1", children: [[
...
_jsxs("div", { className: `p-3 cursor-pointer transition-colors ${isSelected ? 'bg-blue-50 border-l-4 border-l-blue-500' : 'hover:bg-gray-50',
            ...
            < button, "onClick=", (e) => { e.stopPropagation(); onReject(); }, "className=\"text-xs px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors\" > Reject"] });
div >
;
div >
;
...
const SideBySideView = ({ _____fromGraph, _____toGraph, _____diff, _____selectedChange, _____showRegions, _____showPaths, _____zoom, _____pan, _____onZoomChange, _____onPanChange }) => ()
    < div, className = "h-full flex" >
    (_jsxs("div", { className: "flex-1 border-r border-gray-300 bg-white", children: [_jsx("div", { className: "h-8 bg-gray-100 border-b border-gray-300 flex items-center px-3 text-sm font-medium text-gray-700", children: "Original Version" }), _jsx("div", { className: "h-full bg-gray-50 flex items-center justify-center text-gray-500", children: "Graph Visualization (Original)" })] })
        ,
            _jsxs("div", { className: "flex-1 bg-white", children: [_jsx("div", { className: "h-8 bg-gray-100 border-b border-gray-300 flex items-center px-3 text-sm font-medium text-gray-700", children: "New Version" }), _jsx("div", { className: "h-full bg-gray-50 flex items-center justify-center text-gray-500", children: "Graph Visualization (New)" })] }));
div >
;
;
const OverlayView = ({ _____fromGraph, _____toGraph, _____diff, _____selectedChange, _____showRegions, _____showPaths, _____zoom, _____pan, _____onZoomChange, _____onPanChange }) => ()
    < div, className = "h-full bg-white" >
    (_jsx("div", { className: "h-8 bg-gray-100 border-b border-gray-300 flex items-center px-3 text-sm font-medium text-gray-700", children: "Overlay View" })
        ,
            _jsx("div", { className: "h-full bg-gray-50 flex items-center justify-center text-gray-500", children: "Graph Visualization (Overlay with Changes)" }));
div >
;
;
const ChangesOnlyView = ({ _____diff, filteredChanges, _____selectedChange, getChangeColor, getChangeIcon }) => ()
    < div, className = "h-full bg-white p-4" >
    (_jsx("h3", { className: "text-lg font-medium text-gray-900 mb-4", children: "Changes Summary" })
        ,
            _jsx("div", { className: "space-y-4", children: filteredChanges.map((change, index) => ()
