...
export class GraphDiffEngine {
    static POSITION_THRESHOLD = 10; // pixels
    static SIMILARITY_THRESHOLD = 0.95;
    ignore_position_changes;
    ignore_style_changes;
    position_threshold;
    deep_property_comparison;
}
{ }
{ }
// Main diff computation method
async;
computeDiff(fromGraph, GraphData, toGraph, GraphData);
Promise < GraphDiff > {
...
return diff;
detectNodeChanges(fromNodes, (Map), toNodes, (Map));
DiffChange;
{
    const changes = [];
    // Find added nodes
    for (const [nodeId, node] of toNodes) {
        if (!fromNodes.has(nodeId)) {
            ...
            node,
                significance;
            this.calculateSignificance('added', 'node', node),
            ;
        }
        ;
        // Find removed nodes
        for (const [nodeId, node] of fromNodes) {
            if (!toNodes.has(nodeId)) {
                ...
                node,
                    significance;
                this.calculateSignificance('removed', 'node', node),
                ;
            }
            ;
            // Find modified nodes
            for (const [nodeId, fromNode] of fromNodes) {
                ...
                return changes;
                detectEdgeChanges(fromEdges, (Map), toEdges, (Map));
                DiffChange;
                {
                    const changes = [];
                    // Find added edges
                    for (const [edgeId, edge] of toEdges) {
                        if (!fromEdges.has(edgeId)) {
                            ...
                            edge,
                                significance;
                            this.calculateSignificance('added', 'edge', edge),
                            ;
                        }
                        ;
                        // Find removed edges
                        for (const [edgeId, edge] of fromEdges) {
                            if (!toEdges.has(edgeId)) {
                                ...
                                edge,
                                    significance;
                                this.calculateSignificance('removed', 'edge', edge),
                                ;
                            }
                            ;
                            // Find modified edges
                            for (const [edgeId, fromEdge] of fromEdges) {
                                ...
                                return changes;
                                compareNodes(fromNode, GraphNode, toNode, GraphNode);
                                DiffChange;
                                {
                                    const changes = [];
                                ...
                                if (!this.options.ignore_style_changes) {
                                    ...
                                    fromNode.id,
                                        significance;
                                    Math.min(change.significance, 0.3); // Style changes are less significant,
                                }
                                ;
                            }
                            ;
                            ...
                            if (fromNode.type !== toNode.type) {
                                ...
                                toNode.type,
                                    significance;
                                0.8; // Type changes are significant,
                            }
                            ;
                            return changes;
                            compareEdges(fromEdge, GraphEdge, toEdge, GraphEdge);
                            DiffChange;
                            {
                                const changes = [];
                                ...
                                if (fromEdge.source !== toEdge.source || fromEdge.target !== toEdge.target) {
                                    ...
                                    'connection',
                                        old_value;
                                    {
                                        source: fromEdge.source, target;
                                        fromEdge.target;
                                    }
                                    new_value: {
                                        source: toEdge.source, target;
                                        toEdge.target;
                                    }
                        ...
                        Array < Omit < DiffChange, 'element_type' | 'element_id' >> {
                            const: changes
                        } < DiffChange, 'element_type' | 'element_id' >> ;
                        [];
                        // Get all unique keys
                        ...
                        if (fromValue === undefined && toValue !== undefined) {
                            ...
                            toValue,
                                significance;
                            this.calculatePropertySignificance(key, toValue),
                            ;
                        }
                        ;
                    }
                    if (fromValue !== undefined && toValue === undefined) {
                        ...
                        fromValue,
                            significance;
                        this.calculatePropertySignificance(key, fromValue),
                        ;
                    }
                    ;
                }
                if (fromValue !== toValue) {
                    ...
                    }
                    {
                        ...
                        toValue,
                            significance;
                        this.calculatePropertySignificance(key, { from: fromValue, to: toValue });
                    }
                    ;
                    return changes;
                    calculateDistance(from, { x: number, y: number }, to, { x: number, y: number });
                    number;
                    {
                        return Math.sqrt(Math.pow(to.x - from.x, 2) + Math.pow(to.y - from.y, 2));
                        calculateSignificance(changeType, string, elementType, string, element, HTMLElement);
                        number;
                        {
                                ...
                                case 'moved':
                                    ...
                                    if (elementType === 'edge') {
                                        ...
                                        if (element?.data?.importance) {
                                            ...
                                            calculatePropertySignificance(propertyName, string, value, Error);
                                            number;
                                            {
                                                ...
                                                if (typeof value === 'object' && value !== null) {
                                                    ...
                                                    return baseSig;
                                                    calculateSummary(changes, DiffChange, fromGraph, GraphData, toGraph, GraphData);
                                                    {
                                                        const summary = {
                                                            ...
                                                            if (change.element_type === 'node') {
                                                                switch (change.type) {
                                                                    ...
                                                                    case 'removed':
                                                                        ...
                                                                        break;
                                                                    case 'modified':
                                                                        summary.modified_nodes++;
                                                                        break;
                                                                    case 'moved':
                                                                        summary.moved_nodes++;
                                                                        break;
                                                                }
                                                                if (change.element_type === 'edge') {
                                                                    switch (change.type) {
                                                                        case 'added':
                                                                            summary.added_edges++;
                                                                            break;
                                                                        case 'removed':
                                                                            ...
                                                                            break;
                                                                        case 'modified':
                                                                            summary.modified_edges++;
                                                                            break;
                                                                    }
                                                                    if (change.element_type === 'property') {
