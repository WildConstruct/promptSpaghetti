                    ...
                    overflow: 'hidden',
                            ...
                            overflow: 'hidden',
                                    ...
                                    color: styles.textSecondary
                                                ...
                                                textDecoration: 'none',
                                            }, children: crumb.name })] }, crumb.path))) }), _jsxs("div", { style: {
                                    flex: 1,
                                    padding: '16px',
                                    overflow: 'auto',
                                            ...
                                            color: styles.textSecondary,
                                            textAlign: 'center',
                                        }, children: [_jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\uD83D\uDCC1" }), _jsx("div", { style: { fontSize: '16px', marginBottom: '8px' }, children: "No projects found" }), _jsx("div", { style: { fontSize: '14px' }, children: viewState.filterText ? 'Try adjusting your search terms' : 'Create your first project to get started' })] })) : ()
                                        < div, " style=", ({
                                        display: 'grid',
                                        ...
                                        : '1fr',
                                        gap), ": viewState.viewMode === 'grid' ? '16px' : '4px', }}>", filteredAndSortedFiles.map(file => ()
                                        < div, key = { file, : .id }, onClick = {}()), " => handleFileSelect(file)} onDoubleClick=", () => handleFileDoubleClick(file), "onContextMenu=", (e) => handleFileContextMenu(e, file), "style=", {
                                        ...
                                        if (selectedFile?.id !== file.id) {
                                            ...
                                            overflow: 'hidden',
                                                    ...
                                                    textOverflow: 'ellipsis',
                                                    whiteSpace: 'nowrap',
                                                }, children: [file.metadata.title || file.name.replace('.psg', ''), file.isFavorite && _jsx("span", { style: { marginLeft: '4px' }, children: "\u2B50" })] }), viewState.viewMode !== 'list' && ()
                                                < div, " style=", {
                                                fontSize: '12px',
                                                color: styles.textSecondary,
                                                marginBottom: '4px',
                                            }, ">", file.nodeCount, " nodes \u2022 ", formatFileSize(file.size)] }), ")}", _jsx("div", { style: {
                                            fontSize: '11px',
                                            color: styles.textSecondary,
                                        }, children: formatDate(file.lastModified) })] }), viewState.viewMode === 'list' && ()
                                < div, " style=", {
                                display: 'flex',
                                alignItems: 'center',
                                gap: '16px',
                                ...
                                color: styles.textSecondary,
                                minWidth: '200px',
                            }, ">", _jsxs("span", { children: [file.nodeCount, " nodes"] }), _jsx("span", { children: formatFileSize(file.size) })] }), ")}"] }), "))}"] }));
};
div >
;
div >
...
e.stopPropagation();
    >
        { contextMenuActions, : .map(action => (), action.separator ? ()
                < div
                :
            , key = { action, : .id }, style = {}, {
                height: '1px',
...
style = {};
{
    ...
    'none',
        color;
    action.destructive ? '#ef4444' : styles.text,
        fontSize;
    '13px',
        textAlign;
    'left',
...
e.currentTarget.style.backgroundColor = 'transparent';
    >
        _jsx("span", { children: action.icon });
{
    action.label;
}
button >
...
_jsx("input", { ref: fileInputRef, type: "file", accept: ".psg", style: { display: 'none' }, onChange: (e) => {
        // Handle file upload
