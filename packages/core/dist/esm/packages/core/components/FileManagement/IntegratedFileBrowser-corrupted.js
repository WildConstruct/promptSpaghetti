...
showHidden: boolean;
filterText: string;
export const IntegratedFileBrowser = ({ onFileSelected,
    onProjectLoad,
    onNewProject,
    onFileAction,
    theme = 'cinema',
...
currentProject;
{
    ...
    const [viewState, setViewState] = useState({ viewMode: 'grid',
        sortBy: 'modified',
        sortDirection: 'desc',
        showHidden: false,
        filterText: '' });
...
;
const loadRecentFiles = useCallback(() => { setRecentFiles(projectManager.getRecentFiles(10)); }, [projectManager]);
...
const handleFileDelete = useCallback((file) => {
    if (confirm(`Are you sure you want to delete "${file.name}"? This action cannot be undone.`)) { // Mock deletion - in real implementation would delete from server/filesystem
...
const handleFileDuplicate = useCallback((file) => {
    ...
    metadata: { }
}, ...file.metadata, title, file.metadata.title + ' Copy', created, new Date());
;
...
return (_jsxs("div", { style: {
        ...
        fontSize: '14px'
                ...
                backgroundColor: styles.secondary
            }, "borderBottom:": true }), " `1px solid $", styles.border, "` gap: '12px'; }>", _jsxs("div", { style: { flex: 1, maxWidth: '300px' }, children: [_jsx("input", { type: "text", placeholder: "Search projects...", value: viewState.filterText, onChange: (e) => setViewState(prev => ({ ...prev, filterText: e.target.value })), style: {
                        width: '100%',
                        padding: '8px 12px',
                        backgroundColor: styles.tertiary
                    }, "border:": true }), " `1px solid $", styles.border, "` borderRadius: '6px' color: styles.text fontSize: '13px' outline: 'none'; />"] }), _jsx("div", { style: { display: 'flex', alignItems: 'center', gap: '8px' }, children: _jsx("select", { value: `${viewState.sortBy}-${viewState.sortDirection}`, onChange: (e) => {
                ...
                } }) }), _jsx("div", { style: { display: 'flex', backgroundColor: styles.tertiary, borderRadius: '4px', padding: '2px' }, children: ['list', 'grid', 'details'].map(mode => (_jsx("button", { onClick: () => setViewState(prev => ({ ...prev, viewMode: mode })), style: {
                    padding: '6px 8px',
                    ...
                    >
                        { mode } === 'list' ? '☰' : mode === 'grid' ? '⊞' : '≡' }, mode))) }), "))}"] })) /* New Project Button */;
{ /* New Project Button */ }
{
    showCreateControls && (_jsx("button", { onClick: onNewProject, style: {
            padding: '8px 12px',
            ...
            >
                _jsx("span", { children: "\u2795" }), New: true, Project: true, button: true, children: ")}" }));
    div >
        { /* Content Area */}
        < div;
    style = {};
...
style = {};
{
 ...
 >
    (_jsx("h3", { style: {
        ...
        ,
            _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' }, children: [_jsx("button", { style: {
                            padding: '8px 12px',
                            ...
                            alignItems: 'center',
                            gap: '8px'
                        }, onMouseOver: (e) => e.currentTarget.style.backgroundColor = styles.hover, onMouseOut: (e) => e.currentTarget.style.backgroundColor = 'transparent', children: ["\uD83D\uDD50 Recent (", recentFiles.length, ")"] })] }));
div >
    { /* Recent Files */};
{
    recentFiles.length > 0 && (_jsxs("div", { style: { padding: '0 16px', marginTop: '16px' }, children: [_jsx("h3", { style: {
                    margin: '0 0 8px 0',
                    fontSize: '12px',
                    fontWeight: '600',
                    color: styles.textSecondary,
                    ...
                    letterSpacing: '0.5px'
                        ...
                        whiteSpace: 'nowrap'
                    }, onMouseOver: (e) => e.currentTarget.style.backgroundColor = styles.hover, onMouseOut: (e) => e.currentTarget.style.backgroundColor = 'transparent', title: file.name, children: ["\uD83D\uDCC4 ", file.name.replace('.psg', '')] }, file.id))) })] }));
}
div >
    { /* Main File Area */}
    < div;
...
color: styles.textSecondary;
 >
    { breadcrumbs, : .map((crumb, index) => (_jsxs(React.Fragment, { children: [index > 0 && _jsx("span", { children: '>' }), _jsx("button", { onClick: () => setCurrentFolder(crumb.path), style: {
                        backgroundColor: 'transparent',
                        border: 'none',
                        ...
                        >
                            { crumb, : .name } })] }, crumb.path)), React.Fragment >
        ) };
div >
    { /* File Grid/List */}
    < div;
...
div >
;
filteredAndSortedFiles.length === 0 ? (_jsxs("div", { style: {
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
...
style = {};
{
 ...
 >
    _jsxs("div", { style: {
            ...
            textOverflow: 'ellipsis',
            whiteSpace: 'nowrap'
        }, children: [file.metadata.title || file.name.replace('.psg', ''), file.isFavorite && _jsx("span", { style: { marginLeft: '4px' }, children: "\u2B50" })] });
{
