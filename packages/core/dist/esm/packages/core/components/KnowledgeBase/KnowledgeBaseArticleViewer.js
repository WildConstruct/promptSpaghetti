...
;
return;
_jsxs("div", { ref: setSectionRef, id: section.anchor, className: "mb-6", children: [_jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-3", children: section.title }), _jsx("div", { className: "prose max-w-none text-gray-700", children: _jsx("div", { dangerouslySetInnerHTML: { __html: section.content } }) })] });
;
[];
;
// Render stars
...
;
return;
_jsxs("div", { className: "knowledge-base-article-viewer h-full flex bg-gray-50", children: [_jsx("div", { className: "fixed top-0 left-0 right-0 h-1 bg-gray-200 z-50", children: _jsx("div", { className: "h-full bg-blue-600 transition-all duration-150", style: { width: `${viewerState.readingProgress}%` } }) }), viewerState.showTableOfContents && ()
        ...
        { /* Article excerpt */};
{
    ...
    className = "bg-blue-50 border-l-4 border-blue-400 p-4 mb-8" >
        _jsx("p", { className: "text-blue-800 font-medium", children: article.excerpt });
    div >
    ;
}
{ /* Article sections */ }
_jsx("div", { className: "max-w-4xl", children: article.sections.map(section => renderSectionContent(section)) });
{ /* Code examples */ }
{
    article.codeExamples.length > 0 && ()
        < div;
    className = "mt-8" >
        (_jsx("h2", { className: "text-xl font-bold text-gray-900 mb-4", children: "Code Examples" })
            ,
                _jsx("div", { className: "space-y-6", children: article.codeExamples.map(example => ()
                        < div, key = { example, : .id }, className = "border border-gray-200 rounded-lg overflow-hidden" >
                        _jsxs("div", { className: "bg-gray-50 px-4 py-2 border-b border-gray-200", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("h3", { className: "font-medium text-gray-900", children: example.title }), _jsx("span", { className: "text-sm text-gray-500", children: example.language })] }), example.description && ()
                                    < p, " className=\"text-sm text-gray-600 mt-1\">", example.description] })) })
                    ,
                        _jsx("pre", { className: "bg-gray-900 text-gray-100 p-4 overflow-x-auto", children: _jsx("code", { children: example.code }) }));
    {
        example.output && ()
            < div;
        className = "bg-gray-50 px-4 py-2 border-t border-gray-200" >
            (_jsx("div", { className: "text-sm text-gray-600", children: "Output:" })
                ,
                    _jsx("pre", { className: "text-sm text-gray-800 mt-1", children: example.output }));
        div >
        ;
    }
    div >
...
{ /* Related articles */ }
{
    article.relatedArticles.length > 0 && ()
        < div;
    className = "mt-8" >
        (_jsx("h2", { className: "text-xl font-bold text-gray-900 mb-4", children: "Related Articles" })
            ,
                _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [article.relatedArticles.slice(0, 4).map(relatedId => ()
                            < button, key = { relatedId }, onClick = {}()), " => onArticleSelect?.(relatedId)} className=\"text-left p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-md transition-all\" >", _jsx("div", { className: "font-medium text-blue-600 mb-1", children: "Related Article" }), _jsx("div", { className: "text-sm text-gray-600", children: "Click to view related content" })] }));
}
div >
;
div >
...
className = "bg-white border-t border-gray-200 px-8 py-6" >
    _jsx("div", { className: "max-w-4xl", children: _jsxs("div", { className: "mb-6", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900 mb-3", children: "Rate this article" }), _jsxs("div", { className: "flex items-center space-x-4", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("span", { className: "text-sm text-gray-600", children: "Your rating:" }), renderStars(existingRating?.rating || viewerState.userRating, true, handleRatingSubmit)] }), existingRating && ()
                            < span, " className=\"text-sm text-green-600\">Thank you for rating!"] }), ")}"] }) });
{ /* Feedback Section */ }
_jsxs("div", { children: [_jsxs("div", { className: "flex items-center justify-between mb-3", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Feedback" }), _jsx("button", { onClick: () => setViewerState(prev => ({ ...prev, showFeedbackForm: !prev.showFeedbackForm })), className: "text-sm text-blue-600 hover:text-blue-800", children: viewerState.showFeedbackForm ? 'Cancel' : 'Leave Feedback' })] }), viewerState.showFeedbackForm && ()
            < div, " className=\"border border-gray-200 rounded-lg p-4\">", _jsx("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4 mb-4", children: _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Feedback Type" }), _jsx("select", { value: viewerState.feedbackType, onChange: (e) => setViewerState(prev => ({ ...prev, feedbackType: e.target.value })), className: "w-full border border-gray-300 rounded-md px-3 py-2 text-sm", children: Object.values(FeedbackType).map(type => ()
                            < option, key = { type }, value = { type } >
                            {}) }), "))}"] }) })] })
    ,
        _jsxs("div", { className: "mb-4", children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Your feedback" }), _jsx("textarea", { value: viewerState.userFeedback, onChange: (e) => setViewerState(prev => ({ ...prev, userFeedback: e.target.value })), placeholder: "Share your thoughts about this article...", rows: 3, className: "w-full border border-gray-300 rounded-md px-3 py-2 text-sm" })] })
            ,
                _jsx("div", { className: "flex justify-end", children: _jsx("button", { onClick: handleFeedbackSubmit, disabled: !viewerState.userFeedback.trim() || viewerState.loading, className: "px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed", children: viewerState.loading ? 'Submitting...' : 'Submit Feedback' }) });
div >
;
div >
;
    ...
    { /* Recommendations Sidebar */};
{
    ...
    className = "w-80 bg-white border-l border-gray-200 overflow-y-auto" >
        _jsxs("div", { className: "p-6", children: [_jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-4", children: "Recommended for You" }), _jsxs("div", { className: "space-y-4", children: [viewerState.recommendations.slice(0, 5).map(recommendation => ()
                            < button, key = { recommendation, : .articleId }, onClick = {}()), " => onArticleSelect?.(recommendation.articleId)} className=\"w-full text-left p-3 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-sm transition-all\" >", _jsxs("div", { className: "flex items-center justify-between mb-2", children: [_jsx("span", { className: "text-sm font-medium text-blue-600", children: "Recommended Article" }), _jsxs("span", { className: "text-xs text-gray-500", children: [Math.round(recommendation.score * 100), "% match"] })] }), _jsx("div", { className: "text-sm text-gray-600", children: formatCategoryName(recommendation.reason) })] }), "))}"] });
    div >
    ;
    div >
    ;
