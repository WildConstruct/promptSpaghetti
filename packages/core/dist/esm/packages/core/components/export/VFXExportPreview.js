        ...
        try {
            ...
            const mockPreview = {
                ...
                equivalence: {
                    ...
                    confidence: 0.95,
                }, }, { id: , 'technical-brief': , type: , 'technical': , language: , 'english': , style: { formality: , 'technical': , length: , 'concise': , perspective: , 'objective': , }, content: { naturalLanguage: , 'Wide-angle exterior shot of large creature asset positioned on architectural ruins. Golden hour lighting conditions with atmospheric haze. Photorealistic rendering quality required.': , technicalNotes: [], 'Creature scale: approximately 15-20 meters': , 'Asset complexity: hero-level detail required': , 'Lighting setup: single key sun source + fill bounce': , 'Atmospheric effects: volumetric fog at ground level': , creativeNotes: [], 'Creature pose: regal, non-threatening': , 'Color palette: warm golden tones': , 'Mood: majestic, contemplative': , productionNotes: [], 'Render time estimate: 8-12 hours per frame': , 'Memory requirements: 32GB+ recommended': , 'Dependencies: creature rigging, environment modeling':  }, metadata: { targetAudience: , 'vfx-supervisor': , expertiseLevel: , 'expert': , context: , 'production': , }, equivalence: { marsMapping: { metadata: , true: , actions: , false: , rendering: , true: , style: , false: , }, structuredPrompt: , 'Wide shot, ancient red dragon, stone ruins, golden hour lighting, photorealistic': , confidence: , 0.87: , }, }, hollywoodProtocol, 
            // ControlNet integration metadata tags
            controlNetTags, subjects, effects;
        }
        finally { }
        marsIntegration: {
            enabled: true,
...
const renderValidationTab = () => (_jsxs("div", { className: "space-y-6", children: [_jsx("div", { className: `rounded-lg p-4 ${validation.isValid
         ...
         }), ": 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800', }`} >", _jsxs("div", { className: "flex items-center", children: [validation.isValid ? (_jsx(FiCheck, { className: "w-6 h-6 text-green-600 mr-3" })) : (_jsx(FiX, { className: "w-6 h-6 text-red-600 mr-3" })), _jsxs("div", { children: [_jsx("h4", { className: `text-lg font-semibold ${validation.isValid ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200',
...
return (_jsx("div", { className: "vfx-export-preview", children: _jsxs("div", { className: "border-b border-gray-200 dark:border-gray-700 mb-6", children: [_jsxs("nav", { className: "flex space-x-8", children: [[
                        { id: 'preview', label: 'Preview', icon: FiEye },
                        { id: 'validation', label: 'Validation', icon: FiCheck },
                        { id: 'json', label: 'JSON', icon: FiCode }
                    ].map(tab => (_jsx("button", { onClick: () => setActiveTab(tab.id), className: `flex items-center space-x-2 py-2 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === tab.id
                            ? 'border-blue-500 text-blue-600 dark:text-blue-400' : ,
                     }, tab.id))), ": 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300', }`} >", _jsx(tab.icon, { className: "w-4 h-4" }), _jsx("span", { children: tab.label })] }), "))}"] }) })) /* Tab Content */;
{ /* Tab Content */ }
