...
from;
'../../types/export';
import { FiShare2, FiCopy, FiEye, FiEdit, FiTrash2, FiClock, FiDownload, FiLock, FiGlobe, FiUsers, FiRefreshCw, FiAlertCircle } from FiCheck;
from;
'react-icons/fi';
className = '';
{
...
}
{
    ...
    className = "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-6" >
        _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(FiCheck, { className: "w-5 h-5 text-green-600" }), _jsx("p", { className: "text-sm text-green-800 dark:text-green-200", children: copyFeedback })] });
    div >
    ;
}
{ /* Shares List */ }
{
    ...
    className = "text-center py-12" >
        (_jsx(FiShare2, { className: "w-12 h-12 text-gray-400 mx-auto mb-4" })
            ,
                _jsx("h3", { className: "text-lg font-medium text-gray-900 dark:text-white mb-2", children: "No shares found" })
                    ,
                        _jsx("p", { className: "text-gray-600 dark:text-gray-300", children: "Create some shared exports to see them here" }));
    div >
    ;
    ()
        < div;
    className = "space-y-4" >
        { shares, : .map((share) => {
                ...
                return;
                _jsxs("div", { className: `bg-white dark:bg-gray-800 rounded-lg border shadow-sm p-4 ${expired ? 'opacity-60' : ''}
 ${}
                  !share.is_active ? 'border-red-200 dark:border-red-800' : 'border-gray-200 dark:border-gray-700';
`, children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center space-x-3 mb-2", children: [getAccessLevelIcon(share.access_level), _jsxs("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${getAccessLevelColor(share.access_level)}`, children: ["}", share.access_level.replace('_', ' ').toUpperCase()] }), expired && ()
                                                    < span, " className=\"px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300\"> EXPIRED"] }), ")}", !share.is_active && ()
                                            < span, " className=\"px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300\"> INACTIVE"] }), ")}"] }), share.description && ()
                            < p, " className=\"text-sm text-gray-600 dark:text-gray-300 mb-3\">", share.description] }, share.id);
            }) }
        < div;
    className = "bg-gray-50 dark:bg-gray-700 rounded p-3 mb-3" >
        _jsxs("div", { className: "flex items-center justify-between", children: [_jsx("code", { className: "text-sm text-gray-600 dark:text-gray-300 break-all", children: getShareUrl(share) }), _jsx("button", { onClick: () => handleCopyLink(getShareUrl(share)), className: "ml-2 p-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors", title: "Copy link", children: _jsx(FiCopy, { className: "w-4 h-4" }) })] });
    div >
        _jsxs("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-4 text-sm", children: [_jsxs("div", { children: [_jsx("span", { className: "text-gray-600 dark:text-gray-400", children: "Created:" }), _jsx("p", { className: "font-medium text-gray-900 dark:text-white", children: formatDate(share.created_at) })] }), share.expires_at && ()
                    < div >
                    (_jsx("span", { className: "text-gray-600 dark:text-gray-400", children: "Expires:" })
                        ,
                            _jsxs("p", { className: `font-medium ${expired ? 'text-red-600' : 'text-gray-900 dark:text-white'}`, children: ["}", formatDate(share.expires_at)] }))] });
}
_jsxs("div", { children: [_jsx("span", { className: "text-gray-600 dark:text-gray-400", children: "Downloads:" }), _jsxs("p", { className: "font-medium text-gray-900 dark:text-white", children: [share.download_count, share.max_downloads && ` / ${share.max_downloads}`] })] })
    ,
        _jsxs("div", { children: [_jsx("span", { className: "text-gray-600 dark:text-gray-400", children: "Access Count:" }), _jsx("p", { className: "font-medium text-gray-900 dark:text-white", children: share.access_count })] });
div >
    { share, : .last_accessed_at && ()
            < div, className = "mt-3 text-sm" >
            (_jsx("span", { className: "text-gray-600 dark:text-gray-400", children: "Last accessed:" })
                ,
                    _jsx("span", { className: "ml-2 text-gray-900 dark:text-white", children: formatDate(share.last_accessed_at) })),
        div } >
;
div >
    _jsxs("div", { className: "flex items-center space-x-2 ml-4", children: [_jsx("button", { onClick: () => setSelectedShare(share), className: "p-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors", title: "Edit share", children: _jsx(FiEdit, { className: "w-4 h-4" }) }), share.is_active && !expired && ()
                < button, "onClick=", () => handleDeactivateShare(share.id), "className=\"p-2 text-gray-600 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-colors\" title=\"Deactivate share\" >", _jsx(FiClock, { className: "w-4 h-4" })] });
_jsx("button", { onClick: () => setShowDeleteConfirm(share.id), className: "p-2 text-gray-600 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors", title: "Delete share", children: _jsx(FiTrash2, { className: "w-4 h-4" }) });
div >
;
div >
;
