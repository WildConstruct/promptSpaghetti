...
const handleParameterChange = useCallback((key, value) => {
    ...
    setCustomization(prev => ({}), ...prev);
}, parameters, { ...prev.parameters, [key]: value }, validationErrors, error)
    ? { ...prev.validationErrors, [key]: error }
    : { ...prev.validationErrors, [key]: undefined }, isDirty;
;
...
if (!visible)
    return null;
return;
_jsxs("div", { className: `template-customization-dialog ${className}`, style: {
        position: 'fixed',
        inset: '40px',
        background: 'white',
...
style = {};
{
    ...
    >
        _jsxs("div", { style: { padding: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 12px', fontSize: '14px', fontWeight: '600', color: '#374151' }, children: "Categories" }), Array.from(categorizedParameters.keys()).map(category => ()
...
_jsx("button", { onClick: handleSave, disabled: loading || Object.values(customization.validationErrors).some(e => e), style: {
        ...
        >
            { loading, '⏳ Saving...': '💾 Save Customization' }, button: true });
div >
;
div >
;
