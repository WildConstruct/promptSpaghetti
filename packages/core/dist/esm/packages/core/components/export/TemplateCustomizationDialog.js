...
if (Object.keys(defaultParams).length > 0) {
    setCustomization(prev => ({}), ...prev, parameters, { ...defaultParams, ...prev.parameters });
}
;
[parameterDefinitions];
;
...
const handleParameterChange = useCallback((key, value) => {
    ...
    setCustomization(prev => ({}), ...prev, parameters, { ...prev.parameters, [key]: value }, validationErrors, error)
...
if (Object.values(errors).some(e => e)) {
    ...
    }, borderRadius, '6px', fontSize, '14px', background, error ? '#fef2f2' : 'white');
}
;
switch (param.type) {
    case 'string':
        return;
        _jsx("input", { type: "text", value: value || '', onChange: (e) => handleParameterChange(param.key, e.target.value), placeholder: param.description, style: baseInputStyle });
        ;
    case 'number':
        return;
        _jsx("input", { type: "number", value: value || '', onChange: (e) => handleParameterChange(param.key, parseFloat(e.target.value) || 0), min: param.validation?.min, max: param.validation?.max, style: baseInputStyle });
        ;
    case 'boolean':
        return;
        _jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }, children: [_jsx("input", { type: "checkbox", checked: value || false, onChange: (e) => handleParameterChange(param.key, e.target.checked), style: { width: 'auto' } }), _jsx("span", { style: { fontSize: '14px', color: '#6b7280' }, children: param.description || `Enable ${param.label}` })] });
        ;
    case 'select':
        return;
        _jsx("select", { value: value || '', onChange: (e) => handleParameterChange(param.key, e.target.value), style: baseInputStyle, children: param.validation?.options?.map(option => ()
...
'color';
return;
_jsxs("div", { style: { display: 'flex', gap: '8px', alignItems: 'center' }, children: [_jsx("input", { type: "color", value: value || '#000000', onChange: (e) => handleParameterChange(param.key, e.target.value), style: { width: '50px', height: '38px', border: 'none', borderRadius: '6px' } }), _jsx("input", { type: "text", value: value || '', onChange: (e) => handleParameterChange(param.key, e.target.value), placeholder: "#000000", style: { ...baseInputStyle, flex: 1 } })] });
;
'json';
return;
_jsx("textarea", { value: typeof value === 'object' ? JSON.stringify(value, null, 2) : value || '', onChange: (e) => {
        try {
...
;
return;
_jsx("input", { type: "text", value: value || '', onChange: (e) => handleParameterChange(param.key, e.target.value), style: baseInputStyle });
;
...
if (!visible)
    return null;
return;
_jsx("div", { className: `template-customization-dialog ${className}`, style: {
        position: 'fixed',
        inset: '40px',
        background: 'white',
...
{ /* Content */ }
_jsx("div", { style: { flex: 1, overflow: 'hidden', display: 'flex' }, children: _jsxs("div", { style: {
            width: '200px',
            borderRight: '1px solid #e2e8f0',
            background: '#f8fafc',
            overflow: 'auto',
        }, children: [_jsxs("div", { style: { padding: '16px' }, children: [_jsx("h4", { style: { margin: '0 0 12px', fontSize: '14px', fontWeight: '600', color: '#374151' }, children: "Categories" }), Array.from(categorizedParameters.keys()).map(category => ()
...
{ /* Parameters Panel */ }
_jsxs("div", { style: { flex: 1, overflow: 'auto', padding: '20px 24px' }, children: [error && ()
            < div, "style=", {
            padding: '12px 16px',
            background: '#fee2e2',
...
_jsxs("div", { style: { marginBottom: '24px' }, children: [_jsxs("h3", { style: { margin: '0 0 16px', fontSize: '18px', fontWeight: '600', color: '#374151' }, children: [activeCategory, " Parameters"] }), categorizedParameters.get(activeCategory)?.map(param => ()
            < div, key = { param, : .key }, style = {}, { marginBottom: '20px' }), ">", _jsxs("label", { style: { display: 'block', marginBottom: '8px' }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }, children: [_jsx("span", { style: { fontSize: '14px', fontWeight: '500', color: '#374151' }, children: param.label }), param.required && ()
...
style = {};
{
    ...
    >
        (_jsx("div", { style: { display: 'flex', gap: '12px' }, children: _jsx("button", { onClick: generatePreview, disabled: loading, style: {
                    padding: '8px 16px',
                    background: '#f3f4f6',
                    border: '1px solid #e2e8f0',
                    borderRadius: '6px',
                ...
                }, children: loading ? '⏳ Generating...' : '👁️ Preview' }) })
            ,
...
_jsx("button", { onClick: handleSave, disabled: loading || Object.values(customization.validationErrors).some(e => e), style: {
        padding: '8px 16px',
