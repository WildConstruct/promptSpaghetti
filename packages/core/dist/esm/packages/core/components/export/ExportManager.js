...
className = '';
{
        ...
        const handleShareExport = (job) => {
            ...
            const renderTabContent = () => {
                switch (activeTab) {
                    case 'templates':
                        return;
                        _jsx(ExportTemplateList, { templates: templates, onTemplateSelect: handleTemplateSelect, onQuickExport: handleQuickExport, loading: loading, error: error });
                }
            };
        };
    };
    ...
    'jobs';
    return;
    _jsx(ExportJobList, { jobs: jobs, onCancel: cancelExportJob, onRefresh: fetchJobs, onShare: handleShareExport, loading: loading, error: error });
    ;
    'statistics';
    return;
    _jsx(ExportStatsDashboard, { statistics: statistics, loading: loading, error: error });
    ;
    'shares';
    return;
    _jsx(ShareManager, { projectId: projectId });
    ;
    return null;
}
;
    ...
    { /* Export Wizard Modal */};
{
    ...
    className = "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" >
        _jsx("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto mx-4", children: _jsx(ExportWizard, { projectId: projectId, template: selectedTemplate, onComplete: handleWizardComplete, onCancel: handleWizardCancel }) });
    div >
    ;
}
{ /* Share Dialog Modal */ }
{
    shareDialogJob && ()
        < div;
    className = "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" >
        _jsx("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto mx-4 p-6", children: _jsx(ShareDialog, { exportJob: shareDialogJob, onClose: handleShareDialogClose, onShareCreated: handleShareCreated }) });
    div >
    ;
}
{ /* Import Dialog Modal */ }
{
    showImportDialog && ()
        < div;
    className = "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" >
        _jsx("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto mx-4 p-6", children: _jsx(ImportDialog, { onClose: () => setShowImportDialog(false), onImportComplete: handleImportComplete }) });
    div >
    ;
}
div >
