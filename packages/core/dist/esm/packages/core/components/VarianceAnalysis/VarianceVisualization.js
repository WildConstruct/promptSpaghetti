...
const contentTypeData = varianceAnalysis?.contentTypes ?
    Object.entries(varianceAnalysis.contentTypes).map(([type, count], index) => ({}), label, type.charAt(0).toUpperCase() + type.slice(1), value, count) : ;
                    ...
                    color: '#374151'
                        ...
                        gap: 8
                            ...
                            position: 'relative'
                                ...
                                }, "width:": true }), " `$", (item.value / max) * 100, "%`} transition: 'width 0.5s ease' borderRadius: 10; } />", showValues && ()
                                < div, " style=", {
                                position: 'absolute',
                                right: 8,
                                top: '50%',
...
_jsxs("div", { style: {
        ...
        padding: 16
                ...
                padding: '10px 0'
                    ...
                    }, "height:": true }), " `$", (bin.value / Math.max(...bins.map(b => b.value))) * 100, "px`} borderRadius: '4px 4px 0 0' display: 'flex' alignItems: 'center' justifyContent: 'center' color: 'white' fontSize: 10 fontWeight: 500 transition: 'height 0.5s ease'; }>", bin.value > 0 ? bin.value : ''] }), _jsx("div", { style: {
                fontSize: 9,
                color: '#6b7280',
                textAlign: 'center',
                lineHeight: 1,
...
style = {};
{
 ...
 >
    { [{ id: 'length', label: '📏 Length Analysis', icon: '📏' }]: { id: 'similarity', label: '🔗 Similarity Matrix', icon: '🔗' } };
{
    id: 'content', label;
    '📝 Content Types', icon;
    '📝';
...
map(chart => ()
    < button, key = { chart, : .id }, onClick = {}(), setActiveChart(chart.id), style = {}, {
    flex: 1,
    padding: 12,
    border: 'none',
        ...
        ,
            _jsxs("div", { style: {
                    ...
                    padding: 16
                            ...
                            display: 'grid'
                        }, "gridTemplateColumns:": true }), " `repeat($", results.length, ", 1fr)`} gap: 2 fontSize: 10; }>", results.map((result1, i) => (), results.map((result2, j) => {
                        const similarity = i === j ? 1 : ();
                            ...
                            width: 20,
                            height: 20
                        }, "background:": true }, `${i}-${j}`), " `rgba(59, 130, 246, $", similarity, ")`} borderRadius: 2 display: 'flex' alignItems: 'center' justifyContent: 'center' color: similarity > 0.5 ? 'white' : '#374151'; >", i === j ? '•' : ''] }));
;
div >
    _jsx("div", { style: {
            fontSize: 11,
