...
borderRadius: professionalBorderRadius.md,
    minWidth;
180, // Slightly larger for better professional appearance
    minHeight;
90,
    boxShadow;
selected,
...
'var(--font-primary)',
    transition;
'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', // Professional easing curve
    zIndex;
1,
    pointerEvents;
'auto',
    ...
    '&::before';
{
    ...
    0,
        background;
    'linear-gradient(180deg, rgba(255, 255, 255, 0.05) 0%, rgba(255, 255, 255, 0.01) 50%, rgba(0, 0, 0, 0.05) 100%)',
        borderRadius;
    'inherit',
        pointerEvents;
    'none';
...
color: '#ffffff',
    padding;
`${professionalSpacing[2]} ${professionalSpacing[3]}`;
...
'center',
    gap;
professionalSpacing[2],
    borderRadius;
`${professionalBorderRadius.md} ${professionalBorderRadius.md} 0 0`;
...
();
    ...
    < div;
style = { headerStyle } >
    (_jsx("span", { style: { fontSize: 16, filter: 'drop-shadow(0 1px 1px rgba(0,0,0,0.3))' }, children: typeof nodeMeta.icon === 'string' ? nodeMeta.icon : '🔧' })
        ,
    ...
    { /* Properties with monospace font for technical data */};
{
    ...
    _jsx("span", { style: { color: professionalColors.text.secondary }, children: String(v).length > 22 ? String(v).slice(0, 22) + '…' : String(v) });
    div >
    ;
}
div >
