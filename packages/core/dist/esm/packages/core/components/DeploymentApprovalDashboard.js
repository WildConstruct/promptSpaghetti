...
import { RocketLaunchIcon, ShieldCheckIcon, BoltIcon, BuildingOfficeIcon, CodeBracketIcon, ClockIcon, CheckCircleIcon, XCircleIcon, ExclamationTriangleIcon, EyeIcon, ComputerDesktopIcon, ServerStackIcon, GlobeAltIcon, ChevronRightIcon } from ArrowTopRightOnSquareIcon;
from;
    ...
    const _____getEnvironmentIcon = (env) => {
        ...
        const _____getEnvironmentBadgeColor = (env) => {
            ...
            if (loading && deploymentRequests.length === 0) {
                return;
                _jsx("div", { className: "flex items-center justify-center h-64", children: _jsxs("div", { className: "flex items-center space-x-2 text-gray-600", children: [_jsx(ArrowTopRightOnSquareIcon, { className: "h-5 w-5 animate-spin" }), _jsx("span", { children: "Loading deployment approvals..." })] }) });
            }
        };
    };
};
...
if (error) {
    return;
    _jsx("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4", children: _jsxs("div", { className: "flex items-center", children: [_jsx(XCircleIcon, { className: "h-5 w-5 text-red-500 mr-2" }), _jsxs("span", { className: "text-red-700", children: ["Error loading deployment approvals: ", error] })] }) });
    ;
    return;
    _jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx(RocketLaunchIcon, { className: "h-6 w-6 text-blue-600" }), _jsx("h2", { className: "text-2xl font-semibold text-gray-900", children: "Deployment Approvals" }), loading && ()
                                < ArrowTopRightOnSquareIcon, " className=\"h-4 w-4 text-gray-400 animate-spin\" /> )}"] }), _jsxs("div", { className: "flex items-center space-x-3", children: [_jsxs("span", { className: "text-sm text-gray-600", children: [filteredRequests.length, " requests"] }), _jsx("button", { onClick: fetchDeploymentRequests, className: "p-2 hover:bg-gray-100 rounded-full transition-colors", title: "Refresh", children: _jsx(ArrowTopRightOnSquareIcon, { className: "h-4 w-4 text-gray-600" }) })] })] }), _jsx("div", { className: "border-b border-gray-200", children: _jsxs("nav", { className: "-mb-px flex space-x-8", children: [[
...
const DeploymentRequestCard, DeploymentApprovalRequest;
...
({ request, onSelect, currentUserId }) => {
    ...
    const getEnvironmentBadgeColor = (env) => {
        ...
        return;
        _jsx("div", { className: "bg-white border border-gray-200 rounded-lg p-6 hover:border-gray-300 transition-colors cursor-pointer", children: _jsxs("div", { className: "flex items-start justify-between mb-4", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [getStatusIcon(request.status), _jsxs("div", { children: [_jsx("h3", { className: "text-lg font-semibold text-gray-900", children: request.title }), _jsxs("p", { className: "text-sm text-gray-600", children: ["Deployment ", request.deployment_id.substring(0, 8)] })] })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("span", { className: `px-2 py-1 text-xs font-medium rounded-full border ${urgencyColors[request.urgency]}`, children: ["}", request.urgency.toUpperCase()] }), _jsxs("span", { className: `px-2 py-1 text-xs font-medium rounded-full border ${getEnvironmentBadgeColor(request.environment)}`, children: ["}", _jsxs("div", { className: "flex items-center space-x-1", children: [getEnvironmentIcon(request.environment), _jsx("span", { children: request.environment.toUpperCase() })] })] }), request.auto_approved && ()
                                < span, " className=\"px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800 border border-green-200\"> AUTO"] }), ")}"] }) })
            ,
                _jsxs("div", { className: "mb-4", children: [_jsx("p", { className: "text-gray-700 mb-2", children: request.description }), _jsxs("div", { className: "flex items-center space-x-4 text-sm text-gray-600", children: [_jsxs("span", { children: ["By ", request.requested_by] }), _jsx("span", { children: "\u2022" }), _jsx("span", { children: new Date(request.requested_at).toLocaleString() }), _jsx("span", { children: "\u2022" }), _jsxs("span", { children: [request.metadata.changed_files, " files, ", request.metadata.lines_changed, " lines"] })] })] });
        { /* Approval Progress */ }
        ...
        { /* Criteria Status */ }
        _jsxs("div", { className: "mb-4", children: [_jsxs("div", { className: "flex flex-wrap gap-2", children: [request.criteria.map(criterion => ()
                            < div, key = { criterion, : .type }, className = {} `flex items-center space-x-1 px-2 py-1 rounded-full text-xs ${criterion.status === 'approved'
                            ? 'bg-green-100 text-green-800 border border-green-200'
                            : criterion.status === 'rejected'
                                ? 'bg-red-100 text-red-800 border border-red-200'
                                : 'bg-gray-100 text-gray-800 border border-gray-200'}
`), ">", getCriterionIcon(criterion.type), _jsx("span", { children: criterion.type.replace('-', ' ').toUpperCase() }), criterion.status === 'approved' && _jsx(CheckCircleIcon, { className: "h-3 w-3" }), criterion.status === 'rejected' && _jsx(XCircleIcon, { className: "h-3 w-3" })] }), "))}"] });
    };
    div >
        { /* Quality Metrics */}
        < div;
    className = "flex items-center justify-between text-xs text-gray-600 border-t pt-3" >
        _jsxs("div", { className: "flex items-center space-x-4", children: [_jsxs("span", { children: ["Coverage: ", request.metadata.test_coverage, "%"] }), _jsxs("span", { children: ["Security: ", request.metadata.security_scan_status] }), request.metadata.breaking_changes && ()
                    < span, " className=\"text-orange-600\">\u26A0 Breaking Changes"] });
};
div >
    _jsxs("button", { onClick: () => onSelect(request), className: "flex items-center space-x-1 text-blue-600 hover:text-blue-800", children: [_jsx("span", { children: "View Details" }), _jsx(ChevronRightIcon, { className: "h-3 w-3" })] });
div >
;
div >
;
...
const DeploymentRequestDetail, DeploymentApprovalRequest;
onClose: () => void ;
currentUserId: string;
onUpdate: () => void ;
 > ;
