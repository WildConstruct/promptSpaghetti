...
if (!enabled)
    return null;
return;
_jsxs("div", { className: "contextual-help-system", children: [_jsxs("div", { className: "help-launcher", children: [_jsx("button", { className: "help-button", onClick: () => setActiveHelp(helpDatabase[0]), title: "Get contextual help", children: _jsx("span", { className: "help-icon", children: "\uD83D\uDCA1" }) }), showProgressiveHints && currentContext.isEmpty && ()
                    < div, " className=\"progressive-hint\">", _jsx("span", { className: "hint-icon", children: "\uD83D\uDC4B" }), _jsx("span", { className: "hint-text", children: "Start by dragging a node from the palette" })] }), ")}"] });
{ /* Active Help Tooltip */ }
{
    activeHelp && ()
        < div;
...
className = "help-header" >
    (_jsxs("div", { className: "help-title", children: [_jsxs("span", { className: "help-type-icon", children: [activeHelp.type === 'getting-started' && '🎬', activeHelp.type === 'node-creation' && '📝', activeHelp.type === 'connection-flow' && '🔗', activeHelp.type === 'weight-adjustment' && '⚖️', activeHelp.type === 'preview-generation' && '▶️', activeHelp.type === 'professional-workflow' && '🏆', activeHelp.type === 'advanced-features' && '🚀', activeHelp.type === 'troubleshooting' && '🔧'] }), _jsx("h3", { children: activeHelp.title })] })
        ,
            _jsxs("div", { className: "help-controls", children: [_jsx("button", { className: "expand-button", onClick: () => setIsExpanded(!isExpanded), title: isExpanded ? 'Collapse help' : 'Expand help', children: isExpanded ? '▼' : '▶' }), _jsx("button", { className: "close-button", onClick: closeHelp, title: "Close help", children: "\u00D7" })] }));
div >
    { /* Help Content */}
    < div;
className = "help-content" >
    _jsx("p", { className: "help-description", children: activeHelp.content });
{
    activeHelp.filmTerminology && ()
        < div;
    className = "film-terminology" >
        (_jsx("span", { className: "film-icon", children: "\uD83C\uDFAD" })
            ,
                _jsx("p", { children: activeHelp.filmTerminology }));
    div >
    ;
}
{
    isExpanded && activeHelp.actionItems && ()
        < div;
    className = "action-items" >
        (_jsx("h4", { children: "Next Steps:" })
            ,
                _jsx("ul", { children: activeHelp.actionItems.map((action, index) => ()
                        < li, key = { index } > { action }) }));
}
ul >
;
...
;
{
    isExpanded && activeHelp.relatedFeatures && ()
        < div;
    className = "related-features" >
        (_jsx("h4", { children: "Related Features:" })
            ,
                _jsx("div", { className: "feature-tags", children: activeHelp.relatedFeatures.map((feature, index) => ()
    ...
    { /* Tour Navigation */};
{
    ...
    className = "tour-navigation" >
        (_jsxs("div", { className: "tour-progress", children: ["Step ", tourStep + 1, " of ", helpDatabase.filter(h => h.level === 'beginner').length] })
            ,
                _jsx("button", { className: "tour-next", onClick: nextTourStep, children: "Next \u2192" }));
    div >
    ;
}
div >
...
{ /* Quick Tour Launcher */ }
{
    ...
    className = "tour-launcher" >
        _jsx("button", { className: "start-tour-btn", onClick: () => startTour('getting-started'), children: "\uD83C\uDF93 Start Director Tour" });
    div >
    ;
}
_jsx("style", { children: `
