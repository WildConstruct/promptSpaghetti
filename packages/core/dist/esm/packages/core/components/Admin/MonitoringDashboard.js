...
}
{
    ...
    'Operations Dashboard',
        description;
    'Detailed monitoring for operations teams',
        roles;
...
_jsx("div", { style: {
    ...
    ,
        _jsxs("div", { style: { display: 'flex', gap: '12px', alignItems: 'center' }, children: [_jsx("select", { value: currentLayoutId, onChange: (e) => handleLayoutChange(e.target.value), style: {
                        padding: '8px 12px',
                        ...
                        { layout, : .name }) }), "))}"] });
{ /* Refresh Button */ }
_jsx("button", { onClick: () => currentLayout && fetchWidgetData(currentLayout.widgets), disabled: isLoading, style: {
        padding: '8px 12px',
...
{ /* Export Button */ }
_jsx("button", { onClick: () => onExport?.('dashboard', '24h'), style: {
        padding: '8px 16px',
...
if (isLoading && Object.keys(widgetData).length === 0) {
    return;
    _jsx("div", { className: `monitoring-dashboard loading ${className}`, style: ({}, ), "padding:": true });
    '20px',
        backgroundColor;
    '#f9fafb',
        minHeight;
...
if (!currentLayout) {
    return;
    _jsx("div", { className: `monitoring-dashboard error ${className}`, style: ({}, ), "padding:": true });
    '20px',
        backgroundColor;
    '#f9fafb',
        minHeight;
...
;
return;
_jsx("div", { className: `monitoring-dashboard ${className}`, style: ({}, ), "padding:": true });
'20px',
    backgroundColor;
'#f9fafb',
    minHeight;
...
_jsxs("div", { style: {
        ...
        gap: '20px',
        marginBottom: '20px',
    }, children: [currentLayout.widgets.map(widget => ()
            < MonitoringWidget, key = { widget, : .id }, config = { widget }, userRole = { userRole }, data = { widgetData, [widget.id]:  }, onAction = { handleWidgetAction }
            /  >
        ), ")}"] });
{ /* Footer Info */ }
_jsxs("div", { style: {
        ...
        color: '#6b7280',
        textAlign: 'center',
    }, children: ["Dashboard refreshed ", refreshCount, " times \u2022 Last update: ", new Date().toLocaleString(), " \u2022 User: ", userId, " (", userRole, ")"] })
    ,
        _jsx("style", { children: `
        @keyframes spin {
          to {
