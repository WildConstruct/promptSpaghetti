...
(function (AssignmentType) {
    ...
    AssignmentType[AssignmentType["enum"] = void 0] = "enum";
    AssignmentType[AssignmentType["BulkOperationType"] = void 0] = "BulkOperationType";
...
export const BulkAssignmentTools = ({
    assignmentType,
    operationType,
    availableTargets,
    availableResources,
...
const handleStepChange = (step) => {
    ...
    const handleExecute = async () => {
        ...
        ;
        return;
        _jsxs("div", { className: "bulk-assignment-tools", children: [_jsx(BulkAssignmentHeader, { assignmentType: assignmentType, operationType: operationType, operation: operation, onTemplateLoad: (template) => {
                        // Load template configuration
                        setOperation(prev => ({}), ...prev, template, parameters);
                    } }), ": ", ...(prev.parameters, ), " ...template.defaultParameters }, resources: availableResources.filter(r => template.defaultResources.includes(r.id)); })); }} availableTemplates=", availableTemplates, "/>", _jsx(BulkAssignmentWizard, { steps: steps, currentStep: currentStep, onStepChange: handleStepChange, canProceed: canProceedToNextStep }), _jsxs("div", { className: "bulk-assignment-content", children: [currentStep === 1 && ()
                            < TargetSelectionStep, "targets=", availableTargets, "selectedTargets=", operation.targets, "onTargetsChange=", (targets) => setOperation(prev => ({ ...prev, targets })), "assignmentType=", assignmentType, "operationType=", operationType, "/> )}", currentStep === 2 && ()
                            < ResourceSelectionStep, "resources=", availableResources, "selectedResources=", operation.resources, "onResourcesChange=", (resources) => setOperation(prev => ({ ...prev, resources })), "assignmentType=", assignmentType, "operationType=", operationType, "targets=", operation.targets, "/> )}", currentStep === 3 && ()
                            < ParametersConfigurationStep, "parameters=", operation.parameters, "onParametersChange=", (parameters) => setOperation(prev => ({ ...prev, parameters })), "assignmentType=", assignmentType, "operationType=", operationType, "/> )}", currentStep === 4 && ()
                            < ConflictResolutionStep, "operation=", operation, "onConflictsResolved=", (conflicts) => setOperation(prev => ({ ...prev, conflicts })), "onOperationUpdated=", setOperation, "/> )}", currentStep === 5 && ()
                            < ExecutionStep, "operation=", operation, "onExecute=", handleExecute, "onCancel=", onCancel, "readonly=", readonly, "/> )}"] }), _jsx(BulkAssignmentActions, { currentStep: currentStep, totalSteps: steps.length, canProceed: canProceedToNextStep, onPrevious: () => handleStepChange(currentStep - 1), onNext: () => handleStepChange(currentStep + 1), onExecute: handleExecute, onReset: () => {
                        setCurrentStep(1);
                        setOperation(prev => ({}), ...prev, targets);
                    } }), ": [], resources: [], status: 'draft', conflicts: [], results: [], })); }} isExecuting=", operation.status === 'executing', "readonly=", readonly, "/>"] });
    };
};
;
;
...
{
    return;
    _jsxs("div", { className: "bulk-assignment-wizard", children: [steps.map((step) => ()
                < div, key = { step, : .number }, className = {} `wizard-step ${currentStep === step.number ? 'active' : ''} ${currentStep > step.number ? 'completed' : ''}`), "onClick=", () => onStepChange(step.number), ">", _jsx("div", { className: "step-number", children: step.number }), _jsxs("div", { className: "step-content", children: [_jsx("div", { className: "step-title", children: step.title }), _jsx("div", { className: "step-description", children: step.description })] }), currentStep > step.number && _jsx("div", { className: "step-check", children: "\u2713" })] });
}
div >
;
;
...
const handleTargetToggle = (target) => {
    ...
    const handleSelectAll = () => {
        ...
        ;
        return;
        _jsxs("div", { className: "target-selection-step", children: [_jsxs("div", { className: "selection-controls", children: [_jsxs("div", { className: "search-filters", children: [_jsx("input", { type: "text", placeholder: "Search targets...", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: "search-input" }), _jsxs("select", { value: filterType, onChange: (e) => setFilterType(e.target.value), className: "filter-select", children: [_jsx("option", { value: "all", children: "All Types" }), _jsx("option", { value: "user", children: "Users" }), _jsx("option", { value: "team", children: "Teams" }), _jsx("option", { value: "service", children: "Services" }), _jsx("option", { value: "role", children: "Roles" })] }), _jsxs("select", { value: filterDepartment, onChange: (e) => setFilterDepartment(e.target.value), className: "filter-select", children: [_jsx("option", { value: "", children: "All Departments" }), departments.map(dept => ()
                                            < option, key = { dept }, value = { dept } > { dept })] }), "))}"] }), _jsxs("label", { className: "checkbox-label", children: [_jsx("input", { type: "checkbox", checked: showConflicts, onChange: (e) => setShowConflicts(e.target.checked) }), "Show only conflicted targets"] })] }), _jsxs("div", { className: "bulk-actions", children: [_jsx("button", { onClick: handleSelectAll, className: "bulk-select-btn", children: selectedTargets.length === filteredTargets.length ? 'Deselect All' : 'Select All' }), _jsxs("span", { className: "selection-count", children: [selectedTargets.length, " of ", filteredTargets.length, " selected"] })] })] })
            ,
                _jsxs("div", { className: "targets-list", children: [filteredTargets.map(target => ()
                            < TargetCard, key = { target, : .id }, target = { target }, selected = { selectedTargets, : .some(t => t.id === target.id) }, onToggle = {}()), " => handleTargetToggle(target)} assignmentType=", assignmentType, "operationType=", operationType, "/> ))}"] });
        {
            filteredTargets.length === 0 && ()
                < div;
            className = "empty-state" >
                (_jsx("p", { children: "No targets match your current filters." })
                    ,
                        _jsx("button", { onClick: () => {
                                setSearchTerm('');
                                setFilterType('all');
...
;
{
    ...
    return;
    _jsxs("div", { className: `target-card ${selected ? 'selected' : ''}`, children: ["}", _jsxs("div", { className: "target-header", children: [_jsx("input", { type: "checkbox", checked: selected, onChange: onToggle, className: "target-checkbox" }), _jsxs("div", { className: "target-info", children: [_jsxs("div", { className: "target-primary", children: [_jsx("span", { className: "target-icon", children: getTypeIcon(target.type) }), _jsx("span", { className: "target-name", children: target.name }), _jsx("span", { className: "target-type", children: target.type })] }), _jsxs("div", { className: "target-secondary", children: [target.email && _jsx("span", { className: "target-email", children: target.email }), target.department && _jsx("span", { className: "target-department", children: target.department })] })] }), _jsxs("div", { className: "target-indicators", children: [currentAssignments.length > 0 && ()
                                < span, " className=\"assignment-count\">", currentAssignments.length, " current"] }), ")}", conflicts.length > 0 && ()
                        < span, " className=\"conflict-indicator\"> \u26A0\uFE0F ", conflicts.length, " conflicts"] }), ")}", _jsx("button", { className: "details-toggle", onClick: () => setShowDetails(!showDetails), children: showDetails ? '▼' : '▶' })] });
    div >
        { showDetails } && ()
        < div;
    className = "target-details" >
        { currentAssignments, : .length > 0 && ()
                ...
                _jsx("h4", { children: "Current Assignments:" }) };
    {
        currentAssignments.map(assignment => ()
            < div, key = { assignment, : .id }, className = "assignment-item" >
            (_jsx("span", { children: assignment.resourceName })
                ,
                    _jsxs("span", { className: `status status-${assignment.status}`, children: ["}", assignment.status] })), { assignment, : .expiresAt && ()
                < span, className = "expiry" >
                Expires }, { assignment, : .expiresAt.toLocaleDateString() }, span >
        );
    }
    div >
    ;
...
_jsx("span", { className: "conflict-type", children: conflict.type })
    ,
        _jsx("span", { className: "conflict-description", children: conflict.description });
div >
;
div >
;
...
{
    return;
    _jsxs("div", { className: "bulk-assignment-actions", children: [_jsxs("div", { className: "primary-actions", children: [currentStep > 1 && ()
                        < button, "onClick=", onPrevious, "disabled=", readonly || isExecuting, "className=\"btn btn-secondary\" > Previous"] }), ")}", currentStep < totalSteps && ()
                < button, "onClick=", onNext, "disabled=", !canProceed || readonly || isExecuting, "className=\"btn btn-primary\" > Next"] });
}
{
    ...
    onClick = { onExecute };
    disabled = {};
    canProceed || readonly || isExecuting;
}
...
;
div >
    _jsx("div", { className: "secondary-actions", children: _jsx("button", { onClick: onReset, disabled: readonly || isExecuting, className: "btn btn-outline", children: "Reset" }) });
div >
;
;
;
