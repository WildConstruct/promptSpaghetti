...
if (filters.toggleTypes.length > 0) {
    ...
    const renderSeverityBadge = (severity) => {
        ...
        return;
        _jsxs("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${colors[severity] || colors.low}`, children: ["}", severity.toUpperCase()] });
    };
    ;
}
;
// Render dependency type badge
const _____renderDependencyTypeBadge = (type) => {
    const colors = {
...
if (loading) {
    return;
    _jsx("div", { className: "flex items-center justify-center h-64", children: _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("div", { className: "animate-spin rounded-full h-6 w-6 border-b-2 border-blue-600" }), _jsx("span", { className: "text-gray-600", children: "Loading dependency visualization..." })] }) });
    ;
    if (error) {
        return;
        ...
        { /* Details Panel */};
    {
        ...
        className = "w-80 bg-white border-l border-gray-200 overflow-y-auto" >
            _jsx(ToggleDetailsPanel, { toggleId: selectedNode, graph: filteredGraph, impactAnalysis: impactAnalysis, onClose: () => setSelectedNode(null) });
        div >
        ;
    }
    div >
...
;
{
    if (!graph) {
        return;
        _jsx("div", { className: "flex-1 flex items-center justify-center", children: _jsx("div", { className: "text-gray-500", children: "No dependency data available" }) });
        ;
        return;
...
const AnalysisView = ({ analysis }) => {
    return;
    _jsxs("div", { className: "flex-1 p-6 bg-gray-50 overflow-y-auto", children: [_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "bg-white rounded-lg border border-gray-200 p-6", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900 mb-4", children: "Dependency Violations" }), analysis.violations.length === 0 ? ()
                                < p : , " className=\"text-green-600\">No dependency violations detected."] }), ") : ()", _jsx("div", { className: "space-y-3", children: analysis.violations.map((violation) => ()
                            < div, key = { violation, : .id }, className = "border border-red-200 rounded-md p-4 bg-red-50" >
                            _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsx("h4", { className: "text-sm font-medium text-red-800", children: violation.type.replace('_', ' ').toUpperCase() }), _jsx("p", { className: "text-sm text-red-700 mt-1", children: violation.description }), _jsx("div", { className: "mt-2", children: _jsxs("span", { className: "text-xs text-red-600", children: ["Affected Toggles: ", violation.toggles.join(', ')] }) })] }), _jsx("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${violation.severity === 'critical' ? 'bg-red-500 text-white' : ,
                                            violation.severity === 'high' ? 'bg-red-400 text-white' : ,
                                            violation.severity === 'medium' ? 'bg-yellow-400 text-gray-800' : ,
                                            'bg-gray-400 text-white'}`, children: violation.severity.toUpperCase() })] })) }), "))}"] }), ")}"] });
    { /* Recommendations */ }
    _jsxs("div", { className: "bg-white rounded-lg border border-gray-200 p-6", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900 mb-4", children: "Recommendations" }), analysis.recommendations.length === 0 ? ()
                < p : , " className=\"text-gray-500\">No recommendations available."] });
};
()
    < div;
className = "space-y-3" >
    { analysis, : .recommendations.map((rec) => ()
            ...
            _jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsx("h4", { className: "text-sm font-medium text-blue-800", children: rec.type.replace('_', ' ').toUpperCase() }), _jsx("p", { className: "text-sm text-blue-700 mt-1", children: rec.action }), _jsx("p", { className: "text-xs text-blue-600 mt-1", children: rec.rationale })] }), _jsx("span", { className: `px-2 py-1 rounded-full text-xs font-medium ${rec.priority === 'urgent' ? 'bg-red-500 text-white' : ,
                            rec.priority === 'high' ? 'bg-yellow-500 text-white' : ,
                            rec.priority === 'medium' ? 'bg-blue-500 text-white' : ,
                            'bg-gray-500 text-white'}`, children: rec.priority.toUpperCase() })] }), div >
        ) };
div >
;
div >
...
const TreeVisualization = () => ()
    ...
    _jsxs("div", { className: "text-center text-gray-500", children: [_jsx("p", { children: "Tree visualization would render here" }), _jsx("p", { className: "text-sm", children: "Hierarchical dependency tree view" })] });
div >
;
;
const MatrixView = () => ()
    < div, className = "flex-1 flex items-center justify-center bg-gray-50" >
    _jsxs("div", { className: "text-center text-gray-500", children: [_jsx("p", { children: "Dependency matrix would render here" }), _jsx("p", { className: "text-sm", children: "Grid-based dependency relationships" })] });
div >
;
;
const ToggleDetailsPanel = ({ toggleId, onClose }) => ()
    < div, className = "p-4" >
    (_jsxs("div", { className: "flex items-center justify-between mb-4", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Toggle Details" }), _jsx("button", { onClick: onClose, className: "text-gray-400 hover:text-gray-600", children: _jsx("svg", { className: "h-5 w-5", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) }) })] })
        ,
            _jsxs("div", { className: "text-sm", children: [_jsx("p", { className: "font-medium", children: toggleId }), _jsx("p", { className: "text-gray-600 mt-1", children: "Detailed information would display here" })] }));
div >
;
;
