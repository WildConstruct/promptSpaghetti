...
div >
    { showCommentForm } && ()
    < div;
className = "comment-system__form" >
    _jsx(CommentForm, { onSubmit: (content) => handleCreateComment(content), onCancel: () => setShowCommentForm(false), placeholder: "Add a comment...", submitText: "Post Comment", userId: userId });
div >
;
_jsxs("div", { className: "comment-system__content", children: [comments.length === 0 && !loading ? ()
            < div : , " className=\"comment-system__empty\">", _jsxs("div", { className: "empty-state", children: [_jsx("div", { className: "empty-state__icon", children: "\uD83D\uDCAC" }), _jsx("div", { className: "empty-state__title", children: "No comments yet" }), _jsx("div", { className: "empty-state__message", children: "Start a conversation by adding the first comment." }), !showCommentForm && ()
                    < button, "className=\"btn btn--primary\" onClick=", () => setShowCommentForm(true), "> Add First Comment"] }), ")}"] });
div >
;
()
    < div;
className = "comment-system__threads" >
    { comments, : .map((comment, index) => ()
            < CommentThread, key = { comment, : .id }, comment = { comment }, workspaceId = { workspaceId }, userId = { userId }, onReply = {}(content), handleCreateComment(content, comment.id)) };
...
/>;
{
    hasMore && ()
        < div;
    className = "comment-system__load-more" >
        _jsx("button", { className: "btn btn--ghost", onClick: loadMore, disabled: loading, children: loading ? 'Loading...' : 'Load More Comments' });
    div >
    ;
}
div >
