...
_jsxs("div", { className: "field-input", children: [field.type === 'boolean' && ()
            < label, " className=\"checkbox-label\">", _jsx("input", { type: "checkbox", checked: value || false, onChange: (e) => handleFieldChange(field.key, e.target.checked) }), _jsx("span", { className: "checkbox-text", children: value ? 'Enabled' : 'Disabled' })] });
{
    field.type === 'string' && ()
        < input;
    type = "text";
...
;
{
    ...
    className = "array-input" >
        _jsx(ArrayInput, { value: value || [], onChange: (newValue) => handleFieldChange(field.key, newValue), placeholder: "Add item..." });
    div >
    ;
}
div >
...
className = "field-error" >
    (_jsx("span", { className: "error-icon", children: "\u274C" })
        ,
            _jsx("span", { className: "error-text", children: error }));
div >
;
div >
;
...
;
return;
_jsxs("div", { className: "extension-config-panel-overlay", children: [_jsxs("div", { className: "extension-config-panel", children: [_jsxs("div", { className: "config-header", children: [_jsxs("div", { className: "header-left", children: [_jsxs("h2", { children: ["Configure ", extension.name] }), _jsxs("p", { className: "config-subtitle", children: ["v", extension.version, " by ", extension.author] })] }), _jsx("button", { className: "close-btn", onClick: onCancel, children: "\u2715" })] }), _jsxs("div", { className: "config-tabs", children: [_jsx("button", { className: activeTab === 'general' ? 'active' : '', onClick: () => setActiveTab('general'), children: "General" }), _jsx("button", { className: activeTab === 'advanced' ? 'active' : '', onClick: () => setActiveTab('advanced'), children: "Advanced" }), _jsx("button", { className: activeTab === 'security' ? 'active' : '', onClick: () => setActiveTab('security'), children: "Security" })] }), _jsxs("div", { className: "config-content", children: [activeTab === 'general' && ()
                            < div, " className=\"config-section\">", _jsx("h3", { children: "General Settings" }), _jsx("div", { className: "config-fields", children: generalFields.map(renderField) })] }), ")}", activeTab === 'advanced' && ()
                    < div, " className=\"config-section\">", _jsx("h3", { children: "Advanced Settings" }), _jsxs("div", { className: "config-warning", children: [_jsx("span", { className: "warning-icon", children: "\u26A0\uFE0F" }), _jsx("span", { children: "Advanced settings should only be modified by experienced users." })] }), _jsx("div", { className: "config-fields", children: advancedFields.map(renderField) })] }), ")}", activeTab === 'security' && ()
            < div, " className=\"config-section\">", _jsx("h3", { children: "Security & Privacy" }), _jsx("div", { className: "config-fields", children: securityFields.map(renderField) }), _jsxs("div", { className: "security-info", children: [_jsx("h4", { children: "Security Information" }), _jsxs("div", { className: "security-item", children: [_jsx("span", { className: "security-label", children: "Sandboxed:" }), _jsx("span", { className: "security-value", children: extension.security?.sandbox?.enabled ? '✅ Yes' : '❌ No' })] }), _jsxs("div", { className: "security-item", children: [_jsx("span", { className: "security-label", children: "Permissions:" }), _jsx("span", { className: "security-value", children: extension.permissions?.join(', ') || 'None' })] })] })] });
div >
    _jsxs("div", { className: "config-footer", children: [_jsx("div", { className: "footer-left", children: _jsx("button", { className: "reset-btn", onClick: handleReset, children: "\uD83D\uDD04 Reset to Defaults" }) }), _jsxs("div", { className: "footer-right", children: [_jsx("button", { className: "cancel-btn", onClick: onCancel, children: "Cancel" }), _jsx("button", { className: "save-btn", onClick: handleSave, disabled: Object.keys(errors).length > 0, children: "Save Configuration" })] })] });
{
    isDirty && ()
        < div;
    className = "dirty-indicator" >
        (_jsx("span", { className: "dirty-icon", children: "\u2022" })
            ,
                _jsx("span", { children: "Unsaved changes" }));
    div >
    ;
}
div >
...
const ArrayInput = ({ value, onChange, placeholder }) => {
    ...
    const addItem = () => {
        ...
        _jsxs("div", { className: "array-input-container", children: [_jsx("div", { className: "array-items", children: value.map((item, index) => ()
                        ...
                        (_jsx("span", { className: "item-text", children: item })
                            ,
                                _jsx("button", { className: "remove-item-btn", onClick: () => removeItem(index), title: "Remove item", children: "\u2715" }))) }), "))}"] })
            ,
                _jsxs("div", { className: "array-input-row", children: [_jsx("input", { type: "text", className: "array-text-input", value: newItem, onChange: (e) => setNewItem(e.target.value), placeholder: placeholder, onKeyPress: (e) => e.key === 'Enter' && addItem() }), _jsx("button", { className: "add-item-btn", onClick: addItem, disabled: !newItem.trim(), children: "Add" })] });
    };
};
div >
;
