...
import { useExternalDataImport, useHistoricalQueryBuilder } from useDataSourceCache;
from;
'../../hooks/useExternalDataImport';
const ERA_OPTIONS = [
...
onError;
{
    ...
    const { state: dataState, queryData, refreshData, clearCache, validateQuery, getQuerySuggestions };
    exportResults
...
;
const { query, isValid, validationErrors, updateQuery, resetQuery };
buildQuery
            ...
            overflow: 'hidden'
                    ...
                    flexDirection: 'column'
                            ...
                            borderBottom: '1px solid #4a5568'
                                            ...
                                            marginBottom: 6,
                                            fontWeight: 500
                                        }, children: "Historical Era *" }), _jsx("select", { value: Array.isArray(query.era) ? query.era[0] : query.era || '', onChange: (e) => updateQuery({ era: e.target.value }), style: {
                                            width: '100%',
                                            padding: 8,
                                            background: '#2d3748',
                                            border: '1px solid #4a5568',
                                            ...
                                            >
                                                _jsx("option", { value: "", children: "Select era..." }), ...ERA_OPTIONS.map(era => ()
                            ...
                            marginBottom: 6,
                            fontWeight: 500
                        }, children: "Category *" }), _jsx("select", { value: query.category || '', onChange: (e) => updateQuery({ category: e.target.value }), style: {
                            width: '100%',
                            padding: 8,
                            background: '#2d3748',
                            border: '1px solid #4a5568',
                            ...
                            >
                                _jsx("option", { value: "", children: "Select category..." }), ...CATEGORY_OPTIONS.map(category => ()
...
_jsxs("div", { style: { marginBottom: 16 }, children: [_jsx("label", { style: {
                ...
                marginBottom: 6,
                fontWeight: 500
            }, children: "Region" }), _jsx("select", { value: Array.isArray(query.region) ? query.region[0] : query.region || '', onChange: (e) => updateQuery({ region: e.target.value || undefined }), style: {
                width: '100%',
                padding: 8,
                background: '#2d3748',
                border: '1px solid #4a5568',
                ...
                >
                    _jsx("option", { value: "", children: "Any region..." }), ...REGION_OPTIONS.map(region => ()
...
style = {};
{
 ...
 >
    (_jsx("label", { style: {
        ...
        ,
            _jsx("input", { type: "text", value: query.keywords?.join(', ') || '', onChange: (e) => updateQuery({}), "keywords:e": true, target: true, value: true, split: true }));
...
style = {};
{
 ...
 >
    _jsxs("button", { onClick: () => setShowAdvanced(!showAdvanced), style: {
            background: 'none',
            border: 'none',
            color: '#4299e1',
            fontSize: 12,
            ...
            padding: 0,
            marginBottom: 8
        }, children: [showAdvanced ? '▼' : '▶', " Advanced Options"] });
{
    showAdvanced && ()
        < div >
...
style = {};
{
 ...
 >
    _jsx("button", { onClick: handleImportData, disabled: !isValid || dataState.isLoading, style: {
            ...
            >
                { dataState, : .isLoading ? 'Importing...' : 'Import Data' }, button: true, children: _jsx("button", { onClick: handleClearAll, style: {
                padding: '10px 12px',
                background: '#4a5568',
                border: 'none',
                borderRadius: 4,
...
const HistoricalDataResults = ({ results }) => {
    ...
    const toggleExpanded = useCallback((itemId) => { }, setExpandedItems(prev => { }));
