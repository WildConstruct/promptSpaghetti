...
export const DataSourceConfigurationPanel = ({
    visible,
    onClose,
    onSave });
initialDataSources = [];
...
[];
;
const handleEditSource = useCallback((source) => { setEditingSource({}); }, ...source, authentication, {
    type: source.authentication?.type || 'none'
}, credentials, source.authentication?.credentials || {}, headers, source.authentication?.headers || {});
;
setShowForm(true);
[];
;
        ...
        zIndex: 2000
                ...
                overflow: 'hidden'
                        ...
                        flexDirection: 'column'
                                ...
                                alignItems: 'center'
                                ...
                                }, border), ": `1px solid $", selectedSourceId === source.id ? '#4299e1' : '#4a5568', "`} borderRadius: 6 cursor: 'pointer'; onClick=", () => setSelectedSourceId(source.id), ">", _jsxs("div", { style: {
                                        display: 'flex',
                                        justifyContent: 'space-between',
                                    ...
                                    < span, " style=", { color: '#a0aec0', fontSize: 10 }, "> +", source.metadata.tags.length - 3] }), ")}"] }), _jsx("div", { style: { display: 'flex', gap: 4 }, children: _jsx("button", { onClick: (e) => {
                            e.stopPropagation();
                            ...
                            >
                                { source, : .enabled ? 'Disable' : 'Enable' } }) }), _jsx("button", { onClick: (e) => {
                        e.stopPropagation();
...
onUpdate: (updates) => void onSave;
() => void ;
onCancel: () => void onAddTag;
(tag) => void ;
...
({ source, onUpdate, onSave, onCancel, onAddTag, onRemoveTag }) => {
    ...
    _jsxs("div", { style: { height: '100%', overflowY: 'auto', padding: 24 }, children: [_jsxs("div", { style: { maxWidth: 800 }, children: [_jsxs("div", { style: { marginBottom: 24 }, children: [_jsx("h3", { style: { color: '#e2e8f0', marginBottom: 16, fontSize: 16 }, children: "Basic Information" }), _jsxs("div", { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }, children: [_jsxs("div", { children: [_jsx("label", { style: { display: 'block', color: '#a0aec0', fontSize: 12, marginBottom: 4 }, children: "Name *" }), _jsx("input", { type: "text", value: source.name, onChange: (e) => onUpdate({ name: e.target.value }), style: {
                                                    ...
                                                    color: '#e2e8f0',
                                                    fontSize: 14
                                                }, placeholder: "Enter source name" })] }), _jsxs("div", { children: [_jsx("label", { style: { display: 'block', color: '#a0aec0', fontSize: 12, marginBottom: 4 }, children: "Type *" }), _jsx("select", { value: source.type, onChange: (e) => onUpdate({ type: e.target.value }), style: {
                                                    width: '100%',
                                                    padding: 8,
                                                    background: '#1a202c',
                                                    border: '1px solid #4a5568',
    ...
    { /* Tags */ }
    _jsxs("div", { style: { marginBottom: 24 }, children: [_jsx("h3", { style: { color: '#e2e8f0', marginBottom: 16, fontSize: 16 }, children: "Tags" }), _jsxs("div", { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }, children: [source.metadata.tags.map(tag => ()
                        < span, key = { tag }, style = {}, {
                        background: '#4a5568',
                        color: '#e2e8f0',
                        padding: '4px 8px',
        ...
        ,
            _jsx("div", { style: { display: 'flex', gap: 8 }, children: _jsx("input", { type: "text", value: newTag, onChange: (e) => setNewTag(e.target.value), onKeyDown: (e) => e.key === 'Enter' && handleTagAdd(), placeholder: "Add tag...", style: {
                        flex: 1,
                        padding: 6,
                        background: '#1a202c',
                        border: '1px solid #4a5568',
 ...
 >
    _jsx("button", { onClick: onSave, disabled: !source.name.trim(), style: {
            padding: '10px 20px',
            ...
            >
                Save, Data: true, Source: true, button: true, children: _jsx("button", { onClick: onCancel, style: {
                padding: '10px 20px',
                background: '#4a5568',
                border: 'none',
                borderRadius: 4,
