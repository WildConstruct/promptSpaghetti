...
import { CheckCircleIcon, ClockIcon, XCircleIcon, DocumentTextIcon, EyeIcon, GlobeAltIcon, ArchiveBoxIcon, PlusIcon, PencilIcon, ChevronDownIcon, ChevronRightIcon, LockClosedIcon } from ClipboardDocumentListIcon;
from;
'@heroicons/react/24/outline';
import { useWorkflowStore } from '../stores/workflowStore';
export const WorkflowStateManager = ({ workspaceId,
    resourceId,
    currentUserId,
    onStateChange,
    onLockAcquired });
onLockReleased;
{
    const { states, transitions, approvals, locks, statistics, loading, error, fetchStates, fetchTransitions, fetchApprovals, fetchLocks, fetchStatistics, transitionResourceState, approveWorkflow, rejectWorkflow, acquireLock, releaseLock, createState, updateState, deleteState, createTransition };
    deleteTransition
        = useWorkflowStore();
  ...
  lock:`, error);
            ...
            return _jsx(DocumentTextIcon, { className: "h-4 w-4" });
        }
        ;
        const getStateTransitions = (stateId) => { return transitions.filter(t => t.from_state_id === stateId); };
        if (loading) {
            return;
...
{ /* Expanded State Details */ }
{
    ...
    className = "mt-4 space-y-3" >
        _jsxs("div", { className: "border-t pt-3", children: [_jsx("h4", { className: "text-sm font-medium text-gray-900 mb-2", children: "Available Transitions" }), _jsx("div", { className: "space-y-2", children: getStateTransitions(state.id).map(transition => ()
                        < div, key = { transition, : .id }, className = "flex items-center justify-between p-2 bg-gray-50 rounded"
                        >
                            _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("span", { className: "text-sm font-medium", children: transition.name }), transition.requires_approval && ()
                                        < span, " className=\"px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded\"> Requires Approval"] })) }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("span", { className: "text-sm text-gray-600", children: ["\u2192 ", states.find(s => s.id === transition.to_state_id)?.name] }), resourceId && currentResourceState?.id === state.id && ()
                            < button, "onClick=", () => handleStateTransition(transition.to_state_id), "className=\"px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700\" > Transition"] }), ")}"] });
    div >
    ;
}
{
    getStateTransitions(state.id).length === 0 && ()
        < p;
    className = "text-sm text-gray-500 italic" > No;
    transitions;
    available;
    p >
    ;
...
;
{
    activeTab === 'approvals' && ()
        < div;
    className = "space-y-4" >
        _jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Pending Approvals" });
    {
        pendingApprovals.length === 0 ? ()
            < div : ;
        className = "text-center py-8" >
            (_jsx(ClipboardDocumentListIcon, { className: "mx-auto h-12 w-12 text-gray-400" })
                ,
                    _jsx("h3", { className: "mt-2 text-sm font-medium text-gray-900", children: "No pending approvals" })
                        ,
                            _jsx("p", { className: "mt-1 text-sm text-gray-500", children: "All workflow approvals are up to date." }));
        div >
        ;
        ()
            < div;
        className = "space-y-3" >
            { pendingApprovals, : .map(approval => ()
                    ...
                    >
                        (_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx(ClockIcon, { className: "h-4 w-4 text-yellow-500" }), _jsx("span", { className: "text-sm font-medium", children: "Approval Required" })] }), _jsx("span", { className: `px-2 py-1 rounded text-xs ${approval.priority === 'high' ? 'bg-red-100 text-red-800' :
                                                approval.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : }
  'bg-green-100 text-green-800'
`, children: approval.priority })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("button", { onClick: () => handleApprovalAction(approval.id, 'approve'), className: "px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700", children: "Approve" }), _jsx("button", { onClick: () => handleApprovalAction(approval.id, 'reject'), className: "px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700", children: "Reject" })] })] })
                            ,
                                _jsxs("div", { className: "mt-2 text-sm text-gray-600", children: [_jsxs("p", { children: ["Requested by: ", approval.requester_id] }), _jsxs("p", { children: ["Resource: ", approval.resource_id] }), approval.due_date && ()
                                            < p > Due, ": ", new Date(approval.due_date).toLocaleDateString()] }))) };
        div >
        ;
        div >
        ;
...
;
{
    activeTab === 'locks' && ()
        < div;
    className = "space-y-4" >
        _jsxs("div", { className: "flex items-center justify-between", children: [_jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Resource Locks" }), resourceId && ()
                    < button, "onClick=", () => handleLockAction('acquire'), "className=\"inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700\" >", _jsx(LockClosedIcon, { className: "h-4 w-4 mr-2" }), "Acquire Lock"] });
}
div >
    { resourceLocks, : .length === 0 ? ()
            < div : , className = "text-center py-8" >
            (_jsx(LockClosedIcon, { className: "mx-auto h-12 w-12 text-gray-400" })
                ,
                    _jsx("h3", { className: "mt-2 text-sm font-medium text-gray-900", children: "No active locks" })
                        ,
...
className = "space-y-3" >
    { resourceLocks, : .map(lock => ()
            < div, key = { lock, : .id }, className = "border border-gray-200 rounded-lg p-4 hover:bg-gray-50"
            >
                _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx(LockClosedIcon, { className: "h-4 w-4 text-yellow-500" }), _jsxs("div", { children: [_jsxs("span", { className: "text-sm font-medium", children: [lock.lock_type, " Lock"] }), _jsxs("p", { className: "text-xs text-gray-500", children: ["Locked by ", lock.locked_by, " on ", new Date(lock.locked_at).toLocaleString()] })] })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [lock.expires_at && ()
                                    < span, " className=\"text-xs text-gray-500\"> Expires: ", new Date(lock.expires_at).toLocaleString()] }), ")}", lock.locked_by === currentUserId && ()
                            < button, "onClick=", () => handleLockAction('release', lock.id), "className=\"px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700\" > Release"] })) };
div >
;
div >
    { lock, : .lock_reason && ()
...
;
{
    activeTab === 'statistics' && ()
        < div;
    className = "space-y-6" >
        _jsx("h3", { className: "text-lg font-medium text-gray-900", children: "Workflow Statistics" });
    {
        statistics && ()
            < div;
        className = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" >
            (_jsx("div", { className: "bg-blue-50 p-4 rounded-lg", children: _jsxs("div", { className: "flex items-center", children: [_jsx(DocumentTextIcon, { className: "h-8 w-8 text-blue-600" }), _jsxs("div", { className: "ml-4", children: [_jsx("p", { className: "text-sm font-medium text-blue-900", children: "Total States" }), _jsx("p", { className: "text-2xl font-bold text-blue-600", children: statistics.total_states })] })] }) })
                ,
                    _jsx("div", { className: "bg-green-50 p-4 rounded-lg", children: _jsxs("div", { className: "flex items-center", children: [_jsx(CheckCircleIcon, { className: "h-8 w-8 text-green-600" }), _jsxs("div", { className: "ml-4", children: [_jsx("p", { className: "text-sm font-medium text-green-900", children: "Approved" }), _jsx("p", { className: "text-2xl font-bold text-green-600", children: statistics.approval_stats.approved })] })] }) })
                        ,
