...
subscribe(callback, (canUndo, canRedo, current) => void );
() => void { this: .listeners.add(callback),
    return() { }, this: .listeners.delete(callback) };
notifyListeners();
void { const: canUndo = this.canUndo(),
    ...
    ,
    // Professional Undo/Redo UI Component
    const: UndoRedoManager, React, : (.FC) = ({ onStateChange,
        maxHistorySize = 50,
        theme = 'cinema' })
};
{
    ...
    const handleHistorySelect = useCallback((index) => {
        ...
        setShowHistory(false)[undoSystem, onStateChange];
    });
    // Theme styles
...
React.useImperativeHandle(ref => ({ addState: (nodes, edges, description) => { },
    undoSystem, : .addState(nodes, edges, description),
    undo: handleUndo,
    redo: handleRedo,
    canUndo: () => canUndo,
    canRedo: () => canRedo,
    clear: () => undoSystem.clear() }), [undoSystem, handleUndo, handleRedo, canUndo, canRedo]);
return (_jsxs("div", { style: { position: 'relative', display: 'flex', alignItems: 'center', gap: '8px' }, children: [_jsx("button", { onClick: handleUndo, disabled: !canUndo, title: "Undo (\u2318Z)", style: {
                padding: '8px 12px',
                background: styles.background
            }, "border:": true }), " `1px solid $", styles.border, "` borderRadius: '6px' color: canUndo ? styles.text : styles.disabled cursor: canUndo ? 'pointer' : 'not-allowed' opacity: canUndo ? 1 : 0.5 fontSize: '14px' fontWeight: '500' display: 'flex' alignItems: 'center' gap: '6px' transition: 'all var(--transition-normal)' fontFamily: 'var(--font-family-primary)' >", _jsx("span", { style: { fontSize: '16px' }, children: "\u21B6" }), "Undo"] })) /* Redo Button */;
{ /* Redo Button */ }
_jsx("button", { onClick: handleRedo, disabled: !canRedo, title: "Redo (\u2318\u21E7Z)", style: {
        padding: '8px 12px',
        background: styles.background
    }, "border:": true });
...
button >
    { /* History Dropdown */};
{
    showHistory && (_jsx("div", { ref: historyRef, style: {
            position: 'absolute',
            top: '100%',
            right: 0,
            marginTop: '8px',
    ...
    div >
        _jsxs("div", { style: { maxHeight: '300px', overflow: 'auto' }, children: [undoSystem.getHistory().map((state, index) => {
                    const isCurrent = index === undoSystem.getCurrentIndex();
                    const relativeTime = new Date(state.timestamp).toLocaleTimeString();
                    return (_jsx("div", { onClick: () => handleHistorySelect(index), style: {
                            padding: '12px 16px',
                            cursor: 'pointer',
                            background: isCurrent ? styles.accent + '20' : 'transparent'
                        }, "borderLeft:isCurrent": true }, state.id) ? `4px solid ${styles.accent}` : '4px solid transparent');
