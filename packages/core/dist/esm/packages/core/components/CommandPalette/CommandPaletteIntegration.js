...
disabled ?  : boolean;
export const CommandPaletteIntegration = ({
    theme = 'cinema',
    onNodeCreate,
    onNodesDelete,
    onExport,
...
});
{
    ...
    const handleGenerationStart = useCallback(async (flow, params) => {
        ...
        finally // Generate content based on the flow
         { }
    });
...
}
{
    ...
    for (const act of acts) {
        ...
        choices: act.elements.map((element, index) => ({}), text, element, weight, 100 - (index * 5));
    }
}
currentPosition;
;
