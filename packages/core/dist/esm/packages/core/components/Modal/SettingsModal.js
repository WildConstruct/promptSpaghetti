...
import { FiPlayCircle, FiPackage, FiMonitor, FiEye, FiRotateCcw, FiDownload, FiUpload, FiSave, FiCheck } from FiAlertTriangle;
from;
'react-icons/fi';
...
onSettingsChange;
{
    ...
    useEffect(() => {
        const unsubscribe = settingsManager.addChangeListener((event) => { }, setSettings(settingsManager.getSettings()));
...
const renderSettingsSection = (sectionId) => {
    switch (sectionId) {
        case 'seed':
            return;
            _jsx(SeedControls, { settings: settings.seed, onChange: (seedSettings) => handleSettingsUpdate({ seed: seedSettings }) });
    }
};
;
'temperature';
return;
_jsx(TemperatureControls, { settings: settings.temperature, onChange: (tempSettings) => handleSettingsUpdate({ temperature: tempSettings }) });
;
'runCount';
return;
_jsx(RunCountControls, { settings: settings.runCount, onChange: (runSettings) => handleSettingsUpdate({ runCount: runSettings }) });
;
'batch';
return;
_jsx(BatchControls, { settings: settings.batch, onChange: (batchSettings) => handleSettingsUpdate({ batch: batchSettings }) });
;
'performance';
return;
_jsx(PerformanceControls, { settings: settings.performance, onChange: (perfSettings) => handleSettingsUpdate({ performance: perfSettings }) });
;
'ui';
return;
_jsx(UIControls, { settings: settings.ui, onChange: (uiSettings) => handleSettingsUpdate({ ui: uiSettings }) });
;
return null;
;
return;
_jsxs(Modal, { isOpen: isOpen, onClose: onClose, title: "Advanced Settings", size: "large", "aria-describedby": "settings-description", children: [_jsx("div", { id: "settings-description", className: "sr-only", children: "Configure advanced execution, batch processing, performance, and interface settings" }), _jsxs("div", { style: { display: 'flex', height: '600px', gap: '24px' }, children: [_jsx("div", { style: {
                        width: '240px'
                    }, "borderRight:": true }), " `1px solid $", uiColors.ui.border, "`} paddingRight: '24px'; }>", _jsx("div", { style: {
                        marginBottom: '16px',
                        fontSize: '14px',
                        fontWeight: 500,
                        ...
                        letterSpacing: '0.5px'
                    }, children: "Settings Groups" }), SETTINGS_GROUPS.map((group) => ()
                    < button, key = { group, : .id }, onClick = {}()), " => setActiveGroup(group.id)} style=", {
                    width: '100%',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '12px',
                    ...
                    ? `1px solid ${uiColors.accent.primary}` : , ": '1px solid transparent' borderRadius: '8px' cursor: 'pointer' textAlign: 'left' transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' outline: 'none'; onMouseEnter=", (e) => {
                    ...
                    if (activeGroup !== group.id) {
                        ...
                        >
                            _jsx(group.icon, { size: 18, color: activeGroup === group.id
...
{ /* Settings Content */ }
_jsxs("div", { style: { flex: 1, overflow: 'auto' }, children: [SETTINGS_GROUPS.find(g => g.id === activeGroup)?.sections.map(sectionId => ()
            < div, key = { sectionId }, style = {}, { marginBottom: '32px' }), ">", renderSettingsSection(sectionId)] });
div >
;
div >
    { /* Settings Footer */}
...
style = {};
{
 ...
 >
    _jsx("button", { onClick: handleExport, style: {
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            padding: '8px 12px',
...
title = "Reset all settings to defaults (Ctrl+R)"
    >
        _jsx(FiRotateCcw, { size: 14 });
Reset;
button >
    _jsxs("button", { onClick: handleSave, disabled: !hasUnsavedChanges, style: {
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            padding: '8px 16px',
