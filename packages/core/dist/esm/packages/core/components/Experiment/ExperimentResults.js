    ...
    { /* Winner Detection Alert */};
{
    ...
    className = "mb-4 border-green-200 bg-green-50" >
        (_jsx(Award, { className: "h-4 w-4" })
            ,
                _jsxs(AlertDescription, { children: [_jsx("strong", { children: "Statistical Winner Detected!" }), " Variant \"", winningVariant, "\" shows statistically significant improvement with ", ((1 - results.statistical.primaryMetric.pValue) * 100).toFixed(1), "% confidence."] }));
    Alert >
    ;
}
{ /* Key Insights */ }
{
    results.insights.length > 0 && ()
        < Card;
    className = "mb-4" >
        (_jsx(CardHeader, { children: _jsxs(CardTitle, { className: "flex items-center", children: [_jsx(Lightbulb, { className: "w-5 h-5 mr-2" }), "Key Insights"] }) })
            ,
...
value = "trends";
className = "space-y-6" >
    _jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Performance Trends" }) }), _jsx(CardContent, { children: _jsx(ResponsiveContainer, { width: "100%", height: 400, children: _jsxs(LineChart, { data: getTimeSeriesData(), children: [_jsx(CartesianGrid, { strokeDasharray: "3 3" }), _jsx(XAxis, { dataKey: "day" }), _jsx(YAxis, {}), _jsx(Tooltip, {}), _jsx(Legend, {}), results.variants.map((variant, index) => ()
                                < Line, key = { variant, : .variantId }, type = "monotone", dataKey = { variant, : .variantId }, stroke = { COLORS, [index % COLORS.length]:  }, strokeWidth = { 2:  }
                                /  >
                            ), ")}"] }) }) })] });
TabsContent >
    { /* Segments Tab */}
...
className = "space-y-6" >
    { results, : .segments.length > 0 ? ()
        ...
        results, : .segments.map((segment, index) => ()
            < Card, key = { index } >
            (_jsx(CardHeader, { children: _jsxs(CardTitle, { children: ["Segment: ", segment.segment.name] }) })
                ,
                    _jsx(CardContent, { children: _jsxs("div", { className: "text-sm text-gray-600 mb-4", children: ["Sample Size: ", segment.sampleSize.toLocaleString(), " | Significant: ", segment.significance ? 'Yes' : 'No'] }) })), Card >
        )
    }()
    < Card >
    _jsx(CardContent, { className: "text-center py-8", children: _jsx("div", { className: "text-gray-500", children: "No segment analysis available" }) });
Card >
;
TabsContent >
    { /* Statistical Tab */}
...
value = "statistical";
className = "space-y-6" >
    _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-6", children: _jsxs(Card, { children: [_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Statistical Summary" }) }), _jsxs(CardContent, { className: "space-y-4", children: [_jsxs("div", { children: [_jsx("div", { className: "text-sm text-gray-600", children: "P-Value" }), _jsx("div", { className: "text-lg font-semibold", children: results.statistical.primaryMetric.pValue.toFixed(4) })] }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-gray-600", children: "Confidence Level" }), _jsxs("div", { className: "text-lg font-semibold", children: [(results.statistical.primaryMetric.confidenceLevel * 100).toFixed(1), "%"] })] }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-gray-600", children: "Statistical Significance" }), _jsxs("div", { className: "text-lg font-semibold", children: [results.statistical.primaryMetric.statisticalSignificance ? ()
                                            < Badge : , " variant=\"default\" className=\"bg-green-500\">Significant"] }), ") : ()", _jsx(Badge, { variant: "secondary", children: "Not Significant" }), ")}"] })] }), _jsxs("div", { children: [_jsx("div", { className: "text-sm text-gray-600", children: "Practical Significance" }), _jsxs("div", { className: "text-lg font-semibold", children: [results.statistical.primaryMetric.practicalSignificance ? ()
                                    < Badge : , " variant=\"default\" className=\"bg-green-500\">Yes"] }), ") : ()", _jsx(Badge, { variant: "secondary", children: "No" }), ")}"] })] }) });
CardContent >
;
Card >
    { /* Guardrail Metrics */}
    < Card >
    (_jsx(CardHeader, { children: _jsx(CardTitle, { children: "Guardrail Metrics" }) })
        ,
            _jsxs(CardContent, { children: [_jsx("div", { className: "space-y-3", children: results.statistical.guardrailMetrics.map((guardrail, index) => ()
                            < div, key = { index }, className = "flex items-center justify-between" >
                            (_jsx("div", { className: "text-sm", children: guardrail.metricId })
                                ,
                                    _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("div", { className: "text-sm text-gray-600", children: [(guardrail.actualValue * 100).toFixed(2), "%"] }), guardrail.passed ? ()
                                                < CheckCircle : , " className=\"w-4 h-4 text-green-500\" /> ) : ()", _jsx(AlertTriangle, { className: "w-4 h-4 text-red-500" }), ")}"] }))) }), "))}"] }));
CardContent >
;
Card >
;
