            ...
            const getIndicatorColor = () => {
                ...
                const handleDismiss = () => {
                    ...
                    if (!isVisible) {
                        ...
                        return;
                        _jsx("div", { className: `offline-indicator ${position} ${className}`, style: {
                                position: 'fixed',
                                left: 0,
                                right: 0,
                            ...
                            }, "borderTop:position": true }) === 'bottom' ? `2px solid ${colors.border}` : undefined;
                    }
                }, color, padding, zIndex, fontSize, fontWeight, boxShadow;
            };
        };
            >
                _jsxs("div", { className: "flex items-center justify-between max-w-6xl mx-auto", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-lg", children: !status.isOnline ? '📴' : status.queueSize > 0 ? '⏳' : 'ℹ️' }), _jsx("span", { children: message })] }), showQueueInfo && status.queueSize > 0 && ()
                                    < div, " className=\"flex items-center gap-4 text-sm\">", _jsxs("div", { className: "flex items-center gap-1", children: [_jsx("span", { children: "Pending:" }), _jsx("span", { className: "font-bold", children: status.queueSize })] }), status.metrics.syncedOperations > 0 && ()
                                    < div, " className=\"flex items-center gap-1\">", _jsx("span", { children: "Synced:" }), _jsx("span", { className: "font-bold", children: status.metrics.syncedOperations })] }), ")}"] });
    };
}
div >
    _jsxs("div", { className: "flex items-center gap-2", children: [showActions && (), !status.isOnline && onRetryConnection && ()
                < button, "onClick=", onRetryConnection, "className=\"px-3 py-1 text-sm font-medium rounded transition-colors\" style=", {
                backgroundColor: 'transparent',
                border: `1px solid ${colors.button}`
            }, ", color: colors.button; }} onMouseEnter=", (e) => {
