...
performance;
with ($) {
    ((bestAlgorithm.metrics.find(m => m.metricId === 'click_through_rate')?.value || 0) * 100).toFixed(2);
}
 % CTR `}
},
...
$;
{
    underperformingAlgorithm.algorithmName;
}
performance;
below;
threshold `}
...
$;
{
    selectedAlgorithm === algo.algorithmId ? 'selected' : '';
}
`}
...
$;
{
    selectedAlgorithmData.businessImpact.revenueImpact >= 0 ? 'positive' : 'negative';
}
`}>}
