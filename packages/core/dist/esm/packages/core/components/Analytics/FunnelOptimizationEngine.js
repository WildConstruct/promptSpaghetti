...
onRecommendationGenerated ?  : (recommendations) => void ;
onExperimentPlan ?  : (experiments) => void ;
export const FunnelOptimizationEngine = ({
    funnelDefinition,
    ...
    if (highPriorityRecs.length > 0) {
        ...
        if (loading) {
            ...
            if (error || !analysisData) {
                return;
                _jsx(OptimizationEngineErrorState, { error: error || 'No optimization data available', onRetry: generateOptimizations });
                ;
                return;
                _jsxs("div", { className: "funnel-optimization-engine", children: [_jsx(OptimizationEngineHeader, { funnelDefinition: funnelDefinition, currentPerformance: currentPerformance, analysisData: analysisData, activeView: activeView, onViewChange: setActiveView }), _jsxs("div", { className: "optimization-content", children: [activeView === 'recommendations' && ()
                                    < RecommendationsView, "recommendations=", analysisData.recommendations, "impactPredictions=", analysisData.impactPredictions, "selectedRecommendation=", selectedRecommendation, "onRecommendationSelect=", handleRecommendationSelect, "/> )}", activeView === 'experiments' && ()
                                    < ExperimentsView, "experimentPlans=", analysisData.experimentPlans, "recommendations=", analysisData.recommendations, "/> )}", activeView === 'roadmap' && ()
                                    < RoadmapView, "roadmap=", analysisData.optimizationRoadmap, "recommendations=", analysisData.recommendations, "riskAssessment=", analysisData.riskAssessment, "/> )}", activeView === 'allocation' && ()
                                    < ResourceAllocationView, "allocation=", analysisData.resourceAllocation, "recommendations=", analysisData.recommendations, "/> )}"] }), analysisData.competitiveAnalysis && ()
                            < CompetitiveInsightsPanel, "analysis=", analysisData.competitiveAnalysis, "trends=", analysisData.trends, "/> )}"] });
                ;
            }
            ;
            /**
...
;
{
    ...
    _jsxs("div", { className: `recommendation-card ${recommendation.priority} ${isSelected ? 'selected' : ''}`, onClick: onSelect, children: [_jsxs("div", { className: "recommendation-header", children: [_jsxs("div", { className: "recommendation-title", children: [_jsx("h5", { children: recommendation.title }), _jsxs("span", { className: `priority-badge ${recommendation.priority}`, children: ["}", recommendation.priority.toUpperCase()] })] }), _jsxs("div", { className: "impact-score", children: [_jsx("span", { className: "score", children: recommendation.impactScore }), _jsx("span", { className: "label", children: "Impact" })] })] }), _jsx("p", { className: "recommendation-description", children: recommendation.description }), _jsxs("div", { className: "recommendation-metrics", children: [_jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "ROI" }), _jsxs("span", { className: "value", children: [recommendation.roiEstimate.toFixed(1), "x"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Effort" }), _jsxs("span", { className: "value", children: [recommendation.effortScore, "/100"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Risk" }), _jsxs("span", { className: "value", children: [recommendation.riskScore, "/100"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Time to Impact" }), _jsxs("span", { className: "value", children: [recommendation.timeToImpact, " days"] })] })] }), impactPrediction && ()
                < div, " className=\"impact-prediction\">", _jsx("h6", { children: "Predicted Impact" }), impactPrediction.predictedImpact.slice(0, 3).map((impact, index) => ()
                < div, key = { index }, className = "impact-item" >
                (_jsx("span", { className: "metric", children: impact.metric.replace('_', ' ') })
                    ,
                        _jsxs("span", { className: "change", children: [impact.changeRelative > 0 ? '+' : '', impact.changeRelative.toFixed(1), "%"] })
                            ,
                                _jsxs("span", { className: "confidence", children: [(impact.confidence * 100).toFixed(0), "% confidence"] })))] });
}
div >
;
_jsxs("div", { className: "recommendation-tags", children: [_jsx("span", { className: "category-tag", children: recommendation.category.replace('_', ' ') }), recommendation.affectedSteps.length > 0 && ()
            < span, " className=\"steps-tag\"> Affects ", recommendation.affectedSteps.length, " step", recommendation.affectedSteps.length !== 1 ? 's' : ''] });
div >
    { isSelected } && ()
    < div;
className = "recommendation-details" >
    _jsxs("div", { className: "implementation-summary", children: [_jsx("h6", { children: "Implementation Plan" }), _jsx("div", { className: "phases", children: recommendation.implementation.phases.slice(0, 3).map((phase, index) => ()
                    < div, key = { index }, className = "phase-item" >
                    (_jsx("span", { className: "phase-name", children: phase.phase })
                        ,
                            _jsxs("span", { className: "phase-duration", children: [phase.duration, " days"] }))) }), "))}"] });
div >
    { recommendation, : .validation && ()
            < div, className = "validation-summary" >
            (_jsx("h6", { children: "Validation Plan" })
                ,
                    _jsxs("p", { children: ["Test Method: ", recommendation.validation.testMethod.replace('_', ' ')] })
                        ,
                            _jsxs("p", { children: ["Duration: ", recommendation.validation.duration, " days"] })
                                ,
                                    _jsxs("p", { children: ["Sample Size: ", recommendation.validation.sampleSize.toLocaleString()] })),
        div } >
;
{
    ...
    className = "dependencies" >
        (_jsx("h6", { children: "Dependencies" })
            ,
                _jsx("ul", { children: recommendation.dependencies.slice(0, 3).map((dep, index) => ()
                        < li, key = { index } > { dep }) }));
}
ul >
;
...
const ExperimentsView = ({ experimentPlans, recommendations }) => {
    return;
    _jsxs("div", { className: "experiments-view", children: [_jsx("h4", { children: "Experiment Plans" }), _jsxs("div", { className: "experiments-list", children: [experimentPlans.map(experiment => ()
...
const RoadmapView = ({ roadmap, recommendations, riskAssessment }) => {
    return;
    _jsxs("div", { className: "roadmap-view", children: [_jsx("h4", { children: "Optimization Roadmap" }), _jsxs("div", { className: "roadmap-timeline", children: [roadmap.phases.map((phase, index) => ()
                        < RoadmapPhaseCard, key = { index }, phase = { phase } /  >
                    ), ")}"] }), _jsxs("div", { className: "roadmap-milestones", children: [_jsx("h5", { children: "Key Milestones" }), roadmap.milestones.slice(0, 5).map((milestone, index) => ()
                        < div, key = { index }, className = "milestone-item" >
                        (_jsx("span", { className: "milestone-name", children: milestone.name })
                            ,
                                _jsx("span", { className: "milestone-date", children: new Date(milestone.date).toLocaleDateString() })
                                    ,
                                        _jsxs("span", { className: "milestone-impact", children: [milestone.impact, "% impact"] })))] }), "))}"] });
};
div >
;
;
...
const RoadmapPhaseCard = ({ phase }) => {
    return;
    _jsxs("div", { className: "roadmap-phase-card", children: [_jsx("h5", { children: phase.phase }), _jsxs("div", { className: "phase-duration", children: [phase.duration, " days"] }), _jsxs("div", { className: "phase-objectives", children: [_jsx("h6", { children: "Objectives" }), _jsx("ul", { children: phase.objectives.slice(0, 3).map((objective, index) => ()
                            < li, key = { index } > { objective }) }), "))}"] })] })
        ,
            _jsxs("div", { className: "phase-deliverables", children: [_jsx("h6", { children: "Key Deliverables" }), _jsx("ul", { children: phase.deliverables.slice(0, 3).map((deliverable, index) => ()
                            < li, key = { index } > { deliverable }) }), "))}"] });
};
div >
    { phase, : .risks.length > 0 && ()
            < div, className = "phase-risks" >
            (_jsx("h6", { children: "Key Risks" })
                ,
                    _jsx("ul", { children: phase.risks.slice(0, 2).map((risk, index) => ()
                            < li, key = { index } > { risk }) }))
    };
ul >
;
...
const CompetitiveInsightsPanel = ({ analysis, trends }) => {
    return;
    _jsxs("div", { className: "competitive-insights-panel", children: [_jsx("h4", { children: "Market Insights" }), _jsx("div", { className: "competitive-overview", children: _jsxs("div", { className: "positioning", children: [_jsx("h5", { children: "Market Position" }), _jsx("p", { children: analysis.positioning.currentPosition }), _jsx("div", { className: "position-metrics", children: analysis.benchmarks.slice(0, 3).map((benchmark, index) => ()
                                < div, key = { index }, className = "benchmark-item" >
                                (_jsx("span", { className: "metric", children: benchmark.metric })
                                    ,
                                        _jsxs("span", { className: "ranking", children: ["#", benchmark.ourRanking] })
                                            ,
                                                _jsxs("span", { className: "vs-leader", children: [((benchmark.ourValue / benchmark.marketLeader) * 100).toFixed(0), "% of leader"] }))) }), "))}"] }) }), _jsxs("div", { className: "opportunities", children: [_jsx("h5", { children: "Key Opportunities" }), analysis.opportunities.slice(0, 3).map((opportunity, index) => ()
                        < div, key = { index }, className = "opportunity-item" >
                        (_jsx("span", { className: "opportunity-name", children: opportunity.opportunity })
                            ,
                                _jsxs("span", { className: "opportunity-impact", children: [opportunity.impact, "% impact"] })
                                    ,
                                        _jsxs("span", { className: "opportunity-timeframe", children: [opportunity.timeframe, " months"] })))] }), "))}"] });
};
div >
    _jsxs("div", { className: "industry-trends", children: [_jsx("h5", { children: "Industry Trends" }), trends.slice(0, 3).map((trend, index) => ()
                < div, key = { index }, className = "trend-item" >
                (_jsx("span", { className: "trend-name", children: trend.trend })
                    ,
                        _jsxs("span", { className: "relevance", children: [trend.relevance.toFixed(0), "% relevant"] })
                            ,
                                _jsxs("span", { className: "adoption", children: [trend.adoptionRate.toFixed(0), "% adoption"] })))] });
div >
;
div >
;
...
const OptimizationEngineLoadingState = () => ()
    < div, className = "optimization-engine-loading" >
    (_jsx("div", { className: "loading-spinner" })
        ,
            _jsx("p", { children: "Analyzing optimization opportunities..." }));
div >
;
;
const OptimizationEngineErrorState = ({ error, onRetry }) => ()
    < div, className = "optimization-engine-error" >
    (_jsxs("div", { className: "error-message", children: [_jsx("h3", { children: "Error Loading Optimization Engine" }), _jsx("p", { children: error })] })
        ,
...
riskAssessment: {
    ...
    return {
        ...
        resourceAllocation: {
            totalBudget: 100000,
            allocations: recommendations.map((rec, index) => ({}), recommendationId, rec.id, allocatedBudget, rec.effortScore * 500, allocatedTime, rec.timeToImplement, allocatedResources, rec.implementation.resources.map(res => ({}), type, res.type, amount, res.hours, duration, rec.timeToImplement, utilization, 0.8))
        },
...
capacity: {
    ...
    0.8,
        bottlenecks;
    ['Development capacity', 'Design resources'],
    ;
}
optimization: {
            ...
            competitiveAnalysis: {
                    ...
                    positioning;
                {
                    currentPosition: 'Strong challenger with growth potential',
                        targetPosition;
                    'Market leader in conversion optimization',
                        differentiators;
                    ['Advanced analytics', 'AI-powered recommendations'],
                        weaknesses;
                    ['Template browsing UX', 'Mobile experience'],
                        opportunities;
                    ['Template preview innovation', 'Personalization'],
                        strategies;
