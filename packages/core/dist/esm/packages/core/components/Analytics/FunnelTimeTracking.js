...
onAnomalyDetected ?  : (anomaly) => void ;
onExport ?  : (data) => void ;
;
factors: string;
...
const ConversionRateTimeline = ({ data, granularity }) => {
    ...
    return;
    _jsxs("div", { className: "conversion-rate-timeline", children: [_jsx("h4", { children: "Conversion Rate Over Time" }), _jsxs("svg", { width: chartWidth, height: chartHeight, className: "timeline-chart", children: [_jsx("g", { transform: "translate(60, 20)", children: data.map((point, index) => {
                            const x = (index / (data.length - 1)) * (chartWidth - 120);
                            const y = ((maxRate - point.overallMetrics.conversionRate) / maxRate) * (chartHeight - 40);
                            return;
                            _jsxs("g", { children: [_jsx("circle", { cx: x, cy: y, r: "4", fill: "#3b82f6", className: "data-point" }), index < data.length - 1 && ()
                                        < line, "x1=", x, "y1=", y, "x2=", (index + 1) / (data.length - 1) * (chartWidth - 120), "y2=", ((maxRate - data[index + 1].overallMetrics.conversionRate) / maxRate) * (chartHeight - 40), "stroke=\"#3b82f6\" strokeWidth=\"2\" /> )}", index % Math.ceil(data.length / 6) === 0 && ()
                                        < text, "x=", x, "y=", chartHeight - 10, "textAnchor=\"middle\" fontSize=\"12\" fill=\"#666\" >", new Date(point.timestamp).toLocaleDateString()] }, point.timestamp);
                        }) }), "); })}", [0, 25, 50, 75, 100].map(tick => { }), "const y = ((100 - tick) / 100) * (chartHeight - 40); return;", _jsxs("g", { children: [_jsxs("text", { x: "-10", y: y + 4, textAnchor: "end", fontSize: "12", fill: "#666", children: [tick, "%"] }), _jsx("line", { x1: "0", y1: y, x2: chartWidth - 120, y2: y, stroke: "#e5e7eb", strokeWidth: "1" })] }, tick), "); })}"] })] });
};
div >
;
;
...
const VelocityTimeline = ({ data, granularity }) => {
    return;
    _jsxs("div", { className: "velocity-timeline", children: [_jsx("h4", { children: "Conversion Velocity Trends" }), _jsx("div", { className: "velocity-metrics", children: data.slice(-5).map((point, index) => ()
                    < div, key = { point, : .timestamp }, className = "velocity-point" >
                    (_jsx("div", { className: "timestamp", children: point.period })
                        ,
                            _jsxs("div", { className: "velocity", children: [point.conversionVelocity.toFixed(1), "/hr"] })
                                ,
                                    _jsxs("div", { className: `trend ${point.velocityTrend}`, children: ["}", point.velocityTrend === 'accelerating' ? '↗' :
                                                point.velocityTrend === 'decelerating' ? '↘' : '→'] }))) }), "))}"] });
    {
        data.length > 0 && data[data.length - 1].bottleneckAnalysis.length > 0 && ()
            < div;
        className = "current-bottlenecks" >
            _jsx("h5", { children: "Current Bottlenecks" });
        {
            data[data.length - 1].bottleneckAnalysis.slice(0, 3).map(bottleneck => ()
                < div, key = { bottleneck, : .stepId }, className = "bottleneck-item" >
                (_jsx("span", { className: "step-name", children: bottleneck.stepName })
                    ,
                        _jsxs("span", { className: `severity ${bottleneck.severity > 70 ? 'high' : bottleneck.severity > 40 ? 'medium' : 'low'}`, children: ["}", bottleneck.severity.toFixed(0), "% severity"] })
                            ,
                                _jsxs("span", { className: "impact", children: [bottleneck.impact, " users/hr affected"] })));
        }
    }
};
div >
...
const TrendsView = ({ trendAnalysis, comparativePeriods }) => {
    ...
    return;
    _jsxs("div", { className: "trends-view", children: [_jsxs("div", { className: "trends-overview", children: [_jsx("h4", { children: "Significant Trends" }), _jsxs("div", { className: "trends-grid", children: [significantTrends.map((trend, index) => ()
                                < TrendCard, key = { index }, trend = { trend } /  >
                            ), ")}"] })] }), comparativePeriods.length > 0 && ()
                < div, " className=\"comparative-analysis\">", _jsx("h4", { children: "Period Comparison" }), comparativePeriods.map((comparison, index) => ()
...
const SeasonalityView = ({ seasonalPatterns }) => {
    return;
    _jsxs("div", { className: "seasonality-view", children: [_jsx("h4", { children: "Seasonal Patterns" }), _jsxs("div", { className: "patterns-grid", children: [seasonalPatterns.map((pattern, index) => ()
...
const SeasonalPatternCard = ({ pattern }) => {
    return;
    _jsxs("div", { className: "seasonal-pattern-card", children: [_jsxs("h5", { children: [pattern.pattern.toUpperCase(), " Pattern"] }), _jsx("p", { children: pattern.description }), _jsxs("div", { className: "pattern-metrics", children: [_jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Strength" }), _jsxs("span", { className: "value", children: [(pattern.strength * 100).toFixed(0), "%"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Reliability" }), _jsxs("span", { className: "value", children: [(pattern.reliability * 100).toFixed(0), "%"] })] }), _jsxs("div", { className: "metric", children: [_jsx("span", { className: "label", children: "Business Impact" }), _jsxs("span", { className: "value", children: [pattern.businessImpact.toFixed(1), "%"] })] })] }), pattern.peaks.length > 0 && ()
                < div, " className=\"peaks-troughs\">", _jsx("h6", { children: "Peak Periods" }), pattern.peaks.slice(0, 3).map((peak, index) => ()
                < div, key = { index }, className = "peak-item" >
                (_jsx("span", { className: "period", children: peak.period })
                    ,
                        _jsxs("span", { className: "value", children: [peak.value.toFixed(1), "%"] })))] });
};
div >
;
{
...
const AnomaliesView = ({ anomalies, onAnomalyInvestigate }) => {
    ...
    return;
    _jsxs("div", { className: "anomalies-view", children: [_jsxs("div", { className: "anomalies-summary", children: [_jsx("h4", { children: "Performance Anomalies" }), _jsxs("div", { className: "summary-stats", children: [_jsxs("div", { className: "stat", children: [_jsx("span", { className: "label", children: "Active" }), _jsx("span", { className: "value", children: activeAnomalies.length })] }), _jsxs("div", { className: "stat critical", children: [_jsx("span", { className: "label", children: "Critical" }), _jsx("span", { className: "value", children: criticalAnomalies.length })] })] })] }), _jsxs("div", { className: "anomalies-list", children: [activeAnomalies
                        .sort((a, b) => {
                        const severityOrder = { critical: 4, high: 3, medium: 2, low: 1 };
                        return severityOrder[b.severity] - severityOrder[a.severity];
                    })
                        .map(anomaly => ()
                        < AnomalyCard, key = { anomaly, : .id }, anomaly = { anomaly }, onInvestigate = {}()), " => onAnomalyInvestigate(anomaly)} /> ))}"] })] });
};
;
;
const AnomalyCard = ({ anomaly, onInvestigate }) => {
    return;
    _jsxs("div", { className: `anomaly-card ${anomaly.severity}`, children: ["}", _jsxs("div", { className: "anomaly-header", children: [_jsxs("h5", { children: [anomaly.stepName || 'Overall Funnel', " - ", anomaly.metric] }), _jsxs("span", { className: `severity-badge ${anomaly.severity}`, children: ["}", anomaly.severity.toUpperCase()] }), _jsxs("span", { className: `status-badge ${anomaly.investigationStatus}`, children: ["}", anomaly.investigationStatus.replace('_', ' ').toUpperCase()] })] }), _jsxs("div", { className: "anomaly-details", children: [_jsxs("div", { className: "values", children: [_jsxs("span", { className: "expected", children: ["Expected: ", anomaly.expectedValue.toFixed(2)] }), _jsxs("span", { className: "actual", children: ["Actual: ", anomaly.actualValue.toFixed(2)] }), _jsxs("span", { className: "deviation", children: [anomaly.deviation.toFixed(1), "\u03C3 deviation"] })] }), _jsxs("div", { className: "anomaly-info", children: [_jsx("span", { className: "type", children: anomaly.anomalyType.replace('_', ' ') }), _jsxs("span", { className: "confidence", children: [(anomaly.confidence * 100).toFixed(0), "% confidence"] }), _jsxs("span", { className: "impact", children: ["$", anomaly.businessImpact.toLocaleString(), " impact"] }), "}"] })] }), anomaly.possibleCauses.length > 0 && ()
                < div, " className=\"possible-causes\">", _jsx("h6", { children: "Possible Causes" }), anomaly.possibleCauses
                .sort((a, b) => b.likelihood - a.likelihood)
                .slice(0, 2)
                .map((cause, index) => ()
                < div, key = { index }, className = "cause-item" >
                (_jsx("span", { className: "category", children: cause.category })
                    ,
                        _jsx("span", { className: "description", children: cause.description })
                            ,
                                _jsxs("span", { className: "likelihood", children: [(cause.likelihood * 100).toFixed(0), "% likely"] })))] });
};
div >
;
...
;
;
const TimeTrackingErrorState = ({ error, onRetry }) => ()
