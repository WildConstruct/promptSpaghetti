...
try { }
    ...
    setState(prev => ({}), ...prev, loading, false, error, error instanceof Error ? error.message : 'Failed to load usage data');
}
;
...
const formatPatternData = useCallback((patternData) => {
    if (!patternData || !patternData.data)
        return [];
    return patternData.data.map((item) => ({}), period, new Date(item.period).toLocaleDateString(), value, item.value, timestamp, new Date(item.period).getTime());
});
[];
;
/**
...
const generateHourlyDistribution = useCallback(() => {
    ...
    return hourlyData.map((value, hour) => ({}), hour, `${hour.toString().padStart(2, '0')}:00`);
});
value,
    percentage;
(value / Math.max(...hourlyData)) * 100;
;
[state.hourlyPattern];
;
/**
...
if (state.loading) {
    return;
    _jsx("div", { className: "usage-patterns", children: _jsxs("div", { className: "loading-container", children: [_jsx("div", { className: "loading-spinner" }), _jsx("p", { children: "Loading usage patterns..." })] }) });
    ;
    if (state.error) {
        return;
        _jsx("div", { className: "usage-patterns", children: _jsx(Alert, { variant: "destructive", children: _jsxs(AlertDescription, { children: [state.error, _jsx(Button, { variant: "outline", size: "sm", onClick: loadUsageData, className: "ml-2", children: "Retry" })] }) }) });
        ;
