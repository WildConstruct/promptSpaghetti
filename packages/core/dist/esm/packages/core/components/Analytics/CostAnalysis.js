    ...
    if (budgetsResponse.success && budgetsResponse.data) {
        for (const budget of budgetsResponse.data) {
            try {
                ...
                setState(prev => ({}), ...prev, loading, false, costSummary, costResponse.data, forecast, forecastResponse.success ? forecastResponse.data : null, budgets, budgetsResponse.success ? budgetsResponse.data : [], budgetUsage, budgetUsageMap, recommendations, recommendationsResponse.success ? recommendationsResponse.data : []);
            }
            finally { }
            ;
        }
        try { }
            ...
            setState(prev => ({}), ...prev, loading, false, error, error instanceof Error ? error.message : 'Failed to load cost data');
        }
        ;
    }
    ...
    if (state.loading) {
        ...
        TabsContent >
            (_jsx(TabsContent, { value: "budgets", className: "space-y-6", children: _jsxs("div", { className: "flex justify-between items-center", children: [_jsx("h3", { className: "text-lg font-semibold", children: "Budget Management" }), _jsx(Button, { onClick: () => {
                            ...
                            } }), ": parseFloat(amount), period, userId, organizationId }); }}> Create Budget"] }) })
                ,
                    _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4", children: [state.budgets.map((budget) => ()
                                < BudgetCard, key = { budget, : .id }, budget = { budget }, usage = { state, : .budgetUsage.get(budget.id) }, onUpdate = { handleUpdateBudget }
                                /  >
                            ), ")}"] }));
        {
        ...
        TabsContent >
            (_jsx(TabsContent, { value: "forecast", className: "space-y-6", children: _jsx(CostForecastChart, { forecastData: state.forecast, loading: state.loading }) })
                ,
                    _jsx(TabsContent, { value: "recommendations", className: "space-y-6", children: _jsx("div", { className: "space-y-4", children: state.recommendations.map((rec, index) => ()
                                < Card, key = { index } >
                                (_jsx(CardHeader, { children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsx(CardTitle, { className: "text-lg", children: rec.title }), _jsx(Badge, { variant: rec.priority === 'high' ? 'destructive' : rec.priority === 'medium' ? 'warning' : 'secondary', children: rec.priority })] }) })
                                    ,
                                        _jsx(CardContent, { children: _jsxs("div", { className: "space-y-3", children: [_jsx("p", { className: "text-gray-600", children: rec.description }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(DollarSign, { className: "w-4 h-4 text-green-600" }), _jsxs("span", { className: "text-sm font-medium", children: ["Potential savings: $", rec.estimatedSavings?.toFixed(2) || '0.00'] })] }), _jsxs("div", { className: "space-y-2", children: [_jsx("div", { className: "text-sm font-medium", children: "Action Items:" }), _jsx("ul", { className: "text-sm text-gray-600 space-y-1", children: rec.actionItems?.map((item, i) => ()
                                                                    ...
                                                                    _jsx("span", { className: "text-blue-600", children: "\u2022" }), { item }) }), "))}"] })] }) }))) }) }));
    }
    div >
