...
const InsightCard = ({ insight, isSelected, onClick }) => {
    return;
    _jsxs("div", { className: `insight-card ${insight.severity} ${isSelected ? 'selected' : ''}`, onClick: onClick, children: [_jsxs("div", { className: "insight-header", children: [_jsx("div", { className: "insight-type", children: insight.type.replace('_', ' ') }), _jsx("div", { className: "insight-severity", children: insight.severity })] }), _jsx("h4", { className: "insight-title", children: insight.title }), _jsx("p", { className: "insight-description", children: insight.description }), insight.evidence.statisticalTest && ()
                < div, " className=\"statistical-indicator\">", _jsxs("span", { className: "p-value", children: ["p = ", insight.evidence.statisticalTest.pValue.toFixed(4)] }), _jsxs("span", { className: `significance ${insight.evidence.statisticalTest.isSignificant ? 'significant' : 'not-significant'}`, children: ["}", insight.evidence.statisticalTest.isSignificant ? 'Significant' : 'Not Significant'] })] });
};
{
...
const SimpleFunnelChart = ({ steps }) => {
    ...
    return;
    _jsx("div", { className: "simple-funnel-chart", children: steps.map((step, index) => {
            const width = (step.entries / maxEntries) * 100;
            return;
            _jsxs("div", { className: "funnel-step", children: [_jsx("div", { className: "step-bar", style: { width: `${width}%` }, children: _jsxs("div", { className: "step-info", children: [_jsx("span", { className: "step-name", children: step.stepName }), _jsxs("span", { className: "step-rate", children: [step.conversionRate.toFixed(1), "%"] })] }) }), index < steps.length - 1 && ()
                        < div, " className=\"step-arrow\">\u2193"] }, step.stepId);
        }) });
};
;
div >
...
;
;
const ComparisonErrorState = ({ error, onRetry }) => ()
