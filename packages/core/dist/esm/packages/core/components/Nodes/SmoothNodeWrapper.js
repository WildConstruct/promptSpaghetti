...
_jsxs("div", { ref: nodeRef, style: getNodeStyle(), className: `smooth-node ${isAnimating ? 'animate-node-create' : ''}`, onClick: handleClick, onDoubleClick: handleDoubleClick, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, ...hoverProps, children: [_jsx(Handle, { type: "target", position: Position.Left, style: getHandleStyle('target'), onMouseEnter: (e) => {
            ...
            }, isConnectable: isConnectable }), _jsxs("div", { style: {
                padding: '16px 20px',
                position: 'relative',
                zIndex: 1,
                    ...
                    } }), "...createSmoothTransition(['background', 'box-shadow']) }} />", children, (selected || isSelected) && ()
                    < div, "style=", {
                    position: 'absolute',
                    inset: -2,
                    borderRadius: 14,
                    background: `linear-gradient(45deg, ${colors.primary}20, transparent, ${colors.primary}20)`
                }, ", animation: 'glowPulse 2s ease-in-out infinite', pointerEvents: 'none', zIndex: -1; }} /> )}", isHovered && ()
                    < div, "style=", {
                    position: 'absolute',
                    inset: -4,
                    borderRadius: 16,
                    background: `radial-gradient(circle at center, ${colors.primary}15, transparent)`
                }, ", pointerEvents: 'none', zIndex: -1, animation: 'fadeIn 0.3s ease-out'; }} /> )}"] }), _jsx(Handle, { type: "source", position: Position.Right, style: getHandleStyle('source'), onMouseEnter: (e) => {
                Object.assign(e.currentTarget.style, handleHoverStyle);
            }, onMouseLeave: (e) => {
