...
import { createSmoothTransition, animationDurations, easingFunctions, useSmoothHover } from globalAnimationManager;
from;
...
import '../../styles/smoothAnimations.css';
{
    ...
    const getNodeColors = () => {
        const colors = {
            ...
            'Concat': { primary: '#10b981',
                secondary: '#047857',
                accent: '#34d399',
                background: 'rgba(16, 185, 129, 0.1)' },
            'Output': { primary: '#f59e0b',
                secondary: '#d97706',
                accent: '#fbbf24',
                background: 'rgba(245, 158, 11, 0.1)' },
            'SetVariable': { primary: '#8b5cf6',
                secondary: '#7c3aed',
                accent: '#a78bfa',
                background: 'rgba(139, 92, 246, 0.1)' },
            'GetVariable': { primary: '#06b6d4',
                secondary: '#0891b2',
                accent: '#22d3ee',
                background: 'rgba(6, 182, 212, 0.1)' },
            'Conditional': { primary: '#ef4444',
                secondary: '#dc2626',
                accent: '#f87171',
...
minWidth: 160;
minHeight: 80;
createSmoothTransition()['transform', 'box-shadow', 'border-color', 'background'];
...
createSmoothTransition()['background', 'border-color', 'box-shadow', 'transform'];
...
;
return;
_jsxs("div", { ref: nodeRef, style: getNodeStyle(), className: `smooth-node ${isAnimating ? 'animate-node-create' : ''}`, onClick: handleClick, onDoubleClick: handleDoubleClick, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, ...hoverProps, children: [_jsx(Handle, { type: "target", position: Position.Left, style: getHandleStyle('target'), onMouseEnter: (e) => {
                Object.assign(e.currentTarget.style, handleHoverStyle);
            }, onMouseLeave: (e) => {
                Object.assign(e.currentTarget.style, getHandleStyle('target'));
            }, isConnectable: isConnectable }), _jsx("div", { style: {
                padding: '16px 20px',
                position: 'relative',
                zIndex: 1
            }
            ...
            }, "boxShadow:": true }), " `0 0 8px $", colors.primary, "60`} ...createSmoothTransition(['background', 'box-shadow']) />", children, (selected || isSelected) && ()
            < div, "style=", {
            position: 'absolute',
            inset: -2,
            borderRadius: 14
        }, "background: `linear-gradient(45deg, $", colors.primary, "20, transparent, $", colors.primary, "20)`} animation: 'glowPulse 2s ease-in-out infinite' pointerEvents: 'none' zIndex: -1; /> )}", isHovered && ()
            < div, "style=", {
            position: 'absolute',
            inset: -4,
            borderRadius: 16
        }, "background: `radial-gradient(circle at center, $", colors.primary, "15, transparent)`} pointerEvents: 'none' zIndex: -1 animation: 'fadeIn 0.3s ease-out'; /> )}"] });
{ /* Output handle */ }
_jsx(Handle, { type: "source", position: Position.Right, style: getHandleStyle('source'), onMouseEnter: (e) => {
        Object.assign(e.currentTarget.style, handleHoverStyle);
    }, onMouseLeave: (e) => {
...
export const NodeCreationAnimator = ({
    children,
    isCreating });
onAnimationComplete;
{
    ...
    return;
    _jsx("div", { className: isCreating ? 'animate-node-create' : '', style: {
            ...createSmoothTransition(['transform', 'opacity'])
        }
            >
                { children }, div: true, children: "); }; export default SmoothNodeWrapper;" });
}
