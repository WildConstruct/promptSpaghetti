...
_jsxs("div", { style: {
        ...
        fontSize: currentSize.fontSize
                ...
                fontSize: currentSize.emojiSize
                    ...
                    }, "color:": true }), " '#6b7280'; }>", reaction.count] }, reaction.type)] });
;
{
...
_jsxs("div", { style: {
        ...
        border: '1px solid #e5e7eb',
        borderRadius: '8px'
    }, children: [DEFAULT_REACTIONS.map(reaction => { }), "const count = summary.reactionCounts[reaction.id] || 0; const isUserReaction = summary.userReaction === reaction.id; return;", _jsx("button", { onClick: () => handleReactionClick(reaction.id), disabled: disabled || !userId || loading, title: reaction.description, style: {
                display: 'flex',
                alignItems: 'center',
                gap: '4px',
                padding: '6px 8px',
                ...
                >
                    _jsx("span", { style: { fontSize: currentSize.emojiSize }, children: reaction.emoji }), ...showLabels && ()
                < span, style: {
                fontSize: `calc(${currentSize.fontSize} * 0.9)`
            }, "color:isUserReaction": true }, reaction.id), " ? '#3b82f6' : '#374151'; }>", reaction.label] });
{
    showCounts && count > 0 && ()
        < span;
    style = {};
...
ReactionSummary;
{
    ...
    if (isRemoving) { // Remove reaction
        if (newCounts[reactionType] > 0) {
            ...
            else { // Add reaction (remove previous if exists)
