...
import { HierarchyColors, TypographyScale, SpacingScale, ComponentSizes, classifyFieldPriority, AccessibilityUtils } from type;
FieldPriority;
from;
'../VisualHierarchy/HierarchyDesignSystem';
{
    const { complexityLevel, shouldShowAdvancedFeatures };
...
const headerStyles = React.useMemo(() => ({}), ...ComponentSizes.header, cursor, shouldAutoExpand ? 'default' : 'pointer', display, 'flex', alignItems, 'center', justifyContent, 'space-between', userSelect, 'none', backgroundColor, colors.background);
...
outlineOffset: 2;
':hover';
shouldAutoExpand ? {} : { backgroundColor: colors.accent,
