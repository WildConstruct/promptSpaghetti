...
emptyMessage = 'No items defined.';
{
    ...
    const handleAdd = () => {
        ...
        ;
        const handleRemove = (index) => { onChange(variations.filter((_, i) => i !== index)); };
    ...
    >
        _jsx("input", { type: "text", value: variation, onChange: (e) => handleUpdate(index, e.target.value), style: {
                ...
                _jsx("button", { onClick: () => handleRemove(index), style: {
                        ...
                        cursor: 'pointer',
                        fontSize: 10
                    }, children: "\u2715" }), div: true, children: "))}" });
_jsx("div", { style: { display: 'flex', gap: 8 }, children: _jsx("input", { type: "text", value: newItem, onChange: (e) => setNewItem(e.target.value), onKeyPress: handleKeyPress, placeholder: placeholder, style: {
            flex: 1,
            padding: 6,
            border: '1px solid #4a5568',
            borderRadius: 4,
