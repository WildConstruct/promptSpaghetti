...
const handleOutputFormatChange = useCallback((outputFormat) => { onChange({}); }, ...settings);
outputFormat;
;
[settings, onChange];
;
// Handle naming pattern change
const handleNamingPatternChange = useCallback((namingPattern) => { onChange({}); }, ...settings);
namingPattern;
;
[settings, onChange];
;
// Handle metadata toggle
const handleIncludeMetadataChange = useCallback((includeMetadata) => { onChange({}); }, ...settings);
includeMetadata;
;
[settings, onChange];
;
// Handle auto-download toggle
const handleAutoDownloadChange = useCallback((autoDownload) => { onChange({}); }, ...settings);
autoDownload;
;
[settings, onChange];
;
...
_jsxs("div", { style: { marginBottom: '24px' }, children: [_jsxs("div", { style: {
                ...
                marginBottom: '16px'
                        ...
                        marginBottom: '6px'
                                ...
                                width: '100px',
                                padding: '8px 12px'
                            }, "border:": true }), " `1px solid $", uiColors.ui.border, "`} borderRadius: '6px' backgroundColor: uiColors.background.primary color: uiColors.text.primary fontSize: '14px' outline: 'none' textAlign: 'center'; onFocus=", (e) => {
                        ...
                        gap: '8px'
                            ...
                            _jsx("button", { onClick: () => handleOutputFormatChange(format.value), style: {
                                    ...
                                    ? `1px solid ${uiColors.accent.primary}` :  }, format.value);
                        }), ": `1px solid $", uiColors.ui.border, "`} borderRadius: '6px' cursor: 'pointer' transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' textAlign: 'left'; onMouseEnter=", (e) => {
                            if (!isSelected) {
...
_jsxs("div", { style: { marginBottom: '20px' }, children: [_jsx("label", { style: {
                ...
                width: '100%',
                padding: '8px 12px'
            }, "border:": true }), " `1px solid $", uiColors.ui.border, "`} borderRadius: '6px' backgroundColor: uiColors.background.primary color: uiColors.text.primary fontSize: '14px' outline: 'none'; onFocus=", (e) => {
            e.target.style.borderColor = uiColors.accent.primary;
        }, "onBlur=", (e) => {
            e.target.style.borderColor = uiColors.ui.border;
        }, "/>", _jsxs("div", { style: {
