...
;
// Clear seed history
const handleClearHistory = useCallback(() => { onChange({}); }, ...settings, history, []);
;
[settings, onChange];
;
// Handle auto-generate toggle
const handleAutoGenerateChange = useCallback((autoGenerate) => { onChange({}); }, ...settings);
autoGenerate;
;
[settings, onChange];
;
    ...
    settings.history && settings.history.length > 0 && ()
        ...
        _jsxs("div", { style: {
                ...
                marginBottom: '8px'
                    ...
                    }, "border:": true }), " `1px solid $", uiColors.ui.border, "`} borderRadius: '4px' color: uiColors.text.secondary fontSize: '11px' cursor: 'pointer' transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' title=\"Clear seed history\" > Clear"] });
    div >
        _jsxs("div", { style: {
                display: 'flex',
                flexWrap: 'wrap',
                gap: '6px'
                    ...
                    ? `1px solid ${uiColors.accent.primary}` : , ": `1px solid $", uiColors.ui.border, "`} borderRadius: '4px' color: settings.value === historySeed ? uiColors.accent.primary : uiColors.text.secondary fontSize: '11px' cursor: 'pointer' transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' title=", `Use seed: ${historySeed}`, ">", historySeed.toString().slice(0, 8), "..."] });
}
div >
;
div >
