...
const handleEnableCachingChange = useCallback((enableCaching) => { onChange({}); }, ...settings);
enableCaching;
;
[settings, onChange];
;
// Handle memory usage toggle
const handleShowMemoryUsageChange = useCallback((showMemoryUsage) => { onChange({}); }, ...settings);
showMemoryUsage;
;
[settings, onChange];
;
// Handle execution logging toggle
const handleLogExecutionStepsChange = useCallback((logExecutionSteps) => { onChange({}); }, ...settings);
logExecutionSteps;
;
[settings, onChange];
;
...
_jsxs("div", { style: { marginBottom: '24px' }, children: [_jsxs("div", { style: {
                ...
                borderRadius: '8px'
            }, "border:": true }), " `1px solid $", uiColors.ui.border, "`} }>", _jsxs("div", { style: {
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                fontSize: '14px',
                ...
                marginLeft: 'auto'
            }, children: [enabledCount, "/", totalCount, " settings enabled"] }), _jsxs("div", { style: {
                display: 'flex',
                alignItems: 'center',
                gap: '4px',
                fontSize: '12px',
                ...
                color: enabledCount <= 2
                    ...
                    : enabledCount <= 3
                        ? '#f59e0b'
                        : '#ef4444'
            }, children: [enabledCount <= 2 ? ()
                    <  >
                    _jsx(FiEye, { size: 12 })
                    :
                , "Optimized"] }), ") : enabledCount ", _jsx(, {}), "= 3 ? ()", _jsxs(_Fragment, { children: [_jsx(FiActivity, { size: 12 }), "Balanced"] }), ") : ()", _jsxs(_Fragment, { children: [_jsx(FiEyeOff, { size: 12 }), "Debug Mode"] }), ")}"] });
div >
    { /* Performance Settings Sections */};
{
    performanceSections.map((section) => {
        ...
        _jsxs("div", { style: { marginBottom: '24px' }, children: [_jsxs("div", { style: {
                        ...
                        }), ": `1px solid $", uiColors.ui.border, "`} borderRadius: '6px' transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)' }>", _jsx(SettingIcon, { size: 16, color: setting.enabled
