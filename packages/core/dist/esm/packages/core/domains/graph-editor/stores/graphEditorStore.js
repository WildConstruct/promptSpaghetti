...
clearSelection: () => void ;
toggleNodeSelection: (nodeId) => void ;
// Execution
setExecuting: (isExecuting) => void ;
setExecutionResults: (results) => void ;
clearExecutionResults: () => void ;
// Validation
setValidationErrors: (errors) => void ;
clearValidationErrors: () => void ;
// Preview seeds
setPreviewSeeds: (seeds) => void ;
addPreviewSeed: () => void ;
removePreviewSeed: (index) => void ;
// State management
...
export const useGraphEditorStore = create()();
devtools();
...
, 
// Initial state
graph, createEmptyGraph(), selectedNodeIds, [], draggedNodeId, null, isExecuting, false, executionResults, {}, validationErrors, [], previewSeeds, [1, 42, 100], autosaveEnabled, true, isDirty, false, 
// Configuration
config, DEFAULT_CONFIG, 
// History
history, [], historyIndex, -1, maxHistorySize, 50, 
// Graph operations
...
// Selection management
selectNodes: (nodeIds, isMultiSelect = false) => set((state) => {
...
};
{
    ...
    export const useGraphEditorState = () => useGraphEditorStore((state) => ({}), graph, state.graph, selectedNodeIds, state.selectedNodeIds, isExecuting, state.isExecuting, validationErrors, state.validationErrors, isDirty, state.isDirty);
}
;
export const useGraphEditorConfig = () => useGraphEditorStore((state) => state.config);
export const useGraphEditorHistory = () => useGraphEditorStore((state) => ({}), canUndo, state.canUndo(), canRedo, state.canRedo(), undo, state.undo, redo, state.redo);
;
