...
setConnectionAnnotationPreferences: (preferences) => void ;
...
newProject: () => void setCurrentProject;
(metadata) => void updateProjectSettings;
(settings) => void markProjectSaved;
() => void ;
...
addEdge: (edge) => set(state => ({ edges: [...state.edges, edge],
    hasUnsavedChanges: true }));
updateNode: (nodeId, partial) => set(state => ({ nodes: state.nodes.map(node => (node.id === nodeId ? mergeNodeData(node, partial) : node)),
    hasUnsavedChanges: true }));
addVariation: (nodeId, variation) => set(state => ({ nodes: addVariationToNode(state.nodes, nodeId, variation),
    hasUnsavedChanges: true }));
removeVariation: (nodeId, variationIndex) => set(state => ({ nodes: removeVariationFromNode(state.nodes, nodeId, variationIndex),
    hasUnsavedChanges: true }));
updateVariation: (nodeId, variationIndex, newValue) => set(state => ({ nodes: updateVariationInNode(state.nodes, nodeId, variationIndex, newValue),
    hasUnsavedChanges: true }));
reorderVariations: (nodeId, fromIndex, toIndex) => set(state => ({ nodes: reorderVariationsInNode(state.nodes, nodeId, fromIndex, toIndex),
    hasUnsavedChanges: true }));
duplicateNode: (nodeId) => set(state => {
    ...
    data: { }
}, ...nodeToClone.data, label, `${nodeToClone.data.label} (Copy)`);
return { nodes: [...state.nodes, newNode], hasUnsavedChanges: true };
deleteNode: (nodeId) => set(state => ({ nodes: state.nodes.filter(n => n.id !== nodeId),
    edges: state.edges.filter(e => e.source !== nodeId && e.target !== nodeId),
    hasUnsavedChanges: true }));
// Sticky notes operations (Epic 8.7)
setStickyNotes: (notes) => set(state => ({ stickyNotes: notes,
    annotations: {
        ...state.annotations,
        stickyNotes: notes,
        metadata: {
...
setNodeLabelConfigs: (configs) => set(state => ({ annotations: {
        ...state.annotations,
...
addNodeLabelConfig: (config) => set(state => ({ annotations: {
        ...state.annotations,
        nodeLabelConfigs: {
            ...state.annotations.nodeLabelConfigs[config.id], config
        },
...
updateNodeLabelConfig: (labelId, updates) => set(state => {
    ...
    return {
        annotations: {
            ...state.annotations,
            nodeLabelConfigs: {
                ...state.annotations.nodeLabelConfigs[labelId]
            }
        }
    };
    {
    }
}, ...existingConfig, ...updates, timestamp, new Date().toISOString(), metadata, { ...state.annotations.metadata,
    modified: new Date().toISOString() }, hasUnsavedChanges, true);
...
setLabelPreferences: preferences => set(state => ({ annotations: {
        ...state.annotations,
        labelPreferences: {
