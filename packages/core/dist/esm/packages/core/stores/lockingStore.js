...
export const useLockingStore = create()();
devtools();
(set, get) => ({
    ...initialState,
    // Utility actions
...
if (!response.ok) {
    ...
    set(state => ({}), locks, state.locks.filter(lock => lock.id !== lockId), isLoading, false);
}
;
return { success: true };
try { }
...
breakLock: async (lockId, userId, justification) => {
    set({ isLoading: true, error: null });
    try {
...
;
resolveLockConflict: async (conflictId, resolution, userId) => {
    set({ isLoading: true, error: null });
    try {
...
if (!response.ok) {
    ...
    set(state => ({}), conflicts, state.conflicts.map(conflict => ), conflict.id === conflictId
        ? { ...conflict, status: 'resolved', resolved_at: new Date().toISOString() }
        : conflict),
        isLoading;
    false;
}
...
;
removeFromQueue: async (queueId, userId) => {
    set({ isLoading: true, error: null });
    try {
...
if (!response.ok) {
    ...
    set(state => ({}), queue, state.queue.filter(item => item.id !== queueId), isLoading, false);
}
;
return { success: true };
try { }
...
// Notifications
fetchNotifications: async (userId, unreadOnly = false) => {
    set({ isLoading: true, error: null });
    try {
...
if (!response.ok) {
    ...
    set(state => ({}), notifications, state.notifications.map(notification => ), notification.id === notificationId
...
;
updatePolicy: async (workspaceId, policyData) => {
    set({ isLoading: true, error: null });
    try {
        const response = await fetch(`${API_BASE}/api/locking/workspaces/${workspaceId}/lock-policy`, {});
    }
