...
(set, get) => ({
    ...
    createState: async (data) => {
        ...
        ;
        set(state => ({}), states, [...state.states, newState].sort((a, b) => a.sort_order - b.sort_order), loading, false);
    },
    return: newState });
try { }
catch (error) {
...
createTransition: async (data) => {
    ...
    ;
    set(state => ({}), transitions, [...state.transitions, newTransition], loading, false);
};
;
...
createApproval: async (data) => {
    ...
    ;
    set(state => ({}), approvals, [...state.approvals, newApproval], loading, false);
};
;
return newApproval;
try { }
...
set(state => ({}), approvals, state.approvals.map(a => ), a.id === approvalId
...
;
set(state => ({}), locks, [...state.locks, newLock], loading, false);
;
return newLock;
try { }
catch (error) {
