...
import { Activity, ActivityQuery, ActivityMetrics } from ActivitySeverity;
from;
...
subscribeWithSelector((set, get) => ({}), ...initialState, 
// Data Actions
setActivities, (activities) => set({ activities }), addActivity, (activity) => set((state) => ({})));
...
addFilter: (key, value) => set((state) => ({}), activeFilters, { ...state.activeFilters, [key]: value });
    ...
    try {
        ...
        getSummaryStats: (state) => {
            ...
            return {
                ...
                byType: activities.reduce((acc, activity) => { }, acc[activity.type] = (acc[activity.type] || 0) + 1),
                return: acc
            }, {};
