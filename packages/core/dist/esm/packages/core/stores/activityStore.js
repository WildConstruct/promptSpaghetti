...
async;
getMetrics(query, ActivityQuery);
...
() => set({ currentQuery: initialState.currentQuery }),
    // Filter Actions
    setActiveFilters;
(filters) => set({ activeFilters: filters }),
    addFilter;
(key, value) => set((state) => ({}), activeFilters, { ...state.activeFilters, [key]: value });
...
setViewMode: (mode) => set({ viewMode: mode }),
    setSorting;
(sortBy, sortOrder) => set({ sortBy, sortOrder }),
    setGroupBy;
(groupBy) => set({ groupBy }),
    // Pagination Actions
    setPage;
...
async (query) => {
    ...
    try {
        ...
        false,
            hasNextPage;
        (queryToUse.offset || 0) + result.activities.length < result.totalCount,
            hasPreviousPage;
        (queryToUse.offset || 0) > 0,
        ;
    }
    finally { }
    ;
...
export const useActivityStream = () => {
    ...
    const startStream = React.useCallback(async (filters) => {
        ...
        const interval = setInterval(() => {
            const mockActivity = {
                id: `live_${Date.now()}` };
        }, timestamp, new Date().toISOString(), type, 'system_event', severity, 'info', status, 'completed', action, 'health_check', description, 'System health check completed', category, 'monitoring', source, 'health-service', environment, 'development', metadata, {}, tags, [], createdAt, new Date().toISOString(), version, 1);
    });
