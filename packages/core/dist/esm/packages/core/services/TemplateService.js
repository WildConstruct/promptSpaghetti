...
const uuidv4 = () => { return null; }, edges;
(edge => ({ ...edge }));
annotations;
;
...
Promise < Template | null > {
    ...
    async searchTemplates(filter = {}) {
         ...
         */
        async;
        getRecentTemplates(limit, number = 10);
        Promise < Template > { const: templates = await this.searchTemplates({}),
            sortBy: 'created',
            sortOrder: 'desc' };
    },
    return: templates.slice(0, limit),
...
}, ...template.metadata, usageCount, template.metadata.usageCount + 1, lastModified, new Date().toISOString());
// Clone template graph data
const graphData = JSON.parse(JSON.stringify(template.graph));
...
if (!options.preservePositions || options.offsetX || options.offsetY) {
    ...
    graphData.nodes = graphData.nodes.map(node => ({}), ...node, position, {
...
graphData.annotations.stickyNotes = graphData.annotations.stickyNotes.map(note => ({}), ...note, position, {
    x: note.position.x + offsetX,
    y: note.position.y + offsetY
});
;
// Update region group bounds
graphData.annotations.regionGroups = graphData.annotations.regionGroups.map(region => ({}), ...region, bounds, {
    ...region.bounds,
...
graphData.edges = graphData.edges.map(edge => ({}), ...edge, id, `${edge.id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, source, idMap.get(edge.source) || edge.source, target, idMap.get(edge.target) || edge.target);
;
...
;
graphData.annotations.regionGroups = graphData.annotations.regionGroups.map(region => ({}), ...region, nodeIds, region.nodeIds.map(nodeId => idMap.get(nodeId) || nodeId));
;
return graphData;
/**
 ...
 */
async;
addReview(templateId, string, review, (Omit));
Promise < Review > {
    const: template = await this.storage.load(templateId),
    if(, template) {
        throw new Error(`Template with id "${templateId}" not found`);
