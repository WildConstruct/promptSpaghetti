    ...
    for (const [key, dataPoints] of this.historicalData.entries()) {
        if (!framework || key.startsWith(framework)) {
            ...
            return {
                ...
                if(diffDays, , ) { }, return: `${Math.floor(diffDays / 7)} week${Math.floor(diffDays / 7) !== 1 ? 's' : ''}`
            };
            if (diffDays < 365)
                    ...
                    for (let i = 1; i < sortedPoints.length; i++) {
                        ...
                        if (change > 10) { // Significant change threshold
                            ...
                            date: current.timestamp,
                                event;
                            `Significant ${change > 0 ? 'improvement' : 'degradation'} in ${current.metric}`;
                        }
                    }
