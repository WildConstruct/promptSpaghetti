...
if (cached) {
    ...
    try {
        ...
        endDate = new Date(), includeRealTime = true, includeSentiment = true, includeTopics = true } = options;
        // Get core metrics
        ...
        const analytics = {
            ...
            trends: {
                ...
                topicEvolution: topicData.map(t => ({}), topic, t.topic, timeline, [{ date: endDate.toISOString().split('T')[0], count: t.mentionCount }])
            }
        }, breakdowns;
        this.getUserTypeBreakdown(resourceId, resourceType, startDate, endDate),
            byTimeOfDay;
        timeSeriesData.hourly,
            byDayOfWeek;
        timeSeriesData.weekly,
            byLanguage;
        await this.getLanguageDistribution(resourceId, resourceType, startDate, endDate),
            bySentiment;
        sentimentData ? {
            positive: sentimentData.positive,
            neutral: sentimentData.neutral,
        ...
        comparisons;
    {
        previousPeriod: await this.getPreviousPeriodComparison(resourceId, resourceType, startDate, endDate),
            benchmark;
        await this.getBenchmarkComparison(resourceType),
            similarResources;
        await this.getSimilarResourcesComparison(resourceId, resourceType),
        ;
    }
    ;
    // Cache the result
...
Promise < number > {
    ...
    return() { } }(currentPeriod.totalComments - previousPeriod.totalComments) / previousPeriod.totalComments;
 * 100;
async;
