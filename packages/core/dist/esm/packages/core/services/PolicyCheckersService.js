...
void {
    ...
    async executeCheck() { }
}((request, policyType) => { const results = await this.executeChecks({}); }, ...request, checksRequested, [policyType]);
return results[0];
...
results: results.map(r => ({}), type, r.policyType, status, r.status, score, r.score, violationCount, r.violations.length);
;
...
Promise < boolean > { return: rule.policyType === this.type,
...
Promise < boolean > { return: rule.policyType === this.type,
    ...
    async validateRule(rule) {
        ...
        };
        {
            ...
            version = '1.0.0';
            async;
            check(request, PolicyCheckRequest);
            ...
            'GDPR Compliance',
                description;
            'Data handling does not fully comply with GDPR requirements',
                severity;
            'medium';
        }
        ;
