                                ...
                                if (filter.status?.length) {
                                    ...
                                    WorkflowStats;
                                    {
                                        ...
                                        return {
                                            ...
                                            const: moderationWorkflowService = ModerationWorkflowService.getInstance(),
                                            // Convenience functions
                                            const: createWorkflow = (),
                                            workflowData: (Omit),
                                            createdBy: string, moderationWorkflowService, : .createWorkflow(workflowData, createdBy),
                                            const: executeWorkflow = (workflowId, itemId, triggeredBy) => moderationWorkflowService.executeWorkflow(workflowId, itemId, triggeredBy),
                                            const: getWorkflows = (filter) => moderationWorkflowService.getWorkflows(filter),
                                            const: getWorkflowStats = () => moderationWorkflowService.getWorkflowStats()
                                        };
                                    }
                                }
                            }
