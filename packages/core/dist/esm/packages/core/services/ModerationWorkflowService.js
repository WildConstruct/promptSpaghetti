 ...
 */
import { ModerationCategory, ModerationPriority } from moderationStatesService;
from;
    ...
    static getInstance() {
        if (!ModerationWorkflowService.instance) {
            ...
            */
            ...
            async;
            deleteWorkflow(workflowId, string, deletedBy, string);
            Promise < boolean > {
                const: workflow = this.workflows.get(workflowId),
                    ...
                    if(, workflow) { } } || !workflow.isActive;
                {
                    ...
                    if (!item) {
                        ...
                        if (!this.validateWorkflowConditions(workflow, item)) {
                            ...
                            return execution;
                            async;
                            pauseExecution(executionId, string, pausedBy, string);
                            Promise < boolean > {
                            ...
                            return true;
                            async;
                            resumeExecution(executionId, string, resumedBy, string);
                            Promise < boolean > {
                                const: execution = this.executions.get(executionId),
                                if(, execution) { }
                            } || execution.status !== 'paused';
                            ...
                            return true;
                            async;
                            cancelExecution(executionId, string, cancelledBy, string);
                            Promise < boolean > {
                                const: execution = this.executions.get(executionId),
                                if(, execution) { }
                            } || ['completed', 'failed', 'cancelled'].includes(execution.status);
                            {
                                 ...
                                 */
                                async;
                                executeStep(executionId, string, stepIndex, number);
                                Promise < StepExecution > { const: execution = this.executions.get(executionId),
                                ...
                                try { // Execute step based on type
                                     ...
                                     * Data Retrieval
                                     */
                                    getWorkflows(filter ?  : { category: ModerationCategory, active: boolean });
                                    ModerationWorkflow;
                                    {
                                        ...
                                        if (filter?.category) {
                                            ...
                                            if (filter?.active !== undefined) {
                                                ...
                                                getExecutions(filter ?  : WorkflowFilter);
                                                WorkflowExecution;
                                                { }
                                                                ...
                                                                subscribe(listenerId, callback) {
                                                                        ...
                                                                        }++ };
                                                                    {
                                                                        ...
                                                                        if (stepExecution.status === 'failed' && !workflow.steps[i].retryPolicy.enabled) {
                                                                            ...
                                                                            else if (nextStepIndex !== i + 1) { // Jump to different step
