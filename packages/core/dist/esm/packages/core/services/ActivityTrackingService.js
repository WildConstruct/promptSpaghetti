...
params.duration,
    severity,
    status;
params.statusCode < 400 ? 'completed' : 'failed',
    metadata;
{
                        ...
                        if (this.config.enableRealTime && this.config.streaming) {
                            ...
                            Promise < Activity | null > {
                                ...
                                async deleteActivity(id) {
                                    ...
                                    Promise < Activity > {
                                        const: fullActivities = activities.map(activity => ({}), id, this.generateActivityId(), timestamp, new Date().toISOString(), severity, 'info', status, 'completed', environment, this.getEnvironment(), metadata, {}, tags, [], createdAt, new Date().toISOString(), version, 1, ...activity)
                                    };
                                    as;
                                    Activity;
                                    ...
                                    if (this.config.enableRealTime && this.config.streaming) {
                                        for (const activity of storedActivities) {
                                            ...
                                            flush();
                                            Promise < void  > {
                                                : .pendingActivities.length === 0, return: ,
