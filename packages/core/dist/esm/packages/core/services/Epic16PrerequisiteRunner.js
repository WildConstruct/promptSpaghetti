...
async;
run();
Promise < PrerequisiteRunnerResult > { const: startTime = Date.now(),
    try: {
        : .options.configFile
    } };
{
    ...
    async;
    getQuickStatus();
    Promise < { status: 'healthy' | 'degraded' | 'critical' };
    message: string;
    details: any;
        > {
            try: {
...
Promise < void  > {
    ...
    async generateOutput(report, autoFixResults) {
    ...
    generateHTMLReport(report, autoFixResults) {
