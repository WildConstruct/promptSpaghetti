...
filterChecks();
void {
    : .options.onlyChecks && this.options.onlyChecks.length > 0
};
{
...
async;
generateOutput(report, PrerequisiteReport, autoFixResults ?  : Record);
Promise < string | undefined > {
    : .options.outputFile && this.options.format === 'console'
};
{
    return undefined;
    let output;
    ...
    switch (this.options.format) {
        ...
        default:
            ...
            return outputPath;
            generateHTMLReport(report, PrerequisiteReport, autoFixResults ?  : Record);
            string;
            {
...
<body>
    ...
    <div class="timestamp">
        Report ID: ${report.reportId} | Version: ${report.version}
            ...
            generateMarkdownReport(report, PrerequisiteReport, autoFixResults ?  : Record);
            string;
            {
...
if (report.recommendations.length > 0) {
    ...
    if (autoFixResults) {
        ...
        markdown += '\n';
        return markdown;
        displaySummary(result, PrerequisiteRunnerResult);
        void {
...
if (report.recommendations.length > 0) {
    ...
    if (!report.overall.passed) {
        ...
        log(message, string, level, 'info' | 'warn' | 'error' | 'debug' | 'green' | 'red' | 'yellow', 'info');
        void {
            : .options.verbose && level === 'debug'
        };
        {
            return;
            let colorCode = '';
