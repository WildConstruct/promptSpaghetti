...
import { VFXExportFormat, VFXExportOptions, VFXPromptVariant, VFXGraphConnection } from VFXRenderingData;
from;
'../types/VFXExport';
        ...
        return {
            ...
            generator: {
                ...
                export: {
                    ...
                    , // List of breaking changes from base version }
                    compatibility: {
                        controlNet: true,
    ...
    buildExecutionData(executionResults, options) {
            ...
            performance;
        {
            totalTime: executionResults?.executionTime || 0,
                nodePerformance;
            this.buildNodePerformance(executionResults),
                memoryUsage;
            options?.includePerformanceData ? 1024 * 1024 * 10 : undefined; // 10MB placeholder }
        }
        history: {
...
buildNodePerformance(executionResults ?  : any);
{
    ...
    if (executionResults?.nodePerformance) {
        for (const [nodeId, time] of Object.entries(executionResults.nodePerformance)) {
            ...
            return performance;
            buildExtensions(options, VFXExportOptions);
            {
                const extensions = {};
            ...
            ;
            return extensions;
            buildRenderingData(variables ?  : Record);
            VFXRenderingData;
    ...
    `.trim();
                    ...
                    executionMetadata;
                {
                    totalNodes: Object.keys(nodeSeeds).length,
                        executionTime;
                    executionResults?.executionTime || 0,
                        memorySnapshot;
                    process.memoryUsage();
                }
                undefined;
        ...
        current ${currentEnv.nodeVersion}`);
                                                ...
                                                if (issues.length === 0 && randomization.rngState) {
                                                    confidence = 'exact';
                                                    if (randomization.masterSeed && Object.keys(randomization.nodeSeed).length > 0) {
                                                ...
                                                ;
                                                preserveNodeConfiguration(node, Node);
                                                ...
                                                };
                                                {
                                                    ...
                                                    if (Array.isArray(node.data[field])) {
                                                        ...
                                                        return config;
                                                        calculateWeightDistribution(weights, number);
                                                        {
                                                            percentages: number;
                                                            entropy: number;
                                                            uniformity: number;
