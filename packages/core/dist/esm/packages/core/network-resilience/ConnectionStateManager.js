                    ...
                    if (metrics.packetLoss !== undefined) {
                        ...
                        if (metrics.bandwidth !== undefined) {
                            ...
                            if (metrics.jitter !== undefined) {
                                ...
                                async;
                                testConnection();
                                Promise < ConnectionMetrics > { const: startTime = performance.now(),
                                    try: {
                                ...
                                }
                                {
                                    ...
                                    addToStateHistory(state, ConnectionState, timestamp, number, reason ?  : string);
                                    void {
                                        this: .stateData.stateHistory.push({ state, timestamp, reason }),
                                        : .stateData.stateHistory.length > this.config.maxHistorySize
                                    };
                                    {
                                        this.stateData.stateHistory = this.stateData.stateHistory.slice(-this.config.maxHistorySize);
                                        updateDowntime();
                                        void {
                                            : .stateData.disconnectedAt && this.stateData.lastConnected };
                                        {
