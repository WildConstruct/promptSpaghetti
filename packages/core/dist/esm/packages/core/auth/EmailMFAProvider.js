...
import { z } from 'zod';
import { MFAMethodType, MFAMethodStatus, MFAVerificationResult, MFAEnrollmentRequest, MFA_CONSTANTS } from '../types/MFATypes';
...
;
checkEmailReputation(email, string);
Promise;
getRateLimitState(userId, string, action, string);
Promise;
updateRateLimitState(userId, string, action, string, count, number);
Promise;
// Audit logging
logVerificationAttempt(attempt, unknown);
Promise;
logSecurityEvent(event, unknown);
Promise;
// ========================================
...
z.string().min(1).max(100),
    emailAddress;
z.string().email().max(320); // RFC 5321 limit,
;
const EmailVerificationSchema = z.object({});
configurationId: z.string().uuid(),
    code;
z.string().regex(/^\d{6}$/), // 6-digit numeric code
    backupCode;
z.boolean().optional();
;
// ========================================
...
// ========================================
async;
enrollMethod(userId, string, request, MFAEnrollmentRequest);
Promise < MFAEnrollmentResponse > {
    ...
    if(existingConfig) { }
} && existingConfig.status !== MFAMethodStatus.REVOKED;
{
    ...
    if (!emailValid) {
        ...
        if (!reputation.valid || reputation.risk > 70) {
            ...
            async;
            completeEnrollment(userId, string, verificationId, string, code, string);
            Promise < void  > {
                    ...
                    if (decryptedCode !== code) {
                        ...
                        if (verification.attempts >= 3) {
                            ...
                            if (configuration) {
                                ...
                                // ========================================
                                async;
                                initiateVerification(userId, string, configurationId, string, context, RiskAssessmentContext);
                                Promise < string > {
                                    // Get configuration
                                ...
                                } || configuration.userId !== userId;
                                {
                                    ...
                                    if (configuration.status !== MFAMethodStatus.ACTIVE) {
                                        ...
                                        async;
                                        verifyCode(request, MFAVerificationRequest, context, RiskAssessmentContext);
                                        Promise < MFAVerificationResponse > {
                                            ...
                                            try: {
                                                    ...
                                                    if (configuration.lockedUntil && configuration.lockedUntil > new Date()) {
                                                        ...
                                                        if (!rateLimitCheck.allowed) {
                                                            ...
                                                            if (!activeVerification) {
                                                                ...
                                                                codeMatches,
                                                                    result;
                                                                codeMatches ? MFAVerificationResult.SUCCESS : MFAVerificationResult.INVALID_CODE,
                                                                    ipAddress;
                                                                context.ipAddress,
                                                                    userAgent;
                                                                context.userAgent,
                                                                ...
                                                                Date.now() - startTime,
                                                                    codeLength;
                                                                validated.code.length,
                                                                    riskScore;
                                                                activeVerification.metadata.riskScore,
                                                                ;
                                                            }
                                                            ;
                                                            if (!codeMatches) {
                                                                ...
                                                                if (configuration.failedAttempts >= MFA_CONSTANTS.SECURITY.MAX_FAILED_ATTEMPTS) {
                                                                    ...
                                                                    'high',
                                                                        description;
                                                                    'Account locked due to repeated failed MFA attempts',
                                                                        metadata;
                                                                    {
                                                                        ipAddress: context.ipAddress,
                                                                            userAgent;
                                                            ...
                                                            try { }
                                                                ...
                                                                false,
                                                                    result;
                                                                MFAVerificationResult.INVALID_CODE,
                                                                    ipAddress;
                                                                context.ipAddress,
                                                                    userAgent;
                                                                context.userAgent,
                                                ...
                                                // Email Sending Methods
                                                // ========================================
                                                async sendEnrollmentVerification(userId, emailAddress) {
                                                    const verificationCode = this.generateVerificationCode();
                                                    const verificationId = crypto.randomUUID();
                                                    const verification = {
                                                        id: verificationId,
                                                ...
                                                code: string,
                                                displayName: string,
                                                includeSecurityWarning: boolean = false, void:  > {
                                                    const: template, EmailTemplate = {
                                                ...
                                                async checkRateLimitForVerification(userId) {
                                                ...
                                                async cleanupExpiredVerifications(userId) {
                                            ...
                                            async updateEmailAddress(userId, configurationId, newEmailAddress) {
                                                ...
                                                if (!configuration || configuration.userId !== userId) {
                                                    ...
                                                    if (!emailValid) {
                                                        ...
                                                        async;
                                                        disableMethod(userId, string, configurationId, string);
                                                        Promise < void  > {
                                                            const: configuration = await this.storage.getConfigurationById(configurationId),
                                                            if(, configuration) { }
                                                        } || configuration.userId !== userId;
                                                        {
                                                            ...
                                                            await this.storage.updateConfiguration(configurationId, configuration);
                                                            async;
                                                            revokeMethod(userId, string, configurationId, string);
                                                            Promise < void  > {
                                                                const: configuration = await this.storage.getConfigurationById(configurationId),
                                                                if(, configuration) { }
                                                            } || configuration.userId !== userId;
