...
export var CodeType;
(function (CodeType) {
    CodeType["EMAIL_VERIFICATION"] = "email_verification";
    CodeType["SMS_VERIFICATION"] = "sms_verification";
    CodeType["TOTP_SECRET"] = "totp_secret";
    CodeType["BACKUP_CODE"] = "backup_code";
    CodeType["RECOVERY_TOKEN"] = "recovery_token";
    CodeType["API_TOKEN"] = "api_token";
    CodeType[CodeType["export"] = void 0] = "export";
    CodeType[CodeType["enum"] = void 0] = "enum";
    CodeType[CodeType["CodeFormat"] = void 0] = "CodeFormat";
})(CodeType || (CodeType = {}));
{
...
type: z.nativeEnum(CodeType),
    format;
z.nativeEnum(CodeFormat),
    length;
z.number().min(4).max(64),
    expiryMinutes;
z.number().min(1).max(43200).optional(), // Max 30 days,
    userContext;
z.object({});
...
class EntropyCalculator {
     ...
     */
    static calculateEntropy(code, charset) {
    ...
    averageAttempts = possibleCombinations / 2;
    worstCaseAttempts = possibleCombinations;
    averageSeconds = averageAttempts / attemptsPerSecond;
    worstCaseSeconds = worstCaseAttempts / attemptsPerSecond;
...
}
{
    ...
    validateEntropyRequirements(entropy, number, codeType, CodeType);
    boolean;
    {
        const requirements = {
...
}
{
    ...
    if (state.attempts >= limit.maxAttempts) {
        ...
        };
        {
            ...
            hashingAlgorithm: string = 'sha256';
            ...
            constructor();
            {
                 ...
                 */
                async;
                generateCode(options, CodeGenerationOptions);
                ...
                if (!rateLimit.allowed) {
                    ...
                    };
                    {
                        return {
                            ...
                            : .validateCodeFormat(inputCode, storedData.format)
                        };
                        {
                            return {
                                valid: false,
                            ...
                            if (defaultOptions.constantTimeValidation) {
                                ...
                                if (processingTime < minProcessingTime) {
                                     ...
                                     */
                                    async;
                                    generateAPIToken(userId, string, expiryDays ?  : number);
                                    Promise < GeneratedCode > {
                                        return: this.generateCode({}),
                                     ...
                                     */
                                    resetRateLimit(userId, string, codeType, CodeType);
                                    void {
                                        const: key = `${userId}:${codeType}`
                                    };
                                    this.rateLimitService.resetRateLimit(key);
                                    generateSecureCode(options, CodeGenerationOptions);
                                    string;
                                    {
                                        if (options.format === CodeFormat.UUID) {
                                            ...
                                            for (let i = 0; i < options.length; i++) {
                                                ...
                                                if (options.enforceComplexity) {
                                                    ...
                                                    return code;
                                                    getCharset(options, CodeGenerationOptions);
                                                    string;
                                                    {
                                                        if (options.customCharset) {
                                                            ...
                                                            if (options.excludeSimilar) {
                                                                for (const char of SIMILAR_CHARACTERS) {
                                                                    ...
                                                                    return charset;
                                                                    enforceComplexity(code, string, charset, string);
                                                                    string;
                                                                    {
                                                                        ...
                                                                        if (hasNumbers && hasLetters) {
                                                                            if (!/\d/.test(code)) {
                                                                                ...
                                                                                if (!/[A-Z]/.test(code)) {
                                                                                    ...
                                                                                    return code;
                                                                                    hashCode(code, string, salt, string);
                                                                                    string;
                                                                                    {
                                                                                        ...
                                                                                        boolean;
                                                                                        {
                                                                                            if (a.length !== b.length) {
                                                                                                ...
                                                                                                for (let i = 0; i < a.length; i++) {
                                                                                                    ...
                                                                                                    return result === 0;
                                                                                                    validateCodeFormat(code, string, format, CodeFormat);
                                                                                                    boolean;
                                                                                                    {
                                                                                                        ...
                                                                                                        export class CodeGenerationUtils {
                                                                                                            entropy = generatedCode.metadata.entropy;
                                                                                                            crackTime = EntropyCalculator.calculateCrackTime(entropy);
                                                                                                        }
                                                                                                        let strength;
                                                                                                        if (entropy < 30)
                                                                                                            strength = 'weak';
                                                                                                            ...
                                                                                                            if (generatedCode.format === CodeFormat.NUMERIC) {
                                                                                                                ...
                                                                                                                if (!generatedCode.expiresAt) {
                                                                                                                    ...
                                                                                                                    string;
                                                                                                                    {
                                                                                                                        ...
                                                                                                                        formatCodeForDisplay(code, string, format, CodeFormat);
                                                                                                                        string;
                                                                                                                        {
                                                                                                                            switch (format) {
