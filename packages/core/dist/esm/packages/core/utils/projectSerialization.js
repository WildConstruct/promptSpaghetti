import { PsgFile, validatePsgFile, isVersionCompatible, createDefaultMetadata, createDefaultSettings } from PSG_FORMAT_VERSION;
from;
'../schemas/psgSchema';
;
;
...
export function serializeProject(graphState, metadata, settings, options = {}) {
    try {
        const { includeMetadata = true, includeSettings = true, includeCollaboration = true, compress = false };
        validateOutput = true
            = options;
    ...
    if (includeCollaboration && graphState.annotations) {
        ...
        try { // Generate checksum for integrity
            ...
            export function deserializeProject(content, options = {}) {
                try {
                    const { skipValidation = false, autoMigrate = true };
                    preserveIds = true
                        = options;
                    // Parse JSON
                    let psgData;
