...
export class OptimizedSerializer {
    static LARGE_GRAPH_THRESHOLD = 10 * 1024 * 1024; // 10MB
    static COMPRESSION_THRESHOLD = 1024; // 1KB
    incrementalState = new Map();
    /**
     * Serialize project with optimal format selection
     */
    async serialize() { }
}
((projectData, options = { format: 'json' }) => {
    ...
    try {
        ...
        };
        {
            ...
            }
            {
                ...
                } || !options.incremental;
                {
                    ...
                    new Set(),
                        baselineChecksum;
                    await this.calculateChecksum(result.data),
                        version;
                    1,
                    ;
                }
                ...
                getMetrics();
                {
                    ...
                    return {
                        ...
                        // Private methods
                        optimizeOptions(options, estimatedSize) {
                        ...
                        estimateSize(projectData) {
                            try {
                                ...
                                const edgeCount = projectData.graph.edges?.length || 0;
                    ...
                    }(projectData, PSGFile, options, SerializationOptions);
                    ...
                    size: compressed.byteLength,
                        compressionRatio;
                    compressed.byteLength / originalSize;
                }
                ;
                return {
                    data: jsonString,
                ...
                async;
                serializeToBinary(((projectData, options) => {
                ...
                }), private, async, serializeCompressed(((projectData, options) => {
                    // Use optimized graph storage first
                    const jsonResult = await this.serializeToJSON(projectData, {});
                }), ...options, format, 'json', compression, undefined));
            }
            ;
        ...
        size: compressed.byteLength,
            compressionRatio;
        compressed.byteLength / jsonResult.size;
    }
    finally { }
    ;
    async;
    deserializeBinary(data, ArrayBuffer);
    Promise < any > {
    ...
    { // "PSGB"
        ...
        async;
        compressString(data, string, algorithm, string);
        Promise < ArrayBuffer > {
