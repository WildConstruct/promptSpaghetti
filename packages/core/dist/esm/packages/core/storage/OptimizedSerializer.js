...
((projectData, options = { format: 'json' }) => {
    ...
    try {
        ...
        Promise < PSGFile > { const: startTime = performance.now(),
            try: {
                let, parsedData: any,
                if(, data) { }
            } === 'string' };
        ...
        optimizeOptions(options, SerializationOptions, estimatedSize, number);
            ...
            if (estimatedSize > this.COMPRESSION_THRESHOLD && !optimized.compression) {
                ...
                if (estimatedSize > this.LARGE_GRAPH_THRESHOLD * 5) {
                    ...
                    if (optimized.prettyPrint === undefined) {
                        ...
                        return optimized;
                        estimateSize(projectData, PSGFile);
                        number;
                        {
                            try {
                                ...
                                try {
                                    ...
                                    return 0;
                                    async;
                                    serializeToJSON((projectData, options) => {
                            ...
                            async;
                            serializeCompressed(((projectData, options) => {
