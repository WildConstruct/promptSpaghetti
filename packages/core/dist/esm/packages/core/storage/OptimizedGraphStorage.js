...
export class OptimizedGraphStorage {
                                ...
                                void {
                                    ...
                                    */
                                    ...
                                    try: { const: nodes = OptimizedGraphStorage.decompressNodes(compressed.node_data),
                                        ...
                                        return: new OptimizedGraphStorage(nodes, edges) }, catch(error) {
    ...
    hasIncoming = (this.indexes.incomingEdges.get(nodeId)?.length || 0) > 0;
    hasOutgoing = (this.indexes.outgoingEdges.get(nodeId)?.length || 0) > 0;
...
if (!hasIncoming) {
    ...
    this.indexes.rootNodes.delete(nodeId);
    this.indexes.isolatedNodes.delete(nodeId);
    addToSetMap(map, (Map), key, K, value, V);
    void {
        if(, map) { }, : .has(key)
    };
    {
        ...
        removeFromSetMap(map, (Map), key, K, value, V);
        void {
            ...
            size, JSON, : .stringify(node).length * 2
        };
        ;
    ...
    Uint8Array;
    {
        ...
        Uint8Array;
        {
            ...
            return new TextEncoder().encode(jsonString);
            decompressNodes(data, Uint8Array);
            Node;
            {
                const jsonString = new TextDecoder().decode(data);
                return JSON.parse(jsonString);
                decompressEdges(data, Uint8Array);
                Edge;
                {
                    const jsonString = new TextDecoder().decode(data);
                    return JSON.parse(jsonString);
