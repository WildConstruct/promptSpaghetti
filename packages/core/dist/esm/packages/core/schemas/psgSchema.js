...
autoSave: z.boolean().default(true),
    backupInterval;
z.number().min(1).default(5), // minutes,
    maxBackups;
...
z.number().min(5).max(50).default(20),
    theme;
z.enum(['light', 'dark', 'auto']).default('auto'),
    showMinimap;
...
stickyNotes: z.array(z.object({}), id, z.string(), content, z.string(), position, z.object({ x: z.number(), y: z.number() }), size, z.object({ width: z.number(), height: z.number() }), color, z.string(), author, z.string().optional(), timestamp, z.string().datetime());
([]),
    annotations;
z.object({});
nodeLabels: z.record(z.string()).default({}),
    regionGroups;
z.array(z.object({}), id, z.string(), name, z.string(), nodeIds, z.array(z.string()), position, z.object({ x: z.number(), y: z.number() }), size, z.object({ width: z.number(), height: z.number() }), color, z.string(), collapsed, z.boolean().default(false));
([]),
    connectionLabels;
...
fileType: z.literal('psg').describe('File type identifier'),
    formatVersion;
z.string().describe('Format version for compatibility checking'),
    // Core project data
    metadata;
...
GraphSchema,
    // Extended features
    collaboration;
CollaborationDataSchema.optional(),
    // Extensibility for future features
    extensions;
z.record(z.unknown()).optional().describe('Extension data for future features'),
    // File integrity
    checksum;
