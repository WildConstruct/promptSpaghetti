export class ExternalDataService {
 ...
 || 'material',
    content;
item[fieldMapping.content] || item.name || item.description,
    description;
item[fieldMapping.description],
    metadata;
{
    era: this.parseEras(item[fieldMapping.era] || item.period),
        authenticity;
    parseFloat(item[fieldMapping.authenticity]) || 0.5,
        source;
    config.source_name || 'external',
        tags;
    this.parseTags(item[fieldMapping.tags] || item.keywords || []),
    ;
}
relationships: {
    compatible: [],
...
return node;
;
filterData(data, any, config, any);
any;
{
    ...
    for (const [field, value] of Object.entries(config.filters || {})) {
        ...
        ;
        validateData(data, any, config, any);
        any;
        {
            return data.filter(item => { });
            // Basic validation
            if (!item.name && !item.content && !item.description) {
                ...
                if (config.required_fields) {
                    for (const field of config.required_fields) {
                        ...
                        ;
                        enrichData(data, any, config, any);
                        any;
                        {
                            return data.map(item => ({}), ...item, ...config.additional_fields, enriched_at, new Date().toISOString());
                        }
                        ;
                        isValidUTDGNode(node, any);
                        node;
                        is;
                        UTDGNode;
                        {
                                                ...
                                                string;
                                                {
                                                    ...
                                                    if (typeof tagData === 'string') {
                                                        ...
                                                        if (Array.isArray(tagData)) {
                                                            ...
                                                            return [];
                                                            buildApiUrl(endpoint, string, query, HistoricalQuery);
                                                            string;
                                                            {
                                                                        ...
                                                                        if (query.category) {
                                                                            ...
                                                                            if (query.limit) {
                                                                                ...
                                                                                if (query.offset) {
                                                                                    ...
                                                                                    return url.toString();
                                                                                    buildAuthHeaders(auth ?  : AuthConfig);
                                                                                    HeadersInit;
                                                                                    {
                                                                                        const headers = {
                                                                                ...
                                                                                ;
                                                                                getSourcesForQuery(query, HistoricalQuery);
                                                                                DataSource;
                                                                                {
                                                                                    ...
                                                                                    if (query.era) {
                                                                                        ...
                                                                                        ;
                                                                                        processResults(results, UTDGNode, query, HistoricalQuery);
                                                                                        UTDGNode;
                                                                                        {
                                                                                        ...
                                                                                        number;
                                                                                        {
                                                                                            ...
                                                                                            generateCacheKey(query, HistoricalQuery, sourceIds ?  : string);
                                                                                            string;
                                                                                            {
                                                                                                const keyData = {
                                                                                                    ...query,
