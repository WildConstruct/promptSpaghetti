...
'high',
    title;
`Unify items to ${dominantEra} period`;
...
specific_actions: this.generateEraUnificationActions(affectedNodes, dominantEra),
    historical_context;
`${dominantEra} had specific materials, techniques, and styles that differed from other periods`;
trade_offs: ['May reduce visual variety', 'Increases historical accuracy'],
    example;
this.getEraExampleReplacement(dominantEra, affectedNodes[0]);
;
...
'medium',
    title;
'Use transitional period elements',
    description;
'Select items from transitional periods that bridge different eras',
    specific_actions;
[{},
    ...
    historical_context;
'Historical periods often overlapped, creating transitional phases',
    trade_offs;
...
'high',
    title;
`Use ${socialClass}-appropriate items`;
...
specific_actions: [{},
    ...
    historical_context;
this.historicalDatabase.getSocialClassContext(socialClass, context.era),
    example;
appropriateAlternatives[0];
;
;
return suggestions;
...
'Use locally available materials',
    description;
`Replace ${node.content} with materials available in ${context.era.name}`;
specific_actions: [{},
    action_type, 'replace_node',
    ...
    historical_context;
this.historicalDatabase.getMaterialContext(node.content, context.era),
    trade_offs;
['May change visual appearance', 'Increases historical accuracy'],
    example;
...
'Add cultural context',
    description;
`Provide proper historical and cultural context for ${node.content}`;
specific_actions: [{},
    action_type, 'add_context',
    ...
    historical_context;
`${node.content} had specific cultural and religious significance in ${context.era.name}`;
...
'medium',
    title;
'Use secular alternatives',
    description;
'Replace ceremonial items with secular equivalents',
    specific_actions;
[{},
    action_type, 'replace_node',
    ...
    historical_context;
'Secular versions often existed alongside ceremonial ones';
;
;
return suggestions;
...
'high',
    title;
`Align items to ${commonPeriod.start}-${commonPeriod.end} period`;
...
'medium',
    title;
`Use ${context.region}-specific items`;
...
specific_actions: [{},
    ...
    historical_context;
this.historicalDatabase.getRegionalContext(context.region, context.era);
;
return suggestions;
generateImprovementSuggestions(((nodes, context) => {
    const suggestions = [];
    ...
    constraint_id), 'general_improvement', type, 'educational', priority, 'low', title, 'Improve overall historical authenticity', description, `Current authenticity: ${Math.round(avgAuthenticity * 100)}%. Consider using more verified historical sources.`, specific_actions, [{},
    action_type, 'add_context',
...
'Add item variety',
    description;
'Consider adding different types of items for visual interest',
    specific_actions;
[{},
    ...
    historical_context;
'Historical contexts typically included diverse object types';
;
return suggestions;
findConstraintById(id, string);
HistoricalConstraint | null;
{
    // This would typically query a constraint database
    return null;
    generateEraUnificationActions(nodes, UTDGNode, targetEra, string);
    SpecificAction;
    {
        return nodes.map(node => ({}), action_type, 'replace_node', description, `Replace ${node.content} with ${targetEra}-appropriate alternative`);
    }
}
target_node_ids: [node.id],
    rationale;
`${node.content} is not authentic to ${targetEra}`;
;
getEraExampleReplacement(era, string, node, UTDGNode);
string;
{
    ...
    return examples[era] || 'Consult historical sources for appropriate alternatives';
    findCommonPeriod(nodes, UTDGNode);
    {
        start: number, end;
        number;
    }
...
number;
{
    ...
    class HistoricalKnowledge {
        getAppropriateItemsForClass(socialClass, era) {
            // Simplified - would query actual historical database
            const classItems = {
                peasant: ['rough wool cloth', 'hemp rope', 'wooden tools', 'leather boots'],
                merchant: ['fine wool', 'brass buttons', 'cotton fabric', 'quality leather'],
                noble: ['silk fabric', 'silver decorations', 'fine fur', 'golden ornaments'],
                clergy: ['simple robe', 'prayer beads', 'illuminated manuscript', 'sacred symbols'],
                royal: ['purple silk', 'gold thread', 'precious gems', 'ermine fur'],
            };
            return classItems[socialClass] || [];
            getSocialClassContext(socialClass, SocialClass, era, Era);
            string;
            {
            ...
            getAvailableMaterials(era, Era, region ?  : string);
            string;
            {
