    ...
    static getInstance() {
        ...
        MedievalClothing;
        {
            ...
            UTDGNode;
            {
                 ...
                 */
                getAccessories(criteria, { era: Era,
                    social_class: SocialClass,
                    type: string } = {});
                UTDGNode;
                {
                    ...
                    filter(item => item.medieval_specific.garment_type === 'chemise' || item.medieval_specific.garment_type === 'braies');
                ...
                filter(item => item.medieval_specific.garment_type === 'tunic' || item.medieval_specific.garment_type === 'gown');
...
clothing.push({ id: 'medieval_monk_robe_001',
    ...
    variations: [] }, constraints, [], medieval_specific, {
    garment_type: 'robe',
    construction_method: 'sewn',
    fabric_type: 'wool',
...
;
return accessories;
eraMatches(itemEras, Era, targetEra, Era);
boolean;
{
    return itemEras.some(era => era.name === targetEra.name ||
        (era.period.start <= targetEra.period.end && era.period.end >= targetEra.period.start));
}
generateOutfitDescription(outfit, (MedievalClothing | UTDGNode)[], criteria, any);
string;
{
...
'warning',
    message;
'Some materials may have been rare or unavailable in medieval Europe',
    historical_basis;
'Trade routes and material availability varied significantly in medieval times';
{
...
'suggestion',
    message;
'Consider social class appropriateness for clothing and accessories',
    historical_basis;
'Medieval society had strict hierarchies reflected in clothing and possessions';
;
