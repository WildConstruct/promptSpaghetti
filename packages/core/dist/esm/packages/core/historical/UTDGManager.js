 ...
 */
import { UTDGNode, Era, HistoricalQuery, HistoricalQueryResult, ContentGenerationConfig, GeneratedContent, VFXExportData, ValidationReport, ConstraintValidationResult } from HISTORICAL_ERAS;
from;
'../types/UTDG';
...
export class UTDGManager {
    ...
    static getInstance() {
        ...
        */
        async;
        queryHistoricalContent(query, HistoricalQuery);
        ...
        */
        async;
        generateHistoricalContent(config, ContentGenerationConfig);
        Promise < GeneratedContent > { const: startTime = performance.now(),
    ...
    ;
    break;
    'market_day' = { era: HISTORICAL_ERAS.MEDIEVAL_HIGH,
 ...
 */
exportForVFX(nodes, UTDGNode, era, Era, scene_description, string);
VFXExportData;
 ...
 */
validateUTDGData(nodes, UTDGNode);
ValidationReport;
{
    const issues = [];
    ...
    for (const node of nodes) {
        ...
        if (node.content)
            filledFields++;
        if (node.metadata?.era?.length > 0)
            filledFields++;
        if (node.metadata?.authenticity !== undefined)
            filledFields++;
    ...
    consistencyScore = constraintResult.valid ? 1.0 : Math.max(0, 1 - constraintResult.violations.length / nodes.length);
 ...
 */
registerExternalSource(source, any);
void { this: .externalDataService.registerDataSource(source) };
/**
...
queryMedievalDemo(query, HistoricalQuery);
HistoricalQueryResult;
{
    ...
    if (!query.category || query.category.includes('material')) {
        ...
        generateHistoricalContentSync(config, ContentGenerationConfig);
        GeneratedContent;
        {
            const startTime = performance.now();
            ...
            getRelevantNodeTypes(scenario, string);
            any;
            {
                switch (scenario) {
                ...
                filterByConfiguration(nodes, UTDGNode, config, ContentGenerationConfig);
                UTDGNode;
                {
                    return nodes.filter(node => {
                        if (config.required_elements) {
                            const hasRequired = config.required_elements.some(element => node.content.toLowerCase().includes(element.toLowerCase()) ||
                                node.metadata.tags.some(tag => tag.toLowerCase().includes(element.toLowerCase())));
                            if (!hasRequired)
                                return false;
                        }
                        if (config.forbidden_elements) {
                            const hasForbidden = config.forbidden_elements.some(element => node.content.toLowerCase().includes(element.toLowerCase()) ||
                                node.metadata.tags.some(tag => tag.toLowerCase().includes(element.toLowerCase())));
                            if (hasForbidden)
                                return false;
                        }
                    ...
                    applyCreativityFilter(nodes, UTDGNode, config, ContentGenerationConfig);
                    UTDGNode;
                    {
                        ...
                        else if (config.creativity_factor > 0.7) { // High creativity - include more variation
                            const shuffled = nodes.sort(() => Math.random() - 0.5);
                            return shuffled.slice(0, targetCount);
                        }
                        else { // Medium creativity - balanced selection
                            return nodes
                                .sort((a, b) => {
                                ...
                                .slice(0, targetCount);
                            getAppliedConstraints(nodes, UTDGNode, era, Era);
                            any;
                            {
                                ...
                                calculateAccuracyScore(nodes, UTDGNode, validation, ConstraintValidationResult);
                                number;
                                {
                                    if (nodes.length === 0)
                                        return 0;
                                ...
                                generateRecommendations(overallScore, number, metrics, any);
                                string;
                                {
                                    ...
                                    extractMaterialProperties(node, UTDGNode);
                                    ...
                                    extractPatternDescription(node, UTDGNode);
                                    string;
                                    {
                                        return node.content.split('.')[0] || node.content;
                                    }
                                    extractColorPalette(node, UTDGNode);
                                    string;
                                    { // Extract colors mentioned in content and tags
                                        ...
                                        extractCharacterTypes(nodes, UTDGNode);
                                        string;
                                        {
                                            const types = new Set();
                                            nodes.forEach(node => {
                                            ...
                                            generateClothingCombinations(nodes, UTDGNode);
                                            string[];
                                            {
                                                const combinations = [];
                                                ...
                                                if (garments.length >= 2) {
                                                    ...
                                                    return combinations;
                                                    calculateSocialDistribution(nodes, UTDGNode);
                                                    ...
                                                    return distribution;
                                                    determineArchitecturalStyle(era, Era);
                                                    string;
                                                    {
                                                        ...
                                                        string;
                                                        {
                                                            ...
                                                            if (sourcedNodes.length > 0) {
                                                                ...
                                                                ;
                                                                generateAtmosphericNotes(era, Era);
                                                                string;
                                                                {
                                                                    return [
