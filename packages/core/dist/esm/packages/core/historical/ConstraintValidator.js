 ...
 */
addConstraint(constraint, HistoricalConstraint);
void { this: .constraints.push(constraint) };
/**
 * Remove a constraint by ID
 */
removeConstraint(constraintId, string);
void { this: .constraints = this.constraints.filter(c => c.id !== constraintId) };
/**
 * Configure which enforcement levels are active
 */
setEnforcement(levels, ('strict' | 'warning' | 'suggestion')[]);
void { this: .enabledEnforcement = levels };
/**
 * Validate a set of UTDG nodes against historical constraints
 */
validateNodes(nodes, UTDGNode);
ConstraintValidationResult;
{
    ...
    for (const constraint of this.constraints) {
        ...
        if (result) {
            ...
            break;
            return { valid: violations.length === 0,
                violations,
                warnings,
                suggestions };
        }
         ...
         */
        validateForEra(nodes, UTDGNode, era, Era);
        ConstraintValidationResult;
        { // Filter constraints relevant to the era
            const eraConstraints = this.constraints.filter(constraint => constraint.eras.some(cEra => this.erasOverlap(cEra, era)));
         ...
         */
        getSuggestions(nodes, UTDGNode, era, Era);
        string;
        {
            ...
            if (nodes.length > 0) {
                ...
                if (avgAuthenticity < 0.7) {
                    ...
                    if (missingSourceNodes.length > 0) {
                        ...
                        return suggestions;
                        evaluateConstraint(constraint, HistoricalConstraint, nodes, UTDGNode);
                        ConstraintViolation | ConstraintWarning | ConstraintSuggestion | null;
                        {
                            ...
                            const baseResult = { constraint_id: constraint.id,
                ...
                ;
                return null;
                checkEraCompatibility(constraint, HistoricalConstraint, nodes, UTDGNode);
                string;
                {
                    const violatingNodes = [];
                            ...
                            if (eraGroups.size > 1) {
                                ...
                                for (let i = 0; i < eraNames.length; i++) {
                                    for (let j = i + 1; j < eraNames.length; j++) {
                                        ...
                                        checkSocialClassAppropriateness(constraint, HistoricalConstraint, nodes, UTDGNode);
                                        string;
                                        {
                                            const violatingNodes = [];
                                            for (const node of nodes) {
                                                if (node.metadata.social_class && constraint.social_classes) {
                                                    ...
                                                    return violatingNodes;
                                                    checkMaterialAvailability(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                    string;
                                                    {
                                                        const violatingNodes = [];
                                                        for (const node of nodes) {
                                                                ...
                                                                for (const nodeEra of nodeEras) {
                                                                    for (const constraintEra of constraintEras) {
                                                                        if (this.erasOverlap(nodeEra, constraintEra)) {
                                                                            ...
                                                                            return violatingNodes;
                                                                            checkCulturalAppropriateness(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                                            string;
                                                                            {
                                                                                const violatingNodes = [];
                                                                                for (const node of nodes) {
                                                                                    // Check for culturally sensitive items
                                                                                    const culturalTags = node.metadata.tags.filter(tag => tag.includes('religious') || tag.includes('sacred') || tag.includes('ceremonial') || tag.includes('ritual'));
                                                                                    if (culturalTags.length > 0) {
                                                                                        ...
                                                                                        return violatingNodes;
                                                                                        checkTemporalConsistency(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                                                        string;
                                                                                        {
                                                                                            const violatingNodes = [];
                                                                                            if (nodes.length < 2)
                                                                                            ...
                                                                                            for (const node of nodes) {
                                                                                                ...
                                                                                                for (const node of nodes) {
                                                                                                    const nodeValidInPeriod = node.metadata.era.some(era => era.period.start <= commonPeriod.end && era.period.end >= commonPeriod.start);
                                                                                                    if (!nodeValidInPeriod) {
                                                                                                        violatingNodes.push(node.id);
                                                                                                    }
                                                                                                    return violatingNodes;
                                                                                                    checkRegionalAuthenticity(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                                                                    string;
                                                                                                    {
                                                                                                        const violatingNodes = [];
                                                                                                        const constraintRegions = constraint.regions || [];
                                                                                                        ...
                                                                                                        for (const node of nodes) {
                                                                                                            ...
                                                                                                            const hasValidRegion = nodeRegions.some(region => constraintRegions.some(cRegion => region.toLowerCase().includes(cRegion.toLowerCase()) || cRegion.toLowerCase().includes(region.toLowerCase())));
                                                                                                            if (!hasValidRegion) {
                                                                                                                violatingNodes.push(node.id);
                                                                                                            }
                                                                                                            return violatingNodes;
                                                                                                            evaluateGenericConstraint(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                                                                            string;
                                                                                                            { // This can be extended for custom constraint rules
                                                                                                                return [];
                                                                                                            }
                                                                                                            generateAlternatives(constraint, HistoricalConstraint, nodes, UTDGNode);
                                                                                                            string;
                                                                                                            {
                                                                                                                const alternatives = [];
                                                                                                                switch (constraint.rule) {
                                                                                                                ...
                                                                                                                boolean;
                                                                                                                {
                                                                                                                    ...
                                                                                                                    if (!hasCommonRegion && timeDifference > 200)
                                                                                                                        return true;
                                                                                                                    return false;
                                                                                                                }
                                                                                                                ...
                                                                                                                }
                                                                                                                {
                                                                                                                    ...
                                                                                                                    'material_availability',
                                                                                                                        eras;
                                                                                                                    [HISTORICAL_ERAS.MEDIEVAL_EARLY],
                                                                                                                        regions;
                                                                                                                    ['Northern Europe'],
                                                                                                                        enforcement;
                                                                                                                    'warning',
                                                                                                                        message;
                                                                                                                    'Silk was extremely rare and expensive in early medieval Northern Europe',
                                                                                                                        historical_basis;
                                                                                                            ...
                                                                                                            }
                                                                                                            {
                                                                                                                id: 'social-class-clothing',
                                                                                                                    rule;
                                                                                                                'social_class_appropriateness',
                                                                                                                    eras;
                                                                                                                [HISTORICAL_ERAS.MEDIEVAL_HIGH, HISTORICAL_ERAS.MEDIEVAL_LATE],
                                                                                                                    social_classes;
                                                                                                                ['peasant'],
                                                                                                                    enforcement;
                                                                                                                'warning',
                                                                                                                    message;
                                                                                                                'Elaborate clothing items inappropriate for peasant social class',
                                                                                                                    historical_basis;
                                                                                                                'Sumptuary laws regulated clothing by social class in medieval Europe';
                                                                                                            }
                                                                                                        }
                                                                                                        {
                                                                                                            ...
                                                                                                            'cultural_appropriateness',
                                                                                                                eras;
                                                                                                            Object.values(HISTORICAL_ERAS),
                                                                                                                enforcement;
                                                                                                            'suggestion',
                                                                                                                message;
                                                                                                            'Religious items should be used with cultural sensitivity and historical context',
                                                                                                                historical_basis;
                                                                                                            'Religious artifacts had sacred significance and specific usage contexts';
                                                                                                        }
                                                                                                        ;
