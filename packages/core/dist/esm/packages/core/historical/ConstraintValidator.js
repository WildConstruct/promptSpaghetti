...
export class ConstraintValidator {
     ...
     */
    evaluateConstraint(constraint, nodes) {
     ...
     */
    checkEraCompatibility(constraint, nodes) {
        const violatingNodes = [];
     ...
     * Check if items are appropriate for the specified social classes
     */
    checkSocialClassAppropriateness(constraint, nodes) {
        const violatingNodes = [];
        for (const node of nodes) {
     ...
     */
    checkMaterialAvailability(constraint, nodes) {
        const violatingNodes = [];
        for (const node of nodes) {
     ...
     */
    checkCulturalAppropriateness(constraint, nodes) {
        const violatingNodes = [];
        for (const node of nodes) {
     ...
     */
    checkTemporalConsistency(constraint, nodes) {
        const violatingNodes = [];
        if (nodes.length < 2)
            return violatingNodes;
     ...
     */
    checkRegionalAuthenticity(constraint, nodes) {
        const violatingNodes = [];
        const constraintRegions = constraint.regions || [];
     ...
     */
    evaluateGenericConstraint(constraint, nodes) {
     ...
     */
    generateAlternatives(constraint, nodes) {
        const alternatives = [];
        switch (constraint.rule) {
            case 'era_compatibility':
