...
export const useNodeDisclosureControl = (nodeId, nodeType) => {
    ...
    const setDisclosureLevel = useCallback((level) => { }, setNodeDisclosureLevel(nodeId, level));
}, [nodeId, setNodeDisclosureLevel];
// Toggle whether to use global default
const setUseGlobalDefault = useCallback((useGlobal) => { setNodeUseGlobalDefault(nodeId, useGlobal); }, [nodeId, setNodeUseGlobalDefault]);
// Check if a section should be shown at current disclosure level
const shouldShowSection = useCallback((sectionLevel) => {
...
export const useDisclosurePreferences = () => {
    ...
    const setGlobalLevel = useCallback((level) => { }, setGlobalDisclosureLevel(level));
}, [setGlobalDisclosureLevel];
const setInheritance = useCallback((inheritance) => { setPreferenceInheritance(inheritance); }, [setPreferenceInheritance]);
