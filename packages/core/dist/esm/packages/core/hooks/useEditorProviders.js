...
saveGraph: () => Promise;
loadGraph: (data) => void ;
exportGraph: (format) => any;
validateGraph: () => void ;
executeGraph: () => Promise;
void  > ;
executeCustomAction: (hookId, actionName, ...args) => any;
...
validationErrors: any = [];
{
    ...
    const editorContext = useMemo(() => ({}), nodes, initialNodes, edges, initialEdges, selectedNodeId, isLoading, hasUnsavedChanges, graphStore.hasUnsavedChanges);
}
validationErrors;
[initialNodes, initialEdges, selectedNodeId, isLoading, graphStore.hasUnsavedChanges, validationErrors];
;
// Create editor actions
const editorActions = useMemo(() => ({}), addNode, async (node) => { });
                                                                                                            ...
                                                                                                            for (const hook of hooks) {
                                                                                                                if (hook.onPreExecution) {
                                                                                                                    ...
                                                                                                                    for (const hook of hooks) {
                                                                                                                        ...
                                                                                                                        try { }
                                                                                                                        catch (error) { // Execute error hooks
                                                                                                                            const hooks = getSortedHooks();
                                                                                                                            for (const hook of hooks) {
                                                                                                                                ...
                                                                                                                                const getSortedHooks = useCallback(() => { return Array.from(providerRegistry.values()).sort((a, b) => a.priority - b.priority); }, []);
