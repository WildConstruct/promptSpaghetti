...
export function useWebSocket(options) {
    const { url = 'ws://localhost:8001', documentId, userId = 'anonymous', authToken, enabled = true, reconnectInterval = DEFAULT_CONFIG.reconnectInterval, maxReconnectAttempts = DEFAULT_CONFIG.maxReconnectAttempts, onGraphUpdate, onPresenceUpdate, onUserJoin, onUserLeave };
    onError
        = options;
...
useEffect(() => {
    ...
    client.on('graph_update', (update) => { optionsRef.current.onGraphUpdate?.(update); });
    client.on('presence_update', (presence) => { optionsRef.current.onPresenceUpdate?.(presence); });
    client.on('user_join', (user) => { optionsRef.current.onUserJoin?.(user); });
    client.on('user_leave', (user) => { optionsRef.current.onUserLeave?.(user); });
    client.on('error', (error) => { optionsRef.current.onError?.(error); });
...
const sendGraphUpdate = useCallback((update) => { return clientRef.current?.sendGraphUpdate(update) || false; }, []);
const sendPresenceUpdate = useCallback((presence) => { return clientRef.current?.sendPresenceUpdate(presence) || false; }, []);
...
export function usePresence(documentId, userId, userName, userAvatar) {
    ...
    const { sendCursorUpdate, sendSelectionUpdate, sendActivityUpdate, requestPresenceData };
    isConnected
        = useWebSocket({});
    documentId;
    userId;
    ...
    (data) => {
        ...
        data.users.forEach((user) => { }, usersMap.set(user.userId, user));
    };
    ;
        ...
        onCursorUpdate: (data) => {
            ...
            onSelectionUpdate: (data) => {
                ...
                onActivityUpdate: (data) => {
                    if (data.userId !== userId) {
                        setUserActivity(prev => new Map(prev.set(data.userId, {}), currentTool, data.currentTool, isTyping, data.isTyping, focusedNodeId, data.focusedNodeId));
                    }
                };
                onUserJoin: (user) => {
    ...
    }, ...userData, cursor, selection, selection?.nodeIds || [], currentTool, activity?.currentTool, isTyping, activity?.isTyping, focusedNodeId, activity?.focusedNodeId);
}
;
return users;
