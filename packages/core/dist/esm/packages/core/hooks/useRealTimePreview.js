...
Promise;
{
    ...
    const exportVariants = useCallback(() => {
        return {
            template,
            variables,
            variants: variants.map(v => ({}), seed, v.seed, result, v.result, weights, v.weightSnapshot, variables, v.variables, timestamp, v.timestamp, executionTime, v.executionTime)
        };
    }), performance, exportTimestamp;
    ();
}
;
