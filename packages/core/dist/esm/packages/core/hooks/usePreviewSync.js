...
GraphChangeAnalysis => {
    ...
    for (const nodeId of commonNodes) {
        ...
        // Preview state store
        const { isRealTimeEnabled, enableRealTimeSync, updateGraphHash, lastGraphHash, shouldAutoRefresh, getCachedResults, setCachedResults, updatePerformanceMetrics };
        performanceMetrics
...
useEffect(() => {
    const initializePreviewRunner = async () => {
        ...
        finally // Execute preview with caching and performance tracking
         { }
        [];
    };
});
...
// Enable/disable sync
const enableSync = useCallback((syncEnabled) => { enableRealTimeSync(syncEnabled); }, [enableRealTimeSync]);
// Get current change analysis
const getChangeAnalysis = useCallback(() => { return lastChangeAnalysisRef.current; }, []);
// Cleanup on unmount
useEffect(() => {
    return () => {
