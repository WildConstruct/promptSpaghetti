...
export const PSGFileSchema = z.object({ metadata: ProjectMetadataSchema,
    graph: GraphContentSchema }, settings, EditorSettingsSchema.default({}), exportMetadata, ExportMetadataSchema);
;
edges: Edge;
metadata: Partial;
settings ?  : Partial;
                ...
                if (consistencyErrors.length > 0) {
                    ...
                    try {
                        ...
                        if (zodError instanceof z.ZodError) {
                            const formattedErrors = zodError.errors.map(err => ({}), path, err.path.join('.'), message, err.message, received, err.code === 'invalid_type' ? err.received : undefined);
                        }
                    }
                    ;
                    ...
                    if (!compatibility.compatible) {
                        ...
                        try {
                            ...
                            return {
                                 ...
                                 */
                                function: updatePSGFileMetadata(psgFile, PSGFile),
                                changes: (Partial),
                                PSGFile
                            };
                            {
