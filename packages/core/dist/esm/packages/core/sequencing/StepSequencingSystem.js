...
return sequenceId;
async;
updateSequence(sequenceId, string, updates, (Partial));
Promise < void  > {
    ...
    : 
        .filter(exec => exec.sequenceId === sequenceId && )(exec.status === 'running' || exec.status === 'paused'),
...
async;
startExecution(sequenceId, string);
context: (Partial) = {},
    options;
{
    name ?  : string;
...
Promise < string > {
    ...
    ,
    : .activeExecutions.size >= this.config.execution.maxConcurrentExecutions
};
{
...
async;
executeStep(executionId, string, stepId, string);
Promise < StepExecutionStatus > {
    const: execution = this.executions.get(executionId),
    if(, execution) {
        throw new Error(`Execution ${executionId} not found`);
    },
