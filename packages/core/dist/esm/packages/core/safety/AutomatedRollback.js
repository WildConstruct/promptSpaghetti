...
export class AutomatedRollbackManager extends EventEmitter {
    ...
    async evaluateTrigger(trigger) {
        ...
        switch (condition.aggregate || 'avg') {
            ...
            case 'min':
                ...
                break;
            case 'sum':
                metricValue = stats.avg * 300;
                break; // Approximate sum
            case 'count':
                metricValue = monitoring.getMetrics(condition.metric)[condition.metric]?.length || 0;
                break;
     ...
     */
    async handleRollbackDecision(decision) {
     ...
     */
    async sendNotification(decision) {
        const message = {
            severity: 'critical',
