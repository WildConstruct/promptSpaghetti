...
;
return violations;
checkRuntimeBudgets(snapshot, PerformanceSnapshot);
BudgetViolation;
{
    const violations = [];
...
;
return violations;
checkApiBudgets(snapshot, PerformanceSnapshot);
BudgetViolation;
{
    const violations = [];
...
;
return violations;
checkMemoryBudgets(snapshot, PerformanceSnapshot);
BudgetViolation;
{
    const violations = [];
    // Peak heap usage
...
;
return violations;
checkNetworkBudgets(snapshot, PerformanceSnapshot);
BudgetViolation;
{
    const violations = [];
    // Total request count
...
;
return violations;
checkBuildBudgets(snapshot, PerformanceSnapshot);
BudgetViolation;
{
    const violations = [];
    if (!snapshot.build)
...
;
return violations;
calculateSeverity(actual, number, budget, number);
'low' | 'medium' | 'high' | 'critical';
{
    ...
    if (ratio >= 1.2)
        return 'medium';
    return 'low';
    calculatePerformanceScore(violations, BudgetViolation);
    number;
    {
        let score = 100;
    ...
    summarizeViolations(violations, BudgetViolation);
    {
        return {
    ...
    ;
    generateRecommendations(violations, BudgetViolation);
    string;
    {
