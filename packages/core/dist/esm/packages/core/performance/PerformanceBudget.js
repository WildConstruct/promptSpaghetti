...
export class PerformanceBudgetManager extends EventEmitter {
    ...
    constructor(config) {
        ...
        */
        checkBudget(snapshot, PerformanceSnapshot);
        BudgetCheckResult;
        {
            const violations = [];
    ...
    checkBundleBudgets(snapshot) {
        ...
        if (snapshot.bundles.main > this.config.bundles.main) {
            ...
            'main-bundle-size',
                budget;
            this.config.bundles.main,
                actual;
            snapshot.bundles.main,
                threshold;
            this.config.bundles.main * 1.1, // 10% tolerance,
                severity;
            this.calculateSeverity(snapshot.bundles.main, this.config.bundles.main),
                impact;
        ...
        if (snapshot.bundles.vendor > this.config.bundles.vendor) {
            ...
            'vendor-bundle-size',
                budget;
            this.config.bundles.vendor,
                actual;
            snapshot.bundles.vendor,
                threshold;
            this.config.bundles.vendor * 1.1,
                severity;
            this.calculateSeverity(snapshot.bundles.vendor, this.config.bundles.vendor),
                impact;
        ...
        if (snapshot.bundles.total > this.config.bundles.total) {
            ...
            'total-bundle-size',
                budget;
            this.config.bundles.total,
                actual;
            snapshot.bundles.total,
                threshold;
            this.config.bundles.total * 1.1,
                severity;
            this.calculateSeverity(snapshot.bundles.total, this.config.bundles.total),
                impact;
        ...
        if (oversizedChunks.length > 0) {
            ...
            'chunk-size',
                budget;
            this.config.bundles.chunks,
                actual;
            Math.max(...oversizedChunks),
                threshold;
            this.config.bundles.chunks * 1.1,
                severity;
            this.calculateSeverity(Math.max(...oversizedChunks), this.config.bundles.chunks),
                impact;
    ...
    checkRuntimeBudgets(snapshot) {
        ...
        if (snapshot.runtime.fcp && snapshot.runtime.fcp > this.config.runtime.firstContentfulPaint) {
            ...
            'first-contentful-paint',
                budget;
            this.config.runtime.firstContentfulPaint,
                actual;
            snapshot.runtime.fcp,
                threshold;
            this.config.runtime.firstContentfulPaint * 1.1,
                severity;
            this.calculateSeverity(snapshot.runtime.fcp, this.config.runtime.firstContentfulPaint),
                impact;
        ...
        if (snapshot.runtime.lcp && snapshot.runtime.lcp > this.config.runtime.largestContentfulPaint) {
            ...
            'largest-contentful-paint',
                budget;
            this.config.runtime.largestContentfulPaint,
                actual;
            snapshot.runtime.lcp,
                threshold;
            this.config.runtime.largestContentfulPaint * 1.1,
                severity;
            this.calculateSeverity(snapshot.runtime.lcp, this.config.runtime.largestContentfulPaint),
                impact;
        ...
        if (snapshot.runtime.fid && snapshot.runtime.fid > this.config.runtime.firstInputDelay) {
            ...
            'first-input-delay',
                budget;
            this.config.runtime.firstInputDelay,
                actual;
            snapshot.runtime.fid,
                threshold;
            this.config.runtime.firstInputDelay * 1.1,
                severity;
            this.calculateSeverity(snapshot.runtime.fid, this.config.runtime.firstInputDelay),
                impact;
        ...
        if (snapshot.runtime.cls && snapshot.runtime.cls > this.config.runtime.cumulativeLayoutShift) {
            ...
            'cumulative-layout-shift',
                budget;
            this.config.runtime.cumulativeLayoutShift,
                actual;
            snapshot.runtime.cls,
                threshold;
            this.config.runtime.cumulativeLayoutShift * 1.1,
                severity;
            this.calculateSeverity(snapshot.runtime.cls * 1000, this.config.runtime.cumulativeLayoutShift * 1000),
                impact;
        ...
        if (snapshot.runtime.tti && snapshot.runtime.tti > this.config.runtime.timeToInteractive) {
            ...
            'time-to-interactive',
                budget;
            this.config.runtime.timeToInteractive,
                actual;
            snapshot.runtime.tti,
                threshold;
            this.config.runtime.timeToInteractive * 1.1,
                severity;
            this.calculateSeverity(snapshot.runtime.tti, this.config.runtime.timeToInteractive),
                impact;
    ...
    checkApiBudgets(snapshot) {
        ...
        if (snapshot.api.graphExecution && snapshot.api.graphExecution > this.config.api.graphExecution) {
            ...
            'graph-execution-time',
                budget;
            this.config.api.graphExecution,
                actual;
            snapshot.api.graphExecution,
                threshold;
            this.config.api.graphExecution * 1.2,
                severity;
            this.calculateSeverity(snapshot.api.graphExecution, this.config.api.graphExecution),
                impact;
        ...
        if (snapshot.api.preview && snapshot.api.preview > this.config.api.preview) {
            ...
            'preview-generation-time',
                budget;
            this.config.api.preview,
                actual;
            snapshot.api.preview,
                threshold;
            this.config.api.preview * 1.2,
                severity;
            this.calculateSeverity(snapshot.api.preview, this.config.api.preview),
                impact;
    ...
    checkMemoryBudgets(snapshot) {
        ...
        if (snapshot.memory.peak > this.config.memory.peakHeap) {
            ...
            'peak-heap-usage',
                budget;
            this.config.memory.peakHeap,
                actual;
            snapshot.memory.peak,
                threshold;
            this.config.memory.peakHeap * 1.1,
                severity;
            this.calculateSeverity(snapshot.memory.peak, this.config.memory.peakHeap),
                impact;
        ...
        if (recentSnapshots.length >= 5) {
            ...
            if (memoryTrend > this.config.memory.leakThreshold) {
                ...
                'memory-leak-trend',
                    budget;
                this.config.memory.leakThreshold,
                    actual;
                memoryTrend,
                    threshold;
                this.config.memory.leakThreshold,
    ...
    checkNetworkBudgets(snapshot) {
        ...
        if (snapshot.network.requestCount > this.config.network.totalRequests) {
            ...
            'total-request-count',
                budget;
            this.config.network.totalRequests,
                actual;
            snapshot.network.requestCount,
                threshold;
            this.config.network.totalRequests * 1.1,
                severity;
            this.calculateSeverity(snapshot.network.requestCount, this.config.network.totalRequests),
                impact;
        ...
        if (snapshot.network.transferSize > this.config.network.totalTransferSize) {
            ...
            'total-transfer-size',
                budget;
            this.config.network.totalTransferSize,
                actual;
            snapshot.network.transferSize,
                threshold;
            this.config.network.totalTransferSize * 1.1,
                severity;
            this.calculateSeverity(snapshot.network.transferSize, this.config.network.totalTransferSize),
                impact;
    ...
    checkBuildBudgets(snapshot) {
        ...
        if (snapshot.build.buildTime > this.config.build.buildTime) {
            ...
            'build-time',
                budget;
            this.config.build.buildTime,
                actual;
            snapshot.build.buildTime,
                threshold;
            this.config.build.buildTime * 1.2,
                severity;
            this.calculateSeverity(snapshot.build.buildTime, this.config.build.buildTime),
                impact;
    ...
    calculatePerformanceScore(violations) {
        let score = 100;
...
calculateMemoryTrend(snapshots, PerformanceSnapshot);
number;
{
    ...
    */
    getPerformanceTrends();
    {
        ...
        return {
            ...
            violations: recent.map((_, i) => , this.violations.filter(v => ), v.timestamp >= (recent[i]?.timestamp || 0) &&
                v.timestamp < (recent[i + 1]?.timestamp || Infinity)).length
        };
        /**
         * Update budget configuration
         */
        updateBudget(newConfig, (Partial));
        void {
            ...
            getBudgetConfig() {
                ...
                void {
                    ...
                    const: defaultPerformanceBudget, PerformanceBudgetConfig = {
                        bundles: {
