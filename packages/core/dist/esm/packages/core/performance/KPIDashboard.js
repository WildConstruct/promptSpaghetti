...
import { corePerformanceKPIs, getKPIsByCategory } from getCriticalKPIs;
from;
'./PerformanceKPIs';
;
categories: Record;
...
export class KPIDashboard extends EventEmitter {
    ...
    config = {
        ...
        setupEventHandlers() {
            ...
            this.monitoringService.on('kpi-alert-created', (alert) => {
...
if (this.reports.length > 50) {
    ...
    if (metrics.overview.averageScore >= 90) {
    ...
    else if (metrics.overview.averageScore >= 70) {
        keyFindings.push('✅ Good performance with room for optimization');
    }
    else {
        ...
        if (metrics.overview.trendsDegrading > metrics.overview.trendsImproving) {
                ...
                if (metrics.categories.runtime?.critical > 0) {
                    ...
                    if (metrics.categories.api?.warning > 0 || metrics.categories.api?.critical > 0) {
                        ...
                        if (metrics.categories.memory?.warning > 0) {
                            ...
                            ;
                            generateChartData(startTime, number, endTime, number);
                            DashboardReport['charts'];
                            {
                                ...
                                const performanceScore = periodBaselines.map(baseline => ({}), timestamp, baseline.timestamp, score, this.calculateBaselineScore(baseline));
                            }
                        }
                        ;
                        ...
                        map(k => ({}), kpiId, k.kpiId, name, corePerformanceKPIs.find(kpi => kpi.id === k.kpiId)?.name || k.kpiId, score, this.statusToScore(k.status), trend, k.trend);
                    }
                }
            ...
            ;
            calculateBaselineScore(baseline, BaselineSnapshot);
            number;
            {
                ...
                number;
                {
                    switch (status) {
                        ...
                        default:
                            return 70;
                            generateAlertTimelineData(startTime, number, endTime, number);
                            Array < { timestamp: number, count: number, severity: string } > {
                    ...
                    ;
                    return points;
                    calculateAverageScore(kpiStatus, (Array));
                    number;
                    {
                        if (kpiStatus.length === 0)
                            return 0;
                        ...
                        void {
                            ...
                            */
                            refreshAlertWidgets() {
                                ...
                                * Get widget data
                                */
                                getWidgetData(layoutId, string, widgetId, string);
                                any;
                                {
                        ...
                        generateMetricData(widget, DashboardWidget);
                        any;
                        {
                            const metrics = this.getDashboardMetrics();
                            switch (widget.id) {
                                ...
                                default:
                                    return { value: 0 };
                                    generateChartData(widget, DashboardWidget);
                                    any;
                                    ...
                                    ;
                                    generateTableData(widget, DashboardWidget);
                                    any;
                                    {
                                        switch (widget.id) {
                                            case 'all-kpis-table':
                        ...
                        ;
                        return { headers: [], rows: [] };
                        generateAlertData(widget, DashboardWidget);
                        any;
                        {
                            const activeAlerts = this.monitoringService.getActiveAlerts();
                            return {
                            ...
                            generateTrendData(widget, DashboardWidget);
                            any;
                            {
                                const kpiStatus = this.monitoringService.getCurrentKPIStatus();
                             ...
                             */
                            startAutoRefresh(layoutId, string);
                            void { const: layout = this.layouts.get(layoutId),
                                if(, layout) { } } || !layout.autoRefresh;
                            return;
                             ...
                             */
                            stopAutoRefresh(layoutId, string);
                            void { const: interval = this.refreshIntervals.get(layoutId),
                                if(interval) {
                                ...
                                importDashboardConfig(configJson) {
                                    try {
                                        ...
                                        if (data.layouts) {
                                            this.layouts.clear();
                                            data.layouts.forEach(([id, layout]) => { }, this.layouts.set(id, layout));
                                        }
                                        ;
                                        if (data.config) {
                                             ...
                                             */
                                            getDashboardReports(limit ?  : number);
                                            DashboardReport;
                                            {
                                                 ...
                                                 */
                                                getDashboardReport(reportId, string);
                                                DashboardReport | null;
                                                {
                                                     ...
                                                     * Clear old reports
                                                     */
                                                    clearOldReports(retentionDays, number = 30);
                                                    number;
                                                    {
