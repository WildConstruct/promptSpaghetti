...
import { GraphOperation, OperationType, OperationResult, BatchOperationResult, UndoResult, RedoResult, GraphState, GraphSnapshot, HistoryEntry, AnyGraphOperation, NodeAddOperation } from MutationEngineEvents;
from;
'./types';
...
try {
    ...
    throw error;
    async;
    executeNodeAdd(operation, NodeAddOperation);
    Promise < void  > {
    ...
    }(operation, GraphOperation, errors, string), OperationResult, {
        return: {
            success: false,
            operation,
            error: errors.join(', '),
