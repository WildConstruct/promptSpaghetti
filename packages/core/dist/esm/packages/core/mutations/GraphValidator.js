...
import { GraphOperation, OperationType, ValidationFunction } from './types';
                ...
                if (!this.config.allowDangerousOperations) {
                    if (operation.type === OperationType.GRAPH_CLEAR) {
                        ...
                        type: 'DANGEROUS_OPERATION',
                            message;
                        'Graph clear operation is not allowed',
                            severity;
                        'error',
                        ;
                    }
...
;
async;
validateNodeAdd((), operation, NodeAddOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
if (!this.isValidPosition(position)) {
    ...
    type: 'INVALID_POSITION',
        message;
    'Node position is invalid or outside allowed bounds',
        severity;
    'warning',
        nodeId;
    node.id,
...
return errors;
async;
validateNodeDelete((), operation, NodeDeleteOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
if (connectedEdges.length > 0) {
    ...
    type: 'NODE_HAS_CONNECTIONS',
        message;
    `Node has ${connectedEdges.length} connected edges that will be removed`;
}
severity: 'warning',
    nodeId;
;
return errors;
async;
validateNodeUpdate((), operation, NodeUpdateOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
return errors;
async;
validateEdgeAdd((), operation, EdgeAddOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
return errors;
async;
validateEdgeDelete((), operation, EdgeDeleteOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
    ...
    if(, edgeExists) {
        ...
        type: 'EDGE_NOT_FOUND',
            message;
        `Edge with ID ${edgeId} does not exist`;
    }
},
    severity;
'error',
...
return errors;
async;
validateVariationAdd((), operation, VariationAddOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
if (variation.length > 1000) {
    ...
    type: 'VARIATION_TOO_LONG',
        message;
    'Variation exceeds maximum length of 1000 characters',
        severity;
    'error',
        nodeId;
}
...
return errors;
async;
validateVariationDelete((), operation, VariationDeleteOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
if (index < 0 || index >= variations.length) {
    ...
    type: 'VARIATION_INDEX_OUT_OF_BOUNDS',
        message;
    `Variation index ${index} is out of bounds`;
}
severity: 'error',
    nodeId;
;
return errors;
async;
validateVariationUpdate((), operation, VariationUpdateOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
return errors;
async;
validateVariationReorder((), operation, VariationReorderOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
...
if (fromIndex < 0 || fromIndex >= variations.length) {
    ...
    type: 'FROM_INDEX_OUT_OF_BOUNDS',
        message;
    `From index ${fromIndex} is out of bounds`;
}
severity: 'error',
    nodeId;
;
if (toIndex < 0 || toIndex >= variations.length) {
    ...
    type: 'TO_INDEX_OUT_OF_BOUNDS',
        message;
    `To index ${toIndex} is out of bounds`;
}
severity: 'error',
    nodeId;
;
return errors;
async;
validateOperationSchema((), operation, GraphOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
    ...
    if (nodeType && nodeSchemas[nodeType]) {
        try {
            ...
            type: 'SCHEMA_VALIDATION_FAILED',
                message;
            `Node schema validation failed: ${schemaError.message}`;
        }
    }
    severity: 'error',
        nodeId;
    nodeOp.payload.node.id;
}
;
return errors;
async;
validateStructuralConstraints((), operation, GraphOperation, state, GraphState);
Promise < ValidationError > {
    const: errors, ValidationError = [],
    // Simulate the operation and validate resulting structure
    const: simulatedState = this.simulateOperation(operation, state),
    : .hasCycles(simulatedState)
};
{
    errors.push({});
    type: 'CYCLE_DETECTED',
        message;
    'Operation would create a cycle in the graph',
        severity;
    'error',
    ;
}
