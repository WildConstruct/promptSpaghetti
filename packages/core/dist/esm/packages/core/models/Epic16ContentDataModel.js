...
examples: z.array(z.object({}), title, z.string().max(100), input, z.string().max(1000), expectedOutput, z.string().max(2000));
max(5).default([]);
...
// Assets
assets: z.array(z.object({}), name, z.string().max(255), url, z.string().url(), size, z.number().int().min(0), type, z.string().max(50));
max(10).default([]);
// Timestamps
createdAt: z.date();
publishedAt: z.date().optional();
...
attachments: z.array(z.object({}), id, z.string().uuid(), name, z.string().max(255), url, z.string().url(), size, z.number().int().min(0), mimeType, z.string().max(100));
max(5).default([]);
// Thread information
...
// Content structure
tableOfContents: z.array(z.object({}), id, z.string(), title, z.string().max(200), level, z.number().int().min(1).max(6), anchor, z.string().max(100));
optional();
// SEO and metadata
slug: z.string().min(1).max(200);
metaDescription: z.string().max(160).optional();
...
shortDescription: z.string().max(300).optional();
// Content structure
steps: z.array(z.object({}), id, z.string().uuid(), title, z.string().max(200), content, z.string().max(10000), order, z.number().int().min(0), estimatedDuration, z.number().int().min(0).optional(), // minutes
...
assets: z.array(z.object({}), name, z.string().max(255), url, z.string().url(), description, z.string().max(500).optional());
([]);
...
clickedResults: z.array(z.object({}), contentId, z.string().uuid(), contentType, z.nativeEnum(ContentType), position, z.number().int().min(0), clickedAt, z.date());
([]);
// Search context
...
// Content
items: z.array(z.object({}), contentId, z.string().uuid(), contentType, z.nativeEnum(ContentType), addedAt, z.date(), order, z.number().int().min(0), note, z.string().max(500).optional());
([]);
// Categorization
category: z.string().max(50).optional();
tags: z.array(z.string().min(1).max(30)).max(10).default([]);
// Collaboration
collaborators: z.array(z.object({}), userId, z.string().uuid(), role, z.enum(['viewer', 'editor', 'admin']), addedAt, z.date());
([]);
// Engagement
...
;
// Validation schemas
export const Epic16ContentSchemas = { UserProfile: UserProfileSchema,
