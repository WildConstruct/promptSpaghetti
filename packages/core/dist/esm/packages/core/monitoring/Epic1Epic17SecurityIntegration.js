...
'email' | 'slack' | 'webhook' | 'admin_dashboard' | 'security_dashboard';
target: string;
    ...
    : .config.epic1Integration.enabled };
{
    ...
    if (this.config.epic17Integration.enabled) {
            ...
            */
            ...
            adminOverview: {
                activeAdminSessions: epic17Dashboard.adminSystemOverview.activeAdminSessions,
                    recentOperations;
                this.getRecentAdminOperations(),
                    integrationStatus;
                epic17Dashboard.integrationHealth.map(health => ({}), integration, health.integration, status, health.status, responseTime, health.responseTime);
            }
        ...
        securityOverview: {
            ...
            securityDashboard.criticalAlerts,
                securitySystemsHealth;
            securityDashboard.systemPerformance.map(perf => ({}), systemId, perf.systemId, healthScore, perf.healthScore, status, perf.healthScore > 80 ? 'healthy' : perf.healthScore > 60 ? 'degraded' : 'critical');
        }
