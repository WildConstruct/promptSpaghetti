 ...
 */
startExecution(nodeId, string, nodeType, string, context, AdvancedExecutionContext);
string;
{
...
PerformanceMetrics;
{
    ...
    */
    getAggregatedMetrics(nodeType, string);
    AggregatedMetrics | null;
    {
        ...
        */
            ...
            boolean;
            {
                ...
                if (alert) {
                    ...
                    */
                    ...
                    const typeAverages = Array.from(typePerformance.entries()).map(([type, total]) => ({}), type, avgDuration, total / (typeCounts.get(type) || 1));
                }
            }
            ;
            ...
            number;
            {
                if (typeof process !== 'undefined' && process.memoryUsage) {
                    ...
                    if (typeof performance !== 'undefined' && performance.memory) {
                        ...
                        return 0;
                        analyzeContextSize(context, AdvancedExecutionContext);
                        PerformanceMetrics['contextSize'];
                        {
                        ...
                        ;
                        determineCacheHit(context, AdvancedExecutionContext, result, any);
                        boolean;
                        {
                            ...
                            ;
                            storeMetrics(metrics, PerformanceMetrics);
                            void {
                            ...
                            details: {
                                duration: metrics.duration, threshold;
                                this.config.slowExecutionThreshold;
                            }
                            resolved: false;
                        }
                        ;
                    ...
                    details: {
                        memoryDelta: metrics.memoryUsage.delta, threshold;
        ...
        void { const: typeMetrics = new Map(),
            : .metrics.values() };
        {
        ...
        ;
        calculateAggregation(nodeType, string, metrics, PerformanceMetrics);
        AggregatedMetrics;
        {
                        ...
                        if (averageDuration > this.config.slowExecutionThreshold) {
                            ...
                            if (cacheHitRate < 50 && averageContextSize > 100) {
                                ...
                                if (averageMemoryDelta > this.config.memoryThreshold * 0.5) {
                                    ...
                                    ;
                                    cleanup();
                                    void { const: cutoffTime = Date.now() - this.config.retentionPeriod,
                                        : .metrics };
                                    {
