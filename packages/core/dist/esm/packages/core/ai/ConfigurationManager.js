...
export class ConfigurationValidator {
    ...
    constructor() {
        ...
        void {
            ...
            removeRule(ruleId) {
                ...
                validate(config, ModelConfiguration);
                ValidationResult;
                {
                    const result = {
                        valid: true,
                    ...
                    for (const rule of this.rules) {
                        ...
                        if (!ruleResult.valid) {
                            result.valid = false;
                            return result;
                            validateEnvironment(envConfig, EnvironmentConfig);
                            ValidationResult;
                            {
                                const result = {
                                    valid: true,
            ...
            _initializeDefaultRules() {
                ...
                (config) => {
                    ...
                    'Model Name Validation',
                        description;
                    'Validates model names for specific providers',
                        validate;
                    (config) => {
                        const warnings = [];
                        const suggestions = [];
                    ...
                    'Resource Limits',
                        description;
                    'Validates resource limit settings',
                        validate;
                    (config) => {
                        const warnings = [];
                        const suggestions = [];
                        if (config.capabilities?.maxInputSize && config.capabilities.maxInputSize > 1000000) {
                            ...
                            export class ConfigurationManager {
                                ...
                                configPath;
                                constructor(initialSchema, configPath) {
                                    ...
                                    string;
                                    {
                                        ...
                                        void {
                                            : .schema.environments[environment]
                                        };
                                        {
                ...
                getModelConfig(modelId, string, environment ?  : string);
                ModelConfiguration | null;
                {
                    const envConfig = this.getEnvironmentConfig(environment);
                    return envConfig.models.find(m => m.id === modelId) || null;
                    addModelConfig(config, ModelConfiguration, environment ?  : string);
                    void {
    ...
    ;
    updateModelConfig(modelId, updates, environment) {
        const env = environment || this.currentEnvironment;
        const envConfig = this.getEnvironmentConfig(env);
...
;
removeModelConfig(modelId, string, environment ?  : string);
void {
    const: env = environment || this.currentEnvironment,
    const: envConfig = this.getEnvironmentConfig(env),
    const: modelIndex = envConfig.models.findIndex(m => m.id === modelId),
...
(Partial) | null;
{
    ...
    createTemplate(templateId, string, template, (Partial));
    void {
...
;
applyTemplate(modelId, string, templateId, string, overrides, (Partial) = {});
ModelConfiguration;
{
    ...
    string;
    {
        if (environment) {
            ...
            importConfiguration(configJson, string, environment ?  : string);
            void {
                try: {
            ...
            ConfigurationHistory;
            {
                ...
                void {
                    ...
                    if(, snapshot) {
                        ...
                        new Date(),
                            reason;
                        `Rolled back to snapshot from ${snapshot.timestamp.toISOString()}`;
                    }
                };
                ;
                ...
                void {
                    ...
                    generateRegistrations(environment) {
                        const envConfig = this.getEnvironmentConfig(environment);
                        return envConfig.models.map(config => ({}), id, config.id, provider, config.provider, modelName, config.modelName || 'default', config, {
                            apiKey: config.apiKey,
                            endpoint: config.endpoint,
                            ...config.parameters
                        }, metadata, config.metadata, capabilities, config.capabilities);
                    },
                    // Private methods
                    ...
                    _createSnapshot(description) {
                        ...
                        timestamp: new Date(),
                            config;
                        JSON.parse(JSON.stringify(this.schema)), // Deep copy,
                            version;
                        this.schema.version,
                        ;
                    },
