            ...
            if (!this.config.endpoint) {
                ...
                if (!this.config.modelName) {
                    ...
                    try { }
                        ...
                        async;
                        process(input, unknown, options ?  : LocalRequestOptions);
                        Promise < unknown > {
                            try: {
    ...
    try { }
        ...
        string;
        {
            switch (this.config.modelType) {
                ...
                default:
                    return '/generate';
                    _buildPayload(messages, OllamaMessage, options ?  : LocalRequestOptions);
                    any;
                    {
                        const basePayload = {
                            ...
                            : .config.modelType
                        }, { case: , 'ollama': , return: basePayload };
                    }
                case 'huggingface':
                    return {
                    ...
                    try { }
                        ...
                        if (attempt < maxRetries) {
                            ...
                            _convertToMessages(input, any, systemPrompt ?  : string);
                            OllamaMessage;
                            {
                                const messages = [];
                                ...
                                if (systemPrompt) {
                                    ...
                                    if (typeof input === 'string') {
                                        ...
                                        return messages;
                                        _extractContent(response, LocalModelResponse);
                                        any;
                                        {
                                            let content = '';
                                            if (response.message?.content) {
                                                ...
                                                _estimateTokenCount(messages, OllamaMessage);
                                                number;
                                                {
                                                    const totalText = messages.map(msg => msg.content).join(' ');
                                                    return Math.ceil(totalText.length / 4);
                                                    _updateCapabilitiesFromModelInfo(modelInfo, any);
                                                    void {
