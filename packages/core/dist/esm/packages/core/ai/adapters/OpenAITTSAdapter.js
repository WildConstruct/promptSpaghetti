                ...
                async initialize() {
                    try {
                        ...
                        try { }
                            ...
                            async;
                            process(input, any, options ?  : TTSRequestOptions);
...
, 'voice' | 'model' | 'response_format' | 'speed' | 'text' >>  & (Omit) & { text: string };
{
    const defaults = {
        voice: 'alloy',
        model: 'tts-1',
    ...
    if (!validVoices.includes(processed.voice)) {
        ...
        if (!validModels.includes(processed.model)) {
            ...
            return processed;
            async;
            _generateSpeech(text, string, options, TTSRequestOptions);
            Promise < ArrayBuffer > {
    ...
    if (this.config.organization) {
        ...
        try { }
            ...
            if (attempt < maxRetries) {
                ...
                async;
                _analyzeAudioData(audioData, ArrayBuffer, format, string);
                Promise < any > {
