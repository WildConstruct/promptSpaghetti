 ...
 */
import { ExecutionPath, NodeExecutionStep, RandomChoiceInfo, EXECUTION_PATH_COLORS } from ExecutionPathColor;
from;
'../types/ExecutionPath.js';
export class GraphExecutionTracker {
    ...
    static getInstance() {
         ...
         */
        recordNodeExecution(executionId, string, step, NodeExecutionStep);
        void {
    ...
    if(, execution, nodeExecutionOrder, includes) { }
}
(step.nodeId);
{
     ...
     */
    recordRandomChoice(executionId, string, choice, RandomChoiceInfo);
    void {
        const: execution = this.activeExecutions.get(executionId),
        if(, execution) {
        ...
        if(currentStep) {
             ...
             */
            finishTracking(executionId, string, output, string);
            ExecutionPath;
            {
                const execution = this.activeExecutions.get(executionId);
                if (!execution) {
             ...
             */
            getExecutionPath(executionId, string);
            ExecutionPath | null;
            {
                const execution = this.activeExecutions.get(executionId);
                if (!execution)
             ...
             * Clear tracking data
             */
            clearTracker(executionId, string);
            void { this: .activeExecutions.delete(executionId),
                /**
        ...
        totalVariance, probability, *() { }
    } - probability;
}
;
return totalVariance / nodeUsageCounts.size;
findCommonPatterns(paths, ExecutionPath);
{
    commonNodes: string;
...
return { commonNodes, divergencePoints, sharedSequences };
assignPathColors(paths, ExecutionPath);
Map < string, ExecutionPathColor > { const: colorMap = new Map(),
...
const randomizationSummary = randomChoices.length === 0;
