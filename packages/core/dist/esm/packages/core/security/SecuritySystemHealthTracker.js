...
'ping' | 'http_status' | 'database_query' | 'custom_script' | 'port_check' | 'ssl_cert' | 'disk_space' | 'memory_usage' | 'cpu_usage';
name: string;
    ...
    initializeEventHandlers() {
        ...
        };
        {
            ...
            if (updatedSystem.health_checks.enabled) {
                ...
                systemId: string,
                    check;
                SecuritySystemNode['health_checks']['checks'][0],
                ;
...
SecuritySystemNode['current_state']['status'];
{
    ...
    return 'warning';
    async;
    updateAvailabilityMetrics((), systemId, string, status, SecuritySystemNode['current_state']['status']);
    Promise < void  > {
        const: system = this.systems.get(systemId),
        if(, system) { }, return: ,
        const: now = Date.now(),
