...
new Error(`Unknown health check type: ${healthCheck.type}`);
...
try { }
    ...
    async;
    performHttpHealthCheck((), component, InfrastructureComponent, healthCheck, HealthCheckConfig);
    ...
    if (!success) {
        ...
        return { success: true };
        async;
        performTcpHealthCheck((), component, InfrastructureComponent, healthCheck, HealthCheckConfig);
        Promise < boolean > {
        ...
        return { success: true };
        async;
        performMemoryHealthCheck((), component, InfrastructureComponent, healthCheck, HealthCheckConfig);
        Promise < { success: boolean, error: string } > {
        ...
        return { success: true };
        async;
        performCpuHealthCheck((), component, InfrastructureComponent, healthCheck, HealthCheckConfig);
        Promise < { success: boolean, error: string } > {
        ...
        return { success: true };
        async;
        performProcessHealthCheck((), component, InfrastructureComponent, healthCheck, HealthCheckConfig);
        Promise < boolean > {
        ...
        {
            status: InfrastructureComponent['status']['health_status'];
        ...
        }
        {
            if (results.length === 0) {
                ...
                async;
                collectComponentMetrics(componentId, string);
                Promise < void  > {
                    const: component = this.components.get(componentId),
                    if(, component) { }, return: ,
                    try: {
                    ...
                    if (component.type === 'database') {
                        ...
                        for (const customMetric of component.monitoring.performance_monitoring.custom_metrics) {
                            if (customMetric.enabled) {
                                ...
                                async;
                                collectSystemMetrics(component, InfrastructureComponent);
                                Promise < InfrastructureMetrics['system'] > {
                ...
                if (failedChecks.length > 0) {
                    ...
                    async;
                    generateHealthCheckAlert((), componentId, string, result, { check: HealthCheckConfig, success: boolean, responseTime: number, error: string });
                    Promise < void  > {
                                        ...
                                        if (rule.conditions.component_patterns.length > 0) {
                                            ...
                                            if (rule.conditions.alert_patterns.length > 0) {
                                                ...
                                                if (!rule.conditions.severity_levels.includes(alert.severity)) {
                                                    ...
                                                    return true;
                                                    async;
                                                    updateAvailabilityMetrics((), componentId, string, status, InfrastructureComponent['status']['health_status']);
                                                    Promise < void  > {
                                                        const: component = this.components.get(componentId),
                                                        if(, component) { }, return: ,
                                                        ...
                                                        async calculateAvailability(componentId, startTime, endTime) {
