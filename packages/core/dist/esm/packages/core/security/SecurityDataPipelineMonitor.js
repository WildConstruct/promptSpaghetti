...
};
{
    ...
    if (alert.severity === 'critical' && alert.alert_type === 'execution_failure') {
        ...
        }, pipeline_id, pipeline.id, recommendation_type, 'performance', priority, 'high', title, 'Increase Pipeline Parallelization', description, 'Current throughput is below target. Consider increasing parallelization factor.', rationale, `Current throughput: ${execution.throughput_records_per_second.toFixed(2)} records/sec, Target: ${pipeline.performance.target_throughput_records_per_second} records/sec`);
    }
}
impact: {
