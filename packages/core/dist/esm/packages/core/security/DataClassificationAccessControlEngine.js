...
Promise < AccessDecision > {
    ...
    try: {
        ...
        if(cachedDecision) { }
    } && this.isCacheValid(cachedDecision) };
{
    return {
        ...cachedDecision,
        metadata: {
    ...
    try { }
                                ...
                                if (this.permissionMatches(permission, request)) {
                                    ...
                                    if (permission.conditions) {
                                        ...
                                        try { }
                                             ...
                                             */
                                            async;
                                            evaluateABAC(request, AccessRequest);
                                            Promise < ABACDecision > {
                                                                ...
                                                                if (policyResult.conditions) {
                                                                    ...
                                                                    if (classificationPolicy) {
                                                                        ...
                                                                        try { }
                                                                             ...
                                                                             */
                                                                            async;
                                                                            combinedDecision(rbac, RBACDecision, abac, ABACDecision);
                                                                            Promise < AccessDecision > {
                                                ...
                                                }(role, DataClassificationRole, classification, DataClassificationLevel), boolean
                                            };
                                            {
        ...
        }(role, DataClassificationRole, request, AccessRequest);
        Promise < boolean > {
            for(, constraint, of, role) { }, : .constraints };
        {
            ...
            if (!satisfied) {
                ...
                return true;
                async;
                evaluateConstraint(constraint, any, request, AccessRequest);
                Promise < boolean > {
                    switch(constraint) { }, : .type };
                {
                    ...
                    return true;
                    evaluateTimeConstraint(constraint, any, timestamp, Date);
                    boolean;
                    {
                        ...
                        if (constraint.operator === 'BETWEEN' && Array.isArray(constraint.value)) {
                            ...
                            return true;
                            evaluateLocationConstraint(constraint, any, location, any);
                            boolean;
                            {
                                if (constraint.operator === 'IN' && Array.isArray(constraint.value)) {
                                    return constraint.value.includes(location.country);
                                    return true;
                                    evaluatePurposeConstraint(constraint, any, purpose, string);
                                    boolean;
                                    {
                                        if (constraint.operator === 'EQUALS') {
                                            ...
                                            return true;
                                            async;
                                            getRolePermissions(role, DataClassificationRole);
                                            Promise < DataClassificationPermission > {
                                                const: permissions, DataClassificationPermission = [],
                                                for(, permissionId, of, role) { }, : .permissions };
                                            {
                                                ...
                                                if (permission) {
                                                    ...
                                                    }, private, checkAccessControlMatrix(classification, DataClassificationLevel), operation, DataOperation, userRoles, DataClassificationRole);
                                                    boolean;
                                                    {
                                                        ...
                                                        if (!matrixEntry) {
                                                            ...
                                                            matrixEntry.includes(role.name);
                                                            ;
                                                            async;
                                                            getApplicablePolicies(request, AccessRequest);
                                                            Promise < ABACPolicy > {
                                                                ...
                                                                : .abacModel.policies };
                                                            {
                                                                if (policy.enabled && await this.isPolicyApplicable(policy, request)) {
                                                                    ...
                                                                    async;
                                                                    isPolicyApplicable(policy, ABACPolicy, request, AccessRequest);
                                                                    Promise < boolean > {
                                                                        return: ,
