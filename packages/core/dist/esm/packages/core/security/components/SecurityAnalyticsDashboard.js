...
if (allowedInsights && allowedInsights.length > 0) {
    ...
    ;
    setState(prev => ({}), ...prev, summary, insights, patterns, isLoading, false, lastUpdate, new Date());
}
;
try { }
catch (error) {
...
if (state.isLoading && !state.summary) {
    ...
    _jsx("div", { style: {
            ...
            fontFamily: 'Inter, system-ui, sans-serif',
                    ...
                    } }), ", borderTop: `4px solid $", themeStyles.primary, "`} }, borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 16px'; }} />", _jsx("div", { children: "Loading Security Analytics..." })] }) });
    ;
    return;
    _jsxs("div", { style: {
            background: themeStyles.background,
            ...
            minHeight: '100vh',
                    ...
                    borderBottom: `1px solid ${themeStyles.border}`
                            ...
                            fontSize: '16px',
                        }, children: [executiveMode ? 'Executive Security Overview' : 'Operational Security Monitoring', " \u2022 Wild Construct Platform", state.lastUpdate && ()
                                < span, " style=", { marginLeft: '16px', fontSize: '14px' }, "> Last updated: ", state.lastUpdate.toLocaleTimeString()] }), ")}"] })] });
    { /* Controls */ }
        ...
        _jsx("option", { value: "all", children: "All Categories" });
{
    Object.values(ThreatCategory).map(category => ()
        ...
        { category, : .replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()) }, option >
    );
}
select >
    { /* Refresh Button */}
...
style = {};
{
    ...
    600,
        cursor;
    state.isLoading ? 'not-allowed' : 'pointer',
        opacity;
    state.isLoading ? 0.6 : 1,
    ;
}
    >
        { state, : .isLoading ? '🔄' : '↻' };
...
'20px',
    borderLeft;
`4px solid ${getRiskColor(state.summary.overallRisk.level)}`;
 >
    _jsxs("div", { style: {
            display: 'flex',
            justifyContent: 'space-between',
            ...
            marginBottom: '12px',
                ...
                } }), ", color: getRiskColor(state.summary.overallRisk.level), borderRadius: '4px', fontWeight: 600; }}>", state.summary.overallRisk.trend.toUpperCase()] });
div >
    (_jsx("div", { style: {
            ...
            marginBottom: '8px',
        }, children: state.summary.overallRisk.level.toUpperCase() })
        ,
            _jsxs("div", { style: {
                    fontSize: '14px',
                    color: themeStyles.textSecondary,
                }, children: ["Score: ", state.summary.overallRisk.score, "/100"] }));
div >
    { /* Security Events */}
    < div;
style = {};
...
'20px';
 >
    (_jsx("h3", { style: { margin: '0 0 12px 0', fontSize: '14px', color: themeStyles.textSecondary, fontWeight: 600 }, children: "SECURITY EVENTS" })
        ,
            _jsx("div", { style: {
                    fontSize: executiveMode ? '36px' : '32px',
                    fontWeight: 800,
                    color: themeStyles.text,
                    marginBottom: '8px',
                }, children: formatNumber(state.summary.eventVolume.total) })
                ,
                    _jsxs("div", { style: {
                            fontSize: '14px',
                            color: themeStyles.textSecondary,
                            display: 'flex',
                            gap: '12px',
                        }, children: [_jsxs("span", { children: ["Critical: ", state.summary.eventVolume.bySeverity.critical || 0] }), _jsxs("span", { children: ["High: ", state.summary.eventVolume.bySeverity.high || 0] })] }));
div >
    { /* Active Threats */}
    < div;
style = {};
...
'20px';
 >
    (_jsx("h3", { style: { margin: '0 0 12px 0', fontSize: '14px', color: themeStyles.textSecondary, fontWeight: 600 }, children: "ACTIVE THREATS" })
        ,
            _jsx("div", { style: {
                    fontSize: executiveMode ? '36px' : '32px',
                    fontWeight: 800,
                    ...
                    marginBottom: '8px',
                }, children: state.summary.threatLandscape.activeThreats })
                ,
                    _jsxs("div", { style: {
                            fontSize: '14px',
                            color: themeStyles.textSecondary,
                        }, children: ["New: ", state.summary.threatLandscape.newPatterns] }));
div >
    { /* Security Posture */}
    < div;
style = {};
...
'20px';
 >
    (_jsx("h3", { style: { margin: '0 0 12px 0', fontSize: '14px', color: themeStyles.textSecondary, fontWeight: 600 }, children: "SECURITY POSTURE" })
        ,
            _jsxs("div", { style: {
                    fontSize: executiveMode ? '36px' : '32px',
                    fontWeight: 800,
                    ...
                    marginBottom: '8px',
                }, children: [state.summary.systemHealth.securityPosture, "%"] })
                ,
                    _jsxs("div", { style: {
                            fontSize: '14px',
                            color: themeStyles.textSecondary,
                        }, children: ["Compliance: ", state.summary.systemHealth.complianceScore, "%"] }));
div >
;
div >
;
...
_jsxs("div", { style: {
        ...
        gap: '24px',
                ...
                gap: '8px',
                        ...
                        borderRadius: '4px',
                        fontWeight: 600,
                    }, children: state.patterns.length })] }), state.patterns.length === 0 ? ()
            < div : , " style=", {
            textAlign: 'center',
            padding: '40px 20px',
            color: themeStyles.textMuted,
        }, ">", _jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\u2705" }), _jsx("div", { children: "No active threat patterns detected" })] });
()
    < div;
style = {};
{
    maxHeight: '400px', overflowY;
    'auto';
}
 >
                        ...
                        alignItems: 'center',
                          ...
                          pattern.riskScore >= 60 ? themeStyles.danger :
                            pattern.riskScore >= 40 ? themeStyles.warning : themeStyles.success}20`,
                                ...
                                fontSize: '12px',
                                color: themeStyles.textMuted,
                            }, children: [pattern.occurrences, "x"] })] })] })
            ,
                _jsx("p", { style: {
                        margin: '0 0 12px 0',
                        fontSize: '14px',
                    ...
                    ,
                        _jsxs("div", { style: {
                                ...
                                gap: '8px',
                                flexWrap: 'wrap',
                            }, children: [pattern.indicators.slice(0, 3).map(indicator => ()
 ...
 >
    _jsxs("h3", { style: {
            ...
            gap: '8px',
                    ...
                    borderRadius: '4px',
                    fontWeight: 600,
                }, children: state.insights.length })] });
{
 ...
 >
    (_jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\uD83D\uDD0D" })
        ,
            _jsx("div", { children: "Analyzing security patterns..." }));
div >
;
()
    < div;
...
'12px',
    borderLeft;
`4px solid ${getRiskColor(insight.severity)}`;
    >
        _jsxs("div", { style: {
                display: 'flex',
                justifyContent: 'space-between',
                ...
                marginBottom: '8px',
                    ...
                    } }), ", color: getRiskColor(insight.severity), padding: '2px 6px', borderRadius: '4px', fontWeight: 600; }}>", insight.severity.toUpperCase()] });
div >
    _jsx("p", { style: {
            margin: '0 0 12px 0',
            fontSize: '13px',
    ...
    (_jsx("div", { style: {
        ...
        ,
            _jsx("ul", { style: {
                    ...
                    fontSize: '12px',
                    color: themeStyles.textSecondary,
                }, children: insight.recommendations.immediate.slice(0, 2).map((rec, index) => ()
                    < li, key = { index } > { rec }) }));
ul >
;
div >
    ...
    (_jsx("h3", { style: {
        ...
        ,
            _jsxs("div", { style: {
                    ...
                    color: themeStyles.textSecondary,
                    marginBottom: '20px',
                }, children: ["Current security posture shows ", _jsx("strong", { style: { color: getRiskColor(state.summary.overallRisk.level) }, children: state.summary.overallRisk.level.toUpperCase() }), " risk level with ", _jsx("strong", { children: formatNumber(state.summary.eventVolume.total) }), " security events analyzed.", _jsxs("strong", { children: [" ", state.summary.threatLandscape.activeThreats] }), " active threat patterns identified.", _jsxs("strong", { children: [" ", state.summary.userBehavior.highRiskUsers.length] }), " users require elevated monitoring."] }));
{ /* Key Recommendations */ }
{
    ...
    style = {};
    {
        background: themeStyles.tertiary,
            borderLeft;
        `4px solid ${themeStyles.critical}`;
    }
}
padding: '16px',
    borderRadius;
'8px';
 >
    (_jsx("h4", { style: { margin: '0 0 12px 0', color: themeStyles.critical, fontSize: '16px' }, children: "\uD83D\uDEA8 Immediate Action Required" })
        ,
            _jsxs("ul", { style: { margin: 0, paddingLeft: '20px', color: themeStyles.textSecondary }, children: [state.insights.filter(i => i.severity === RiskLevel.CRITICAL).length > 0 && ()
                        < li > Address, " ", state.insights.filter(i => i.severity === RiskLevel.CRITICAL).length, " critical security insights"] }));
{
    state.summary.systemHealth.securityPosture < 70 && ()
        < li > Improve;
    security;
    posture(currently, { state, : .summary.systemHealth.securityPosture } % );
    li >
    ;
}
{
