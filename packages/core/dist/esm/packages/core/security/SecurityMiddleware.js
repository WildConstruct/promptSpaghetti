...
return (req, res, next) => {
     ...
     */
    function applySecurityHeaders(res, config, req) {
        // HTTP Strict Transport Security (HSTS)
        if (config.hsts.enabled) {
            applyHSTSHeader(res, config.hsts);
            // Content Security Policy (CSP)
            if (config.csp.enabled) {
                applyCSPHeader(res, config.csp, res.locals.cspNonce);
                // X-Frame-Options
                        ...
                        if (config.xssProtection.enabled) {
                            ...
                            if (config.referrerPolicy.enabled) {
                                ...
                                if (config.permissionsPolicy.enabled) {
                                     ...
                                     * Apply HSTS header
                                     */
                                    function applyHSTSHeader(res, config) {
                                    ...
                                    if (config.includeSubDomains) {
                                        ...
                                        if (config.preload) {
                                             ...
                                             */
                                            function applyCSPHeader(res, config, nonce) {
                                                const directives = [];
                                             ...
                                             */
                                            function applyFrameOptionsHeader(res, config) {
                                                                                        ...
                                                                                        if (!headers['x-content-type-options']) {
                                                                                            ...
                                                                                            if (!headers['x-frame-options'] && !headers['content-security-policy']?.includes('frame-ancestors')) {
                                                                                                ...
                                                                                                export const securityMiddleware = {
                                                                                                    ...
                                                                                                    custom: (config) => createSecurityMiddleware(config),
                                                                                                };
