...
(function (DataEndpointCategory) {
    ...
    DataEndpointCategory[DataEndpointCategory["export"] = void 0] = "export";
    DataEndpointCategory[DataEndpointCategory["interface"] = void 0] = "interface";
    DataEndpointCategory[DataEndpointCategory["DataRetrievalLimits"] = void 0] = "DataRetrievalLimits";
...
async;
checkDataRetrievalLimit(subject, SubjectAttributes, object, ObjectAttributes, operation, DataOperation, requestDetails, DataRequestDetails);
    ...
    if (totalBytes + requestDetails.estimatedBytes > limits.limits.bytesPerHour) {
        ...
        if (totalRecords + requestDetails.estimatedRecords > limits.limits.recordsPerHour) {
            ...
            Promise < { allowed: boolean, reason: string, retryAfter: number } > {
                ...
                object: ObjectAttributes,
                operation: DataOperation,
                requestDetails: DataRequestDetails, Promise() {
                    const userHistory = this.accessHistory.get(subject.userId) || [];
                ...
                : .isClassificationEscalation(recentClassifications, object.classification)
            };
            {
                ...
                if (recentRequests.length > 50) {
                    ...
                    }, private, async, applyAdaptiveFactors(baseLimits, DataRetrievalLimits), subject, SubjectAttributes, object, ObjectAttributes);
                    Promise < DataRetrievalLimits > {
                        : .config.enableAdaptiveLimits };
                    {
                        return baseLimits;
                            ...
                            if (!subject.location.withinApprovedRegions) {
                                ...
                                if (!subject.device.managed || subject.device.riskScore > 50) {
                                    ...
                                    DataRetrievalLimits;
                                    {
                                        ...
                                        updateUserQuota(userId, string, details, unknown);
                                        Promise < void  > { return: Promise.resolve() };
                                        getEndpointFromOperation(operation, DataOperation);
                                        string;
                                        {
                                            return 'data_access';
                                        }
                                        getTypicalRequestSize(history, DataAccessAttempt);
                                        number;
                                        {
                                            return 1048576;
                                        }
                                        isClassificationEscalation(((recent, current) => { return false; }), private, calculateAnomalyScore(history, DataAccessAttempt), number, { return: 0 }, private, initializeUserQuota(userId, string), void { /* Implementation */}, private, getSecondsUntilMidnight(), number, { return: 86400 });
