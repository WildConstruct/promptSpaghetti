...
app.use(createClassificationEnforcementMiddleware({}), environment, 'production', detailedErrors, false, classificationExtractor, async (req) => {
        ...
        async (req, res) => {
            ...
            if (decision?.conditions) {
                ...
                allowedOperations: ['write'],
                    requiredControls;
                ['auth-strong_mfa', 'encryption', 'approval-workflow'],
                ;
            }
            createOperationValidationMiddleware(),
                async (req, res) => {
                    ...
                    if (enforcement) {
                        ...
                        'fin-' + Date.now(),
                            message;
                        'Financial data securely stored',
                        ;
                    }
                    ;
                    ;
                    ...
                    , 
                    // Only show details in development
                    ...(process.env.NODE_ENV === 'development' && {}), violations, enforcement?.violations, requiredControls, enforcement?.requiredControls);
                };
        };
    ;
};
