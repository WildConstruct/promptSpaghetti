...
const DEFAULT_SECURITY_CONFIG = {
    ...
    interface, SecurityConfigurationPageProps }, { userRole: , 'admin':  };
...
*/
export const SecurityConfigurationPage = ({
    userRole,
    userId,
    organizationId,
    theme = 'cinema'
            ...
            const: handleValidateConfig = useCallback(async (config) => {
                try {
                    ...
                    return {
                        ...
                        errors: validation.errors.map(error => ({}), field, error.field, message, error.message, severity, error.severity)
                    };
                }
                finally { }
            }),
            warnings: validation.warnings.map(warning => ({}), field, warning.field, message, warning.message, impact, warning.impact)
        };
...
if (isLoading) {
    ...
    _jsxs("div", { style: {
            ...
            fontFamily: 'Inter, system-ui, sans-serif',
                ...
                } }), ", padding: '16px 24px'; }}>", _jsxs("div", { style: {
                    display: 'flex',
 ...
 >
    (_jsx("span", { style: { color: '#dc2626', fontSize: '18px' }, children: "\u26A0\uFE0F" })
        ,
            _jsxs("div", { children: [_jsx("div", { style: {
                            ...
                            fontSize: '14px',
                            marginBottom: '2px',
                        }, children: "Configuration Error" }), _jsx("div", { style: { color: '#b91c1c', fontSize: '13px' }, children: error })] })
                ,
                    _jsx("button", { onClick: () => setError(null), style: {
                            marginLeft: 'auto',
...
;
{
 ...
 >
    (_jsx("span", { style: { color: '#059669', fontSize: '18px' }, children: "\u2705" })
        ,
            _jsxs("div", { children: [_jsx("div", { style: {
                            color: '#059669',
                            fontWeight: 500,
                            fontSize: '14px',
                            marginBottom: '2px',
                        }, children: "Success" }), _jsx("div", { style: { color: '#047857', fontSize: '13px' }, children: successMessage })] })
                ,
                    _jsx("button", { onClick: () => setSuccessMessage(null), style: {
                            marginLeft: 'auto',
                            background: 'transparent',
                            border: 'none',
                            color: '#059669',
...
export const BasicSecurityConfigExample = () => {
    ...
    if (!config.enableRealTimeAnalytics) {
        ...
        field: 'enableRealTimeAnalytics',
            message;
        'Real-time analytics recommended for security',
            impact;
        'high',
        ;
    }
...
return;
_jsx(SecurityAlertingConfigurationUI, { currentConfig: config, onConfigChange: handleConfigChange, onValidateConfig: handleValidateConfig, userRole: "admin", complianceFrameworks: [ComplianceFramework.SOC2], theme: "cinema" });
;
;
/**
...
export const HookBasedExample = () => {
    ...
    () => console.log('Configuration saved successfully!'),
        onSaveError;
    (error) => console.error('Failed to save configuration:', error),
    ;
};
const handleConfigChange = useCallback(async (newConfig) => {
