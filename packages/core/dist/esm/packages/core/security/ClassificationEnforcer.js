...
export class ClassificationEnforcer {
  ...
  async validateAccessRequirements(requirements, operation, currentControls) {
    ...
    // Check audit logging
    if (
      requirements.auditLogging !== 'STANDARD' &&
  ...
  async validateOperationRequirements(classification, operation, requirements, currentControls) {
    ...
    if (['export', 'share'].includes(operation.operation)) {
      if (requirements.transmission.endToEndEncryption && !currentControls.includes('e2e-encryption')) {
        violations.push('End-to-end encryption required for transmission');
      }
      if (requirements.transmission.certificatePinning && !currentControls.includes('cert-pinning')) {
        violations.push('Certificate pinning required');
      }
    }
    // Processing requirements
    if (requirements.processing.isolationRequired && !currentControls.includes('process-isolation')) {
      violations.push('Process isolation required');
    }
    // Caching restrictions
    if (!requirements.processing.cachingRestrictions.allowed && currentControls.includes('caching')) {
  ...
  convertDataHandlingToHandlingRequirements(dataReqs, classification) {
    ...
    return {
      ...
      transmission: {
        ...
        endToEndEncryption:
          dataReqs.encryption.inTransit && (classification === 'RESTRICTED' || classification === 'CONFIDENTIAL'),
      },
      processing: {
      ...
      access: {
        ...
        exportRestrictions:
          dataReqs.transfer.restrictions?.includes('export-control') || classification === 'RESTRICTED',
      },
      monitoring: {
