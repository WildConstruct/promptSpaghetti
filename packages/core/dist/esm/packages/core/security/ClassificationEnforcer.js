...
export class ClassificationEnforcer {
     ...
     */
    async enforceClassification(classification, operation, currentControls = []) {
        ...
        try {
            ...
            if (this.isUserExempted(operation.userId)) {
                return this.createEnforcementResult(true, classification, [], [], currentControls, 0, auditId);
            }
            // Validate access requirements
            const accessViolations = await this.validateAccessRequirements(requirements.access, operation, currentControls);
            // Validate operation-specific requirements
            const operationViolations = await this.validateOperationRequirements(classification, operation, requirements, currentControls);
            // Combine all violations
            const allViolations = [...accessViolations, ...operationViolations];
            // Calculate risk score
            const riskScore = this.calculateRiskScore(classification, operation, allViolations, currentControls);
            // Determine if operation is allowed
            const allowed = this.shouldAllowOperation(allViolations, riskScore, classification, operation);
            // Get required controls
            const requiredControls = this.getRequiredControls(classification, operation, requirements);
            // Log the enforcement decision
            if (this.config.auditLogging) {
                await this.logEnforcementDecision(auditId, operation, classification, allowed, allViolations, riskScore);
            }
            // Alert on violations if needed
            if (!allowed && this.config.alertingEnabled) {
                await this.alertOnViolation(classification, operation, allViolations, riskScore);
            }
            return this.createEnforcementResult(allowed, classification, allViolations, requiredControls, currentControls, riskScore, auditId, this.generateRecommendations(classification, allViolations));
        }
        catch (error) {
            ...
            if (this.config.strictMode) {
                return this.createEnforcementResult(false, classification, ['Enforcement system error'], [], currentControls, 100, auditId);
            }
            // Otherwise, allow with high risk score
            return this.createEnforcementResult(true, classification, ['Enforcement system error - allowed with risk'], [], currentControls, 90, auditId);
        }
    }
    /**
     * Make an access control decision
     */
    async makeAccessDecision(userId, dataId, classification, operation, context) {
        ...
        const authDecision = this.checkAuthenticationRequirements(requirements.access, fullContext);
        if (!authDecision.met) {
            return {
                granted: false,
        ...
        const authzDecision = this.checkAuthorizationRequirements(requirements.access, fullContext, classification);
        ...
        if (requirements.access.purposeLimitation) {
            const purposeDecision = this.checkPurposeLimitation(fullContext.purpose, classification);
     ...
     */
    async validateOperation(operation, classification, dataElement) {
        const requirements = this.getEffectiveRequirements(classification);
        const issues = [];
    ...
    getEffectiveRequirements(classification) {
        ...
        const baseRequirements = this.convertDataHandlingToHandlingRequirements(dataHandlingReqs, classification);
     ...
     * Validate access requirements
     */
    async validateAccessRequirements(requirements, operation, currentControls) {
        const violations = [];
     ...
     * Validate operation-specific requirements
     */
    async validateOperationRequirements(classification, operation, requirements, currentControls) {
        const violations = [];
     ...
     * Calculate risk score
     */
    calculateRiskScore(classification, operation, violations, currentControls) {
        let score = 0;
     ...
     */
    shouldAllowOperation(violations, riskScore, classification, operation) {
     ...
     */
    getRequiredControls(classification, operation, requirements) {
        const controls = [];
     ...
     * Check authentication requirements
     */
    checkAuthenticationRequirements(requirements, context) {
     ...
     * Check authorization requirements
     */
    checkAuthorizationRequirements(requirements, context, classification) {
     ...
     * Check purpose limitation
     */
    checkPurposeLimitation(purpose, classification) {
     ...
     */
    getAccessConditions(classification, context) {
        const conditions = [];
     ...
     */
    async logEnforcementDecision(auditId, operation, classification, allowed, violations, riskScore) {
        const event = {
            id: auditId,
            timestamp: new Date(),
     ...
     * Alert on violation
     */
    async alertOnViolation(classification, operation, violations, riskScore) {
     ...
     */
    generateRecommendations(classification, violations) {
     ...
     * Convert DataHandlingRequirements to HandlingRequirements
     */
    convertDataHandlingToHandlingRequirements(dataReqs, classification) {
     ...
     */
    createEnforcementResult(allowed, classification, violations, requiredControls, appliedControls, riskScore, auditId, recommendations) {
        return {
            allowed,
            classification,
            violations,
 ...
 */
export function createClassificationEnforcer(preset = 'production') {
    const configs = {
        development: {
            strictMode: false,
