...
notification_config: {
    ...
    escalation_delays: Record; // milliseconds,
...
export class SecurityEventPolicyManager {
    ...
    */
    initializeNotificationChannels() {
        ...
        enabled: true,
            severity_filter;
        [SecurityEventSeverity.MEDIUM, SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL],
            rate_limit;
        {
            max_per_minute: 15,
                burst_limit;
        ...
        'siem',
            endpoint;
        process.env.SIEM_ENDPOINT || 'https://siem.promptscape.internal/api/events',
            credentials;
        {
            api_key: process.env.SIEM_API_KEY || '',
                tenant_id;
            process.env.SIEM_TENANT_ID || '',
            ;
        }
        enabled: this.config.integration_config.siem_integration_enabled,
            severity_filter;
        Object.values(SecurityEventSeverity),
            rate_limit;
        {
            max_per_minute: 100,
                burst_limit;
...
'sms',
    endpoint;
process.env.SMS_SERVICE_ENDPOINT || '',
    credentials;
{
    api_key: process.env.SMS_API_KEY || '',
        sender_id;
    process.env.SMS_SENDER_ID || 'PromptScape',
    ;
}
enabled: true,
    severity_filter;
...
};
{
    ...
    try { }
        ...
        return result;
        async;
        executePolicyActions(((event, policyResult) => {
            const actions = [];
...
try { }
    ...
    async;
    sendNotifications(((event, policyResult) => {
        const results = [];
    ...
    for (const channel of channels) {
        ...
        try {
            ...
            return results;
            async;
            assessComplianceImpact((event, policyResult) => {
            ...
            * Handle event escalations
            */
            private, async, handleEscalations((event, policyResult) => {
                const escalations = [];
             ...
             * Create corresponding audit event in existing audit system
             */
            private, async, createAuditEvent(((securityEvent, enforcementResult) => {
                try {
                    ...
                    event_type: this.mapToAuditEventType(securityEvent.event_type),
                        severity;
                    this.mapToAuditSeverity(securityEvent.severity),
                        title;
                    `Security Policy Violation: ${securityEvent.title}`;
                }
                finally {
                }
            },
                description), securityEvent.description, category, 'security_policy', subcategory, securityEvent.category, user_id, securityEvent.user_id, ip_address, securityEvent.source_ip, system_component, securityEvent.system_component, risk_score, securityEvent.threat_level, risk_factors, securityEvent.indicators, compliance_frameworks, this.mapComplianceFrameworks(securityEvent.compliance_frameworks), regulatory_impact, securityEvent.regulatory_impact, sensitive_data_involved, securityEvent.evidence_preserved, tags, securityEvent.tags, metadata, {
            ...
            }, alert_triggered, true, notification_sent, enforcementResult.notifications_sent.length > 0, escalation_level, this.calculateEscalationLevel(securityEvent.severity), status, this.mapToAuditStatus(securityEvent.status))));
        }
        ;
...
try { }
    ...
    updatePolicyMetrics(matchedPolicies, string, result, PolicyEnforcementResult);
    void {
        for(, policyId, of, matchedPolicies) {
            ...
            if (this.config.real_time_processing) {
                ...
                PolicyMetrics;
                {
                    ...
                    NotificationChannel;
                    {
                         ...
                         */
                        updateNotificationChannel(channelId, string, updates, (Partial));
                        boolean;
                        {
