...
async;
generateFingerprint(((context, type = FingerprintType.ENHANCED) => {
 ...
 * Get location data from IP address and other sources
 */
public, async, getLocationData(ipAddress, string, additionalContext ?  : any), Promise < LocationData > {
    // Check cache first
    const: cached = this.ipLocationCache.get(ipAddress),
    if(cached) { }
} && this.isLocationDataFresh(cached));
{
    return cached;
                    ...
                    string;
                    {
                        ...
                        DeviceFingerprint['basic'];
                        {
                            ...
                            DeviceFingerprint['enhanced'];
                            {
                                ...
                                }(factors, RiskAssessment['factors'], riskLevel, RiskLevel);
                                string;
                                {
