...
export class AuditIntegration {
   ...
   */
  async logDataAccess(context, resourceType, resourceId, data, metadata) {
    ...
    if (this.enforcer) {
      enforcementResult = await this.enforcer.enforceClassification(classification, context);
    }
    // Log the access
  ...
  async logAdminOperation(operation, context, target, metadata) {
    await this.logger.log({
      ...
      authorized: true,
      success: true,
      sensitiveAccess: target.classification ? this.isSensitiveClassification(target.classification) : false,
      metadata: {
  ...
  async logBatchOperation(context, operation, resources, success, metadata) {
    const highestClassification = resources.reduce((highest, resource) => {
      ...
      return this.getClassificationPriority(level) > this.getClassificationPriority(highest) ? level : highest;
   ...
   */
  isSensitiveClassification(level) {
    return [
   ...
   */
  getClassificationPriority(level) {
    const priorities = {
  ...
  async generateComplianceReport(startDate, endDate, options) {
    ...
    for (const log of logs) {
      ...
      // Operation breakdown
      report.operationBreakdown[log.operation] = (report.operationBreakdown[log.operation] || 0) + 1;
      // Risk metrics
      if (log.riskScore) {
