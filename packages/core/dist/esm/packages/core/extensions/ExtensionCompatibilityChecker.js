...
(extension, context) => {
    ...
    for (const check of checks) {
        ...
        if (!check.compatible) {
            ...
            return result;
            checkSystemCompatibility(extension, ExtensionManifest, context, CompatibilityContext);
            CompatibilityCheck;
            {
                const issues = [];
                const warnings = [];
                ...
                if (extension.compatibility?.min_system_version) {
                    ...
                    if (systemVersion.compareTo(minVersion) < 0) {
                        ...
                        'error',
                            message;
                        `System version ${context.systemVersion} is below minimum required ${extension.compatibility.min_system_version}`;
                    }
                }
            ...
            if (extension.compatibility?.max_system_version) {
                ...
                if (systemVersion.compareTo(maxVersion) > 0) {
                    ...
                    'error',
                        message;
                    `System version ${context.systemVersion} is above maximum supported ${extension.compatibility.max_system_version}`;
                }
            }
            currentVersion: context.systemVersion,
                requiredVersion;
            extension.compatibility.max_system_version;
        }
        ;
        ...
        if (extension.capabilities?.requires) {
            for (const capability of extension.capabilities.requires) {
                if (!this.systemCapabilities.available.includes(capability)) {
                    ...
                    'error',
                        message;
                    `Required system capability not available: ${capability}`;
                }
            }
        ...
        checkDependencyCompatibility(extension, ExtensionManifest, context, CompatibilityContext);
        CompatibilityCheck;
        {
            const issues = [];
            const warnings = [];
            ...
            if (!range.satisfies(depVersion)) {
                ...
                'error',
                    message;
                `Dependency ${depId} version ${depExtension.version} doesn't satisfy ${versionRange}`;
            }
        }
        dependencyId: depId,
            currentVersion;
...
checkPlatformCompatibility(extension, ExtensionManifest, context, CompatibilityContext);
CompatibilityCheck;
{
    const issues = [];
    const warnings = [];
    ...
    if (extension.compatibility?.platforms) {
        if (!extension.compatibility.platforms.includes(context.platform)) {
            ...
            'error',
                message;
            `Platform ${context.platform} is not supported. Supported platforms: ${extension.compatibility.platforms.join(', ')}`;
        }
    }
    currentVersion: context.platform,
        requiredVersion;
    extension.compatibility.platforms.join(' | ');
}
;
...
if (extension.compatibility?.browsers && context.browserInfo) {
    for (const [browser, requiredVersion] of Object.entries(extension.compatibility.browsers)) {
        ...
        if (!range.satisfies(browserVersion)) {
            ...
            'error',
                message;
            `Browser ${browser} version ${currentVersion} doesn't satisfy ${requiredVersion}`;
        }
    }
    currentVersion,
        requiredVersion;
...
if (extension.capabilities?.requires) {
    for (const capability of extension.capabilities.requires) {
        ...
        if (feature && !feature.available) {
            ...
            'error',
                message;
            `Required platform feature not available: ${capability}`;
        }
    }
    dependencyId: capability;
}
...
checkPermissionCompatibility(extension, ExtensionManifest, context, CompatibilityContext);
CompatibilityCheck;
{
    const issues = [];
    const warnings = [];
    ...
    checkSecurityCompatibility(extension, ExtensionManifest, context, CompatibilityContext);
    CompatibilityCheck;
    {
        const issues = [];
        const warnings = [];
        ...
        if (extension.security?.content_security_policy) {
            if (!this.validateCSP(extension.security.content_security_policy)) {
                ...
                'error',
                    message;
                'Invalid or overly permissive Content Security Policy',
                ;
            }
            ;
                                                            ...
                                                            for (const depId of Object.keys(extension.dependencies.extensions)) {
                                                                ...
                                                                if (depExtension) {
                                                                    ...
                                                                    if (circular.length > 0) {
                                                                        return circular;
                                                                        return [];
                                                                        generateRecommendations(result, ExtensionCompatibilityResult);
                                                                        string;
                                                                        {
                                                                                        ...
                                                                                        if (circularIssues.length > 0) {
                                                                                            ...
                                                                                            if (result.warnings.some(w => w.includes('dangerous permission'))) {
                                                                                                ...
                                                                                                if (result.warnings.some(w => w.includes('sandbox'))) {
                                                                                                    ...
                                                                                                    addCompatibilityRule(rule, CompatibilityRule);
                                                                                                    void {
                                                                                                        ...
                                                                                                        */
                                                                                                        ...
                                                                                                        initializeDefaultRules() {
                                                                                                            ...
                                                                                                            'Semantic Versioning Compatibility',
                                                                                                                description;
                                                                                                            'Ensures extensions follow semantic versioning',
                                                                                                                check;
                                                                                                            (extension) => {
                                                                                                                try {
                                                                                                        ...
                                                                                                        check: (extension) => {
                                                                                                            ...
                                                                                                            if (extension.extension_type === 'ui' && !extension.ui) {
                                                                                                                ...
                                                                                                                'error',
                                                                                                                    message;
                                                                                                                'UI extension must have ui configuration',
                                                                                                                ;
                                                                                                            }
                                                                                                            ;
                                                                                                            if (extension.extension_type === 'node' && !extension.runtime?.node_types) {
                                                                                                                ...
                                                                                                                'error',
                                                                                                                    message;
                                                                                                                'Node extension must specify node_types',
                                                                                                                ;
                                                                                                            }
                                                                                                            ;
                                                                                                            return {
