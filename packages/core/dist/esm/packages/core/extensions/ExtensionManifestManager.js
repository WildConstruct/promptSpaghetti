 ...
 */
import { extensionManifestParser } from './ExtensionManifest';
...
export class ExtensionManifestManager {
    ...
    */
    async loadManifest(filePath) {
        ...
        finally // Store in main collection
         {
        }
    }
    ;
...
;
async;
loadManifestsFromDirectory(directoryPath, string);
Promise < ManifestBatchLoadResult > {
...
ExtensionManifest | undefined;
{
    ...
    ExtensionManifest;
    {
        ...
        getManifestsByType(extensionType, string);
        ExtensionManifest;
        {
            return Array.from(this.manifests.values())
                .filter(manifest => manifest.extension_type === extensionType);
            searchManifests(query, string);
            ExtensionManifest;
            {
                ...
                ;
                getManifestDependencies(extensionId, string);
                ...
                ExtensionValidationResult;
                {
                    ...
                    updateManifest(extensionId, string, updatedManifest, ExtensionManifest);
                    boolean;
                    {
                            ...
                            if (!validation.valid) {
                                ...
                                for (const [filePath, cached] of this.manifestCache.entries()) {
                                    if (cached.manifest.id === extensionId) {
                                        ...
                                        return true;
                                        removeManifest(extensionId, string);
                                        boolean;
                                        {
                                            const manifest = this.manifests.get(extensionId);
                                            if (!manifest) {
                                                ...
                                                for (const [filePath, cached] of this.manifestCache.entries()) {
                                                    if (cached.manifest.id === extensionId) {
                                                        ...
                                                        return true;
                                                        async;
                                                        refreshManifest(extensionId, string);
                                                        Promise < boolean > {
                                                    ...
                                                    };
                                                    {
                                                        ...
                                                        stopWatchingFile(filePath, string);
                                                        void {
                                                    ...
                                                    ExtensionManifestBuilder;
                                                    {
                                                        ...
                                                        return this;
                                                        setExtensionType(type, 'node' | 'ui' | 'transform' | 'storage');
                                                        ExtensionManifestBuilder;
                                                        {
