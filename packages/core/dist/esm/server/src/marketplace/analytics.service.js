import { TimeRange, AggregationType, AnalyticsEventSchema, AnalyticsQuerySchema, CustomReportSchema } from './analytics.types';
export class AnalyticsService {
    ...
    async trackEvent(eventData) {
        ...
        const [event] = await this.db.query(`INSERT INTO analytics_events 
    ...
    async batchTrackEvents(events) {
        ...
        const results = await this.db.query(`INSERT INTO analytics_events 
       (template_id, user_id, event_type, event_data, metadata, timestamp, created_at)
       VALUES ${values}
       RETURNING *`, params);
    ...
    async getTemplateMetrics(templateId, timeRange, startDate, endDate) {
        ...
        // Get basic metrics
        const [metricsRow] = await this.db.query(`SELECT 
       ...
       WHERE template_id = $1 AND timestamp BETWEEN $2 AND $3`, [templateId, start, end]);
        // Get demographics
    ...
    async createCustomReport(creatorId, reportData) {
        ...
        const [report] = await this.db.query(`INSERT INTO custom_reports 
    ...
    async getCustomReports(creatorId) {
        const reports = await this.db.query('SELECT * FROM custom_reports WHERE creator_id = $1 ORDER BY created_at DESC', [creatorId]);
    ...
    async getTemplateDemographics(templateId, start, end) {
        ...
        const [countriesData, devicesData] = await Promise.all([
            this.db.query(`SELECT metadata->>'location'->>'country' as country, COUNT(*) as count
         FROM analytics_events 
         ...
         ORDER BY count DESC LIMIT 10`, [templateId, start, end]),
            this.db.query(`SELECT metadata->>'device_type' as device, COUNT(*) as count
         FROM analytics_events 
         WHERE template_id = $1 AND timestamp BETWEEN $2 AND $3
         AND metadata->>'device_type' IS NOT NULL
         GROUP BY metadata->>'device_type'
         ORDER BY count DESC`, [templateId, start, end])
        ]);
    ...
    async getTemplateTrends(templateId, start, end) {
        const dailyMetrics = await this.db.query(`SELECT 
       ...
       ORDER BY date`, [templateId, start, end]);
        return {
