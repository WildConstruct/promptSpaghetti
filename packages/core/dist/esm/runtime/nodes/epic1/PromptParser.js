...
export class PromptParser {
    nodeCounter = 0;
    listSeparators = ['or', 'and', ','];
    descriptiveWords = ['with', 'in', 'wearing', 'holding', 'carrying'];
    /**
     ...
     */
    getTokenType(patternName, value) {
     ...
     */
    identifySegments(tokens, originalText) {
        const segments = [];
        let currentSegment = [];
        let isInList = false;
        let listItems = [];
     ...
     */
    tokensToText(tokens, originalText) {
        if (tokens.length === 0)
            return '';
    ...
    splitOnDescriptiveWords(text) {
        ...
        return parts.filter(part => part && !this.descriptiveWords.includes(part.toLowerCase().trim()));
    }
    /**
     * Generate nodes from segments
     */
    generateNodes(segments) {
        const nodes = [];
        const mappings = [];
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            switch (segment.suggestedNodeType) {
                case Epic1NodeType.WeightedChoice:
                    ...
                    const options = (segment.metadata?.alternatives || [segment.text])
                        .map((alt, idx) => ({
                        id: `opt-${idx}`,
                        text: alt.trim(),
     ...
     */
    identifyConcatOpportunities(segments) {
        const opportunities = [];
     ...
     */
    adjustBoundary(analysis, segmentIndex, newStartIndex, newEndIndex) {
     ...
     */
    mergeSegments(analysis, firstIndex, secondIndex) {
