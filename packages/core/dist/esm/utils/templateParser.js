...
priority: Math.min(10, history.count), // Priority based on usage
    relatedVariables;
this.findRelatedVariables(variableName);
;
                ...
                */
                ...
                }
                {
                    ...
                    void {
                        ...
                        const: parseTemplate = (template) => templateParser.parseTemplate(template),
                        const: getVariableSuggestions = (partialName) => context, string,
                        includeHistory: boolean = true, VariableSuggestion,
                        templateParser, : .getVariableSuggestions(partialName, context, includeHistory),
                        const: substituteVariables = (template, values) => templateParser.substituteVariables(template, values),
                        const: getPreviewWithSamples = (template) => templateParser.getPreviewWithSamples(template),
                        // Context management functions
                        const: trackVariableUsage = (variableName) => templateParser.trackVariableUsage(variableName),
                        // Clear cache utility
                        const: VARIABLE_CATEGORIES = [
                            'character', 'setting', 'action', 'mood', 'object',
                            'cinematic', 'temporal', 'descriptive', 'narrative', 'custom'
                        ],
                        type, VariableCategory = typeof VARIABLE_CATEGORIES[number],
                        /**
