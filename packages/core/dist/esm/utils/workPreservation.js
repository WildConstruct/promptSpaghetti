 ...
 */
export async function exportGraphState(nodes, edges, viewport) {
    const snapshot = {
 ...
 */
export async function importGraphState(snapshot) {
    // Validate snapshot
 ...
 */
export async function preserveWorkBeforeAuth(nodes, edges, viewport) {
    try {
        const snapshot = await exportGraphState(nodes, edges, viewport);
        // Store in sessionStorage (persists for session)
        sessionStorage.setItem(STORAGE_KEYS.PRE_AUTH_WORK, JSON.stringify(snapshot));
        // Also backup to localStorage (persists longer)
        localStorage.setItem(STORAGE_KEYS.WORK_BACKUP, JSON.stringify(snapshot));
 ...
 */
export async function restoreWorkAfterAuth() {
    try {
        ...
        const snapshot = JSON.parse(preserved);
        // Clean up storage
...
export function useWorkPreservation() {
    const preserveAndAuthenticate = async (nodes, edges, viewport, authCallback, options) => {
        try {
 ...
 */
export function RecoveryPrompt({ onRecover, onDiscard }) {
