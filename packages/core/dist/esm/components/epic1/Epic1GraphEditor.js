...
import { isStorageAvailable, STORAGE_KEY } from '../../utils/persistenceUtils';
...
const Epic1GraphEditorInner = ({ initialNodes = [], initialEdges = [], onNodesChange: onNodesChangeProp, onEdgesChange: onEdgesChangeProp, onExecute, showPreview = true, previewPosition = 'right', previewWidth = '400px', previewDebounceDelay = 300, previewSeeds, showAssetLibrary = true, assetLibraryPosition = 'left', }) => {
    ...
    const [nodes, setNodes, onNodesChangeBase] = useNodesState(persistedState?.nodes || initialNodes);
    const [edges, setEdges, onEdgesChangeBase] = useEdgesState(persistedState?.edges || initialEdges);
    ...
    const saveToLocalStorage = useCallback((currentNodes, currentEdges) => {
        if (!isStorageAvailable())
            return;
        try {
            const state = {
    ...
    // Debounced autosave
    const debouncedSave = useMemo(() => debounce((nodes, edges) => {
    ...
    const onNodesChange = useCallback((changes) => {
        ...
        if (hasDraggingChange) {
            ...
            const boxDragChanges = changes.filter(change => change.type === 'position' &&
                change.dragging &&
            ...
            if (boxDragChanges.length > 0) {
                ...
                boxDragChanges.forEach(boxChange => {
                    ...
                    if (!box || !box.data.locked)
                        return;
                    // Find nodes contained in this box
                    const containedNodeIds = nodes.filter(node => {
                        if (node.id === box.id || node.type === 'boundingBox' || node.type === 'postItNote')
                            return false;
    ...
    const normalizePresetPath = useCallback((path) => {
        if (!path)
            return path;
    ...
    const insertPresetByMeta = useCallback(async (meta, position) => {
        try {
            let content = null;
    ...
    const handleCreatePostIt = useCallback((position) => {
        const newNode = {
    ...
    const handleGroupNodes = useCallback((nodesToGroup) => {
    ...
    // Ungroup nodes
    const handleUngroupNodes = useCallback((nodesToUngroup) => {
    ...
    const handleCreateBoundingBox = useCallback((position) => {
        const newNode = {
    ...
    const content = (_jsxs("div", { className: "epic1-graph-editor", style: editorStyle, onDrop: onDrop, onDragOver: onDragOver, children: [_jsxs(ReactFlow, { nodes: enhancedNodes, edges: edges.map(edge => ({
                ...
                })), onNodesChange: onNodesChange, onEdgesChange: onEdgesChange, onConnect: onConnect, onPaneClick: handlePaneClick, onPaneContextMenu: handlePaneClick, onSelectionStart: () => setIsSelecting(true), onSelectionEnd: () => {
                ...
                }, position: "right", defaultTab: showAssetLibrary ? 'assets' : isPreviewVisible ? 'preview' : null, showAssets: showAssetLibrary, showPreview: showPreview })), contextMenuPosition && contextMenuNodeId && (_jsx(NodeContextMenu, { nodeId: contextMenuNodeId, nodeType: nodes.find(n => n.id === contextMenuNodeId)?.type || 'textBlock', position: contextMenuPosition, onClose: () => setContextMenuPosition(null), onSaveAsPreset: handleSaveAsPreset, onAttachNote: () => {
                    ...
                    setContextMenuPosition(null);
                } })), console.log('[Render] Context menu state:', { contextMenuPosition, contextMenuNodeId }), contextMenuPosition && !contextMenuNodeId && (_jsx(CanvasContextMenu, { position: contextMenuPosition, onAddNote: (pos) => {
                ...
                }, onClose: () => setContextMenuPosition(null) })), _jsx(SaveAsPresetDialog, { isOpen: !!saveAsPresetNodeId, nodeData: saveAsPresetNode?.data || null, nodeType: saveAsPresetNode?.type || 'textBlock', onClose: () => setSaveAsPresetNodeId(null), onSave: handleSavePreset })] }));
...
const Epic1GraphEditorMonolithic = (props) => {
...
export const Epic1GraphEditorConditional = process.env.FORCE_LEGACY_EDITOR === 'true' ? Epic1GraphEditorMonolithic : Epic1GraphEditorFinal;
