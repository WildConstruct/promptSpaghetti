...
export const SuccessCelebration = ({ type = 'achievement', title = 'Achievement Unlocked!', message = 'Great job!', duration = 3000, }) => {
    ...
    if (!isVisible || !onboardingState.preferences.enableCelebrations)
        return null;
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "celebration-message", style: {
                    position: 'fixed',
                    top: '50%',
                    left: '50%',
                    ...
                    minWidth: '300px',
                        ...
                        }, children: type === 'tutorial_complete' ? '🎉' : type === 'first_edit' ? '✨' : '🏆' }), _jsx("h2", { style: {
                            fontSize: '24px',
                            fontWeight: 600,
                            marginBottom: '8px',
                            color: '#1a1a1a',
                            ...
                            color: '#4a4a4a',
                            margin: 0,
                        }, children: message })] }), _jsx("div", { className: "confetti-container", style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    zIndex: 10000,
                    overflow: 'hidden',
                }, children: particles.map((particle) => (_jsx("div", { style: {
                        position: 'absolute',
                        left: particle.x,
                        top: particle.y,
                        width: '10px',
                        height: '10px',
                        backgroundColor: ['#ff6b6b', '#4ecdc4', '#45b7d1', '#f9ca24', '#6c5ce7'][particle.id % 5],
                        borderRadius: '50%',
                        animation: `confettiFall ${2 + Math.random()}s ease-out forwards`,
                        animationDelay: `${Math.random() * 0.5}s`,
                    } }, particle.id))) }), _jsx("style", { children: `
...
export const useSuccessCelebration = () => {
    ...
    const celebrateAchievement = (title, message) => {
        celebrate({
            type: 'achievement',
            title,
            message,
