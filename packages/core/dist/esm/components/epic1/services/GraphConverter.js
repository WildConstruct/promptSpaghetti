...
export class GraphConverter {
     ...
     */
    static convertToRuntimeGraph(flowNodes, flowEdges) {
        try {
            const runtimeNodes = new Map();
            for (const node of flowNodes) {
                // Skip nodes without proper type or position
                if (!node.type || !node.position) {
                    console.warn('GraphConverter: Skipping invalid node:', node.id, 'type:', node.type, 'position:', node.position);
                    continue;
                }
            ...
            console.error('GraphConverter: Error converting to runtime graph:', error);
            return null;
        }
    }
    /**
     ...
     */
    static exportGraph(nodes, edges) {
        const graphData = {
