...
export function useKeyboardNavigation({ nodes, selectedNodeId, onNodeSelect, onEscapePress, onEditCancel, enabled = true }) {
    ...
    const focusNodeInput = useCallback((nodeId, delay = 50) => {
        ...
        const timeoutId = window.setTimeout(() => {
            try {
                const inputElement = document.querySelector(`[data-node-id="${nodeId}"] input, [data-node-id="${nodeId}"] textarea`);
