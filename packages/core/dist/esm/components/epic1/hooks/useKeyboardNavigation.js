...
export function useKeyboardNavigation({ nodes, selectedNodeId, onNodeSelect, onEscapePress, onEditCancel, enabled = true }) {
    ...
    const getNextNode = useCallback((currentNodeId, reverse = false) => {
    ...
    const autoFocusFirstNode = useCallback(() => {
        ...
        if (editableNodes.length > 0 && !selectedNodeId) {
            ...
            setTimeout(() => {
                const inputElement = document.querySelector(`[data-node-id="${firstNode.id}"] input, [data-node-id="${firstNode.id}"] textarea`);
                inputElement?.focus();
            }, 100);
        }
    ...
    const handleKeyDown = useCallback((event) => {
        ...
        if (event.key === 'Tab') {
            ...
            if (nextNode) {
                ...
                setTimeout(() => {
                    const inputElement = document.querySelector(`[data-node-id="${nextNode.id}"] input, [data-node-id="${nextNode.id}"] textarea`);
                    inputElement?.focus();
                }, 50);
            }
        }
