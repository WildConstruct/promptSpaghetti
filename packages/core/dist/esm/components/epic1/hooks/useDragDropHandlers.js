...
export function useDragDropHandlers({ setNodes, setEdges, reactFlowInstance, showToast, addNodeWithBounce }) {
    ...
    const normalizePresetPath = useCallback((path) => {
        ...
        if (path.startsWith('presets/'))
            return `/${path}`;
        if (path.startsWith('asset-browser/presets/') ||
    ...
    const insertPresetByMeta = useCallback(async (meta, position) => {
        try {
            ...
            const count = (validation.nodeCount ?? 0) > 0
