 ...
 */
export const MEDIEVAL_PERIODS = { EARLY_MEDIEVAL: HISTORICAL_ERAS.MEDIEVAL_EARLY,
 ...
 */
export class MedievalDemoDatabase { private static instance: MedievalDemoDatabase;
  ...
  static getInstance(): MedievalDemoDatabase { if (!MedievalDemoDatabase.instance) {
  ...
  garment_type?: string;
  ceremonial?: boolean } = {}
  ): MedievalClothing { return this.clothingDatabase.filter(item => {
      if (criteria.era && !this.eraMatches(item.metadata.era, criteria.era)) {
        return false }
      if (
        criteria.social_class &&
        (!item.metadata.social_class || !item.metadata.social_class.includes(criteria.social_class))
      ) { return false }
      if (
        criteria.gender &&
        criteria.gender !== 'unisex' &&
        item.metadata.gender &&
        item.metadata.gender !== criteria.gender &&
        item.metadata.gender !== 'unisex'
      ) { return false }
      if (criteria.garment_type && item.medieval_specific.garment_type !== criteria.garment_type) { return false }
      if (criteria.ceremonial !== undefined && item.metadata.ceremonial !== criteria.ceremonial) { return false }
      return true;
    });
  ...
  ): UTDGNode { return this.materialDatabase.filter(item => {
      if (criteria.era && !this.eraMatches(item.metadata.era, criteria.era)) {
        return false }
      if (criteria.fabric_type && !item.metadata.tags.includes(criteria.fabric_type)) { return false }
      return true;
    });
  ...
  social_class?: SocialClass;
  type?: string } = {}
  ): UTDGNode { return this.accessoryDatabase.filter(item => {
      if (criteria.era && !this.eraMatches(item.metadata.era, criteria.era)) {
        return false }
      if (
        criteria.social_class &&
  ...
  season?: 'spring' | 'summer' | 'autumn' | 'winter' }): { outfit: (MedievalClothing | UTDGNode)[] }
  description: string;
 ...
 { const outfit: (MedievalClothing | UTDGNode)[] = [];
  ...
  gender: criteria.gender }
      ...
      item => item.medieval_specific.garment_type === 'chemise' || item.medieval_specific.garment_type === 'braies'
    );
    if (undergarments.length > 0) { outfit.push(undergarments[0]);
      historical_notes.push('Medieval people wore linen undergarments for hygiene and warmth') }

    // Main garment
    const mainGarments = this.getClothing({ era: criteria.era,
  social_class: criteria.social_class,
  gender: criteria.gender,
  ceremonial: criteria.occasion === 'ceremonial' }
}).filter(
      item => item.medieval_specific.garment_type === 'tunic' || item.medieval_specific.garment_type === 'gown'
    );
    if (mainGarments.length > 0) { outfit.push(mainGarments[0]);
      if (criteria.social_class === 'noble') {
        historical_notes.push('Noble garments featured fine wool or silk with elaborate decoration') } else if (criteria.social_class === 'peasant') { historical_notes.push('Peasant clothing was practical, made from coarse wool or hemp') }


    // Outer layer for cold weather
    if (criteria.season === 'winter' || criteria.season === 'autumn') { const cloaks = this.getClothing({
  era: criteria.era,
  social_class: criteria.social_class }
}).filter(item => item.medieval_specific.garment_type === 'cloak');
      if (cloaks.length > 0) { outfit.push(cloaks[0]);
        historical_notes.push('Cloaks were essential for warmth and weather protection') }


    // Accessories
    const accessories = this.getAccessories({ era: criteria.era,
  social_class: criteria.social_class }
});
   ...
   */
  private initializeDatabase(): void { this.clothingDatabase = this.createMedievalClothing();
   ...
   */
  private createMedievalClothing(): MedievalClothing { const clothing: MedievalClothing = [];
  // Peasant Male Tunic
  clothing.push({
  ...
  seasonal_use: 'all_season' }
    ...
    // Noble Female Gown
    clothing.push({ id: 'medieval_noble_female_gown_001',
  type: 'garment',
  content:,
  ...
  ceremonial_context: 'court' }
    ...
    // Monk's Robe
    clothing.push({ id: 'medieval_monk_robe_001',
  type: 'garment',
  content:,
   ...
   */
  private createMedievalMaterials(): UTDGNode { const materials: UTDGNode = [];
  // Wool Material
  materials.push({
  ...
  social_class: ['peasant'] }
    ...
    // Linen Material
    materials.push({ id: 'medieval_linen_material_001',
  type: 'material',
  content:,
   ...
   */
  private createMedievalAccessories(): UTDGNode { const accessories: UTDGNode = [];
  // Leather Belt
  accessories.push({
   ...
   */
  private eraMatches(itemEras: Era, targetEra: Era): boolean { return itemEras.some(
      era =>
   ...
   */
  private generateOutfitDescription(outfit: (MedievalClothing | UTDGNode)[], criteria: any): string { const garmentNames = outfit.map(item => {
    ...
    return (
      `A ${era} ${gender} ${socialClass} wearing ${garmentNames.join(', ')}. ` +
