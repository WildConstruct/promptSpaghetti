...
export class ExternalDataService {
  ...
  constructor() { this.initializeDefaultDataSources() }

  /**
   * Register a new data source
   */
  registerDataSource(dataSource: DataSource): void { this.dataSources[dataSource.id] = dataSource;
   ...
   */
  unregisterDataSource(sourceId: string): void { delete this.dataSources[sourceId];
   ...
   */
  getDataSources(): DataSource { return Object.values(this.dataSources) }

  /**
   * Get a specific data source by ID
   */
  getDataSource(sourceId: string): DataSource | null { return this.dataSources[sourceId] || null }

  /**
   * Query historical data from external sources
   */
  async queryHistoricalData(query: HistoricalQuery, sourceIds?: string): Promise<HistoricalQueryResult> { const startTime = performance.now();
  ...
  sources_used: sourcesUsed }
    ...
    // Cache the result
    if (result.nodes.length > 0) { this.setCachedData(cacheKey, {
  nodes: result.nodes
   ...
   */
  clearCache(sourceId?: string): void { if (sourceId) {
      this.clearCacheForSource(sourceId) } else { this.cache.clear() }


  /**
   * Get cache statistics
   */
  getCacheStats(): {total_entries: number, total_size: number, hit_rate: number} { const entries = Array.from(this.cache.values());
    const totalSize = entries.reduce((size, entry) => {
      return size + JSON.stringify(entry.data).length }, 0);

    return { total_entries: entries.length,
   ...
   */
  private async queryApiSource(source: DataSource, query: HistoricalQuery): Promise<UTDGNode> { if (!source.endpoint) {
   ...
   */
  private async queryDatabaseSource(source: DataSource, query: HistoricalQuery): Promise<UTDGNode> { // This would integrate with actual database connectors
    throw new Error('Database sources not implemented yet') }

  /**
   * Query a file-based data source (JSON/CSV files)
   */
  private async queryFileSource(source: DataSource, query: HistoricalQuery): Promise<UTDGNode> { if (!source.endpoint) {
    ...
    try {
      ...
      let data;
      if (source.endpoint.endsWith('.json')) { data = await response.json() } else if (source.endpoint.endsWith('.csv')) { const text = await response.text();
        data = this.parseCsvData(text) } else { throw new Error('Unsupported file format') }
   ...
   */
  private applyTransforms(data: any, transforms: DataTransform): UTDGNode { let transformedData = data;
  ...
  private mapFields(data: any, config: any): UTDGNode {
    ...
    return data.map(item => { const node: Partial<UTDGNode> = { }
  ...
  private filterData(data: any, config: any): any {
    return data.filter(item => {
      for (const [field, value] of Object.entries(config.filters || {})) { if (item[field] !== value) {
          return false }
   ...
   */
  private validateData(data: any, config: any): any { return data.filter(item => {
      // Basic validation
      if (!item.name && !item.content && !item.description) {
        return false }

      // Custom validation rules
      if (config.required_fields) { for (const field of config.required_fields) {
          if (!item[field]) {
            return false }
   ...
   */
  private enrichData(data: any, config: any): any { return data.map(item => ({
  ...item
   ...
   */
  private isValidUTDGNode(node: any): node is UTDGNode { return node && 
   ...
   */
  private parseEras(eraData: any): Era { if (!eraData) return [];
    ...
    if (Array.isArray(eraData)) { return eraData.map(this.parseEras).flat() }
   ...
   */
  private parseTags(tagData: any): string { if (!tagData) return [];
    if (typeof tagData === 'string') {
      return tagData.split(',').map(tag => tag.trim()) }
    if (Array.isArray(tagData)) { return tagData.map(tag => String(tag).trim()) }
    return [];
   ...
   */
  private buildApiUrl(endpoint: string, query: HistoricalQuery): string { const url = new URL(endpoint);
  // Add query parameters
  if (query.era) {
  url.searchParams.set('era', Array.isArray(query.era) ? query.era.join(',') : query.era) }
    if (query.region) { url.searchParams.set('region', Array.isArray(query.region) ? query.region.join(',') : query.region) }
    if (query.category) { url.searchParams.set('category', Array.isArray(query.category) ? query.category.join(',') : query.category) }
    if (query.limit) { url.searchParams.set('limit', query.limit.toString()) }
    if (query.offset) { url.searchParams.set('offset', query.offset.toString()) }
   ...
   */
  private buildAuthHeaders(auth?: AuthConfig): HeadersInit { const headers: HeadersInit = {
   ...
   */
  private getSourcesForQuery(query: HistoricalQuery): DataSource { return Object.values(this.dataSources).filter(source => {
  ...
  if (!hasEraOverlap) return false }
      ...
      if (query.category) { const queryTypes = Array.isArray(query.category) ? query.category : [query.category];
   ...
   * Process and deduplicate results
   */
  private processResults(results: UTDGNode, query: HistoricalQuery): UTDGNode { // Remove duplicates based on content similarity
    ...
    return unique.sort((a, b) => { if (query.sort_by === 'authenticity') {
   ...
   */
  private calculateSimilarity(text1: string, text2: string): number { // Simple Jaccard similarity
   ...
   */
  private generateCacheKey(query: HistoricalQuery, sourceIds?: string): string { const keyData = {
  ...query,
   ...
   */
  private getCachedData(key: string): CacheEntry | null { const entry = this.cache.get(key);
   ...
   * Cache query result
   */
  private setCachedData(key: string, data: any): void { // Use default TTL of 1 hour
   ...
   */
  private clearCacheForSource(sourceId: string): void { for (const [key, entry] of this.cache.entries()) {
   ...
   */
  private initializeDefaultDataSources(): void { // Medieval demo source
...
class RateLimiter {
  ...
  canMakeRequest(): boolean { const now = Date.now();
  ...
  recordRequest(): void { this.requests.push(Date.now()) }
