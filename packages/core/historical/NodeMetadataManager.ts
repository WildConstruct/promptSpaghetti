...
export interface QualityMetadata { authenticity: number; // 0-1 historical authenticity score;
...
export class NodeMetadataManager { private static instance: NodeMetadataManager;
  ...
  private initializeGenreDefinitions(): void { const genres: GenreTag = [
  {
  id: 'epic-fantasy'
  name: 'Epic Fantasy'
  category: 'literary'
   ...
   */
  public getNodeMetadata(nodeId: string): NodeMetadata | undefined { return this.nodeMetadata.get(nodeId);
  /**
   ...
   */
  public removeNodeTags(nodeId: string, tagIds: string): void { const metadata = this.getNodeMetadata(nodeId);
  if (metadata) {
  ...
  */
  ...
  for (const [nodeId, metadata] of this.nodeMetadata.entries()) { const matches = searchTags.map(searchTag => {)
  ...
  return nodeValue === value });
      ...
      if (isMatch) { results.push(nodeId);
  return results;
  /**
  ...
  */
  ...
  type: 'material',
    ...
    const socialClassKeywords = { 'noble': ['noble', 'lord', 'lady', 'duke', 'duchess', 'count', 'countess'],
  ...
  public validateTagCompatibility(tags: NodeTag): { valid: boolean; conflicts: string; warnings: string } {
    ...
    return { valid: conflicts.length === 0,
  conflicts }
  warnings
};
  /**
   ...
   */
  public importMetadata(metadataList: NodeMetadata): { imported: number; errors: string } { let imported = 0;
  const errors: string = [];
