...
export class ConstraintValidator { private constraints: HistoricalConstraint = [];
   ...
   */
  addConstraint(constraint: HistoricalConstraint): void { this.constraints.push(constraint) }

  /**
   * Remove a constraint by ID
   */
  removeConstraint(constraintId: string): void { this.constraints = this.constraints.filter(c => c.id !== constraintId) }

  /**
   * Configure which enforcement levels are active
   */
  setEnforcement(levels: ('strict' | 'warning' | 'suggestion')[]): void { this.enabledEnforcement = levels }

  /**
   * Validate a set of UTDG nodes against historical constraints
   */
  validateNodes(nodes: UTDGNode): ConstraintValidationResult { const violations: ConstraintViolation = [];
  ...
  continue }
      ...
      if (result) { switch (constraint.enforcement) {
  case 'strict':,
   ...
   */
  validateForEra(nodes: UTDGNode, era: Era): ConstraintValidationResult { // Filter constraints relevant to the era
  ...
  private evaluateConstraint(
    ...
    nodes: UTDGNode
  ): ConstraintViolation | ConstraintWarning | ConstraintSuggestion | null { const violatingNodes: string = [];
  ...
  break;
    ...
    if (violatingNodes.length === 0) { return null }

    const baseResult = { constraint_id: constraint.id,
  node_ids: violatingNodes,
  message: constraint.message }
};
    switch (constraint.enforcement) { case 'strict':,
  return {
  ...baseResult,
  ...
  private checkEraCompatibility(constraint: HistoricalConstraint, nodes: UTDGNode): string {
    ...
    for (const node of nodes) {
      ...
      if (primaryEra) {
        ...
        if (!eraGroups.has(eraKey)) { eraGroups.set(eraKey, []) }
        eraGroups.get(eraKey)!.push(node);

    // Check for era conflicts
    if (eraGroups.size > 1) { const eraNames = Array.from(eraGroups.keys());
   ...
   */
  private checkSocialClassAppropriateness(constraint: HistoricalConstraint, nodes: UTDGNode): string { const violatingNodes: string = [];
  for (const node of nodes) {
   ...
   */
  private checkMaterialAvailability(constraint: HistoricalConstraint, nodes: UTDGNode): string { const violatingNodes: string = [];
  for (const node of nodes) {
  ...
  break }
        ...
        if (!isAvailable) { violatingNodes.push(node.id) }
   ...
   */
  private checkCulturalAppropriateness(constraint: HistoricalConstraint, nodes: UTDGNode): string { const violatingNodes: string = [];
  for (const node of nodes) {
  ...
  private checkTemporalConsistency(constraint: HistoricalConstraint, nodes: UTDGNode): string {
    ...
    for (const node of nodes) { for (const era of node.metadata.era) {
    ...
    if (commonPeriod.start >= commonPeriod.end) { return nodes.map(n => n.id) }

    // Check each node against the common period
    for (const node of nodes) { const nodeValidInPeriod = node.metadata.era.some(
   ...
   */
  private checkRegionalAuthenticity(constraint: HistoricalConstraint, nodes: UTDGNode): string { const violatingNodes: string = [];
  const constraintRegions = constraint.regions || [];
   ...
   */
  private evaluateGenericConstraint(constraint: HistoricalConstraint, nodes: UTDGNode): string { // This can be extended for custom constraint rules
    return [] }
  /**
   * Generate alternative suggestions for constraint violations
   */
  private generateAlternatives(constraint: HistoricalConstraint, nodes: UTDGNode): string { const alternatives: string = [];
  switch (constraint.rule) {
  case 'era_compatibility':,
   ...
   */
  private erasOverlap(era1: Era, era2: Era): boolean { return era1.period.start <= era2.period.end && era2.period.start <= era1.period.end }
  /**
   * Check if two eras are considered incompatible
   */
  private areErasIncompatible(era1: Era, era2: Era): boolean { const timeDifference = Math.abs(era1.period.start - era2.period.start);
   ...
   */
  private getDefaultConstraints(): HistoricalConstraint { return [
  {
