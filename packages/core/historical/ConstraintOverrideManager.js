...
export class ConstraintOverrideManager {
     ...
     */
    createOverride(constraintId, reason, options = {}) {
     ...
     */
    isConstraintOverridden(constraintId, context = {}) {
     ...
     */
    removeConstraintOverrides(constraintId) {
        let removed = 0;
        for (const [id, override] of this.overrides.entries()) {
            if (override.constraint_id === constraintId) {
                this.overrides.delete(id);
     ...
     */
    calculateAuthenticityImpact(constraintIds) {
        let totalImpact = 0;
     ...
     */
    getSuggestedOverrides(constraints, context) {
        const suggestions = [];
     ...
     */
    matchesOverrideConditions(override, context) {
        ...
        if (conditions.era && context.era) {
            const eraMatch = conditions.era.some(era => era.name === context.era.name ||
        ...
        if (conditions.node_types && context.nodeTypes) {
            const typeMatch = conditions.node_types.some(type => context.nodeTypes.includes(type));
            if (!typeMatch) {
                return false;
            }
        }
        ...
        if (conditions.social_classes && context.socialClasses) {
            const classMatch = conditions.social_classes.some(cls => context.socialClasses.includes(cls));
            if (!classMatch) {
                return false;
            }
        }
