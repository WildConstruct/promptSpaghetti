...
export class ConstraintOverrideManager { private overrides: Map<string, ConstraintOverride> = new Map();
 ...
 = {}
  ...
  ): boolean { for (const override of this.overrides.values()) {
  ...
  */
  ...
  for (const [id, override] of this.overrides.entries()) { if (override.expires_at && new Date(override.expires_at) < now) {
  this.overrides.delete(id) } else { active.push(override);
  return active;
  /**
  ...
  */
  ...
  fromDate?: string;
  toDate?: string;
}): ConstraintOverride { let history = this.overrideHistory;
    if (filters) {
   ...
   */
  calculateAuthenticityImpact(constraintIds: string): number { let totalImpact = 0;
    ...
    for (const constraintId of constraintIds) {
      ...
      if (override?.conditions?.max_authenticity_impact) {
        totalImpact += override.conditions.max_authenticity_impact } else if (override) { // Default impact for overridden constraints
        totalImpact += 0.1;
  ...
  getSuggestedOverrides();
    ...
    for (const constraint of constraints) { // Suggest overrides for less critical constraints in creative contexts
 ...
 else if (context.narrativeNeeds?.includes('visual_impact') && constraint.rule === 'social_class_appropriateness') { suggestedReason = {
  category: 'narrative',
 ...
 else if (constraint.rule === 'cultural_appropriateness' && context.creativeGoals?.includes('educational')) { suggestedReason = {
