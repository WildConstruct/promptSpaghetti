 ...
 */
export class UTDGManager { private static instance: UTDGManager;
  ...
  static getInstance(): UTDGManager { if (!UTDGManager.instance) {
  ...
  */
  async queryHistoricalContent(query: HistoricalQuery): Promise<HistoricalQueryResult> { // Try external sources first
  try {
 ...
 catch (error) { console.warn('External data query failed, falling back to local demo:', error);
  ...
  */
  async generateHistoricalContent(config: ContentGenerationConfig): Promise<GeneratedContent> { const startTime = performance.now();
   ...
   */
  generateMedievalDemo(scenario: 'court_scene' | 'village_life' | 'monastery' | 'market_day'): GeneratedContent { let config: ContentGenerationConfig;
  switch (scenario) {
  case 'court_scene':
  config = {
  ...
  prefer_common_items: false }
      ...
      break;
    case 'village_life':
      config = { era: HISTORICAL_ERAS.MEDIEVAL_HIGH
  region: 'England'
   ...
   */
  exportForVFX(nodes: UTDGNode, era: Era, scene_description: string): VFXExportData { // Generate material descriptions for VFX
  ...
  historical_source: node.metadata.source }
    ...
    const crowdControlData = { character_types: this.extractCharacterTypes(nodes)
  ...
  social_stratification: this.calculateSocialDistribution(nodes) }
    ...
    const backdropData = { architectural_style: this.determineArchitecturalStyle(era)
  materials: materials.map(m => m.name)
  atmospheric_conditions: this.getAtmosphericConditions(era) }
};
    // Generate atmospheric data
    const meteorData = { weather_patterns: this.getWeatherPatterns(era)
   ...
   */
  validateUTDGData(nodes: UTDGNode): ValidationReport { const issues: any = [];
  ...
  freshness: freshnessScore }
      ...
      issues
      recommendations: this.generateRecommendations(overallScore, { completeness: completenessScore
  consistency: consistencyScore
  historical_accuracy: accuracyScore
  source_reliability: reliabilityScore
  freshness: freshnessScore }
   ...
   */
  registerExternalSource(source: any): void { this.externalDataService.registerDataSource(source) }
  /**
  * Get constraint validator for custom validation
  */
  getConstraintValidator(): ConstraintValidator { return this.constraintValidator }
  /**
  * Get external data service for custom queries
  */
  getExternalDataService(): ExternalDataService { return this.externalDataService }
  /**
  * Query medieval demo database directly
  */
  private queryMedievalDemo(query: HistoricalQuery): HistoricalQueryResult { const startTime = performance.now();
  ...
  gender: query.filters?.gender }
    ...
    // Query accessories
    if (!query.category || query.category.includes('accessory')) { const accessories = this.medievalDemo.getAccessories({
  era
  social_class: query.social_class }
});
   ...
   */
  private generateHistoricalContentSync(config: ContentGenerationConfig): GeneratedContent { const startTime = performance.now();
  ...
  occasion: 'ceremonial' }
      ...
      nodes = outfit.outfit;
 else { // Generate based on configuration
  const query: HistoricalQuery = {
  era: config.era.name
  social_class: config.social_class
  category: 'garment'
  ...
  private getRelevantNodeTypes(scenario: string): any { switch (scenario) {
  ...
  private filterByConfiguration(nodes: UTDGNode, config: ContentGenerationConfig): UTDGNode { return nodes.filter(node => {
      ...
      if (config.forbidden_elements) { const hasForbidden = config.forbidden_elements.some(element =>
          node.content.toLowerCase().includes(element.toLowerCase()) ||
          node.metadata.tags.some(tag => tag.toLowerCase().includes(element.toLowerCase()))
        );
  ...
  private applyCreativityFilter(nodes: UTDGNode, config: ContentGenerationConfig): UTDGNode { const targetCount = Math.min(nodes.length, 10);
    if (config.creativity_factor < 0.3) {
      ...
      return nodes
        ...
        .slice(0, targetCount) } else if (config.creativity_factor > 0.7) { // High creativity - include more variation
      const shuffled = nodes.sort(() => Math.random() - 0.5);
      return shuffled.slice(0, targetCount) } else { // Medium creativity - balanced selection
      return nodes
        .sort((a, b) => {
  ...
  private calculateAccuracyScore(nodes: UTDGNode, validation: ConstraintValidationResult): number { if (nodes.length === 0) return 0;
  ...
  private generateRecommendations(overallScore: number, metrics: any): string { const recommendations: string = [];
  if (metrics.completeness < 0.7) {
  recommendations.push('Improve data completeness by filling missing required fields') }
    if (metrics.consistency < 0.7) { recommendations.push('Address constraint violations to improve data consistency') }
    if (metrics.historical_accuracy < 0.7) { recommendations.push('Verify historical accuracy with authoritative sources') }
    if (metrics.source_reliability < 0.5) { recommendations.push('Add external source validation for more nodes') }
    
    if (overallScore > 0.8) { recommendations.push('Data quality is excellent - ready for production use') } else if (overallScore > 0.6) { recommendations.push('Data quality is good - minor improvements recommended') } else { recommendations.push('Data quality needs significant improvement before production use') }
    
    return recommendations;

  
  // VFX Export helper methods
  private extractMaterialProperties(node: UTDGNode): Record<string, any> { return {
  type: node.type
  ...
  private extractPatternDescription(node: UTDGNode): string { return node.content.split('.')[0] || node.content }
  
  private extractColorPalette(node: UTDGNode): string { // Extract colors mentioned in content and tags
  const colors: string = [];
  ...
  private extractCharacterTypes(nodes: UTDGNode): string { const types = new Set<string>();
    nodes.forEach(node => {
      if (node.metadata.social_class) {
        node.metadata.social_class.forEach(cls => types.add(cls)) }
      if (node.metadata.occupation) { node.metadata.occupation.forEach(occ => types.add(occ)) }
    });
    return Array.from(types);

  private generateClothingCombinations(nodes: UTDGNode): string[] { const combinations: string = [];
  ...
  private calculateSocialDistribution(nodes: UTDGNode): Record<string, number> {
    ...
    nodes.forEach(node => { if (node.metadata.social_class) {
    ...
    Object.keys(distribution).forEach(key => { distribution[key] = distribution[key] / total });
    
    return distribution;

  private determineArchitecturalStyle(era: Era): string { if (era.name.includes('Medieval')) {
      return 'Gothic' } else if (era.name.includes('Roman')) { return 'Classical Roman' } else if (era.name.includes('Renaissance')) { return 'Renaissance' }
    return 'Period Appropriate';

  private getAtmosphericConditions(era: Era): string { return ['torchlight', 'candlelight', 'natural daylight', 'fireplace glow'] }
  
  private getWeatherPatterns(era: Era): string { return ['clear', 'overcast', 'light rain', 'mist'] }
  
  private getSeasonalConditions(era: Era): string { return ['spring mild', 'summer warm', 'autumn cool', 'winter cold'] }
  
  private getTimePreferences(era: Era): string { return ['dawn', 'midday', 'afternoon', 'dusk'] }
  ...
  private generateLightingNotes(era: Era): string { return [
  ...
  private generateAtmosphericNotes(era: Era): string { return [
