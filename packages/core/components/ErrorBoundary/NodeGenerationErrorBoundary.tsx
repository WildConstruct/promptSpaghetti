...
export interface NodeGenerationErrorBoundaryProps { children: ReactNode;
...
export interface NodeGenerationErrorBoundaryState { hasError: boolean;
  error: Error | null;
...
> { private maxRetries = 3;
  ...
  static getDerivedStateFromError(error: Error): NodeGenerationErrorBoundaryState { return {
  hasError: true,
  error,
  errorInfo: null,
  retryCount: 0 }
};
  componentDidCatch(error: Error, errorInfo: ErrorInfo) { console.error('Node generation error caught by boundary:', error, errorInfo);
  ...
  private handleRetry = () => { if (this.state.retryCount >= this.maxRetries) {
  ...
  retryCount: prevState.retryCount + 1 }
    ...
    this.retryTimeoutId = setTimeout(() => { // Force re-render to retry the operation
      this.forceUpdate() }, 1000);
  };
  private handleReset = () => { this.setState({)
  hasError: false,
  error: null,
  errorInfo: null,
  retryCount: 0 }
  ...
  private getThemeStyles = () => { const theme = this.props.theme || 'cinema';
  const themes = {
  light: {,
  background: '#ffffff',
  secondary: '#f8fafc',
  ...
  private renderErrorDetails = (error: Error, errorInfo: ErrorInfo | null) => { const styles = this.getThemeStyles();
  ...
  private renderDefaultFallback = (error: Error) => { const styles = this.getThemeStyles();
 ...
 */
export function useErrorHandler() { return (error: Error, errorInfo?: ErrorInfo) => {,
