...
export interface IntegratedFileBrowserProps { onFileSelected?: (file: PSGFile) => void;
...
interface FileContextMenuAction { id: string;
  label: string;
  icon: string;
...
interface FolderViewState { viewMode: 'list' | 'grid' | 'details';
...
export const IntegratedFileBrowser: React.FC<IntegratedFileBrowserProps> = ({ onFileSelected
  onProjectLoad
  onNewProject
  onFileAction
  theme = 'cinema'
  ...
  x: number;
  y: number;
  file: PSGFile } | null>(null);
  const [viewState, setViewState] = useState<FolderViewState>({ viewMode: 'grid'
  sortBy: 'modified'
  sortDirection: 'desc'
  showHidden: false
  filterText: '' }
  ...
  useEffect(() => { loadFiles();
    loadRecentFiles();
    loadFavoriteFiles() }, [currentFolder]);
  const loadFiles = useCallback(async () => { setLoading(true);
    try {
      ...
      setFiles(mockFiles);
 catch (error) { console.error('Failed to load files:', error) } finally { setLoading(false) }
  ...
  const filteredAndSortedFiles = useMemo(() => { let filtered = files;
  // Apply text filter
  ...
  // File actions
  const handleFileSelect = useCallback((file: PSGFile) => { setSelectedFile(file);
    onFileSelected?.(file) }, [onFileSelected]);
  const handleFileDoubleClick = useCallback((file: PSGFile) => { projectManager.addToRecentFiles(file);
    loadRecentFiles();
    onProjectLoad?.(file) }, [projectManager, loadRecentFiles, onProjectLoad]);
  const handleFileContextMenu = useCallback((e: React.MouseEvent, file: PSGFile) => { e.preventDefault();
  setContextMenu({
  x: e.clientX
  y: e.clientY }
  file
  ...
  const handleToggleFavorite = useCallback((file: PSGFile) => { const isFavorite = projectManager.toggleFavorite(file.id);
    file.isFavorite = isFavorite;
    loadFavoriteFiles();
    setContextMenu(null) }, [projectManager, loadFavoriteFiles]);
  ...
  const handleFileDuplicate = useCallback((file: PSGFile) => { // Mock duplication - in real implementation would duplicate on server/filesystem
  ...
  useEffect(() => { const handleClickOutside = () => setContextMenu(null);
    if (contextMenu) {
      document.addEventListener('click', handleClickOutside);
      return () => document.removeEventListener('click', handleClickOutside) }
  }, [contextMenu]);
  const formatFileSize = (bytes: number): string => { if (bytes === 0) return '0 B';
    const k = 1024;
