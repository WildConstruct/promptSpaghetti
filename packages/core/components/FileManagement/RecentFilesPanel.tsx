...
export interface RecentFilesPanelProps { onFileSelected?: (file: PSGFile) => void;
...
export const RecentFilesPanel: React.FC<RecentFilesPanelProps> = ({ onFileSelected
  onFileLoad
  onClearRecents
  theme = 'cinema'
  maxRecentFiles = 10
  ...
  useEffect(() => { loadRecentFiles();
    loadFavoriteFiles() }, [maxRecentFiles]);
  ...
  const handleFileSelect = useCallback(
    (file: PSGFile) => { setSelectedFile(file);
      onFileSelected?.(file) },
    [onFileSelected]
  );
  const handleFileLoad = useCallback(
    (file: PSGFile) => { projectManager.addToRecentFiles(file);
      loadRecentFiles();
      onFileLoad?.(file) },
  ...
  const handleClearRecents = useCallback(() => { if (confirm('Are you sure you want to clear all recent files? This action cannot be undone.')) {
  ...
  const formatDate = (date: Date): string => { const now = new Date();
    ...
    if (diffDays === 0) {
      return 'Today' } else if (diffDays === 1) { return 'Yesterday' } else if (diffDays < 7) {
  ...
  color: styles.textSecondary }
            ...
            >
              ...
              <div style={{ fontSize: '14px' }}>Open some projects to see them here</div>
            </div>
          ) : (
            <div
              style={ {
