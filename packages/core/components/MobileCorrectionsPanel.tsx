...
interface MobileCorrectionsPanelProps { isOpen: boolean;
  onClose: () => void }
...
export const MobileCorrectionsPanel: React.FC<MobileCorrectionsPanelProps> = ({ isOpen }
  onClose
}) => { const {
    rules
    addRule
 ...
 = useCorrectionsStore();
  ...
  const filteredRules = useMemo(() => { if (!searchQuery) return rules;
  ...
  const handleUpdateRule = useCallback((rule: CorrectionRule) => { updateRule(rule.id, rule);
    setSelectedRule(null) }, [updateRule]);
  const handleDeleteRule = useCallback((id: string) => { if (window.confirm('Delete this rule?')) {
      deleteRule(id);
      setSelectedRule(null) }, [deleteRule]);
