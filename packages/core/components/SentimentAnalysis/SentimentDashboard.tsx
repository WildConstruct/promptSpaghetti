...
export const SentimentDashboard: React.FC<SentimentDashboardProps> = ({ )
  ...
  useEffect(() => { loadAnalytics();
    ...
    if (refreshInterval && refreshInterval > 0) {
      ...
      return () => clearInterval(interval) }, [resourceId, resourceType, selectedTimeRange, refreshInterval]);
  const loadAnalytics = async () => { setIsLoading(true);
    setError(null);
    try {
 ...
 catch (err) { setError(err instanceof Error ? err.message : 'Failed to load sentiment analytics') } finally { setIsLoading(false) };
  const getSentimentColor = (sentiment: SentimentType) => { switch (sentiment) {
  case 'positive': return '#059669';
  case 'negative': return '#dc2626';
  default: return '#6b7280' };
  const getEmotionIcon = (emotion: EmotionType) => { const icons = {
  joy: '😄'
  sadness: '😢'
  anger: '😠'
  fear: '😨'
  ...
  const getToxicityColor = (level: ToxicityLevel) => { switch (level) {
  case 'severe': return '#dc2626';
  case 'high': return '#ea580c';
  case 'medium': return '#d97706';
  case 'low': return '#facc15';
  default: return '#059669' };
  const formatPercentage = (num: number) => { return num.toFixed(1) + '%' };
