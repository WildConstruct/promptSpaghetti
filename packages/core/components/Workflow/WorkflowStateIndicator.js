...
export const WorkflowStateHistory = ({ history, className = '' }) => {
    return (_jsx("div", { className: `space-y-3 ${className}`, children: history.map((entry, index) => (_jsxs("div", { className: "flex items-start space-x-3", children: [_jsx(WorkflowStateIndicator, { state: entry.state, stateConfig: entry.stateConfig, showLabel: false, size: "sm" }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center space-x-2 text-sm", children: [_jsx("span", { className: "font-medium text-gray-900", children: entry.stateConfig?.name || entry.state }), entry.actor && (_jsxs("span", { className: "text-gray-500", children: ["by ", entry.actor] })), _jsx("span", { className: "text-gray-400", children: new Date(entry.timestamp).toLocaleString() })] }), entry.comment && (_jsx("p", { className: "text-sm text-gray-600 mt-1", children: entry.comment }))] })] }, index))) }));
};
