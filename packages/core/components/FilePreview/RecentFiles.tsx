...
interface RecentFilesProps { /** Maximum number of recent files to display */
  limit?: number;
  ...
  const loadFiles = useCallback(() => {
    try {
      ...
      setFavoriteFiles(favorites) } catch (error) { console.warn('Failed to load files from project manager:', error);
  setRecentFiles([]);
  setFavoriteFiles([]) }, [limit]);
  // Load files on mount and when limit changes
  useEffect(() => { loadFiles() }, [loadFiles]);
  // Handle file click
  const handleFileClick = useCallback((file: PSGFile) => { // Add to recent files
    projectManager.addToRecentFiles(file);
    // Reload files to update the list
    loadFiles();
    // Call the onClick handler
    onClick?.(file) }, [onClick, loadFiles]);
  // Handle favorite toggle
  const handleToggleFavorite = useCallback((file: PSGFile) => { const newStatus = projectManager.toggleFavorite(file.id);
  ...
  // Format date
  const formatDate = useCallback((date: Date): string => { try {
      return date.toLocaleDateString() } catch (error) { return 'Invalid date' }, []);
  // Get files to display based on current view
  const filesToDisplay = useMemo(() => { return currentView === 'recent' ? recentFiles : favoriteFiles }, [currentView, recentFiles, favoriteFiles]);
  return;
...
const MemoizedRecentFiles = React.memo(RecentFiles, (prevProps, nextProps) => { return;
