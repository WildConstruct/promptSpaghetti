...
export interface DiffChange { type: 'added' | 'removed' | 'modified' | 'moved';
...
export interface GraphDiff { id: string;
...
export class GraphDiffEngine { private static readonly POSITION_THRESHOLD = 10; // pixels
 ...
 = {}) {}
  ...
  async computeDiff(fromGraph: GraphData, toGraph: GraphData): Promise<GraphDiff> { const startTime = Date.now();
  ...
  created_at: new Date().toISOString() }
    ...
    return diff;
  private detectNodeChanges(fromNodes: Map<string, GraphNode>, toNodes: Map<string, GraphNode>): DiffChange { const changes: DiffChange = [];
  // Find added nodes
  ...
  significance: this.calculateSignificance('added', 'node', node) }
    ...
    // Find removed nodes
    for (const [nodeId, node] of fromNodes) { if (!toNodes.has(nodeId)) {
  changes.push({)
  type: 'removed'
  element_type: 'node'
  element_id: nodeId
  ...
  significance: this.calculateSignificance('removed', 'node', node) }
    ...
    // Find modified nodes
    for (const [nodeId, fromNode] of fromNodes) { const toNode = toNodes.get(nodeId);
  if (!toNode) continue;
  ...
  significance: this.calculateSignificance('added', 'edge', edge) }
    ...
    // Find removed edges
    for (const [edgeId, edge] of fromEdges) { if (!toEdges.has(edgeId)) {
  changes.push({)
  type: 'removed'
  element_type: 'edge'
  element_id: edgeId
  ...
  significance: this.calculateSignificance('removed', 'edge', edge) }
    ...
    // Find modified edges
    for (const [edgeId, fromEdge] of fromEdges) { const toEdge = toEdges.get(edgeId);
  if (!toEdge) continue;
  ...
  significance: Math.min(change.significance, 0.3) // Style changes are less significant }
    ...
    if (fromNode.type !== toNode.type) { changes.push({)
  type: 'modified'
  element_type: 'property'
  element_id: fromNode.id
  property_path: 'type'
  ...
  significance: 0.8 // Type changes are significant }
    ...
    return changes;
  private compareEdges(fromEdge: GraphEdge, toEdge: GraphEdge): DiffChange { const changes: DiffChange = [];
  ...
  element_id: fromEdge.id }
    ...
    // Check for type changes
    if (fromEdge.type !== toEdge.type) { changes.push({)
  type: 'modified'
  element_type: 'property'
  element_id: fromEdge.id
  property_path: 'type'
  ...
  toObj: unknown
    ...
    for (const key of allKeys) {
      ...
      if (fromValue === undefined && toValue !== undefined) { // Property added
  changes.push({)
  type: 'added'
 ...
 else if (fromValue !== undefined && toValue === undefined) { // Property removed
  changes.push({)
  type: 'removed'
  property_path: fullPath
  old_value: fromValue
  significance: this.calculatePropertySignificance(key, fromValue) }
 ...
 else if (fromValue !== toValue) { // Property modified
        if (this.options.deep_property_comparison && )
          ...
          changes.push(...this.compareObjectProperties(fromValue, toValue, fullPath)) } else { changes.push({)
  type: 'modified'
            property_path: fullPath
            old_value: fromValue
            new_value: toValue }
    ...
    return changes;
  private calculateDistance(from: { x: number; y: number }, to: { x: number; y: number }): number { return Math.sqrt(Math.pow(to.x - from.x, 2) + Math.pow(to.y - from.y, 2));
  ...
  'position': 0.5 }
    ...
    if (typeof value === 'object' && value !== null) { return baseSig * 1.2; // Objects are more complex
  return baseSig;
  ...
  complexity_score: 0 }
    ...
    for (const change of changes) { if (change.element_type === 'node') {
  switch (change.type) {
  case 'added': summary.added_nodes++; break;
  case 'removed': summary.removed_nodes++; break;
  case 'modified': summary.modified_nodes++; break;
  case 'moved': summary.moved_nodes++; break } else if (change.element_type === 'edge') { switch (change.type) {
  case 'added': summary.added_edges++; break;
  case 'removed': summary.removed_edges++; break;
  case 'modified': summary.modified_edges++; break } else if (change.element_type === 'property') { summary.property_changes++;
...
> = [];
  ...
  change_type: change.type }
    ...
    return { changed_regions: changedRegions,
...
>();
  ...
  let position: { x: number; y: number } | null = null;
      ...
      if (change.element_type === 'node') { const node = allNodes.find(n => n.id === change.element_id);
        if (node) {
          position = node.position } else if (change.element_type === 'edge') { // For edges, use midpoint of connected nodes
  ...
  max_significance?: number }): DiffChange {
