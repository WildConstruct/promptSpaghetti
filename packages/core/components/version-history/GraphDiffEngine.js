...
export class GraphDiffEngine {
    ...
    async computeDiff(fromGraph, toGraph) {
    ...
    detectNodeChanges(fromNodes, toNodes) {
        const changes = [];
        // Find added nodes
    ...
    detectEdgeChanges(fromEdges, toEdges) {
        const changes = [];
        // Find added edges
    ...
    compareNodes(fromNode, toNode) {
        ...
        const dataChanges = this.compareObjectProperties(fromNode.data || {}, toNode.data || {}, 'data');
        ...
        if (!this.options.ignore_style_changes) {
            const styleChanges = this.compareObjectProperties(fromNode.style || {}, toNode.style || {}, 'style');
    ...
    compareEdges(fromEdge, toEdge) {
        ...
        // Check for data changes
        const dataChanges = this.compareObjectProperties(fromEdge.data || {}, toEdge.data || {}, 'data');
        dataChanges.forEach(change => {
            changes.push({
                ...change,
                element_type: 'property',
    ...
    compareObjectProperties(fromObj, toObj, basePath) {
        const changes = [];
        // Get all unique keys
    ...
    calculateSignificance(changeType, elementType, element) {
    ...
    calculatePropertySignificance(propertyName, value) {
    ...
    calculateSummary(changes, fromGraph, toGraph) {
        const summary = {
        ...
        const totalElements = Math.max(fromGraph.nodes.length + fromGraph.edges.length, toGraph.nodes.length + toGraph.edges.length, 1);
    ...
    generateVisualizationData(changes, fromGraph, toGraph) {
    ...
    groupChangesSpatially(changes, fromGraph, toGraph) {
    ...
    static filterChanges(diff, filters) {
    ...
    static getChangesByElement(diff, elementId) {
        return diff.changes.filter(c => c.element_id === elementId);
    }
    static getSignificantChanges(diff, threshold = 0.7) {
