...
describe('NodeReplacementModal', () => {
  const mockNodes = [
    { id: 'node-1', type: 'textBlock', data: { content: 'Original text' }, position: { x: 0, y: 0 } },
    { id: 'node-2', type: 'variable', data: { name: 'heroName' }, position: { x: 100, y: 0 } },
    { id: 'node-3', type: 'weightedChoice', data: { options: ['A', 'B'] }, position: { x: 200, y: 0 } },
  ];
  ...
  describe('Rendering', () => {
    ...
    it('should not render when closed', () => {
      ...
      expect(screen.queryByTestId('node-replacement-modal')).not.toBeInTheDocument();
    });
  ...
  describe('Replacement Actions', () => {
    ...
    it('should handle batch replacement of same type', async () => {
      ...
      const batchCheckbox = screen.getByLabelText(/replace all text block nodes/i);
    ...
    it('should show confirmation for batch operations', async () => {
      ...
      expect(screen.getByText(/this will replace \d+ nodes/i)).toBeInTheDocument();
    });
  });
  ...
  describe('Validation', () => {
    ...
    it('should validate node connections compatibility', () => {
      ...
      render(<NodeReplacementModal {...defaultProps} selectedNode={nodeWithConnections} />);
      
      const incompatibleOption = screen.getByTestId('node-type-output');
      expect(incompatibleOption).toHaveClass('incompatible');
      expect(incompatibleOption).toHaveAttribute('title', expect.stringContaining('incompatible'));
    });
  ...
  describe('Undo Support', () => {
    it('should show undo information', () => {
      ...
      expect(screen.getByText(/this action can be undone/i)).toBeInTheDocument();
    });
  ...
  describe('Performance', () => {
    it('should handle large node lists efficiently', () => {
      ...
      render(<NodeReplacementModal {...defaultProps} availableNodeTypes={manyNodeTypes} />);
