...
interface RecentProjectsMenuProps { onLoadRecentProject: (entry: RecentProjectEntry) => void;
  className?: string;
  ...
  const handleProjectClick = (entry: RecentProjectEntry) => { setIsOpen(false);
    onLoadRecentProject(entry);
    // Update last access date
    RecentProjectsManager.updateLastAccess(entry.name) };
  const handleClearRecent = () => { if (confirm('Clear all recent projects?')) {
