...
export const VFXChecklistSystem = ({ checklist, currentUser, onChecklistUpdate, onItemCreate, onItemUpdate, onItemDelete, onCommentCreate, readonly = false, showStatistics = true, compactView = false, className = '' }) => {
    ...
    const statistics = useMemo(() => {
        ...
        const overdue = checklist.items.filter(item => item.dueDate && new Date(item.dueDate) < new Date() && item.status !== 'approved').length;
...
const VFXChecklistItemCard = ({ item, checklist, currentUser, onStatusChange, onPriorityChange, onAssigneeChange, onProgressChange, onCommentCreate, onItemUpdate, onItemDelete, readonly = false, compact = false, isSelected = false, onSelect }) => {
    ...
    return (_jsx(Card, { className: `checklist-item ${isSelected ? 'ring-2 ring-blue-500' : ''} ${isOverdue ? 'border-red-300' : ''}`, children: _jsx(CardContent, { className: "p-4", children: _jsxs("div", { className: "flex items-start gap-4", children: [_jsx("div", { className: "flex-shrink-0 mt-1", children: _jsx("button", { onClick: () => onStatusChange(item.id, item.status === 'approved' ? 'pending' : 'approved'), disabled: readonly, className: `w-5 h-5 border-2 rounded flex items-center justify-center ${item.status === 'approved'
                                ? 'bg-green-500 border-green-500 text-white'
                                : 'border-gray-300 hover:border-gray-400'}`, children: item.status === 'approved' && _jsx(CheckSquare, { className: "w-3 h-3" }) }) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex-1", children: [_jsx("h3", { className: `font-medium ${item.status === 'approved' ? 'line-through text-gray-500' : 'text-gray-900'}`, children: item.title }), item.description && (_jsx("p", { className: "text-sm text-gray-600 mt-1", children: item.description }))] }), _jsxs("div", { className: "flex items-center gap-2 ml-4", children: [_jsxs(Badge, { variant: "secondary", style: { backgroundColor: `${statusConfig.color}20`, color: statusConfig.color }, className: "flex items-center gap-1", children: [_jsx(StatusIcon, { className: "w-3 h-3" }), statusConfig.label] }), _jsx(Badge, { variant: "secondary", style: { backgroundColor: `${priorityConfig.color}20`, color: priorityConfig.color }, children: priorityConfig.label }), isOverdue && (_jsxs(Badge, { variant: "destructive", className: "flex items-center gap-1", children: [_jsx(AlertTriangle, { className: "w-3 h-3" }), "Overdue"] }))] })] }), !compact && (_jsxs("div", { className: "mt-3", children: [_jsxs("div", { className: "flex items-center justify-between text-xs text-gray-600 mb-1", children: [_jsx("span", { children: "Progress" }), _jsxs("span", { children: [item.completion, "%"] })] }), _jsx("div", { className: "w-full bg-gray-200 rounded-full h-2", children: _jsx("div", { className: "bg-blue-600 h-2 rounded-full transition-all", style: { width: `${item.completion}%` } }) })] })), !compact && (_jsxs("div", { className: "flex items-center gap-4 mt-3 text-xs text-gray-500", children: [item.assignee && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Users, { className: "w-3 h-3" }), _jsx("span", { children: item.assignee.name })] })), item.dueDate && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Calendar, { className: "w-3 h-3" }), _jsx("span", { children: new Date(item.dueDate).toLocaleDateString() })] })), item.estimatedHours && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Timer, { className: "w-3 h-3" }), _jsxs("span", { children: [item.estimatedHours, "h est."] })] })), item.comments.length > 0 && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(FileText, { className: "w-3 h-3" }), _jsxs("span", { children: [item.comments.length, " comments"] })] })), item.subtasks.length > 0 && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(CheckSquare, { className: "w-3 h-3" }), _jsxs("span", { children: [item.subtasks.filter(t => t.completed).length, "/", item.subtasks.length, " subtasks"] })] }))] })), !compact && item.subtasks.length > 0 && (_jsx("div", { className: "mt-3 pl-4 border-l-2 border-gray-200", children: _jsxs("div", { className: "space-y-1", children: [item.subtasks.slice(0, 3).map(subtask => (_jsxs("div", { className: "flex items-center gap-2 text-sm", children: [_jsx("input", { type: "checkbox", checked: subtask.completed, onChange: () => {
                                                        const updatedSubtasks = item.subtasks.map(st => st.id === subtask.id ? { ...st, completed: !st.completed } : st);
                                                        onItemUpdate(item.id, { subtasks: updatedSubtasks });
                                                    }, disabled: readonly, className: "w-3 h-3" }), _jsx("span", { className: subtask.completed ? 'line-through text-gray-500' : 'text-gray-700', children: subtask.title })] }, subtask.id))), item.subtasks.length > 3 && (_jsxs("div", { className: "text-xs text-gray-500", children: ["+", item.subtasks.length - 3, " more subtasks"] }))] }) })), !compact && (_jsxs("div", { className: "flex items-center justify-between mt-3", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsxs(Button, { variant: "ghost", size: "sm", onClick: () => setShowComments(!showComments), children: [_jsx(FileText, { className: "w-4 h-4 mr-1" }), "Comments (", item.comments.length, ")"] }), onSelect && (_jsxs(Button, { variant: "ghost", size: "sm", onClick: onSelect, children: [_jsx(Eye, { className: "w-4 h-4 mr-1" }), isSelected ? 'Hide' : 'Details'] }))] }), canEdit && (_jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Button, { variant: "ghost", size: "sm", children: _jsx(Edit3, { className: "w-4 h-4" }) }), _jsx(Button, { variant: "ghost", size: "sm", onClick: () => onItemDelete(item.id), children: _jsx(Trash2, { className: "w-4 h-4" }) })] }))] })), showComments && (_jsxs("div", { className: "mt-4 pt-4 border-t space-y-3", children: [item.comments.map(comment => (_jsxs("div", { className: "flex gap-3", children: [_jsx("div", { className: "flex-shrink-0", children: _jsx("div", { className: "w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium text-white", style: { backgroundColor: comment.author.color }, children: comment.author.name.charAt(0) }) }), _jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "text-sm font-medium", children: comment.author.name }), _jsx("span", { className: "text-xs text-gray-500", children: new Date(comment.timestamp).toLocaleDateString() })] }), _jsx("p", { className: "text-sm text-gray-700 mt-1", children: comment.content })] })] }, comment.id))), !readonly && (_jsxs("div", { className: "flex gap-3", children: [_jsx("div", { className: "flex-shrink-0", children: _jsx("div", { className: "w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium text-white", style: { backgroundColor: currentUser.color }, children: currentUser.name.charAt(0) }) }), _jsxs("div", { className: "flex-1", children: [_jsx("textarea", { value: newComment, onChange: (e) => setNewComment(e.target.value), placeholder: "Add a comment...", className: "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm h-16" }), _jsx("div", { className: "flex justify-end mt-2", children: _jsx(Button, { variant: "default", size: "sm", onClick: handleCommentSubmit, disabled: !newComment.trim(), children: "Comment" }) })] })] }))] }))] })] }) }) }));
};
