...
export const AdvancedPromptingCollaborationDemo: React.FC<AdvancedPromptingCollaborationDemoProps> = ({ className }) => { const [showPanel, setShowPanel] = useState(false);
  ...
  const getRoleColor = (role: FilmIndustryRole): string => { const colors = {
  ...
  const handleUserSelect = async (user: FilmIndustryUser) => { setSelectedUser(user);
  try {
  ...
  maxCollaborators: 10 }
      ...
      setShowPanel(true);
      setDemoStep(1);
 catch (error) { console.error('Failed to initialize collaboration:', error) };
  const handleMARSRegionCreate = (region: MARSRegionTemplate) => { console.log('MARS region created in demo:', region);
  setDemoStep(prev => Math.max(prev, 2)) };
  const handleZadaPatternCreate = (pattern: ZadaPromptPattern) => { console.log('Zada pattern created in demo:', pattern);
  setDemoStep(prev => Math.max(prev, 3)) };
  const handleVFXExport = (data: Record<string, unknown>) => { setExportData(data);
