...
interface AdvancedPromptingCollaborationPanelProps { collaborationService: AdvancedPromptingCollaborationService;
  ...
  useEffect(() => {
    ...
    const handleSessionCreated = (session: PromptingMethodologySession) => { setSessions(prev => [...prev, session]) };
    const handleMARSRegionCreated = ({ region }: { region: MARSRegionTemplate }) => { setMarsTemplates(prev => [...prev, region]);
      onMARSRegionCreate?.(region) };
    const handleZadaPatternCreated = ({ pattern }: { pattern: ZadaPromptPattern }) => { setZadaPatterns(prev => [...prev, pattern]);
    ...
    return () => { collaborationService.off('session_created', handleSessionCreated);
  ...
  // Session management
  const handleCreateSession = useCallback(async () => { if (!newSessionTitle.trim()) return;
    try {
      ...
      setNewSessionTitle('');
 catch (error) { console.error('Failed to create session:', error) }, [newSessionTitle, selectedMethodology, currentUser.id, collaborationService]);
  const handleJoinSession = useCallback(async (sessionId: string) => { try {
      ...
      if (success) {
        ...
        setCurrentSession(session || null) } catch (error) { console.error('Failed to join session:', error) }, [currentUser.id, collaborationService]);
  // VFX Export
  const handleVFXExport = useCallback(async () => { if (!currentSession) return;
    try {
 ...
 catch (error) { console.error('Failed to generate VFX export:', error) }, [currentSession, currentUser.id, collaborationService, onVFXExport]);
  // Role-based UI customization
  const getRoleIcon = (role: FilmIndustryRole): string => { const icons = {
  director: '🎬',
  vfx_supervisor: '✨',
  pipeline_td: '⚙️',
  vfx_artist: '🎨',
  ...
  const getMethodologyColor = (methodology: PromptingMethodology): string => { const colors = {
