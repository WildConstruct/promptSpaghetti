...
export const ComparisonTools = ({ sessions, activeSessionId, onSessionSelect, _____onSessionCreate, onSessionUpdate, onSessionDelete, onExportReport, className = '' }) => {
    ...
    return (_jsx("div", { className: `comparison-tools ${className}`, children: _jsxs(Card, { className: "comparison-tools-card", children: [_jsxs(CardHeader, { children: [_jsxs("div", { className: "comparison-tools-header", children: [_jsxs(CardTitle, { className: "comparison-tools-title", children: [_jsx(GitBranch, { size: 18 }), "Comparison Tools"] }), _jsxs("div", { className: "comparison-tools-actions", children: [_jsxs(Button, { variant: "outline", size: "sm", onClick: () => { }, children: [_jsx(Plus, { size: 14 }), "New Comparison"] }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => { }, disabled: selectedSessions.length < 2, children: [_jsx(BarChart3, { size: 14 }), "Batch Compare"] })] })] }), _jsxs("div", { className: "comparison-tools-controls", children: [_jsxs("div", { className: "search-and-filter", children: [_jsxs("div", { className: "search-container", children: [_jsx(Search, { size: 14, className: "search-icon" }), _jsx("input", { type: "text", placeholder: "Search comparisons...", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), className: "search-input" })] }), _jsxs(Button, { variant: "outline", size: "sm", onClick: () => { }, children: [_jsx(Filter, { size: 14 }), "Filters"] })] }), _jsxs("div", { className: "view-controls", children: [_jsxs("div", { className: "view-mode-selector", children: [_jsx(Button, { variant: viewMode === 'grid' ? 'primary' : 'ghost', size: "sm", onClick: () => setViewMode('grid'), children: _jsx(Layers, { size: 14 }) }), _jsx(Button, { variant: viewMode === 'list' ? 'primary' : 'ghost', size: "sm", onClick: () => setViewMode('list'), children: _jsx(FileText, { size: 14 }) }), _jsx(Button, { variant: viewMode === 'timeline' ? 'primary' : 'ghost', size: "sm", onClick: () => setViewMode('timeline'), children: _jsx(Clock, { size: 14 }) })] }), _jsxs("select", { value: sortBy, onChange: (e) => setSortBy(e.target.value), className: "sort-select", children: [_jsx("option", { value: "date", children: "Sort by Date" }), _jsx("option", { value: "name", children: "Sort by Name" }), _jsx("option", { value: "similarity", children: "Sort by Similarity" }), _jsx("option", { value: "changes", children: "Sort by Changes" })] })] })] })] }), _jsxs(CardContent, { className: "comparison-tools-content", children: [viewMode === 'grid' && (_jsx("div", { className: "comparison-grid", children: sortedSessions.map(session => (_jsx(ComparisonSessionCard, { session: session, isActive: session.id === activeSessionId, isSelected: selectedSessions.includes(session.id), onSelect: () => onSessionSelect(session.id), onToggleSelection: (selected) => {
                                    setSelectedSessions(prev => selected
                                        ? [...prev, session.id]
                                        : prev.filter(id => id !== session.id));
                                }, onUpdate: (updates) => onSessionUpdate(session.id, updates), onDelete: () => onSessionDelete(session.id), onExport: (format) => onExportReport(session.id, format) }, session.id))) })), viewMode === 'list' && (_jsx("div", { className: "comparison-list", children: sortedSessions.map(session => (_jsx(ComparisonSessionRow, { session: session, isActive: session.id === activeSessionId, isSelected: selectedSessions.includes(session.id), onSelect: () => onSessionSelect(session.id), onToggleSelection: (selected) => {
                                    setSelectedSessions(prev => selected
                                        ? [...prev, session.id]
                                        : prev.filter(id => id !== session.id));
                                }, onUpdate: (updates) => onSessionUpdate(session.id, updates), onDelete: () => onSessionDelete(session.id) }, session.id))) })), viewMode === 'timeline' && (_jsx(ComparisonTimeline, { sessions: sortedSessions, activeSessionId: activeSessionId, onSessionSelect: onSessionSelect }))] })] }) }));
};
const ComparisonSessionCard = ({ session, isActive, isSelected, onSelect, onToggleSelection, onUpdate, onDelete, onExport }) => {
    const [showActions, setShowActions] = useState(false);
    return (_jsxs("div", { className: `comparison-session-card ${isActive ? 'active' : ''} ${isSelected ? 'selected' : ''}`, onClick: onSelect, onMouseEnter: () => setShowActions(true), onMouseLeave: () => setShowActions(false), children: [_jsxs("div", { className: "session-card-header", children: [_jsx("input", { type: "checkbox", checked: isSelected, onChange: (e) => {
                            e.stopPropagation();
...
const ComparisonSessionRow = ({ session, isActive, isSelected, onSelect, onToggleSelection, onUpdate, onDelete }) => {
    return (_jsxs("div", { className: `comparison-session-row ${isActive ? 'active' : ''} ${isSelected ? 'selected' : ''}`, onClick: onSelect, children: [_jsx("div", { className: "row-checkbox", children: _jsx("input", { type: "checkbox", checked: isSelected, onChange: (e) => {
                        e.stopPropagation();
                        onToggleSelection(e.target.checked);
...
const ComparisonTimeline = ({ sessions, activeSessionId, onSessionSelect }) => {
    ...
    return (_jsx("div", { className: "comparison-timeline", children: groupedSessions.map(({ date, sessions }) => (_jsxs("div", { className: "timeline-group", children: [_jsxs("div", { className: "timeline-date", children: [_jsx("h3", { children: date.toLocaleDateString('en-US', {
                                weekday: 'long',
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                            }) }), _jsxs("div", { className: "session-count", children: [sessions.length, " comparisons"] })] }), _jsx("div", { className: "timeline-sessions", children: sessions.map(session => (_jsxs("div", { className: `timeline-session ${session.id === activeSessionId ? 'active' : ''}`, onClick: () => onSessionSelect(session.id), children: [_jsx("div", { className: "timeline-marker" }), _jsxs("div", { className: "session-content", children: [_jsxs("div", { className: "session-header", children: [_jsx("h4", { className: "session-name", children: session.name }), _jsx("div", { className: "session-time", children: session.createdAt.toLocaleTimeString([], {
                                                    hour: '2-digit',
                                                    minute: '2-digit'
                                                }) })] }), _jsxs("div", { className: "session-details", children: [_jsxs("div", { className: "comparison-summary", children: [_jsx("span", { className: "source", children: session.sourceItem.name }), _jsx(ArrowLeftRight, { size: 12 }), _jsx("span", { className: "target", children: session.targetItem.name })] }), _jsxs("div", { className: "session-badges", children: [_jsx(Badge, { variant: "outline", size: "sm", children: session.comparisonType }), _jsx(Badge, { variant: "outline", size: "sm", children: session.sourceItem.type })] })] })] })] }, session.id))) })] }, date.toDateString()))) }));
};
export const AdvancedDiffViewer = ({ comparison, session, onSessionUpdate, onAnnotationAdd, onAnnotationUpdate, onAnnotationDelete, className = '' }) => {
