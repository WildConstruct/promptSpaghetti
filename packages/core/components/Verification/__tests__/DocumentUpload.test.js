...
describe('DocumentUpload', () => {
    ...
    describe('Basic Rendering', () => {
        test('renders upload area with default text', () => {
            render(_jsx(DocumentUpload, { ...defaultProps }));
        ...
        test('renders custom placeholder text', () => {
            render(_jsx(DocumentUpload, { ...defaultProps, placeholder: "Upload your ID documents" }));
            expect(screen.getByText('Upload your ID documents')).toBeInTheDocument();
        });
        test('shows disabled state when disabled prop is true', () => {
            render(_jsx(DocumentUpload, { ...defaultProps, disabled: true }));
    ...
    describe('File Selection', () => {
        ...
        test('validates file sizes', async () => {
            const user = userEvent.setup();
            render(_jsx(DocumentUpload, { ...defaultProps, maxFileSize: 1 })); // 1MB limit
        ...
        test('enforces maximum file count', async () => {
            const user = userEvent.setup();
            render(_jsx(DocumentUpload, { ...defaultProps, maxFiles: 2 }));
            const files = [
    ...
    describe('File Display and Management', () => {
        ...
        test('displays existing files', () => {
            ...
            render(_jsx(DocumentUpload, { ...defaultProps, existingFiles: existingFiles }));
    ...
    describe('Custom Configuration', () => {
        test('respects custom accepted types', async () => {
            const user = userEvent.setup();
            render(_jsx(DocumentUpload, { ...defaultProps, acceptedTypes: ['image/jpeg'] }));
        ...
        test('respects custom max file size', () => {
            render(_jsx(DocumentUpload, { ...defaultProps, maxFileSize: 5 }));
            expect(screen.getByText(/Max 5MB each/)).toBeInTheDocument();
        });
        test('respects custom max files', () => {
            render(_jsx(DocumentUpload, { ...defaultProps, maxFiles: 3 }));
