...
export const VerificationStatusTracker = ({ userId, onRefresh, onRequestVerification }) => {
    ...
    const loadVerificationStatus = useCallback(async () => {
        ...
        try {
            ...
            const items = VERIFICATION_TYPES.map(type => {
    ...
    if (isLoading) {
        return (_jsxs("div", { className: "verification-status-tracker loading", children: [_jsx("div", { className: "loading-spinner", children: "Loading verification status..." }), _jsx("style", { jsx: true, children: `
    ...
    return (_jsxs("div", { className: "verification-status-tracker", children: [_jsxs("div", { className: "tracker-header", children: [_jsxs("div", { className: "header-main", children: [_jsx("h2", { children: "Verification Status" }), _jsx("button", { className: "refresh-btn", onClick: handleRefresh, title: "Refresh status", children: "\uD83D\uDD04 Refresh" })] }), _jsxs("div", { className: "progress-summary", children: [_jsx("div", { className: "progress-bar", children: _jsx("div", { className: "progress-fill", style: { width: `${completionPercentage}%` } }) }), _jsxs("div", { className: "progress-text", children: [completedCount, " of ", totalCount, " verifications completed (", completionPercentage, "%)"] })] }), _jsxs("div", { className: "last-updated", children: ["Last updated: ", lastRefresh.toLocaleTimeString()] })] }), _jsx("div", { className: "verification-items", children: verificationItems.map((item) => (_jsxs("div", { className: "verification-item", children: [_jsxs("div", { className: "item-header", children: [_jsxs("div", { className: "item-title", children: [_jsx("span", { className: "status-icon", children: getStatusIcon(item.status) }), _jsxs("div", { className: "title-text", children: [_jsx("h3", { children: item.title }), _jsxs("span", { className: "priority-badge", style: { backgroundColor: getPriorityColor(item.priority) }, children: [item.priority, " priority"] })] })] }), _jsx("div", { className: "status-badge", style: {
                                        color: getStatusColor(item.status),
                                        borderColor: getStatusColor(item.status)
                                    }, children: getStatusText(item.status) })] }), _jsx("div", { className: "item-description", children: item.description }), item.status !== 'not_started' && (_jsxs("div", { className: "status-details", children: [item.submittedAt && (_jsxs("div", { className: "detail-item", children: [_jsx("strong", { children: "Submitted:" }), " ", item.submittedAt.toLocaleString()] })), item.requestId && (_jsxs("div", { className: "detail-item", children: [_jsx("strong", { children: "Request ID:" }), " ", item.requestId] })), item.estimatedCompletion && (_jsxs("div", { className: "detail-item", children: [_jsx("strong", { children: "Est. Completion:" }), " ", item.estimatedCompletion] }))] })), item.nextSteps && item.nextSteps.length > 0 && (_jsxs("div", { className: "next-steps", children: [_jsx("strong", { children: "Next Steps:" }), _jsx("ul", { children: item.nextSteps.map((step, index) => (_jsx("li", { children: step }, index))) })] })), _jsxs("div", { className: "item-actions", children: [item.status === 'not_started' && onRequestVerification && (_jsx("button", { className: "btn btn-primary", onClick: () => onRequestVerification(item.type), children: "Start Verification" })), item.status === 'requires_update' && onRequestVerification && (_jsx("button", { className: "btn btn-warning", onClick: () => onRequestVerification(item.type), children: "Update Information" })), item.status === 'rejected' && onRequestVerification && (_jsx("button", { className: "btn btn-secondary", onClick: () => onRequestVerification(item.type), children: "Resubmit" }))] })] }, item.type))) }), _jsx("style", { jsx: true, children: `
...
function getEstimatedCompletion(status, type) {
