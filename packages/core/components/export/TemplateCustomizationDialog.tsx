...
interface TemplateCustomizationDialogProps { template: ExportTemplate;
  visible?: boolean;
  onClose?: () => void;
...
interface ParameterDefinition { key: string;
  label: string;
...
interface CustomizationState { parameters: Record<string, any>;
...
export const TemplateCustomizationDialog: React.FC<TemplateCustomizationDialogProps> = ({ )
  ...
  const parameterDefinitions = useMemo(() => { const formatParams = DEFAULT_PARAMETERS[template.export_format] || [];
    ...
    return formatParams }, [template.export_format, activeCategory]);
  // Get parameters grouped by category
  const categorizedParameters = useMemo(() => { const grouped = new Map<string, ParameterDefinition>();
  ...
  if (!(param.key in customization.parameters)) {
    ...
    if (Object.keys(defaultParams).length > 0) { setCustomization(prev => ({)
  ...prev }
  ...
  const handleParameterChange = useCallback((key: string, value: Error) => { const param = parameterDefinitions.find(p => p.key === key);
    if (!param) return;
  ...
  // Generate preview
  const generatePreview = useCallback(async () => { if (!onPreview) return;
  setLoading(true);
  try {
  ...
  format_options: customization.parameters }
      ...
      setShowPreview(true);
      setError(null);
 catch (err) { setError(err instanceof Error ? err.message : 'Failed to generate preview') } finally { setLoading(false) }, [template, customization.parameters, onPreview, previewTemplate]);
  // Save customization
  ...
  const error = validateParameter(param, customization.parameters[param.key]);
    ...
    if (Object.values(errors).some(e => e)) {
      ...
      return;
    setLoading(true);
    try { const customizedTemplate = {
  ...template
  format_options: customization.parameters
  custom_fields: customization.customFields }
};
      if (onSave) { onSave(customizedTemplate) } else { await updateTemplate(template.id, {)
  format_options: customization.parameters
  custom_fields: customization.customFields }
});
      setCustomization(prev => ({ ...prev, isDirty: false }));
      setError(null);
 catch (err) { setError(err instanceof Error ? err.message : 'Failed to save customization') } finally { setLoading(false) }, [template, customization, parameterDefinitions, validateParameter, onSave, updateTemplate]);
  // Render parameter input
  const renderParameterInput = useCallback((param: ParameterDefinition) => { const value = customization.parameters[param.key];
          ...
          <textarea
            ...
            onChange={ (e) => {
              try {
                ...
                handleParameterChange(param.key, parsed) } catch { handleParameterChange(param.key, e.target.value) }}
            placeholder={param.description}
            rows={4}
            style={ {
  ...baseInputStyle
