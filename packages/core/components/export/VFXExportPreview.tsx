...
interface VFXExportPreviewProps { exportData: CreateExportJob;
  onValidationComplete?: (isValid: boolean, results: VFXValidationResult | null) => void }



export const VFXExportPreview: React.FC<VFXExportPreviewProps> = ({ exportData, onValidationComplete }) => { const [previewData, setPreviewData] = useState<Partial<VFXExportFormat> | null>(null);
  ...
  useEffect(() => {
    generatePreview() }, [exportData]);

  const generatePreview = async () => { setLoading(true);
    try {
 ...
 catch (error) { console.error('Preview generation failed:', error);
  ...
  if (!previewData || !validation) { return <div className="text-center p-8 text-gray-500 dark:text-gray-400">Failed to generate preview</div> }
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            { label: 'ControlNet Pipeline', compatible: validation.compatibility.controlNet }].map(item => (
      ...
      <div className="border-b border-gray-200 dark:border-gray-700 mb-6">
        <nav className="flex space-x-8">
          {[
            ...
            { id: 'json', label: 'JSON', icon: FiCode }].map(tab => (
            <button
              key={tab.id}
