...
export const NodeRenderer = memo(({ id, data, selected = false, onSelect, getNodeMeta, getCategoryColor }: { id: string, data: any, selected?: boolean, onSelect: (id: string) => void, getNodeMeta: (nodeType: string) => any, getCategoryColor: (category: string) => string }) => {
    ...
    try {
        ...
        return (_jsxs("div", { ref: nodeRef, role: "button", "data-testid": `node-${id}`, tabIndex: 0, ...hoverProps, className: `animate-node-hover animate-node-select ${selected ? 'selected' : ''} ${isDeleting ? 'animate-node-delete' : ''}`, onClick: (e) => {
                e.stopPropagation();
                onSelect(id);
            }, onKeyDown: (e) => {
            ...
            var(--bg-tertiary) 0%, var(--bg-secondary) 50%, var(--bg-tertiary) 100%)`,
                ...
                return summary ? `${label}. ${summary}` : label;
                        ...
                        background: 'linear-gradient(180deg, rgba(255,255,255,0.02) 0%, rgba(255,255,255,0.005) 100%)'
                                ...
                                fontFamily: 'var(--font-mono)' // Monospace for technical properties
                                    ...
                                    opacity: 0.8
                                            ...
                                            fontWeight: 500
                                        }, children: [k, ":"] }), ' ', _jsx("span", { style: { color: 'var(--text-secondary)' }, children: String(v).length > 22 ? String(v).slice(0, 22) + '…' : String(v) })] }, k))) }))] }), _jsx(Handle, { type: "target", position: Position.Left, id: "target", className: "animate-hover-scale", style: {
                        width: 14,
                        height: 14,
                        borderRadius: '50%',
                    ...
                    }, isConnectable: true }), _jsx(Handle, { type: "source", position: Position.Right, id: "source", className: "animate-hover-scale", style: {
                        width: 14,
                        height: 14,
                        borderRadius: '50%',
