...
export const BatchControls: React.FC<BatchControlsProps> = ({ )
  ...
  const handleOutputFormatChange = useCallback((outputFormat: 'individual' | 'combined' | 'csv' | 'json') => { onChange({)
  ...settings }
      outputFormat
    });
  }, [settings, onChange]);
  // Handle naming pattern change
  const handleNamingPatternChange = useCallback((namingPattern: string) => { onChange({)
  ...settings }
      namingPattern
    });
  }, [settings, onChange]);
  // Handle metadata toggle
  const handleIncludeMetadataChange = useCallback((includeMetadata: boolean) => { onChange({)
  ...settings }
      includeMetadata
    });
  }, [settings, onChange]);
  // Handle auto-download toggle
  const handleAutoDownloadChange = useCallback((autoDownload: boolean) => { onChange({)
  ...settings }
      autoDownload
    });
  }, [settings, onChange]);
