...
export const PerformanceControls: React.FC<PerformanceControlsProps> = ({ )
  ...
  const handleEnableCachingChange = useCallback((enableCaching: boolean) => { onChange({)
  ...settings }
      enableCaching
    });
  }, [settings, onChange]);
  // Handle memory usage toggle
  const handleShowMemoryUsageChange = useCallback((showMemoryUsage: boolean) => { onChange({)
  ...settings }
      showMemoryUsage
    });
  }, [settings, onChange]);
  // Handle execution logging toggle
  const handleLogExecutionStepsChange = useCallback((logExecutionSteps: boolean) => { onChange({)
  ...settings }
      logExecutionSteps
    });
  }, [settings, onChange]);
