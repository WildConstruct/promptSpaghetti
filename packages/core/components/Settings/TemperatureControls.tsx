...
export const TemperatureControls: React.FC<TemperatureControlsProps> = ({ )
  ...
  const handleValueChange = useCallback((value: number) => { onChange({)
  ...settings
  ...
  const handleShowIndicatorChange = useCallback((showIndicator: boolean) => { onChange({)
  ...settings }
      showIndicator
    });
  }, [settings, onChange]);
  // Get temperature description
  const getTemperatureDescription = (temp: number): string => { if (temp < 0.5) return 'Very Conservative - Highly predictable results';
