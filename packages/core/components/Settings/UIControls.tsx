...
export const UIControls: React.FC<UIControlsProps> = ({ )
  ...
  const handleShowTooltipsChange = useCallback((showTooltips: boolean) => { onChange({)
  ...settings }
      showTooltips
    });
  }, [settings, onChange]);
  // Handle keyboard shortcuts toggle
  const handleEnableKeyboardShortcutsChange = useCallback((enableKeyboardShortcuts: boolean) => { onChange({)
  ...settings }
      enableKeyboardShortcuts
    });
  }, [settings, onChange]);
  // Handle animations toggle
  const handleReduceAnimationsChange = useCallback((reduceAnimations: boolean) => { onChange({)
  ...settings }
      reduceAnimations
    });
  }, [settings, onChange]);
  // Handle high contrast toggle
  const handleHighContrastChange = useCallback((highContrast: boolean) => { onChange({)
  ...settings }
      highContrast
    });
  }, [settings, onChange]);
