...
export const DragReorderDemo: React.FC<DragReorderDemoProps> = ({ )
  ...
  React.useEffect(() => { setDemoOptions(scenarios[activeScenario as keyof typeof scenarios].options) }, [activeScenario]);
  // Handle scenario change
  const handleScenarioChange = useCallback((scenarioId: string) => { setActiveScenario(scenarioId);
    setDemoOptions(scenarios[scenarioId as keyof typeof scenarios].options) }, []);
  // Handle options change
  const handleOptionsChange = useCallback((newOptions: WeightedOption) => { setDemoOptions(newOptions) }, []);
  // Generate preview results
  const [previewResults, setPreviewResults] = useState<string>([]);
  const [isGenerating, setIsGenerating] = useState(false);
  const generatePreview = useCallback(async () => { setIsGenerating(true);
  ...
  setIsGenerating(false) }, [demoOptions]);
  // Calculate statistics
  const statistics = useMemo(() => { const totalWeight = demoOptions.reduce((sum, opt) => sum + opt.weight, 0);
...
function calculateEvenness(options: WeightedOption): number { if (options.length === 0) return 0;
  ...
  const variance = weights.reduce((sum, weight) => {
    return sum + Math.pow(weight - expectedWeight, 2) }, 0) / weights.length;
