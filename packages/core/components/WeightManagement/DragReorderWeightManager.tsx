...
export interface WeightStatistics { totalWeight: number;
...
function calculateWeightStatistics(options: WeightedOption): WeightStatistics { if (options.length === 0) {
  return {
  totalWeight: 0,
  averageWeight: 0,
  medianWeight: 0,
 ...
 else { // Simple bimodal detection: check if there are distinct clusters,
...
export const DragReorderWeightManager: React.FC<DragReorderProps> = ({ )
  ...
  // Handle drag start
  const handleDragStart = useCallback((event: DragStartEvent) => { setIsDragging(true);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) { const oldIndex = options.findIndex(option => option.id === active.id);
      const newIndex = options.findIndex(option => option.id === over.id);
      onChange(arrayMove(options, oldIndex, newIndex)) }, [options, onChange]);
  ...
  const handleWeightChange = useCallback((optionId: string, newWeight: number) => {
    ...
    if (option && onWeightChange) { const newTotal = newOptions.reduce((sum, opt) => sum + opt.weight, 0);
  const percentage = newTotal > 0 ? (newWeight / newTotal) * 100 : 0;
  onWeightChange(optionId, newWeight, percentage);
  onChange(newOptions) }, [options, onChange, onWeightChange, minWeight, maxWeight]);
  // Handle lock toggle
  ...
  // Bulk operations
  const handleBulkWeightChange = useCallback((operation: 'normalize' | 'equal' | 'random' | 'clear') => { let newOptions = [...options];
  switch (operation) {
  case 'normalize':,
  ...
  const handleItemSelect = useCallback((optionId: string, isSelected: boolean) => { const newSelected = new Set(selectedItems);
    if (isSelected) {
      newSelected.add(optionId) } else { newSelected.delete(optionId);
    setSelectedItems(newSelected);
    setShowBulkActions(newSelected.size > 0) }, [selectedItems]);
  // Theme styles
  ...
  borderRadius: '8px'
      ...
      {/* Bulk Actions Panel */}
      { showBulkActions && enableBulkOperations && (<div style={{)
          background: themeStyles.hover }
          border: `1px solid ${themeStyles.border}`}
