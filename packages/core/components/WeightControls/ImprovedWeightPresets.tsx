...
export interface WeightPreset {
  ...
  description: string;
  icon: string;
  pattern: (count: number) => number[];
}

// Raw weight presets - NO normalization to 100
const RAW_WEIGHT_PRESETS: WeightPreset[] = [
  {
    id: 'equal',
    name: 'Equal',
  ...
  },
  {
    ...
    pattern: (count) => {
      if (count === 1) return [50];
      if (count === 2) return [60, 60];
      return Array(count).fill(0).map((_, i) => {
        if (i === 0 || i === count - 1) return 70;
        return 20;
      });
    }
  }
...
interface ImprovedWeightPresetsProps {
...
export const ImprovedWeightPresets: React.FC<ImprovedWeightPresetsProps> = ({
  options,
  onApplyPreset,
  compact = false,
  className = ''
  ...
  const calculatePercentages = (weights: number[]): string[] => {
  ...
  return (
    <div className={`weight-presets-full ${className}`} style={{
      background: '#2d3748',
      borderRadius: '8px',
      padding: '16px',
      color: '#e2e8f0'
      ...
      <div style={{
        ...
        <strong>Raw Weight System:</strong> Weights shown are raw values (0-100). 
        The actual probability percentages are calculated from these weights and shown in blue.
        This makes it more intuitive to set relative importance without worrying about normalization.
      </div>
    </div>
  );
};
