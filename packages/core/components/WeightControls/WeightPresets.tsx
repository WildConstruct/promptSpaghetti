...
export interface WeightPreset { id: string;
  name: string;
  description: string;
...
export const BUILT_IN_PRESETS: WeightPreset = [
  ...
  for (let i = 0; i < totalSteps; i++) { // Linear decrease from high to low
  ...
  const weights: number = [];
  const totalSteps = options.length;
  for (let i = 0; i < totalSteps; i++) { // Linear increase from low to high
  ...
  for (let i = 0; i < options.length; i++) { // Gaussian-like distribution
  ...
  for (let i = 0; i < options.length; i++) { const weight = Math.round(Math.pow(base, i) * 50 + 5);
  weights.push(weight);
  // Normalize to 100%
  const sum = weights.reduce((a, b) => a + b, 0);
  return weights.map(w => Math.round((w / sum) * 100)) }
  ...
  const fib = [1, 1];
  for (let i = 2; i < options.length; i++) { fib[i] = fib[i-1] + fib[i-2];
  // Normalize to 100%
  ...
  const handleApplyPreset = (preset: WeightPreset) => { const newWeights = preset.pattern(options);
    onApplyPreset(newWeights) };
  const handleSaveCurrentWeights = () => { if (!customPresetName.trim()) return;
  ...
  const resetToEqual = () => { const equalPreset = BUILT_IN_PRESETS.find(p => p.id === 'equal');
    if (equalPreset) {
