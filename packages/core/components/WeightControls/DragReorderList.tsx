...
export interface DragReorderListProps { options: WeightControlOption;
...
interface DragState { isDragging: boolean;
...
export const DragReorderList: React.FC<DragReorderListProps> = ({ )
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent, index: number) => { if (disabled || event.button !== 0) return; // Only left mouse button
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => { if (!dragStartRef.current || dragState.isDragging) return;
  ...
  const handleDragOver = useCallback((event: MouseEvent) => { if (!dragState.isDragging || !listRef.current) return;
  event.preventDefault();
  ...
  y: event.clientY - prev.dragOffset.y }
    ...
    let dropIndex = -1;
    for (let i = 0; i < itemRefs.current.length; i++) { const itemEl = itemRefs.current[i];
  if (!itemEl) continue;
  ...
  const handleMouseUp = useCallback(() => { if (dragState.isDragging && dragState.draggedIndex !== null && dragState.dragOverIndex !== null) {
  ...
  React.useEffect(() => { if (dragStartRef.current || dragState.isDragging) {
  ...
  const handleTouchStart = useCallback((event: React.TouchEvent, index: number) => { if (disabled || event.touches.length !== 1) return;
  ...
  const handleTouchMove = useCallback((event: TouchEvent) => { if (!dragStartRef.current || event.touches.length !== 1) return;
  const touch = event.touches[0];
