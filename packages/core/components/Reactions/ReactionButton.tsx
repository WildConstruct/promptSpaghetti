...
export interface ReactionButtonProps { contentId: string;
  contentType: 'template' | 'comment' | 'review' | 'project' | 'user';
  userId?: string;
...
export const ReactionButton: React.FC<ReactionButtonProps> = ({ )
  ...
  useEffect(() => { loadReactionSummary() }, [contentId, userId]);
  const loadReactionSummary = async () => { try {
      // In real implementation, this would call the API
      // For now, we'll simulate reaction data
      const mockSummary = generateMockSummary(contentId, userId);
      setSummary(mockSummary);
      if (onSummaryUpdate) {
        onSummaryUpdate(mockSummary) } catch (err) { console.error('Failed to load reaction summary:', err) };
  const handleReactionClick = useCallback(async (reactionType: string) => { if (disabled || !userId || loading) return;
    setLoading(true);
    setError(null);
  ...
  reaction: ${err.message}`);}
      ...
      setTimeout(() => setAnimatingReaction(null), 300) }, [contentId, contentType, userId, summary, onReaction, onSummaryUpdate, disabled, loading]);
  const togglePicker = useCallback(() => { if (disabled || !userId) return;
    setShowPicker(!showPicker) }, [disabled, userId, showPicker]);
  // Component sizing
  ...
  const renderReactionPicker = () => { if (!showPicker) return null;
  return;
  <div style={{
  position: 'absolute'
  bottom: '100%'
  ...
  const renderContent = () => { switch (variant) {
  case 'minimal':,
...
function generateMockSummary(contentId: string, userId?: string): ReactionSummary {
  ...
  const topReactions = Object.entries(reactionCounts);
    .map(([type, count]) => { const reactionDef = DEFAULT_REACTIONS.find(r => r.id === type);
  return {
  type
  ...
  const sentimentScore = Object.entries(reactionCounts);
    .reduce((score, [type, count]) => { const reactionDef = DEFAULT_REACTIONS.find(r => r.id === type);
      return score + (reactionDef?.weight || 0) * count }, 0) / Math.max(totalReactions, 1);
...
function updateSummaryAfterReaction(currentSummary: ReactionSummary)
  ...
  if (isRemoving) { // Remove reaction
    if (newCounts[reactionType] > 0) {
      newCounts[reactionType]--;
      if (newCounts[reactionType] === 0) {
        delete newCounts[reactionType] } else { // Add reaction (remove previous if exists)
