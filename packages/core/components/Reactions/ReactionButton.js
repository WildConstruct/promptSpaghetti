...
export const ReactionButton = ({ contentId, contentType, userId, onReaction, onSummaryUpdate, variant = 'compact', size = 'medium', showCounts = true, showLabels = false, disabled = false, maxReactions = 10, className = '' }) => {
    // State management
    ...
    const renderReactionPicker = () => {
        ...
        return (_jsx("div", { style: {
                ...
                gap: '4px',
                minWidth: '200px'
            }, children: DEFAULT_REACTIONS.map(reaction => (_jsx("button", { onClick: () => handleReactionClick(reaction.id), disabled: loading, title: reaction.description, style: {
                    padding: '8px',
                    ...
                    }, children: reaction.emoji }) }, reaction.id))) }));
    };
    // Render compact variant (most common)
    const renderCompactVariant = () => (_jsxs("div", { style: {
            position: 'relative',
            display: 'flex',
            alignItems: 'center',
            gap: currentSize.gap,
            fontSize: currentSize.fontSize
        }, children: [topReactions.length > 0 && (_jsx("div", { style: {
                    display: 'flex',
                    alignItems: 'center',
                    gap: '2px'
                    ...
                    return (_jsxs("button", { onClick: () => handleReactionClick(reaction.type), disabled: disabled || !userId || loading, title: `${reactionDef?.description} (${reaction.count})`, style: {
                            display: 'flex',
                            alignItems: 'center',
                            gap: '3px',
                            ...
                            e.currentTarget.style.backgroundColor = isUserReaction ? '#eff6ff' : 'transparent';
                                    ...
                                    transition: 'transform 0.3s ease'
                                }, children: reactionDef?.emoji }), showCounts && reaction.count > 0 && (_jsx("span", { style: {
                                    fontSize: `calc(${currentSize.fontSize} * 0.9)`,
                                    fontWeight: '500',
                                    color: isUserReaction ? '#3b82f6' : '#6b7280'
                                }, children: reaction.count })), showLabels && (_jsx("span", { style: {
                                    fontSize: `calc(${currentSize.fontSize} * 0.85)`,
                                    color: isUserReaction ? '#3b82f6' : '#6b7280'
                                }, children: reactionDef?.label }))] }, reaction.type));
                }) })), userId && (_jsx("button", { onClick: togglePicker, disabled: disabled || loading, title: "Add reaction", style: {
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    padding: currentSize.padding,
                ...
                }, children: loading ? '⏳' : '😊' })), renderReactionPicker(), showCounts && summary.totalReactions > 0 && (_jsx("span", { style: {
                    fontSize: `calc(${currentSize.fontSize} * 0.9)`,
                    color: '#6b7280',
                    fontWeight: '500'
                }, children: summary.totalReactions > 0 && `+${summary.totalReactions}` }))] }));
    // Render minimal variant (just emoji count)
    const renderMinimalVariant = () => (_jsxs("div", { style: {
            display: 'flex',
            alignItems: 'center',
            gap: '4px',
            fontSize: currentSize.fontSize
        }, children: [topReactions.slice(0, 3).map(reaction => {
                const reactionDef = DEFAULT_REACTIONS.find(r => r.id === reaction.type);
                return (_jsxs("span", { title: `${reactionDef?.description} (${reaction.count})`, style: {
                        display: 'flex',
                        alignItems: 'center',
                        gap: '2px',
                        fontSize: currentSize.emojiSize
            ...
            }), summary.totalReactions > topReactions.length && (_jsxs("span", { style: {
                    fontSize: `calc(${currentSize.fontSize} * 0.8)`,
                    color: '#9ca3af'
                }, children: ["+", summary.totalReactions - topReactions.reduce((sum, r) => sum + r.count, 0)] }))] }));
    // Render picker variant (always show all options)
    const renderPickerVariant = () => (_jsx("div", { style: {
            display: 'flex',
            flexWrap: 'wrap',
            gap: '4px',
            padding: '8px',
            ...
            return (_jsxs("button", { onClick: () => handleReactionClick(reaction.id), disabled: disabled || !userId || loading, title: reaction.description, style: {
                    display: 'flex',
                    alignItems: 'center',
                    gap: '4px',
                    padding: '6px 8px',
                    ...
                    cursor: disabled || !userId || loading ? 'not-allowed' : 'pointer',
                    transition: 'all 0.2s ease'
                }, children: [_jsx("span", { style: { fontSize: currentSize.emojiSize }, children: reaction.emoji }), showLabels && (_jsx("span", { style: {
                            fontSize: `calc(${currentSize.fontSize} * 0.9)`,
                            color: isUserReaction ? '#3b82f6' : '#374151'
                        }, children: reaction.label })), showCounts && count > 0 && (_jsx("span", { style: {
                            fontSize: `calc(${currentSize.fontSize} * 0.8)`,
                            fontWeight: '600',
                            color: isUserReaction ? '#3b82f6' : '#6b7280',
    ...
    return (_jsx("div", { className: `reaction-button ${className}`, style: { position: 'relative' }, children: renderContent() }));
};
// Helper functions
...
function updateSummaryAfterReaction(currentSummary, reactionType, isRemoving) {
