...
export const ReviewDashboard = ({ onReviewSelect, onAssignmentAction, className = '' }) => {
    // State management
    ...
    const filteredReviews = useMemo(() => {
        ...
        if (filters.searchQuery) {
            ...
            filtered = filtered.filter(review => review.title.toLowerCase().includes(query) ||
    ...
    if (loading) {
        return (_jsx("div", { className: `review-dashboard loading ${className}`, children: _jsxs("div", { className: "loading-spinner", children: [_jsx(RefreshCw, { className: "animate-spin", size: 24 }), _jsx("span", { children: "Loading dashboard..." })] }) }));
    }
...
// Queue Tab Component
const QueueTab = ({ reviews, filters, onFilterChange, onReviewSelect, onAssignmentAction }) => {
    return (_jsxs("div", { className: "queue-tab", children: [_jsx(ReviewFilters, { filters: filters, onFilterChange: onFilterChange }), _jsx(ReviewList, { reviews: reviews, onReviewSelect: onReviewSelect, onAssignmentAction: onAssignmentAction })] }));
};
// Supporting Components
const SummaryCard = ({ title, value, icon, trend, className = '' }) => {
    return (_jsxs("div", { className: `summary-card ${className}`, children: [_jsxs("div", { className: "card-header", children: [_jsx("span", { className: "card-icon", children: icon }), _jsx("span", { className: "card-title", children: title })] }), _jsx("div", { className: "card-value", children: value }), trend && (_jsxs("div", { className: `card-trend ${trend.direction}`, children: [trend.direction === 'up' ? _jsx(ArrowUp, { size: 12 }) : _jsx(ArrowDown, { size: 12 }), _jsx("span", { children: Math.abs(trend.value) })] }))] }));
};
const PrioritySection = ({ title, icon, reviews, onReviewSelect, className = '' }) => {
    return (_jsxs("div", { className: `priority-section ${className}`, children: [_jsxs("div", { className: "section-header", children: [icon, _jsx("h3", { children: title }), _jsxs("span", { className: "count", children: ["(", reviews.length, ")"] })] }), _jsx("div", { className: "section-content", children: reviews.map(review => (_jsx(ReviewSummaryCard, { review: review, onClick: () => onReviewSelect(review) }, review.reviewId))) })] }));
};
const ReviewSummaryCard = ({ review, onClick }) => {
