...
export interface MonitoringViewConfig { layout: 'executive' | 'operational' | 'analytics' | 'compliance' }
  ...
  interface MonitoringInterfaceProps { userRole: string;
  userId: string;
...
export const MonitoringInterface: React.FC<MonitoringInterfaceProps> = ({ )
  ...
  const fetchMetrics = useCallback(async () => { try {
  ...
  setIsLoading(false) }, []);
  // Fetch alerts
  const fetchAlerts = useCallback(async () => { try {
      const response = await fetch('/api/system/alerts/active');
      const alertsData = await response.json();
      setAlerts(alertsData) } catch (error) { console.error('Failed to fetch alerts:', error) }, []);
  // Handle alert actions
  const handleAlertAction = useCallback((alertId: string, action: string) => { onAlertAction?.(alertId, action);
  ...
  // Auto-refresh effect
  useEffect(() => { const config = viewConfigs[currentView];
    const interval = setInterval(() => {
      fetchMetrics();
      fetchAlerts() }, config.refreshInterval);
    // Initial fetch
    fetchMetrics();
    fetchAlerts();
  ...
  const getSeverityColor = (severity: AlertData['severity']) => { switch (severity) {
  ...
  const AlertSummary = () => { const criticalAlerts = alerts.filter(a => a.severity === 'critical' && !a.resolved).length;
  ...
  const KeyMetrics = () => { if (!metrics) return <div>Loading metrics...</div>;
  return;
  <div style={{
  backgroundColor: '#FFFFFF'
  borderRadius: '8px'
