...
export interface RecommendationMetrics { overall_performance: {
    ...
    algorithm_performance: Record<string, { precision: number;
        recall: number;
        f1_score: number;
        latency_ms: number;
        cache_hit_rate: number }>;
    featured_content_performance: { impressions: number;
        clicks: number;
        conversions: number;
        revenue: number;
    ...
    real_time_stats: { current_recommendations_per_minute: number;
        active_users: number;
