...
export interface RecommendationAlgorithm { id: string;
  name: string;
...
export interface FeaturedContentConfig { algorithm_weights: { }
...
export interface RecommendationMetrics { overall_performance: { }
...
export interface RecommendationEngineAdminProps { className?: string }



export const RecommendationEngineAdmin: React.FC<RecommendationEngineAdminProps> = ({ className = '' }) => { const [activeTab, setActiveTab] = useState('algorithms');
  ...
  const [metrics] = useState<RecommendationMetrics>({ overall_performance: {
  ...
  revenue_impact: 284750 }
    ...
    algorithm_performance: { 'collaborative-filtering': {
  precision: 0.78
  recall: 0.65
  f1_score: 0.71
  latency_ms: 45
  cache_hit_rate: 0.89 }
      ...
      'content-based': { precision: 0.71
  recall: 0.58
  f1_score: 0.64
  latency_ms: 32
  cache_hit_rate: 0.92 }

      'deep-learning': { precision: 0.85
  recall: 0.72
  f1_score: 0.78
  latency_ms: 125
  cache_hit_rate: 0.76 }

      'performance-based': { precision: 0.82
  recall: 0.69
  f1_score: 0.75
  latency_ms: 28
  cache_hit_rate: 0.95 }
  ...
  const getAlgorithmTypeIcon = (type: string) => { switch (type) {
                ...
                <div className="algorithm-header">
                  ...
                  <div className="algorithm-controls">
                    ...
                    <label className="enable-switch">
                      <input type="checkbox" checked={algorithm.enabled} readOnly />
                      <span>Enabled</span>
                    </label>
                  </div>
                </div>
                  ...
                  <div className="metrics-grid">
                    <div className="metric-item">
                      ...
                      <span className="metric-value">
                        {(algorithm.performance_metrics.precision * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="metric-item">
                    ...
                    <div className="metric-item">
                      ...
                      <span className="metric-value">
                        {algorithm.performance_metrics.click_through_rate.toFixed(1)}%
                      </span>
                    </div>
                    <div className="metric-item">
            ...
            <div className="weights-grid">
              {Object.entries(featuredConfig.algorithm_weights).map(([key, value]) => (
                <div key={key} className="weight-control">
                  <label className="weight-label">{key.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}</label>
            ...
            <CardContent>
              ...
              <div className="metric-content">
                <div className="metric-value">
                  {metrics.overall_performance.total_recommendations_served.toLocaleString()}
                </div>
              ...
              <div className="stat-item">
                ...
                <div className="stat-content">
                  <div className="stat-value">
                    {metrics.real_time_stats.current_recommendations_per_minute.toLocaleString()}
                  </div>
  ...
  align-items: flex-start }
        .header-info h2 { font-size: 1.875rem;
  font-weight: 700;
  color: #1f2937;
  ...
  font-size: 1rem }
        ...
        .metrics-section { display: flex;
  flex-direction: column;
  gap: 1rem }
        .algorithms-header
        .config-header
        .metrics-header { display: flex;
  justify-content: space-between;
  align-items: center }
        .algorithms-header h3
        .config-header h3
        .metrics-header h3 { font-weight: 600;
  color: #1f2937;
  margin: 0 }
  ...
  flex-direction: column;
  gap: 1rem }
        .algorithm-card .card-content { padding-top: 0 }
        .algorithm-header { display: flex;
  justify-content: space-between;
  align-items: flex-start }
  gap: 1rem;

        .algorithm-info { flex: 1 }
        .algorithm-title { display: flex;
  align-items: center;
  gap: 0.5rem;
  margin-bottom: 0.5rem }
        .algorithm-title h4 { font-weight: 600;
  color: #1f2937;
  margin: 0 }
        .algorithm-info p { color: #6b7280;
  font-size: 0.875rem }
  margin: 0;

        .algorithm-controls { display: flex;
  flex-direction: column;
  gap: 0.75rem;
  align-items: flex-end }
        .weight-control { display: flex;
  flex-direction: column;
  gap: 0.25rem;
  align-items: center }
        .weight-control label { font-size: 0.875rem }
  color: #374151;

        .weight-slider { width: 80px }
        .enable-switch { display: flex;
  align-items: center;
  gap: 0.5rem;
  font-size: 0.875rem }
  color: #374151;
  cursor: pointer;
        ...
        .algorithm-metrics { margin-bottom: 1rem }
  ...
  font-weight: 600;
        ...
        .algorithm-actions { display: flex;
  gap: 0.5rem;
  justify-content: flex-end }
        .config-sections { display: flex;
  flex-direction: column;
  gap: 1rem }
        .config-section .card-title { display: flex;
  align-items: center;
  gap: 0.5rem }
        .weights-grid { display: grid;
  ...
  color: #374151 }
        .config-input { padding: 0.5rem 0.75rem }
  border: 1px solid #d1d5db;
  border-radius: 6px;
  font-size: 0.875rem;

        .category-boosts { display: flex;
  flex-direction: column;
  gap: 0.5rem }
        .category-boost-item { display: flex;
  align-items: center;
  gap: 0.75rem }
  ...
  text-transform: capitalize }
        .boost-input { width: 80px;
  padding: 0.25rem 0.5rem;
  border: 1px solid #d1d5db;
  border-radius: 4px;
  font-size: 0.875rem }
        .ab-testing-config { display: flex;
  flex-direction: column;
  gap: 1rem }
        .ab-header { display: flex;
  justify-content: space-between;
  align-items: center;
  padding-bottom: 1rem;
  ...
  color: #374151;
  cursor: pointer }
        .traffic-allocation { display: flex;
  flex-direction: column;
  gap: 0.25rem;
  align-items: center }
        .traffic-allocation label { font-size: 0.875rem }
  color: #374151;

        .traffic-slider { width: 120px }
        .variants-list h5 { font-weight: 600;
  color: #1f2937;
  margin: 0 0 0.75rem 0 }
        .variant-item { display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0.75rem;
  ...
  border-radius: 6px;
  margin-bottom: 0.5rem }
        .variant-info h6 { font-weight: 600;
  color: #1f2937;
  margin: 0 0 0.25rem 0 }
        .variant-allocation { font-size: 0.875rem }
  color: #6b7280;

        .variant-metrics { display: flex;
  gap: 1rem }
        .variant-metric span { font-size: 0.875rem }
  color: #374151;
  font-weight: 500;

        .metrics-overview { margin-bottom: 1.5rem }
        .overview-cards { display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 1rem }
        .metric-card .card-content { padding: 1.5rem }
        .metric-header { display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 1rem }
        .metric-trend { display: flex;
  align-items: center;
  gap: 0.25rem;
  font-size: 0.75rem;
  font-weight: 500;
  ...
  margin-top: 0.5rem;
        ...
        .detailed-metrics { display: flex;
  flex-direction: column;
  gap: 1rem }
        .algorithm-comparison { overflow-x: auto }
  ...
  align-items: center;
        ...
        .algorithm-name { font-weight: 500;
  color: #1f2937;
  gap: 0.5rem }
        .enabled-badge { font-size: 0.75rem
  padding: 0.125rem 0.375rem;
  background: #d1fae5 }
  color: #059669;

        .metric-cell { justify-content: center;
  font-weight: 600;
  color: #374151 }
  ...
  margin-top: 0.25rem;
        ...
        @media (max-width: 1200px) { .overview-cards {
  grid-template-columns: repeat(2, 1fr) }
          .real-time-stats { grid-template-columns: repeat(2, 1fr) }
          .weights-grid { grid-template-columns: 1fr }
          .filters-grid { grid-template-columns: 1fr }
          .filter-item.span-2 { grid-column: span 1 }

        @media (max-width: 768px) { .admin-header {
  flex-direction: column;
  gap: 1rem }
          .overview-cards { grid-template-columns: 1fr }
          .real-time-stats { grid-template-columns: 1fr }
          .algorithm-header { flex-direction: column;
  gap: 0.75rem }
          .ab-header { flex-direction: column;
  align-items: stretch;
  gap: 1rem }
          .variant-item { flex-direction: column;
  align-items: stretch;
  gap: 0.75rem }
