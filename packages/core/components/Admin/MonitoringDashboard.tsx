...
export interface MonitoringDashboardProps { userRole: string;
  userId: string;
...
export const MonitoringDashboard: React.FC<MonitoringDashboardProps> = ({ )
  ...
  const availableLayouts = useMemo(() => { return DASHBOARD_LAYOUTS.filter(layout => layout.roles.includes(userRole)) }, [userRole]);
  // Get current layout
  const currentLayout = useMemo(() => { if (customLayout) return customLayout;
    return availableLayouts.find(layout => layout.id === currentLayoutId) || availableLayouts[0] }, [customLayout, currentLayoutId, availableLayouts]);
  // Fetch widget data
  ...
  try {
          ...
          let data = {};
          switch (widget.id) { case 'system-health':
            data = {
  ...
  source: 'security-scanner'];
      ...
      results.forEach(({ widgetId, data }) => { newWidgetData[widgetId] = data });
      setWidgetData(newWidgetData);
      setIsLoading(false);
      setRefreshCount(prev => prev + 1);
 catch (error) { console.error('Failed to fetch widget data:', error);
  setConnectionStatus('disconnected');
  setIsLoading(false) }, []);
  // Auto-refresh effect
  useEffect(() => { if (!currentLayout) return;
    const refreshData = () => {
  ...
  const handleWidgetAction = useCallback((widgetId: string, action: string, params?: Record<string, unknown>) => {
    ...
    switch (action) { case 'view-details':
  ...
  const handleLayoutChange = useCallback((layoutId: string) => { setCurrentLayoutId(layoutId);
