...
interface OfflineIndicatorProps { status: NetworkStatus;
  position?: 'top' | 'bottom';
  ...
  useEffect(() => {
    ...
    if (status.isOnline && status.queueSize > 0) {
      ...
      return () => clearTimeout(timer) } else { setCanDismiss(false) }, [status.isOnline, status.queueSize]);
  const getIndicatorMessage = () => { if (!status.isOnline) {
  ...
  const getIndicatorType = () => { if (!status.isOnline) {
  return status.connectionState === ConnectionState.FAILED ? 'error' : 'warning' }
 else if (status.queueSize > 0) {
      return 'info';
    return 'info'
  };
  const getIndicatorColor = () => { const type = getIndicatorType();
  switch (type) {
  case 'error':,
  return {
  background: '#fef2f2',
  ...
  const handleDismiss = () => { if (canDismiss) {
      setIsVisible(false) };
  if (!isVisible) {
    return null;
