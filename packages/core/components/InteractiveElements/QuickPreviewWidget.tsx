...
interface QuickPreviewWidgetProps { element: QuickPreviewElement;
  ...
  const nextImage = useCallback(() => { setCurrentImageIndex((prev) =>
  prev < templateData.thumbnails.length - 1 ? prev + 1 : 0);
  setImageLoaded(false) }, [templateData.thumbnails.length]);
  const prevImage = useCallback(() => { setCurrentImageIndex((prev) =>
  ...
  const toggleZoom = useCallback(() => { if (isZoomed) {
      setIsZoomed(false);
      setZoomLevel(1) } else { setIsZoomed(true);
      setZoomLevel(1.5) }, [isZoomed]);
  // Handle wishlist toggle
  const handleWishlistToggle = useCallback(async () => { setWishlistLoading(true);
    try {
      // Simulate API call
 ...
 catch (error) { console.error('Failed to update wishlist:', error) } finally { setWishlistLoading(false) }, [element.id, interactiveService, userId, templateId, templateData]);
  // Handle share
  const handleShare = useCallback(async (platform: string) => { onShare?.(templateId, platform);
  ...
  // Handle download
  const handleDownload = useCallback(async () => { onDownload?.(templateId);
    await interactiveService.trackInteraction(element.id, {)
  type: InteractionType.CLICK,
      user_id: userId,
      timestamp: new Date(),
  ...
  // Handle purchase
  const handlePurchase = useCallback(async () => { onPurchase?.(templateId);
    await interactiveService.trackInteraction(element.id, {)
  type: InteractionType.CLICK,
      user_id: userId,
      timestamp: new Date(),
  ...
  // Format price
  const formatPrice = useCallback((price: number, currency: string) => { return new Intl.NumberFormat('en-US', {)
  style: 'currency',
  currency: currency }
}).format(price);
  }, []);
  ...
  return;
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div 
        ref={containerRef}
        className="bg-white rounded-lg shadow-2xl max-w-6xl w-full max-h-full overflow-hidden flex"
        style={ { 
  ...
  filter: imageLoaded ? 'none' : 'blur(4px)';
          ...
          {previewConfig.zoom_enabled && ()
            <div className="absolute bottom-4 left-4 flex space-x-2">
              <button
                ...
                className="bg-black bg-opacity-50 text-white p-2 rounded hover:bg-opacity-75 transition-all disabled:opacity-50"
              >
          ...
          {templateData.thumbnails.length > 1 && ()
            <div className="absolute bottom-4 right-4 bg-black bg-opacity-50 text-white px-3 py-1 rounded text-sm">
              {currentImageIndex + 1} / {templateData.thumbnails.length}
            </div>
          )}
        </div>
        {/* Details Panel */}
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h2 className="text-xl font-bold text-gray-900 mb-2">{templateData.title}</h2>
                <div className="flex items-center space-x-2 mb-2">
                  {renderStars(templateData.rating)}
                  <span className="text-sm text-gray-600">
                    ({templateData.reviewCount} reviews)
                  </span>
                </div>
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                  <span>{templateData.downloadCount} downloads</span>
                  <span>•</span>
            ...
            <div className="flex items-center space-x-3 mb-4">
              {templateData.authorAvatar ? ()
                <img
                  src={templateData.authorAvatar}
                  alt={templateData.author}
                  className="w-8 h-8 rounded-full"
                />
              ) : ()
                <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center">
                  <span className="text-xs font-medium text-gray-600">
                    {templateData.author.charAt(0).toUpperCase()}
                  </span>
                </div>
              )}
              <div>
                <div className="text-sm font-medium text-gray-900">{templateData.author}</div>
                <div className="text-xs text-gray-500">Template Creator</div>
              </div>
            </div>
            {/* Price and Actions */}
            <div className="flex items-center justify-between">
              <div>
                {templateData.originalPrice && templateData.originalPrice > templateData.price ? ()
                  <div>
                    <span className="text-lg font-bold text-green-600">
                      {formatPrice(templateData.price, templateData.currency)}
                    </span>
                    <span className="text-sm text-gray-500 line-through ml-2">
                      {formatPrice(templateData.originalPrice, templateData.currency)}
                    </span>
                  </div>
                ) : ()
                  <span className="text-lg font-bold text-gray-900">
                    {templateData.price === 0 ? 'Free' : formatPrice(templateData.price, templateData.currency)}
                  </span>
                )}
              </div>
...
`}
          ...
          <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    {templateData.whatsIncluded.map((item, index) => ()
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-gray-600">{templateData.fileSize}</div>
                  </div>
                  <div>
                    <span className="font-medium text-gray-900">Format:</span>
                    <div className="text-gray-600">{templateData.fileFormat.join(', ')}</div>
                  </div>
                  <div>
                    <span className="font-medium text-gray-900">License:</span>
                    <div className="text-gray-600">{templateData.license}</div>
                  </div>
                  <div>
                    <span className="font-medium text-gray-900">Updated:</span>
                    <div className="text-gray-600">{templateData.lastUpdated.toLocaleDateString()}</div>
                  </div>
                </div>
                <div>
                  <h3 className="font-medium text-gray-900 mb-2">Compatibility</h3>
                  <div className="flex flex-wrap gap-2">
                    {templateData.compatibility.map((app) => ()
            ...
            {activeTab === 'reviews' && ()
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    {renderStars(templateData.rating, 'md')}
                    <span className="font-bold">{templateData.rating}</span>
                    <span className="text-gray-500">({templateData.reviewCount} reviews)</span>
                  </div>
                </div>
                {/* Mock reviews */}
                <div className="space-y-4">
          ...
          <div className="border-t border-gray-200 p-6">
            <div className="space-y-3">
              {templateData.isPurchased || templateData.canDownload ? ()
                <button
                  onClick={handleDownload}
                  className="w-full bg-green-600 text-white py-3 px-4 rounded-md hover:bg-green-700 transition-all font-medium"
                >
                  {previewConfig.download_enabled ? ()
                    <>
              ...
              {templateData.demoUrl && ()
                <button
                  onClick={() => window.open(templateData.demoUrl, '_blank')}
