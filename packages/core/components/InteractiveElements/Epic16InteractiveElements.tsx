...
interface ElementState { id: string;
  ...
  useEffect(() => { const initializeElements = async () => {
  setLoading(true);
  setError(null);
  try {
  ...
  targetContext: []
        ...
        for (const elementData of elementsToCreate) { const element = await interactiveService.createElement(elementData);
 ...
 catch (err) { setError(err instanceof Error ? err.message : 'Failed to initialize interactive elements') } finally { setLoading(false) };
    initializeElements();
    // Cleanup
    return () => { interactiveService.removeAllListeners() };
  ...
  const getChatElement = useCallback(() => { return elements.find(el => el.element.type === InteractiveElementType.LIVE_CHAT) }, [elements]);
  const getProgressElements = useCallback(() => { return elements.filter(el => el.element.type === InteractiveElementType.PROGRESS_BAR) }, [elements]);
