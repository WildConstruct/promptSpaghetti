...
export interface DataSourceConfigurationPanelProps { visible: boolean;
  onClose: () => void;
...
export const DataSourceConfigurationPanel: React.FC<DataSourceConfigurationPanelProps> = ({ )
  ...
  useEffect(() => {
    if (initialDataSources.length > 0) {
      setDataSources(initialDataSources) }, [initialDataSources]);
  const handleNewSource = useCallback(() => {
    const newId = `custom-source-${Date.now()}`;}
    setEditingSource({ ...defaultDataSourceForm, id: newId });
    setShowForm(true);
  }, []);
  const handleEditSource = useCallback((source: DataSource) => { setEditingSource({)
  ...source
      authentication: {
  ...
  const handleSaveSource = useCallback(() => { if (!editingSource) return;
  ...
  const handleDeleteSource = useCallback((sourceId: string) => { const updatedSources = dataSources.filter(ds => ds.id !== sourceId);
    setDataSources(updatedSources);
    if (selectedSourceId === sourceId) {
      setSelectedSourceId(null);
    onSave(updatedSources) }, [dataSources, selectedSourceId, onSave]);
  ...
  const addTag = useCallback((tag: string) => { if (!editingSource || editingSource.metadata.tags.includes(tag)) return;
  updateEditingSource({)
  metadata: {
  ...
  const removeTag = useCallback((tag: string) => { if (!editingSource) return;
  updateEditingSource({)
  metadata: {
  ...editingSource.metadata
...
> = ({ source, onUpdate, onSave, onCancel, onAddTag, onRemoveTag }) => { const [newTag, setNewTag] = useState('');
  const handleTagAdd = useCallback(() => {
    if (newTag.trim()) {
      onAddTag(newTag.trim());
      setNewTag('') }, [newTag, onAddTag]);
  return;
