...
export const AuditTrailViewer = ({ workspaceId, resourceId, onClose }) => {
    ...
    const filteredEntries = useMemo(() => {
        ...
        if (filters.start_date) {
            entries = entries.filter(entry => new Date(entry.action_timestamp) >= filters.start_date);
        }
        if (filters.end_date) {
            entries = entries.filter(entry => new Date(entry.action_timestamp) <= filters.end_date);
        }
        if (filters.search_term) {
            const searchLower = filters.search_term.toLowerCase();
            entries = entries.filter(entry => entry.action_type.toLowerCase().includes(searchLower) ||
    ...
    const actionTypes = useMemo(() => Array.from(new Set(auditHistory.map(entry => entry.action_type))).sort(), [auditHistory]);
    const actors = useMemo(() => Array.from(new Set(auditHistory.map(entry => entry.actor_id))).sort(), [auditHistory]);
    ...
    if (error) {
        return (_jsx("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4", children: _jsxs("div", { className: "flex items-center", children: [_jsx(XCircleIcon, { className: "h-5 w-5 text-red-600 mr-2" }), _jsxs("span", { className: "text-red-800", children: ["Failed to load audit history: ", error] })] }) }));
    }
    return (_jsxs("div", { className: "bg-white rounded-lg shadow-sm border border-gray-200", children: [_jsx("div", { className: "border-b border-gray-200 p-4", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx(ClipboardDocumentListIcon, { className: "h-6 w-6 text-gray-600" }), _jsxs("div", { children: [_jsx("h2", { className: "text-lg font-semibold text-gray-900", children: "Audit Trail" }), _jsxs("p", { className: "text-sm text-gray-500", children: [filteredEntries.length, " entries found"] })] })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("button", { onClick: () => setShowFilters(!showFilters), className: "flex items-center space-x-1 px-3 py-1 text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors", children: [_jsx(FunnelIcon, { className: "h-4 w-4" }), _jsx("span", { children: "Filters" }), _jsx(ChevronDownIcon, { className: `h-3 w-3 transition-transform ${showFilters ? 'rotate-180' : ''}` })] }), _jsxs("button", { onClick: () => setShowExportDialog(true), className: "flex items-center space-x-1 px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors", children: [_jsx(ArrowDownTrayIcon, { className: "h-4 w-4" }), _jsx("span", { children: "Export" })] }), onClose && (_jsx("button", { onClick: onClose, className: "text-gray-400 hover:text-gray-600 transition-colors", children: _jsx(XCircleIcon, { className: "h-5 w-5" }) }))] })] }) }), showFilters && (_jsx("div", { className: "border-b border-gray-200 p-4 bg-gray-50", children: _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-4", children: [_jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Action Type" }), _jsxs("select", { value: filters.action_type || '', onChange: (e) => handleFilterChange('action_type', e.target.value || undefined), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500", children: [_jsx("option", { value: "", children: "All Actions" }), actionTypes.map(type => (_jsx("option", { value: type, children: type.replace('_', ' ').toUpperCase() }, type)))] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Actor" }), _jsxs("select", { value: filters.actor_id || '', onChange: (e) => handleFilterChange('actor_id', e.target.value || undefined), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500", children: [_jsx("option", { value: "", children: "All Actors" }), actors.map(actor => (_jsx("option", { value: actor, children: actor }, actor)))] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Search" }), _jsxs("div", { className: "relative", children: [_jsx(MagnifyingGlassIcon, { className: "absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" }), _jsx("input", { type: "text", value: filters.search_term || '', onChange: (e) => handleFilterChange('search_term', e.target.value || undefined), placeholder: "Search entries...", className: "w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" })] })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Start Date" }), _jsx("input", { type: "datetime-local", value: filters.start_date ? filters.start_date.toISOString().slice(0, 16) : '', onChange: (e) => handleFilterChange('start_date', e.target.value ? new Date(e.target.value) : undefined), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" })] }), _jsxs("div", { children: [_jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "End Date" }), _jsx("input", { type: "datetime-local", value: filters.end_date ? filters.end_date.toISOString().slice(0, 16) : '', onChange: (e) => handleFilterChange('end_date', e.target.value ? new Date(e.target.value) : undefined), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500" })] })] }) })), _jsx("div", { className: "divide-y divide-gray-200 max-h-96 overflow-y-auto", children: filteredEntries.length === 0 ? (_jsxs("div", { className: "p-8 text-center text-gray-500", children: [_jsx(ClipboardDocumentListIcon, { className: "h-12 w-12 mx-auto mb-4 text-gray-300" }), _jsx("p", { children: "No audit entries found matching your criteria." })] })) : (filteredEntries.map((entry) => (_jsx("div", { className: "p-4 hover:bg-gray-50 transition-colors", children: _jsxs("div", { className: "flex items-start space-x-3", children: [_jsx("div", { className: "flex-shrink-0 mt-1", children: getActionTypeIcon(entry.action_type) }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("span", { className: `px-2 py-1 text-xs font-medium rounded-full ${getActionTypeColor(entry.action_type)}`, children: entry.action_type.replace('_', ' ').toUpperCase() }), _jsx("span", { className: "text-sm text-gray-500", children: formatTimestamp(entry.action_timestamp) })] }), _jsxs("div", { className: "mt-1 flex items-center space-x-4 text-sm text-gray-600", children: [_jsxs("div", { className: "flex items-center space-x-1", children: [_jsx(UserIcon, { className: "h-4 w-4" }), _jsx("span", { children: entry.actor_id })] }), _jsx("div", { className: "flex items-center space-x-1", children: _jsxs("span", { children: ["Resource: ", entry.resource_id] }) })] }), entry.comment && (_jsx("p", { className: "mt-2 text-sm text-gray-700", children: entry.comment })), entry.metadata && Object.keys(entry.metadata).length > 0 && (_jsx("div", { className: "mt-2 text-xs text-gray-500", children: _jsxs("details", { className: "cursor-pointer", children: [_jsxs("summary", { className: "hover:text-gray-700", children: ["Additional metadata (", Object.keys(entry.metadata).length, " items)"] }), _jsx("pre", { className: "mt-1 bg-gray-100 p-2 rounded text-xs overflow-x-auto", children: JSON.stringify(entry.metadata, null, 2) })] }) }))] })] }) }, entry.id)))) }), showExportDialog && (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: _jsxs("div", { className: "bg-white rounded-lg p-6 w-full max-w-md", children: [_jsx("h3", { className: "text-lg font-semibold mb-4", children: "Export Audit History" }), _jsxs("div", { className: "space-y-3", children: [_jsx("button", { onClick: () => handleExport('csv'), className: "w-full text-left px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors", children: "CSV Format" }), _jsx("button", { onClick: () => handleExport('json'), className: "w-full text-left px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors", children: "JSON Format" }), _jsx("button", { onClick: () => handleExport('pdf'), className: "w-full text-left px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors", children: "PDF Report" })] }), _jsx("div", { className: "mt-4 flex justify-end space-x-2", children: _jsx("button", { onClick: () => setShowExportDialog(false), className: "px-4 py-2 text-gray-600 hover:text-gray-800 transition-colors", children: "Cancel" }) })] }) }))] }));
};
