...
export const ExperimentBuilder = ({ experiment, onSave, onPreview, onStart, onPause, className = '' }) => {
    ...
    const updateVariant = useCallback((variantId, field, value) => {
        const updatedVariants = state.experiment.variants?.map(variant => variant.id === variantId
            ? { ...variant, [field]: value }
            : variant) || [];
        setState(prev => ({
            ...prev,
    ...
    const updateMetric = useCallback((metricId, field, value) => {
        const updatedMetrics = state.experiment.metrics?.map(metric => metric.id === metricId
            ? { ...metric, [field]: value }
            : metric) || [];
        setState(prev => ({
            ...prev,
    ...
    const handleStart = useCallback(async () => {
        ...
        try {
            ...
            setState(prev => ({
                ...prev,
                experiment: { ...prev.experiment, status: 'running' }
            }));
        }
        catch (error) {
    ...
    useEffect(() => {
        ...
        if (primaryMetric && primaryMetric.minimumDetectableEffect) {
            ...
            const sampleSize = Math.ceil(2 * Math.pow(1.96 + 0.84, 2) * baselineRate * (1 - baselineRate) / Math.pow(mde, 2));
            setState(prev => ({
                ...prev,
