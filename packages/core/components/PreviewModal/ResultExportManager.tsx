...
export interface ResultExportManagerProps { results: EnhancedPreviewResult;
...
export const ResultExportManager: React.FC<ResultExportManagerProps> = ({ )
  ...
  const selectedResults = useMemo(() => {
    return results.filter(result => selectedResultIds.includes(result.id)) }, [results, selectedResultIds]);
  // Group formats by category
  const formatsByCategory = useMemo(() => { return EXPORT_FORMATS.reduce((acc, format) => {
      if (!acc[format.category]) {
        acc[format.category] = [];
      acc[format.category].push(format);
      return acc }, {} as Record<string, ExportFormat>);
  }, []);
  ...
  const handleFormatSelect = useCallback((format: ExportFormat) => { setSelectedFormat(format);
  ...
  compressOutput: format.extension === 'zip' }
    ...
    if (format.vfxCompatible) { defaultOptions.vfxOptions = {
  ...
  resolution: [1920, 1080] }
    ...
    if (format.category === 'script') { defaultOptions.filmOptions = {
  ...
  const handleExport = useCallback(async () => { if (!selectedFormat || !exportOptions || selectedResults.length === 0) {
  return;
  setIsExporting(true);
  try {
  ...
  filename: generateFilename(selectedFormat, selectedResults.length) }
      ...
      setShowAdvancedOptions(false) } catch (error) { const exportError = error instanceof Error ? error : new Error('Export failed');
 ...
 finally { setIsExporting(false) }, [selectedFormat, exportOptions, selectedResults, selectedResultIds, resultManagement, onExportComplete, onExportError]);
...
async function prepareExportPayload(((
  ...
  const basePayload = { results: results.map(result => ({)
  id: result.id
      seed: result.seed
  ...
  switch (format.category) { case 'vfx':
  return {
  ...basePayload
  vfxData: {
