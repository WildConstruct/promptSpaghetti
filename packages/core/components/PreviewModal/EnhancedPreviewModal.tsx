...
export interface EnhancedPreviewResult { seed: number;
  output?: string;
  error?: string;
...
interface EnhancedPreviewModalProps { open: boolean;
  loading: boolean;
  error: string | null }
...
export const [expandedResult, setExpandedResult] = useState<string | null>(null);
  ...
  const resultStats = useMemo(() => { if (!results.length) return null;
  ...
  const handleResultSelect = useCallback((resultId: string, selected: boolean) => { const newSelected = new Set(selectedResults);
    if (selected) {
      newSelected.add(resultId) } else { newSelected.delete(resultId);
    setSelectedResults(newSelected);
    onResultSelect?.(resultId, selected) }, [selectedResults, onResultSelect]);
  const handleSelectAll = useCallback(() => { const allResultIds = new Set(results.map(r => r.id));
    setSelectedResults(allResultIds);
    results.forEach(r => onResultSelect?.(r.id, true)) }, [results, onResultSelect]);
  const handleClearSelection = useCallback(() => { selectedResults.forEach(id => onResultSelect?.(id, false));
    setSelectedResults(new Set()) }, [selectedResults, onResultSelect]);
  const handleBatchExport = useCallback(async () => { if (selectedResults.size === 0) return;
    try {
      await onResultExport?.(Array.from(selectedResults)) } catch (error) { throw ErrorFactory.createGraphExecutionError()
        'Failed to export selected results'
        error as Error }
        { operation: 'batch_export' }
      );
  }, [selectedResults, onResultExport]);
  const handleRating = useCallback((resultId: string, rating: number) => { setRatingInProgress(resultId);
    onResultRate?.(resultId, rating);
    setTimeout(() => setRatingInProgress(null), 500) }, [onResultRate]);
  const handleNoteSave = useCallback((resultId: string) => { onResultNote?.(resultId, tempNote);
    setNoteEditing(null);
    setTempNote('') }, [tempNote, onResultNote]);
  const getContentTypeIcon = (contentType?: string) => { switch (contentType) {
