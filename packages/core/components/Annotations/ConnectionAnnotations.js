...
export const ConnectionLabel = ({ edge, x, y, onLabelChange, _____onLabelStyleChange }) => {
    ...
    const labelStyle = useMemo(() => ({
        ...
        whiteSpace: 'nowrap',
        userSelect: edge.interactive ? 'text' : 'none',
        zIndex: 1000,
        pointerEvents: 'all',
        maxWidth: 200,
        textAlign: 'center'
    ...
    return (_jsx("div", { style: labelStyle, onClick: handleClick, onDoubleClick: handleClick, children: isEditing ? (_jsx("input", { type: "text", value: editValue, onChange: (e) => setEditValue(e.target.value), onKeyDown: handleKeyDown, onBlur: handleBlur, autoFocus: true, style: {
...
export const AnnotatedEdgeComponent = ({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, style = {}, data, markerEnd, onLabelChange, onLabelStyleChange }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("path", { id: id, style: style, className: "react-flow__edge-path", d: edgePath, markerEnd: markerEnd }), data && (_jsx(ConnectionLabel, { edge: data, x: finalLabelX, y: finalLabelY, onLabelChange: onLabelChange, onLabelStyleChange: onLabelStyleChange }))] }));
};
export const ConnectionLabelEditor = ({ edge, onUpdateEdge, onClose }) => {
    ...
    const [labelPosition, setLabelPosition] = useState(typeof edge?.labelPosition === 'number' ? edge.labelPosition.toString() : (edge?.labelPosition || 'center'));
...
export const createAnnotatedEdge = (baseEdge, label, options) => ({
    ...baseEdge,
    label: label || '',
    labelStyle: {
        fontSize: 12,
...
export const updateEdgeLabel = (edges, edgeId, updates) => {
    return edges.map(edge => edge.id === edgeId ? { ...edge, ...updates } : edge);
};
export const toggleEdgeLabel = (edges, edgeId) => {
    return edges.map(edge => edge.id === edgeId
...
export const optimizeLabelPositions = (edges) => {
