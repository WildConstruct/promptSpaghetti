...
export const RegionAnnotationSystem: React.FC<RegionAnnotationSystemProps> = ({ )
  ...
  // Get canvas context
  const getContext = useCallback(() => { const canvas = canvasRef.current;
  ...
  const isPointInRegion = useCallback((point: { x: number; y: number }, region: RegionAnnotation): boolean => { switch (region.area.shape) {
  case 'rectangle':
  ...
  const getNodesInRegion = useCallback((region: RegionAnnotation): string => { return nodes
  .filter(node => {)
  const nodeCenter = {
  ...
  const drawGrid = useCallback((ctx: CanvasRenderingContext2D) => { if (!showGrid) return;
    ...
    for (let y = 0; y <= height; y += gridSize) {
      ...
      ctx.lineTo(width, y);
      ctx.stroke();
    ctx.restore() }, [showGrid, width, height]);
  // Draw single region
  const drawRegion = useCallback((ctx: CanvasRenderingContext2D, region: RegionAnnotation) => { if (!region.visible) return;
  ctx.save();
  // Apply styles
  ...
  ctx.fillRect(handle.x - handleSize/2, handle.y - handleSize/2, handleSize, handleSize) });
    ...
    if (region.marsZone) { const marsConfig = MARS_ZONES[region.marsZone];
  ...
  const renderRegions = useCallback(() => {
    ...
    if (isCreating && creationPoints.length > 0) { ctx.save();
      ctx.strokeStyle = '#3b82f6';
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.globalAlpha = 0.7;
      ...
      ctx.restore() }, [regions, selectedRegion, width, height, drawGrid, drawRegion, getContext, isCreating, creationPoints, creationTool]);
  ...
  const handleMouseDown = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => { if (readonly) return;
    ...
    if (isCreating) {
      ...
      if (creationTool === 'rectangle' && creationPoints.length === 1) {
        ...
        return } else { // Check if clicking on existing region
      ...
      if (clickedRegion) {
        ...
        onNodesInRegion?.(nodeIds) } else { onRegionSelect?.(null);
        onNodesInRegion?.([]) }, [readonly, isCreating, creationTool, creationPoints, regions, getCanvasCoordinates, isPointInRegion, onRegionSelect, onNodesInRegion, getNodesInRegion]);
  const handleMouseMove = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => { if (!isCreating || readonly) return;
    const point = getCanvasCoordinates(event);
    if (creationTool === 'rectangle' && creationPoints.length === 1) {
      setCreationPoints([creationPoints[0], point]) }, [isCreating, readonly, creationTool, creationPoints, getCanvasCoordinates]);
  // Complete region creation
  const completeRegionCreation = useCallback(() => { if (creationPoints.length < 2) return;
  ...
  // Filter regions
  const filteredRegions = useMemo(() => { return regions.filter(region => {)
  ...
  if (event.key === 'Escape' && isCreating) { setIsCreating(false);
  setCreationPoints([]) } else if (event.key === 'Enter' && isCreating && creationPoints.length >= 2) { completeRegionCreation() };
