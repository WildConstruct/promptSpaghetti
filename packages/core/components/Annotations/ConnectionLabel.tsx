...
interface ConnectionLabelProps { label: ConnectionLabelType;
  ...
  // Get label styles
  const getLabelStyles = useCallback(() => { const baseStyle = CONNECTION_LABEL_STYLES[label.style] || CONNECTION_LABEL_STYLES.default;
    return {
      ...baseStyle }
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => { const value = e.target.value;
    ...
    if (value.length <= maxLength) {
      setEditValue(value) }, []);
  // Save label changes
  const saveLabel = useCallback(() => { const newContent = editValue.trim();
  setIsEditing(false);
  onAction({)
  type: 'update',
  labelId: label.id,
  ...
  // Handle drag start
  const handleMouseDown = useCallback((e: React.MouseEvent) => { if (!canEdit || isEditing) return;
  e.preventDefault();
  e.stopPropagation();
  setIsDragging(true);
  setDragStart({)
  ...
  useEffect(() => { if (!isDragging) return;
