...
interface ConnectionAnnotationsLayerProps { canEdit?: boolean;
...
export const ConnectionAnnotationsLayer: React.FC<ConnectionAnnotationsLayerProps> = ({ canEdit = true
  showTooltips = true
  visible = true }
  onSelectionChange
 ...
 = useGraphStore();
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number;
  y: number;
  connectionId: string } | null>(null);
  const layerRef = useRef<HTMLDivElement>(null);
  const svgRef = useRef<SVGSVGElement>(null);
  // Get connection path coordinates for label positioning
  const getConnectionPath = useCallback((connectionId: string) => { const edge = edges.find(e => e.id === connectionId);
  if (!edge) return null;
  ...
  const handleLabelAction = useCallback((action: ConnectionLabelAction) => { switch (action.type) {
      case 'create':
  ...
  connectionId: action.connectionId
        ...
        break;
      case 'update':
        if (action.labelId && action.label) { updateConnectionLabel(action.labelId, action.label);
  break;
  case 'delete':
  if (action.labelId) {
  ...
  const handleConnectionContextMenu = useCallback((e: React.MouseEvent, connectionId: string) => { e.preventDefault();
  e.stopPropagation();
  ...
  const handleContextMenuAction = useCallback((action: string, connectionId: string) => { setContextMenu(null);
  switch (action) {
  case 'addLabel':
  handleLabelAction({)
  type: 'create'
  ...
  useEffect(() => { if (onSelectionChange) {
      onSelectionChange(Array.from(selectedAnnotations)) }, [selectedAnnotations, onSelectionChange]);
