...
export const NodeLabelsLayer: React.FC<NodeLabelsLayerProps> = ({ )
  ...
  const createLabelConfig = useCallback((nodeId: string, customLabel: string = ''): NodeLabelConfig => { return {
  id: generateLabelId()
  nodeId
  customLabel
  ...
  const handleLabelAction = useCallback((action: NodeLabelAction) => {
    ...
    switch (action.type) { case 'create': }
  if (!action.config && action.customLabel) { const newConfig = createLabelConfig(action.nodeId, action.customLabel);
  updatedConfigs[newConfig.id] = newConfig } else if (action.config) { const newConfig = {
  ...
  id: action.labelId || generateLabelId() }
        ...
        break;
      case 'update':
        if (action.labelId && updatedConfigs[action.labelId]) { updatedConfigs[action.labelId] = {
  ...updatedConfigs[action.labelId]
  ...action.config
  timestamp: new Date().toISOString() }
};
          if (action.customLabel !== undefined) { updatedConfigs[action.labelId].customLabel = action.customLabel;
  break;
  case 'delete':
  ...
  const getLabelConfigForNode = useCallback((nodeId: string): NodeLabelConfig | null => { // Find existing config for this node
    ...
    return null }, [labelConfigs]);
  // Handle right-click to create/edit labels
  const handleContextMenu = useCallback((e: React.MouseEvent, nodeId: string) => { if (readOnly) return;
  e.preventDefault();
  e.stopPropagation();
  ...
  const handleQuickLabelCreate = useCallback((nodeId: string) => { if (readOnly) return;
    const existingConfig = getLabelConfigForNode(nodeId);
    if (!existingConfig) {
 ...
 else { handleQuickLabelCreate(selectedNodeId);
