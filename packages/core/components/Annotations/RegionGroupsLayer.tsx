...
import { RegionGroup as RegionGroupType, 
  RegionGroupAction,
...
export const RegionGroupsLayer: React.FC<RegionGroupsLayerProps> = ({ )
  ...
  const getNodesInBounds = useCallback((bounds: { x: number; y: number; width: number; height: number }) => { return nodes.filter(node => {)
  ...
  const handleGroupAction = useCallback((action: RegionGroupAction) => { const updatedGroups = [...regionGroups];
 ...
 as RegionGroupType;
        ...
        break;
      case 'update':
        if (groupIndex >= 0 && action.group) { updatedGroups[groupIndex] = {
  ...
  lastModified: new Date().toISOString() }
        ...
        break;
      case 'delete':
        if (groupIndex >= 0) { updatedGroups.splice(groupIndex, 1);
  break;
  case 'move':,
  ...
  lastModified: new Date().toISOString();
        ...
        break;
      case 'resize':
        if (groupIndex >= 0 && action.bounds) { updatedGroups[groupIndex] = {
  ...updatedGroups[groupIndex],
  ...
  lastModified: new Date().toISOString() }
        ...
        break;
      case 'collapse':
        if (groupIndex >= 0) { updatedGroups[groupIndex] = {
  ...updatedGroups[groupIndex],
  collapsed: true,
  lastModified: new Date().toISOString() }
};
  ...
  lastModified: new Date().toISOString() }
        ...
        break;
      case 'addNodes':
        if (groupIndex >= 0 && action.nodeIds) { const existingNodeIds = updatedGroups[groupIndex].nodeIds;
  ...
  lastModified: new Date().toISOString() }
        ...
        break;
      case 'removeNodes':
        if (groupIndex >= 0 && action.nodeIds) { const remainingNodeIds = updatedGroups[groupIndex].nodeIds.filter(;);
            id => !action.nodeIds!.includes(id)
          );
          if (remainingNodeIds.length === 0) {
            // Remove empty group
            updatedGroups.splice(groupIndex, 1) } else { updatedGroups[groupIndex] = {
  ...updatedGroups[groupIndex],
  ...
  const handleSelectionComplete = useCallback((bounds: { x: number; y: number; width: number; height: number }) => { setIsDragSelecting(false);
    createRegionGroup(bounds) }, [createRegionGroup]);
  // Handle drag selection cancel
  const handleSelectionCancel = useCallback(() => { setIsDragSelecting(false) }, []);
  // Handle context menu
  const handleContextMenu = useCallback((e: React.MouseEvent) => { if (readOnly) return;
  e.preventDefault();
  e.stopPropagation();
  setContextMenu({)
  x: e.clientX,
  ...
  if (e.key === 'g' || e.key === 'G') { if (!e.ctrlKey && !e.metaKey && !e.altKey && !isDragSelecting) {
  e.preventDefault();
  ...
  const getNodeCountForGroup = useCallback((groupId: string) => { const group = regionGroups.find(g => g.id === groupId);
  return group ? group.nodeIds.length : 0 }, [regionGroups]);
  return;
    <>
      <div
        ref={layerRef}
