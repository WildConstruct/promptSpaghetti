...
export const ConnectionAnnotationsLayer = ({ canEdit = true, showTooltips = true, visible = true, onSelectionChange }) => {
    ...
    const getConnectionPath = useCallback((connectionId) => {
    ...
    const calculateLabelPosition = useCallback((connectionId, positionType, offset = 0.5) => {
        ...
        switch (action.type) {
            case 'create':
                if (action.connectionId && action.content) {
                    const position = calculateLabelPosition(action.connectionId, 'middle', 0.5);
                    const newLabel = {
    ...
    const handleContextMenuAction = useCallback((action, connectionId) => {
        setContextMenu(null);
        switch (action) {
            case 'addLabel':
                handleLabelAction({
    ...
    if (!visible)
        return null;
    return (_jsxs("div", { ref: layerRef, "data-testid": "connection-annotations-layer", style: {
            position: 'absolute',
            inset: 0,
            pointerEvents: 'none',
            zIndex: 50
            ...
            ` })] }), edges.map(edge => {
                        ...
                        if (!pathData || !annotation)
                            return null;
                        return (_jsxs("g", { children: [_jsx("path", { d: pathData.path, stroke: annotation.color || '#6b7280', strokeWidth: annotation.strokeWidth || 2, strokeOpacity: annotation.opacity || 1, strokeDasharray: annotation.visualStyle === 'dashed' ? '8 4' :
                                        annotation.visualStyle === 'dotted' ? '2 3' :
                                            annotation.visualStyle === 'animated' ? '8 4' : 'none', className: `
                  ${annotation.visualStyle === 'animated' ? 'connection-animated' : ''}
                  ${isHighlighted ? 'connection-highlighted' : ''}
                `, fill: "none", style: { pointerEvents: 'stroke' }, onContextMenu: (e) => handleConnectionContextMenu(e, edge.id) }), annotation.showDirection && (_jsx("polygon", { points: "0,-4 8,0 0,4", fill: annotation.color || '#6b7280', transform: `translate(${pathData.target.x - 8}, ${pathData.target.y}) rotate(${Math.atan2(pathData.target.y - pathData.source.y, pathData.target.x - pathData.source.x) * 180 / Math.PI})` }))] }, `enhanced-${edge.id}`));
                    })] }), annotations.connectionLabels?.map(label => (_jsx(ConnectionLabel, { label: label, onAction: handleLabelAction, canEdit: canEdit, showTooltip: showTooltips, isHighlighted: selectedAnnotations.has(label.id), connectionPath: getConnectionPath(label.connectionId)?.path }, label.id))), contextMenu && (_jsxs("div", { "data-testid": "connection-context-menu", style: {
                    position: 'absolute',
                    left: contextMenu.x,
                    top: contextMenu.y,
                    background: 'white',
                    ...
                    zIndex: 1000,
                    pointerEvents: 'all'
                }, children: [_jsx("button", { onClick: () => handleContextMenuAction('addLabel', contextMenu.connectionId), style: {
                            width: '100%',
                            padding: '8px 16px',
                            border: 'none',
                            background: 'none',
                            ...
                            e.currentTarget.style.background = 'none';
                        }, children: "\uD83C\uDFA8 Edit Style" })] })), (!annotations.connectionLabels || annotations.connectionLabels.length === 0) && canEdit && (_jsxs("div", { style: {
                    position: 'absolute',
                    top: '50%',
                    left: '50%',
                    ...
                    pointerEvents: 'all'
                }, children: [_jsx("div", { style: { fontSize: '24px', marginBottom: '8px' }, children: "\uD83D\uDD17" }), _jsx("div", { style: { fontWeight: 'bold', marginBottom: '4px', color: '#374151' }, children: "Connection Annotations" }), _jsx("div", { style: { fontSize: '14px', color: '#6b7280', lineHeight: '1.4' }, children: "Right-click on connections to add labels and visual enhancements" }), _jsx("div", { style: { fontSize: '12px', color: '#9ca3af', marginTop: '8px' }, children: "Keyboard: Ctrl+L to add label" })] }))] }));
};
