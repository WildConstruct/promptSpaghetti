...
interface RegionGroupProps { group: RegionGroupType;
  ...
  const shouldShowGroup = useCallback(() => { if (!group.visible) return false;
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => { if (!canMove || isEditingLabel) return;
  e.stopPropagation();
  e.preventDefault();
  setIsDragging(true);
  setDragStart({)
  ...
  const handleResizeMouseDown = useCallback((e: React.MouseEvent, handle: string) => { if (!canResize || isEditingLabel) return;
  e.stopPropagation();
  e.preventDefault();
  setIsResizing(true);
  setResizeStart({)
  ...
  type: 'move'
        ...
        switch (resizeStart.handle) { case 'se': // Southeast
  ...
  bounds: newBounds }
    ...
    const handleMouseUp = () => { setIsDragging(false);
      setIsResizing(false) };
    if (isDragging || isResizing) { document.addEventListener('mousemove', handleMouseMove);
  ...
  const startEditingLabel = useCallback(() => { if (!canEdit) return;
    setIsEditingLabel(true);
    setEditLabelValue(group.label) }, [canEdit, group.label]);
  const saveLabel = useCallback(() => { setIsEditingLabel(false);
    onAction({)
  type: 'update'
      groupId: group.id }
  ...
  const cancelEditLabel = useCallback(() => { setIsEditingLabel(false);
    setEditLabelValue(group.label) }, [group.label]);
  // Handle key events for label editing
  const handleLabelKeyDown = useCallback((e: React.KeyboardEvent) => { switch (e.key) {
  case 'Enter':
  e.preventDefault();
  saveLabel();
  break;
  ...
  useEffect(() => { if (isEditingLabel && labelInputRef.current) {
      labelInputRef.current.focus();
      labelInputRef.current.select() }, [isEditingLabel]);
  // Handle collapse/expand
  const handleToggleCollapse = useCallback(() => { onAction({)
