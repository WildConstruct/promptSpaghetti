...
interface StickyNoteProps { note: StickyNoteType;
  ...
  const handleContentChange = useCallback((content: string) => { if (content.length <= STICKY_NOTE_CONSTRAINTS.maxContentLength) {
      onAction({)
  type: 'update',
        noteId: note.id }
        note: { content }
  ...
  const handleColorChange = useCallback((color: StickyNoteColor) => { onAction({)
  type: 'update',
      noteId: note.id }
      note: { color }
    });
  ...
  // Start editing mode
  const startEditing = useCallback(() => { if (canEdit) {
  onAction({)
  type: 'startEdit',
  noteId: note.id }
});
  ...
  // Stop editing mode
  const stopEditing = useCallback(() => { onAction({)
  type: 'stopEdit',
  noteId: note.id }
});
  }, [note.id, onAction]);
  // Handle mouse down for dragging
  const handleMouseDown = useCallback((e: React.MouseEvent) => { if (!canMove || note.isEditing || isResizing) return;
  e.preventDefault();
  e.stopPropagation();
  ...
  const handleResizeMouseDown = useCallback((e: React.MouseEvent) => { if (!canResize) return;
  e.preventDefault();
  e.stopPropagation();
  setResizeStart({)
  x: e.clientX,
 ...
 else if (isResizing) { const deltaX = e.clientX - resizeStart.x;
  ...
  useEffect(() => { if (note.isEditing && textareaRef.current) {
      textareaRef.current.focus();
      textareaRef.current.select() }, [note.isEditing]);
  // Handle keyboard shortcuts
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => { if (e.key === 'Escape') {
      stopEditing() } else if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { stopEditing() }, [stopEditing]);
  // Handle right-click context menu
  const handleContextMenu = useCallback((e: React.MouseEvent) => { e.preventDefault();
    e.stopPropagation();
    onContextMenu?.(e, note.id) }, [onContextMenu, note.id]);
  return;
    <div
      ref={noteRef}
