...
export const WeightDistributionChart = ({ options, type = 'pie', width = 240, height = 240, showLabels = true, showPercentages = true, showLegend = false, colorScheme = 'cinema4d', animationDuration = 300, onOptionHover, onOptionClick }) => {
    ...
    const PieChart = () => {
        ...
        return (_jsxs("g", { children: [processedOptions.map((option) => {
                    ...
                    return (_jsxs("g", { children: [_jsx("path", { d: pathData, fill: option.color, stroke: "rgba(255, 255, 255, 0.2)", strokeWidth: "1", style: {
                                    transform,
                                    transformOrigin,
                                    transition: `transform ${animationDuration}ms cubic-bezier(0.4, 0, 0.2, 1)`,
                                    filter: isHovered ? 'brightness(1.1) drop-shadow(0 4px 8px rgba(0,0,0,0.2))' : 'none',
                                    cursor: 'pointer'
                                }, onMouseEnter: () => handleOptionHover(option), onMouseLeave: () => handleOptionHover(null), onClick: () => handleOptionClick(option) }), showLabels && option.percentage > 5 && (_jsxs("g", { children: [_jsx("text", { x: labelX, y: labelY - 6, textAnchor: "middle", fontSize: "11", fontWeight: "500", fill: "#e5e7eb", style: { pointerEvents: 'none' }, children: option.text.length > 12 ? `${option.text.slice(0, 12)}...` : option.text }), showPercentages && (_jsxs("text", { x: labelX, y: labelY + 6, textAnchor: "middle", fontSize: "10", fill: "#9ca3af", style: { pointerEvents: 'none' }, children: [option.percentage.toFixed(1), "%"] }))] }))] }, option.id));
                }), type === 'donut' && (_jsx("text", { x: centerX, y: centerY, textAnchor: "middle", fontSize: "14", fontWeight: "600", fill: "#e5e7eb", style: { pointerEvents: 'none' }, children: "Weights" }))] }));
    };
    // Bar Chart Component
    const BarChart = () => {
        ...
        return (_jsxs("g", { transform: `translate(${margin.left}, ${margin.top})`, children: [processedOptions.map((option, index) => {
                    const y = index * (barHeight + 8);
                    const barWidth = option.normalizedWeight * maxBarWidth;
                    const isHovered = hoveredOption === option.id;
                    return (_jsxs("g", { children: [_jsx("rect", { x: 0, y: y, width: maxBarWidth, height: barHeight, fill: "rgba(55, 65, 81, 0.3)", rx: 2 }), _jsx("rect", { x: 0, y: y, width: barWidth, height: barHeight, fill: option.color, rx: 2, style: {
                    ...
                    1
                                    ...
                                    cursor: 'pointer'
                                }, onMouseEnter: () => handleOptionHover(option), onMouseLeave: () => handleOptionHover(null), onClick: () => handleOptionClick(option) }), _jsx("text", { x: -8, y: y + barHeight / 2, textAnchor: "end", fontSize: "11", fontWeight: "500", fill: "#e5e7eb", dominantBaseline: "middle", style: { pointerEvents: 'none' }, children: option.text.length > 10 ? `${option.text.slice(0, 10)}...` : option.text }), showPercentages && (_jsxs("text", { x: barWidth + 8, y: y + barHeight / 2, textAnchor: "start", fontSize: "10", fill: "#9ca3af", dominantBaseline: "middle", style: { pointerEvents: 'none' }, children: [option.percentage.toFixed(1), "%"] }))] }, option.id));
                }), _jsx("text", { x: chartWidth / 2, y: -8, textAnchor: "middle", fontSize: "12", fontWeight: "600", fill: "#e5e7eb", style: { pointerEvents: 'none' }, children: "Weight Distribution" })] }));
    };
    // Legend Component
    const Legend = () => (_jsx("div", { style: {
            display: 'flex',
            flexWrap: 'wrap',
            gap: 12,
            marginTop: 16,
            ...
            border: '1px solid rgba(55, 65, 81, 0.6)'
        }, children: processedOptions.map((option) => (_jsxs("div", { style: {
                display: 'flex',
                alignItems: 'center',
                gap: 6,
                padding: '4px 8px',
                ...
                transition: 'background 0.2s ease'
            }, onMouseEnter: () => handleOptionHover(option), onMouseLeave: () => handleOptionHover(null), onClick: () => handleOptionClick(option), children: [_jsx("div", { style: {
                        width: 12,
                        height: 12,
                        borderRadius: '50%',
                        ...
                        color: '#e5e7eb',
                        fontWeight: 500
                    }, children: [option.text, " (", option.percentage.toFixed(1), "%)"] })] }, option.id))) }));
    if (options.length === 0) {
    ...
    return (_jsxs("div", { style: { position: 'relative' }, children: [_jsx("svg", { width: width, height: height, style: {
                    ...
                    whiteSpace: 'nowrap',
                    zIndex: 1000
                }, children: [processedOptions.find(opt => opt.id === hoveredOption)?.text, showPercentages && ` (${processedOptions.find(opt => opt.id === hoveredOption)?.percentage.toFixed(1)}%)`] }))] }));
};
