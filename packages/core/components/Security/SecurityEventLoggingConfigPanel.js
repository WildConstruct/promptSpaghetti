 ...
 */
export const SecurityEventLoggingConfigPanel = () => {
    ...
    const renderOverviewTab = () => (_jsxs("div", { className: "config-overview", children: [_jsxs("div", { className: "overview-header", children: [_jsx("h2", { children: "Security Event Logging Overview" }), _jsx("p", { children: "Configure comprehensive security event logging, monitoring, and compliance reporting" })] }), _jsxs(Card, { className: "status-card", children: [_jsxs("div", { className: "card-header", children: [_jsx("h3", { children: "System Status" }), _jsx(Badge, { variant: config.enabled ? 'success' : 'error', children: config.enabled ? 'Enabled' : 'Disabled' })] }), _jsxs("div", { className: "status-grid", children: [_jsxs("div", { className: "status-item", children: [_jsx("span", { className: "label", children: "Active Destinations" }), _jsx("span", { className: "value", children: config.destinations.filter(d => d.enabled).length })] }), _jsxs("div", { className: "status-item", children: [_jsx("span", { className: "label", children: "Monitored Event Types" }), _jsx("span", { className: "value", children: config.event_types.length })] }), _jsxs("div", { className: "status-item", children: [_jsx("span", { className: "label", children: "Active Alert Rules" }), _jsx("span", { className: "value", children: config.alert_rules.filter(r => r.enabled).length })] }), _jsxs("div", { className: "status-item", children: [_jsx("span", { className: "label", children: "Compliance Frameworks" }), _jsx("span", { className: "value", children: config.compliance_settings.frameworks.length })] })] })] }), _jsxs(Card, { className: "quick-actions", children: [_jsx("h3", { children: "Quick Actions" }), _jsxs("div", { className: "action-buttons", children: [_jsx(Button, { variant: "primary", onClick: () => setActiveTab('destinations'), children: "Configure Destinations" }), _jsx(Button, { variant: "secondary", onClick: () => setActiveTab('alerts'), children: "Manage Alert Rules" }), _jsx(Button, { variant: "secondary", onClick: () => setActiveTab('compliance'), children: "Compliance Settings" }), _jsx(Button, { variant: "outline", onClick: handleTestConfiguration, disabled: loading, children: "Test Configuration" })] })] }), _jsxs(Card, { className: "policies-card", children: [_jsx("h3", { children: "Security Event Policies" }), _jsx("div", { className: "policies-grid", children: policies.map(policy => (_jsxs("div", { className: "policy-item", children: [_jsxs("div", { className: "policy-header", children: [_jsx("span", { className: "policy-name", children: policy.policy_name }), _jsx(Badge, { variant: policy.enabled ? 'success' : 'warning', children: policy.enabled ? 'Active' : 'Disabled' })] }), _jsxs("div", { className: "policy-details", children: [_jsxs("span", { children: ["Event Types: ", policy.event_types.length] }), _jsxs("span", { children: ["Min Severity: ", policy.severity_threshold] }), _jsxs("span", { children: ["Compliance: ", policy.compliance_mapping.frameworks.join(', ')] })] })] }, policy.policy_id))) })] })] }));
    const renderDestinationsTab = () => (_jsx(LogDestinationManager, { destinations: config.destinations, onDestinationsChange: (destinations) => handleConfigChange({ destinations }) }));
    const renderEventTypesTab = () => (_jsx(EventTypeSelector, { selectedTypes: config.event_types, onSelectionChange: (event_types) => handleConfigChange({ event_types }) }));
    const renderAlertsTab = () => (_jsx(AlertRuleBuilder, { alertRules: config.alert_rules, onRulesChange: (alert_rules) => handleConfigChange({ alert_rules }) }));
    const renderRetentionTab = () => (_jsx(RetentionPolicyEditor, { policies: config.retention_policies, onPoliciesChange: (retention_policies) => handleConfigChange({ retention_policies }) }));
    const renderPerformanceTab = () => (_jsx(PerformanceSettingsPanel, { settings: config.performance_settings, onSettingsChange: (performance_settings) => handleConfigChange({ performance_settings }) }));
    const renderComplianceTab = () => (_jsx(ComplianceFrameworkSettings, { settings: config.compliance_settings, onSettingsChange: (compliance_settings) => handleConfigChange({ compliance_settings }) }));
    return (_jsxs("div", { className: "security-logging-config", children: [_jsxs("div", { className: "config-header", children: [_jsxs("div", { className: "header-content", children: [_jsx("h1", { children: "\uD83D\uDEE1\uFE0F Security Event Logging Configuration" }), _jsx("p", { children: "Manage security event logging, monitoring, and compliance settings" })] }), _jsxs("div", { className: "header-actions", children: [hasUnsavedChanges && (_jsx(Badge, { variant: "warning", children: "Unsaved Changes" })), _jsx(Button, { variant: "outline", onClick: () => loadConfiguration(), disabled: loading, children: "Refresh" }), _jsx(Button, { variant: "primary", onClick: handleSaveConfiguration, disabled: loading || !hasUnsavedChanges, children: loading ? 'Saving...' : 'Save Configuration' })] })] }), errors.length > 0 && (_jsxs(Alert, { variant: "error", className: "config-errors", children: [_jsx("strong", { children: "Configuration Errors:" }), _jsx("ul", { children: errors.map((error, index) => (_jsx("li", { children: error }, index))) })] })), _jsxs(Tabs, { value: activeTab, onValueChange: setActiveTab, className: "config-tabs", children: [_jsxs("div", { className: "tabs-list", children: [_jsx("button", { className: `tab-trigger ${activeTab === 'overview' ? 'active' : ''}`, onClick: () => setActiveTab('overview'), children: "Overview" }), _jsx("button", { className: `tab-trigger ${activeTab === 'destinations' ? 'active' : ''}`, onClick: () => setActiveTab('destinations'), children: "Destinations" }), _jsx("button", { className: `tab-trigger ${activeTab === 'events' ? 'active' : ''}`, onClick: () => setActiveTab('events'), children: "Event Types" }), _jsx("button", { className: `tab-trigger ${activeTab === 'alerts' ? 'active' : ''}`, onClick: () => setActiveTab('alerts'), children: "Alert Rules" }), _jsx("button", { className: `tab-trigger ${activeTab === 'retention' ? 'active' : ''}`, onClick: () => setActiveTab('retention'), children: "Retention" }), _jsx("button", { className: `tab-trigger ${activeTab === 'performance' ? 'active' : ''}`, onClick: () => setActiveTab('performance'), children: "Performance" }), _jsx("button", { className: `tab-trigger ${activeTab === 'compliance' ? 'active' : ''}`, onClick: () => setActiveTab('compliance'), children: "Compliance" })] }), _jsxs("div", { className: "tab-content", children: [activeTab === 'overview' && renderOverviewTab(), activeTab === 'destinations' && renderDestinationsTab(), activeTab === 'events' && renderEventTypesTab(), activeTab === 'alerts' && renderAlertsTab(), activeTab === 'retention' && renderRetentionTab(), activeTab === 'performance' && renderPerformanceTab(), activeTab === 'compliance' && renderComplianceTab()] })] }), showConfirmDialog && (_jsx(Dialog, { open: showConfirmDialog, onOpenChange: setShowConfirmDialog, title: "Unsaved Changes", description: "You have unsaved changes. Are you sure you want to leave this page?", children: _jsxs("div", { className: "dialog-actions", children: [_jsx(Button, { variant: "outline", onClick: () => setShowConfirmDialog(false), children: "Cancel" }), _jsx(Button, { variant: "primary", onClick: () => {
...
const EventTypeSelector = ({ selectedTypes, onSelectionChange }) => {
    ...
    return (_jsxs("div", { className: "event-type-selector", children: [_jsxs("div", { className: "selector-header", children: [_jsx("h2", { children: "Security Event Types" }), _jsx("p", { children: "Select which types of security events to monitor and log" }), _jsx("div", { className: "selection-summary", children: _jsxs(Badge, { variant: "info", children: [selectedTypes.length, " of ", Object.values(SecurityEventType).length, " types selected"] }) })] }), Object.entries(eventTypeCategories).map(([category, types]) => {
                const selectedCount = types.filter(type => selectedTypes.includes(type)).length;
                const allSelected = selectedCount === types.length;
                return (_jsxs(Card, { className: "category-card", children: [_jsxs("div", { className: "category-header", children: [_jsx(Checkbox, { checked: allSelected, indeterminate: selectedCount > 0 && selectedCount < types.length, onChange: () => handleCategoryToggle(category) }), _jsx("h3", { children: category }), _jsxs(Badge, { variant: selectedCount > 0 ? 'success' : 'default', children: [selectedCount, "/", types.length] })] }), _jsx("div", { className: "event-types-grid", children: types.map(eventType => (_jsxs("div", { className: "event-type-item", children: [_jsx(Checkbox, { checked: selectedTypes.includes(eventType), onChange: () => handleTypeToggle(eventType) }), _jsx("span", { className: "event-type-name", children: eventType.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase()) })] }, eventType))) })] }, category));
            })] }));
};
/**
 * Retention Policy Editor Component
 */
const RetentionPolicyEditor = ({ policies, onPoliciesChange }) => {
...
const ComplianceFrameworkSettings = ({ settings, onSettingsChange }) => {
    ...
    return (_jsxs("div", { className: "compliance-settings", children: [_jsxs("div", { className: "settings-header", children: [_jsx("h2", { children: "Compliance Framework Settings" }), _jsx("p", { children: "Configure regulatory compliance frameworks and validation rules" })] }), _jsxs("div", { className: "settings-sections", children: [_jsxs(Card, { className: "settings-section", children: [_jsx("h3", { children: "Enabled Frameworks" }), _jsx("div", { className: "frameworks-grid", children: Object.values(ComplianceFramework).map(framework => (_jsxs("label", { className: "framework-checkbox", children: [_jsx(Checkbox, { checked: settings.frameworks.includes(framework), onChange: (checked) => updateFrameworks(framework, checked) }), _jsxs("div", { className: "framework-info", children: [_jsx("span", { className: "framework-name", children: framework }), _jsx("span", { className: "framework-description", children: getFrameworkDescription(framework) })] })] }, framework))) })] }), _jsxs(Card, { className: "settings-section", children: [_jsx("h3", { children: "General Settings" }), _jsx("div", { className: "form-group", children: _jsxs("label", { children: [_jsx(Checkbox, { checked: settings.automated_reporting, onChange: (checked) => onSettingsChange({ ...settings, automated_reporting: checked }) }), "Enable Automated Compliance Reporting"] }) }), _jsx("div", { className: "form-group", children: _jsxs("label", { children: [_jsx(Checkbox, { checked: settings.external_notifications, onChange: (checked) => onSettingsChange({ ...settings, external_notifications: checked }) }), "Enable External Compliance Notifications"] }) })] }), _jsxs(Card, { className: "settings-section", children: [_jsx("h3", { children: "Validation Rules" }), _jsx("p", { children: "Define field validation rules for compliance frameworks" }), _jsx(Button, { variant: "outline", onClick: addValidationRule, className: "mb-4", children: "Add Validation Rule" }), _jsx("div", { className: "validation-rules", children: settings.validation_rules.map((rule, index) => (_jsxs("div", { className: "validation-rule", children: [_jsxs("div", { className: "rule-config", children: [_jsx(Select, { value: rule.framework, onValueChange: (value) => updateValidationRule(index, { framework: value }), children: Object.values(ComplianceFramework).map(framework => (_jsx("option", { value: framework, children: framework }, framework))) }), _jsx(Input, { value: rule.field, onChange: (e) => updateValidationRule(index, { field: e.target.value }), placeholder: "Field name" }), _jsxs("label", { children: [_jsx(Checkbox, { checked: rule.required, onChange: (checked) => updateValidationRule(index, { required: checked }) }), "Required"] }), _jsx(Button, { variant: "outline", size: "sm", onClick: () => removeValidationRule(index), children: "Remove" })] }), rule.pattern && (_jsx(Input, { value: rule.pattern, onChange: (e) => updateValidationRule(index, { pattern: e.target.value }), placeholder: "Validation pattern (regex)" }))] }, index))) }), settings.validation_rules.length === 0 && (_jsx("div", { className: "empty-validation-rules", children: _jsx("p", { children: "No validation rules configured. Add rules to enforce compliance requirements." }) }))] })] })] }));
};
