...
> = ({ alertRules, onRulesChange }) => { const [_____selectedRule, setSelectedRule] = useState<AlertRule | null>(null);
  ...
  const handleSaveRule = (rule: AlertRule) => {
    ...
    if (isNew) { onRulesChange([...alertRules, updatedRule]) } else { onRulesChange(alertRules.map(r => r.id === rule.id ? updatedRule : r));
  setEditingRule(null);
  setShowCreateDialog(false) };
  const handleDeleteRule = (ruleId: string) => { onRulesChange(alertRules.filter(r => r.id !== ruleId));
...
> = ({ rule, open, onClose, onSave }) => {
  ...
  const handleSave = () => { const validationErrors = validateRule(editedRule);
...
const BasicRuleSettings: React.FC<{ ,
  rule: AlertRule;
  onUpdate: (updates: Partial<AlertRule>) => void }> = ({ rule, onUpdate }) => {
  return;
...
const ConditionBuilder: React.FC<{ ,
  rule: AlertRule;
  onUpdate: (updates: Partial<AlertRule>) => void }> = ({ rule, onUpdate }) => { const addCondition = () => {
...
const ActionBuilder: React.FC<{ ,
  rule: AlertRule;
  onUpdate: (updates: Partial<AlertRule>) => void }> = ({ rule, onUpdate }) => { const addAction = (actionType: string) => {
...
const ActionConfigForm: React.FC<{ ,
  action: AlertAction;
  onUpdate: (updates: Partial<AlertAction>) => void }> = ({ action, onUpdate }) => {
...
const NotificationSettings: React.FC<{ ,
  rule: AlertRule;
  onUpdate: (updates: Partial<AlertRule>) => void }> = ({ rule, onUpdate }) => { return;
...
// Utility functions
function validateRule(rule: AlertRule): string { const errors: string = [];
  if (!rule.name.trim()) {
