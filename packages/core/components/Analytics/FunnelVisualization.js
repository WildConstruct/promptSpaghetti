 ...
 */
export const FunnelVisualization = ({ funnelDefinition, analyticsInfrastructure, timeRange, segments = [], cohorts = [], comparisonMode = 'none', realTimeUpdates = false, onStepClick, onConfigChange }) => {
...
const FunnelFilters = ({ filters, segments, cohorts, onFiltersChange }) => {
    ...
    return (_jsxs("div", { className: "funnel-filters", children: [_jsxs("div", { className: "filters-header", children: [_jsx("h3", { children: "Filters" }), _jsx(FilterDropdown, { onAddFilter: addFilter, segments: segments, cohorts: cohorts })] }), _jsx("div", { className: "active-filters", children: filters.map((filter, index) => (_jsx(FilterTag, { filter: filter, onUpdate: (updates) => updateFilter(index, updates), onRemove: () => removeFilter(index) }, index))) })] }));
};
const FilterDropdown = ({ onAddFilter, segments, cohorts }) => {
    ...
    return (_jsxs("div", { className: "filter-dropdown", children: [_jsx("button", { onClick: () => setIsOpen(!isOpen), children: "Add Filter +" }), isOpen && (_jsxs("div", { className: "filter-dropdown-menu", children: [_jsxs("div", { className: "filter-category", children: [_jsx("h4", { children: "Segments" }), segments.map(segment => (_jsx("button", { onClick: () => handleFilterAdd('segment', segment.id), children: segment.name }, segment.id)))] }), _jsxs("div", { className: "filter-category", children: [_jsx("h4", { children: "Cohorts" }), cohorts.map(cohort => (_jsx("button", { onClick: () => handleFilterAdd('cohort', cohort.id), children: cohort.name }, cohort.id)))] }), _jsxs("div", { className: "filter-category", children: [_jsx("h4", { children: "Other" }), _jsx("button", { onClick: () => handleFilterAdd('device'), children: "Device Type" }), _jsx("button", { onClick: () => handleFilterAdd('location'), children: "Location" }), _jsx("button", { onClick: () => handleFilterAdd('source'), children: "Traffic Source" })] })] }))] }));
};
const FilterTag = ({ filter, onUpdate, onRemove }) => {
    return (_jsxs("div", { className: "filter-tag", children: [_jsxs("span", { className: "filter-type", children: [filter.type, ":"] }), _jsx("input", { type: "text", value: filter.value, onChange: (e) => onUpdate({ value: e.target.value }), placeholder: "Enter value..." }), _jsxs("select", { value: filter.operator, onChange: (e) => onUpdate({ operator: e.target.value }), children: [_jsx("option", { value: "equals", children: "equals" }), _jsx("option", { value: "in", children: "in" }), _jsx("option", { value: "between", children: "between" }), _jsx("option", { value: "greater_than", children: "greater than" }), _jsx("option", { value: "less_than", children: "less than" })] }), _jsx("button", { onClick: onRemove, className: "remove-filter", children: "\u00D7" })] }));
};
const FunnelChart = ({ metrics, configuration, onStepClick }) => {
    ...
    return (_jsx("div", { className: "funnel-chart-container", children: chartComponent }));
};
const StandardFunnelChart = ({ metrics, configuration, onStepClick }) => {
    const maxUsers = Math.max(...metrics.stepMetrics.map(s => s.totalUsers));
    return (_jsx("div", { className: "standard-funnel-chart", children: metrics.stepMetrics.map((stepMetric, index) => {
            const width = (stepMetric.totalUsers / maxUsers) * 100;
            const isLastStep = index === metrics.stepMetrics.length - 1;
            return (_jsxs("div", { className: "funnel-step", children: [_jsx("div", { className: `step-bar ${getStepColorClass(stepMetric, configuration.colorScheme)}`, style: { width: `${width}%` }, onClick: () => onStepClick(stepMetric), children: _jsxs("div", { className: "step-content", children: [_jsx("div", { className: "step-name", children: stepMetric.name }), _jsxs("div", { className: "step-metrics", children: [configuration.showMetrics.includes('user_count') && (_jsxs("span", { className: "metric", children: [stepMetric.totalUsers.toLocaleString(), " users"] })), configuration.showMetrics.includes('conversion_rate') && !isLastStep && (_jsxs("span", { className: "metric", children: [stepMetric.conversionRate.toFixed(1), "% convert"] })), configuration.showMetrics.includes('drop_off_rate') && !isLastStep && (_jsxs("span", { className: "metric", children: [stepMetric.dropOffRate.toFixed(1), "% drop off"] })), configuration.showMetrics.includes('value_generated') && stepMetric.valueGenerated > 0 && (_jsxs("span", { className: "metric", children: ["$", stepMetric.valueGenerated.toLocaleString()] }))] })] }) }), !isLastStep && (_jsx("div", { className: "step-connector", children: _jsxs("div", { className: "drop-off-indicator", children: [stepMetric.dropOffRate.toFixed(1), "% drop off"] }) }))] }, stepMetric.stepId));
        }) }));
};
...
const ComparisonMetric = ({ label, baseline, comparison, format }) => {
    ...
    return (_jsxs("div", { className: "comparison-metric", children: [_jsx("div", { className: "metric-label", children: label }), _jsxs("div", { className: "metric-values", children: [_jsx("span", { className: "baseline", children: formatValue(baseline) }), _jsx("span", { className: "arrow", children: "\u2192" }), _jsx("span", { className: "comparison", children: formatValue(comparison) })] }), _jsxs("div", { className: `metric-change ${changeDirection}`, children: [change > 0 ? '+' : '', change.toFixed(1), "%"] })] }));
};
const InsightCard = ({ insight }) => {
    return (_jsxs("div", { className: `insight-card ${insight.type}`, children: [_jsxs("div", { className: "insight-header", children: [_jsx("span", { className: "insight-type", children: insight.type }), insight.stepId && _jsxs("span", { className: "insight-step", children: ["Step: ", insight.stepId] })] }), _jsxs("div", { className: "insight-content", children: [_jsx("div", { className: "insight-description", children: insight.description }), insight.recommendation && (_jsxs("div", { className: "insight-recommendation", children: [_jsx("strong", { children: "Recommendation:" }), " ", insight.recommendation] }))] }), _jsxs("div", { className: "insight-stats", children: [_jsxs("span", { className: "change", children: [insight.change > 0 ? '+' : '', insight.change.toFixed(1), "%"] }), _jsxs("span", { className: "significance", children: ["Significance: ", (insight.significance * 100).toFixed(1), "%"] })] })] }));
};
const FunnelInsights = ({ metrics, comparisonData, funnelDefinition }) => {
    const insights = useMemo(() => {
        return generateFunnelInsights(metrics, comparisonData, funnelDefinition);
    }, [metrics, comparisonData, funnelDefinition]);
    return (_jsxs("div", { className: "funnel-insights", children: [_jsx("h3", { children: "Funnel Insights" }), _jsxs("div", { className: "insights-grid", children: [_jsxs("div", { className: "insight-section", children: [_jsx("h4", { children: "Biggest Drop-off Points" }), insights.biggestDropOffs.map((dropOff, index) => (_jsxs("div", { className: "drop-off-insight", children: [_jsx("span", { className: "step-name", children: dropOff.stepName }), _jsxs("span", { className: "drop-off-rate", children: [dropOff.dropOffRate.toFixed(1), "%"] }), _jsxs("span", { className: "affected-users", children: [dropOff.affectedUsers.toLocaleString(), " users"] })] }, index)))] }), _jsxs("div", { className: "insight-section", children: [_jsx("h4", { children: "Conversion Opportunities" }), insights.opportunities.map((opportunity, index) => (_jsxs("div", { className: "opportunity-insight", children: [_jsx("div", { className: "opportunity-description", children: opportunity.description }), _jsxs("div", { className: "opportunity-impact", children: ["Potential impact: +", opportunity.potentialImpact.toFixed(1), "% conversion rate"] })] }, index)))] }), _jsxs("div", { className: "insight-section", children: [_jsx("h4", { children: "Performance Trends" }), insights.trends.map((trend, index) => (_jsxs("div", { className: "trend-insight", children: [_jsx("span", { className: "trend-description", children: trend.description }), _jsx("span", { className: `trend-direction ${trend.direction}`, children: trend.direction === 'up' ? '↗' : trend.direction === 'down' ? '↘' : '→' })] }, index)))] })] })] }));
};
// Loading and Error States
const FunnelLoadingState = () => (_jsxs("div", { className: "funnel-loading", children: [_jsx("div", { className: "loading-spinner" }), _jsx("p", { children: "Loading funnel data..." })] }));
const FunnelErrorState = ({ error, onRetry }) => (_jsxs("div", { className: "funnel-error", children: [_jsxs("div", { className: "error-message", children: [_jsx("h3", { children: "Error Loading Funnel" }), _jsx("p", { children: error })] }), _jsx("button", { onClick: onRetry, className: "retry-button", children: "Retry" })] }));
// Utility Functions
...
async function processFunnelMetrics(metricResults, funnelDefinition) {
    // Simplified implementation - in production, this would process actual metric results
    const stepMetrics = funnelDefinition.steps.map((step, index) => {
...
async function loadComparisonData(query, comparisonMode) {
    // Simplified implementation - in production, this would load actual comparison data
    return null;
}
function generateFunnelInsights(metrics, comparisonData, funnelDefinition) {
