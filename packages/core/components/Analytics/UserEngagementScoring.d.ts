...
export interface ModelValidation { accuracy: number;
    precision: number;
    recall: number;
    f1Score: number;
...
export interface ScoringModelPerformance { processingTime: number;
    throughput: number;
    memoryUsage: number;
    errorRate: number;
    drift: number }
...
export interface DimensionWeights { [dimensionId: string]: number }
}
}
...
export interface DemographicProfile { ageRange?: {
        min: number;
        max: number }
}
    };
...
export interface PlatformPreference { web: number;
    mobile: number;
    desktop: number }
}
}
...
export interface InteractionQualityMetrics { intentionality: number;
    efficiency: number;
    completion: number;
    satisfaction: number }
}
}
export interface BehaviorMetrics { engagementConsistency: number;
...
export interface SocialInfluenceData { socialEngagement: number;
...
export interface ScoreContext { events: string[];
    factors: string[];
    anomalies: string[] }
}
}
...
export interface PersonalizationImpact { engagementLift: number;
