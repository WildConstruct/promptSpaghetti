...
const generateMockSession = () => ({
    ...
    pageViews: Array.from({ length: Math.floor(Math.random() * 8) + 1 }, () => ({
        ...
        exitType: ['navigation', 'close', 'refresh', 'timeout'][Math.floor(Math.random() * 4)]
    })),
    interactions: Array.from({ length: Math.floor(Math.random() * 100) + 20 }, () => ({
        ...
        type: ['click', 'scroll', 'hover', 'keypress', 'form_input'][Math.floor(Math.random() * 5)],
        element: {
...
// Main component
export const UserSessionRecording = ({ sessionConfig, analyticsInfrastructure, privacySettings, replayEnabled = true, analyticsEnabled = true, onSessionAnalyzed, onPatternDetected, onExport }) => {
