...
export const [error, setError] = useState<string | null>(null);
  ...
  const loadAttributionData = useCallback(async () => { try {
      setLoading(true);
      setError(null);
  ...
  funnelId: funnelDefinition.id,
      ...
      if (result.success && result.data) { const analysisData = await processAttributionData(;);
          result.data,
          attributionConfig }
          channels
        );
 ...
 else { setError(result.error || 'Failed to load attribution analysis data') } catch (err) { setError(err instanceof Error ? err.message : 'Unknown error occurred') } finally { setLoading(false) }, [funnelDefinition, analyticsInfrastructure, timeRange, segments, cohorts, attributionConfig, channels, onInsightGenerated]);
  ...
  const generateModelComparison = (models: AttributionModel): AttributionModelComparison => { const comparisons: AttributionModelComparison = [];
  ...
  const generateTouchpointAnalysis = (): TouchpointAnalysisData => { return funnelDefinition.steps.map(step => ({)
  step,
  ...
  const generateCrossChannelInsights = (channelList: MarketingChannel): CrossChannelInsight => { return [
  {
  insightType: 'synergy',
  ...
  const generateBudgetRecommendations = (channelList: MarketingChannel): BudgetAllocationRecommendation => { return channelList.map(channel => ({)
  channelId: channel.id,
  channelName: channel.name,
  ...
  const generateROIAnalysis = (channelList: MarketingChannel): ChannelROIAnalysis => { return channelList.map(channel => ({)
  channelId: channel.id,
  channelName: channel.name,
  cost: channel.cost,
  ...
  const generateConversionPaths = (channelList: MarketingChannel): ConversionPathData => { const paths = [
  ...
  const generateAttributionTrends = (channelList: MarketingChannel): AttributionTrendData => { const periods = ['Week 1', 'Week 2', 'Week 3', 'Week 4'];
  ...
  const generateAttributionInsights = (data: AttributionAnalysisData): AttributionInsight => { return [
      {
  ...
  // Initial data load
  useEffect(() => { loadAttributionData() }, [loadAttributionData]);
  // Handle export
  const handleExport = useCallback(() => { if (!attributionData || !onExport) return;
