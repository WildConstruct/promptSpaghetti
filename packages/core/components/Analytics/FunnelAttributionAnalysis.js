...
export const FunnelAttributionAnalysis = ({ funnelDefinition, analyticsInfrastructure, timeRange, attributionConfig = defaultAttributionConfig, channels = [], segments = [], cohorts = [], comparisonMode = 'channel_comparison', onInsightGenerated, onExport }) => {
    ...
    const generateChannelAttributionData = (channelList, models) => {
        return channelList.map(channel => ({
            ...
            journeyRole: {
                primaryRole: ['discovery', 'consideration', 'conversion', 'retention'][Math.floor(Math.random() * 4)],
                roleDistribution: {
