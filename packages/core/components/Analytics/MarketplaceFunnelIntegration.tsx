...
export const [error, setError] = useState<string | null>(null);
  ...
  const loadIntegrationData = useCallback(async () => { try {
      setLoading(true);
      setError(null);
  ...
  funnelId: funnelDefinition.id }
      ...
      if (marketplaceContext.templateContext) { query.filters?.push(
          { field: 'template_id'
          operator: 'eq' }
          value: marketplaceContext.templateContext.templateId }
        );
      if (marketplaceContext.creatorContext) { query.filters?.push(
          { field: 'creator_id'
          operator: 'eq' }
          value: marketplaceContext.creatorContext.creatorId }
        );
      const result = await analyticsInfrastructure.executeQuery(query);
      if (result.success && result.data) { const processedData = await processIntegrationData(;);
          result.data
          marketplaceContext }
          userRole
        );
        setIntegrationData(processedData);
 else { setError(result.error || 'Failed to load marketplace integration data') } catch (err) { setError(err instanceof Error ? err.message : 'Unknown error occurred') } finally { setLoading(false) }, [funnelDefinition, analyticsInfrastructure, marketplaceContext, userRole]);
  ...
  const generateFunnelSummary = (): FunnelSummaryData => { return {
  ...
  const generateTemplateInsights = (templateContext: TemplateContext): TemplateInsightData => { return [
  {
  ...
  const generateCreatorOptimizations = (creatorContext: CreatorContext): CreatorOptimizationData => { return [
  {
  ...
  const generateMarketplaceMetrics = (): MarketplaceMetricData => { return {
  ...
  const generateRecommendedActions = (role: UserRole): RecommendedAction => { return [
  {
  ...
  const generatePerformanceAlerts = (): PerformanceAlert => { return [
  {
  ...
  const generateIntegrationHealth = (): IntegrationHealthData => { return {
  ...
  const handleRefresh = useCallback(() => { setRefreshing(true);
    loadIntegrationData().finally(() => setRefreshing(false)) }, [loadIntegrationData]);
  // Handle optimization action
  const handleOptimizationAction = useCallback((actionType: string, details: Record<string, any>) => { const action: OptimizationAction = {
  actionType: actionType as any
  details
  userId: marketplaceContext.creatorContext?.creatorId || marketplaceContext.adminContext?.adminId || 'anonymous'
  timestamp: Date.now() }
};
    if (onOptimizationAction) { onOptimizationAction(action) }, [marketplaceContext, onOptimizationAction]);
  ...
  context
    ...
    if (onInsightInteraction) { onInsightInteraction(interaction) }, [marketplaceContext, onInsightInteraction]);
  // Setup auto-refresh
  useEffect(() => { if (widgetConfig.refreshInterval > 0) {
  ...
  // Initial data load
  useEffect(() => { loadIntegrationData() }, [loadIntegrationData]);
  // Handle export
  const handleExport = useCallback(() => { if (!integrationData || !onExport) return;
