...
export const RecommendationEffectivenessTracking: React.FC<RecommendationEffectivenessTrackingProps> = ({ )
  ...
  const handleAnalyzeEffectiveness = useCallback(() => { setLoading(true);
  setTimeout(() => {
  setLoading(false);
  ...
  return currentScore > bestScore ? current : best;
          ...
          averageCTR: algorithmMetrics.reduce((sum, algo) => { const ctr = algo.metrics.find(m => m.metricId === 'click_through_rate')?.value || 0;
            return sum + ctr }, 0) / algorithmMetrics.length,
  ...
  const overallStats = useMemo(() => { if (!algorithmMetrics.length) return null;
    const avgCTR = algorithmMetrics.reduce((sum, algo) => {
      const ctr = algo.metrics.find(m => m.metricId === 'click_through_rate')?.value || 0;
      return sum + ctr }, 0) / algorithmMetrics.length;
    const avgConversion = algorithmMetrics.reduce((sum, algo) => { const conv = algo.metrics.find(m => m.metricId === 'conversion_rate')?.value || 0;
      return sum + conv }, 0) / algorithmMetrics.length;
    const totalRevenueImpact = algorithmMetrics.reduce((sum, algo) => sum + algo.businessImpact.revenueImpact, 0);
    return { totalAlgorithms: algorithmMetrics.length,
  ...
  const selectedAlgorithmData = useMemo(() => { return selectedAlgorithm ? algorithmMetrics.find(a => a.algorithmId === selectedAlgorithm) : null }, [selectedAlgorithm, algorithmMetrics]);
  return;
