...
interface PerformanceMetrics { total_rules: number;
  active_rules: number;
...
interface CorrectionsStatsDashboardProps { isOpen: boolean;
  onClose: () => void }

export const CorrectionsStatsDashboard: React.FC<CorrectionsStatsDashboardProps> = ({ isOpen }
  onClose
  ...
  const fetchMetrics = async () => {
    try {
      ...
      if (response.ok) { const data = await response.json();
        ...
        setError(null) } else { setError('Failed to fetch statistics') }
 catch (err) { setError('Failed to fetch statistics') } finally { setLoading(false) }
  };
