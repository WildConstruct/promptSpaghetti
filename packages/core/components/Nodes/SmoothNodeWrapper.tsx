...
export interface SmoothNodeWrapperProps extends NodeProps { children: React.ReactNode;
  nodeType?: string;
  isSelected?: boolean;
  ...
  useEffect(() => {
    if (nodeRef.current) {
      ...
      if (globalAnimationManager.registerAnimation(animationId)) { setIsAnimating(true);
        setTimeout(() => {
          setIsAnimating(false);
          globalAnimationManager.unregisterAnimation(animationId) }, animationDurations.complex);
  }, [id]);
  ...
  const handleClick = useCallback((e: React.MouseEvent) => { e.stopPropagation();
    ...
    onNodeClick?.(id) }, [id, onNodeClick]);
  const handleDoubleClick = useCallback((e: React.MouseEvent) => { e.stopPropagation();
    onNodeDoubleClick?.(id) }, [id, onNodeDoubleClick]);
  ...
  const handleMouseEnter = useCallback(() => { setIsHovered(true);
    ...
    hoverTimeoutRef.current = setTimeout(() => {
      if (nodeRef.current) {
        nodeRef.current.style.transform = 'translateY(-2px) scale(1.02)' }, 50);
  }, []);
  ...
  const getNodeColors = () => { const colors = {
  'WeightedChoice': {
  primary: '#3b82f6'
  secondary: '#1e40af'
  accent: '#60a5fa'
  ...
  const getNodeStyle = (): React.CSSProperties => { const baseStyle: React.CSSProperties = {
  position: 'relative'
      borderRadius: 12
...
export interface NodeCreationAnimatorProps { children: React.ReactNode }
  isCreating: boolean;
  ...
  onAnimationComplete
    ...
    if (isCreating && onAnimationComplete) {
      const timer = setTimeout(() => {
        onAnimationComplete() }, animationDurations.complex);
