...
interface SharePreviewProps { shareConfig: ShareConfig;
  shareLink: ShareLink;
  ...
  fontSize: '14px' }
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div>{shareConfig.title}</div>
            <div style={{ fontSize: '12px', marginTop: '4px' }}>
              Interactive {shareConfig.resourceType}
            </div>
          </div>
        </div>
      </div>
  ...
  color: '#374151' }
        ...
        <textarea
          value={shareLink.embedCode?.responsive || ''}
          readOnly
          style={ {
  width: '100%'
  height: '80px'
  ...
  const renderPreview = () => { switch (activePreview) {
  case 'twitter':,
