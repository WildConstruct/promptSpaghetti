...
export const ShareAnalyticsDashboard: React.FC<ShareAnalyticsDashboardProps> = ({ )
  ...
  useEffect(() => { const fetchMetrics = async () => {
  setIsLoading(true);
  setError(null);
  try {
 ...
 catch (err) { setError(err instanceof Error ? err.message : 'Failed to load metrics') } finally { setIsLoading(false) };
    fetchMetrics();
  ...
  const formatNumber = (num: number) => {
    ...
    return num.toString() };
  const formatPercentage = (num: number) => { return (num * 100).toFixed(1) + '%' };
  if (isLoading) { return;
  <div style={{
  padding: '40px'
  textAlign: 'center'
