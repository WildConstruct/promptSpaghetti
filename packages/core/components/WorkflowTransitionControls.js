...
export const WorkflowTransitionControls = ({ resourceId, currentStateId, currentUserId, workspaceId, onTransitionComplete, onApprovalRequested, disabled = false }) => {
    ...
    const executeTransition = async (transition, transitionComment) => {
        if (!transition)
            return;
        try {
            const result = await transitionResourceState(resourceId, transition.to_state_id, currentUserId, {
    ...
    const getTransitionButton = (transition) => {
        ...
        return (_jsxs("button", { onClick: () => handleTransitionClick(transition), disabled: isDisabled, className: buttonClass, title: validation.error || transition.description || `Transition to ${toState.name}`, children: [getTransitionIcon(transition), _jsx("span", { children: transition.name }), _jsx(ArrowRightIcon, { className: "h-3 w-3" }), _jsx("span", { className: "px-2 py-1 rounded text-xs", style: { backgroundColor: `${toState.color}20`, color: toState.color }, children: toState.name }), transition.requires_approval && (_jsx("span", { className: "text-xs bg-yellow-200 text-yellow-800 px-1 rounded", children: "Approval Required" }))] }, transition.id));
    };
    if (loading) {
        return (_jsxs("div", { className: "flex items-center space-x-2 text-sm text-gray-500", children: [_jsx("div", { className: "animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600" }), _jsx("span", { children: "Loading transitions..." })] }));
    }
    if (error) {
        return (_jsxs("div", { className: "flex items-center space-x-2 text-sm text-red-600", children: [_jsx(ExclamationTriangleIcon, { className: "h-4 w-4" }), _jsx("span", { children: "Error loading transitions" })] }));
    }
    if (!currentState) {
        return (_jsx("div", { className: "text-sm text-gray-500", children: "Current state not found" }));
    }
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center space-x-2 text-sm", children: [_jsx("span", { className: "text-gray-600", children: "Current state:" }), _jsx("span", { className: "px-2 py-1 rounded text-sm font-medium", style: { backgroundColor: `${currentState.color}20`, color: currentState.color }, children: currentState.name }), resourceLocked && (_jsxs("span", { className: "flex items-center space-x-1 px-2 py-1 bg-red-100 text-red-800 text-xs rounded", children: [_jsx(LockClosedIcon, { className: "h-3 w-3" }), _jsx("span", { children: "Locked" })] }))] }), availableTransitions.length === 0 ? (_jsx("div", { className: "text-sm text-gray-500", children: "No transitions available from this state" })) : (_jsxs("div", { className: "space-y-2", children: [_jsx("div", { className: "text-sm font-medium text-gray-700", children: "Available transitions:" }), _jsx("div", { className: "flex flex-wrap gap-2", children: availableTransitions.map(getTransitionButton) })] })), resourceLocked && (_jsxs("div", { className: "flex items-center space-x-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800", children: [_jsx(ExclamationTriangleIcon, { className: "h-4 w-4" }), _jsx("span", { children: "Resource is locked for state changes" })] })), showCommentDialog && selectedTransition && (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: _jsxs("div", { className: "bg-white rounded-lg p-6 max-w-md w-full mx-4", children: [_jsxs("h3", { className: "text-lg font-medium text-gray-900 mb-4", children: ["Confirm Transition: ", selectedTransition.name] }), selectedTransition.description && (_jsx("p", { className: "text-sm text-gray-600 mb-4", children: selectedTransition.description })), selectedTransition.requires_approval && (_jsxs("div", { className: "flex items-center space-x-2 p-3 bg-yellow-50 border border-yellow-200 rounded mb-4", children: [_jsx(ClockIcon, { className: "h-4 w-4 text-yellow-500" }), _jsx("span", { className: "text-sm text-yellow-800", children: "This transition requires approval" })] })), _jsxs("div", { className: "mb-4", children: [_jsxs("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: ["Comment ", selectedTransition.requires_approval ? '(required)' : '(optional)'] }), _jsx("textarea", { value: comment, onChange: (e) => setComment(e.target.value), className: "w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500", rows: 3, placeholder: "Add a comment about this transition..." })] }), _jsxs("div", { className: "flex justify-end space-x-3", children: [_jsx("button", { onClick: () => {
                                        setShowCommentDialog(false);
                                        setSelectedTransition(null);
                                        setComment('');
