...
export const EditorProviderWrapper: React.FC<EditorProviderWrapperProps> = ({ )
 ...
 = useEditorProviders()
  ...
  useEffect(() => { const registerBuiltInProvider = async (hookFactory: () => ProviderHook) => { }
  try { const hook = hookFactory();
  registry.register(hook);
  onProviderRegistered?.(hook) } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  ...
  useEffect(() => { providers.forEach(provider => {)
  try {
        registry.register(provider);
        onProviderRegistered?.(provider) } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  ...
  try {
          ...
          onProviderUnregistered?.(provider.id) } catch (error) { console.error('[EditorProvider] Failed to unregister provider:', provider.id, error) });
    };
  ...
  return newErrors });
 catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
      ...
      for (const hook of hooks) {
        try {
          ...
          if (typeof hookFn === 'function') {
            await (hookFn as any)(...args) } catch (error) {
  ...
  executeCustomAction: (hookId: string, actionName: string, ...args: unknown) => { try {
        return registry.executeCustomAction(hookId, actionName, ...args) } catch (error) {
        const errorObj = error instanceof Error ? error : new Error('Unknown error');
  ...
  try { await editorActions.addNode(node) } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  console.error('[EditorProvider] Error adding node:', errorObj);
  onProviderError?.(errorObj, 'addNode');
  throw error }
  updateNode: async (nodeId: string, data: Record<string, unknown>) => { try {
        await editorActions.updateNode(nodeId, data) } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  console.error('[EditorProvider] Error updating node:', errorObj);
  onProviderError?.(errorObj, 'updateNode');
  throw error }
  removeNode: async (nodeId: string) => { 
      try {
        await editorActions.removeNode(nodeId) } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  console.error('[EditorProvider] Error removing node:', errorObj);
  onProviderError?.(errorObj, 'removeNode');
  throw error }
  executeGraph: async () => { 
      try {
        return await editorActions.executeGraph() } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  console.error('[EditorProvider] Error executing graph:', errorObj);
  onProviderError?.(errorObj, 'executeGraph');
  throw error }), [editorActions, onProviderError]);
  return;
    <>
      { children({)
...
export const withEditorProviders = <T extends {}>()
  ...
  initialEdges: Edge;
  selectedNodeId: string | null;
  validationErrors?: unknown }>((props, ref) => { const {
      initialNodes
      initialEdges
      selectedNodeId
      validationErrors }
