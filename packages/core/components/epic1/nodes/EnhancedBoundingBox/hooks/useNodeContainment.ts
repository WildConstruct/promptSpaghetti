...
export function useNodeContainment(
  ...
  const containedNodes = useMemo(() => {
    ...
    if (cached && Date.now() - cached.timestamp < BOUNDING_BOX_CONSTANTS.performance.CACHE_TTL) {
      hitCountRef.current++;
      perfMonitor.record('boundingBox.cacheHit', 1);
      console.debug(`[Cache Hit] Containment for ${boxId}: ${cached.nodes.length} nodes`);
      return cached.nodes;
    }
    ...
    console.debug(`[Cache Miss] Calculated containment for ${boxId}: ${contained.length} nodes in ${duration.toFixed(2)}ms`);
    
    return contained;
  }, [boxId, allNodes, boxPosition.x, boxPosition.y, checkSize.width, checkSize.height, isCollapsed]);
