...
import { ContextualTooltips, TooltipConfig } from './ContextualTooltips';
...
export const TooltipManagerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [activeTooltips, setActiveTooltips] = useState<TooltipConfig[]>([]);
  const [queue, setQueue] = useState<TooltipQueue>({ tooltips: [], currentIndex: 0 });
  ...
  const value: TooltipManagerContextType = {
    showTooltip,
    hideTooltip,
    hideAllTooltips,
    queueTooltips,
  ...
  return (
    <TooltipManagerContext.Provider value={value}>
      {children}
      <ContextualTooltips 
        additionalTooltips={activeTooltips}
        enabled={onboardingState.preferences.showTooltips}
      />
...
export const useTooltipSequence = () => {
  ...
  const startSequence = useCallback((sequence: keyof typeof tooltipPresets) => {
