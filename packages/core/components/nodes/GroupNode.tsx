...
const GroupNode: React.FC<NodeProps<GroupNodeData>> = memo(({ 
  ...
  const { measure } = useRenderPerformance('GroupNode');
  ...
  }, [group.id, onToggle, measure]);

  const handleNameEdit = useCallback((e: React.FocusEvent<HTMLInputElement>) => {
    if (!onEdit) return;
    
    const newName = e.target.value.trim();
    if (newName && newName !== group.name) {
      measure('group:rename', () => {
        onEdit(group.id, { name: newName });
      });
    }
    setIsEditing(false);
  }, [group.id, group.name, onEdit, measure]);
    ...
    >
      ...
      <div style={{
        ...
        {performanceMetrics?.lastOperationTime && (
          <span style={{
            fontSize: '10px',
            color: performanceMetrics.lastOperationTime > 50 ? '#ff6b6b' : '#51cf66'
          }}>
