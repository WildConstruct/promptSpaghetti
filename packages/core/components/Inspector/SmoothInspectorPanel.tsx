...
const getFilmmakerFriendlyName = (nodeType: string): string => { const friendlyNames: Record<string, string> = {
...
export const SmoothInspectorPanel = ({ )
 ...
 = useUISettingsStore();
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => { if (!isResizing) return;
    ...
    setWidth(clampedWidth) }, [isResizing, minWidth, maxWidth]);
  ...
  const handleCollapse = useCallback(() => { setCollapsed(!collapsed);
    if (isMaximized) {
      setIsMaximized(false) }, [collapsed, isMaximized]);
  // Handle maximize with animation
  const handleMaximize = useCallback(() => { setIsMaximized(!isMaximized);
    if (collapsed) {
      setCollapsed(false) }, [isMaximized, collapsed]);
  // Debug mode toggle with animation
  const handleDebugToggle = useCallback(() => { setDebugMode(!debugMode) }, [debugMode, setDebugMode]);
  if (!node || !schema) {
    return;
