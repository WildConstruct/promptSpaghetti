...
const getFilmmakerFriendlyError = (message: string): string => { const errorMappings: Record<string, string> = {
...
export interface BaseNodeEditorProps { nodeId: string;
...
export interface EditorFieldProps { label: string;
  value: unknown;
  fieldKey: string;
  zodType: ZodTypeAny;
  error?: string;
...
export const BaseNodeEditor: React.FC<BaseNodeEditorProps> = ({ )
  ...
  const updateField = (key: string, val: unknown) => {
    ...
    if (schema) { try {
        ...
        if (fieldSchema) { const parsed = fieldSchema.safeParse(val);
  setFieldErrors((prev) => ({ )
  ...prev
  ...
  const getFieldSchema = (key: string): ZodTypeAny | null => {
    ...
    try {
      ...
      if (typeof shape === 'function') { return (shape() as Record<string, unknown>)[key] ?? null } else if (shape) { return shape[key] ?? null } else if ((schema as { _def?: { shape?: unknown } })._def?.shape) {
 ...
 catch (error) { console.warn('Error getting field schema:', key, error);
  return null };
  const renderField = (key: string): React.ReactNode => { const zodType = getFieldSchema(key);
  ...
  onChange: (value) => updateField(key, value) }
    ...
    if (children && React.isValidElement(children)) { return React.cloneElement(children as React.ReactElement, {)
  key }
        ...fieldProps
      });
  ...
  const getFieldKeys = (): string => {
    ...
    try {
      ...
      if ((schema as { shape?: Record<string, unknown> }).shape) {
        ...
        if (typeof maybeShape === 'function') { shape = maybeShape() } else if (maybeShape) { shape = maybeShape } else if ((schema as { _def?: { shape?: unknown } })._def?.shape) {
        const s = (schema as { _def: { shape: unknown } })._def.shape;
 ...
 catch (error) { console.warn('Error getting field keys:', error);
  return [] };
