...
export interface VariationListProps { nodeId: string;
  variations: string;
...
export const VariationList: React.FC<VariationListProps> = ({ nodeId
  variations
  onAdd
  onRemove
  onUpdate
  ...
  useEffect(() => { if (editingIndex !== null && editInputRef.current) {
      editInputRef.current.focus();
      editInputRef.current.select() }
  }, [editingIndex]);
  const handleAdd = () => { if (!newVariation.trim()) return;
    if (onAdd) {
      onAdd(newVariation.trim()) } else { addVariation(nodeId, newVariation.trim());
    setNewVariation('') };
  const handleRemove = (index: number) => { if (onRemove) {
      onRemove(index) } else { removeVariation(nodeId, index) };
  const handleUpdate = (index: number, newValue: string) => { if (onUpdate) {
      onUpdate(index, newValue) } else { updateVariation(nodeId, index, newValue) };
  const handleReorder = (fromIndex: number, toIndex: number) => { if (onReorder) {
      onReorder(fromIndex, toIndex) } else { reorderVariations(nodeId, fromIndex, toIndex) };
  const startEdit = (index: number) => { setEditingIndex(index);
    setEditingValue(variations[index]) };
  const commitEdit = () => { if (editingIndex !== null && editingValue.trim()) {
  ...
  const handleDrop = (e: React.DragEvent, index: number) => { e.preventDefault();
              ...
              {editingIndex === index ? (
                <input
                  ...
                  onKeyDown={ (e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      commitEdit() } else if (e.key === 'Escape') { cancelEdit() }}
                  onBlur={commitEdit}
                  style={ {
  flex: 1
  padding: '4px 8px'
