...
interface CorrectionSuggestion { id: string;
  ruleId: string;
  ruleName: string;
  original: string;
  suggested: string;
  ...
  ruleId: rule.id
    ...
    return suggestions.sort((a, b) => { if (a.confidence !== b.confidence) {
        return b.confidence - a.confidence;
      return a.start - b.start });
  }, [getActiveRules, enableInlineCorrections]);
  // Update suggestions when text changes
  useEffect(() => { if (enableInlineCorrections && localValue) {
      const newSuggestions = generateSuggestions(localValue);
      setSuggestions(newSuggestions) } else { setSuggestions([]) }, [localValue, generateSuggestions, enableInlineCorrections]);
  // Auto-apply corrections if enabled
  useEffect(() => { if (autoApplyCorrections && localValue && !isFocused) {
  ...
  const handleChange = (newValue: string) => { setLocalValue(newValue);
    onChange(newValue) };
  const handleCursorPositionChange = () => { if (textareaRef.current) {
      setCursorPosition(textareaRef.current.selectionStart) };
  const applySuggestion = (suggestion: CorrectionSuggestion) => { const newText = localValue.slice(0, suggestion.start) + ;
  ...
  const dismissSuggestion = (suggestionId: string) => { setSuggestions(prev => prev.filter(s => s.id !== suggestionId)) };
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => { // Ctrl+Enter to apply all suggestions
  ...
  const getWordCount = (text: string): number => { return text.trim().split(/\\s+/).filter(word => word.length > 0).length };
  const escapeRegExp = (string: string): string => {
    return string.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\<HTMLTextAreaElement>) => { // Ctrl+Enter to apply all suggestions
  if (e.ctrlKey && e.key === 'Enter') {
  e.preventDefault();
  const correctedText = applyCorrections(localValue);
  if (correctedText !== localValue) {
  ...
  const suggestionStyle = { position: 'absolute' as const,
  top: '100%',
  left: 0,
  right: 0,
  background: '#1a202c',
