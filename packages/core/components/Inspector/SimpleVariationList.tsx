...
export interface SimpleVariationListProps { variations: string;
...
export const SimpleVariationList: React.FC<SimpleVariationListProps> = ({ )
  ...
  const handleAdd = () => {
    if (newItem.trim()) {
      ...
      setNewItem('') };
  const handleRemove = (index: number) => { onChange(variations.filter((_, i) => i !== index)) };
  const handleUpdate = (index: number, value: string) => { const updated = [...variations];
    updated[index] = value;
    onChange(updated) };
