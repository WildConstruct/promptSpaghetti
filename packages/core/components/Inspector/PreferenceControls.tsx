...
export interface PreferenceControlsProps { nodeId?: string;
  nodeType?: string;
...
const PreferenceControls: React.FC<PreferenceControlsProps> = ()
 ...
 = useUISettingsStore();
  ...
  const handleGlobalLevelChange = (level: string) => { setGlobalDisclosureLevel(level as 'basic' | 'advanced' | 'debug') };
  const handleNodeLevelChange = (level: string) => { if (nodeId) {
      setNodeDisclosureLevel(nodeId, level as 'basic' | 'advanced' | 'debug') };
  const handleUseGlobalToggle = (useGlobal: boolean) => { if (nodeId) {
      setNodeUseGlobalDefault(nodeId, useGlobal) };
  const handleInheritanceChange = (inheritance: string) => { setPreferenceInheritance(inheritance as 'global' | 'nodeType' | 'individual') };
