...
export const WeightControlSlider: React.FC<WeightControlSliderProps> = ({ )
  ...
  const handleReorder = (fromIndex: number, toIndex: number) => { const newOptions = [...localOptions];
    const [movedOption] = newOptions.splice(fromIndex, 1);
    newOptions.splice(toIndex, 0, movedOption);
    setLocalOptions(newOptions);
    onOptionsChange(newOptions) };
  const handleApplyPreset = (newWeights: number) => { const updatedOptions = localOptions.map((option, index) => ({)
  ...option
  weight: newWeights[index] || 0 }
    ...
    if (onPreviewRequest) { onPreviewRequest(updatedOptions) };
...
export const getOptionColor = (index: number): string => { const colors = [
...
export const useWeightControlIntegration = (onPreviewRequest?: (options: WeightControlOption) => void) => { const [lastUpdateTime, setLastUpdateTime] = useState(Date.now());
  ...
  onPreviewRequest(newOptions) }, 300); // 300ms debounce for optimal UX
  }, [onPreviewRequest]);
