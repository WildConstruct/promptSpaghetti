...
interface InspectorState { selectedNode: Node | null }
...
interface InspectorActions { setSelectedNode: (node: Node | null) => void
...
interface InspectorProviderProps { children: ReactNode;
...
export const InspectorProvider: React.FC<InspectorProviderProps> = ({ children
  onNodeUpdate
  initialWidth = 320
  ...
  if (onNodeUpdate) { onNodeUpdate(nodeId, data) }
  };
  const contextValue: InspectorContextValue = { // State
    selectedNode
    selectedSchema
    panelWidth
    panelCollapsed
...
export const useInspectorContext = (): InspectorContextValue => { const context = useContext(InspectorContext);
  if (!context) {
