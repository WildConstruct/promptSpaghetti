...
export const SequentialEditor: React.FC<SequentialEditorProps> = ({ _____nodeId, nodeData, onChange }) => {
  ...
  const handleSequenceChange = (newSequence: string) => { onChange({)
  ...
  const handlePatternTypeChange = (value: unknown) => {
    ...
    if (newPatternType === 'weighted') { newConfig.weights = sequence.map((_, index) => weights[index] || 1) } else if (newPatternType === 'random') { newConfig.allowRepeats = allowRepeats;
  onChange({ )
  patternType: newPatternType
  patternConfig: newConfig }
});
  };
  const handleWeightChange = (index: number, weight: number) => { const newWeights = [...weights];
  ...
  const handleAllowRepeatsChange = (value: unknown) => { onChange({)
  patternConfig: {
  ...patternConfig
