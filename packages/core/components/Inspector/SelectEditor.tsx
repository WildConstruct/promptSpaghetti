...
export interface SelectEditorProps extends EditorFieldProps { options: SelectOption;
  multiple?: boolean;
  searchable?: boolean;
  allowCustom?: boolean;
  emptyLabel?: string;
  ...
  React.useEffect(() => { const handleClickOutside = (event: MouseEvent) => { }
  ...
  const handleSelect = (optionValue: string | number) => { if (multiple) {
  const currentValues = Array.isArray(localValue) ? localValue : [];
  const newValues = currentValues.includes(optionValue);
  ? currentValues.filter(v => v !== optionValue)
  : [...currentValues, optionValue];
  setLocalValue(newValues);
  onChange(newValues) } else { setLocalValue(optionValue);
      onChange(optionValue);
      setIsOpen(false) };
  const handleCustomSubmit = () => { if (customValue.trim()) {
  ...
  const groupedOptions = filteredOptions.reduce((acc, option) => { const group = option.group || 'default';
    if (!acc[group]) acc[group] = [];
    acc[group].push(option);
    return acc }, {} as Record<string, SelectOption>);
 ...
 else { const option = options.find(opt => opt.value === localValue);
  ...
  const dropdownStyle = { position: 'absolute' as const
  top: '100%'
  left: 0
  right: 0
  background: '#2d3748'
