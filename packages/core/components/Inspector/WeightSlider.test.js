...
describe('WeightSlider', () => {
    test('renders with correct initial value', () => {
        ...
        const { getByRole } = render(_jsx(WeightSlider, { value: 50, onChange: mockOnChange, min: 0, max: 100 }));
        const slider = getByRole('slider');
        expect(slider).toBeInTheDocument();
    });
    test('updates value when slider is moved', () => {
        const mockOnChange = jest.fn();
        const { container } = render(_jsx(WeightSlider, { value: 25, onChange: mockOnChange, min: 0, max: 100, step: 1 }));
    ...
    test('respects min/max bounds', () => {
        const mockOnChange = jest.fn();
        const { container } = render(_jsx(WeightSlider, { value: 5, onChange: mockOnChange, min: 10, max: 90 }));
        const sliderTrack = container.querySelector('.slider-track');
        if (sliderTrack) {
            const rect = { left: 0, width: 100 };
            Object.defineProperty(sliderTrack, 'getBoundingClientRect', {
    ...
    test('shows numeric input when showNumeric is true', () => {
        const mockOnChange = jest.fn();
        const { container } = render(_jsx(WeightSlider, { value: 42, onChange: mockOnChange, showNumeric: true }));
    ...
    test('handles keyboard navigation', () => {
        const mockOnChange = jest.fn();
        const { container } = render(_jsx(WeightSlider, { value: 50, onChange: mockOnChange, min: 0, max: 100, step: 10 }));
    ...
    test('displays label when provided', () => {
        const mockOnChange = jest.fn();
        const { getByText } = render(_jsx(WeightSlider, { value: 30, onChange: mockOnChange, label: "Test Weight" }));
        expect(getByText('Test Weight')).toBeInTheDocument();
    });
    test('disables interaction when disabled prop is true', () => {
        const mockOnChange = jest.fn();
        const { container } = render(_jsx(WeightSlider, { value: 30, onChange: mockOnChange, disabled: true }));
        const sliderTrack = container.querySelector('.slider-track');
        if (sliderTrack) {
