...
interface InlineNodeEditorProps { node: Node<NodeData>;
  isActive: boolean;
...
export const InlineNodeEditor: React.FC<InlineNodeEditorProps> = ({ node
  isActive
  onUpdate
  onClose
  onSubmit
  ...
  useEffect(() => { if (isActive && firstInputRef.current) {
      firstInputRef.current.focus();
      firstInputRef.current.select() }
  }, [isActive]);
  // Handle clicks outside editor to close
  useEffect(() => { const handleClickOutside = (event: MouseEvent) => { }
  if (isActive && editorRef.current && !editorRef.current.contains(event.target as Node)) { handleSubmit() }
    };
  ...
  const validateData = useCallback((data: NodeData): Record<string, string> => {
    ...
    switch (node.type) { case 'weightedChoice': }
  if (!data.choices || data.choices.length === 0) { newErrors.choices = 'At least one choice is required' }
        break;
      case 'concat':
        if (!data.template?.trim()) { newErrors.template = 'Template is required' }
        break;
      case 'variable':
        if (!data.variableName?.trim()) { newErrors.variableName = 'Variable name is required' }
        break;
  ...
  const handleSubmit = useCallback(() => { const newErrors = validateData(localData);
    setErrors(newErrors);
    if (Object.keys(newErrors).length === 0) {
...
interface NodeSpecificEditorProps { nodeType: string;
  data: NodeData;
...
const NodeSpecificEditor: React.FC<NodeSpecificEditorProps> = ({ nodeType
  data
  onChange
  errors }
  theme = 'cinema'
