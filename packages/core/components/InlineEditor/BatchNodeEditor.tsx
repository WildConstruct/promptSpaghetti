...
interface BatchNodeEditorProps { selectedNodes: Node<NodeData>[] }
  isActive: boolean;
...
export const BatchNodeEditor: React.FC<BatchNodeEditorProps> = ({ )
  ...
  const handleFieldSelection = useCallback((field: string, selected: boolean) => { const newSelectedFields = new Set(selectedFields);
    if (selected) {
      newSelectedFields.add(field) } else {
  ...
  if (field in updates) {
      ...
      if (Object.keys(nodeUpdate).length > 0) { nodeUpdates[node.id] = nodeUpdate });
...
interface BatchEditFormProps { nodeAnalysis: any;
...
interface FieldEditorProps { field: string;
  value: any;
...
interface BatchPreviewProps { selectedNodes: Node<NodeData>[];
  updates: Record<string, any>;
  selectedFields: Set<string>;
  colors: any;
