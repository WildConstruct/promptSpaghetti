...
export interface InlineEditableNodeProps extends NodeProps<NodeData> { onDoubleClick?: (nodeId: string, event: React.MouseEvent) => void;
  ...
  const handleClick = useCallback((event: React.MouseEvent) => { const now = Date.now();
    ...
    if (timeSinceLastClick < 300) {
      ...
      if (onDoubleClick) {
        onDoubleClick(id, event) } else { activateEditor(id) }, [lastClickTime, id, onDoubleClick, activateEditor]);
  // Keyboard activation (Enter key when focused)
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => { if (event.key === 'Enter' && !isBeingEdited) {
      event.preventDefault();
      activateEditor(id) }, [id, activateEditor, isBeingEdited]);
  // Visual feedback states
  const getNodeStyles = () => { const baseStyles = {
  padding: 12,
  borderRadius: 8,
  border: '2px solid',
  minWidth: 160,
...
interface NodePreviewProps { type?: string;
  data: NodeData;
  theme: 'light' | 'dark' | 'cinema' }

const NodePreview: React.FC<NodePreviewProps> = ({ type, data, theme }) => { const previewStyle = {
  fontSize: 12
  opacity: 0.8
  marginTop: 4
