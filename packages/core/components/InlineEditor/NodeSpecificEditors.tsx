...
interface NodeEditorProps { data: NodeData;
  ...
  const removeChoice = useCallback((index: number) => { const newChoices = choices.filter((_, i) => i !== index);
    onChange('choices', newChoices) }, [choices, onChange]);
  const reorderChoices = useCallback((fromIndex: number, toIndex: number) => { const newChoices = [...choices];
    const [movedChoice] = newChoices.splice(fromIndex, 1);
    newChoices.splice(toIndex, 0, movedChoice);
    onChange('choices', newChoices) }, [choices, onChange]);
  // Normalize weights to 100%
  const normalizeWeights = useCallback(() => { const totalWeight = choices.reduce((sum, choice) => sum + (choice.weight || 0), 0);
