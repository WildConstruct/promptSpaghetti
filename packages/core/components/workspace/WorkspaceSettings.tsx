...
export const WorkspaceSettings: React.FC<WorkspaceSettingsProps> = ({ )
 ...
 catch (error) { console.error('Failed to update workspace:', error) } finally { setIsSubmitting(false) };
  ...
  const handleArchive = async () => { setIsSubmitting(true);
    try {
      await onArchive() } catch (error) { console.error('Failed to archive workspace:', error) } finally { setIsSubmitting(false);
