...
const LEGAL_TERMINOLOGY_DB: LegalTerminology = [
  ...
  const validateTerminology = useCallback(async () => { if (!text || text.trim().length === 0) {
 ...
 catch (error) { console.error('Terminology validation error:', error) } finally { setIsValidating(false);
      setTimeout(() => setValidationProgress(0), 1000) }, [text, settings, onValidationResults]);
  useEffect(() => { if (autoValidate && text) {
      const timeoutId = setTimeout(() => {
        validateTerminology() }, 500); // Debounce validation
  ...
  const tokenizeText = (text: string): { token: string; start: number; end: number }[] => {
    ...
    const regex = /\b[\w'-]+\b/g;
    let match;
    while ((match = regex.exec(text)) !== null) { tokens.push({)
  token: match[0],
  start: match.index,
  ...
  start: token.start,
    ...
    for (let i = 0; i < tokens.length - 1; i++) {
      ...
      if (isLegalTerm) { legalTerms.push({)
  term: twoWordTerm,
  start: tokens[i].start,
    ...
    for (let i = 0; i < tokens.length - 2; i++) {
      ...
      if (threeWordTerm) { legalTerms.push({)
  term: threeWordTerm,
  start: tokens[i].start,
  end: tokens[i + 2].end }
});
  ...
  const validateTerm = async (;);
    ...
    let isValid = true;
    let confidence = 1.0;
    if (!isJurisdictionMatch) { isValid = false;
      confidence -= 0.3;
  ...
  const findSimilarTerms = (term: string): LegalTerminology => { // Simple similarity matching - in real implementation would use more sophisticated algorithms
  ...
  const findAlternativeTerms = (term: string, settings: ValidationSettings): LegalTerminology => { return LEGAL_TERMINOLOGY_DB
      .filter(dbTerm => )
      ...
      .slice(0, 2) };
  const calculateSimilarity = (str1: string, str2: string): number => { // Simple Levenshtein-based similarity
  const longer = str1.length > str2.length ? str1 : str2;
  const shorter = str1.length > str2.length ? str2 : str1;
  if (longer.length === 0) return 1.0;
  const editDistance = levenshteinDistance(longer, shorter);
  return (longer.length - editDistance) / longer.length };
  const levenshteinDistance = (str1: string, str2: string): number => { const matrix = [];
  ...
  const extractContext = (text: string, start: number, end: number): string => { const contextRadius = 50;
  ...
  const normalizedTerm = result.term.toLowerCase();
      ...
      if (!usage.variations.includes(result.term)) { usage.variations.push(result.term) });
    // Check for inconsistent capitalization or variations
    termUsage.forEach((usage, normalizedTerm) => { if (usage.variations.length > 1) {
