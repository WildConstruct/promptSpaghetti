...
const createMockFile = (name, size, type, content) => {
    const file = new File([content || 'mock file content'], name, { type });
    Object.defineProperty(file, 'size', { value: size });
    return file;
};
describe('LegalDocumentParser Component', () => {
    const mockOnDocumentParsed = jest.fn();
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe('Initial Rendering', () => {
        it('renders document parser interface', () => {
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy', 'agreement'] }));
            expect(screen.getByText(/Legal Document Parser/i)).toBeInTheDocument();
            expect(screen.getByText(/Upload Document/i)).toBeInTheDocument();
            expect(screen.getByText(/Drag and drop/i)).toBeInTheDocument();
        });
        it('displays supported file types', () => {
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy', 'regulation'] }));
        ...
        it('shows file size limits', () => {
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract'], maxFileSize: 5 * 1024 * 1024 }));
            expect(screen.getByText(/Maximum file size: 5 MB/i)).toBeInTheDocument();
        });
        it('shows default file size limit when not specified', () => {
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract'] }));
    ...
    describe('File Upload Interface', () => {
        it('opens file dialog when upload area is clicked', async () => {
            ...
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy'] }));
    ...
    describe('File Upload and Validation', () => {
        ...
        it('validates file size before upload', async () => {
            const user = userEvent.setup();
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract'], maxFileSize: 1024 }));
    ...
    describe('Document Parsing Process', () => {
        ...
        it('calls onDocumentParsed when parsing completes', async () => {
            ...
            await user.upload(fileInput, validFile);
            await waitFor(() => {
                expect(mockOnDocumentParsed).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Document Type Detection', () => {
        it('detects contract documents', async () => {
            const user = userEvent.setup();
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy'] }));
            const contractFile = createMockFile('service-contract.pdf', 1000, 'application/pdf', 'This agreement establishes terms between parties for services');
            const fileInput = screen.getByLabelText(/choose file/i);
            await user.upload(fileInput, contractFile);
            await waitFor(() => {
                expect(mockOnDocumentParsed).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('detects policy documents', async () => {
            const user = userEvent.setup();
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy'] }));
            const policyFile = createMockFile('privacy-policy.pdf', 1000, 'application/pdf', 'This privacy policy describes data collection and usage');
            const fileInput = screen.getByLabelText(/choose file/i);
        ...
        it('falls back to generic type when detection fails', async () => {
            const user = userEvent.setup();
            render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract', 'policy'] }));
            const unknownFile = createMockFile('unknown.pdf', 1000, 'application/pdf', 'Some generic legal text without clear type indicators');
            const fileInput = screen.getByLabelText(/choose file/i);
    ...
    describe('Custom Class Names', () => {
        it('applies custom className correctly', () => {
            const { container } = render(_jsx(LegalDocumentParser, { onDocumentParsed: mockOnDocumentParsed, supportedTypes: ['contract'], className: "custom-document-parser" }));
