...
describe('LegalToolkit Component', () => { const mockOnWorkflowComplete = jest.fn<unknown, unknown>();
  beforeEach(() => {
    jest.clearAllMocks() });
  describe('Initial Rendering', () => {
    ...
    it('renders all navigation tabs', () => { render(<LegalToolkit />);
    ...
    it('starts with upload view active', () => { render(<LegalToolkit />);
  ...
  describe('Navigation and View Management', () => { it('enables/disables navigation tabs correctly', () => {
      render(<LegalToolkit />);
      const analyzeTab = screen.getByText('Analyze Contract').closest('button');
    ...
    it('navigates between views when tabs are clicked', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      ...
      await waitFor(() => { expect(screen.getByTestId('citation-manager')).toBeInTheDocument() });
    });
    it('prevents navigation to disabled views', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
  ...
  describe('Document Upload Workflow', () => { it('handles document parsing correctly', async () => {
    ...
    it('passes correct supported types to document parser', () => { render(<LegalToolkit />);
  ...
  describe('Contract Analysis Workflow', () => { it('performs contract analysis after document upload', async () => {
      const user = userEvent.setup();
      render(<LegalToolkit />);
    ...
    it('displays contract analyzer with correct document', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      const parseBtn = screen.getByTestId('parse-document-btn');
      await user.click(parseBtn);
      await waitFor(() => {
  ...
  describe('Compliance Checking Workflow', () => { it('performs compliance checking', async () => {
      const user = userEvent.setup();
      render(<LegalToolkit />);
      ...
      await waitFor(() => { expect(screen.getByTestId('compliance-checker')).toBeInTheDocument() });
    ...
    it('passes correct regulations to compliance checker', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      const parseBtn = screen.getByTestId('parse-document-btn');
      await user.click(parseBtn);
      await waitFor(async () => {
        const complianceTab = screen.getByText('Check Compliance');
        await user.click(complianceTab) });
      await waitFor(() => { const regulationsSpan = screen.getByTestId('regulations');
        expect(regulationsSpan).toHaveTextContent('gdpr,ccpa') });
    });
  });
  describe('Citation Management Workflow', () => { it('manages citations correctly', async () => {
      const user = userEvent.setup();
      render(<LegalToolkit />);
      const citationsTab = screen.getByText('Manage Citations');
      await user.click(citationsTab);
    ...
    it('handles citation editing', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      const citationsTab = screen.getByText('Manage Citations');
      await user.click(citationsTab);
    ...
    it('handles citation deletion', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      const citationsTab = screen.getByText('Manage Citations');
      await user.click(citationsTab);
      // Add a citation first
  ...
  describe('Terminology Validation Workflow', () => { it('validates terminology correctly', async () => {
      const user = userEvent.setup();
      render(<LegalToolkit />);
      // Upload document first
      const parseBtn = screen.getByTestId('parse-document-btn');
      ...
      await waitFor(() => { expect(screen.getByTestId('terminology-validator')).toBeInTheDocument() });
    ...
    it('passes document content to terminology validator', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
      const parseBtn = screen.getByTestId('parse-document-btn');
      await user.click(parseBtn);
      await waitFor(async () => {
        const terminologyTab = screen.getByText('Validate Terminology');
        await user.click(terminologyTab) });
      await waitFor(() => { const textLengthSpan = screen.getByTestId('text-length');
        expect(textLengthSpan).toHaveTextContent(mockDocument.content.length.toString()) });
    });
  });
  describe('Summary View', () => { it('displays complete summary after full workflow', async () => {
      const user = userEvent.setup();
      render(<LegalToolkit />);
      ...
      await waitFor(async () => { const complianceTab = screen.getByText('Check Compliance');
        await user.click(complianceTab) });
      await waitFor(async () => { const checkBtn = screen.getByTestId('check-compliance-btn');
        await user.click(checkBtn) });
      await waitFor(async () => { const citationsTab = screen.getByText('Manage Citations');
        await user.click(citationsTab) });
      await waitFor(async () => { const addBtn = screen.getByTestId('add-citation-btn');
        await user.click(addBtn) });
      await waitFor(async () => { const terminologyTab = screen.getByText('Validate Terminology');
        await user.click(terminologyTab) });
      await waitFor(async () => { const validateBtn = screen.getByTestId('validate-terms-btn');
        await user.click(validateBtn) });
      // Navigate to summary
      await waitFor(async () => { const summaryTab = screen.getByText('Review Summary');
        await user.click(summaryTab) });
      await waitFor(() => { expect(screen.getByText('📄 Document Summary')).toBeInTheDocument();
  ...
  describe('Workflow Completion', () => {
    it('calls onWorkflowComplete when Export Results is clicked', async () => {
      ...
      await waitFor(async () => { const analysisBtn = screen.getByTestId('complete-analysis-btn');
        await user.click(analysisBtn) });
      // Add citations
      await waitFor(async () => { const citationsTab = screen.getByText('Manage Citations');
        await user.click(citationsTab) });
      ...
      await waitFor(async () => { const validateBtn = screen.getByTestId('validate-terms-btn');
        await user.click(validateBtn) });
      await waitFor(() => { const exportBtn = screen.getByText('Export Results');
        return user.click(exportBtn) });
      await waitFor(() => { expect(mockOnWorkflowComplete).toHaveBeenCalledWith({)
  ...
  terminologyResults: mockTerminologyResults }
    ...
    it('enables export button only when workflow is complete', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
    ...
    it('allows starting new document workflow', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
  ...
  describe('Progress Calculation', () => { it('calculates progress correctly based on completed steps', async () => {
  const user = userEvent.setup();
  render(<LegalToolkit />);
  // Start with 0%
  ...
  describe('Error Handling and Edge Cases', () => { it('handles missing document gracefully', () => {
      render(<LegalToolkit />);
      const analyzeTab = screen.getByText('Analyze Contract').closest('button');
      expect(analyzeTab).toHaveClass('disabled');
      expect(analyzeTab).toHaveAttribute('disabled') });
    it('handles empty citations list', () => { render(<LegalToolkit />);
      const citationsTab = screen.getByText('Manage Citations');
  ...
  describe('Accessibility', () => { it('provides proper ARIA labels and roles', () => {
      render(<LegalToolkit />);
      const uploadTab = screen.getByText('Upload Document').closest('button');
      expect(uploadTab).toHaveAttribute('type', 'button');
      const progressBar = document.querySelector('.progress-bar');
      expect(progressBar).toBeInTheDocument() });
    it('supports keyboard navigation', async () => { const user = userEvent.setup();
      render(<LegalToolkit />);
  ...
  describe('Responsive Design', () => { it('renders correctly on mobile viewports', () => {
