...
describe('TerminologyValidator Component', () => {
    const mockOnValidationResults = jest.fn();
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe('Initial Rendering', () => {
        it('renders terminology validator interface', () => {
            render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults }));
        ...
        it('shows jurisdiction when provided', () => {
            render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults, jurisdiction: "US-CA" }));
            expect(screen.getByText(/Jurisdiction: US-CA/i)).toBeInTheDocument();
        });
        it('shows practice area when provided', () => {
            render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults, practiceArea: "contract law" }));
            expect(screen.getByText(/Practice Area: contract law/i)).toBeInTheDocument();
        });
        it('starts auto-validation when enabled', () => {
            render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults, autoValidate: true }));
    ...
    describe('Validation Process', () => {
        ...
        it('calls onValidationResults when validation completes', async () => {
            ...
            await user.click(validateButton);
            await waitFor(() => {
                expect(mockOnValidationResults).toHaveBeenCalledWith(expect.arrayContaining([
    ...
    describe('Text Editing and Re-validation', () => {
        ...
        it('updates validation when text changes', async () => {
            const user = userEvent.setup();
            const { rerender } = render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults, autoValidate: true }));
            await waitFor(() => {
                expect(mockOnValidationResults).toHaveBeenCalled();
            });
            mockOnValidationResults.mockClear();
            rerender(_jsx(TerminologyValidator, { text: "Different text with new legal terms like estoppel.", onValidationResults: mockOnValidationResults, autoValidate: true }));
            await waitFor(() => {
                expect(mockOnValidationResults).toHaveBeenCalled();
            });
        });
    ...
    describe('Error Handling', () => {
        it('handles validation errors gracefully', async () => {
            const user = userEvent.setup();
            render(_jsx(TerminologyValidator, { text: "", onValidationResults: mockOnValidationResults }));
            const validateButton = screen.getByText(/Validate Terms/i);
            await user.click(validateButton);
            await waitFor(() => {
        ...
        it('handles network errors during validation', async () => {
            ...
            render(_jsx(TerminologyValidator, { text: "[NETWORK_ERROR]", onValidationResults: mockOnValidationResults }));
            const validateButton = screen.getByText(/Validate Terms/i);
            await user.click(validateButton);
            await waitFor(() => {
    ...
    describe('Custom Class Names', () => {
        it('applies custom className correctly', () => {
            const { container } = render(_jsx(TerminologyValidator, { text: mockText, onValidationResults: mockOnValidationResults, className: "custom-terminology-validator" }));
