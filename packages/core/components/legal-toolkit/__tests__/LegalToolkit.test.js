...
jest.mock('../LegalDocumentParser', () => ({
    LegalDocumentParser: ({ onDocumentParsed, supportedTypes }) => (_jsxs("div", { "data-testid": "legal-document-parser", children: [_jsx("button", { onClick: () => onDocumentParsed(mockDocument), "data-testid": "parse-document-btn", children: "Parse Document" }), _jsx("span", { "data-testid": "supported-types", children: supportedTypes.join(',') })] }))
}));
jest.mock('../ContractAnalyzer', () => ({
    ContractAnalyzer: ({ document, onAnalysisComplete }) => (_jsxs("div", { "data-testid": "contract-analyzer", children: [_jsx("button", { onClick: () => onAnalysisComplete(mockAnalysis), "data-testid": "complete-analysis-btn", children: "Complete Analysis" }), _jsx("span", { "data-testid": "document-id", children: document.id })] }))
}));
jest.mock('../ComplianceChecker', () => ({
    ComplianceChecker: ({ document, regulations, onComplianceResults }) => (_jsxs("div", { "data-testid": "compliance-checker", children: [_jsx("button", { onClick: () => onComplianceResults(mockComplianceResults), "data-testid": "check-compliance-btn", children: "Check Compliance" }), _jsx("span", { "data-testid": "regulations", children: regulations.join(',') })] }))
}));
jest.mock('../CitationManager', () => ({
    CitationManager: ({ citations, onCitationAdd, onCitationEdit, onCitationDelete }) => (_jsxs("div", { "data-testid": "citation-manager", children: [_jsx("button", { onClick: () => onCitationAdd(mockCitation), "data-testid": "add-citation-btn", children: "Add Citation" }), _jsx("button", { onClick: () => onCitationEdit('1', { ...mockCitation, shortForm: 'Updated' }), "data-testid": "edit-citation-btn", children: "Edit Citation" }), _jsx("button", { onClick: () => onCitationDelete('1'), "data-testid": "delete-citation-btn", children: "Delete Citation" }), _jsx("span", { "data-testid": "citation-count", children: citations.length })] }))
}));
jest.mock('../TerminologyValidator', () => ({
    TerminologyValidator: ({ text, onValidationResults }) => (_jsxs("div", { "data-testid": "terminology-validator", children: [_jsx("button", { onClick: () => onValidationResults(mockTerminologyResults), "data-testid": "validate-terms-btn", children: "Validate Terms" }), _jsx("span", { "data-testid": "text-length", children: text.length })] }))
}));
// Mock data
const mockDocument = {
    id: 'doc-123',
...
describe('LegalToolkit Component', () => {
    ...
    describe('Initial Rendering', () => {
        ...
        it('initializes with correct initial document when provided', () => {
            render(_jsx(LegalToolkit, { initialDocument: mockDocument }));
    ...
    describe('Workflow Completion', () => {
        it('calls onWorkflowComplete when Export Results is clicked', async () => {
            const user = userEvent.setup();
            render(_jsx(LegalToolkit, { onWorkflowComplete: mockOnWorkflowComplete }));
    ...
    describe('Custom Class Names', () => {
        it('applies custom className correctly', () => {
            const { container } = render(_jsx(LegalToolkit, { className: "custom-toolkit" }));
