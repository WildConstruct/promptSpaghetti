...
export const ComplianceChecker = ({ document, regulations = ['gdpr', 'ccpa'], onComplianceResults, autoCheck = true, className = '' }) => {
        ...
        for (const framework of frameworks) {
            for (const category of framework.categories) {
                for (const requirement of category.requirements) {
                    const checkResult = await checkRequirement(document, framework, category, requirement);
                    if (checkResult) {
    ...
    const checkRequirement = async (doc, framework, category, requirement) => {
        const content = doc.content.toLowerCase();
        // Check for keyword matches
        const keywordMatches = requirement.keywords.filter(keyword => content.includes(keyword.toLowerCase()));
        // Check for pattern matches
        const patternMatches = requirement.patterns.some(pattern => pattern.test(doc.content));
    ...
    const toggleFramework = (frameworkId) => {
        setSelectedFrameworks(prev => prev.includes(frameworkId)
    ...
    return (_jsxs("div", { className: `compliance-checker ${className}`, children: [_jsx("style", { children: `
          .compliance-checker {
            background: white;
            border-radius: 8px;
        ...
        ` }), _jsxs("div", { className: "checker-header", children: [_jsx("h2", { className: "checker-title", children: "Regulatory Compliance Check" }), _jsx("div", { className: "framework-selector", children: COMPLIANCE_FRAMEWORKS.map(framework => (_jsxs("label", { className: `framework-toggle ${selectedFrameworks.includes(framework.id) ? 'selected' : ''}`, children: [_jsx("input", { type: "checkbox", className: "framework-checkbox", checked: selectedFrameworks.includes(framework.id), onChange: () => toggleFramework(framework.id) }), _jsx("span", { children: framework.name })] }, framework.id))) }), _jsxs("div", { className: "check-controls", children: [_jsx("button", { className: "check-button", onClick: runComplianceCheck, disabled: isChecking || selectedFrameworks.length === 0, children: isChecking ? 'Checking...' : 'Run Compliance Check' }), isChecking && (_jsx("div", { className: "progress-container", children: _jsx("div", { className: "progress-bar", children: _jsx("div", { className: "progress-fill", style: { width: `${checkProgress}%` } }) }) }))] })] }), _jsxs("div", { className: "results-section", children: [complianceResults.length > 0 && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "results-filters", children: [_jsxs("div", { className: "filter-group", children: [_jsx("label", { children: "Severity:" }), _jsxs("select", { className: "filter-select", value: filterSeverity, onChange: (e) => setFilterSeverity(e.target.value), children: [_jsx("option", { value: "all", children: "All" }), _jsx("option", { value: "critical", children: "Critical" }), _jsx("option", { value: "error", children: "High" }), _jsx("option", { value: "warning", children: "Medium" }), _jsx("option", { value: "info", children: "Low" })] })] }), _jsxs("div", { className: "filter-group", children: [_jsx("label", { children: "Status:" }), _jsxs("select", { className: "filter-select", value: filterStatus, onChange: (e) => setFilterStatus(e.target.value), children: [_jsx("option", { value: "all", children: "All" }), _jsx("option", { value: "compliant", children: "Compliant" }), _jsx("option", { value: "partial", children: "Partial" }), _jsx("option", { value: "non_compliant", children: "Non-Compliant" })] })] })] }), _jsxs("div", { className: "results-summary", children: [_jsxs("div", { className: "summary-card", children: [_jsx("div", { className: "summary-number", style: { color: getStatusColor('compliant') }, children: complianceResults.filter(r => r.status === 'compliant').length }), _jsx("div", { className: "summary-label", children: "Compliant" })] }), _jsxs("div", { className: "summary-card", children: [_jsx("div", { className: "summary-number", style: { color: getStatusColor('partial') }, children: complianceResults.filter(r => r.status === 'partial').length }), _jsx("div", { className: "summary-label", children: "Partial" })] }), _jsxs("div", { className: "summary-card", children: [_jsx("div", { className: "summary-number", style: { color: getStatusColor('non_compliant') }, children: complianceResults.filter(r => r.status === 'non_compliant').length }), _jsx("div", { className: "summary-label", children: "Non-Compliant" })] }), _jsxs("div", { className: "summary-card", children: [_jsx("div", { className: "summary-number", style: { color: '#4299e1' }, children: complianceResults.length }), _jsx("div", { className: "summary-label", children: "Total Checks" })] })] })] })), filteredResults.length > 0 ? (_jsx("div", { className: "results-list", children: filteredResults.map(result => (_jsxs("div", { className: "result-item", children: [_jsxs("div", { className: "result-header", onClick: () => toggleExpanded(result.id), children: [_jsxs("div", { className: "result-title-section", children: [_jsx("div", { className: "result-icon", children: getStatusIcon(result.status) }), _jsxs("div", { children: [_jsx("div", { className: "result-title", children: result.regulation }), _jsx("div", { className: "result-regulation", children: result.requirement })] })] }), _jsxs("div", { className: "result-badges", children: [_jsx("div", { className: "result-badge", style: { backgroundColor: getStatusColor(result.status) }, children: result.status.replace('_', ' ').toUpperCase() }), _jsx("div", { className: "result-badge", style: { backgroundColor: getSeverityColor(result.severity) }, children: result.severity.toUpperCase() }), _jsx("div", { className: `expand-icon ${expandedResults.has(result.id) ? 'expanded' : ''}`, children: "\u25BC" })] })] }), expandedResults.has(result.id) && (_jsxs("div", { className: "result-details", children: [_jsx("div", { className: "result-description", children: result.description }), result.remediation && result.remediation.length > 0 && (_jsxs("div", { className: "remediation-section", children: [_jsx("div", { className: "remediation-title", children: "Recommended Actions:" }), _jsx("ul", { className: "remediation-list", children: result.remediation.map((action, index) => (_jsx("li", { className: "remediation-item", children: action }, index))) })] }))] }))] }, result.id))) })) : complianceResults.length > 0 ? (_jsxs("div", { className: "no-results", children: [_jsx("div", { className: "no-results-icon", children: "\uD83D\uDD0D" }), _jsx("div", { children: "No results match your current filters." })] })) : !isChecking ? (_jsxs("div", { className: "no-results", children: [_jsx("div", { className: "no-results-icon", children: "\uD83D\uDCCB" }), _jsx("div", { children: "No compliance check results yet. Click \"Run Compliance Check\" to begin." })] })) : null] })] }));
};
