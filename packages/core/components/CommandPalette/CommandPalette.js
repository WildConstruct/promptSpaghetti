...
export const CommandPalette = ({ isOpen, onClose, nodes, edges, selectedNodes, onGenerationStart, onNodeCreate, onNodeDelete, onExport, onTemplateApply, theme = 'cinema', recentCommands = [], customActions = [] }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { style: {
                    ...
                    paddingTop: '100px'
                            ...
                            setShowGenerationWizard(false);
                            setActiveGenerationFlow(null);
                        }, theme: theme }))] }), "); }; // Generation Wizard Component interface GenerationWizardProps ", flow, ": GenerationFlow; currentStep: number; params: Record", _jsx("string", {}), ", any>; onParamsChange: (params: Record", _jsx("string", {}), ", any>) => void; onStepChange: (step: number) => void; onExecute: () => void; onCancel: () => void; theme: 'light' | 'dark' | 'cinema'; } const GenerationWizard: React.FC", _jsxs(GenerationWizardProps, { children: [" = (", (flow,
                        currentStep,
                        params,
                        onParamsChange,
                        onStepChange,
                        onExecute,
                        onCancel,
                        theme), ") => ", , "const step = flow.steps[currentStep]; const isLastStep = currentStep === flow.steps.length - 1; const getThemeStyles = () => ", , "const themes = ", light, ": ", background, ": '#ffffff', secondary: '#f8fafc', border: '#e5e7eb', text: '#374151', accent: '#3b82f6' }, dark: ", background, ": '#1f2937', secondary: '#111827', border: '#4b5563', text: '#f9fafb', accent: '#60a5fa' }, cinema: ", background, ": '#1a1a1a', secondary: '#0d1117', border: '#ff7c00', text: '#ffffff', accent: '#ff7c00' } }; return themes[theme]; }; const styles = getThemeStyles(); return (", _jsxs("div", { style: {
                            width: '700px',
                            maxHeight: '600px',
                            background: styles.background,
                            border: `1px solid ${styles.border}`,
                            ...
                            fontFamily: 'Inter, system-ui, sans-serif'
                                    ...
                                    alignItems: 'center'
                                            ...
                                            cursor: 'pointer'
                                                    ...
                                                    fontWeight: 600,
                                                    cursor: 'pointer'
                                                }, children: isLastStep ? '✨ Generate' : 'Next' })] })] })] }), "); }; export default CommandPalette;"] })] }));
};
