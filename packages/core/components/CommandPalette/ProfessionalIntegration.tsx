...
export const ProfessionalIntegration: React.FC<ProfessionalIntegrationProps> = ({ nodes
  edges
  selectedNodes
  selectedEdges
  onNodesChange
  ...
  const handleGenerationStart = useCallback(
    async (flow: { nodes: Node; edges: Edge }, params: Record<string, unknown>) => { console.log('Starting generation flow:', flow.name, params);
  ...
  // Undo/Redo Actions
  const handleUndo = useCallback(() => { if (undoRedoRef.current) {
  ...
  const handleRedo = useCallback(() => { if (undoRedoRef.current) {
  ...
  // Selection Actions
  const handleSelectAll = useCallback(() => { onNodesSelect(nodes);
    onEdgesSelect(edges) }, [nodes, edges, onNodesSelect, onEdgesSelect]);
  const handleDelete = useCallback(() => { if (selectedNodes.length > 0) {
  ...
  const handleDuplicate = useCallback(() => {
    selectedNodes.forEach(node => {
      ...
      onNodeCreate(node.type || 'text', position, { ...node.data }
  ...
  const handleToggleFullscreen = useCallback(() => { if (!document.fullscreenElement) {
  ...
  const handleAutosaveRestore = useCallback(
    (autosaveState: { nodes: Node; edges: Edge }) => { onNodesChange(autosaveState.nodes);
      onEdgesChange(autosaveState.edges) }
    [onNodesChange, onEdgesChange]
  );
  // Selection Change Handler
  const handleSelectionChange = useCallback(
    (selection: { nodes: Node; edges: Edge }) => { onNodesSelect(selection.nodes);
