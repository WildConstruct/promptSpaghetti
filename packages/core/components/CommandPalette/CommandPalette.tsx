...
export interface GenerationStep { id: string;
  title: string;
  description: string;
...
export interface GenerationField { id: string;
  label: string;
...
export interface CommandPaletteProps { isOpen: boolean;
  onClose: () => void;
  nodes: Node;
  edges: Edge;
  selectedNodes: Node;
...
interface GenerationWizardProps { flow: GenerationFlow;
  currentStep: number;
...
const GenerationWizard: React.FC<GenerationWizardProps> = ({ flow
  currentStep
  params
  onParamsChange
  onStepChange
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen
  onClose
  nodes
  edges
  selectedNodes
  ...
  const filteredActions = useMemo(() => { if (!searchQuery.trim()) {
      return commandActions }
    const query = searchQuery.toLowerCase();
    return commandActions.filter(action => { return (
  ...
  const groupedActions = useMemo(() => {
    ...
    filteredActions.forEach(action => { if (!groups[action.category]) {
  ...
  // Category icons
  const categoryIcons: Record<string, string> = { generation: '✨'
  editing: '✏️'
  navigation: '🧭'
  export: '📤'
  templates: '📋'
  ...
  useEffect(() => { if (isOpen && searchInputRef.current) {
      searchInputRef.current.focus() }
  }, [isOpen]);

  // Helper functions
  const createNodeAtCenter = useCallback(
    (nodeType: string) => { const viewport = reactFlowInstance?.getViewport();
  const center = {
  ...
  const executeAction = useCallback(async (action: CommandPaletteAction) => { try {
      await action.action() } catch (error) { console.error('Failed to execute command:', error) }
  }, []);

  const executeGenerationFlow = useCallback(async () => { if (!activeGenerationFlow) return;
    try {
      await onGenerationStart(activeGenerationFlow, generationParams);
      setShowGenerationWizard(false);
      setActiveGenerationFlow(null);
      onClose() } catch (error) { console.error('Generation failed:', error) }
