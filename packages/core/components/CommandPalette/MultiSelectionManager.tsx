...
export const MultiSelectionManager: React.FC<MultiSelectionManagerProps> = ({ nodes
  edges
  selectedNodes
  selectedEdges
  onNodesSelect
  ...
  if ((e.metaKey || e.ctrlKey) && e.key === 'a') { e.preventDefault();
  handleSelectAll() }
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => { if (disabled || e.button !== 0) return; // Only handle left click
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => { if (!isSelecting || disabled) return;
      const rect = e.currentTarget.getBoundingClientRect();
  ...
  const updateSelectionFromRect = useCallback(
    (startX: number, startY: number, endX: number, endY: number) => { if (!reactFlowInstance) return;
      ...
      const nodesInSelection = nodes.filter(node => {
        ...
        return (
          nodeX >= flowStartX && nodeY >= flowStartY && nodeX + nodeWidth <= flowEndX && nodeY + nodeHeight <= flowEndY
        ) });
      // Update selection
  ...
  const handleNodeSelection = useCallback(
    (node: Node, event: MouseEvent) => { if (event.metaKey || event.ctrlKey) {
  // Toggle selection
  const isSelected = selectedNodes.some(n => n.id === node.id);
  let newSelection: Node;
  if (isSelected) {
  newSelection = selectedNodes.filter(n => n.id !== node.id) } else { newSelection = [...selectedNodes, node] }
...
interface SelectionInfoPanelProps { selectedNodes: Node;
  selectedEdges: Edge;
...
const SelectionInfoPanel: React.FC<SelectionInfoPanelProps> = ({ selectedNodes
  selectedEdges
  onClearSelection
  onSelectAll
  onInvertSelection
  ...
  lineHeight: 1.4 }
            ...
            >
              ⌘A Select All • ⌘I Invert • Esc Clear
              <br />
              Click+Drag Rectangle Select • ⌘+Click Toggle
              <br />
              ⇧+Click Range Select
            </div>
          </div>
