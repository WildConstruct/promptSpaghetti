...
export class AutosaveSystem {
  ...
  // Save current state
  save(nodes: Node, edges: Edge): AutosaveState { const timestamp = Date.now();
  ...
  sessionId: this.sessionId }
    ...
    if (existingData.length > this.maxVersions) { existingData.splice(0, existingData.length - this.maxVersions) }
  ...
  getStoredData(): AutosaveState { try {
  const data = localStorage.getItem(this.storageKey);
  return data ? JSON.parse(data) : [] } catch (error) { console.error('Failed to load autosave data:', error);
  return [] }


  // Get the latest autosave
  getLatest(): AutosaveState | null { const data = this.getStoredData();
  return data.length > 0 ? data[data.length - 1] : null }

  // Check if there's a conflict (different session modified the data)
  hasConflict(): boolean { const latest = this.getLatest();
  ...
  private getNextVersion(): number { const data = this.getStoredData();
  return data.length > 0 ? Math.max(...data.map(s => s.version)) + 1 : 1 }

  // Subscribe to autosave events
  subscribe(callback: (status: AutosaveStatus) => void): () => void { this.listeners.add(callback);
    return () => this.listeners.delete(callback) }

  private notifyListeners(status: AutosaveStatus): void { this.listeners.forEach(listener => listener(status)) }

  // Validate data integrity
  validateChecksum(state: AutosaveState): boolean { const currentChecksum = this.generateChecksum(state.nodes, state.edges);
    return currentChecksum === state.checksum }


export interface AutosaveStatus { type: 'saved' | 'restored' | 'cleared' | 'error';
  timestamp?: number;
  version?: number;
  error?: string }


export const AutosaveManager: React.FC<AutosaveManagerProps> = ({ nodes
  edges
  ...
  useEffect(() => { const versions = autosaveSystem.getStoredData();
  ...
  useEffect(() => {
    ...
    const saveIfChanged = () => {
      ...
      if (currentData !== lastDataRef.current && nodes.length > 0) { autosaveSystem.save(nodes, edges);
  ...
  // Manual save
  const handleManualSave = useCallback(() => { autosaveSystem.save(nodes, edges);
    setAvailableVersions(autosaveSystem.getStoredData()) }, [nodes, edges, autosaveSystem]);
  // Restore from version
  const handleRestore = useCallback(
    (version?: number) => { const state = autosaveSystem.restore(version);
      if (state && onRestore) {
        onRestore(state);
  ...
  // Clear autosaves
  const handleClear = useCallback(() => { if (confirm('Clear all autosave data? This cannot be undone.')) {
  ...
  const getStatusColor = () => { if (!status) return styles.textSecondary;
  switch (status.type) {
  case 'saved':
  ...
  const getStatusText = () => { if (!status) return 'Ready';
  switch (status.type) {
  case 'saved':
  return 'Saved';
  case 'error':
