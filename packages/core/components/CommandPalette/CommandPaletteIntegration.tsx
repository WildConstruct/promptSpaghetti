...
export const CommandPaletteIntegration: React.FC<CommandPaletteIntegrationProps> = ({ )
  ...
  const handleOpen = useCallback(() => { if (disabled) return;
    setIsOpen(true) }, [disabled]);
  const handleClose = useCallback(() => { setIsOpen(false) }, []);
  // Set up keyboard shortcuts
  useCommandPaletteShortcuts(handleOpen, handleClose, isOpen);
  // Generation flow handler
  const handleGenerationStart = useCallback(async (flow: GenerationFlow, params: Record<string, any>) => { setIsGenerating(true);
    try {
  ...
  weight: 80 - (index * 10);
    ...
    if (complexity !== 'simple') { const backgroundChoices = generateBackgroundChoices(role, genre);
  ...
  choices: backgroundChoices }
    ...
    if (includeDialogue && complexity === 'advanced') { const dialogueChoices = generateDialogueChoices(genre, traits);
  ...
  const generateStoryStructure = async (;);
    ...
    const currentPosition = { ...startPosition };
    const spacing = 250;
    for (const act of acts) { const actNode = createCharacterNode(act.title, {)
  ...
  const generateBackgroundChoices = (role: string, genre: string) => { const backgrounds = {
  protagonist: {
  ...
  const generateDialogueChoices = (genre: string, _____traits: string) => { const baseStyles = {
  ...
  const generateDialogueStylesForTone = (tone: string) => { const toneStyles = {
  dramatic: [
