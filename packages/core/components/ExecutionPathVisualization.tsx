...
interface ExecutionPathVisualizationProps { results: PreviewResultWithPath;
...
export const ExecutionPathVisualization: React.FC<ExecutionPathVisualizationProps> = ({ results
  onNodeHighlight }
  config = {}
  className = ''
}) => {
  ...
  const toggleResultExpanded = (index: number) => { const newExpanded = new Set(expandedResults);
    if (newExpanded.has(index)) {
      newExpanded.delete(index) } else { newExpanded.add(index) }
    setExpandedResults(newExpanded);
  };
  const handlePathSelect = (pathId: string, nodeIds: string) => { setSelectedPath(selectedPath === pathId ? null : pathId);
  if (onNodeHighlight) {
