...
export const LoadProjectDialog: React.FC<LoadProjectDialogProps> = ({ isOpen
  onClose }
  onLoad
}) => {
  ...
  const handleLoadClick = () => { if (hasUnsavedChanges) {
      setShowUnsavedWarning(true) } else { performLoad() }
  };
  const performLoad = async () => { setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      onLoad?.(result);
      if (result.success) {
        onClose() } else { setError(result.error || 'Failed to load project') }
 catch (err) { setError('An unexpected error occurred') } finally { setIsLoading(false) }
  };
  const handleConfirmLoad = () => { performLoad() };
  const handleCancelWarning = () => { setShowUnsavedWarning(false) };
