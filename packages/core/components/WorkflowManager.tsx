...
interface WorkflowManagerProps { isOpen: boolean;
  onClose: () => void }
...
export const WorkflowManager: React.FC<WorkflowManagerProps> = ({ isOpen }
  onClose
}) => { const {
    rules
    getDraftRules
 ...
 = useCorrectionsStore();
  ...
  const handleApprove = (ruleId: string) => { approveRule(ruleId, 'system'); // In real app, would use actual user ID
    setShowApprovalDialog(false);
    setCurrentRule(null);
    setApprovalComment('') };
  const handleBulkApprove = () => { selectedRules.forEach(ruleId => {
  approveRule(ruleId, 'system') });
    setSelectedRules(new Set());
  };
  const handleDeprecate = (ruleId: string, reason: string) => { deprecateRule(ruleId, reason);
    setShowDeprecationDialog(false);
    setCurrentRule(null);
    setDeprecationReason('') };
  const handleBulkDeprecate = () => { const reason = prompt('Enter deprecation reason:');
  if (reason) {
  selectedRules.forEach(ruleId => {
  deprecateRule(ruleId, reason) });
      setSelectedRules(new Set());
  };
  const toggleRuleSelection = (ruleId: string) => { const newSelected = new Set(selectedRules);
    if (newSelected.has(ruleId)) {
      newSelected.delete(ruleId) } else { newSelected.add(ruleId);
    setSelectedRules(newSelected) };
  const selectAllRules = (ruleList: CorrectionRule) => { const allIds = new Set(ruleList.map(rule => rule.id));
    setSelectedRules(allIds) };
  const clearSelection = () => { setSelectedRules(new Set()) };
  const getStatusColor = (status: CorrectionRule['status']) => { switch (status) {
  case 'draft':
  return '#fbb040';
  case 'published':
  return '#68d391';
  ...
  const renderTabContent = () => { const currentRules = rulesByStatus[activeTab];
