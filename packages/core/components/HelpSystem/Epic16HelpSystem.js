...
export const Epic16HelpSystem = ({ nodes = [], edges = [], selectedNodeId, currentPage = '', userRole = 'new-user', userLevel = 'beginner', pageContext = {}, helpEnabled = true, autoContextualHelp = true, showGuidedTours = true, helpComplexity = 'beginner', onHelpAnalytics }) => {
    ...
    if (!helpEnabled)
        return null;
    return (_jsxs("div", { className: "epic16-help-system", children: [(helpMode === 'core' || helpMode === 'hybrid') && nodes.length > 0 && (_jsx(ContextualHelpSystem, { nodes: nodes, edges: edges, selectedNodeId: selectedNodeId, userLevel: userLevel, enabled: helpEnabled, autoTrigger: autoContextualHelp, showProgressiveHints: showGuidedTours, onHelpContentViewed: (contentId) => trackHelpEffectiveness(contentId, true), onUserLevelChange: (level) => {
                    onHelpAnalytics?.('user_level_changed', {
                        from: userLevel,
                        to: level,
                        context: helpContext
                    });
                } })), (helpMode === 'marketplace' || helpMode === 'hybrid') && (_jsx(MarketplaceHelpOverlay, { currentPage: currentPage, userRole: userRole, selectedTemplate: pageContext.template, cartItems: pageContext.cartItems, purchaseStage: pageContext.purchaseStage, forumContext: pageContext.forumContext, showMarketplaceHelp: helpEnabled, enableGuidedTours: showGuidedTours, helpComplexity: helpComplexity, coreHelpManager: helpContentManager, onHelpInteraction: handleHelpInteraction, onTourCompleted: handleTourCompleted, onFeedbackSubmitted: (feedback) => {
                    onHelpAnalytics?.('feedback_submitted', {
                        ...feedback,
                        context: helpContext,
                        userRole
                    });
                } })), autoContextualHelp && (_jsx(MarketplaceContextualHelp, { userRole: userRole, userLevel: userLevel, currentPage: currentPage, pageContext: pageContext, enabled: helpEnabled, intelligenceLevel: "smart", triggerSensitivity: "medium", helpContentManager: helpContentManager, onContextualHelpTriggered: handleContextualHelpTriggered, onUserStruggleDetected: handleUserStruggleDetected, onHelpEffectiveness: trackHelpEffectiveness })), _jsxs("div", { className: "help-system-status", children: [_jsx("div", { className: "help-mode-indicator", children: _jsxs("span", { className: `mode-badge ${helpMode}`, children: [helpMode === 'core' && '🎬 Editor', helpMode === 'marketplace' && '🏪 Marketplace', helpMode === 'hybrid' && '🔄 Integrated'] }) }), process.env.NODE_ENV === 'development' && (_jsxs("div", { className: "help-debug-info", children: [_jsxs("div", { children: ["Mode: ", helpMode] }), _jsxs("div", { children: ["Journey: ", userJourney.slice(-3).join(' → ')] }), _jsxs("div", { children: ["Effectiveness: ", Object.keys(helpEffectiveness).length, " tracked"] })] }))] }), _jsx("style", { jsx: true, children: `
        .epic16-help-system {
          position: relative;
        }
