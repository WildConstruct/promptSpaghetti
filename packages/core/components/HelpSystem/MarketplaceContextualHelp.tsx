...
export interface MarketplaceContextualHelpProps { // User context
  userId?: string;
...
export const MarketplaceContextualHelp: React.FC<MarketplaceContextualHelpProps> = ({ )
  ...
  priority: 10
      cooldownMinutes: 5], [currentPage]);
  // Behavior tracking and context analysis
  const updateBehaviorContext = useCallback((updates: Partial<UserBehaviorContext>) => { setUserBehavior(prev => ({)
  ...prev
  ...updates
  ...
  const detectUserStruggle = useCallback((behavior: UserBehaviorContext) => { const struggles: string = [];
  ...
  const evaluateRules = useCallback((behavior: UserBehaviorContext): ContextualHelpRule | null => { const now = Date.now();
      ...
      if (rule.conditions.userBehavior) {
        for (const [key, value] of Object.entries(rule.conditions.userBehavior)) {
          ...
          if (typeof value === 'number' && typeof behaviorValue === 'number') {
            if (behaviorValue < value) return false } else if (behaviorValue !== value) { return false;
  ...
  timeOnPage: Date.now() - pageStartTime.current }
      ...
      if (intelligenceLevel !== 'basic') { const applicableRule = evaluateRules(currentBehavior);
        if (applicableRule && !activeHelp) {
          triggerContextualHelp(applicableRule) }, 2000); // Check every 2 seconds
    return () => { if (behaviorTrackingInterval.current) {
