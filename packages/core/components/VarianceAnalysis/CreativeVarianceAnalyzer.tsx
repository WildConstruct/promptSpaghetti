    ...
    return ();
      <div className={`creative-variance-analyzer ${className}`} style={{}
        ...
        <div style={{ fontSize: 14, color: '#64748b' }}>
          Generate at least 2 results to enable creative variance analysis
        </div>
      </div>
    );
  }
          ...
          <div>
            ...
            <div style={{ 
              ...
              <MetricCard
                ...
                value={varianceAnalysis.uniquenessScore}
                suffix="%"
                description="Content variation across results"
                color="#10b981"
              />
              <MetricCard
                title="Creativity"
                value={varianceAnalysis.creativityScore}
                suffix="%"
                description="Creative potential and originality"
                color="#8b5cf6"
              />
              <MetricCard
        ...
        {activeTab === 'content' && ()
          <div style={{ 
            ...
            <MetricCard
              ...
              description="Variation in content length"
              color="#6366f1"
            />
            <MetricCard
              title="Length Distribution"
              value={Math.round(varianceAnalysis.lengthDistribution.avg)}
              suffix=" chars"
              description={`Range: ${varianceAnalysis.lengthDistribution.min}-${varianceAnalysis.lengthDistribution.max}`}
              color="#10b981"
            />
            <MetricCard
              title="Reading Complexity"
              value={advancedMetrics.readingComplexity}
              suffix="%"
              description="Text complexity and readability"
              color="#f59e0b"
            />
            <MetricCard
              title="Vocabulary Richness"
              value={advancedMetrics.vocabularyRichness}
              suffix="%"
            ...
            <MetricCard
              ...
              value={advancedMetrics.repetitionIndex}
              suffix="%"
              description="Content repetition across results"
              color="#ef4444"
            />
            <MetricCard
              title="Diversity Index"
              value={Math.round(varianceAnalysis.diversityIndex * 100)}
              suffix="%"
              description="Content type diversity"
              color="#06b6d4"
            />
          </div>
        )}
        {activeTab === 'creative' && ()
          <div style={{ 
            ...
            <MetricCard
              ...
              suffix="%"
              description="Concept richness per 100 words"
              color="#8b5cf6"
            />
            <MetricCard
              title="Conceptual Leaps"
              value={advancedMetrics.conceptualLeaps}
              suffix="%"
              description="Thematic variety and innovation"
              color="#06b6d4"
            />
            <MetricCard
              title="Narrative Consistency"
              value={advancedMetrics.narrativeConsistency}
              suffix="%"
              description="Story coherence across results"
              color="#10b981"
            />
            <MetricCard
              title="Tone Variation"
              value={varianceAnalysis.toneVariation}
              suffix="%"
              description="Emotional range across content"
              color="#f59e0b"
            />
            <MetricCard
              title="Semantic Coherence"
              value={advancedMetrics.semanticCoherence}
              suffix="%"
              description="Meaning consistency"
              color="#3b82f6"
            />
            <MetricCard
              title="Revision Potential"
              value={advancedMetrics.revisionPotential}
              suffix="%"
              description="Foundation for further development"
              color="#ef4444"
            />
          </div>
        )}
        {activeTab === 'professional' && ()
          <div>
            <div style={{ 
              ...
              />
              <MetricCard
                title="Client Presentability"
                value={advancedMetrics.clientPresentability}
                suffix="%"
                description="Ready for client presentation"
                color="#10b981"
              />
              <MetricCard
                title="Professional Suitability"
                value={varianceAnalysis.professionalSuitability}
                suffix="%"
              ...
              <MetricCard
                ...
                value={varianceAnalysis.genreConsistency}
                suffix="%"
                description="Thematic and stylistic coherence"
                color="#f59e0b"
              />
            </div>
