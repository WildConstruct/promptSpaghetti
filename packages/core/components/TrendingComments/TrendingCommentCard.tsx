...
interface TrendingCommentCardProps { comment: TrendingComment;
  rank: number;
  ...
  const getVelocityIcon = (trend: string) => { switch (trend) {
  case 'accelerating': return '🚀';
  case 'declining': return '📉';
  case 'steady': return '📋';
  default: return '🔆' };
  const shouldTruncateContent = (content: string) => { return content.length > 300 };
  const getTruncatedContent = (content: string) => { return content.substring(0, 300) + '...' };
  ...
  const contentToShow = (!isExpanded && shouldTruncateContent(comment.content)) ;
    ...
    : comment.content;
  const handleEngagementClick = (engagementType: string) => { onEngagement(comment.commentId, engagementType) };
  return;
    <div style={ {
