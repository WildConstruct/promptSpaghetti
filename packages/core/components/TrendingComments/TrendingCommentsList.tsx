...
interface TrendingCommentsListProps { resourceId: string;
  ...
  useEffect(() => { loadTrendingComments() }, [resourceId, resourceType, period, sortOrder]);
  const loadTrendingComments = async (loadMore = false) => { setIsLoading(true);
  setError(null);
  try {
  ...
  includeReplies: true }
      ...
      if (loadMore) { setComments(prev => [...prev, ...trendingResponse.results.trendingComments]);
        setOffset(prev => prev + limit) } else { setComments(trendingResponse.results.trendingComments);
        setOffset(limit);
      setResponse(trendingResponse);
      setHasMore(trendingResponse.pagination.hasMore) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load trending comments') } finally { setIsLoading(false) };
  const handleLoadMore = () => { if (!isLoading && hasMore) {
      loadTrendingComments(true) };
  const handleEngagement = async (commentId: string, engagementType: string) => { try {
      // Track engagement
  ...
  if (comment.commentId === commentId) {
          ...
          switch (engagementType) { case 'like':
  ...
  const formatNumber = (num: number) => { if (num >= 1000) {
      return (num / 1000).toFixed(1) + 'K';
    return num.toString() };
  const renderFilters = () => { if (!showFilters) return null;
  return;
  <div style={{
  padding: '16px'
  ...
  const renderAnalytics = () => { if (!showAnalytics || !response) return null;
