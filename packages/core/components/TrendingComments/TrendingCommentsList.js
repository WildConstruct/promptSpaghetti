...
export const TrendingCommentsList = ({ resourceId, resourceType, initialPeriod = '24h', initialSortOrder = 'trending', limit = 20, showFilters = true, showAnalytics = true, onCommentEngagement }) => {
    ...
    const handleEngagement = async (commentId, engagementType) => {
        try {
            // Track engagement
            await trendingService.trackEngagement(commentId, 'current-user-id', // TODO: Get from auth context
            engagementType);
        ...
        return (_jsxs("div", { style: {
                ...
                border: '1px solid #bae6fd'
                        ...
                        gap: '12px'
                                        ...
                                        fontSize: '12px',
                                        color: '#6b7280'
                                    }, children: "Health Score" })] })] }), summary.topHashtags.length > 0 && (_jsxs("div", { style: { marginTop: '12px' }, children: [_jsx("span", { style: {
                                fontSize: '12px',
                                color: '#6b7280',
                                marginRight: '8px'
                            }, children: "Trending topics:" }), summary.topHashtags.map(hashtag => (_jsxs("span", { style: {
    ...
    return (_jsxs("div", { style: {
            ...
            border: '1px solid #e5e7eb'
                    ...
                    borderBottom: '1px solid #e5e7eb'
                        ...
                        alignItems: 'center',
                        gap: '8px'
                    }, children: ["\uD83D\uDD25 Trending Comments", response && (_jsxs("span", { style: {
                                fontSize: '14px',
                                fontWeight: '400',
                                color: '#6b7280'
                            }, children: ["(", response.results.qualifiedComments, " of ", response.results.totalComments, ")"] }))] }) }), _jsxs("div", { style: { padding: '20px' }, children: [renderFilters(), renderAnalytics(), isLoading && comments.length === 0 ? (_jsxs("div", { style: {
                            padding: '40px',
                            textAlign: 'center',
                            color: '#6b7280'
                        }, children: [_jsx("div", { style: {
                                    ...
                                    margin: '0 auto 16px'
                                } }), "Loading trending comments..."] })) : comments.length === 0 ? (_jsxs("div", { style: {
                            padding: '40px',
                            textAlign: 'center',
                            color: '#6b7280'
                        }, children: [_jsx("div", { style: { fontSize: '48px', marginBottom: '16px' }, children: "\uD83D\uDCAC" }), _jsx("div", { children: "No trending comments found for this time period." }), _jsx("div", { style: { fontSize: '14px', marginTop: '8px' }, children: "Try selecting a different time period or sort order." })] })) : (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '16px' }, children: [comments.map((comment, index) => (_jsx(TrendingCommentCard, { comment: comment, rank: index + 1, onEngagement: handleEngagement }, comment.commentId))), hasMore && (_jsx("div", { style: { textAlign: 'center', marginTop: '20px' }, children: _jsx("button", { onClick: handleLoadMore, disabled: isLoading, style: {
                                        padding: '12px 24px',
