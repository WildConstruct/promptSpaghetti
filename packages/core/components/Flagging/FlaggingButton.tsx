...
export interface FlaggingReason { id: string;
  label: string;
  description: string;
...
export interface FlagSubmission { contentId: string;
...
export interface FlaggingButtonProps { contentId: string;
  contentType: 'template' | 'comment' | 'review' | 'user' | 'project';
  userId: string;
...
export interface FlaggingStatus { contentId: string;
  canFlag: boolean;
...
export const FlaggingButton: React.FC<FlaggingButtonProps> = ({ )
  ...
  React.useEffect(() => { loadFlaggingStatus() }, [contentId, userId]);
  const loadFlaggingStatus = async () => { try {
  ...
  status: 'none' }
      ...
      if (onStatusChange) { onStatusChange(mockStatus) } catch (err) { console.error('Failed to load flagging status:', err) };
  const handleFlagClick = useCallback(() => { if (disabled || status.userHasFlagged) {
      return;
    setShowModal(true);
    setError(null);
    setSuccess(false) }, [disabled, status.userHasFlagged]);
  const handleSubmitFlag = async () => { if (!selectedReason) {
  ...
  timestamp: new Date().toISOString() }
      ...
      if (onFlag) { await onFlag(submission);
  // Update status
  ...
  const getButtonColor = () => { if (disabled || status.userHasFlagged) return '#9ca3af';
    if (status.alreadyFlagged) return '#d97706';
    return '#6b7280' };
  const getButtonText = () => { if (status.userHasFlagged) return '✓ Flagged';
  if (variant === 'icon') return '🚩';
  return showLabel ? '🚩 Flag' : '🚩' };
  const getTooltipText = () => { if (status.userHasFlagged) return 'You have already flagged this content';
    if (disabled) return 'Flagging is not available';
    return 'Report this content for review' };
  // Render button
  const renderButton = () => { const baseStyle = {
  ...buttonStyles[size]
