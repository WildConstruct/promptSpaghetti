...
export interface CommentSubmissionFormProps { resourceId: string;
  resourceType: 'template' | 'project' | 'user' | 'marketplace_item';
  parentCommentId?: string;
  authorId: string;
...
export const CommentSubmissionForm: React.FC<CommentSubmissionFormProps> = ({ )
  ...
  // Auto-save drafts
  useEffect(() => { if (config.autoSaveDrafts && content.length > 10) {
      const timer = setTimeout(() => {
        saveDraft() }, 2000); // Save after 2 seconds of inactivity
      return () => clearTimeout(timer);
  }, [content, attachments, mentions, hashtags, config.autoSaveDrafts]);
  // Character count tracking
  useEffect(() => { setCharacterCount(content.length) }, [content]);
  ...
  const handleContentChange = useCallback((newContent: string) => { setContent(newContent);
    ...
    setValidationErrors([]) }, [config.enableMentions, config.enableHashtags]);
  // Validation
  const validateSubmission = useCallback((): ValidationError => {
    ...
    if (content.length > config.maxContentLength) { errors.push({ )
        field: 'content' }
        message: `Comment exceeds maximum length of ${config.maxContentLength} characters` }
      });
    // Attachment validation
    if (attachments.length > config.maxAttachments) { errors.push({ )
        field: 'attachments' }
        message: `Maximum ${config.maxAttachments} attachments allowed` }
      });
    // File size validation
    for (const attachment of attachments) { if (attachment.size && attachment.size > config.maxFileSize) {
        errors.push({ )
          field: 'attachments' }
          message: `File "${attachment.name}" exceeds maximum size of ${formatFileSize(config.maxFileSize)}` }
        });
    // Content moderation checks
    if (config.moderationSettings.flagSuspiciousContent) { const suspiciousPatterns = detectSuspiciousContent(content);
  ...
  // Submit handler
  const handleSubmit = async () => { if (disabled || isSubmitting) return;
  ...
  hasHashtags: hashtags.length > 0 }
      ...
      resetForm();
      clearDraft();
      console.log('✅ Comment submitted successfully') } catch (error) { console.error('❌ Comment submission failed:', error);
      setValidationErrors([{ )
        field: 'submission' }
  ...
  // Cancel handler
  const handleCancel = () => { if (content.trim() && config.enableDrafts) {
      saveDraft();
    resetForm();
    if (onCancel) {
      onCancel() };
  // File upload handler
  const handleFileUpload = async (files: FileList) => { if (!config.enableAttachments) return;
  ...
  field: 'attachments' }
        ...
        continue;
      // Validate file size
      if (file.size > config.maxFileSize) { setValidationErrors(prev => [...prev, {)
  field: 'attachments' }
  ...
  uploadedAt: new Date().toISOString() }
    ...
    if (attachments.length + newAttachments.length > config.maxAttachments) { setValidationErrors(prev => [...prev, {)
  field: 'attachments' }
  ...
  // Draft management
  const saveDraft = () => { if (!config.enableDrafts) return;
  ...
  setHashtags(draft.hashtags) } catch (error) { console.error('Failed to load draft:', error) };
  const clearDraft = () => {
    if (!config.enableDrafts) return;
  ...
  const resetForm = () => { setContent('');
    ...
    setCharacterCount(0) };
  // Load draft on mount
  useEffect(() => { loadDraft() }, [resourceId, authorId]);
  return;
...
interface AttachmentPreviewProps { attachment: CommentAttachment;
  onRemove: () => void }

const AttachmentPreview: React.FC<AttachmentPreviewProps> = ({ attachment, onRemove }) => { return;
  <div style={{
  display: 'flex'
  alignItems: 'center'
  gap: '8px'
...
// Helper functions
function extractMentions(content: string): string { const mentionRegex = /@(\w+)/g;
