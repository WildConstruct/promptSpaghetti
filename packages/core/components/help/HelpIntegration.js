...
export function withHelp(WrappedComponent: any, helpContent: any) {
    const WithHelpComponent = (props: any) => {
...
export const HelpfulInput = ({ helpId, helpTitle, helpDescription, helpCategory = 'basic', helpExamples, helpShortcut, label, error, className = '', style, ...inputProps }: { helpId: string, helpTitle?: string, helpDescription?: string, helpCategory?: string, helpExamples?: any[], helpShortcut?: string, label?: string, error?: string, className?: string, style?: any, [key: string]: any }) => {
...
export const HelpfulButton = ({ helpId, helpTitle, helpDescription, helpCategory = 'basic', helpExamples, helpShortcut, variant = 'secondary', size = 'medium', children, className = '', style, ...buttonProps }: { helpId: string, helpTitle?: string, helpDescription?: string, helpCategory?: string, helpExamples?: any[], helpShortcut?: string, variant?: string, size?: string, children: any, className?: string, style?: any, [key: string]: any }) => {
    const { complexityLevel } = useUISettingsStore();
    const { showHelpHints } = useHelpSystem();
    const helpContent = useFieldHelp(helpId, {
        title: helpTitle,
...
export const HelpfulSection = ({ helpId, helpTitle, helpDescription, helpCategory = 'basic', helpExamples, title, children, collapsible = false, defaultExpanded = true, className = '', style }: { helpId: string, helpTitle?: string, helpDescription?: string, helpCategory?: string, helpExamples?: any[], title: string, children: any, collapsible?: boolean, defaultExpanded?: boolean, className?: string, style?: any }) => {
...
export const useContextualHelp = (helpContent: any) => {
    const { showHelpHints, addHelpContent } = useHelpSystem();
    React.useEffect(() => {
        addHelpContent(helpContent);
    }, [helpContent.id]);
    const wrapWithHelp = (element: any) => {
        if (!showHelpHints)
            return element;
...
export const OnboardingOverlay = ({ isActive, children }: { isActive: boolean, children: any }) => {
    if (!isActive)
