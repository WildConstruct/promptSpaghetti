...
export const CommentModerationPanel: React.FC<CommentModerationPanelProps> = ({ )
  ...
  const loadComments = useCallback(async () => { setLoading(true);
    setError(null);
    try {
      ...
      setStats(mockStats);
      if (onStatsUpdate) {
        onStatsUpdate(mockStats) } catch (err) {
      setError(`Failed to load comments: ${err.message}`);}
      console.error('Comment loading failed:', err);
 finally { setLoading(false) }, [filters, onStatsUpdate]);
  ...
  const handleFiltersChange = useCallback((newFilters: Partial<CommentModerationFilters>) => {
    ...
    if (onFiltersChange) { onFiltersChange(updatedFilters) }, [filters, onFiltersChange]);
  // Handle comment selection
  const handleCommentSelection = useCallback((commentId: string, selected: boolean) => { setSelectedComments(prev => {)
  if (selected) {
        return [...prev, commentId] } else { return prev.filter(id => id !== commentId) });
  }, []);
  // Handle bulk selection
  const handleSelectAll = useCallback((selectAll: boolean) => { if (selectAll) {
      const visibleCommentIds = comments.map(comment => comment.commentId);
      setSelectedComments(visibleCommentIds) } else { setSelectedComments([]) }, [comments]);
  ...
  const handleThreadToggle = useCallback((commentId: string) => { setExpandedThreads(prev => {)
  const newSet = new Set(prev);
      if (newSet.has(commentId)) {
        newSet.delete(commentId) } else { newSet.add(commentId);
      return newSet });
  }, []);
  // Computed values
  const hasPermission = useCallback((permission: string) => { return config.permissions.includes(permission) || config.permissions.includes('moderation:admin') }, [config.permissions]);
  ...
  show: selectedCount > 0 }
    ...
    { type: 'hide' as const, 
      ...
      show: selectedCount > 0].filter(action => action.show && hasPermission(action.permission)), [selectedCount, hasPermission]);
  return;
...
interface CommentModerationItemProps { comment: TrendingComment;
  selected: boolean;
  ...
  const moderationData = { status: 'pending' as const,
...
async function generateMockComments(_____filters: CommentModerationFilters): Promise<TrendingComment> {
  ...
  return comments;
function generateMockCommentContent(): string { const contents = [
...
function calculateMockStats(comments: TrendingComment): CommentModerationStats { return {
