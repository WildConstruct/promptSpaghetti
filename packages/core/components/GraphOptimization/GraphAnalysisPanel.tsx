...
interface GraphAnalysisResult { nodeCount: number;
  edgeCount: number;
  ...
  */
  ...
  const calculateDepth = (nodeId: string, visited = new Set<string>()): number => { if (visited.has(nodeId)) return 0; // Cycle detection
    visited.add(nodeId);
  ...
  if (deadNodes.length > 0) { optimizationOpportunities.push({)
  type: 'dead_code',
  ...
  if (constantNodes.length > 0) { optimizationOpportunities.push({)
  type: 'constant_prop',
      severity: 'medium' }
  ...
  if (parallelizableNodes.length > 2) { optimizationOpportunities.push({)
...
export const GraphAnalysisPanel: React.FC<GraphAnalysisPanelProps> = ({ )
  ...
  switch (severity) { case 'high': return '#dc3545';
  case 'medium': return '#ffc107';
  case 'low': return '#28a745' };
  const getComplexityColor = (complexity: 'low' | 'medium' | 'high') => { switch (complexity) {
