...
interface RealTimePreviewPanelProps { visible?: boolean;
  onClose?: () => void;
  className?: string;
 ...
 = usePreviewSync({ )
  ...
  const formatTime = useCallback((timestamp: number | null) => { if (!timestamp) return 'Never';
    return new Date(timestamp).toLocaleTimeString() }, []);
  // Handle result selection
  const handleResultClick = useCallback((index: number) => { setSelectedResult(selectedResult === index ? null : index) }, [selectedResult]);
  // Handle lock/unlock result
  const handleToggleLock = useCallback((index: number) => { const result = results[index];
    if (!result) return;
    if (result.locked) {
      unlockResult(index) } else { setSelectedResult(index);
      setShowLockDialog(true) }, [results, unlockResult]);
  // Handle lock confirmation
  const handleConfirmLock = useCallback(() => { if (selectedResult !== null) {
      lockResult(selectedResult, lockNote.trim() || undefined);
      setLockNote('');
      setShowLockDialog(false);
      setSelectedResult(null) }, [selectedResult, lockNote, lockResult]);
  ...
  const changeAnalysis = useMemo(() => { const analysis = getChangeAnalysis();
