...
describe('RealTimePreviewPanel Component', () => { beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('Rendering', () => {
    ...
    test('shows sync status correctly', () => { render(<RealTimePreviewPanel />);
      expect(screen.getByText('Active')).toBeInTheDocument() });
    test('displays preview results', () => { render(<RealTimePreviewPanel />);
    ...
    test('shows locked indicator for locked results', () => { render(<RealTimePreviewPanel />);
      const lockIcons = screen.getAllByText('🔒');
      expect(lockIcons.length).toBeGreaterThan(0) });
  });
  describe('Controls', () => { test('displays real-time sync checkbox', () => {
  ...
  describe('Result Interactions', () => { test('handles result selection', async () => {
  const user = userEvent.setup();
  render(<RealTimePreviewPanel />);
  ...
  borderColor: '#0ea5e9' }
    ...
    test('handles lock/unlock toggle', async () => { const user = userEvent.setup();
      render(<RealTimePreviewPanel />);
    ...
    test('opens lock dialog for unlocked result', async () => { const user = userEvent.setup();
      render(<RealTimePreviewPanel />);
    ...
    test('handles lock confirmation with note', async () => { const user = userEvent.setup();
      render(<RealTimePreviewPanel />);
      // Open lock dialog
      const lockButtons = screen.getAllByTitle('Lock result');
      await user.click(lockButtons[0]);
    ...
    test('handles lock dialog cancellation', async () => { const user = userEvent.setup();
      render(<RealTimePreviewPanel />);
      // Open lock dialog
      const lockButtons = screen.getAllByTitle('Lock result');
      await user.click(lockButtons[0]);
  ...
  describe('Loading and Error States', () => { test('shows loading state', () => {
  const loadingState = {
  ...mockStoreState
  isLoading: true
  results: [] }
    ...
    test('shows error state', () => { const errorState = {
  ...mockStoreState
  ...
  results: [] }
    ...
    test('shows empty state', () => { const emptyState = {
  ...mockStoreState
  results: []
  isLoading: false
  error: null }
    ...
    test('disables refresh button when syncing', () => { const syncingState = {
  ...mockSyncState
  isSyncing: true }
};
  ...
  executionTimeMs: 100 + i
      ...
      const stateWithManyResults = { ...mockStoreState
  results: manyResults }
};
  ...
  describe('Sync Status Display', () => { test('shows disabled status when sync disabled', () => {
  ...
  syncCount: 0 }
    ...
    test('shows syncing status during sync', () => { const syncingState = {
  ...mockSyncState
  isSyncing: true }
};
      mockUsePreviewSync.mockReturnValue(syncingState as any as unknown);
    ...
    test('shows ready status when enabled but no syncs', () => { const readyState = {
  ...mockSyncState
  syncCount: 0
  isSyncing: false }
};
  ...
  describe('Event Handling', () => {
    ...
    test('prevents event propagation on lock button clicks', async () => { const user = userEvent.setup();
  render(<RealTimePreviewPanel />);
  const lockButtons = screen.getAllByTitle('Lock result');
