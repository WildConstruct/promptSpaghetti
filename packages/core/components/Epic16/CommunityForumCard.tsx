...
interface CommunityForumCardProps { post: ForumPost;
...
export const CommunityForumCard: React.FC<CommunityForumCardProps> = ({ )
  ...
  const handleVote = (voteType: 'like' | 'dislike') => { if (voteType === 'like') {
  setUserVote(userVote === 'like' ? null : 'like');
  onLike?.(post.id) } else { setUserVote(userVote === 'dislike' ? null : 'dislike');
  onDislike?.(post.id) };
