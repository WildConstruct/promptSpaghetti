 ...
 * Main Security Dashboard Component
 */
export const SecurityDashboardMain = ({ workspaceId, userId, userRole, initialDashboardType = DashboardType.OPERATIONAL, theme = DashboardTheme.CINEMA, enableRealTimeUpdates = true, refreshInterval = 30 }) => {
    // State management
    ...
    const dataService = useMemo(() => new SecurityDashboardDataService(workspaceId), [workspaceId]);
    // Theme styles
    ...
    const handleRealTimeUpdate = useCallback((data) => {
        switch (data.type) {
            ...
            case 'security_alert':
                ...
                break;
            case 'alert_update':
                setSecurityAlerts(prev => prev.map(alert => alert.id === data.payload.id ? { ...alert, ...data.payload } : alert));
                break;
            case 'compliance_update':
                setComplianceStatus(prev => prev.map(status => status.framework === data.payload.framework
                    ? { ...status, ...data.payload }
                    : status));
                break;
            default:
    ...
    const renderDashboardContent = () => {
        ...
        switch (currentDashboardType) {
            case DashboardType.OPERATIONAL:
                return (_jsx(OperationalSecurityDashboard, { alerts: securityAlerts, metrics: {
                        alerts: {
                            total: securityAlerts.length,
                            newLast24h: securityAlerts.filter(a => Date.now() - a.timestamp.getTime() < 24 * 60 * 60 * 1000).length,
            ...
            case DashboardType.EXECUTIVE:
                return (_jsx(ExecutiveSecurityDashboard, { securityMetrics: securityMetrics, alerts: securityAlerts, complianceStatus: complianceStatus, ...commonProps }));
            case DashboardType.COMPLIANCE:
                return (_jsx(ComplianceSecurityDashboard, { complianceStatus: complianceStatus, alerts: securityAlerts.filter(a => a.category === 'policy_violation'), ...commonProps }));
            case DashboardType.ANALYTICS:
                return (_jsx(SecurityDashboardWorkflow, { workspaceId: workspaceId, userId: userId, userRole: userRole, dashboardType: currentDashboardType }));
            default:
    ...
    if (loading) {
        return (_jsxs("div", { style: {
                ...
                fontFamily: 'Inter, system-ui, sans-serif'
                        ...
                        marginBottom: '24px'
                    } }), _jsx("div", { style: { fontSize: '18px', fontWeight: 600, marginBottom: '8px' }, children: "Loading Security Dashboard..." }), _jsx("div", { style: { fontSize: '14px', color: themeStyles.textSecondary }, children: "Initializing real-time monitoring and threat detection" }), _jsx("style", { children: `
          @keyframes spin {
            ...
            fontFamily: 'Inter, system-ui, sans-serif'
                    ...
                    zIndex: 100
                        ...
                        height: '64px'
                                    ...
                                    cursor: 'pointer',
                                    transition: 'all 0.2s ease'
                                }, children: [_jsx("span", { children: item.icon }), _jsx("span", { children: item.label })] }, item.type))) }), _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '16px' }, children: [_jsxs("div", { style: {
                                        display: 'flex',
                                        alignItems: 'center',
                                        gap: '8px',
                                        padding: '6px 12px',
                                        ...
                                        fontWeight: 600
                                            ...
                                            } }), isConnected ? 'LIVE' : 'OFFLINE'] }), securityMetrics && (_jsxs("div", { style: {
                                        padding: '6px 12px',
                                    ...
                                    }, children: ["Risk: ", securityMetrics.riskLevel] }))] })] }) }), _jsx("main", { children: renderDashboardContent() })] }));
};
