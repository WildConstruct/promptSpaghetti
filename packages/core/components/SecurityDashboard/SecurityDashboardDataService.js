...
export class SecurityDashboardDataService {
    ...
    async getSecurityMetrics() {
        ...
        try {
            const response = await this.apiRequest(`/security/metrics/${this.workspaceId}`);
    ...
    async getActiveAlerts(filters) {
        ...
        try {
            ...
            const response = await this.apiRequest(`/security/alerts/${this.workspaceId}?${queryParams.toString()}`);
            if (response.success && response.data) {
    ...
    async getComplianceStatus() {
        ...
        try {
            const response = await this.apiRequest(`/security/compliance/${this.workspaceId}`);
            if (response.success && response.data) {
                // Transform dates
     ...
     */
    async executeSecurityAction(actionType, payload) {
        try {
            const action = {
                type: actionType,
                payload,
            ...
            const response = await this.apiRequest(`/security/actions/${this.workspaceId}`, {
                method: 'POST',
    ...
    async updateAlert(alertId, updates) {
        try {
            const response = await this.apiRequest(`/security/alerts/${this.workspaceId}/${alertId}`, {
                method: 'PATCH',
