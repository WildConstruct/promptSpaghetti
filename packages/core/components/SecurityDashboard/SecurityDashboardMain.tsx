...
export const SecurityDashboardMain: React.FC<SecurityDashboardMainProps> = ({ )
  ...
  useEffect(() => { const initializeDashboard = async () => {
      try {
        ...
        if (enableRealTimeUpdates) { initializeRealTimeUpdates();
    ...
    const connectWebSocket = () => {
      try {
        ...
        ws.onmessage = (event) => { try {
            ...
            handleRealTimeUpdate(data) } catch (error) { console.error('Failed to process real-time update:', error) };
 ...
 catch (error) { console.error('Failed to connect to real-time updates:', error);
  setIsConnected(false) };
    // Initial connection
    connectWebSocket();
    // Cleanup function
  ...
  const _____handleSecurityAction = useCallback(async (action: string, payload: unknown) => { try {
      await dataService.executeSecurityAction(action, payload) } catch (error) { console.error('Failed to execute security action:', error) }, [dataService]);
 ...
 catch (error) { console.error('Failed to update alert:', error) }, [dataService]);
  ...
  const renderDashboardContent = () => { if (!securityMetrics || !securityAlerts) {
  return null;
  const commonProps = {
  theme
  refreshInterval
  ...
  return acc;
              ...
              bySeverity: securityAlerts.reduce((acc, alert) => { acc[alert.severity] = (acc[alert.severity] || 0) + 1;
