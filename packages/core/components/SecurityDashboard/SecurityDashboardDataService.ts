...
export class SecurityDashboardDataService {
  ...
  constructor(workspaceId: string, config?: Partial<DataServiceConfig>) { this.workspaceId = workspaceId;
  this.config = {
  ...
  async getSecurityMetrics(): Promise<SecurityMetrics> {
    ...
    try {
      ...
      if (response.success && response.data) { this.setCache(cacheKey, response.data, this.config.cacheTimeout);
        return response.data } else { throw new Error(response.error || 'Failed to fetch security metrics') } catch (error) { console.error('Failed to fetch security metrics:', error);
  ...
  */
      ...
      if (filters) {
        Object.entries(filters).forEach(([key, value]) => {
          if (Array.isArray(value)) {
            value.forEach(v => queryParams.append(key, v)) } else if (value !== undefined) { queryParams.set(key, String(value)) });
      const response = await this.apiRequest<SecurityAlert>(;);
        `/security/alerts/${this.workspaceId}?${queryParams.toString()}`}
      );
      if (response.success && response.data) { // Transform dates and ensure proper typing
  ...
  timestamp: action.timestamp ? new Date(action.timestamp) : undefined }
        ...
        return alerts;
 else { throw new Error(response.error || 'Failed to fetch security alerts') } catch (error) {
  ...
  async getComplianceStatus(): Promise<ComplianceStatus> {
    ...
    try {
      ...
      if (response.success && response.data) { // Transform dates
 ...
 else { throw new Error(response.error || 'Failed to fetch compliance status') } catch (error) { console.error('Failed to fetch compliance status:', error);
  // Return fallback data
  ...
  */
  ...
  if (!response.success) { throw new Error(response.error || 'Failed to execute security action');
  // Emit event for real-time updates
  this.emit('security_action_executed', action) } catch (error) {
 ...
 else { throw new Error(response.error || 'Failed to update alert') } catch (error) { console.error('Failed to update alert:', error);
  throw error;
  // =============================================================================
  ...
  */
  ...
  const eventListeners = this.listeners.get(event);
  if (eventListeners) { eventListeners.forEach(callback => {)
  try {
  ...
  initializeWebSocket(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onmessage = (event) => { try {
            const data = JSON.parse(event.data);
            this.handleWebSocketMessage(data) } catch (error) { console.error('Failed to parse WebSocket message:', error) };
        this.ws.onclose = () => {
 ...
 catch (error) { console.error('Failed to initialize WebSocket:', error);
  reject(error) });
  /**
   ...
   */
  private async apiRequest<T>(endpoint: string)
    options?: RequestInit
  ...
  timeout: this.config.timeout
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) { try {
  ...
  failed:`, error);}
        if (attempt === this.config.retryAttempts) { return {
  success: false,
   ...
   * Cache management
   */
  private setCache(key: string, data: Record<string, unknown>, ttlSeconds: number): void { this.cache.set(key, {)
  data
  expires: Date.now() + (ttlSeconds * 1000) }
});
  private getFromCache(key: string): any | null { const cached = this.cache.get(key);
  ...
  */
  ...
  private getFallbackSecurityAlerts(): SecurityAlert { return [
  {
  id: 'alert-001'
  severity: 'high'
  category: 'malware'
