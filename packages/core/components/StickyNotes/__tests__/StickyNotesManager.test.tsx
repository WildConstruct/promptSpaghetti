...
describe('StickyNotesManager Component', () => { beforeEach(() => {
    jest.clearAllMocks();
    mockUseGraphStore.mockReturnValue(mockStoreState as any as unknown as unknown as unknown) });
  describe('Rendering', () => { test('renders sticky notes layer when not disabled', () => {
      render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
    ...
    test('passes default props correctly', () => { render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
      );
  ...
  describe('Store Integration', () => { test('retrieves notes from graph store', () => {
      render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
      ...
      expect(screen.getByTestId('notes-count')).toHaveTextContent('2') });
    test('updates store when notes change', () => { render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
      );
    ...
    test('handles empty notes array', () => { mockUseGraphStore.mockReturnValue({)
  stickyNotes: [],
 ...
 as any);
  ...
  describe('React Flow Integration', () => { test('provides correct canvas size', () => {
      render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
  ...
  describe('Error Handling', () => { test('handles store errors gracefully', () => {
    ...
    test('handles missing React Flow context', () => {
      ...
      expect(() => { render(<StickyNotesManager />) }).not.toThrow();
    });
  });
  describe('Performance', () => { test('does not re-render when store state is unchanged', () => {
 ...
 as any);
  ...
  describe('Props Validation', () => {
    ...
    test('handles string props correctly', () => { render();
        <ReactFlowProvider>
  ...
  describe('Integration with Graph Editor', () => {
    ...
    test('respects canvas boundaries', () => { render();
        <ReactFlowProvider>
          <StickyNotesManager />
        </ReactFlowProvider>
      );
