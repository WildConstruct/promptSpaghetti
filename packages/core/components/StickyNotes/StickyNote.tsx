...
interface StickyNoteProps { note: StickyNoteType;
  ...
  onBringToFront: () => void;
  className?: string;
const COLOR_THEMES: Record<StickyNoteColor, { background: string;
  border: string;
  text: string;
  shadow: string;
...
const CATEGORY_ICONS: Record<StickyNoteCategory, string> = { general: '📝'
  technical: '⚙️'
  creative: '💡'
  feedback: '💬'
  question: '❓'
...
export const StickyNote: React.FC<StickyNoteProps> = ({ )
  ...
  // Handle drag start
  const handleMouseDown = (e: React.MouseEvent) => { if (!note.behavior.draggable || editing || isResizing) return;
  e.preventDefault();
  e.stopPropagation();
  setIsDragging(true);
  setDragStart({)
  ...
  // Handle resize start
  const handleResizeStart = (e: React.MouseEvent, handle: string) => { e.preventDefault();
  e.stopPropagation();
  setIsResizing(true);
  ...
  const handleDoubleClick = (e: React.MouseEvent) => { if (note.behavior.editable) {
      e.preventDefault();
      e.stopPropagation();
      onStartEdit() };
  // Handle content change
  const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => { const newContent = {
  ...note.content
  text: e.target.value }
};
  ...
  // Handle key events
  const handleKeyDown = (e: React.KeyboardEvent) => { if (editing) {
      if (e.key === 'Escape') {
        e.preventDefault();
        onStopEdit() } else { if (e.key === 'Delete' && selected) {
        e.preventDefault();
        onDelete() } else if (e.key === 'Enter' && selected) { e.preventDefault();
        onStartEdit() };
