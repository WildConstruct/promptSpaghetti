 ...
 */
export const AuditManagementDashboard = () => {
    ...
    const renderRiskScore = (score) => {
        ...
        if (score >= 7)
            ...
            color = 'gold';
        return (_jsxs("div", { style: { display: 'flex', alignItems: 'center' }, children: [_jsx(Progress, { percent: score * 10, size: "small", strokeColor: color, showInfo: false, style: { width: 60, marginRight: 8 } }), _jsxs("span", { children: [score, "/10"] })] }));
    };
    ...
    const renderAnomalousPatterns = () => {
        if (dashboardState.anomalousPatterns.length === 0)
            return null;
        return (_jsxs("div", { style: { marginBottom: 24 }, children: [_jsx("h3", { children: "\uD83D\uDEA8 Anomalous Patterns Detected" }), dashboardState.anomalousPatterns.map((pattern, index) => (_jsx(Alert, { message: pattern.description, description: pattern.recommendation, type: pattern.severity === 'critical' ? 'error' : pattern.severity === 'high' ? 'warning' : 'info', showIcon: true, style: { marginBottom: 8 }, action: _jsx(Button, { size: "small", onClick: () => console.log('Investigate pattern:', pattern), children: "Investigate" }) }, index)))] }));
    };
    // Audit events table
    const renderAuditEventsTable = () => {
        const columns = [
            ...
            },
            {
                ...
                render: (frameworks) => (_jsx("div", { children: frameworks.map(framework => (_jsx(Tag, { size: "small", children: framework.toUpperCase() }, framework))) }))
            },
            {
                title: 'Actions',
                key: 'actions',
                render: (record) => (_jsx(Space, { children: _jsx(Button, { size: "small", onClick: () => setDashboardState(prev => ({ ...prev, selectedEvent: record })), children: "View Details" }) }))
            }
        ];
        return (_jsx(Table, { columns: columns, dataSource: dashboardState.events, loading: dashboardState.loading, pagination: {
    ...
    const renderAnalyticsCharts = () => {
        ...
        return (_jsxs(Row, { gutter: 16, children: [_jsx(Col, { span: 12, children: _jsx(Card, { title: "Severity Distribution", style: { marginBottom: 16 }, children: _jsx(Pie, { data: severityData, angleField: "value", colorField: "type", radius: 0.8, label: {
                                type: 'outer',
    ...
    const renderEventDetailsModal = () => {
        ...
        return (_jsx(Modal, { title: `Audit Event Details - ${event.title}`, visible: !!dashboardState.selectedEvent, onCancel: () => setDashboardState(prev => ({ ...prev, selectedEvent: null })), width: 800, footer: [
                _jsx(Button, { onClick: () => setDashboardState(prev => ({ ...prev, selectedEvent: null })), children: "Close" }, "close")
            ], children: _jsxs(Tabs, { defaultActiveKey: "details", children: [_jsxs(TabPane, { tab: "Event Details", children: [_jsxs(Row, { gutter: 16, children: [_jsxs(Col, { span: 12, children: [_jsxs("p", { children: [_jsx("strong", { children: "Event ID:" }), " ", event.id] }), _jsxs("p", { children: [_jsx("strong", { children: "Timestamp:" }), " ", event.timestamp.toLocaleString()] }), _jsxs("p", { children: [_jsx("strong", { children: "Event Type:" }), " ", event.event_type] }), _jsxs("p", { children: [_jsx("strong", { children: "Severity:" }), " ", renderSeverityBadge(event.severity)] }), _jsxs("p", { children: [_jsx("strong", { children: "Status:" }), " ", _jsx(Tag, { color: "blue", children: event.status })] }), _jsxs("p", { children: [_jsx("strong", { children: "Risk Score:" }), " ", renderRiskScore(event.risk_score)] })] }), _jsxs(Col, { span: 12, children: [_jsxs("p", { children: [_jsx("strong", { children: "User ID:" }), " ", event.user_id || 'System'] }), _jsxs("p", { children: [_jsx("strong", { children: "IP Address:" }), " ", event.ip_address || 'N/A'] }), _jsxs("p", { children: [_jsx("strong", { children: "System Component:" }), " ", event.system_component] }), _jsx("p", { children: _jsx("strong", { children: "Compliance Frameworks:" }) }), _jsx("div", { children: event.compliance_frameworks.map(framework => (_jsx(Tag, { children: framework.toUpperCase() }, framework))) })] })] }), _jsxs("div", { style: { marginTop: 16 }, children: [_jsx("p", { children: _jsx("strong", { children: "Description:" }) }), _jsx("p", { children: event.description })] }), _jsxs("div", { style: { marginTop: 16 }, children: [_jsx("p", { children: _jsx("strong", { children: "Risk Factors:" }) }), event.risk_factors.map(factor => (_jsx(Tag, { color: "orange", children: factor }, factor)))] })] }, "details"), _jsx(TabPane, { tab: "Metadata", children: _jsx("pre", { children: JSON.stringify(event.metadata, null, 2) }) }, "metadata"), _jsxs(TabPane, { tab: "Chain Integrity", children: [_jsxs("p", { children: [_jsx("strong", { children: "Chain Hash:" }), " ", event.chain_hash] }), _jsxs("p", { children: [_jsx("strong", { children: "Previous Hash:" }), " ", event.previous_hash || 'N/A'] }), _jsx(Button, { type: "primary", icon: _jsx(SecurityScanOutlined, {}), children: "Verify Integrity" })] }, "integrity")] }) }));
    };
    // Filter panel
    const renderFilterPanel = () => (_jsxs(Card, { style: { marginBottom: 16 }, children: [_jsxs(Row, { gutter: 16, children: [_jsxs(Col, { span: 6, children: [_jsx("label", { children: "Date Range:" }), _jsx(RangePicker, { style: { width: '100%' }, onChange: (dates) => setFilters(prev => ({
                                    ...prev,
                                    dateRange: dates ? [dates[0]?.toDate(), dates[1]?.toDate()] : [undefined, undefined]
                                })) })] }), _jsxs(Col, { span: 4, children: [_jsx("label", { children: "Event Types:" }), _jsx(Select, { mode: "multiple", style: { width: '100%' }, placeholder: "Select types", onChange: (values) => setFilters(prev => ({ ...prev, eventTypes: values })), children: Object.values(AuditEventType).map(type => (_jsx(Select.Option, { value: type, children: type.replace('_', ' ').toUpperCase() }, type))) })] }), _jsxs(Col, { span: 4, children: [_jsx("label", { children: "Severity:" }), _jsx(Select, { mode: "multiple", style: { width: '100%' }, placeholder: "Select severity", onChange: (values) => setFilters(prev => ({ ...prev, severities: values })), children: Object.values(AuditSeverity).map(severity => (_jsx(Select.Option, { value: severity, children: severity.toUpperCase() }, severity))) })] }), _jsxs(Col, { span: 4, children: [_jsx("label", { children: "Compliance:" }), _jsx(Select, { mode: "multiple", style: { width: '100%' }, placeholder: "Select frameworks", onChange: (values) => setFilters(prev => ({ ...prev, complianceFrameworks: values })), children: Object.values(ComplianceFramework).map(framework => (_jsx(Select.Option, { value: framework, children: framework.toUpperCase() }, framework))) })] }), _jsxs(Col, { span: 6, children: [_jsx("label", { children: "Search:" }), _jsx(Search, { placeholder: "Search events...", onChange: (e) => setFilters(prev => ({ ...prev, searchText: e.target.value })), onSearch: applyFilters })] })] }), _jsx(Row, { gutter: 16, style: { marginTop: 16 }, children: _jsx(Col, { children: _jsxs(Space, { children: [_jsx(Button, { type: "primary", icon: _jsx(FilterOutlined, {}), onClick: applyFilters, children: "Apply Filters" }), _jsx(Button, { icon: _jsx(ReloadOutlined, {}), onClick: loadAuditData, children: "Refresh" }), _jsx(Button, { icon: _jsx(DownloadOutlined, {}), onClick: () => exportAuditData('csv'), children: "Export CSV" }), _jsx(Button, { icon: _jsx(DownloadOutlined, {}), onClick: () => exportAuditData('json'), children: "Export JSON" })] }) }) })] }));
...
const ComplianceReportsTab = () => {
    ...
    return (_jsxs("div", { children: [_jsxs(Row, { gutter: 16, style: { marginBottom: 16 }, children: [_jsx(Col, { span: 6, children: _jsx(Select, { style: { width: '100%' }, value: selectedFramework, onChange: setSelectedFramework, children: Object.values(ComplianceFramework).map(framework => (_jsx(Select.Option, { value: framework, children: framework.toUpperCase() }, framework))) }) }), _jsx(Col, { children: _jsx(Button, { type: "primary", onClick: generateComplianceReport, loading: loading, children: "Generate Report" }) })] }), reportData && (_jsxs("div", { children: [_jsxs(Row, { gutter: 16, style: { marginBottom: 16 }, children: [_jsx(Col, { span: 6, children: _jsx(Card, { children: _jsx(Statistic, { title: "Total Events", value: reportData.summary.total_events }) }) }), _jsx(Col, { span: 6, children: _jsx(Card, { children: _jsx(Statistic, { title: "Critical Events", value: reportData.summary.critical_events }) }) }), _jsx(Col, { span: 6, children: _jsx(Card, { children: _jsx(Statistic, { title: "High Risk Events", value: reportData.summary.high_risk_events }) }) }), _jsx(Col, { span: 6, children: _jsx(Card, { children: _jsx(Statistic, { title: "Unresolved Events", value: reportData.summary.unresolved_events }) }) })] }), _jsxs(Card, { title: "Risk Analysis", style: { marginBottom: 16 }, children: [_jsxs("p", { children: [_jsx("strong", { children: "Average Risk Score:" }), " ", reportData.risk_analysis.average_risk_score?.toFixed(2)] }), _jsxs("p", { children: [_jsx("strong", { children: "High Risk Events:" }), " ", reportData.risk_analysis.high_risk_events?.length || 0] })] }), _jsx(Card, { title: "Recommendations", children: _jsx("ul", { children: reportData.recommendations.map((rec, index) => (_jsx("li", { children: rec }, index))) }) })] }))] }));
};
