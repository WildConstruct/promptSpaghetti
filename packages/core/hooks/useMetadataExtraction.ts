...
export function useMetadataExtraction(options: UseMetadataExtractionOptions = {}) {
  ...
  const extractMetadata = useCallback((nodeId: string, text: string) => {
  ...
  const getNodeMetadata = useCallback((nodeId: string): SegmentMetadata | undefined => {
    return metadataState[nodeId]?.metadata;
  }, [metadataState]);

  // Check if extraction is in progress for a node
  const isExtracting = useCallback((nodeId: string): boolean => {
    return metadataState[nodeId]?.extracting || false;
  }, [metadataState]);

  // Manually trigger extraction (bypasses debounce)
  const extractNow = useCallback(async (nodeId: string, text: string): Promise<SegmentMetadata | null> => {
  ...
  const extractBatch = useCallback(async (segments: Array<{ nodeId: string; text: string }>) => {
  ...
  const calculateRelevance = useCallback((metadata: SegmentMetadata, filters: Partial<SegmentMetadata>) => {
