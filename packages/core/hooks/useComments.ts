...
interface UseCommentsOptions { workspaceId: string;
 ...
 = options;
  ...
  const fetchComments = useCallback(async (pageNum: number = 1, append: boolean = false) => { try {
      if (!append) {
  ...
  'X-User-Id': userId }
      ...
      if (append) { setComments(prev => [...prev, ...data.data]) } else { setComments(data.data);
      setHasMore(data.pagination.has_next);
      setPage(pageNum) } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
  setError(errorMessage);
  console.error('Failed to fetch comments:', err) } finally { setLoading(false);
      setLoadingMore(false) }, [workspaceId, targetType, targetId, userId, limit, sortOrder]);
  ...
  body: JSON.stringify(commentData);
    ...
    setComments(prev => { )
  if (sortOrder === 'desc') {
        return [newComment, ...prev] } else { return [...prev, newComment] });
    return newComment;
  }, [userId, sortOrder]);
  ...
  const loadMore = useCallback(() => { if (!hasMore || loadingMore) return;
    fetchComments(page + 1, true) }, [fetchComments, page, hasMore, loadingMore]);
  // Refresh comments (reset to first page)
  const refresh = useCallback(() => { setPage(1);
    fetchComments(1, false) }, [fetchComments]);
  ...
  useEffect(() => { if (comments.length > 0) {
