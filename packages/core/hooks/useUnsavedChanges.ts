...
interface UseUnsavedChangesOptions { hasUnsavedChanges: boolean;
  projectName?: string;
  ...
  const confirmNavigation = useCallback((action: string, callback: () => void) => { if (hasUnsavedChanges) {
      setDialogAction(action);
      setPendingCallback(() => callback);
      setShowUnsavedDialog(true) } else { // No unsaved changes, proceed immediately
      callback() }, [hasUnsavedChanges]);

  // Handle save and continue
  const handleSave = useCallback(async () => { if (onSave) {
      try {
        ...
        if (saveSuccessful && pendingCallback) {
          ...
          setShowUnsavedDialog(false) } else if (!saveSuccessful) { // Save failed, keep dialog open
          console.warn('Save operation failed') } catch (error) {
  console.error('Error during save operation:', error);
  // Keep dialog open on error
 else { // No save handler provided, just proceed
      if (pendingCallback) {
        pendingCallback();
        setPendingCallback(null);

      setShowUnsavedDialog(false) }, [onSave, pendingCallback]);

  // Handle don't save and continue
  const handleDontSave = useCallback(() => { if (pendingCallback) {
      pendingCallback();
      setPendingCallback(null);

    setShowUnsavedDialog(false) }, [pendingCallback]);

  // Handle cancel (stay on current page)
  const handleCancel = useCallback(() => { setPendingCallback(null);
