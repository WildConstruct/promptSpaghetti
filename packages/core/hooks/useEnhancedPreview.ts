...
export interface VarianceAnalysis { // Content analysis
...
export interface PreviewPerformanceStats { totalExecutionTime: number;
...
export const useEnhancedPreviewResultManagement = () => { return null; };
  ...
  const calculateVarianceAnalysis = useCallback((results: EnhancedPreviewResult): VarianceAnalysis => { const validResults = results.filter(r => !r.error && r.output);
  ...
  return sum - (proportion * Math.log2(proportion)) }, 0);
  ...
  sessionId?: string;
  userId?: string;
  graphId?: string } = {}) => { // Cancel any existing run
  ...
  userId,
  graphId,
  enableProfiling: true // Request detailed execution info }
}),
        signal: controller.signal;
  });
      if (!response.ok) { throw ErrorFactory.createAPIError()
          response.status }
  ...
  : 0,
  fastestExecution: executionTimes.length > 0 ? Math.min(...executionTimes) : 0,
  slowestExecution: executionTimes.length > 0 ? Math.max(...executionTimes) : 0,
  throughput: enhancedResults.length / ((overallEndTime - overallStartTime) / 1000),
  ...
  varianceAnalysis
      ...
      if (enableAutoSave) { for (const result of enhancedResults) {
  ...
  const selectResult = useCallback((resultId: string) => { setState(prev => ({)
  ...prev,
  ...
  const deselectResult = useCallback((resultId: string) => { setState(prev => ({)
  ...prev,
  ...
  const toggleResultSelection = useCallback((resultId: string) => { const isSelected = state.selectedResultIds.has(resultId);
    if (isSelected) {
      deselectResult(resultId) } else { selectResult(resultId) }, [state.selectedResultIds, selectResult, deselectResult]);
  const selectAllResults = useCallback(() => { const allIds = state.results.map(r => r.id);
    setState(prev => ({)
  ...prev,
  ...
  const clearSelection = useCallback(() => { setState(prev => ({)
  ...prev,
      selectedResultIds: new Set() }
  ...
  const saveResult = useCallback(async (resultId: string, metadata?: any) => { const result = state.results.find(r => r.id === resultId);
    if (!result) {
  ...
  const rateResult = useCallback((resultId: string, rating: number) => { setState(prev => ({)
  ...prev,
  results: prev.results.map(r => ),
  r.id === resultId
  ? {
  ...
  const addNoteToResult = useCallback((resultId: string, note: string) => { setState(prev => ({)
  ...prev,
  results: prev.results.map(r => ),
  r.id === resultId
  ? {
  ...
  const tagResult = useCallback((resultId: string, tags: string) => { setState(prev => ({)
  ...prev,
  results: prev.results.map(r => ),
  r.id === resultId
  ? {
  ...
  const cancelPreview = useCallback(() => { abortRef.current?.abort();
  setState(prev => ({)
  ...prev,
  loading: false,
