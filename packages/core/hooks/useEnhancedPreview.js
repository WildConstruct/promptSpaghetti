...
const calculateVarianceAnalysis = useCallback((results) => {
    ...
    const professionalSuitability = Math.min(100, (avgWordCount > 50 ? 50 : avgWordCount) + // Adequate length
...
const runPreview = useCallback(async (graph, options = {}) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw ErrorFactory.createAPIError(response.status, `Preview request failed: ${response.statusText}`, '/preview');
        }
        ...
        throw ErrorFactory.createGraphExecutionError(`Preview execution failed: ${errorMessage}`, error, { operation: 'enhanced_preview' });
    }
}, [
    maxResults,
    generateSeeds,
...
const selectResult = useCallback((resultId) => {
    setState(prev => ({
        ...
        results: prev.results.map(r => r.id === resultId ? { ...r, selected: true } : r)
    }));
}, []);
const deselectResult = useCallback((resultId) => {
    setState(prev => ({
        ...
        results: prev.results.map(r => r.id === resultId ? { ...r, selected: false } : r)
    }));
}, []);
...
const saveResult = useCallback(async (resultId, metadata) => {
    ...
    if (!result) {
        throw ErrorFactory.createValidationError('resultId', resultId, 'existing result', { operation: 'save_result' });
    }
    const savedId = await resultManagement.saveResult(result, metadata);
    setState(prev => ({
        ...prev,
        results: prev.results.map(r => r.id === resultId ? { ...r, saved: true } : r)
    }));
    return savedId;
...
const rateResult = useCallback((resultId, rating) => {
    setState(prev => ({
        ...prev,
        results: prev.results.map(r => r.id === resultId
            ? {
                ...r,
                metadata: {
                    ...r.metadata,
