...
interface UseExternalDataImportReturn { // State
...
export const useExternalDataImport = ({ )
  ...
  onError }
  onSuccess
: UseExternalDataImportProps = {}): UseExternalDataImportReturn => { const [state, setState] = useState<ExternalDataImportState>({)
  isLoading: false
  hasError: false
  error: null
  lastQuery: null
  ...
  useEffect(() => { const manager = dataSourceManager;
  ...
  // Setup auto-refresh
  useEffect(() => { if (autoRefresh && state.lastQuery) {
      refreshIntervalRef.current = setInterval(() => {
        refreshData() }, refreshInterval);
      return () => { if (refreshIntervalRef.current) {
  ...
  const queryData = useCallback(async (;);
    ...
    sourceIds?: string
  ): Promise<QueryResult> => { setState(prev => ({ )
  ...prev
  isLoading: true
  hasError: false
  error: null }
}));
    try { const manager = dataSourceManager;
  ...
  lastUpdateTime: new Date().toISOString() }
      ...
      return results } catch (error) { const errorObj = error instanceof Error ? error : new Error('Unknown error');
  setState(prev => ({)
  ...prev
  isLoading: false
  hasError: true
  ...
  const refreshData = useCallback(async (): Promise<void> => { if (state.lastQuery) {
      await queryData(state.lastQuery) }, [state.lastQuery, queryData]);
  // Clear all caches
  ...
  // Add new data source
  const addDataSource = useCallback((source: DataSource) => { const manager = dataSourceManager;
  ...
  // Remove data source
  const removeDataSource = useCallback((sourceId: string) => { const manager = dataSourceManager;
  const sources = (manager as any).dataSources;
  ...
  const getDataSource = useCallback((sourceId: string): DataSource | null => { return state.availableDataSources.find(s => s.id === sourceId) || null }, [state.availableDataSources]);
  // Start real-time updates
  const startRealTimeUpdates = useCallback(() => { if (!enableRealTimeUpdates) return;
    realTimeUpdateRef.current = setInterval(() => {
      if (state.lastQuery) {
        queryData(state.lastQuery) }, 30000); // Update every 30 seconds
  }, [enableRealTimeUpdates, state.lastQuery, queryData]);
  // Stop real-time updates
  const stopRealTimeUpdates = useCallback(() => { if (realTimeUpdateRef.current) {
      clearInterval(realTimeUpdateRef.current);
      realTimeUpdateRef.current = undefined }, []);
  // Validate query structure
  const validateQuery = useCallback((query: HistoricalQuery): { valid: boolean; errors: string } => { const errors: string = [];
  if (!query.category) {
  ...
  const getQuerySuggestions = useCallback((partial: Partial<HistoricalQuery>): string => { const suggestions: string = [];
  // Era suggestions
  if (!partial.era) {
  suggestions.push(
  ...
  // Region suggestions
  if (!partial.region) { suggestions.push(
  'Add regions like "europe", "asia", "middle-east" for more specific results'
  );
  return suggestions }, []);
  // Export results in different formats
  const exportResults = useCallback((format: 'json' | 'csv'): string => { if (format === 'json') {
...
export const useQueryBuilder = () => { return null; });
  const getCacheStats = useCallback(() => { const manager = dataSourceManager;
  ...
  const clearCache = useCallback(() => { const manager = dataSourceManager;
    (manager as any).cache.clear();
    getCacheStats() }, [getCacheStats]);
  const optimizeCache = useCallback(() => { const manager = dataSourceManager;
