...
export const useTemplatePreview = ()
  ...
  const generateSampleValues = useCallback((variables: ExtractedVariable): Record<string, string> => {
    ...
    for (const variable of variables) { if (!variableValues[variable.name]) {
        // First try to use the inferred default value from type inference
        if (variable.defaultValue) {
          samples[variable.name] = variable.defaultValue } else { // Use contextual defaults based on node type and template
          const contextualDefault = getContextualDefaults(variable.name, template, undefined);
          samples[variable.name] = contextualDefault;
    return samples }, [variableValues, template]);
  // Generate intelligent sample values based on variable names
  const generateSmartSample = (variableName: string): string => { // Use our enhanced contextual defaults
  ...
  seed: 0
        ...
        if (config.enablePerformanceTracking) { setPerformance(prev => ({)
  ...prev
  ...
  lastUpdate: Date.now() }
        ...
        setIsGenerating(false) }, [template, variableValues, extractedVariables, parseResult, config, isGenerating, generateSampleValues]);
  // Debounced preview update
  const requestPreview = useCallback(() => { if (!config.autoRefresh) return;
    // Clear existing debounce timeout
    if (debounceTimeoutRef.current) {
      clearTimeout(debounceTimeoutRef.current);
    // Set new debounced timeout
    debounceTimeoutRef.current = setTimeout(() => {
      generatePreviews() }, config.debounceMs);
  }, [generatePreviews, config]);
  // Force immediate preview generation
  const forcePreview = useCallback(() => { // Clear any pending debounced calls
    if (debounceTimeoutRef.current) {
      clearTimeout(debounceTimeoutRef.current);
      debounceTimeoutRef.current = null;
    generatePreviews(true) }, [generatePreviews]);
  ...
  const refreshVariant = useCallback(async (variantId: string): Promise<void> => {
    ...
    for (const variable of extractedVariables) { if (!variableValues[variable.name]) {
  ...
  const getPerformanceInsights = useCallback(() => { return {
  ...
  useEffect(() => { if (template.trim()) {
      requestPreview() } else { clearVariants();
    // Cleanup on unmount
    return () => {
      if (debounceTimeoutRef.current) {
        clearTimeout(debounceTimeoutRef.current);
