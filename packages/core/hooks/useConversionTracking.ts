...
export interface ConversionTrackingConfig { enableAutoTracking?: boolean;
  trackPageViews?: boolean;
  trackUserInteractions?: boolean;
  experimentId?: string;
  variantId?: string }

export const useConversionTracking = (config: ConversionTrackingConfig = {}) => { const {
 ...
 = config;
  ...
  useEffect(() => { if (trackPageViews && !hasTrackedPageView.current) {
  ...
  const trackDirectorAction = useCallback((action: string, context: Record<string, any> = {}) => { if (!enableAutoTracking) return;
  ...
  const trackNodeCreation = useCallback((nodeType: string, nodeData: any) => { const eventKey = 'node_created';
  ...
  const trackConnectionCreation = useCallback((source: string, target: string) => { const eventKey = 'connection_made';
  ...
  const trackAdvancedFeature = useCallback((featureName: string, featureContext: Record<string, any> = {}) => { conversionTracker.trackEvent('advanced_feature_used', {)
  ...
  const trackGraphState = useCallback((nodes: Node, edges: Edge) => { const graphComplexity = {
  ...
  ['Conditional', 'Sequential', 'Markov', 'WeightedAdvanced'].includes(n.data?.nodeType)
    ...
    if (graphComplexity.nodeCount === 1) { trackDirectorAction('first-node-created', graphComplexity) } else if (graphComplexity.nodeCount === 5) { trackDirectorAction('workflow-established', graphComplexity) } else if (graphComplexity.nodeCount >= 10) { trackDirectorAction('complex-project-created', graphComplexity);
  ...
  return { // Direct tracking methods
    trackDirectorAction
    trackNodeCreation
