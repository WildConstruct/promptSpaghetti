...
export function useActivityFeed(workspaceId, userId, filters = {}, options = {}) {
    ...
    // Fetch activity feed
    const fetchActivities = useCallback(async (filters, page = 1, append = false) => {
        try {
            if (!append) {
                setLoading(true);
            }
...
export function useProjectActivityFeed(projectId, userId, options = {}) {
    const { limit = 20, autoRefresh = false, refreshInterval = 30000 } = options;
    const [activities, setActivities] = useState([]);
    const [loading, setLoading] = useState(true);
    const [loadingMore, setLoadingMore] = useState(false);
