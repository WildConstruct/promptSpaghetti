...
describe('usePreviewSeeds - Individual Result Management', () => { beforeEach(() => {
 ...
 as Response);
  ...
  describe('Result Locking', () => {
    it('should lock a result and update state', async () => {
      ...
      await act(async () => { await result.current.runPreview(mockGraph) });
      expect(result.current.results).toHaveLength(2);
      expect(result.current.lockedResults).toHaveLength(0);
      // Lock the first result
      act(() => { result.current.lockResult(0, 'Favorite result') });
    ...
    it('should unlock a previously locked result', async () => {
      ...
      await act(async () => { await result.current.runPreview(mockGraph) });
      act(() => { result.current.lockResult(0) });
      expect(result.current.lockedResults).toEqual([0]);
      expect(result.current.results[0].locked).toBe(true);
      // Unlock the result
      act(() => { result.current.unlockResult(0) });
      expect(result.current.lockedResults).toHaveLength(0);
    ...
    it('should handle locking multiple results', async () => {
      ...
      await act(async () => { await result.current.runPreview(mockGraph) });
      // Lock both results
      act(() => { result.current.lockResult(0, 'First favorite');
    ...
    it('should preserve locked state during full regeneration', async () => {
      ...
      await act(async () => { await result.current.runPreview(mockGraph) });
      // Lock first result
      act(() => { result.current.lockResult(0, 'Keep this one') });
  ...
  describe('Individual Result Regeneration', () => { it('should regenerate a specific result without affecting others', async () => {
 ...
 as Response);
      ...
      await act(async () => { await result.current.regenerateResult(0) });
    ...
    it('should handle regeneration errors gracefully', async () => { (fetch as jest.MockedFunction<typeof fetch>)
  .mockResolvedValueOnce({)
  ok: true
  json: async () => mockApiResponse }
 as Response)
    ...
    it('should track regenerating state correctly', async () => { // Mock a delayed response to test regenerating state
 ...
 as Response)
  ...
  executionTimeMs: 100 }
      ...
      await act(async () => { await regenerationPromise });
  ...
  describe('Edge Cases', () => {
    it('should handle attempts to lock non-existent result index', () => {
      ...
      act(() => { result.current.lockResult(999) });
    ...
    it('should handle attempts to regenerate non-existent result index', async () => {
      ...
      await act(async () => { await result.current.regenerateResult(999) });
