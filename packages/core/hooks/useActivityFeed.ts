...
import { ActivityEventWithActorInfo, 
  ActivityEventFilter }
  PaginatedResponse 
...
export function useActivityFeed()
  ...
  const buildQueryString = useCallback((filters: ActivityEventFilter, page: number) => { const params = new URLSearchParams({)
  page: page.toString()
  ...
  'X-User-Id': userId }
      ...
      if (append) { setActivities(prev => [...prev, ...data.data]) } else { setActivities(data.data);
      setHasMore(data.pagination.has_next);
      setPage(page) } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
  setError(errorMessage);
  console.error('Failed to fetch activities:', err) } finally { setLoading(false);
      setLoadingMore(false) }, [workspaceId, userId, buildQueryString]);
  ...
  'X-User-Id': userId }
      ...
      setStats(statsData);
 catch (err) { console.error('Failed to fetch activity stats:', err) }, [workspaceId, userId]);
  // Fetch event types
  ...
  'X-User-Id': userId }
      ...
      setEventTypes(types);
 catch (err) { console.error('Failed to fetch event types:', err) }, [workspaceId, userId]);
  // Load more activities (pagination)
  const loadMore = useCallback(() => { if (!hasMore || loadingMore) return;
    fetchActivities(filters, page + 1, true) }, [filters, page, hasMore, loadingMore, fetchActivities]);
  // Refresh activities (reset to first page)
  const refresh = useCallback(() => { setPage(1);
    fetchActivities(filters, 1, false) }, [filters, fetchActivities]);
 ...
 catch (err) { console.error('Failed to fetch activity event:', err);
  return null }, [userId]);
  // Initial fetch and setup
  useEffect(() => { fetchActivities(filters, 1, false);
    fetchStats();
    fetchEventTypes() }, [fetchActivities, fetchStats, fetchEventTypes, filters]);
  ...
  useEffect(() => { // TODO: Integrate with WebSocket for real-time activity updates }
  // This would listen for workspace activity events and update the feed accordingly
  const handleVisibilityChange = () => { if (!document.hidden && autoRefresh) {
  refresh() };
...
export function useProjectActivityFeed()
  ...
  const [hasMore, setHasMore] = useState(false);
  const [page, setPage] = useState(1);
  const fetchProjectActivities = useCallback(async (page: number = 1, append: boolean = false) => { try {
      if (!append) {
        setLoading(true) } else { setLoadingMore(true);
  setError(null);
  ...
  'X-User-Id': userId }
      ...
      setPage(page) } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
  setError(errorMessage);
  console.error('Failed to fetch project activities:', err) } finally { setLoading(false);
      setLoadingMore(false) }, [projectId, userId, limit]);
  const loadMore = useCallback(() => { if (!hasMore || loadingMore) return;
    fetchProjectActivities(page + 1, true) }, [page, hasMore, loadingMore, fetchProjectActivities]);
  const refresh = useCallback(() => { setPage(1);
    fetchProjectActivities(1, false) }, [fetchProjectActivities]);
