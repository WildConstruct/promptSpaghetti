...
interface UseDiffSessionResult { session: VisualDiffSession | null;
  ...
  ...options.headers
    ...
    return data }, []);
  const createSession = useCallback(async (request: CreateDiffSessionRequest) => { setLoading(true);
  setError(null);
  try {
 ...
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to create session';
  setError(errorMessage);
  console.error('Failed to create diff session:', err) } finally { setLoading(false) }, [apiCall]);
  ...
  method: 'PATCH'
      ...
      setSession(result.data);
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to update session';
  setError(errorMessage);
  console.error('Failed to update diff session:', err) } finally { setLoading(false) }, [session, apiCall]);
  ...
  method: 'DELETE';
      ...
      setSession(null);
      setComparison(null);
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to delete session';
  setError(errorMessage);
  console.error('Failed to delete diff session:', err) } finally { setLoading(false) }, [apiCall]);
