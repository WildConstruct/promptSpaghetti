...
enableRealTimeUpdates = false, cacheStrategy = 'conservative', onError, onSuccess } = {}) => {
    ...
    useEffect(() => {
        ...
        const sources = Array.from(manager.dataSources.values());
    ...
    // Query external data
    const queryData = useCallback(async (query, sourceIds) => {
        setState(prev => ({
            ...prev,
            isLoading: true,
            hasError: false,
    ...
    const updateDataSource = useCallback((sourceId, updates) => {
        ...
        if (existingSource) {
            ...
            setState(prev => ({
                ...prev,
                availableDataSources: prev.availableDataSources.map(s => s.id === sourceId ? updatedSource : s),
    ...
    const toggleDataSource = useCallback((sourceId) => {
        setState(prev => {
            const source = prev.availableDataSources.find(s => s.id === sourceId);
            if (!source)
                return prev;
            const updatedSources = prev.availableDataSources.map(s => s.id === sourceId ? { ...s, enabled: !s.enabled } : s);
            return {
                ...prev,
    ...
    const getQuerySuggestions = useCallback((partial) => {
        const suggestions = [];
        // Era suggestions
        if (!partial.era) {
