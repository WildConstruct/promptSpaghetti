...
export function useAutosave(options: AutosaveOptions = {}) {
  ...
  const measureSavePerformance = useCallback(
    (startTime: number, nodeCount: number) => {
      const duration = performance.now() - startTime;

      // Log warning if save takes too long
      if (duration > 50) {
        console.warn(
          `Autosave took ${duration.toFixed(2)}ms for ${nodeCount} nodes`
        );
      }
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('autosave-performance', {
      ...
      if (e.newValue) {
        try {
          ...
          if (remoteVersion > versionRef.current) {
            ...
            window.dispatchEvent(
              new CustomEvent('storage-conflict', {
                detail: {
