...
interface UseAutosaveProps { nodes: Node;
  edges: Edge;
  intervalMs?: number;
  storageKey?: string }


interface UseAutosaveReturn { showRestorePrompt: boolean }
},
...
: UseAutosaveProps): UseAutosaveReturn => {
  ...
  useEffect(() => { try {
      ...
      if (saved) {
        ...
        if (draft && draft.nodes && draft.edges) {
          ...
          setShowRestorePrompt(true) } catch (error) { console.warn('Failed to parse saved draft:', error);
  localStorage.removeItem(storageKey) }, [storageKey]);
  ...
  useEffect(() => { console.log('Clearing localStorage to prevent infinite loops');
