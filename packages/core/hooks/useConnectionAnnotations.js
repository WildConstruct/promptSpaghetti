...
export const useConnectionAnnotations = ({ edges, onEdgesChange, autoOptimizePositions = true }) => {
    ...
    const addLabel = useCallback((edgeId, label, options) => {
        const updatedEdges = annotatedEdges.map(edge => edge.id === edgeId
            ? {
                ...edge,
                label,
                showLabel: true,
    ...
    const showAllLabelsToggle = useCallback(() => {
        ...
        const updatedEdges = annotatedEdges.map(edge => edge.label ? { ...edge, showLabel: newShowAll } : edge);
        onEdgesChange(updatedEdges);
    }, [state.showAllLabels, annotatedEdges, onEdgesChange]);
    // Hide all labels
    const hideAllLabels = useCallback(() => {
        setState(prev => ({ ...prev, showAllLabels: false }));
        const updatedEdges = annotatedEdges.map(edge => edge.label ? { ...edge, showLabel: false } : edge);
        onEdgesChange(updatedEdges);
    }, [annotatedEdges, onEdgesChange]);
    // Clear all labels
    ...
    const getVisibleLabelsCount = useCallback(() => {
        return annotatedEdges.filter(edge => edge.label && edge.showLabel && edge.label.trim().length > 0).length;
    }, [annotatedEdges]);
    return {
        // State
        annotatedEdges,
...
export const createPresetConnection = (baseEdge, preset, label) => {
    return createAnnotatedEdge(baseEdge, label, connectionAnnotationPresets[preset]);
};
