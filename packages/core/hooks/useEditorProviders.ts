...
export const useEditorProviders = ()
  ...
  for (const hook of hooks) { if (hook.onNodeAdd) {
  ...
  loadGraph: (data: { nodes: Node; edges: Edge }) => { setIsLoading(true);
      try {
  ...
  executeGraph: async () => {
      ...
      try {
        ...
        for (const hook of hooks) { if (hook.onPostExecution) {
            hook.onPostExecution(result, editorContext);
        return result } catch (error) { // Execute error hooks
        const hooks = getSortedHooks();
        for (const hook of hooks) {
          if (hook.onExecutionError) {
            hook.onExecutionError(error as Error, editorContext);
        throw error } finally { setIsLoading(false) }), [editorContext, graphStore, initialNodes, initialEdges, reactFlowInstance]);
  // Helper function to get hooks sorted by priority
  const getSortedHooks = useCallback((): ProviderHook => { return Array.from(providerRegistry.values()).sort((a, b) => a.priority - b.priority) }, []);
  ...
  register: (hook: ProviderHook) => { }
      ...
      if (providersInitialized && hook.onInit) { hook.onInit(editorContext, editorActions) }
  ...
  useEffect(() => { const hooks = getSortedHooks();
    hooks.forEach(hook => {)
  if (hook.onInit) {
  ...
  useEffect(() => { if (providersInitialized && prevNodesRef.current !== initialNodes) {
      const hooks = getSortedHooks();
      hooks.forEach(hook => {)
  ...
  useEffect(() => { if (providersInitialized && prevEdgesRef.current !== initialEdges) {
      const hooks = getSortedHooks();
      hooks.forEach(hook => {)
  ...
  useEffect(() => { if (providersInitialized && prevSelectedRef.current !== selectedNodeId) {
      const hooks = getSortedHooks();
      hooks.forEach(hook => {)
  ...
  useEffect(() => { if (providersInitialized) {
      const hooks = getSortedHooks();
      hooks.forEach(hook => {)
...
export const createProviderHook = (config: ProviderHook): ProviderHook => { return {
...
export const createConsoleLoggerHook = (): ProviderHook => createProviderHook({ )
  ...
  onNodesChange: (nodes, prevNodes) => { console.log('[EditorProvider] Nodes changed', { )
  count: nodes.length
  prevCount: prevNodes.length }
})

  onEdgesChange: (edges, prevEdges) => { console.log('[EditorProvider] Edges changed', { )
  count: edges.length
...
export const createAutoSaveHook = (interval = 30000): ProviderHook => { let autoSaveTimer: NodeJS.Timeout | null = null;
  ...
  onInit: (context, actions) => { }
  autoSaveTimer = setInterval(() => { if (context.hasUnsavedChanges) {
  console.log('[EditorProvider] Auto-saving...');
  actions.saveGraph() }, interval)
...
export const createValidationHook = (): ProviderHook => createProviderHook({ )
  ...
  onNodeUpdate: (nodeId, updates, context) => { // Add validation metadata to updates
  return {
  ...updates
  _validated: false
  _validationTimestamp: Date.now() }
