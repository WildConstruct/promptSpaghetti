...
export interface UseWebSocketOptions { url?: string;
  documentId: string;
  userId?: string;
  authToken?: string;
  enabled?: boolean;
...
export interface UseWebSocketReturn { connectionState: ConnectionState;
  isConnected: boolean;
...
export function useWebSocket(options: UseWebSocketOptions): UseWebSocketReturn { const {
 ...
 = options;
  ...
  useEffect(() => { if (!enabled || !documentId) {
  return;
  ...
  authToken
    ...
    const handleConnectionStateChange = () => { setConnectionState(client.getConnectionState());
    ...
    client.on('graph_update', (update: GraphUpdatePayload) => { optionsRef.current.onGraphUpdate?.(update) });
    client.on('presence_update', (presence: PresenceUpdatePayload) => { optionsRef.current.onPresenceUpdate?.(presence) });
    client.on('user_join', (user: any) => { optionsRef.current.onUserJoin?.(user) });
    client.on('user_leave', (user: any) => { optionsRef.current.onUserLeave?.(user) });
    client.on('error', (error: any) => { optionsRef.current.onError?.(error) });
    // Auto-connect if enabled
    if (enabled) { client.connect(documentId, userId).catch((error) => {
  ...
  const sendGraphUpdate = useCallback((update: GraphUpdatePayload): boolean => { return clientRef.current?.sendGraphUpdate(update) || false }, []);
  const sendPresenceUpdate = useCallback((presence: PresenceUpdatePayload): boolean => { return clientRef.current?.sendPresenceUpdate(presence) || false }, []);
  const connect = useCallback(async (): Promise<void> => { if (clientRef.current && documentId) {
      await clientRef.current.connect(documentId, userId) }, [documentId, userId]);
  const disconnect = useCallback((): void => { clientRef.current?.disconnect() }, []);
  const clearQueue = useCallback((): void => { clientRef.current?.clearMessageQueue();
    setQueuedMessages(0) }, []);
...
export function usePresence(documentId: string, userId: string, userName?: string, userAvatar?: string) {
 ...
 = useWebSocket({ )
  ...
  useEffect(() => { if (isConnected) {
      requestPresenceData() }, [isConnected, requestPresenceData]);
  ...
  const updateSelection = useCallback((newSelection: string) => { setSelection(newSelection);
    sendSelectionUpdate(newSelection) }, [sendSelectionUpdate]);
  const updateActivity = useCallback((tool?: string, typing?: boolean, focusedNode?: string) => { if (tool !== undefined) setCurrentTool(tool);
  ...
  useEffect(() => { if (isTyping) {
      const timeout = setTimeout(() => {
        setIsTyping(false);
        sendActivityUpdate(currentTool, false, focusedNodeId) }, 3000);
      return () => clearTimeout(timeout);
  }, [isTyping, currentTool, focusedNodeId, sendActivityUpdate]);
  // Combine all user data
  const allUsers = useMemo(() => { const users: any = [];
