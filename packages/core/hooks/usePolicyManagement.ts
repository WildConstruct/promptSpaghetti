...
export interface PolicyEvaluationOptions { userId?: string;
...
export const usePolicyManagement = (config: PolicyManagementHookConfig = {}) => { const {
    autoEvaluate = false
 ...
 = config;
  ...
  enabled?: boolean }) => { try {
      ...
      setError(null) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load policies') } finally { setIsLoading(false) }, [policyManager]);
  // Create new policy
  const createPolicy = useCallback(async (;);
    ...
    try {
      ...
      return newPolicy } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to create policy';
  setError(errorMessage);
  throw new Error(errorMessage) } finally { setIsLoading(false) }, [policyManager, loadPolicies]);
  ...
  const updatePolicy = useCallback(async (;);
    ...
    try {
      ...
      await loadPolicies(); // Refresh policies
      return updatedPolicy } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to update policy';
  setError(errorMessage);
  throw new Error(errorMessage) } finally { setIsLoading(false) }, [policyManager, loadPolicies]);
  // Delete policy
  const deletePolicy = useCallback(async (policyId: string, deletedBy: string) => {
    try {
      ...
      await loadPolicies(); // Refresh policies
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to delete policy';
  setError(errorMessage);
  throw new Error(errorMessage) } finally { setIsLoading(false) }, [policyManager, loadPolicies]);
  ...
  contentContext: evaluationOptions.contentContext
      ...
      return results;
 catch (err) { const errorMessage = err instanceof Error ? err.message : 'Policy evaluation failed';
  setError(errorMessage);
  throw new Error(errorMessage) } finally { setIsLoading(false) }, [policyManager]);
  // Quick policy check for specific scenarios
  const checkVFXHistoricalAccuracy = useCallback(async (;);
    templateId: string
    historicalPeriod: string
    culturalContext: string
    expertReviewed: boolean = false): Promise<{ allowed: boolean; violations: string; reviewRequired: boolean }> => { const results = await evaluatePolicies({)
  entityType: 'TEMPLATE',
      entityId: templateId,
      operation: {,
  ...
  contentContext: { historicalPeriod,
  culturalContext,
  ...
  const checkDataProtectionCompliance = useCallback(async (;);
    ...
    dataType: string,
    operation: string,
    dataClassification: string): Promise<{ compliant: boolean; frameworks: string; actions: string }> => { const results = await evaluatePolicies({)
  userId,
      entityType: 'USER',
      entityId: userId,
      operation: {,
  type: operation }
    ...
    return { compliant: nonCompliant.length === 0,
  frameworks }
  actions
};
  }, [evaluatePolicies]);
  // Generate compliance report
  const generateComplianceReport = useCallback(async (framework: ComplianceFramework) => { try {
      setIsLoading(true);
      const report = await policyManager.generateComplianceReport(framework);
      return report } catch (err) { const errorMessage = err instanceof Error ? err.message : 'Failed to generate compliance report';
  setError(errorMessage);
  throw new Error(errorMessage) } finally { setIsLoading(false) }, [policyManager]);
  // Get policy statistics
  const getPolicyStatistics = useCallback(() => { const stats = {
  ...
  return acc;
      ...
      byType: policies.reduce((acc, policy) => { acc[policy.type] = (acc[policy.type] || 0) + 1;
  ...
  type?: PolicyType;
  status?: PolicyStatus;
  search?: string }) => { let filtered = policies;
    if (filters.domain) {
  ...
  const getRecentEvaluations = useCallback((limit: number = 20) => { return evaluationResults
      .sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())
      .slice(0, limit) }, [evaluationResults]);
  // Get policy violations
  const getPolicyViolations = useCallback((filters?: { )
  severity?: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  resolved?: boolean;
  entityType?: 'USER' | 'TEMPLATE' | 'PROJECT' | 'TRANSACTION' | 'CONTENT';
  limit?: number }) => { let filtered = violations;
  if (filters?.severity) {
  ...
  return filters?.limit ? sorted.slice(0, filters.limit) : sorted }, [violations]);
  // Setup event listeners
  useEffect(() => { if (!enableRealTimeUpdates) return;
  ...
  setViolations(prev => [violation, ...prev].slice(0, 100)); // Keep last 100 violations
    ...
    const handlePolicyCreated = ({ policy }: { policy: UnifiedPolicy }) => { setPolicies(prev => [policy, ...prev]) };
    const handlePolicyUpdated = ({ newPolicy }: { newPolicy: UnifiedPolicy }) => { setPolicies(prev => prev.map(p => p.id === newPolicy.id ? newPolicy : p)) };
    const handlePolicyDeleted = ({ policy }: { policy: UnifiedPolicy }) => { setPolicies(prev => prev.filter(p => p.id !== policy.id)) };
    ...
    return () => { policyManager.off('policyViolation', handlePolicyViolation);
