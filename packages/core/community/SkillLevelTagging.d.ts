...
export interface SkillLevelDimensions { technical_complexity: number;
...
export interface ContentSkillTag { id: string;
    ...
    secondary_skills: Array<{ domain: SkillDomain;
        subcategory?: string;
        level: SkillLevel;
        weight: number }>;
    learning_path_info: { suitable_for_paths: string[];
...
export interface DifficultyIndicators { reading_level: number;
...
export interface CommunitySkillFeedback { perceived_difficulty: {
        average_rating: number;
        rating_distribution: Record<string, number>;
        total_ratings: number }
}
    };
    level_appropriateness: { too_easy_votes: number;
        just_right_votes: number;
        too_hard_votes: number;
        total_votes: number };
    learning_effectiveness: { helped_learn_skill: number;
        clear_explanations: number;
        good_examples: number;
        would_recommend: number };
    improvement_areas: Array<{ area: string;
...
export interface AdaptiveElements { has_difficulty_options: boolean;
...
export interface LearningModule { id: string;
    ...
    content_items: Array<{ content_id: string;
        content_type: string;
        required: boolean;
        estimated_time: number;
...
export interface PathAdaptiveFeatures { skill_gap_detection: boolean;
...
export interface PathSuccessMetrics { target_completion_rate: number;
...
export interface CompletionCriteria { content_completion_required: number;
...
export interface SkillAssessmentEngine { analyzeContent(content: any): Promise<SkillLevelClassification>;
...
export interface UserSkillProfile { user_id: string;
    ...
    learning_history: { content_completed: ContentCompletionRecord[];
...
export declare const SKILL_LEVEL_DEFINITIONS: { readonly beginner: {
    ...
    readonly intermediate: { readonly description: "Has some experience and understands basic concepts";
    ...
    readonly advanced: { readonly description: "Experienced with the fundamentals, ready for complex applications";
    ...
    readonly expert: { readonly description: "Deep expertise, ready for cutting-edge topics and research";
...
export declare const SKILL_PROGRESSION_PATTERNS: { readonly gradual: {
    ...
    readonly accelerated: { readonly level_progression: readonly ["beginner", "advanced", "expert"];
        readonly overlap_percentage: 10;
        readonly reinforcement_frequency: 2;
        readonly assessment_frequency: 3 };
    readonly specialized: { readonly level_progression: readonly ["intermediate", "advanced", "expert"];
