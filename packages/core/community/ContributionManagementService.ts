...
export class ContributionManagementService implements ContributionRepository { private apiClient: any;
  ...
  // ====================================
  async submitContribution(submissionData: Omit<ContributionSubmission, 'id' | 'created_at' | 'updated_at'>): Promise<ContributionSubmission> {
  try {
  ...
  target_status: 'draft' }
      ...
      const submission: Omit<ContributionSubmission, 'id'> = { ...submissionData
  status: 'submitted'
 ...
 catch (error) { console.error('Failed to submit contribution:', error);
  throw error;
 ...
 catch (error) { console.error('Failed to get user contributions:', error);
      throw error;
  // ====================================
  // Workflow Management
  // ====================================
  ...
  newStage: WorkflowStage }
    ...
    try {
      ...
      const updatedWorkflow: ContributionWorkflow = { ...currentWorkflow
 ...
 catch (error) { console.error('Failed to advance workflow stage:', error);
  throw error;
 ...
 catch (error) { console.error('Failed to submit review feedback:', error);
  throw error;
  ...
  status: approval.approval_type === 'full' ? 'approved' : 'revision_needed' }
      ...
      if (approval.approval_type === 'full') { await this.advanceWorkflowStage(contributionId, 'publication', 'Contribution approved for publication');
      // Notify contributor
      await this.notifyContributor(contributionId, 'approval', approval) } catch (error) { console.error('Failed to approve contribution:', error);
  throw error;
  // ====================================
 ...
 catch (error) { console.error('Failed to run quality assessment:', error);
  throw error;
  ...
  quality_gate_results: results }
      ...
      await this.updateContribution(contributionId, { workflow: updatedWorkflow });
      return results;
 catch (error) { console.error('Failed to check quality gates:', error);
  throw error;
  // ====================================
  ...
  workflow: updatedWorkflow }
      ...
      if (schedule.auto_publish && schedule.scheduled_date) { await this.scheduleAutomaticPublication(contributionId, schedule.scheduled_date) } catch (error) { console.error('Failed to schedule publication:', error);
  throw error;
 ...
 catch (error) { console.error('Failed to publish contribution:', error);
  throw error;
  ...
  async getSystemMetrics(timeRange?: string): Promise<ContributionSystemMetrics> {
    ...
    try {
      ...
      return response.data;
 catch (error) { console.error('Failed to get system metrics:', error);
  throw error;
  // ====================================
 ...
 catch (error) { // Default to comprehensive workflow for new contributors
      return 'comprehensive';
  ...
  private initializeEngagement(): ContributionEngagement { return {
      total_views: 0
      unique_views: 0 }
      view_sources: {}
      likes: 0
  ...
  private createQualityGates(gateNames: string) { return gateNames.map(gateName => {)
  ...
  private mapGateToStage(gateName: string): WorkflowStage { const mapping: Record<string, WorkflowStage> = {
  ...
  private mapStageToStatus(stage: WorkflowStage): ContributionStatus { const mapping: Record<WorkflowStage, ContributionStatus> = {
  ...
  private isValidStageTransition(currentStage: WorkflowStage, newStage: WorkflowStage): boolean { const validTransitions: Record<WorkflowStage, WorkflowStage> = {
