...
export interface CommunityContentQualityMetrics { contentId: string;
    versionId: string;
    assessmentDate: Date;
    assessorId?: string;
...
export interface EditorialQualityMetrics { score: number;
    accuracy: {
...
export interface TechnicalQualityMetrics { score: number;
    formatting: {
...
export interface ContentEngagementMetrics { score: number;
...
export interface CommunityValueMetrics { score: number;
    contribution_value: {
...
export interface AutomatedContentAnalysis { language_analysis: {
...
export interface AutomatedIssue { type: 'grammar' | 'spelling' | 'formatting' | 'accessibility' | 'seo' | 'structure' | 'plagiarism' | 'factual';
...
export interface EditorialReview { reviewer_id: string;
...
export interface QualityFlag { type: 'plagiarism' | 'factual_error' | 'policy_violation' | 'accessibility_issue' | 'quality_concern';
    severity: 'low' | 'medium' | 'high' | 'critical';
    description: string;
    evidence?: string;
...
export interface QualityAssessmentWorkflow { content_id: string;
    version_id: string;
...
export declare class ContentQualityAssessmentService {
    ...
    runAutomatedAnalysis(contentId: string, versionId: string, options?: { )
        ...
        language?: string }): Promise<AutomatedContentAnalysis>;
    assignEditorialReview(contentId: string, versionId: string, reviewerId: string, options?: { )
        review_type?: 'quick_review' | 'comprehensive_review' | 'specialist_review';
        target_completion?: string;
        special_instructions?: string }): Promise<QualityAssessmentWorkflow>;
    ...
    resolveQualityIssue(contentId: string, versionId: string, flagId: string, resolution: { )
        ...
        resolved_by: string }): Promise<void>;
    getQualityBenchmarks(category?: string, contentType?: string, timeRange?: 'week' | 'month' | 'quarter' | 'year'): Promise<{ overall_average: number;
        grade_distribution: Record<string, number>;
        common_issues: Array<{
            issue_type: string;
            frequency: number;
            avg_severity: number }>;
        improvement_trends: Array<{ metric: string;
            trend: 'improving' | 'stable' | 'declining';
            change_percentage: number }>;
    }>;
    getQualityDashboard(userId?: string): Promise<{ personal_stats?: {
            content_count: number;
            avg_quality_score: number;
            improvement_over_time: number;
            recent_reviews: EditorialReview[] };
        community_stats: { total_content_assessed: number;
        ...
        review_queue: { pending_reviews: number;
    ...
    generateImprovementPlan(contentId: string, versionId: string, targetGrade: 'A+' | 'A' | 'B+' | 'B'): Promise<{ current_score: number;
        target_score: number;
        improvement_needed: number;
        estimated_effort_hours: number;
        prioritized_recommendations: QualityRecommendation[];
        success_probability: number }>;
    applyAutomatedFixes(contentId: string, versionId: string, fixTypes: Array<'grammar' | 'formatting' | 'seo' | 'accessibility'>, confidence_threshold?: number): Promise<{ fixes_applied: number;
    ...
    getQualityGuidelines(contentType?: string, difficultyLevel?: string): Promise<{ general_guidelines: string[];
        ...
        common_mistakes: Array<{ mistake: string;
            impact: string;
            how_to_avoid: string }>;
    }>;

export declare const QUALITY_ASSESSMENT_CONFIG: { readonly GRADE_THRESHOLDS: {
        readonly 'A+': 95;
        readonly A: 90;
        readonly 'B+': 85;
        readonly B: 80;
        ...
        readonly D: 60;
        readonly F: 0 };
    readonly PUBLICATION_THRESHOLDS: { readonly community: 70;
        readonly featured: 85;
        readonly official: 90 };
    readonly AUTOMATED_CHECKS: { readonly grammar: {
            readonly weight: 0.15;
            readonly threshold: 80 };
        readonly readability: { readonly weight: 0.2;
            readonly threshold: 70 };
        readonly structure: { readonly weight: 0.15;
            readonly threshold: 75 };
        readonly seo: { readonly weight: 0.1;
            readonly threshold: 70 };
        readonly accessibility: { readonly weight: 0.15;
            readonly threshold: 80 };
        readonly plagiarism: { readonly weight: 0.25;
            readonly threshold: 95 };
    };
    readonly REVIEW_TIMEOUTS: { readonly quick_review: 24;
