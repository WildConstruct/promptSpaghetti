...
export interface HealthCheckDefinition { id: string;
  name: string;
  description: string;
  version: string;
...
export enum HealthCheckCategory { SYSTEM = 'system',
  DATABASE = 'database',
  NETWORK = 'network',
  STORAGE = 'storage',
  MEMORY = 'memory',
  ...
  export interface HealthCheckConfig { type: HealthCheckType;
...
export enum HealthCheckType { HTTP_ENDPOINT = 'http_endpoint'
  ...
  // ==========================================
  export interface EndpointConfig { url: string;
...
export interface QueryConfig { connectionString?: string;
  database: string;
  query: string;
...
export interface ScriptConfig { language: 'javascript' | 'shell' | 'python' | 'powershell' }
  script: string;
...
export interface CompositeConfig { checks: string; // IDs of child checks }
  logic: CompositeLogic;
  aggregation: AggregationStrategy;


export enum CompositeLogic { ALL_PASS = 'all_pass'
  ANY_PASS = 'any_pass'
...
export interface ParameterValidation { minLength?: number;
  maxLength?: number;
  pattern?: string;
  enumValues?: string;
  minValue?: number;
...
export interface ValidationRules { input: InputValidation;
...
export interface InputValidation { required: string;
...
export interface OutputValidation { expectedFormat: 'json' | 'text' | 'xml' | 'binary';
...
export interface RuntimeValidation { maxExecutionTime: number;
  memoryLimit?: number;
  cpuLimit?: number;
...
export interface SecurityValidation { allowedOrigins?: string;
  requiresAuthentication: boolean;
  requiredPermissions: string;
  sensitiveDataHandling: SensitiveDataPolicy;
  auditLevel: AuditLevel }

export enum SensitiveDataPolicy { NONE = 'none'
  ...
  DETAILED = 'detailed' }
  FULL = 'full'
  export interface ExecutionConfig { schedule?: ScheduleConfig;
...
export interface ScheduleConfig { enabled: boolean;
...
export interface TriggerConfig { type: TriggerType;
  condition: string; // Expression or condition }
  parameters?: Record<string, unknown>;


export enum TriggerType { ON_DEMAND = 'on_demand',
  SCHEDULED = 'scheduled',
  ...
  export interface ExecutionEnvironment { runtime: RuntimeEnvironment;
  resources: ResourceLimits;
  network: NetworkConfig;
  storage: StorageConfig }

export enum RuntimeEnvironment { LOCAL = 'local',
  CONTAINER = 'container',
...
export interface AlertThresholds { responseTime: ThresholdConfig;
...
export interface ThresholdConfig { warning: number;
  critical: number;
  unit: string;
...
export interface NotificationConfig { channel: NotificationChannel;
  recipients: string;
  template: string;
  conditions: NotificationCondition;
  rateLimit?: RateLimitConfig }

export enum NotificationChannel { EMAIL = 'email',
  SMS = 'sms',
  SLACK = 'slack',
  WEBHOOK = 'webhook',
...
export interface SuppressionConfig { enabled: boolean;
  rules: SuppressionRule;
  maintenanceWindows: MaintenanceWindow }



export interface SuppressionRule { condition: string; // Expression;
...
export interface HealthCheckResult { checkId: string;
  executionId: string;
  timestamp: Date;
  duration: number;
  status: HealthStatus;
...
export enum HealthStatus { HEALTHY = 'healthy',
  DEGRADED = 'degraded',
  UNHEALTHY = 'unhealthy',
  CRITICAL = 'critical',
  UNKNOWN = 'unknown',
  TIMEOUT = 'timeout' }
  ERROR = 'error'
  export interface ResultDetails { summary: string;
  findings: Finding;
...
export interface Finding { type: 'info' | 'warning' | 'error' | 'critical' }
  category: string;
  description: string;
  impact: string;
  remediation?: string;
...
export interface ResultMetrics { responseTime?: number;
  throughput?: number;
  errorCount?: number;
  successRate?: number;
  availability?: number;
...
export class HealthCheckDefinitionBuilder {
  ...
  constructor(id: string, name: string) { this.definition = {
  id
  name
  version: '1.0.0'
  tags: []
  ...
  description(desc: string): this { this.definition.description = desc;
    return this;
  ...
  httpEndpoint(config: EndpointConfig): this {
    ...
    return this;
  databaseQuery(config: QueryConfig): this { this.definition.config = {
    ...
    return this;
  schedule(cronExpression: string, timezone = 'UTC'): this { if (!this.definition.execution) {
  ...
  timeout: 5000 }
    ...
    this.definition.execution!.schedule = { enabled: true
  cronExpression
  timezone
  maxConcurrentExecutions: 1 }
};
    return this;
  alerting(config: Partial<AlertingConfig>): this { this.definition.alerting = {
      enabled: true
      thresholds: { }
...
export interface SuccessCondition { field: string;
...
export enum ComparisonOperator { EQUALS = 'equals'
  ...
  IN = 'in' }
  NOT_IN = 'not_in'
  export interface ResponseValidation { contentType?: string;
  bodyContains?: string;
...
export interface JsonPathValidation { path: string;
...
export interface QueryExpectation { minRows?: number;
  maxRows?: number;
  exactRows?: number;
  columns?: string;
...
export interface QueryConstraint { column: string;
  operator: ComparisonOperator;
  value: unknown }



export interface AuthenticationConfig { type: 'basic' | 'bearer' | 'api_key' | 'oauth2' | 'custom' }
...
export interface RetryCondition { statusCode?: number;
  errorType?: string;
...
export interface NetworkConfig { allowedHosts?: string;
  blockedHosts?: string;
  proxy?: ProxyConfig;
...
export interface ProxyConfig { host: string;
  port: number;
  username?: string;
  password?: string }
...
export interface StorageConfig { tempDirectory?: string;
...
export interface IsolationConfig { sandboxed: boolean;
...
export interface ResourceQuota { maxFileHandles: number;
  maxProcesses: number;
  maxThreads: number;
...
export interface CleanupConfig { enabled: boolean;
...
export interface CleanupAction { type: 'delete_files' | 'close_connections' | 'kill_processes' | 'custom';
  target?: string;
  customScript?: string }



export interface RateLimitConfig { maxNotifications: number;
...
export class HealthCheckDefinitionValidator { static validate(definition: HealthCheckDefinition): ValidationResult { }
  const errors: string = [];
  const warnings: string = [];
  // Basic validation
  if (!definition.id || definition.id.length < 3) { errors.push('Health check ID must be at least 3 characters long');
  if (!definition.name || definition.name.length < 5) {
  errors.push('Health check name must be at least 5 characters long');
  // Configuration validation
  if (!definition.config) {
  errors.push('Health check configuration is required') } else { if (definition.config.timeout < 1000) {
  ...
  private static isValidCronExpression(expression: string): boolean { // Basic cron expression validation (simplified)
