...
export class SecureCodeGenerator extends EventEmitter {
     ...
     */
    async createVerificationCode(code, userId, purpose, options = {}) {
        const { expirationMinutes = 15, maxAttempts = 5, metadata = {} } = options;
        // Generate unique ID
     ...
     */
    async validateCode(inputCode, storedCode, options = {}) {
        const opts = {
        ...
        try {
            ...
            const isValid = await this.constantTimeValidation(inputCode, storedCode.codeHash, Buffer.from(storedCode.salt, 'base64'));
            ...
            const result = this.createValidationResult(isValid, updatedCode, isValid ? undefined : 'invalid');
     ...
     */
    validateCodeFormat(code, expectedFormat = 'numeric') {
        ...
        const alphabet = this.getAlphabet({ format: expectedFormat, excludeAmbiguous: true });
    ...
    validateGenerationOptions(options) {
        if (options.length < this.config.MIN_LENGTH || options.length > this.config.MAX_LENGTH) {
            throw new Error(`Code length must be between ${this.config.MIN_LENGTH} and ${this.config.MAX_LENGTH}`);
        }
    ...
    generateSecureRandomCode(alphabet, length) {
        const code = [];
    ...
    hashCode(code, salt) {
        return crypto.pbkdf2Sync(code, salt, this.config.HASH_ITERATIONS, 32, // 256-bit output
        this.config.HASH_ALGORITHM).toString('base64');
    }
    async constantTimeValidation(inputCode, storedHash, salt) {
        // Hash the input code with the same parameters
        const inputHash = this.hashCode(inputCode, salt);
        // Constant-time comparison to prevent timing attacks
        return crypto.timingSafeEqual(Buffer.from(storedHash, 'base64'), Buffer.from(inputHash, 'base64'));
    }
    createValidationResult(valid, code, reason) {
        const result = {
            valid,
            code,
...
export class VerificationCodeFactory {
    ...
    async createEmailVerificationCode(userId, email) {
        ...
        const data = await this.generator.createVerificationCode(code, userId, 'email_verification', {
     ...
     */
    async createSMSVerificationCode(userId, phoneNumber) {
        const code = this.generator.generateCode({
            length: 6,
            format: 'numeric'
        });
        const data = await this.generator.createVerificationCode(code, userId, 'sms_verification', {
     ...
     */
    async createPasswordResetCode(userId, email) {
        const code = this.generator.generateCode({
            length: 8,
            format: 'alphanumeric'
        });
        const data = await this.generator.createVerificationCode(code, userId, 'password_reset', {
     ...
     */
    async createTOTPBackupCodes(userId) {
        const rawCodes = this.generator.generateRecoveryCodes(10);
        const data = [];
        for (const code of rawCodes) {
            const codeData = await this.generator.createVerificationCode(code, userId, 'totp_backup', {
     ...
     */
    async validateVerificationCode(inputCode, storedCode) {
