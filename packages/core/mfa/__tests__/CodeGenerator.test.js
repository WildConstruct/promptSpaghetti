...
import { SecureCodeGenerator, VerificationCodeFactory, CodeUtils } from '../CodeGenerator';
describe('SecureCodeGenerator', () => {
    ...
    describe('Verification Code Creation', () => {
        test('should create verification code with all required properties', async () => {
            ...
            const verificationCode = await generator.createVerificationCode(code, userId, purpose);
        ...
        test('should respect custom expiration and max attempts', async () => {
            const verificationCode = await generator.createVerificationCode('123456', 'user123', 'test', {
    ...
    describe('Code Validation', () => {
        ...
        beforeEach(async () => {
            verificationCode = await generator.createVerificationCode(testCode, 'user123', 'test');
        });
    ...
    describe('Code Format Validation', () => {
        ...
        test('should handle invalid input', () => {
            ...
            expect(generator.validateCodeFormat(null, 'numeric')).toEqual({
                valid: false,
                reason: 'Code must be a non-empty string'
            });
        });
...
describe('VerificationCodeFactory', () => {
    ...
    describe('Email Verification Codes', () => {
        test('should create email verification code', async () => {
            const { code, data } = await factory.createEmailVerificationCode('user123', 'test@example.com');
            expect(code).toMatch(/^\d{6}$/);
    ...
    describe('SMS Verification Codes', () => {
        test('should create SMS verification code', async () => {
            const { code, data } = await factory.createSMSVerificationCode('user123', '+1234567890');
            expect(code).toMatch(/^\d{6}$/);
    ...
    describe('Password Reset Codes', () => {
        test('should create password reset code', async () => {
            const { code, data } = await factory.createPasswordResetCode('user123', 'test@example.com');
            expect(code).toMatch(/^[ABCDEFGHJKMNPQRSTUVWXYZ23456789]{8}$/);
