...
export class StatisticalEngine {
    ...
    analyzeExperimentResults(variants, metrics, controlVariantId) {
        ...
        const primaryResults = this.calculatePrimaryMetricResults(variants, controlVariant, primaryMetric);
        // Calculate guardrail metrics
        const guardrailResults = this.calculateGuardrailResults(variants, controlVariant, metrics.filter(m => m.isGuardrail));
        // Generate insights
     ...
     */
    calculateSampleSize(baselineRate, minimumDetectableEffect, power = 0.8, alpha = 0.05, twoTailed = true) {
        // For proportion tests (conversion rates)
        if (baselineRate > 0 && baselineRate < 1) {
            return this.calculateProportionSampleSize(baselineRate, minimumDetectableEffect, power, alpha, twoTailed);
        }
        // For continuous metrics (latency, cost)
        return this.calculateContinuousSampleSize(baselineRate, // mean
        minimumDetectableEffect, power, alpha, twoTailed);
    }
    /**
     * Perform t-test for continuous metrics
     */
    tTest(controlValues, treatmentValues, twoTailed = true) {
        ...
        const confidenceInterval = [effect - margin, effect + margin];
        return {
            pValue,
            testStatistic,
     ...
     */
    chiSquareTest(controlSuccesses, controlTotal, treatmentSuccesses, treatmentTotal) {
        ...
        const chiSquare = Math.pow(controlSuccesses - expectedControl, 2) / expectedControl +
        ...
        const se = Math.sqrt((controlRate * (1 - controlRate)) / controlTotal +
     ...
     */
    bayesianAnalysis(controlSuccesses, controlTotal, treatmentSuccesses, treatmentTotal, priorAlpha = 1, priorBeta = 1) {
        // Beta-Binomial model
        ...
        const credibleInterval = this.betaCredibleInterval(treatmentPosteriorAlpha, treatmentPosteriorBeta, this.confidenceLevel);
        return {
     ...
     */
    detectWinner(variants, controlVariantId, metric) {
        const controlVariant = variants.find(v => v.variantId === controlVariantId);
        if (!controlVariant) {
        ...
        for (const variant of variants) {
            if (variant.variantId === controlVariantId)
                continue;
            ...
            if (metric.type === 'conversion') {
                ...
                result = this.chiSquareTest(controlSuccesses, controlVariant.sampleSize, treatmentSuccesses, variant.sampleSize);
            }
            else {
    ...
    calculatePrimaryMetricResults(variants, controlVariant, metric) {
    ...
    calculateGuardrailResults(variants, controlVariant, guardrailMetrics) {
        const results = [];
    ...
    generateInsights(variants, controlVariant, metrics) {
        const insights = [];
        // Check for winner
        const primaryMetric = metrics.find(m => m.isPrimary);
        if (primaryMetric) {
    ...
    calculateProportionSampleSize(baselineRate, minimumDetectableEffect, power, alpha, twoTailed) {
    ...
    calculateContinuousSampleSize(baselineMean, minimumDetectableEffect, power, alpha, twoTailed) {
    ...
    calculateTTestPValue(tStat, df, twoTailed) {
    ...
    betaCredibleInterval(alpha, beta, probability) {
