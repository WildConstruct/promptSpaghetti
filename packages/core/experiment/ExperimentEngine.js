...
export class ExperimentEngine {
    ...
    storage;
    metrics;
    constructor(config, allocationConfig, storage, metrics) {
        this.config = config;
    ...
    async updateExperiment(id, updates) {
        ...
        if (experiment.status === 'running' && this.hasSignificantChanges(updates)) {
            throw new ExperimentError('Cannot modify running experiment in ways that would invalidate results', 'INVALID_UPDATE', id);
        }
    ...
    async startExperiment(id) {
        ...
        if (experiment.status !== 'draft') {
            throw new ExperimentError(`Cannot start experiment in status: ${experiment.status}`, 'INVALID_STATUS', id);
        }
    ...
    async stopExperiment(id, reason) {
        ...
        if (experiment.status !== 'running' && experiment.status !== 'paused') {
            throw new ExperimentError(`Cannot stop experiment in status: ${experiment.status}`, 'INVALID_STATUS', id);
        }
        const updatedExperiment = await this.updateExperiment(id, {
            status: 'completed',
            schedule: {
    ...
    async assignUser(request) {
        try {
            ...
            const existingAssignment = await this.storage.getUserAssignment(request.userId, request.experimentId);
    ...
    validateExperiment(experiment) {
        ...
        if (experiment.variants.length > this.config.maxVariants) {
            throw new ExperimentError(`Maximum ${this.config.maxVariants} variants allowed`, 'VALIDATION_ERROR');
        }
        ...
        if (experiment.trafficAllocation) {
            ...
            for (const variant of experiment.variants) {
                if (!(variant.id in experiment.trafficAllocation)) {
                    throw new ExperimentError(`Variant ${variant.id} missing traffic allocation`, 'VALIDATION_ERROR');
                }
            }
        }
        // Validate metrics
 ...
 */
export function createExperimentEngine(storage, metrics, config, allocationConfig) {
    const defaultConfig = {
        maxVariants: 12,
