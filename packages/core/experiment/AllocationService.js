...
export class AllocationService {
    ...
    storage;
    metrics;
    constructor(config, cache, storage, metrics) {
        this.config = config;
        this.cache = cache;
    ...
    async assignUser(request) {
        ...
        try {
            ...
            throw new AllocationError(`Assignment failed: ${error.message}`, 'ASSIGNMENT_FAILED', request.userId, request.experimentId);
        }
    }
    /**
     * Get assignments for multiple experiments
     */
    async bulkAssignUser(userId, experimentIds, sessionId, debugMode = false) {
        const results = {};
        // Process assignments in parallel for better performance
        const assignments = await Promise.allSettled(experimentIds.map(experimentId => this.assignUser({
            userId,
            experimentId,
            sessionId,
            debugMode
     ...
     */
    async forceAssignUser(userId, experimentId, variantId, reason, sessionId) {
     ...
     */
    async removeUserAssignment(userId, experimentId) {
    ...
    async rotateSalt() {
        ...
        this.config.saltStorage.previousSalts = this.config.saltStorage.previousSalts.filter(entry => entry.rotatedAt > threeMonthsAgo);
    ...
    async handleOverride(request, experiment) {
        const variant = experiment.variants.find(v => v.id === request.overrideVariant);
        if (!variant) {
            throw new AllocationError('Override variant not found', 'VARIANT_NOT_FOUND', request.userId, request.experimentId);
        }
        const assignment = {
            userId: request.userId,
            experimentId: experiment.id,
    ...
    createSuccessResponse(assignment, variant, reason, debugMode = false, experiment) {
        return {
    ...
    async updateCache(cacheKey, assignment) {
        try {
            await this.cache.set(cacheKey, JSON.stringify(assignment), this.config.cacheTtl);
        }
        catch (error) {
 ...
 */
export function createAllocationService(config, storage, metrics, cache) {
    ...
    return new AllocationService(config, cache || defaultCache, storage, metrics);
}
