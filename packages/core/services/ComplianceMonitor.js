...
export class ComplianceMonitor {
     ...
     */
    async sendCriticalComplianceAlert(violation, result) {
     ...
     */
    async checkDataEncryption(context) {
        const evidence = [];
        let score = 100;
        const issues = [];
     ...
     */
    async checkDataRetention(context) {
        let score = 100;
        const issues = [];
        const remediation = [];
...
export class EnhancedComplianceMonitor extends ComplianceMonitor {
    ...
    async generateEnhancedDashboard() {
        ...
        try {
            ...
            const trendReports = await Promise.all(frameworks.map(framework => this.historicalAnalyzer.generateTrendReport(framework, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // Last 7 days
            new Date()).catch(() => null)));
    ...
    async recordComplianceMeasurement(framework, metricName, actualValue, context) {
        ...
        try {
            ...
            const matchingBaseline = baselines.find(b => b.framework === framework && b.name.toLowerCase().includes(metricName.toLowerCase()));
            if (matchingBaseline) {
                await this.baselineTracker.recordMeasurement(matchingBaseline.id, actualValue, context, 'Automated measurement from compliance monitor');
            }
        }
        catch (error) {
    ...
    async generateForecastAlerts() {
        ...
        try {
            ...
            const criticalBaselines = baselines.filter(b => b.framework === 'GDPR' || b.framework === 'SOC2' || b.framework === 'MPA').slice(0, 3); // Limit to prevent too many forecasts
