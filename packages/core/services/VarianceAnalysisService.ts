...
export class VarianceAnalysisService {
  ...
  */
  ...
  private calculateStructuralDiversity(outputs: string): number {,
  if (outputs.length < 2) return 0;
  ...
  private calculateExecutionPathDiversity(executionPaths: any): number {
    if (executionPaths.length < 2) return 0;
    const pathSignatures = executionPaths.map(path => {)
  if (!path || !path.steps) return '';
      return path.steps.map((step: any) => `${step.nodeType}:${step.nodeId}`).join('->');}
    });
  ...
  private calculateCreativityScore(outputs: string, executionPaths: any): number {,
  ...
  const suggestions: VarianceSuggestion = [];
  ...
  private categorizeMetric(value: number, lowThreshold: number, highThreshold: number): 'low' | 'medium' | 'high' {
    if (value < lowThreshold) return 'low';
    if (value < highThreshold) return 'medium';
    return 'high';
  private getMetricColor(value: number, lowThreshold: number, highThreshold: number): string {
    if (value < lowThreshold) return '#ef4444';
    if (value < highThreshold) return '#f59e0b';
    return '#10b981';
