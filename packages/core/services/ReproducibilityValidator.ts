...
export interface ReproducibilityValidationOptions { strictMode?: boolean;           // Require exact reproducibility;
...
export class ReproducibilityValidator { private static instance: ReproducibilityValidator;
  ...
  ): ReproducibilityValidationReport { const report: ReproducibilityValidationReport = {,
  isValid: true,
 ...
 else { report.integrity.seedsValid = true;
 ...
 else if (Object.keys(randomization.nodeSeed).length === 0) { report.warnings.push({)
 ...
 else { // Validate node seeds match graph nodes
  ...
  impact: 'reproducibility',
      ...
      if (extraSeeds.length > 0) { report.warnings.push({)
  ...
  impact: 'quality',
    ...
    // Check RNG state
    if (!randomization.rngState) { report.warnings.push({)
 ...
 else { report.exactReproducible = true;
  ...
  suggestion: 'Generate reproducibility hash for data validation' }
    ...
    if (!randomization.executionSequence) { report.warnings.push({)
  ...
  suggestion: 'Record node execution sequence for deterministic reproduction' }
    ...
    if (randomization.nodeRngStates) { this.validateNodeRngStates(randomization.nodeRngStates, exportData, report);
  ...
  severity: 'medium'
      ...
      if (!state.state || typeof state.state !== 'string') { report.errors.push({)
  ...
  severity: 'medium'
      ...
      if (typeof state.callCount !== 'number' || state.callCount < 0) { report.warnings.push({)
  ...
  severity: 'high',
      ...
      if (!node.configuration) { report.warnings.push({)
  ...
  impact: 'reproducibility',
      ...
      if (node.reproducibilityData) { this.validateNodeReproducibilityData(node, report) } else { report.warnings.push({)
  ...
  private validateNodeReproducibilityData(((
    node: any,
    report: ReproducibilityValidationReport
  ): void { const reprData = node.reproducibilityData;
  ...
  impact: 'quality';
    ...
    if (!reprData.configurationHash) { report.warnings.push({)
  code: 'MISSING_CONFIGURATION_HASH' }
        message: `Node ${node.id} missing configuration hash`}
},
  impact: 'reproducibility';
  });
 else { // Validate configuration hash if both exist
  ...
  suggestion: 'Include version information for compatibility checking' }
    ...
    if (!metadata.generator.coreVersion || !metadata.generator.exporterVersion) { report.warnings.push({)
  ...
  impact: 'reproducibility';
    ...
    if (!environment.nodeVersion || !environment.platform) { report.warnings.push({)
 ...
 else { report.integrity.checksumValid = true;
  ...
  impact: 'quality';
      ...
      if (!nodeIds.has(conn.target.nodeId)) { report.warnings.push({)
  ...
  impact: 'quality';
    ...
    if (executionPath) { executionPath.forEach(nodeId => {)
 ...
 else { // Rough estimate based on graph complexity
  ...
  priority: 'medium' }
    ...
    if (!exportData.execution.randomization.nodeRngStates) { report.suggestions.push({)
  ...
  priority: 'high' }
    ...
    if (!exportData.metadata.generator.dependencies) { report.suggestions.push({)
  ...
  priority: 'medium' }
    ...
    // Debug suggestions
    if (report.errors.length > 0) { report.suggestions.push({)
  ...
  private assessOverallValidity(((
    report: ReproducibilityValidationReport,
    options: ReproducibilityValidationOptions
  ): void { const criticalErrors = report.errors.filter(e => e.severity === 'critical').length;
    const highErrors = report.errors.filter(e => e.severity === 'high').length;
    if (criticalErrors > 0) {
      report.isValid = false } else if (options.strictMode && highErrors > 0) { report.isValid = false } else if (options.strictMode && !report.exactReproducible) { report.isValid = false } else if (!options.allowApproximate && !report.exactReproducible && !report.approximateReproducible) { report.isValid = false } else { report.isValid = true;
  ...
  differences: string;
  reproductionTime: number }> { const startTime = Date.now();
  try {
  ...
  private calculateConfigurationHash(configuration: any): string { if (!configuration || typeof configuration !== 'object') {
      return 'null-config';
