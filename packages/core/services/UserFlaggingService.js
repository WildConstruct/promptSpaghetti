...
export class UserFlaggingService {
     ...
     */
    async getFlaggingStatus(contentId, userId) {
        try {
     ...
     */
    async getUserFlagReports(userId, options = {}) {
        try {
            ...
            if (options.timeRange) {
                filteredReports = filteredReports.filter(report => report.createdAt >= options.timeRange.start &&
     ...
     */
    async getFlaggingAnalytics(timeRange) {
        ...
        try {
            const allReports = Array.from(this.flagReports.values())
                .filter(report => report.createdAt >= range.start &&
     ...
     */
    async updateFlagStatus(reportId, update) {
        try {
    ...
    async checkForDuplicates(submission) {
        ...
        const existing = Array.from(this.flagReports.values())
            .find(report => report.contentId === submission.contentId &&
    ...
    async hasUserFlagged(contentId, userId) {
        return Array.from(this.flagReports.values())
            .some(report => report.contentId === contentId &&
    ...
    getLatestModeratorNote(contentId) {
        ...
        if (reports.length === 0)
            return undefined;
        const latest = reports.reduce((latest, report) => report.reviewedAt > latest.reviewedAt ? report : latest);
    ...
    calculateUserAccuracy(userId, reports) {
        ...
        if (resolvedReports.length === 0)
            return 0;
        const accurateReports = resolvedReports.filter(r => r.resolution?.action !== 'no_action');
    ...
    calculateContentTrends(reports) {
        return {
    ...
    calculateModerationEfficiency(reports) {
        return {
    ...
    async notifyContentOwner(report) {
