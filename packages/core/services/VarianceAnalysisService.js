...
export class VarianceAnalysisService {
     ...
     */
    analyzeVariance(results: any[]): any {
        if (results.length < 2) {
     ...
     */
    createDiversityIndicators(metrics: any): any[] {
        const indicators = [];
     ...
     */
    getVarianceLevelInfo(level: string) {
        const info = {
            low: {
                color: '#ef4444',
                background: '#fef2f2',
    ...
    calculateLengthVariance(outputs: string[]) {
        if (outputs.length < 2)
            return 0;
    ...
    calculateVocabularyDiversity(outputs: string[]) {
        if (outputs.length < 2)
            return 0;
    ...
    calculateOverallVarianceScore(diversityMetrics: any, creativeRange: any) {
    ...
    generateVarianceSuggestions(diversityMetrics: any, creativeRange: any, overallVariance: string, executionPaths: any[]) {
        const suggestions = [];
