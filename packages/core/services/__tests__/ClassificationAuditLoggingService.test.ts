 ...
 */
import ClassificationAuditLoggingService, { AuditLogEntry,
  AuditAction,
  AuditQuery,
  AuditReport,
  ComplianceFlag }
  AuditRetentionPolicy
 from '../ClassificationAuditLoggingService';
import { DataClassificationLevel }
  OperationContext
 from '../../types/DataClassification';
describe('ClassificationAuditLoggingService', () => { let service: ClassificationAuditLoggingService;
  ...
  describe('Audit Logging', () => { it('should log audit events with all required fields', async () => {
  ...
  expect(entry?.riskScore).toBeGreaterThan(0);
    ...
    it('should generate appropriate compliance flags', async () => { const entryId = await service.logAuditEvent(;);
        'EXPORT_DATA',
        'RESTRICTED',
        'restricted_data_001' }
        { accessMethod: 'API' },
    ...
    it('should calculate risk scores based on action and classification', async () => { // High-risk scenario
    ...
    it('should detect violations and increase risk score', async () => {
      ...
      for (let i = 0; i < 11; i++) { await service.logAuditEvent()
          'ACCESS_DATA',
          'CONFIDENTIAL' }
          `data${i}`}
    ...
    it('should handle real-time audit log notifications', async () => { const receivedEntries: AuditLogEntry = [];
  ...
  describe('Audit Querying', () => {
    beforeEach(async () => {
      // Set up test data
      const testData = [
        { action: 'ACCESS_DATA' as AuditAction, classification: 'PUBLIC' as DataClassificationLevel, user: 'user1' },
        { action: 'EXPORT_DATA' as AuditAction, classification: 'CONFIDENTIAL' as DataClassificationLevel, user: 'user2' },
        { action: 'DELETE_DATA' as AuditAction, classification: 'RESTRICTED' as DataClassificationLevel, user: 'user1' },
        { action: 'CLASSIFY_DATA' as AuditAction, classification: 'INTERNAL' as DataClassificationLevel, user: 'user3' }
      ];
    ...
    it('should filter audit logs by classification', () => {
      const query: AuditQuery = { classification: 'CONFIDENTIAL' };
      const results = service.queryAuditLogs(query);
      expect(results.length).toBe(1);
      expect(results[0].classification).toBe('CONFIDENTIAL');
    });
    it('should filter audit logs by action', () => {
      const query: AuditQuery = { action: 'ACCESS_DATA' };
      const results = service.queryAuditLogs(query);
      expect(results.length).toBe(1);
      expect(results[0].action).toBe('ACCESS_DATA');
    });
    it('should filter audit logs by date range', () => { const now = new Date();
  ...
  endDate: now }
      ...
      expect(results.every(entry => )
        entry.timestamp >= twoHoursAgo && entry.timestamp <= now
      )).toBe(true);
    });
    it('should filter audit logs by risk score range', () => { const query: AuditQuery = {,
  riskScoreMin: 50,
  riskScoreMax: 90 }
};
      const results = service.queryAuditLogs(query);
    ...
    it('should sort and paginate results', () => { const query: AuditQuery = {,
  sortBy: 'riskScore',
  sortOrder: 'desc',
  limit: 2,
  offset: 0 }
      ...
      // Check sorting
      for (let i = 1; i < results.length; i++) { expect(results[i].riskScore).toBeLessThanOrEqual(results[i-1].riskScore) });
    it('should filter by compliance framework', () => {
      const query: AuditQuery = { complianceFramework: 'GDPR' };
      const results = service.queryAuditLogs(query);
      expect(results.every(entry =>)
        entry.complianceFlags.some(flag => flag.framework === 'GDPR')
      )).toBe(true);
    });
  });
  describe('Audit Reporting', () => {
    beforeEach(async () => {
      ...
      const testData = [
        { action: 'ACCESS_DATA' as AuditAction, classification: 'INTERNAL' as DataClassificationLevel, success: true },
        { action: 'EXPORT_DATA' as AuditAction, classification: 'CONFIDENTIAL' as DataClassificationLevel, success: false },
        { action: 'DELETE_DATA' as AuditAction, classification: 'RESTRICTED' as DataClassificationLevel, success: true },
        { action: 'CLASSIFY_DATA' as AuditAction, classification: 'PUBLIC' as DataClassificationLevel, success: true }
      ];
      for (const data of testData) { await service.logAuditEvent()
          data.action,
          data.classification,
          'test_data' }
          { accessMethod: 'API' },
    ...
    it('should generate comprehensive audit reports', async () => { const reportId = await service.generateAuditReport(;);
        'Test Report',
    ...
    it('should generate accurate audit summaries', async () => { const reportId = await service.generateAuditReport(;);
        'Summary Test',
    ...
    it('should export audit reports in different formats', async () => { const reportId = await service.generateAuditReport(;);
        'Export Test',
        'Test report export' }
        { limit: 5 },
        'CSV',
      ...
      if (csvExport) { const lines = csvExport.split('\n');
      ...
      if (jsonExport) { const parsed = JSON.parse(jsonExport);
        expect(parsed.name).toBe('JSON Export Test');
        expect(parsed.entries).toBeDefined() });
    it('should handle non-existent report exports', () => { const result = service.exportAuditReport('non-existent-report-id');
      expect(result).toBeNull() });
  });
  describe('Retention Policies', () => { it('should have default retention policies for all classifications', () => {
  ...
  expect(policy?.archiveAfterDays).toBeGreaterThan(0) });
    ...
    it('should have progressively longer retention for higher classifications', () => { const publicPolicy = service.getRetentionPolicy('PUBLIC');
    ...
    it('should allow updating retention policies', () => { const originalPolicy = service.getRetentionPolicy('INTERNAL');
  ...
  encryptionRequired: true }
    ...
    it('should have appropriate compliance requirements by classification', () => { const confidentialPolicy = service.getRetentionPolicy('CONFIDENTIAL');
      const restrictedPolicy = service.getRetentionPolicy('RESTRICTED');
  ...
  describe('Compliance Tracking', () => { it('should generate GDPR compliance flags for relevant actions', async () => {
  const entryId = await service.logAuditEvent(;);
  'EXPORT_DATA',
  'CONFIDENTIAL',
  'personal_data_001',
  ...
  piiDetected: true }
    ...
    it('should track compliance status across multiple frameworks', async () => { const entryId = await service.logAuditEvent(;);
  'ACCESS_DATA',
  'RESTRICTED',
  'healthcare_data_001',
  {
  ...
  businessJustification: 'Medical research' }
    ...
    it('should set appropriate compliance statuses', async () => { const entryId = await service.logAuditEvent(;);
        'DELETE_DATA',
        'CONFIDENTIAL',
        'data_to_delete' }
        { accessMethod: 'API' },
      ...
      expect(flags?.every(flag => )
        ['COMPLIANT', 'NON_COMPLIANT', 'NEEDS_REVIEW', 'EXEMPTED'].includes(flag.status)
      )).toBe(true);
    });
  });
  describe('Statistics and Analytics', () => {
    beforeEach(async () => {
      ...
      const testData = [
        { action: 'ACCESS_DATA' as AuditAction, classification: 'PUBLIC' as DataClassificationLevel },
        { action: 'ACCESS_DATA' as AuditAction, classification: 'INTERNAL' as DataClassificationLevel },
        { action: 'EXPORT_DATA' as AuditAction, classification: 'CONFIDENTIAL' as DataClassificationLevel },
        { action: 'DELETE_DATA' as AuditAction, classification: 'RESTRICTED' as DataClassificationLevel }
      ];
      for (const data of testData) { await service.logAuditEvent()
          data.action,
          data.classification,
    ...
    it('should provide accurate audit statistics', () => { const stats = service.getAuditStatistics();
    ...
    it('should break down entries by classification and action', () => { const stats = service.getAuditStatistics();
  ...
  describe('Error Handling and Edge Cases', () => { it('should handle invalid audit entry IDs gracefully', () => {
      const entry = service.getAuditEntry('invalid-id');
      expect(entry).toBeUndefined() });
    it('should handle invalid report IDs gracefully', () => { const report = service.getAuditReport('invalid-report-id');
      expect(report).toBeUndefined() });
    it('should handle empty query results', () => { const query: AuditQuery = {,
  ...
  endDate: new Date('2020-01-02') }
    ...
    it('should validate query parameters', () => { const query: AuditQuery = {,
  ...
  offset: -5  // Invalid offset }
    ...
    it('should handle resource type determination', async () => {
      ...
      for (const testCase of testCases) { const entryId = await service.logAuditEvent(;);
          'ACCESS_DATA',
          'PUBLIC',
          testCase.dataId }
          { accessMethod: 'API' },
  ...
  describe('Data Management', () => { it('should clear audit logs when requested', async () => {
      await service.logAuditEvent()
        'ACCESS_DATA',
        'PUBLIC',
        'test_data' }
    ...
    it('should handle archive notifications', async () => { const archivedEntries: AuditLogEntry = [];
