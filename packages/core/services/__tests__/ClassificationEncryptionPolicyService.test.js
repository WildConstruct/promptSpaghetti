 ...
 */
import ClassificationEncryptionPolicyService from '../ClassificationEncryptionPolicyService';
describe('ClassificationEncryptionPolicyService', () => {
    ...
    describe('Default Encryption Policies', () => {
        ...
        it('should have progressively stricter encryption requirements for higher classifications', () => {
            ...
            // Key rotation
            expect(publicPolicy?.requirements.keyRotationDays).toBeGreaterThan(restrictedPolicy?.requirements.keyRotationDays || 0);
            // HSM requirements
    ...
    describe('Encryption Compliance Validation', () => {
        it('should validate compliant encryption for internal data', async () => {
            const result = await service.validateEncryptionCompliance('data123', 'INTERNAL', {
                encrypted: true,
                algorithm: 'AES-256',
                keyLength: 256,
        ...
        it('should detect encryption requirement violations', async () => {
            const result = await service.validateEncryptionCompliance('data123', 'CONFIDENTIAL', {
                encrypted: false
            }, mockContext);
        ...
        it('should validate algorithm compliance', async () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors.some(error => error.includes('not approved for CONFIDENTIAL'))).toBe(true);
        });
        it('should validate key length requirements', async () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors.some(error => error.includes('Key length 128 is insufficient'))).toBe(true);
        });
        it('should validate key rotation compliance', async () => {
            ...
            expect(result.warnings.some(warning => warning.includes('Key rotation is overdue'))).toBe(true);
        });
        it('should record compliance assessments', async () => {
            await service.validateEncryptionCompliance('data123', 'INTERNAL', {
                encrypted: true,
                algorithm: 'AES-256',
                keyLength: 256,
                lastRotationDate: new Date()
        ...
        it('should handle invalid classification levels gracefully', async () => {
            const result = await service.validateEncryptionCompliance('data123', 'INVALID', {
                encrypted: true,
                algorithm: 'AES-256',
                keyLength: 256
            }, mockContext);
    ...
    describe('Audit Event Recording', () => {
        it('should record audit events for compliance checks', async () => {
            await service.validateEncryptionCompliance('data123', 'CONFIDENTIAL', {
                encrypted: true,
                algorithm: 'AES-256-GCM',
                keyLength: 256,
                lastRotationDate: new Date()
        ...
        it('should record different audit event types', async () => {
            await service.validateEncryptionCompliance('data1', 'INTERNAL', { encrypted: true,
                algorithm: 'AES-256',
                keyLength: 256 }, mockContext);
        ...
        it('should filter audit events by classification and type', async () => {
            await service.validateEncryptionCompliance('data1', 'INTERNAL', { encrypted: true,
                algorithm: 'AES-256',
                keyLength: 256 }, mockContext);
            await service.validateEncryptionCompliance('data2', 'CONFIDENTIAL', { encrypted: true,
                algorithm: 'AES-256-GCM',
                keyLength: 256 }, mockContext);
    ...
    describe('Policy Management', () => {
        ...
        it('should throw error when updating non-existent policy', async () => {
            await expect(service.updateEncryptionPolicy('INVALID', {})).rejects.toThrow('No encryption policy found for classification: INVALID');
        });
    });
    ...
    describe('Compliance Scoring', () => {
        ...
        it('should calculate individual compliance scores', async () => {
            await service.validateEncryptionCompliance('data1', 'CONFIDENTIAL', { encrypted: true,
                algorithm: 'AES-256-GCM',
                keyLength: 256 }, mockContext);
    ...
    describe('Encryption Recommendations', () => {
        ...
        it('should handle invalid classification levels gracefully', () => {
            const recs = service.getEncryptionRecommendations('INVALID');
            expect(recs).toEqual([]);
        });
    });
    describe('Algorithm Compliance Logic', () => {
        ...
        it('should enforce classification-based algorithm restrictions', () => {
            ...
            const restrictedOnlyAlgorithms = restrictedAlgorithms.filter(alg => alg.minClassification === 'RESTRICTED');
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should handle unknown algorithms gracefully', async () => {
            ...
            expect(result.valid).toBe(false);
            expect(result.errors.some(error => error.includes('not in the approved algorithms list'))).toBe(true);
        });
