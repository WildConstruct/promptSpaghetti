...
export class ResultExportService { /**
  ...
  */
  ...
  async exportBatchResults(results: PreviewResultWithPath
    ...
    options: ResultExportOptions
    sourceGraph?: any
  ): Promise<ExportResult> { const selectedResults = selectedIndices.map(index => results[index]).filter(Boolean);
  ...
  options: ResultExportOptions
    sourceGraph?: any
  ): Promise<ExportResult> { const comparisonData = {
  results
  ...
  validateExportOptions(format: ExportFormat, options: ResultExportOptions): string {
    ...
    if (['fountain-script', 'final-draft'].includes(format)) { if (!options.filmOptions?.includeDirectorNotes && !options.includeMetadata) {
  ...
  */
  ...
  case 'json-complete':,
  multiplier = 3.0;
  if (options.includeExecutionPaths) multiplier += 2.0;
  if (options.includeDebugInfo) multiplier += 1.5;
  break;
  case 'csv-analysis':,
  multiplier = 1.2;
  break;
  ...
  warning: estimatedKB > 10240 ? 'Large export size - may take time to generate' : undefined }
    ...
    return { estimatedSize: estimatedKB,
  unit: 'KB',
  ...
  options: ResultExportOptions,
    filename: string,
    exportType: 'individual' | 'batch' | 'comparison'): Promise<ExportResult> { // Transform data to match existing export system format
  ...
  filename
    ...
    try { return await exportResults(exportRequest) } catch (error) { // Fallback to custom export handling
  ...
  options: ResultExportOptions
    exportType: string): any { const baseData = {
  exportType
  results: 'results' in data ? data.results : [data.result]
  exportedAt: data.exportedAt
  sourceGraph: data.sourceGraph }
};
    if (options.includeMetadata) { baseData.metadata = this.extractMetadata(data);
  ...
  options: ResultExportOptions
    filename: string
    exportType: string): Promise<ExportResult> { switch (options.format) {
  case 'plain-text':
  ...
  private exportExecutionTimeline(data: any, options: ResultExportOptions): ExportResult { const results = 'results' in data ? data.results : [data.result];
  const timeline = {
  ...
  private exportVarianceReport(data: any, options: ResultExportOptions): ExportResult { const results = 'results' in data ? data.results : [data.result];
  ...
  private exportBatchSummary(data: any, options: ResultExportOptions): ExportResult { const summary = {
  ...
  private getFileExtension(format: ExportFormat): string { const extensions: Record<ExportFormat, string> = {
  'plain-text': 'txt'
  'json-simple': 'json'
  'json-complete': 'json'
  'csv-analysis': 'csv'
  ...
  private calculateAggregateStats(results: PreviewResultWithPath) { const executionTimes = results;
  ...
  private calculateVarianceScore(results: PreviewResultWithPath): number { if (results.length < 2) return 0;
  ...
  const wordCounts = allWords.reduce((counts, word) => { counts[word] = (counts[word] || 0) + 1;
  return counts }, {} as Record<string, number>);
    return Object.entries(wordCounts)
      .filter(([word, count]) => count > results.length / 2)
      .map(([word]) => word)
      .slice(0, 10);
  private generateComparisonAnalysis(results: PreviewResultWithPath) { return {
  totalResults: results.length
  ...
  private getExecutionTimeSpread(results: PreviewResultWithPath) { const times = results;
  .map(r => r.executionTimeMs)
  .filter((t): t is number => typeof t === 'number')
  .sort((a, b) => a - b);
  ...
  private analyzeRandomization(results: PreviewResultWithPath) { const randomizationCounts = results.map(r => ;);
  ...
  private extractMetadata(data: any) { return {
  exportedAt: data.exportedAt
  ...
  private extractExecutionPaths(results: PreviewResultWithPath) { return results
  ...
  private extractDebugInfo(results: PreviewResultWithPath) { return results
  .filter(r => r.debugInfo)
  .map(r => ({)
  seed: r.seed
  debugInfo: r.debugInfo }
}));
  private transformVFXData(results: PreviewResultWithPath, vfxOptions: any) { return {
  ...
  private generateAnalysis(results: PreviewResultWithPath, analysisOptions: any) {
    const analysis: any = {};
    if (analysisOptions.varianceAnalysis) { analysis.variance = {
  score: this.calculateVarianceScore(results)
  distribution: this.analyzeVarianceDistribution(results) }
};
    if (analysisOptions.performanceBreakdown) { analysis.performance = {
  ...
  timeSpread: this.getExecutionTimeSpread(results) }
    ...
    if (analysisOptions.creativityMetrics) { analysis.creativity = {
  uniqueOutputs: new Set(results.map(r => r.output)).size
  averageLength: Math.round(results.reduce((sum, r) => sum + (r.output?.length || 0), 0) / results.length)
  vocabularyDiversity: this.calculateVocabularyDiversity(results) }
};
    return analysis;
  private analyzeVarianceDistribution(results: PreviewResultWithPath) { const lengths = results.map(r => r.output?.length || 0);
  ...
  private calculateVocabularyDiversity(results: PreviewResultWithPath): number { const allWords = results;
  .map(r => r.output || '')
  .join(' ')
  .toLowerCase()
  .split(/\s+/)
 ...
 : null
  ...
  private generateVarianceAnalysis(results: PreviewResultWithPath) { return {
  ...
  private findUniqueElements(results: PreviewResultWithPath): string { const allWords = results;
      .map(r => r.output || '')
      .flatMap(output => output.toLowerCase().split(/\s+/))
      .filter(word => word.length > 3);
    ...
    return Object.entries(wordCounts)
      ...
      .map(([word]) => word)
      .slice(0, 20);
  private generateVarianceRecommendations(results: PreviewResultWithPath): string { const recommendations: string = [];
  ...
  recommendations.push('Try using different weight distributions in WeightedChoice nodes') } else if (varianceScore > 80) { recommendations.push('High variance detected - consider adding constraints for consistency');
      recommendations.push('Review randomization logic to ensure coherent outputs') } else { recommendations.push('Good balance of variety and consistency achieved');
