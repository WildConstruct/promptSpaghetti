...
export class HybridPromptExportService {
    ...
    async exportHybridPrompt(graph: any, executionResults: any, options: any = {
        ...
        const hybridExtensions = await this.buildHybridExtensions(graph, executionResults, options);
    ...
    async buildHybridExtensions(graph: any, executionResults: any, options: any): Promise<any> {
        const extensions = {
            mars: {
                framework: 'MARS-v1.0',
                tags: await this.marsExtractor.extractMARSTags(executionResults.finalPrompt, executionResults.variables),
                structured: await this.marsExtractor.createStructuredPrompt(executionResults.finalPrompt)
            },
            zada: {
                approach: 'screenplay-style',
                variants: await this.zadaGenerator.generateNaturalLanguageVariants(executionResults.finalPrompt, executionResults.variables, options.targetAudience),
                director_friendly: await this.zadaGenerator.createDirectorAccessiblePrompt(executionResults.finalPrompt, executionResults.variables)
            },
            hollywood: {
                protocol: 'reproducibility-v1',
                seeds: this.seedManager.generateHollywoodSeeds(graph),
                iteration_tracking: this.seedManager.createIterationHistory(executionResults)
            }
        };
        return extensions;
    }
...
class MARSFrameworkExtractor {
    async extractMARSTags(prompt, variables) {
    ...
    async createStructuredPrompt(prompt) {
    ...
    filterCameraVariables(variables) {
        return Object.fromEntries(Object.entries(variables).filter(([key]) => key.toLowerCase().includes('camera') ||
    ...
    inferCameraAngle(prompt, variables) {
        const angleKeywords = {
    ...
    inferCameraMovement(prompt, variables) {
    ...
    inferDepthOfField(prompt, variables) {
        const dofKeywords = {
    ...
    extractSecondarySubjects(prompt) {
    ...
    extractPhysicalBlocking(prompt, variables) {
        ...
        const foundPositions = positionWords.filter(position => prompt.toLowerCase().includes(position));
        return foundPositions.join(', ') || 'center frame';
    }
    inferLightingStyle(prompt, variables) {
    ...
    inferColorGrading(prompt, variables) {
        const colorKeywords = {
    ...
    inferAtmosphericConditions(prompt, variables) {
    ...
    inferBackgroundTreatment(prompt, variables) {
    ...
    determineVisualHierarchy(prompt, variables) {
...
class ZadaNaturalLanguageGenerator {
    async generateNaturalLanguageVariants(prompt, variables, targetAudience) {
        const variants = [];
    ...
    async createDirectorAccessiblePrompt(prompt, variables) {
        return {
...
${this.generateTechnicalConsiderations(prompt, variables)}`;
    ...
    extractMainAction(prompt) {
        ...
        const foundAction = actionWords.find(action => prompt.toLowerCase().includes(action));
    ...
    extractSetting(prompt, variables) {
        ...
        const locationVars = Object.entries(variables).filter(([key]) => key.toLowerCase().includes('location') ||
    ...
    inferCameraMovementNaturally(prompt) {
        const movements = {
    ...
    describeComposition(prompt, variables) {
    ...
    describeSubjectNaturally(prompt, variables) {
        const subject = this.extractPrimarySubject(prompt);
    ...
    generateCinematographerNotes(prompt, variables) {
    ...
    generateLightingDirectorNotes(prompt, variables) {
    ...
    generateVFXSupervisorNotes(prompt, variables) {
    ...
    extractCreativeIntent(prompt, variables) {
    ...
    generatePerformanceNotes(prompt, variables) {
        const emotion = this.extractEmotionalStateNaturally(prompt);
        return `Performance should convey ${emotion}. ${this.extractActionDirection(prompt)}`;
    }
    generateTechnicalConsiderations(prompt, variables) {
    ...
    determineVisualHierarchyNaturally(prompt) {
    ...
    identifyPrimaryFocusNaturally(prompt) {
        return this.extractPrimarySubject(prompt);
    }
    extractEmotionalStateNaturally(prompt) {
    ...
    extractPhysicalBlockingNaturally(prompt) {
    ...
    inferLightingStyleNaturally(prompt) {
        const styles = {
    ...
    inferCameraAngleNaturally(prompt) {
        const angles = {
    ...
    inferLensChoiceNaturally(prompt) {
    ...
    extractKeyVisualElements(prompt, variables) {
        const elements = [];
    ...
    inferCinematicReferences(prompt) {
    ...
    inferColorPalette(prompt, variables) {
        if (prompt.toLowerCase().includes('warm'))
    ...
    extractActionDirection(prompt) {
        const action = this.extractMainAction(prompt);
        return `Focus on natural ${action} with authentic emotion.`;
    }
    extractTechnicalRequirements(prompt, variables) {
        const requirements = [];
...
class HollywoodSeedManager {
    generateHollywoodSeeds(graph) {
        ...
        const iterationSeeds = Array.from({ length: 5 }, (_, i) => this.hashSeed(`${masterSeed}_iteration_${i}`));
        return {
    ...
    createIterationHistory(executionResults) {
        return [{
    ...
    generateChecksum(masterSeed, componentSeeds, iterationSeeds) {
...
export { MARSFrameworkExtractor, ZadaNaturalLanguageGenerator, HollywoodSeedManager };
