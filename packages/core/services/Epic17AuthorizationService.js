...
export class Epic17AuthorizationService extends EventEmitter {
    ...
    async evaluateAuthorization(context, evaluationId) {
    ...
    isPolicyApplicable(policy, context) {
        ...
        if (context.resource && target.resources.length > 0) {
            const resourceMatch = target.resources.some(selector => this.matchesResourceSelector(context.resource, selector));
            if (!resourceMatch)
                return false;
        }
    ...
    async evaluatePolicyConditions(policy, context) {
        const results = [];
    ...
    generateDecisionReason(decision, appliedPolicies, conditions) {
        if (decision === 'allow') {
    ...
    async auditAuthorizationDecision(context, result) {
