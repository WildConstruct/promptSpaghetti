...
export interface HybridExportFormat extends VFXExportFormat { // Hybrid prompting extensions
  hybridPrompting: {,
  mars: {,
  framework: 'MARS-v1.0' }
...
export interface MARSFrameworkTags { // Camera and composition tags
  CAM: {;
...
export interface MARSStructuredPrompt { raw_mars: string; // Original MARS-tagged prompt;
  parsed_structure: { }
...
export interface DirectorAccessiblePrompt { screenplay_style: string; // "FADE IN: CLOSE-UP on character's face...";
...
export interface HollywoodSeedProtocol { master_seed: number; // Primary reproducibility seed;
...
export class HybridPromptExportService {
 ...
 = { includeMARS: true
    ...
    const hybridExport: HybridExportFormat = { ...baseExport
...
class MARSFrameworkExtractor {};
    return tags;
  async createStructuredPrompt(prompt: string): Promise<MARSStructuredPrompt> { // Parse prompt into MARS structured format
  ...
  composition_rules: this.mapToControlNetComposition(prompt) }
    ...
    return structured;
  private extractCameraTags(prompt: string, variables: Record<string, string>): MARSFrameworkTags['CAM'] { // Intelligent extraction of camera parameters from prompt/variables
  ...
  private extractSubjectTags(prompt: string, variables: Record<string, string>): MARSFrameworkTags['SUBJ'] { return {
  ...
  private extractEffectsTags(prompt: string, variables: Record<string, string>): MARSFrameworkTags['FX'] { return {
  ...
  private extractFocalTags(prompt: string, variables: Record<string, string>): MARSFrameworkTags['FOCAL'] { return {
  ...
  private filterCameraVariables(variables: Record<string, string>): Record<string, string> { return Object.fromEntries()
  ...
  'POV': ['point of view', 'first person', 'through eyes'] }
    ...
    for (const [shotType, keywords] of Object.entries(shotKeywords)) { if (keywords.some(keyword => )
  ...
  'eye': ['eye level', 'straight on', 'neutral'] }
    ...
    for (const [angle, keywords] of Object.entries(angleKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  return angle as any;
  ...
  'static': ['static', 'locked off', 'tripod', 'steady'] }
    ...
    for (const [movement, keywords] of Object.entries(movementKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  ...
  'rack-focus': ['rack focus', 'focus pull', 'shifting focus'] }
    ...
    for (const [dof, keywords] of Object.entries(dofKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  return dof as any;
  ...
  'natural': ['natural', 'daylight', 'window light', 'available'] }
    ...
    for (const [style, keywords] of Object.entries(lightingKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  return style as any;
  ...
  'cinematic': ['cinematic', 'film', 'movie-like', 'theatrical'] }
    ...
    for (const [grade, keywords] of Object.entries(colorKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  return grade as any;
  ...
  'dusty': ['dusty', 'dust', 'particles', 'gritty'] }
    ...
    for (const [atmosphere, keywords] of Object.entries(atmosphereKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
  ...
  'sharp': ['sharp', 'detailed', 'clear background'] }
    ...
    for (const [treatment, keywords] of Object.entries(treatmentKeywords)) { if (keywords.some(keyword => )
  prompt.toLowerCase().includes(keyword) ||
  Object.values(variables).some(v => v.toLowerCase().includes(keyword))
  )) {
...
${this.generateTechnicalConsiderations(prompt, variables)}`;}
  ...
  private inferShotTypeNaturally(prompt: string): string { const shotTypes = {
  ...
  'extreme wide shot': ['extreme wide', 'landscape', 'aerial'] }
    ...
    for (const [naturalName, keywords] of Object.entries(shotTypes)) { if (keywords.some(keyword => prompt.toLowerCase().includes(keyword))) {
  return naturalName;
  return 'medium shot';
  ...
  private determineVisualHierarchyNaturally(prompt: string): string { if (prompt.toLowerCase().includes('foreground')) return 'Foreground-dominant';
  ...
  'Cool, modern': ['cool', 'blue', 'contemporary'] }
    ...
    for (const [naturalStyle, keywords] of Object.entries(styles)) { if (keywords.some(keyword => prompt.toLowerCase().includes(keyword))) {
  return naturalStyle;
  return 'Natural, even';
