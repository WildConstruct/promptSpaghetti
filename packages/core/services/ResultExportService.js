...
export class ResultExportService {
     ...
     */
    async exportIndividualResult(result: any, resultIndex: number, totalResults: number, options: any, sourceGraph: any): Promise<any> {
        const exportData = {
            result,
            index: resultIndex,
            totalResults,
     ...
     */
    async exportBatchResults(results: any[], selectedIndices: number[], options: any, sourceGraph: any): Promise<any> {
        ...
        const filename = options.filename || this.generateFilename(options.format, 'batch', selectedResults.map(r => r.seed));
        return await this.performExport(exportData, options, filename, 'batch');
    }
    /**
     * Export all results with comparison analysis
     */
    async exportComparison(results: any[], options: any, sourceGraph: any): Promise<any> {
     ...
     */
    validateExportOptions(format: string, options: any) {
        const errors = [];
     ...
     */
    estimateExportSize(results: any[], format: string, options: any) {
     ...
     */
    async performExport(data: any, options: any, filename: string, exportType: string) {
    ...
    mapToExistingFormat(format: string) {
        const formatMap = {
    ...
    transformDataForExport(data: any, options: any, exportType: string) {
        const baseData = {
            exportType,
    ...
    transformOptionsForExport(options: any) {
        return {
    ...
    async handleCustomExport(data: any, options: any, filename: string, exportType: string) {
    ...
    exportPlainText(data: any, options: any) {
    ...
    exportExecutionTimeline(data: any, options: any) {
        const results = 'results' in data ? data.results : [data.result];
        const timeline = {
    ...
    exportVarianceReport(data: any, options: any) {
        const results = 'results' in data ? data.results : [data.result];
    ...
    exportBatchSummary(data: any, options: any) {
        const summary = {
    ...
    generateFilename(format: string, type: string, seeds: any) {
    ...
    calculateAggregateStats(results: any[]) {
    ...
    calculateVarianceScore(results: any[]) {
        if (results.length < 2)
            return 0;
    ...
    findCommonElements(results: any[]) {
    ...
    generateComparisonAnalysis(results: any[]) {
        return {
            totalResults: results.length,
    ...
    getExecutionTimeSpread(results: any[]) {
        const times = results
            .map(r => r.executionTimeMs)
            .filter((t) => typeof t === 'number')
            .sort((a, b) => a - b);
    ...
    analyzeRandomization(results: any[]) {
        const randomizationCounts = results.map(r => r.executionPath?.randomizationPoints.length || 0);
        return {
    ...
    extractExecutionPaths(results: any[]) {
        return results
    ...
    transformVFXData(results: any[], vfxOptions: any) {
        return {
    ...
    generateAnalysis(results: any[], analysisOptions: any) {
        const analysis = {};
    ...
    analyzeVarianceDistribution(results: any[]) {
        const lengths = results.map(r => r.output?.length || 0);
        const mean = lengths.reduce((sum, len) => sum + len, 0) / lengths.length;
        return {
            mean,
            standardDeviation: Math.sqrt(lengths.reduce((sum, len) => sum + Math.pow(len - mean, 2), 0) / lengths.length),
            range: {
    ...
    calculateVocabularyDiversity(results: any[]) {
        const allWords = results
            .map(r => r.output || '')
            .join(' ')
            .toLowerCase()
    ...
    buildExecutionTimeline(executionPath: any) {
    ...
    generateVarianceAnalysis(results: any[]) {
        return {
    ...
    findUniqueElements(results: any[]) {
        const allWords = results
            .map(r => r.output || '')
            .flatMap(output => output.toLowerCase().split(/\s+/))
            .filter(word => word.length > 3);
    ...
    generateVarianceRecommendations(results: any[]) {
    ...
    generateBatchBreakdown(results: any[]) {
        return {
    ...
    groupByExecutionTime(results: any[]) {
    ...
    groupByOutputLength(results: any[]) {
    ...
    groupByRandomizationCount(results: any[]) {
