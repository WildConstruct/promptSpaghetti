        ...
        for (let i = 0; i < texts.length; i += batchSize) {
            ...
            for (const textData of batch) {
                try {
                    ...
                    // Perform analysis
                    const analysis = await this.performSentimentAnalysis(textData, validatedRequest.options);
    ...
    calculateSentimentAnalytics(resourceId: string, resourceType: string, timeRange: any, analyses: any[]) {
        ...
        const analytics = {
            ...
            emotionAnalytics: {
                ...
                distribution: Object.fromEntries(Object.entries(emotionCounts).map(([emotion, count]) => [
                    emotion,
                    total > 0 ? (count / total) * 100 : 0
                ])),
                averageScores: Object.fromEntries(Object.entries(emotionScores).map(([emotion, scores]) => [
                    emotion,
            ...
            toxicityAnalytics: {
                ...
                distribution: Object.fromEntries(Object.entries(toxicityCounts).map(([level, count]) => [
                    level,
                    total > 0 ? (count / total) * 100 : 0
                ])),
                categories: {
