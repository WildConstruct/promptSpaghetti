...
export class GraphAnalyzer {
  ...
  async detectConflicts(nodes: Node[], edges: Edge[]): Promise<Conflict[]> {
    ...
    if (this.llmService) {
      const semanticConflicts = await this.detectSemanticConflicts(nodes, edges);
  ...
  analyzeComplexity(nodes: Node[], edges: Edge[]): ComplexityReport {
    ...
    const avgBranching = outDegrees.size > 0
      ? Array.from(outDegrees.values()).reduce((a, b) => a + b, 0) / outDegrees.size
      : 0;
    ...
    if (avgBranching > 5) {
      suggestions.push('High branching factor detected. Consider consolidating some paths.');
    }
    
    if (maxDepth > 10) {
      suggestions.push('Deep graph structure. Consider flattening or using fragments.');
    }
    ...
    if (duplicateNodes.length > 0) {
      suggestions.push(`Found ${duplicateNodes.length} potentially duplicate nodes.`);
    }
