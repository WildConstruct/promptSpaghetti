...
import { ContinuityTracker, ExtraProfile, ValidationResult } from './ContinuityTracker';
...
export class BulkOperationsManager {
  ...
  async bulkExtractMetadata(
    ...
    try {
      const results: Array<{ id: string; metadata?: SegmentMetadata; error?: string }> = [];
        ...
        const batchResults = await Promise.allSettled(
          batch.map(async item => {
            try {
              ...
              operation.failed++;
              operation.errors.push(`Failed to extract metadata for ${item.id}: ${error}`);
        ...
        const batchResults = await Promise.all(
          batch.map(async id => {
            try {
              ...
              operation.failed++;
              operation.errors.push(`Failed to apply template to ${id}: ${error}`);
          ...
          batch.map(async extraId => {
            try {
              const result = await this.continuityTracker.validateContinuity(
                extraId,
                { scene_id: sceneId, time_of_day: 'afternoon', location: 'urban', indoor: false }
              );
              operation.completed++;
              return result;
            } catch (error) {
              operation.failed++;
              operation.errors.push(`Failed to sync continuity for ${extraId}: ${error}`);
              return null;
            }
          })
        );
        
        results.push(...batchResults.filter(r => r !== null) as ValidationResult[]);
  ...
  parseNaturalLanguageQuery(query: string): NaturalLanguageQuery {
    ...
    if (lowerQuery.includes('high intensity') || lowerQuery.includes('intense')) {
      parsed.parsed!.conditions.push({
        field: 'intensity',
        operator: 'greater',
        value: 7
    ...
    const stopWords = new Set(['in', 'at', 'the', 'all', 'find', 'show', 'get', 'with', 'without']);
  ...
  async executeNaturalLanguageSearch(
    ...
    for (const item of items) {
      ...
      for (const keyword of parsed.parsed!.keywords) {
        ...
        if (item.metadata.subject?.toLowerCase().includes(lowerKeyword) ||
            item.metadata.action?.toLowerCase().includes(lowerKeyword) ||
            item.metadata.tags?.some(tag => tag.toLowerCase().includes(lowerKeyword))) {
          score += 0.5;
          maxScore += 0.5;
        }
      }
  ...
  private createOperation(type: BulkOperation['type'], total: number): BulkOperation {
