class WorkflowService {
    ...
    async getWorkflowConfig(configId: string): Promise<any> {
    ...
    async updateWorkflowConfig(configId: string, updates: any): Promise<any> {
        const response = await fetch(`${this.baseUrl}/workflows/configs/${configId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
    ...
    // Workflow Instances
    async getWorkflowInstance(resourceId: string, resourceType: string): Promise<any> {
        const params = new URLSearchParams({
            resourceId,
            resourceType
        });
