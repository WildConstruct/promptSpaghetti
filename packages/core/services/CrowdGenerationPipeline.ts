...
export interface CrowdGenerationRequest { scene: {;
  era: Era;
  region: string;
...
export interface CrowdActivity { primary: string; // e.g., "market day", "religious ceremony", "royal procession";
...
export class CrowdGenerationPipeline { private historicalDataService: HistoricalDataService;
  ...
  */
   ...
   */
  private async prepareHistoricalContext(request: CrowdGenerationRequest): Promise<HistoricalContext> { const clothingQuery: HistoricalQuery = {
  ...
  ): Promise<CrowdIndividual> { const individuals: CrowdIndividual = [];
  ...
  */
  ...
  // Helper methods
  private mapActivityToOccasion(activity: string): 'daily' | 'ceremonial' | 'military' | 'religious' { const mapping: Record<string, 'daily' | 'ceremonial' | 'military' | 'religious'> = {
  'market day': 'daily'
  ...
  private generateDemographics(demographics: CrowdDemographics): any { // Implementation for demographic generation
  return {
  ...
  private generatePosition(index: number, crowd: CrowdGenerationRequest['crowd']): any { // Implementation for crowd positioning
  return {
  x: Math.random() * 100
  y: 0
  z: Math.random() * 100
...
interface HistoricalContext { clothing: HistoricalItem;
  socialStructure: any;
  culturalRules: any;
