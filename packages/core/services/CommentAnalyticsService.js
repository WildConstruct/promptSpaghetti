...
export class CommentAnalyticsService {
    ...
    async recordEngagementEvent(commentId, userId, sessionId, engagementType, metadata = {}) {
        try {
         ...
         VALUES ($1, $2, $3, $4, $5, NOW())`, [commentId, userId, sessionId, engagementType, JSON.stringify(metadata)]);
    ...
    async getTimeSeriesData(resourceId, resourceType, startDate, endDate) {
       ...
       ORDER BY date`, [resourceId, resourceType, startDate, endDate]);
        ...
        const hourlyResult = await this.db.query(`SELECT 
    ...
    async getSentimentAnalysis(resourceId, resourceType, startDate, endDate) {
       ...
       GROUP BY sentiment`, [resourceId, resourceType, startDate, endDate]);
    ...
    async calculateGrowthRate(resourceId, resourceType, startDate, endDate) {
        ...
        if (previousPeriod.totalComments === 0)
            return 100;
    ...
    async getPeakEngagementTime(resourceId, resourceType, startDate, endDate) {
       ...
       ORDER BY count DESC
       LIMIT 1`, [resourceId, resourceType, startDate, endDate]);
        if (result.rows.length === 0)
            return '12:00';
