...
export class ModerationStatesService {
     ...
     * Item Management
     */
    async createModerationItem(itemData, createdBy) {
    ...
    async transitionItem(itemId, toStateId, reason, triggeredBy, metadata) {
    ...
    async assignReviewer(itemId, reviewerId, assignedBy, dueDate) {
        const item = this.items.get(itemId);
        if (!item)
            return false;
        const assignment = {
    ...
    async escalateItem(itemId, reason, escalatedBy) {
        const item = this.items.get(itemId);
        if (!item)
            return false;
        const updatedItem = {
    ...
    getModerationItems(filter) {
        ...
        if (filter.searchQuery) {
            ...
            items = items.filter(i => i.content.originalContent.toLowerCase().includes(query) ||
    ...
    getModerationStats() {
        ...
        const itemsProcessedToday = items.filter(i => i.updatedAt >= today && i.updatedAt < tomorrow).length;
        const itemsResolvedToday = items.filter(i => i.resolvedAt && i.resolvedAt >= today && i.resolvedAt < tomorrow).length;
        const now = new Date();
    ...
    findValidTransition(fromStateId, toStateId) {
        return Array.from(this.transitions.values()).find(t => t.fromStates.includes(fromStateId) && t.toState === toStateId) || null;
    }
    ...
    getApplicableAutoActions(item) {
        return Array.from(this.autoActions.values()).filter(action => action.enabled && this.matchesAutoActionTriggers(item, action));
    }
...
export const createModerationItem = (itemData, createdBy) => moderationStatesService.createModerationItem(itemData, createdBy);
