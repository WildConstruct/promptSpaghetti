...
export class WildConstructVFXExporter {
    ...
    buildGraphStructure(graph, executionResults) {
        ...
        const vfxConnections = graph.edges.map(edge => ({
            id: edge.id,
            source: {
                nodeId: edge.source,
    ...
    buildExecutionData(executionResults, options) {
        ...
        const reproducibilityData = this.buildReproducibilityData(masterSeed, nodeSeeds, executionResults, options);
        return {
            randomization: {
                masterSeed,
                nodeSeed: nodeSeeds,
    ...
    validateExport(exportData) {
        ...
        const nodesWithoutConfig = exportData.graph?.nodes?.filter(node => !node.configuration || Object.keys(node.configuration).length === 0) || [];
        ...
        const weightedNodes = exportData.graph?.nodes?.filter(node => node.type === 'WeightedChoice') || [];
        const nodesWithoutWeights = weightedNodes.filter(node => !node.configuration?.weightedChoiceData?.weights ||
...
This export is ready for integration into VFX pipelines and supports Wild Construct's future modules.
     ...
     */
    buildReproducibilityData(masterSeed, nodeSeeds, executionResults, options) {
     ...
     */
    validateReproducibility(exportData) {
        const issues = [];
        const requirements = [];
     ...
     */
    async reproduceFromExport(exportData, graph) {
        try {
            ...
            const originalVariables = Object.fromEntries(Object.entries(exportData.prompt.variables).map(([k, v]) => [k, v.value]));
...
export const generateVFXDocumentation = (exportData) => vfxExporter.generateDocumentation(exportData);
export const validateVFXExport = (exportData) => vfxExporter.validateExport(exportData);
