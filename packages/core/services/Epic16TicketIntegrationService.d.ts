...
export interface TicketMetadata { source: 'web' | 'api' | 'email' | 'chat' | 'phone' | 'community';
    userAgent?: string;
    ipAddress?: string;
    referrer?: string;
...
export interface SLAMetric { target: number;
    actual?: number;
    deadline: Date;
    breached: boolean;
...
export interface TicketIntegrationConfig { defaultWorkflow: string;
    ...
    security: { encryptAttachments: boolean;
        scanUploads: boolean;
        retentionPeriod: number };
    analytics: { trackMetrics: boolean;
 ...
 */
export declare class Epic16TicketIntegrationService extends EventEmitter { private tickets;
    private workflows;
    ...
    getTicketMetrics(timeRange: { )
        start: Date;
        end: Date }): Promise<{ totalTickets: number;
