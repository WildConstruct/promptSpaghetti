...
export interface ActivityEvidence { type: EvidenceType;
    description: string;
    data: any;
    timestamp: Date;
    source: string;
...
export interface DetectionPattern { id: string;
    name: string;
    description: string;
    confidence: number;
...
export interface ActivityResolution { action: ResolutionAction;
    reason: string;
    timestamp: Date;
    resolvedBy: string;
    notes?: string;
...
export interface ResponseAction { type: ResponseType;
    status: ActionStatus;
    triggeredAt: Date;
...
export interface DeviceFingerprint { userAgent: string;
...
export interface DetectionRule { id: string;
    name: string;
    description: string;
...
export interface AggregationRule { type: AggregationType;
    field?: string;
    groupBy: string[];
...
export interface TimeWindow { duration: number;
    sliding: boolean;
    bucketSize?: number }
}
}
export interface RuleThreshold { value: number;
    operator: ConditionOperator;
    dynamic: boolean;
    baseline?: number;
...
export interface AdaptationConfig { enabled: boolean;
    learningPeriod: number;
    adaptationRate: number;
    minimumSamples: number }
}
}
export interface RuleAction { type: ResponseType;
    parameters: Record<string, any>;
    delay?: number;
    condition?: string }
}
}
export interface RuleAnalytics { totalTriggers: number;
...
export interface LoginPattern { averageSessionDuration: number;
...
export interface TimeRange { start: number;
    end: number;
    frequency: number }
}
}
export interface LocationFrequency { country: string;
    region: string;
    city: string;
    frequency: number;
    lastSeen: Date }
...
export interface DevicePattern { fingerprint: DeviceFingerprint;
    frequency: number;
    lastSeen: Date;
    trusted: boolean }
}
}
export interface LocationPattern { location: GeoLocation;
    frequency: number;
    lastSeen: Date;
    velocity: number }
}
}
export interface ActivityPattern { pageViewsPerSession: number;
...
export interface PageFrequency { page: string;
    frequency: number;
    averageTime: number }
}
}
export interface ClickPattern { elementType: string;
    frequency: number;
    timing: number }
}
}
export interface SearchPattern { queries: string[];
    frequency: number;
    categories: string[] }
}
}
export interface UploadPattern { frequency: number;
    averageFileSize: number;
    commonFileTypes: string[];
    uploadTimes: TimeRange[] }
}
}
export interface ContentPattern { postingFrequency: number;
...
export interface SentimentDistribution { positive: number;
    neutral: number;
    negative: number }
}
}
export interface LanguagePattern { language: string;
    frequency: number;
    complexity: number }
}
}
export interface TransactionPattern { averageTransactionAmount: number;
...
export interface SocialPattern { connectionGrowthRate: number;
...
export interface AnomalyBaseline { loginFrequency: StatisticalBaseline;
...
export interface StatisticalBaseline { mean: number;
...
export interface BehaviorAnomaly { type: string;
    timestamp: Date;
    severity: number;
    description: string;
...
export interface ThreatIntelligence { id: string;
    type: ThreatType;
    source: string;
    confidence: number;
...
export interface ThreatIndicator { type: IndicatorType;
    value: string;
    confidence: number;
    context?: string;
...
export interface ThreatImpact { confidentiality: ImpactLevel;
...
export interface Mitigation { type: MitigationType;
    description: string;
...
export declare class Epic16SuspiciousActivityService extends EventEmitter { private activities;
    private rules;
...
export interface SecurityMetrics { totalActivities: number;
    ...
    topTargets: Array<{ userId: string;
        count: number }>;
