...
\b/g, '[EMAIL]');
  ...
  if (toxicityScore >= 0.8) toxicityLevel = 'severe';
  else if (toxicityScore >= 0.6) toxicityLevel = 'high';
  else if (toxicityScore >= 0.3) toxicityLevel = 'medium';
  ...
  if (words.length >= 10 && words.length <= 200) score += 20;
  else if (words.length < 5 || words.length > 500) score -= 20;
  // Sentence structure
  ...
  if (constructiveWords.some(word => text.toLowerCase().includes(word))) score += 20;
  return {
  ...
  private calculateOverallConfidence(sentiment: any, emotions: any, toxicity: any): number {
  ...
  private calculateOverallToxicityLevel(counts: Record<string, number>): ToxicityLevel {
    ...
    if (severeRate > 0.1) return 'severe';
    if (highRate > 0.2) return 'high';
    if ((counts.medium + counts.high + counts.severe) / total > 0.3) return 'medium';
    if (counts.low / total > 0.1) return 'low';
    return 'none';
  ...
  private getCachedAnalysis(cacheKey: string): SentimentAnalysis | null {,
  ...
  sourceType: 'comment' as FeedbackSourceType,
        ...
        modelUsed: 'transformer' as AnalysisModelType,
        modelVersion: '1.0.0',
