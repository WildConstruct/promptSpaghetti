...
export interface ActivityDetails { // Graph and node changes
...
export class ActivityTimelineService { private static instance: ActivityTimelineService;
  ...
  ): Promise<ActivityEvent> { return this.trackActivity({)
  ...
  displayName: activity.userDisplayName || userId }
      ...
      if (activity.timestamp > userActivity[userId].lastActivity) { userActivity[userId].lastActivity = activity.timestamp });
  ...
  lastActivity: data.lastActivity }
    ...
    return { totalEvents: activities.length
  uniqueUsers
   ...
   */
  getUserTimeline(userId: string, filter?: Partial<ActivityFilter>): ActivityEvent { return this.getActivities({)
  ...filter
  userIds: [userId] }
});
  /**
   ...
   */
  getCollaborativeActivities(workspaceId?: string, filter?: Partial<ActivityFilter>): ActivityEvent { return this.getActivities({)
  ...filter
  types: ['collaboration']
   ...
   */
  subscribe(listenerId: string, callback: (event: ActivityEvent) => void): void { this.listeners.set(listenerId, callback);
  /**
  ...
  */
  ...
  projectId?: string;
  location?: GeolocationInfo;
}): ActivitySession { // End current session if exists
   ...
   */
  endSession(): ActivitySession | null { if (!this.currentSession) return null;
  ...
  private initializeSession(): void { // Initialize with basic client info
  ...
  private updateCurrentSession(event: ActivityEvent): void { if (!this.currentSession) return;
  ...
  errorsOnly: true
    ...
    if (event.type === 'graph_operation' && event.impact === 'high') { // Track high-impact activities for productivity insights
  ...
  try {
  callback(event) } catch (error) { console.error('Error in activity listener:', error) });
  private emitActivity(event: ActivityEvent): void { // Integration point with external systems
  ...
  private calculateGraphOperationImpact(details: Partial<ActivityDetails>): ActivityImpact { const changeCount = (details.nodeChanges?.length || 0) + ;
  ...
  private calculatePerformanceImpact(duration: number, success: boolean): ActivityImpact { if (!success) return 'high';
  ...
  const key = String(item[field]);
    ...
    return grouped;
  private startBackgroundProcessing(): void { // Clean up old activities every hour
    setInterval(() => {
      this.cleanupOldActivities() }, 60 * 60 * 1000);
    // Process session analytics every 30 minutes
    setInterval(() => { this.processSessionAnalytics() }, 30 * 60 * 1000);
  private cleanupOldActivities(): void { const cutoffDate = new Date();
