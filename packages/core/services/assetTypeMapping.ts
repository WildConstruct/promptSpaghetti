...
export type CompatType = 'WeightedChoice' | 'TextBlock' | 'Concat' | 'Output' | 'Variable' | 'Unknown';
...
export function mapAssetToNodeType(asset: { id: string; name?: string; type?: string; metadata?: any }): MappedType {
  const name = (asset.name || '').toLowerCase();
  const keywords: string[] = ((asset as any).metadata?.keywords || []).map((k: string) => String(k).toLowerCase());
  ...
  if (text.includes('concat') || text.includes('combine') || text.includes('merge')) {
    return { reactFlowType: 'concat', compatType: 'Concat' };
  }
  if (text.includes('output')) {
    return { reactFlowType: 'output', compatType: 'Output' };
  }
  if (text.includes('variable') || text.includes('name') || text.includes('role')) {
...
export function mapReactFlowTypeToCompat(reactFlowType: string | undefined): CompatType {
...
export function canConnect(sourceCompat: CompatType, targetCompat: CompatType): boolean {
