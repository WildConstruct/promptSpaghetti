...
export async function demonstrateParsing(prompt: string) {
  ...
  llmResult.nodes.forEach(node => {
    ...
    if (node.data.metadata) {
      console.log(`    → Chunks: ${node.data.metadata.chunks?.length || 0}, Role: ${node.data.metadata.semanticRole}`);
    }
  ...
  console.log('\n📊 COMPARISON:');
  console.log('─'.repeat(40));
  console.log(`Standard nodes: ${standardResult.nodes.length} vs Enhanced nodes: ${llmResult.nodes.length}`);
  console.log(`Standard detected variables: ${standardResult.metadata.intelligence?.detectedVariables?.length || 0}`);
  console.log(`Enhanced detected variables: ${llmResult.metadata.intelligence?.detectedVariables?.length || 0}`);
  console.log(`Enhanced structure type: ${llmResult.metadata.intelligence?.structureType}`);
  console.log(`Enhanced confidence: ${llmResult.metadata.intelligence?.confidence}`);
