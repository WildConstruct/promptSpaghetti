...
export const ExtensionSystemUtils = { /**
  ...
  */
   ...
   */
  compareVersions(version1: string, version2: string): number { const v1Parts = version1.split('.').map(Number);
   ...
   */
  isValidSemanticVersion(version: string): boolean { const semverRegex = /^\d+\.\d+\.\d+$/;
    return semverRegex.test(version) }
  /**
   * Generate extension ID from name
   */
  generateExtensionId(name: string): string { return name.toLowerCase()
      .replace(/[^a-z0-9]/g, '-')
      .replace(/-+/g, '-')
      .replace(/^-|-$/g, '') };

// Extension System Constants
export const ExtensionSystemConstants = { // Extension Types
