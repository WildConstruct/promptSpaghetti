...
import { ExtensionVersionManager } from './ExtensionLifecycleManager.js';
...
export class PluginRegistry extends EventEmitter {
     ...
     * Uninstall a plugin
     */
    async uninstallPlugin(pluginId, removeData = false) {
        try {
    ...
    async searchPlugins(options = {}) {
        try {
            ...
            for (const [registry, plugins] of this.remotePluginCache) {
                ...
                if (options.query) {
                    filtered = filtered.filter(p => p.name.toLowerCase().includes(options.query.toLowerCase()) ||
