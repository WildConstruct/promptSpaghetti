...
export class ExtensionLifecycleManager {
   ...
   * Get extension by ID
   */
  public getExtension(extensionId: string): BaseExtension | undefined {
  ...
  */
  public getExtensionsByState(state: ExtensionLifecycleState): BaseExtension {,
  ...
  * Get extension state
  */
  public getExtensionState(extensionId: string): ExtensionLifecycleState {,
  const entry = this.extensions.get(extensionId);
  return entry ? entry.state : ExtensionLifecycleState.UNINITIALIZED;
  /**
  * Get extension context
  */
  public getExtensionContext(extensionId: string): ExtensionContext | undefined {,
  return this.contexts.get(extensionId);
  /**
  * Get extension health status
  */
  public getExtensionHealth(extensionId: string): ExtensionHealthStatus {,
  const entry = this.extensions.get(extensionId);
  if (!entry) {
  return {
  status: 'error',
   ...
   */
  private getExtensionEntry(extensionId: string): ExtensionLifecycleEntry {
    const entry = this.extensions.get(extensionId);
    if (!entry) {
      throw new ExtensionError()
        ExtensionErrorType.INITIALIZATION_ERROR,
    ...
    return entry;
  private setState(entry: ExtensionLifecycleEntry, state: ExtensionLifecycleState): void {
  entry.state = state;
  ...
  private createExtensionContext(extension: BaseExtension): ExtensionContext {
  return {
  ...
  private createEventEmitter(extensionId: string): any {
  // This would be a scoped event emitter
  return this.eventEmitter;
  private createRuntime(extensionId: string): any {,
  return {
  version: '1.0.0',
  ...
  private createUIContext(extensionId: string): any {
  return {
