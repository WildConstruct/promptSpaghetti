...
export class DependencyResolver {
     ...
     */
    resolveConflicts(graph, manifests) {
     ...
     */
    areVersionRangesCompatible(ranges) {
        if (ranges.length <= 1)
            return true;
        try {
     ...
     */
    intersectVersionRanges(range1, range2) {
        try {
     ...
     */
    isOptionalEdge(graph, from, to) {
        return graph.edges.some(edge => edge.from === from && edge.to === to && edge.optional);
    }
    /**
     * Generate suggestions for resolving circular dependencies
     */
    generateCircularDependencySuggestions(cycle) {
        const suggestions = [];
     ...
     */
    getAvailableVersions(packageId) {
