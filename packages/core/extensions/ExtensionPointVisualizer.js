...
export class ExtensionPointVisualizer {
     ...
     */
    generateArchitectureOverview(options = {}) {
    ...
    buildGraph(options) {
        const registry = extensionPointRegistry;
        const extensionPoints = registry.getAll();
        const nodes = extensionPoints.map(ep => ({
            id: ep.id,
            name: ep.name,
     ...
     */
    generateGraphviz(graph, options) {
     ...
     */
    generateMermaidStyling(options) {
        let styling = '\n';
