...
export class PluginLoader {
    ...
    async loadPlugins(sources) {
        ...
        for (const pluginId of loadOrder) {
            const source = sources.find(s => s.location.includes(pluginId) || s.location.endsWith(pluginId));
            if (source) {
                try {
     ...
     */
    async resolvePluginDependencies(manifest) {
        const dependencies = [];
        if (manifest.dependencies) {
            for (const [depName, versionRange] of Object.entries(manifest.dependencies)) {
                ...
                if (loaded) {
                    ...
                    if (!this.versionManager.isVersionCompatible(loaded.manifest.version, versionRange)) {
    ...
    validatePluginPermissions(manifest) {
        if (manifest.permissions) {
            ...
            const requestedDangerous = manifest.permissions.filter(p => dangerousPermissions.some(d => p.startsWith(d)));
