...
(function (ExtensionState) {
    ExtensionState["UNINITIALIZED"] = "uninitialized";
    ExtensionState["INITIALIZING"] = "initializing";
    ExtensionState["INITIALIZED"] = "initialized";
    ExtensionState["ACTIVATING"] = "activating";
    ExtensionState["ACTIVE"] = "active";
    ExtensionState["DEACTIVATING"] = "deactivating";
    ExtensionState["DEACTIVATED"] = "deactivated";
    ExtensionState["DISPOSED"] = "disposed";
    ExtensionState["ERROR"] = "error";
})(ExtensionState || (ExtensionState = {}));
// Mock extension implementations for testing
const createMockExtension = (id, dependencies = []) => ({
    id,
    name: `Extension ${id}`,
    version: '1.0.0',
...
const createFailingExtension = (id) => ({
    id,
...
describe('Epic 24.2 - Extension Lifecycle Manager Unit Tests', () => {
    let lifecycleManager; // Will be properly typed when ExtensionLifecycleManager exists
    let mockExtensions;
    ...
    beforeEach(() => {
        ...
        lifecycleManager = {
            ...
            registerExtension: jest.fn().mockImplementation((ext) => {
                mockExtensions.set(ext.id, ext);
            }),
            unregisterExtension: jest.fn().mockImplementation((id) => {
                mockExtensions.delete(id);
            }),
            activateExtension: jest.fn(),
            deactivateExtension: jest.fn(),
            disposeExtension: jest.fn(),
            getExtensionState: jest.fn(),
            getActiveExtensions: jest.fn(),
            getExtensionHealth: jest.fn(),
            addEventListener: jest.fn(),
            removeEventListener: jest.fn(),
            createExtensionContext: jest.fn(),
            validateExtensionDependencies: jest.fn(),
            resolveActivationOrder: jest.fn()
        };
        // Setup default state tracking
        lifecycleManager.getExtensionState.mockImplementation((id) => {
        ...
        lifecycleManager.getActiveExtensions.mockImplementation(() => {
            return Array.from(mockExtensions.values()).filter(ext => ext.state === ExtensionState.ACTIVE);
        });
    });
    afterEach(() => {
    ...
    describe('1. Lifecycle Manager Initialization', () => {
        ...
        it('should setup event system during initialization', async () => {
            lifecycleManager.addEventListener.mockImplementation((event, handler) => {
        ...
        it('should handle initialization failures gracefully', async () => {
            lifecycleManager.initialize.mockRejectedValueOnce(new Error('Initialization failed'));
    ...
    describe('2. Extension Registration and Management', () => {
        ...
        it('should prevent duplicate extension registration', () => {
            ...
            lifecycleManager.registerExtension.mockImplementationOnce((ext) => {
    ...
    describe('3. Extension State Transitions', () => {
        ...
        it('should transition from ACTIVE to DEACTIVATED', async () => {
            ...
            lifecycleManager.getExtensionState.mockReturnValue(ExtensionState.ACTIVE);
            lifecycleManager.deactivateExtension.mockImplementationOnce(async (id) => {
                const ext = mockExtensions.get(id);
                if (ext) {
        ...
        it('should transition to ERROR state on activation failure', async () => {
            ...
            lifecycleManager.activateExtension.mockImplementationOnce(async (id) => {
                const ext = mockExtensions.get(id);
                if (ext && ext.activate) {
                    try {
                        await ext.activate({});
        ...
        it('should prevent invalid state transitions', async () => {
            ...
            lifecycleManager.deactivateExtension.mockRejectedValueOnce(new Error('Cannot deactivate extension in UNINITIALIZED state'));
    ...
    describe('4. Extension Context Management', () => {
        it('should create isolated context for each extension', () => {
            const mockContext = {
                ...
                logging: { log: jest.fn(), error: jest.fn() },
                runtime: { version: '1.0.0' },
                ui: { createElement: jest.fn() },
                api: { call: jest.fn() }
            };
            lifecycleManager.createExtensionContext.mockReturnValue(mockContext);
        ...
        it('should provide sandbox isolation between extensions', () => {
            ...
            lifecycleManager.createExtensionContext.mockImplementationOnce((id) => {
                return {
                    storage: {
        ...
        it('should cleanup context when extension is disposed', async () => {
            ...
            const mockContext = {
                ...
                logging: { flush: jest.fn() }
            };
            lifecycleManager.createExtensionContext.mockReturnValue(mockContext);
            lifecycleManager.disposeExtension.mockImplementationOnce(async (id) => {
    ...
    describe('5. Dependency Resolution and Activation Order', () => {
        ...
        it('should validate extension dependencies', () => {
            ...
            lifecycleManager.validateExtensionDependencies.mockImplementationOnce((id) => {
                const ext = mockExtensions.get(id);
        ...
        it('should activate extensions in dependency order', async () => {
            ...
            lifecycleManager.activateExtension.mockImplementation(async (id) => {
    ...
    describe('6. Health Monitoring and Error Recovery', () => {
        it('should monitor extension health status', () => {
            ...
            lifecycleManager.getExtensionHealth.mockReturnValue(mockHealth);
    ...
    describe('7. Event System and Notifications', () => {
        ...
        it('should emit error events for failed operations', async () => {
            ...
            lifecycleManager.activateExtension.mockRejectedValueOnce(new Error('Activation failed'));
            try {
    ...
    describe('8. Resource Management and Cleanup', () => {
        it('should dispose of all extensions on shutdown', async () => {
            ...
            lifecycleManager.disposeExtension.mockResolvedValue(true);
            // Simulate shutdown - dispose all extensions
            const disposalPromises = extensions.map(ext => lifecycleManager.disposeExtension(ext.id));
        ...
        it('should prevent memory leaks during extension lifecycle', async () => {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                lifecycleManager.registerExtension(extension);
                lifecycleManager.activateExtension.mockResolvedValue(true);
                lifecycleManager.deactivateExtension.mockResolvedValue(true);
                lifecycleManager.disposeExtension.mockResolvedValue(true);
    ...
    describe('9. Concurrent Operations and Thread Safety', () => {
        it('should handle concurrent extension activations', async () => {
            ...
            const activationPromises = extensions.map(id => lifecycleManager.activateExtension(id));
