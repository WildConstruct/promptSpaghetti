...
describe('Epic 24.2 - Plugin Loader Integration Tests', () => {
    let testDir;
    let pluginSystem; // Mock plugin system implementation
    let mockPluginFiles;
    beforeEach(async () => {
        jest.clearAllMocks();
        ...
        pluginSystem = {
            ...
            loadAndActivatePlugin: jest.fn(),
            deactivateAndUnloadPlugin: jest.fn(),
            getSystemStatus: jest.fn()
        };
        // Setup default implementations
        pluginSystem.loader.loadPlugin.mockImplementation(async (source) => {
        ...
        pluginSystem.lifecycle.activateExtension.mockResolvedValue(true);
        pluginSystem.lifecycle.deactivateExtension.mockResolvedValue(true);
        pluginSystem.lifecycle.getExtensionState.mockReturnValue('active');
    });
    afterEach(async () => {
    ...
    describe('1. End-to-End Plugin Loading Workflows', () => {
        it('should complete full load-activate-deactivate-unload cycle', async () => {
            ...
            pluginSystem.loadAndActivatePlugin.mockImplementationOnce(async (source) => {
                // Step 1: Load plugin
            ...
            pluginSystem.deactivateAndUnloadPlugin.mockImplementationOnce(async (id) => {
                // Step 1: Deactivate
        ...
        it('should handle plugin loading from filesystem with real directory structure', async () => {
            ...
            await fs.writeFile(path.join(pluginDir, 'index'), `
        module.exports = {
          activate() {
        ...
        `);
            ...
            pluginSystem.loader.loadPlugin.mockImplementationOnce(async (source) => {
                // Verify files exist
                await fs.access(path.join(source, 'package.json'));
                await fs.access(path.join(source, 'index'));
                const manifestContent = await fs.readFile(path.join(source, 'package.json'), 'utf-8');
                return {
        ...
        it('should integrate plugin cache with lifecycle state management', async () => {
            ...
            pluginSystem.loader.getCachedPlugin.mockReturnValue(cachedPlugin);
    ...
    describe('2. Multi-Plugin Dependency Resolution Integration', () => {
        it('should resolve and load plugin dependencies in correct order', async () => {
            ...
            const loadOrder = [];
            pluginSystem.loadAndActivatePlugin.mockImplementation(async (source) => {
        ...
        it('should handle circular dependency detection across components', async () => {
            ...
            pluginSystem.loadAndActivatePlugin.mockImplementation(async (source) => {
                ...
                const plugin = circularPlugins[pluginId];
                if (plugin.dependencies) {
                    for (const depId of Object.keys(plugin.dependencies)) {
                        await pluginSystem.loadAndActivatePlugin(`/plugins/${depId}`);
                    }
    ...
    describe('3. Error Handling and Recovery Integration', () => {
        it('should handle plugin loading failures with proper cleanup', async () => {
            ...
            pluginSystem.loadAndActivatePlugin.mockImplementationOnce(async (source) => {
                ...
                await pluginSystem.lifecycle.registerExtension(plugin);
                // Activation fails
                pluginSystem.lifecycle.activateExtension.mockRejectedValueOnce(new Error('Activation failed: runtime error'));
                try {
    ...
    describe('4. Performance and Concurrency Integration', () => {
        it('should handle concurrent plugin operations without race conditions', async () => {
            ...
            const promises = plugins.map(id => pluginSystem.loadAndActivatePlugin(`/plugins/${id}`));
    ...
    describe('5. System State Synchronization', () => {
        it('should maintain consistent state across all components', async () => {
            ...
            pluginSystem.lifecycle.registerExtension.mockImplementation(async (plugin) => {
                systemState.lifecycle.set(plugin.id, { ...plugin, state: 'registered' });
            });
            pluginSystem.lifecycle.activateExtension.mockImplementation(async (id) => {
            ...
            pluginSystem.registry.register.mockImplementation((plugin) => {
        ...
        it('should propagate state changes through event system', async () => {
            ...
            pluginSystem.lifecycle.addEventListener.mockImplementation((event, handler) => {
            ...
            pluginSystem.loadAndActivatePlugin.mockImplementationOnce(async () => {
                ...
                pluginSystem.lifecycle.addEventListener('pluginLoaded', (data) => {
                    eventLog.push({ event: 'pluginLoaded', data });
                });
                pluginSystem.lifecycle.addEventListener('pluginActivated', (data) => {
