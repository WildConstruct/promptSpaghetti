...
interface DependencyConstraint { name: string;
  version: string;
  optional?: boolean }


interface PluginManifest { id: string;
  name: string;
  version: string;
...
interface DependencyNode { plugin: PluginManifest;
  dependencies: DependencyNode;
  resolved: boolean;
  circular?: boolean }


interface DependencyResolver { resolveDependencies(plugins: PluginManifest): Promise<PluginManifest>;
...
describe('Epic 24.2 - Dependency Resolver Unit Tests', () => { let dependencyResolver: unknown; // Mock implementation
  ...
  peerDependencies: { 'theme-plugin': '>=1.0.0' }
  ...
  optionalDependencies: { 'analytics-plugin': '^1.0.0'];
    ...
    it('should resolve linear dependency chain correctly', async () => {
      ...
      dependencyResolver.resolveDependencies.mockImplementationOnce(async (plugins: PluginManifest) => {
        ...
        const visit = (plugin: PluginManifest) => {
          ...
          if (plugin.dependencies) { Object.keys(plugin.dependencies).forEach(depId => {)
  const dep = plugins.find(p => p.id === depId);
    ...
    it('should build complete dependency graph with all relationships', () => { const expectedGraph = new Map<string, DependencyNode>();
  ...
  resolved: false }
    ...
    it('should handle missing dependencies gracefully', async () => { const pluginWithMissingDep: PluginManifest = {
  id: 'broken-plugin'
  name: 'Broken Plugin'
  version: '1.0.0'
  dependencies: {
  'nonexistent-plugin': '^1.0.0' }
    ...
    it('should resolve complex multi-level dependencies', async () => { const complexPlugins: PluginManifest = [
        {
          id: 'level-1-a'
          name: 'Level 1 A'
          version: '1.0.0' }
  ...
  describe('2. Version Constraint Validation', () => { it('should validate semantic version constraints correctly', () => {
  ...
  'ui-plugin': '>=1.4.0' }
      ...
      dependencyResolver.validateVersionConstraints.mockImplementation()
        (plugin: PluginManifest, available: PluginManifest) => { const availableMap = new Map(available.map(p => [p.id, p]));
    ...
    it('should reject incompatible version constraints', () => { const incompatiblePlugin: PluginManifest = {
  ...
  'utils-plugin': '^3.0.0' // Available version is 2.1.0 }
    ...
    it('should handle peer dependencies validation', () => { const pluginWithPeers: PluginManifest = {
  id: 'peer-test-plugin'
  ...
  'theme-plugin': '>=1.0.0' }
      ...
      dependencyResolver.checkCompatibility.mockImplementation()
        (plugin: PluginManifest, available: PluginManifest) => { if (!plugin.peerDependencies) return true;
      ...
      expect(result).toBe(true) });
    it('should validate engine requirements', () => { const pluginWithEngines: PluginManifest = {
  ...
  describe('3. Circular Dependency Detection', () => { it('should detect simple circular dependencies', () => {
    ...
    it('should detect complex circular dependencies', () => { const complexCircular = new Map<string, string>([);
  ['plugin-a', ['plugin-b']]
  ...
  expect(result).toEqual(['plugin-a', 'plugin-b', 'plugin-d', 'plugin-a']);
    ...
    it('should return null for acyclic dependency graphs', () => { const acyclicDeps = new Map<string, string>([);
        ['base-plugin', []]
    ...
    it('should detect self-referencing dependencies', () => { const selfReferencingDeps = new Map<string, string>([);
  ...
  describe('4. Dependency Conflict Resolution', () => { it('should resolve version conflicts by choosing highest compatible version', () => {
  const conflicts = [
  {
  plugin: 'plugin-a'
  ...
  expect(dependencyResolver.resolveConflicts).toHaveBeenCalledWith(conflicts);
    ...
    it('should handle irreconcilable version conflicts', () => { const irreconcilableConflicts = [
  {
  plugin: 'old-plugin'
  dependency: 'api-lib'
  ...
  dependencyResolver.resolveConflicts.mockImplementation(() => { throw new Error('Irreconcilable version conflict for api-lib: 1.0.0 vs 2.0.0') });
      expect(() => dependencyResolver.resolveConflicts(irreconcilableConflicts))
        .toThrow('Irreconcilable version conflict');
    });
    it('should prefer peer dependency versions in conflict resolution', () => { const peerDependencyConflicts = [
  {
  ...
  dependencyResolver.resolveConflicts.mockImplementation((conflicts) => { const resolution = new Map<string, string>();
  conflicts.forEach(conflict => {)
  const peerVersion = peerVersions.get(conflict.dependency);
  if (peerVersion) {
  resolution.set(conflict.dependency, peerVersion) } else { // Use highest version from conflict
  ...
  describe('5. Installation Order Generation', () => { it('should generate correct installation order for complex dependency tree', () => {
  ...
  'crypto-utils': '^1.0.0' }
    ...
    it('should handle optional dependencies in installation order', () => { const pluginsWithOptional: PluginManifest = [
  {
  id: 'main-plugin'
  name: 'Main Plugin'
  version: '1.0.0'
  ...
  optionalDependencies: { 'optional-dep': '^1.0.0' }
      ...
      dependencyResolver.getInstallationOrder.mockImplementation((plugins: PluginManifest) => { // Optional dependencies can be installed in any order relative to main plugin
  ...
  describe('6. Performance and Edge Cases', () => { it('should handle large dependency graphs efficiently', async () => {
  ...
  version: '1.0.0',
    ...
    it('should handle empty dependency lists', async () => { dependencyResolver.resolveDependencies.mockResolvedValue([] as unknown as unknown as unknown as unknown as unknown);
    ...
    it('should validate malformed dependency specifications', () => { const malformedPlugin: PluginManifest = {,
  id: 'malformed-plugin',
  ...
  '': '1.0.0' // Empty dependency name }
      ...
      dependencyResolver.validateVersionConstraints.mockImplementation()
        (plugin: PluginManifest) => { if (!plugin.dependencies) return true;
