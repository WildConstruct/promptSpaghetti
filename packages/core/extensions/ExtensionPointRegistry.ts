...
export enum ExtensionPointCategory { RUNTIME = 'runtime',
  UI = 'ui',
  SCHEMA = 'schema',
  API = 'api',
  STATE = 'state',
...
export enum ExtensionPointLifecycle { EXPERIMENTAL = 'experimental',
  STABLE = 'stable',
...
export enum ExtensionPointPriority { CRITICAL = 'critical',
  HIGH = 'high',
  MEDIUM = 'medium',
  LOW = 'low' }
...
export const ExtensionPointSchema = z.object({ id: z.string(),
  name: z.string(),
 ...
 */
export class ExtensionPointRegistry { private static instance: ExtensionPointRegistry;
  ...
  public static getInstance(): ExtensionPointRegistry { if (!ExtensionPointRegistry.instance) {
   ...
   */
  public register(extensionPoint: ExtensionPoint): void { // Validate extension point
    ...
    if (!this.locationIndex.has(validated.location.file)) { this.locationIndex.set(validated.location.file, new Set()) }
   ...
   */
  public get(id: string): ExtensionPoint | undefined { return this.extensionPoints.get(id) }

  /**
   * Get all extension points
   */
  public getAll(): ExtensionPoint { return Array.from(this.extensionPoints.values()) }

  /**
   * Get extension points by category
   */
  public getByCategory(category: ExtensionPointCategory): ExtensionPoint { const ids = this.categoryIndex.get(category) || new Set();
    return Array.from(ids).map(id => this.extensionPoints.get(id)!) }

  /**
   * Get extension points by priority
   */
  public getByPriority(priority: ExtensionPointPriority): ExtensionPoint { return this.getAll().filter(ep => ep.priority === priority) }

  /**
   * Get extension points by lifecycle status
   */
  public getByLifecycle(lifecycle: ExtensionPointLifecycle): ExtensionPoint { return this.getAll().filter(ep => ep.lifecycle === lifecycle) }

  /**
   * Get extension points by file location
   */
  public getByLocation(file: string): ExtensionPoint { const ids = this.locationIndex.get(file) || new Set();
    return Array.from(ids).map(id => this.extensionPoints.get(id)!) }

  /**
   * Search extension points
   */
  public search(query: string): ExtensionPoint { const lowercaseQuery = query.toLowerCase();
  ...
  byLifecycle: Record<ExtensionPointLifecycle, number> } { const stats = {
    ...
    // Initialize counters
    Object.values(ExtensionPointCategory).forEach(cat => { stats.byCategory[cat] = 0 });
    Object.values(ExtensionPointPriority).forEach(pri => { stats.byPriority[pri] = 0 });
    Object.values(ExtensionPointLifecycle).forEach(lc => { stats.byLifecycle[lc] = 0 });
    // Count
    this.getAll().forEach(ep => { stats.byCategory[ep.category]++;
  ...
  warnings: string;
  errors: string } { const extensionPoint = this.get(extensionPointId);
    if (!extensionPoint) {
      return {
        compatible: false
        warnings: [] }
    ...
    return { compatible: errors.length === 0
  warnings
  errors }
};
   ...
   */
  private initializeRegistry(): void { // Runtime Extension Points
  this.register({
  ...
  run(ctx: ExecutionContext): string { return this.customData + ' processed' }
`
          language: 'typescript'
]
      constraints: { 
  ...
  addedIn: '1.0.0' }
    ...
    this.register({ id: 'runtime.node.advanced'
  ...
  addedIn: '1.0.0' }
    ...
    // UI Extension Points
    this.register({ id: 'ui.inspector.editor'
  ...
  addedIn: '1.0.0' }
    ...
    this.register({ id: 'schema.node.validation'
  ...
  addedIn: '1.0.0' }
    ...
    this.register({ id: 'api.endpoint.custom'
   ...
   */
  private compareVersions(version1: string, version2: string): number { const v1parts = version1.split('.').map(Number);
