...
export class ExtensionDevelopmentKit {
     ...
     */
    createExtensionSkeleton(config) {
...
`;
    ...
    validateExtension(extension) {
        ...
        if (typeValidation.type) {
            const interfaceValidation = extensionInterfaceValidator.validateInterface(extension, `${typeValidation.type.charAt(0).toUpperCase() + typeValidation.type.slice(1)}Extension`);
...
export class ${this.toPascalCase(id)}Extension extends BaseExtensionImpl implements ${interfaceName} {
  ...
  ${typeSpecificMethods}
    ...
    generateTypeSpecificMethods(type) {
        switch (type) {
            case 'node':
                return `
...
`;
    ...
    generateAPIReference(extension, type) {
...
- \`getKeybindingContributions(): KeybindingContribution[]\` - Get keybinding contributions`;
    ...
    generateExamples(extension, type) {
        return `
### Basic Usage
\`\`\`typescript
...
const Component = extension.createComponentInstance('my-component', {});`;
    ...
    async testPhase(result, phase, testFn) {
        const phaseResult = {
            phase,
            success: true,
            duration: 0,
