...
export enum TransformType { TEXT = 'text'
  JSON = 'json'
  ARRAY = 'array'
  OBJECT = 'object'
  STRING = 'string'
...
export interface TransformDefinition { // Basic metadata
  id: string;
  name: string;
  description: string;
  version: string;
...
export interface TransformHelpConfiguration { // Documentation
...
export interface TransformPerformanceConfiguration { // Memory limits
...
export interface TransformSecurityConfiguration { // Sandboxing
  sandboxed?: boolean;
  // Permissions
  permissions?: string;
  // Input validation
...
export interface TransformPipelineConfiguration { // Pipeline support
...
export interface TransformMetadata { author: string;
  license: string;
  repository?: string;
  documentation?: string;
  examples?: TransformExample;
...
export interface TransformContext { // Basic context
  transformId: string;
  executionId: string;
  timestamp: Date;
  // Extension context
...
export namespace TransformExtensionHelpers { export function createTransformDefinition(config: Partial<TransformDefinition>): TransformDefinition {
    return {
      ...
      transformClass: config.transformClass || class implements DataTransform { id = config.id || 'custom-transform';
        name = config.name || 'Custom Transform';
        type = config.type || TransformType.CUSTOM;
        version = config.version || '1.0.0';
  ...
  export function validateTransformDefinition(definition: TransformDefinition): ExtensionValidationResult { const errors: string = [];
  ...
  export function createTransformRegistry(): TransformRegistry {
    ...
    return {
      ...
      get(transformId: string) { return registry.get(transformId) }
      getAll() { return Array.from(registry.values()) }
      getByType(type: TransformType) { return Array.from(registry.values()).filter(def => def.type === type) }
      getByCategory(category: string) { return Array.from(registry.values()).filter(def => def.ui.category === category) }
      ...
      filter(predicate: (definition: TransformDefinition) => boolean) { return Array.from(registry.values()).filter(predicate) }
      getCompatible(inputType: string, outputType: string) { return Array.from(registry.values()).filter(def => {
      ...
      validate(definition: TransformDefinition) { return validateTransformDefinition(definition) }
