...
export interface ServerProjectMetadata extends ProjectMetadata { id: string;
  userId?: number }

export interface ServerProject extends PSGFile { id: string;
  userId?: number }

export interface ProjectListResponse { projects: ServerProject;
  total: number;
  limit: number;
  offset: number }
...
export class ServerProjectManager { private static readonly API_BASE = '/api';
  /**
    ...
    try {
      ...
      Object.entries(query).forEach(([key, value]) => { if (value !== undefined) {
          if (Array.isArray(value)) {
            value.forEach(v => url.searchParams.append(key, v)) } else { url.searchParams.set(key, value.toString()) }
