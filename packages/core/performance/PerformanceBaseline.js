...
export class PerformanceBaseline extends EventEmitter {
     ...
     */
    async captureBaseline(testConditions) {
     ...
     */
    async captureAllKPIs(testConditions) {
        const kpiSnapshots = [];
    ...
    calculateCategoryAverages(snapshots) {
        ...
        for (const category of categories) {
            ...
            const categorySnapshots = snapshots.filter(snapshot => categoryKPIs.some(kpi => kpi.id === snapshot.kpiId));
     ...
     */
    compareBaselines(baseline1, baseline2) {
        const improved = [];
        const degraded = [];
        const unchanged = [];
        for (const kpi of corePerformanceKPIs) {
