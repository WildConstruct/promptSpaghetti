...
export interface PerformanceTarget { kpiId: string;
  target: number;
  warning: number;
  critical: number;
  reasoning: string;
...
export const performanceTargets: Record<string, PerformanceTarget> = { // === CORE WEB VITALS (Google Standards) ===
  runtime_fcp: {
  kpiId: 'runtime_fcp'
 ...
 */
export const environmentAdjustments: Record<string, Record<string, number>> = { development: {
  // More lenient targets for development environment
  multiplier_warning: 1.5
  multiplier_critical: 2.0 }

  staging: { // Production-like targets but slightly more lenient
  multiplier_warning: 1.2
  multiplier_critical: 1.5 }

  production: { // Strict production targets
 ...
 */
export const deviceAdjustments: Record<string, Record<string, number>> = { 'high-end': {
  // High-end devices can handle more aggressive targets
  cpu_multiplier: 0.8
  memory_multiplier: 0.8
  network_multiplier: 0.9 }

  'mid-range': { // Standard targets
  cpu_multiplier: 1.0
  memory_multiplier: 1.0
  network_multiplier: 1.0 }

  'low-end': { // More conservative targets for low-end devices
  cpu_multiplier: 1.5
 ...
 */
export function getAdjustedTargets(config: PerformanceTargetConfig): Record<string, PerformanceTarget> {
  const adjusted: Record<string, PerformanceTarget> = {};
  ...
  for (const [kpiId, target] of Object.entries(performanceTargets)) { let targetMultiplier = 1.0;
 ...
 */
export function getTargetsForUserSegment(segment: PerformanceTargetConfig['userSegment']): Record<string, PerformanceTarget> { const adjustments: Record<string, number> = {
  ...
  const adjusted: Record<string, PerformanceTarget> = {};
  for (const [kpiId, target] of Object.entries(performanceTargets)) { // For user experience metrics, all segments have the same expectations
    if (kpiId.startsWith('ux_')) {
      adjusted[kpiId] = target } else { adjusted[kpiId] = {
  ...target
 ...
 */
export function validatePerformanceTargets(currentMetrics: Record<string, number>)
...
>;
  ...
  let score = 100;
  for (const [kpiId, current] of Object.entries(currentMetrics)) { const target = targets[kpiId];
  if (!target) continue;
  ...
  severity: 'critical' }
      ...
      score -= 20;
 else if (current > target.warning) { violations.push({)
  kpiId,
  current,
  target: target.target,
  severity: 'warning' }
  ...
  return { passed: violations.filter(v => v.severity === 'critical').length === 0,
...
function generateTargetRecommendations(((
  ...
  if (config.environment === 'production') { recommendations.push('🎯 Production environment requires strict adherence to targets');
  ...
  */
  ...
  return { totalTargets: targets.length
