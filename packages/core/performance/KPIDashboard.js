...
export class KPIDashboard extends EventEmitter {
    ...
    constructor(monitoringService, baseline, config = {
    ...
    getDashboardMetrics() {
        ...
        for (const category of categoryNames) {
            ...
            const categoryStatus = kpiStatus.filter(k => categoryKPIs.some(kpi => kpi.id === k.kpiId));
            categories[category] = {
     ...
     */
    generateDashboardReport(type = 'summary', periodHours = 24) {
        const now = Date.now();
     ...
     */
    generateChartData(startTime, endTime) {
        const baselineHistory = this.baseline.getBaselineHistory();
        const periodBaselines = baselineHistory.filter(b => b.timestamp >= startTime && b.timestamp <= endTime);
     ...
     */
    calculateBaselineScore(baseline) {
        const scores = baseline.kpiSnapshots.map(snapshot => this.statusToScore(snapshot.status));
     ...
     */
    generateAlertTimelineData(startTime, endTime) {
     ...
     */
    calculateAverageScore(kpiStatus) {
     ...
     * Clear old reports
     */
    clearOldReports(retentionDays = 30) {
