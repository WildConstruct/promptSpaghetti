...
export interface PerformanceBudgetConfig { // Bundle Size Budgets
  bundles: {;
...
export class PerformanceBudgetManager extends EventEmitter { private config: PerformanceBudgetConfig;
  ...
  */
  ...
  timestamp
    ...
    // Limit history
    if (this.snapshots.length > this.maxSnapshotHistory) { this.snapshots = this.snapshots.slice(-this.maxSnapshotHistory);
  ...
  timestamp: Date.now() }
    ...
    // Vendor bundle size
    if (snapshot.bundles.vendor > this.config.bundles.vendor) { violations.push({)
  category: 'bundle'
  ...
  timestamp: Date.now() }
    ...
    // Total bundle size
    if (snapshot.bundles.total > this.config.bundles.total) { violations.push({)
  category: 'bundle'
  ...
  timestamp: Date.now() }
    ...
    if (oversizedChunks.length > 0) { violations.push({)
  category: 'bundle'
  metric: 'chunk-size'
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private checkRuntimeBudgets(snapshot: PerformanceSnapshot): BudgetViolation { const violations: BudgetViolation = [];
  ...
  timestamp: Date.now() }
    ...
    if (snapshot.runtime.lcp && snapshot.runtime.lcp > this.config.runtime.largestContentfulPaint) { violations.push({)
  category: 'runtime'
  ...
  timestamp: Date.now() }
    ...
    // First Input Delay
    if (snapshot.runtime.fid && snapshot.runtime.fid > this.config.runtime.firstInputDelay) { violations.push({)
  category: 'runtime'
  ...
  timestamp: Date.now() }
    ...
    if (snapshot.runtime.cls && snapshot.runtime.cls > this.config.runtime.cumulativeLayoutShift) { violations.push({)
  category: 'runtime'
  ...
  timestamp: Date.now() }
    ...
    // Time to Interactive
    if (snapshot.runtime.tti && snapshot.runtime.tti > this.config.runtime.timeToInteractive) { violations.push({)
  category: 'runtime'
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private checkApiBudgets(snapshot: PerformanceSnapshot): BudgetViolation { const violations: BudgetViolation = [];
  ...
  timestamp: Date.now() }
    ...
    if (snapshot.api.preview && snapshot.api.preview > this.config.api.preview) { violations.push({)
  category: 'api'
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private checkMemoryBudgets(snapshot: PerformanceSnapshot): BudgetViolation { const violations: BudgetViolation = [];
  // Peak heap usage
  ...
  timestamp: Date.now() }
    ...
    if (recentSnapshots.length >= 5) { const memoryTrend = this.calculateMemoryTrend(recentSnapshots);
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private checkNetworkBudgets(snapshot: PerformanceSnapshot): BudgetViolation { const violations: BudgetViolation = [];
  // Total request count
  ...
  timestamp: Date.now() }
    ...
    // Transfer size
    if (snapshot.network.transferSize > this.config.network.totalTransferSize) { violations.push({)
  category: 'network'
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private checkBuildBudgets(snapshot: PerformanceSnapshot): BudgetViolation { const violations: BudgetViolation = [];
  ...
  timestamp: Date.now() }
    ...
    return violations;
  private calculateSeverity(actual: number, budget: number): 'low' | 'medium' | 'high' | 'critical' { const ratio = actual / budget;
  ...
  private summarizeViolations(violations: BudgetViolation) { return {
  ...
  private generateRecommendations(violations: BudgetViolation): string { const recommendations = new Set<string>();
    violations.forEach(violation => {)
  violation.suggestions.forEach(suggestion => {)
  recommendations.add(suggestion) });
    });
    return Array.from(recommendations);
  private calculateMemoryTrend(snapshots: PerformanceSnapshot): number { if (snapshots.length < 2) return 0;
  ...
  */
   ...
   */
  clearHistory(): void { this.violations = [];
  this.snapshots = [];
