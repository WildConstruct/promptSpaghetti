...
export class WebVitalsIntegration extends EventEmitter {
   ...
   * Get metric by name
   */
  getMetric(name: string): EnhancedMetric | undefined { return this.metrics.get(name);
  /**
  * Get all metrics
  */
  ...
  */
  ...
  body: JSON.stringify(analyticsData);
      ...
      if (response.ok) { this.emit('analytics-sent', analyticsData);
        return true } else { console.warn('Failed to send Web Vitals analytics:', response.statusText);
  ...
  value: metric.value
    ...
    if (enhancedMetric.rating === 'poor') { this.emit('poor-performance', enhancedMetric);
  ...
  private calculateRating(metric: Metric): 'good' | 'needs-improvement' | 'poor' {
    ...
    return 'poor';
  private getConnectionInfo(): { type?: string; deviceMemory?: number; effectiveType?: string } { // Type assertion for experimental APIs
  ...
  private generateAnalytics(): WebVitalsAnalytics { const metrics = this.getAllMetrics();
  return {
  ...
  private createEmptyAnalytics(): WebVitalsAnalytics { return {
      sessionId: this.sessionId,
      timestamp: Date.now(),
      metrics: [],
      summary: { }
  ...
  private getMetricSummary(name: string): { value: number; rating: string } { const metric = this.getMetric(name);
  return {
  value: metric?.value || 0,
  rating: metric?.rating || 'good' }
};
  private getDeviceInfo() { if (typeof window === 'undefined') {
      return {
        userAgent: 'Node.js' }
        viewport: { width: 0, height: 0 },
        devicePixelRatio: 1,
  ...
  private getPageInfo() { if (typeof window === 'undefined') {
  return {
  url: '',
  referrer: '',
  title: '',
  ...
  private handleVisibilityChange(): void { if (document.visibilityState === 'hidden') {
...
if (typeof window !== 'undefined') { // Initialize after DOM is ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => webVitals.initialize()) } else {
