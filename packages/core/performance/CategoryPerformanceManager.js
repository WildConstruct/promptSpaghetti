...
export class CategoryPerformanceManager extends EventEmitter {
     ...
     */
    async applyOptimization(actionId) {
     ...
     */
    async forceOptimization(category) {
    ...
    optimizeCategories() {
        ...
        for (const [category, metrics] of this.categoryMetrics) {
            ...
            const autoApplyActions = actions.filter(action => action.expectedImpact.confidence > 0.8 &&
    ...
    generateOptimizationActions(category, metrics) {
        const actions = [];
        const categoryConfig = this.config.categories[category];
        ...
        if (utilizationRate > categoryConfig.scalingRules.scaleUpThreshold / 100) {
            actions.push({
                ...
                parameters: {
                    targetInstances: Math.min(categoryConfig.resourceLimits.maxConcurrentNodes * 1.5, categoryConfig.scalingRules.maxInstances)
                },
                expectedImpact: {
                    performanceGain: 30,
                    resourceCost: 40,
    ...
    calculateRecommendationPriority(metrics) {
    ...
    estimateOptimizationImpact(metrics, config) {
    ...
    executeScaleUp(action) {
        ...
        categoryConfig.resourceLimits.maxConcurrentNodes = Math.min(targetInstances, categoryConfig.scalingRules.maxInstances);
        return true;
    }
    executeScaleDown(action) {
        ...
        if (metrics && metrics.activeNodes < categoryConfig.resourceLimits.maxConcurrentNodes * 0.5) {
            categoryConfig.resourceLimits.maxConcurrentNodes = Math.max(Math.ceil(categoryConfig.resourceLimits.maxConcurrentNodes * 0.8), metrics.activeNodes + 2 // Keep some buffer
            );
            return true;
        }
        return false;
    }
    executeCacheOptimization(action) {
        const categoryConfig = this.config.categories[action.category];
        if (action.parameters.increaseCacheSize) {
            categoryConfig.cacheStrategy.maxSize = Math.min(categoryConfig.cacheStrategy.maxSize * 1.5, 10000 // Max cache size limit
            );
        }
        if (action.parameters.optimizeTTL) {
            categoryConfig.cacheStrategy.ttlMs = Math.min(categoryConfig.cacheStrategy.ttlMs * 1.2, 3600000 // 1 hour max
            );
        }
        return true;
    }
    executeThrottling(action) {
        const categoryConfig = this.config.categories[action.category];
        ...
        categoryConfig.resourceLimits.maxConcurrentNodes = Math.max(Math.ceil(originalLimit * reductionRate), 1);
