 ...
 */
export enum NodeSafetyLevel { SAFE = 'SAFE',
...
export interface ASTNodeWhitelistConfig { allowedNodeTypes: Set<string> }
...
export class ASTNodeWhitelistFilter { private config: ASTNodeWhitelistConfig;
  ...
  */
   ...
   */
  private validateNode(node: acorn.Node, blockedNodes: BlockedNodeInfo): void { if (!node || typeof node !== 'object') {
      return;
    this.nodeCount++;
    this.currentDepth++;
  ...
  position: node.loc?.start;
      ...
      return;
    // Check depth limit
    if (this.config.maxDepth && this.currentDepth > this.config.maxDepth) { blockedNodes.push({)
  nodeType: node.type
        safetyLevel: NodeSafetyLevel.DANGEROUS }
  ...
  position: node.loc?.start;
    ...
    if (this.config.dangerousNodeTypes.has(node.type)) { blockedNodes.push({)
  nodeType: node.type
        safetyLevel: NodeSafetyLevel.DANGEROUS }
  ...
  nodeCount: this.nodeCount }
      ...
      return;
    if (this.config.restrictedNodeTypes.has(node.type)) { blockedNodes.push({)
  nodeType: node.type
  ...
  position: node.loc?.start;
      ...
      return;
    if (!this.config.allowedNodeTypes.has(node.type)) { blockedNodes.push({)
  nodeType: node.type
        safetyLevel: NodeSafetyLevel.DANGEROUS }
   ...
   */
  private validateIdentifier(node: acorn.Node & { name: string }, blockedNodes: BlockedNodeInfo): void { const dangerousIdentifiers = [
  ...
  object: acorn.Node
    property: acorn.Node }
    computed: boolean ;
  }, blockedNodes: BlockedNodeInfo): void { // Check for dangerous property names
...
}, blockedNodes: BlockedNodeInfo): void { // Only allow calls to whitelisted functions in the evaluation context
   ...
   */
  private validateLiteral(node: acorn.Node & { value: Error, raw?: string }, blockedNodes: BlockedNodeInfo): void { // Check for dangerous string literals that might be used for code injection
   ...
   * Recursively validate child nodes
   */
  private validateChildNodes(node: acorn.Node, blockedNodes: BlockedNodeInfo): void { // Walk through all properties that might contain child nodes
      ...
      if (Array.isArray(value)) {
        for (const item of value) {
          if (item && typeof item === 'object' && item.type) {
            this.validateNode(item, blockedNodes) } else if (value && typeof value === 'object' && value.type) { this.validateNode(value, blockedNodes);
  /**
 ...
 */
export function createGeneralExpressionFilter(): ASTNodeWhitelistFilter { const config: ASTNodeWhitelistConfig = {,
  allowedNodeTypes: new Set([),
  'Literal',
  'Identifier',
  'BinaryExpression',
