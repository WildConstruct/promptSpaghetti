...
import { AdvancedRuntimeNode, AdvancedExecutionUtils, ValidationHelpers, SerializationHelpers } from '../advanced';
// Test implementation of AdvancedRuntimeNode
class TestAdvancedNode extends AdvancedRuntimeNode {
    value;
    constructor(id, value, config) {
        const defaultConfig = {
            deterministic: true,
            cacheable: true,
            stateful: false,
    ...
    serialize() {
        return SerializationHelpers.createAdvancedNodeData(this.id, 'TestAdvanced', this.config, { value: this.value });
    }
}
// Test implementation of stateful node
class TestStatefulNode extends AdvancedRuntimeNode {
    increment;
    constructor(id, increment = 1) {
    ...
    serialize() {
        return SerializationHelpers.createAdvancedNodeData(this.id, 'TestStateful', this.config, { increment: this.increment });
    }
}
describe('AdvancedRuntimeNode Foundation', () => {
    ...
    describe('SerializationHelpers', () => {
        it('should create advanced node data', () => {
            ...
            const data = SerializationHelpers.createAdvancedNodeData('test-id', 'TestType', config, { key: 'value' });
    ...
    describe('Seeded Random Number Generation', () => {
        it('should create deterministic RNG', () => {
            ...
            const rng1 = node.createSeededRNG(12345);
            const rng2 = node.createSeededRNG(12345);
        ...
        it('should create different sequences for different seeds', () => {
            const node = new TestAdvancedNode('test', 'value');
            const rng1 = node.createSeededRNG(12345);
            const rng2 = node.createSeededRNG(54321);
        ...
        it('should create node-specific sequences', () => {
            ...
            const rng1 = node1.createSeededRNG(12345);
            const rng2 = node2.createSeededRNG(12345);
