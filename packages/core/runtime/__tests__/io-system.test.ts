...
import { AdvancedIOHandler,
  ...
  ResolvedInputs
 from '../io-system';
import { ValidationHelpers } from '../advanced';
describe('Advanced I/O System', () => { describe('IOSpecBuilder', () => {
    ...
    it('should build multi-input specification', () => { const spec = IOSpecBuilder.createMultiInput(['Input A', 'Input B', 'Input C']);
    ...
    it('should build custom specification with builder pattern', () => { const spec = new IOSpecBuilder();
  ...
  describe('AdvancedIOHandler', () => { let ioHandler: AdvancedIOHandler;
  let spec: IOSpec;
  beforeEach(() => {
  ...
  ioHandler = new AdvancedIOHandler(spec) });
    describe('Input Validation', () => { it('should validate valid inputs', () => {
      ...
      it('should detect missing required inputs', () => { const inputs = new Map<string, any>([);
          ['count', 3] }
          ['mode', 'fast']
      ...
      it('should validate input constraints', () => { const inputs = new Map<string, any>([);
  ['title', 'Test Title'] }
  ...
  expect(result.errors).toContain('Value \'invalid\' is not in allowed values: fast, slow');
      ...
      it('should handle optional inputs with defaults', () => { const inputs = new Map<string, any>([);
          ['title', 'Test Title']
    ...
    describe('Input Resolution', () => { it('should resolve connected inputs', () => {
      ...
      it('should use defaults for missing optional inputs', () => { const connectedInputs = new Map<string, any>([);
          ['title', 'Title Only']
      ...
      it('should perform type coercion with warnings', () => { const connectedInputs = new Map<string, any>([);
  ['title', 'Valid Title']
  ...
  to: 'number' }
    ...
    describe('Output Validation', () => { it('should validate valid outputs', () => {
        ...
        expect(result.valid).toBe(true);
        expect(result.errors).toHaveLength(0) });
      it('should detect invalid output types', () => { const outputs = new Map<string, any>([);
  ...
  expect(result.errors).toContain('Invalid type for Result: expected string, got number') });
    ...
    describe('Type Coercion', () => { it('should coerce strings to numbers', () => {
      ...
      it('should coerce values to string arrays', () => { const arraySpec = new IOSpecBuilder();
  .addInput({)
  id: 'items'
  label: 'Items'
  ...
  required: true }
        ...
        const inputs = new Map<string, any>([['items', [1, 2, 3]]]);
        const resolved = handler.resolveInputs(inputs, 'test-node');
        expect(resolved.values.get('items')).toEqual(['1', '2', '3']);
      });
      it('should handle boolean coercion', () => { const boolSpec = new IOSpecBuilder();
  .addInput({)
  id: 'flag'
  label: 'Flag'
  dataType: 'boolean'
  ...
  id: 'text'
        ...
        // Valid length
        const validInputs = new Map<string, any>([['text', 'hello']]);
        const validResult = handler.validateInputs(validInputs);
        expect(validResult.valid).toBe(true);
        // Too short
        const shortInputs = new Map<string, any>([['text', 'hi']]);
        ...
        // Too long
        const longInputs = new Map<string, any>([['text', 'this is too long']]);
        const longResult = handler.validateInputs(longInputs);
        expect(longResult.valid).toBe(false);
        expect(longResult.errors).toContain('Length 16 is above maximum 10');
      });
      it('should validate pattern constraints', () => { const patternSpec = new IOSpecBuilder();
          .addInput({)
  id: 'email'
            label: 'Email'
            dataType: 'string'
            required: true }
            constraints: { pattern: '^[^@]+@[^@]+\\.[^@]+
        const validInputs = new Map<string, any>([['email', 'test@example.com']]);
        const validResult = handler.validateInputs(validInputs);
        expect(validResult.valid).toBe(true);
        // Invalid email
        const invalidInputs = new Map<string, any>([['email', 'not-an-email']]);
  ...
  id: 'text'
        ...
        // Valid text
        const validInputs = new Map<string, any>([['text', 'good text']]);
        const validResult = handler.validateInputs(validInputs);
        expect(validResult.valid).toBe(true);
        // Invalid text
        const invalidInputs = new Map<string, any>([['text', 'bad text']]);
        const invalidResult = handler.validateInputs(invalidInputs);
        expect(invalidResult.valid).toBe(false);
  ...
  describe('TypedInputs', () => { let typedInputs: TypedInputs;
    ...
    it('should get typed values correctly', () => { expect(typedInputs.getString('title')).toBe('Test Title');
    ...
    it('should return defaults for missing values', () => { expect(typedInputs.getString('missing', 'default')).toBe('default');
    ...
    it('should handle type coercion gracefully', () => { // When a number is stored as string, getString should convert it
      expect(typedInputs.getString('count')).toBe('42');
      // When a string is requested as number
      expect(typedInputs.getNumber('title')).toBeNaN() });
    it('should provide metadata access', () => { const titleMetadata = typedInputs.getMetadata('title');
      ...
      expect(countMetadata?.warnings).toContain('Type coerced from string to number') });
    it('should check for warnings', () => { expect(typedInputs.hasWarnings('title')).toBe(false);
  ...
  describe('Data Type Validation', () => {
    ...
    it('should validate all data types correctly', () => { const validInputs = new Map<string, any>([);
        ['str', 'hello']
        ['num', 42]
        ['bool', true]
        ['arr', [1, 'mixed', true]] }
    ...
    it('should detect invalid data types', () => { const invalidInputs = new Map<string, any>([);
