...
export class VideoAnalysisNode extends AdvancedRuntimeNode {
  ...
  private _estimateResolution(size: number, duration: number): { width: number; height: number } {
    // Estimate based on file size and duration
    if (duration <= 0) return { width: 1280, height: 720 };
...
export class VideoCompositionNode extends AdvancedRuntimeNode {
  ...
  async executeAdvanced(inputs: TypedInputs, context: AdvancedExecutionContext): Promise<NodeExecutionResult> { try {
      ...
      const textOverlays = inputs.get('text_overlays') as any || [];
