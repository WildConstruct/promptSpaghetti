...
describe('Epic 1 Integration Tests', () => {
  describe('Complete workflow tests', () => {
    ...
    it('should handle complex variable workflow', async () => {
      ...
      const firstName = new VariableNode('firstName',
        { name: 'firstName', defaultValue: 'John' },
        { mode: 'set' }
      );
      
      const lastName = new VariableNode('lastName',
        { name: 'lastName', defaultValue: 'Doe' },
        { mode: 'set' }
      );
      
      const age = new VariableNode('age',
        { name: 'age', defaultValue: 25 },
        { mode: 'set', variableType: 'number' }
      );
      
      // Use variables in template
      const template = new TextBlockNode('template',
    ...
    it('should handle variable get/set patterns', async () => {
      ...
      // Set a variable
      const setter = new VariableNode('setter',
        { name: 'counter', defaultValue: 0 },
        { mode: 'set', variableType: 'number' }
      );
      
      // Get the variable
      const getter1 = new VariableNode('getter1',
        { name: 'counter' },
        { mode: 'get' }
      );
      
      const getter2 = new VariableNode('getter2',
        { name: 'counter' },
        { mode: 'get' }
      );
      
      // Use in templates
      const template1 = new TextBlockNode('template1', 'First access: {{counter}}');
      const template2 = new TextBlockNode('template2', 'Second access: {{counter}}');
  ...
  describe('PSG format integration', () => {
    it('should load and execute PSG format', async () => {
      const psgData = {
        ...
        edges: [
          { id: 'e1', source: 'text1', target: 'output1' }
        ]
      };
  ...
  describe('Multi-seed execution', () => {
    it('should generate different outputs with different seeds', async () => {
      ...
      const results = await executeWithSeeds(graph, ['seed1', 'seed2', 'seed3', 'seed4', 'seed5']);
    ...
    it('should generate preview with statistics', async () => {
      ...
      const number = new VariableNode('number',
  ...
  describe('Error handling integration', () => {
    it('should handle missing variables gracefully', async () => {
      ...
      expect(result.stats.warnings.some(w => w.message.includes('Unknown variable'))).toBe(true);
    });
