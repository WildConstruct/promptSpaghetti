...
describe('PromptParser', () => {
  ...
  describe('Basic parsing', () => {
    it('should parse a simple sentence', () => {
      ...
      expect(result.segments[0].suggestedNodeType).toBe(Epic1NodeType.TextBlock);
    ...
    it('should parse a sentence with punctuation', () => {
      ...
      expect(result.segments).toHaveLength(1);
      expect(result.segments[0].text).toBe('A weary merchant in tattered robes.');
      expect(result.segments[0].endIndex).toBe(prompt.length);
    });
    ...
    it('should detect lists with "or"', () => {
      ...
      expect(result.segments).toHaveLength(1);
      expect(result.segments[0].suggestedNodeType).toBe(Epic1NodeType.WeightedChoice);
  ...
  describe('Complex parsing', () => {
    it('should parse the example from the story', () => {
      const prompt = 'A weary merchant in tattered robes, carrying scrolls or books or potions';
      const result = parser.parse(prompt);
...
Third line with choices: apples or oranges`;
      ...
      expect(lastTextSegment.suggestedNodeType).toBe(Epic1NodeType.WeightedChoice);
    });
