...
import { SequentialNode, createSequentialNode, createSequencePattern, LinearPattern, CyclicalPattern, RandomPattern, WeightedPattern, SequentialPresets } from '../Sequential.js';
...
describe('Sequential Node', () => {
    ...
    describe('Validation', () => {
        ...
        test('should warn about invalid sequence items', () => {
            const node = new SequentialNode('invalid-items', ['valid', null, undefined]);
            const result = node.validate();
            expect(result.valid).toBe(true); // Warning, not error
            expect(result.warnings.length).toBeGreaterThan(0);
        });
        test('should validate weighted pattern configuration', () => {
            const validWeightedNode = createSequentialNode('valid-weighted', ['a', 'b'], 'weighted', { weights: [1, 2] });
