...
describe('MarkovNode', () => {
  ...
  describe('StandardTransitionMatrix', () => {
    ...
    test('should validate transition matrix correctly', () => { // Valid matrix
      ...
      expect(() => { new StandardTransitionMatrix()
          ['A', 'B'] }
          { A: { B: -0.5 }, B: { A: 1.0 } }
        );
      }).toThrow();
    ...
    test('should handle edge cases in transitions', () => { const matrix = new StandardTransitionMatrix(;);
        ['lone'] }
        { lone: {} } // No transitions
      );
  ...
  describe('MarkovNode Core Functionality', () => { test('should initialize with first state', () => {
    ...
    test('should transition between states deterministically', () => { const node = createMarkovNode(;);
        'markov1',
        ['A', 'B'] }
        { A: { B: 1.0 }, B: { A: 1.0 } },
        'A'
    ...
    test('should maintain state across multiple executions', () => { const node = createMarkovNode(;);
        'markov1',
        ['first', 'second', 'third'] }
        {
          first: { second: 1.0 },
      ...
      for (let i = 0; i < 5; i++) { results.push(node.run(ctx));
      expect(results).toEqual(['first', 'second', 'third', 'third', 'third']) });
    test('should handle probabilistic transitions with deterministic seeds', () => { const node = createMarkovNode(;);
        'markov1',
        ['heads', 'tails'] }
        {
  ...
  variables: {},
      ...
      for (let i = 0; i < 10; i++) { results2.push(node2.run(ctx2));
      expect(results1).toEqual(results2) });
    test('should respect termination conditions', () => { const node = createMarkovNode(;);
        'markov1',
        ['active', 'terminated'] }
        {
  ...
  const results: string = [];
  for (let i = 0; i < 10; i++) { results.push(node.run(ctx));
  ...
  describe('MarkovNode Configuration', () => { test('should handle maxTransitions configuration', () => {
      const node = createMarkovNode(;);
        'markov1',
        ['A', 'B'] }
        { A: { B: 1.0 }, B: { A: 1.0 } },
    ...
    test('should handle termination states', () => { const node = createMarkovNode(;);
        'markov1',
    ...
    test('should handle loop detection', () => { const node = createMarkovNode(;);
      ...
      const results: string = [];
      for (let i = 0; i < 15; i++) { results.push(node.run(ctx));
  ...
  describe('MarkovNode State Management', () => { test('should track transition history', () => {
      const node = createMarkovNode(;);
        'markov1',
        ['X', 'Y', 'Z'] }
    ...
    test('should reset state correctly', () => { const node = createMarkovNode(;);
        'markov1',
        ['start', 'middle', 'end'] }
    ...
    test('should provide current state inspection', () => { const node = createMarkovNode(;);
        'markov1',
        ['alpha', 'beta'] }
  ...
  describe('MarkovNode Validation', () => { test('should validate node configuration', () => {
      ...
      const invalidNode = createMarkovNode(;);
        ...
        { A: { B: 1.0 }, B: { A: 1.0 } },
        'A',
        { maxTransitions: -5 } // Invalid config
      );
      expect(invalidNode.validate().valid).toBe(false);
    });
    test('should validate termination states', () => { const node = createMarkovNode(;);
        'markov1',
        ['valid', 'state'] }
        { valid: { state: 1.0 }, state: { valid: 1.0 } },
        'valid',
        { terminationStates: ['invalid'] } // Non-existent state
      );
  ...
  describe('MarkovNode Serialization', () => { test('should serialize node data correctly', () => {
      const node = createMarkovNode(;);
        'markov1',
        ['serialize', 'test'] }
  ...
  describe('MarkovPresets', () => { test('should create toggle preset correctly', () => {
    ...
    test('should create random walk preset correctly', () => { const matrix = MarkovPresets.randomWalk(['A', 'B', 'C']);
      expect(matrix.states).toEqual(['A', 'B', 'C']);
    ...
    test('should create absorbing preset correctly', () => { const matrix = MarkovPresets.absorbing(['normal', 'absorb1', 'absorb2'], ['absorb1', 'absorb2']);
  ...
  describe('MarkovNode Performance and Determinism', () => { test('should execute within performance expectations', () => {
      const node = createMarkovNode(;);
        'perf-test' }
    ...
    test('should produce deterministic results across multiple runs', () => { const createTestNode = () => createMarkovNode(;);
        'determinism-test',
        ['det1', 'det2', 'det3'] }
        {
  ...
  variables: {},
      ...
      const results2: string = [];
      for (let i = 0; i < 20; i++) { results2.push(node2.run(ctx2));
      expect(results1).toEqual(results2) });
  });
});