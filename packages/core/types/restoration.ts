...
export const RestorationAttemptSchema = z.object({ id: z.string().uuid(),
...
export const RestorationConflictSchema = z.object({ id: z.string().uuid()
...
export const RestorationOperationSchema = z.object({ id: z.string().uuid()
  restorationAttemptId: z.string().uuid()
...
export const RestorationPreviewSessionSchema = z.object({ id: z.string().uuid()
  projectId: z.string().uuid()
  sourceSnapshotId: z.string().uuid()
  targetSnapshotId: z.string().uuid().optional()
...
// Bookmark types
export const RestorationBookmarkSchema = z.object({ id: z.string().uuid()
  projectId: z.string().uuid()
...
// Configuration types
export const RestorationConfigSchema = z.object({ restorationType: RestorationTypeSchema
  restorationStrategy: RestorationStrategySchema
...
export const CreateRestorationAttemptRequestSchema = z.object({ projectId: z.string().uuid()
  sourceSnapshotId: z.string().uuid()
  targetSnapshotId: z.string().uuid().optional()
  config: RestorationConfigSchema }
});

export const RestorationPreviewRequestSchema = z.object({ projectId: z.string().uuid()
  sourceSnapshotId: z.string().uuid()
  targetSnapshotId: z.string().uuid().optional()
  config: RestorationConfigSchema }
});

export const ConflictResolutionRequestSchema = z.object({ restorationAttemptId: z.string().uuid()
  conflictId: z.string().uuid()
  resolutionStrategy: ResolutionStrategySchema
  resolvedValue: z.record(z.unknown()).optional() }
});

export const RestorationBookmarkRequestSchema = z.object({ projectId: z.string().uuid()
  name: z.string().max(255)
  description: z.string().optional()
  sourceSnapshotId: z.string().uuid()
  targetSnapshotId: z.string().uuid().optional()
...
// Response types
export const RestorationPreviewResponseSchema = z.object({ sessionId: z.string().uuid()
  preview: z.object({)
...
export const RestorationProgressResponseSchema = z.object({ restorationAttemptId: z.string().uuid()
  status: RestorationStatusSchema
  progressPercentage: z.number().int().min(0).max(100)
...
export const RestorationStatsResponseSchema = z.object({ totalAttempts: z.number().int()
...
export interface RestorationContext { projectId: string;
  userId: string;
  sourceSnapshot: any;
  targetSnapshot?: any;
  currentState: any;
...
export interface ConflictResolutionResult { conflictId: string;
  resolved: boolean;
  resolvedValue?: any;
...
export interface RestorationEvent { type: 'progress' | 'conflict' | 'completed' | 'failed' | 'cancelled' }
  restorationAttemptId: string;
  data: any;
  timestamp: Date;
...
export interface ConflictEvent { type: 'conflict_detected' | 'conflict_resolved' }
  conflictId: string;
  restorationAttemptId: string;
  data: any;
  timestamp: Date;
...
export const RestorationFilterSchema = z.object({ projectId: z.string().uuid().optional()
...
// Validation helpers
export const validateRestorationConfig = (config: unknown): RestorationConfig => { return RestorationConfigSchema.parse(config) };

export const validateRestorationAttempt = (attempt: unknown): RestorationAttempt => { return RestorationAttemptSchema.parse(attempt) };

export const validateConflictResolution = (resolution: unknown): ConflictResolutionRequest => { return ConflictResolutionRequestSchema.parse(resolution) };

// Constants
export const RESTORATION_DEFAULTS = { PREVIEW_EXPIRY_MINUTES: 30
...
export const CONFLICT_DESCRIPTIONS = { node_modified: 'Node properties have been modified in both versions'
...
export const RESOLUTION_STRATEGY_DESCRIPTIONS = { keep_source: 'Keep the value from the source snapshot'
