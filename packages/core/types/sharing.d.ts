...
export interface Collaborator extends UserInfo { role: SharePermission;
    addedAt: Date;
    permissions: string[];
    invitedBy: string;
    acceptedAt?: Date }
}
export interface SharingConfig { accessLevel: ShareAccessLevel;
...
export interface ShareSecurityConfig { dataClassification: 'public' | 'internal' | 'confidential' | 'restricted';
    ...
    accessControls: { ipWhitelist: string[];
...
export interface MergeConflict { path: string;
...
export interface ViewerInfo { id?: string;
    email?: string;
    name?: string;
...
export interface ConversionMetrics { viewToDownload: number;
...
export interface ShareAnalyticsResponse { success: boolean;
    analytics: ShareAnalytics;
    error?: string;
...
export interface SharingSystemConfig { maxShareDuration: number;
