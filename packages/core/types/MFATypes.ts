...
export enum MFAMethodType { TOTP = 'totp',
  EMAIL = 'email',
  SMS = 'sms'
...
export interface SMSVerification { id: string;
  userId: string;
  phoneNumber: string;
...
export interface RateLimitRule { action: string;           // 'totp_verify', 'email_send', 'sms_send';
...
export function isTOTPConfiguration(config: BaseMFAConfiguration): config is TOTPConfiguration { return config.methodType === MFAMethodType.TOTP;
