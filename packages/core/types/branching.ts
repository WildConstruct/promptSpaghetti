...
export const BranchTimelineResponseSchema = z.object({ timeline: z.array(z.object({)
  date: z.date()
...
export interface BranchContext { projectId: string;
  branchId: string;
  userId: string;
  userPermissions: { }
  canRead: boolean;
...
export interface MergeContext { mergeRequestId: string;
...
export interface BranchHierarchy { branch: ProjectBranch;
...
export interface BranchMetrics { commitsPerDay: Record<string, number>;
...
export const validateCreateBranchRequest = (request: unknown): CreateBranchRequest => { return CreateBranchRequestSchema.parse(request) };

export const validateUpdateBranchRequest = (request: unknown): UpdateBranchRequest => { return UpdateBranchRequestSchema.parse(request) };

export const validateCreateMergeRequestRequest = (request: unknown): CreateMergeRequestRequest => { return CreateMergeRequestRequestSchema.parse(request) };

export const validateMergeBranchRequest = (request: unknown): MergeBranchRequest => { return MergeBranchRequestSchema.parse(request) };

export const validateBranchFilter = (filter: unknown): BranchFilter => { return BranchFilterSchema.parse(filter) };

export const validateMergeRequestFilter = (filter: unknown): MergeRequestFilter => { return MergeRequestFilterSchema.parse(filter) };

// Constants
export const BRANCHING_DEFAULTS = { DEFAULT_BRANCH_TYPE: 'feature' as BranchType
...
export const BRANCH_TYPE_DESCRIPTIONS = { main: 'Main development branch'
...
export const BRANCH_STATUS_DESCRIPTIONS = { active: 'Active development branch'
  merged: 'Branch has been merged'
  abandoned: 'Branch has been abandoned'
  archived: 'Branch has been archived' }
 as const;

export const PROTECTION_LEVEL_DESCRIPTIONS = { none: 'No protection - anyone can push'
  protected: 'Protected - requires review'
  locked: 'Locked - only admins can push' }
 as const;

export const MERGE_STRATEGY_DESCRIPTIONS = { merge: 'Create a merge commit'
...
export interface BranchEvent { type: 'branch_created' | 'branch_updated' | 'branch_deleted' | 'branch_merged' }
  branchId: string;
  projectId: string;
  userId: string;
  data: any;
...
export interface MergeRequestEvent { type: 'merge_request_created' | 'merge_request_updated' | 'merge_request_merged' | 'merge_request_closed' }
...
export interface BranchNotification { type: 'branch_conflict' | 'merge_request_review' | 'branch_merged' | 'branch_updated' }
  title: string;
  message: string;
  branchId: string;
  projectId: string;
