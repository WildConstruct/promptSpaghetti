...
export interface NodeChange { id: string;
  type: string;
...
export interface EdgeChange { id: string;
  source: string;
  target: string;
  change_type: MatchType;
  old_properties?: Record<string, unknown>;
...
export class ComparisonError extends Error { constructor();
  message: string,
  public code: string,
...
export interface DiffSessionEvent { type: 'session_created' | 'session_updated' | 'session_expired' }
  session_id: string;
  user_id: string;
  timestamp: Date;
  data?: Record<string, unknown>;
