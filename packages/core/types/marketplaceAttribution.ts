 ...
 */
import { z } from 'zod';
import { ResourceTypeSchema,
  ChangeTypeSchema,
  AuthorTypeSchema,
  AggregationPeriodSchema,
  ChangeAttributionSchema }
...
export const TemplateAttributionSchema = z.object({ id: z.string().uuid(),
...
export const RevenueAttributionSchema = z.object({ id: z.string().uuid(),
  templateId: z.string().uuid(),
  ...
  attributionCalculatedAt: z.date(),
  attributionMethod: z.enum(['automated', 'manual_review', 'dispute_resolution']),
  calculatedBy: z.string().uuid().optional(),
  // Verification
  isVerified: z.boolean().default(false),
...
export const CreatorAttributionProfileSchema = z.object({ id: z.string().uuid(),
  userId: z.string().uuid(),
...
export const MarketplaceAttributionAnalyticsSchema = z.object({ id: z.string().uuid(),
...
export const MarketplaceChangeAttributionSchema = ChangeAttributionSchema.extend({ resourceType: MarketplaceResourceTypeSchema
...
export const MarketplaceAttributionFilterSchema = AttributionFilterSchema.extend({ resourceType: MarketplaceResourceTypeSchema.optional()
...
export const CreateTemplateAttributionRequestSchema = z.object({ templateId: z.string().uuid()
  templateVersionId: z.string().uuid().optional()
  primaryCreatorId: z.string().uuid()
  collaborators: z.array(z.object({)
  userId: z.string().uuid()
...
export const CreateAttributionClaimRequestSchema = z.object({ templateId: z.string().uuid()
  claimType: z.enum(['ownership', 'collaboration', 'derivation', 'inspiration'])
  claimDescription: z.string()
  evidenceUrls: z.array(z.string().url()).default([]) }
  metadata: z.record(z.unknown()).default({})
...
export const UpdateRevenueAttributionRequestSchema = z.object({ purchaseId: z.string().uuid()
...
export const TemplateAttributionResponseSchema = z.object({ attribution: TemplateAttributionSchema
...
export const CreatorDashboardResponseSchema = z.object({ profile: CreatorAttributionProfileSchema
...
export const MARKETPLACE_RESOURCE_TYPE_DESCRIPTIONS = { template: 'Marketplace template'
...
export const MARKETPLACE_CHANGE_TYPE_DESCRIPTIONS = { template_create: 'Template created'
...
export const MARKETPLACE_ATTRIBUTION_DEFAULTS = { DEFAULT_CREATOR_SHARE: 85, // 85% to creator
...
export const validateCreateTemplateAttributionRequest = (request: unknown): CreateTemplateAttributionRequest => { return CreateTemplateAttributionRequestSchema.parse(request) };

export const validateCreateAttributionClaimRequest = (request: unknown): CreateAttributionClaimRequest => { return CreateAttributionClaimRequestSchema.parse(request) };

export const validateUpdateRevenueAttributionRequest = (request: unknown): UpdateRevenueAttributionRequest => { return UpdateRevenueAttributionRequestSchema.parse(request) };

export const validateMarketplaceAttributionFilter = (filter: unknown): MarketplaceAttributionFilter => { return MarketplaceAttributionFilterSchema.parse(filter) };