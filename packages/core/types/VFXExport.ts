...
export interface VFXExportMetadata { // Export identification
  exportId: string;
...
export interface VFXPromptData { // Final generated prompt
  finalPrompt: string;
...
export interface VFXGraphStructure { // Node network
  nodes: VFXGraphNode;
...
export interface VFXExecutionData { // Randomization state
  randomization: { }
  masterSeed: number;
...
export interface VFXExtensions { // Future ControlNet integration
  controlNet?: { }
...
export interface VFXRenderingData { // Render resolution and format
  resolution: {;
  width: number;
  height: number;
  ...
  quality: { samples?: number;                 // Render samples/iterations,
...
export interface VFXCameraParams { fov?: number;                     // Field of view in degrees;
...
export interface VFXExporter { exportGraph();
  graph: any }
...
export interface UTDGMetadata { creationDate: string;
  lastModified: string;
  accuracy: number; // 0-1;
  complexity: 'simple' | 'moderate' | 'complex' }
...
export interface VFXMARSStructure { // [CAM] - Camera and cinematography metadata
  metadata: {;
  camera: {;
...
export interface VFXMARSInteraction { type: 'dialogue' | 'physical' | 'emotional' | 'spatial' | 'narrative';
...
export interface VFXHollywoodProtocol { version: '1.0' | '1.1' | '2.0';     // Protocol version;
...
export interface VFXHumanReadableLayer { // Executive summary for producers and directors
  executiveSummary: {;
...
export interface VFXControlNetTags { // Camera-related ControlNet metadata
  camera: { }
...
export interface VFXControlNetTag { // Tag identification
