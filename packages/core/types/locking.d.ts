...
export interface LockPolicy { id: string;
    workspace_id: string;
    name: string;
    description?: string;
...
export interface LockConflict { id: string;
    resource_id: string;
...
export interface LockQueue { id: string;
    resource_id: string;
    user_id: string;
    lock_type: string;
    priority: number;
...
export interface LockNotification { id: string;
    user_id: string;
    lock_id?: string;
    resource_id: string;
...
export interface LockingStatistics { total_locks: number;
    active_locks: number;
...
export interface LockingState { locks: WorkflowLock[];
...
export interface LockingActions { fetchLocks: (workspaceId: string) => Promise<void>;
    ...
    releaseLock: (lockId: string, userId: string) => Promise<{ success: boolean;
        error?: string }>;
    breakLock: (lockId: string, userId: string, justification?: string) => Promise<{ success: boolean;
        error?: string }>;
    fetchConflicts: (workspaceId: string, status?: string) => Promise<void>;
    resolveLockConflict: (conflictId: string, resolution: string, userId: string) => Promise<{ success: boolean;
        error?: string }>;
    fetchQueue: (workspaceId: string) => Promise<void>;
    removeFromQueue: (queueId: string, userId: string) => Promise<{ success: boolean;
        error?: string }>;
    ...
    updatePolicy: (workspaceId: string, policy: Partial<LockPolicy>) => Promise<{ success: boolean;
        error?: string }>;
    clearError: () => void;
