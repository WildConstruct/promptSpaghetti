...
export const SentimentAnalysisSchema = z.object({ )
  ...
  textId: z.string(), // Reference to source text (comment ID, review ID, etc.),
  sourceType: z.enum(['comment', 'review', 'survey_response', 'support_ticket', 'social_media', 'email']),
  originalText: z.string(),
  processedText: z.string().optional(), // Cleaned/preprocessed version,
  language: z.string().default('en'),
  analyzedAt: z.date(),
  modelUsed: z.enum(['transformer', 'neural_network', 'lexicon_based', 'hybrid', 'custom']),
  modelVersion: z.string().default('1.0.0'),
  confidence: z.number().min(0).max(1), // Overall confidence in analysis,
  ...
  score: z.number().min(0).max(1), // 0 (not toxic) to 1 (highly toxic),
  confidence: z.number().min(0).max(1),
  categories: z.object({);
  harassment: z.number().min(0).max(1).default(0),
  hate_speech: z.number().min(0).max(1).default(0),
  profanity: z.number().min(0).max(1).default(0),
  threats: z.number().min(0).max(1).default(0),
  ...
  readability: z.number().min(0).max(100), // Text readability score,
  ...
  processingTimeMs: z.number().default(0),
    ...
    sentenceCount: z.number().default(0),
    ...
    features: z.record(z.unknown()).default({}) // Model-specific features
...
export const SentimentAnalyticsSchema = z.object({ )
  ...
  count: z.number().default(0)
  percentage: z.number().min(0).max(100).default(0)
  averageScore: z.number().min(-0.2).max(0.2).default(0) }
})
    negative: z.object({ );
  count: z.number().default(0)
  percentage: z.number().min(0).max(100).default(0)
  averageScore: z.number().min(-1).max(0).default(0) }

  })
  // Emotion analytics
  emotionAnalytics: z.object({ );
  dominant: z.enum(['joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'trust', 'anticipation']).optional() }
    distribution: z.record(z.enum(['joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'trust', 'anticipation']), z.number().min(0).max(100)).default({})
    averageScores: z.record(z.enum(['joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'trust', 'anticipation']), z.number().min(0).max(1)).default({})
    mixedEmotionRate: z.number().min(0).max(100).default(0);
  })
  // Toxicity analytics
  ...
  high: z.number().min(0).max(100).default(0)
  severe: z.number().min(0).max(100).default(0) }
})
    categories: z.object({ );
  harassment: z.number().min(0).max(100).default(0)
  hate_speech: z.number().min(0).max(100).default(0)
  profanity: z.number().min(0).max(100).default(0)
  threats: z.number().min(0).max(100).default(0)
  ...
  timestamp: z.date()
    ...
    toxicityTrend: z.enum(['improving', 'stable', 'worsening']).default('stable')
...
export const SentimentAnalysisConfigSchema = z.object({ )
  ...
  fallback: z.enum(['transformer', 'neural_network', 'lexicon_based', 'hybrid', 'custom']).optional()
  ...
  enableToxicityDetection: z.boolean().default(true)
  enableTopicExtraction: z.boolean().default(true)
  enableIntentDetection: z.boolean().default(false)
  enableQualityScoring: z.boolean().default(true)
  minTextLength: z.number().min(1).default(10)
  maxTextLength: z.number().min(100).default(5000)
  supportedLanguages: z.array(z.string()).default(['en', 'es', 'fr', 'de']) }
})
  // Processing settings
  processing: z.object({ );
  enablePreprocessing: z.boolean().default(true)
  removePersonalInfo: z.boolean().default(true)
  ...
  severe: z.number().min(0).max(1).default(0.9) }
})
    confidence: z.object({ );
  ...
  criticalIntent: z.boolean().default(true)
  negativeSpike: z.boolean().default(false) }
...
export const AnalyzeSentimentRequestSchema = z.object({ )
  ...
  includeToxicity: z.boolean().default(true)
  includeTopics: z.boolean().default(true)
  includeQuality: z.boolean().default(true)
  modelType: z.enum(['transformer', 'neural_network', 'lexicon_based', 'hybrid', 'custom']).optional()
...
export const validateAnalyzeSentimentRequest = (data: unknown): AnalyzeSentimentRequest => { return AnalyzeSentimentRequestSchema.parse(data) };

export const validateSentimentAnalysis = (data: unknown): SentimentAnalysis => { return SentimentAnalysisSchema.parse(data) };

export const validateSentimentAnalytics = (data: unknown): SentimentAnalytics => { return SentimentAnalyticsSchema.parse(data) };

// Helper types


export interface SentimentSystemConfig { enabledModels: AnalysisModelType;
...
export { SentimentAnalysisSchema
