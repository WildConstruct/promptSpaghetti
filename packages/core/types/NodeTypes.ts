...
// Type guards
export function isRuntimeNodeType(type: string): type is RuntimeNodeType { return ['WeightedChoice', 'Concat', 'Output', 'Include', 'SetVariable', 'GetVariable'].includes(type) }

export function isUINodeType(type: string): type is UINodeType { return ['Subject', 'Connector', 'Attribute', 'Action'].includes(type) }

// Node data factory functions
export function createBaseNodeData(id: string, label: string): BaseNodeData { return {
  id,
  label,
  variations: [],
  category: 'general',
...
export function createWeightedChoiceNodeData(id: string, label: string = 'Weighted Choice'): WeightedChoiceNodeData { return {
...
export function createConcatNodeData(id: string, label: string = 'Concat'): ConcatNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'Concat',
  separator: ' ',
  joinMode: 'space',
...
export function createOutputNodeData(id: string, label: string = 'Output'): OutputNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'Output',
  template: '',
  format: 'text',
...
export function createIncludeNodeData(id: string, label: string = 'Include'): IncludeNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'Include',
  name: '',
...
export function createSetVariableNodeData(id: string, label: string = 'Set Variable'): SetVariableNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'SetVariable',
  variableName: '',
  value: '',
...
export function createGetVariableNodeData(id: string, label: string = 'Get Variable'): GetVariableNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'GetVariable',
  variableName: '',
  defaultValue: '',
...
export function createSubjectNodeData(id: string, label: string = 'Subject'): SubjectNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'Subject',
...
export function createActionNodeData(id: string, label: string = 'Action'): ActionNodeData { return {
  ...createBaseNodeData(id, label),
  type: 'Action',
  actionType: 'verb',
  tense: 'present',
...
export function createNodeData(type: NodeType, id: string, label?: string): NodeData { switch (type) {
  case 'WeightedChoice':,
...
export function serializeForRuntime(nodeData: NodeData): RuntimeNodeData | null { if (!isRuntimeNodeType(nodeData.type)) {
  ...
  switch (nodeData.type) { case 'WeightedChoice':,
  return {
  ...base,
...
export function deserializeFromRuntime(runtimeData: RuntimeNodeData): NodeData | null { if (!isRuntimeNodeType(runtimeData.type)) {
    return null }
  ...
  case 'Include':
    return { ...createIncludeNodeData(id),
  name: runtimeData.name || '' }
};

  case 'SetVariable':
    return { ...createSetVariableNodeData(id),
  variableName: runtimeData.key || '',
  value: runtimeData.value || '' }
};

  case 'GetVariable':
    return { ...createGetVariableNodeData(id),
  variableName: runtimeData.key || '' }
};
...
export function validateNodeData(nodeData: Partial<NodeData>): string { const errors: string = [];
  if (!nodeData.id) {
  errors.push('Node ID is required') }

  if (!nodeData.type) { errors.push('Node type is required') }

  if (!nodeData.label) { errors.push('Node label is required') }

  // Type-specific validation
  if (nodeData.type === 'WeightedChoice') { const data = nodeData as Partial<WeightedChoiceNodeData>;
    if (!data.choices || data.choices.length === 0) {
      errors.push('WeightedChoice nodes must have at least one choice') }
    if (data.choices && data.weights && data.choices.length !== data.weights.length) { errors.push('WeightedChoice nodes must have matching choices and weights arrays') }


  if (nodeData.type === 'Include') { const data = nodeData as Partial<IncludeNodeData>;
    if (!data.name) {
