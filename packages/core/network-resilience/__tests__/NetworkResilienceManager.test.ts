...
// Mock WebSocket
class MockWebSocket { static CONNECTING = 0;
  static OPEN = 1;
  static CLOSING = 2;
  static CLOSED = 3;
  ...
  setTimeout(() => { this.readyState = MockWebSocket.OPEN;
  if (this.onopen) {
  this.onopen(new Event('open')) }, 10);
  send(data: string) {
    // Mock sending data
  ...
  addEventListener(type: string, listener: EventListener) { if (type === 'message' && typeof listener === 'function') {
...
describe('NetworkResilienceManager', () => { let manager: NetworkResilienceManager;
  beforeEach(() => {
  ...
  describe('Initialization', () => { test('should initialize successfully', async () => {
  const events: any = [];
  ...
  expect(events[0].userId).toBe('test-user') });
    test('should prevent double initialization', async () => { await manager.initialize('test-doc', 'test-user');
  ...
  describe('Connection Management', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should connect to WebSocket server', async () => { const connectionEvents: any = [];
  ...
  event.newState === ConnectionState.CONNECTING
    ...
    test('should handle connection failure', async () => { // Mock failed WebSocket connection
  const OriginalWebSocket = (global as any).WebSocket;
  (global as any).WebSocket = class extends MockWebSocket {
  constructor(url: string) { }
  super(url);
  setTimeout(() => { if (this.onerror) {
  this.onerror(new Event('error')) }, 10);
      };
      try { await manager.connect('ws://invalid-url');
  fail('Should have thrown an error') } catch (error) { expect(error).toBeDefined();
      (global as any).WebSocket = OriginalWebSocket });
    test('should disconnect gracefully', async () => { await manager.connect('ws://localhost:8000');
  ...
  describe('Operation Queuing', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should queue operations', () => { const operationId = manager.queueOperation({)
  type: 'graph_update' }
    ...
    test('should emit queued operation events', () => { const queuedEvents: any = [];
  ...
  type: 'presence_update' }
    ...
    test('should reject operations when disabled', () => { manager.setEnabled(false);
      expect(() => {
        manager.queueOperation({)
  type: 'graph_update' }
          payload: { nodeId: 'test' },
    ...
    test('should clear queue', () => { manager.queueOperation({)
  type: 'graph_update' }
  ...
  describe('Status and Metrics', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should provide network status', () => { const status = manager.getStatus();
  ...
  metrics: expect.any(Object) }
    ...
    test('should track metrics', () => { const metrics = manager.getMetrics();
  ...
  conflicts: expect.any(Number) }
    ...
    test('should update metrics over time', (done) => { const metricsEvents: any = [];
  ...
  type: 'graph_update' }
      ...
      setTimeout(() => { expect(metricsEvents.length).toBeGreaterThan(0);
        done() }, 150);
    });
  });
  describe('Synchronization', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should force synchronization', async () => { const syncEvents: any = [];
  ...
  expect(syncEvents).toHaveLength(1) });
    ...
    test('should handle sync errors gracefully', async () => { // This would require mocking the sync recovery to fail
      // For now, just ensure no errors are thrown
      try {
        await manager.forceSync() } catch (error) { // Sync errors should be handled gracefully
        expect(error).toBeDefined() });
  });
  describe('Enable/Disable', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should enable and disable network resilience', () => { const enabledEvents: any = [];
  ...
  expect(enabledEvents).toContain(true) });
    test('should stop reconnection when disabled', () => { // Start a connection attempt
  ...
  describe('State Export', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should export complete state', () => { manager.queueOperation({)
  type: 'graph_update' }
        payload: { nodeId: 'test' },
        priority: 'high',
        requiresOrder: false,
  ...
  describe('Event Handling', () => { beforeEach(async () => {
      await manager.initialize('test-doc', 'test-user') });
    test('should handle WebSocket messages', async () => { await manager.connect('ws://localhost:8000');
  ...
  type: 'graph_update' }
    ...
    test('should handle authentication responses', async () => { await manager.connect('ws://localhost:8000', 'test-token');
  ...
  type: 'auth_response' }
    ...
    test('should handle malformed messages gracefully', async () => { await manager.connect('ws://localhost:8000');
  ...
  data: 'invalid json' }
        ...
        expect(() => { mockWs.websocket.onmessage(malformedEvent) }).not.toThrow();
    });
  });
  describe('Cleanup', () => { test('should cleanup resources properly', async () => {
  await manager.initialize('test-doc', 'test-user');
  await manager.connect('ws://localhost:8000');
  const spy = jest.spyOn(manager, 'removeAllListeners');
  manager.cleanup();
  expect(spy).toHaveBeenCalled() });
    test('should handle cleanup when not initialized', () => { // Should not throw error
      expect(() => {
        manager.cleanup() }).not.toThrow();
    });
  });
});