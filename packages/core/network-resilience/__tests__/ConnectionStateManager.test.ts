...
describe('ConnectionStateManager', () => { let manager: ConnectionStateManager;
  beforeEach(() => {
  ...
  describe('State Management', () => { test('should initialize with disconnected state', () => {
      expect(manager.getState()).toBe(ConnectionState.DISCONNECTED);
      expect(manager.getQuality()).toBe(ConnectionQuality.UNKNOWN);
      expect(manager.isOnline()).toBe(false) });
    test('should update connection state', () => { const stateChanges: any = [];
  ...
  expect(manager.isOnline()).toBe(true) });
    test('should not emit duplicate state changes', () => { const stateChanges: any = [];
  manager.on('state_changed', (event) => stateChanges.push(event));
  manager.setState(ConnectionState.CONNECTED);
  manager.setState(ConnectionState.CONNECTED); // Duplicate
  expect(stateChanges).toHaveLength(1) });
    test('should track state history', () => { manager.setState(ConnectionState.CONNECTING);
      manager.setState(ConnectionState.CONNECTED);
  ...
  describe('Connection Quality', () => { test('should update quality based on metrics', () => {
  ...
  measurementCount: 1 }
    ...
    test('should calculate quality thresholds correctly', () => { // Good quality
  ...
  measurementCount: 1 }
    ...
    test('should use exponential moving average for metrics', () => { // First measurement
  manager.updateMetrics({)
  latency: 100,
  packetLoss: 0.1,
  jitter: 20,
  ...
  describe('Connection Testing', () => { test('should perform successful connection test', async () => {
  ...
  status: 200 }
    ...
    test('should handle failed connection test', async () => { (fetch as jest.Mock).mockRejectedValue(new Error('Network error'));
      const metrics = await manager.testConnection();
      expect(metrics.latency).toBeGreaterThan(0);
      expect(metrics.packetLoss).toBe(1) });
    test('should handle response with error status', async () => { (fetch as jest.Mock).mockResolvedValue({)
  ok: false,
  status: 500 }
});
      const metrics = await manager.testConnection();
  ...
  describe('Stability Checks', () => { test('should determine connection stability', () => {
  manager.setState(ConnectionState.CONNECTED);
  ...
  measurementCount: 1 }
    ...
    test('should consider offline as unstable', () => { manager.setState(ConnectionState.OFFLINE);
      expect(manager.isStable()).toBe(false) });
  });
  describe('Statistics and Metrics', () => { test('should provide connection statistics', () => {
  manager.setState(ConnectionState.CONNECTED);
  manager.updateMetrics({)
  latency: 100,
  packetLoss: 0.05,
  ...
  measurementCount: 5 }
    ...
    test('should track reconnection attempts', () => { // Start from disconnected state
      manager.setState(ConnectionState.DISCONNECTED);
  ...
  describe('Network Information API', () => { test('should capture network information when available', () => {
  ...
  describe('Browser Events', () => {
    test('should handle online/offline events', () => {
      ...
      setTimeout(() => { expect(stateChanges.some(change => )
  ...
  describe('Cleanup and Reset', () => { test('should reset state and metrics', () => {
  manager.setState(ConnectionState.CONNECTED);
  manager.updateMetrics({)
  latency: 100,
  packetLoss: 0.05,
  ...
  measurementCount: 5 }
    ...
    test('should cleanup resources properly', () => { const spy = jest.spyOn(manager, 'removeAllListeners');
      manager.cleanup();
