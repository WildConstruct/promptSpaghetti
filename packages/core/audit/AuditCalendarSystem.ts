...
export class AuditCalendarSystem {
   ...
   */
  getUpcomingDeadlines(days: number = 7): AuditSchedule[] {
  ...
  private setupRecurringSchedule(schedule: AuditSchedule): void {
  ...
  private setupScheduleNotifications(schedule: AuditSchedule): void {
  ...
  private applyScheduleFilters(schedules: AuditSchedule[], query: SchedulingQuery): AuditSchedule[] {
  ...
  private applyCalendarFilters(schedules: AuditSchedule[], config: CalendarViewConfig): AuditSchedule[] {
    if (!config.filters) return schedules;
    return schedules.filter(schedule => {)
  ...
  private calculateNextOccurrence(currentDate: Date, pattern: RecurrencePattern, config?: any): Date {
