...
import { AuditCalendarSystem, AuditActivityType, SchedulePriority, ScheduleStatus, RecurrencePattern, NotificationTiming, createAuditSchedule, queryAuditSchedules, generateCalendarView } from '../AuditCalendarSystem';
describe('AuditCalendarSystem', () => {
    ...
    describe('Recurring Schedules', () => {
        it('should generate recurring daily instances', () => {
            ...
            const instances = calendarSystem.generateRecurringInstances(recurringSchedule.id, new Date('2025-08-21T23:59:59Z'));
            expect(instances.length).toBeGreaterThan(0);
            expect(instances.length).toBeLessThanOrEqual(30);
            // Verify instances are properly spaced (daily)
            if (instances.length > 1) {
                const daysDiff = Math.floor((instances[1].scheduled_start.getTime() - instances[0].scheduled_start.getTime()) / (1000 * 60 * 60 * 24));
        ...
        it('should generate recurring weekly instances', () => {
            ...
            const instances = calendarSystem.generateRecurringInstances(recurringSchedule.id, new Date('2025-10-25T23:59:59Z'));
            expect(instances.length).toBeGreaterThan(0);
