 ...
 */
export class AuditManagementAPI { private auditSystem = auditManagementSystem;
  /**
...
> { try {
  ...
  max_risk_score: validatedRequest.max_risk_score }
      ...
      // Format response
      const response = { events: result.events.map(this.formatAuditEventForAPI),
  pagination: {,
  page: result.page,
 ...
 as AuditQuery : undefined;
      ...
      if (validatedRequest.metrics.includes('anomaly_detection')) { analytics.anomaly_detection = this.auditSystem.detectAnomalousPatterns();
   ...
   */
  async detectAnomalies(timeWindow?: number): Promise<{ success: boolean;
  patterns?: any;
  error?: string }> { try {
   ...
   */
  async getSystemHealth(): Promise<{ success: boolean;
  health?: any;
  error?: string }> { try {
      const health = {
        status: 'healthy',
  ...
  // Helper methods
  private formatAuditEventForAPI(event: AuditEvent): any { return {
  id: event.id,
  ...
  private generateTrendAnalysis(analytics: any): any { return {
