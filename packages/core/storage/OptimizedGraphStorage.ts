...
interface GraphIndexes { // Type-based indexes
  ...
  */
  ...
  if (!existing) return false;
    ...
    if (updates.type && updates.type !== existing.type) { this.updateNodeIndexes(existing, 'remove');
  ...
  */
  ...
  cacheHitRatio?: number };
    ...
    for (const [type, nodeSet] of this.indexes.nodesByType) { nodeTypes[type] = nodeSet.size;
    const totalConnections = Array.from(this.nodeMap.keys());
      .reduce((sum, nodeId) => {
        return sum + this.getIncomingEdges(nodeId).length + this.getOutgoingEdges(nodeId).length }, 0);
    return { nodeCount: this.nodeMap.size
   ...
   */
  toCompressedFormat(): CompressedGraphData | { nodes: Node; edges: Edge } { const nodeCount = this.nodeMap.size;
  ...
  edges: this.edges }
    ...
    try { // Compress node and edge data
 ...
 catch (error) { console.warn('Compression failed, falling back to regular format:', error);
  return {
  nodes: this.nodes
  edges: this.edges }
};
   ...
   */
  static fromStorageFormat(data: any): OptimizedGraphStorage { if (data.compressed && data.format_version === '2.0.0') {
  ...
  */
  ...
  try { const nodes = OptimizedGraphStorage.decompressNodes(compressed.node_data);
  const edges = OptimizedGraphStorage.decompressEdges(compressed.edge_data);
  return new OptimizedGraphStorage(nodes, edges) } catch (error) { console.error('Failed to decompress graph data:', error);
  ...
  */
  ...
  private loadData(nodes: Node, edges: Edge): void { // Clear existing data
    this.nodeMap.clear();
    this.edgeMap.clear();
    this.indexes = this.createEmptyIndexes();
    // Load nodes
  ...
  this.edgeMap.set(edge.id, edge);
      this.updateEdgeIndexes(edge, 'add') });
    this.invalidateCache();
  private updateNodeIndexes(node: Node, operation: 'add' | 'remove'): void { const nodeType = (node.data as NodeData)?.type || node.type as NodeType;
    if (operation === 'add') {
      // Add to type index
      ...
      this.updateConnectivityIndexes(node.id) } else { // Remove from type index
  ...
  if (operation === 'add') { // Update source/target indexes
  ...
  this.updateConnectivityIndexes(edge.target) } else { // Remove from indexes
  ...
  if (!hasIncoming && !hasOutgoing) { this.indexes.isolatedNodes.add(nodeId);
  this.indexes.leafNodes.delete(nodeId);
  this.indexes.rootNodes.delete(nodeId) } else if (!hasIncoming) { this.indexes.rootNodes.add(nodeId);
      this.indexes.leafNodes.delete(nodeId);
      this.indexes.isolatedNodes.delete(nodeId) } else if (!hasOutgoing) { this.indexes.leafNodes.add(nodeId);
      this.indexes.rootNodes.delete(nodeId);
      this.indexes.isolatedNodes.delete(nodeId) } else { this.indexes.leafNodes.delete(nodeId);
  this.indexes.rootNodes.delete(nodeId);
