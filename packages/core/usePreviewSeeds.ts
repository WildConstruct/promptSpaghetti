...
export function usePreviewSeeds() { const [loading, setLoading] = useState(false);
  ...
  const updateVarianceMetrics = useCallback((newResults: PreviewResult) => { if (newResults.length >= 2) {
      const metrics = varianceAnalysisService.analyzeVariance(newResults as PreviewResult);
      setVarianceMetrics(metrics) } else { setVarianceMetrics(null) }, []);
  const runPreview = useCallback(async (graph: unknown, specificSeed?: number, resultIndex?: number) => { // Store the graph for regeneration purposes
  ...
  locked: results[index]?.locked || false
      ...
      if (executionTimes.length > 0) { const averageTime = Math.round(executionTimes.reduce((sum, time) => sum + time, 0) / executionTimes.length);
        setPerformanceStats({)
  totalTime }
          averageTime
        });
 ...
 catch (err: unknown) { if (err?.name !== 'AbortError') {
        setError(err?.message ?? 'Unknown error') } finally { setLoading(false) }, [results]);
  ...
  const regenerateSpecificResult = useCallback(async (graph: unknown, seed: number, resultIndex: number) => { // Check if result is locked
  ...
  seedStart: Math.floor(Math.random() * 10000) // Use new random seed }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `HTTP ${response.status}: ${response.statusText}`);}
      const data = await response.json();
      if (data.results && data.results.length > 0) { const newResult = data.results[0];
  ...
  seed: newResult.seed
        ...
        setResults(prev => { const updated = [...prev];
 ...
 finally { setRegeneratingResults(prev => prev.filter(i => i !== resultIndex)) }, [results]);
  // Epic 8.5 Task 3: Result management actions
  const lockResult = useCallback((index: number, note?: string) => { setResults(prev => {
  const updated = [...prev];
  if (updated[index]) {
  updated[index] = {
  ...updated[index]
  ...
  const unlockResult = useCallback((index: number) => { setResults(prev => {
  const updated = [...prev];
  if (updated[index]) {
  updated[index] = {
  ...updated[index]
  ...
  const regenerateResult = useCallback(async (index: number) => { if (!lastGraphRef.current || results[index]?.locked) {
      return }
 ...
 */
function generateDebugInfo(executionPath: ExecutionPath) { const debugInfo = ExecutionPathAnalyzer.generateDebugInfo(executionPath);
  return {
