...
export class StepSequencingSystem extends EventEmitter {
    ...
    async startExecution(sequenceId, context = {}, options = {}) {
        const sequence = this.sequences.get(sequenceId);
        if (!sequence) {
            throw new Error(`Sequence ${sequenceId} not found`);
        }
    ...
    // Rollback operations
    async rollbackExecution(executionId, toStepId) {
        const execution = this.executions.get(executionId);
        if (!execution) {
            throw new Error(`Execution ${executionId} not found`);
        }
        ...
        if (toStepId) {
            ...
            execution.completedSteps = execution.completedSteps.filter(id => !rollbackEntries.some(entry => entry.stepId === id));
        }
        else {
    ...
    listSequences(filters) {
        ...
        if (filters) {
            ...
            if (filters.tags?.length) {
                sequences = sequences.filter(seq => filters.tags.some(tag => seq.metadata.tags.includes(tag)));
            }
    ...
    listExecutions(filters) {
        ...
        if (filters) {
            ...
            if (filters.dateRange) {
                executions = executions.filter(exec => exec.startTime >= filters.dateRange.start &&
    ...
    async validateStepDependencies(execution, step) {
        for (const depId of step.dependencies) {
    ...
    async evaluateStepConditions(execution, step) {
    ...
    async evaluateCondition(execution, condition) {
        let result = false;
    ...
    async validateStepCompletion(execution, step) {
    ...
    addToRollbackStack(execution, step) {
        if (!step.rollback?.enabled)
            return;
        const entry = {
            stepId: step.id,
            timestamp: new Date(),
    ...
    updateStepPerformance(step, startTime, status, attempts) {
        const duration = performance.now() - startTime;
        const record = {
            timestamp: new Date(),
            duration,
    ...
    updateExecutionProgress(execution, stepId, status, duration) {
        if (status === 'completed') {
    ...
    getNextStep(sequence, currentStepId, status) {
    ...
    registerBuiltInHandlers() {
        ...
        this.registerActionHandler('function', async (params, context) => {
