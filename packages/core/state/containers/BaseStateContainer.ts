...
export interface StateSubscriber<T> { (state: T, prevState: T): void;
...
export interface StateMiddleware<T> { name: string;
  order: number;
...
export abstract class BaseStateContainer<T> extends EventEmitter {
  ...
  constructor(config: Partial<StateContainerConfig> = {}) { super();
  this.config = {
  ...
  source: changeInfo?.source || 'local' }
      ...
      if (this.config.enablePerformanceProfiling) { this.profileStateUpdate(newState, prevState, change) } else { this.applyStateUpdate(newState, prevState, change) } finally { this.isUpdating = false;
  ...
  public restoreSnapshot(snapshotId: string): void {
    ...
    this.setState(() => snapshot.state, { type: 'RESTORE'
  source: 'system' }
});
  public clearHistory(): void { this.history = [this.history[this.history.length - 1]]; // Keep current state
  ...
  change
      ...
      for (const middleware of this.middleware) { if (middleware.afterUpdate) {
          await middleware.afterUpdate(this.state, prevState, change) } catch (error) { // Run error middleware
  for (const middleware of this.middleware) {
  ...
  subscriber(currentState, previousState) } catch (error) { console.error('Error in state subscriber:', error);
  ...
  protected async persistState(): Promise<void> {
    ...
    try {
      ...
      const stateData = { state: this.state
  timestamp: Date.now()
  domain: this.getDomainName()
  version: '1.0' }
};
  ...
  protected async loadPersistedState(): Promise<void> {
    ...
    try {
      ...
      if (this.config.enableValidation) { const validation = this.validateState(persistedData.state);
  if (!validation.valid) {
  ...
  prevState: T
    ...
    try {
      ...
      await globalPerformanceProfiler.sampleOperation()
        this.getDomainName()
        change.type
        async () => { await this.applyStateUpdate(newState, prevState, change) }
        { changeId: change.id
 ...
 catch (error) { console.warn('DevTools recording failed:', error);
 ...
 = {}): void {
  ...
  public async getDevToolsInfo(): Promise<any> {
    ...
    try {
      ...
      if (this.config.enablePerformanceProfiling) {
        const { globalPerformanceProfiler } = await import('../devtools/PerformanceProfiler');
        devToolsInfo.performance = { isActive: globalPerformanceProfiler.isProfilingActive()
...
export class StateValidationError extends Error { constructor(message: string, public errors: ValidationError) {
  super(message);
  ...
  if (!dependencies) { return selector(state);
