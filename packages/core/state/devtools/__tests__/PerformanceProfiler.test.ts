...
describe('PerformanceProfiler', () => { let profiler: PerformanceProfiler;
  beforeEach(() => {
  profiler = new PerformanceProfiler();
  jest.clearAllMocks();
  mockPerformance.now.mockImplementation(() => Date.now()) });
  afterEach(() => { profiler.removeAllListeners();
  ...
  cacheHitRate: 0.9 }
    ...
    it('should start in inactive state', () => { expect(profiler.isProfilingActive()).toBe(false);
      expect(profiler.getCurrentProfileId()).toBeNull();
      expect(profiler.getActiveProfiles()).toHaveLength(0) });
  });
  describe('Profile Management', () => { it('should start performance profiles', () => {
  ...
  domains: ['test-domain'] }
    ...
    it('should stop performance profiles', () => { const profileId = profiler.startProfile('Stop Test');
      expect(profiler.isProfilingActive()).toBe(true);
    ...
    it('should pause and resume profiles', () => { const profileId = profiler.startProfile('Pause Test');
  ...
  profile: expect.any(Object) }
    ...
    it('should auto-stop profiles after duration', (done) => {
      ...
      setTimeout(() => { expect(completeHandler).toHaveBeenCalled();
        expect(profiler.isProfilingActive()).toBe(false);
        done() }, 100);
    });
  });
  describe('Operation Sampling', () => { beforeEach(() => {
  ...
  errorCount: 0 }
    ...
    it('should handle operation errors', () => { const testOperation = () => {
        throw new Error('Test error') };
      expect(() => { profiler.sampleOperation('test-domain', 'error-operation', testOperation) }).toThrow('Test error');
      // Error count should be tracked even when operation throws
    });
    it('should not sample when profiling is inactive', () => { profiler.stopProfile(profiler.getCurrentProfileId()!);
  ...
  describe('Memory Profiling', () => { it('should take memory snapshots', () => {
  ...
  leaks: expect.any(Array) }
    ...
    it('should maintain snapshot history', () => { for (let i = 0; i < 5; i++) {
    ...
    it('should limit snapshot history size', () => { // Take more than the limit (100)
      for (let i = 0; i < 150; i++) {
        profiler.takeMemorySnapshot();
      const snapshots = profiler.getMemorySnapshots();
  ...
  componentName: 'TestComponent',
    ...
    it('should track render profiles by component', () => { profiler.profileRender('ComponentA', () => 'A');
    ...
    it('should create alerts for slow renders', () => { const alertHandler = jest.fn();
  ...
  describe('Alert System', () => { beforeEach(() => {
      profiler.startProfile('Alert Test') });
    it('should create performance alerts', () => { const alertHandler = jest.fn();
  profiler.on('alertCreated', alertHandler);
  ...
  value: 200 }
    ...
    it('should clear alerts', () => { // Create an alert first
      mockPerformance.now
  ...
  describe('Configuration', () => { it('should update configuration', () => {
  ...
  config: expect.objectContaining(newConfig) }
    ...
    it('should respect updated thresholds', () => { // Update threshold to be very low
  ...
  describe('Data Export', () => {
    ...
    it('should export profile data', () => { const profileId = profiler.getCurrentProfileId()!;
    ...
    it('should handle export of non-existent profiles', () => { const exportData = profiler.exportProfile('nonexistent');
  ...
  describe('Performance Analysis', () => {
    ...
    it('should identify performance patterns', () => { const profileId = profiler.getCurrentProfileId()!;
      const profile = profiler.stopProfile(profileId);
      expect(profile).toBeDefined();
    ...
    it('should generate performance summary', () => { const profileId = profiler.getCurrentProfileId()!;
      const profile = profiler.stopProfile(profileId);
    ...
    it('should provide performance recommendations', () => { const profileId = profiler.getCurrentProfileId()!;
      const profile = profiler.stopProfile(profileId);
  ...
  describe('Global Instance', () => { it('should provide a global PerformanceProfiler instance', () => {
  ...
  describe('Edge Cases and Error Handling', () => { it('should handle operations when profiling is disabled', () => {
      expect(() => {
        profiler.sampleOperation('test', 'no-profile', () => 'result') }).not.toThrow();
    });
    it('should handle memory profiling when performance.memory is unavailable', () => { const originalMemory = global.performance.memory;
      delete (global.performance as any).memory;
      expect(() => {
        profiler.takeMemorySnapshot() }).not.toThrow();
      (global.performance as any).memory = originalMemory;
    });
    it('should handle profile operations on non-existent profiles', () => { expect(profiler.pauseProfile('nonexistent')).toBe(false);
      expect(profiler.resumeProfile('nonexistent')).toBe(false);
      expect(profiler.stopProfile('nonexistent')).toBeNull() });
    it('should handle concurrent profile operations', () => { const profile1Id = profiler.startProfile('Profile 1');
