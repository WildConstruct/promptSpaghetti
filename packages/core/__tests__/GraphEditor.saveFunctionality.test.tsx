...
describe('GraphEditor Save Functionality', () => { const mockGraphStoreState = {
    nodes: []
    edges: []
    currentProject: null
  ...
  beforeEach(() => { mockUseGraphStore.mockReturnValue(mockGraphStoreState as unknown);
    jest.clearAllMocks() });
  ...
  test('opens load dialog when load button is clicked', async () => {
    ...
    await waitFor(() => { expect(screen.getByTestId('load-dialog')).toBeInTheDocument() });
  });
