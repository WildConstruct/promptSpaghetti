...
// Mock useExport hook
const mockUseExport = {};
jest.mock('../hooks/useExport', () => ({ useExport: () => mockUseExport }
}));
...
const defaultProps = {};
describe('AdvancedExportTemplateManager', () => { beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('Basic Rendering', () => {
    it('renders without crashing when visible', async () => {
      ...
      await waitFor(() => { expect(screen.getByText('📋 Advanced Export Templates')).toBeInTheDocument() });
    });
    ...
    it('displays template count in header', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText(/2 templates • 2 filtered/)).toBeInTheDocument() });
    });
  });
  describe('Template Loading', () => {
    it('loads templates on mount', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(mockUseExport.getTemplates).toHaveBeenCalledWith({)
  sortBy: 'name'
  sortOrder: 'asc' }
});
      });
    ...
    it('loads template statistics', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(mockUseExport.getTemplateStats).toHaveBeenCalledWith('template-1');
  ...
  describe('Template Display', () => {
    it('displays template cards with correct information', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('JSON Standard Export')).toBeInTheDocument();
    ...
    it('shows template statistics', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('📊 145 uses')).toBeInTheDocument();
  ...
  describe('Navigation Tabs', () => {
    it('shows all navigation tabs', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('🔍 Browse Templates')).toBeInTheDocument();
    ...
    it('hides sharing tabs when disabled', async () => {
      ...
      await waitFor(() => { expect(screen.queryByText('🌐 Shared Templates')).not.toBeInTheDocument();
  ...
  describe('Template Actions', () => {
    ...
    it('handles template sharing toggle', async () => {
      mockUseExport.shareTemplate.mockResolvedValue(undefined as unknown);
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('🌐 Make Public')).toBeInTheDocument() });
    ...
    it('handles template deletion for non-system templates', async () => {
      ...
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('🗑️ Delete')).toBeInTheDocument() });
      const deleteButton = screen.getByText('🗑️ Delete');
      await userEvent.click(deleteButton);
      await waitFor(() => { expect(mockUseExport.deleteTemplate).toHaveBeenCalledWith('template-1') });
      // Restore window.confirm
      window.confirm = originalConfirm;
    });
    it('does not show delete button for system templates', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('VFX Pipeline Export')).toBeInTheDocument() });
  ...
  describe('Error Handling', () => {
    ...
    it('handles missing template stats gracefully', async () => {
      ...
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { // Should still render templates even if stats fail
  ...
  describe('Accessibility', () => {
    ...
    it('handles keyboard navigation', async () => {
      render(<AdvancedExportTemplateManager {...defaultProps} />);
      await waitFor(() => { const firstButton = screen.getAllByRole('button')[0];
        firstButton.focus();
