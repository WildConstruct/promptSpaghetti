...
describe('Safe Math Context', () => { beforeEach(() => {
    MathFunctionAuditor.clearAuditLog() });
  afterEach(() => { MathFunctionAuditor.clearAuditLog() });
  describe('createSafeMathContext', () => { it('should create Math object with only safe functions', () => {
    ...
    it('should create Math object without prototype', () => { const safeMath = createSafeMathContext();
    ...
    it('should have read-only constants', () => { const safeMath = createSafeMathContext();
      // Try to modify constants
      expect(() => {
        (safeMath as any).PI = 3 }).toThrow();
      expect(() => { (safeMath as any).E = 2 }).toThrow();
      // Values should remain unchanged
      expect(safeMath.PI).toBe(Math.PI);
      expect(safeMath.E).toBe(Math.E);
    });
  });
  describe('Safe Math Functions', () => { let safeMath: any;
  beforeEach(() => {
  safeMath = createSafeMathContext() });
    describe('Math.min', () => { it('should work with valid numbers', () => {
        expect(safeMath.min(1, 2, 3)).toBe(1);
        expect(safeMath.min(-5, 0, 5)).toBe(-5);
        expect(safeMath.min(42)).toBe(42) });
      it('should validate inputs', () => { expect(() => safeMath.min()).toThrow('requires at least one argument');
      ...
      it('should enforce range limits', () => { expect(() => safeMath.min(Number.MAX_VALUE)).toThrow('out of safe range');
        expect(() => safeMath.min(-Number.MAX_VALUE)).toThrow('out of safe range') });
    });
    describe('Math.max', () => { it('should work with valid numbers', () => {
        expect(safeMath.max(1, 2, 3)).toBe(3);
        expect(safeMath.max(-5, 0, 5)).toBe(5);
        expect(safeMath.max(42)).toBe(42) });
      it('should validate inputs', () => { expect(() => safeMath.max()).toThrow('requires at least one argument');
  ...
  describe('createAuditedSafeMathContext', () => { it('should audit allowed function access', () => {
    ...
    it('should block and audit dangerous function access', () => { const safeMath = createAuditedSafeMathContext('test');
    ...
    it('should prevent modification of Math object', () => { const safeMath = createAuditedSafeMathContext('test');
      expect(() => {
        safeMath.min = () => 0 }).toThrow('Cannot modify Math object');
      expect(() => {
        safeMath.newFunc = () => {};
      }).toThrow('Cannot modify Math object');
      expect(() => { delete safeMath.min }).toThrow('Cannot delete Math properties');
    });
    it('should access constants without auditing', () => { const safeMath = createAuditedSafeMathContext('test');
  ...
  describe('validateMathFunctionCall', () => { it('should validate safe functions', () => {
      SAFE_MATH_FUNCTIONS.forEach(func => {)
  expect(validateMathFunctionCall(func)).toBe(true) });
    });
    it('should reject blocked functions', () => { BLOCKED_MATH_FUNCTIONS.forEach(func => {)
  expect(validateMathFunctionCall(func)).toBe(false) });
    });
    it('should reject unknown functions', () => { expect(validateMathFunctionCall('unknownFunc')).toBe(false);
      expect(validateMathFunctionCall('eval')).toBe(false);
      expect(validateMathFunctionCall('constructor')).toBe(false) });
  });
  describe('MathFunctionAuditor', () => { it('should track audit events', () => {
    ...
    it('should provide summary statistics', () => { MathFunctionAuditor.logAttempt('min', true, 'Allowed', 'test');
  MathFunctionAuditor.logAttempt('min', true, 'Allowed', 'test');
  MathFunctionAuditor.logAttempt('random', false, 'Blocked', 'test');
  MathFunctionAuditor.logAttempt('random', false, 'Blocked', 'test');
  MathFunctionAuditor.logAttempt('random', false, 'Blocked', 'test');
  ...
  expect(summary['random:blocked']).toBe(3) });
    it('should limit audit log size', () => { // Generate many events
      for (let i = 0; i < 1100; i++) {
        MathFunctionAuditor.logAttempt('test', true, 'Test', 'test');
      const log = MathFunctionAuditor.getAuditLog();
      expect(log.length).toBeLessThanOrEqual(1000) });
  });
  describe('Utility Functions', () => { describe('isInSafeRange', () => {
      ...
      it('should reject unsafe numbers', () => { expect(isInSafeRange(NaN)).toBe(false);
    ...
    describe('safeNumberCoercion', () => { it('should handle valid numbers', () => {
        expect(safeNumberCoercion(42)).toBe(42);
        expect(safeNumberCoercion(-42.5)).toBe(-42.5);
        expect(safeNumberCoercion(0)).toBe(0) });
      it('should coerce booleans', () => { expect(safeNumberCoercion(true)).toBe(1);
        expect(safeNumberCoercion(false)).toBe(0) });
      it('should parse valid strings', () => { expect(safeNumberCoercion('42')).toBe(42);
        expect(safeNumberCoercion('-42.5')).toBe(-42.5);
        expect(safeNumberCoercion('0')).toBe(0);
      ...
      it('should reject unsafe numbers', () => { expect(() => safeNumberCoercion(NaN)).toThrow('received NaN');
  ...
  describe('Constants', () => { it('should define numeric limits', () => {
    ...
    it('should have complete function lists', () => { expect(SAFE_MATH_FUNCTIONS).toContain('min');
