...
describe('RateLimiter', () => { let rateLimiter: RateLimiter;
  ...
  afterEach(() => { mockStore.destroy() });
  describe('Basic Rate Limiting', () => {
    test('should allow requests within limit', async () => {
      const context: RateLimitContext = { ip: '192.168.1.1' };
      for (let i = 0; i < 10; i++) { const result = await rateLimiter.checkLimit(context);
        expect(result.allowed).toBe(true);
        expect(result.info.remainingRequests).toBe(10 - i - 1) });
    test('should block requests exceeding limit', async () => {
      const context: RateLimitContext = { ip: '192.168.1.1' };
      // Make 10 allowed requests
      for (let i = 0; i < 10; i++) { await rateLimiter.checkLimit(context);
    ...
    test('should reset after window expires', async () => { // Use shorter window for testing
  ...
  store: mockStore }
    ...
    test('should handle different IPs independently', async () => {
      ...
      for (let i = 0; i < 10; i++) { await rateLimiter.checkLimit(context1);
  ...
  describe('Key Generators', () => {
    ...
    test('should generate composite keys', () => { const context: RateLimitContext = {,
  ip: '192.168.1.1',
  userId: 'user123',
  method: 'POST',
  path: '/api/data' }
  ...
  describe('Configuration', () => { test('should validate configuration', () => {
  expect(() => {
  new RateLimiter({)
  ...
  maxRequests: 0 // Invalid }
    ...
    test('should use default configuration', () => { const defaultLimiter = new RateLimiter();
  ...
  describe('Headers', () => { test('should include standard rate limit headers', async () => {
  ...
  store: mockStore }
    ...
    test('should include legacy headers when enabled', async () => { const limiter = new RateLimiter({)
  windowMs: 60000,
  maxRequests: 10,
  legacyHeaders: true,
  store: mockStore }
  ...
  describe('Presets', () => { test('should create authentication preset', () => {
    ...
    test('should create API preset', () => { const config = RateLimitPresets.api();
      expect(config.windowMs).toBe(15 * 60 * 1000);
      expect(config.maxRequests).toBe(1000) });
    test('should create from preset', () => { const limiter = RateLimitUtils.fromPreset('passwordReset', {)
  store: mockStore }
});
  ...
  describe('Error Handling', () => { test('should handle store errors gracefully', async () => {
      const failingStore = {
...
describe('MemoryRateLimitStore', () => {
  ...
  describe('Basic Operations', () => {
    ...
    test('should return null for non-existent keys', async () => { const result = await store.get('non-existent');
      expect(result).toBeNull() });
    test('should increment counters', async () => { const result1 = await store.increment('counter', 60000);
      expect(result1.hits).toBe(1);
      const result2 = await store.increment('counter', 60000);
      expect(result2.hits).toBe(2) });
    test('should reset counters', async () => { await store.increment('counter', 60000);
    ...
    test('should provide statistics', async () => { await store.increment('key1', 60000);
...
describe('RedisRateLimitStore', () => { let mockRedis: MockRedisClient;
  ...
  hits: data.hits }
    ...
    test('should increment with Redis', async () => { const result1 = await store.increment('counter', 60000);
      expect(result1.hits).toBe(1);
      const result2 = await store.increment('counter', 60000);
      expect(result2.hits).toBe(2) });
    test('should reset with Redis', async () => { await store.increment('counter', 60000);
      await store.reset('counter');
      const result = await store.get('counter');
      expect(result).toBeNull() });
    test('should get statistics', async () => { await store.increment('key1', 60000);
      await store.increment('key2', 60000);
      const stats = await store.getStats();
      expect(stats.redisAvailable).toBe(true);
      expect(stats.totalKeys).toBeGreaterThanOrEqual(0) });
  });
  describe('Fallback Behavior', () => { test('should fallback to memory when Redis fails', async () => {
      // Mock Redis failure
...
describe('RateLimitConfigurationManager', () => { let configManager: RateLimitConfigurationManager;
  ...
  describe('Profile Management', () => { test('should load default profiles', () => {
    ...
    test('should set active profile', () => { const success = configManager.setActiveProfile('development');
      expect(success).toBe(true);
      const activeProfile = configManager.getActiveProfile();
      expect(activeProfile?.id).toBe('development') });
    test('should add custom profile', () => { const customProfile = RateLimitConfigurationPresets.createWebAppProfile();
  ...
  describe('Rule Matching', () => { test('should find matching rules', () => {
    ...
    test('should respect rule priority', () => { configManager.setActiveProfile('production');
      const requestContext = {
        ip: '192.168.1.1',
        method: 'GET',
        path: '/api/users' }
      ...
      for (let i = 1; i < matchingRules.length; i++) { expect(matchingRules[i-1].priority).toBeGreaterThanOrEqual(matchingRules[i].priority) });
    test('should create rate limit config from rule', () => { const rule: DynamicRateLimitRule = {,
  id: 'test-rule',
        name: 'Test Rule',
  ...
  describe('Rule Management', () => { beforeEach(() => {
      configManager.setActiveProfile('development') });
    test('should add rule to active profile', () => { const newRule: DynamicRateLimitRule = {,
  id: 'new-rule',
        name: 'New Rule',
    ...
    test('should update existing rule', () => { const activeProfile = configManager.getActiveProfile();
  ...
  maxRequests: 20 }
    ...
    test('should remove rule', () => { const activeProfile = configManager.getActiveProfile();
  ...
  describe('Import/Export', () => { test('should export configuration', () => {
      const exported = configManager.exportConfiguration();
      expect(exported).toBeDefined();
      const parsed = JSON.parse(exported);
      expect(parsed.context).toEqual(context);
      expect(parsed.profiles).toBeDefined() });
    test('should import configuration', () => { const customProfile = RateLimitConfigurationPresets.createAPIProfile();
  const config = {
  context,
...
describe('ConditionEvaluator', () => { describe('IP Conditions', () => {
  ...
  value: '192.168.1.1' }
    ...
    test('should evaluate IP contains', () => { const condition = {
  type: 'ip' as const,
  ...
  value: '192.168' }
    ...
    test('should evaluate IP in list', () => { const condition = {
  type: 'ip' as const,
  ...
  describe('Endpoint Conditions', () => { test('should evaluate endpoint patterns', () => {
  const condition = {
  ...
  value: '/api' }
    ...
    test('should evaluate endpoint regex', () => { const condition = {
  type: 'endpoint' as const,
  ...
  describe('Header Conditions', () => { test('should evaluate header values', () => {
  const condition = {
  ...
  value: 'Bearer token123' }
    ...
    test('should handle missing headers', () => { const condition = {
  type: 'header' as const,
  ...
  describe('Negation', () => { test('should handle negated conditions', () => {
  const condition = {
  type: 'ip' as const,
  operator: 'equals' as const,
  value: '192.168.1.1',
...
describe('Integration Tests', () => { test('should work with configuration manager and rate limiter', async () => {
  const context: ConfigurationContext = {,
  environment: 'production' }
};
    const configManager = new RateLimitConfigurationManager(context);
    configManager.setActiveProfile('production');
    const requestContext = { ip: '192.168.1.1',
  method: 'POST',
  path: '/api/auth/login' }
};
    const matchingRules = configManager.findMatchingRules(requestContext);
...
describe('Utility Functions', () => { test('should extract IP from headers', () => {
  const headers = {
  ...
  test('should format rate limit info for logging', () => { const info = {
  totalHits: 5,
  totalHitsInWindow: 5,
  remainingRequests: 5,
  resetTime: new Date(),
