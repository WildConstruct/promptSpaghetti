...
class TestErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            return _jsxs("div", { "data-testid": "error-boundary", children: ["Error: ", this.state.error?.message] });
        }
...
describe('InspectorSidebar - Enhanced Testing', () => {
    ...
    describe('Error Handling', () => {
        it('handles null node gracefully', () => {
            expect(() => {
                render(_jsx(InspectorSidebar, { node: null, schema: schema, onChange: () => { } }));
            }).not.toThrow();
        });
        it('handles undefined node gracefully', () => {
            expect(() => {
                render(_jsx(InspectorSidebar, { node: undefined, schema: schema, onChange: () => { } }));
            }).not.toThrow();
        });
        it('handles null schema gracefully', () => {
            expect(() => {
                render(_jsx(InspectorSidebar, { node: node, schema: null, onChange: () => { } }));
            }).not.toThrow();
        });
        it('handles malformed node data', () => {
            ...
            expect(() => {
                render(_jsx(InspectorSidebar, { node: malformedNode, schema: schema, onChange: () => { } }));
            }).not.toThrow();
        });
        it('handles invalid schema gracefully', () => {
            const invalidSchema = 'not a schema';
            expect(() => {
                render(_jsx(InspectorSidebar, { node: node, schema: invalidSchema, onChange: () => { } }));
            }).not.toThrow();
        });
        it('handles onChange errors gracefully', async () => {
            ...
            render(_jsx(TestErrorBoundary, { onError: onError, children: _jsx(InspectorSidebar, { node: node, schema: schema, onChange: handleChange }) }));
            const input = screen.getByLabelText('label');
        ...
        it('handles schema validation errors', async () => {
            ...
            const user = userEvent.setup();
            render(_jsx(InspectorSidebar, { node: node, schema: strictSchema, onChange: () => { } }));
    ...
    describe('Performance Concerns', () => {
        it('debounces rapid input changes', async () => {
            const handleChange = jest.fn();
            const user = userEvent.setup();
            render(_jsx(InspectorSidebar, { node: node, schema: schema, onChange: handleChange, debounceMs: 300 }));
            const input = screen.getByLabelText('label');
            // Rapid typing
        ...
        it('efficiently renders complex schemas', () => {
            ...
            const startTime = performance.now();
            render(_jsx(InspectorSidebar, { node: complexNode, schema: complexSchema, onChange: () => { } }));
            const endTime = performance.now();
    ...
    describe('Accessibility & Code Quality', () => {
        ...
        it('follows TypeScript strict mode', () => {
            ...
            expect(() => {
                render(_jsx(InspectorSidebar, { ...strictProps }));
            }).not.toThrow();
        });
        it('cleans up properly on unmount', () => {
            const { unmount } = render(_jsx(InspectorSidebar, { node: node, schema: schema, onChange: () => { } }));
    ...
    describe('Data Type Handling', () => {
        ...
        it('handles boolean inputs correctly', async () => {
            ...
            const handleChange = jest.fn();
            const user = userEvent.setup();
            render(_jsx(InspectorSidebar, { node: booleanNode, schema: booleanSchema, onChange: handleChange }));
        ...
        it('handles optional fields correctly', () => {
            ...
            expect(() => {
                render(_jsx(InspectorSidebar, { node: nodeWithoutOptional, schema: schema, onChange: () => { } }));
            }).not.toThrow();
    ...
    describe('Edge Cases', () => {
        it('handles extremely long string values', async () => {
            ...
            render(_jsx(InspectorSidebar, { node: nodeWithLongValue, schema: schema, onChange: () => { } }));
            const input = screen.getByLabelText('label');
        ...
        it('handles special characters and unicode', () => {
            ...
            render(_jsx(InspectorSidebar, { node: nodeWithSpecialChars, schema: schema, onChange: () => { } }));
            const input = screen.getByLabelText('label');
        ...
        it('handles circular references in node data', () => {
            ...
            expect(() => {
                render(_jsx(InspectorSidebar, { node: circularNode, schema: schema, onChange: () => { } }));
            }).not.toThrow();
        });
    });
});
