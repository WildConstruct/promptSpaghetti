...
jest.mock('reactflow', () => ({
    ...
    ReactFlow: ({ children, nodes, edges, onNodesChange, onEdgesChange, onConnect, ...props }) => (_jsxs("div", { "data-testid": "reactflow-wrapper", ...props, children: [_jsx("div", { "data-testid": "reactflow-nodes", children: nodes?.map((node) => (_jsx("div", { "data-testid": `flow-node-${node.id}`, "data-node-type": node.type, style: { position: 'absolute', left: node.position.x, top: node.position.y }, children: node.data?.label || node.type }, node.id))) }), _jsx("div", { "data-testid": "reactflow-edges", children: edges?.map((edge) => (_jsx("div", { "data-testid": `flow-edge-${edge.id}`, "data-source": edge.source, "data-target": edge.target }, edge.id))) }), children] })),
    Handle: ({ type, position, id, ...props }) => (_jsx("div", { "data-testid": `handle-${type}-${id}`, "data-handle-type": type, "data-handle-position": position, ...props })),
    Position: {
        Top: 'top',
        Right: 'right',
        Bottom: 'bottom',
...
describe('GraphEditor Scene Rendering Integration', () => {
    const createMockGraph = (nodeCount = 3, edgeCount = 2) => {
    ...
    it('handles empty graph rendering', () => {
        render(_jsx(GraphEditor, { initialNodes: [], initialEdges: [] }));
    ...
    it('renders large graphs efficiently', async () => {
        ...
        const { container } = render(_jsx(GraphEditor, { initialNodes: nodes, initialEdges: edges }));
        await waitFor(() => {
    ...
    it('renders nodes at correct positions', () => {
        ...
        render(_jsx(GraphEditor, { initialNodes: nodes, initialEdges: [] }));
    ...
    it('renders minimap when enabled', () => {
        ...
        render(_jsx(GraphEditor, { initialNodes: nodes, initialEdges: edges, showMinimap: true }));
    ...
    it('handles edge creation and deletion rendering', () => {
        ...
        render(_jsx(GraphEditor, { initialNodes: nodes, initialEdges: [], onConnect: onEdgeCreate, onEdgesDelete: onEdgeDelete }));
