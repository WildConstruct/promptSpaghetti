...
describe('SecurityValidation Class', () => { describe('validateSafeString', () => {
  ...
  expect(SecurityValidation.validateSafeString(str)).toBe(true) });
    ...
    it('should reject strings with dangerous patterns', () => { const dangerousStrings = [
        'eval(')
        'Function(')
        'constructor',
        'prototype',
  ...
  expect(SecurityValidation.validateSafeString(str)).toBe(false) });
    ...
    it('should handle non-string inputs', () => { const nonStrings: any = [
        null,
        undefined,
        123,
        true,
  ...
  expect(SecurityValidation.validateSafeString(input)).toBe(false) });
    ...
    it('should block many dangerous patterns (case insensitive)', () => { const blockedPatterns = [
        'EVAL(')
        'Eval(')
        'eVaL(')
        'CONSTRUCTOR',
  ...
  describe('validateSafeExpression', () => { it('should accept safe expressions', () => {
  ...
  expect(SecurityValidation.validateSafeExpression(expr)).toBe(false) });
    ...
    it('should enforce expression length limits', () => { const longExpression = 'a'.repeat(501);
    ...
    it('should handle empty and whitespace expressions', () => { expect(SecurityValidation.validateSafeExpression('')).toBe(true);
  ...
  describe('validateVariableName', () => { it('should accept valid variable names', () => {
      const validNames = [
        'a',
        'x',
        'var1',
  ...
  expect(SecurityValidation.validateVariableName(name)).toBe(true) });
    ...
    it('should reject invalid variable names', () => { const invalidNames = [
        '', // empty
        ' ', // space
  ...
  expect(SecurityValidation.validateVariableName(name)).toBe(false) });
    ...
    it('should reject reserved keywords and dangerous patterns', () => { const reservedKeywords = [
        '__proto__'
        'constructor'
        'prototype'
        '__defineGetter__'
  ...
  expect(SecurityValidation.validateVariableName(keyword)).toBe(false) });
    ...
    it('should enforce exact 64 character limit', () => { expect(SecurityValidation.validateVariableName('a'.repeat(64))).toBe(true);
  ...
  expect(SecurityValidation.validateVariableName(input)).toBe(false) });
    ...
    it('should validate alphanumeric pattern', () => { expect(VARIABLE_NAME_PATTERN.test('abc123')).toBe(true);
  ...
  describe('validateSafePropertyKey', () => { it('should accept safe property keys', () => {
      const safeKeys = [
        'a'
        'property'
        'user_name'
  ...
  expect(SecurityValidation.validateSafePropertyKey(key)).toBe(true) });
    ...
    it('should reject dangerous property keys', () => { const dangerousKeys = [
        '', // empty
        '__proto__'
        'constructor'
        'prototype'
  ...
  expect(SecurityValidation.validateSafePropertyKey(key)).toBe(false) });
    ...
    it('should check for prototype pollution patterns', () => { const pollutionAttempts = [
        '__proto__'
        'constructor'
        'prototype'
  ...
  expect(SecurityValidation.validateSafePropertyKey(key)).toBe(false) });
    ...
    it('should enforce 64 character limit', () => { expect(SecurityValidation.validateSafePropertyKey('a'.repeat(64))).toBe(true);
  ...
  describe('validateSafeValue', () => { it('should accept safe primitive values', () => {
      const safeValues = [
        null
        undefined
        true
  ...
  expect(SecurityValidation.validateSafeValue(value)).toBe(true) });
    ...
    it('should reject unsafe string values', () => { const unsafeStrings = [
        'eval(')
        'constructor'
        '__proto__'
        'Function(')
  ...
  expect(SecurityValidation.validateSafeValue(str)).toBe(false) });
    ...
    it('should reject unsafe numeric values', () => { const unsafeNumbers = [
        NaN
        Infinity }
        -Infinity
      ];
  ...
  expect(SecurityValidation.validateSafeValue(num)).toBe(false) });
    ...
    it('should handle safe arrays', () => { const safeArrays = [
        []
        ['safe', 'string', 'values']
        [1, 2, 3]
        [true, false]
  ...
  expect(SecurityValidation.validateSafeValue(arr)).toBe(true) });
    ...
    it('should reject unsafe arrays', () => { const unsafeArrays = [
  ...
  describe('sanitizeString', () => { it('should remove dangerous patterns', () => {
    ...
    it('should trim and limit length', () => { const longString = '  ' + 'a'.repeat(10005) + '  ';
  ...
  describe('safePropertyAccess', () => {
    ...
    it('should handle null/undefined objects', () => { expect(SecurityValidation.safePropertyAccess(null, 'prop')).toBe(null);
...
describe('SecureValidation Schemas', () => { describe('safeString schema', () => {
    ...
    it('should reject unsafe strings', () => { const schema = SecureValidation.safeString();
    ...
    it('should enforce custom length limits', () => { const schema = SecureValidation.safeString(10);
      expect(schema.safeParse('short').success).toBe(true);
      expect(schema.safeParse('this is too long').success).toBe(false) });
  });
  describe('safeExpression schema', () => { it('should validate safe expressions', () => {
  ...
  describe('variableName schema', () => { it('should validate proper variable names', () => {
  ...
  describe('safeValue schema', () => {
    ...
    it('should reject unsafe values', () => { const schema = SecureValidation.safeValue();
      expect(schema.safeParse('eval(').success).toBe(false);
      expect(schema.safeParse(NaN).success).toBe(false);
      expect(schema.safeParse(Infinity).success).toBe(false) });
  });
});
describe('SecurityTesting Utilities', () => { describe('INJECTION_PATTERNS', () => {
    ...
    it('should have multiple diverse attack vectors', () => { expect(SecurityTesting.INJECTION_PATTERNS.length).toBeGreaterThan(10);
  ...
  describe('testInjectionProtection', () => { it('should test validator against all injection patterns', () => {
  ...
  expect.stringMatching(/eval|constructor|Function|proto/)
    ...
    it('should handle perfect validator', () => { const perfectValidator = () => false; // Blocks everything;
    ...
    it('should handle permissive validator', () => { const permissiveValidator = () => true; // Allows everything;
  ...
  describe('runSecurityTests', () => { let consoleSpy: jest.SpyInstance;
  beforeEach(() => {
  consoleSpy = jest.spyOn(console, 'log').mockImplementation() });
    afterEach(() => { consoleSpy.mockRestore() });
    it('should run all security tests and return results', () => { const result = SecurityTesting.runSecurityTests();
  ...
  expect.stringMatching(/🔐 Security Tests Complete: \d+ passed, \d+ failed/)) });
    it('should test all validator functions', () => { SecurityTesting.runSecurityTests();
...
describe('Constants and Patterns', () => { describe('VARIABLE_NAME_PATTERN', () => {
  ...
  describe('VARIABLE_NAME_MAX_LENGTH', () => { it('should be set to 64', () => {
      expect(VARIABLE_NAME_MAX_LENGTH).toBe(64) });
    it('should be enforced by validation functions', () => { const maxLengthName = 'a'.repeat(VARIABLE_NAME_MAX_LENGTH);
...
describe('Edge Cases and Error Handling', () => { it('should handle extremely long inputs gracefully', () => {
  ...
  it('should handle special unicode characters', () => { const unicodeStrings = [
      '🔥🚀✨'
      'café'
      'naïve'
      'Ålborg'
  ...
  it('should handle deeply nested objects and arrays', () => { const deepObject = {
  level1: {
  level2: {
  level3: {
  value: 'deep' }
  ...
  it('should handle circular references gracefully', () => {
    ...
    const validationPromise = new Promise<boolean>((resolve) => { try {
        result = SecurityValidation.validateSafeValue(circular);
        resolve(result) } catch (error) { resolve(false) });
    return Promise.race([validationPromise, timeoutPromise]).then((res) => { expect(typeof res).toBe('boolean');
      // The important thing is that it completed without hanging
      expect(true).toBe(true) });
  });
  it('should handle empty and whitespace-only strings', () => { const emptyStrings = ['', '   ', '\t', '\n', '\r\n', ' \t\n '];
