...
import { measureExecution, PerformanceTimer, PerformanceTracker, globalPerformanceTracker } from '../utils/performance';
describe('Epic 20.1 - Core Performance Utilities Unit Tests', () => {
    ...
    describe('5. Real-World Integration Scenarios', () => {
        ...
        it('should measure and compare algorithm performance', async () => {
            ...
            const target = 750;
            const { result: linearResult, metrics: linearMetrics } = await measureExecution(() => linearSearch(data, target), { algorithm: 'linear-search' });
            const { result: binaryResult, metrics: binaryMetrics } = await measureExecution(() => binarySearch(data, target), { algorithm: 'binary-search' });
