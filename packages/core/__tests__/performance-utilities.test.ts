...
describe('Epic 20.1 - Core Performance Utilities Unit Tests', () => { beforeEach(() => {
    // Clear global tracker before each test
    globalPerformanceTracker.clear();
    jest.clearAllMocks() });
  describe('1. measureExecution Function', () => { it('should measure synchronous function execution time', async () => {
      const syncFunction = () => {
    ...
    it('should measure asynchronous function execution time', async () => { const asyncFunction = async () => {
    ...
    it('should include custom metadata in metrics', async () => { const testMetadata = {
  ...
  version: '1.0.0' }
    ...
    it('should handle function errors and still provide metrics', async () => { const errorFunction = () => {
        throw new Error('Test error message') };
      await expect(measureExecution(errorFunction)).rejects.toThrow('Test error message');
    });
    it('should track memory usage accurately', async () => { const memoryIntensiveFunction = () => {
  ...
  timestamp: Date.now();
    ...
    it('should handle async function errors with proper metrics', async () => { const asyncErrorFunction = async () => {
        await new Promise(resolve => setTimeout(resolve, 50));
        throw new Error('Async error') };
      try { await measureExecution(asyncErrorFunction);
        fail('Should have thrown an error') } catch (error) { expect(error).toBeInstanceOf(Error);
        expect((error as Error).message).toBe('Async error') });
    it('should measure execution time precision accurately', async () => { const preciseFunction = () => {
  ...
  describe('2. PerformanceTimer Class', () => { it('should create timer and measure elapsed time', () => {
      ...
      expect(metrics.startTime).toBeLessThan(metrics.endTime);
      expect(metrics.endTime - metrics.startTime).toBe(metrics.duration) });
    it('should reset timer and start new measurement', () => { const timer = new PerformanceTimer();
      // Let some time pass
      const start = Date.now();
    ...
    it('should provide consistent timing measurements', () => { const timer = new PerformanceTimer();
    ...
    it('should handle multiple stops without reset', () => { const timer = new PerformanceTimer();
      // First stop
      const start = Date.now();
  ...
  describe('3. PerformanceTracker Class', () => { let tracker: PerformanceTracker;
  beforeEach(() => {
  tracker = new PerformanceTracker() });
    it('should add and retrieve metrics for operations', () => { const testMetrics: ExecutionMetrics = {,
  duration: 100,
  ...
  memory: 1024 }
    ...
    it('should accumulate multiple metrics for same operation', () => { const operation = 'multi-test';
  for (let i = 0; i < 5; i++) {
  ...
  memory: (i + 1) * 512 }
    ...
    it('should calculate accurate average metrics', () => { const operation = 'avg-test';
  ...
  memory: duration * 2 }
    ...
    it('should return null for non-existent operations', () => { const avgMetrics = tracker.getAverageMetrics('non-existent');
    ...
    it('should handle metrics without memory information', () => { const metricsWithoutMemory: ExecutionMetrics = {,
  duration: 150,
  ...
  endTime: Date.now() }
    ...
    it('should handle large numbers of metrics efficiently', () => { const operation = 'stress-test';
  ...
  describe('4. Global Performance Tracker', () => { it('should maintain global state across operations', () => {
  ...
  endTime: 200 }
    ...
    it('should integrate with measureExecution for global tracking', async () => { const testFunction = () => {
  ...
  operation: 'global-measure-test' }
    ...
    it('should be clearable for test isolation', () => { globalPerformanceTracker.addMetric('test-isolation', {)
  duration: 50,
  startTime: 0,
  endTime: 50 }
});
  ...
  type: 'SELECT' }
    ...
    it('should measure API endpoint performance', async () => { const simulatedApiCall = async () => {
  ...
  method: 'GET' }
    ...
    it('should track batch operation performance', () => { const tracker = new PerformanceTracker();
      const batchSize = 100;
    ...
    it('should measure and compare algorithm performance', async () => {
      ...
      const linearSearch = (arr: number, target: number) => { for (let i = 0; i < arr.length; i++) {
          if (arr[i] === target) return i;
        return -1 };
      // Algorithm 2: Binary search (requires sorted array)
      const binarySearch = (arr: number, target: number) => { let left = 0, right = arr.length - 1;
        while (left <= right) {
  ...
  describe('6. Error Handling and Edge Cases', () => {
    ...
    it('should handle complex object returns', async () => { const complexFunction = () => ({)
  nested: {,
  array: [1, 2, 3] }
          object: { key: 'value' }
  },
