...
describe('PasswordStrengthIndicator', () => {
    let mockValidator;
    beforeEach(() => {
        mockValidator = new PasswordComplexityValidator();
    ...
    describe('Basic Rendering', () => {
        test('should not render when password is empty', () => {
            const { container } = render(_jsx(PasswordStrengthIndicator, { password: "" }));
            expect(container.firstChild).toBeNull();
        });
        test('should render strength meter for valid password', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator }));
            await waitFor(() => {
        ...
        test('should show loading state during validation', () => {
            ...
            mockValidator.validatePassword.mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({
                valid: true,
                score: 75,
                strength: 'good',
                ruleResults: [],
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, debounceMs: 50 }));
    ...
    describe('Strength Visualization', () => {
        ...
        strengthTestCases.forEach(({ score, strength, color }) => {
            test(`should display correct color and label for ${strength} password`, async () => {
                ...
                render(_jsx(PasswordStrengthIndicator, { password: "test", validator: mockValidator }));
                await waitFor(() => {
    ...
    describe('Compact Mode', () => {
        test('should render in compact mode', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, compact: true }));
            await waitFor(() => {
        ...
        test('should hide details in compact mode', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, compact: true, showDetails: true, showSuggestions: true }));
            await waitFor(() => {
                expect(screen.queryByText('Requirements')).not.toBeInTheDocument();
    ...
    describe('Detailed Display', () => {
        test('should show rule results when showDetails is true', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, showDetails: true }));
            await waitFor(() => {
        ...
        test('should show suggestions when showSuggestions is true', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, showSuggestions: true }));
            await waitFor(() => {
        ...
        test('should show entropy when showEntropy is true', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, showEntropy: true }));
            await waitFor(() => {
        ...
        test('should show crack time when showCrackTime is true', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, showCrackTime: true }));
            await waitFor(() => {
    ...
    describe('Error and Warning Display', () => {
        test('should display errors', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "weak", validator: mockValidator }));
            await waitFor(() => {
        ...
        test('should display warnings', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "Password123", validator: mockValidator }));
            await waitFor(() => {
        ...
        test('should not show warnings in compact mode', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "Password123", validator: mockValidator, compact: true }));
            await waitFor(() => {
    ...
    describe('Context-Aware Validation', () => {
        test('should pass context to validator', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", context: context, validator: mockValidator }));
            await waitFor(() => {
                expect(mockValidator.validatePassword).toHaveBeenCalledWith('TestPassword123', context);
            });
        });
    });
    describe('Validation Change Callback', () => {
        test('should call onValidationChange when result updates', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, onValidationChange: onValidationChange }));
            await waitFor(() => {
                expect(onValidationChange).toHaveBeenCalledWith(expect.objectContaining({
                    valid: true,
                    score: 75,
                    strength: 'good'
                }));
    ...
    describe('Theme Support', () => {
        test('should support light theme', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, theme: "light" }));
            await waitFor(() => {
                expect(screen.getByText('Password Strength')).toBeInTheDocument();
            });
        });
        test('should support dark theme', async () => {
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, theme: "dark" }));
            await waitFor(() => {
                expect(screen.getByText('Password Strength')).toBeInTheDocument();
            });
        });
        test('should auto-detect theme', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, theme: "auto" }));
            await waitFor(() => {
                expect(screen.getByText('Password Strength')).toBeInTheDocument();
            });
        });
    ...
    describe('Debouncing', () => {
        test('should debounce validation calls', async () => {
            const { rerender } = render(_jsx(PasswordStrengthIndicator, { password: "a", validator: mockValidator, debounceMs: 100 }));
            // Rapidly change password
            rerender(_jsx(PasswordStrengthIndicator, { password: "ab", validator: mockValidator, debounceMs: 100 }));
            rerender(_jsx(PasswordStrengthIndicator, { password: "abc", validator: mockValidator, debounceMs: 100 }));
    ...
    describe('Suggestion Limiting', () => {
        test('should limit suggestions display to 3 items', async () => {
            ...
            render(_jsx(PasswordStrengthIndicator, { password: "weak", validator: mockValidator, showSuggestions: true }));
            await waitFor(() => {
    ...
    describe('Custom Styling', () => {
        test('should apply custom className', () => {
            const { container } = render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123", validator: mockValidator, className: "custom-class" }));
...
describe('usePasswordStrength Hook', () => {
    ...
    test('should return password strength data', async () => {
        const TestComponent = () => {
            const { result, isValidating, isValid, score, strength, suggestions, errors } = usePasswordStrength('TestPassword123!', undefined, mockValidator);
            return (_jsxs("div", { children: [_jsx("div", { "data-testid": "is-validating", children: isValidating.toString() }), _jsx("div", { "data-testid": "is-valid", children: isValid.toString() }), _jsx("div", { "data-testid": "score", children: score }), _jsx("div", { "data-testid": "strength", children: strength }), _jsx("div", { "data-testid": "suggestions", children: suggestions.join(', ') }), _jsx("div", { "data-testid": "errors", children: errors.join(', ') })] }));
        };
    ...
    test('should handle empty password', () => {
        const TestComponent = () => {
            ...
            return (_jsxs("div", { children: [_jsx("div", { "data-testid": "is-valid", children: isValid.toString() }), _jsx("div", { "data-testid": "score", children: score }), _jsx("div", { "data-testid": "strength", children: strength })] }));
        };
        render(_jsx(TestComponent, {}));
    ...
    test('should use default validator when none provided', async () => {
        const TestComponent = () => {
            ...
            return _jsx("div", { "data-testid": "score", children: score });
        };
        render(_jsx(TestComponent, {}));
...
describe('Integration Tests', () => {
    test('should work with real validator', async () => {
        ...
        render(_jsx(PasswordStrengthIndicator, { password: "TestPassword123!", validator: realValidator, showDetails: true, showSuggestions: true, showEntropy: true, showCrackTime: true }));
    ...
    test('should handle rapid password changes', async () => {
        const realValidator = new PasswordComplexityValidator();
        const onValidationChange = jest.fn();
        const { rerender } = render(_jsx(PasswordStrengthIndicator, { password: "a", validator: realValidator, onValidationChange: onValidationChange, debounceMs: 50 }));
        // Simulate rapid typing
        const passwords = ['a', 'ab', 'abc', 'Test', 'TestP', 'TestPass', 'TestPass1', 'TestPass1!'];
        for (const password of passwords) {
            rerender(_jsx(PasswordStrengthIndicator, { password: password, validator: realValidator, onValidationChange: onValidationChange, debounceMs: 50 }));
        }
