...
describe('PSG Schema Validation', () => {
  ...
  describe('validatePsgFile', () => {
    test('returns success for valid file', () => {
      const validFile = {
        fileType: 'psg',
        formatVersion: PSG_FORMAT_VERSION,
  ...
  describe('isVersionCompatible', () => {
    test('same version is compatible without migration', () => {
    ...
    test('future major version is incompatible', () => {
      const result = isVersionCompatible('2.0.0');
      expect(result.compatible).toBe(false);
      expect(result.requiresMigration).toBe(false);
      expect(result.message).toContain('newer version');
    });
    test('older major version requires migration', () => {
      const result = isVersionCompatible('0.9.0');
      expect(result.compatible).toBe(true);
  ...
  describe('createDefaultMetadata', () => {
    test('creates valid metadata with required fields', () => {
    ...
    test('uses anonymous author when not provided', () => {
      const metadata = createDefaultMetadata('Test Project');
      expect(metadata.author).toBe('Anonymous');
    });
    test('generates valid datetime strings', () => {
      const metadata = createDefaultMetadata('Test Project');
  ...
  describe('createDefaultSettings', () => {
    test('creates valid settings with all required fields', () => {
    ...
    test('settings pass schema validation', () => {
      const settings = createDefaultSettings();
      const result = PsgFileSchema.shape.settings.safeParse(settings);
      expect(result.success).toBe(true);
    });
  });
  describe('Collaboration data schema', () => {
