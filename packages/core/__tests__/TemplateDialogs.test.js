...
describe('SaveTemplateDialog', () => {
    ...
    it('should render when open', () => {
        render(_jsx(SaveTemplateDialog, { isOpen: true, onClose: mockOnClose, onSave: mockOnSave }));
    ...
    it('should not render when closed', () => {
        render(_jsx(SaveTemplateDialog, { isOpen: false, onClose: mockOnClose, onSave: mockOnSave }));
    ...
    it('should reset form on successful save', async () => {
        const user = userEvent.setup();
        mockOnSave.mockResolvedValue({ success: true });
        const { rerender } = render(_jsx(SaveTemplateDialog, { isOpen: true, onClose: mockOnClose, onSave: mockOnSave }));
        const nameInput = screen.getByLabelText(/Template Name/);
        ...
        rerender(_jsx(SaveTemplateDialog, { isOpen: true, onClose: mockOnClose, onSave: mockOnSave }));
    ...
    it('should populate initial data', () => {
        ...
        render(_jsx(SaveTemplateDialog, { isOpen: true, onClose: mockOnClose, onSave: mockOnSave, initialData: initialData }));
...
describe('TemplateBrowser', () => {
    ...
    beforeEach(() => {
        ...
        templateService.searchTemplates.mockResolvedValue(sampleTemplates);
    });
    it('should render when open', async () => {
        render(_jsx(TemplateBrowser, { isOpen: true, onClose: mockOnClose, onApplyTemplate: mockOnApplyTemplate }));
    ...
    it('should not render when closed', () => {
        render(_jsx(TemplateBrowser, { isOpen: false, onClose: mockOnClose, onApplyTemplate: mockOnApplyTemplate }));
    ...
    it('should handle search', async () => {
        ...
        await waitFor(() => {
            expect(templateService.searchTemplates).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should show empty state', async () => {
        const { templateService } = require('../services/TemplateService');
        templateService.searchTemplates.mockResolvedValue([]);
        render(_jsx(TemplateBrowser, { isOpen: true, onClose: mockOnClose, onApplyTemplate: mockOnApplyTemplate }));
        await waitFor(() => {
    ...
    it('should handle template application', async () => {
        ...
        expect(mockOnApplyTemplate).toHaveBeenCalledWith('template-1', expect.objectContaining({
    ...
    it('should handle template deletion for owned templates', async () => {
        const user = userEvent.setup();
        const { templateService } = require('../services/TemplateService');
        render(_jsx(TemplateBrowser, { isOpen: true, onClose: mockOnClose, onApplyTemplate: mockOnApplyTemplate, currentAuthor: "author1" }));
        await waitFor(() => {
            expect(screen.getByText('Character Generator')).toBeInTheDocument();
        });
