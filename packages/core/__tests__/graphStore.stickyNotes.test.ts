...
const mockNotes: StickyNote = [
  ...
  { id: 'note-2' }
    ...
    zIndex: 2];
describe('Graph Store - Sticky Notes Operations', () => { beforeEach(() => {
  ...
  describe('Initial State', () => { test('initializes with empty sticky notes array', () => {
      expect(mockStore.stickyNotes).toEqual([]) });
    test('initializes annotations object correctly', () => { expect(mockStore.annotations).toEqual({)
  stickyNotes: [] }
        nodeLabels: {}
        regionGroups: []
        connectionLabels: []
  ...
  version: '1.0.0' }
    ...
    test('initializes with hasUnsavedChanges as false', () => { expect(mockStore.hasUnsavedChanges).toBe(false) });
  });
  describe('setStickyNotes', () => { test('sets sticky notes array', () => {
    ...
    test('updates metadata timestamp', () => { const beforeTime = new Date().toISOString();
      store.setStickyNotes(mockNotes);
    ...
    test('replaces existing notes completely', () => { // First set some notes
      store.setStickyNotes(mockNotes);
    ...
    test('handles empty array', () => { store.setStickyNotes(mockNotes);
      store.setStickyNotes([]);
      expect(store.stickyNotes).toEqual([]);
      expect(store.annotations.stickyNotes).toEqual([]) });
  });
  describe('addStickyNote', () => { test('adds single note to empty array', () => {
      ...
      expect(store.hasUnsavedChanges).toBe(true) });
    test('adds note to existing array', () => { store.setStickyNotes([mockNotes[0]]);
    ...
    test('updates metadata timestamp', () => { const beforeTime = new Date().toISOString();
      ...
      expect(store.annotations.metadata.modified).toBeGreaterThanOrEqual(beforeTime);
      expect(store.annotations.metadata.modified).toBeLessThanOrEqual(afterTime) });
    test('maintains order when adding notes', () => { store.addStickyNote(mockNotes[1]);
      store.addStickyNote(mockNotes[0]);
      expect(store.stickyNotes[0]).toEqual(mockNotes[1]);
      expect(store.stickyNotes[1]).toEqual(mockNotes[0]) });
  });
  describe('updateStickyNote', () => { beforeEach(() => {
      store.setStickyNotes(mockNotes) });
    ...
    test('updates multiple properties at once', () => { const updates = {
        content: 'New content'
  ...
  describe('deleteStickyNote', () => { beforeEach(() => {
      store.setStickyNotes(mockNotes) });
    test('deletes existing note', () => { store.deleteStickyNote('note-1');
      expect(store.stickyNotes).toHaveLength(1);
      expect(store.stickyNotes[0].id).toBe('note-2');
      expect(store.hasUnsavedChanges).toBe(true) });
    test('updates annotations simultaneously', () => { store.deleteStickyNote('note-1');
    ...
    test('can delete all notes', () => { store.deleteStickyNote('note-1');
      store.deleteStickyNote('note-2');
      expect(store.stickyNotes).toEqual([]);
      expect(store.annotations.stickyNotes).toEqual([]) });
    test('maintains order of remaining notes', () => { // Add a third note
      const thirdNote = {
        id: 'note-3' }
  ...
  describe('Integration with Project State', () => {
    ...
    test('sticky notes are included in template data', () => { store.setStickyNotes(mockNotes);
  ...
  describe('Error Handling', () => { test('handles invalid note data gracefully', () => {
 ...
 as any;
      expect(() => { store.addStickyNote(incompleteNote) }).not.toThrow();
    ...
    test('handles null/undefined updates', () => {
      ...
      expect(() => { store.updateStickyNote('note-1', null as any) }).not.toThrow();
    });
  });
