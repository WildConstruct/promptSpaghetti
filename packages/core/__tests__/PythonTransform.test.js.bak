...
describe('PythonTransformNode', () => {
    ...
    beforeEach(() => {
        ...
        PythonExecutorClient.mockImplementation(() => mockClient);
        // Create test context
...
`,
    ...
    describe('Security Features', () => {
        it('should handle security events', async () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Python security event in node test-node:'), expect.objectContaining({
                level: 'warning',
                type: 'memory_warning',
            }));
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Python security event in node test-node:'), expect.objectContaining({
        ...
        it('should handle warnings', async () => {
            ...
            // Verify result
            expect(result).toBe('result');
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Python warning in node test-node:'), expect.objectContaining({
                warning: 'Warning 1',
            }));
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Python warning in node test-node:'), expect.objectContaining({
                warning: 'Warning 2',
