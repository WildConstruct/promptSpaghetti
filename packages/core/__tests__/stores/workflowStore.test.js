...
const createMockTransition = (overrides = {}) => ({
    id: 'transition-1',
    workspace_id: 'workspace-1',
...
const createMockLock = (overrides = {}) => ({
    id: 'lock-1',
    workspace_id: 'workspace-1',
    resource_id: 'resource-1',
    locked_by: 'user-1',
...
const mockApiError = (status = 500, statusText = 'Internal Server Error') => {
    mockFetch.mockResolvedValueOnce({
        ok: false,
        status,
        statusText,
...
describe('Workflow Store', () => {
    ...
    describe('State Management', () => {
        describe('fetchStates', () => {
            it('should fetch states successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/states/workspace-1', expect.objectContaining({
        ...
        describe('createState', () => {
            it('should create state successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/states', expect.objectContaining({
                    method: 'POST',
        ...
        describe('updateState', () => {
            it('should update state successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/states/state-1', expect.objectContaining({
                    method: 'PUT',
        ...
        describe('deleteState', () => {
            it('should delete state successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/states/state-1', expect.objectContaining({ method: 'DELETE' }));
    ...
    describe('Transition Management', () => {
        describe('fetchTransitions', () => {
            it('should fetch all transitions for workspace', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/transitions/workspace-1', expect.any(Object));
            ...
            it('should fetch transitions filtered by from_state_id', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/transitions/workspace-1?from_state_id=state-1', expect.any(Object));
            });
        });
    ...
    describe('State Transitions', () => {
        describe('transitionResourceState', () => {
            it('should transition resource state successfully', async () => {
                ...
                const { result } = renderHook(() => useWorkflowStore());
                await act(async () => {
                    const result_data = await result.current.transitionResourceState('resource-1', 'state-2', 'user-1', { comment: 'Moving to review', force: false });
                    expect(result_data).toEqual(transitionResult);
                });
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/resources/resource-1/transition', expect.objectContaining({
                    method: 'POST',
                    body: JSON.stringify({
                        to_state_id: 'state-2',
            ...
            it('should handle transition requiring approval', async () => {
                ...
                const { result } = renderHook(() => useWorkflowStore());
                await act(async () => {
                    const result_data = await result.current.transitionResourceState('resource-1', 'state-2', 'user-1');
    ...
    describe('Approval Management', () => {
        describe('fetchApprovals', () => {
            it('should fetch approvals with filters', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/approvals/workspace-1?status=pending&priority=high', expect.any(Object));
        ...
        describe('approveWorkflow', () => {
            it('should approve workflow successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/approvals/approval-1/approve', expect.objectContaining({
                    method: 'POST',
    ...
    describe('Lock Management', () => {
        describe('acquireLock', () => {
            it('should acquire lock successfully', async () => {
                ...
                const { result } = renderHook(() => useWorkflowStore());
                await act(async () => {
                    const lock = await result.current.acquireLock('resource-1', 'user-1', 'edit', { reason: 'Editing content', duration: 3600 });
                    expect(lock).toEqual(newLock);
                });
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/locks', expect.objectContaining({
                    method: 'POST',
                    body: JSON.stringify({
                        resource_id: 'resource-1',
                        lock_type: 'edit',
        ...
        describe('releaseLock', () => {
            it('should release lock successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/locks/lock-1', expect.objectContaining({ method: 'DELETE' }));
        ...
        describe('releaseLocksByResource', () => {
            ...
            it('should release locks by specific type for resource', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/locks/resource/resource-1?lock_type=edit', expect.objectContaining({ method: 'DELETE' }));
    ...
    describe('History and Statistics', () => {
        describe('fetchHistory', () => {
            it('should fetch history with filters', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/history/workspace-1?resource_id=resource-1&limit=10', expect.any(Object));
        ...
        describe('fetchStatistics', () => {
            it('should fetch statistics successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/statistics/workspace-1', expect.any(Object));
    ...
    describe('Utility Functions', () => {
        ...
        describe('isResourceLocked', () => {
            ...
            it('should check specific lock type', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/validate/lock?resource_id=resource-1&lock_type=edit', expect.any(Object));
            });
        });
        describe('performMaintenance', () => {
            it('should perform maintenance successfully', async () => {
                ...
                expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/workflow/maintenance', expect.objectContaining({ method: 'POST' }));
            });
        });
    });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle very large datasets', async () => {
            ...
            const largeStateArray = Array.from({ length: 1000 }, (_, i) => createMockState({ id: `state-${i}`, name: `State ${i}`, sort_order: i }));
