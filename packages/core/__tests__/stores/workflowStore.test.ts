...
const mockApiResponse = (data: any) => { mockFetch.mockResolvedValueOnce({)
  ok: true
  json: async () => data }
 as Response);
};
...
const mockApiError = (status: number = 500, statusText: string = 'Internal Server Error') => { mockFetch.mockResolvedValueOnce({)
  ok: false
    status
    statusText }
...
describe('Workflow Store', () => { beforeEach(() => {
  mockFetch.mockClear();
  ...
  describe('State Management', () => {
    describe('fetchStates', () => {
      it('should fetch states successfully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.fetchStates('workspace-1') });
  ...
  'x-user-id': 'current-user-id' }
      ...
      it('should handle fetch states error', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.fetchStates('nonexistent-workspace') });
        expect(result.current.states).toEqual([]);
        expect(result.current.loading).toBe(false);
        expect(result.current.error).toBe('API call failed: 404 Workspace not found');
      });
    });
    describe('createState', () => { it('should create state successfully', async () => {
  const newStateData = {
  workspace_id: 'workspace-1'
  name: 'New State'
  color: '#blue'
  sort_order: 2 }
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const state = await result.current.createState(newStateData);
  ...
  body: JSON.stringify(newStateData) }
    ...
    describe('updateState', () => {
      it('should update state successfully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const state = await result.current.updateState('state-1', updates);
  ...
  body: JSON.stringify(updates) }
    ...
    describe('deleteState', () => {
      it('should delete state successfully', async () => {
        ...
        mockApiResponse({});
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.deleteState('state-1') });
        expect(mockFetch).toHaveBeenCalledWith()
          'http://localhost:8000/api/workflow/states/state-1'
  ...
  describe('Transition Management', () => { describe('fetchTransitions', () => {
      it('should fetch all transitions for workspace', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.fetchTransitions('workspace-1') });
        expect(mockFetch).toHaveBeenCalledWith()
      ...
      it('should fetch transitions filtered by from_state_id', async () => {
        ...
        mockApiResponse(mockTransitions);
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.fetchTransitions('workspace-1', 'state-1') });
        expect(mockFetch).toHaveBeenCalledWith()
    ...
    describe('createTransition', () => { it('should create transition successfully', async () => {
  ...
  requires_approval: false }
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const transition = await result.current.createTransition(transitionData);
    ...
    describe('deleteTransition', () => {
      it('should delete transition successfully', async () => {
        ...
        mockApiResponse({});
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.deleteTransition('transition-1') });
        expect(result.current.transitions).toEqual([transition2]);
      });
    });
  });
  describe('State Transitions', () => { describe('transitionResourceState', () => {
  ...
  workflow_history_id: 'history-1' }
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.transitionResourceState(;);
            'resource-1'
            'state-2'
            'user-1' }
  ...
  force: false }
      ...
      it('should handle transition requiring approval', async () => { const transitionResult: StateTransitionResult = {
  success: true
  ...
  describe('Approval Management', () => {
    describe('fetchApprovals', () => {
      it('should fetch approvals with filters', async () => {
        ...
        await act(async () => { await result.current.fetchApprovals('workspace-1', filters) });
        expect(mockFetch).toHaveBeenCalledWith()
  ...
  workflow_history_id: 'history-1' }
        ...
        mockApiResponse(transitionResult);
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.approveWorkflow('approval-1', 'user-2', 'Looks good!');
          expect(result_data).toEqual(transitionResult) });
        expect(mockFetch).toHaveBeenCalledWith()
  ...
  method: 'POST' }
    ...
    describe('rejectWorkflow', () => {
      it('should reject workflow successfully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.rejectWorkflow('approval-1', 'user-2', 'Needs more work');
  ...
  describe('Lock Management', () => {
    describe('acquireLock', () => {
      it('should acquire lock successfully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const lock = await result.current.acquireLock(;);
            'resource-1'
            'user-1'
            'edit' }
  ...
  duration: 3600 }
      ...
      it('should use default lock type when not specified', async () => {
        ...
        mockApiResponse(newLock);
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.acquireLock('resource-1', 'user-1') });
        expect(mockFetch).toHaveBeenCalledWith()
          'http://localhost:8000/api/workflow/locks'
          expect.objectContaining({ )
  body: JSON.stringify({);
  ...
  lock_type: 'edit' }
    ...
    describe('releaseLock', () => {
      it('should release lock successfully', async () => {
        ...
        mockApiResponse({});
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.releaseLock('lock-1', 'user-1');
          expect(result_data).toBe(true) });
        expect(mockFetch).toHaveBeenCalledWith()
    ...
    describe('releaseLocksByResource', () => {
      it('should release all locks for resource', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const count = await result.current.releaseLocksByResource('resource-1', 'user-1');
          expect(count).toBe(2) });
      ...
      it('should release locks by specific type for resource', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const count = await result.current.releaseLocksByResource('resource-1', 'user-1', 'edit');
          expect(count).toBe(1) });
        expect(mockFetch).toHaveBeenCalledWith()
  ...
  describe('History and Statistics', () => {
    describe('fetchHistory', () => {
      it('should fetch history with filters', async () => {
        ...
        await act(async () => { await result.current.fetchHistory('workspace-1', filters) });
        expect(mockFetch).toHaveBeenCalledWith()
    ...
    describe('fetchStatistics', () => {
      it('should fetch statistics successfully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { await result.current.fetchStatistics('workspace-1') });
        expect(mockFetch).toHaveBeenCalledWith()
  ...
  describe('Utility Functions', () => { describe('validateStateTransition', () => {
  ...
  can_transition: true }
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.validateStateTransition('resource-1', 'state-2');
      ...
      it('should handle validation error gracefully', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.validateStateTransition('resource-1', 'invalid-state');
    ...
    describe('canUserTransitionState', () => {
      it('should return true when user can transition', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const canTransition = await result.current.canUserTransitionState('user-1', 'resource-1', 'state-2');
    ...
    describe('isResourceLocked', () => {
      it('should return lock status', async () => {
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const isLocked = await result.current.isResourceLocked('resource-1');
          expect(isLocked).toBe(true) });
      });
      it('should check specific lock type', async () => {
        mockApiResponse({ is_locked: false });
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const isLocked = await result.current.isResourceLocked('resource-1', 'edit');
    ...
    describe('performMaintenance', () => { it('should perform maintenance successfully', async () => {
  ...
  maintenance_timestamp: new Date().toISOString() }
        ...
        const { result } = renderHook(() => useWorkflowStore());
        await act(async () => { const result_data = await result.current.performMaintenance();
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle concurrent API calls', async () => {
      ...
      const { result } = renderHook(() => useWorkflowStore());
      await act(async () => { // Fire both calls simultaneously
