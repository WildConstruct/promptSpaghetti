...
describe('TemplateService', () => {
    ...
    describe('createFromGraph', () => {
        ...
        it('should create template from graph data', async () => {
            const template = await service.createFromGraph(sampleNodes, sampleEdges, sampleSaveData, 'test-author');
        ...
        it('should calculate complexity correctly', async () => {
            ...
            const simpleTemplate = await service.createFromGraph(sampleNodes, sampleEdges, sampleSaveData, 'test-author');
            ...
            const mediumTemplate = await service.createFromGraph(mediumNodes, mediumEdges, sampleSaveData, 'test-author');
            ...
            const complexTemplate = await service.createFromGraph(complexNodes, complexEdges, { ...sampleSaveData, name: 'Complex Template' }, 'test-author');
        ...
        it('should validate template before saving', async () => {
            ...
            await expect(service.createFromGraph(sampleNodes, sampleEdges, invalidSaveData, 'test-author')).rejects.toThrow('Template validation failed');
        });
        it('should extract annotations correctly', async () => {
            ...
            const template = await service.createFromGraph(nodesWithLabels, edgesWithLabels, sampleSaveData, 'test-author');
    ...
    describe('searchTemplates', () => {
        beforeEach(async () => {
            ...
            const templates = [
                await service.createFromGraph([{ id: 'n1', type: 'WeightedChoice', position: { x: 0, y: 0 }, data: {} }], [], {
        ...
        it('should sort results correctly', async () => {
            ...
            expect(byName[0].name.toLowerCase()).toBeLessThanOrEqual(byName[1].name.toLowerCase());
        });
    ...
    describe('instantiateTemplate', () => {
        ...
        it('should throw error for nonexistent template', async () => {
            await expect(service.instantiateTemplate('nonexistent-id', {
                preservePositions: true,
                mergeWithCurrent: false
    ...
    describe('addReview', () => {
        let template;
        beforeEach(async () => {
            template = await service.createFromGraph([{ id: 'n1', type: 'Output', position: { x: 0, y: 0 }, data: {} }], [], {
        ...
        it('should throw error for nonexistent template', async () => {
            await expect(service.addReview('nonexistent-id', {
                author: 'reviewer',
                rating: 5,
                comment: 'Great!',
                helpful: 0
    ...
    describe('validateTemplate', () => {
        ...
        it('should detect validation errors', async () => {
            ...
            expect(validation.errors.some(error => error.includes('invalid source'))).toBe(true);
        });
    });
    describe('LocalTemplateStorage', () => {
        it('should handle storage errors gracefully', () => {
            ...
            expect(storage.save(template)).rejects.toThrow('Template storage failed: disk full or quota exceeded');
        });
        ...
        it('should prevent duplicate names', async () => {
            ...
            await expect(storage.save(template2)).rejects.toThrow('Template with name "Duplicate Name" already exists');
        });
    });
    describe('getPopularTemplates', () => {
        beforeEach(async () => {
            // Create templates with different ratings
            const template1 = await service.createFromGraph([{ id: 'n1', type: 'Output', position: { x: 0, y: 0 }, data: {} }], [], { name: 'Template 1', description: 'Test', category: 'general', tags: [], isPublic: true, includeAnnotations: true }, 'author');
            const template2 = await service.createFromGraph([{ id: 'n1', type: 'Output', position: { x: 0, y: 0 }, data: {} }], [], { name: 'Template 2', description: 'Test', category: 'general', tags: [], isPublic: true, includeAnnotations: true }, 'author');
    ...
    describe('getRecentTemplates', () => {
        it('should return templates sorted by creation date', async () => {
            ...
            const template1 = await service.createFromGraph([{ id: 'n1', type: 'Output', position: { x: 0, y: 0 }, data: {} }], [], { name: 'Old Template', description: 'Test', category: 'general', tags: [], isPublic: true, includeAnnotations: true }, 'author');
            await new Promise(resolve => setTimeout(resolve, 10));
            const template2 = await service.createFromGraph([{ id: 'n1', type: 'Output', position: { x: 0, y: 0 }, data: {} }], [], { name: 'New Template', description: 'Test', category: 'general', tags: [], isPublic: true, includeAnnotations: true }, 'author');
