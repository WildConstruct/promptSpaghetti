...
jest.mock('../python-executor-client', () => { const mockClient = {
  execute: jest.fn(),
  validate: jest.fn(),
  health: jest.fn() }
};
  return { PythonExecutorClient: jest.fn(() => mockClient),
...
describe('PythonTransformNode', () => {
  ...
  describe('Basic Execution', () => { it('should execute Python code successfully', async () => {
  ...
  variables: { testVar: 'test value' }
    ...
    it('should handle execution failure with error fallback', async () => { // Mock execution failure
  mockClient.execute.mockResolvedValue({)
  success: false
  ...
  cache_hit: false }
    ...
    it('should handle execution failure with skip fallback', async () => { // Create node with skip fallback
      node = new PythonTransformNode('test-node', {)
  code: 'invalid code' }
  ...
  cache_hit: false }
    ...
    it('should handle execution failure with default fallback', async () => { // Create node with default fallback
  node = new PythonTransformNode('test-node', {)
  code: 'invalid code'
  pythonConfig: {
  ...
  cache_hit: false }
    ...
    it('should handle client errors with skip fallback', async () => { // Create node with skip fallback
      node = new PythonTransformNode('test-node', {)
  ...
  describe('Configuration', () => {
    ...
    it('should use custom configuration', async () => { // Create node with custom configuration
  node = new PythonTransformNode('test-node', {)
  code: 'def transform(input_data): return input_data'
  timeout: 60
  memoryLimit: '256MB'
  ...
  strict_mode: false }
    ...
    it('should handle custom executor URL', async () => { // Create node with custom executor URL
  node = new PythonTransformNode('test-node', {)
  code: 'def transform(input_data): return input_data'
  pythonConfig: {
  ...
  describe('Security Features', () => { it('should handle security events', async () => {
  ...
  type: 'dangerous_pattern' }
    ...
    it('should handle warnings', async () => { // Mock execution with warnings
  mockClient.execute.mockResolvedValue({)
  success: true
  result: 'result'
  execution_time: 0.1
  ...
  describe('Result Processing', () => { it('should handle string results', async () => {
  ...
  cache_hit: false }
    ...
    it('should handle number results', async () => { // Mock execution with number result
  mockClient.execute.mockResolvedValue({)
  success: true
  result: 42
  execution_time: 0.1
  ...
  cache_hit: false }
    ...
    it('should handle null/undefined results', async () => { // Mock execution with null result
  mockClient.execute.mockResolvedValue({)
  success: true
  result: null
  execution_time: 0.1
  ...
  describe('Validation', () => { it('should validate code successfully', async () => {
  ...
  strict_mode: true }
    ...
    it('should validate code with errors', async () => { // Mock validation with errors
  mockClient.validate.mockResolvedValue({)
  valid: false
  ...
  warnings: [] }
    ...
    it('should handle validation service errors', async () => { // Mock validation service error
  ...
  describe('Service Health', () => { it('should check service availability', async () => {
  ...
  uptime: 3600 }
    ...
    it('should handle service unavailability', async () => { // Mock health check failure
      mockClient.health.mockRejectedValue(new Error('Service unavailable'));
      // Check availability
      const available = await node.isServiceAvailable();
      // Verify result
      expect(available).toBe(false) });
    it('should get service health', async () => { // Mock health check success
  const healthData = {
  status: 'healthy'
  version: '1.0.0'
  uptime: 3600 }
    ...
    it('should handle service health errors', async () => { // Mock health check failure
      mockClient.health.mockRejectedValue(new Error('Service unavailable'));
      // Get health
      const health = await node.getServiceHealth();
      // Verify result
  ...
  describe('Statistics', () => { it('should track execution statistics', async () => {
  // Mock successful execution
  mockClient.execute.mockResolvedValue({)
  success: true
  result: 'result'
  ...
  cache_hit: false }
    ...
    it('should track mixed success/failure statistics', async () => { // Mock mixed execution results
  mockClient.execute
