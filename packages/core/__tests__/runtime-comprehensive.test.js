import { WeightedChoiceNode, ConcatNode, OutputNode, IncludeNode, SetVariableNode, GetVariableNode, RuntimeNode } from '../runtime';
...
describe('Runtime Engine - Comprehensive Tests', () => {
    ...
    describe('IncludeNode', () => {
        ...
        it('handles null/undefined lookup safely', () => {
            const node1 = new IncludeNode('inc1', 'key', null);
            const node2 = new IncludeNode('inc2', 'key', undefined);
            const ctx = createContext();
            expect(node1.run(ctx)).toBe('');
            expect(node2.run(ctx)).toBe('');
        });
        it('handles non-object lookup safely', () => {
            const node1 = new IncludeNode('inc1', 'key', 'string');
            const node2 = new IncludeNode('inc2', 'key', 123);
            const node3 = new IncludeNode('inc3', 'key', true);
            const ctx = createContext();
            expect(node1.run(ctx)).toBe('');
            expect(node2.run(ctx)).toBe('');
            expect(node3.run(ctx)).toBe('');
        ...
        it('handles non-string values in lookup', () => {
            ...
            const ctx = createContext({ variables: { defaultText: 'Default' } });
            expect(new IncludeNode('inc1', 'number', lookup).run(ctx)).toBe('Default');
            expect(new IncludeNode('inc2', 'boolean', lookup).run(ctx)).toBe('Default');
            expect(new IncludeNode('inc3', 'object', lookup).run(ctx)).toBe('Default');
            expect(new IncludeNode('inc4', 'array', lookup).run(ctx)).toBe('Default');
            expect(new IncludeNode('inc5', 'null', lookup).run(ctx)).toBe('Default');
            expect(new IncludeNode('inc6', 'undefined', lookup).run(ctx)).toBe('Default');
        });
        it('only returns string values from lookup', () => {
            ...
            const node1 = new IncludeNode('inc1', 'valid', lookup);
            const node2 = new IncludeNode('inc2', 'invalid', lookup);
            const ctx = createContext();
    ...
    describe('RuntimeNode base class', () => {
        ...
        it('can be extended with async run method', async () => {
            class AsyncTestNode extends RuntimeNode {
                async run(ctx) {
    ...
    describe('Type safety and contracts', () => {
        it('WeightedChoiceNode always returns string', () => {
            ...
            const result = node.run(createContext());
