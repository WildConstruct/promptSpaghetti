...
describe('InspectorPanel - Comprehensive Testing', () => {
    ...
    describe('Error Handling', () => {
        ...
        it('handles undefined node gracefully', () => {
            expect(() => {
                render(_jsx(InspectorPanel, { ...defaultProps, node: undefined }));
            }).not.toThrow();
        });
        it('handles malformed node data gracefully', () => {
            ...
            expect(() => {
                render(_jsx(InspectorPanel, { ...defaultProps, node: malformedNode }));
            }).not.toThrow();
        });
        it('handles invalid schema gracefully', () => {
            expect(() => {
                render(_jsx(InspectorPanel, { ...defaultProps, schema: null }));
            }).not.toThrow();
        });
        it('handles onChange errors gracefully', async () => {
            ...
            render(_jsx(TestErrorBoundary, { onError: onError, children: _jsx(InspectorPanel, { ...defaultProps, onChange: errorOnChange }) }));
        ...
        it('handles store access errors gracefully', () => {
            ...
            const onError = jest.fn();
            render(_jsx(TestErrorBoundary, { onError: onError, children: _jsx(InspectorPanel, { ...defaultProps }) }));
    ...
    describe('Performance Concerns', () => {
        it('debounces rapid prop changes', async () => {
            ...
            render(_jsx(InspectorPanel, { ...defaultProps, onChange: onChange }));
            const input = screen.queryByRole('textbox');
            if (input) {
                // Rapid typing
        ...
        it('memoizes complex computations', () => {
            ...
            const startTime = performance.now();
            const { rerender } = render(_jsx(InspectorPanel, { ...defaultProps, schema: complexSchema }));
            // Rerender with same props - should use memoized result
            rerender(_jsx(InspectorPanel, { ...defaultProps, schema: complexSchema }));
            const endTime = performance.now();
        ...
        it('efficiently handles large variation lists', () => {
            ...
            const startTime = performance.now();
            render(_jsx(InspectorPanel, { ...defaultProps, node: largeVariationNode }));
            const endTime = performance.now();
    ...
    describe('Accessibility & Code Quality', () => {
        ...
        it('handles cleanup properly on unmount', () => {
            const { unmount } = render(_jsx(InspectorPanel, { ...defaultProps }));
    ...
    describe('Panel Resizing', () => {
        ...
        it('persists resize preferences', () => {
            const { rerender } = render(_jsx(InspectorPanel, { ...defaultProps }));
            // Simulate resize
            const inspectorPanel = screen.getByText('Test Node Properties').closest('aside');
            const resizeHandle = inspectorPanel?.querySelector('[data-testid="resize-handle"]');
            if (resizeHandle) {
            ...
            rerender(_jsx(InspectorPanel, { ...defaultProps }));
