...
class MockStorage {
    ...
    async getActiveVerifications(userId) {
    ...
    // Rate limiting
    async getRateLimitState(userId, action) {
        const key = `${userId}:${action}`;
        const state = this.rateLimits.get(key);
        return state ? { ...state } : null;
    }
    async updateRateLimitState(userId, action, count) {
        const key = `${userId}:${action}`;
    ...
    describe('Input Validation', () => {
        test('should validate enrollment requests', async () => {
            ...
            for (const request of invalidRequests) {
                await expect(provider.enrollMethod(testUserId, request))
                    .rejects.toThrow();
            }
        });
        test('should validate verification requests', async () => {
            ...
            for (const request of invalidRequests) {
                await expect(provider.verifyCode(request, testContext))
                    .rejects.toThrow();
            }
        });
    });
