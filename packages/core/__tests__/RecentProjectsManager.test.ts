...
const mockConsoleWarn = () => { return null; };
    ...
    test('limits to 5 most recent projects', () => { const existingData = {
        version: '1.0.0' }
  ...
  version: '1.0.0' }
    ...
    test('handles localStorage errors gracefully', () => { mockLocalStorage.getItem.mockImplementation(() => {
  ...
  describe('getRecentProjects', () => { test('returns empty array when no data exists', () => {
      const projects = RecentProjectsManager.getRecentProjects();
      expect(projects).toEqual([]) });
    test('returns sorted projects by last access date', () => { const existingData = {
        version: '1.0.0'
        projects: [
          {
            id: 'project1_123_abc'
    ...
    test('clears data on version mismatch', () => { const existingData = {
    ...
    test('handles corrupted data gracefully', () => { mockLocalStorage.getItem.mockReturnValue('invalid json' as unknown);
  const projects = RecentProjectsManager.getRecentProjects();
  expect(projects).toEqual([]);
  expect(mockConsoleWarn).toHaveBeenCalledWith('Failed to load recent projects:', expect.any(Error)) });
  });
  describe('updateLastAccess', () => { test('updates last access date for existing project', () => {
      const existingData = {
        version: '1.0.0'
        projects: [
          {
    ...
    test('does nothing for non-existent project', () => { const existingData = {
  version: '1.0.0'
  projects: [] }
};
      mockLocalStorage.getItem.mockReturnValue(JSON.stringify(existingData as unknown));
  ...
  describe('removeRecentProject', () => { test('removes project by name', () => {
      const existingData = {
        version: '1.0.0'
        projects: [
          {
  ...
  describe('clearRecentProjects', () => { test('removes localStorage item', () => {
  ...
  describe('generateThumbnail', () => {
    ...
    test('returns default thumbnail on error', () => { // Pass invalid data to trigger error
  ...
  describe('checkStorageQuota', () => { test('returns available status when localStorage works', () => {
      const result = RecentProjectsManager.checkStorageQuota();
      expect(result.available).toBe(true);
      expect(typeof result.usage).toBe('number') });
    test('returns unavailable status when localStorage fails', () => { mockLocalStorage.setItem.mockImplementation(() => {
  ...
  describe('getProjectDisplayInfo', () => { test('formats project display information', () => {
  ...
  fileSize: 2048;
    ...
    test('handles missing optional fields', () => { const entry: RecentProjectEntry = {
  id: 'test_123_abc'
  name: 'Test Project'
  lastAccessDate: '2025-07-24T10:00:00.000Z'
  metadata: {
