...
describe('ProjectManager', () => {
  ...
  beforeEach(() => { mockCreateElement.mockClear();
  ...
  id: 'node1'
    ...
    mockSaveOptions = { name: 'Test Project'
  ...
  describe('sanitizeFileName', () => { test('removes invalid characters', () => {
  const result = ProjectManager.sanitizeFileName('test<>:"/\\|?*file');
  expect(result).toBe('test_file.psg') });
    test('replaces spaces with underscores', () => { const result = ProjectManager.sanitizeFileName('test file name');
      expect(result).toBe('test_file_name.psg') });
    test('adds .psg extension if missing', () => { const result = ProjectManager.sanitizeFileName('testfile');
      expect(result).toBe('testfile.psg') });
    test('preserves existing .psg extension', () => { const result = ProjectManager.sanitizeFileName('testfile.psg');
      expect(result).toBe('testfile.psg') });
    test('handles empty string', () => { const result = ProjectManager.sanitizeFileName('');
      expect(result).toBe('untitled.psg') });
    test('removes leading/trailing underscores', () => { const result = ProjectManager.sanitizeFileName('___test___file___');
      expect(result).toBe('test_file.psg') });
  });
  describe('saveProjectToDevice', () => { test('successfully saves project with all options', async () => {
    ...
    test('handles project with no description', async () => { const optionsWithoutDescription = {
  ...mockSaveOptions,
  description: undefined }
    ...
    test('creates default fileName when not provided', async () => { const optionsWithoutFileName = {
  ...mockSaveOptions,
  fileName: undefined }
};
      const result = await ProjectManager.saveProjectToDevice(;);
    ...
    test('sanitizes fileName', async () => { const optionsWithInvalidFileName = {
  ...mockSaveOptions,
  fileName: 'test<>file?.psg' }
    ...
    test('calls serialization functions', async () => { const result = await ProjectManager.saveProjectToDevice(;);
        mockGraphData,
        mockSaveOptions }
        mockSettings
      );
  ...
  describe('loadProjectFromDevice', () => { test('creates Promise-based interface', () => {
  ...
  describe('integration with existing functionality', () => { test('maintains singleton pattern', () => {
      const instance1 = ProjectManager.getInstance();
      const instance2 = ProjectManager.getInstance();
      expect(instance1).toBe(instance2) });
    test('preserves existing recent files functionality', () => { const manager = ProjectManager.getInstance();
    ...
    test('preserves existing favorite files functionality', () => { const manager = ProjectManager.getInstance();
