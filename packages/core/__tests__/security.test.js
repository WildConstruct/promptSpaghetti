...
import { SecurityValidation, SecurityTesting } from '../validation/security';
import { GraphSchema } from '../graphSchema';
import { SetVariableNode, GetVariableNode, IncludeNode } from '../runtime';
describe('Security Validation Framework', () => {
    describe('SecurityValidation.validateSafeString', () => {
        ...
        it('should reject non-string inputs', () => {
            expect(SecurityValidation.validateSafeString(123)).toBe(false);
            expect(SecurityValidation.validateSafeString(null)).toBe(false);
            expect(SecurityValidation.validateSafeString(undefined)).toBe(false);
            expect(SecurityValidation.validateSafeString({})).toBe(false);
        });
    });
...
describe('Runtime Security Tests', () => {
    ...
    describe('IncludeNode Runtime Security', () => {
        ...
        it('should handle non-string results safely', () => {
            const lookup = {
                template1: 123, // Non-string value
                template2: null,
                template3: undefined,
            };
...
describe('Comprehensive Security Test Suite', () => {
    ...
    it('should detect injection attempts in string validation', () => {
        const result = SecurityTesting.testInjectionProtection(SecurityValidation.validateSafeString, 'String Validation');
        expect(result.failed).toBe(0);
        expect(result.passed).toBeGreaterThan(0);
    });
    it('should detect injection attempts in expression validation', () => {
        const result = SecurityTesting.testInjectionProtection(SecurityValidation.validateSafeExpression, 'Expression Validation');
        expect(result.failed).toBe(0);
        expect(result.passed).toBeGreaterThan(0);
    });
    it('should detect injection attempts in property key validation', () => {
        const result = SecurityTesting.testInjectionProtection(SecurityValidation.validateSafePropertyKey, 'Property Key Validation');
        expect(result.failed).toBe(0);
        expect(result.passed).toBeGreaterThan(0);
    });
});
