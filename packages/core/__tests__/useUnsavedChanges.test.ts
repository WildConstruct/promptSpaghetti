...
describe('useUnsavedChanges', () => { beforeEach(() => {
    jest.clearAllMocks() });
  afterEach(() => { jest.clearAllMocks() });
  test('sets up beforeunload event listener when hasUnsavedChanges is true', () => { renderHook(() => useUnsavedChanges({)
  ...
  test('beforeunload handler prevents default when hasUnsavedChanges is true', () => { renderHook(() => useUnsavedChanges({)
  hasUnsavedChanges: true
  projectName: 'Test Project' }
}));
  ...
  test('beforeunload handler does not prevent default when hasUnsavedChanges is false', () => { renderHook(() => useUnsavedChanges({)
  ...
  projectName: 'Test Project' }
    ...
    act(() => { result.current.confirmNavigation('test action', callback) });
  ...
  projectName: 'Test Project' }
    ...
    act(() => { result.current.confirmNavigation('creating a new project', callback) });
  ...
  projectName: 'Test Project' }
    ...
    // First navigation
    act(() => { result.current.confirmNavigation('action 1', callback1) });
    expect(result.current.dialogAction).toBe('action 1');
    // Second navigation (should override first)
    act(() => { result.current.confirmNavigation('action 2', callback2) });
