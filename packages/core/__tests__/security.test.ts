...
describe('Security Validation Framework', () => { describe('SecurityValidation.validateSafeString', () => {
  ...
  expect(SecurityValidation.validateSafeString(input)).toBe(false) });
    ...
    it('should allow safe strings', () => { const safeInputs = [
        'hello world',
        'user input text',
  ...
  describe('SecurityValidation.validateSafeExpression', () => { it('should block dangerous expression patterns', () => {
  ...
  expect(SecurityValidation.validateSafeExpression(expr)).toBe(false) });
    ...
    it('should allow safe expressions', () => { const safeExpressions = [
        'variable === "value"',
        'number > 10',
  ...
  expect(SecurityValidation.validateSafeExpression(expr)).toBe(true) });
    ...
    it('should reject overly long expressions', () => { const longExpression = 'a'.repeat(1000);
      expect(SecurityValidation.validateSafeExpression(longExpression)).toBe(false) });
  });
  describe('SecurityValidation.validateSafePropertyKey', () => { it('should block dangerous property keys', () => {
      const dangerousKeys = [
        '__proto__',
        'constructor',
        'prototype',
  ...
  expect(SecurityValidation.validateSafePropertyKey(key)).toBe(false) });
    ...
    it('should allow safe property keys', () => { const safeKeys = [
        'username',
        'user_id',
        'data-value',
        'item123',
  ...
  expect(SecurityValidation.validateSafePropertyKey(key)).toBe(true) });
    ...
    it('should reject invalid key formats', () => { const invalidKeys = [
        '',
        'key with spaces',
        'key.with.dots',
        'key/with/slashes',
  ...
  describe('SecurityValidation.validateSafeValue', () => { it('should allow safe primitive values', () => {
    ...
    it('should validate safe arrays', () => { expect(SecurityValidation.validateSafeValue(['a', 'b', 'c'])).toBe(true);
    ...
    it('should reject infinite and NaN numbers', () => { expect(SecurityValidation.validateSafeValue(Infinity)).toBe(false);
  ...
  describe('SecurityValidation.safePropertyAccess', () => {
    ...
    it('should handle invalid objects safely', () => { expect(SecurityValidation.safePropertyAccess(null, 'key', 'fallback')).toBe('fallback');
...
describe('Zod Schema Security Tests', () => { describe('DEBT-001: SetVariableNodeSchema Security', () => {,
  ...
  expect(() => GraphSchema.parse(graph)).toThrow() });
    ...
    it('should accept safe values in SetVariable schema', () => { const safeGraphs = [
  {
  nodes: [{,
  id: 'test',
  type: 'SetVariable',
  ...
  describe('DEBT-002: ConditionalNodeSchema Security', () => { it('should reject dangerous expressions in Conditional schema', () => {
  const dangerousGraphs = [
  {
  nodes: [{,
  id: 'test',
  ...
  expect(() => GraphSchema.parse(graph)).toThrow() });
    ...
    it('should accept safe expressions in Conditional schema', () => { const safeGraphs = [
  {
  nodes: [{,
  id: 'test',
  type: 'Conditional',
  ...
  describe('DEBT-003: IncludeNodeSchema Security', () => { it('should reject dangerous property keys in Include schema', () => {
  const dangerousGraphs = [
  {
  nodes: [{,
  id: 'test',
  ...
  expect(() => GraphSchema.parse(graph)).toThrow() });
    ...
    it('should accept safe property keys in Include schema', () => { const safeGraphs = [
  {
  nodes: [{,
  id: 'test',
  type: 'Include',
...
describe('Runtime Security Tests', () => {
  ...
  describe('SetVariableNode Runtime Security', () => { it('should reject dangerous variable keys at runtime', () => {
  ...
  const node = new SetVariableNode('test', key, 'value');
    ...
    it('should accept safe variable keys at runtime', () => { const safeKeys = ['username', 'user_id', 'data123'];
      safeKeys.forEach(key => {)
  const node = new SetVariableNode('test', key, 'test-value');
    ...
    it('should handle safe values correctly', () => {
      ...
      testCases.forEach(({ key, value }) => { const node = new SetVariableNode('test', key, value);
        node.run(ctx);
  ...
  describe('GetVariableNode Runtime Security', () => { beforeEach(() => {
  ctx.variables = {
  safeKey: 'safe value',
  normalProperty: 'normal value' }
};
    });
    it('should reject dangerous variable keys at runtime', () => { const dangerousKeys = ['__proto__', 'constructor', 'prototype'];
      dangerousKeys.forEach(key => {)
  const node = new GetVariableNode('test', key);
        const result = node.run(ctx);
        expect(result).toBeUndefined() });
    });
    it('should return safe variable values', () => { const node = new GetVariableNode('test', 'safeKey');
      const result = node.run(ctx);
      expect(result).toBe('safe value') });
    it('should return undefined for non-existent keys', () => { const node = new GetVariableNode('test', 'nonExistentKey');
      const result = node.run(ctx);
      expect(result).toBeUndefined() });
  });
  describe('IncludeNode Runtime Security', () => { it('should handle safe property access', () => {
  const lookup = {
  ...
  template2: 'Another template' }
    ...
    it('should reject dangerous property access', () => { const lookup = {
  template1: 'Hello World' }
};
      const dangerousKeys = ['__proto__', 'constructor', 'prototype'];
      dangerousKeys.forEach(key => {)
  const node = new IncludeNode('test', key, lookup);
    ...
    it('should handle non-existent properties safely', () => { const lookup = {
  template1: 'Hello World' }
};
  ...
  const node = new IncludeNode('test', 'key', lookup as any);
    ...
    it('should handle non-string results safely', () => { const lookup = {
...
describe('Comprehensive Security Test Suite', () => { it('should run all security tests and pass', () => {
    const testResults = SecurityTesting.runSecurityTests();
    expect(testResults).toBe(true) });
  it('should detect injection attempts in string validation', () => { const result = SecurityTesting.testInjectionProtection(;);
  ...
  it('should detect injection attempts in expression validation', () => { const result = SecurityTesting.testInjectionProtection(;);
  ...
  it('should detect injection attempts in property key validation', () => { const result = SecurityTesting.testInjectionProtection(;);
...
describe('Integration Security Tests', () => {
  ...
  it('should handle edge cases safely', () => {
    ...
    edgeCases.forEach(({ key, value }) => { const node = new SetVariableNode('test', key, value);
      node.run(ctx);
