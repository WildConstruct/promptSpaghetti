...
describe('Epic 8.5 Task 6: Weight Integration', () => { const mockNodeData = {
  id: 'test-node'
  ...
  test('useWeightControlIntegration should debounce preview requests', async () => {
    ...
    await waitFor(() => { expect(mockPreviewRequest).toHaveBeenCalledTimes(1) }, { timeout: 500 });
    expect(mockPreviewRequest).toHaveBeenCalledWith(options);
  });
  test('should integrate with weight visualization', () => { const nodeDataWithWeights = {
  ...mockNodeData
  ...
  test('should call global preview request on weight changes', async () => {
    ...
    const updatedNodeData = { ...mockNodeData
