...
import { WeightControlSlider } from '../components/Inspector/WeightControlSlider';
...
describe('WeightControlSlider', () => {
    ...
    const mockOnPreviewRequest = jest.fn();
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe('Rendering', () => {
        it('renders weight control slider with options', () => {
            render(_jsx(WeightControlSlider, { options: mockOptions, onOptionsChange: mockOnOptionsChange }));
        ...
        it('renders empty state when no options provided', () => {
            render(_jsx(WeightControlSlider, { options: [], onOptionsChange: mockOnOptionsChange }));
    ...
    describe('Weight Distribution Visualization', () => {
        ...
        it('shows visual weight distribution bar', () => {
            const { container } = render(_jsx(WeightControlSlider, { options: mockOptions, onOptionsChange: mockOnOptionsChange }));
    ...
    describe('Preset Weight Patterns', () => {
        ...
        it('normalizes weights when normalize button is clicked', async () => {
            ...
            render(_jsx(WeightControlSlider, { options: unnormalizedOptions, onOptionsChange: mockOnOptionsChange }));
    ...
    describe('Locking Functionality', () => {
        ...
        it('respects locked weights during equalization', async () => {
            ...
            render(_jsx(WeightControlSlider, { options: optionsWithLock, onOptionsChange: mockOnOptionsChange }));
            const equalButton = screen.getByText('⚖️ Equal');
            fireEvent.click(equalButton);
            await waitFor(() => {
    ...
    describe('Real-time Preview Integration', () => {
        it('triggers preview requests when weights change', async () => {
            render(_jsx(WeightControlSlider, { options: mockOptions, onOptionsChange: mockOnOptionsChange, onPreviewRequest: mockOnPreviewRequest, showPreview: true, previewDebounceMs: 100 }));
            const sliders = screen.getAllByRole('slider');
            const firstSlider = sliders[0];
            fireEvent.change(firstSlider, { target: { value: '40' } });
        ...
        it('does not trigger preview when showPreview is false', async () => {
            render(_jsx(WeightControlSlider, { options: mockOptions, onOptionsChange: mockOnOptionsChange, onPreviewRequest: mockOnPreviewRequest, showPreview: false }));
            const sliders = screen.getAllByRole('slider');
            const firstSlider = sliders[0];
            fireEvent.change(firstSlider, { target: { value: '40' } });
    ...
    describe('Disabled State', () => {
        it('disables controls when disabled prop is true', () => {
            render(_jsx(WeightControlSlider, { options: mockOptions, onOptionsChange: mockOnOptionsChange, disabled: true }));
            const sliders = screen.getAllByRole('slider');
