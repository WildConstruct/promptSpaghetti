import { AdvancedIOHandler, IOSpecBuilder, TypedInputs } from '../runtime/io-system';
...
describe('Runtime IO System - Comprehensive Tests', () => {
    describe('AdvancedIOHandler', () => {
        ...
        it('validates inputs correctly', () => {
            ...
            expect(invalidResult.errors).toContain('Invalid type for Number Input: expected number, got string');
        });
        ...
        it('performs type coercion', () => {
            ...
            expect(resolved.metadata.get('num')?.warnings).toContain('Type coerced from string to number');
        });
        it('validates outputs', () => {
            ...
            expect(invalidResult.valid).toBe(false);
            expect(invalidResult.errors).toContain('Invalid type for Result: expected string, got number');
        });
        ...
        it('validates string constraints', () => {
            ...
            expect(patternResult.errors).toContain('Value does not match required pattern: ^[A-Z]+$');
        });
        ...
        it('validates allowed values', () => {
            ...
            expect(result.errors).toContain('Value \'option4\' is not in allowed values: option1, option2, option3');
        });
                ...
                inputs: [
                    {
                        ...
                        constraints: {
                            customValidator: (value) => {
                                if (value === 'forbidden') {
                                    return ValidationHelpers.createInvalidResult(['Value \'forbidden\' is not allowed'], ['Consider using a different value']);
                                }
        ...
        it('handles coercion failures', () => {
            ...
            expect(resolved.metadata.get('num')?.warnings).toContain('Type coercion failed: Cannot convert not-a-number to number');
        });
    ...
    describe('IOSpecBuilder', () => {
        ...
        it('creates multi-input specifications', () => {
            const multi = IOSpecBuilder.createMultiInput(['First', 'Second', 'Third'], 'Combined');
    ...
    describe('Type coercion edge cases', () => {
        ...
        it('handles invalid number array coercion', () => {
            ...
            expect(resolved.metadata.get('numArr')?.warnings).toContain('Type coercion failed: Cannot convert not-a-number to number');
        });
    });
