...
describe('Variable Name Validation', () => { describe('VARIABLE_NAME_PATTERN', () => {
    it('should match valid alphanumeric names', () => {
      ...
      for (const name of validNames) { expect(VARIABLE_NAME_PATTERN.test(name)).toBe(true) });
    it('should reject invalid characters', () => {
      ...
      for (const name of invalidNames) { expect(VARIABLE_NAME_PATTERN.test(name)).toBe(false) });
  });
  describe('validateVariableName', () => { it('should validate correct variable names', () => {
      const validNames = [
        'userName'
        'user_id'
        'user-id'
      ...
      for (const name of validNames) { expect(SecurityValidation.validateVariableName(name)).toBe(true) });
    it('should enforce 64 character limit', () => { // 64 characters - should pass
    ...
    it('should reject reserved keywords', () => { const reservedKeywords = [
        '__proto__'
        'constructor'
        'prototype'
        '__defineGetter__'
      ...
      for (const keyword of reservedKeywords) { expect(SecurityValidation.validateVariableName(keyword)).toBe(false) });
    it('should reject names containing dangerous patterns', () => { const dangerousNames = [
        'my__proto__var'
        'constructorName'
        'prototypeValue'
        'user__proto__id'
      ...
      for (const name of dangerousNames) { expect(SecurityValidation.validateVariableName(name)).toBe(false) });
    it('should handle edge cases', () => {
      expect(SecurityValidation.validateVariableName('')).toBe(false);
      expect(SecurityValidation.validateVariableName(null as any)).toBe(false);
      expect(SecurityValidation.validateVariableName(undefined as any)).toBe(false);
      expect(SecurityValidation.validateVariableName(123 as any)).toBe(false);
      expect(SecurityValidation.validateVariableName({} as any)).toBe(false);
      expect(SecurityValidation.validateVariableName([] as any)).toBe(false);
    });
  });
  describe('Zod Schema - variableName', () => { const variableNameSchema = SecureValidation.variableName();
    it('should validate correct variable names', () => {
      ...
      for (const name of validNames) { const result = variableNameSchema.safeParse(name);
        expect(result.success).toBe(true) });
    it('should provide meaningful error messages', () => { // Empty string
  ...
  describe('Integration with SetVariable', () => {
    it('should match SetVariable security requirements', () => {
      ...
      for (const testCase of testCases) { const result = SecurityValidation.validateVariableName(testCase.name);
        expect(result).toBe(testCase.expected) });
  });
  describe('Performance', () => { it('should handle validation efficiently', () => {
      ...
      for (let i = 0; i < iterations; i++) { for (const name of testNames) {
