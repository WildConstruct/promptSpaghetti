 ...
 */
import { DataSensitivityLevel, DataSensitivityUtils, DATA_SENSITIVITY_DEFINITIONS, DATA_SENSITIVITY_GUIDELINES, DataSensitivityLevelSchema } from '../security/DataSensitivityLevels';
describe('DataSensitivityLevels', () => {
    ...
    describe('DATA_SENSITIVITY_DEFINITIONS', () => {
        ...
        describe('marking requirements', () => {
            ...
            it('should have distinct colors for different levels', () => {
                const colors = Object.values(DataSensitivityLevel).map(level => DATA_SENSITIVITY_DEFINITIONS[level].markingRequirements.color);
    ...
    describe('DataSensitivityUtils', () => {
        ...
        describe('validateSensitivityAssignment', () => {
            it('should validate appropriate assignments', () => {
                const result = DataSensitivityUtils.validateSensitivityAssignment('marketing', DataSensitivityLevel.PUBLIC);
            ...
            it('should reject inappropriate assignments for PII data', () => {
                const result = DataSensitivityUtils.validateSensitivityAssignment('email', DataSensitivityLevel.PUBLIC);
            ...
            it('should consider context for PII detection', () => {
                const result = DataSensitivityUtils.validateSensitivityAssignment('customer_data', DataSensitivityLevel.INTERNAL, { containsPII: true });
                expect(result.valid).toBe(false);
                expect(result.recommendedLevel).toBe(DataSensitivityLevel.RESTRICTED);
                expect(result.reasons[0]).toContain('PII');
            });
            it('should handle publicly available data', () => {
                const result = DataSensitivityUtils.validateSensitivityAssignment('general_data', DataSensitivityLevel.CONFIDENTIAL, { publiclyAvailable: true });
                expect(result.valid).toBe(false);
        ...
        describe('compareSensitivityLevels', () => {
            it('should correctly compare sensitivity levels', () => {
                ...
                expect(DataSensitivityUtils.compareSensitivityLevels(DataSensitivityLevel.RESTRICTED, DataSensitivityLevel.PUBLIC)).toBeGreaterThan(0);
                // Lower sensitivity should return negative  
                expect(DataSensitivityUtils.compareSensitivityLevels(DataSensitivityLevel.PUBLIC, DataSensitivityLevel.RESTRICTED)).toBeLessThan(0);
                // Same level should return 0
                expect(DataSensitivityUtils.compareSensitivityLevels(DataSensitivityLevel.CONFIDENTIAL, DataSensitivityLevel.CONFIDENTIAL)).toBe(0);
            });
        ...
        describe('getHigherSensitivityLevel', () => {
            it('should return the higher sensitivity level', () => {
                expect(DataSensitivityUtils.getHigherSensitivityLevel(DataSensitivityLevel.PUBLIC, DataSensitivityLevel.RESTRICTED)).toBe(DataSensitivityLevel.RESTRICTED);
                expect(DataSensitivityUtils.getHigherSensitivityLevel(DataSensitivityLevel.CONFIDENTIAL, DataSensitivityLevel.INTERNAL)).toBe(DataSensitivityLevel.CONFIDENTIAL);
                expect(DataSensitivityUtils.getHigherSensitivityLevel(DataSensitivityLevel.INTERNAL, DataSensitivityLevel.INTERNAL)).toBe(DataSensitivityLevel.INTERNAL);
            });
        });
    ...
    describe('Integration tests', () => {
        it('should handle complex classification scenarios', () => {
            ...
            const emailValidation = DataSensitivityUtils.validateSensitivityAssignment('email', DataSensitivityLevel.RESTRICTED);
        ...
        it('should provide consistent classification recommendations', () => {
            ...
            const emailRule = DATA_SENSITIVITY_GUIDELINES.automatedClassificationRules
                .find(rule => rule.dataType === 'email');
            const manualValidation = DataSensitivityUtils.validateSensitivityAssignment('email', emailRule.recommendedLevel);
