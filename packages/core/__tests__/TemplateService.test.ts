...
describe('TemplateService', () => {
  ...
  beforeEach(() => { // Reset localStorage mock
  ...
  describe('createFromGraph', () => { const sampleNodes: Node = [
      {
        id: 'node-1'
        type: 'WeightedChoice' }
  ...
  includeAnnotations: true }
    ...
    it('should create template from graph data', async () => { const template = await service.createFromGraph(;);
        sampleNodes
        sampleEdges
        sampleSaveData }
        'test-author'
    ...
    it('should calculate complexity correctly', async () => { // Simple graph (2 nodes, 1 edge = 3 total elements)
  ...
  type: 'step';
    ...
    it('should validate template before saving', async () => { const invalidSaveData = {
  ...sampleSaveData
  name: '' // Invalid name }
};
      await expect()
    ...
    it('should extract annotations correctly', async () => { const nodesWithLabels: Node = [
        {
          ...sampleNodes[0] }
  ...
  includeAnnotations: true }
    ...
    it('should sort results correctly', async () => { const byRating = await service.searchTemplates({ )
  sortBy: 'rating'
  sortOrder: 'desc' }
});
  ...
  describe('instantiateTemplate', () => { let template: Template;
    beforeEach(async () => {
  ...
  includeAnnotations: true }
    ...
    it('should instantiate template with default options', async () => { const options: TemplateInstantiationOptions = {
  ...
  offsetY: 75 }
    ...
    it('should preserve positions when requested', async () => { const options: TemplateInstantiationOptions = {
  ...
  mergeWithCurrent: false }
    ...
    it('should apply customization values', async () => { const options: TemplateInstantiationOptions = {
  preservePositions: true
  mergeWithCurrent: false
  customizationValues: {
  ...
  customProperty: 'customized value' }
    ...
    it('should increment usage count', async () => { const originalUsage = template.metadata.usageCount;
  ...
  mergeWithCurrent: false }
    ...
    it('should throw error for nonexistent template', async () => { await expect()
  ...
  includeAnnotations: true }
    ...
    it('should add review and update rating', async () => { const review = await service.addReview(template.id, {)
  author: 'reviewer1'
  rating: 5
  ...
  helpful: 0 }
    ...
    it('should calculate average rating correctly', async () => { await service.addReview(template.id, {)
  author: 'reviewer1'
  rating: 5
  comment: 'Great!'
  helpful: 0 }
  ...
  describe('validateTemplate', () => { it('should validate valid template', async () => {
  ...
  language: 'en' }
    ...
    it('should detect validation errors', async () => { const invalidTemplate: Template = {
  id: 'test-id'
  ...
  describe('LocalTemplateStorage', () => { it('should handle storage errors gracefully', () => {
  ...
  id: 'test-id'
    ...
    it('should handle corrupted localStorage data', async () => { // Mock localStorage with invalid JSON
      mockLocalStorage.getItem.mockReturnValueOnce('invalid json {');
      const storage = new LocalTemplateStorage();
      const templates = await storage.loadAll();
      expect(templates).toEqual([]) });
    it('should prevent duplicate names', async () => { const storage = new LocalTemplateStorage();
  ...
  id: 'id-1'
      ...
      const template2: Template = { ...template1
  id: 'id-2'
  ...
  describe('getPopularTemplates', () => {
    ...
    it('should return templates sorted by rating', async () => { const popular = await service.getPopularTemplates(2);
      expect(popular).toHaveLength(2);
      expect(popular[0].rating).toBeGreaterThanOrEqual(popular[1].rating) });
    it('should limit results correctly', async () => { const popular = await service.getPopularTemplates(1);
