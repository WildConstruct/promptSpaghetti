...
class MockFileReader { result: string | null = null
...
describe('User Interaction Integration Tests', () => { beforeEach(() => {
    jest.clearAllMocks();
    mockLocalStorage.getItem.mockReturnValue(null as unknown) });
  afterEach(() => { jest.clearAllMocks() });
  describe('Keyboard Shortcuts Integration', () => { test('Ctrl+S and Cmd+S keyboard shortcuts work correctly', () => {
  ...
  return };
    ...
    test('keyboard shortcuts are prevented from browser default behavior', () => { const handleKeyDown = (event: KeyboardEvent) => { }
  if ((event.ctrlKey || event.metaKey) && event.key === 's') { event.preventDefault() };
    ...
    test('keyboard shortcuts work with modifier key combinations', () => { let saveTriggered = false;
  ...
  describe('Drag-and-Drop Integration', () => { test('drag-and-drop of .psg files works correctly', async () => {
  let droppedProject: unknown = null;
  let statusMessage = '';
  // Simulate drag-and-drop handler (as in GraphEditor)
  const handleDrop = async (event: DragEvent) => { }
  event.preventDefault();
  if (event.dataTransfer?.files && event.dataTransfer.files.length > 0) { const file = event.dataTransfer.files[0];
 ...
 else { statusMessage = 'Only .psg files are supported for drag and drop' };
      // Create mock file and drag event
      const mockFile = new File([''], 'test-project.psg', { type: 'application/json' });
      const dragEvent = { preventDefault: jest.fn<unknown, unknown>()
  dataTransfer: {
  files: [mockFile] }
 as any;
    ...
    test('drag-and-drop rejects non-.psg files', async () => { let statusMessage = '';
  const handleDrop = async (event: DragEvent) => { }
  event.preventDefault();
  if (event.dataTransfer?.files && event.dataTransfer.files.length > 0) { const file = event.dataTransfer.files[0];
  if (file.name.toLowerCase().endsWith('.psg')) {
  statusMessage = 'PSG file accepted' } else { statusMessage = 'Only .psg files are supported for drag and drop' };
      // Test with .txt file
 ...
 as any;
    ...
    test('drag-and-drop handles case-insensitive file extensions', async () => { let projectLoaded = false;
  const handleDrop = async (event: DragEvent) => { }
  event.preventDefault();
  if (event.dataTransfer?.files && event.dataTransfer.files.length > 0) { const file = event.dataTransfer.files[0];
  if (file.name.toLowerCase().endsWith('.psg')) {
  projectLoaded = true };
      ...
      for (const file of testFiles) { projectLoaded = false;
        const dragEvent = {
 ...
 as any;
    ...
    test('drag-and-drop handles file reading errors', async () => {
      ...
      const handleDrop = async (event: DragEvent) => { event.preventDefault();
 ...
 as any;
  ...
  return mockOnSave();
      ...
      act(() => { result.current.confirmNavigation('creating a new project', () => {
  ...
  projectName: 'Test Project' }
      ...
      // Trigger navigation
      act(() => { result.current.confirmNavigation('loading a project', () => {
          actionCompleted = true });
      });
      expect(result.current.showUnsavedDialog).toBe(true);
      // User chooses don't save
      act(() => { result.current.handleDontSave() });
      expect(actionCompleted).toBe(true);
      expect(result.current.showUnsavedDialog).toBe(false);
    });
  ...
  projectName: 'Test Project' }
      ...
      // Trigger navigation
      act(() => { result.current.confirmNavigation('closing the application', () => {
          actionCompleted = true });
      });
      expect(result.current.showUnsavedDialog).toBe(true);
      // User cancels
    ...
    test('browser beforeunload protection works correctly', () => { renderHook(() => useUnsavedChanges({)
  hasUnsavedChanges: true
  projectName: 'Test Project' }
}));
      ...
      // Test the handler
      const mockEvent = { preventDefault: jest.fn<unknown, unknown>()
  ...
  projectName: 'Test Project' }
      ...
      // Trigger navigation
      act(() => { result.current.confirmNavigation('any action', () => {
          actionCompleted = true });
      });
  ...
  projectName: 'Current Project' }
      ...
      const handleKeyDown = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key === 'o') {
          event.preventDefault();
  ...
  projectName: 'Current Project' }
      ...
      const dragEvent = { preventDefault: jest.fn<unknown, unknown>() }
        dataTransfer: { files: [mockFile] }
 as any;
      await act(async () => { await handleDrop(dragEvent) });
      // Should show confirmation dialog
      expect(result.current.showUnsavedDialog).toBe(true);
      expect(projectLoaded).toBe(false);
      // User confirms don't save
      await act(async () => { result.current.handleDontSave() });
  ...
  projectName: 'Test Project' }
      ...
      actions.forEach((action, index) => { act(() => {
