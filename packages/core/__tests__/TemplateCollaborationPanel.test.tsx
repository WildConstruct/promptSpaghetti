...
const mockAnalytics = { totalUses: 1250
  uniqueUsers: 45
  successRate: 0.92
  averageRating: 4.3
  forkCount: 8 }
...
const defaultProps = {};
describe('TemplateCollaborationPanel', () => { beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('Basic Rendering', () => {
    it('renders without crashing when visible', async () => {
      ...
      await waitFor(() => { expect(screen.getByText('👥 Template Collaboration')).toBeInTheDocument() });
    });
    ...
    it('displays template information in header', async () => {
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText(/Collaboration Test Template • 3 collaborators/)).toBeInTheDocument() });
    });
  });
  describe('Data Loading', () => {
    it('loads collaboration data on mount', async () => {
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { expect(mockUseExport.getTemplateCollaborators).toHaveBeenCalledWith('template-1');
  ...
  describe('Navigation Tabs', () => {
    ...
    it('switches between tabs', async () => {
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('Invite Collaborator')).toBeInTheDocument() });
  ...
  describe('Collaborators Tab', () => {
    ...
    it('handles collaborator invitation', async () => { const newCollaborator = {
  id: 'user-4'
  name: 'New User'
  ...
  lastActive: '2024-01-16T00:00:00Z' }
      ...
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { expect(screen.getByPlaceholderText('colleague@company.com')).toBeInTheDocument() });
      const emailInput = screen.getByPlaceholderText('colleague@company.com');
      await userEvent.type(emailInput, 'new@example.com');
      const inviteButton = screen.getByText(/Invite/);
      await userEvent.click(inviteButton);
      await waitFor(() => { expect(mockUseExport.inviteCollaborator).toHaveBeenCalledWith('template-1', {)
  email: 'new@example.com'
  role: 'viewer' }
});
      });
    ...
    it('allows role changes for non-owners', async () => {
      ...
      await waitFor(() => { expect(mockUseExport.updateCollaboratorRole).toHaveBeenCalledWith('template-1', 'user-2', 'viewer') });
    });
    it('allows removing non-owner collaborators', async () => {
      ...
      await waitFor(() => { expect(mockUseExport.removeCollaborator).toHaveBeenCalledWith('template-1', 'user-2') });
  ...
  describe('Activity Tab', () => {
    ...
    it('shows activity timestamps', async () => {
      ...
      const activityTab = screen.getByText('📈 Activity');
      await userEvent.click(activityTab);
      await waitFor(() => { // Check for formatted dates (exact format may vary based on locale)
    ...
    it('shows different activity indicators', async () => {
      ...
      const activityTab = screen.getByText('📈 Activity');
      await userEvent.click(activityTab);
      await waitFor(() => { // Activity indicators are shown as colored dots
  ...
  describe('Analytics Tab', () => {
    ...
    it('shows analytics cards with proper styling', async () => {
      ...
      const analyticsTab = screen.getByText('📊 Analytics');
      await userEvent.click(analyticsTab);
      await waitFor(() => { expect(screen.getByText('Total Uses')).toBeInTheDocument();
  ...
  describe('Sharing Tab', () => {
    it('displays sharing settings', async () => {
      ...
      await waitFor(() => { expect(screen.getByText('Public Sharing Settings')).toBeInTheDocument();
    ...
    it('handles public toggle', async () => {
      ...
      const sharingTab = screen.getByText('🌐 Sharing');
      await userEvent.click(sharingTab);
      await waitFor(() => { expect(screen.getByText('Make Template Public')).toBeInTheDocument() });
      const publicToggle = screen.getByRole('checkbox', { name: /Make Template Public/ });
      await userEvent.click(publicToggle);
      await waitFor(() => { expect(mockUseExport.updateShareSettings).toHaveBeenCalledWith('template-1', {)
  isPublic: true
  allowForks: true
  allowComments: true
  requireApproval: false }
    ...
    it('shows additional settings when public', async () => {
      ...
      const sharingTab = screen.getByText('🌐 Sharing');
      await userEvent.click(sharingTab);
      await waitFor(() => { expect(screen.getByText('Allow Forks')).toBeInTheDocument();
    ...
    it('generates share links', async () => {
      ...
      const publicToggle = screen.getByRole('checkbox', { name: /Make Template Public/ });
      await userEvent.click(publicToggle);
      await waitFor(() => { expect(screen.getByText('🔗 Generate Share Link')).toBeInTheDocument() });
      const generateButton = screen.getByText('🔗 Generate Share Link');
      await userEvent.click(generateButton);
      await waitFor(() => { expect(mockUseExport.generateShareLink).toHaveBeenCalledWith('template-1') });
    });
  });
  describe('Footer Actions', () => {
    it('handles template forking', async () => {
      ...
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { expect(screen.getByText('🍴 Fork Template')).toBeInTheDocument() });
      const forkButton = screen.getByText('🍴 Fork Template');
      await userEvent.click(forkButton);
      await waitFor(() => { expect(mockUseExport.forkTemplate).toHaveBeenCalledWith('template-1') });
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('handles invitation errors', async () => {
      ...
      const inviteButton = screen.getByText(/Invite/);
      await userEvent.click(inviteButton);
      await waitFor(() => { expect(screen.getByText(/Error: Invitation failed/)).toBeInTheDocument() });
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('provides proper button roles', async () => {
      render(<TemplateCollaborationPanel {...defaultProps} />);
      await waitFor(() => { const buttons = screen.getAllByRole('button');
