...
describe('Drag and Drop PSG Files', () => {
  ...
  test('handles .psg file drop successfully', async () => {
    const mockFile = {
  ...
  test('handles non-.psg file drop', async () => {
  const mockFile = {
  ...
  test('handles case-insensitive .psg extension', async () => {
    const mockFile = {
      name: 'TEST-PROJECT.PSG',
  ...
  test('handles deserialization failure', async () => {
  const mockFile = {
  ...
  test('handles null dataTransfer', async () => {
  const event = {
  preventDefault: jest.fn<unknown, unknown>(),
  dataTransfer: null,
} as any;
  ...
  test('updates graph store state correctly on successful load', async () => {
    const mockFile = {
      name: 'test-project.psg',
      text: jest.fn<unknown, unknown>().mockResolvedValue('{"fileType":"psg"}' as unknown)
    };
