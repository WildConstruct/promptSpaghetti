 ...
 */
import { SecurityAuditLogger, securityAudit, SecuritySeverity, SecurityEventCategory, auditSecurityEvent } from '../runtime/security-audit-logger';
describe('SecurityAuditLogger', () => {
    ...
    describe('Event Logging', () => {
        it('should log basic security event', () => {
            const eventId = logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Test expression validated', { expression: 'x > 5' });
        ...
        it('should generate unique event IDs', () => {
            const ids = new Set();
            for (let i = 0; i < 10; i++) {
                const id = logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, `Event ${i}`);
                ids.add(id);
            }
    ...
    describe('Expression Tracking', () => {
        it('should track unique expressions', () => {
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Test', { expression: 'x > 5' });
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Test', { expression: 'y < 10' });
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Test', { expression: 'x > 5' }); // Duplicate
            const stats = logger.getStatistics();
            expect(stats.uniqueExpressions).toBe(2);
        });
        it('should get events for specific expression', () => {
            const expr = 'dangerous.eval()';
            logger.logEvent(SecuritySeverity.WARNING, SecurityEventCategory.EXPRESSION_VALIDATION, 'Warning', { expression: expr });
            logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Blocked', { expression: expr }, true);
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Other', { expression: 'safe.value' });
        ...
        it('should check if expression has been blocked', () => {
            ...
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Safe', { expression: safeExpr }, false);
            logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Dangerous', { expression: dangerousExpr }, true);
    ...
    describe('Statistics', () => {
        beforeEach(() => {
            ...
            for (let i = 0; i < 3; i++) {
                logger.logEvent(SecuritySeverity.WARNING, SecurityEventCategory.MATH_FUNCTION_BLOCKED, `Math.random ${i}`, { expression: 'Math.random()' }, true);
            }
            for (let i = 0; i < 2; i++) {
                logger.logEvent(SecuritySeverity.CRITICAL, SecurityEventCategory.PROTOTYPE_POLLUTION_ATTEMPT, `__proto__ ${i}`, { expression: 'obj.__proto__' }, true);
            }
        });
        ...
        it('should track top blocked patterns', () => {
            ...
            for (let i = 0; i < 5; i++) {
                logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Blocked', { expression: `eval(${i})` }, true);
            }
            for (let i = 0; i < 3; i++) {
                logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Blocked', { expression: `new Function(${i})` }, true);
            }
            const stats = logger.getStatistics();
    ...
    describe('Memory Management', () => {
        ...
        it('should handle blocked pattern memory limits', () => {
            ...
            for (let i = 0; i < 1000; i++) {
                logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Blocked', { expression: `dangerous_${i}_pattern` }, true);
            }
    ...
    describe('Export Functionality', () => {
        beforeEach(() => {
            logger.logEvent(SecuritySeverity.INFO, SecurityEventCategory.EXPRESSION_VALIDATION, 'Test 1', { expression: 'x > 5', nodeType: 'Conditional' });
            logger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.AST_NODE_BLOCKED, 'Test 2', { expression: 'eval()', nodeType: 'CallExpression' }, true);
        });
    ...
    describe('Console Logging', () => {
        it('should log to console when enabled', () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('[SECURITY WARNING]'), expect.any(String));
        });
        it('should use appropriate console colors and icons', () => {
            ...
            expect(consoleLogSpy).toHaveBeenLastCalledWith(expect.stringContaining('ℹ️'), expect.any(String));
            consoleLogger.logEvent(SecuritySeverity.WARNING, SecurityEventCategory.EXPRESSION_VALIDATION, 'Warning');
            expect(consoleLogSpy).toHaveBeenLastCalledWith(expect.stringContaining('⚠️'), expect.any(String));
            consoleLogger.logEvent(SecuritySeverity.ERROR, SecurityEventCategory.EXPRESSION_VALIDATION, 'Error');
            expect(consoleLogSpy).toHaveBeenLastCalledWith(expect.stringContaining('❌'), expect.any(String));
