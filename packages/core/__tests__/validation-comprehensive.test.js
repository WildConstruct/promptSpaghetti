 ...
 */
import { SecurityValidation, SecureValidation, SecurityTesting, VARIABLE_NAME_PATTERN, VARIABLE_NAME_MAX_LENGTH } from '../validation/security';
...
describe('SecurityTesting Utilities', () => {
    ...
    describe('testInjectionProtection', () => {
        it('should test validator against all injection patterns', () => {
            const mockValidator = jest.fn((input) => !input.includes('eval'));
    ...
    describe('runSecurityTests', () => {
        ...
        it('should run all security tests and return results', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/🔐 Security Tests Complete: \d+ passed, \d+ failed/));
        });
        it('should test all validator functions', () => {
            SecurityTesting.runSecurityTests();
            // Should have tested all major validators
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/Security Test \[Safe String Validation\]: \d+ passed, \d+ failed/));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/Security Test \[Safe Expression Validation\]: \d+ passed, \d+ failed/));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/Security Test \[Safe Property Key Validation\]: \d+ passed, \d+ failed/));
        });
    });
});
