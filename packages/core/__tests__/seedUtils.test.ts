...
describe('createRNG', () => { it('returns deterministic numbers for same seed', () => {
  ...
  it('returns different sequences for different seeds', () => { const rng1 = createRNG('a');
    const rng2 = createRNG('b');

    // it's extremely unlikely first value matches
    expect(rng1()).not.toBeCloseTo(rng2()) });

  it('values are within [0,1)', () => { const rng = createRNG(123);
    const value = rng();
