...
describe('nodeDataUtils', () => { describe('createDefaultNodeData', () => {
    ...
    it('creates default data for WeightedChoice node', () => { const data = createDefaultNodeData('WeightedChoice') as WeightedChoiceNodeData;
      expect(data.type).toBe('WeightedChoice');
      expect(data.weights).toEqual([1]);
      expect(data.options).toEqual(['option']) });
    it('creates default data for Action node', () => { const data = createDefaultNodeData('Action') as ActionNodeData;
      expect(data.type).toBe('Action');
      expect(data.actions).toEqual(['action']);
      expect(data.tense).toBe('present') });
  });
  describe('variation management', () => { const mockNode: SubjectNodeData = {,
  id: 'test',
  label: 'Test',
  ...
  pluralForm: 'tests' }
    ...
    it('adds variation to node', () => { const result = addVariationToNode(mockNode, 'var3');
      expect(result.variations).toEqual(['var1', 'var2', 'var3']) });
    it('removes variation from node', () => { const result = removeVariationFromNode(mockNode, 1);
      expect(result.variations).toEqual(['var1']) });
    it('updates variation in node', () => { const result = updateVariationInNode(mockNode, 0, 'updated');
      expect(result.variations).toEqual(['updated', 'var2']) });
    it('reorders variations in node', () => { const result = reorderVariationsInNode(mockNode, 0, 1);
      expect(result.variations).toEqual(['var2', 'var1']) });
  });
  describe('variation utilities', () => { const nodeWithVariations: SubjectNodeData = {,
  id: 'test',
  label: 'Test',
  type: 'Subject' as const,
  ...
  pluralForm: 'tests' }
    ...
    it('detects if node has variations', () => { expect(hasVariations(nodeWithVariations)).toBe(true);
      expect(hasVariations(nodeWithoutVariations)).toBe(false) });
    it('gets variation count', () => { expect(getVariationCount(nodeWithVariations)).toBe(3);
      expect(getVariationCount(nodeWithoutVariations)).toBe(0) });
    it('gets random variation with seed', () => { // With seed, should be deterministic
    ...
    it('returns label when no variations', () => { const result = getRandomVariation(nodeWithoutVariations);
      expect(result).toBe('Test') });
  });
  describe('validation', () => { it('validates valid node data', () => {
  ...
  pluralForm: 'tests' }
    ...
    it('validates invalid node data', () => { const invalidNode: SubjectNodeData = {,
  id: '',
  label: '',
  type: 'Subject' as const,
  variations: [],
  ...
  pluralForm: 'tests' }
    ...
    it('validates WeightedChoice specific rules', () => { const invalidWeightedChoice = {
  id: 'test',
  label: 'Test',
  ...
  weights: [1] // Mismatch with options length }
    ...
    it('validates variable names', () => { const invalidSetVariable = {
  id: 'test',
  label: 'Test',
