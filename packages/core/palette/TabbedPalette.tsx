...
export const TabbedPalette: React.FC<TabbedPaletteProps> = ({ )
  ...
  useEffect(() => {
    searchEngine.updateNodes(nodes) }, [nodes, searchEngine]);
  // Load favorites
  useEffect(() => { setFavorites(favoritesManager.getFavorites());
  ...
  sortByRelevance: true }
      ...
      if (activeTab === SPECIAL_CATEGORIES.SEARCH_RESULTS) {
        setActiveTab('content') }, [searchQuery, searchEngine, maxSearchResults, activeTab]);
  // Get available categories
  const availableCategories = useMemo(() => { const categories = getAllCategories();
    ...
    return [...specialCats, ...categories] }, [favorites, searchResults, searchQuery, showFavorites]);
  // Get nodes for active category
  const getNodesForCategory = useCallback((categoryId: string): NodeMeta => { if (categoryId === SPECIAL_CATEGORIES.FAVORITES) {
  ...
  const handleFavoriteToggle = useCallback((nodeId: string, event: React.MouseEvent) => { event.stopPropagation();
    event.preventDefault();
    favoritesManager.toggleFavorite(nodeId) }, [favoritesManager]);
  // Handle category collapse toggle
  const handleCategoryToggle = useCallback((categoryId: string) => { const newCollapsed = new Set(collapsedCategories);
    if (newCollapsed.has(categoryId)) {
      newCollapsed.delete(categoryId) } else { newCollapsed.add(categoryId);
    setCollapsedCategories(newCollapsed) }, [collapsedCategories]);
  // Render node item
