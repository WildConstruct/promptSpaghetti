...
import { FiEdit3, FiGitBranch, FiTarget, FiDatabase, FiZap, FiCpu, FiCode, FiStar, FiSearch, FiFolder } from 'react-icons/fi';
/**
...
export function getCategoriesByDifficulty(difficulty) {
    return getAllCategories().filter(cat => cat.metadata?.difficulty === difficulty || cat.metadata?.difficulty === 'beginner');
}
/**
...
export function filterCategories(options) {
    ...
    if (options.difficulty) {
        categories = categories.filter(cat => cat.metadata?.difficulty === options.difficulty ||
            cat.metadata?.difficulty === 'beginner');
    }
    if (options.usage) {
        categories = categories.filter(cat => cat.metadata?.usage === options.usage);
    }
    if (options.expanded !== undefined) {
        categories = categories.filter(cat => cat.defaultExpanded === options.expanded);
    }
    return categories;
}
