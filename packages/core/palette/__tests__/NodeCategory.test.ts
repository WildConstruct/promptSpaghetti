...
import { NODE_CATEGORIES,
  ...
  filterCategories
 from '../NodeCategory';
describe('NodeCategory System', () => { describe('Category Constants', () => {
    ...
    test('should have comprehensive category definitions', () => { const expectedCategories = [
  ...
  expect(NODE_CATEGORIES[categoryId]).toBeDefined();
    ...
    test('should have valid node category mappings', () => { const expectedNodes = [
  ...
  describe('getNodeCategories', () => { test('should return categories for known nodes', () => {
    ...
    test('should return default category for unknown nodes', () => { expect(getNodeCategories('UnknownNode')).toEqual(['content']) });
    test('should handle empty node ID', () => { expect(getNodeCategories('')).toEqual(['content']) });
  });
  describe('getCategoryById', () => { test('should return category for valid IDs', () => {
    ...
    test('should return undefined for invalid IDs', () => { expect(getCategoryById('invalid')).toBeUndefined();
      expect(getCategoryById('')).toBeUndefined() });
  });
  describe('getAllCategories', () => { test('should return all non-special categories', () => {
  ...
  expect(Object.values(SPECIAL_CATEGORIES)).not.toContain(category.id) });
    ...
    test('should return categories sorted by order', () => { const categories = getAllCategories();
      for (let i = 1; i < categories.length; i++) {
        expect(categories[i].order).toBeGreaterThanOrEqual(categories[i - 1].order) });
    test('should include all expected main categories', () => { const categories = getAllCategories();
  ...
  describe('getCategoriesByDifficulty', () => { test('should filter categories by difficulty level', () => {
  ...
  describe('searchCategories', () => { test('should return all categories for empty query', () => {
      const allCategories = getAllCategories();
      expect(searchCategories('')).toEqual(allCategories);
      expect(searchCategories('   ')).toEqual(allCategories) });
    test('should find categories by name', () => { const results = searchCategories('content');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(cat => cat.name.toLowerCase().includes('content'))).toBe(true) });
    test('should find categories by description', () => { const results = searchCategories('building');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(cat => cat.description.toLowerCase().includes('building'))).toBe(true) });
    test('should find categories by keywords', () => { const results = searchCategories('text');
      expect(results.length).toBeGreaterThan(0);
    ...
    test('should be case insensitive', () => { const lowerResults = searchCategories('content');
    ...
    test('should handle partial matches', () => { const results = searchCategories('adv');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(cat => cat.id === 'advanced')).toBe(true) });
  });
  describe('getCategoryColor', () => { test('should return color with default opacity', () => {
      const color = getCategoryColor('content');
      expect(color).toMatch(/^rgba\(\d+, \d+, \d+, 1\)$/) });
    test('should return color with custom opacity', () => { const color = getCategoryColor('content', 0.5);
      expect(color).toMatch(/^rgba\(\d+, \d+, \d+, 0\.5\)$/) });
    test('should return default gray for invalid category', () => { const color = getCategoryColor('invalid');
      expect(color).toBe('rgba(107, 114, 128, 1)') });
    test('should handle edge opacity values', () => { expect(getCategoryColor('content', 0)).toMatch(/^rgba\(\d+, \d+, \d+, 0\)$/);
  ...
  expect(category.defaultExpanded).toBe(false) });
    ...
    test('should combine multiple filters', () => { const filteredCategories = filterCategories({)
  difficulty: 'beginner',
  usage: 'common',
  expanded: true }
});
  ...
  describe('Category Metadata Validation', () => { test('should have valid metadata for all categories', () => {
  ...
  expect(typeof keyword).toBe('string');
          ...
          if (category.metadata.difficulty) { expect(['beginner', 'intermediate', 'advanced']).toContain(category.metadata.difficulty);
          if (category.metadata.usage) {
            expect(['common', 'specialized', 'experimental']).toContain(category.metadata.usage) });
    });
    test('should have unique category IDs', () => { const allCategories = Object.values(NODE_CATEGORIES);
