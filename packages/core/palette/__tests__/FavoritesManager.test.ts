...
describe('FavoritesManager', () => { let favoritesManager: FavoritesManager;
  beforeEach(() => {
  // Clear all mocks
  jest.clearAllMocks();
  ...
  describe('Basic Favorites Management', () => { test('should start with empty favorites', () => {
      expect(favoritesManager.getFavorites()).toEqual([]);
      expect(favoritesManager.getFavoritesCount()).toBe(0) });
    test('should add favorites', () => { const result = favoritesManager.addFavorite('node1');
    ...
    test('should not add duplicate favorites', () => { favoritesManager.addFavorite('node1');
      const result = favoritesManager.addFavorite('node1');
      expect(result).toBe(false);
      expect(favoritesManager.getFavorites()).toEqual(['node1']);
      expect(favoritesManager.getFavoritesCount()).toBe(1) });
    test('should remove favorites', () => { favoritesManager.addFavorite('node1');
    ...
    test('should not remove non-existent favorites', () => { favoritesManager.addFavorite('node1');
      const result = favoritesManager.removeFavorite('node2');
      expect(result).toBe(false);
      expect(favoritesManager.getFavorites()).toEqual(['node1']) });
    test('should toggle favorites', () => { // Toggle on
      ...
      expect(result).toBe(false);
      expect(favoritesManager.isFavorite('node1')).toBe(false) });
    test('should clear all favorites', () => { favoritesManager.addFavorite('node1');
      favoritesManager.addFavorite('node2');
      favoritesManager.addFavorite('node3');
      favoritesManager.clearFavorites();
      expect(favoritesManager.getFavorites()).toEqual([]);
      expect(favoritesManager.getFavoritesCount()).toBe(0) });
    test('should set favorites from array', () => { const nodeIds = ['node1', 'node2', 'node3'];
  ...
  describe('Reordering', () => { beforeEach(() => {
      favoritesManager.setFavorites(['node1', 'node2', 'node3', 'node4']) });
    test('should reorder favorites', () => { const newOrder = ['node3', 'node1', 'node4', 'node2'];
      favoritesManager.reorderFavorites(newOrder);
      expect(favoritesManager.getFavorites()).toEqual(newOrder) });
    test('should handle partial reordering', () => { const partialOrder = ['node2', 'node4'];
  ...
  describe('Persistence', () => { test('should save to localStorage on changes', () => {
    ...
    test('should load from localStorage on initialization', () => { const savedData = {
  nodeIds: ['node1', 'node2']
  version: '1.0.0'
  lastModified: new Date().toISOString() }
    ...
    test('should handle legacy format (simple array)', () => { const legacyData = ['node1', 'node2', 'node3'];
    ...
    test('should handle corrupted localStorage data', () => { localStorageMock.getItem.mockReturnValue('invalid json' as unknown);
      ...
      expect(consoleSpy).toHaveBeenCalled();
      consoleSpy.mockRestore() });
    test('should handle localStorage errors gracefully', () => { localStorageMock.setItem.mockImplementation(() => {
  ...
  describe('Import/Export', () => { test('should export favorites data', () => {
    ...
    test('should import valid favorites data', () => { const importData = {
  ...
  lastModified: new Date().toISOString() }
    ...
    test('should filter invalid node IDs on import', () => { const importData = {
  ...
  lastModified: new Date().toISOString() }
    ...
    test('should handle import errors gracefully', () => { const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
      const result = favoritesManager.importFavorites(null as any);
      expect(result).toBe(false);
      expect(consoleSpy).toHaveBeenCalled();
      consoleSpy.mockRestore() });
  });
  describe('Edge Cases', () => { test('should handle empty string node IDs', () => {
      const result = favoritesManager.addFavorite('');
      expect(result).toBe(true);
      expect(favoritesManager.isFavorite('')).toBe(true) });
    test('should handle whitespace node IDs', () => { const result = favoritesManager.addFavorite('  whitespace  ');
      expect(result).toBe(true);
      expect(favoritesManager.isFavorite('  whitespace  ')).toBe(true) });
    test('should handle special characters in node IDs', () => { const specialId = 'node-with-special_chars.123';
      const result = favoritesManager.addFavorite(specialId);
      expect(result).toBe(true);
      expect(favoritesManager.isFavorite(specialId)).toBe(true) });
    test('should maintain order when adding/removing', () => { favoritesManager.addFavorite('node1');
      favoritesManager.addFavorite('node2');
      favoritesManager.addFavorite('node3');
      expect(favoritesManager.getFavorites()).toEqual(['node1', 'node2', 'node3']);
