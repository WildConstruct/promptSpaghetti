...
const mockNodes: NodeMeta = [
  ...
  describe('PaletteSearch', () => { let searchEngine: PaletteSearch;
  beforeEach(() => {
  searchEngine = new PaletteSearch(mockNodes) });
  describe('Constructor and Initialization', () => { test('should initialize with nodes', () => {
      expect(searchEngine).toBeInstanceOf(PaletteSearch) });
    test('should create search engine with factory function', () => { const engine = createPaletteSearch(mockNodes);
      expect(engine).toBeInstanceOf(PaletteSearch) });
    test('should handle empty nodes array', () => { const emptyEngine = new PaletteSearch([]);
      const results = emptyEngine.search('test');
      expect(results).toEqual([]) });
  });
  describe('Basic Search Functionality', () => { test('should return empty results for empty query', () => {
      expect(searchEngine.search('')).toEqual([]);
      expect(searchEngine.search('   ')).toEqual([]) });
    test('should find nodes by exact label match', () => { const results = searchEngine.search('Character');
      expect(results.length).toBeGreaterThan(0);
      expect(results[0].node.label).toBe('Character');
      expect(results[0].relevance).toBeGreaterThan(0.8) });
    test('should find nodes by partial label match', () => { const results = searchEngine.search('Char');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(r => r.node.label === 'Character')).toBe(true) });
    test('should find nodes by ID', () => { const results = searchEngine.search('Subject');
      expect(results.length).toBeGreaterThan(0);
      expect(results[0].node.id).toBe('Subject') });
    test('should find nodes by tooltip content', () => { const results = searchEngine.search('characters');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(r => r.node.tooltip.toLowerCase().includes('characters'))).toBe(true) });
    test('should be case insensitive', () => { const lowerResults = searchEngine.search('character');
    ...
    test('should handle special characters in query', () => { const results = searchEngine.search('if/then');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(r => r.node.label === 'If/Then')).toBe(true) });
  });
  describe('Search Results and Relevance', () => { test('should return results sorted by relevance', () => {
      const results = searchEngine.search('content');
      if (results.length > 1) {
        for (let i = 1; i < results.length; i++) {
          expect(results[i].relevance).toBeLessThanOrEqual(results[i - 1].relevance) });
    test('should include matched fields information', () => { const results = searchEngine.search('Character');
    ...
    test('should include node categories', () => { const results = searchEngine.search('Character');
  ...
  describe('Fuzzy Matching', () => { test('should find approximate matches', () => {
    ...
    test('should handle typos in common words', () => { const results = searchEngine.search('custm'); // Custom with typo;
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(r => r.node.label.toLowerCase().includes('custom'))).toBe(true) });
  });
  describe('Multi-term Search', () => { test('should find nodes matching multiple terms', () => {
      const results = searchEngine.search('custom script');
      expect(results.length).toBeGreaterThan(0);
      expect(results[0].node.label).toBe('Custom Script') });
    test('should handle partial multi-term matches', () => { const results = searchEngine.search('random choice');
      expect(results.length).toBeGreaterThan(0);
      expect(results.some(r => r.node.label === 'Random Selection')).toBe(true) });
    test('should reduce relevance when terms don\'t match', () => { const fullMatchResults = searchEngine.search('Custom Script');
  ...
  describe('Category-based Search', () => {
    ...
    test('should search within specific categories', () => { const results = searchEngine.searchInCategories('value', ['memory']);
      expect(results.length).toBeGreaterThan(0);
      results.forEach(result => {)
  expect(result.categories).toContain('memory') });
    });
    test('should handle multiple category filter', () => { const results = searchEngine.searchInCategories('content', ['content', 'output']);
      results.forEach(result => {)
  expect();
          result.categories.includes('content') || result.categories.includes('output')
        ).toBe(true) });
    });
    test('should return empty results for non-matching categories', () => { const results = searchEngine.searchInCategories('character', ['advanced']);
      expect(results.length).toBe(0) });
  });
  describe('Suggestions and Auto-complete', () => { test('should provide search suggestions', () => {
      const suggestions = searchEngine.getSuggestions('cha');
      expect(Array.isArray(suggestions)).toBe(true);
      expect(suggestions.length).toBeGreaterThan(0);
      expect(suggestions.some(s => s.startsWith('cha'))).toBe(true) });
    test('should limit suggestions count', () => { const suggestions = searchEngine.getSuggestions('c', 3);
      expect(suggestions.length).toBeLessThanOrEqual(3) });
    test('should return empty suggestions for empty query', () => { const suggestions = searchEngine.getSuggestions('');
      expect(suggestions).toEqual([]) });
    test('should provide fuzzy suggestions for short queries', () => { const suggestions = searchEngine.getSuggestions('cu');
      expect(suggestions.length).toBeGreaterThan(0) });
    test('should not suggest the exact query term', () => { const suggestions = searchEngine.getSuggestions('custom');
      expect(suggestions).not.toContain('custom') });
  });
  describe('Popular Terms', () => { test('should return popular search terms', () => {
    ...
    test('should limit popular terms count', () => { const popularTerms = searchEngine.getPopularTerms(3);
      expect(popularTerms.length).toBeLessThanOrEqual(3) });
    test('should return terms sorted by frequency', () => { const popularTerms = searchEngine.getPopularTerms();
  ...
  describe('Node Updates', () => { test('should update search index when nodes change', () => {
  ...
  expect(results[0].node.label).toBe('New Test Node');
    ...
    test('should clear old results after update', () => { const newNodes: NodeMeta = [
  {
  id: 'OnlyNode',
  label: 'Only Node',
  icon: '📱',
  ...
  describe('Performance and Edge Cases', () => {
    ...
    test('should handle nodes with empty fields', () => { const nodesWithEmpty: NodeMeta = [
  {
  id: '',
  label: '',
  icon: '',
  ...
  expect(Array.isArray(results)).toBe(true);
    ...
    test('should handle special Unicode characters', () => { const unicodeNodes: NodeMeta = [
  {
  id: 'unicode',
  label: 'Café ñoño 中文',
  icon: '🌍',
...
describe('highlightSearchTerms', () => { test('should highlight single term', () => {
    const result = highlightSearchTerms('Hello world', ['world']);
    expect(result).toBe('Hello <mark>world</mark>') });
  test('should highlight multiple terms', () => { const result = highlightSearchTerms('Hello beautiful world', ['Hello', 'world']);
    expect(result).toBe('<mark>Hello</mark> beautiful <mark>world</mark>') });
  test('should be case insensitive', () => { const result = highlightSearchTerms('Hello World', ['hello', 'WORLD']);
    expect(result).toBe('<mark>Hello</mark> <mark>World</mark>') });
  test('should handle overlapping matches', () => { const result = highlightSearchTerms('Hello Hello', ['Hello']);
    expect(result).toBe('<mark>Hello</mark> <mark>Hello</mark>') });
  test('should handle empty terms array', () => { const result = highlightSearchTerms('Hello world', []);
    expect(result).toBe('Hello world') });
  test('should handle non-matching terms', () => { const result = highlightSearchTerms('Hello world', ['xyz']);
    expect(result).toBe('Hello world') });
  test('should escape HTML in input text', () => { const result = highlightSearchTerms('Hello <script>alert("xss")</script>', ['Hello']);
