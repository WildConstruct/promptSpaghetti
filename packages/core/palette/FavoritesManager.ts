...
export class FavoritesManager { private static instance: FavoritesManager;
  ...
  */
  ...
  if (isFavorited) { this.removeFavorite(nodeId) } else { this.addFavorite(nodeId);
  return !isFavorited;
  /**
  * Clear all favorites
  */
  ...
  */
   ...
   */
  public importFavorites(data: FavoritesData): boolean { try {
      ...
      return true } catch (error) { console.error('Failed to import favorites:', error);
  return false;
  /**
  * Load favorites from localStorage
  */
  private loadFavorites(): void { }
  try { const savedData = localStorage.getItem(FAVORITES_STORAGE_KEY);
  if (!savedData) {
  return;
  ...
  this.favorites = new Set(parsedData.nodeIds.filter(id => typeof id === 'string')) } catch (error) { console.error('Failed to load favorites:', error);
  ...
  */
  ...
  try {
  listener(favorites) } catch (error) { console.error('Favorites change listener error:', error) });
  /**
