...
class TestFrameworkNode {};
  ...
  protected async onDestroy(): Promise<void> { // Test cleanup
  ...
  afterEach(async () => { await framework.shutdown() });
  describe('Node Creation and Management', () => { it('should create a node successfully', async () => {
  ...
  stateful: false }
    ...
    it('should retrieve created nodes', async () => { await framework.createNode('Test', 'test-node-1', {)
  deterministic: true
  cacheable: false
  stateful: false }
}, {});
    ...
    it('should destroy nodes properly', async () => { await framework.createNode('Test', 'test-node-1', {)
  deterministic: true
  cacheable: false
  stateful: false }
}, {});
  ...
  describe('Node Execution', () => { it('should execute nodes and return results', async () => {
  ...
  prng: () => 0.5, // Mock PRNG
    ...
    it('should execute nodes in batch', async () => { const context: AdvancedExecutionContext = {,
  variables: new Map(),
  nodeStates: new Map(),
  evaluationDepth: 0,
  cache: new Map(),
  ...
  prng: () => 0.5,
    ...
    it('should emit execution events', async () => { const events: any = [];
  ...
  describe('Lifecycle Hooks', () => { it('should execute global lifecycle hooks', async () => {
  ...
  prng: () => 0.5,
    ...
    it('should execute type-specific lifecycle hooks', async () => { const hookCalls: string = [];
  ...
  describe('Metrics and Monitoring', () => { it('should track framework metrics', async () => {
  // Create some nodes
  ...
  stateful: false }
    ...
    it('should track node metrics', async () => { const node = await framework.createNode('Test', 'node-metrics', {)
  deterministic: true,
  cacheable: false,
  stateful: false }
}, {});
  ...
  describe('Error Handling', () => { it('should handle node creation errors', async () => {
  ...
  stateful: false }
    ...
    it('should handle node destruction errors', async () => { await expect(framework.destroyNode('non-existent-node')).rejects.toThrow('not found') });
  });
  describe('Framework Shutdown', () => { it('should shutdown gracefully', async () => {
...
describe('NodeRegistry', () => { let registry: NodeRegistry;
  beforeEach(() => {
  registry = new NodeRegistry() });
  describe('Node Type Registration', () => { it('should register node types successfully', () => {
  ...
  experimental: false }
    ...
    it('should prevent duplicate registration', () => { const definition: NodeDefinition = {,
  type: 'DuplicateTest',
  ...
  experimental: false }
      ...
      registry.registerNode(definition);
      expect(() => { registry.registerNode(definition) }).toThrow('Node type \'DuplicateTest\' is already registered');
    });
    it('should unregister node types', () => { const definition: NodeDefinition = {,
  type: 'UnregisterTest',
  ...
  describe('Node Creation', () => { beforeEach(() => {
  ...
  experimental: false }
    ...
    it('should create node instances', () => { const node = registry.createNode('CreationTest', 'test-instance', {)
  deterministic: true,
  cacheable: false,
  stateful: false }
}, { testData: 'value' });
    ...
    it('should merge default and provided config', () => { const definition = registry.getDefinition('CreationTest')!;
  ...
  describe('Type Aliases', () => { beforeEach(() => {
  registry.registerNode({)
  type: 'AliasTest',
  ...
  experimental: false }
    ...
    it('should register and resolve aliases', () => { registry.registerAlias('AT', 'AliasTest');
  ...
  stateful: false }
    ...
    it('should prevent aliases for non-existent types', () => { expect(() => {
        registry.registerAlias('BadAlias', 'NonExistentType') }).toThrow('Cannot create alias for unregistered type: NonExistentType');
    });
  });
  describe('Search and Filtering', () => { beforeEach(() => {
      registry.registerNode({)
  type: 'BasicNode',
  ...
  experimental: true }
    ...
    it('should get nodes by category', () => { const basicNodes = registry.getNodesByCategory('basic');
    ...
    it('should return all registered types', () => { const types = registry.getRegisteredTypes();
