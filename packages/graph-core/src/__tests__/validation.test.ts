...
describe('GraphValidator', () => {
  ...
  describe('Node Validation', () => {
    ...
    it('should validate WeightedChoice node data', () => {
      ...
      expect(result.valid).toBe(false);
      expect(
        result.errors.some(e => e.type === 'INVALID_DATA' && e.message.includes('non-negative number weight'))
      ).toBe(true);
    });
    ...
    it('should validate node inputs array', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.type === 'INVALID_DATA' && e.message.includes('inputs must be an array'))).toBe(
        true
      );
    });
    ...
    it('should detect references to non-existent input nodes', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.type === 'MISSING_NODE' && e.message.includes('non-existent input node'))).toBe(
        true
      );
    });
  });
  ...
  describe('Edge Validation', () => {
    ...
    it('should detect edges with non-existent source nodes', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.type === 'MISSING_NODE' && e.message.includes('non-existent source node'))).toBe(
        true
      );
    });
    ...
    it('should detect edges with non-existent target nodes', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.type === 'MISSING_NODE' && e.message.includes('non-existent target node'))).toBe(
        true
      );
    });
  ...
  describe('Performance Analysis', () => {
    it('should warn about large graphs', () => {
      ...
      expect(result.warnings.some(w => w.type === 'PERFORMANCE_CONCERN' && w.message.includes('Large graph'))).toBe(
        true
      );
    });
    ...
    it('should warn about high edge-to-node ratios', () => {
      ...
      expect(
        result.warnings.some(w => w.type === 'PERFORMANCE_CONCERN' && w.message.includes('High edge-to-node ratio'))
      ).toBe(true);
    });
  });
  ...
  describe('Connectivity Analysis', () => {
    it('should warn about disconnected nodes', () => {
      const graph: GraphDocument = {
        ...
        edges: new Map([['edge1', { id: 'edge1', source: 'connected1', target: 'connected2' }]]),
        metadata: {
          version: '1.0.0',
          created: new Date(),
          modified: new Date(),
      ...
      expect(
        result.warnings.some(w => w.type === 'DISCONNECTED_NODE' && w.message.includes('completely disconnected'))
      ).toBe(true);
    });
    ...
    it('should warn when graph has no Output nodes', () => {
      const graph: GraphDocument = {
        ...
        edges: new Map([['edge1', { id: 'edge1', source: 'node1', target: 'node2' }]]),
        metadata: {
          version: '1.0.0',
          created: new Date(),
          modified: new Date(),
      ...
      expect(result.warnings.some(w => w.type === 'DISCONNECTED_NODE' && w.message.includes('no Output nodes'))).toBe(
        true
      );
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle validation exceptions gracefully', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.type === 'MALFORMED_GRAPH' && e.message.includes('Validation failed'))).toBe(
        true
      );
    });
