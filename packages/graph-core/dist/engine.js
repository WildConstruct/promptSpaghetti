...
export class GraphEngine {
  ...
  async executeNodeWithDeps(node, nodeMap, dependencies, context, visited, executing) {
    // Detect cycles
  ...
  async executeNode(node, nodeMap, context, visited, executing) {
  ...
  createRuntimeNode(node, inputs, context) {
    ...
    switch (node.type) {
      ...
      case 'Include':
        return new IncludeRuntimeNode(node.id, String(data.name || ''), data.lookup || {});
      case 'SetVariable':
...
class WeightedChoiceRuntimeNode extends RuntimeNode {
  choices;
...
class ConcatRuntimeNode extends RuntimeNode {
  template;
  inputs;
  context;
  constructor(id, template, inputs, context) {
    super(id);
...
class OutputRuntimeNode extends RuntimeNode {
  text;
  inputs;
  context;
  constructor(id, text, inputs, context) {
    super(id);
    this.text = text;
    this.inputs = inputs;
    this.context = context;
...
class IncludeRuntimeNode extends RuntimeNode {
  name;
  lookup;
...
class SetVariableRuntimeNode extends RuntimeNode {
  key;
  value;
...
class GetVariableRuntimeNode extends RuntimeNode {
  key;
  constructor(id, key) {
    super(id);
    this.key = key;
