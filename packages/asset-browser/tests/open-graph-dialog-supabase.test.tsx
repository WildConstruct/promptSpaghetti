...
describe('OpenGraphDialog - Supabase tab', () => {
  ...
  async function renderDlg(props: Partial<React.ComponentProps<typeof OpenGraphDialog>> = {}) {
  ...
  it('does not show Supabase tab when gating conditions are not met', async () => {
    ...
    await renderDlg({ userId: 'u1', supabaseList: async () => ({ ok: true, data: [] }), supabaseGet: async () => ({ ok: true, data: '{}' }) });
    expect(screen.queryByRole('button', { name: 'Supabase' })).toBeNull();
  });

  it('uses userId from UserProvider context when prop is not provided', async () => {
    const list = jest.fn().mockResolvedValue({ ok: true, data: [{ name: 'c.psg' }] });
    const get = jest.fn().mockResolvedValue({ ok: true, data: JSON.stringify({ ok: true }) });

    const supabase = {
      auth: {
        getSession: jest.fn().mockResolvedValue({ data: { session: { user: { id: 'ctx-2' } } } }),
    ...
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'Supabase' })); });

    await waitFor(() => {
      expect(screen.getByRole('listbox', { name: 'Supabase Graphs' })).toBeInTheDocument();
  ...
  it('shows empty state when list returns no items', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/No Supabase graphs available/i)).toBeInTheDocument();
    });
  });

  it('lists items and opens one successfully', async () => {
    const list = jest.fn().mockResolvedValue({ ok: true, data: [{ name: 'a.psg' }, { name: 'b.psg' }] });
    const get = jest.fn().mockResolvedValue({ ok: true, data: JSON.stringify({ openedFrom: 'supabase', id: 1 }) });
  ...
  it('shows error when list fails', async () => {
    await renderDlg({
      userId: 'user-123',
      enableSupabase: true,
      supabaseList: async () => ({ ok: false as const, error: { message: 'boom' } }),
  ...
  it('shows error when open fails', async () => {
    const list = jest.fn().mockResolvedValue({ ok: true, data: [{ name: 'x.psg' }] });
    const get = jest.fn().mockResolvedValue({ ok: false, error: { message: 'nope' } });
    await renderDlg({ userId: 'u1', enableSupabase: true, supabaseList: list, supabaseGet: get });
  ...
  it('shows error when JSON is invalid', async () => {
    const list = jest.fn().mockResolvedValue({ ok: true, data: [{ name: 'bad.psg' }] });
