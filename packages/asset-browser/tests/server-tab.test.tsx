...
describe('ServerTab', () => {
  ...
  it('renders empty state when manifest is empty', async () => {
    ...
    await waitFor(() => {
      ...
      expect(statuses.some(s => /No server graphs available/i.test(s.textContent || ''))).toBe(true);
    });
  });
  ...
  it('shows error and allows retry', async () => {
    const success = [{ filename: 'a.psg', title: 'Alpha', updatedAt: new Date().toISOString() }];
  ...
  it('treats 404 manifest as empty state with guidance', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (global as any).fetch = jest.fn(async () => ({ ok: false, status: 404 })) as any;
    ...
    await waitFor(() => {
      ...
      expect(alerts.some(a => /Failed to load/i.test(a.textContent || ''))).toBe(false);
