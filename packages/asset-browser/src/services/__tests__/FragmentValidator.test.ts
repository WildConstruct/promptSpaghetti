  ...
  describe('validate', () => {
    ...
    it('should warn about Output nodes in fragments', async () => {
      const withOutput = JSON.stringify({
        version: '1.0.0',
        nodes: [{ id: 'node1', type: 'Output', x: 100, y: 100 }],
        edges: []
      });
      ...
      expect(result.warnings).toContainEqual(
        'Fragment contains Output nodes (will be filtered out)'
      );
    ...
    it('should detect edges with invalid references', async () => {
      const invalidEdges = JSON.stringify({
        version: '1.0.0',
        nodes: [{ id: 'node1', type: 'WeightedChoice', x: 100, y: 100 }],
      ...
      expect(result.errors).toContainEqual(
        'Edge references non-existent target: nonexistent'
      );
    });
    ...
    it('should reject unknown format', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContainEqual(
        'Unknown fragment format - not PSG or PSGLib'
      );
    });
  });
