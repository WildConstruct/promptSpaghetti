...
import { loadAssetFragments, type AssetFragmentManifest, type FragmentEntry } from '../services/AssetFragmentLoader';
...
export function AssetFragmentsTab(): JSX.Element {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to load asset fragments';
      setError(msg);
      setStatus('error');
    }
  }, []);
  ...
  const allFragments = useMemo(() => {
    ...
    const fragments: Array<FragmentEntry & { category: string; categoryName: string }> = [];
  ...
  const handleDragStart = (e: React.DragEvent, fragment: FragmentEntry, categoryKey: string) => {
    ...
    <section aria-label="Asset Fragments" style={{ padding: 12 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <span style={{
                  fontSize: 10,
                  padding: '2px 6px',
                  backgroundColor: fragment.type === 'SIMPLE' ? '#e8f5e9' :
                                    fragment.type === 'CONTEXTUAL' ? '#fff3e0' : '#f3e5f5',
                  borderRadius: 3,
                  color: '#333'
                }}>
                  {fragment.type}
      ...
      {manifest.statistics && (
        <footer style={{
          ...
          <strong>Library Stats:</strong> {manifest.statistics.total_fragments} fragments • {manifest.statistics.total_nodes} nodes • {manifest.statistics.total_options} options
        </footer>
      )}
    </section>
  );
