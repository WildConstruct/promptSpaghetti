 ...
 */
export const TouchableNode = ({ node, isSelected, canConnect, handlers, children }) => {
    ...
    return (_jsxs("div", { ref: nodeRef, className: `touchable-node ${isSelected ? 'selected' : ''} ${isDragging ? 'dragging' : ''}`, style: {
            position: 'absolute',
        ...
        }, children: [children, canConnect && (_jsxs(_Fragment, { children: [_jsx(ConnectionHandle, { type: "input", nodeId: node.id, onConnect: handlers.onNodeConnect }), _jsx(ConnectionHandle, { type: "output", nodeId: node.id, onConnect: handlers.onNodeConnect })] }))] }));
};
const ConnectionHandle = ({ type, nodeId, onConnect }) => {
    const handleRef = useRef(null);
    const [isConnecting, setIsConnecting] = useState(false);
    const connectionLine = useRef(null);
    ...
    const updateConnectionLine = (endPos) => {
        if (!connectionLine.current || !startPos.current)
            return;
    ...
    return (_jsx("div", { ref: handleRef, className: `connection-handle ${type}`, "data-node-id": nodeId, "data-handle-type": type, style: {
            position: 'absolute',
...
export const SelectionBox = ({ onSelect, nodes, children }) => {
    ...
    return (_jsxs("div", { ref: containerRef, style: { position: 'relative', width: '100%', height: '100%' }, children: [children, selectionBounds && (_jsx("div", { className: "selection-box", style: {
                    position: 'absolute',
...
//# sourceMappingURL=NodeGestures.js.map