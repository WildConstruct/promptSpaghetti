 ...
 */
export const TouchContextMenu = ({ items, onSelect, onDismiss, position, anchor = 'auto', showShortcuts = false, enableHaptics = true }) => {
    ...
    const renderMenuItem = (item, index) => {
        if (item.type === 'separator') {
            return _jsx("div", { className: "context-menu-separator" }, item.id);
        }
        const isSelected = selectedIndex === index;
        const hasSubmenu = item.type === 'submenu' && item.submenu;
        const isSubmenuOpen = submenuOpen === item.id;
        return (_jsxs("div", { children: [_jsxs("button", { className: `
            context-menu-item
            ${isSelected ? 'selected' : ''}
            ${item.disabled ? 'disabled' : ''}
            ${item.danger ? 'danger' : ''}
            ${item.checked ? 'checked' : ''}
          `, onClick: () => handleItemSelect(item, index), disabled: item.disabled, style: {
                    ...
                    }, children: [_jsxs("div", { className: "menu-item-content", children: [item.icon && _jsx("span", { className: "menu-item-icon", children: item.icon }), _jsx("span", { className: "menu-item-label", children: item.label })] }), _jsxs("div", { className: "menu-item-extras", children: [showShortcuts && item.shortcut && (_jsx("span", { className: "menu-item-shortcut", children: item.shortcut })), hasSubmenu && (_jsx("span", { className: "menu-item-arrow", children: "\u25B6" })), item.type === 'checkbox' && item.checked && (_jsx("span", { className: "menu-item-check", children: "\u2713" }))] })] }), hasSubmenu && isSubmenuOpen && (_jsx("div", { className: "context-submenu", children: item.submenu.map((subItem, subIndex) => renderMenuItem(subItem, items.length + subIndex)) }))] }, item.id));
    };
    return (_jsx("div", { ref: menuRef, className: "touch-context-menu", style: {
            position: 'fixed',
            left: menuPosition.x,
            top: menuPosition.y,
            zIndex: 10000,
            ...
            touchAction: 'none',
            userSelect: 'none'
        }, onKeyDown: handleKeyDown, tabIndex: 0, children: items.map((item, index) => renderMenuItem(item, index)) }));
};
export const ContextMenuProvider = ({ items, longPressDelay = 500, enableHaptics = true, children }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { ref: containerRef, style: { width: '100%', height: '100%' }, children: children }), menuState.visible && (_jsx(TouchContextMenu, { items: menuState.items, position: menuState.position, onDismiss: handleDismiss, enableHaptics: enableHaptics }))] }));
};
/**
...
//# sourceMappingURL=TouchContextMenu.js.map