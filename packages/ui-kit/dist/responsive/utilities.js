...
import { getCurrentBreakpoint, defaultBreakpoints } from './breakpoints';
 ...
 */
export function useBreakpointMatch(query) {
    const { breakpoint } = useEnhancedResponsive();
    return useMemo(() => {
        const sortedKeys = Object.keys(defaultBreakpoints);
        const currentIndex = sortedKeys.indexOf(breakpoint);
 ...
 */
export function responsiveClasses(baseClass, modifiers) {
 ...
 */
export function responsiveSpacing(values) {
    const { breakpoint } = useEnhancedResponsive();
 ...
 */
export function useContainerQuery(ref, queries) {
