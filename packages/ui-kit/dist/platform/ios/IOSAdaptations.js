...
export const IOSNavigationBar = ({ title, leftItems, rightItems, transparent = false, large = false, onBack }) => {
    ...
    return (_jsx("nav", { className: `ios-navigation-bar ${large ? 'large' : ''} ${scrolled ? 'scrolled' : ''}`, style: {
            position: 'fixed',
            top: 0,
            left: 0,
            right: 0,
            ...
            zIndex: 1000
        }, children: _jsxs("div", { className: "nav-content", style: {
                display: 'flex',
                alignItems: 'center',
                ...
                transition: 'height 0.3s ease'
            }, children: [_jsxs("div", { className: "nav-left", style: { flex: 1, display: 'flex', alignItems: 'center' }, children: [onBack && (_jsxs("button", { onClick: onBack, className: "ios-back-button", style: {
                                display: 'flex',
                                alignItems: 'center',
                                gap: '4px',
                                padding: '8px',
                                ...
                                fontSize: '17px'
                            }, children: [_jsx("svg", { width: "12", height: "20", viewBox: "0 0 12 20", fill: "currentColor", children: _jsx("path", { d: "M10 0L0 10L10 20L11.5 18.5L3 10L11.5 1.5L10 0Z" }) }), "Back"] })), leftItems] }), _jsx("div", { className: "nav-center", style: {
                        position: 'absolute',
                        left: '50%',
                    ...
                    }, children: title }), _jsx("div", { className: "nav-right", style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }, children: rightItems })] }) }));
};
export const IOSTabBar = ({ items, activeItem, onItemSelect }) => {
    const { bottom } = React.useContext(IOSSafeAreaContext);
    return (_jsx("div", { className: "ios-tab-bar", style: {
            position: 'fixed',
            bottom: 0,
            left: 0,
            right: 0,
            ...
            WebkitBackdropFilter: 'blur(20px)'
        }, children: items.map(item => (_jsxs("button", { onClick: () => onItemSelect(item.id), className: `tab-item ${activeItem === item.id ? 'active' : ''}`, style: {
                flex: 1,
                display: 'flex',
                ...
                fontSize: '10px',
                position: 'relative'
            }, children: [_jsx("div", { style: { fontSize: '24px' }, children: item.icon }), _jsx("span", { children: item.label }), item.badge && item.badge > 0 && (_jsx("div", { className: "badge", style: {
                        position: 'absolute',
                        top: '4px',
                        right: 'calc(50% - 16px)',
...
export const IOSSwitch = ({ checked, onChange, disabled = false }) => {
    return (_jsx("button", { className: `ios-switch ${checked ? 'checked' : ''} ${disabled ? 'disabled' : ''}`, onClick: () => !disabled && onChange(!checked), role: "switch", "aria-checked": checked, disabled: disabled, style: {
            width: '51px',
            height: '31px',
            borderRadius: '31px',
...
export const IOSActionSheet = ({ visible, title, message, actions, onDismiss }) => {
    ...
    if (!visible)
        return null;
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "ios-action-sheet-backdrop", onClick: onDismiss, style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
                ...
                } }), _jsxs("div", { className: "ios-action-sheet", style: {
                    position: 'fixed',
                    left: '8px',
                    right: '8px',
                    bottom: bottom + 8,
                    ...
                    animation: 'slideUp 0.3s ease'
                }, children: [(title || message) && (_jsxs("div", { className: "action-sheet-header", style: {
                            backgroundColor: 'var(--color-background)',
                            borderRadius: '13px',
                            marginBottom: '8px',
                            padding: '16px',
                            textAlign: 'center'
                        }, children: [title && (_jsx("div", { style: { fontSize: '13px', fontWeight: 600, marginBottom: '4px' }, children: title })), message && (_jsx("div", { style: { fontSize: '13px', color: 'var(--color-text-secondary)' }, children: message }))] })), _jsx("div", { className: "action-sheet-actions", style: {
                            backgroundColor: 'var(--color-background)',
                            borderRadius: '13px',
                            overflow: 'hidden'
                        }, children: actions
                            .filter(action => action.style !== 'cancel')
                            .map((action, index) => (_jsx("button", { onClick: () => {
                                action.onPress();
                                onDismiss();
                            }, style: {
                                width: '100%',
                            ...
                            }, children: action.text }, index))) }), actions.find(action => action.style === 'cancel') && (_jsx("button", { onClick: () => {
