...
export const MobileGraphCanvas = ({
  graph,
  selectedNodeId,
  onNodeSelect,
  onNodeEdit,
  ...
  return _jsxs('div', {
    ...
    children: [
      _jsxs('div', {
        ...
        children: [
          ...
          graph.nodes.map(node =>
            _jsx(
              MobileNode,
              { node: node, isSelected: node.id === selectedNodeId, onClick: () => handleNodeClick(node.id) },
              node.id
            )
          ),
        ],
      ...
      !readOnly &&
        _jsx(MobileFAB, { position: 'bottom-right', onClick: onAddNode, hapticFeedback: true, children: '+' }),
    ],
  });
};
const MobileNode = ({ node, isSelected, onClick }) => {
  ...
  return _jsxs('div', {
    ...
    onClick: onClick,
    children: [
      _jsx('div', { style: { fontSize: 24, marginBottom: 4 }, children: nodeIcons[node.type] || '📦' }),
      _jsx('div', {
        style: {
          fontSize: 12,
          fontWeight: 500,
