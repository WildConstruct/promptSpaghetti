...
export function sizeToPixels(size: ComponentSize, theme: Theme): number {
  const sizeMap = {
    xs: theme.spacing.xs,
    sm: theme.spacing.sm,
    md: theme.spacing.md,
...
export function createFocusStyles(theme: Theme): Record<string, any> {
  return {
    '&:focus': {
      outline: 'none',
...
export function createButtonVariantStyles(
