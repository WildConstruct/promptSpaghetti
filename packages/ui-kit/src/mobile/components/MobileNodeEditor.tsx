...
export const MobileNodeEditor: React.FC<MobileNodeEditorProps> = ({
  ...
  return (
    ...
    >
          ...
          <div className="connections-tab">
            <CollapsiblePanel title="Incoming Connections" defaultOpen>
              {node.data.inputs?.length ? (
                ...
                <p style={{ color: 'var(--color-text-secondary)', margin: 0 }}>No incoming connections</p>
              )}
            </CollapsiblePanel>
            ...
            <CollapsiblePanel title="Outgoing Connections" defaultOpen>
              {node.data.outputs?.length ? (
                ...
                <p style={{ color: 'var(--color-text-secondary)', margin: 0 }}>No outgoing connections</p>
              )}
            </CollapsiblePanel>
          </div>
        )}
      ...
      >
        <MobileButton variant="secondary" onClick={onClose} style={{ flex: 1 }}>
          Cancel
        </MobileButton>
        <MobileButton variant="primary" onClick={onClose} style={{ flex: 1 }}>
          Done
        </MobileButton>
      </div>
    </div>
...
function renderNodeProperties(node: GraphNode, onChange: (property: string, value: any) => void) {
  ...
  switch (node.type) {
    ...
    default:
      return <div style={{ color: 'var(--color-text-secondary)' }}>No editable properties for this node type</div>;
  }
}
