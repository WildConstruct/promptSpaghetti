...
export const MultiTouchController: React.FC<MultiTouchControllerProps> = ({
  ...
  const handleMultiTouchTap = useCallback((event: GestureEvent) => {
  ...
  const handleMultiTouchDoubleTap = useCallback((event: GestureEvent) => {
    const touchCount = event.touches.length;
  ...
  const handleMultiTouchLongPress = useCallback((event: GestureEvent) => {
    const touchCount = event.touches.length;
  ...
  const handleMultiTouchSwipe = useCallback((event: GestureEvent) => {
    const touchCount = event.touches.length;
  ...
  const handleMultiTouchPinch = useCallback((event: GestureEvent) => {
    if (event.touches.length === 3 && event.scale) {
      handlers.onThreeFingerPinch?.(event.scale);
    }
  }, [handlers]);
  
  const handleMultiTouchRotate = useCallback((event: GestureEvent) => {
...
export class GestureCombination {
  ...
  addGesture(gesture: GestureEvent): void {
    ...
    this.gestures = this.gestures.filter(
      g => (now - g.touches[0].timestamp) < this.timeWindow
    );
