 ...
 */
export function getSwipeDirection(deltaX: number, deltaY: number, threshold: number = 30): SwipeDirection | null {
 ...
 */
export function getPinchScale(touches1: TouchPoint[], touches2: TouchPoint[]): number {
 ...
 */
export function getRotation(touches1: TouchPoint[], touches2: TouchPoint[]): number {
...
export const gestureHelpers = {
  ...
  isLongPress: (gesture: GestureState, config = defaultGestureConfig): boolean => {
    ...
    return (
      gesture.touches.length === 1 && duration >= config.longPressDelay && movement <= config.longPressMoveThreshold
    );
  },
