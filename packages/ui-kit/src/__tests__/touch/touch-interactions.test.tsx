...
import { createTouchEvent, mockHapticFeedback, cleanup, mockPlatform, platformPresets } from '../setup/test-framework';
import { TouchManager } from '../../touch/TouchManager';
import { TouchableNode, SelectionBox, MultiTouchController, TouchContextMenu, ContextMenuProvider } from '../../touch';
...
describe('Touch Gestures', () => {
  ...
  describe('TouchableNode', () => {
    ...
    it('should handle node selection on tap', async () => {
      ...
      render(
        <TouchableNode node={mockNode} isSelected={false} canConnect={true} handlers={{ onNodeSelect }}>
          <div>Test Node</div>
        </TouchableNode>
      );
    ...
    it('should handle node drag', async () => {
      ...
      render(
        <TouchableNode node={mockNode} isSelected={false} canConnect={true} handlers={{ onNodeMove }}>
          <div>Test Node</div>
        </TouchableNode>
      );
    ...
    it('should handle double tap for edit', async () => {
      ...
      render(
        <TouchableNode node={mockNode} isSelected={false} canConnect={true} handlers={{ onNodeEdit }}>
          <div>Test Node</div>
        </TouchableNode>
      );
  ...
  describe('Touch Context Menu', () => {
    ...
    it('should dismiss context menu on outside tap', async () => {
      const items = [{ id: 'edit', label: 'Edit', onSelect: jest.fn() }];

      render(
        <>
          <TouchContextMenu items={items} position={{ x: 100, y: 100 }} onDismiss={jest.fn()} />
          <div>Outside</div>
        </>
      );
