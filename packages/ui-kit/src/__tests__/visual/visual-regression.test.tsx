...
describe('Visual Regression Tests', () => {
  ...
  describe('Responsive Components', () => {
    ...
    viewports.forEach(viewport => {
          ...
          <Grid gutter={16}>
            <Row>
              <Col xs={12} sm={6} md={4} lg={3}>
                <div style={{ background: '#f0f0f0', padding: '20px' }}>Column 1</div>
              </Col>
              <Col xs={12} sm={6} md={4} lg={3}>
                <div style={{ background: '#e0e0e0', padding: '20px' }}>Column 2</div>
              </Col>
              <Col xs={12} sm={6} md={4} lg={3}>
                <div style={{ background: '#d0d0d0', padding: '20px' }}>Column 3</div>
              </Col>
              <Col xs={12} sm={6} md={4} lg={3}>
                <div style={{ background: '#c0c0c0', padding: '20px' }}>Column 4</div>
              </Col>
            </Row>
          </Grid>
        );

        await visualRegression.capture(`grid-layout-${viewport.name}`, container as HTMLElement);
      });
      ...
      it(`should match ResponsiveImage on ${viewport.name}`, async () => {
        ...
        await visualRegression.capture(`responsive-image-${viewport.name}`, container as HTMLElement);
      });
    });
  });
    ...
    it('should match Material app bar', async () => {
      const { container } = render(
        <MaterialAppBar
          ...
          actions={[<button key="search">🔍</button>, <button key="more">⋮</button>]}
        />
      );
      ...
      it(`should match ${platform.name} button styles`, async () => {
        ...
        const { container } = render(
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '20px' }}>
            ...
            <PlatformButton variant="primary" size="large">
              Large Button
            </PlatformButton>
          </div>
        );

        await visualRegression.capture(`platform-buttons-${platform.name}`, container as HTMLElement);
      });
    });
  });
  ...
  describe('Adaptive Navigation', () => {
    ...
    it('should match mobile navigation', async () => {
      ...
      const { container } = render(
        <AdaptiveNavigation title="Mobile App" items={navigationItems} activeItem="home" onNavigate={() => {}} />
      );
    ...
    it('should match tablet navigation rail', async () => {
      ...
      const { container } = render(
        <AdaptiveNavigation title="Tablet App" items={navigationItems} activeItem="search" onNavigate={() => {}} />
      );
    ...
    it('should match desktop side navigation', async () => {
      ...
      const { container } = render(
        <AdaptiveNavigation title="Desktop App" items={navigationItems} activeItem="settings" onNavigate={() => {}} />
      );
          ...
          <MobileCard
            ...
            footer={
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <MobileButton variant="secondary" size="small">
                  Cancel
                </MobileButton>
              </div>
            }
          >
            <div style={{ padding: '20px', background: 'var(--color-surface)' }}>Card content in light theme</div>
          </MobileCard>
        </div>
      );
    ...
    it('should match dark theme components', async () => {
      ...
      const { container } = render(
        <div style={{ padding: '20px', background: 'var(--color-background)' }}>
          ...
          >
            <div style={{ padding: '20px', background: 'var(--color-surface)' }}>Card content in dark theme</div>
          </MobileCard>
        </div>
      );
    ...
    it('should match hover states', async () => {
      ...
      const { container } = render(
        <div style={{ display: 'flex', gap: '20px', padding: '20px' }}>
          ...
          <button className="desktop-button" disabled>
            Disabled
          </button>
        </div>
      );
