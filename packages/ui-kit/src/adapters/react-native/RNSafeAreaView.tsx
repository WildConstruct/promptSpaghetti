...
export const RNSafeAreaView = forwardRef<any, RNSafeAreaViewProps>(
  ({ children, edges = ['top', 'bottom', 'left', 'right'], mode = 'padding', style, testID, ...props }, ref) => {
    ...
    if (RNSafeAreaViewComponent) {
      return <RNSafeAreaViewComponent {...rnProps}>{children}</RNSafeAreaViewComponent>;
    }
    ...
    return (
      <div ref={ref} style={webSafeAreaStyle} data-testid={testID} {...props}>
        {children}
      </div>
    );
  }
