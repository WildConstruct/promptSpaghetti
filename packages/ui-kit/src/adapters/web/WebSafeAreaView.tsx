...
export const WebSafeAreaView = forwardRef<HTMLDivElement, WebSafeAreaViewProps>(
  ({ children, edges = ['top', 'bottom', 'left', 'right'], mode = 'padding', style, className, ...props }, ref) => {
    ...
    return (
      ...
      `}</style>
        ...
        <div ref={ref} className={`web-safe-area-view ${className || ''}`} style={safeAreaViewStyles} {...props}>
          {children}
        </div>
      </>
    );
