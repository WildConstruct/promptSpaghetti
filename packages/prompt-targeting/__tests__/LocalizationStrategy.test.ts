...
describe('DocumentationLocalizationStrategy', () => {
  ...
  describe('Translation Loading', () => {
    ...
    it('falls back to primary language for unsupported language', async () => {
      ...
      (strategy as any).fetchTranslationFromSource = jest.fn<unknown[], unknown>().mockImplementation((language: string) => {
        if (language === 'xyz') {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Failed to load translation for xyz')
      );
  ...
  describe('String Localization', () => {
    ...
    it('applies interpolations correctly', async () => {
      ...
      (strategy as any).fetchTranslationFromSource = jest.fn<unknown[], unknown>().mockResolvedValue(mockTranslation as unknown);
      
      const result = await strategy.getLocalizedString(
        'greeting', 
        'en', 
        'api',
        { name: 'John', platform: 'OpenAI' }
      );
  ...
  describe('Content Validation', () => {
    it('validates appropriate content successfully', async () => {
      const result = await strategy.validatePromptContent(
        'Please generate a polite response about technology',
        'ja'
      );
    ...
    it('detects culturally inappropriate content', async () => {
      const result = await strategy.validatePromptContent(
        'This content contains controversial political topics',
        'ja'
      );
...
describe('Integration Tests', () => {
  it('completes full localization workflow', async () => {
    const strategy = new DocumentationLocalizationStrategy(
    ...
    const validation = await strategy.validatePromptContent(
      'Por favor, genere una respuesta educada',
      'es'
    );
  ...
  it('handles multiple languages simultaneously', async () => {
    ...
    const translations = await Promise.all(
      languages.map(lang => strategy.loadTranslation(lang))
    );
    ...
    const adaptations = await Promise.all(
      languages.map(lang => strategy.loadCulturalAdaptation(lang))
    );
