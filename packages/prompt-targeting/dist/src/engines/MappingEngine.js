 ...
 */
import { TranslationError, ValidationException } from '../types';
import { createHash } from 'crypto';
/**
 * Implementation of the core mapping engine
 */
export class DefaultMappingEngine {
    registry;
    cache;
    config;
    logger = console;
     ...
     */
    async translate(graph, targetPlatform, config) {
     ...
     */
    async translateBatch(graph, targetPlatforms, config) {
        const startTime = Date.now();
        try {
            // Limit concurrency
            const results = {};
     ...
     */
    async validateTranslation(graph, targetPlatform, config) {
        const startTime = Date.now();
        try {
            ...
            // Perform validation
            const result = await this.withTimeout(adaptor.validate(graph, config), this.config.translationTimeout, `Validation timeout for ${targetPlatform}`);
     ...
     */
    async executeTranslation(graph, targetPlatform, config) {
        // Find best adaptor for the platform
        const adaptor = this.findBestAdaptor(targetPlatform);
        if (!adaptor) {
            throw new TranslationError(`No adaptor found for platform: ${targetPlatform}`, targetPlatform, 'NO_ADAPTOR_FOUND');
        }
        ...
        try {
            const result = await this.withTimeout(adaptor.transform(graph, config), this.config.translationTimeout, `Translation timeout for ${targetPlatform}`);
            // Validate result
            ...
            throw new TranslationError(`Translation failed for ${targetPlatform}: ${error instanceof Error ? error.message : 'Unknown error'}`, targetPlatform, 'TRANSLATION_EXECUTION_FAILED', { adaptorId: adaptor.id, error });
        }
    }
    /**
     * Find the best adaptor for a given platform
     */
    findBestAdaptor(platform) {
     ...
     */
    generateCacheKey(graph, platform, config) {
     ...
     */
    validateTranslationResult(result, platform) {
        if (!result.platform) {
