...
(function (ValidationCategory) {
    ...
    ValidationCategory["CONTENT"] = "content";
    ValidationCategory["PLATFORM"] = "platform";
...
export class ValidationRulesEngine extends EventEmitter {
            ...
            if (!result.result.passed) {
                ...
                if (result.severity === ValidationSeverity.ERROR || result.severity === ValidationSeverity.CRITICAL) {
                    errors.push({
                        ...item,
                        severity: 'error',
                        suggestion: result.rule.getSuggestion?.(report.results[0], result.result),
                    });
                }
                else {
                    warnings.push({
                        ...item,
                        optimization: result.rule.getSuggestion?.(report.results[0], result.result),
                    });
                }
            }
        }
    ...
    getApplicableRules(context) {
        return this.getRules().filter(rule => {
            ...
            if (!this.config.enabledCategories.includes(rule.category))
                return false;
            if (this.getSeverityOrder(rule.severity) < this.getSeverityOrder(this.config.minSeverity))
                return false;
     ...
     */
    async executeRuleWithTimeout(rule, context) {
     ...
     */
    generateReport(ruleResults, totalExecutionTime) {
     ...
     */
    calculateValidationScore(results, summary) {
        const weights = {
     ...
     */
    createEmptyReport(executionTime) {
        return {
            valid: true,
            score: 100,
            executionTime,
     ...
     */
    updateRuleMetrics(ruleId, executionTime, failed) {
        if (!this.config.enableMetrics)
            return;
...
class EmptyGraphRule {
    ...
    async validate(context) {
        ...
        const contentNodes = graph.nodes.filter((node) => node.data?.text || node.data?.content);
...
class TokenLimitRule {
    ...
    getSuggestion(context, result) {
        const overLimit = result.details?.overLimit;
        if (overLimit > 0) {
...
class MissingRequiredPropertiesRule {
...
class EmptyContentRule {
    ...
    async validate(context) {
        ...
        for (const node of graph.nodes) {
            ...
            const isContentNode = ['output', 'text', 'prompt', 'template', 'concat', 'transform', 'generate'].some(type => nodeType.toLowerCase().includes(type));
...
class ContentQualityRule {
    ...
    assessContentQuality(content) {
        ...
        const repeatedWords = Array.from(wordCounts.entries()).filter(([_, count]) => count > Math.max(3, words.length * 0.1)).length;
        if (repeatedWords > 0) {
...
class PlatformCompatibilityRule {
    ...
    checkNodeCompatibility(node, capabilities, platform) {
        const issues = [];
        const nodeData = node.data || {};
    ...
    checkGraphCompatibility(graph, capabilities, platform) {
        const issues = [];
    ...
    checkOpenAICompatibility(nodeData) {
        const issues = [];
    ...
    checkMidjourneyCompatibility(nodeData) {
        const issues = [];
    ...
    checkDALLECompatibility(nodeData) {
        const issues = [];
    ...
    extractAllContent(graph) {
        if (!graph.nodes)
            return '';
        return graph.nodes
            .map((node) => this.extractContent(node.data || {}))
...
class OptimizationOpportunityRule {
...
//# sourceMappingURL=ValidationRulesEngine.js.map