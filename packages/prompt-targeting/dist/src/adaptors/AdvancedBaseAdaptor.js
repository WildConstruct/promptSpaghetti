...
export class AdvancedBaseAdaptor extends BaseAdaptor {
        ...
        try {
            ...
            for (const stage of this.pipeline) {
                ...
                if (!stageResult.success) {
                    throw new TranslationError(`Pipeline stage '${stage.name}' failed: ${stageResult.error?.message}`, this.platforms[0], 'PIPELINE_STAGE_FAILED', { stage: stage.name, error: stageResult.error });
                }
    ...
    async validate(graph, config) {
        ...
        try {
            ...
            this.stats.errors++;
            throw new ValidationException(`Enhanced validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, [], 'ENHANCED_VALIDATION_FAILED');
        }
    }
    /**
    ...
    async executeStage(stage, data, context) {
        ...
        const maxAttempts = typeof retries === 'object' && retries !== null
    ...
    isRetryableError(error, context) {
        const retries = context.config?.pipeline?.retries;
        const retryableErrors = typeof retries === 'object' && retries !== null
    ...
    async postprocessResult(result, context) {
        ...
        if (!isValid) {
            throw new TranslationError('Postprocessing validation failed', context.targetPlatform, 'POSTPROCESSING_VALIDATION_FAILED');
        }
        return filtered;
    }
    /**
