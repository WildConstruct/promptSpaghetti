...
export class MidjourneyAdaptor extends BaseAdaptor {
    ...
    buildMidjourneyPrompt(graph, config) {
        ...
        const promptParts = [mainDescription, styleDescription, technicalDetails].filter(Boolean).join(', ');
    ...
    extractAspectRatio(graph, config) {
        // Check config first
        const midjourneyOverrides = config?.platformOverrides?.midjourney;
        if (midjourneyOverrides?.aspect) {
            return midjourneyOverrides.aspect;
        }
        // Check graph nodes
        if (graph.nodes) {
            for (const node of graph.nodes) {
     ...
     */
    async isValidAspectRatio(ratio) {
        try {
