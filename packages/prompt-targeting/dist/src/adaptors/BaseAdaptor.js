 ...
 */
import { AdaptorError, ValidationException, TranslationError, } from '../types';
...
export class BaseAdaptor {
    ...
    async validate(graph, config) {
        ...
        try {
            ...
            throw new ValidationException(`Validation failed for adaptor ${this.id}: ${error instanceof Error ? error.message : 'Unknown error'}`, [], 'VALIDATION_FAILED');
        }
    }
    /**
    ...
    async transform(graph, config) {
        ...
        try {
            ...
            if (!validation.valid) {
                throw new ValidationException('Graph validation failed before transformation', validation.errors, 'PRE_TRANSFORM_VALIDATION_FAILED');
            }
