 ...
 */
export class OpenAIAdaptor extends BaseAdaptor implements TextToTextAdaptor {
  public readonly id = 'openai-gpt';
  public readonly version = '1.0.0';
  public readonly name = 'OpenAI GPT';
  public readonly description = 'OpenAI GPT models for text generation';
  ...
  private buildOpenAIParameters(graph: any, config?: AdaptorConfig): Record<string, unknown> {
    ...
    if (config?.platformOverrides?.openai) {
      const overrides = config.platformOverrides.openai as Record<string, unknown>;
  ...
  private getModelCapabilities(model: string): { maxTokens: number } {
    const modelCapabilities: Record<string, { maxTokens: number }> = {
   ...
   */
  private detectImageContent(graph: any): boolean {
