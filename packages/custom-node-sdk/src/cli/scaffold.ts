...
async function promptForConfig(): Promise<NodeScaffoldConfig> {
...
function getDefaultConfig(): NodeScaffoldConfig {
  return {
...
${config.inputs.map(
  input => `- **${input.name}** (${input.type}${input.required ? ',
  required' : ',
  optional'}
): ${input.description || 'No description'}`).join('\n')}
