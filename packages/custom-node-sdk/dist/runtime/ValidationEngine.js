...
export class ValidationEngine {
   ...
   */
  buildZodSchema(type, validation, required = true) {
    let schema;
    ...
    switch (type) {
      case 'string':
        ...
        if (validation?.pattern) schema = schema.regex(new RegExp(validation.pattern));
   ...
   */
  validateValidationRules(inputName, validation, type) {
    const errors = [];
    if (type === 'string') {
