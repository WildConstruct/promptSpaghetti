...
export class MappingEngine {
    ...
    async translate(request) {
        ...
        try {
            ...
            if (!adaptor) {
                throw new TranslationError(`No adaptor registered for platform: ${request.targetPlatform}`, 'ADAPTOR_NOT_FOUND', { platform: request.targetPlatform }, false);
            }
            // Check cache first
    ...
    validateRequest(request) {
        ...
        for (const edge of request.graph.edges || []) {
            if (!nodeIds.has(edge.source)) {
                throw new TranslationError(`Edge references invalid source node: ${edge.source}`, 'INVALID_GRAPH', { edgeId: edge.id, sourceId: edge.source }, false);
            }
            if (!nodeIds.has(edge.target)) {
                throw new TranslationError(`Edge references invalid target node: ${edge.target}`, 'INVALID_GRAPH', { edgeId: edge.id, targetId: edge.target }, false);
            }
        }
    }
    /**
