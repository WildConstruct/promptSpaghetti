...
export class OpenAIGPTAdaptor extends BaseAdaptor {
    constructor(context) {
        super('openai-gpt', '1.0.0', 'openai-gpt', 'OpenAI GPT Models', 'Adaptor for OpenAI GPT-3.5 and GPT-4 text generation models', context);
    }
    async capabilities() {
        return {
    ...
    async doValidate(graph) {
        ...
        imageNodes.forEach(node => {
            results.push(this.createValidationResult(`unsupported-image-${node.id}`, 'warning', 'medium', 'Image node not supported', {
        ...
        if (graph.nodes.length > capabilities.maxNodes) {
            results.push(this.createValidationResult('too-many-nodes', 'error', 'high', 'Too many nodes in graph', {
    ...
    async processGraph(graph, transformations) {
        ...
        for (const imageNode of imageNodes) {
            const description = imageNode.data.description || imageNode.data.content || `[Image: ${imageNode.data.label || 'untitled'}]`;
