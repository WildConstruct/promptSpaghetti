...
export class MidjourneyAdaptor extends BaseAdaptor {
    constructor(context) {
        super('midjourney', '1.0.0', 'midjourney', 'Midjourney Image Generator', 'Adaptor for Midjourney text-to-image generation', context);
        this.supportedAspectRatios = ['1:1', '2:3', '3:2', '4:5', '5:4', '16:9', '9:16'];
        this.supportedVersions = ['v6', 'v5.2', 'v5.1', 'v5', 'v4'];
    }
    async capabilities() {
        return {
            supportedNodeTypes: [
    ...
    async doValidate(graph) {
        ...
        if (textOnlyNodes.length === graph.nodes.length && textOnlyNodes.length > 0) {
            results.push(this.createValidationResult('text-only-graph', 'warning', 'medium', 'Graph contains only text nodes', {
        ...
        if (estimatedPromptLength > capabilities.maxPromptLength) {
            results.push(this.createValidationResult('prompt-too-long', 'warning', 'high', 'Prompt may be too long', {
        ...
        if (versionParams.conflicts.length > 0) {
            versionParams.conflicts.forEach(conflict => {
                results.push(this.createValidationResult(`version-conflict-${conflict.param}`, 'warning', 'medium', 'Version-specific parameter conflict', {
     ...
     */
    buildParameterString(parameters, transformations) {
        const paramParts = [];
        // Aspect ratio
     ...
     */
    optimizeForMidjourney(prompt, transformations) {
        ...
        const hasQualityModifiers = qualityKeywords.some(keyword => optimized.toLowerCase().includes(keyword.toLowerCase()));
     ...
     */
    extractVersionSpecificParams(graph) {
        const extractedParams = this.extractParameters(graph);
