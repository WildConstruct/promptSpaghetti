...
export class DallEAdaptor extends BaseAdaptor {
    constructor(context) {
        super('dall-e', '1.0.0', 'openai-dalle', 'DALL-E Image Generator', 'Adaptor for OpenAI DALL-E text-to-image generation', context);
        this.supportedSizes = [
            '256x256',
            '512x512',
            '1024x1024', // DALL-E 2
        ...
        this.supportedQualities = ['standard', 'hd'];
        this.supportedStyles = ['vivid', 'natural'];
        this.supportedModels = ['dall-e-2', 'dall-e-3'];
    }
    async capabilities() {
        return {
            supportedNodeTypes: [
    ...
    async doValidate(graph) {
        ...
        if (estimatedPromptLength > capabilities.maxPromptLength) {
            results.push(this.createValidationResult('prompt-too-long', 'error', 'high', 'Prompt exceeds DALL-E maximum length', {
        ...
        if (model === 'dall-e-3') {
            if (extractedParams.n && extractedParams.n > 1) {
                results.push(this.createValidationResult('dalle3-multiple-images', 'error', 'medium', 'DALL-E 3 only supports single image generation', {
            ...
            if (extractedParams.size && !['1024x1024', '1792x1024', '1024x1792'].includes(extractedParams.size)) {
                results.push(this.createValidationResult('dalle3-invalid-size', 'warning', 'medium', 'Size not optimized for DALL-E 3', {
        ...
        if (model === 'dall-e-2') {
            if (extractedParams.quality && extractedParams.quality !== 'standard') {
                results.push(this.createValidationResult('dalle2-quality-not-supported', 'warning', 'low', 'Quality parameter not supported in DALL-E 2', {
            ...
            if (extractedParams.style && extractedParams.style !== 'vivid') {
                results.push(this.createValidationResult('dalle2-style-not-supported', 'warning', 'low', 'Style parameter not supported in DALL-E 2', {
    ...
    async processGraph(graph, transformations) {
        ...
        if (outputNodes.length === 0) {
            ...
            const contentNodes = graph.nodes.filter(node => ['text', 'style', 'image'].includes(node.type) && (node.data.content || node.data.description));
     ...
     */
    optimizeForDallE(prompt, parameters, transformations) {
    ...
    checkContentPolicy(graph) {
        ...
        graph.nodes.forEach(node => {
            ...
            if (flaggedWords.length > 0) {
                results.push(this.createValidationResult(`content-policy-${node.id}`, 'warning', 'high', 'Potential content policy issue', {
     ...
     */
    buildApiParameters(parameters) {
        const apiParams = {};
        // Map internal parameters to API parameters
        if (parameters.model)
            apiParams.model = parameters.model;
        if (parameters.size)
            apiParams.size = parameters.size;
    ...
    validateParameter(name, value, spec) {
        if (spec.type === 'number') {
            if (typeof value !== 'number' || isNaN(value)) {
                return this.createValidationResult(`invalid-param-${name}`, 'error', 'medium', `Invalid number value for parameter "${name}"`, {
                    description: `Expected number, got ${typeof value}`,
                    autoFixable: true,
                });
            }
            if (spec.min !== undefined && value < spec.min) {
                return this.createValidationResult(`param-too-low-${name}`, 'warning', 'low', `Parameter "${name}" value too low`, {
                    description: `Value ${value} is below minimum ${spec.min}`,
                    autoFixable: true,
                });
            }
            if (spec.max !== undefined && value > spec.max) {
                return this.createValidationResult(`param-too-high-${name}`, 'warning', 'low', `Parameter "${name}" value too high`, {
        ...
        if (spec.type === 'enum' && spec.options) {
            if (!spec.options.includes(value)) {
                return this.createValidationResult(`invalid-enum-${name}`, 'error', 'medium', `Invalid enum value for parameter "${name}"`, {
    ...
    normalizeParameter(value, spec) {
        if (spec.type === 'number') {
            ...
            if (spec.min !== undefined)
                return Math.max(spec.min, num);
            if (spec.max !== undefined)
                return Math.min(spec.max, num);
            return num;
        }
        if (spec.type === 'enum' && spec.options) {
