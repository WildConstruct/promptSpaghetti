import { MappingEngine, ValidationEngine, OpenAIGPTAdaptor, MidjourneyAdaptor, ConsoleLogger, MemoryCache, MemoryMetrics, } from '../index.js';
/**
...
export class PromptTargetingExample {
    ...
    async complexGraphTranslation() {
        ...
        const platforms = ['openai-gpt', 'midjourney'];
        for (const platform of platforms) {
            ...
            if (response.success) {
                ...
                if (platform === 'midjourney') {
                    ...
                    const content = response.targetPrompt?.content;
                    console.log('User Message:', content?.messages?.find((m) => m.role === 'user')?.content);
                }
            }
            else {
                console.log('❌ Translation failed:', response.error?.message);
...
export function createPromptTargetingExample() {
