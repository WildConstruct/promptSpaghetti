...
export class MetadataManager {
     ...
     */
    registerSchema(platform, schema) {
     ...
     */
    extractMetadata(result, platform, baseMetadata) {
        const extraction = {
        ...
        try {
            ...
            if (result.prompt) {
                ...
                extraction.metadata.transformations = result.prompt.metadata.transformations?.map((t) => t.action) || [];
                extraction.metadata.warnings = result.prompt.metadata.warnings?.map((w) => w.message) || [];
            }
            ...
            items = items.filter(item => {
                return Object.entries(options.filters).every(([key, value]) => {
                    ...
                    if (typeof value === 'object' && value !== null) {
                        if ('min' in value && itemValue < value.min)
                            return false;
                        if ('max' in value && itemValue > value.max)
                            return false;
                        if ('equals' in value && itemValue !== value.equals)
                            return false;
                        if ('contains' in value && !String(itemValue).includes(value.contains))
                            return false;
                    }
    ...
    getAnalytics() {
        ...
        for (let i = 7; i >= 0; i--) {
            ...
            const dayItems = items.filter(item => item.created.toDateString() === date.toDateString());
     ...
     * Export metadata
     */
    exportMetadata(format, filter) {
        const searchResult = filter
            ? this.searchMetadata(filter)
            : {
     ...
     */
    autoGenerateCategories(metadata) {
        const categories = [];
    ...
    convertToCSV(items) {
        if (items.length === 0)
            return '';
        const headers = Object.keys(items[0]).filter(key => typeof items[0][key] !== 'object' || items[0][key] instanceof Date);
