...
export class AdaptorTestFramework {
    ...
    async testInterfaceCompliance(adaptor) {
        ...
        if (!capabilities)
            throw new Error('capabilities() returned null/undefined');
    ...
    async testLifecycleCompliance(adaptor, context) {
        ...
        if (!health)
            throw new Error('healthCheck() returned null/undefined');
    ...
    async testValidationCompliance(adaptor) {
        ...
        for (const result of results) {
            if (!result.id)
                throw new Error('ValidationResult missing id');
    ...
    async testTransformationCompliance(adaptor) {
        ...
        if (!result)
            throw new Error('transform() returned null/undefined');
        // Validate result structure
        if (!result.platform)
            throw new Error('TargetPrompt missing platform');
        if (!result.content)
            throw new Error('TargetPrompt missing content');
        if (!result.format)
            throw new Error('TargetPrompt missing format');
        if (!result.metadata)
            throw new Error('TargetPrompt missing metadata');
        // Test quality estimation
        const quality = await adaptor.estimateQuality(testGraph);
        if (!quality)
            throw new Error('estimateQuality() returned null/undefined');
    ...
    async testCacheEfficiency(adaptor, testGraphs) {
        ...
        this.testResults.performance.cacheEfficiency = Math.max(0, 100 - (avgCacheTime / this.testResults.performance.transformationTime) * 100);
     ...
     */
    generateSummary() {
        const totalErrors = this.testResults.compliance.errors.length +
