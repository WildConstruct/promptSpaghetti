  ...
  extractMetadata(result: any, platform: Platform, baseMetadata?: Partial<GenerationMetadata>): ExtractionResult {
    ...
    try {
      ...
      if (result.prompt) {
        ...
        extraction.metadata.transformations = result.prompt.metadata.transformations?.map((t: any) => t.action) || [];
        extraction.metadata.warnings = result.prompt.metadata.warnings?.map((w: any) => w.message) || [];
      }
