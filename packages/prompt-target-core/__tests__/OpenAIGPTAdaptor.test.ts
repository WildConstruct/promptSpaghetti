  ...
  describe('validate', () => {
    it('should validate a simple text graph', async () => {
      ...
      const criticalErrors = results.filter(r => 
        r.type === 'error' && r.severity === 'critical'
      );
    ...
    it('should detect unsupported image nodes', async () => {
      ...
      const imageWarnings = results.filter(r => 
    ...
    it('should reject empty graphs', async () => {
      ...
      const emptyGraphErrors = results.filter(r => 
        r.id === 'empty-graph'
      );
