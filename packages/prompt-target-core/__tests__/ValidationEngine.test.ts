...
import { PromptGraph } from '../src/types/index.js';
...
describe('ValidationEngine', () => {
  ...
  describe('structural validation', () => {
    ...
    it('should detect disconnected nodes', async () => {
      const disconnectedGraph: PromptGraph = {
        id: 'disconnected',
        version: '1.0',
        nodes: [
          {
    ...
    it('should detect invalid edges', async () => {
      const invalidEdgeGraph: PromptGraph = {
        id: 'invalid-edges',
        version: '1.0',
        nodes: [
          {
  ...
  describe('platform-specific validation', () => {
    it('should validate OpenAI GPT compatibility', async () => {
      const gptCompatibleGraph: PromptGraph = {
        id: 'gpt-compatible',
        version: '1.0',
        nodes: [
          {
    ...
    it('should detect OpenAI GPT incompatibilities', async () => {
      const gptIncompatibleGraph: PromptGraph = {
        id: 'gpt-incompatible',
        version: '1.0',
        nodes: [
          {
      ...
      const imageWarnings = gptResult?.results.filter(r => 
        r.message.includes('Image node not supported')
      );
      expect(imageWarnings?.length).toBeGreaterThan(0);
    });

    it('should validate Midjourney compatibility', async () => {
      const midjourneyCompatibleGraph: PromptGraph = {
  ...
  describe('cross-platform validation', () => {
    it('should analyze cross-platform compatibility', async () => {
      ...
      const gptImageWarnings = gptResult?.results.filter(r => 
        r.message.includes('Image node not supported')
      );
    ...
    it('should identify cross-platform feature gaps', async () => {
      ...
      const featureIssues = report.crossPlatformIssues.filter(issue => 
        issue.type === 'feature_support'
      );
  ...
  describe('auto-fix suggestions', () => {
    it('should generate auto-fix suggestions for common issues', async () => {
      ...
      const orphanFixes = report.autoFixSuggestions.filter(suggestion =>
