...
import { PluginContext, PromptGraph, PromptNode, PromptEdge } from '../src/types/index.js';

describe('DallEAdaptor', () => {
  let adaptor: DallEAdaptor;
  ...
  describe('validation', () => {
    it('should validate prompt length', async () => {
      ...
      const graph: PromptGraph = createTestGraph([
  ...
  describe('transformation', () => {
    ...
    it('should transform with custom parameters', async () => {
      ...
      const apiParams = (result.metadata as any)?.apiParameters;
    ...
    it('should truncate overly long prompts', async () => {
      ...
      expect(result.content as string).toMatch(/\.\.\.$/); // ends with ...
    ...
    it('should generate proper metadata', async () => {
      ...
      expect((result.metadata as any)?.apiParameters).toBeDefined();
    });
  });

  describe('quality estimation', () => {
    it('should calculate quality score based on graph complexity', async () => {
      const simpleGraph: PromptGraph = createTestGraph([
        { id: 'node1', type: 'text', data: { content: 'cat' } }
      ]);

      const complexGraph: PromptGraph = createTestGraph([
  ...
  describe('error handling', () => {
    it('should handle missing graph nodes gracefully', async () => {
      const emptyGraph: PromptGraph = createTestGraph([]);
  ...
  function createTestGraph(nodes: Partial<PromptNode>[], edges: Partial<PromptEdge>[] = []): PromptGraph {
    return {
      id: 'test-graph',
