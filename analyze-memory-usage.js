...
async function analyzeMemoryUsage() {
  ...
  for (const size of graphSizes) {
    ...
    console.log(
      `  ${size.toString().padEnd(4)} nodes: ${formatMemory(graphMemory)}, ${(memoryPerNodeInGraph / 1024).toFixed(2)} KB/node`
    );
  }
  ...
  if (memoryGrowth) {
    ...
    if (memoryGrowth.growthRate > 1024 * 1024) {
      // 1MB/sec
  ...
  console.log(
    `  - Linear scaling: ${(graphMemories[4].memoryPerNode / graphMemories[0].memoryPerNode).toFixed(2)}x from small to large graphs`
  );
...
if (require.main === module) {
  analyzeMemoryUsage()
    .then(results => {
      ...
      console.log(
        `  - Total optimization potential: ${formatMemory(
