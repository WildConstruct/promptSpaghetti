...
async function analyzeSchemaValidationPerformance() {
  ...
  for (const [nodeType, schema] of Object.entries(nodeSchemas)) {
    ...
    console.log(`  ${' '.repeat(15)}   ${validOpsPerSec.toFixed(0)} ops/sec valid, ${invalidOpsPerSec.toFixed(0)} ops/sec invalid`);
  }
  ...
  for (const nodeType of Object.keys(nodeSchemas)) {
    ...
    console.log(`  ${nodeType.padEnd(15)}: ${speedup.toFixed(1)}x faster (${improvementPercent.toFixed(1)}% improvement)`);
  }
  ...
  const totalValidationOverhead = Object.values(nodeExecutionTimes)
    .reduce((sum, times) => sum + times.overhead, 0);
  const avgOverheadPercent = Object.values(nodeExecutionTimes)
    .reduce((sum, times) => sum + times.overheadPercent, 0) / Object.keys(nodeExecutionTimes).length;
  ...
  console.log(`  - Overall performance improvement: ${(potentialSavings / totalValidationOverhead * avgOverheadPercent).toFixed(1)}%`);
...
if (require.main === module) {
  analyzeSchemaValidationPerformance()
    .then(results => {
      ...
      const avgValidTime = Object.values(results.validationPerformance)
        .reduce((sum, perf) => sum + perf.validTime, 0) / Object.keys(results.validationPerformance).length;
      const avgOverhead = Object.values(results.executionOverhead)
        .reduce((sum, overhead) => sum + overhead.overheadPercent, 0) / Object.keys(results.executionOverhead).length;
