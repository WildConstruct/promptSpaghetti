...
import { MVP_SHIP_PHASES } from './mvpShip.config.mjs';
...
function getSelectedPhases(selectedPhaseId) {
  if (!selectedPhaseId) {
    return MVP_SHIP_PHASES;
  }

  const selectedPhase = MVP_SHIP_PHASES.find(
    phase => phase.id === selectedPhaseId
  );

  if (!selectedPhase) {
    const availablePhases = MVP_SHIP_PHASES.map(phase => phase.id).join(', ');
    console.error(
      `Unknown MVP ship phase "${selectedPhaseId}". Available phases: ${availablePhases}`
    );
    process.exit(1);
  }
...
async function main() {
  ...
  console.log('Phase 4d MVP ship bar');
  console.log(
    selectedPhaseId
  ...
  for (const [index, phase] of phases.entries()) {
    try {
      ...
      console.error('\nMVP ship validation failed.');
      console.error(error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  }

  console.log('\nMVP ship validation passed.');
}
