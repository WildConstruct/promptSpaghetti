...
async function analyzeUndefinedVariables(directory) {
  ...
  try {
    ...
    const jsxUndefinedMatches = output.match(/(.*?):\s*line\s*(\d+).*?'(\w+)'\s+is\s+not\s+defined.*?react\/jsx-no-undef/g) || [];
    
    const allMatches = [...undefinedMatches, ...jsxUndefinedMatches];
    
    const issues = allMatches.map(match => {
...
async function addMissingImports(filePath, issues) {
  ...
  for (const [module, variables] of Object.entries(importsByModule)) {
    if (module === 'react') {
      ...
      const existingReactImport = content.match(/import\s+(?:React(?:\s*,\s*)?)?(?:\{\s*([^}]*)\s*\})?\s+from\s+['"]react['"];/);
