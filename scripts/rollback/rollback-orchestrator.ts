...
export class RollbackOrchestrator {
  ...
  private async rollbackFrontend(
    ...
    try {
      ...
      const previousVersion =
        config.targetVersion || (await this.getPreviousVersion('frontend'));
      await execAsync(`
  ...
  private async rollbackBackend(
    ...
    try {
      ...
      const previousVersion =
        config.targetVersion || (await this.getPreviousVersion('backend'));
  ...
  private async verifySystemHealth(): Promise<boolean> {
    ...
    const allHealthy = Object.values(health).every(check => check.passed);
