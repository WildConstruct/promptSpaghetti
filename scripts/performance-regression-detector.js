...
class PerformanceRegressionDetector {
  ...
  async collectQualityTestMetrics() {
    try {
      ...
      this.logStep(
        `Quality tests: ${qualityTestDuration}ms, ${report.passedTests}/${report.totalTests} passed`,
        'info'
      );
    } catch (error) {
  ...
  async detectRegressions() {
    ...
    this.compareMetric('testExecution', 'duration', 'Test Execution Time', this.thresholds.testExecution, 'ms');

    // Compare bundle sizes
    this.compareMetric('bundle', 'total', 'Bundle Size', this.thresholds.bundleSize, 'bytes');

    // Compare build times
    this.compareMetric('build', 'duration', 'Build Time', this.thresholds.buildTime, 'ms');

    // Compare memory usage
    this.compareMetric('memory', 'heapUsedDelta', 'Memory Usage', this.thresholds.memoryUsage, 'bytes');

    // Compare quality test performance
    this.compareMetric('qualityTests', 'duration', 'Quality Test Duration', this.thresholds.testExecution, 'ms');

    this.logStep(
      `Found ${this.regressions.length} regressions and ${this.improvements.length} improvements`,
  ...
  async analyzeTrends() {
    ...
    metrics.forEach(metricPath => {
      ...
      if (trend) {
        this.logStep(
          `${metricPath}: ${trend.direction} trend (${trend.changeRate}% per week)`,
