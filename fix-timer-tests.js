      ...
      if (matches) {
        matches.forEach(match => {
          ...
          if (timeMatch) {
            ...
            const replacement = timeout <= 100 ? 
              `await new Promise(resolve => setImmediate(resolve));` :
              `jest.advanceTimersByTime(${timeout});`;
            updatedContent = updatedContent.replace(match, replacement);
            timersFixed++;
            console.log(`✅ ${path.relative('.', filePath)}: Fixed setTimeout(${timeout}ms) → ${timeout <= 100 ? 'setImmediate' : 'advanceTimersByTime'}`);
          }
        });
      }
      ...
      if (directMatches) {
        directMatches.forEach(match => {
          ...
          if (timeMatch) {
            ...
            updatedContent = updatedContent.replace(match, replacement);
            timersFixed++;
            console.log(`✅ ${path.relative('.', filePath)}: Fixed direct setTimeout(${timeout}ms) → ${timeout <= 100 ? 'setImmediate' : 'Promise+advanceTimersByTime'}`);
          }
        });
      }
