...
class DatabaseEnhancer {
  ...
  buildEpicIndex(tasks) {
    ...
    Object.values(index).forEach(epic => {
      epic.stats.avgPriority = epic.stats.total > 0 ? 
        Object.entries(epic.stats.byPriority).reduce((sum, [p, count]) => sum + (parseInt(p) * count), 0) / epic.stats.total : 0;
    });
  ...
  generateSearchKeywords(task) {
    ...
    // Title words
    if (task.title) {
      task.title.toLowerCase().split(/\W+/).forEach(word => {
    ...
    if (task.epicMeta) {
      ...
      task.epicMeta.title.toLowerCase().split(/\W+/).forEach(word => {
        if (word.length > 2) keywords.add(word);
      });
    }
  ...
  async validateSchema() {
    ...
    console.log(`Tasks with Epic Metadata: ${stats.tasksWithEpicMeta} (${Math.round((stats.tasksWithEpicMeta/stats.totalTasks)*100)}%)`);
    console.log(`Tasks with Story Metadata: ${stats.tasksWithStoryMeta} (${Math.round((stats.tasksWithStoryMeta/stats.totalTasks)*100)}%)`);
    console.log(`Tasks with DB Indexes: ${stats.tasksWithIndexes} (${Math.round((stats.tasksWithIndexes/stats.totalTasks)*100)}%)`);
