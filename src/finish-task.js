...
async function finishEpic1Task() {
  ...
  try {
    ...
    if (task.status === 'COMPLETED' && oldStatus !== 'COMPLETED') {
      const storyTasks = epic1Data.tasks.filter(
        t => t.storyId === task.storyId
      );
      const completedTasks = storyTasks.filter(
        t => t.status === 'COMPLETED'
      ).length;
      const storyProgress = Math.round(
        (completedTasks / storyTasks.length) * 100
      );
    ...
    console.log(
      `  Story: ${task.storyTitle} (${task.metadata.storyProgress}% complete)`
    );
  ...
  try {
    const result = await transaction(state => {
      ...
      if (
        ...
        const assignments =
          state.assignments && state.assignments[task.assignee];
        if (assignments) {
