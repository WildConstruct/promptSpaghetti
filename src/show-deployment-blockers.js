  ...
  try {
    ...
    const blockers = Object.values(state.tasks)
      ...
      .sort((a, b) => {
        ...
        const aFoundational = ['TYPES-DB-001', 'TYPES-AUTH-001'].includes(a.metadata?.taskCode);
        const bFoundational = ['TYPES-DB-001', 'TYPES-AUTH-001'].includes(b.metadata?.taskCode);

        if (aFoundational && !bFoundational) return -1;
        if (!aFoundational && bFoundational) return 1;

        return (a.metadata?.taskCode || '').localeCompare(b.metadata?.taskCode || '');
      });
    ...
    const foundational = blockers.filter(t => ['TYPES-DB-001', 'TYPES-AUTH-001'].includes(t.metadata?.taskCode));
    ...
    const infrastructure = blockers.filter(t =>
      ['FASTIFY-PLUGIN-001', 'SECURITY-TYPES-001', 'EXPORT-CONFLICTS-001'].includes(t.metadata?.taskCode)
    );
    ...
    infrastructure.forEach(t => {
      ...
      console.log(`   ${status} ${t.id}: ${t.metadata?.taskCode}`);
      console.log(`      "${t.title}"`);
      console.log(`      ⏱️  ${t.estimate} | Dependencies: ${t.dependencies.join(', ') || 'None'}`);
    ...
    const features = blockers.filter(t => ['TEMPLATE-PARSER-001', 'VFX-TYPES-001'].includes(t.metadata?.taskCode));
    ...
    const completed = blockers.filter(t => t.state === 'DONE' || t.state === 'COMPLETED').length;
    ...
    console.log('1. 🔥 HIGHEST PRIORITY: Grab deployment blocker tasks immediately');
    console.log('2. 📍 Start with foundational: TYPES-DB-001 and TYPES-AUTH-001');
    console.log('3. 📝 To grab tasks: node src/grab-tasks.js <your-agent-id> 1');
    ...
    console.error('❌ Failed to load deployment blocker status:', error.message);
    process.exit(1);
  }
}
