...
async function showPriorityTasks() {
  ...
  try {
    ...
    const epic8Tasks = priorityTasks.filter(task => 
    ...
    // Summary statistics
    const availableEpic8Tasks = epic8Tasks.filter(t => t.state === 'UNASSIGNED' && (!t.assignee || t.assignee === null));
    ...
    console.log('=' * 70);
    console.log(`🔓 Available Tasks: ${totalAvailable} (${availableEpic8Tasks.length} epic8 + ${availableAuthTasks.length} auth + ${availableFileTasks.length} file)`);
