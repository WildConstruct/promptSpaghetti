...
const selectSinceStmt = db.prepare(
  'SELECT id, ts, type, actor, payload, version FROM events WHERE id > ? ORDER BY id LIMIT ?'
);
const selectByTypeStmt = db.prepare(
  'SELECT id, ts, type, actor, payload, version FROM events WHERE type = ? AND id > ? ORDER BY id LIMIT ?'
);
const selectLatestStmt = db.prepare(
  'SELECT id, ts, type, actor, payload, version FROM events ORDER BY id DESC LIMIT ?'
);
