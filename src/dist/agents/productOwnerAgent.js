...
class ProductOwnerAgent extends agentBase_1.AgentRunner {
  ...
  filterRelevant(events) {
    const relevantTypes = ['PHASE_CHANGED', 'TASK_MOVED_TO_REVIEW', 'STORY_CREATED', 'METRICS_PUBLISHED'];
  ...
  createStory(goalId) {
    const storyTemplates = [
      ...
      },
      {
        ...
        acceptance: ['Shows key metrics prominently', 'Updates in real-time', 'Mobile responsive design'],
      },
      {
        title: 'Export functionality',
        acceptance: ['Can export data as CSV', 'Can export data as JSON', 'Includes date range selection'],
      },
    ];
