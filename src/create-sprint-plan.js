...
const EPIC_PRIORITIES = {
  ...
  collaboration: [
    { epic: 23, name: 'Collaborative Workspaces & Real-time Co-editing', priority: 5, tasks: 41, dependencies: [11, 22] },
  ...
  research: [
    ...
    { epic: 29, name: 'Advanced LLM Research & Experimental Features', priority: 6, tasks: 0, dependencies: [25, 26, 27] }
  ]
};
...
function analyzeTasksByEpic(state) {
  ...
  Object.values(EPIC_PRIORITIES).flat().forEach(epic => {
  ...
  SPRINT_THEMES.forEach(phase => {
    ...
    phase.sprints.forEach(sprintNum => {
      ...
      const availableEpics = allEpics.filter(epic => {
        ...
        const depsCompleted = epic.dependencies.every(dep => 
          dependencyGraph[dep]?.completed || dependencyGraph[dep]?.analysis.totalTasks === 0
        );
      ...
      availableEpics.forEach(epic => {
        ...
        if (analysis && analysis.totalStoryPoints > 0) {
          ...
          if (epicPortionPoints >= 10) { // Minimum viable chunk
            sprint.epics.push({
              epic: epic.epic,
              name: epic.name,
...
async function createSprintPlan() {
  ...
  try {
    ...
    SPRINT_THEMES.forEach(phase => {
      ...
      console.log(`   Phase ${phase.phase}: ${phase.theme} - ${phaseSprints.length} sprints, ${phaseWeeks} weeks, ${phasePoints} points`);
    });
