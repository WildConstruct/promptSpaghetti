...
class TestCaseGenerator {
  ...
  analyzeFileStructure(content, filePath) {
    ...
    // Extract exports
    const exportMatches = [...content.matchAll(/export\s+(?:default\s+)?(?:class|function|const|let|var)?\s*([a-zA-Z_$][a-zA-Z0-9_$]*)?/g)];
  ...
  generateEdgeCaseTests(codeAnalysis) {
    ...
    for (const func of codeAnalysis.functions.slice(0, 5)) { // Limit to first 5 functions
  ...
  generateFunctionMocks(func, codeAnalysis) {
    ...
    const externalImports = codeAnalysis.imports.filter(imp => 
      !imp.module.startsWith('.') && !imp.module.startsWith('/')
    );
    
    for (const imp of externalImports.slice(0, 3)) { // Limit to 3 mocks
  ...
  async getStatistics() {
    return {
      ...
      averageTestsPerTask: this.analysisResults.tasksAnalyzed > 0 
