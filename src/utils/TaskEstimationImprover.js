...
class TaskEstimationImprover {
  ...
  async trainModels() {
    ...
    try {
      ...
      if (trainingData.length < this.config.learning.minimumSamples) {
        console.log(`⚠️  Insufficient training data (${trainingData.length} samples, need ${this.config.learning.minimumSamples})`);
        return false;
      }
  ...
  prepareTrainingData() {
    ...
    const completions = this.estimationHistory.filter(record => 
  ...
  evaluateModels(modelResults, testSet) {
    ...
    evaluation.ensemble = {
      ...
      mse: this.calculateMSEFromPredictions(ensemblePredictions, testSet.map(s => s.target)),
      mae: this.calculateMAEFromPredictions(ensemblePredictions, testSet.map(s => s.target))
    };
  ...
  assessDataQuality(features) {
    ...
    const completeness = Object.values(features).filter(v => v !== undefined && v !== null).length / Object.keys(features).length;
  ...
  async checkRetrainingNeeds() {
    const newCompletions = this.estimationHistory.filter(record => 
      record.type === 'completion' && 
  ...
  async getStatistics() {
    ...
    return {
      ...
      dataQuality: {
        ...
        dataCompletenessRate: completions.length > 0 ? 
          (completions.filter(c => c.originalEstimate !== undefined).length / completions.length * 100).toFixed(1) + '%' : '0%'
      }
    };
  }
