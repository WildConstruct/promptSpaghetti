...
function checkCommitStatus() {
  ...
  if (!fs.existsSync(commitTrackingFile)) {
    ...
    console.log(
      '🎯 GitHub automation will start tracking after first QA approval'
    );
    return;
  }
  ...
  if (commitData.readyForPush) {
    ...
    console.log(
      `⏳ Need ${remaining} more approvals to trigger GitHub automation`
    );
  } else {
  ...
  if (unpushedCount > 0) {
    ...
    commitData.unpushedApprovals.forEach((approval, index) => {
      console.log(
        `   ${index + 1}. ${approval.taskId} (${approval.approvedAt})`
      );
    });
  }
}
