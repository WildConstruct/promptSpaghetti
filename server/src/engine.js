    ...
    try {
        ...
        async function dfs(nodeId) {
            ...
            try {
                ...
                if (analyticsCollector) {
                    analyticsCollector.recordNodeExecution(nodeId, node.type, graphId, executionTimeMs, true, // success
                    JSON.stringify(resolvedInputs).length, JSON.stringify(result).length);
                }
                return result;
            }
            catch (error) {
        ...
        if (analyticsCollector) {
            analyticsCollector.recordGraphExecutionComplete(graphId, executionTimeMs, totalOutputLength, graph.nodes.length, graph.edges?.length || 0);
        }
        ...
        const executionTimeMs = endTime - startTime;
        if (analyticsCollector) {
            analyticsCollector.recordGraphExecutionError(graphId, error instanceof Error ? error.message : String(error), graph.nodes.length, graph.edges?.length || 0, executionTimeMs);
        }
