...
import { ConcatNode, GetVariableNode, IncludeNode, OutputNode, SetVariableNode, WeightedChoiceNode } from '../../packages/core/runtime/index.js';
 ...
 */
function processTemplateVariables(template, executionContext) {
...
export function initializeAnalytics() {
    try {
        ...
        analyticsCollector.on('events_flushed', (events) => {
            if (analyticsDAO) {
                events.forEach((event) => analyticsDAO.storeEvent(event));
            }
        });
 ...
 */
export async function executeGraph(graph, sessionId, userId) {
        ...
        if (analyticsCollector && analyticsBuffer.length > 0) {
            ...
            analyticsBuffer.forEach(event => {
                if (event.type === 'nodeExecution') {
                    analyticsCollector.recordNodeExecution(event.nodeId, event.nodeType, event.graphId, event.executionTimeMs, event.success, event.error);
                }
    ...
    try {
        ...
        async function dfs(nodeId, depth = 0) {
            ...
            try {
                ...
                const executionTimeMs = nodeEndTime - nodeStartTime;
                if (analyticsCollector) {
                    analyticsCollector.recordNodeExecution(nodeId, node.type, graphId, executionTimeMs, false, // success
                    undefined, undefined, error instanceof Error ? error.message : String(error));
                }
                throw error;
            }
        }
        ...
        if (analyticsCollector) {
            analyticsCollector.recordGraphExecutionComplete(graphId, executionTimeMs, totalOutputLength, graph.nodes.length, countGraphConnections(graph));
        }
        ...
        const executionTimeMs = endTime - startTime;
        if (analyticsCollector) {
            analyticsCollector.recordGraphExecutionError(graphId, error instanceof Error ? error.message : String(error), graph.nodes.length, countGraphConnections(graph), executionTimeMs);
        }
 ...
 */
export async function executeGraphLegacy(graph, sessionId, userId) {
 ...
 */
function countGraphConnections(graph) {
...
function createRuntimeNode(node, resolvedInputs, executionContext) {
    switch (node.type) {
        // Basic Epic 3 nodes
        case 'WeightedChoice':
