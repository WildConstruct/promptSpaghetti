...
function validateDeterministicExecution(graph, seed) {
  ...
  for (let i = 0; i < 3; i++) {
    const response = http.post('http://localhost:8000/preview', JSON.stringify({
      graph: graph,
      seeds: [seed]
    }), {
  ...
  const isConsistent = results.length === 3 && 
...
export default function () {
  ...
  const response = http.post('http://localhost:8000/preview', JSON.stringify({
    ...
    tags: { 
      graph_type: selectedGraph === testGraphs.simple ? 'simple' : 
                  selectedGraph === testGraphs.complex ? 'complex' : 'advanced'
    }
  });
