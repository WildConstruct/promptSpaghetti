...
export async function ticketRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{
    ...
    try {
      const filters = request.query.status || request.query.assigned_to || request.query.epic_id
        ? {
  ...
  fastify.patch<{
    ...
    try {
      ...
      const updatedTicket = await ticketDAO.updateTicketStatus(
        request.params.id,
        status,
        changed_by
      );
  ...
  fastify.get('/tickets/config/github', {
    schema: {
      response: {
