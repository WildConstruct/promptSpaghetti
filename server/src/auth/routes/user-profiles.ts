...
export async function registerUserProfileRoutes(
  ...
  fastify.post('/profiles/:userId/connection', {
    ...
    try {
      ...
      const connection = await profileService.updateConnection(
        request.user!.id,
        userId,
        action,
        connectionType
  ...
  fastify.get('/profile/connections', {
  ...
  fastify.get('/profile/completion', {
    preHandler: [requirePermission('user:profile:read')]
  }, async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
      const profile = await profileService.getUserProfile(
  ...
  fastify.get('/skills/popular', {
    ...
    try {
      ...
      const skills = [
        ...
        'UI/UX Design', 'Agile', 'GraphQL', 'MongoDB', 'Redis',
        'Microservices', 'DevOps', 'CI/CD', 'Git', 'Testing',
    ...
    try {
      const metadata = {
        ...
        industries: [
          ...
          'Real Estate', 'Transportation', 'Energy', 'Entertainment', 'Other'
        ],
        timezones: [
  ...
  fastify.get('/profiles/statistics', {
