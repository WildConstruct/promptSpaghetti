...
export async function registerDataRetentionRoutes(
  ...
  fastify.post('/retention/policies', {
  ...
  fastify.put('/retention/policies/:id', {
    preHandler: [requirePermission('admin:retention:write')]
  }, async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post('/retention/execute', {
  ...
  fastify.post('/retention/export', {
    ...
    try {
      ...
      if (requestData.userId !== requestedBy && 
          !request.user?.permissions.includes('admin:data:export')) {
  ...
  fastify.get('/retention/data-types', {
    preHandler: [requirePermission('user:data:export')]
  }, async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get('/retention/health', {
    ...
    try {
      ...
      const health = {
        ...
        lastExecution: recentHistory.length > 0 
          ? recentHistory[0].executedAt 
          : null,
  ...
  fastify.post('/retention/initialize', {
  ...
  fastify.post('/retention/schedule', {
    preHandler: [requirePermission('admin:system:write')]
  }, async (request: AuthenticatedRequest, reply: FastifyReply) => {
    try {
