...
export class OptimizationCLI {
  ...
  private setupCommands(): void {
    this.program.name('optimization-cli').description('Administrative optimization tools CLI').version('1.0.0');

    // Recommendations commands
    const recommendations = this.program.command('recommendations').description('Manage optimization recommendations');
    ...
    // Policies commands
    const policies = this.program.command('policies').description('Manage optimization policies');
    ...
    const optimize = this.program.command('optimize').description('Execute optimization operations');
    ...
    const profiles = this.program.command('profiles').description('Manage optimization profiles');
    ...
    const analytics = this.program.command('analytics').description('Generate optimization analytics and reports');
    ...
    // Utility commands
    const utils = this.program.command('utils').description('Utility commands for optimization management');
  ...
  private async applyRecommendation(recommendationId: string, options: any): Promise<void> {
    ...
    try {
      ...
      const result = await this.optimizationService.applyOptimizationRecommendation(recommendationId, options.user, {
