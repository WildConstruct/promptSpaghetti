...
export const TICKET_TRANSITIONS: 
  Record<TicketStatusType, TicketStatusType[]> = {
    [TicketStatus.OPEN]: [
      TicketStatus.IN_PROGRESS, 
      TicketStatus.BLOCKED, 
      TicketStatus.CLOSED
...
export const GitHubAutomationConfigSchema = z.object({
  ...
  pr_title_template: z.string()
    .default('[{{ticket.id}}] {{ticket.title}}'),
  pr_body_template: z.string().optional(),
  commit_message_template: z.string()
    .default('feat({{ticket.id}}): {{description}}'),
...
export type GitHubAutomationConfig = 
  z.infer<typeof GitHubAutomationConfigSchema>;
...
export type TicketWebhookEvent = 
  z.infer<typeof TicketWebhookEventSchema>;