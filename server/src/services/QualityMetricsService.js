    ...
    try {
      ...
      return {
        ...
        compliance: {
          ...
          overallScore: 88,
          gaps: [{ framework: 'GDPR', requirement: 'Data Retention Policy', status: 'partial', priority: 'medium' }],
        },
        accessControl: {
  ...
  calculateOverallQualityScore(metrics) {
    ...
    const componentScores = {
      ...
      codeQuality: Math.min(
        100,
        ((100 - metrics.codeQuality.linting.totalIssues) * metrics.codeQuality.maintainability.index) / 100
      ),
    ...
    if (metrics.testCoverage.overall.percentage < this.config.thresholds.testCoverage.target) {
      recommendations.push({
        ...
        actions: [
          ...
          {
            description: 'Implement integration tests for critical paths',
            type: 'code_change',
            effort: 'high',
            automated: false,
          },
