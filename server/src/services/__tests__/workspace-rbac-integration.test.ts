...
describe('Epic 23 RBAC Integration', () => {
  ...
  describe('Permission Checking System', () => {
    ...
    test('should enforce admin permissions correctly', async () => {
      ...
      await expect(service.inviteUserToWorkspace(workspaceId, testUserId, 'viewer', adminId))
    ...
    test('should enforce editor permissions correctly', async () => {
      ...
      await expect(service.updateWorkspace(
        workspaceId,
        { description: 'Editor trying to update' },
        editorId
    ...
    test('should enforce viewer permissions correctly', async () => {
      ...
      }, viewerId)).rejects.toThrow('Insufficient permissions');
    ...
    test('should enforce commenter permissions correctly', async () => {
      ...
      await expect(service.createProject({
        ...
        description: 'Should fail'
      }, commenterId)).rejects.toThrow('Insufficient permissions');
    });

    test('should deny access to unauthorized users', async () => {
      // Unauthorized user should not have access
      await expect(service.getWorkspace(workspaceId, unauthorizedId))
        .rejects.toThrow('Access denied');

      await expect(service.getProject(projectId, unauthorizedId))
        .rejects.toThrow('Access denied');

      await expect(service.getResource(resourceId, unauthorizedId))
        .rejects.toThrow('Access denied');
    });
  });
  ...
  describe('Multi-tenant Isolation', () => {
    ...
    test('should enforce workspace isolation for authorized users', async () => {
      ...
      await expect(service.enforceWorkspaceIsolation(
        adminId,
        workspaceId,
  ...
  describe('Role Management Operations', () => {
    ...
    test('should allow admins to update user roles', async () => {
      ...
      const success = await service.updateUserRole(
        workspaceId,
        testUserId,
        'editor',
        adminId
    ...
    test('should prevent non-admins from updating roles', async () => {
      ...
      await expect(service.updateUserRole(
        ...
        'admin',
        editorId
      )).rejects.toThrow('Insufficient permissions to assign roles');
    });
    ...
    test('should prevent owners from demoting themselves', async () => {
      await expect(service.updateUserRole(
        ...
        'viewer',
        ownerId
      )).rejects.toThrow('Workspace owners cannot change their own role');
    });
    ...
    test('should allow users to remove themselves', async () => {
      ...
      const success = await service.removeUserFromWorkspace(
        workspaceId,
        testUserId,
        testUserId
      );
    ...
    test('should prevent non-owners from removing the workspace owner', async () => {
      await expect(service.removeUserFromWorkspace(
        workspaceId,
        ownerId,
        adminId
      )).rejects.toThrow('Workspace owner cannot be removed by others');
    });
  });
  ...
  describe('Performance and Edge Cases', () => {
    ...
    test('should handle non-existent workspace gracefully', async () => {
      ...
      await expect(service.getWorkspace(fakeWorkspaceId, ownerId))
        .resolves.toBeNull();
    });
    ...
    test('should handle permission checking for non-existent users', async () => {
      ...
      await expect(service.getWorkspace(workspaceId, fakeUserId))
        .rejects.toThrow('Access denied');
    });
