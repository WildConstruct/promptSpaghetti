...
export class ContentQualityMetricsService {
  ...
  async getCreatorQualityDashboard(creatorId, timeRange = TimeRange.LAST_30D) {
    ...
    const templates = await this.db.query('SELECT id, title, category FROM templates WHERE creator_id = $1', [
      creatorId,
    ]);
  ...
  async getMarketplaceQualityInsights(timeRange = TimeRange.LAST_30D) {
    ...
    const [overallMetrics, categoryBreakdown, qualityDistribution, trends, topPerformers, qualityFactors] =
      await Promise.all([
  ...
  async assessEngagement(templateId, metrics, timeRange) {
    ...
    const score = Math.round(
      usage.adoptionRate * 0.25 + usage.retentionRate * 0.25 + interaction.likeRate * 0.25 + virality.growthRate * 0.25
    );
    return {
      score,
      usage,
