-- Epic 16 Marketplace Database Schema
-- PostgreSQL 15+ compatible with Epic 11 Auth integration
...
CREATE OR REPLACE VIEW marketplace_template_stats AS
SELECT 
    t.id,
    t.title,
    t.price_cents,
    ...
    AVG(r.stars)::DECIMAL(3,2) as avg_rating,
    SUM(p.amount_cents) as total_revenue,
    MAX(p.created_at) as last_purchase_at,
    COUNT(DISTINCT e.id) FILTER (WHERE e.event_type = 'view') as total_views,
    COUNT(DISTINCT e.id) FILTER (WHERE e.event_type = 'preview') as total_previews
FROM marketplace_templates t
LEFT JOIN marketplace_purchases p ON t.id = p.template_id AND p.status = 'succeeded'
LEFT JOIN template_reviews r ON t.id = r.template_id AND r.moderation_status = 'approved'
LEFT JOIN marketplace_events e ON t.id = e.template_id
GROUP BY t.id, t.title, t.price_cents, t.status;

-- Materialized view for search performance (refreshed nightly)
...
GROUP BY t.id, t.title, t.description, t.tags, t.price_cents, t.status, t.search_vector, 
         t.created_at, t.featured_at, stats.avg_rating, stats.total_purchases, stats.total_reviews;

CREATE UNIQUE INDEX IF NOT EXISTS idx_search_index_id ON marketplace_search_index(id);
CREATE INDEX IF NOT EXISTS idx_search_index_vector ON marketplace_search_index USING GIN(search_vector);
CREATE INDEX IF NOT EXISTS idx_search_index_price ON marketplace_search_index(price_cents);
CREATE INDEX IF NOT EXISTS idx_search_index_rating ON marketplace_search_index(avg_rating DESC);
CREATE INDEX IF NOT EXISTS idx_search_index_popularity ON marketplace_search_index(total_purchases DESC);

-- Function to refresh search index
CREATE OR REPLACE FUNCTION refresh_marketplace_search_index()
RETURNS void AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY marketplace_search_index;
END;
$$ LANGUAGE plpgsql;