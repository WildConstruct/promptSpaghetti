...
window.dragDebug = {
  ...
  checkReactFlow: function() {
    ...
    const hasDropHandler = rf?._reactInternalInstance || rf?.__reactInternalFiber;
    console.log(`React handlers attached: ${hasDropHandler ? 'Maybe' : 'Unknown'}`);
    
    // Check for DnD provider
    const dndContext = document.querySelector('[data-rbd-droppable-id]');
    console.log(`DnD context: ${dndContext ? '✅ Found' : '⚠️ Not found (might be React DnD)'}`);
...
console.log('%c🐛 Drag & Drop Debugger V2 Loaded', 'color: #ff9900; font-size: 16px; font-weight: bold');
console.log('\nCommands:');
