...
window.safeAddNode = {
  ...
  addNode: function(nodeType = 'output', x = 200, y = 200) {
    ...
    if (!reactDevTools) {
      console.error('React DevTools not found. Please install React DevTools extension.');
      console.log('Alternative: Use the UI buttons if they work, or try refreshing the page.');
      return;
    }
    ...
    console.log(`   $r.setNodes(nodes => [...nodes, ${JSON.stringify(nodeData, null, 2)}])`);
  ...
  createTestGraph: function() {
    ...
    console.log(`   $r.setEdges(edges => [...edges, ${JSON.stringify(edge, null, 2)}])`);
  ...
  checkStatus: function() {
    ...
    nodeElements.forEach((node, i) => {
      const label = node.querySelector('.epic1-node-type-label')?.textContent || 
    ...
    const outputNodes = document.querySelectorAll('.epic1-editable-node.output');
...
// Show instructions
console.log('%c🧪 Safe Node Addition Tools Loaded', 'color: #00ff00; font-size: 16px; font-weight: bold');
console.log('\nCommands:');
