...
window.debugDragDrop = {
  ...
  addNodeProgrammatically: function (nodeType = 'textBlock') {
    ...
    console.log(
      `✅ Drop event dispatched for ${nodeType} at (${x.toFixed(0)}, ${y.toFixed(0)})`
    );
  },
  ...
  applyFixes: function () {
    ...
    document.querySelectorAll('.node-item').forEach(item => {
      item.draggable = true;
      console.log(
        `✅ Made ${item.querySelector('.node-label')?.textContent} draggable`
      );
    });
...
// Auto-run diagnostic
console.log(
  '%c🧪 Drag & Drop Debug Tools Loaded',
  'color: #00ff00; font-size: 16px; font-weight: bold'
);
console.log('\nAvailable commands:');
console.log(
  '  debugDragDrop.runFullDiagnostic()    - Full diagnostic & auto-fix'
);
console.log('  debugDragDrop.monitorDragEvents()    - Monitor drag events');
console.log(
  '  debugDragDrop.addNodeProgrammatically(type) - Add node directly'
);
