...
export class ContextAwareSecurityChecker {
  ...
  async checkSecurity(
    ...
    const results = await Promise.all(
      applicableRules.map(rule => this.executeContextualRule(rule, enrichedContext))
    );
    ...
    const fpPrediction = await this.falsePositivePredictionService
      .predictFalsePositive(operation, enrichedContext);
