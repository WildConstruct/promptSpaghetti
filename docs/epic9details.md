...
* **Shard hotspot** – Uneven doc popularity may overload a worker shard. *Mitigation*: expose shard-load metrics (ops/sec, mem) in Prometheus; add optional rebalance coordinator that can update shard assignments via NATS KV.
...
* State transitions (e.g., *review → approved*) published as CRDT metadata ops so that every client reflects workflow status in real time.
