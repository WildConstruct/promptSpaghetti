...
• **Feature Flag** – set `window.FEATURE_ADV_MODAL = false` _before_ app bootstrap (e.g. in a `<script>` tag in `index.html`) to revert to the legacy UI.

## Slot Controls & New Slots

The modal generates one control per _lockable_ grammar rule:
...
Example – _Texture_ checkbox grid:
