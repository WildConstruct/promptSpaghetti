...
class SmartEdgeRouter implements EdgeRouter {
  ...
  route(edge: Edge, nodes: Node[], options?: RoutingOptions): RoutedEdge {
    ...
    const source = this.getNodeConnection(edge.source, edge.sourceHandle, nodes)
    const target = this.getNodeConnection(edge.target, edge.targetHandle, nodes)
...
class OrthogonalRouter implements RoutingAlgorithm {
  ...
  private createGrid(
    ...
    obstacles.forEach(obstacle => {
      ...
      const endX = Math.ceil((obstacle.x + obstacle.width - bounds.x) / resolution)
      const endY = Math.ceil((obstacle.y + obstacle.height - bounds.y) / resolution)
...
class AnnotationPerformanceOptimizer {
  ...
  private getVisibleAnnotations(viewport: Viewport): Set<string> {
    ...
    return new Set(
      this.spatialIndex.search(bounds).map(a => a.id)
    )
  }
