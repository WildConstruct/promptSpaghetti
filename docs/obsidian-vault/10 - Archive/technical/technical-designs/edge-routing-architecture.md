  ...
  invalidatePathsForNode(nodeId: string): void {
    ...
    [this.L1Cache, this.L2Cache, this.L3Cache].forEach(cache => {
      cache.forEach((path, key) => {
        if (path.obstacles.includes(nodeId) || key.includes(nodeId)) {
...
export class IntelligentPathCalculator {
  ...
  private calculateBezierPath(edge: Edge, obstacles: ObstacleMap): EdgePath {
    ...
    const controlPoints = this.findBezierControlPoints(
      source,
      target,
      obstacles
    );
  ...
  private async calculateAStarPath(
    edge: Edge,
    obstacles: ObstacleMap
  ): Promise<EdgePath> {
...
export class RoutingWorkerPool {
  ...
  async calculatePaths(
    requests: PathCalculationRequest[]
  ): Promise<EdgePath[]> {
  ...
  try {
    ...
    switch (type) {
      case 'calculateBatch':
        ...
        break;
      case 'astar':
        results = calculateAStarPath(
          event.data.source,
          event.data.target,
          event.data.obstacles
        );
        break;
      default:
...
export class AdaptivePerformanceManager {
  ...
  private checkForAdaptation() {
    const recentMetrics = this.performanceHistory.slice(
      -this.adaptationThreshold
    );
  ...
  private adaptToLowerStrategy() {
    ...
    console.log(
      `Adapted to lower performance strategy: ${this.currentStrategy}`
    );
  }
  ...
  private adaptToHigherStrategy() {
    ...
    console.log(
      `Adapted to higher performance strategy: ${this.currentStrategy}`
    );
  }
}
```
...
export class MemoryOptimizedRouter {
  ...
  private setupMemoryMonitoring() {
    ...
    setInterval(() => {
      ...
      if (memInfo && memInfo.usedJSHeapSize > 150 * 1024 * 1024) {
        // 150MB
...
describe('Edge Routing Performance', () => {
  ...
  test.each([100, 500, 1000, 2000])(
    'Routes %i edges within performance targets',
    async nodeCount => {
      ...
      const paths = await router.calculateEdgePaths(
        graph.edges,
        graph.obstacles
      );
      ...
      const memoryUsed =
        ((performance as any).memory?.usedJSHeapSize || 0) - startMemory;
  ...
  test('Cache hit ratio exceeds 85% for typical workflows', async () => {
    ...
    for (const operation of workflow.operations) {
      ...
      if (cached) {
        ...
        const path = await router.calculatePath(
          operation.edge,
          operation.obstacles
        );
