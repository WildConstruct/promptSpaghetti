...
class GraphManager {
  executeGraph() { /* execution logic */ }
  validateGraph() { /* validation logic */ }
  renderGraph() { /* UI logic */ }
  saveGraph() { /* persistence logic */ }
  optimizeGraph() { /* optimization logic */ }
}

// ✅ Preferred: Separate concerns
class GraphExecutor { execute(graph: Graph): Promise<Result>; }
class GraphValidator { validate(graph: Graph): ValidationResult; }
class GraphRenderer { render(graph: Graph): JSX.Element; }
```
...
class ResourceManager {
  ...
  async dispose(): Promise<void> {
    const disposalPromises = Array.from(this.resources)
...
describe('GraphExecutor', () => {
  describe('executeGraph', () => {
    ...
    it('should handle validation errors', async () => {
      ...
      // Act & Assert
      await expect(executor.executeGraph(invalidGraph))
        .rejects
...
type NodeConfigFor<T extends NodeType> = 
  T extends 'WeightedChoice' ? WeightedChoiceConfig :
  T extends 'Conditional' ? ConditionalConfig :
  never;

type RuntimeNodeFor<T extends NodeType> =
  T extends 'WeightedChoice' ? WeightedChoiceNode :
  T extends 'Conditional' ? ConditionalNode :
  never;
```
...
interface GraphExecutionStartedEventV1 extends DomainEvent<{
  graphId: string;
  nodeCount: number;
...
interface GraphExecutionStartedEventV2 extends DomainEvent<{
  graphId: string;
  nodeCount: number;
  executionContext: ExecutionContext;
