...
class PerformanceMonitor {
  ...
  trackApiCall<T>(
    ...
    return this.measureExecution(`api:${method}:${url}`, apiCall, 'api', {
      url,
      method
    }) as Promise<T>;
  }
  ...
  getStats(): {
    ...
    if (this.metrics.length === 0) {
      return {
        total: 0,
        byType: { component: 0, api: 0, user_interaction: 0, custom: 0 },
        averages: {}
      };
    }
...
export function usePerformanceTracking(
  ...
  useEffect(() => {
    endRef.current = performanceMonitor.startTiming(
      `hook:${name}`,
      'component'
    );
    return () => {
