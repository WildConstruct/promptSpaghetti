...
describe('usePerformanceTracking', () => {
  it('records mount and unmount timings', () => {
    ...
    unmount();
    const metricNames = performanceMonitor
      .getMetrics()
      .map(metric => metric.name);
    expect(metricNames.some(name => name.includes('hook:test-hook'))).toBe(
      true
    );
  });
});
...
describe('withPerformanceTracking', () => {
  it('wraps component with perf tracking', () => {
    ...
    const metrics = performanceMonitor.getMetrics();
    expect(
      metrics.some((metric: PerformanceMetric) => metric.type === 'component')
    ).toBe(true);
  });
});
