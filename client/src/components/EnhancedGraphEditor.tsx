...
const InspectorPanel: React.FC<{
  ...
  const [options, setOptions] = useState<Array<{ label: string; value: string; weight?: number }>>(
  ...
  if (!selectedNode) {
    return (
      ...
      >
        ...
        <div style={{ textAlign: 'center', lineHeight: 1.5 }}>Select a node to edit its properties</div>
      </div>
    );
  }
      ...
      >
        <div
          style={{
            ...
            borderRadius: '50%',
            backgroundColor:
              professionalColors.nodes[selectedNode.type as keyof typeof professionalColors.nodes] ||
              professionalColors.ui.border,
            marginRight: '12px',
          }}
        />
        <div>
  ...
  nodes.forEach(node => {
    ...
    switch (node.type) {
      ...
      case 'output': {
        ...
        const inputNodes = edges.filter(edge => edge.target === node.id).map(edge => edge.source);
...
const EnhancedGraphEditorInner: React.FC<EnhancedGraphEditorProps> = ({ initialNodes = [], initialEdges = [] }) => {
  const [nodes, setNodes, onNodesChange] = useNodesState(
    initialNodes && Array.isArray(initialNodes) && initialNodes.length > 0 ? (initialNodes as Node[]) : defaultNodes
  );
  const [edges, setEdges, onEdgesChange] = useEdgesState(
    initialEdges && Array.isArray(initialEdges) && initialEdges.length > 0 ? (initialEdges as Edge[]) : defaultEdges
  );
  ...
  const updateNode = useCallback(
    (nodeId: string, updates: Partial<NodeData>) => {
      setNodes(nds => nds.map(node => (node.id === nodeId ? { ...node, data: { ...node.data, ...updates } } : node)));
    },
    [setNodes]
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <ProfessionalPalette collapsed={paletteCollapsed} onToggle={() => setPaletteCollapsed(!paletteCollapsed)} />
        <div ref={reactFlowWrapper} style={{ flex: 1, height: '100%', position: 'relative' }}>
          ...
          >
            ...
            <button
              ...
              style={{
                background: stickyNotesVisible
                  ? professionalColors.accent.orange + '30'
                  : professionalColors.background.secondary,
            ...
            <button
              ...
              style={{
                background: showAnnotations
                  ? professionalColors.accent.purple + '30'
                  : professionalColors.background.secondary,
          ...
          >
            ...
            />
            <Background variant={BackgroundVariant.Dots} gap={20} size={1} color={professionalColors.ui.border} />
          </ReactFlow>
                ...
                onMouseDown={e => {
                  ...
                  const handleMouseMove = (e: MouseEvent) => {
                    if (!isDragging) {
                      ...
                      const moveDistance =
                        Math.abs(e.clientX - (rect.left + startX)) + Math.abs(e.clientY - (rect.top + startY));
        ...
        <InspectorPanel selectedNode={selectedNode} onUpdateNode={updateNode} onDeleteNode={deleteNode} />
      </div>
                        ...
                        >
                          <div
                            style={{
                              ...
                              borderRadius: '50%',
                              backgroundColor:
                                professionalColors.nodes[nodeType as keyof typeof professionalColors.nodes] ||
                                professionalColors.ui.border,
                              marginRight: '8px',
                            }}
                          />
