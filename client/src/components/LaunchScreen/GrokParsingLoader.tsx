...
export default function GrokParsingLoader({ prompt = "", message = "Parsing..." }: GrokParsingLoaderProps) {
  // Extract first few words for node labels
  const words = prompt.trim().split(/\s+/).filter(w => w.length > 0).slice(0, 5);
  ...
  return (
    <div style={{ 
      ...
      <svg width="600" height="450" viewBox="0 0 600 450" style={{ maxWidth: '100%', height: 'auto' }}>
          ...
          <motion.g key={i}>
            ...
            <motion.text
              ...
              animate={{ 
                ...
                x: dest.y > 225 
                  ? [300 + (dest.x - 300) * 0.15, 
                     dest.x < 300 ? dest.x - 15 : dest.x + 15]  // Bottom: offset based on which side
