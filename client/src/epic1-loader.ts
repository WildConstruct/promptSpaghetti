...
export async function loadEpic1Editor() {
  try {
    ...
    const baseModule = await import(
      '@promptscape/core/components/epic1/nodes/BaseEditableNode'
    );
    console.log('BaseEditableNode loaded:', baseModule);

    // Step 2: Load individual node types
    const textBlockModule = await import(
      '@promptscape/core/components/epic1/nodes/TextBlockNode'
    );
    console.log('TextBlockNode loaded:', textBlockModule);

    const weightedChoiceModule = await import(
      '@promptscape/core/components/epic1/nodes/WeightedChoiceNode'
    );
    console.log('WeightedChoiceNode loaded:', weightedChoiceModule);

    const outputModule = await import(
      '@promptscape/core/components/epic1/nodes/OutputNode'
    );
    ...
    const editorModule = await import(
      '@promptscape/core/components/epic1/Epic1GraphEditor'
    );
