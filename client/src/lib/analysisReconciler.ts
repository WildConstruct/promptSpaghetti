...
import { PromptAnalysis, NodeMapping, GeneratedNode, GeneratedNodeInternal } from './simplePromptParser';
...
export function reconcileAnalysis(prev: PromptAnalysis, prevText: string, newText: string, options: ReconcileOptions = {}): PromptAnalysis {
  ...
  const prevNodeMap = new Map(prev.nodes.map((g) => [g.node.id, g.node] as const));
  ...
  for (const slice of prevSlices) {
    ...
    while (idx !== -1 && !checked.has(idx)) {
      ...
      const approxStart = Math.floor((idx / Math.max(1, normNew.length)) * newText.length);
      const window = newText.slice(approxStart, approxStart + slice.text.length + 50);
    ...
    if (bestScore >= threshold && bestStart >= 0) {
      ...
      const prevNode = prevNodeMap.get(slice.nodeId) as GeneratedNodeInternal | undefined;
      if (prevNode) {
        nodes.push({ node: { id: prevNode.id, nodeType: prevNode.nodeType, getPreviewText: () => newText.slice(bestStart, bestEnd) } });
      } else {
        nodes.push({ node: { id: slice.nodeId, nodeType: 'Text', getPreviewText: () => newText.slice(bestStart, bestEnd) } });
      }
      mappings.push({ nodeId: slice.nodeId, startIndex: bestStart, endIndex: bestEnd, highlightColor: slice.color });
  ...
  const usedRanges: Array<[number, number]> = mappings.map((m) => [m.startIndex, m.endIndex]);

  function fillGap(start: number, end: number) {
    if (end - start <= 0) return;
    const id = `node-${Math.random().toString(36).slice(2, 9)}`;
    nodes.push({ node: { id, nodeType: 'Text', getPreviewText: () => newText.slice(start, end) } });
  ...
  nodes.push({ node: { id: 'output', nodeType: 'Output', getPreviewText: () => 'Output' } });
  ...
  nonOutput.sort((a, b) => (startMap.get(a.node.id) ?? Number.MAX_SAFE_INTEGER) - (startMap.get(b.node.id) ?? Number.MAX_SAFE_INTEGER));
