          ...
          nodes: [
            {
              ...
              type: 'Conditional',
              branches: [
                { condition: 'eval("malicious code")', output: 'Dangerous' }
              ],
              defaultOutput: 'Safe'
            },
            {
  ...
  describe('Rate Limiting Regression', () => {
    it('should handle burst requests appropriately', async () => {
      ...
      const burstRequests = Array(10)
        .fill(null)
        .map(() =>
          request(server.server).post('/preview').send(validGraphRequest)
        );
