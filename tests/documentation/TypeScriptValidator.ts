  ...
  private async validateImport(moduleName: string): Promise<ImportValidationResult> {
    try {
      ...
      const builtinModules = [
        'fs', 'path', 'url', 'util', 'events', 'stream', 'http', 'https',
        'crypto', 'os', 'child_process', 'cluster', 'worker_threads',
        'assert', 'buffer', 'console', 'dns', 'domain', 'net', 'querystring',
        'readline', 'repl', 'string_decoder', 'timers', 'tls', 'tty',
        'dgram', 'v8', 'vm', 'zlib', 'inspector', 'perf_hooks', 'async_hooks'
      ];
