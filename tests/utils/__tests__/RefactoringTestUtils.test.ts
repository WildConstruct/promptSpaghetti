...
describe('Refactoring Test Utilities', () => {
  ...
  describe('MigrationTestHelper', () => {
    ...
    test('should test backward compatibility', () => {
      ...
      const getValueResult = result.results.find(
        r => r.testCase.methodName === 'getValue'
      );
      expect(getValueResult?.compatible).toBe(false);

      const calculateResult = result.results.find(
        r => r.testCase.methodName === 'calculate'
      );
