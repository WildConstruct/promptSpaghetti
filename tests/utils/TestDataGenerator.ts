...
export class TestDataGenerator {
  ...
  generateGraph(options: GraphGenerationOptions): Graph {
    const { nodeCount, seed, scenario, complexity, includeAdvancedNodes } =
      options;
    ...
    switch (scenario) {
      ...
      case TestScenario.PERFORMANCE_STRESS:
        return this.generatePerformanceStressGraph(
          nodeCount,
          seed,
          includeAdvancedNodes
        );
  ...
  private generateLinearGraph(nodeCount: number, seed: number): Graph {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      if (i === 0) {
        ...
        const nodeType =
          this.randomInt(seed + i, 0, 1) === 0 ? 'Concat' : 'WeightedChoice';
  ...
  private generateBranchingGraph(
    ...
    const branchFactor =
      complexity === 'simple' ? 2 : complexity === 'moderate' ? 3 : 4;
    let nodeIndex = 0;
    ...
    for (let level = 1; level < levels && nodeIndex < nodeCount; level++) {
      ...
      for (const parentId of currentLevelNodes) {
        ...
        for (
          let child = 0;
          child < childCount && nodeIndex < nodeCount;
          child++
        ) {
          ...
          nodes.push({
            ...
            data: {
              choices: [
                {
                  value: `Branch ${level}-${child} A`,
                  weight: this.random(seed + nodeIndex, 1, 5)
                },
                {
                  value: `Branch ${level}-${child} B`,
                  weight: this.random(seed + nodeIndex + 1, 1, 5)
                }
              ]
            },
            position: {
  ...
  private generatePerformanceStressGraph(
    nodeCount: number,
    seed: number,
    includeAdvancedNodes: boolean
  ): Graph {
  ...
  generateUser(options: UserGenerationOptions): User {
    const { role, permissions, organizationId, isVerified, mfaEnabled } =
      options;
