...
describe('Test Data Generators', () => {
  describe('AdvancedGraphGenerator', () => {
    ...
    it('should generate validation scenarios with errors', () => {
      ...
      expect(['self-loop', 'disconnected', 'circular-dependency', 'missing-input', 'duplicate-edges']
        .some(type => scenario.name.includes(type))).toBe(true);
    });
    ...
    it('should generate security scenarios with malicious patterns', () => {
      ...
      expect(scenario.expectedBehavior).toBe('error');
      expect(['malicious-expression', 'prototype-pollution', 'code-injection', 'resource-exhaustion']
        .some(type => scenario.name.includes(type))).toBe(true);
    });
    ...
    it('should generate edge case scenarios', () => {
      ...
      expect(['empty', 'single-node', 'large-weights', 'zero-weights', 'deep-nesting']
        .some(type => scenario.name.includes(type))).toBe(true);
    });
