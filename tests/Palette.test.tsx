...
describe('Palette component', () => {
  const nodes = [{ id: 'WeightedChoice', label: 'WeightedChoice', icon: '🔀', tooltip: 'Branch' }];
  it('renders node icons and tooltips', () => {
    render(<Palette nodes={nodes as unknown} collapsed={false} onToggle={() => {}} />);
    expect(screen.getByLabelText(/WeightedChoice/)).toBeTruthy();
  });

  it('sets drag data on dragStart', () => {
    const mockDragStart = jest.fn();
    render(<Palette nodes={nodes as unknown} collapsed={false} onToggle={() => {}} onDragStart={mockDragStart} />);
