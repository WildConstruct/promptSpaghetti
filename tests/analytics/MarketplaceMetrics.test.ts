...
describe('MarketplaceMetrics', () => {
  ...
  describe('Template Analytics', () => {
    ...
    test('gets top performing templates by revenue', () => {
      ...
      expect(topTemplates[0].metrics.revenue.total)
        .toBeGreaterThanOrEqual(topTemplates[1].metrics.revenue.total);
    });
    ...
    test('gets top performing templates by downloads', () => {
      ...
      expect(topTemplates[0].metrics.downloads)
        .toBeGreaterThanOrEqual(topTemplates[1].metrics.downloads);
    });

    test('gets top performing templates by rating', () => {
      const topTemplates = marketplaceMetrics.getTopPerformingTemplates('rating', 5);
      
      expect(topTemplates[0].metrics.ratings.average)
        .toBeGreaterThanOrEqual(topTemplates[1].metrics.ratings.average);
    });
  });
  ...
  describe('Data Validation', () => {
    ...
    test('maintains data consistency across operations', () => {
      ...
      marketplaceMetrics.trackEvent('template_viewed', {}, templateId, 'template');
      marketplaceMetrics.trackEvent('template_previewed', {}, templateId, 'template');
      marketplaceMetrics.trackEvent('template_purchased', {}, templateId, 'template', 25.00);
