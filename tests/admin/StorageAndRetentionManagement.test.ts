...
import {
  DataCategory,
  Jurisdiction
} from '../../server/src/types/DataRetentionPeriods';
...
describe('Storage Management and Retention Policy Admin Systems', () => {
  ...
  describe('StorageManagementService', () => {
    describe('Storage Pool Management', () => {
      test('should create storage pool with comprehensive configuration', async () => {
        ...
        const result = await storageService.createStoragePool(
          poolData,
          'admin'
        );
      ...
      test('should update storage pool configuration', async () => {
        ...
        const result = await storageService.updateStoragePool(
          'pool-1',
          updates,
          'admin'
        );
    ...
    describe('Storage Quota Management', () => {
      test('should create and enforce storage quotas', async () => {
        ...
        const result = await storageService.createStorageQuota(
          quotaData,
          'admin'
        );
    ...
    describe('Storage Optimization', () => {
      ...
      test('should execute optimization and return results', async () => {
            ...
            rows: [
              {
                ...
                target:
                  '{"targetType":"global","dataCategories":["TECHNICAL"],"storageTypes":["HOT","WARM"]}',
                status: 'scheduled',
        ...
        const result = await storageService.executeOptimization(
      ...
      test('should generate optimization recommendations', async () => {
        ...
        jest
          .spyOn(storageService as any, 'getCurrentStorageMetrics')
          .mockResolvedValue(mockMetrics);
        jest
          .spyOn(storageService as any, 'findOldData')
          .mockResolvedValue({ size: 150 });
        jest
          .spyOn(storageService as any, 'findDuplicateData')
          .mockResolvedValue({ size: 25 });

        const recommendations =
          await storageService.getOptimizationRecommendations();
    ...
    describe('Storage Monitoring and Metrics', () => {
      test('should calculate comprehensive storage metrics', async () => {
        ...
        jest
          .spyOn(storageService as any, 'getStoragePools')
          .mockResolvedValue(mockPools);
        jest
          .spyOn(storageService as any, 'calculatePoolMetrics')
          .mockResolvedValue({
            healthScore: 85,
            performance: 90,
            efficiency: 75
        ...
        jest
          .spyOn(storageService as any, 'calculateGrowthRate')
          .mockResolvedValue(5);
        jest
          .spyOn(storageService as any, 'calculateCompressionRatio')
          .mockResolvedValue(0.8);
        jest
          .spyOn(storageService as any, 'calculateDeduplicationRatio')
          .mockResolvedValue(0.9);
        jest
          .spyOn(storageService as any, 'calculateIOPSUtilization')
          .mockResolvedValue(70);
        jest
          .spyOn(storageService as any, 'calculateThroughputUtilization')
          .mockResolvedValue(65);
        jest
          .spyOn(storageService as any, 'calculateCostPerGB')
          .mockResolvedValue(0.12);
  ...
  describe('RetentionPolicyAdminService', () => {
    beforeEach(() => {
      ...
      (mockRetentionFramework.getPolicy as jest.Mock).mockResolvedValue(
        mockPolicyData
      );
    });
    ...
    describe('Admin Policy Management', () => {
      test('should create admin retention policy with enhanced controls', async () => {
        const policyData = {
          ...
          description:
            'Comprehensive retention policy for personal data under GDPR',
          categories: [DataCategory.PERSONAL_IDENTIFIABLE],
          jurisdiction: [Jurisdiction.GDPR],
          enabled: true,
          rules: []
      ...
      test('should retrieve admin policies with filtering', async () => {
        ...
        const result =
          await retentionService.getAdminRetentionPolicies(filters);
        ...
        for (const policy of result) {
          ...
          expect(policy.categories).toContain(
    ...
    describe('Policy Templates', () => {
      test('should create reusable policy template', async () => {
        ...
        const result = await retentionService.createPolicyTemplate(
          templateData,
          'admin'
        );
      ...
      test('should create retention exception with approval workflow', async () => {
        const exceptionData = {
          ...
          businessJustification:
            'Legal department has requested hold for case #2024-001',
    ...
    describe('Compliance Monitoring', () => {
      test('should perform comprehensive compliance assessment', async () => {
        ...
        const result = await retentionService.performComplianceAssessment(
          scope,
          'compliance-admin'
        );
  ...
  describe('Integration Workflows', () => {
    test('should integrate storage optimization with retention policies', async () => {
      ...
      const policyData = {
        ...
        description:
          'Automatically manage storage lifecycle for cost optimization',
      ...
      expect(retentionPolicy.categories).toEqual([
        DataCategory.BEHAVIORAL,
        DataCategory.TECHNICAL
      ]);
      expect(optimization.target.dataCategories).toEqual([
        DataCategory.BEHAVIORAL,
        DataCategory.TECHNICAL
      ]);
